...
export type ProviderAuthMethodView = { id: string; label: string; enabled: boolean; status: string; fields: string[]; helpText: string }; // 新增代码+ProviderSettingsStore：定义前端认证方式 view model；如果没有这行，组件会直接消费 snake_case。
...
const UNSAFE_KEYS = new Set(["api_key", "apikey", "apiKey", "authorization", "Authorization", "bearer", "Bearer"]); // 新增代码+ProviderSettingsStore：定义 renderer 禁止保留的字段名；如果没有这行，后端 bug 可能把 secret 带进状态。
...
export function isUnsafeProviderPayloadKey(key: string): boolean { // 新增代码+ProviderSettingsStore：函数段开始，判断字段名是否危险；如果没有这段，测试和脱敏逻辑无法共享规则。
  return UNSAFE_KEYS.has(key); // 新增代码+ProviderSettingsStore：返回字段是否禁止；如果没有这行，危险字段判断不会生效。
...
function authMethodView(method: GuiProviderAuthMethod): ProviderAuthMethodView { // 新增代码+ProviderSettingsStore：函数段开始，把后端认证方式转成前端字段；如果没有这段，组件要理解 snake_case。
  return { id: method.id, label: method.label, enabled: Boolean(method.enabled), status: String(method.status), fields: Array.isArray(method.fields) ? method.fields : [], helpText: String(method.help_text ?? "") }; // 新增代码+ProviderSettingsStore：返回认证方式 view model；如果没有这行，按钮和帮助文案字段会不稳定。
