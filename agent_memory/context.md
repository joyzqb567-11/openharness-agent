...
## 2026-06-07 Phase123 Computer Use 应用发现工具上下文

- 本轮新增 `computer_discover` 只读工具，正式接入 schema、catalog、executor、LearningAgent handler 和 `computer_use`/`computer-use` 兼容 operation=discover。
- `computer_discover` 调用统一 `learning_agent/computer_use/windows_app_inventory.py::query_windows_app_inventory()`，返回清洗后的 `display_name`、`app_name`、`launch_kind`、`launch_id`、`source`、`installed_app_verified` 和 `match_score`。
- 设计边界：discover 是“应用候选发现层”，不是启动器；模型仍应在主循环里读取用户自然语言、工具 schema、discover 结果、屏幕观察和工具结果，再决定是否调用 `computer_action(action=launch_app)`。
- 该清单不是硬白名单，返回字段包含 `not_hard_whitelist=true`，避免再次把 Computer Use 设计成只能控制固定应用。
- 本轮没有实现 Windows AppX/AUMID 全量枚举，也没有实现多后端 launch resolver；这些仍是后续任务。
- 本轮学习备份已保存到 `learning_agent/test/computer_discover_tool_20260607_071350`。

## 2026-06-06 通用 Computer Use session/窗口/观察/纠偏架构上下文

- 用户明确要求先做通用 Computer Use session/窗口/观察/纠偏架构，不要继续做 Paint 专用补丁。
- 本轮真实日志根因证据来自 `learning_agent/acceptance_controller/runs/computer_use_full_model_loop_paint_tree_real-20260606_222309/latest_run_readable.md`：模型已经启动了新的 Paint 窗口，但后续观察/动作上下文仍混入旧 Paint 窗口和旧“太空/火箭”画布，导致模型把旧内容当成当前任务继续加工。
- 已实现第一层通用架构：用户每次输入 `/computer use --full` 成功打开 full 会话后，交互层会清理旧 `computer_use*` observation events 和旧 `computer-window` / `computer_use` artifact，避免旧截图继续进入新任务主循环。
- 已实现 controller 级通用窗口守卫：`ComputerUseController.observe()` 现在和 `execute()` 一样复用 agent-owned target drift guard；如果模型在 launch_app 绑定新窗口后又观察旧窗口，controller 会拒绝并返回 `target_drift_blocks_action=true`。
- 已实现模型主循环通用纠偏约束：full desktop harness 要求每次观察后先判断画面是否服务于用户本轮目标；如果不一致，必须先新建、清空、导航、重新聚焦或选择正确窗口，再继续操作。
- 该架构面向所有本机普通 GUI 软件，不是 Paint 专用逻辑；Paint 树场景只是本轮真实验收样本。
- 学习备份已复制到 `learning_agent/test/computer_use_session_target_guard_20260606_224526`。
- 重要边界：这轮修的是会话隔离、旧窗口观察漂移和观察后纠偏入口；它还不等于任意软件任务已经成熟，也不等于视觉质量 verifier 已完成。
- 真实可见终端验收已通过：`learning_agent/acceptance_controller/runs/computer_use_full_model_loop_paint_tree_real-20260606_224604/result.json` 显示 `completed=true`、`assertion.passed=true`，最终截图中真实 Paint 画布包含一棵树。
- 本次真实日志没有再出现旧“太空/小星体/火箭”污染，也没有旧窗口 `mspaintapp:pid:48256` / `hwnd:74840`。

## 2026-06-06 Computer Use Full Final Evidence Summary Context

- 用户指出真实 Paint 场景中 agent 的事件回复和预期不同：终端应该像 Codex 一样给出过程和证据，而不是只输出短答或刷屏。
- 当前已修复一个具体链路：当 `/computer use --full` GUI 桌面任务已有真实 Computer Use 观察/动作证据，但模型最终回答太短时，`LearningAgent.run_events` 会追加可读证据摘要。
- 摘要字段固定包含 `computer_use`、目标应用/窗口、`screenshot`、`screenshot_path`、`real_desktop_touched`、`low_level_event_count` 和成功动作数。
- 该机制只是最终回答收束保护，不是新的绘图 planner，也不是视觉质量 verifier。
- 后续仍要用真实可见终端验收确认 `/computer use --full` + 自然语言 Paint 任务在用户可见终端中能稳定输出这些证据。

---

## 2026-06-06 Computer Use Full Paint Cat Route Context

- 用户明确指出：`/computer use --full` 的真实验收不能只看单元测试或日志，必须证明真实可见终端中输入自然 prompt 后打开本机 Paint 并完成绘图。
- 当前修复方向：先把猫作为受支持 Paint 绘图对象补进真实闭环，并让默认 LearningAgent full runtime 显式开启受支持 Paint 绘图桥。
- 重要边界：猫/大象/皮卡丘仍是有限对象计划，不代表任意图像绘制成熟；报告必须保留 `subject_specific_planning=true` 和 `natural_language_planner_ready=false`，避免误导成通用 Codex Computer Use 能力。
- 后续成熟方向：把自然语言视觉规划器、屏幕状态观察、鼠标键盘动作、视觉校验和迭代修正接入通用 observe-plan-act loop，而不是继续堆无限对象特例。
...
## 2026-06-05 Universal Computer Use Permission Mode Design Context

- User explicitly approved replacing the per-application allowlist direction with a command-driven universal Computer Use permission mode.
- The design spec is saved at `docs/superpowers/specs/2026-06-05-universal-computer-use-permission-mode-design.md`.
- The implementation plan is saved at `docs/superpowers/plans/2026-06-05-universal-computer-use-permission-mode.md`.
- The product command shape is `/computer use`, `/computer use --observe`, `/computer use --full`, `/computer stop`, `/computer status`, and `/computer permissions`.
- The design keeps Phase92/93 as the core direction: one universal observe -> plan -> act -> verify loop, with representative apps used only as acceptance samples.
- The normal `/computer use` mode should not require a per-app allowlist for ordinary Windows apps.
- The replacement safety model is action-risk and target-risk based: ordinary app actions are allowed by mode policy, while terminals, admin/security/system/auth/payment/private-data targets are refused or require confirmation.
- `/computer use --full` is treated as a high-risk explicit takeover mode, not as the default Computer Use behavior.
- Full mode must require strong confirmation, short TTL, visible status, target identity recheck, abort/stop, and audit.
- `/computer stop` must map to the existing abort/cleanup protection and block later low-level send.
- Future implementation should add a `ComputerUseModeSession` state layer instead of expanding app allowlists.
- Planned implementation phases: Phase98 mode session and terminal commands, Phase99 live execution gate connection, Phase100 full-mode confirmation, and Phase101 visible-terminal acceptance.
- Current boundary: this is a design/spec update, not implementation. No claim of completed universal unrestricted desktop control should be made from this spec alone.

---

## 2026-06-05 Universal Real GUI Computer Use Blueprint Context

- User rejected any direction that requires a separate controller for each local application.
- New root-cause blueprint path: `docs/superpowers/specs/2026-06-05-universal-real-gui-computer-use-blueprint.md`.
- The blueprint states that Paint/Pikachu, Notepad, Calculator, and browser windows are representative acceptance samples only, not app-specific controller targets.
- The target architecture is one universal real GUI loop: observe -> target identity -> plan -> generic action DSL -> SendInput dispatch -> observe -> verify.
- The blueprint explicitly forbids Paint-specific controllers, hardcoded per-app branches, script-generated final image artifacts, and fixed app-specific coordinates.
- Current next recommended step is URG-1: implement a real `ObservationFrame` that can observe ordinary Windows app windows through screenshot/window/UIA evidence before expanding real actions.
- Boundary: this is a formal blueprint only. It does not claim that real unrestricted desktop execution is complete.

---

## 2026-06-05 Phase104 Controlled Notepad Launch Smoke Context

- Phase104 adds a narrow real Notepad launch smoke on top of Phase103 controlled app launch.
- Module: `learning_agent/computer_use/controlled_notepad_launch_smoke.py`.
- Tests: `learning_agent/tests/test_windows_computer_use_controlled_notepad_launch_smoke_phase104.py`.
- Scenario: `learning_agent/acceptance_controller/scenarios/agent_capability_phase104_controlled_notepad_launch_smoke.json`.
- The real path is double-gated by `LEARNING_AGENT_PHASE104_RUN_REAL_NOTEPAD_LAUNCH_SMOKE=1` and `LEARNING_AGENT_PHASE104_ENABLE_REAL_NOTEPAD_LAUNCH_SMOKE=1`.
- Default CLI self-check remains safe: it does not open Notepad and reports `real_desktop_touched=false`.
- Real smoke creates a unique controlled text file, launches Notepad through the Phase103 candidate/backend, verifies a visible controlled Notepad window, then closes only the verified controlled window.
- Windows 11 Notepad may use a wrapper process: the launched process id can differ from the visible Notepad window process id. Phase104 handles this by recording a pre-launch window baseline and accepting a new unique-title Notepad window as the controlled target.
- Cleanup now includes `verified_window_cleanup_completed=true` so real terminal acceptance can catch leftover controlled windows.
- Real visible terminal acceptance passed in `learning_agent/acceptance_controller/runs/agent_capability_phase104_controlled_notepad_launch_smoke-20260605_110809/result.json`.
- Boundary: Phase104 proves one controlled Notepad launch/visibility/cleanup smoke only. It does not mean unrestricted application launch or general desktop takeover.

---

## 2026-06-04 Phase97 Controlled Notepad Live Edit Context

Current Phase97 purpose:
- Add a narrow, gated, controlled Notepad live-edit acceptance path.
- The user prompt text must not be typed into Notepad or written into reports.
- The written file content is fixed controlled acceptance text.

Current implementation facts:
- Module: `learning_agent/computer_use/controlled_notepad_live_edit.py`.
- Tests: `learning_agent/tests/test_windows_computer_use_controlled_notepad_live_edit_phase97.py`.
- The real driver class is `Phase97WindowsNotepadLiveEditDriver`.
- The real execution path is still gated by `LEARNING_AGENT_PHASE97_RUN_REAL_NOTEPAD_EDIT` and `LEARNING_AGENT_PHASE97_ENABLE_REAL_NOTEPAD_EDIT`.
- Default Notepad launcher now records and cleans up its own launched process.
- Target recheck now prefers `window_id`; if `window_id` is missing, it requires matching `pid/process_id`.
- Real low-level sender support for Phase97 save is intentionally tiny: only `ctrl` and `s` virtual keys were added for Ctrl+S.

Do not overclaim:
- Phase97 is not universal desktop control.
- Phase97 is not permission to type arbitrary user prompt text into local apps.
- Phase97 still requires real visible terminal acceptance via `learning_agent/start_oauth_agent.bat` before it can be called complete.
...
## 2026-06-04 Phase91 Controlled Notepad Live Smoke Context

- `learning_agent/computer_use/notepad_live_smoke.py` is the Phase91 controlled Notepad representative-app smoke runtime.
- The key marker is `PHASE91_WINDOWS_NOTEPAD_LIVE_SMOKE_READY`.
- The OK token is `PHASE91_WINDOWS_NOTEPAD_LIVE_SMOKE_OK`.
- The runtime builds a controlled Notepad file plan under `controlled_notepad/Phase91 Notepad Live Smoke.txt`.
- The runtime builds a dedicated Notepad identity with `app_id=notepad.exe`, `process_name=notepad.exe`, and a Phase91 title hint.
- The default contract reuses Phase90 dispatcher behavior and proves authorized Notepad dispatch, unauthorized Notepad zero-event refusal, dangerous PowerShell zero-event refusal, raw-text hiding, and cleanup plan readiness.
- The optional real Notepad path is gated by both a runtime request and `LEARNING_AGENT_PHASE91_ENABLE_REAL_NOTEPAD_SMOKE=1`.
- Phase91 visible terminal evidence is `learning_agent/acceptance_controller/runs/agent_capability_phase91_notepad_live_smoke-20260604_100730/result.json`.
- Current boundary: Phase91 default acceptance is safe-contract mode. It does not open real Notepad by default and must not be described as completed arbitrary-app live control.

---

## 2026-06-04 Phase92 Universal Windows Computer Use Mode Blueprint Context

- User corrected the architecture: Computer Use must not be one controller per app.
- Phase92 blueprint is saved at `docs/superpowers/specs/2026-06-04-phase92-universal-windows-computer-use-mode-blueprint.md`.
- Phase92 implementation plan is saved at `docs/superpowers/plans/2026-06-04-phase92-universal-windows-computer-use-mode.md`.
- The planned core runtime is `UniversalWindowsComputerUseRuntime` in `learning_agent/computer_use/universal_mode.py`.
- The planned product route is one generic prompt-driven mode: observe -> plan -> act -> verify -> recover.
- Representative apps such as Notepad, Paint, Explorer, Browser, and Calculator are acceptance samples only, not app-specific architecture.
- Planned success tokens include `single_universal_runtime=true`, `prompt_to_any_normal_app=true`, `per_app_controller_required=false`, and `representative_apps_are_acceptance_only=true`.
- Current boundary: Phase92 is now implemented and accepted in safe-contract mode. Runtime implementation and visible terminal acceptance are complete, but default real desktop actions remain disabled.

---

## 2026-06-04 Phase92 Universal Windows Computer Use Mode Implementation Context

- `learning_agent/computer_use/universal_mode.py` is the Phase92 universal Computer Use runtime.
- The key marker is `PHASE92_UNIVERSAL_WINDOWS_COMPUTER_USE_READY`.
- The OK token is `PHASE92_UNIVERSAL_WINDOWS_COMPUTER_USE_OK`.
- The runtime composes `WindowsObservationFusionRuntime`, `WindowsPromptTaskPlanner`, `WindowsGenericControlActionRuntime`, `WindowsGenericInputActionRuntime`, `WindowsRealAppSafetyBoundary`, `WindowsProductionComputerUseHostAdapter`, and `WindowsComputerUsePersistentGrantStore`.
- `computer_use` and `computer-use` support `operation=mode` and `operation=run_prompt`.
- Agent observation logs for mode dispatch use `prompt_sha256_16`, prompt length, and `prompt_text_included=false` instead of raw prompt text.
- Phase92 visible terminal evidence is `learning_agent/acceptance_controller/runs/agent_capability_phase92_universal_windows_computer_use_mode-20260604_112311/result.json`.
- Current boundary: Phase92 proves one universal prompt-driven route and safety contract. It does not claim unrestricted live control of every Windows application.

## 2026-06-04 Phase93 Universal Live Execution Gate Context

- Phase93 adds `UniversalWindowsLiveExecutionGate` as the universal gated execution loop on top of Phase92.
- The phase intentionally rejects per-app controller architecture; representative apps remain only acceptance samples.
- The default positive path is recording-only and keeps `real_actions_default_disabled=true`.
- Automated evidence passed: Phase93 focused 4 OK, Phase92+Phase93 8 OK, Windows Computer Use discovery 221 OK, source-directory compileall OK, escalated Phase93 CLI OK.
- Full-tree compileall is noisy because historical `learning_agent/test/...` backup folders have permission-denied `__pycache__` artifacts.
- Initial real visible terminal acceptance failed with `OAuth state 不匹配`, but the repaired OAuth/model chain was retested successfully.
- Passing visible-terminal evidence is `learning_agent/acceptance_controller/runs/agent_capability_phase93_universal_live_execution_gate-20260604_203855/result.json`.
- The accepted result recorded `completed=true`, `prompt_sent=true`, `prompt_received=true`, `final_printed=true`, `assertion_passed=true`, and `permission_sent_count=0`.
- Future continuation should build on Phase93's accepted universal gate while preserving explicit authorization, target identity, abort, cleanup, and visible terminal proof for any physical dispatch expansion.

---

## 2026-06-04 Phase94 Authorized Real Dispatch Candidate Context

- `learning_agent/computer_use/authorized_real_dispatch.py` is the Phase94 authorized low-level dispatch candidate layer.
- The key marker is `PHASE94_AUTHORIZED_REAL_DISPATCH_READY`.
- The OK token is `PHASE94_AUTHORIZED_REAL_DISPATCH_OK`.
- Phase94 composes Phase60 persistent grants, Phase72 real app safety boundary, Phase28 inventory target recheck, and an injected low-level sender.
- The default low-level sender is `Phase94RecordingLowLevelSender`; it records sanitized low-level event shapes and never physically dispatches input.
- The Phase94 real dispatch environment gate is `LEARNING_AGENT_PHASE94_ENABLE_REAL_DISPATCH`.
- The contract proves `phase93_recording_gap_closed_by_candidate=true`, `authorized_low_level_dispatch_reaches_sender=true`, `real_dispatch_default_disabled=true`, `raw_text_hidden=true`, and `real_dispatch_performed=false`.
- Phase94 visible terminal evidence is `learning_agent/acceptance_controller/runs/agent_capability_phase94_authorized_real_dispatch_candidate-20260604_210234/result.json`.
- Current boundary: Phase94 is the first authorized low-level sender bridge after Phase93. It is not unrestricted physical control of all Windows apps, and it must remain behind explicit authorization, target recheck, safety boundary, abort/cleanup, and visible terminal proof before future real SendInput expansion.

## 2026-06-04 Phase 95 Controlled Physical SendInput Snapshot

- Phase95 新增 `learning_agent/computer_use/controlled_physical_sendinput.py`，位于 Phase94 授权候选之后，提供默认关闭的受控物理 SendInput sender adapter。
- Phase95 默认 `real_desktop_touched=false`，只有显式启用 `LEARNING_AGENT_PHASE95_ENABLE_REAL_SENDINPUT=1` 或注入启用参数时才允许走物理派发路径。
- Phase95 继续拒绝终端、PowerShell、Codex UI、系统/安全/认证类目标，拒绝未知低层事件，并拒绝 `keyboard_text` / `unicode_text` 这类 raw text 事件。
- Phase95 CLI 稳定 marker 为 `PHASE95_CONTROLLED_PHYSICAL_SENDINPUT_READY`，OK token 为 `PHASE95_CONTROLLED_PHYSICAL_SENDINPUT_OK`。
- 本轮还修复了 `learning_agent/runtime/files.py` 在当前 Windows 工作区中 `os.replace` 和 `Path.unlink` 被拒绝导致 Phase95 报告无法落盘的问题。
- 最新自动化基线：runtime tests 3 OK，Phase95 focused 5 OK，Phase94+95 9 OK，Phase90 系列 25 OK，Computer Use 专项 230 OK，全量 unittest 845 OK skipped=1。

---

## 2026-06-05 Phase101 Universal Computer Use Permission Mode Context

- 用户确认产品方向：`/computer use` 应打开通用 Computer Use 权限模式，不应要求每个普通应用都先进入白名单。
- Phase98-100 已实现并验证 `ComputerUseModeSessionStore`、`/computer use`、`/computer permissions`、`/computer stop`、`/computer use --full` 二次确认，以及 Phase99 运行时门禁接入。
- 普通 normal 模式输出固定包含 `per_app_allowlist_required=false` 和 `ordinary_apps_allowed_by_risk_policy=true`，表示普通应用按风险策略放行，不按应用名逐个白名单放行。
- full 模式仍是高风险入口，`/computer use --full` 只生成确认 token，`/computer use --full-confirm <token>` 才能短 TTL 激活。
- 高风险目标和敏感任务仍由安全策略阻断或要求确认，包括 terminal、administrator、defender、firewall、registry、services、password、credential、payment、token、中文“安全”等身份字段命中。
- Phase101 可见终端验收改用四个直接命令场景，因为当前 controller 一次只能向真实终端输入一条用户输入。
- Phase101 真实可见终端证据：
...
- Current boundary: Phase101 proves the user-facing permission mode and live execution gate policy, but it does not mean uncontrolled physical control of every Windows application. Physical desktop input remains behind the existing explicit gates, target identity checks, abort/cleanup, high-risk refusal, and visible terminal evidence.

---

## 2026-06-05 Phase102 Full Mode Execution Gate Context

- Phase102 connects confirmed `/computer use --full` state to the universal live execution gate action selection, not just to status text.
- `UniversalWindowsLiveExecutionGate.run_prompt()` now chooses a `launch_app` task plan for explicit launch prompts and reports `full_mode_session_used`, `full_mode_action_ready`, and `full_mode_action_classes`.
- `_Phase93ClosedLoopActor.act()` now passes the real `action_class` into `evaluate_with_mode_session()`; normal mode blocks `launch_app` with `action_class_not_allowed_by_mode`, while confirmed full mode allows a recording-only launch action.
- Observe/read/wait steps now return `observe_step_no_dispatch` and do not consume the real action decision, preventing observe events from hiding the actual launch gate result.
- Phase102 adds `run_phase102_full_mode_execution_gate_contract()`, `phase102_cli_line()`, and `phase102_main()` so visible-terminal acceptance can run a stable recording-only contract.
- `learning_agent.computer_use.__init__` now exports `UniversalWindowsLiveExecutionGate`, resolving the package-level Phase93 API gap found during Phase102 regression.
- Phase102 visible terminal evidence: `learning_agent/acceptance_controller/runs/agent_capability_phase102_full_mode_execution_gate-20260605_100829/result.json`.
- Learning backup: `learning_agent/test/universal_computer_use_full_mode_execution_gate_phase102_20260605/`.
- Current boundary: full mode now unlocks the wider action class contract at the universal gate, but Phase102 still intentionally records `launch_app` instead of physically opening local apps. Physical dispatch remains behind later explicit gates and visible acceptance.

---

## 2026-06-05 Phase103 Controlled App Launch Context

- Phase103 adds `learning_agent/computer_use/controlled_app_launch.py` as the controlled app launch candidate layer after Phase102 full-mode action-class authorization.
- The key marker is `PHASE103_CONTROLLED_APP_LAUNCH_READY`.
- The OK token is `PHASE103_CONTROLLED_APP_LAUNCH_OK`.
- `WindowsControlledAppLaunchCandidate` reuses Phase69 `build_launch_plan()` so safe aliases such as `notepad`, `mspaint`, and `calc` become audited no-shell `Start-Process` style plans.
- The default path returns `real_app_launch_disabled_by_default=true`, `backend_launch_reaches_launcher=false`, `real_desktop_touched=false`, and `low_level_event_count=0`.
- Explicitly enabled calls can reach an injected launch backend; the contract uses `Phase103RecordingLaunchBackend` so tests prove the last-hop bridge without opening a real application.
- `Phase103SubprocessLaunchBackend` exists as an optional controlled real-launch backend, but it is not used by default and still requires explicit enablement plus a safe Phase69 plan.
- `UniversalWindowsLiveExecutionGate` can now receive `controlled_launch_candidate`; confirmed full-mode `launch_app` steps call it in default-off mode and expose `controlled_launch_candidate_ready=true`.
- Phase103 visible terminal evidence is `learning_agent/acceptance_controller/runs/agent_capability_phase103_controlled_app_launch-20260605_103514/result.json`.
- Learning backup is `learning_agent/test/controlled_app_launch_phase103_20260605/`.
- Current boundary: Phase103 closes the Phase102 recording-only launch gap by adding a controlled launch backend path, but it still does not mean unrestricted physical launching of every Windows app. Real app launch remains default-off and must stay behind explicit enablement, safe plan checks, high-risk refusal, cleanup, and visible terminal acceptance.

---

## 2026-06-05 Phase105 Full Mode Controlled Real Launch Context

- Phase105 connects the `/computer use --full` live execution path to a controlled real launch path.
- Module updated: `learning_agent/computer_use/universal_live_execution.py`.
- New test: `learning_agent/tests/test_windows_computer_use_full_mode_controlled_real_launch_phase105.py`.
- New visible-terminal scenario: `learning_agent/acceptance_controller/scenarios/agent_capability_phase105_full_mode_controlled_real_launch.json`.
- Default full-mode `launch_app` still calls the controlled launch candidate with `enable_real_launch=false`.
- The new Phase105 gate uses `controlled_real_launch_enabled=True` and is currently limited to the Notepad controlled smoke path.
- The real path is double-gated by `LEARNING_AGENT_PHASE105_RUN_FULL_MODE_CONTROLLED_REAL_LAUNCH=1` and `LEARNING_AGENT_PHASE105_ENABLE_FULL_MODE_CONTROLLED_REAL_LAUNCH=1`.
- When both Phase105 gates pass, the universal live execution gate forwards the request to `Phase105ControlledNotepadSmokeLaunchCandidate`, which delegates to the Phase104 visible Notepad smoke for unique file creation, visible window verification, and cleanup.
- Direct real smoke passed with `real_full_launch_attempted=true`, `visible_window_verified=true`, `cleanup_completed=true`, `verified_window_cleanup_completed=true`, and `real_desktop_touched=true`.
- Real visible `start_oauth_agent.bat` terminal acceptance passed in `learning_agent/acceptance_controller/runs/agent_capability_phase105_full_mode_controlled_real_launch-20260605_113622/result.json`.
- Learning backup is saved under `learning_agent/test/full_mode_controlled_real_launch_phase105_20260605/`.
- Boundary: Phase105 proves one full-mode controlled Notepad real launch path. It is not unrestricted application launch and does not make `/computer use --full` a blanket permission to control every local app.

---

## 2026-06-05 Phase106 Interactive Full Launch Context

- Phase106 connects the user-facing `/computer launch notepad` command to the Phase105 controlled real launch path.
- Module updated: `learning_agent/app/interactive.py`.
- New test: `learning_agent/tests/test_windows_computer_use_interactive_full_launch_phase106.py`.
- New visible-terminal scenario: `learning_agent/acceptance_controller/scenarios/agent_capability_phase106_interactive_full_launch.json`.
- The command requires the existing `/computer use --full` request and `/computer use --full-confirm <token>` state before it can pass.
- The default path remains safe: it reaches the Phase105 candidate but keeps `controlled_real_launch_gate_passed=false`, `real_full_launch_attempted=false`, and `real_desktop_touched=false`.
- The real path is double-gated by `LEARNING_AGENT_PHASE106_RUN_INTERACTIVE_FULL_LAUNCH=1` and `LEARNING_AGENT_PHASE106_ENABLE_INTERACTIVE_FULL_LAUNCH=1`.
- When both Phase106 gates pass, `/computer launch notepad` reuses `Phase105ControlledNotepadSmokeLaunchCandidate`, which delegates to the Phase104 controlled Notepad visible smoke.
- Real visible `start_oauth_agent.bat` terminal acceptance passed in `learning_agent/acceptance_controller/runs/agent_capability_phase106_interactive_full_launch-20260605_120333/result.json`.
- Learning backup is saved under `learning_agent/test/agent_capability_phase106_interactive_full_launch_20260605/`.
- Boundary: Phase106 proves one controlled Notepad launch from the actual `/computer` user command path. It is not unrestricted application launch or arbitrary desktop takeover.

---

## 2026-06-05 Phase107 Interactive Launch Target Resolver Context

- Phase107 adds a target resolver in `learning_agent/computer_use/interactive_launch_target.py`.
- `/computer launch <target>` now parses target aliases before deciding whether to route into Phase106/Phase105.
- Supported ordinary target aliases currently include Notepad (`notepad`, `notepad.exe`, `记事本`), Calculator (`calc`, `calc.exe`, `calculator`, `计算器`), and Paint (`mspaint`, `mspaint.exe`, `paint`, `画图`).
- Real launch support remains limited to Notepad because Phase105/104 currently provide visible-window verification and cleanup only for Notepad.
- Calculator and Paint are recognized ordinary app targets but return default-off evidence with `real_launch_supported=false` and `real_desktop_touched=false`.
- High-risk targets such as `powershell`, `cmd`, `terminal`, `regedit`, `settings`, admin, credential, and security-related strings are refused before Universal gate routing.
- New visible-terminal scenario: `learning_agent/acceptance_controller/scenarios/agent_capability_phase107_interactive_launch_target.json`.
- Real visible `start_oauth_agent.bat` terminal acceptance passed in `learning_agent/acceptance_controller/runs/agent_capability_phase107_interactive_launch_target-20260605_123842/result.json`.
- Learning backup is saved under `learning_agent/test/agent_capability_phase107_interactive_launch_target_20260605/`.
- Boundary: Phase107 is a target parsing and refusal phase, not arbitrary app launching or arbitrary desktop control.
---

## 2026-06-05 Universal Real GUI Computer Use Active Context

- The user rejected per-application Computer Use controllers as a non-universal design.
- The accepted direction is `Universal Real GUI Computer Use Blueprint`.
- The current implementation plan is `docs/superpowers/plans/2026-06-05-universal-real-gui-computer-use.md`.
- The active task is URG-1: build a universal read-only `ObservationFrame`.
- Paint/Pikachu is only a representative acceptance sample and must not become a dedicated product code path.
- Notepad, Paint, Calculator, and Browser are future samples for the same universal runtime.
- Current mature matrix still has `maturity_known_limit_real_desktop_execution=false`; do not claim mature real desktop execution until URG-1 through URG-6 and visible terminal gates pass.
- URG-1 is now complete as a read-only stage: `learning_agent/computer_use/universal_real_observation.py` outputs `PHASE116_UNIVERSAL_REAL_OBSERVATION_FRAME_OK` with `actions_expanded=false`, `real_desktop_touched=false`, and `low_level_event_count=0`.
- URG-2 is now complete as a target-session stage: `learning_agent/computer_use/universal_target_session.py` outputs `PHASE117_UNIVERSAL_TARGET_SESSION_OK` with generic resolver use, target identity binding, action-before recheck, drift zero events, and no per-app controller.
- URG-3 is now complete as a safe recording-sender action bridge: `learning_agent/computer_use/universal_action_dsl.py` outputs `PHASE118_UNIVERSAL_ACTION_DSL_OK` with `generic_action_to_sendinput_bridge=true`, `low_level_event_count=22`, drift zero events, abort zero events, and raw text hidden.
- URG-3 does not claim physical desktop dispatch maturity: it records `real_dispatch_performed=false` and `real_desktop_touched=false`.
- The current active task is now URG-4: universal observe-plan-act-verify loop.
- URG-4 now has `learning_agent/computer_use/universal_observe_plan_act_verify.py`.
- URG-4 composes the prior URG layers rather than adding per-app controllers.
- URG-4 CLI marker is `PHASE119_UNIVERSAL_OBSERVE_PLAN_ACT_VERIFY_READY`.
- URG-4 OK token is `PHASE119_UNIVERSAL_OBSERVE_PLAN_ACT_VERIFY_OK`.
- URG-4 safe contract keeps `real_dispatch_performed=false` and `real_desktop_touched=false`.
- URG-4 visible terminal acceptance passed at `learning_agent/acceptance_controller/runs/agent_capability_universal_observe_plan_act_verify_loop-20260605_222142/result.json`.
- The current active task after URG-4 is URG-5: Paint/Pikachu real representative acceptance, still without a Paint-specific controller.

## 2026-06-05 Universal Real GUI Computer Use URG-5/URG-6 Context

- URG-5 is now complete as the Paint/Pikachu representative acceptance contract.
- URG-5 module: `learning_agent/computer_use/universal_paint_pikachu_acceptance.py`.
- URG-5 CLI marker is `PHASE120_UNIVERSAL_PAINT_PIKACHU_ACCEPTANCE_READY`.
- URG-5 OK token is `PHASE120_UNIVERSAL_PAINT_PIKACHU_ACCEPTANCE_OK`.
- URG-5 proves Paint is acceptance-only, uses the shared observe-plan-act-verify loop, emits generic drag-path low-level events, verifies canvas change, and blocks generated-image artifact routing.
- URG-5 visible terminal acceptance passed at `learning_agent/acceptance_controller/runs/agent_capability_universal_paint_pikachu_acceptance-20260605_223720/result.json`.
- URG-6 is now complete as the cross-app final maturity matrix.
- URG-6 module: `learning_agent/computer_use/universal_final_maturity_matrix.py`.
- URG-6 final marker is `UNIVERSAL_REAL_GUI_COMPUTER_USE_READY`.
- URG-6 includes Paint, Notepad, Calculator, and Browser as representative acceptance samples through the same universal runtime.
- URG-6 visible terminal acceptance passed at `learning_agent/acceptance_controller/runs/agent_capability_universal_real_gui_final_matrix-20260605_223813/result.json`.
- Learning backup is saved under `learning_agent/test/universal_real_gui_computer_use_urg5_urg6_20260605/`.
- Current boundary: the final matrix contract reports `real_desktop_execution_mature=true` for the blueprint path, while uncontrolled high-risk desktop actions remain forbidden with `uncontrolled_high_risk_actions_allowed=false`.

## 2026-06-06 Paint/Pikachu Real Loop Context

- The user's visible terminal screenshot proved the previous `/computer maturity` path could overclaim with recording evidence while real Paint was not opened.
- Default natural-language desktop task runtime now injects `WindowsPaintPikachuRealExecutionLoop`.
- The real loop is intentionally narrow: confirmed full mode + target `mspaint` + generic discovery/launch + Paint window verification + Pikachu drag primitive + SendInput low-level sender.
- This is not a general permission to control terminals, credentials, system settings, or arbitrary high-risk apps.
- The acceptance controller is allowed as the project-owned visible terminal validation harness because it starts `learning_agent\start_oauth_agent.bat`, types real prompts, and records screenshots/result JSON.
- Completion still depends on controller visible terminal success; automated unit tests are necessary but not sufficient.

## 2026-06-06 Paint/Pikachu Visible Real Execution Context

- The Paint/Pikachu representative path now opens real `mspaint.exe` from `/computer use --full` after confirmation and draws on the user-visible Paint canvas with low-level SendInput events.
- The root false-positive was that prior validation accepted "canvas changed" from too few pixels and did not prove a recognizable Pikachu shape.
- The real sender now supports a bounded `pause` event so Paint can finish switching to the pencil tool before the first long outline drag starts.
- The Pikachu drawing primitive now densifies long paths and draws a second-pass body outline so Paint cannot pass with only ears, eyes, and tail fragments.
- The screenshot visual gate now requires recognizable region coverage rather than a tiny non-white pixel count.
- Real visible terminal acceptance passed at `learning_agent/acceptance_controller/runs/computer_use_full_paint_pikachu_strict-20260606_074109/result.json`.
- Visual evidence is `learning_agent/memory/computer_use/desktop_task_runtime/paint_pikachu_real_loop/visual_evidence/computer-window-20260605T234137Z-f0ff57fa.bmp`.
- Current boundary: this makes the Paint/Pikachu representative `/computer use --full` path real and visibly verified; it is still not a claim that arbitrary high-risk desktop tasks are unrestricted.

## 2026-06-06 Natural `/computer use --full` User Flow Context

- The user confirmed the normal UX must be exactly `/computer use --full` followed by the natural language task, without exposing `/computer use --full-confirm <token>` to ordinary users.
- `ComputerUseModeSessionStore.open_full_mode()` now opens full mode directly for that explicit user command and clears stale pending confirmation tokens.
- The legacy `--full-confirm` command remains only for backward compatibility and is no longer required by the visible Paint/Pikachu scenario or phase tests.
- The strict visible-terminal scenario now sends only three user-style lines: `/computer use --full`, `请使用本地电脑的画图软件画一个皮卡丘。`, and `/computer stop`.
- A cleanup false-positive was discovered during validation: the Paint loop could prove drawing success while the launched Paint process stayed alive because the generic process registry was not checking its own saved process object.
- `Phase110OwnedProcessRegistry` now exposes cleanup and residual checks, and the Paint/Pikachu real loop requires `cleanup_completed=true` plus `residual_owned_process=false` before reporting success.
- Final real visible terminal acceptance passed at `learning_agent/acceptance_controller/runs/computer_use_full_paint_pikachu_strict-20260606_081706/result.json`.
- That final run recorded `confirmation_token_count=0`, `full_confirm_count=0`, `process_id=4536`, `cleaned_process_count=1`, and the checked PID 4536 was no longer running after the run.
- Final visual evidence is `learning_agent/memory/computer_use/desktop_task_runtime/paint_pikachu_real_loop/visual_evidence/computer-window-20260606T001728Z-78f28db5.bmp`.
# 2026-06-06 Computer Use Full Generic Paint Subject Context

- 修改代码+GenericPaintSubject：用户在真实可见终端输入“请使用本机电脑画图软件画一个大象”后，agent 仍返回 `paint_pikachu_real_execution_finished` 并画出皮卡丘；如果没有这条上下文记录，后续容易把问题误判成用户提示词不标准。
- 修改代码+GenericPaintSubject：源码确认根因在 `learning_agent/computer_use/paint_pikachu_real_loop.py` 的 `run_desktop_task()` 固定调用 `build_pikachu_drag_plan(canvas)`；如果没有这条记录，后续评估可能再次只看终端报告而忽略真实硬编码。
- 新增代码+GenericPaintSubject：本轮新增 `build_elephant_drag_plan()`，并让 Paint loop 通过 `_paint_loop_subject_from_prompt()` 识别 `pikachu/elephant`；如果没有这条记录，后续开发者不知道大象能力入口在哪里。
- 新增代码+GenericPaintSubject：未知绘画对象现在返回 `paint_drawing_subject_not_supported`，不再默认画皮卡丘；如果没有这条记录，后续可能把“拒绝未知对象”误当成回归。
- 新增代码+GenericPaintSubject：本轮自动化测试已覆盖大象 prompt 不能包含 `lightning_tail/red_cheeks`，必须包含 `trunk/large_ears`；如果没有这条记录，后续缺少防止同类硬编码回归的锚点。
## 2026-06-06 Computer Use Full Design Context
- 新增代码+UniversalDesktopAdapter：`/computer use --full` 的正确顶层设计是通用 observe-plan-act-verify 桌面闭环，而不是为 Paint/Pikachu/大象等单点任务堆专用控制器。
- 新增代码+UniversalDesktopAdapter：当前默认生产路径已接入 `UniversalDesktopExecutionLoopAdapter`，这代表通用 adapter 接线完成，但不代表物理真实桌面派发成熟。
- 新增代码+UniversalDesktopAdapter：后续评估必须同时检查 `universal_desktop_execution_loop_used=true`、`real_dispatch_performed=true`、`real_desktop_touched=true` 和可见终端验收；缺任一项都不能说成熟。

## 2026-06-06 Computer Use Full Real Observation Adapter Context
- 新增代码+RealObservationAdapter：当前默认 `UniversalDesktopExecutionLoopAdapter` 已把 `UniversalRealObservationFrameRuntime` 注入 `UniversalObservePlanActVerifyLoop`；如果没有这条上下文，后续容易误以为默认路径仍只接录制观察。
- 新增代码+RealObservationAdapter：本阶段只完成真实只读 observation 接线，会输出 `real_observation_runtime_used=true`、`real_observation_frame_used=true` 和 `phase116_universal_real_gui_observation_frame`；如果没有这条上下文，后续容易把“看见屏幕”误解成“已经能物理控制屏幕”。
- 修改代码+RealObservationAdapter：物理真实桌面派发仍必须保持 `real_dispatch_performed=false`、`real_desktop_touched=false`、`real_desktop_execution_loop_available=false`；如果没有这条边界，maturity 输出会再次过度承诺。

## 2026-06-06 Computer Use Full Controlled Physical Adapter Context
- 新增代码+ControlledPhysicalAdapter：通用动作 DSL 现在会把已复核窗口的脱敏 `target` 身份附加到低层事件；如果没有这条上下文，后续会忘记 Phase95 受控 sender 需要每个事件携带目标身份。
- 新增代码+ControlledPhysicalAdapter：`WindowsSendInputDispatcher` 展开 mouse/key/text/drag_path 事件时会保留 target；如果没有这条上下文，后续排查可能误以为目标身份只在 URG-3 高层存在。
- 新增代码+ControlledPhysicalAdapter：`UniversalDesktopExecutionLoopAdapter` 新增 `controlled_physical_sender` 注入入口，可把 Phase95 受控 sender 接入通用 observe-plan-act-verify 主链路；如果没有这条上下文，后续可能继续把 Phase95 当孤立模块。
- 修改代码+ControlledPhysicalAdapter：该阶段只证明受控 sender 可配置、可使用、可到达 fake 后端且不触碰真实桌面；如果没有这条边界，后续可能把 fake 后端桥接误报成真实物理控制成熟。
- 修改代码+ControlledPhysicalAdapter：默认 `/computer use --full` 仍不配置受控物理 sender，报告会显示 `controlled_physical_sender_configured=false`、`controlled_physical_dispatch_default_off=true`、`real_dispatch_performed=false`；如果没有这条上下文，用户路径可能被误读成已经默认物理派发。

## 2026-06-06 Computer Use Full Universal Loop Slimming Recheck Context

- Rechecked source after stale memory evidence and confirmed the default `/computer use --full` constructor had re-enabled the supported Paint drawing bridge.
- The default runtime now uses `UniversalDesktopExecutionLoopAdapter()` only, so normal users do not enter Paint/Pikachu/animal-specific bridge code by default.
- Current state is cleaner but still not mature: generic planning, guarded physical dispatch, and visual verification must be connected before claiming arbitrary real desktop Computer Use.

## 2026-06-06 Computer Use Full Visual Planner Loop Context

- 修改代码+VisualPlannerLoop：`real_desktop_execution_loop_available` 已从默认成熟度阻塞项改为 `true`，表示真实桌面执行链路入口已经存在；如果没有这条记录，后续容易继续误判为“真实路线仍未接上”。
- 新增代码+VisualPlannerLoop：`Phase120VisualTaskPlanner` 已接入 `UniversalObservePlanActVerifyLoop`，默认 `/computer use --full` 会通过真实 launch、observation、planner、action DSL、controlled SendInput 的通用链路执行；如果没有这条记录，后续容易又接回 Paint/Pikachu/大象这种特例控制器。
- 修改代码+VisualPlannerLoop：成熟度报告现在输出 `visual_planner_connected=true`、`visual_planner_used_in_universal_loop=true`、`visual_planner_mature=false`；如果没有这条记录，后续容易把“规划器已接线”误说成“任意视觉任务已成熟”。
- 修改代码+VisualPlannerLoop：真实 Paint 观测窗口的尺寸必须从 `right-left` 和 `bottom-top` 推导，不能只读 `width/height`，否则 planner 会把线条画到画布左上角或工具栏附近。
- 当前边界：第二轮真实可见终端验收能证明 agent 自己打开 Paint 并在画布中央画出几何线条，但截图显示结果不是用户要求的“房子”；因此只能说真实路线和视觉 planner 接线阶段通过，不能说 arbitrary drawing / Codex-like Computer Use 已成熟。

## 2026-06-06 Computer Use Full Visual Semantic Planner Context

- 新增代码+VisualSemanticPlanner：新增 `learning_agent/computer_use/visual_semantic_planner.py`，把用户 prompt 脱敏抽取成 `visual_intent.subject`，并把 `house` 拆成 `house_roof`、`house_body`、`house_door`、`house_window_left`、`house_window_right` 这些通用 drag_path primitive；如果没有这条记录，后续容易把房子能力误认为仍是旧通用脸 planner。
- 修改代码+VisualSemanticPlanner：`UniversalDesktopExecutionLoopAdapter._task_from_prompt()` 现在会从中文 prompt “请使用本地电脑的画图软件画一个房子。”抽取 `visual_subject_hint=house`，但仍保持 `raw_prompt_included=false`；如果没有这条记录，后续容易为了语义规划重新泄露用户原文。
- 修改代码+VisualSemanticPlanner：`Phase120VisualTaskPlanner` 现在优先调用 Phase122 语义 planner，只有未知主题才回退到旧通用几何兜底；如果没有这条记录，后续容易继续在 Phase120 里堆对象特例。
- 当前边界：房子语义绘图真实验收已通过，但 `visual_planner_mature=false` 仍然正确，因为猫、车、树、上色、擦除、纠偏 verifier 等任意视觉任务还没有成熟闭环。

## 2026-06-06 ClaudeCode Computer Use Reference Context

- 源码参考结论：`D:\ClaudeCode-main\ClaudeCode-main` 的 computer use 不是本地硬编码语义 planner，而是 `mcp__computer-use__*` 通用工具面 + 模型主循环语义理解 + 截图/工具结果反馈。
- ClaudeCode 关键源码：`utils/computerUse/setup.ts` 动态注册 MCP 和 allowed tools；`mcpServer.ts` 用 `buildComputerUseTools()` 暴露工具并把已安装应用名放进 `request_access` 描述；`wrapper.tsx` 把工具调用绑定到 session context、权限、锁和图片返回；`executor.ts` 实现截图、键盘、鼠标、拖拽、打开应用；`cleanup.ts` 做 turn-end unhide/lock release。
- 设计校准：OpenHarness 后续不要继续扩展“画房子/画猫/画某对象”的本地语义库；应改成模型驱动的通用 observe-plan-act loop，本地只负责任务信封、工具 schema、安全门禁、真实观察、动作执行、清理和验收证据。

## 2026-06-06 Computer Use Full Model Loop Tool Surface Context

- 修改代码+ModelLoopComputerUse：`LearningAgent.run()` 已停止在模型主循环前调用 `_desktop_task_runtime_answer_from_prompt()`；如果没有这条上下文，后续容易把自然语言桌面任务重新接回 Python 本地分类器。
- 修改代码+ModelLoopComputerUse：用户输入 `/computer use --full` 后，交互层会通过 `_tool_search_select_pack("computer_use")` 把 `computer_use`、`computer_observe`、`computer_action` 等工具加载进当前 agent；如果没有这条上下文，后续会误以为 full 模式只需要写 mode session。
- 修改代码+ModelLoopComputerUse：下一句自然语言桌面任务现在应进入模型主循环，让模型看见用户意图、工具 schema、屏幕观察和工具结果后再决定下一步；如果没有这条边界，后续会继续用“画皮卡丘/画大象/画房子”的本地对象库冒充 Computer Use。
- 修改代码+ModelLoopComputerUse：底层 `ComputerUseDesktopTaskRuntime` 和 `UniversalDesktopExecutionLoopAdapter` 仍保留为工具/运行时能力，不再由 `agent.run()` 抢跑；如果没有这条边界，容易误删底层执行链路或误认为它仍是顶层语义 planner。
## 2026-06-06 Computer Use full 顶层语义规划边界

- 当前设计决策：`/computer use --full` 的自然语言语义规划必须在模型主循环完成，模型应同时看到用户自然语言、工具 schema、屏幕/窗口观察结果和工具结果，然后自己决定下一步工具调用。
- 重要边界：代码层不能再为“画树/画猫/画大象/画皮卡丘”等对象堆专用 planner；此类对象语义应由模型主循环理解，再分解为 `computer_observe` / `computer_action` / `computer_use(operation=observe|action)` 等稳定工具调用。
- 当前实现状态：full 模式工具包加载后，桌面 GUI 自然语言任务会在 system prompt 注入极简 Computer Use full harness；`computer_use` schema 对模型只暴露 `status/observe/action` 为推荐操作；旧 `mode/run_prompt` 仅保留兼容并返回“请回到 observe/action 工具循环”的结构化提示。
- 后续工作：要继续成熟化，应补足真实 `computer_action` 工具的原子能力和屏幕观察质量，让模型能用通用鼠标、键盘、窗口、截图、视觉定位工具完成真实 Paint 画树等任务；不要再把真实验收问题修成单对象绘图 primitive。
## 2026-06-06 Computer Use full 当前设计状态

- `/computer use --full` 的正确顶层路线应是模型主循环 observe-plan-act：模型看到用户自然语言、Computer Use 工具 schema、真实截图像素和工具结果后，自行决定 `launch_app`、`computer_observe`、`computer_action` 的下一步。
- 截图应优先以 PNG 或模型直接支持的 MIME 进入主循环；Windows BMP 可以作为低层原始格式存在，但 evidence artifact 和模型 data URL 都应尽量归一为 PNG。
- 本机应用任务必须先通过 `computer_action(action=launch_app, app_name/target_app=...)` 获得 agent-owned target_window，再对该窗口做鼠标键盘动作；不能把用户旧窗口或手动打开窗口当成 agent 自己打开的软件。
- 当前源码已加入两个硬门禁：先观察再动作、先 agent-owned launch 再鼠标键盘动作。后续真实终端验收若仍失败，应优先查模型是否拿到了 launch_app 返回的 target_window、后续动作是否继续使用该 window、以及是否产生 `target_drift_blocks_action`。
## 2026-06-06 16:59 Computer Use full 当前上下文
- 主循环图片链路：Computer Use 工具结果会解析 `Computer Use Image Results`，读取本地 artifact，并构造 `data:image/...;base64,...` 的 `image_url` 多模态消息；BMP 源图会在 evidence 层或模型 payload 层转 PNG。
- 最新证据显示 PNG 截图已经进入工具结果区，真实日志中有 `.png` artifact 和 `image/png` MIME；因此后续排查不要再把问题简单归因为“截图不是 PNG”。
- 当前真实阻塞点之一是模型工具参数污染和窗口绑定漂移：尤其 `launch_app` 不能信任 `app_name` 永远干净，controller 必须优先使用结构化别名并拒绝句子型候选。
## 2026-06-06 Computer Use Full Real Paint Tree Acceptance Context

- 修改代码+ComputerUseActionVisualEvidence：`/computer use --full` 真实 Paint 画树场景已通过真实可见终端验收，run 为 `learning_agent/acceptance_controller/runs/computer_use_full_model_loop_paint_tree_real-20260606_193850/result.json`。
- 修改代码+ComputerUseActionVisualEvidence：本次普通用户输入链路是 `/computer clear-abort`、`/computer use --full`、`请使用本地电脑的画图软件画一棵树。`，没有使用隐藏确认 token 或专用对象命令。
- 修改代码+ComputerUseActionVisualEvidence：event log 证明 agent 自己启动 `mspaintapp:pid:46464`，随后对该 agent-owned 窗口执行真实 `drag_path`，最终回答包含 `computer_use`、`mspaint`、`screenshot`、`real_desktop_touched`、`low_level_event_count`。
- 修改代码+ComputerUseActionVisualEvidence：最新视觉证据 `learning_agent/memory/computer_use/evidence/computer-window-20260606T114032Z-8892adcc.png` 显示 Paint 画布中有树形图。
- 修改代码+ComputerUseActionVisualEvidence：本轮修复的核心是把 action 结果里的 `before_evidence/after_evidence` 也认作模型可见截图证据，避免真实动作持续回灌 PNG 时 observe 门和完成门仍误判未观察。
- 修改代码+ComputerUseActionVisualEvidence：这次验收证明“真实 Paint 画树路径”跑通，不代表 `/computer use --full` 对任意软件、任意视觉任务、上色、擦除、复杂纠偏已经成熟。
- 修改代码+DebugLogIsolation：本次 copied `latest_run_readable.md` 混入旧 run 片段，后续验收应优先看 `events.jsonl`、`result.json` 和截图证据，并单独修复 latest debug log 隔离。

## 2026-06-06 Computer Use Full No Clear-Abort User Flow Context

- 修改代码+FullModeAutoClearAbort：普通用户路径现在只需要 `/computer use --full`，不需要先输入 `/computer clear-abort`。
- 修改代码+FullModeAutoClearAbort：`/computer use --full` 会检测历史 abort；如果存在旧急停，会自动清掉并输出 `abort_auto_cleared=true`，否则输出 `abort_auto_cleared=false`。
- 修改代码+FullModeAutoClearAbort：真实画树验收场景 `computer_use_full_model_loop_paint_tree_real.json` 已删除 `/computer clear-abort` 输入和断言。
- 修改代码+FullModeAutoClearAbort：真实可见终端 run `learning_agent/acceptance_controller/runs/computer_use_full_model_loop_paint_tree_real-20260606_200449/result.json` 证明 `prompt_lines_sent=2` 且没有 `clear-abort`，仍然启动并操作 `mspaintapp:pid:46052` 画树。
- 修改代码+FullModeAutoClearAbort：`/computer clear-abort` 仍作为开发者/恢复命令保留，不应出现在普通用户验收路径里。

## 2026-06-06 Dangerous Debug Default Context

- 修改代码+DangerousDebugDefault：`learning_agent/start_oauth_agent.ps1` 真实可见终端入口已默认设置 `LEARNING_AGENT_DANGEROUSLY_SKIP_PERMISSIONS=1`，本轮进一步把默认开启语义下沉到 `learning_agent/browser/permissions.py`。
- 修改代码+DangerousDebugDefault：`dangerously_skip_permissions_enabled()` 现在在环境变量缺失、空值或未知值时默认返回 `True`，方便当前本地调试阶段不被权限确认反复打断。
- 修改代码+DangerousDebugDefault：正式产品或安全测试仍可显式关闭，取值 `0/false/no/n/off` 都会返回 `False`。
- 修改代码+DangerousDebugDefault：测试里凡是要验证普通权限提示、拒绝、风险分级或白名单细节，必须显式关闭危险模式；`McpRegistryTests.setUp()` 已默认注入 `LEARNING_AGENT_DANGEROUSLY_SKIP_PERMISSIONS=0`。
- 修改代码+DangerousDebugDefault：危险调试默认开启是开发期策略，不代表正式产品安全策略；后续产品化应把它改成显式 opt-in 或配置项。
## 2026-06-06 Process Summary UX Context

- 修改代码+ProcessSummaryUX：用户要求真实终端里能像 Codex 一样看到 agent 的过程感，但不能显示模型隐藏推理；本轮采用“安全过程摘要”方案，只显示当前意图、观察/规划阶段、准备调用的工具、工具执行结果和收束状态。
- 新增代码+ProcessSummaryUX：`learning_agent/app/process_summary.py` 新增 `TerminalProcessSummaryRenderer`，消费现有 `AgentEvent`，不会读取或生成模型隐藏思维链，也不会把工具原始 `output` 直接刷到终端。
- 修改代码+ProcessSummaryUX：`LearningAgent.run(..., event_callback=None)` 与 `run_agent_with_harness_session(..., event_callback=None)` 已透传事件回调；回调异常会写入 harness 的 `event_callback_failed`，不会中断主任务。
- 修改代码+ProcessSummaryUX：`learning_agent/app/interactive.py` 在普通自然语言 prompt 路径里为每轮创建新的 `TerminalProcessSummaryRenderer`，因此 `/computer use --full` 后的下一句自然语言任务会显示主循环过程摘要。
- 边界：这是终端用户体验和可观测性增强，不代表 `/computer use --full` 任意应用、任意视觉任务已经完全成熟；成熟度仍必须看真实桌面 observe-plan-act-verification 结果。
## 2026-06-06 Terminal UI / Acceptance Event Boundary

- 真实用户终端不应显示 `::learning-agent-acceptance {...}` 原始 JSON 事件；机器验收事件默认写入 `LEARNING_AGENT_ACCEPTANCE_EVENT_LOG` 指向的 JSONL 文件。
- 如果将来外部工具确实需要从 stdout 读取机器标记，必须显式设置 `LEARNING_AGENT_ACCEPTANCE_EVENT_STDOUT=1`；不要把 stdout 机器标记作为普通用户 UI 的默认设计。
- Codex 风格过程显示应由 `learning_agent/app/process_summary.py` 输出 `Agent > ...` 摘要，内容包括当前意图、观察/动作、工具结果、下一步计划，但不展示隐藏推理和原始工具 payload。
- 长 Computer Use 循环应做节流：保留前几步和周期性进度，避免每个 `computer_observe/computer_action` 事件逐行刷屏。
## 2026-06-06 Computer Use Final Evidence And Bound Observation Context

- 修改代码+FinalEvidenceSummary：Computer Use full 的最终用户回答不能只相信模型自由文本；如果模型只说“已完成。”，收束层必须把本轮真实工具证据补进回答，避免真实执行已经发生但 controller/用户看不到关键证据。
- 修改代码+FinalEvidenceSummary：证据来源是当前 full GUI 任务的 `computer_use_action` 和 `computer_use_observe` 事件，包括目标应用、窗口、截图、`real_desktop_touched`、`low_level_event_count` 和成功动作数。
- 修改代码+BoundObservationTarget：通用 observe-plan-act loop 的 observation 层必须优先观察 session 绑定的 `target_window`，不能只靠 `target_hint=mspaint` 从桌面窗口列表里猜目标。
- 修改代码+BoundObservationTarget：当桌面上有多个 Paint 或用户旧窗口时，只靠 target_hint 会导致观察漂移；后续任何真实验收都应检查 action/observe/report 里的 target_window 是否一致。
- 当前边界：最终证据摘要和绑定窗口观察是必要基础设施修复，不等于任意绘图语义已经成熟；水果拼盘、上色、复杂纠偏仍需要继续查视觉 planner 和 verifier。
## 2026-06-06 Phase121 Computer Use app catalog

- 本轮目标：把 `/computer use --full` 的应用发现从“只靠模型猜 app 名”推进到“枚举本机候选 + 清洗候选 + 以非硬白名单提示交给模型选择”。
- 已新增 `learning_agent/computer_use/app_names.py`：负责清洗应用显示名、过滤高风险系统工具/后台 helper/提示注入名称、读取开始菜单和 App Paths 注册表候选，并格式化为模型可见的候选清单。
- 已接入 `learning_agent/core/agent.py` 的 full 模式 harness：模型主循环现在会看到 `Available desktop application candidates (cleaned model hints, not a hard whitelist)`，并能看到如 `Paint [app_name=mspaint]` 这样的稳定启动名。
- 设计边界：候选清单是模型提示和启动名规范化层，不是用户限制白名单；用户仍应能通过通用 Computer Use loop 操作普通应用。
- 仍未完成：`learning_agent/computer_use/generic_app_discovery.py` 的 Phase108 内部 query discovery 还没有整体迁移到新 app catalog 模块，后续要继续减少重复发现逻辑。
- 追加事实：真实终端日志暴露出 Command Prompt、中文卸载器、Run、Event Viewer、Performance Monitor、Task Scheduler、Visual Studio Installer、Git Bash 等噪声后，已把这些纳入过滤规则；最终真实验收 run `computer_use_full_model_loop_paint_tree_real-20260606_234618` 的候选清单不再包含这些已锁定噪声。
## 2026-06-07 Computer Use Windows App Inventory ClaudeCode Alignment Context

- 修改代码+WindowsAppInventory：本轮按 ClaudeCode `appNames.ts` / `mcpServer.ts` 的设计原则，把 OpenHarness 的应用发现收敛为“枚举 + 清洗 + 给模型选择”的模型提示层，而不是硬白名单或猜 app 名。
- 修改代码+WindowsAppInventory：新增 `learning_agent/computer_use/windows_app_inventory.py` 作为唯一应用清单融合层，当前真实来源包含 Windows 开始菜单、App Paths 注册表、Uninstall 注册表辅助源，并保留 `appx_package` 结构字段供下一步 AppX/AUMID 接入。
- 修改代码+WindowsAppInventory：旧兼容包装层已从正式源码删除，应用发现、清洗、格式化和中文应用提示解析统一收敛到 `learning_agent/computer_use/windows_app_inventory.py`，避免后续继续误接旧逻辑。
- 修改代码+WindowsAppInventory：`learning_agent/computer_use/generic_app_discovery.py` 已改为复用统一 inventory，Phase108 应用发现和 full 主循环模型提示现在共享同一套候选清洗规则。
- 修改代码+WindowsAppInventory：模型可见文本明确包含 `not a hard whitelist`、`app_name`、`launch_kind`、`source`，目的是像 ClaudeCode 一样把清单作为模型选择提示，而不是限制用户只能打开清单里的应用。
- 修改代码+WindowsAppInventory：真实可见终端 run 暴露出 `Task Manager`、`README`、`Module Docs`、`Tools for UWP Apps`、`Windows Software Development Kit` 仍混入模型提示；已按证据补入过滤回归，避免系统管理/文档/SDK 元入口再次污染普通应用清单。
- 修改代码+WindowsAppInventory：当前边界是“真实 AppX/AUMID 枚举与 launch resolver 尚未完整落地”，所以 Windows 设置页里的 UWP 应用数量仍可能比 inventory 真实可启动候选更多；下一步应补 AppX 枚举和启动执行链路。

## 2026-06-07 Computer Use App Inventory Slimming Context

- 修改代码+WindowsAppInventorySlimming：正式源码中不再保留旧应用清单兼容文件，`learning_agent/computer_use/app_names.py` 和旧 Phase121 测试文件已删除。
- 修改代码+WindowsAppInventorySlimming：`learning_agent/core/agent.py` 的 full 主循环和应用名规范化入口都直接导入 `windows_app_inventory.py`，模型主循环仍能看到 cleaned candidates、`not a hard whitelist`、`app_name=mspaint`。
- 修改代码+WindowsAppInventorySlimming：`learning_agent/tests/test_windows_computer_use_windows_app_inventory_phase122.py` 现在同时覆盖统一 inventory、中文别名解析、full harness 接入，旧测试职责已迁移。
- 修改代码+WindowsAppInventorySlimming：正式路径扫描已确认没有旧兼容 API 名或旧模块导入残留；旧历史只保存在 `learning_agent/test/computer_use_remove_app_names_20260607_065306/` 作为学习快照。
- 修改代码+WindowsAppInventorySlimming：该瘦身只解决“应用清单入口分叉/误接风险”，不等于 AppX/AUMID 全量发现、launch resolver 多后端或复杂 Computer Use 纠偏能力已经完成。
# 2026-06-07 Phase124 Windows Launch Resolver 多后端上下文
- 本轮新增 `learning_agent/computer_use/windows_launch_resolver.py`，把 Windows 应用启动身份从“猜 exe”升级为 resolver 计划：`exe/start_process_exe -> argv_no_shell`，`appx/aumid -> appx_aumid`，`shortcut -> start_menu_shortcut`，`uninstall_record -> not_launchable_inventory_record`。
- Phase124 不是硬白名单；它消费 Phase122 `windows_app_inventory` 的 `display_name/app_name/launch_kind/launch_id/source/aliases`，然后生成可审计 `launch_plan`。
- `learning_agent/computer_use/generic_app_discovery.py` 已接入 `resolve_windows_launch_plan()`，Phase108 报告现在保留 `best_candidate_launch_id`、`best_candidate_launch_kind`、`best_candidate_app_name`、`resolver_launch_backend` 和 `safe_resolver_launch_plan`。
- 旧 bug 根因已确认：Phase108 之前把 `launch_id` 交给 `_phase108_executable_name()`，导致 AppX/AUMID、shortcut、uninstall_record 都被压扁成 `*.exe` 风格计划。
- `learning_agent/computer_use/generic_launch_backend.py` 仍只有 Phase110 `argv_no_shell`/`subprocess.Popen(shell=False)` 真实后端；本轮已收紧 request 构造，非 argv 后端不会再把 `canonical_target` 回填到 `executable`。
- AppX/shortcut 目前是“解析和计划层已正确表达、Phase110 Popen 后端明确不支持”，后续如果要真实打开 AppX/shortcut，需要新增专门 backend，并解决 PID/窗口归属验证问题。
- 本轮学习备份位于 `learning_agent/test/windows_launch_resolver_phase124_20260607`。
## 2026-06-07 Phase128 当前 Computer Use maturity 口径

- 当前 `/computer maturity` 已改为“分层通用本机软件验收矩阵”，不是 Paint 专用成熟度矩阵。
- 通用基础字段：`generic_local_software_scope=true`、`generic_app_control_foundation_ready=true`、`model_loop_computer_use_foundation_ready=true`、`real_desktop_evidence_gate_defined=true`。
- 视觉缺口字段：`visual_quality_verifier_available=false`、`visual_quality_acceptance_passed=false`、`visual_correction_acceptance_available=false`、`visual_correction_acceptance_passed=false`。
- 解释边界：`visual_planner_mature=false` 只表示视觉质量与纠偏验收未成熟，不表示模型主循环语义规划、工具 schema、截图回流或通用软件控制基础失败。
- Paint 定位：`paint_is_representative_visual_sample_only=true`，Paint 只是视觉压力样本，不是 `/computer use --full` 架构中心。

## 2026-06-07 Phase129 Computer Use 源码架构审计矩阵上下文

- 修改代码+SourceArchitectureMaturity：`/computer maturity` 现在会枚举 `learning_agent/computer_use/*.py` 全量源码文件，当前审计到 `computer_use_source_file_count=86`。
- 修改代码+SourceArchitectureMaturity：新增 `computer_use_source_architecture_audit` 子报告，逐文件记录 `file`、`layer`、`parseable`、`error`，并统计 `layer_counts`。
- 修改代码+SourceArchitectureMaturity：当前源码审计显示 `computer_use_source_inventory_available=true`、`computer_use_all_source_files_parseable=true`、`computer_use_source_audit_coverage=true`、`computer_use_architecture_layers_mapped=true`、`computer_use_required_design_layers_present=true`、`computer_use_main_chain_files_present=true`。
- 修改代码+SourceArchitectureMaturity：历史特例污染扫描已显式暴露为 `computer_use_legacy_pollution_scan_available=true`，旧 Paint/Pikachu/Notepad/representative 文件只作为代表样例风险清单展示，不能再冒充通用成熟能力。
- 修改代码+SourceArchitectureMaturity：边界字段固定为 `computer_use_source_audit_static_only=true` 和 `computer_use_source_audit_semantic_complete=false`；它证明源码清单、语法、分层和主链路存在，不证明每个函数业务语义 100% 正确。
- 修改代码+SourceArchitectureMaturity：学习备份已保存到 `learning_agent/test/source_architecture_maturity_phase129_20260607/`。
## 2026-06-07 Phase130 Runtime Reachability Maturity
- 本轮基于源码事实把 `computer_use` 全量源码审计从“文件存在/可解析/分层”升级到“运行时可达性分类”。
- 当前模型主循环的应用发现主入口是 `learning_agent/core/agent.py::_computer_discover -> windows_app_inventory.query_windows_app_inventory`，不是 `generic_app_discovery.py`。
- `generic_app_discovery.py` 当前仍被交互命令、兼容合同和旧 bridge 引用，角色应归为 `compatibility_wrapper`，不能直接当主入口，也不能在未迁移引用前直接删除。
- 本轮矩阵分类显示：active runtime core、active tool surface、compatibility wrapper、representative sample、maturity acceptance、supporting infrastructure、legacy_or_deprecated_risk、unknown_reachability；当前 unknown 和 delete-now 风险均应为 0。

## 2026-06-07 Phase131 Computer Use Runtime Trace Context
- 新增代码+RuntimeTrace：本轮把 Computer Use 主循环运行时证据接入 `learning_agent/core/agent.py`，覆盖模型请求、工具调用、工具结果，以及 `computer_status`、`computer_observe`、`computer_discover`、`computer_action`、兼容 `computer_use` 分发摘要；如果没有这条上下文，后续会继续只能靠静态源码或旧日志猜测模型是否真的看见/调用工具。
- 新增代码+RuntimeTrace：`_computer_use_runtime_trace_report()` 现在能报告 `model_request_seen`、`tool_schema_visible`、`tool_call_seen`、`tool_result_seen`、`screenshot_returned_to_model`、`tools_called` 等字段；如果没有这条上下文，后续排查“模型有没有看到工具/截图/结果”会再次陷入截图和终端文本误判。
- 修改代码+RuntimeTrace：`/computer maturity` 新增 `computer_use_runtime_trace_available=true`、`computer_use_runtime_trace_model_loop_hooks_present=true`、`computer_use_runtime_trace_tool_call_hooks_present=true`、`computer_use_runtime_trace_tool_result_hooks_present=true`、`computer_use_runtime_trace_tool_internal_hooks_present=true`、`computer_use_runtime_trace_regression_test_present=true`；如果没有这条上下文，真实终端看不到本轮 trace 接线状态。
- 修改代码+RuntimeTrace：runtime trace 子报告明确输出 `computer_use_runtime_trace_static_only=true` 和 `computer_use_runtime_trace_semantic_complete=false`；如果没有这条边界，后续可能把“trace 接通”误说成“任意真实桌面任务已成熟”。
- 当前源码事实：runtime reachability report 显示 `active_runtime_core=38`、`active_tool_surface=9`、`compatibility_wrapper=13`、`representative_sample=8`、`maturity_acceptance=11`、`legacy_or_deprecated_risk=0`、`unknown_reachability=0`；如果没有这条事实，后续容易把 86 个文件继续误判成全是核心或全是废弃。
- 当前瘦身事实：`generic_app_discovery.py` 不是模型主循环 `computer_discover` 主入口，主入口是 `windows_app_inventory.py`，但 `generic_app_discovery.py` 仍属于被引用的 compatibility_wrapper，当前不能直接删除；如果没有这条事实，后续可能误删仍被旧命令/兼容合同使用的文件。

## 2026-06-07 Phase131 Final Visible Acceptance Update
- 修改代码+TerminalFocusGuard：真实可见终端验收第一次失败在 `learning_agent/acceptance_controller/controller.ps1` 的窗口聚焦逻辑，原因是 Windows Terminal/cmd 启动早期 `MainWindowHandle` 可能刷新为空，空句柄传给 `ShowWindowAsync` 会导致 controller 自身崩溃；如果没有这条记录，后续可能误判为 agent 功能失败。
- 修改代码+TerminalFocusGuard：已在 `Activate-AgentWindow` 调用 Win32 API 前刷新进程对象、二次检查空句柄、等待并重新枚举目标窗口；如果没有这条记录，真实可见终端验收仍可能因为窗口句柄竞态中断。
- 真实可见终端验收已通过：controller 启动 `learning_agent/start_oauth_agent.bat` 并运行场景 `agent_capability_phase127_model_loop_maturity_matrix`，结果目录为 `learning_agent/acceptance_controller/runs/agent_capability_phase127_model_loop_maturity_matrix-20260607_112908/`。
- 独立 verifier 已通过同一 run，确认 payload 中包含 runtime trace、runtime reachability、source audit、model-loop architecture 等 token；如果没有这条记录，最终判断会只依赖 controller 即时输出。

## 2026-06-07 Phase132 Computer Use compatibility slimming context

- 本轮按用户要求继续“源码瘦身”，目标不是新增绘图特例，而是减少旧接口、旧文件名、旧 Phase 入口造成的误接和记忆污染。
- 已从正式源码删除第一批旧兼容文件：`learning_agent/computer_use/app_window_control.py`、`controlled_app_launch.py`、`generic_app_discovery.py`、`generic_real_launch_candidate.py`、`interactive_launch_target.py`。
- 旧能力已收敛到统一入口：应用/窗口控制与 launch resolver 相关逻辑进入 `learning_agent/computer_use/windows_launch_resolver.py`；真实/录制 launch backend 与 Phase109 兼容薄类进入 `learning_agent/computer_use/generic_launch_backend.py`。
- 当前生产旧模块引用扫描已确认：排除 `learning_agent/test`、`learning_agent/tests`、`learning_agent/runs` 后，没有生产代码继续导入上述旧模块路径。
- 边界：这次瘦身解决的是“旧接口误接风险”，不等于 `/computer use --full` 任意软件、任意视觉任务、任意纠偏能力已经完全成熟；真实成熟度仍要看模型主循环、截图回流、工具调用、窗口归属和真实终端验收。

## 2026-06-07 Phase133 Computer Use core preloop slimming context

- 本轮继续第二批瘦身，目标是清掉 `LearningAgent` 主类里已经不被 `run()` 调用、但名字会误导后续开发者的旧模型前桌面任务 runtime 入口。
- 已从 `learning_agent/core/agent.py` 删除 `_desktop_task_runtime_answer_from_prompt()` 和只服务它的 `_format_desktop_task_runtime_answer()`，避免自然语言桌面任务再次绕过模型主循环直接进入旧 Python runtime。
- 保留 `_desktop_task_runtime_for_current_run()`，因为当前测试和 maturity 矩阵仍用它验证默认 runtime 是否接到 `UniversalDesktopExecutionLoopAdapter`、受控物理 sender 和真实 Windows SendInput 后端；它不是本轮的死入口。
- `/computer maturity` 已新增并输出 `preloop_desktop_runtime_method_removed=true`，该字段来自 `core/agent.py` AST 源码自检，要求旧预执行函数本体不存在，而不只是 `run()` 没调用它。
- 学习快照已保存到 `learning_agent/test/computer_use_core_preloop_slimming_phase133_20260607/`。
## 2026-06-07 Phase134 Computer Use runtime factory slimming context

- 修改代码+RuntimeFactorySlimming：`LearningAgent` 主类不再保留 `_desktop_task_runtime_for_current_run()`；默认桌面 runtime 工厂职责迁移到 `learning_agent/computer_use/desktop_task_runtime.py` 的 `build_default_desktop_task_runtime(workspace)`。
- 修改代码+RuntimeFactorySlimming：新的模块级工厂仍然构造 `ComputerUseDesktopTaskRuntime`、`WindowsSendInputLowLevelSender`、`WindowsControlledPhysicalSendInputSender` 和 `UniversalDesktopExecutionLoopAdapter(enable_real_target_launch=True)`，所以瘦身不是断开 full 路线，而是把职责放回 Computer Use runtime 层。
- 修改代码+RuntimeFactorySlimming：`/computer maturity` 和真实终端验收场景现在必须看到 `agent_runtime_factory_method_removed=true` 与 `default_desktop_task_runtime_factory_available=true`，才能说明主类旧工厂已删且新模块级工厂存在。
- 边界：本轮仍不是视觉质量成熟验收；`passed=false` 如果由视觉质量/纠偏门禁导致，是预期结果，不能倒推模型主循环基础未接通。
## 2026-06-07 Phase135 Computer Use ordinary app inventory no-50-limit context

- 修改代码+FullOrdinaryAppInventory：`learning_agent/computer_use/windows_app_inventory.py` 已移除旧的 50 个候选默认上限，`build_windows_app_inventory()` 和 `query_windows_app_inventory()` 省略 `max_count` 时返回全量清洗后的普通应用候选；如果没有这条上下文，后续开发者可能再次把“候选显示数量”误接成“普通应用可发现范围”。
- 修改代码+FullOrdinaryAppInventory：`learning_agent/core/agent.py::_computer_discover()` 不再把省略 `max_results` 默认成 10，只在模型显式传 `max_results` 或兼容 `limit` 时才限制显示数量；如果没有这条上下文，主循环 discover 入口可能又隐藏第 11 个以后的普通应用。
- 修改代码+FullOrdinaryAppInventory：`learning_agent/tools/schemas.py` 的 `computer_discover`、`computer_use operation=discover`、`computer-use operation=discover` schema 都已改为“省略表示返回所有清洗后的普通应用候选”，不再宣称默认 10 或固定 50 上限；如果没有这条上下文，模型可见工具说明可能与真实实现重新分裂。
- 修改代码+FullOrdinaryAppInventory：`/computer maturity` 已新增并输出 `legacy_fifty_app_inventory_limit_removed=true`、`ordinary_launchable_apps_unlimited_inventory=true`、`computer_discover_default_unlimited_ordinary_apps=true`、`computer_discover_schema_default_unlimited_ordinary_apps=true`；如果没有这条上下文，用户无法从终端直接确认普通应用全量放开状态。
- 边界：高风险工具过滤仍保留，普通应用全量放开不等于 cmd、powershell、regedit、settings、taskmgr 等高风险系统工具也会被默认暴露；如果没有这条边界，后续可能把“普通应用不限制”误解成“所有可执行入口都无差别放开”。

## 2026-06-07 Phase136 WeChat full-mode launch context
- `/computer use --full` 当前普通应用启动链路已确认经过模型主循环：用户自然语言 -> full harness/tool schema -> `computer_action launch_app` -> `computer_observe` 截图/窗口状态 -> 最终回答。
- 微信失败的当前事实修复点：`windows_app_inventory.py` 负责候选与别名，`desktop_task_router.py` 负责中文任意应用名路由，`core/agent.py` 负责模型主循环 full harness 约束，`universal_target_session.py` 负责 agent-owned/proxy window 绑定。
- 真实验收 run：`learning_agent/acceptance_controller/runs/computer_use_full_open_wechat_probe-20260607_155536/`，结果确认真实启动微信且可见窗口已验证。

## 2026-06-07 Phase137 context
