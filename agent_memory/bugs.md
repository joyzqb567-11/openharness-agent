...
## 2026-06-27 Manual API Key Launch OAuth Link Fix

- Closed risk: API key/default 启动模式下，OpenAI browser/headless OAuth 不再默认显示为可用 mock flow。
- Fix: `build_openai_auth_config()` 默认从 `mock` 改为 `api_key_only`；`mock_enabled` 只在显式 mock 模式下为 true；provider catalog 根据真实 OAuth/mock/API key only 三种状态返回 `available`、`mock_available` 或 `oauth_config_required`；auth-attempt start 在无配置时直接返回 `openai_oauth_not_configured`。
- Verification: 新增 `test_default_config_uses_api_key_only_and_blocks_oauth_attempts`、`test_openai_oauth_methods_require_explicit_real_or_mock_configuration`、`test_auth_attempt_start_rejects_unconfigured_openai_oauth`，均先按旧实现失败，再在修复后通过。
- Verification: 相关后端 pytest 17 passed、Python py_compile 通过、桌面前端 Vitest 19 files / 82 tests passed。
- Remaining note: 若目标是打开 OpenAI 官方 OAuth 官网，仍必须显式以真实 OAuth 模式启动，并满足 `OPENHARNESS_OPENAI_AUTH_MODE=real_browser`、`OPENHARNESS_OPENAI_EXPERIMENTAL=1`、`OPENHARNESS_PROVIDER_SECRET_STORE=os_encrypted`、`OPENHARNESS_OPENAI_CLIENT_ID`；默认 API key only 模式会提示使用 API key 或补齐配置，不再给假链接。
