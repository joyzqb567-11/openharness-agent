...
## 2026-06-27 Desktop GUI Toolchain Control Center Task 1

- 已在隔离 worktree `.worktrees/gui-toolchain-control-center` 的 `codex/gui-toolchain-control-center` 分支执行蓝图 Task 1：新增后端工具链清单 contract，并把右侧检查器扩展出“链路”页签。
- 后端新增 `learning_agent/app/gui_toolchain.py`，复用 `learning_agent.tools.catalog.build_builtin_tool_catalog()` 生成工具分组；`learning_agent/app/gui_bridge.py` 新增 `GET /v2/gui/toolchain`，使用既有 GUI token 门禁。
- 前端新增 `ToolchainPanel.tsx`，并通过 `guiClient.toolchain()`、`AppShell` 首屏加载和 `StatusInspector` 页签把工具链清单展示到真实 Desktop GUI。
