...
class CodexCliChatModel:  # 作用: 通过本机 codex exec 调用 Codex/GPT-5.5 的旧桥接模型
    ...
    def chat(self, messages: list[dict[str, Any]], tools: list[dict[str, Any]]) -> ModelMessage:  # 作用: 把 agent messages/tools 包装成 prompt 交给 codex exec
        ...
        with tempfile.TemporaryDirectory(prefix="learning_agent_codex_") as raw_dir:  # 作用: 创建临时目录保存 schema 和输出文件
            ...
            return self._parse_model_message(raw_output)  # 作用: 把 JSON 文本解析为内部 ModelMessage
...
class CodexOAuthChatModel:  # 作用: 参考 opencode2 的 OpenAI OAuth/API 链路实现 GPT-5.5 模型适配器
    ...
    def from_env(cls) -> "CodexOAuthChatModel":  # 作用: 读取 CODEX_MODEL 和 OAuth 超时配置
        ...
        try:  # 作用: 捕获非法数字
            ...
            timeout_seconds = 300  # 作用: 回退到 5 分钟
        return cls(model=model, oauth_timeout_seconds=timeout_seconds)  # 作用: 返回配置好的 OAuth 模型
    ...
    def chat(self, messages: list[dict[str, Any]], tools: list[dict[str, Any]]) -> ModelMessage:  # 作用: 使用 OAuth token 请求 Codex Responses API
        try:  # 作用: 捕获登录、刷新、请求和解析异常
            ...
            return CodexCliChatModel._parse_model_message(raw_output)  # 作用: 复用结构化 JSON 解析器
    ...
    def _build_responses_input(self, messages: list[dict[str, Any]], tools: list[dict[str, Any]]) -> list[dict[str, object]]:  # 作用: 构造 Responses API input 列表
        return [  # 作用: 返回一个最小消息数组
            {  # 作用: user 消息对象开始
                "role": "user",  # 作用: 使用 user role 承载 agent 上下文
                "content": [  # 作用: content 必须是内容片段数组
                    {"type": "input_text", "text": self._build_prompt(messages=messages, tools=tools)}  # 作用: 把 prompt 放进 input_text
                ],  # 作用: content 数组结束
            }  # 作用: user 消息对象结束
        ]  # 作用: input 列表结束
