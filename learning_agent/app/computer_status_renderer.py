...
def _tool_rendering_joined_apps(values: Any) -> str:  # 新增代码+Phase7ToolRenderingStatus：函数段开始，把 allowed_apps 渲染成短文本；如果没有这段函数，空列表和坏列表会在工具渲染区显示不稳定。
    apps = [_short_text(value, 80) for value in _as_list(values) if str(value or "").strip()]  # 新增代码+Phase7ToolRenderingStatus：清洗并截断 app 名；如果没有这一行，空值或超长名称会污染终端布局。
    return ",".join(apps) if apps else "(none)"  # 新增代码+Phase7ToolRenderingStatus：有值时逗号连接，无值时返回明确空态；如果没有这一行，用户分不清无授权还是漏字段。
# 新增代码+Phase7ToolRenderingStatus：函数段结束，_tool_rendering_joined_apps 到此结束；如果没有这个边界说明，初学者不容易看出 allowed_apps 渲染范围。


def _tool_rendering_screenshot_text(dims: Any) -> str:  # 新增代码+Phase7ToolRenderingStatus：函数段开始，把截图尺寸渲染成宽高文本；如果没有这段函数，空截图尺寸会显示成难读的 `x`。
    dims_dict = _as_dict(dims)  # 新增代码+Phase7ToolRenderingStatus：安全读取尺寸字典；如果没有这一行，坏类型会让渲染器崩溃。
    width = _safe_int(dims_dict.get("width", 0), 0)  # 新增代码+Phase7ToolRenderingStatus：读取并兜底宽度；如果没有这一行，空宽度会显示不稳定。
    height = _safe_int(dims_dict.get("height", 0), 0)  # 新增代码+Phase7ToolRenderingStatus：读取并兜底高度；如果没有这一行，空高度会显示不稳定。
    return f"{width}x{height}"  # 新增代码+Phase7ToolRenderingStatus：返回稳定宽高文本；如果没有这一行，工具渲染区无法展示截图上下文。
# 新增代码+Phase7ToolRenderingStatus：函数段结束，_tool_rendering_screenshot_text 到此结束；如果没有这个边界说明，初学者不容易看出截图尺寸渲染范围。


def _tool_rendering_last_action_verified(last_action: dict[str, Any]) -> bool:  # 新增代码+Phase7ToolRenderingStatus：函数段开始，判断最近动作是否已验证；如果没有这段函数，不同字段名会导致 verified 显示漂移。
    return bool(last_action.get("verified") or last_action.get("verification_passed") or last_action.get("verified_after_observe"))  # 新增代码+Phase7ToolRenderingStatus：兼容常见验证字段；如果没有这一行，已验证动作可能被误显示为 false。
# 新增代码+Phase7ToolRenderingStatus：函数段结束，_tool_rendering_last_action_verified 到此结束；如果没有这个边界说明，初学者不容易看出验证字段兼容范围。


def _computer_use_tool_rendering_lines(lock: dict[str, Any], current_context: dict[str, Any]) -> list[str]:  # 新增代码+Phase7ToolRenderingStatus：函数段开始，生成 Computer Use Tool Rendering 专区；如果没有这段函数，关键状态仍会分散在多个旧区块里。
    allowed_apps = _as_list(current_context.get("allowed_apps", []))  # 新增代码+Phase7ToolRenderingStatus：读取允许应用列表；如果没有这一行，request_access 状态无法显示授权范围。
    display = _as_dict(current_context.get("selected_display", {}))  # 新增代码+Phase7ToolRenderingStatus：读取选中显示器；如果没有这一行，多屏上下文无法显示。
    last_action = _as_dict(current_context.get("last_action", {}))  # 新增代码+Phase7ToolRenderingStatus：读取最近动作；如果没有这一行，动作验证状态无法显示。
    return [  # 新增代码+Phase7ToolRenderingStatus：返回专区行列表；如果没有这一行，调用方无法一次性追加稳定区块。
        "Computer Use Tool Rendering",  # 新增代码+Phase7ToolRenderingStatus：输出专门区块标题；如果没有这一行，顶层矩阵无法确认 ClaudeCode 风格工具反馈区存在。
        f"- request_access=allowed_apps:{len(allowed_apps)} flags:{_flag_text(current_context.get('grant_flags', {}))} expired:{_bool_token(current_context.get('grant_expired'))} revoked:{_bool_token(current_context.get('grant_revoked'))}",  # 新增代码+Phase7ToolRenderingStatus：输出 request_access 授权摘要；如果没有这一行，用户看不到授权数量、权限位、过期和撤销状态。
        f"- allowed_apps={_tool_rendering_joined_apps(allowed_apps)}",  # 新增代码+Phase7ToolRenderingStatus：输出允许应用清单；如果没有这一行，模型和用户不知道当前可控 app。
        f"- display={_short_text(display.get('display_id', ''), 80)} pinned:{_bool_token(current_context.get('display_pinned'))}",  # 新增代码+Phase7ToolRenderingStatus：输出显示器和 pin 状态；如果没有这一行，多屏坐标上下文仍不透明。
        f"- screenshot={_tool_rendering_screenshot_text(current_context.get('last_screenshot_dims', {}))}",  # 新增代码+Phase7ToolRenderingStatus：输出最后截图尺寸；如果没有这一行，坐标换算缺少可读证据。
        f"- active_lock=locked:{_bool_token(lock.get('locked'))} stale:{_bool_token(lock.get('stale'))} abort:{_bool_token(lock.get('abort_requested'))} owner={_short_text(lock.get('owner_session_id', ''), 80)}",  # 新增代码+Phase7ToolRenderingStatus：输出活动锁和急停状态；如果没有这一行，用户看不清桌面控制权是否安全。
        f"- cleanup=completed:{_bool_token(current_context.get('cleanup_completed'))} hidden_windows:{len(_as_list(current_context.get('hidden_windows', [])))} host_hidden:{_bool_token(current_context.get('host_windows_hidden'))}",  # 新增代码+Phase7ToolRenderingStatus：输出 cleanup 与隐藏窗口状态；如果没有这一行，任务结束状态是否归零不透明。
        f"- last_action={_short_text(last_action.get('action', ''), 80)} verified:{_bool_token(_tool_rendering_last_action_verified(last_action))} decision:{_short_text(last_action.get('decision', ''), 100)}",  # 新增代码+Phase7ToolRenderingStatus：输出最近动作和验证结果；如果没有这一行，未验证动作可能被误当成功。
    ]  # 新增代码+Phase7ToolRenderingStatus：结束专区行列表；如果没有这一行，Python 无法正确关闭结构。
# 新增代码+Phase7ToolRenderingStatus：函数段结束，_computer_use_tool_rendering_lines 到此结束；如果没有这个边界说明，初学者不容易看出专用渲染区范围。


def render_computer_status(snapshot: dict[str, Any]) -> str:  # 修改代码+Phase98UniversalComputerUseMode: 函数段开始，渲染 `/computer status` 并加入 Phase98 模式摘要；如果没有这段函数，终端状态会看不到 mode/full/ttl 权限边界。
    ...
    mode_session = _as_dict(snapshot.get("mode_session", {}))  # 新增代码+Phase98UniversalComputerUseMode: 读取 Phase98 通用模式 session；如果没有这行代码，status 页面无法显示 `/computer use` 当前模式。
    ...
    lines.extend(_computer_use_tool_rendering_lines(lock, current_context))  # 新增代码+Phase7ToolRenderingStatus：追加专门工具渲染状态区；如果没有这一行，request_access、display、screenshot、lock、cleanup 和 last action 会继续分散在旧区块。
    ...
    lines.append("Computer Use MCP")  # 新增代码+ComputerUseMCP: 输出独立 MCP server 状态区标题；如果没有这行代码，用户在 /computer status 里看不出推荐入口已经切到 ClaudeCode 风格 MCP。
    lines.append("- server=computer-use transport=stdio prefix=mcp__computer-use__ capability_pack=computer_use")  # 新增代码+ComputerUseMCP: 显示 server 名、传输方式、工具前缀和能力包；如果没有这行代码，排查时无法确认模型应看到哪类工具名。
    lines.append("- recommended_path=mcp__computer-use__request_access -> mcp__computer-use__open_application -> mcp__computer-use__observe/screenshot -> mcp__computer-use__left_click/type/key/scroll/computer_batch")  # 修改代码+ComputerUseMcpV2ResidualCleanup：显示 v2 原子工具推荐路径；如果没有这行代码，状态页仍会用模糊 action 字样让用户误以为旧 computer_action 还可见。
    lines.append("- shell_surface=blocked_for_computer_use_mcp bash=false powershell=false command=false script=false")  # 新增代码+ComputerUseMCP: 明确 Computer Use MCP 禁止 shell 表面；如果没有这行代码，状态页无法解释为什么桌面任务不该暴露 bash/powershell。
    ...
    lines.append("- risk=low command=/computer use : open normal universal Computer Use mode")  # 新增代码+Phase98UniversalComputerUseMode: 展示 normal mode 命令；如果没有这行代码，用户不知道如何打开通用模式。
    lines.append("- risk=low command=/computer use --observe : open observe-only Computer Use mode")  # 新增代码+Phase98UniversalComputerUseMode: 展示 observe mode 命令；如果没有这行代码，用户不知道如何只观察不动作。
    lines.append("- risk=high command=/computer use --full : request strong confirmation for full mode")  # 新增代码+Phase98UniversalComputerUseMode: 展示 full 请求命令；如果没有这行代码，用户不知道 full 需要二次确认。
    lines.append("- risk=medium command=/computer stop : stop mode session and request global abort")  # 新增代码+Phase98UniversalComputerUseMode: 展示 stop 命令；如果没有这行代码，用户不知道如何停止当前模式。
    lines.append("- risk=low command=/computer permissions : show current mode permission fields")  # 新增代码+Phase98UniversalComputerUseMode: 展示 permissions 命令；如果没有这行代码，用户不知道如何查看动作权限。
...
# 新增代码+PostParityStatus：函数段开始，_post_parity_ready_word 把布尔门禁翻译成用户能读懂的 ready/not ready；如果没有这段函数，状态渲染会直接暴露 True/False，代码小白不容易理解。
def _post_parity_ready_word(value: Any) -> str:  # 新增代码+PostParityStatus：声明 post-parity 状态词转换函数；如果没有这行代码，下面每个门禁都要重复写判断逻辑。
    return "ready" if value is True else "not ready"  # 新增代码+PostParityStatus：只有明确 True 才显示 ready；如果没有这行代码，None、缺字段或假值可能被误认为已准备好。
# 新增代码+PostParityStatus：函数段结束，_post_parity_ready_word 到此结束；如果没有这个边界说明，初学者不容易看出 ready/not ready 规则范围。

# 新增代码+PostParityStatus：函数段开始，render_post_parity_status 渲染 post-parity 成熟度门禁；如果没有这段函数，用户和未来 agent 看不到 Phase140-147 的整体成熟状态。
def render_post_parity_status(report: dict[str, Any]) -> str:  # 新增代码+PostParityStatus：声明 post-parity 状态渲染入口；如果没有这行代码，Phase146 测试和终端状态集成都没有可调用 API。
    lines = [  # 新增代码+PostParityStatus：创建稳定顺序的输出行列表；如果没有这行代码，状态文本顺序可能漂移，不利于验收和阅读。
        "Computer Use Post-Parity Status",  # 新增代码+PostParityStatus：输出固定标题；如果没有这行代码，用户不容易知道这段状态属于 post-parity 成熟度。
        f"overall: {_post_parity_ready_word(report.get('passed'))}",  # 新增代码+PostParityStatus：输出整体是否 ready；如果没有这行代码，用户只能自己综合判断全部门禁。
        f"parity floor: {_post_parity_ready_word(report.get('parity_floor_gate'))}",  # 新增代码+PostParityStatus：输出 ClaudeCode parity 底线是否仍然守住；如果没有这行代码，后续增强可能悄悄绕过旧底线。
        f"real-world workflows: {_post_parity_ready_word(report.get('real_world_workflow_matrix'))}",  # 新增代码+PostParityStatus：输出真实工作流矩阵是否 ready；如果没有这行代码，真实场景覆盖缺口不明显。
        f"evidence ledger: {_post_parity_ready_word(report.get('evidence_ledger_gate'))}",  # 新增代码+PostParityStatus：输出证据账本是否 ready；如果没有这行代码，用户不知道能否信任 accepted/rejected 记录。
        f"failure triage: {_post_parity_ready_word(report.get('failure_triage_gate'))}",  # 新增代码+PostParityStatus：输出失败分诊是否 ready；如果没有这行代码，失败原因分类能力不可见。
        f"self-improvement dry run: {_post_parity_ready_word(report.get('self_improvement_dry_run_gate'))}",  # 新增代码+PostParityStatus：输出安全自我改进 dry-run 是否 ready；如果没有这行代码，用户不知道 agent 是否只产出安全提案。
        f"visible terminal final gate: {_post_parity_ready_word(report.get('visible_terminal_e2e_gate'))}",  # 新增代码+PostParityStatus：输出最终真实可见终端门禁是否 ready；如果没有这行代码，最终验收要求容易被遗漏。
    ]  # 新增代码+PostParityStatus：结束输出行列表；如果没有这行代码，Python 语法不完整。
    return "\n".join(lines) + "\n"  # 新增代码+PostParityStatus：返回带末尾换行的多行状态文本；如果没有这行代码，调用方拿不到可打印输出。
# 新增代码+PostParityStatus：函数段结束，render_post_parity_status 到此结束；如果没有这个边界说明，初学者不容易看出 post-parity 状态渲染范围。
