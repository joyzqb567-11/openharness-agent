...
try:  # 新增代码+AppSplit: 优先按包路径导入 app/core/models/mcp 依赖；若没有这行代码，包运行模式下 CLI 无法复用已拆分层。
    ...
    from learning_agent.app.gui_bridge import create_gui_bridge_server  # 新增代码+DesktopGUIBridgeCLI: 导入桌面 GUI bridge server 构造器；如果没有这行代码，CLI 无法启动 GUI 专用后端。
    ...
    if error.name not in {"learning_agent", "learning_agent.app", "learning_agent.app.doctor", "learning_agent.app.http_bridge", "learning_agent.app.gui_bridge", "learning_agent.app.interactive", "learning_agent.app.terminal_permissions", "learning_agent.core", "learning_agent.core.config", "learning_agent.models", "learning_agent.models.codex_cli", "learning_agent.models.codex_oauth", "learning_agent.models.openai_chat", "learning_agent.mcp", "learning_agent.mcp.config", "learning_agent.mcp.registry", "learning_agent.tools", "learning_agent.tools.catalog_runtime"}:  # 修改代码+DesktopGUIBridgeCLI: 允许 gui_bridge 路径差异进入 fallback；若没有这行代码，直接脚本模式可能找不到新 GUI bridge 模块。
    ...
    from app.gui_bridge import create_gui_bridge_server  # 新增代码+DesktopGUIBridgeCLI: 脚本模式导入桌面 GUI bridge server；如果没有这行代码，直接运行时 GUI bridge 会断开。
...
def _run_desktop_bridge_command(raw_args: list[str]) -> bool:  # 新增代码+DesktopGUIBridgeCLI: 函数段开始，处理无需模型的 desktop-bridge 命令；如果没有这段函数，GUI bridge 启动会被普通 agent 初始化拖慢。
    if not raw_args or raw_args[0] not in {"desktop-bridge", "gui-bridge"}:  # 新增代码+DesktopGUIBridgeCLI: 只拦截桌面 GUI bridge 命令；如果没有这行代码，普通 run/doctor 会被误处理。
        return False  # 新增代码+DesktopGUIBridgeCLI: 返回未处理，让 main 继续原有流程；如果没有这行代码，普通命令会提前退出。
    workspace = Path.cwd()  # 新增代码+DesktopGUIBridgeCLI: 默认使用当前目录作为桌面项目工作区；如果没有这行代码，GUI bridge 不知道服务哪个项目。
    host = "127.0.0.1"  # 新增代码+DesktopGUIBridgeCLI: 默认只绑定本机；如果没有这行代码，bridge 可能被局域网访问。
    port = 8776  # 新增代码+DesktopGUIBridgeCLI: 使用 GUI bridge 默认端口；如果没有这行代码，前端无法使用稳定默认地址。
    token = ""  # 新增代码+DesktopGUIBridgeCLI: 默认让 server 自动生成随机 token；如果没有这行代码，启动认证状态不明确。
    index = 1  # 新增代码+DesktopGUIBridgeCLI: 从命令名后开始解析参数；如果没有这行代码，解析循环会重复处理命令名。
    while index < len(raw_args):  # 新增代码+DesktopGUIBridgeCLI: 遍历 desktop-bridge 参数；如果没有这行代码，用户无法传 workspace/port。
        item = raw_args[index]  # 新增代码+DesktopGUIBridgeCLI: 读取当前参数名；如果没有这行代码，解析逻辑没有输入。
        value = raw_args[index + 1] if index + 1 < len(raw_args) else ""  # 新增代码+DesktopGUIBridgeCLI: 读取参数值；如果没有这行代码，缺值错误无法被发现。
        if item == "--workspace" and value:  # 新增代码+DesktopGUIBridgeCLI: 识别工作区参数；如果没有这行代码，用户无法指定项目根目录。
            workspace = Path(value).expanduser().resolve()  # 新增代码+DesktopGUIBridgeCLI: 规范化工作区路径；如果没有这行代码，相对路径可能跑偏。
            index += 2  # 新增代码+DesktopGUIBridgeCLI: 跳过参数和值；如果没有这行代码，循环会重复处理值。
            continue  # 新增代码+DesktopGUIBridgeCLI: 继续解析后续参数；如果没有这行代码，会落入未知参数错误。
        if item == "--host" and value:  # 新增代码+DesktopGUIBridgeCLI: 识别 host 参数；如果没有这行代码，测试和高级用户无法改监听地址。
            host = str(value)  # 新增代码+DesktopGUIBridgeCLI: 保存监听地址；如果没有这行代码，host 覆盖不会生效。
            index += 2  # 新增代码+DesktopGUIBridgeCLI: 跳过参数和值；如果没有这行代码，循环会重复处理值。
            continue  # 新增代码+DesktopGUIBridgeCLI: 继续解析后续参数；如果没有这行代码，会落入未知参数错误。
        if item == "--port" and value:  # 新增代码+DesktopGUIBridgeCLI: 识别端口参数；如果没有这行代码，用户无法指定端口。
            port = int(value)  # 新增代码+DesktopGUIBridgeCLI: 把端口转成整数；如果没有这行代码，server 无法绑定端口。
            index += 2  # 新增代码+DesktopGUIBridgeCLI: 跳过参数和值；如果没有这行代码，循环会重复处理值。
            continue  # 新增代码+DesktopGUIBridgeCLI: 继续解析后续参数；如果没有这行代码，会落入未知参数错误。
        if item == "--token" and value:  # 新增代码+DesktopGUIBridgeCLI: 识别显式 token 参数；如果没有这行代码，自动化测试无法固定 token。
            token = str(value)  # 新增代码+DesktopGUIBridgeCLI: 保存显式 token；如果没有这行代码，server 会改用随机 token。
            index += 2  # 新增代码+DesktopGUIBridgeCLI: 跳过参数和值；如果没有这行代码，循环会重复处理值。
            continue  # 新增代码+DesktopGUIBridgeCLI: 继续解析后续参数；如果没有这行代码，会落入未知参数错误。
        raise SystemExit(f"desktop-bridge 参数错误：未知或缺值参数 {item}")  # 新增代码+DesktopGUIBridgeCLI: 对坏参数直接给出清楚错误；如果没有这行代码，拼错参数会静默忽略。
    server = create_gui_bridge_server(workspace=workspace, host=host, port=port, token=token)  # 新增代码+DesktopGUIBridgeCLI: 创建 GUI bridge server；如果没有这行代码，CLI 没有可运行服务。
    bridge_host, bridge_port = server.server_address  # 新增代码+DesktopGUIBridgeCLI: 读取实际监听地址；如果没有这行代码，端口 0 无法打印真实端口。
    print(f"Desktop GUI bridge 已启动：http://{bridge_host}:{bridge_port}")  # 新增代码+DesktopGUIBridgeCLI: 打印 GUI bridge URL；如果没有这行代码，Electron 或用户不知道连接地址。
    print("接口：GET /v1/gui/bootstrap，GET /v1/gui/health")  # 新增代码+DesktopGUIBridgeCLI: 打印当前接口；如果没有这行代码，用户不知道可用端点。
    print("Token header：X-OpenHarness-Desktop-Token")  # 新增代码+DesktopGUIBridgeCLI: 打印 token header 名；如果没有这行代码，前端配置容易写错。
    print(f"Token：{server.token}")  # 新增代码+DesktopGUIBridgeCLI: 启动时一次性打印 token 供 Electron 接入；如果没有这行代码，随机 token 无法传给前端。
    try:  # 新增代码+DesktopGUIBridgeCLI: 捕获 Ctrl+C；如果没有这行代码，用户停止时会看到 traceback。
        server.serve_forever()  # 新增代码+DesktopGUIBridgeCLI: 开始处理 GUI bridge 请求；如果没有这行代码，命令会立即退出。
    except KeyboardInterrupt:  # 新增代码+DesktopGUIBridgeCLI: 处理用户中断；如果没有这行代码，Ctrl+C 不友好。
        print("\nDesktop GUI bridge 已停止。")  # 新增代码+DesktopGUIBridgeCLI: 打印停止提示；如果没有这行代码，用户不知道是否正常退出。
    finally:  # 新增代码+DesktopGUIBridgeCLI: 无论如何都释放 socket；如果没有这行代码，端口可能残留占用。
        server.server_close()  # 新增代码+DesktopGUIBridgeCLI: 关闭监听 socket；如果没有这行代码，后续启动可能端口占用。
    return True  # 新增代码+DesktopGUIBridgeCLI: 告诉 main 已处理 desktop-bridge；如果没有这行代码，main 会继续启动普通 agent。
# 新增代码+DesktopGUIBridgeCLI: 函数段结束，_run_desktop_bridge_command 到此结束；如果没有这个边界说明，用户不容易看出它只负责 GUI bridge CLI。
...
def main(agent_cls: type[Any] | None = None, permission_callback: Callable[..., bool] | None = None, argv: list[str] | None = None) -> None:  # 新增代码+AppSplit: CLI 主入口；若没有这行代码，app 包没有可调用的主命令入口。
    raw_args = sys.argv[1:] if argv is None else argv  # 新增代码+DesktopGUIBridgeCLI: 保存原始参数供 desktop-bridge 轻量解析；如果没有这行代码，GUI bridge 仍要走普通 argparse。
    if _run_desktop_bridge_command(raw_args):  # 新增代码+DesktopGUIBridgeCLI: 优先处理无需模型的 GUI bridge；如果没有这行代码，desktop-bridge 会错误初始化模型。
        return  # 新增代码+DesktopGUIBridgeCLI: GUI bridge 命令结束后退出；如果没有这行代码，停止服务后会进入普通交互模式。
    ...
    main_args = parse_main_args(raw_args)  # 新增代码+DesktopGUIBridgeCLI: 复用已经保存的原始参数；如果没有这行代码，普通命令和 GUI bridge 的参数来源会分裂。
...
if __name__ == "__main__":  # 新增代码+DesktopGUIBridgeCLI: 允许 `python -m learning_agent.app.cli` 直接运行；如果没有这行代码，计划中的 CLI 命令不会执行 main。
    main()  # 新增代码+DesktopGUIBridgeCLI: 调用 CLI 主入口；如果没有这行代码，模块运行会立即退出。
