...
try:  # 新增代码+AppSplit: 优先按包路径导入 app/core/models/mcp 依赖；若没有这行代码，包运行模式下 CLI 无法复用已拆分层。
    ...
    if error.name not in {"learning_agent", "learning_agent.app", "learning_agent.app.doctor", "learning_agent.app.http_bridge", "learning_agent.app.interactive", "learning_agent.core", "learning_agent.core.config", "learning_agent.models", "learning_agent.models.codex_cli", "learning_agent.models.codex_oauth", "learning_agent.models.openai_chat", "learning_agent.mcp", "learning_agent.mcp.config", "learning_agent.mcp.registry"}:  # 新增代码+AppSplit: 只允许目标路径缺失时 fallback；若没有这行代码，内部真实 bug 会被误吞。
...
def _resolve_agent_dependencies(agent_cls: type[Any] | None, permission_callback: Callable[..., bool] | None) -> tuple[type[Any], Callable[..., bool]]:  # 新增代码+AppSplit: 解析 CLI 所需的 agent 类和权限回调；若没有这行代码，app.cli 直接导入主文件会形成循环依赖。
    ...
    try:  # 修改代码+ToolSchemaSplit: 独立调用 app.cli.main 时从 core.agent 读取主类；若没有这行代码，CLI 仍会依赖旧兼容入口。
        from learning_agent.core.agent import LearningAgent, ask_permission_from_terminal_customer_mode  # 修改代码+ToolSchemaSplit: 导入核心 agent 和客户模式权限入口；若没有这行代码，直接调用 app.cli.main 没有可运行对象。
        ...
        from core.agent import LearningAgent, ask_permission_from_terminal_customer_mode  # 修改代码+ToolSchemaSplit: 脚本模式兜底导入核心 agent；若没有这行代码，app.cli 直接运行时没有 agent 类。
    return LearningAgent, ask_permission_from_terminal_customer_mode  # 修改代码+ToolSchemaSplit: 返回动态导入的核心依赖；若没有这行代码，CLI 无法继续创建 agent。
...
def main(agent_cls: type[Any] | None = None, permission_callback: Callable[..., bool] | None = None, argv: list[str] | None = None) -> None:  # 新增代码+AppSplit: CLI 主入口；若没有这行代码，app 包没有可调用的主命令入口。
    ...
    visible_tools = agent._tool_schema_names()  # 新增代码+AppSplit: 缓存当前模型可见工具名；若没有这行代码，run/bridge/交互模式会重复读取且展示不一致。
