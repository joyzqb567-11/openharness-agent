...
class DesktopGuiBridgeHandler(http.server.BaseHTTPRequestHandler):  # 新增代码+DesktopGUIBridge: 类段开始，处理 GUI bridge HTTP 请求；如果没有这个类，Electron 前端没有后端入口。
    ...
    def do_GET(self) -> None:  # 新增代码+DesktopGUIBridge: 处理 GET 请求；如果没有这段函数，GUI 无法读取 bootstrap 和事件。
        parsed_url = urllib.parse.urlparse(self.path)  # 新增代码+DesktopGUIBridge: 解析 URL；如果没有这行代码，带 query 的事件请求无法读取参数。
        path = parsed_url.path  # 新增代码+DesktopGUIBridge: 提取路径；如果没有这行代码，带 query 的请求无法路由。
        query = urllib.parse.parse_qs(parsed_url.query)  # 新增代码+DesktopGUIBridge: 解析 query 参数；如果没有这行代码，事件轮询无法读取 since_sequence 和 limit。
        ...
        if path in {"/v1/gui/bootstrap", "/gui/bootstrap"}:  # 新增代码+DesktopGUIBridge: 识别 bootstrap 端点；如果没有这行代码，前端首屏请求会 404。
            ...
            return  # 新增代码+DesktopGUIBridge: 响应完成后退出；如果没有这行代码，handler 可能继续发送第二个响应。
        if path in {"/v1/gui/events", "/gui/events"}:  # 新增代码+DesktopGUIBridge: 识别事件轮询端点；如果没有这行代码，前端无法读取工具和任务事件。
            if not self._guard_request(require_token=True):  # 新增代码+DesktopGUIBridge: 事件端点必须通过 token；如果没有这行代码，运行事件可能被未授权读取。
                return  # 新增代码+DesktopGUIBridge: 门禁失败后退出；如果没有这行代码，会继续返回事件。
            try:  # 新增代码+DesktopGUIBridge: 捕获 query 数字解析错误；如果没有这行代码，坏参数会变成 500。
                since_raw = query.get("since_sequence", [""])[0]  # 新增代码+DesktopGUIBridge: 读取事件游标；如果没有这行代码，前端无法增量读取。
                limit_raw = query.get("limit", ["50"])[0]  # 新增代码+DesktopGUIBridge: 读取事件数量限制；如果没有这行代码，默认限制不稳定。
                since_sequence = int(since_raw) if since_raw else None  # 新增代码+DesktopGUIBridge: 转换游标；如果没有这行代码，store 无法按序过滤。
                limit = int(limit_raw) if limit_raw else 50  # 新增代码+DesktopGUIBridge: 转换数量限制；如果没有这行代码，helper 会收到字符串。
            except ValueError as error:  # 新增代码+DesktopGUIBridge: 处理非法数字；如果没有这行代码，前端拿不到结构化错误。
                self._send_error(400, "bad_query", f"events query 参数必须是整数：{error}")  # 新增代码+DesktopGUIBridge: 返回 query 错误；如果没有这行代码，坏请求不可机器解析。
                return  # 新增代码+DesktopGUIBridge: 参数错误后退出；如果没有这行代码，会继续读取事件。
            self._send_json(200, build_gui_events_payload(self.server.workspace, since_sequence=since_sequence, limit=limit))  # 新增代码+DesktopGUIBridge: 返回事件 payload；如果没有这行代码，GUI watcher 无法工作。
            return  # 新增代码+DesktopGUIBridge: 响应完成后退出；如果没有这行代码，handler 可能继续发送第二个响应。
