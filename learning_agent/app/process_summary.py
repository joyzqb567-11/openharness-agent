...
class TerminalProcessSummaryRenderer:  # 新增代码+ProcessSummaryUX: 函数段开始，定义把 AgentEvent 翻译成终端摘要的渲染器；若没有这段代码，用户只能看到最终答案，看不到 agent 正在观察、规划或调用工具的过程。本段会和 session_runtime 的 event_callback 配合，段落到类定义结束为止。
    def __init__(self, stream: TextIO | None = None, prefix: str = "Agent > ") -> None:  # 新增代码+ProcessSummaryUX: 初始化渲染器并允许测试注入 StringIO；若没有这行代码，测试无法稳定捕获真实终端文案。
        ...
        self._last_visual_observation_summary = ""  # 新增代码+VisibleObservationSummary: 记录上一次已经显示的视觉观察摘要；若没有这行代码，模型重复返回同一句观察时会再次刷屏。
    ...
    def handle_event(self, event: Any) -> None:  # 新增代码+ProcessSummaryUX: 函数段开始，接收主循环事件并选择安全摘要文案；若没有这段代码，interactive.py 无法把 run_events 转成 Codex 风格过程提示。本段只显示摘要，不显示隐藏推理和原始工具输出，段落到函数 return 路径结束。
        ...
        if event_type == "model_message_completed":  # 新增代码+ProcessSummaryUX: 识别模型消息完成事件；若没有这行代码，用户不知道模型是否已经产生下一步决定。
            ...
            visual_observation_summary = self._visual_observation_summary_from_model_payload(payload)  # 新增代码+VisibleObservationSummary: 从模型完成事件里提取可显示的截图观察摘要；若没有这行代码，模型看到了什么仍然不会出现在真实终端。
            if visual_observation_summary:  # 新增代码+VisibleObservationSummary: 只有确实存在安全视觉摘要时才显示；若没有这行代码，空摘要会产生无意义空行。
                self._print(visual_observation_summary)  # 新增代码+VisibleObservationSummary: 把模型看到的画面状态显示给用户；若没有这行代码，用户无法判断模型是否真的观察了截图。
            ...
            if isinstance(tool_calls, list) and tool_calls:  # 新增代码+ProcessSummaryUX: 判断本轮是否包含工具调用；若没有这行代码，所有模型消息都会被错误显示成最终回复。
                if visual_observation_summary or self._should_print_model_turn(turn_index):  # 修改代码+VisibleObservationSummary: 有新观察摘要时同步显示工具选择，否则继续按轮次节流；若没有这行代码，用户看得到观察却看不到模型准备调用什么工具。
    ...
    def _visual_observation_summary_from_model_payload(self, payload: dict[str, Any]) -> str:  # 新增代码+VisibleObservationSummary: 函数段开始，从模型事件中提取可安全展示的视觉观察摘要；若没有这段代码，终端无法说明模型到底观察到了什么。本段只显示模型主动写出的简短摘要，不显示隐藏推理，段落到 return 结束。
        decision_note = self._decision_note_from_model_payload(payload)  # 新增代码+VisibleObservationSummary: 读取模型公开 decision_note；若没有这行代码，视觉观察摘要没有数据来源。
        if not decision_note:  # 新增代码+VisibleObservationSummary: 没有公开摘要时直接跳过；若没有这行代码，空内容会继续进入过滤逻辑。
            return ""  # 新增代码+VisibleObservationSummary: 返回空字符串表示不显示；若没有这行代码，调用方无法区分无摘要和有效摘要。
        summary = self._safe_summary_text(decision_note)  # 新增代码+VisibleObservationSummary: 压缩换行和多余空白并限制长度；若没有这行代码，终端可能被长段文本刷屏。
        if not summary:  # 新增代码+VisibleObservationSummary: 清洗后为空则跳过；若没有这行代码，空白字符串可能被打印。
            return ""  # 新增代码+VisibleObservationSummary: 返回空摘要兜底；若没有这行代码，调用方仍可能打印空行。
        lowered = summary.lower()  # 新增代码+VisibleObservationSummary: 准备大小写无关检查原始图片负载关键词；若没有这行代码，base64/data:image 过滤会漏掉不同大小写。
        if "base64" in lowered or "data:image" in lowered:  # 新增代码+VisibleObservationSummary: 拒绝把图片编码或 data URL 打到终端；若没有这行代码，用户界面可能泄露大段原始图像数据。
            return ""  # 新增代码+VisibleObservationSummary: 原始图像负载不是用户可读摘要，直接跳过；若没有这行代码，终端会被不可读编码污染。
        visual_markers = ("观察", "画面", "截图", "屏幕", "窗口", "画布", "轮廓", "颜色", "已填", "选中", "空白", "树冠", "树干", "Paint")  # 新增代码+VisibleObservationSummary: 定义视觉观察关键词，确保只显示和屏幕状态有关的摘要；若没有这行代码，普通内部决策摘要也可能被误显示。
        if not any(marker in summary for marker in visual_markers):  # 新增代码+VisibleObservationSummary: 过滤没有视觉含义的 decision_note；若没有这行代码，终端可能显示泛化规划而不是看图结果。
            return ""  # 新增代码+VisibleObservationSummary: 非视觉摘要不显示；若没有这行代码，过程摘要会重新变吵。
        if summary == self._last_visual_observation_summary:  # 新增代码+VisibleObservationSummary: 去掉连续重复的视觉观察；若没有这行代码，同一截图状态可能反复刷屏。
            return ""  # 新增代码+VisibleObservationSummary: 重复摘要直接跳过；若没有这行代码，去重状态不会生效。
        self._last_visual_observation_summary = summary  # 新增代码+VisibleObservationSummary: 记录本次已显示摘要；若没有这行代码，下一次无法判断是否重复。
        return summary  # 新增代码+VisibleObservationSummary: 返回安全、去重后的视觉观察摘要；若没有这行代码，调用方拿不到要显示的文本。

    def _decision_note_from_model_payload(self, payload: dict[str, Any]) -> str:  # 新增代码+VisibleObservationSummary: 函数段开始，兼容读取顶层和 message 内部的 decision_note；若没有这段代码，生产事件和测试事件字段形态稍变就会丢失摘要。本段只读公开摘要字段，段落到 return 结束。
        direct_note = payload.get("decision_note", "")  # 新增代码+VisibleObservationSummary: 先读取顶层 decision_note；若没有这行代码，未来直接字段事件无法显示观察摘要。
        if isinstance(direct_note, str) and direct_note.strip():  # 新增代码+VisibleObservationSummary: 顶层字段是非空字符串时优先使用；若没有这行代码，合法顶层摘要会被忽略。
            return direct_note  # 新增代码+VisibleObservationSummary: 返回顶层公开摘要；若没有这行代码，后续只能依赖嵌套 message。
        message = payload.get("message", {})  # 新增代码+VisibleObservationSummary: 读取生产主循环常见的嵌套 message；若没有这行代码，真实模型消息里的 decision_note 不会被发现。
        if isinstance(message, dict):  # 新增代码+VisibleObservationSummary: message 是字典时安全读取；若没有这行代码，非字典消息会导致 `.get` 崩溃。
            nested_note = message.get("decision_note", "")  # 新增代码+VisibleObservationSummary: 从 message.decision_note 读取公开摘要；若没有这行代码，真实 Computer Use 观察描述不会显示。
            return nested_note if isinstance(nested_note, str) else ""  # 新增代码+VisibleObservationSummary: 只返回字符串摘要；若没有这行代码，异常类型会污染终端文案。
        return ""  # 新增代码+VisibleObservationSummary: 没有可读 decision_note 时返回空；若没有这行代码，调用方需要处理 None。

    def _safe_summary_text(self, raw_text: str) -> str:  # 新增代码+VisibleObservationSummary: 函数段开始，把模型公开摘要压缩成适合终端的一行；若没有这段代码，长文本和换行会破坏真实终端可读性。本段不改写语义，只做安全裁剪，段落到 return 结束。
        collapsed = " ".join(str(raw_text).replace("`", "").split())  # 新增代码+VisibleObservationSummary: 移除代码围栏符号并压缩空白；若没有这行代码，摘要可能多行刷屏或混入代码块样式。
        if len(collapsed) <= 180:  # 新增代码+VisibleObservationSummary: 短摘要可以完整显示；若没有这行代码，所有摘要都会被不必要截断。
            return collapsed  # 新增代码+VisibleObservationSummary: 返回未截断的安全摘要；若没有这行代码，短观察也无法原样显示。
        return collapsed[:177].rstrip() + "..."  # 新增代码+VisibleObservationSummary: 长摘要截断到一行可读范围；若没有这行代码，真实终端可能再次被长段文本占满。
    ...
    def _tool_name_from_call(self, tool_call: Any) -> str:  # 新增代码+ProcessSummaryUX: 函数段开始，兼容字典和对象形式的工具调用；若没有这段代码，不同模型适配器的 tool_call 形态会让摘要失效。本段只提取名称，段落到 return 结束。
        if isinstance(tool_call, dict):  # 新增代码+ProcessSummaryUX: 处理 transcript 中常见的字典工具调用；若没有这行代码，测试和 JSON 事件无法显示工具名。
            return str(tool_call.get("tool_name", tool_call.get("name", "tool")))  # 修改代码+ComputerUseMcpV2ResidualCleanup：优先读取生产事件的 tool_name 字段；若没有这行代码，真实终端会只能显示泛化 tool 名称。
    ...
    def _tool_action_summary(self, tool_name: str, arguments: dict[str, Any]) -> str:  # 新增代码+ProcessSummaryUX: 函数段开始，把工具参数压缩成人话；若没有这段代码，终端只能显示生硬工具名。本段刻意不输出完整参数，段落到最终 return 结束。
        ...
        if tool_name in {"mcp__computer-use__observe", "mcp__computer-use__screenshot"}:  # 修改代码+ComputerUseMcpV2ResidualCleanup：只识别 v2 MCP 屏幕观察工具；若没有这行代码，观察动作会显示得过于泛化。
            return "观察屏幕和窗口状态"  # 新增代码+ProcessSummaryUX: 返回观察摘要；若没有这行代码，用户不知道 agent 正在看屏幕。
        if tool_name.startswith("mcp__computer-use__"):  # 修改代码+ComputerUseMcpV2ResidualCleanup：只把 v2 MCP 工具视为桌面动作工具；若没有这行代码，鼠标键盘动作无法显示成可懂意图。
            v2_action = tool_name.removeprefix("mcp__computer-use__") if tool_name.startswith("mcp__computer-use__") else ""  # 修改代码+ComputerUseMcpV2ResidualCleanup：从 MCP 工具名提取原子动作；若没有这行代码，open_application 这类工具无法被人话摘要识别。
            effective_action = action or v2_action  # 修改代码+ComputerUseMcpV2ResidualCleanup：优先用参数 action，缺省时用 v2 原子名；若没有这行代码，MCP 原子工具摘要会缺少动作类型。
            if effective_action in {"launch_app", "open_application"}:  # 新增代码+ProcessSummaryUX: 识别启动应用动作；若没有这行代码，打开 Paint/Notepad 等关键步骤无法被用户理解。
            ...
            if effective_action in {"click", "mouse_click", "left_click", "double_click", "right_click"}:  # 新增代码+ProcessSummaryUX: 识别鼠标点击动作；若没有这行代码，点击会被显示成泛化桌面动作。
                return "执行鼠标点击"  # 新增代码+ProcessSummaryUX: 返回点击摘要；若没有这行代码，用户看不到下一步是点击。
            if effective_action in {"drag_path", "drag", "mouse_drag"}:  # 修改代码+ComputerUseMcpV2ResidualCleanup：用合并后的动作名识别拖拽；若没有这行代码，mcp__computer-use__computer_batch 等 v2 动作摘要会漏掉拖拽意图。
                return "执行鼠标拖拽轨迹"  # 新增代码+ProcessSummaryUX: 返回拖拽摘要；若没有这行代码，用户不知道 agent 正在画线或拖动。
            if effective_action in {"type_text", "type", "keyboard_type"}:  # 新增代码+ProcessSummaryUX: 识别键盘输入动作；若没有这行代码，输入文本步骤会显得不透明。
                return "输入文本"  # 新增代码+ProcessSummaryUX: 返回输入摘要；若没有这行代码，用户不知道 agent 准备键入内容。
            return f"执行桌面动作 {effective_action or '未命名动作'}"  # 新增代码+ProcessSummaryUX: 返回桌面动作兜底摘要；若没有这行代码，新动作类型会没有可读提示。
    ...
    def _should_print_tool_event(self, tool_name: str, count: int, arguments: dict[str, Any]) -> bool:  # 修改代码+ProcessSummaryNoiseControl: 函数段开始，判断工具事件是否应展示；若没有这段代码，桌面工具在绘图任务中会产生大量重复行。本段保护终端 UX，同时保留关键启动和周期进度，段落到 return 结束。
        if not tool_name.startswith("mcp__computer-use__"):  # 修改代码+ComputerUseMcpV2ResidualCleanup：只有 v2 MCP 桌面工具参与节流；若没有这行代码，文件和终端工具进度可能被误压缩。
        ...
        if action == "launch_app" or tool_name == "mcp__computer-use__open_application":  # 修改代码+ProcessSummaryNoiseControl: 启动应用属于关键动作；若没有这行代码，用户看不到 agent 是否真的尝试打开软件。
    ...
    def _tool_result_summary(self, tool_name: str, payload: dict[str, Any]) -> str:  # 新增代码+ProcessSummaryUX: 函数段开始，把工具结果压缩成安全摘要；若没有这段代码，终端可能泄露长日志或敏感字段。本段只输出统计和类别，不输出原始 output，段落到最终 return 结束。
        ...
        if tool_name.startswith("mcp__computer-use__"):  # 修改代码+ComputerUseMcpV2ResidualCleanup：只识别 v2 MCP 桌面工具结果；若没有这行代码，桌面观察/动作的结果会显示得不贴近任务。
