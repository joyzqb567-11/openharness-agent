...
from typing import Any, Callable, Protocol  # 修改代码+RealHarnessAdapter：增加 Any 供真实 adapter 注入参数使用；如果没有这一行，DefaultHarnessGuiAgentAdapter 无法保存可选工厂。
...
_DEFAULT_ALLOWED_TOOL_NAMES = object()  # 新增代码+GuiToolchainReuse：区分“没传白名单”和“显式传 None”；如果没有这一行，默认无工具和 full_access 全工具会继续互相冲突。
...
class DefaultHarnessGuiAgentAdapter:  # 新增代码+GuiAgentAdapter：类段开始，真实 harness adapter 的 feature-flagged shell；如果没有这个类，调用方无法显式请求真实模式并得到稳定失败。
    def __init__(self, enabled: bool = False, model_factory: Any | None = None, permission_requester: Any | None = None, max_turns: int = 1, allowed_tool_names: set[str] | None | object = _DEFAULT_ALLOWED_TOOL_NAMES, mcp_tool_registry: Any | None = None) -> None:  # 修改代码+GuiToolchainReuse：函数段开始，保存真实 adapter 开关、权限回调、工具白名单和 MCP registry；如果没有这段，GUI 无法复用 core 工具链路。
        self.enabled = enabled  # 新增代码+GuiAgentAdapter：保存 feature flag；如果没有这一行，禁用状态无法判断。
        self.model_factory = model_factory  # 新增代码+RealHarnessAdapter：保存可选模型工厂；如果没有这一行，测试无法注入离线假模型。
        self.permission_requester = permission_requester  # 新增代码+RealHarnessAdapter：保存可选权限回调；如果没有这一行，Phase 4 无法复用同一个 shell。
        self.max_turns = max(int(max_turns), 1)  # 新增代码+RealHarnessAdapter：保存真实主循环轮数上限；如果没有这一行，错误值可能让 agent 不发模型请求。
        if allowed_tool_names is _DEFAULT_ALLOWED_TOOL_NAMES:  # 新增代码+GuiToolchainReuse：识别调用方省略白名单的旧默认路径；如果没有这一行，旧默认无工具防护会被误当 full_access。
            self.allowed_tool_names = set()  # 新增代码+GuiToolchainReuse：省略白名单时保持默认无工具；如果没有这一行，旧 Computer Use 防护测试会失效。
        elif allowed_tool_names is None:  # 新增代码+GuiToolchainReuse：识别 GUI full_access 显式传入 None；如果没有这一行，全量工具策略无法表达。
            self.allowed_tool_names = None  # 新增代码+GuiToolchainReuse：显式 None 原样保留给 LearningAgent；如果没有这一行，shell/MCP/tool_search 会被空集合封死。
        else:  # 新增代码+GuiToolchainReuse：处理 read_only/workspace_write 的具体白名单；如果没有这一行，集合白名单无法落到 adapter。
            self.allowed_tool_names = set(allowed_tool_names)  # 新增代码+GuiToolchainReuse：复制具体白名单避免调用方后续修改；如果没有这一行，运行中工具边界可能被外部集合改变。
        self.mcp_tool_registry = mcp_tool_registry  # 新增代码+GuiToolchainReuse：保存可选 MCP registry；如果没有这一行，full_access GUI 仍无法发现 mcp_servers.json 外部工具。
    # 修改代码+RealHarnessAdapter：函数段结束，DefaultHarnessGuiAgentAdapter.__init__ 到此结束；如果没有这个边界说明，用户不容易看出初始化范围。

    def run(self, request: GuiAgentRunRequest, emit_event: GuiEventEmitter, is_cancelled: GuiCancelChecker) -> GuiAgentRunResult:  # 修改代码+RealHarnessAdapter：函数段开始，禁用时稳定失败，启用时委托真实 core adapter；如果没有这段，显式真实模式没有入口。
        ...
        from learning_agent.app.gui_harness_adapter import RealHarnessGuiAgentAdapter  # 新增代码+RealHarnessAdapter：启用时懒加载真实 adapter 避免模块循环；如果没有这一行，shell 无法调用 core agent。

        delegate = RealHarnessGuiAgentAdapter(enabled=True, model_factory=self.model_factory, permission_requester=self.permission_requester, max_turns=self.max_turns, allowed_tool_names=self.allowed_tool_names, mcp_tool_registry=self.mcp_tool_registry)  # 修改代码+GuiToolchainReuse：创建真实委托对象并传递工具白名单和 MCP registry；如果没有这一行，GUI full_access 无法复用外部工具链路。
        return delegate.run(request, emit_event, is_cancelled)  # 新增代码+RealHarnessAdapter：把 GUI 请求交给真实 adapter；如果没有这一行，GUI 仍不会进入 LearningAgent.run。
    # 修改代码+RealHarnessAdapter：函数段结束，DefaultHarnessGuiAgentAdapter.run 到此结束；如果没有这个边界说明，用户不容易看出真实 adapter shell 范围。
