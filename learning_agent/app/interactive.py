...
import os  # 新增代码+Phase106InteractiveFullLaunch：读取 Phase106 真实启动双环境门；如果没有这行代码，真实验收无法显式区分默认安全模式和允许打开 Notepad 的模式。
...
PHASE106_INTERACTIVE_FULL_LAUNCH_MARKER = "PHASE106_INTERACTIVE_FULL_LAUNCH_READY"  # 新增代码+Phase106InteractiveFullLaunch：定义用户命令接入 full 启动的稳定 ready marker；如果没有这行代码，测试和真实终端验收无法可靠定位 Phase106 输出。
PHASE106_INTERACTIVE_FULL_LAUNCH_OK_TOKEN = "PHASE106_INTERACTIVE_FULL_LAUNCH_OK"  # 新增代码+Phase106InteractiveFullLaunch：定义 Phase106 成功 token；如果没有这行代码，失败输出和成功输出很难被验收器区分。
PHASE106_INTERACTIVE_FULL_LAUNCH_REQUEST_ENV = "LEARNING_AGENT_PHASE106_RUN_INTERACTIVE_FULL_LAUNCH"  # 新增代码+Phase106InteractiveFullLaunch：定义真实启动请求环境变量；如果没有这行代码，验收无法明确表达“本次要跑真实 Notepad smoke”。
PHASE106_INTERACTIVE_FULL_LAUNCH_ENABLE_ENV = "LEARNING_AGENT_PHASE106_ENABLE_INTERACTIVE_FULL_LAUNCH"  # 新增代码+Phase106InteractiveFullLaunch：定义真实启动允许环境变量；如果没有这行代码，单个环境变量误设就可能打开真实桌面应用。
...
try:  # 新增代码+AppSplit: 优先按包路径导入轮次格式化和验收事件 helper；若没有这行代码，包运行模式下交互层无法复用通用能力。
    ...
    from learning_agent.app.process_summary import TerminalProcessSummaryRenderer  # 新增代码+ProcessSummaryUX: 导入终端过程摘要渲染器；若没有这行代码，真实交互入口无法显示 Codex 风格进度摘要。
    ...
    from learning_agent.computer_use_mcp_v2.windows_runtime.approval import WindowsComputerUseApprovalModel  # 新增代码+Phase38WindowsComputerApproval: 导入 Windows approval 终端状态模型；如果没有这行代码，/computer status 无法显示 session allowlist 和 grant flags。
    from learning_agent.computer_use_mcp_v2.windows_runtime.abort_streaming_hooks import WindowsComputerUseAbortStreamingHooks  # 新增代码+Phase61AbortStreamingHooks: 导入 abort/cleanup/streaming hook 状态模型；如果没有这行代码，/computer abort-hooks 和 /computer status 无法显示中断钩子。
    from learning_agent.computer_use_mcp_v2.windows_runtime.high_level_tools import WindowsHighLevelComputerToolRuntime  # 新增代码+Phase62HighLevelTools: 导入高层 Computer Tool 状态模型；如果没有这行代码，/computer high-level-tools 和 /computer status 无法显示 Phase62。
    from learning_agent.computer_use_mcp_v2.windows_runtime.controller_takeover import WindowsComputerUseControllerTakeoverDebugSurface  # 新增代码+Phase63ControllerTakeover: 导入外部 agent controller 接管调试面；如果没有这行代码，/computer controller 和 /computer status 无法显示 Phase63。
    from learning_agent.computer_use_mcp_v2.windows_runtime.controller import ComputerUseController  # 新增代码+Phase51ComputerStatusUI: 导入 Computer Use 控制器用于 `/computer observe` 只读入口；如果没有这行代码，observe 命令只能停留在文案。
    from learning_agent.computer_use_mcp_v2.windows_runtime.lock import ComputerUseLockManager  # 新增代码+Phase31ComputerUseLockAbortEvidence: 导入桌面锁管理器；如果没有这行代码，/computer 终端命令无法读取或修改 abort 状态。
    from learning_agent.computer_use_mcp_v2.windows_runtime.mode_session import ComputerUseModeSessionStore  # 新增代码+Phase98UniversalComputerUseMode: 导入通用 Computer Use 模式 session store；如果没有这行代码，/computer use 无法真实打开 normal/observe/full 模式状态。
    from learning_agent.computer_use_mcp_v2.windows_runtime.native_diagnostics import format_phase43_capability_matrix_lines, run_phase43_native_capability_matrix_contract  # 新增代码+Phase43WindowsNativeCapabilityMatrix: 导入 Phase43 终端能力矩阵渲染入口；如果没有这行代码，/computer status 看不到 WGC/UIA/SendInput 差距。
    from learning_agent.computer_use_mcp_v2.windows_runtime.persistent_grants import WindowsComputerUsePersistentGrantStore  # 新增代码+Phase60PersistentGrants: 导入生产级持久授权 store；如果没有这行代码，/computer approve/grants/revoke 无法真正落盘和评估。
    from learning_agent.computer_use_mcp_v2.windows_runtime.security_policy import WindowsComputerUseSecurityPolicy  # 新增代码+Phase48WindowsSecurityPolicy: 导入 Phase48 安全策略终端状态模型；如果没有这行代码，/computer status 无法显示 grant_classes 和高风险默认拒绝。
    from learning_agent.computer_use_mcp_v2.windows_runtime.session_context import DEFAULT_SESSION_CONTEXT_ID, ComputerUseSessionContextStore  # 新增代码+Phase59SessionContextAppState: 导入统一 session context store；如果没有这行代码，/computer status 无法读取 allowed_apps/grants/display/screenshot/cleanup 同一事实源。
    from learning_agent.computer_use_mcp_v2.windows_runtime.session_runtime import WindowsComputerUseSessionRuntime, format_phase40_runtime_action, format_phase50_recovery_action  # 修改代码+Phase50WindowsRecovery: 导入 Phase40 会话运行时和 Phase50 恢复格式化器；如果没有这行代码，/computer recover/journal 无法显示恢复层。
    from learning_agent.computer_use_mcp_v2.windows_runtime.terminal_grants import ComputerUseTerminalGrantStore  # 新增代码+Phase51ComputerStatusUI: 导入终端授权草案 store；如果没有这行代码，/computer grant/revoke 无法跨命令显示。
    from learning_agent.computer_use_mcp_v2.openharness_bridge.terminal_commands import activate_computer_use_mode as activate_computer_use_mcp_v2_terminal_mode, deactivate_computer_use_mode as deactivate_computer_use_mcp_v2_terminal_mode  # 修改代码+ComputerUseMcpV2ParityTightening：从 OpenHarness 专属桥接层导入 v2 终端模式桥接函数；如果没有这行代码，terminal_commands.py 会继续污染 ClaudeCode 镜像目录。
    ...
    from learning_agent.core.conversation_context import ConversationContextConfig, InteractiveConversationContext  # 新增代码+交互上下文: 导入交互历史管理器；若没有这行代码，真实终端每轮仍只能把当前输入交给模型。
    from learning_agent.core import run_helpers as run_helpers_from_core  # 修改代码+AgentPySplitPhase15B2: 导入 core observation helper；若没有这行代码，删除 agent.py `旧观察薄包装` 后终端模式切换无法写审计事件。
    ...
    from learning_agent.tools import search as search_tools_from_tools  # 修改代码+AgentPyCompatWrapperRemovalL4: 终端 full 模式直接调用工具搜索模块加载能力包；若没有这行代码，删除 agent._tool_search_select_pack 后 `/computer use --full` 无法把工具包接到模型主循环。
except ModuleNotFoundError as error:  # 新增代码+AppSplit: 捕获脚本模式下包路径不可用；若没有这行代码，双击 bat 时交互层可能导入失败。
    if error.name not in {"learning_agent", "learning_agent.app", "learning_agent.app.chrome_status_renderer", "learning_agent.app.computer_status_renderer", "learning_agent.app.process_summary", "learning_agent.app.status_renderer", "learning_agent.browser_extension_host", "learning_agent.browser_extension_host.bridge_server", "learning_agent.browser_extension_host.manifest_installer", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.approval", "learning_agent.computer_use_mcp_v2.windows_runtime.abort_streaming_hooks", "learning_agent.computer_use_mcp_v2.windows_runtime.high_level_tools", "learning_agent.computer_use_mcp_v2.windows_runtime.controller_takeover", "learning_agent.computer_use_mcp_v2.windows_runtime.controller", "learning_agent.computer_use_mcp_v2.windows_runtime.lock", "learning_agent.computer_use_mcp_v2.windows_runtime.mode_session", "learning_agent.computer_use_mcp_v2.windows_runtime.native_diagnostics", "learning_agent.computer_use_mcp_v2.windows_runtime.persistent_grants", "learning_agent.computer_use_mcp_v2.windows_runtime.security_policy", "learning_agent.computer_use_mcp_v2.windows_runtime.session_context", "learning_agent.computer_use_mcp_v2.windows_runtime.session_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.terminal_grants", "learning_agent.core", "learning_agent.core.config", "learning_agent.core.conversation_context", "learning_agent.core.run_helpers", "learning_agent.observability", "learning_agent.observability.acceptance_events", "learning_agent.observability.run_records", "learning_agent.runtime", "learning_agent.runtime.command_queue", "learning_agent.runtime.status_snapshot", "learning_agent.sdk", "learning_agent.sdk.status", "learning_agent.tools", "learning_agent.tools.search"}:  # 修改代码+交互上下文: 允许脚本模式 fallback 覆盖 conversation_context；若没有这行代码，start_oauth_agent.bat 可能因包路径差异无法加载多轮历史。
    ...
    from app.process_summary import TerminalProcessSummaryRenderer  # 新增代码+ProcessSummaryUX: 脚本模式导入终端过程摘要渲染器；若没有这行代码，bat 入口无法显示主循环进度摘要。
    ...
    from computer_use_mcp_v2.windows_runtime.approval import WindowsComputerUseApprovalModel  # 新增代码+Phase38WindowsComputerApproval: 脚本模式导入 Windows approval 终端状态模型；如果没有这行代码，双击 bat 后 /computer status 看不到审批摘要。
    from computer_use_mcp_v2.windows_runtime.abort_streaming_hooks import WindowsComputerUseAbortStreamingHooks  # 新增代码+Phase61AbortStreamingHooks: 脚本模式导入 abort/cleanup/streaming hook 状态模型；如果没有这行代码，双击 bat 后 /computer abort-hooks 不可用。
    from computer_use_mcp_v2.windows_runtime.high_level_tools import WindowsHighLevelComputerToolRuntime  # 新增代码+Phase62HighLevelTools: 脚本模式导入高层 Computer Tool 状态模型；如果没有这行代码，双击 bat 后 /computer high-level-tools 不可用。
    from computer_use_mcp_v2.windows_runtime.controller_takeover import WindowsComputerUseControllerTakeoverDebugSurface  # 新增代码+Phase63ControllerTakeover: 脚本模式导入外部 agent controller 接管调试面；如果没有这行代码，双击 bat 后 /computer controller 不可用。
    from computer_use_mcp_v2.windows_runtime.controller import ComputerUseController  # 新增代码+Phase51ComputerStatusUI: 脚本模式导入 Computer Use 控制器；如果没有这行代码，双击 bat 后 /computer observe 不可用。
    from computer_use_mcp_v2.windows_runtime.lock import ComputerUseLockManager  # 新增代码+Phase31ComputerUseLockAbortEvidence: 脚本模式导入桌面锁管理器；如果没有这行代码，双击 bat 后 /computer abort 不可用。
    from computer_use_mcp_v2.windows_runtime.mode_session import ComputerUseModeSessionStore  # 新增代码+Phase98UniversalComputerUseMode: 脚本模式导入通用 Computer Use 模式 session store；如果没有这行代码，双击 bat 后 /computer use 不可用。
    from computer_use_mcp_v2.windows_runtime.native_diagnostics import format_phase43_capability_matrix_lines, run_phase43_native_capability_matrix_contract  # 新增代码+Phase43WindowsNativeCapabilityMatrix: 脚本模式导入 Phase43 能力矩阵；如果没有这行代码，双击 bat 后 /computer status 看不到 native 能力矩阵。
    from computer_use_mcp_v2.windows_runtime.persistent_grants import WindowsComputerUsePersistentGrantStore  # 新增代码+Phase60PersistentGrants: 脚本模式导入持久授权 store；如果没有这行代码，双击 bat 后 /computer approve/grants/revoke 不可用。
    from computer_use_mcp_v2.windows_runtime.security_policy import WindowsComputerUseSecurityPolicy  # 新增代码+Phase48WindowsSecurityPolicy: 脚本模式导入 Phase48 安全策略模型；如果没有这行代码，双击 bat 后 /computer status 看不到 grant_classes。
    from computer_use_mcp_v2.windows_runtime.session_context import DEFAULT_SESSION_CONTEXT_ID, ComputerUseSessionContextStore  # 新增代码+Phase59SessionContextAppState: 脚本模式导入统一 session context store；如果没有这行代码，双击 bat 后 /computer status 读不到 AppState。
    from computer_use_mcp_v2.windows_runtime.session_runtime import WindowsComputerUseSessionRuntime, format_phase40_runtime_action, format_phase50_recovery_action  # 修改代码+Phase50WindowsRecovery: 脚本模式导入 Phase40/Phase50 运行时格式化器；如果没有这行代码，双击 bat 后 /computer recover/journal 不可用。
    from computer_use_mcp_v2.windows_runtime.terminal_grants import ComputerUseTerminalGrantStore  # 新增代码+Phase51ComputerStatusUI: 脚本模式导入终端授权草案 store；如果没有这行代码，双击 bat 后 /computer grant/revoke 不可用。
    from computer_use_mcp_v2.openharness_bridge.terminal_commands import activate_computer_use_mode as activate_computer_use_mcp_v2_terminal_mode, deactivate_computer_use_mode as deactivate_computer_use_mcp_v2_terminal_mode  # 修改代码+ComputerUseMcpV2ParityTightening：脚本模式从 OpenHarness 专属桥接层导入 v2 终端模式桥接函数；如果没有这行代码，bat 入口会继续依赖 ClaudeCode 镜像目录里的项目私有文件。
    from app.status_renderer import render_status_snapshot  # 新增代码+StatusEcosystem: 脚本模式导入状态渲染器；若没有这行代码，直接运行时 /status 无法显示。
    from core.conversation_context import ConversationContextConfig, InteractiveConversationContext  # 新增代码+交互上下文: 脚本模式导入交互历史管理器；若没有这行代码，bat 入口无法使用多轮上下文。
    from core import run_helpers as run_helpers_from_core  # 修改代码+AgentPySplitPhase15B2: 脚本模式下导入 core observation helper；若没有这行代码，bat 入口终端命令无法写模式切换审计。
    ...
    from tools import search as search_tools_from_tools  # 修改代码+AgentPyCompatWrapperRemovalL4: 脚本模式下直接调用工具搜索模块加载能力包；若没有这行代码，start_oauth_agent.bat 下 full 模式会因为旧包装删除而无法加载 Computer Use 工具。
...
def _format_computer_mode_result(result: dict[str, Any]) -> str:  # 修改代码+Phase98UniversalComputerUseMode: 函数段开始，格式化 `/computer use` 模式结果并保留失败 decision；如果没有这段函数，full-confirm 错误只会显示空 mode 难排查。
    ...
    abort_auto_cleared_line = f"- abort_auto_cleared={_phase98_bool(result.get('abort_auto_cleared', False))}\n" if "abort_auto_cleared" in result else ""  # 新增代码+FullModeAutoClearAbort: 仅在 full 命令主动处理历史急停时显示恢复结果；如果没有这行代码，用户看不到系统已经替他清掉旧 abort。
    return f"Computer Use Mode\n- mode={result.get('mode', '')}\n- full_mode={_phase98_bool(result.get('full_mode', False))}\n- opened={_phase98_bool(result.get('opened', False))}\n- stopped={_phase98_bool(result.get('stopped', False))}\n{decision_line}{abort_auto_cleared_line}- ttl_seconds={_phase98_safe_int(result.get('ttl_seconds', result.get('ttl_original_seconds', 0)))}\n- per_app_allowlist_required={_phase98_bool(result.get('per_app_allowlist_required', False))}\n- ordinary_apps_allowed_by_risk_policy={_phase98_bool(result.get('ordinary_apps_allowed_by_risk_policy', False))}\n- allowed_action_classes={_phase98_csv(result.get('allowed_action_classes', []))}\n- real_desktop_touched=false\n- low_level_event_count={_phase98_safe_int(result.get('low_level_event_count', 0))}\n- marker={result.get('marker', '')}\n- ok_token={result.get('ok_token', '')}\n"  # 修改代码+FullModeAutoClearAbort: 返回包含可选自动清急停字段的模式面板；如果没有这行代码，full 模式恢复历史 abort 的证据不会进入真实终端输出。
# 修改代码+Phase98UniversalComputerUseMode: 函数段结束，_format_computer_mode_result 到此结束；如果没有这个边界说明，读者不容易看出模式结果输出和失败诊断范围。


def _format_computer_full_request(result: dict[str, Any]) -> str:  # 修改代码+FullMaturityMatrix：函数段开始，格式化 `/computer use --full` 强确认请求并附带最终成熟矩阵；如果没有这段函数，用户申请 full 时看不到成熟边界。
    token = str(result.get("confirmation_token", "") or "")  # 修改代码+FullMaturityMatrix：读取确认 token；如果没有这行代码，输出无法告诉用户下一步命令。
    run_maturity_matrix, maturity_cli_line = _phase115_full_maturity_matrix_tools()  # 新增代码+FullMaturityMatrix：延迟读取最终成熟矩阵工具；如果没有这一行，full 请求无法复用 Task7 矩阵。
    maturity_report = run_maturity_matrix()  # 新增代码+FullMaturityMatrix：生成无桌面副作用的最终矩阵；如果没有这一行，强确认面板没有成熟度事实。
    maturity_line = maturity_cli_line(maturity_report)  # 新增代码+FullMaturityMatrix：生成稳定 maturity token 行；如果没有这一行，真实终端验收不能在 full 请求里匹配最终字段。
    return f"Computer Use Full Request\n- strong_confirmation_required={_phase98_bool(result.get('strong_confirmation_required', False))}\n- risk=high\n- confirmation_token={token}\n- confirm_command=/computer use --full-confirm {token}\n- full_mode={_phase98_bool(result.get('full_mode', False))}\n- opened={_phase98_bool(result.get('opened', False))}\n- maturity_matrix={maturity_line}\n- real_desktop_touched=false\n- low_level_event_count={_phase98_safe_int(result.get('low_level_event_count', 0))}\n- marker={result.get('marker', '')}\n- ok_token={result.get('ok_token', '')}\n"  # 修改代码+FullMaturityMatrix：返回待确认面板、最终成熟矩阵和零事件证据；如果没有这一行，full 请求会缺少用户最关心的成熟边界。
# 修改代码+FullMaturityMatrix：函数段结束，_format_computer_full_request 到此结束；如果没有这个边界说明，读者不容易看出 full 请求输出范围。
...
def _format_computer_stop_result(stop_result: dict[str, Any]) -> str:  # 修改代码+Phase98UniversalComputerUseMode: 函数段开始，格式化 `/computer stop` 模式停止结果并安全显示事件数；如果没有这段函数，用户看不到 mode session 是否真的 stopped。
    return f"Computer Use Stop\n- stopped={_phase98_bool(stop_result.get('stopped', False))}\n- cleanup_completed={_phase98_bool(stop_result.get('cleanup_completed', False))}\n- lock_released={_phase98_bool(stop_result.get('lock_released', False))}\n- abort_cleared={_phase98_bool(stop_result.get('abort_cleared', False))}\n- owned_resource_cleanup_completed={_phase98_bool(stop_result.get('owned_resource_cleanup_completed', False))}\n- residual_owned_process={_phase98_bool(stop_result.get('residual_owned_process', False))}\n- real_desktop_touched=false\n- low_level_event_count={_phase98_safe_int(stop_result.get('low_level_event_count', 0))}\n- marker={stop_result.get('marker', '')}\n- ok_token={stop_result.get('ok_token', '')}\n"  # 修改代码+TurnCleanupLifecycleRootRemediation: 返回稳定停止面板并显示 cleanup/lock/abort evidence；如果没有这一行，stop 输出无法证明本轮控制状态已收尾。
# 修改代码+Phase98UniversalComputerUseMode: 函数段结束，_format_computer_stop_result 到此结束；如果没有这个边界说明，读者不容易看出 stop 输出范围。


def _phase106_env_enabled(env_name: str, override: bool | None = None) -> bool:  # 新增代码+Phase106InteractiveFullLaunch：函数段开始，读取 Phase106 布尔环境门；如果没有这段函数，真实启动开关会散落在命令逻辑里。
    if override is not None:  # 新增代码+Phase106InteractiveFullLaunch：优先使用测试或内部调用传入的显式值；如果没有这行代码，单元测试只能修改全局环境变量。
        return bool(override)  # 新增代码+Phase106InteractiveFullLaunch：返回显式布尔值；如果没有这行代码，调用方传入的安全开关不会生效。
    raw_value = str(os.environ.get(env_name, "") or "").strip().lower()  # 新增代码+Phase106InteractiveFullLaunch：读取并规范化环境变量；如果没有这行代码，大小写和空白会让开关判断不稳定。
    return raw_value in {"1", "true", "yes", "on"}  # 新增代码+Phase106InteractiveFullLaunch：只接受明确真值；如果没有这行代码，随便写入任意字符串都可能被当成允许真实启动。
# 新增代码+Phase106InteractiveFullLaunch：函数段结束，_phase106_env_enabled 到此结束；如果没有这个边界说明，初学者不容易看出环境门读取范围。


def _phase106_live_execution_classes() -> tuple[Any, Any]:  # 新增代码+Phase106InteractiveFullLaunch：函数段开始，延迟导入 Phase105/Universal gate；如果没有这段函数，脚本模式和包模式会重复导入分支。
    try:  # 新增代码+Phase106InteractiveFullLaunch：优先按 learning_agent 包路径导入；如果没有这行代码，项目根运行时无法加载标准模块。
        from learning_agent.computer_use_mcp_v2.windows_runtime.universal_live_execution import Phase105ControlledNotepadSmokeLaunchCandidate, UniversalWindowsLiveExecutionGate  # 新增代码+Phase106InteractiveFullLaunch：导入 Phase105 受控 Notepad bridge 和 Universal gate；如果没有这行代码，交互命令无法接到 Phase105。
    except ModuleNotFoundError as error:  # 新增代码+Phase106InteractiveFullLaunch：兼容 start_oauth_agent.bat 从 learning_agent 目录启动的脚本模式；如果没有这行代码，真实可见终端可能导入失败。
        if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.universal_live_execution"}:  # 新增代码+Phase106InteractiveFullLaunch：只兜底包路径缺失；如果没有这行代码，Universal gate 内部 bug 会被误吞。
            raise  # 新增代码+Phase106InteractiveFullLaunch：重新抛出真实内部错误；如果没有这行代码，排查 Phase106 失败会被错误 fallback 干扰。
        from computer_use_mcp_v2.windows_runtime.universal_live_execution import Phase105ControlledNotepadSmokeLaunchCandidate, UniversalWindowsLiveExecutionGate  # type: ignore  # 新增代码+Phase106InteractiveFullLaunch：脚本模式导入同一能力；如果没有这行代码，双击 bat 后 `/computer launch notepad` 不可用。
    return Phase105ControlledNotepadSmokeLaunchCandidate, UniversalWindowsLiveExecutionGate  # 新增代码+Phase106InteractiveFullLaunch：返回两个类给调用方；如果没有这行代码，helper 无法创建受控候选和 live gate。
# 新增代码+Phase106InteractiveFullLaunch：函数段结束，_phase106_live_execution_classes 到此结束；如果没有这个边界说明，初学者不容易看出导入范围。


def _phase107_launch_target_tools() -> tuple[Any, Any]:  # 新增代码+Phase107InteractiveLaunchTarget：函数段开始，延迟导入 Phase107 目标解析器；如果没有这段函数，脚本模式下 `/computer launch` 可能找不到新模块。
    try:  # 新增代码+Phase107InteractiveLaunchTarget：优先按 learning_agent 包路径导入；如果没有这行代码，项目根运行时无法加载标准模块。
        from learning_agent.computer_use_mcp_v2.windows_runtime.windows_launch_resolver import phase107_cli_line, resolve_interactive_launch_target  # 修改代码+CompatSlimming：改为从统一 resolver 导入交互目标解析；如果没有这一行，删除旧 interactive_launch_target.py 后交互命令会断。
    except ModuleNotFoundError as error:  # 新增代码+Phase107InteractiveLaunchTarget：兼容 start_oauth_agent.bat 从 learning_agent 目录运行；如果没有这行代码，真实可见终端可能导入失败。
        if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.windows_launch_resolver"}:  # 修改代码+CompatSlimming：只兜底 resolver 包路径缺失；如果没有这一行，resolver 内部 bug 会被误吞。
            raise  # 新增代码+Phase107InteractiveLaunchTarget：重新抛出真实内部错误；如果没有这行代码，排查 Phase107 会被 fallback 干扰。
        from computer_use_mcp_v2.windows_runtime.windows_launch_resolver import phase107_cli_line, resolve_interactive_launch_target  # type: ignore  # 修改代码+CompatSlimming：脚本模式从统一 resolver 导入交互解析；如果没有这一行，双击 bat 后新命令不可用。
    return resolve_interactive_launch_target, phase107_cli_line  # 新增代码+Phase107InteractiveLaunchTarget：返回解析器和格式化器；如果没有这行代码，调用方拿不到 Phase107 工具。
# 新增代码+Phase107InteractiveLaunchTarget：函数段结束，_phase107_launch_target_tools 到此结束；如果没有这个边界说明，初学者不容易看出导入范围。


def _phase108_generic_app_tools() -> tuple[Any, Any]:  # 新增代码+Phase108GenericAppDiscovery：函数段开始，延迟导入 Phase108 通用应用发现工具；如果没有这段函数，脚本模式下 `/computer launch obsidian` 这类普通未知应用无法走通用发现。
    try:  # 新增代码+Phase108GenericAppDiscovery：优先按 learning_agent 包路径导入；如果没有这行代码，项目根运行时无法加载标准模块。
        from learning_agent.computer_use_mcp_v2.windows_runtime.windows_launch_resolver import phase108_cli_line, resolve_generic_app_launch_target  # 修改代码+CompatSlimming：改为从统一 resolver 导入通用应用解析；如果没有这一行，删除旧 generic_app_discovery.py 后交互命令会断。
    except ModuleNotFoundError as error:  # 新增代码+Phase108GenericAppDiscovery：兼容 start_oauth_agent.bat 从 learning_agent 目录运行；如果没有这行代码，真实可见终端可能导入失败。
        if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.windows_launch_resolver"}:  # 修改代码+CompatSlimming：只兜底 resolver 包路径缺失；如果没有这一行，resolver 内部 bug 会被误吞。
            raise  # 新增代码+Phase108GenericAppDiscovery：重新抛出真实内部错误；如果没有这行代码，排查通用发现会被 fallback 干扰。
        from computer_use_mcp_v2.windows_runtime.windows_launch_resolver import phase108_cli_line, resolve_generic_app_launch_target  # type: ignore  # 修改代码+CompatSlimming：脚本模式从统一 resolver 导入通用应用解析；如果没有这一行，双击 bat 后通用发现不可用。
    return resolve_generic_app_launch_target, phase108_cli_line  # 新增代码+Phase108GenericAppDiscovery：返回解析器和格式化器；如果没有这行代码，调用方拿不到 Phase108 工具。
# 新增代码+Phase108GenericAppDiscovery：函数段结束，_phase108_generic_app_tools 到此结束；如果没有这个边界说明，初学者不容易看出 Phase108 导入范围。


def _phase109_generic_real_launch_tools() -> tuple[Any, Any]:  # 新增代码+Phase109GenericRealLaunchCandidate：函数段开始，延迟导入 Phase109 通用真实启动候选工具；如果没有这段函数，脚本模式下 `/computer launch obsidian` 无法显示通用真实启动候选证据。
    try:  # 新增代码+Phase109GenericRealLaunchCandidate：优先按 learning_agent 包路径导入；如果没有这行代码，项目根运行时无法加载标准模块。
        from learning_agent.computer_use_mcp_v2.windows_runtime.generic_launch_backend import phase109_cli_line, prepare_phase109_generic_real_launch_candidate  # 修改代码+CompatSlimming：改为从通用启动后端导入真实启动候选；如果没有这一行，删除旧 generic_real_launch_candidate.py 后交互命令会断。
    except ModuleNotFoundError as error:  # 新增代码+Phase109GenericRealLaunchCandidate：兼容 start_oauth_agent.bat 从 learning_agent 目录运行；如果没有这行代码，真实可见终端可能导入失败。
        if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.generic_launch_backend"}:  # 修改代码+CompatSlimming：只兜底通用启动后端包路径缺失；如果没有这一行，后端内部 bug 会被误吞。
            raise  # 新增代码+Phase109GenericRealLaunchCandidate：重新抛出真实内部错误；如果没有这行代码，排查通用候选会被 fallback 干扰。
        from computer_use_mcp_v2.windows_runtime.generic_launch_backend import phase109_cli_line, prepare_phase109_generic_real_launch_candidate  # type: ignore  # 修改代码+CompatSlimming：脚本模式从通用启动后端导入候选工具；如果没有这一行，双击 bat 后通用候选不可用。
    return prepare_phase109_generic_real_launch_candidate, phase109_cli_line  # 新增代码+Phase109GenericRealLaunchCandidate：返回候选准备函数和格式化器；如果没有这行代码，调用方拿不到 Phase109 工具。
# 新增代码+Phase109GenericRealLaunchCandidate：函数段结束，_phase109_generic_real_launch_tools 到此结束；如果没有这个边界说明，初学者不容易看出 Phase109 导入范围。


def _phase113_interactive_generic_launch_tools() -> tuple[Any, Any]:  # 新增代码+InteractiveGenericLaunchMaturity：函数段开始，延迟导入 Phase113 交互通用启动桥；如果没有这段函数，脚本模式下 `/computer launch obsidian` 无法显示通用后端接线证据。
    try:  # 新增代码+InteractiveGenericLaunchMaturity：优先按 learning_agent 包路径导入；如果没有这一行，项目根运行时无法加载标准模块。
        from learning_agent.computer_use_mcp_v2.windows_runtime.universal_live_execution import phase113_cli_line, run_phase113_interactive_generic_launch_bridge  # 新增代码+InteractiveGenericLaunchMaturity：导入 Phase113 桥和 token 格式化器；如果没有这一行，交互命令无法接到 Phase110 通用后端。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.universal_live_execution"}:  # 新增代码+InteractiveGenericLaunchMaturity：只兜底包路径缺失；如果没有这一行，Phase113 内部 bug 会被误吞。
            raise  # 新增代码+InteractiveGenericLaunchMaturity：重新抛出真实内部错误；如果没有这一行，排查通用后端接线会被 fallback 干扰。
        from computer_use_mcp_v2.windows_runtime.universal_live_execution import phase113_cli_line, run_phase113_interactive_generic_launch_bridge  # type: ignore  # 新增代码+InteractiveGenericLaunchMaturity：脚本模式导入同一桥；如果没有这一行，双击 bat 后 Phase113 不可用。
    return run_phase113_interactive_generic_launch_bridge, phase113_cli_line  # 新增代码+InteractiveGenericLaunchMaturity：返回桥函数和格式化器；如果没有这一行，调用方拿不到 Phase113 工具。
# 新增代码+InteractiveGenericLaunchMaturity：函数段结束，_phase113_interactive_generic_launch_tools 到此结束；如果没有这个边界说明，初学者不容易看出 Phase113 导入范围。


def _phase115_full_maturity_matrix_tools() -> tuple[Any, Any]:  # 新增代码+FullMaturityMatrix：函数段开始，延迟导入最终成熟矩阵工具；如果没有这段函数，脚本模式下 `/computer maturity` 可能找不到新模块。
    try:  # 新增代码+FullMaturityMatrix：优先按 learning_agent 包路径导入；如果没有这一行，项目根运行时无法加载标准模块。
        from learning_agent.computer_use_mcp_v2.windows_runtime.full_maturity_matrix import computer_use_full_maturity_cli_line, run_computer_use_full_maturity_matrix  # 新增代码+FullMaturityMatrix：导入矩阵生成函数和 token 格式化器；如果没有这一行，交互命令无法输出最终成熟矩阵。
    except ModuleNotFoundError as error:  # 新增代码+FullMaturityMatrix：兼容 start_oauth_agent.bat 从 learning_agent 目录运行；如果没有这一行，真实可见终端可能导入失败。
        if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.full_maturity_matrix"}:  # 新增代码+FullMaturityMatrix：只兜底包路径缺失；如果没有这一行，矩阵内部 bug 会被误吞。
            raise  # 新增代码+FullMaturityMatrix：重新抛出真实内部错误；如果没有这一行，排查 maturity 命令会被 fallback 干扰。
        from computer_use_mcp_v2.windows_runtime.full_maturity_matrix import computer_use_full_maturity_cli_line, run_computer_use_full_maturity_matrix  # type: ignore  # 新增代码+FullMaturityMatrix：脚本模式导入同一矩阵工具；如果没有这一行，双击 bat 后 `/computer maturity` 不可用。
    return run_computer_use_full_maturity_matrix, computer_use_full_maturity_cli_line  # 新增代码+FullMaturityMatrix：返回矩阵生成函数和格式化器；如果没有这一行，调用方拿不到 Task7 工具。
# 新增代码+FullMaturityMatrix：函数段结束，_phase115_full_maturity_matrix_tools 到此结束；如果没有这个边界说明，初学者不容易看出矩阵导入范围。


def _phase106_launch_action_report(report: dict[str, Any]) -> dict[str, Any]:  # 新增代码+Phase106InteractiveFullLaunch：函数段开始，从 Universal gate 报告中提取 launch_app 动作；如果没有这段函数，Phase106 会被 observe/verify 事件干扰。
    events = dict(report.get("loop", {}) or {}).get("events", [])  # 新增代码+Phase106InteractiveFullLaunch：读取闭环事件列表；如果没有这行代码，无法定位动作阶段。
    for event in events:  # 新增代码+Phase106InteractiveFullLaunch：逐个扫描事件；如果没有这行代码，启动动作报告无法被找到。
        action_result = dict(event.get("action_result", {}) or {}) if isinstance(event, dict) else {}  # 新增代码+Phase106InteractiveFullLaunch：安全读取动作结果；如果没有这行代码，异常事件形状会让命令崩溃。
        if event.get("state") == "acted" and action_result.get("action_class") == "launch_app":  # 新增代码+Phase106InteractiveFullLaunch：只选择真正 acted 阶段的 launch_app；如果没有这行代码，观察事件可能被误当成启动结果。
            return action_result  # 新增代码+Phase106InteractiveFullLaunch：返回启动动作报告；如果没有这行代码，调用方拿不到 Phase105 字段。
    return {}  # 新增代码+Phase106InteractiveFullLaunch：找不到启动动作时返回空字典；如果没有这行代码，阻断场景会抛异常而不是输出原因。
# 新增代码+Phase106InteractiveFullLaunch：函数段结束，_phase106_launch_action_report 到此结束；如果没有这个边界说明，初学者不容易看出动作提取范围。


def _run_phase106_interactive_full_launch(workspace: Path, mode_sessions: ComputerUseModeSessionStore, target_app: str, real_launch: bool | None = None, allow_real_gate: bool | None = None, real_launch_candidate: Any | None = None) -> dict[str, Any]:  # 新增代码+Phase106InteractiveFullLaunch：函数段开始，执行 `/computer launch notepad` 的 Phase106 桥接；如果没有这段函数，用户命令无法接到 Phase105 受控启动链。
    resolve_target, _ = _phase107_launch_target_tools()  # 新增代码+Phase107InteractiveLaunchTarget：读取 Phase107 目标解析器；如果没有这行代码，用户输入仍只能硬编码 Notepad。
    target_report = resolve_target(target_app or "notepad")  # 新增代码+Phase107InteractiveLaunchTarget：解析用户输入目标；如果没有这行代码，calc/mspaint/中文别名和高风险目标无法区分。
    target = str(target_report.get("canonical_target", "") or "").strip().lower()  # 新增代码+Phase107InteractiveLaunchTarget：读取规范目标；如果没有这行代码，后续无法判断是否走 Notepad 真实链路。
    if not bool(target_report.get("safe_known_ordinary_app", False)):  # 新增代码+Phase107InteractiveLaunchTarget：先拦截高风险或未知目标；如果没有这行代码，powershell/cmd 等可能进入 Universal gate。
        if bool(target_report.get("unknown_target_refused", False)) and not bool(target_report.get("high_risk_refused", False)):  # 新增代码+Phase108GenericAppDiscovery：只有 Phase107 未知且不是高风险时才交给通用发现；如果没有这行代码，未知普通应用仍会被硬编码白名单卡住。
            resolve_generic_target, _ = _phase108_generic_app_tools()  # 新增代码+Phase108GenericAppDiscovery：读取 Phase108 通用解析器；如果没有这行代码，交互层无法使用新发现内核。
            generic_report = resolve_generic_target(target_app or "")  # 新增代码+Phase108GenericAppDiscovery：按用户原始目标生成通用发现报告；如果没有这行代码，`/computer launch obsidian` 没有候选和安全计划证据。
            if bool(generic_report.get("passed", False)):  # 新增代码+Phase108GenericAppDiscovery：只有普通且安全的通用目标才进入默认关闭成功路径；如果没有这行代码，高风险或无候选目标可能被误当成成功。
                prepare_phase109_candidate, _ = _phase109_generic_real_launch_tools()  # 新增代码+Phase109GenericRealLaunchCandidate：读取 Phase109 通用真实启动候选准备函数；如果没有这行代码，Phase108 只能发现应用但无法展示统一真实启动候选模型。
                phase109_report = prepare_phase109_candidate(generic_report=generic_report, enable_real_launch=False)  # 新增代码+Phase109GenericRealLaunchCandidate：生成默认关闭的通用真实启动候选报告；如果没有这行代码，`/computer launch obsidian` 看不到身份验证和清理模型已准备好的证据。
                run_phase113_bridge, _ = _phase113_interactive_generic_launch_tools()  # 新增代码+InteractiveGenericLaunchMaturity：读取 Phase113 交互通用启动桥；如果没有这一行，Phase109 候选不会进入 Phase110 通用后端编排。
                phase113_report = run_phase113_bridge(raw_target=target_app or "", generic_report=generic_report, phase109_report=phase109_report)  # 新增代码+InteractiveGenericLaunchMaturity：默认关闭地运行通用后端接线；如果没有这一行，`/computer launch obsidian` 仍只能证明候选准备而不能证明后端接通。
                mode_status = mode_sessions.status()  # 修改代码+FullNaturalUserFlow：读取当前 Computer Use full 模式状态；如果没有这行代码，未知普通应用会绕过 `/computer use --full` 打开的权限状态。
                full_mode_session_used = bool(mode_status.get("full_mode", False)) and not bool(mode_status.get("stopped", False))  # 新增代码+Phase108GenericAppDiscovery：确认 full 模式仍有效且未停止；如果没有这行代码，`/computer stop` 后仍可能显示 launch 成功。
                decision = "phase108_generic_target_default_off" if full_mode_session_used else "blocked_by_mode_session"  # 新增代码+Phase108GenericAppDiscovery：区分 full 后默认关闭和未授权阻断；如果没有这行代码，用户不知道为什么没有真实启动。
                blocked_decision = "" if full_mode_session_used else "action_class_not_allowed_by_mode"  # 新增代码+Phase108GenericAppDiscovery：为未 full 场景补充 action-class 阻断原因；如果没有这行代码，失败输出缺少可排查字段。
                return {"marker": PHASE106_INTERACTIVE_FULL_LAUNCH_MARKER, "ok_token": PHASE106_INTERACTIVE_FULL_LAUNCH_OK_TOKEN, "passed": bool(full_mode_session_used and phase113_report.get("passed", False)), "target_app": str(generic_report.get("canonical_target", "") or target_app or ""), "decision": decision, "real_action_blocked_decision": blocked_decision, "real_launch_requested": bool(phase113_report.get("real_launch_requested", False)), "real_enable_gate_passed": bool(phase113_report.get("real_enable_gate_passed", False)), "controlled_real_launch_gate_passed": False, "full_mode_session_used": full_mode_session_used, "controlled_launch_candidate_ready": False, "generic_launch_backend_ready": bool(phase113_report.get("generic_launch_backend_ready", False)), "generic_real_launch_candidate_ready": bool(phase113_report.get("generic_real_launch_candidate_ready", False)), "real_full_launch_attempted": bool(phase113_report.get("real_full_launch_attempted", False)), "visible_window_verified": False, "cleanup_completed": False, "verified_window_cleanup_completed": False, "residual_owned_process": bool(phase113_report.get("residual_owned_process", False)), "real_desktop_touched": bool(phase113_report.get("real_desktop_touched", False)), "uncontrolled_actions_expanded": bool(phase113_report.get("uncontrolled_actions_expanded", False)), "phase107_target_report": target_report, "phase108_discovery_report": generic_report, "phase109_candidate_report": phase109_report, "phase113_generic_launch_report": phase113_report}  # 修改代码+InteractiveGenericLaunchMaturity：返回未知普通 app 的默认关闭报告并附带 Phase113 通用后端接线；如果没有这一行，用户看不到 `/computer launch <app>` 已经不是逐应用补丁路线。
        return {"marker": PHASE106_INTERACTIVE_FULL_LAUNCH_MARKER, "ok_token": PHASE106_INTERACTIVE_FULL_LAUNCH_OK_TOKEN, "passed": False, "target_app": target or str(target_report.get("normalized_target", "")), "decision": str(target_report.get("decision", "")), "real_action_blocked_decision": str(target_report.get("decision", "")), "real_launch_requested": False, "real_enable_gate_passed": False, "controlled_real_launch_gate_passed": False, "full_mode_session_used": False, "controlled_launch_candidate_ready": False, "real_full_launch_attempted": False, "visible_window_verified": False, "cleanup_completed": False, "verified_window_cleanup_completed": False, "residual_owned_process": False, "real_desktop_touched": False, "uncontrolled_actions_expanded": False, "phase107_target_report": target_report}  # 新增代码+Phase107InteractiveLaunchTarget：返回零副作用拒绝报告；如果没有这行代码，高风险目标会缺少稳定审计字段。
    if not bool(target_report.get("real_launch_supported", False)):  # 新增代码+Phase107InteractiveLaunchTarget：识别普通但暂不支持真实启动的目标；如果没有这行代码，calc/mspaint 可能误走 Notepad smoke。
        mode_status = mode_sessions.status()  # 新增代码+Phase107InteractiveLaunchTarget：读取当前 full mode 状态；如果没有这行代码，calc/mspaint 缺少用户授权检查。
        full_mode_session_used = bool(mode_status.get("full_mode", False)) and not bool(mode_status.get("stopped", False))  # 新增代码+Phase107InteractiveLaunchTarget：判断 full 是否仍有效；如果没有这行代码，stop 后也可能显示安全成功。
        prepare_phase109_candidate, _ = _phase109_generic_real_launch_tools()  # 修改代码+CompatSlimming：读取通用真实启动候选准备函数；如果没有这一行，普通应用被 resolver 识别后会停在识别层而不到通用后端。
        phase109_report = prepare_phase109_candidate(generic_report=target_report, enable_real_launch=False)  # 修改代码+CompatSlimming：生成默认关闭的通用真实启动候选报告；如果没有这一行，`/computer launch <app>` 看不到候选层证据。
        run_phase113_bridge, _ = _phase113_interactive_generic_launch_tools()  # 修改代码+CompatSlimming：读取交互通用启动桥；如果没有这一行，普通应用不会接到统一后端编排。
        phase113_report = run_phase113_bridge(raw_target=target_app or "", generic_report=target_report, phase109_report=phase109_report)  # 修改代码+CompatSlimming：默认关闭地运行通用后端接线；如果没有这一行，工具清单发现后不会到执行器面。
        decision = "windows_app_target_default_off_via_generic_backend" if full_mode_session_used else "blocked_by_mode_session"  # 修改代码+CompatSlimming：生成普通目标默认关闭或 mode 拦截原因；如果没有这行代码，用户不知道为何没启动。
        blocked_decision = "" if full_mode_session_used else "action_class_not_allowed_by_mode"  # 新增代码+Phase107InteractiveLaunchTarget：为未 full 场景补充阻断原因；如果没有这行代码，失败输出不易排查。
        return {"marker": PHASE106_INTERACTIVE_FULL_LAUNCH_MARKER, "ok_token": PHASE106_INTERACTIVE_FULL_LAUNCH_OK_TOKEN, "passed": bool(full_mode_session_used and phase113_report.get("passed", False)), "target_app": target, "decision": decision, "real_action_blocked_decision": blocked_decision, "real_launch_requested": bool(phase113_report.get("real_launch_requested", False)), "real_enable_gate_passed": bool(phase113_report.get("real_enable_gate_passed", False)), "controlled_real_launch_gate_passed": False, "full_mode_session_used": full_mode_session_used, "controlled_launch_candidate_ready": False, "generic_launch_backend_ready": bool(phase113_report.get("generic_launch_backend_ready", False)), "generic_real_launch_candidate_ready": bool(phase113_report.get("generic_real_launch_candidate_ready", False)), "real_full_launch_attempted": bool(phase113_report.get("real_full_launch_attempted", False)), "visible_window_verified": False, "cleanup_completed": False, "verified_window_cleanup_completed": False, "residual_owned_process": bool(phase113_report.get("residual_owned_process", False)), "real_desktop_touched": bool(phase113_report.get("real_desktop_touched", False)), "uncontrolled_actions_expanded": bool(phase113_report.get("uncontrolled_actions_expanded", False)), "phase107_target_report": target_report, "phase108_discovery_report": target_report, "phase109_candidate_report": phase109_report, "phase113_generic_launch_report": phase113_report}  # 修改代码+CompatSlimming：返回普通 app 的默认关闭报告并附带通用后端接线；如果没有这一行，用户命令无法证明 discovery/resolver 已接入执行器。
    Phase105ControlledNotepadSmokeLaunchCandidate, UniversalWindowsLiveExecutionGate = _phase106_live_execution_classes()  # 新增代码+Phase106InteractiveFullLaunch：读取 Phase105 bridge 和 Universal gate 类；如果没有这行代码，命令没有执行主体。
    root = _computer_lock_root(Path(workspace)).parent / "phase106_interactive_full_launch"  # 新增代码+Phase106InteractiveFullLaunch：把 Phase106 证据写到 computer_use memory 下；如果没有这行代码，报告和锁目录会混在一起。
    root.mkdir(parents=True, exist_ok=True)  # 新增代码+Phase106InteractiveFullLaunch：确保证据目录存在；如果没有这行代码，真实 smoke bridge 目录创建可能失败。
    requested = _phase106_env_enabled(PHASE106_INTERACTIVE_FULL_LAUNCH_REQUEST_ENV, real_launch)  # 新增代码+Phase106InteractiveFullLaunch：读取“请求真实启动”门；如果没有这行代码，默认安全模式和真实验收模式无法区分。
    allowed = _phase106_env_enabled(PHASE106_INTERACTIVE_FULL_LAUNCH_ENABLE_ENV, allow_real_gate)  # 新增代码+Phase106InteractiveFullLaunch：读取“允许真实启动”门；如果没有这行代码，单个开关误设可能直接触碰桌面。
    controlled_real_gate = bool(requested and allowed)  # 新增代码+Phase106InteractiveFullLaunch：只有双门同时为真才放行 Phase105 真实分支；如果没有这行代码，Phase106 安全边界会变弱。
    candidate = real_launch_candidate if real_launch_candidate is not None else Phase105ControlledNotepadSmokeLaunchCandidate(base_dir=root / f"smoke_bridge_{int(time.time() * 1000)}")  # 新增代码+Phase106InteractiveFullLaunch：创建 Phase105 受控 Notepad bridge；如果没有这行代码，full 命令无法到达真实窗口验证和清理链。
    runtime = UniversalWindowsLiveExecutionGate(base_dir=root / f"live_gate_{int(time.time() * 1000)}", mode_store=mode_sessions, controlled_launch_candidate=candidate, controlled_real_launch_enabled=controlled_real_gate)  # 修改代码+FullNaturalUserFlow：创建复用当前 mode session 的 Universal gate；如果没有这行代码，命令可能绕过 `/computer use --full` 打开的 full 状态。
    universal_report = runtime.run_prompt("请使用 /computer use --full 启动 notepad，并保持受控。", request_real_actions=True)  # 新增代码+Phase106InteractiveFullLaunch：运行用户风格启动 prompt；如果没有这行代码，Phase105 bridge 不会被实际触发。
    launch_report = _phase106_launch_action_report(universal_report)  # 新增代码+Phase106InteractiveFullLaunch：提取 launch_app 动作报告；如果没有这行代码，后续字段无法从动作层读取。
    controlled_result = dict(launch_report.get("controlled_launch_result", {}) or {})  # 新增代码+Phase106InteractiveFullLaunch：读取 Phase105 候选结果；如果没有这行代码，默认关闭和真实 smoke 字段无法上浮。
    phase104_report = dict(controlled_result.get("phase104_smoke_report", {}) or {})  # 新增代码+Phase106InteractiveFullLaunch：读取 Phase104 Notepad smoke 子报告；如果没有这行代码，窗口验证和清理字段不可见。
    full_mode_session_used = bool(universal_report.get("full_mode_session_used", False))  # 新增代码+Phase106InteractiveFullLaunch：汇总是否真正处于 full session；如果没有这行代码，用户看不出命令是否经过授权模式。
    controlled_launch_candidate_ready = bool(launch_report.get("controlled_launch_candidate_ready") or controlled_result.get("controlled_launch_candidate_ready"))  # 新增代码+Phase106InteractiveFullLaunch：汇总 Phase105 候选是否接入；如果没有这行代码，命令可能只停在记录层。
    controlled_real_launch_gate_passed = bool(launch_report.get("controlled_real_launch_gate_passed", False))  # 新增代码+Phase106InteractiveFullLaunch：汇总 Phase105 真实门是否通过；如果没有这行代码，用户看不出是否进入真实启动分支。
    real_full_launch_attempted = bool(phase104_report.get("real_notepad_launch_attempted", False))  # 新增代码+Phase106InteractiveFullLaunch：汇总真实 Notepad 是否尝试启动；如果没有这行代码，真实验收无法证明最后一跳。
    visible_window_verified = bool(phase104_report.get("visible_window_verified", False))  # 新增代码+Phase106InteractiveFullLaunch：汇总真实窗口是否被验证可见；如果没有这行代码，只有进程没有窗口证据。
    cleanup_completed = bool(phase104_report.get("cleanup_completed", False))  # 新增代码+Phase106InteractiveFullLaunch：汇总清理是否完成；如果没有这行代码，真实验收可能留下 Notepad 窗口。
    verified_window_cleanup_completed = bool(phase104_report.get("verified_window_cleanup_completed", False))  # 新增代码+Phase106InteractiveFullLaunch：汇总已验证窗口是否关闭；如果没有这行代码，Windows 11 Notepad 代理窗口残留风险不可见。
    residual_owned_process = bool(phase104_report.get("residual_owned_process", False))  # 新增代码+Phase106InteractiveFullLaunch：汇总是否残留自有进程；如果没有这行代码，清理失败不可见。
    real_desktop_touched = bool(universal_report.get("real_desktop_touched") or controlled_result.get("real_desktop_touched"))  # 新增代码+Phase106InteractiveFullLaunch：汇总是否触碰真实桌面；如果没有这行代码，默认安全和真实 smoke 无法区分。
    uncontrolled_actions_expanded = bool(universal_report.get("uncontrolled_actions_expanded") or controlled_result.get("uncontrolled_actions_expanded"))  # 新增代码+Phase106InteractiveFullLaunch：确认没有扩大成无边界动作；如果没有这行代码，通用 full 模式可能被误读成任意控制。
    default_safe_passed = bool(full_mode_session_used and controlled_launch_candidate_ready and not controlled_real_gate and not real_full_launch_attempted and not real_desktop_touched and not uncontrolled_actions_expanded)  # 新增代码+Phase106InteractiveFullLaunch：判断默认安全路径是否通过；如果没有这行代码，full 确认后默认关闭状态无法验收。
    real_smoke_passed = bool(full_mode_session_used and controlled_real_gate and controlled_real_launch_gate_passed and real_full_launch_attempted and visible_window_verified and cleanup_completed and verified_window_cleanup_completed and not residual_owned_process and not uncontrolled_actions_expanded)  # 新增代码+Phase106InteractiveFullLaunch：判断真实受控 smoke 是否通过；如果没有这行代码，真实启动可能没有窗口验证或清理也被当成成功。
    passed = bool(default_safe_passed or real_smoke_passed)  # 新增代码+Phase106InteractiveFullLaunch：汇总 Phase106 命令是否通过；如果没有这行代码，formatter 无法决定是否输出 OK token。
    return {"marker": PHASE106_INTERACTIVE_FULL_LAUNCH_MARKER, "ok_token": PHASE106_INTERACTIVE_FULL_LAUNCH_OK_TOKEN, "passed": passed, "target_app": target, "decision": str(universal_report.get("real_action_decision", "")), "real_action_blocked_decision": str(universal_report.get("real_action_blocked_decision", "")), "real_launch_requested": requested, "real_enable_gate_passed": allowed, "controlled_real_launch_gate_passed": controlled_real_launch_gate_passed, "full_mode_session_used": full_mode_session_used, "controlled_launch_candidate_ready": controlled_launch_candidate_ready, "real_full_launch_attempted": real_full_launch_attempted, "visible_window_verified": visible_window_verified, "cleanup_completed": cleanup_completed, "verified_window_cleanup_completed": verified_window_cleanup_completed, "residual_owned_process": residual_owned_process, "real_desktop_touched": real_desktop_touched, "uncontrolled_actions_expanded": uncontrolled_actions_expanded, "phase107_target_report": target_report, "universal_report": universal_report, "launch_report": launch_report, "phase104_smoke_report": phase104_report}  # 修改代码+Phase107InteractiveLaunchTarget：返回完整 Phase106 报告并附带 Phase107 目标解析事实；如果没有这行代码，终端输出无法同时证明目标解析和真实链路。
# 新增代码+Phase106InteractiveFullLaunch：函数段结束，_run_phase106_interactive_full_launch 到此结束；如果没有这个边界说明，初学者不容易看出桥接范围。


def _format_phase106_interactive_full_launch_result(report: dict[str, Any]) -> str:  # 新增代码+Phase106InteractiveFullLaunch：函数段开始，把 Phase106 报告格式化成终端输出；如果没有这段函数，真实用户看不到稳定成功/失败字段。
    ok_token = f" {PHASE106_INTERACTIVE_FULL_LAUNCH_OK_TOKEN}" if bool(report.get("passed", False)) else ""  # 新增代码+Phase106InteractiveFullLaunch：只在通过时追加 OK token；如果没有这行代码，失败输出可能被验收误判成功。
    token_line = f"{PHASE106_INTERACTIVE_FULL_LAUNCH_MARKER}{ok_token} target_app={report.get('target_app', '')} full_mode_session_used={_phase98_bool(report.get('full_mode_session_used', False))} controlled_launch_candidate_ready={_phase98_bool(report.get('controlled_launch_candidate_ready', False))} controlled_real_launch_gate_passed={_phase98_bool(report.get('controlled_real_launch_gate_passed', False))} real_full_launch_attempted={_phase98_bool(report.get('real_full_launch_attempted', False))} visible_window_verified={_phase98_bool(report.get('visible_window_verified', False))} cleanup_completed={_phase98_bool(report.get('cleanup_completed', False))} verified_window_cleanup_completed={_phase98_bool(report.get('verified_window_cleanup_completed', False))} residual_owned_process={_phase98_bool(report.get('residual_owned_process', False))} real_desktop_touched={_phase98_bool(report.get('real_desktop_touched', False))} uncontrolled_actions_expanded={_phase98_bool(report.get('uncontrolled_actions_expanded', False))}"  # 新增代码+Phase106InteractiveFullLaunch：生成固定顺序验收 token 行；如果没有这行代码，可见终端验收会依赖脆弱的多行文本。
    lines = ["Computer Full Launch"]  # 新增代码+Phase106InteractiveFullLaunch：输出固定标题；如果没有这行代码，用户不容易看出这是启动命令结果。
    lines.append(f"- {token_line}")  # 新增代码+Phase106InteractiveFullLaunch：把稳定 token 行放进面板；如果没有这行代码，验收器无法复制最终成功行。
    phase107_report = dict(report.get("phase107_target_report", {}) or {})  # 新增代码+Phase107InteractiveLaunchTarget：读取 Phase107 目标解析报告；如果没有这行代码，终端输出无法显示目标识别证据。
    if phase107_report:  # 新增代码+Phase107InteractiveLaunchTarget：仅在有解析报告时追加 Phase107 token；如果没有这行代码，旧报告格式会访问空字段。
        _, phase107_cli_line = _phase107_launch_target_tools()  # 新增代码+Phase107InteractiveLaunchTarget：读取 Phase107 token 格式化器；如果没有这行代码，formatter 会重复拼接解析字段。
        lines.append(f"- {phase107_cli_line(phase107_report)}")  # 新增代码+Phase107InteractiveLaunchTarget：追加目标解析 token 行；如果没有这行代码，calc/mspaint/高风险拒绝无法被真实终端验收。
    phase108_report = dict(report.get("phase108_discovery_report", {}) or {})  # 新增代码+Phase108GenericAppDiscovery：读取 Phase108 通用发现报告；如果没有这行代码，未知普通 app 的动态发现证据不会显示在终端。
    if phase108_report:  # 新增代码+Phase108GenericAppDiscovery：仅在报告存在时追加 Phase108 token；如果没有这行代码，旧 Notepad/Calc 路径会访问空字段。
        _, phase108_cli_line = _phase108_generic_app_tools()  # 新增代码+Phase108GenericAppDiscovery：读取 Phase108 token 格式化器；如果没有这行代码，formatter 需要手写重复字段。
        lines.append(f"- {phase108_cli_line(phase108_report)}")  # 新增代码+Phase108GenericAppDiscovery：追加通用发现 token 行；如果没有这行代码，真实终端无法证明不需要每 app 白名单。
    phase109_report = dict(report.get("phase109_candidate_report", {}) or {})  # 新增代码+Phase109GenericRealLaunchCandidate：读取 Phase109 通用真实启动候选报告；如果没有这行代码，未知普通 app 的身份验证与清理候选证据不会显示在终端。
    if phase109_report:  # 新增代码+Phase109GenericRealLaunchCandidate：仅在报告存在时追加 Phase109 token；如果没有这行代码，旧 Notepad/Calc 路径会访问空字段。
        _, phase109_cli_line = _phase109_generic_real_launch_tools()  # 新增代码+Phase109GenericRealLaunchCandidate：读取 Phase109 token 格式化器；如果没有这行代码，formatter 需要手写重复字段。
        lines.append(f"- {phase109_cli_line(phase109_report)}")  # 新增代码+Phase109GenericRealLaunchCandidate：追加通用真实启动候选 token 行；如果没有这行代码，真实终端无法证明不是逐应用补丁路线。
    phase113_report = dict(report.get("phase113_generic_launch_report", {}) or {})  # 新增代码+InteractiveGenericLaunchMaturity：读取 Phase113 交互通用启动后端接线报告；如果没有这一行，终端输出不会显示 Task5 成熟证据。
    if phase113_report:  # 新增代码+InteractiveGenericLaunchMaturity：仅在报告存在时追加 Phase113 token；如果没有这一行，Notepad/Calc 旧路径会访问空字段。
        _, phase113_cli_line = _phase113_interactive_generic_launch_tools()  # 新增代码+InteractiveGenericLaunchMaturity：读取 Phase113 token 格式化器；如果没有这一行，formatter 需要手写重复字段。
        lines.append(f"- {phase113_cli_line(phase113_report)}")  # 新增代码+InteractiveGenericLaunchMaturity：追加通用后端接线 token 行；如果没有这一行，真实终端无法证明 `/computer launch <app>` 已到达 Phase110。
    lines.append(f"- decision={report.get('decision', '')}")  # 新增代码+Phase106InteractiveFullLaunch：显示 Universal gate 决策；如果没有这行代码，用户不知道是通过、记录还是拦截。
    lines.append(f"- real_action_blocked_decision={report.get('real_action_blocked_decision', '')}")  # 新增代码+Phase106InteractiveFullLaunch：显示被拦截原因；如果没有这行代码，stop 或未 full 的拒绝原因不可见。
    lines.append(f"- real_launch_requested={_phase98_bool(report.get('real_launch_requested', False))}")  # 新增代码+Phase106InteractiveFullLaunch：显示是否请求真实启动；如果没有这行代码，默认安全和真实验收容易混淆。
    lines.append(f"- real_enable_gate_passed={_phase98_bool(report.get('real_enable_gate_passed', False))}")  # 新增代码+Phase106InteractiveFullLaunch：显示真实启动第二道门；如果没有这行代码，用户看不出为何没有打开应用。
    lines.append(f"- real_desktop_touched={_phase98_bool(report.get('real_desktop_touched', False))}")  # 新增代码+Phase106InteractiveFullLaunch：再次明确是否触碰桌面；如果没有这行代码，安全边界不够醒目。
    return "\n".join(lines) + "\n"  # 新增代码+Phase106InteractiveFullLaunch：返回完整终端文本；如果没有这行代码，调用方无法打印或写入验收事件。
# 新增代码+Phase106InteractiveFullLaunch：函数段结束，_format_phase106_interactive_full_launch_result 到此结束；如果没有这个边界说明，初学者不容易看出输出范围。
...
def run_computer_terminal_command(workspace: Path, user_input: str) -> str:  # 新增代码+Phase31ComputerUseLockAbortEvidence: 函数段开始，执行 `/computer` 子命令；如果没有这段函数，真实用户无法在终端查看或触发桌面急停。
    ...
    if subcommand == "parity":  # 新增代码+Phase53ParityGapMatrix: 识别 Phase53 差距矩阵命令；如果没有这行代码，用户无法在真实终端直接查看 ClaudeCode 对齐差距。
        try:  # 新增代码+Phase53ParityGapMatrix: 优先按包路径导入 Phase53 渲染入口；如果没有这行代码，项目根目录运行时无法加载标准包模块。
            from learning_agent.computer_use_mcp_v2.windows_runtime.parity_gap_matrix import format_phase53_parity_gap_lines, run_phase53_parity_gap_matrix_contract  # 新增代码+Phase53ParityGapMatrix: 导入只读差距矩阵渲染和合同函数；如果没有这行代码，/computer parity 没有报告来源。
            ...
            if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.parity_gap_matrix"}:  # 新增代码+Phase53ParityGapMatrix: 只吞掉预期的包路径缺失；如果没有这行代码，真实内部 bug 会被错误隐藏。
                raise  # 新增代码+Phase53ParityGapMatrix: 重新抛出非路径类导入错误；如果没有这行代码，排查 Phase53 模块问题会很困难。
            from computer_use_mcp_v2.windows_runtime.parity_gap_matrix import format_phase53_parity_gap_lines, run_phase53_parity_gap_matrix_contract  # 新增代码+Phase53ParityGapMatrix: 脚本模式导入 Phase53 渲染和合同函数；如果没有这行代码，双击 bat 后 /computer parity 不可用。
    ...
    if subcommand in {"native-gate", "native-reality", "dependencies"}:  # 新增代码+Phase54WindowsNativeRealityGate: 识别 Windows 原生依赖现实门禁命令；如果没有这行代码，用户无法在真实终端看到依赖缺口和下一步。
        try:  # 新增代码+Phase54WindowsNativeRealityGate: 优先按包路径导入 Phase54 渲染入口；如果没有这行代码，项目根目录运行时无法加载标准包模块。
            from learning_agent.computer_use_mcp_v2.windows_runtime.native_reality_gate import format_phase54_native_reality_gate_lines, run_phase54_native_reality_gate_contract  # 新增代码+Phase54WindowsNativeRealityGate: 导入只读现实门禁渲染和合同函数；如果没有这行代码，/computer native-gate 没有报告来源。
            ...
            if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.native_reality_gate"}:  # 新增代码+Phase54WindowsNativeRealityGate: 只吞掉预期的包路径缺失；如果没有这行代码，真实内部 bug 会被错误隐藏。
                raise  # 新增代码+Phase54WindowsNativeRealityGate: 重新抛出非路径类导入错误；如果没有这行代码，排查 Phase54 模块问题会很困难。
            from computer_use_mcp_v2.windows_runtime.native_reality_gate import format_phase54_native_reality_gate_lines, run_phase54_native_reality_gate_contract  # 新增代码+Phase54WindowsNativeRealityGate: 脚本模式导入 Phase54 渲染和合同函数；如果没有这行代码，双击 bat 后 /computer native-gate 不可用。
    ...
    if subcommand == "maturity":  # 新增代码+FullMaturityMatrix：识别 `/computer maturity` 最终成熟矩阵命令；如果没有这一行，用户无法直接查看蓝图收敛结果。
        run_maturity_matrix, maturity_cli_line = _phase115_full_maturity_matrix_tools()  # 新增代码+FullMaturityMatrix：读取最终矩阵工具；如果没有这一行，命令没有结构化报告来源。
        maturity_report = run_maturity_matrix()  # 新增代码+FullMaturityMatrix：运行无真实桌面副作用的矩阵；如果没有这一行，命令无法输出最终成熟字段。
        maturity_line = maturity_cli_line(maturity_report)  # 新增代码+FullMaturityMatrix：生成稳定 token 行；如果没有这一行，真实终端验收无法快速匹配成熟结果。
        maturity_reports = maturity_report.get("reports", {}) if isinstance(maturity_report.get("reports", {}), dict) else {}  # 新增代码+WindowsMultiBackendLaunchExecutor：读取矩阵子报告集合；如果没有这一行，Phase110 子报告只能藏在很长 JSON 里。
        launch_backend_report = maturity_reports.get("generic_real_launch", {}) if isinstance(maturity_reports.get("generic_real_launch", {}), dict) else {}  # 新增代码+WindowsMultiBackendLaunchExecutor：读取 Phase110 通用启动后端子报告；如果没有这一行，真实终端断言容易因为 JSON 截断看不到多后端事实。
        phase110_line = f"{launch_backend_report.get('marker', '')} authorized_argv_backend={_phase98_bool(launch_backend_report.get('authorized_argv_backend', False))} authorized_appx_backend={_phase98_bool(launch_backend_report.get('authorized_appx_backend', False))} authorized_shortcut_backend={_phase98_bool(launch_backend_report.get('authorized_shortcut_backend', False))} multi_backend_dispatch_ready={_phase98_bool(launch_backend_report.get('multi_backend_dispatch_ready', False))} real_desktop_touched={_phase98_bool(launch_backend_report.get('real_desktop_touched', False))}"  # 新增代码+WindowsMultiBackendLaunchExecutor：生成短 Phase110 摘要行；如果没有这一行，controller 只能在超长 report_json 中寻找 AppX/shortcut 后端证据。
        maturity_json = json.dumps(maturity_report, ensure_ascii=False, sort_keys=True)  # 新增代码+FullMaturityMatrix：生成完整 JSON 便于排查；如果没有这一行，失败时只能看到短 token。
        return f"Computer Use Full Maturity\n- {maturity_line}\n- {phase110_line}\n- report_json={maturity_json}\n"  # 修改代码+WindowsMultiBackendLaunchExecutor：返回最终矩阵面板并把 Phase110 多后端摘要放在长 JSON 前；如果没有这一行，真实终端验收会因 payload 截断误判失败。
    if subcommand == "use":  # 新增代码+Phase98UniversalComputerUseMode: 识别 `/computer use` 模式命令；如果没有这行代码，用户无法从真实终端打开通用 Computer Use 模式。
        ...
        if "--full" in use_tokens_lower:  # 修改代码+FullNaturalUserFlow: 识别用户明确输入的 full 命令；如果没有这行代码，普通用户的一行授权入口会继续被当成普通 normal 模式。
            stale_abort_present = bool(lock_manager.status().get("abort_requested", False))  # 新增代码+FullModeAutoClearAbort: 检查历史急停是否仍在阻断动作；如果没有这行代码，普通用户必须先知道并输入 clear-abort。
            if stale_abort_present:  # 新增代码+FullModeAutoClearAbort: 只有确实存在历史急停时才自动清理；如果没有这行代码，每次 full 都会写恢复记录导致审计噪音。
                lock_manager.clear_abort(cleared_by="terminal /computer use --full")  # 新增代码+FullModeAutoClearAbort: 为新的 full 会话清除旧急停标记；如果没有这行代码，用户只输入 full 后后续动作仍会被旧 abort 拦住。
            mode_sessions.open_full_mode(reason="terminal /computer use --full")  # 修改代码+FullModeAutoClearAbort: 直接打开短 TTL 的 full 状态；如果没有这行代码，用户还会被迫输入不自然的动态确认命令。
            mode_result = dict(mode_sessions.status())  # 新增代码+FullModeAutoClearAbort: 复制 mode 状态以追加恢复字段；如果没有这行代码，formatter 看不到本次是否做了自动清理。
            mode_result["abort_auto_cleared"] = stale_abort_present  # 新增代码+FullModeAutoClearAbort: 把自动清理结果显式写进输出；如果没有这行代码，真实终端验收无法证明用户不需要 clear-abort。
            return _format_computer_mode_result(mode_result)  # 修改代码+FullModeAutoClearAbort: 返回统一模式面板和可选自动清理字段；如果没有这行代码，终端和验收脚本仍会看到 full-confirm 提示或缺恢复证据。
    ...
    if subcommand in {"launch", "open"}:  # 新增代码+Phase106InteractiveFullLaunch：识别 `/computer launch notepad` 用户启动命令；如果没有这行代码，Phase105 能力只能通过合同函数调用，真实用户命令无法触发。
        target_app = command_parts[2].strip() if len(command_parts) >= 3 else "notepad"  # 新增代码+Phase106InteractiveFullLaunch：读取目标应用，默认使用 Notepad；如果没有这行代码，缺参数会变成空目标并被错误拒绝。
        phase106_report = _run_phase106_interactive_full_launch(Path(workspace), mode_sessions, target_app)  # 修改代码+FullNaturalUserFlow：复用当前 workspace 和 mode session 运行受控启动桥；如果没有这行代码，命令不会经过 `/computer use --full` 打开的 full 状态。
        return _format_phase106_interactive_full_launch_result(phase106_report)  # 新增代码+Phase106InteractiveFullLaunch：返回稳定终端输出；如果没有这行代码，用户和验收器看不到 Phase106 token 行。
    ...
    if subcommand == "stop":  # 新增代码+Phase98UniversalComputerUseMode: 识别 `/computer stop` 模式停止命令；如果没有这行代码，用户无法用新模式入口终止后续动作。
        ...
        cleanup_result = runtime.cleanup_turn(reason=f"terminal stop cleanup: {reason}")  # 新增代码+TurnCleanupLifecycleRootRemediation：stop 后立即执行 turn cleanup 清锁和清 abort；如果没有这一行，/computer stop 会留下 abort_requested=true 阻塞下一轮。
        stop_result = {**dict(stop_result), "cleanup_completed": bool(cleanup_result.get("cleanup_completed", False)), "lock_released": bool(cleanup_result.get("lock_released", False)), "abort_cleared": bool(cleanup_result.get("abort_cleared", False)), "owned_resource_cleanup_completed": bool(abort_result.get("owned_resource_cleanup_completed", False) or cleanup_result.get("owned_resource_cleanup_completed", False)), "residual_owned_process": bool(abort_result.get("residual_owned_process", False) or cleanup_result.get("residual_owned_process", False))}  # 修改代码+TurnCleanupLifecycleRootRemediation：把 runtime cleanup 结果合并进 stop 面板；如果没有这一行，用户看不到 stop 是否完成完整 turn cleanup。
        return _format_computer_stop_result(stop_result) + _format_computer_lock_action("stop", {"status": cleanup_result.get("status", cleanup_result.get("abort_clear", {}))}) + format_phase40_runtime_action("stop", cleanup_result)  # 修改代码+TurnCleanupLifecycleRootRemediation: 同时输出 mode stop 和 cleanup 后 runtime 证据；如果没有这行代码，用户和回归测试看不到 abort 已清除。
    ...
    if subcommand == "observe":  # 新增代码+Phase51ComputerStatusUI: 识别只读观察命令；如果没有这行代码，用户无法从 `/computer` 面板安全获取窗口观察入口。
        ...
        return observe_result.to_text("mcp__computer-use__observe") + "\n"  # 修改代码+ComputerUseMcpV2ResidualCleanup：/computer observe 也显示 v2 MCP observe 名称；如果没有这行代码，真实终端输出会继续把用户和模型排查带回旧 computer_observe。
    ...
    return "Computer Action\n- 不支持的 /computer 子命令。\n- 可用命令：/computer、/computer status、/computer maturity、/computer use、/computer use --observe、/computer use --full、/computer launch <app>、/computer stop、/computer permissions、/computer parity、/computer native-gate、/computer abort-hooks、/computer high-level-tools、/computer controller、/computer observe [action]、/computer grant <app> [flags]、/computer approve <app> [flags] ttl=60 scope=click、/computer grants、/computer deny <app>、/computer revoke <app>、/computer abort <reason>、/computer clear-abort、/computer cleanup [session_id]、/computer recover、/computer journal、/computer notifications、/computer release [session_id]\n"  # 修改代码+FullNaturalUserFlow：从普通帮助里移除 full-confirm，引导用户使用自然的一行 full 命令；如果没有这行代码，新手用户仍会被旧 token 用法误导。
...
def _reset_computer_use_session_context_for_agent(agent: Any) -> None:  # 新增代码+ComputerUseSessionTargetGuard：函数段开始，清理上一轮 Computer Use 会话残留上下文；如果没有这段函数，旧窗口截图和旧桌面动作会继续进入新任务模型主循环。
    observation_events = getattr(agent, "observation_events", None)  # 新增代码+ComputerUseSessionTargetGuard：读取 agent 的结构化观察列表；如果没有这一行，helper 无法清理旧 computer_use 事件。
    if isinstance(observation_events, list):  # 新增代码+ComputerUseSessionTargetGuard：只在字段确实是列表时处理；如果没有这一行，测试替身或异常 agent 可能因为类型不符崩溃。
        agent.observation_events = [event for event in observation_events if not str(event.get("kind", "") if isinstance(event, dict) else "").startswith("computer_use")]  # 新增代码+ComputerUseSessionTargetGuard：删除旧 Computer Use 观察/动作/模式事件但保留普通开发证据；如果没有这一行，旧窗口截图会污染新 full 任务。
    active_artifacts = getattr(agent, "active_artifacts", None)  # 新增代码+ComputerUseSessionTargetGuard：读取当前活跃产物列表；如果没有这一行，旧截图 PNG/BMP 仍可能被会话摘要引用。
    if isinstance(active_artifacts, list):  # 新增代码+ComputerUseSessionTargetGuard：只在产物字段是列表时处理；如果没有这一行，外部轻量 agent 可能因为字段类型不同而失败。
        agent.active_artifacts = [artifact for artifact in active_artifacts if "computer-window" not in str(artifact).lower() and "computer_use" not in str(artifact).lower()]  # 新增代码+ComputerUseSessionTargetGuard：移除旧 Computer Use 截图/证据产物但保留普通文件产物；如果没有这一行，模型恢复摘要可能继续看到旧桌面图片路径。
    run_helpers_from_core.safe_record_observation(agent, "computer_use_session_context_reset", {"old_computer_use_events_cleared": True, "old_computer_use_artifacts_cleared": True})  # 修改代码+AgentPySplitPhase15B2：通过 core helper 记录 full 会话清理证据；如果没有这一行，删除 `旧观察薄包装` 后真实终端排查不知道新会话是否做过隔离。
# 新增代码+ComputerUseSessionTargetGuard：函数段结束，_reset_computer_use_session_context_for_agent 到此结束；如果没有这个边界说明，读者不容易看出会话清理范围。


def _sync_computer_use_agent_scope_from_terminal_mode(agent: Any, normalized_command: str, computer_output: str) -> str:  # 新增代码+ComputerUseTerminalScopeSync：函数段开始，把 `/computer use` 的终端状态同步到当前 agent 工具池；如果没有这段函数，真实终端打开 full 后模型下一轮仍会按代码模式隐藏 MCP 鼠标键盘工具。
    result = activate_computer_use_mcp_v2_terminal_mode(agent, normalized_command, computer_output, lambda kind, payload: run_helpers_from_core.safe_record_observation(agent, kind, payload))  # 修改代码+ComputerUseMcpV2TerminalBridge：委托 v2 bridge 写入工具池和桌面上下文；如果没有这一行，/computer use 仍会绕过新的 ClaudeCode-style 接线层。
    return str(result.get("mode", "normal"))  # 修改代码+ComputerUseMcpV2TerminalBridge：保留旧函数返回模式名的合同；如果没有这一行，调用方和测试无法稳定读取同步后的模式。
# 新增代码+ComputerUseTerminalScopeSync：函数段结束，_sync_computer_use_agent_scope_from_terminal_mode 到此结束；如果没有这个边界说明，用户不容易看出它只同步 agent 工具池状态。


def _reset_computer_use_agent_scope_after_terminal_stop(agent: Any, normalized_command: str, computer_output: str) -> str:  # 新增代码+ComputerUseTerminalScopeSync：函数段开始，在 `/computer stop` 后恢复 agent 工具池边界；如果没有这段函数，停止 full 后模型仍可能看到 Computer Use MCP 工具。
    result = deactivate_computer_use_mcp_v2_terminal_mode(agent, normalized_command, computer_output, lambda kind, payload: run_helpers_from_core.safe_record_observation(agent, kind, payload))  # 修改代码+ComputerUseMcpV2TerminalBridge：委托 v2 bridge 关闭工具池和桌面上下文；如果没有这一行，/computer stop 仍会绕过新的 ClaudeCode-style 接线层。
    if not result.get("changed", False):  # 修改代码+ComputerUseMcpV2TerminalBridge：只有 v2 bridge 确认发生 stop 同步时才打印结果；如果没有这一行，非 stop 命令也会产生误导输出。
        return ""  # 修改代码+ComputerUseMcpV2TerminalBridge：未同步时保持安静；如果没有这一行，普通 /computer 命令会出现错误的工具池重置提示。
    return "Computer Use Tool Pack\n- model_loop_tools_loaded=false\n- selection_result=Computer Use terminal stop reset agent tool scope to auto.\n"  # 新增代码+ComputerUseTerminalScopeSync：把重置结果打印给真实终端和验收事件；如果没有这一行，用户看不到 stop 已同步到模型工具池。
# 新增代码+ComputerUseTerminalScopeSync：函数段结束，_reset_computer_use_agent_scope_after_terminal_stop 到此结束；如果没有这个边界说明，用户不容易看出 stop 清理范围。


def _activate_computer_use_tool_pack_for_agent(agent: Any, user_input: str, computer_output: str) -> str:  # 新增代码+ModelLoopComputerUse：函数段开始，把终端 `/computer use` 模式和模型主循环工具池接上；如果没有这段函数，用户打开 full 模式后模型仍看不到 computer_use 工具。
    normalized_command = str(user_input or "").strip().lower()  # 新增代码+ModelLoopComputerUse：把用户命令转成稳定小写文本；如果没有这一行，`/computer use --full` 的大小写或空白变化会导致工具包漏加载。
    stop_reset_output = _reset_computer_use_agent_scope_after_terminal_stop(agent, normalized_command, computer_output)  # 新增代码+ComputerUseTerminalScopeSync：先处理 `/computer stop` 的工具池恢复；如果没有这一行，stop 命令不会同步清理 agent 内存工具面。
    if stop_reset_output:  # 新增代码+ComputerUseTerminalScopeSync：判断 stop 是否已经完成同步并生成输出；如果没有这一行，stop 结果会继续落入 use 模式加载判断。
        return stop_reset_output  # 新增代码+ComputerUseTerminalScopeSync：返回 stop 的工具池重置说明；如果没有这一行，真实终端看不到工具池已关闭。
    if not (normalized_command == "/computer use" or normalized_command == "computer use" or normalized_command.startswith("/computer use ") or normalized_command.startswith("computer use ")):  # 新增代码+ModelLoopComputerUse：只响应真正的 use 模式命令；如果没有这一行，`/computer status` 这类只读命令也会意外扩展模型工具面。
        return ""  # 新增代码+ModelLoopComputerUse：非 use 命令不输出额外文本；如果没有这一行，普通 `/computer` 状态会被误判为工具包加载。
    if "full_mode=true" not in computer_output and "mode=full" not in computer_output and "mode=normal" not in computer_output and "mode=observe" not in computer_output:  # 新增代码+ModelLoopComputerUse：要求终端命令确实打开了某种 Computer Use 模式；如果没有这一行，失败输出也会让模型误以为可以操作桌面。
        return ""  # 新增代码+ModelLoopComputerUse：模式未打开时保持安静；如果没有这一行，失败命令会产生误导性的工具加载提示。
    if "--full" in normalized_command and ("full_mode=true" in computer_output or "mode=full" in computer_output):  # 新增代码+ComputerUseSessionTargetGuard：只在用户打开 full 新会话时清理旧 Computer Use 上下文；如果没有这一行，普通 observe/normal 模式也可能过度清理。
        _reset_computer_use_session_context_for_agent(agent)  # 新增代码+ComputerUseSessionTargetGuard：执行旧窗口截图和旧桌面事件清理；如果没有这一行，新 full 任务仍可能继承上一个任务的视觉证据。
    synced_mode = _sync_computer_use_agent_scope_from_terminal_mode(agent, normalized_command, computer_output)  # 新增代码+ComputerUseTerminalScopeSync：在加载工具包前先把 agent 切到 Computer Use scope；如果没有这一行，tool_search 会继续把 MCP 原子工具判为 scope_blocked。
    selection_result = str(search_tools_from_tools.tool_search_select_pack(agent, "computer_use"))  # 修改代码+AgentPyCompatWrapperRemovalL4: 直接通过 tools/search.py 加载 computer_use 能力包；若没有这行代码，删除 agent._tool_search_select_pack 后 full 模式会无法把桌面工具放进模型工具池。
    run_helpers_from_core.safe_record_observation(agent, "computer_use_tool_pack_loaded_from_terminal_command", {"command": normalized_command, "mode": synced_mode, "selection_result": selection_result[:1200]})  # 修改代码+ComputerUseTerminalScopeSync：记录命令、同步模式和加载结果摘要；如果没有这一行，真实终端排查无法知道工具包加载前是否完成 scope 切换。
    loaded_state = "true" if "tool_search 成功" in selection_result else "false"  # 新增代码+ModelLoopComputerUse：把中文加载结果压成稳定布尔字段；如果没有这一行，验收脚本只能解析长文本。
    return "Computer Use Tool Pack\n- model_loop_tools_loaded=" + loaded_state + "\n- selection_result=" + selection_result.replace("\n", " | ")[:3000] + "\n"  # 新增代码+ModelLoopComputerUse：把工具包加载状态打印到真实终端；如果没有这一行，用户看不到 full 模式已进入模型主循环工具面。
# 新增代码+ModelLoopComputerUse：函数段结束，_activate_computer_use_tool_pack_for_agent 到此结束；如果没有这个边界说明，代码小白不容易看出终端命令和模型工具面的接线范围。


def phase106_main(argv: list[str] | None = None) -> int:  # 新增代码+Phase106InteractiveFullLaunch：函数段开始，提供真实可见终端一键验收入口；如果没有这段函数，Phase106 验收需要人工复制动态 token，容易出错。
    _ = argv  # 新增代码+Phase106InteractiveFullLaunch：明确当前不解析命令行参数；如果没有这行代码，读者可能误以为 argv 被忘记处理。
    workspace = Path(__file__).resolve().parents[1]  # 新增代码+Phase106InteractiveFullLaunch：默认使用 learning_agent 目录作为真实终端 workspace；如果没有这行代码，bat 入口和项目根入口可能写到不同 memory。
    full_request_output = run_computer_terminal_command(workspace, "/computer use --full")  # 修改代码+FullNaturalUserFlow：第一步按真实用户的一行命令直接打开 full；如果没有这行代码，验收会继续依赖反直觉的动态 token。
    print(full_request_output, end="")  # 修改代码+FullNaturalUserFlow：把 full 打开结果打印到真实终端；如果没有这行代码，用户看不到 full_mode=true 证据。
    launch_output = run_computer_terminal_command(workspace, "/computer launch notepad")  # 新增代码+Phase106InteractiveFullLaunch：第三步运行新增用户启动命令；如果没有这行代码，验收无法证明 Phase106 交互入口接到 Phase105。
    print(launch_output, end="")  # 新增代码+Phase106InteractiveFullLaunch：打印启动命令结果；如果没有这行代码，验收器无法读取 Phase106 token 行。
    stop_output = run_computer_terminal_command(workspace, "/computer stop Phase106 acceptance cleanup")  # 新增代码+Phase106InteractiveFullLaunch：验收后停止 full mode 并触发急停保护；如果没有这行代码，真实终端可能留下 full 权限状态。
    print(stop_output, end="")  # 新增代码+Phase106InteractiveFullLaunch：打印停止结果；如果没有这行代码，用户看不到权限已经回收。
    return 0 if PHASE106_INTERACTIVE_FULL_LAUNCH_OK_TOKEN in launch_output else 1  # 新增代码+Phase106InteractiveFullLaunch：根据启动输出决定退出码；如果没有这行代码，失败验收也可能被命令行当成成功。
# 新增代码+Phase106InteractiveFullLaunch：函数段结束，phase106_main 到此结束；如果没有这个边界说明，初学者不容易看出一键验收范围。


def phase107_main(argv: list[str] | None = None) -> int:  # 新增代码+Phase107InteractiveLaunchTarget：函数段开始，提供 Phase107 真实可见终端验收入口；如果没有这段函数，目标解析只能靠单元测试不能满足真实终端门禁。
    _ = argv  # 新增代码+Phase107InteractiveLaunchTarget：明确当前不解析命令行参数；如果没有这行代码，读者可能误以为 argv 被遗漏处理。
    workspace = Path(__file__).resolve().parents[1]  # 新增代码+Phase107InteractiveLaunchTarget：默认使用 learning_agent 目录作为真实终端 workspace；如果没有这行代码，bat 入口和测试入口可能写到不同 memory。
    resolve_target, _ = _phase107_launch_target_tools()  # 新增代码+Phase107InteractiveLaunchTarget：读取 Phase107 解析器用于最终 token 常量；如果没有这行代码，验收入口会硬编码 marker。
    marker = str(resolve_target("calc").get("marker", ""))  # 新增代码+Phase107InteractiveLaunchTarget：从解析报告读取 marker；如果没有这行代码，最终验收行可能和模块常量漂移。
    ok_token = str(resolve_target("calc").get("ok_token", ""))  # 新增代码+Phase107InteractiveLaunchTarget：从解析报告读取 OK token；如果没有这行代码，最终验收行可能和模块常量漂移。
    full_request_output = run_computer_terminal_command(workspace, "/computer use --full")  # 修改代码+FullNaturalUserFlow：按真实用户的一行命令直接打开 full；如果没有这行代码，calc 启动默认关闭路径会缺少授权前置。
    print(full_request_output, end="")  # 修改代码+FullNaturalUserFlow：打印 full 打开证据；如果没有这行代码，真实终端看不到 full_mode=true。
    calc_output = run_computer_terminal_command(workspace, "/computer launch calc")  # 新增代码+Phase107InteractiveLaunchTarget：运行普通目标 calc 识别路径；如果没有这行代码，验收不能证明非 Notepad 普通目标被解析。
    print(calc_output, end="")  # 新增代码+Phase107InteractiveLaunchTarget：打印 calc 解析结果；如果没有这行代码，验收器无法匹配 calc token。
    powershell_output = run_computer_terminal_command(workspace, "/computer launch powershell")  # 新增代码+Phase107InteractiveLaunchTarget：运行高风险目标拒绝路径；如果没有这行代码，验收不能证明 powershell 被拦截。
    print(powershell_output, end="")  # 新增代码+Phase107InteractiveLaunchTarget：打印高风险拒绝结果；如果没有这行代码，验收器无法匹配 high_risk_refused=true。
    stop_output = run_computer_terminal_command(workspace, "/computer stop Phase107 acceptance cleanup")  # 新增代码+Phase107InteractiveLaunchTarget：验收后停止 full mode 并触发急停保护；如果没有这行代码，真实终端可能留下 full 状态。
    print(stop_output, end="")  # 新增代码+Phase107InteractiveLaunchTarget：打印停止结果；如果没有这行代码，用户看不到权限已回收。
    calc_recognized_default_off = bool(ok_token in calc_output and "canonical_target=calc" in calc_output and "real_launch_supported=false" in calc_output and "real_full_launch_attempted=false" in calc_output)  # 新增代码+Phase107InteractiveLaunchTarget：汇总 calc 已识别且未真实启动；如果没有这行代码，最终 token 无法表达普通目标默认关闭。
    high_risk_refused = bool(("decision=windows_app_high_risk_target_refused" in powershell_output or "decision=phase107_high_risk_target_refused" in powershell_output) and "high_risk_refused=true" in powershell_output)  # 修改代码+CompatSlimming：汇总 powershell 是否被统一 resolver 高风险拒绝；如果没有这行代码，最终 token 无法表达安全拒绝。
    powershell_zero_side_effect = bool("real_full_launch_attempted=false" in powershell_output and "real_desktop_touched=false" in powershell_output and "uncontrolled_actions_expanded=false" in powershell_output)  # 新增代码+Phase107InteractiveLaunchTarget：汇总高风险拒绝是否零副作用；如果没有这行代码，拒绝路径可能仍有桌面动作。
    passed = bool(calc_recognized_default_off and high_risk_refused and powershell_zero_side_effect)  # 新增代码+Phase107InteractiveLaunchTarget：汇总 Phase107 验收是否通过；如果没有这行代码，CLI 退出码没有依据。
    final_ok = f" {ok_token}" if passed else ""  # 新增代码+Phase107InteractiveLaunchTarget：仅通过时输出 OK token；如果没有这行代码，失败也可能被验收误判成功。
    print(f"{marker}{final_ok} calc_recognized_default_off={_phase98_bool(calc_recognized_default_off)} high_risk_refused={_phase98_bool(high_risk_refused)} powershell_zero_side_effect={_phase98_bool(powershell_zero_side_effect)} real_desktop_touched=false uncontrolled_actions_expanded=false")  # 新增代码+Phase107InteractiveLaunchTarget：打印最终固定 token 行；如果没有这行代码，真实终端最终回答缺少稳定可复制锚点。
    return 0 if passed else 1  # 新增代码+Phase107InteractiveLaunchTarget：用退出码表达验收成败；如果没有这行代码，失败也可能被自动化当作成功。
# 新增代码+Phase107InteractiveLaunchTarget：函数段结束，phase107_main 到此结束；如果没有这个边界说明，初学者不容易看出 Phase107 验收范围。


def phase108_main(argv: list[str] | None = None) -> int:  # 新增代码+Phase108GenericAppDiscovery：函数段开始，提供 Phase108 真实可见终端验收入口；如果没有这段函数，通用发现只能靠单元测试不能满足真实终端门禁。
    _ = argv  # 新增代码+Phase108GenericAppDiscovery：明确当前不解析命令行参数；如果没有这行代码，读者可能误以为 argv 被忘记处理。
    workspace = Path(__file__).resolve().parents[1]  # 新增代码+Phase108GenericAppDiscovery：默认使用 learning_agent 目录作为真实终端 workspace；如果没有这行代码，bat 入口和测试入口可能写到不同 memory。
    resolve_generic_target, _ = _phase108_generic_app_tools()  # 新增代码+Phase108GenericAppDiscovery：读取 Phase108 解析器用于最终 token 常量；如果没有这行代码，验收入口会硬编码 marker。
    marker = str(resolve_generic_target("obsidian").get("marker", ""))  # 新增代码+Phase108GenericAppDiscovery：从通用报告读取 marker；如果没有这行代码，最终验收行可能和模块常量漂移。
    ok_token = str(resolve_generic_target("obsidian").get("ok_token", ""))  # 新增代码+Phase108GenericAppDiscovery：从通用报告读取 OK token；如果没有这行代码，最终验收行可能和模块常量漂移。
    full_request_output = run_computer_terminal_command(workspace, "/computer use --full")  # 修改代码+FullNaturalUserFlow：按真实用户的一行命令直接打开 full；如果没有这行代码，generic launch 默认关闭路径缺少授权前置。
    print(full_request_output, end="")  # 修改代码+FullNaturalUserFlow：打印 full 打开证据；如果没有这行代码，真实终端看不到 full_mode=true。
    generic_output = run_computer_terminal_command(workspace, "/computer launch obsidian")  # 新增代码+Phase108GenericAppDiscovery：运行未知普通应用通用发现路径；如果没有这行代码，验收不能证明不再需要每 app 白名单。
    print(generic_output, end="")  # 新增代码+Phase108GenericAppDiscovery：打印通用发现结果；如果没有这行代码，验收器无法匹配 Phase108 token。
    powershell_output = run_computer_terminal_command(workspace, "/computer launch powershell")  # 新增代码+Phase108GenericAppDiscovery：运行高风险目标拒绝路径；如果没有这行代码，验收不能证明通用发现没有放开 PowerShell。
    print(powershell_output, end="")  # 新增代码+Phase108GenericAppDiscovery：打印高风险拒绝结果；如果没有这行代码，验收器无法匹配 high_risk_refused=true。
    stop_output = run_computer_terminal_command(workspace, "/computer stop Phase108 acceptance cleanup")  # 新增代码+Phase108GenericAppDiscovery：验收后停止 full mode 并触发急停保护；如果没有这行代码，真实终端可能留下 full 状态。
    print(stop_output, end="")  # 新增代码+Phase108GenericAppDiscovery：打印停止结果；如果没有这行代码，用户看不到权限已回收。
    generic_target_default_off = bool(ok_token in generic_output and "interactive_target_resolved=true" in generic_output and "safe_known_ordinary_app=true" in generic_output and "real_launch_supported=false" in generic_output and "real_full_launch_attempted=false" in generic_output and "real_desktop_touched=false" in generic_output)  # 修改代码+CompatSlimming：汇总普通应用是否由统一 resolver 接收但默认不真实启动；如果没有这行代码，最终 token 无法证明旧 discovery 已并入新解析器。
    high_risk_refused = bool(("decision=windows_app_high_risk_target_refused" in powershell_output or "decision=phase107_high_risk_target_refused" in powershell_output) and "high_risk_refused=true" in powershell_output)  # 修改代码+CompatSlimming：汇总 powershell 是否仍被高风险拒绝；如果没有这行代码，通用发现可能被误解成放开所有系统工具。
    high_risk_zero_side_effect = bool("real_full_launch_attempted=false" in powershell_output and "real_desktop_touched=false" in powershell_output and "uncontrolled_actions_expanded=false" in powershell_output)  # 新增代码+Phase108GenericAppDiscovery：汇总高风险拒绝是否零副作用；如果没有这行代码，拒绝路径可能仍触碰桌面。
    passed = bool(generic_target_default_off and high_risk_refused and high_risk_zero_side_effect)  # 修改代码+Phase108GenericAppDiscovery：汇总 Phase108 验收是否通过；如果没有这行代码，CLI 退出码没有依据。
    final_ok = f" {ok_token}" if passed else ""  # 新增代码+Phase108GenericAppDiscovery：仅通过时输出 OK token；如果没有这行代码，失败也可能被验收误判成功。
    print(f"{marker}{final_ok} dynamic_discovery_used=true hardcoded_app_whitelist_required=false per_app_patch_required=false generic_target_default_off={_phase98_bool(generic_target_default_off)} high_risk_refused={_phase98_bool(high_risk_refused)} real_desktop_touched=false uncontrolled_actions_expanded=false")  # 修改代码+Phase108GenericAppDiscovery：打印最终固定 token 行并使用通用目标默认关闭字段；如果没有这行代码，真实终端最终回答缺少稳定可复制锚点。
    return 0 if passed else 1  # 新增代码+Phase108GenericAppDiscovery：用退出码表达验收成败；如果没有这行代码，失败也可能被自动化当作成功。
# 新增代码+Phase108GenericAppDiscovery：函数段结束，phase108_main 到此结束；如果没有这个边界说明，初学者不容易看出 Phase108 验收范围。


def phase109_main(argv: list[str] | None = None) -> int:  # 新增代码+Phase109GenericRealLaunchCandidate：函数段开始，提供 Phase109 真实可见终端验收入口；如果没有这段函数，通用真实启动候选只能靠单元测试不能满足真实终端门禁。
    _ = argv  # 新增代码+Phase109GenericRealLaunchCandidate：明确当前不解析命令行参数；如果没有这行代码，读者可能误以为 argv 被忘记处理。
    workspace = Path(__file__).resolve().parents[1]  # 新增代码+Phase109GenericRealLaunchCandidate：默认使用 learning_agent 目录作为真实终端 workspace；如果没有这行代码，bat 入口和测试入口可能写到不同 memory。
    prepare_phase109_candidate, phase109_cli_line = _phase109_generic_real_launch_tools()  # 新增代码+Phase109GenericRealLaunchCandidate：读取 Phase109 候选准备函数和格式化器；如果没有这行代码，验收入口会硬编码模块行为。
    direct_default_report = prepare_phase109_candidate(raw_target="obsidian", enable_real_launch=False)  # 新增代码+Phase109GenericRealLaunchCandidate：先运行默认关闭候选报告；如果没有这行代码，无法证明默认不调用启动后端。
    print(phase109_cli_line(direct_default_report))  # 新增代码+Phase109GenericRealLaunchCandidate：打印默认关闭候选 token 行；如果没有这行代码，真实终端看不到零副作用候选证据。
    direct_enabled_report = prepare_phase109_candidate(raw_target="obsidian", enable_real_launch=True)  # 新增代码+Phase109GenericRealLaunchCandidate：运行记录型显式启用路径；如果没有这行代码，无法证明进程身份、窗口身份和清理模型已接通。
    print(phase109_cli_line(direct_enabled_report))  # 新增代码+Phase109GenericRealLaunchCandidate：打印记录型身份和清理 token 行；如果没有这行代码，真实终端看不到最后一跳桥接证据。
    marker = str(direct_default_report.get("marker", ""))  # 新增代码+Phase109GenericRealLaunchCandidate：从候选报告读取 marker；如果没有这行代码，最终验收行可能和模块常量漂移。
    ok_token = str(direct_default_report.get("ok_token", ""))  # 新增代码+Phase109GenericRealLaunchCandidate：从候选报告读取 OK token；如果没有这行代码，最终验收行可能和模块常量漂移。
    full_request_output = run_computer_terminal_command(workspace, "/computer use --full")  # 修改代码+FullNaturalUserFlow：按真实用户的一行命令直接打开 full；如果没有这行代码，generic launch 默认关闭路径缺少授权前置。
    print(full_request_output, end="")  # 修改代码+FullNaturalUserFlow：打印 full 打开证据；如果没有这行代码，真实终端看不到 full_mode=true。
    generic_output = run_computer_terminal_command(workspace, "/computer launch obsidian")  # 新增代码+Phase109GenericRealLaunchCandidate：运行未知普通应用通用候选路径；如果没有这行代码，验收不能证明用户命令接入 Phase109。
    print(generic_output, end="")  # 新增代码+Phase109GenericRealLaunchCandidate：打印通用候选结果；如果没有这行代码，验收器无法匹配 Phase109 token。
    powershell_output = run_computer_terminal_command(workspace, "/computer launch powershell")  # 新增代码+Phase109GenericRealLaunchCandidate：运行高风险目标拒绝路径；如果没有这行代码，验收不能证明通用候选没有放开 PowerShell。
    print(powershell_output, end="")  # 新增代码+Phase109GenericRealLaunchCandidate：打印高风险拒绝结果；如果没有这行代码，验收器无法匹配 high_risk_refused=true。
    stop_output = run_computer_terminal_command(workspace, "/computer stop Phase109 acceptance cleanup")  # 新增代码+Phase109GenericRealLaunchCandidate：验收后停止 full mode 并触发急停保护；如果没有这行代码，真实终端可能留下 full 状态。
    print(stop_output, end="")  # 新增代码+Phase109GenericRealLaunchCandidate：打印停止结果；如果没有这行代码，用户看不到权限已回收。
    generic_candidate_default_off = bool("WINDOWS_APP_LAUNCH_TARGET_OK" in generic_output and "interactive_target_resolved=true" in generic_output and "safe_known_ordinary_app=true" in generic_output and "real_launch_supported=false" in generic_output and "real_full_launch_attempted=false" in generic_output and "real_desktop_touched=false" in generic_output)  # 修改代码+CompatSlimming：汇总真实用户命令是否输出统一 resolver 默认关闭证据；如果没有这行代码，最终 token 不能证明交互入口已接入新解析层。
    recording_identity_cleanup_ready = bool(direct_enabled_report.get("recording_backend_reaches_launcher", False) and direct_enabled_report.get("process_identity_verified", False) and direct_enabled_report.get("window_identity_verified", False) and direct_enabled_report.get("cleanup_completed", False) and direct_enabled_report.get("residual_process_check_completed", False) and not direct_enabled_report.get("residual_owned_process", True) and not direct_enabled_report.get("real_desktop_touched", False))  # 新增代码+Phase109GenericRealLaunchCandidate：汇总记录型启用路径是否完成身份和清理；如果没有这行代码，Phase109 只证明默认关闭不证明准备度。
    high_risk_refused = bool(("decision=windows_app_high_risk_target_refused" in powershell_output or "decision=phase107_high_risk_target_refused" in powershell_output) and "high_risk_refused=true" in powershell_output)  # 修改代码+CompatSlimming：汇总 powershell 是否仍被高风险拒绝；如果没有这行代码，通用候选可能被误解成放开所有系统工具。
    high_risk_zero_side_effect = bool("real_full_launch_attempted=false" in powershell_output and "real_desktop_touched=false" in powershell_output and "uncontrolled_actions_expanded=false" in powershell_output)  # 新增代码+Phase109GenericRealLaunchCandidate：汇总高风险拒绝是否零副作用；如果没有这行代码，拒绝路径可能仍触碰桌面。
    passed = bool(generic_candidate_default_off and recording_identity_cleanup_ready and high_risk_refused and high_risk_zero_side_effect)  # 新增代码+Phase109GenericRealLaunchCandidate：汇总 Phase109 验收是否通过；如果没有这行代码，CLI 退出码没有依据。
    final_ok = f" {ok_token}" if passed else ""  # 新增代码+Phase109GenericRealLaunchCandidate：仅通过时输出 OK token；如果没有这行代码，失败也可能被验收误判成功。
    print(f"{marker}{final_ok} generic_real_launch_candidate_ready={_phase98_bool(generic_candidate_default_off)} uses_windows_app_launch_resolver={_phase98_bool(generic_candidate_default_off)} hardcoded_app_whitelist_required=false per_app_patch_required=false real_launch_default_disabled=true default_off_backend_not_called=true recording_backend_reaches_launcher={_phase98_bool(recording_identity_cleanup_ready)} process_identity_verified={_phase98_bool(recording_identity_cleanup_ready)} window_identity_verified={_phase98_bool(recording_identity_cleanup_ready)} cleanup_completed={_phase98_bool(recording_identity_cleanup_ready)} residual_process_check_completed={_phase98_bool(recording_identity_cleanup_ready)} residual_owned_process=false high_risk_refused={_phase98_bool(high_risk_refused)} real_desktop_touched=false uncontrolled_actions_expanded=false")  # 修改代码+CompatSlimming：打印最终固定 token 行并指向统一 resolver；如果没有这行代码，真实终端最终回答会继续暗示旧 discovery 文件仍存在。
    return 0 if passed else 1  # 新增代码+Phase109GenericRealLaunchCandidate：用退出码表达验收成败；如果没有这行代码，失败也可能被自动化当作成功。
# 新增代码+Phase109GenericRealLaunchCandidate：函数段结束，phase109_main 到此结束；如果没有这个边界说明，初学者不容易看出 Phase109 验收范围。


def phase115_main(argv: list[str] | None = None) -> int:  # 新增代码+FullMaturityMatrix：函数段开始，提供最终真实可见终端验收入口；如果没有这段函数，Task8 场景需要复制脆弱的内联命令序列。
    _ = argv  # 新增代码+FullMaturityMatrix：明确当前不解析命令行参数；如果没有这一行，读者可能误以为 argv 被忘记处理。
    workspace = Path(__file__).resolve().parents[1]  # 新增代码+FullMaturityMatrix：默认使用 learning_agent 目录作为真实终端 workspace；如果没有这一行，bat 入口和验收入口可能写到不同 memory。
    run_maturity_matrix, maturity_cli_line = _phase115_full_maturity_matrix_tools()  # 新增代码+FullMaturityMatrix：读取最终矩阵工具用于生成固定最终行；如果没有这一行，最终 token 可能和矩阵模块漂移。
    maturity_line = maturity_cli_line(run_maturity_matrix())  # 新增代码+FullMaturityMatrix：生成最终成熟 token 行；如果没有这一行，最终回答没有稳定可复制锚点。
    maturity_output = run_computer_terminal_command(workspace, "/computer maturity")  # 新增代码+FullMaturityMatrix：第一步运行只读成熟矩阵命令；如果没有这一行，Task8 无法证明用户可直接查看最终矩阵。
    print(maturity_output, end="")  # 新增代码+FullMaturityMatrix：打印 maturity 输出；如果没有这一行，真实终端日志缺少最终矩阵证据。
    full_request_output = run_computer_terminal_command(workspace, "/computer use --full")  # 修改代码+FullNaturalUserFlow：第二步按真实用户的一行命令直接打开 full；如果没有这一行，后续 launch 会缺少授权前置。
    print(full_request_output, end="")  # 修改代码+FullNaturalUserFlow：打印 full 打开输出；如果没有这一行，真实终端看不到 full_mode=true。
    ordinary_output = run_computer_terminal_command(workspace, "/computer launch obsidian")  # 新增代码+FullMaturityMatrix：第四步运行普通应用默认关闭命令；如果没有这一行，Task8 无法证明通用 app 不是逐个白名单补丁。
    print(ordinary_output, end="")  # 新增代码+FullMaturityMatrix：打印普通应用输出；如果没有这一行，验收器无法读取 Phase108/109/113 证据。
    high_risk_output = run_computer_terminal_command(workspace, "/computer launch powershell")  # 新增代码+FullMaturityMatrix：第五步运行高风险拒绝命令；如果没有这一行，Task8 无法证明 full 不是无限权限。
    print(high_risk_output, end="")  # 新增代码+FullMaturityMatrix：打印高风险拒绝输出；如果没有这一行，验收器无法读取 high_risk_refused=true。
    stop_output = run_computer_terminal_command(workspace, "/computer stop Phase115 final maturity acceptance cleanup")  # 新增代码+FullMaturityMatrix：第六步停止 full mode 并触发清理；如果没有这一行，真实终端可能留下 full 状态。
    print(stop_output, end="")  # 新增代码+FullMaturityMatrix：打印停止输出；如果没有这一行，用户看不到权限已回收。
    maturity_ready = bool(maturity_line in maturity_output and "real_desktop_touched=false" in maturity_output and "low_level_event_count=0" in maturity_output)  # 新增代码+FullMaturityMatrix：汇总 maturity 命令是否只读成功；如果没有这一行，最终通过可能漏掉矩阵查询副作用。
    ordinary_default_off = bool("WINDOWS_APP_LAUNCH_TARGET_OK" in ordinary_output and "interactive_target_resolved=true" in ordinary_output and "safe_known_ordinary_app=true" in ordinary_output and "real_launch_supported=false" in ordinary_output and "real_full_launch_attempted=false" in ordinary_output and "real_desktop_touched=false" in ordinary_output)  # 修改代码+CompatSlimming：汇总普通 app 统一 resolver 默认关闭路径是否通过；如果没有这一行，最终 token 可能只证明矩阵不证明交互命令。
    high_risk_refused = bool("high_risk_refused=true" in high_risk_output and "real_full_launch_attempted=false" in high_risk_output and "real_desktop_touched=false" in high_risk_output and "uncontrolled_actions_expanded=false" in high_risk_output)  # 新增代码+FullMaturityMatrix：汇总高风险目标是否零副作用拒绝；如果没有这一行，PowerShell 风险可能被漏判。
    stopped_cleanly = bool("Computer Use Stop" in stop_output and "stopped=true" in stop_output and "real_desktop_touched=false" in stop_output and "low_level_event_count=0" in stop_output)  # 新增代码+FullMaturityMatrix：汇总 stop 是否回收 full 状态；如果没有这一行，验收可能留下权限状态。
    passed = bool(maturity_ready and ordinary_default_off and high_risk_refused and stopped_cleanly)  # 新增代码+FullMaturityMatrix：汇总 Task8 一键验收是否通过；如果没有这一行，退出码没有可靠依据。
    final_line = maturity_line if passed else f"{maturity_line} final_acceptance_passed=false ordinary_default_off={_phase98_bool(ordinary_default_off)} high_risk_refused={_phase98_bool(high_risk_refused)} stopped_cleanly={_phase98_bool(stopped_cleanly)}"  # 新增代码+FullMaturityMatrix：通过时复制最终矩阵行，失败时附带原因字段；如果没有这一行，最终回答缺少稳定结论。
    print(final_line)  # 新增代码+FullMaturityMatrix：打印最后一行固定 token；如果没有这一行，controller 无法要求 final answer 完整复制。
    return 0 if passed else 1  # 新增代码+FullMaturityMatrix：用退出码表达最终验收成败；如果没有这一行，失败也可能被自动化当作成功。
# 新增代码+FullMaturityMatrix：函数段结束，phase115_main 到此结束；如果没有这个边界说明，初学者不容易看出最终验收范围。
...
def run_interactive_session(agent: Any, workspace: Path, visible_tools: list[str], max_turns: int | None, prompt_soft_token_limit: int) -> None:  # 新增代码+AppSplit: 运行用户可见交互式终端循环；若没有这行代码，main 仍要承载所有交互逻辑。
    ...
    conversation_context = InteractiveConversationContext(ConversationContextConfig(prompt_soft_token_limit=prompt_soft_token_limit), artifact_dir=workspace / "memory" / "interactive_context")  # 新增代码+交互上下文: 在 while 外创建本终端窗口专属上下文；若没有这行代码，每轮都会重新开始而无法记住上文。
    interactive_session_id = f"interactive-{uuid4().hex}"  # 新增代码+交互上下文: 给本可见终端窗口生成稳定会话 id；若没有这行代码，压缩 boundary 无法归属到同一窗口。
    interactive_turn_id = 0  # 新增代码+交互上下文: 记录普通自然语言 prompt 的轮次；若没有这行代码，压缩 artifact 无法定位发生在哪一轮。
    while True:  # 新增代码+AppSplit: 开始命令行交互循环；若没有这行代码，agent 只能回答一次或直接退出。
        emit_acceptance_event("agent_ready_for_user_prompt", {"workspace": str(workspace), "visible_tools": visible_tools, "max_turns": max_turns})  # 修改代码+工具循环护栏: 在显示输入提示前写 ready 事件并暴露当前单次任务工具循环上限；若没有这行代码，acceptance controller 无法证明真实终端不是无限循环策略。
        ...
        if user_command in {"/status", "status"}:  # 新增代码+StatusEcosystem: 识别终端状态命令；若没有这行代码，用户只能通过外部 CLI 才能看状态。
            ...
            context_snapshot = conversation_context.snapshot()  # 新增代码+交互上下文: 读取交互上下文快照；若没有这行代码，/status 看不到多轮历史规模。
            print(f"\n交互上下文：消息数 {context_snapshot.message_count}，估算字符 {context_snapshot.estimated_chars}，已压缩 {context_snapshot.compact_count} 次，压缩代次 {context_snapshot.compact_generation}，最近质量 {context_snapshot.last_compact_quality_passed if context_snapshot.last_compact_quality_passed is not None else '无'}，最近原因 {context_snapshot.last_compact_reason or '无'}\n")  # 修改代码+ContextCompactRepairInteractive: 在真实终端显示多轮上下文质量和代次；若没有这行代码，用户看不到 compact 摘要是否通过质量门禁。
            emit_acceptance_event("status_snapshot_printed", {"workspace": str(workspace), "counts": status_snapshot.get("counts", {}), "interactive_context": context_snapshot.__dict__})  # 修改代码+交互上下文: 把上下文状态写入验收事件；若没有这行代码，controller 无法断言交互上下文状态。
        ...
        if is_computer_terminal_command(user_command):  # 新增代码+Phase31ComputerUseLockAbortEvidence: 识别 Computer Use 锁和 abort 命令；如果没有这行代码，/computer abort 会被误发给模型。
            ...
            computer_tool_pack_output = _activate_computer_use_tool_pack_for_agent(agent, user_input, computer_output)  # 新增代码+ModelLoopComputerUse：在 `/computer use` 成功后把 Computer Use 工具包加载进当前 agent；如果没有这一行，下一句自然语言仍会在缺少桌面工具 schema 的模型主循环里运行。
            if computer_tool_pack_output:  # 新增代码+ModelLoopComputerUse：只有确实产生工具包状态时才额外打印；如果没有这一行，普通 `/computer status` 会多出无关输出。
                print(computer_tool_pack_output, end="" if computer_tool_pack_output.endswith("\n") else "\n")  # 新增代码+ModelLoopComputerUse：把工具包加载状态显示在真实终端；如果没有这一行，用户无法确认 full 模式已经给模型主循环提供工具。
            emit_acceptance_event("computer_status_printed", {"workspace": str(workspace), "command": user_input[:200], "output_text": computer_output[:8000], "tool_pack_output": computer_tool_pack_output[:4000]})  # 修改代码+ModelLoopComputerUse：把 Computer Use 工具包加载结果写入验收事件；如果没有这一行，真实终端控制器无法证明 full 命令已经扩展模型工具面。
        ...
        interactive_turn_id += 1  # 新增代码+交互上下文: 普通 prompt 进入上下文轮次计数；若没有这行代码，压缩 boundary 的 turn_id 会一直相同。
        current_run_id = f"{interactive_session_id}-turn-{interactive_turn_id}"  # 新增代码+交互上下文: 为本轮历史注入生成可读 run id；若没有这行代码，artifact 和事件难以关联到具体输入。
        context_before = conversation_context.snapshot()  # 新增代码+交互上下文: 记录注入前上下文状态；若没有这行代码，无法判断本轮是否触发了压缩。
        conversation_history = conversation_context.messages_for_run(user_input, session_id=interactive_session_id, run_id=current_run_id, turn_id=interactive_turn_id)  # 新增代码+交互上下文: 取出预算受控的历史消息；若没有这行代码，agent.run 仍只能收到当前输入。
        context_after = conversation_context.snapshot()  # 新增代码+交互上下文: 记录注入后上下文状态；若没有这行代码，事件无法报告是否刚刚压缩。
        if context_after.compact_count > context_before.compact_count:  # 新增代码+交互上下文: 判断本轮是否发生主动压缩；若没有这行代码，压缩事件不会进入验收日志。
            emit_acceptance_event("interactive_context_compacted", {"session_id": interactive_session_id, "turn_id": interactive_turn_id, "before_estimated_chars": context_before.estimated_chars, "after_estimated_chars": context_after.estimated_chars, "compact_count": context_after.compact_count, "reason": context_after.last_compact_reason})  # 新增代码+交互上下文: 写出压缩事件供 acceptance controller 断言；若没有这行代码，真实终端验收无法证明自动压缩发生过。
        emit_acceptance_event("interactive_context_injected", {"session_id": interactive_session_id, "turn_id": interactive_turn_id, "history_message_count": len(conversation_history), "estimated_chars": context_after.estimated_chars, "compact_count": context_after.compact_count})  # 新增代码+交互上下文: 写出历史注入事件；若没有这行代码，调试时无法证明模型是否收到上文。
        ...
        process_summary_renderer = TerminalProcessSummaryRenderer()  # 新增代码+ProcessSummaryUX: 为本轮普通 prompt 创建过程摘要渲染器；若没有这行代码，用户仍只能看到最终回答而看不到中间观察和工具动作。
        answer = agent.run(user_input, max_turns=max_turns, event_callback=process_summary_renderer.handle_event, conversation_history=conversation_history)  # 修改代码+交互上下文: 调用 agent 主循环并传入同一终端窗口历史；若没有这行代码，第二轮“给我攻略”仍会被当成新任务。
        print(f"\nAgent > {answer}")  # 新增代码+AppSplit: 打印最终回答给用户；若没有这行代码，用户看不到 agent 结果。
        conversation_context.append_exchange(user_input, answer)  # 新增代码+交互上下文: 仅在拿到最终回答后追加本轮问答；若没有这行代码，下一轮无法继承刚完成的对话。
        updated_context_snapshot = conversation_context.snapshot()  # 新增代码+交互上下文: 读取追加后的上下文状态；若没有这行代码，最终事件不知道历史增长结果。
        ...
        final_answer_event_payload["interactive_context"] = updated_context_snapshot.__dict__  # 新增代码+交互上下文: 在最终回答事件里附带上下文状态；若没有这行代码，验收结果无法看到本轮后历史规模。
