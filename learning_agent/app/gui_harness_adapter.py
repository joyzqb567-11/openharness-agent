...
from typing import Any, Callable  # 修改代码+GuiToolchainReuse：声明注入回调和 MCP registry 类型；如果没有这一行，真实 adapter 无法保存外部工具注册表。
...
_DEFAULT_ALLOWED_TOOL_NAMES = object()  # 新增代码+GuiToolchainReuse：区分“没传白名单”和“显式传 None”；如果没有这一行，默认无工具和 full_access 全工具会冲突。
...
class RealHarnessGuiAgentAdapter:  # 新增代码+RealHarnessAdapter：类段开始，把 GUI run 转给 LearningAgent；如果没有这个类，DefaultHarness shell 无法真正接入 core/agent.py。
    def __init__(self, enabled: bool = True, model_factory: GuiChatModelFactory | None = None, permission_requester: GuiPermissionRequester | None = None, max_turns: int = 1, allowed_tool_names: set[str] | None | object = _DEFAULT_ALLOWED_TOOL_NAMES, mcp_tool_registry: Any | None = None) -> None:  # 修改代码+GuiToolchainReuse：函数段开始，保存真实 adapter 依赖和 MCP registry；如果没有这段，GUI full_access 无法复用外部工具。
        ...
        if allowed_tool_names is _DEFAULT_ALLOWED_TOOL_NAMES:  # 新增代码+GuiToolchainReuse：识别调用方省略白名单的默认路径；如果没有这一行，直接构造真实 adapter 会误进入 full_access。
            self.allowed_tool_names = set()  # 新增代码+GuiToolchainReuse：省略白名单时保持默认无工具；如果没有这一行，测试和保守入口会暴露全部工具。
        elif allowed_tool_names is None:  # 新增代码+GuiToolchainReuse：识别 GUI full_access 显式传入 None；如果没有这一行，无法复用 LearningAgent 原工具策略。
            self.allowed_tool_names = None  # 新增代码+GuiToolchainReuse：显式 None 原样保留；如果没有这一行，shell、tool_search、MCP 等工具会被封死。
        else:  # 新增代码+GuiToolchainReuse：处理具体白名单；如果没有这一行，read_only/workspace_write 无法收窄工具。
            self.allowed_tool_names = set(allowed_tool_names)  # 新增代码+GuiToolchainReuse：复制工具集合；如果没有这一行，外部集合修改会影响运行边界。
        self.mcp_tool_registry = mcp_tool_registry  # 新增代码+GuiToolchainReuse：保存可选 MCP registry；如果没有这一行，LearningAgent 无法加载 GUI workspace 的 mcp_servers.json。
    ...
    def _runtime_payload(self, request: GuiAgentRunRequest) -> dict[str, object]:  # 新增代码+RealHarnessAdapter：函数段开始，构造 runtime_path 诊断；如果没有这段，用户无法确认本轮走的是 core agent。
        tools_enabled = self.allowed_tool_names is None or bool(self.allowed_tool_names)  # 修改代码+GuiToolchainReuse：None 表示交给 core 原有工具策略，非空集合表示 GUI 白名单工具；如果没有这一行，full_access 会被误诊断成无工具。
        return {"runtime": REAL_HARNESS_RUNTIME, "transport": "in_process_learning_agent", "provider_id": request.provider_id or "openai", "model_id": request.model_id, "reasoning_effort": request.reasoning_effort, "permission_mode": request.permission_mode, "tools_enabled": tools_enabled}  # 修改代码+GuiToolchainReuse：返回低敏路由摘要和正确工具启用状态；如果没有这一行，GUI 诊断缺少真实接入证据。
    ...
    def run(self, request: GuiAgentRunRequest, emit_event: GuiEventEmitter, is_cancelled: GuiCancelChecker) -> GuiAgentRunResult:  # 新增代码+RealHarnessAdapter：函数段开始，执行真实 core agent 主循环；如果没有这段，GUI 不会真正调用 core/agent.py。
        ...
        agent = LearningAgent(model=model, workspace=workspace_path, ask_permission=lambda text: self._ask_permission(request, text), stop_event=stop_event, allowed_tool_names=self.allowed_tool_names, mcp_tool_registry=self.mcp_tool_registry)  # 修改代码+GuiToolchainReuse：创建真实 LearningAgent 并复用 GUI MCP registry；如果没有这一行，GUI full_access 仍看不到外部工具链路。
