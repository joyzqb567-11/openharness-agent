...
def _chrome_next_command(current_state: str) -> str:  # 新增代码+Phase21TerminalStatusUI: 把 Chrome 当前状态映射成最适合的下一条命令；如果没有这段函数，摘要区无法给用户明确操作入口。
    if current_state == "not_installed":  # 新增代码+Phase21TerminalStatusUI: native host 未安装时先走预览；如果没有这行代码，新用户可能直接尝试高风险确认命令。
        return "/chrome install-preview"  # 新增代码+Phase21TerminalStatusUI: 返回安全安装预览命令；如果没有这行代码，未安装状态没有下一步。
    if current_state == "manifest_ready_needs_confirm":  # 新增代码+Phase21TerminalStatusUI: manifest 已生成时提示确认安装；如果没有这行代码，preview 后流程会断开。
        return "/chrome install-confirm <extension_id> I_UNDERSTAND_WRITE_REGISTRY"  # 新增代码+Phase21TerminalStatusUI: 返回确认安装命令；如果没有这行代码，用户不知道如何注册 native host。
    if current_state in {"registered_unpaired", "extension_connected_unpaired"}:  # 新增代码+Phase21TerminalStatusUI: 已注册但未配对时优先诊断；如果没有这行代码，用户要自己从多行字段推断配对问题。
        return "/chrome pairing-diagnose"  # 新增代码+Phase21TerminalStatusUI: 返回配对诊断命令；如果没有这行代码，未配对状态没有清楚行动入口。
    if current_state == "pairing_pending":  # 新增代码+Phase21TerminalStatusUI: 已有 pending 请求时提示继续看同步状态；如果没有这行代码，用户可能重复创建配对请求。
        return "/chrome session-sync-selftest"  # 新增代码+Phase21TerminalStatusUI: 返回 session sync 自检命令；如果没有这行代码，pending 状态没有下一步。
    if current_state == "session_synced":  # 新增代码+Phase21TerminalStatusUI: 已同步时提示端到端检查；如果没有这行代码，完成态仍然缺少验收入口。
        return "/chrome extension-e2e-check"  # 新增代码+Phase21TerminalStatusUI: 返回扩展端到端检查命令；如果没有这行代码，已配对状态没有验证入口。
    return "/chrome repair"  # 新增代码+Phase21TerminalStatusUI: 异常状态默认查看修复建议；如果没有这行代码，未知状态没有安全兜底命令。


def _chrome_recent_issue(providers: dict[str, Any], browser_events: list[Any]) -> str:  # 新增代码+Phase21TerminalStatusUI: 提取 Chrome 最近问题的一行摘要；如果没有这段函数，provider 错误会埋在详细列表中。
    for provider_name, provider_health in providers.items():  # 新增代码+Phase21TerminalStatusUI: 优先扫描 provider 健康状态；如果没有这行代码，最关键的连接失败不会进入摘要。
        health = _block(provider_health)  # 新增代码+Phase21TerminalStatusUI: 安全读取 provider 对象；如果没有这行代码，坏 provider 值会导致摘要崩溃。
        if not bool(health.get("available", False)):  # 新增代码+Phase21TerminalStatusUI: 只提升不可用 provider；如果没有这行代码，摘要会显示正常轨道造成噪音。
            return f"{provider_name}:{_short_text(health.get('reason', ''), 120)}"  # 新增代码+Phase21TerminalStatusUI: 返回 provider 名和原因；如果没有这行代码，用户不知道是哪条浏览器轨道出问题。
    for event in browser_events[-3:]:  # 新增代码+Phase21TerminalStatusUI: 如果 provider 没问题，再扫描最近浏览器事件；如果没有这行代码，事件错误不会进入摘要。
        event_block = _block(event)  # 新增代码+Phase21TerminalStatusUI: 安全读取事件对象；如果没有这行代码，坏事件值会导致摘要报错。
        event_type = str(event_block.get("event_type", ""))  # 新增代码+Phase21TerminalStatusUI: 读取事件类型；如果没有这行代码，无法筛选错误事件。
        if "error" in event_type or "fail" in event_type or "warning" in event_type:  # 新增代码+Phase21TerminalStatusUI: 只提升问题类事件；如果没有这行代码，普通事件会干扰最近问题。
            return f"{event_type} run_id={_short_text(event_block.get('run_id', ''), 80)}"  # 新增代码+Phase21TerminalStatusUI: 返回事件类型和 run id；如果没有这行代码，用户无法定位问题来源。
    return "(none)"  # 新增代码+Phase21TerminalStatusUI: 没有问题时返回空态；如果没有这行代码，摘要区会出现空字符串让用户误解。
...
def render_chrome_status(snapshot: dict[str, Any]) -> str:  # 新增代码+ChromeTerminalUI: 渲染 `/chrome` 聚焦状态页；若没有这段代码，终端命令没有可复用输出逻辑。
    ...
    current_state = _chrome_current_state(str(native_host.get("installer_state", "")), bool(extension.get("connected", False)), bool(extension.get("paired", False)), str(extension.get("pending_pairing_request_status", "") or ""))  # 新增代码+Phase21TerminalStatusUI: 先计算当前状态供摘要和详细区复用；如果没有这行代码，摘要和当前区可能显示不一致。
    ...
    lines.append(f"- state={current_state} next_hint=see Chrome Summary")  # 修改代码+Phase21TerminalStatusUI: 输出复用后的紧凑状态码并指向摘要；如果没有这行代码，/chrome 不是可扫描操作面板。
    lines.append("Chrome Summary")  # 新增代码+Phase21TerminalStatusUI: 输出紧凑摘要区标题；如果没有这行代码，/chrome 第一屏仍要从多行细节中找重点。
    lines.append(f"- connection=native_host:{str(bool(native_host.get('connected', False))).lower()} extension:{str(bool(extension.get('connected', False))).lower()} paired:{str(bool(extension.get('paired', False))).lower()} commands={extension.get('pending_command_count', 0)}")  # 新增代码+Phase21TerminalStatusUI: 一行展示 Chrome 连接概况；如果没有这行代码，用户无法快速判断插件链路是否通。
    lines.append(f"- next={_chrome_next_command(current_state)} : first command for this Chrome state")  # 新增代码+Phase21TerminalStatusUI: 输出下一条建议命令；如果没有这行代码，小白用户看完状态仍不知道要输入什么。
    lines.append(f"- recent_issue={_chrome_recent_issue(providers, _items(browser.get('events', [])))}")  # 新增代码+Phase21TerminalStatusUI: 输出最近 provider 或浏览器问题；如果没有这行代码，错误会埋在 provider/event 详细列表里。
    ...
    latest_harness = _block(latest_run.get("harness", {}))  # 新增代码+Phase22BrowserHarnessFusion: 读取最近 browser run 的 harness 摘要；如果没有这行代码，/chrome 无法显示长任务验收链接。
    latest_harness_verifier = _block(latest_harness.get("latest_verifier", {}))  # 新增代码+Phase22BrowserHarnessFusion: 读取最新 verifier 结果；如果没有这行代码，/chrome 只能显示 run 但不能显示验收是否通过。
    latest_harness_action = _block(latest_harness.get("latest_action_evidence", {}))  # 新增代码+Phase22BrowserHarnessFusion: 读取最近 action 证据；如果没有这行代码，/chrome 无法显示哪步浏览器动作已验收。
    if latest_harness:  # 新增代码+Phase22BrowserHarnessFusion: 只有存在 harness 投影才输出链接行；如果没有这行代码，空状态会显示误导性 harness 字段。
        lines.append(f"- harness_run_id={latest_harness.get('run_id', '')} harness_verifier_passed={str(bool(latest_harness_verifier.get('passed', False))).lower()} harness_action_evidence_count={latest_harness.get('action_evidence_count', 0)} harness_latest_action={_short_text(latest_harness_action.get('tool_name', ''), 80)}")  # 新增代码+Phase22BrowserHarnessFusion: 输出 Chrome 任务对应的 harness 证据；如果没有这行代码，用户在 /chrome 看不到长任务验收闭环。
    ...
    lines.append("- risk=low confirm=no command=/chrome real-extension-e2e-check : checks real extension bridge evidence")  # 新增代码+Phase24RealExtensionE2E: 暴露真实扩展闭环检查入口；如果没有这行代码，用户只能靠猜测找到 Phase 24 命令。
