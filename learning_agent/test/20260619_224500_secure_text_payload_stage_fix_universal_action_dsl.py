...
try:  # 新增代码+URG3ActionDSL：优先按 learning_agent 包路径导入依赖；如果没有这一段，单测和生产入口无法共享同一实现。
    from learning_agent.computer_use_mcp_v2.windows_runtime.sendinput_dispatcher import WindowsSendInputDispatcher  # 新增代码+URG3ActionDSL：导入统一 SendInput dispatcher；如果没有这一行，URG-3 会重新造底层动作分发逻辑。
    from learning_agent.computer_use_mcp_v2.windows_runtime.universal_target_session import UniversalTargetSessionRuntime  # 新增代码+URG3ActionDSL：导入 URG-2 目标 session 和身份门禁；如果没有这一行，动作前无法复核目标窗口。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.sendinput_dispatcher", "learning_agent.computer_use_mcp_v2.windows_runtime.universal_target_session"}:  # 新增代码+URG3ActionDSL：只兜底包路径缺失；如果没有这一行，真实内部 bug 可能被误吞。
        raise  # 新增代码+URG3ActionDSL：重新抛出非路径类导入错误；如果没有这一行，排查真实依赖问题会困难。
    from computer_use_mcp_v2.windows_runtime.sendinput_dispatcher import WindowsSendInputDispatcher  # type: ignore  # 新增代码+URG3ActionDSL：脚本模式导入统一 SendInput dispatcher；如果没有这一行，bat 入口无法运行 URG-3。
    from computer_use_mcp_v2.windows_runtime.universal_target_session import UniversalTargetSessionRuntime  # type: ignore  # 新增代码+URG3ActionDSL：脚本模式导入目标 session runtime；如果没有这一行，bat 入口无法执行动作前身份复核。
...
def _phase118_window_rect_offset(window: dict[str, Any]) -> dict[str, int]:  # 新增代码+RealLaunchTargetSession：函数段开始，读取目标窗口左上角偏移；如果没有这段函数，窗口相对点会被当成屏幕绝对点。
    rect = window.get("rect", {}) if isinstance(window.get("rect", {}), dict) else {}  # 新增代码+RealLaunchTargetSession：读取窗口 rect 字典；如果没有这一行，缺 rect 时会抛异常。
    return {"left": _phase118_safe_int(rect.get("left")), "top": _phase118_safe_int(rect.get("top"))}  # 新增代码+RealLaunchTargetSession：返回左上角偏移；如果没有这一行，拖拽无法落到目标窗口内。
# 新增代码+RealLaunchTargetSession：函数段结束，_phase118_window_rect_offset 到此结束；如果没有这个边界说明，初学者不容易看出坐标偏移范围。

def _phase118_points_for_action(action: dict[str, Any], target_window: dict[str, Any]) -> list[dict[str, int]]:  # 新增代码+RealLaunchTargetSession：函数段开始，把拖拽点转换成屏幕坐标；如果没有这段函数，真实 Paint 绘制可能打到错误位置。
    raw_points = action.get("points") if isinstance(action.get("points"), list) else []  # 新增代码+RealLaunchTargetSession：只接受列表点位；如果没有这一行，坏 points 类型可能导致异常。
    points = [_phase118_point(point) for point in raw_points]  # 新增代码+RealLaunchTargetSession：清洗所有点位；如果没有这一行，字符串坐标或 tuple 点位不能稳定发送。
    if str(action.get("coordinate_space", "")).strip().lower() not in {"target_window", "window", "relative_to_window"}:  # 新增代码+RealLaunchTargetSession：只在明确声明窗口相对坐标时偏移；如果没有这一行，旧绝对坐标任务会被错误移动。
        return points  # 新增代码+RealLaunchTargetSession：返回原始屏幕坐标；如果没有这一行，普通绝对坐标动作无法保持兼容。
    offset = _phase118_window_rect_offset(target_window)  # 新增代码+RealLaunchTargetSession：读取目标窗口左上角；如果没有这一行，窗口相对点没有换算基准。
    return [{"x": point["x"] + offset["left"], "y": point["y"] + offset["top"]} for point in points]  # 新增代码+RealLaunchTargetSession：把相对点加上窗口偏移；如果没有这一行，拖拽不会落在启动出的窗口里。
# 新增代码+RealLaunchTargetSession：函数段结束，_phase118_points_for_action 到此结束；如果没有这个边界说明，初学者不容易看出点位换算范围。

def _phase118_hwnd_from_window(window: dict[str, Any]) -> int:  # 新增代码+RealLaunchTargetSession：函数段开始，从目标窗口解析 hwnd；如果没有这段函数，focus_window 无法生成 set_foreground 事件。
    if window.get("hwnd") is not None:  # 新增代码+RealLaunchTargetSession：优先使用明确 hwnd 字段；如果没有这一行，真实 inventory 句柄可能被忽略。
        return _phase118_safe_int(window.get("hwnd"))  # 新增代码+RealLaunchTargetSession：返回 hwnd 整数；如果没有这一行，SetForegroundWindow 没有目标。
    text = str(window.get("window_id", "") or "")  # 新增代码+RealLaunchTargetSession：读取协议 window_id；如果没有这一行，缺 hwnd 时无法兜底解析。
    return _phase118_safe_int(text.split(":", 1)[1]) if text.startswith("hwnd:") and ":" in text else 0  # 新增代码+RealLaunchTargetSession：从 hwnd:123 解析句柄；如果没有这一行，录制/真实窗口 id 不能用于聚焦。
# 新增代码+RealLaunchTargetSession：函数段结束，_phase118_hwnd_from_window 到此结束；如果没有这个边界说明，初学者不容易看出句柄解析范围。
...
def _phase118_dispatch_reports_real_dispatch(dispatch_result: dict[str, Any], sent_count: int) -> bool:  # 新增代码+源码复核门禁：函数段开始，从 dispatcher 结果识别是否真的触碰桌面；如果没有这段函数，真实 sender 接入后仍会被误报成 recording。
    sender_result = dict(dispatch_result.get("sender", {}) if isinstance(dispatch_result.get("sender", {}), dict) else {})  # 新增代码+源码复核门禁：读取 dispatcher 嵌套的 sender 返回值；如果没有这一行，真实后端标识会被埋在内层。
    sender_name = str(sender_result.get("sender") or sender_result.get("backend") or "").strip().lower()  # 新增代码+源码复核门禁：读取 sender/backend 名称；如果没有这一行，无法识别 windows_sendinput_low_level。
    sender_count = _phase118_safe_int(sender_result.get("low_level_event_count", dispatch_result.get("low_level_event_count", sent_count)))  # 新增代码+源码复核门禁：读取 sender 确认的事件数；如果没有这一行，只有名称没有数量证据。
    explicit_real = bool(sender_result.get("real_dispatch_performed") or sender_result.get("real_desktop_touched") or sender_result.get("physical_desktop_dispatch_performed"))  # 新增代码+源码复核门禁：优先信任真实 sender 的显式物理派发字段；如果没有这一行，未来真实后端声明会被忽略。
    inferred_real = bool(sender_name == "windows_sendinput_low_level" and sender_count > 0 and int(sent_count) > 0)  # 新增代码+源码复核门禁：根据真实 sender 名称和事件数做保守推断；如果没有这一行，旧真实 sender 返回值会被误判。
    return bool(explicit_real or inferred_real)  # 新增代码+源码复核门禁：返回统一真实派发判断；如果没有这一行，调用方拿不到单一事实源。
# 新增代码+源码复核门禁：函数段结束，_phase118_dispatch_reports_real_dispatch 到此结束；如果没有这个边界说明，初学者不容易看出真实派发识别范围。

def _phase118_low_level_target_identity(window: dict[str, Any]) -> dict[str, Any]:  # 新增代码+ControlledPhysicalAdapter：函数段开始，把已复核窗口压缩成低层事件可携带的脱敏目标身份；如果没有这段函数，Phase95 最后一跳无法知道事件绑定哪个窗口。
    title = str(window.get("title_preview") or window.get("title") or "")  # 新增代码+ControlledPhysicalAdapter：读取窗口标题仅用于哈希；如果没有这一行，目标标题变化无法进入审计指纹。
    return {"app_id": str(window.get("app_id") or window.get("process_name") or "").lower(), "process_name": str(window.get("process_name") or window.get("app_id") or "").lower(), "window_id": str(window.get("window_id") or window.get("hwnd") or ""), "display_id": str(window.get("display_id") or window.get("monitor_id") or ""), "title_sha256_16": _phase118_text_sha256_16(title)}  # 新增代码+ControlledPhysicalAdapter：返回不含完整标题的目标身份；如果没有这一行，受控 sender 缺少安全目标边界。
# 新增代码+ControlledPhysicalAdapter：函数段结束，_phase118_low_level_target_identity 到此结束；如果没有这个边界说明，初学者不容易看出目标身份字段范围。

def _phase118_attach_target_to_events(events: list[dict[str, Any]], target_window: dict[str, Any]) -> list[dict[str, Any]]:  # 新增代码+ControlledPhysicalAdapter：函数段开始，把目标身份附加到每个 dispatcher 事件；如果没有这段函数，dispatcher 展开后的低层事件不会带 target。
    target_identity = _phase118_low_level_target_identity(dict(target_window or {}))  # 新增代码+ControlledPhysicalAdapter：根据已复核窗口生成脱敏目标身份；如果没有这一行，事件只知道坐标不知道窗口。
    if not any(str(value or "") for value in target_identity.values()):  # 新增代码+ControlledPhysicalAdapter：检测目标身份是否完全为空；如果没有这一行，空身份也会被误认为可审计目标。
        return [dict(event) for event in events]  # 新增代码+ControlledPhysicalAdapter：空身份时只复制事件不写 target；如果没有这一行，旧路径可能携带无意义空 target。
    enriched: list[dict[str, Any]] = []  # 新增代码+ControlledPhysicalAdapter：准备保存带 target 的事件副本；如果没有这一行，不能安全复制多条事件。
    for event in events:  # 新增代码+ControlledPhysicalAdapter：逐条处理 dispatcher 事件；如果没有这一行，只能给单个动作加目标。
        copied = dict(event)  # 新增代码+ControlledPhysicalAdapter：复制当前事件；如果没有这一行，调用方原始动作可能被污染。
        copied["target"] = dict(target_identity)  # 新增代码+ControlledPhysicalAdapter：写入脱敏目标身份；如果没有这一行，Phase95 会因 missing_target_identity 拒绝。
        enriched.append(copied)  # 新增代码+ControlledPhysicalAdapter：保存带目标事件；如果没有这一行，处理结果会丢失该事件。
    return enriched  # 新增代码+ControlledPhysicalAdapter：返回全部带目标身份的事件；如果没有这一行，受控物理 sender 接线无法完成。
# 新增代码+ControlledPhysicalAdapter：函数段结束，_phase118_attach_target_to_events 到此结束；如果没有这个边界说明，初学者不容易看出事件加 target 范围。
...
class UniversalActionDslRuntime:  # 新增代码+URG3ActionDSL：类段开始，定义通用动作 DSL 运行时；如果没有这个类，动作桥会散落在测试和 CLI 里。
    ...
    def _events_for_action(self, action: dict[str, Any], target_window: dict[str, Any] | None = None) -> tuple[list[dict[str, Any]], dict[str, Any], dict[str, Any] | None]:  # 修改代码+RealLaunchTargetSession：函数段开始，把 DSL 动作转成 dispatcher 规范事件并可读取目标窗口；如果没有 target_window，聚焦和窗口相对坐标无法执行。
        action_type = _phase118_action_type(action)  # 新增代码+URG3ActionDSL：读取动作类型；如果没有这一行，后续分支不知道要处理哪种动作。
        safe_target_window = dict(target_window or {})  # 新增代码+RealLaunchTargetSession：复制目标窗口供聚焦和坐标换算使用；如果没有这一行，None 目标会导致字段读取异常。
        if action_type == "focus_window":  # 修改代码+RealLaunchTargetSession：focus_window 需要生成真实前台窗口事件；如果没有这一行，启动窗口不会被置前。
            hwnd = _phase118_hwnd_from_window(safe_target_window)  # 新增代码+RealLaunchTargetSession：读取目标窗口句柄；如果没有这一行，set_foreground 没有目标。
            return ([{"type": "set_foreground", "hwnd": hwnd}] if hwnd > 0 else []), {"action_type": action_type, "hwnd": hwnd}, (None if hwnd > 0 else {"decision": "missing_window_handle", "message": "目标窗口缺少 hwnd，无法置前。"})  # 新增代码+RealLaunchTargetSession：返回前台窗口事件或结构化错误；如果没有这一行，聚焦仍会空跑。
        if action_type in {"wait", "observe"}:  # 修改代码+RealLaunchTargetSession：等待和观察仍是零输入动作；如果没有这一行，观察会被误当未知动作。
        ...
        if action_type == "drag_path":  # 新增代码+URG3ActionDSL：处理通用拖拽路径动作；如果没有这一行，绘图 primitive 无法通过 DSL 执行。
            points = _phase118_points_for_action(action, safe_target_window)  # 修改代码+RealLaunchTargetSession：清洗并按需把窗口相对点换成屏幕坐标；如果没有这一行，真实绘图会落到错误位置。
        ...
        if action_type == "type_text":  # 新增代码+URG3ActionDSL：处理通用文本输入动作；如果没有这一行，Notepad 等普通文本场景无法执行。
            ...
            if bool(getattr(self.low_level_sender, "requires_raw_text", False)):  # 修改代码+SecureTextChannel：仅当最后一跳声明需要明文时打开短生命周期通道；如果没有这行代码，真实 Unicode 输入会继续只有哈希而无法打字。
                event["text"] = text  # 修改代码+SecureTextChannel：把明文只交给内存中的 dispatcher 转私有通道；如果没有这行代码，受控 sender 无法把用户文字传给真实后端。
    ...
    def dispatch(self, session: dict[str, Any], action: dict[str, Any], current_window: dict[str, Any] | None = None, abort_requested: bool = False) -> dict[str, Any]:  # 新增代码+URG3ActionDSL：函数段开始，执行一次动作前复核和 DSL 分发；如果没有这段函数，URG-3 没有统一动作入口。
        ...
        events, action_summary, error = self._events_for_action(action, target_window)  # 修改代码+RealLaunchTargetSession：把 DSL 动作转成 dispatcher 事件并传入目标窗口；如果没有 target_window，focus 和相对坐标无法工作。
        ...
        events = _phase118_attach_target_to_events(events, target_window)  # 新增代码+ControlledPhysicalAdapter：把已复核目标身份附加到即将发送的 dispatcher 事件；如果没有这一行，受控物理 sender 无法做最后一跳目标门禁。
        ...
        sender_result = dict(dispatch_result.get("sender", {}) if isinstance(dispatch_result.get("sender", {}), dict) else {})  # 新增代码+RealPhysicalFullMode：读取真实或受控 sender 返回的内层报告；如果没有这一行，非 recording sender 的事件数会被旧列表计数漏掉。
        reported_sent_count = _phase118_safe_int(sender_result.get("low_level_event_count", dispatch_result.get("low_level_event_count", sent_count)))  # 新增代码+RealPhysicalFullMode：从 sender 报告读取低层事件数；如果没有这一行，真实 SendInput 已发送事件时顶层仍可能显示 low_level_event_count=0。
        sent_count = max(sent_count, reported_sent_count)  # 修改代码+RealPhysicalFullMode：用 recording 差值和真实 sender 回报中的较大值作为本次事件数；如果没有这一行，GUI 路由和 OK 门禁会因事件数为 0 被误判失败。
        real_dispatch_performed = _phase118_dispatch_reports_real_dispatch(dispatch_result, sent_count)  # 新增代码+源码复核门禁：根据内层 sender 结果判断本次是否真实派发；如果没有这一行，真实 SendInput 路径会继续被报告成 false。
        self.real_dispatch_performed = bool(self.real_dispatch_performed or real_dispatch_performed)  # 修改代码+源码复核门禁：把本次真实派发累计到 runtime 状态；如果没有这一行，loop 和最终矩阵无法汇总真实动作事实。
        return {"ok": bool(dispatch_result.get("ok", False)), "decision": "sent_to_low_level_sender" if bool(dispatch_result.get("ok", False)) else str(dispatch_result.get("decision", "dispatch_failed")), "message": "通用动作已通过 SendInput dispatcher 展开到低层 sender。", "model": PHASE118_UNIVERSAL_ACTION_DSL_MODEL, "action": action_summary, "low_level_event_count": sent_count, "low_level_event_types": list(dispatch_result.get("low_level_event_types", [])), "actions_expanded": PHASE118_ACTIONS_EXPANDED, "target_verified": True, "target_verification": verification, "dispatch": dispatch_result, "real_dispatch_performed": self.real_dispatch_performed, "real_desktop_touched": self.real_dispatch_performed, "text_redacted": True}  # 修改代码+源码复核门禁：返回动作分发摘要并携带真实派发事实；如果没有这一行，调用方无法知道本次动作是否真正到达低层 sender。
