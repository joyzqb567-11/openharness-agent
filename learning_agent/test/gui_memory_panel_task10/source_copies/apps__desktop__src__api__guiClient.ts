...
export type GuiMemorySummaryPayload = { // 新增代码+DesktopGUIMemoryPanelClient：定义记忆摘要响应类型；如果没有这段，前端只能把 agent_memory 状态当成不透明对象。
  ok: true; // 新增代码+DesktopGUIMemoryPanelClient：标记记忆请求成功；如果没有这行，调用方无法区分成功 payload 和错误 payload。
  schema_version: number; // 新增代码+DesktopGUIMemoryPanelClient：保存协议版本；如果没有这行，后续合同演进没有兼容依据。
  generated_at: string; // 新增代码+DesktopGUIMemoryPanelClient：保存生成时间；如果没有这行，用户无法判断记忆状态是否新鲜。
  workspace_name: string; // 新增代码+DesktopGUIMemoryPanelClient：保存工作区名称；如果没有这行，面板无法确认记忆来源项目。
  reuse_module: string; // 新增代码+DesktopGUIMemoryPanelClient：保存复用模块；如果没有这行，用户无法验收 GUI 没有重写记忆系统。
  memory_root: string; // 新增代码+DesktopGUIMemoryPanelClient：保存记忆根目录；如果没有这行，用户不知道数据来自 agent_memory。
  files: Array<Record<string, unknown>>; // 新增代码+DesktopGUIMemoryPanelClient：保存记忆文件摘要列表；如果没有这行，Context/Progress/Bugs 无法渲染。
  context_summary: Record<string, unknown>; // 新增代码+DesktopGUIMemoryPanelClient：保存 Context 快捷摘要；如果没有这行，摘要栏无法快速显示上下文状态。
  progress_summary: Record<string, unknown>; // 新增代码+DesktopGUIMemoryPanelClient：保存 Progress 快捷摘要；如果没有这行，摘要栏无法快速显示进度状态。
  bugs_summary: Record<string, unknown>; // 新增代码+DesktopGUIMemoryPanelClient：保存 Bugs 快捷摘要；如果没有这行，摘要栏无法快速显示风险状态。
  experience_summary: Record<string, unknown>; // 新增代码+DesktopGUIMemoryPanelClient：保存 Experience 快捷摘要；如果没有这行，经验沉淀状态无法显示。
  status_degraded: boolean; // 新增代码+DesktopGUIMemoryPanelClient：保存读取是否降级；如果没有这行，前端无法提示记忆可信度。
  safe_error: string; // 新增代码+DesktopGUIMemoryPanelClient：保存脱敏错误文案；如果没有这行，失败时可能显示原始异常。
}; // 新增代码+DesktopGUIMemoryPanelClient：记忆摘要类型结束；如果没有这行，TypeScript 类型语法不完整。

export type GuiPromptStatusPayload = { // 新增代码+DesktopGUIMemoryPanelClient：定义 prompt/token 状态响应类型；如果没有这段，前端只能把预算状态当成不透明对象。
  ok: true; // 新增代码+DesktopGUIMemoryPanelClient：标记 prompt 状态请求成功；如果没有这行，调用方无法区分成功 payload 和错误 payload。
  schema_version: number; // 新增代码+DesktopGUIMemoryPanelClient：保存协议版本；如果没有这行，后续合同演进没有兼容依据。
  generated_at: string; // 新增代码+DesktopGUIMemoryPanelClient：保存生成时间；如果没有这行，用户无法判断预算状态是否新鲜。
  workspace_name: string; // 新增代码+DesktopGUIMemoryPanelClient：保存工作区名称；如果没有这行，面板无法确认 prompt 状态来源。
  reuse_module: string; // 新增代码+DesktopGUIMemoryPanelClient：保存复用模块；如果没有这行，用户无法验收 GUI 复用 report_tools。
  tools: Array<Record<string, unknown>>; // 新增代码+DesktopGUIMemoryPanelClient：保存 prompt/token 工具摘要；如果没有这行，工具状态无法渲染。
  tool_count: number; // 新增代码+DesktopGUIMemoryPanelClient：保存工具总数；如果没有这行，摘要栏缺少覆盖规模。
  available_tool_count: number; // 新增代码+DesktopGUIMemoryPanelClient：保存可用工具数；如果没有这行，用户无法快速判断接入完整度。
  context_budget: Record<string, unknown>; // 新增代码+DesktopGUIMemoryPanelClient：保存上下文预算；如果没有这行，max messages/max chars 无法显示。
  prompt_surface: Record<string, unknown>; // 新增代码+DesktopGUIMemoryPanelClient：保存 prompt_surface_report 摘要；如果没有这行，prompt 表面状态不可见。
  token_budget: Record<string, unknown>; // 新增代码+DesktopGUIMemoryPanelClient：保存 token_budget_report 摘要；如果没有这行，token 报告状态不可见。
  snapshot_summary: Record<string, unknown>; // 新增代码+DesktopGUIMemoryPanelClient：保存 compact/resume 快照摘要；如果没有这行，长任务上下文状态不可见。
  status_degraded: boolean; // 新增代码+DesktopGUIMemoryPanelClient：保存读取是否降级；如果没有这行，前端无法提示预算可信度。
  safe_error: string; // 新增代码+DesktopGUIMemoryPanelClient：保存脱敏错误文案；如果没有这行，失败时可能显示原始异常。
}; // 新增代码+DesktopGUIMemoryPanelClient：prompt/token 状态类型结束；如果没有这行，TypeScript 类型语法不完整。

export type GuiNotebookStatusPayload = { // 新增代码+DesktopGUIMemoryPanelClient：定义 notebook 状态响应类型；如果没有这段，前端只能把 notebook 状态当成不透明对象。
  ok: true; // 新增代码+DesktopGUIMemoryPanelClient：标记 notebook 状态请求成功；如果没有这行，调用方无法区分成功 payload 和错误 payload。
  schema_version: number; // 新增代码+DesktopGUIMemoryPanelClient：保存协议版本；如果没有这行，后续合同演进没有兼容依据。
  generated_at: string; // 新增代码+DesktopGUIMemoryPanelClient：保存生成时间；如果没有这行，用户无法判断 notebook 状态是否新鲜。
  workspace_name: string; // 新增代码+DesktopGUIMemoryPanelClient：保存工作区名称；如果没有这行，面板无法确认 notebook 状态来源。
  reuse_module: string; // 新增代码+DesktopGUIMemoryPanelClient：保存复用模块；如果没有这行，用户无法验收 GUI 复用 notebook_tools。
  tools: Array<Record<string, unknown>>; // 新增代码+DesktopGUIMemoryPanelClient：保存 notebook 工具摘要；如果没有这行，notebook_read/edit 状态无法渲染。
  tool_count: number; // 新增代码+DesktopGUIMemoryPanelClient：保存工具总数；如果没有这行，摘要栏缺少工具规模。
  available_tool_count: number; // 新增代码+DesktopGUIMemoryPanelClient：保存可用工具数；如果没有这行，用户无法快速判断接入完整度。
  read_only_first_pass: boolean; // 新增代码+DesktopGUIMemoryPanelClient：保存只读优先策略；如果没有这行，编辑边界不可见。
  edit_exposed_in_gui: boolean; // 新增代码+DesktopGUIMemoryPanelClient：保存编辑入口是否暴露；如果没有这行，风险边界不可见。
  notebook_count: number; // 新增代码+DesktopGUIMemoryPanelClient：保存 notebook 文件数量；如果没有这行，notebook 规模不可见。
  notebooks: string[]; // 新增代码+DesktopGUIMemoryPanelClient：保存 notebook 路径示例；如果没有这行，用户不知道文件在哪里。
  scan_limited: boolean; // 新增代码+DesktopGUIMemoryPanelClient：保存扫描是否截断；如果没有这行，用户可能误信数量是全量。
  status_degraded: boolean; // 新增代码+DesktopGUIMemoryPanelClient：保存读取是否降级；如果没有这行，前端无法提示 notebook 可信度。
  safe_error: string; // 新增代码+DesktopGUIMemoryPanelClient：保存脱敏错误文案；如果没有这行，失败时可能显示原始异常。
}; // 新增代码+DesktopGUIMemoryPanelClient：notebook 状态类型结束；如果没有这行，TypeScript 类型语法不完整。
...
export type GuiAcceptanceScenariosPayload = { // 新增代码+DesktopGUIAcceptanceClient：定义验收场景总览响应类型；如果没有这段，前端只能把验收控制中心当不透明对象。
  ok: true; // 新增代码+DesktopGUIAcceptanceClient：标记场景请求成功；如果没有这行，调用方无法区分成功 payload 和错误 payload。
  schema_version: number; // 新增代码+DesktopGUIAcceptanceClient：保存协议版本；如果没有这行，后续合同演进没有兼容依据。
  workspace: string; // 新增代码+DesktopGUIAcceptanceClient：保存当前 workspace；如果没有这行，用户无法确认验收清单来源。
  generated_at: string; // 新增代码+DesktopGUIAcceptanceClient：保存生成时间；如果没有这行，用户无法判断状态是否新鲜。
  reuse_module: string; // 新增代码+DesktopGUIAcceptanceClient：保存复用模块说明；如果没有这行，用户无法验收 GUI 没有另造验收控制器。
  controller: Record<string, unknown>; // 新增代码+DesktopGUIAcceptanceClient：保存 controller.ps1 与可见终端门禁摘要；如果没有这行，右侧面板缺少控制器边界。
  scenario_count: number; // 新增代码+DesktopGUIAcceptanceClient：保存场景数量；如果没有这行，摘要区缺少规模。
  run_count: number; // 新增代码+DesktopGUIAcceptanceClient：保存运行数量；如果没有这行，摘要区缺少证据规模。
  safe_smoke_scenario_id: string; // 新增代码+DesktopGUIAcceptanceClient：保存安全 smoke 场景 id；如果没有这行，真实 GUI 验收不好定位安全样例。
  scenarios: Array<Record<string, unknown>>; // 新增代码+DesktopGUIAcceptanceClient：保存场景卡片列表；如果没有这行，AcceptancePanel 没有主体数据。
  recent_runs: Array<Record<string, unknown>>; // 新增代码+DesktopGUIAcceptanceClient：保存最近运行摘要；如果没有这行，用户需要额外切换才看到证据。
  status_degraded: boolean; // 新增代码+DesktopGUIAcceptanceClient：保存读取是否降级；如果没有这行，前端无法提示数据可信度。
  safe_error: string; // 新增代码+DesktopGUIAcceptanceClient：保存安全错误文案；如果没有这行，失败时可能显示原始异常。
}; // 新增代码+DesktopGUIAcceptanceClient：验收场景总览类型结束；如果没有这行，TypeScript 类型语法不完整。

export type GuiAcceptanceRunsPayload = { // 新增代码+DesktopGUIAcceptanceClient：定义验收运行列表响应类型；如果没有这段，运行证据刷新没有稳定类型。
  ok: true; // 新增代码+DesktopGUIAcceptanceClient：标记运行列表请求成功；如果没有这行，调用方无法区分成功 payload 和错误 payload。
  schema_version: number; // 新增代码+DesktopGUIAcceptanceClient：保存协议版本；如果没有这行，后续合同演进没有兼容依据。
  workspace: string; // 新增代码+DesktopGUIAcceptanceClient：保存当前 workspace；如果没有这行，用户无法确认运行证据来源。
  generated_at: string; // 新增代码+DesktopGUIAcceptanceClient：保存生成时间；如果没有这行，运行证据新鲜度不可见。
  reuse_module: string; // 新增代码+DesktopGUIAcceptanceClient：保存复用模块说明；如果没有这行，用户无法确认复用 acceptance_controller。
  run_count: number; // 新增代码+DesktopGUIAcceptanceClient：保存运行数量；如果没有这行，摘要区缺少证据规模。
  runs: Array<Record<string, unknown>>; // 新增代码+DesktopGUIAcceptanceClient：保存运行卡片列表；如果没有这行，证据区没有主体数据。
  status_degraded: boolean; // 新增代码+DesktopGUIAcceptanceClient：保存读取是否降级；如果没有这行，前端无法提示数据可信度。
  safe_error: string; // 新增代码+DesktopGUIAcceptanceClient：保存安全错误文案；如果没有这行，失败时可能显示原始异常。
}; // 新增代码+DesktopGUIAcceptanceClient：验收运行列表类型结束；如果没有这行，TypeScript 类型语法不完整。

export type GuiAcceptanceRunActionPayload = { // 新增代码+DesktopGUIAcceptanceClient：定义验收启动动作响应类型；如果没有这段，运行按钮结果无法稳定渲染。
  ok: true; // 新增代码+DesktopGUIAcceptanceClient：标记启动请求已被 bridge 处理；如果没有这行，调用方无法区分成功 payload 和错误 payload。
  schema_version: number; // 新增代码+DesktopGUIAcceptanceClient：保存协议版本；如果没有这行，后续合同演进没有兼容依据。
  action: "run"; // 新增代码+DesktopGUIAcceptanceClient：保存动作名称；如果没有这行，面板无法解释最近执行了哪个按钮。
  scenario_id: string; // 新增代码+DesktopGUIAcceptanceClient：保存目标场景 id；如果没有这行，点击反馈缺少对象。
  status: string; // 新增代码+DesktopGUIAcceptanceClient：保存 planned/launched/unavailable/not_found；如果没有这行，用户看不到启动状态。
  supported: boolean; // 新增代码+DesktopGUIAcceptanceClient：保存后端是否支持真实启动；如果没有这行，前端可能误启用按钮。
  launched: boolean; // 新增代码+DesktopGUIAcceptanceClient：保存是否已经启动进程；如果没有这行，dry-run 和真实启动无法区分。
  message: string; // 新增代码+DesktopGUIAcceptanceClient：保存可读说明；如果没有这行，按钮点击后缺少肉眼反馈。
  safe_error: string; // 新增代码+DesktopGUIAcceptanceClient：保存安全错误文案；如果没有这行，失败时可能显示原始异常。
  command?: string; // 新增代码+DesktopGUIAcceptanceClient：保存脱敏启动命令预览；如果没有这行，dry-run 验收无法看到 controller.ps1 计划。
  pid?: number; // 新增代码+DesktopGUIAcceptanceClient：保存真实启动进程 pid；如果没有这行，用户无法定位刚启动的控制器。
  run: Record<string, unknown>; // 新增代码+DesktopGUIAcceptanceClient：保存 run root 和场景路径摘要；如果没有这行，用户无法找到本次证据目录。
}; // 新增代码+DesktopGUIAcceptanceClient：验收启动动作类型结束；如果没有这行，TypeScript 类型语法不完整。
...
export type GuiDiagnosticsPayload = { // 修改代码+DesktopDiagnosticsToolSurfaceClient：定义 V2 diagnostics 响应类型；如果没有这段，前端只能把诊断 payload 当成不透明对象。
  ok: true; // 修改代码+DesktopDiagnosticsToolSurfaceClient：标记 diagnostics 请求成功；如果没有这行，调用方无法区分成功响应和错误响应。
  schema_version: number; // 修改代码+DesktopDiagnosticsToolSurfaceClient：保存诊断协议版本；如果没有这行，前端无法做协议兼容判断。
  backend_online: boolean; // 修改代码+DesktopDiagnosticsToolSurfaceClient：保存后端在线状态；如果没有这行，诊断页不能直接显示在线/离线。
  health: Record<string, unknown>; // 修改代码+DesktopDiagnosticsToolSurfaceClient：保存 health 摘要；如果没有这行，设置页和诊断页会重复请求后端。
  status_degraded: boolean; // 修改代码+DesktopDiagnosticsToolSurfaceClient：保存整体降级状态；如果没有这行，UI 无法提示数据可信度。
  safe_error: string; // 修改代码+DesktopDiagnosticsToolSurfaceClient：保存脱敏错误文案；如果没有这行，降级时可能显示原始异常。
  snapshot_summary: Record<string, unknown>; // 修改代码+DesktopDiagnosticsToolSurfaceClient：保存快照摘要；如果没有这行，运行指标无法显示。
  trace_summary: Record<string, unknown>; // 新增代码+DesktopDiagnosticsToolSurfaceClient：保存 Trace 摘要；如果没有这行，诊断页无法显示 read_trace 风格状态。
  compact_status: Record<string, unknown>; // 新增代码+DesktopDiagnosticsToolSurfaceClient：保存 Compact 摘要；如果没有这行，诊断页无法显示上下文压缩状态。
  resume_report_summary: Record<string, unknown>; // 新增代码+DesktopDiagnosticsToolSurfaceClient：保存 Resume 摘要；如果没有这行，诊断页无法显示恢复报告入口。
  health_status_summary: Record<string, unknown>; // 新增代码+DesktopDiagnosticsToolSurfaceClient：保存 health_status 摘要；如果没有这行，诊断页无法显示健康警告。
  lsp_diagnostics_summary: Record<string, unknown>; // 新增代码+DesktopDiagnosticsToolSurfaceClient：保存 LSP 摘要；如果没有这行，诊断页无法显示代码理解工具状态。
  repl_summary: Record<string, unknown>; // 新增代码+DesktopDiagnosticsToolSurfaceClient：保存 REPL 摘要；如果没有这行，诊断页无法显示批量编排状态。
  last_error: string; // 修改代码+DesktopDiagnosticsToolSurfaceClient：保存脱敏最近错误；如果没有这行，用户无法复制失败线索。
  release_gate: Record<string, unknown>; // 修改代码+DesktopDiagnosticsToolSurfaceClient：保存 release gate 摘要；如果没有这行，诊断页无法显示最近验收状态。
  diagnostic_bundle: Record<string, unknown>; // 修改代码+DesktopDiagnosticsToolSurfaceClient：保存可复制诊断包；如果没有这行，复制按钮没有稳定数据来源。
}; // 修改代码+DesktopDiagnosticsToolSurfaceClient：diagnostics 类型结束；如果没有这行，TypeScript 类型语法不完整。
...
export function createGuiClient(baseUrl: string, bridgeToken: string, fetcher: FetchLike = fetch) { // 修改代码+DesktopRuntimePanelsClient：函数段开始，创建 GUI bridge 客户端；如果没有这段，渲染进程会到处手写 fetch。
  ...
  return { // 修改代码+DesktopRuntimePanelsClient：返回面向 UI 的 client 方法；如果没有这行，调用方无法使用封装能力。
    ...
    memorySummary(): Promise<GuiMemorySummaryPayload> { // 新增代码+DesktopGUIMemoryPanelClient：读取 agent_memory 只读摘要；如果没有这段，记忆页签拿不到 Context/Progress/Bugs 事实源。
      return requestJson<GuiMemorySummaryPayload>("/v2/gui/memory/summary"); // 新增代码+DesktopGUIMemoryPanelClient：请求 V2 memory summary endpoint；如果没有这行，前端只能停在空面板或硬编码状态。
    }, // 新增代码+DesktopGUIMemoryPanelClient：memorySummary 方法结束；如果没有这行，返回对象语法不完整。
    promptStatus(): Promise<GuiPromptStatusPayload> { // 新增代码+DesktopGUIMemoryPanelClient：读取 prompt surface 和 token budget 状态；如果没有这段，预算页签拿不到真实上下文阈值。
      return requestJson<GuiPromptStatusPayload>("/v2/gui/prompt/status"); // 新增代码+DesktopGUIMemoryPanelClient：请求 V2 prompt status endpoint；如果没有这行，prompt 工具状态不会进入 GUI。
    }, // 新增代码+DesktopGUIMemoryPanelClient：promptStatus 方法结束；如果没有这行，返回对象语法不完整。
    notebookStatus(): Promise<GuiNotebookStatusPayload> { // 新增代码+DesktopGUIMemoryPanelClient：读取 notebook 工具和文件状态；如果没有这段，notebook_read/edit 接入状态不可见。
      return requestJson<GuiNotebookStatusPayload>("/v2/gui/notebook/status"); // 新增代码+DesktopGUIMemoryPanelClient：请求 V2 notebook status endpoint；如果没有这行，notebook 面板无法从后端拿事实。
    }, // 新增代码+DesktopGUIMemoryPanelClient：notebookStatus 方法结束；如果没有这行，返回对象语法不完整。
    ...
    acceptanceScenarios(): Promise<GuiAcceptanceScenariosPayload> { // 新增代码+DesktopGUIAcceptanceClient：读取验收场景总览；如果没有这段，验收页签拿不到 acceptance_controller 事实源。
      return requestJson<GuiAcceptanceScenariosPayload>("/v2/gui/acceptance/scenarios"); // 新增代码+DesktopGUIAcceptanceClient：请求场景端点；如果没有这行，前端只能停在空面板或硬编码状态。
    }, // 新增代码+DesktopGUIAcceptanceClient：acceptanceScenarios 方法结束；如果没有这行，返回对象语法不完整。
    acceptanceRuns(): Promise<GuiAcceptanceRunsPayload> { // 新增代码+DesktopGUIAcceptanceClient：读取验收运行证据；如果没有这段，验收页签无法按需刷新历史 run。
      return requestJson<GuiAcceptanceRunsPayload>("/v2/gui/acceptance/runs"); // 新增代码+DesktopGUIAcceptanceClient：请求运行端点；如果没有这行，前端无法读取 result/events/screenshot 证据。
    }, // 新增代码+DesktopGUIAcceptanceClient：acceptanceRuns 方法结束；如果没有这行，返回对象语法不完整。
    runAcceptanceScenario(scenarioId: string, dryRun = false): Promise<GuiAcceptanceRunActionPayload> { // 新增代码+DesktopGUIAcceptanceClient：请求启动一个验收场景；如果没有这段，运行按钮无法调用后端 controller.ps1。
      return postJson<GuiAcceptanceRunActionPayload>("/v2/gui/acceptance/run", { scenario_id: scenarioId, dry_run: dryRun }); // 新增代码+DesktopGUIAcceptanceClient：发送场景 id 和可选 dry-run；如果没有这行，后端不知道要跑哪个场景。
    }, // 新增代码+DesktopGUIAcceptanceClient：runAcceptanceScenario 方法结束；如果没有这行，返回对象语法不完整。
