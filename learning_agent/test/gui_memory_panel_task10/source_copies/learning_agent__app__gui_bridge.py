...
import os  # 新增代码+RealHarnessAdapter：读取真实 agent runtime feature flag；如果没有这一行，bridge 无法按环境开关启用 core adapter。
...
from learning_agent.app.gui_browser_control import build_gui_browser_action_payload, build_gui_browser_collection_payload, build_gui_browser_workbench_payload  # 新增代码+DesktopGUIBrowserWorkbench：导入 Browser GUI 薄适配器；如果没有这行，bridge 只能显示旧 provider 摘要，无法提供 tabs/console/network/open/refresh 路由。
from learning_agent.app.gui_computer_use import build_gui_computer_use_action_payload, build_gui_computer_use_workbench_payload  # 新增代码+DesktopGUIComputerUseWorkbench：导入 Computer Use GUI 薄适配器；如果没有这行，bridge 需要继续手写状态和动作路由。
from learning_agent.app.gui_context import build_gui_context_for_turn  # 新增代码+GuiContextBridge：导入 GUI 上下文构建器；如果没有这一行，bridge 只能把最新 prompt 交给 adapter，历史消息无法进入真实模型。
from learning_agent.app.gui_harness_controls import build_gui_harness_control_payload as build_real_gui_harness_control_payload, build_gui_harness_controls_capabilities  # 新增代码+DesktopGUIHarnessControls：导入真实 Harness 控制适配器；如果没有这行，GUI 只能返回 pause/resume 占位响应。
from learning_agent.app.gui_mcp_control import build_gui_mcp_collection_payload  # 新增代码+DesktopGUIMcpControl：导入 MCP 只读管理 payload helper；如果没有这行，bridge 无法把 MCP server/resource/prompt 状态接到 GUI。
from learning_agent.app.gui_memory import build_gui_memory_summary_payload, build_gui_notebook_status_payload, build_gui_prompt_status_payload  # 新增代码+DesktopGUIMemoryPanel：导入记忆、提示词预算和 Notebook 只读 payload helper；如果没有这行，bridge 无法把长期上下文状态接到右侧 GUI。
...
from learning_agent.app.gui_execution import build_gui_command_console_payload, build_gui_command_stop_payload, build_gui_command_tail_payload  # 新增代码+DesktopGUICommandConsole：导入后台命令控制台 payload helper；如果没有这行，bridge 无法把 TaskRegistry 后台命令状态暴露给 GUI。
from learning_agent.app.gui_acceptance import build_gui_acceptance_run_payload, build_gui_acceptance_runs_payload, build_gui_acceptance_scenarios_payload  # 新增代码+DesktopGUIAcceptance：导入验收控制中心 payload helper；如果没有这行，bridge 无法把 acceptance_controller 场景、证据和启动入口暴露给 GUI。
...
from learning_agent.app.gui_provider_auth_attempts import cancel_provider_auth_attempt, complete_provider_auth_attempt, get_provider_auth_attempt_status, start_provider_auth_attempt  # 新增代码+OpenAIAuthAttempt：导入 provider auth-attempt 状态机；如果没有这行，browser/headless 连接向导没有后端路由。
from learning_agent.app.gui_provider_openai_models import build_openai_model_registry_payload  # 新增代码+OpenAIModelRegistry：导入 OpenAI 模型注册表 payload；如果没有这行，bridge 无法提供独立模型状态端点。
from learning_agent.app.gui_provider_settings import GuiProviderSettingsError, build_provider_settings_payload, disconnect_provider, load_provider_settings, save_custom_provider, set_model_visibility, set_provider_auth, test_provider_connection  # 修改代码+OpenAIModelRegistry：导入 Provider 设置 helper、mutation、连接探针和 settings 读取；如果没有这行，bridge 无法返回或修改 provider/model 设置页数据。
...
from learning_agent.app.gui_toolchain import build_gui_planning_payload, build_gui_toolchain_payload  # 修改代码+DesktopGUIPlanning：同时导入工具链清单和计划协作 helper；如果没有这行，GUI bridge 无法把 todo/task/team 状态接到右侧面板。
from learning_agent.mcp.runtime import McpToolRegistry, load_mcp_server_configs  # 新增代码+GuiToolchainReuse：复用终端 agent 的 MCP 配置加载和 registry；如果没有这一行，GUI full_access 无法接入 mcp_servers.json。
...
GUI_ALLOWED_ORIGINS = {"", "null", "file://"}  # 修改代码+DesktopGUICors：保留无 Origin、null 和 file 这些桌面安全来源；如果没有这行，Electron 本地页面和标准库测试会被来源门禁挡住。
GUI_ALLOWED_LOOPBACK_ORIGIN_HOSTS = {"127.0.0.1", "localhost"}  # 新增代码+DesktopGUICors：列出允许的本机开发服务器主机；如果没有这行，5277 等备用 Vite 端口会被真实浏览器预检拒绝。
GUI_ALLOWED_LOOPBACK_ORIGIN_SCHEMES = {"http"}  # 新增代码+DesktopGUICors：只允许本机 http 开发来源；如果没有这行，来源判断会过宽，后续维护者难以看出安全边界。
...
GUI_REASONING_EFFORTS = {"low", "medium", "high", "ultra"}  # 新增代码+DirectSSEPayload：集中定义 GUI 可传的推理强度；如果没有这行，前端传错值可能直接污染真实模型请求。
GUI_PERMISSION_MODES = {"read_only", "workspace_write", "full_access"}  # 新增代码+DirectSSEPayload：集中定义 GUI 可传的权限模式；如果没有这行，权限选择字段没有后端白名单。
GuiAnswerRunner = Callable[[str, str, str], str]  # 新增代码+DesktopGUILifecycle：定义后端回答 runner 插入点；如果没有这行，未来接真实 agent 时接口不清楚。


def _origin_is_allowed(origin: str) -> bool:  # 新增代码+DesktopGUICors：函数段开始，判断浏览器来源是否属于桌面 GUI 允许范围；如果没有这段，备用本机 Vite 端口会在真实 GUI 验收时加载失败。
    if origin in GUI_ALLOWED_ORIGINS:  # 新增代码+DesktopGUICors：先接受固定安全来源；如果没有这行，file/null/无来源这些桌面入口会被误拒绝。
        return True  # 新增代码+DesktopGUICors：固定来源命中时直接放行；如果没有这行，后续 URL 解析会错误处理空来源。
    try:  # 新增代码+DesktopGUICors：保护 URL 解析过程；如果没有这行，畸形 Origin 可能把 bridge handler 打成异常。
        parsed_origin = urllib.parse.urlparse(origin)  # 新增代码+DesktopGUICors：解析 Origin 的协议、主机和端口；如果没有这行，无法安全地区分本机 dev server 和外部网页。
        parsed_port = parsed_origin.port  # 新增代码+DesktopGUICors：触发并读取端口校验；如果没有这行，非法端口字符串可能绕过后续判断。
    except ValueError:  # 新增代码+DesktopGUICors：处理非法端口或畸形 URL；如果没有这行，坏 Origin 会变成 500 而不是 403。
        return False  # 新增代码+DesktopGUICors：非法来源一律拒绝；如果没有这行，安全门禁会缺少明确失败路径。
    if parsed_origin.scheme not in GUI_ALLOWED_LOOPBACK_ORIGIN_SCHEMES:  # 新增代码+DesktopGUICors：限制来源协议；如果没有这行，非 http scheme 可能被误当成本机 GUI。
        return False  # 新增代码+DesktopGUICors：协议不匹配时拒绝；如果没有这行，后续主机判断会放大来源范围。
    if parsed_origin.hostname not in GUI_ALLOWED_LOOPBACK_ORIGIN_HOSTS:  # 新增代码+DesktopGUICors：限制来源主机必须是本机回环地址；如果没有这行，恶意网页域名可能借浏览器访问 bridge。
        return False  # 新增代码+DesktopGUICors：非本机主机拒绝；如果没有这行，token 之外会少一层浏览器来源保护。
    if parsed_port is None:  # 新增代码+DesktopGUICors：要求开发来源显式带端口；如果没有这行，普通 http://localhost 来源也会被放行，边界不够清楚。
        return False  # 新增代码+DesktopGUICors：无端口来源拒绝；如果没有这行，来源范围会比 Vite/Electron 真实需求更宽。
    return True  # 新增代码+DesktopGUICors：确认是本机带端口 dev server 后放行；如果没有这行，备用端口下的真实 GUI 仍会加载失败。
# 新增代码+DesktopGUICors：函数段结束，_origin_is_allowed 到此结束；如果没有边界说明，用户不容易看出它只负责 Origin 白名单判断。
...
class GuiTurn:  # 新增代码+DesktopGUILifecycle：turn 类段开始，保存一次 GUI prompt 生命周期；如果没有这个类，cancel/retry/resume 无法共享状态。
    ...
    provider_id: str = ""  # 新增代码+DirectSSEPayload：保存前端选择的 provider；如果没有这行，真实模型路由无法知道用户选了 OpenAI 还是其它 provider。
    model_id: str = ""  # 新增代码+DirectSSEPayload：保存前端选择的模型；如果没有这行，后端只能继续使用硬编码或默认模型。
    reasoning_effort: str = "high"  # 新增代码+DirectSSEPayload：保存推理强度默认值；如果没有这行，Codex 风格“低/中/高/超高”选择无法进入运行时。
    permission_mode: str = "full_access"  # 新增代码+DirectSSEPayload：保存权限模式默认值；如果没有这行，composer 底部权限选择无法影响后端。
...
def build_gui_bootstrap_payload(workspace: str | Path) -> dict[str, Any]:  # 修改代码+DesktopGUIBridge：函数段开始，生成 GUI 启动首屏数据；如果没有这段，桌面壳需要调多个端点拼状态。
    ...
    return {  # 修改代码+DesktopGUIBridge：返回稳定 JSON 对象；如果没有这行，前端无法渲染启动页。
        ...
        "feature_flags": {  # 修改代码+DesktopGUIBridge：返回后端能力开关；如果没有这段，前端无法按能力显示 UI。
            ...
            "memory_panel": True,  # 新增代码+DesktopGUIMemoryPanel：声明记忆和 token 状态面板可用；如果没有这行，前端无法从启动能力里确认长期上下文入口。
...
def _runtime_browser_panel_from_snapshot(workspace: str | Path, snapshot: dict[str, Any]) -> dict[str, Any]:  # 修改代码+DesktopGUIBrowserWorkbench：函数段开始，把 runtime browser panel 委托给 Browser workbench；如果没有这段，runtime panels 和新 Browser 端点会返回两套不同合同。
    return build_gui_browser_workbench_payload(workspace, snapshot=snapshot)  # 修改代码+DesktopGUIBrowserWorkbench：复用 Browser 薄适配器生成 provider/tabs/console/network/downloads/replay；如果没有这行，右侧浏览器页签仍缺少调试摘要和动作状态。
# 修改代码+DesktopGUIBrowserWorkbench：函数段结束，_runtime_browser_panel_from_snapshot 到此结束；如果没有边界说明，初学者不易看出它只是 bridge 委托层。
...
def _runtime_computer_use_panel(workspace: str | Path, snapshot: dict[str, Any] | None = None) -> dict[str, Any]:  # 修改代码+DesktopGUIComputerUseWorkbench：函数段开始，把旧摘要入口委派给新 workbench payload；如果没有这段，runtime panels 和动作端点会返回两套不同 Computer Use 合同。
    return build_gui_computer_use_workbench_payload(workspace, snapshot=snapshot)  # 修改代码+DesktopGUIComputerUseWorkbench：复用新适配器生成完整 payload；如果没有这行，last observation、last action 和安全按钮状态不会进入旧面板入口。
# 修改代码+DesktopGUIComputerUseWorkbench：函数段结束，_runtime_computer_use_panel 到此结束；如果没有边界说明，初学者不易看出它只是 bridge 委托层。
...
def _runtime_permissions_panel_from_snapshot(snapshot: dict[str, Any]) -> dict[str, Any]:  # 新增代码+DesktopRuntimePanels：函数段开始，从事件尾巴生成权限摘要；如果没有这段，权限状态仍只藏在原始事件流。
    ...
    pending_events = [event for event in events if isinstance(event, dict) and str(event.get("event_type", "")) in {"permission_required", "permission_requested", "gui_turn_needs_permission"}]  # 修改代码+RealHarnessPermission：把真实 adapter 发出的 permission_requested 也算进待办；如果没有这行，右侧面板会漏掉 core agent 的权限请求。
...
def build_gui_runtime_panels_payload(workspace: str | Path) -> dict[str, Any]:  # 新增代码+DesktopRuntimePanels：函数段开始，生成 V2 浏览器和 Computer Use 聚合面板 payload；如果没有这段，前端要多次请求并自己拼安全状态。
    ...
    browser_panel = _runtime_browser_panel_from_snapshot(workspace_path, safe_snapshot)  # 修改代码+DesktopGUIBrowserWorkbench：生成完整 Browser workbench 面板摘要；如果没有这行，runtime panels 不会带 console/network/downloads/replay。
    computer_use_panel = _runtime_computer_use_panel(workspace_path, snapshot=safe_snapshot)  # 修改代码+DesktopGUIComputerUseWorkbench：生成 Computer Use workbench 面板并复用已读快照；如果没有这行，last observation 会重复读磁盘或缺失。
...
def _gui_harness_active_run(runs: list[dict[str, Any]]) -> dict[str, Any]:  # 新增代码+DesktopGUIHarnessPanel：函数段开始，从 run 列表选择当前目标；如果没有这段，右侧面板不知道哪个长任务是主目标。
    active_statuses = {"running", "queued", "paused", "needs_permission", "cancelling"}  # 修改代码+DesktopGUIHarnessControls：把 paused 也视为当前任务；如果没有这行，用户一暂停任务，面板可能切到旧任务。
...
def _gui_harness_queue_from_snapshot(snapshot: dict[str, Any], runs: list[dict[str, Any]]) -> list[dict[str, Any]]:  # 新增代码+DesktopGUIHarnessPanel：函数段开始，生成队列摘要；如果没有这段，长任务等待项仍只藏在 runtime queue 或 runs 里。
    ...
    for run in runs:  # 新增代码+DesktopGUIHarnessPanel：遍历 harness runs；如果没有这行，queued/running run 不会作为长任务队列项出现。
        if str(run.get("status", "")) not in {"queued", "running", "paused"}:  # 修改代码+DesktopGUIHarnessControls：队列也展示 paused run；如果没有这行，暂停后的长任务会从任务区消失。
...
def build_gui_harness_status_payload(workspace: str | Path) -> dict[str, Any]:  # 新增代码+DesktopGUIHarnessPanel：函数段开始，生成 V2 长任务 Harness 面板 payload；如果没有这段，前端只能显示静态任务面板。
    ...
    return {"ok": True, "schema_version": GUI_SCHEMA_VERSION, "active_goal": _gui_harness_goal_from_run(active_run), "queue": _gui_harness_queue_from_snapshot(safe_snapshot, runs), "checkpoints": checkpoints, "last_progress": last_progress, "blocked_reason": _gui_harness_blocked_reason(safe_snapshot, active_run), "safe_error": safe_error, "status_degraded": status_degraded, "controls": build_gui_harness_controls_capabilities()}  # 修改代码+DesktopGUIHarnessControls：返回真实 Harness 控制能力；如果没有这行，前端会继续隐藏暂停、恢复、停止和 checkpoint 按钮。
# 新增代码+DesktopGUIHarnessPanel：函数段结束，build_gui_harness_status_payload 到此结束；如果没有边界说明，初学者不易看出它只负责面板数据。


def build_gui_harness_control_payload(action: str, workspace: str | Path) -> dict[str, Any]:  # 修改代码+DesktopGUIHarnessControls：函数段开始，把控制请求转交真实 harness store；如果没有这段，按钮只能返回 unsupported。
    return build_real_gui_harness_control_payload(action, workspace)  # 修改代码+DesktopGUIHarnessControls：复用独立适配层执行真实控制；如果没有这行，bridge 会重复实现文件级控制逻辑。
# 修改代码+DesktopGUIHarnessControls：函数段结束，build_gui_harness_control_payload 到此结束；如果没有边界说明，初学者不易看出它只负责 bridge 委派。
...
class GuiRunManager:  # 新增代码+DesktopGUILifecycle：类段开始，管理 GUI turn 生命周期；如果没有这个类，handler 会堆满业务状态逻辑。
    ...
    def _load_state(self) -> None:  # 新增代码+DesktopGUILifecycle：函数段开始，从磁盘恢复 GUI 状态；如果没有这段，resume 只能在进程内有效。
        ...
        reconciled_any = False  # 新增代码+RealHarnessRecovery：记录是否修复了重启前遗留的运行态；如果没有这行，后面无法判断是否需要落盘恢复结果。
        for loaded_turn in self.turns.values():  # 新增代码+RealHarnessRecovery：逐个检查落盘 turn；如果没有这行，旧 running/needs_permission 状态可能在重启后继续悬挂。
            if loaded_turn.status not in GUI_RUNNING_STATES:  # 新增代码+RealHarnessRecovery：只处理非终态运行状态；如果没有这行，已完成或已失败的历史会被误改。
                continue  # 新增代码+RealHarnessRecovery：终态 turn 保持原样；如果没有这行，恢复过程会污染正常历史记录。
            loaded_turn.status = "failed"  # 新增代码+RealHarnessRecovery：把孤儿运行态收敛成失败；如果没有这行，GUI 会显示永远运行中的假状态。
            loaded_turn.error = "GUI bridge 重启后，上一轮真实 agent 运行已中断，请重新发起或重试。"  # 新增代码+RealHarnessRecovery：写入人类可读恢复说明；如果没有这行，用户不知道为什么旧任务失败。
            loaded_session = self.sessions.get(loaded_turn.session_id)  # 新增代码+RealHarnessRecovery：读取关联会话；如果没有这行，助手消息状态无法同步。
            if loaded_session is not None:  # 新增代码+RealHarnessRecovery：只有会话存在时才同步消息；如果没有这行，坏 state 可能触发 None 访问错误。
                for message in loaded_session.messages:  # 新增代码+RealHarnessRecovery：遍历该会话消息；如果没有这行，助手占位仍会显示 running。
                    if message.turn_id == loaded_turn.turn_id and message.role == "assistant":  # 新增代码+RealHarnessRecovery：只更新对应助手消息；如果没有这行，用户消息可能被误改为失败。
                        message.status = "failed"  # 新增代码+RealHarnessRecovery：同步消息失败状态；如果没有这行，消息列表和 turn 状态会不一致。
                        message.text = loaded_turn.error  # 新增代码+RealHarnessRecovery：同步可见失败说明；如果没有这行，用户只能看到空白失败消息。
            self.event_store.append("gui_turn_failed", {"answer": "", "error": loaded_turn.error, "recovered_after_restart": True}, session_id=loaded_turn.session_id, run_id=loaded_turn.run_id, turn_id=loaded_turn.turn_id)  # 新增代码+RealHarnessRecovery：写入恢复审计事件；如果没有这行，重启收敛没有事实流证据。
            reconciled_any = True  # 新增代码+RealHarnessRecovery：标记本次加载做过修复；如果没有这行，状态修复后不会保存。
        if reconciled_any:  # 新增代码+RealHarnessRecovery：只有实际修复时才写回磁盘；如果没有这行，每次启动都会不必要改写 state 文件。
            self._save_state()  # 新增代码+RealHarnessRecovery：保存孤儿运行态收敛结果；如果没有这行，下一次启动还会看到旧 running 状态。
    ...
    def _agent_permission_timeout_seconds(self) -> float:  # 新增代码+RealHarnessPermission：函数段开始，读取真实 agent 权限等待超时；如果没有这段，权限弹窗卡住时 adapter 没有安全退出点。
        try:  # 新增代码+RealHarnessPermission：保护环境变量解析；如果没有这行，坏环境值会让 GUI worker 崩溃。
            return max(float(os.environ.get("OPENHARNESS_GUI_AGENT_PERMISSION_TIMEOUT_SECONDS", "300")), 1.0)  # 新增代码+RealHarnessPermission：返回至少 1 秒的等待时间；如果没有这行，权限请求可能零秒超时或无限等待。
        except ValueError:  # 新增代码+RealHarnessPermission：处理非数字环境变量；如果没有这行，用户写错配置会变成线程异常。
            return 300.0  # 新增代码+RealHarnessPermission：坏配置回退 300 秒；如果没有这行，真实 GUI 验收窗口可能不够点击。
    # 新增代码+RealHarnessPermission：函数段结束，_agent_permission_timeout_seconds 到此结束；如果没有这个边界，用户不容易看出它只负责超时配置。

    def _ask_permission_for_adapter(self, request: GuiAgentRunRequest, text: str) -> bool:  # 新增代码+RealHarnessPermission：函数段开始，把 core ask_permission 转成 GUI 可点击权限请求；如果没有这段，真实工具权限不会进入桌面弹窗。
        request_id = _new_id("perm")  # 新增代码+RealHarnessPermission：为本次权限请求生成唯一 id；如果没有这行，前端无法把允许或拒绝回传给后端。
        self.record_permission_required(request_id, turn_id=request.turn_id, app_name="OpenHarness Agent", tool_name="agent_harness", reason=text or "真实 agent 请求继续执行需要确认。", risk_summary="请确认该真实 agent 操作符合你的预期。")  # 新增代码+RealHarnessPermission：登记权限请求并写入事件流；如果没有这行，PermissionDialog 不会出现。
        deadline = time.monotonic() + self._agent_permission_timeout_seconds()  # 新增代码+RealHarnessPermission：计算等待截止时间；如果没有这行，权限请求可能永久占住 agent。
        cancel_event = self.cancel_events.get(request.turn_id)  # 新增代码+RealHarnessPermission：读取本轮取消信号；如果没有这行，用户点取消时权限等待不会结束。
        while time.monotonic() < deadline:  # 新增代码+RealHarnessPermission：循环等待用户回答；如果没有这行，函数无法把异步弹窗结果变成同步 bool。
            if cancel_event is not None and cancel_event.is_set():  # 新增代码+RealHarnessPermission：优先响应 GUI 取消；如果没有这行，取消按钮会被权限等待拖住。
                return False  # 新增代码+RealHarnessPermission：取消时拒绝权限；如果没有这行，core 工具可能在取消后继续执行。
            with self.lock:  # 新增代码+RealHarnessPermission：读取权限状态时加锁；如果没有这行，decision endpoint 并发写入可能被读到半状态。
                permission = self.permissions.get(request_id)  # 新增代码+RealHarnessPermission：查找当前权限请求；如果没有这行，无法知道用户是否已回答。
                if permission is not None and permission.status == "approved":  # 新增代码+RealHarnessPermission：识别用户允许；如果没有这行，允许按钮不会放行 core 工具。
                    return True  # 新增代码+RealHarnessPermission：允许时返回 True；如果没有这行，core agent 总会被拒绝。
                if permission is not None and permission.status == "denied":  # 新增代码+RealHarnessPermission：识别用户拒绝；如果没有这行，拒绝按钮不会阻止 core 工具。
                    return False  # 新增代码+RealHarnessPermission：拒绝时返回 False；如果没有这行，拒绝仍可能继续执行。
            time.sleep(0.05)  # 新增代码+RealHarnessPermission：短暂休眠避免忙等；如果没有这行，后台线程会空转占用 CPU。
        with self.lock:  # 新增代码+RealHarnessPermission：超时后加锁写入自动拒绝审计；如果没有这行，超时状态可能和用户点击冲突。
            permission = self.permissions.get(request_id)  # 新增代码+RealHarnessPermission：再次读取权限请求；如果没有这行，超时无法确认请求仍然存在。
            if permission is not None and permission.status == "pending":  # 新增代码+RealHarnessPermission：只处理仍未回答的请求；如果没有这行，可能覆盖用户刚刚点击的结果。
                permission.status = "denied"  # 新增代码+RealHarnessPermission：超时按自动拒绝处理；如果没有这行，pending 请求会一直悬挂。
                permission.decision = "deny"  # 新增代码+RealHarnessPermission：记录拒绝决策；如果没有这行，审计 payload 不知道实际结果。
                permission.decision_reason = "权限请求等待超时，后端已自动拒绝。"  # 新增代码+RealHarnessPermission：写入可读超时原因；如果没有这行，用户不知道为什么工具没有继续。
                permission.answered_at = time.time()  # 新增代码+RealHarnessPermission：记录自动回答时间；如果没有这行，审计无法计算等待时长。
                self.event_store.append("permission_answered", self._permission_payload(permission), session_id=permission.session_id, run_id=permission.run_id, turn_id=permission.turn_id)  # 新增代码+RealHarnessPermission：写入自动拒绝事件；如果没有这行，事件流缺少超时证据。
                self._save_state()  # 新增代码+RealHarnessPermission：保存自动拒绝状态；如果没有这行，重启后权限仍像未回答。
        return False  # 新增代码+RealHarnessPermission：超时统一拒绝权限；如果没有这行，调用方拿不到安全结果。
    # 新增代码+RealHarnessPermission：函数段结束，_ask_permission_for_adapter 到此结束；如果没有这个边界，用户不容易看出权限等待范围。
    ...
    def _adapter_for_turn(self, turn: GuiTurn) -> GuiAgentAdapter:  # 新增代码+GuiAgentAdapter：函数段开始，选择本轮使用 fake adapter 还是真实 harness shell；如果没有这段，真实模式触发语义会散落在 worker 中。
        real_requested = "__real_harness__" in turn.prompt or "__harness_adapter__" in turn.prompt or os.environ.get("OPENHARNESS_GUI_AGENT_DEFAULT_MODE", "").strip().casefold() in {"agent", "real", "harness"}  # 修改代码+RealHarnessAdapter：识别显式触发词或默认真实模式；如果没有这一行，GUI 无法稳定进入 core adapter。
        real_enabled = os.environ.get("OPENHARNESS_GUI_AGENT_RUNTIME", "").strip().casefold() in {"1", "true", "yes", "on", "enabled", "real"}  # 新增代码+RealHarnessAdapter：读取真实 runtime 开关；如果没有这一行，真实 adapter 会被误启用或永远不可用。
        permission_mode = str(turn.permission_mode or "full_access").strip().casefold().replace("-", "_")  # 新增代码+GuiToolchainReuse：规范化 GUI 权限模式；如果没有这一行，workspace-write 这类前端变体会映射失败。
        read_only_enabled = os.environ.get("OPENHARNESS_GUI_AGENT_READ_ONLY_TOOLS", "").strip().casefold() in {"1", "true", "yes", "on", "enabled", "read_only"} or "__read_only_tool__" in turn.prompt or permission_mode == "read_only"  # 修改代码+GuiToolchainReuse：识别只读工具开放条件；如果没有这行，read_only 下拉无法真正收窄工具。
        workspace_write_enabled = permission_mode in {"workspace_write", "workspace", "write"}  # 新增代码+GuiToolchainReuse：识别工作区写入模式；如果没有这一行，GUI 选择工作区写入仍不会开放写文件工具。
        if read_only_enabled:  # 新增代码+GuiToolchainReuse：只读模式走最小工具白名单；如果没有这一行，低风险任务会继承写入或 shell 能力。
            allowed_tool_names: set[str] | None = {"read_file", "list_dir"}  # 修改代码+GuiToolchainReuse：只允许安全读文件和列目录；如果没有这行，read_only 会暴露写入工具。
            max_turns = 2  # 新增代码+GuiToolchainReuse：只读工具需要请求工具和工具后回答两轮；如果没有这一行，读文件后可能没有最终回答。
        elif workspace_write_enabled:  # 新增代码+GuiToolchainReuse：工作区写入模式开放文件和记忆工具；如果没有这一行，写入下拉无法驱动 core 工具链路。
            allowed_tool_names = {"read_file", "list_dir", "write_file", "append_memory", "read", "write", "edit", "tool_search"}  # 新增代码+GuiToolchainReuse：同时开放旧兼容文件工具和新原子文件工具；如果没有这一行，新旧模型任一工具名都会被白名单误挡。
            max_turns = 4  # 新增代码+GuiToolchainReuse：写入任务通常需要工具调用、权限确认和工具后总结；如果没有这一行，agent 容易停在工具结果前。
        else:  # 新增代码+GuiToolchainReuse：完全访问或未知模式交给原工具策略；如果没有这一行，full_access 不能复用现有 shell/MCP/Computer Use 门禁。
            allowed_tool_names = None  # 新增代码+GuiToolchainReuse：None 表示不额外加 GUI 白名单；如果没有这一行，所有工具链路会被空集合封死。
            max_turns = 8  # 新增代码+GuiToolchainReuse：全工具任务需要多轮选择工具和执行；如果没有这一行，tool_search/select 后没有足够轮次继续执行。
        mcp_tool_registry = McpToolRegistry.from_configs(load_mcp_server_configs(self.workspace)) if allowed_tool_names is None else None  # 新增代码+GuiToolchainReuse：full_access 时加载 workspace MCP registry；如果没有这一行，GUI 无法复用终端的 mcp_servers.json 外部工具。
        if real_requested:  # 修改代码+RealHarnessAdapter：只在显式请求真实路径时返回 harness shell；如果没有这一行，普通 fake 基线会被破坏。
            return DefaultHarnessGuiAgentAdapter(enabled=real_enabled, permission_requester=self._ask_permission_for_adapter, max_turns=max_turns, allowed_tool_names=allowed_tool_names, mcp_tool_registry=mcp_tool_registry)  # 修改代码+GuiToolchainReuse：把真实 adapter 接到 GUI 权限回调、工具模式和 MCP registry；如果没有这一行，GUI 工具链路无法复用现有模块。
    ...
    def _build_context_for_turn(self, turn: GuiTurn):  # 新增代码+GuiContextBridge：函数段开始，为单个 GUI turn 构建可发给模型的多轮上下文；如果没有这段，_run_adapter_turn 会继续只传最新 prompt。
        session = self._session(turn.session_id)  # 新增代码+GuiContextBridge：读取当前 turn 所属会话；如果没有这一行，builder 拿不到 GUI 历史消息事实源。
        session_messages = list(session.messages)  # 新增代码+GuiContextBridge：复制当前消息列表快照；如果没有这一行，后台 adapter 运行时可能读到后续并发修改。
        context_result = build_gui_context_for_turn(session_messages=session_messages, current_prompt=turn.prompt, session_id=turn.session_id, run_id=turn.run_id, turn_id=turn.turn_id, workspace=self.workspace)  # 新增代码+GuiContextBridge：调用复用 compact pipeline 的 GUI context builder；如果没有这一行，历史消息不会变成 Direct SSE 的 Responses input。
        self.event_store.append("context_budget", context_result.event_payload, session_id=turn.session_id, run_id=turn.run_id, turn_id=turn.turn_id)  # 新增代码+GuiContextBridge：写入脱敏预算事件；如果没有这一行，真实 GUI 验收无法看到上下文是否进入预算链路。
        if context_result.compacted:  # 新增代码+GuiContextBridge：判断本轮是否发生主动压缩；如果没有这一行，compact_completed 和 compact_skipped 会混在一起无法区分。
            self.event_store.append("compact_completed", context_result.event_payload, session_id=turn.session_id, run_id=turn.run_id, turn_id=turn.turn_id)  # 新增代码+GuiContextBridge：写入脱敏压缩完成事件；如果没有这一行，强制阈值 GUI 验收无法确认 compact 已发生。
        else:  # 新增代码+GuiContextBridge：处理未压缩分支；如果没有这一行，短上下文也缺少可观察的跳过事件。
            self.event_store.append("compact_skipped", context_result.event_payload, session_id=turn.session_id, run_id=turn.run_id, turn_id=turn.turn_id)  # 新增代码+GuiContextBridge：写入脱敏压缩跳过事件；如果没有这一行，短对话无法解释为什么没有 compact_completed。
        return context_result  # 新增代码+GuiContextBridge：返回上下文构建结果；如果没有这一行，调用方拿不到 messages 和 context_metadata。
    # 新增代码+GuiContextBridge：函数段结束，_build_context_for_turn 到此结束；如果没有边界说明，用户不容易看出它只负责上下文和事件。
    ...
    def _run_adapter_turn(self, turn: GuiTurn) -> GuiAgentRunResult:  # 新增代码+GuiAgentAdapter：函数段开始，把 GUI turn 转交给 adapter；如果没有这段，worker 只能调用旧 answer_runner。
        ...
        context_result = self._build_context_for_turn(turn)  # 新增代码+GuiContextBridge：在调用 adapter 前构建多轮上下文；如果没有这一行，Direct SSE adapter 只会看到最新 prompt。
        request = GuiAgentRunRequest(session_id=turn.session_id, turn_id=turn.turn_id, run_id=turn.run_id, prompt=turn.prompt, workspace=str(self.workspace), mode="real" if isinstance(adapter, DefaultHarnessGuiAgentAdapter) else "fake", provider_id=turn.provider_id, model_id=turn.model_id, reasoning_effort=turn.reasoning_effort, permission_mode=turn.permission_mode, messages=context_result.messages, context_metadata=context_result.event_payload)  # 修改代码+GuiContextBridge：构造 adapter 请求并带上 workspace、模型、权限和预构建上下文；如果没有这一行，adapter 无法复用 GUI 历史和 compact 预算结果。
    ...
    def record_permission_required(self, request_id: str, turn_id: str = "", app_name: str = "", reason: str = "", risk_summary: str = "", tool_name: str = "") -> dict[str, Any]:  # 修改代码+DesktopGUIPermissionsV2：函数段开始，把后端权限请求登记为 GUI 可见请求；如果没有这段，权限事件无法变成桌面弹窗和 needs_permission 状态。
        ...
        with self.lock:  # 新增代码+DesktopGUIPermissions：保护权限请求登记；如果没有这行，并发权限事件可能重复写同一 request。
            ...
            self.event_store.append("permission_requested", payload, session_id=permission.session_id, run_id=permission.run_id, turn_id=permission.turn_id)  # 新增代码+RealHarnessPermission：写入真实 agent 统一权限请求事件；如果没有这行，按蓝图监听 permission_requested 的 GUI 不会弹窗。
    ...
    def start_message(self, conversation_id: str, prompt: str, client_request_id: str = "", retry_of: str = "", provider_id: str | None = None, model_id: str | None = None, reasoning_effort: str = "high", permission_mode: str = "full_access") -> dict[str, Any]:  # 修改代码+DirectSSEPayload：函数段开始，提交 GUI prompt 并接收模型路由字段；如果没有这段，POST /messages 无法创建可选模型的 turn。
        ...
        clean_provider_id = provider_id.strip() if isinstance(provider_id, str) else ""  # 新增代码+DirectSSEPayload：清理 providerId 字段；如果没有这行，空格或非字符串会进入后端路由。
        clean_model_id = model_id.strip() if isinstance(model_id, str) else ""  # 新增代码+DirectSSEPayload：清理 modelId 字段；如果没有这行，模型名可能携带隐藏空白。
        clean_reasoning_effort = reasoning_effort if reasoning_effort in GUI_REASONING_EFFORTS else "high"  # 新增代码+DirectSSEPayload：白名单推理强度并默认 high；如果没有这行，非法值可能传给真实模型 API。
        clean_permission_mode = permission_mode if permission_mode in GUI_PERMISSION_MODES else "full_access"  # 新增代码+DirectSSEPayload：白名单权限模式并默认 full_access；如果没有这行，前端错误值会污染权限语义。
        with self.lock:  # 新增代码+DesktopGUILifecycle：保护 turn 创建；如果没有这行，并发请求可能绕过 busy 检查。
            ...
            turn = GuiTurn(turn_id=_new_id("turn"), run_id=_new_id("run"), session_id=session_id, prompt=clean_prompt, retry_of=retry_of, client_request_id=client_request_id, provider_id=clean_provider_id, model_id=clean_model_id, reasoning_effort=clean_reasoning_effort, permission_mode=clean_permission_mode)  # 修改代码+DirectSSEPayload：创建新 turn 并保存 provider/model/权限字段；如果没有这行，后续 adapter 无法读取用户选择。
            ...
            queued_sequence = self._append_event("gui_turn_queued", turn, {"prompt": clean_prompt, "client_request_id": client_request_id, "provider_id": clean_provider_id, "model_id": clean_model_id, "reasoning_effort": clean_reasoning_effort, "permission_mode": clean_permission_mode})  # 修改代码+DirectSSEPayload：写入 queued 事件并带上路由字段；如果没有这行，测试和诊断无法证明所选模型到达后端。
    ...
    def _run_turn_worker(self, turn_id: str) -> None:  # 新增代码+DesktopGUILifecycle：函数段开始，后台推进 turn；如果没有这段，queued 无法进入 running/completed。
        ...
        with self.lock:  # 修改代码+RealHarnessLifecycle：最终执行前只短暂读取状态；如果没有这行，后续无法安全确认 turn 是否已被取消或拒绝。
            turn = self.turns.get(turn_id)  # 修改代码+RealHarnessLifecycle：重新读取最终 turn；如果没有这行，worker 可能拿着旧对象运行。
            if turn is None:  # 新增代码+RealHarnessLifecycle：处理 turn 被清理的边界；如果没有这行，后续会访问不存在对象。
                return  # 新增代码+RealHarnessLifecycle：目标缺失时退出 worker；如果没有这行，线程会抛异常。
            if turn.status in GUI_TERMINAL_STATES:  # 修改代码+RealHarnessLifecycle：如果权限拒绝或取消已产生终态就停止覆盖；如果没有这行，worker 可能把 failed/cancelled 又改成 completed。
                if self.active_turn_id == turn_id:  # 新增代码+RealHarnessLifecycle：只释放属于当前 worker 的 active 标记；如果没有这行，可能误清其它新 turn。
                    self.active_turn_id = None  # 新增代码+RealHarnessLifecycle：释放当前 active turn；如果没有这行，后续 prompt 可能被 busy 阻塞。
                self._save_state()  # 修改代码+RealHarnessLifecycle：保持已有终态落盘；如果没有这行，重启恢复可能看不到拒绝后的最终状态。
                return  # 修改代码+RealHarnessLifecycle：已有终态时退出 worker；如果没有这行，后续 adapter 仍会继续执行并污染状态。
            run_legacy_runner = self.answer_runner_override is not None  # 新增代码+RealHarnessLifecycle：保存是否走旧 runner 的决定；如果没有这行，锁外执行时逻辑会不清楚。
        adapter_result: GuiAgentRunResult | None = None  # 新增代码+RealHarnessLifecycle：准备保存真实 adapter 结果；如果没有这行，锁外执行后的写回没有统一变量。
        legacy_answer = ""  # 新增代码+RealHarnessLifecycle：准备保存旧 runner 回答；如果没有这行，兼容路径无法在锁外传回结果。
        run_error = ""  # 新增代码+RealHarnessLifecycle：准备保存执行异常文本；如果没有这行，异常路径无法在锁内安全写回。
        try:  # 修改代码+RealHarnessLifecycle：在锁外捕获 runner/adapter 失败；如果没有这行，后台线程异常不会变成 GUI 事件。
            if run_legacy_runner:  # 修改代码+RealHarnessLifecycle：显式注入 runner 时保留 V1 兼容路径；如果没有这一行，旧测试和外部自定义 runner 会被强行改走 fake adapter。
                if "__fail__" in turn.prompt:  # 修改代码+RealHarnessLifecycle：V1 兼容路径仍保留失败触发器；如果没有这行，旧失败合同无法稳定覆盖。
                    raise RuntimeError("测试触发的 GUI turn 失败。")  # 修改代码+RealHarnessLifecycle：抛出可控失败；如果没有这行，失败路径无法覆盖。
                legacy_answer = self.answer_runner(turn.prompt, turn.session_id, turn.turn_id)  # 修改代码+RealHarnessLifecycle：锁外调用显式注入 runner；如果没有这行，兼容路径没有最终回答。
            else:  # 修改代码+RealHarnessLifecycle：默认路径走 V2 adapter；如果没有这一行，普通 GUI prompt 仍不会产生 message_delta。
                adapter_result = self._run_adapter_turn(turn)  # 修改代码+RealHarnessLifecycle：锁外执行 fake 或真实 harness shell adapter；如果没有这一行，权限弹窗和 cancel endpoint 会被长任务锁住。
        except Exception as error:  # 修改代码+RealHarnessLifecycle：处理 runner/adapter 异常；如果没有这行，失败不会变成结构化事件。
            run_error = f"GUI turn 运行失败：{error}"  # 修改代码+RealHarnessLifecycle：保存异常文案供锁内写回；如果没有这行，前端只能看到线程静默结束。
        with self.lock:  # 修改代码+RealHarnessLifecycle：只在最终写回时重新加锁；如果没有这行，状态更新会和 cancel/permission 冲突。
            turn = self.turns.get(turn_id)  # 修改代码+RealHarnessLifecycle：写回前再次读取最新 turn；如果没有这行，可能覆盖用户刚刚取消或拒绝后的状态。
            if turn is None:  # 新增代码+RealHarnessLifecycle：处理 turn 消失的边界；如果没有这行，写回阶段会抛异常。
                return  # 新增代码+RealHarnessLifecycle：目标缺失时退出；如果没有这行，后续状态更新没有对象。
            if turn.status in GUI_TERMINAL_STATES:  # 新增代码+RealHarnessLifecycle：如果期间已被拒绝或取消就不覆盖；如果没有这行，真实 adapter 返回后会把 failed/cancelled 改成 completed。
                if self.active_turn_id == turn_id:  # 新增代码+RealHarnessLifecycle：只释放当前 turn 的 active 标记；如果没有这行，可能误清其它运行。
                    self.active_turn_id = None  # 新增代码+RealHarnessLifecycle：释放 active turn；如果没有这行，拒绝或取消后可能仍 busy。
                self._save_state()  # 新增代码+RealHarnessLifecycle：保存最终事实；如果没有这行，重启后状态可能不一致。
                return  # 新增代码+RealHarnessLifecycle：不覆盖既有终态；如果没有这行，后续会污染状态。
            if run_error:  # 修改代码+RealHarnessLifecycle：优先处理执行异常；如果没有这行，adapter 异常可能被误当完成。
                self._set_turn_status(turn, "failed", error=run_error)  # 修改代码+RealHarnessLifecycle：写入失败终态；如果没有这行，前端只能看到 running。
            elif run_legacy_runner:  # 修改代码+RealHarnessLifecycle：处理旧 runner 成功结果；如果没有这行，兼容路径无法完成。
                self._set_turn_status(turn, "completed", answer=legacy_answer)  # 修改代码+RealHarnessLifecycle：写入 V1 runner 完成终态；如果没有这行，GUI 无法展示兼容路径答案。
            elif adapter_result is not None and adapter_result.status == "completed":  # 修改代码+RealHarnessLifecycle：处理 adapter 完成结果；如果没有这一行，完成态会被误当失败。
                self._set_turn_status(turn, "completed", answer=adapter_result.final_text)  # 修改代码+RealHarnessLifecycle：把 adapter final_text 写回助手消息；如果没有这一行，resume 看不到最终回答。
            elif adapter_result is not None and adapter_result.status == "cancelled":  # 修改代码+RealHarnessLifecycle：处理 adapter 取消结果；如果没有这一行，取消会被误当失败。
                self._set_turn_status(turn, "cancelled", answer=adapter_result.final_text, error="" if adapter_result.final_text else (adapter_result.error_message or "用户已在桌面 GUI 中取消本轮运行。"))  # 修改代码+RealHarnessLifecycle：取消时优先把 partial assistant text 写回消息；如果没有这一行，取消会丢掉已可见输出。
            elif adapter_result is not None:  # 修改代码+RealHarnessLifecycle：处理 adapter failed/unavailable 等其它终态；如果没有这一行，失败结果没有稳定兜底。
                self._set_turn_status(turn, "failed", error=adapter_result.error_message or adapter_result.error_code or "GUI adapter 运行失败。")  # 修改代码+RealHarnessLifecycle：写入 adapter 失败终态；如果没有这一行，真实 adapter 未启用会静默失败。
            else:  # 新增代码+RealHarnessLifecycle：处理没有结果也没有异常的防御分支；如果没有这行，极端边界会让 turn 悬挂。
                self._set_turn_status(turn, "failed", error="GUI adapter 没有返回结果。")  # 新增代码+RealHarnessLifecycle：把无结果边界写成失败；如果没有这行，GUI 可能一直 running。
            if self.active_turn_id == turn_id:  # 修改代码+RealHarnessLifecycle：只释放当前 worker 的 active turn；如果没有这行，可能误清新提交的任务。
                self.active_turn_id = None  # 修改代码+RealHarnessLifecycle：释放 active turn；如果没有这行，后续 prompt 无法提交。
            self._save_state()  # 修改代码+RealHarnessLifecycle：最终状态落盘；如果没有这行，resume 看不到终态。
...
class DesktopGuiBridgeHandler(http.server.BaseHTTPRequestHandler):  # 修改代码+DesktopGUIBridge：类段开始，处理 GUI bridge HTTP 请求；如果没有这个类，Electron 前端没有后端入口。
    ...
    def _send_cors_headers(self) -> None:  # 新增代码+DesktopGUICors：函数段开始，只为允许来源写入浏览器跨源响应头；如果没有这段，前端 dev server 无法调用本地 bridge。
        ...
        if not _origin_is_allowed(origin):  # 修改代码+DesktopGUICors：只给固定桌面来源或本机 Vite 来源 CORS 权限；如果没有这行，备用端口验收会失败或来源边界会散落重复。
    ...
    def _origin_allowed(self) -> bool:  # 修改代码+DesktopGUIBridge：函数段开始，检查浏览器来源；如果没有这段，本机恶意网页可能跨源访问 bridge。
        ...
        return _origin_is_allowed(origin)  # 修改代码+DesktopGUICors：复用统一来源判断；如果没有这行，OPTIONS 和业务请求可能对备用 Vite 端口得出不同结果。
    ...
    def do_GET(self) -> None:  # 修改代码+DesktopGUIBridge：函数段开始，处理 GET 请求；如果没有这段，GUI 无法读取 bootstrap/events/health。
        ...
        if path in {"/v2/gui/browser/tabs", "/v2/gui/browser/console", "/v2/gui/browser/network"}:  # 新增代码+DesktopGUIBrowserWorkbench：识别 Browser 只读集合端点；如果没有这行，前端无法按需读取 tabs/console/network。
            if not self._guard_request(require_token=True):  # 新增代码+DesktopGUIBrowserWorkbench：Browser 只读集合仍必须 token；如果没有这行，本地未授权网页可能读取页面和调试摘要。
                return  # 新增代码+DesktopGUIBrowserWorkbench：门禁失败后退出；如果没有这行，会继续返回敏感状态。
            collection_kind = path.rsplit("/", 1)[-1]  # 新增代码+DesktopGUIBrowserWorkbench：从路径末尾取集合类型；如果没有这行，三个 GET 路由需要重复三段逻辑。
            self._send_json(200, build_gui_browser_collection_payload(collection_kind, self.server.workspace))  # 新增代码+DesktopGUIBrowserWorkbench：返回 tabs/console/network 的 V2 payload；如果没有这行，client 对应方法会收到 404。
            return  # 新增代码+DesktopGUIBrowserWorkbench：响应完成后退出；如果没有这行，handler 可能继续匹配后续路由。
        parts = [part for part in path.split("/") if part]  # 新增代码+DesktopGUICommandConsole：拆分 GET 动态路径；如果没有这行，/v2/gui/commands/{id}/tail 无法提取命令 id。
        if path == "/v2/gui/commands":  # 新增代码+DesktopGUICommandConsole：识别后台命令列表端点；如果没有这行，CommandPanel 只能显示前端空壳。
            if not self._guard_request(require_token=True):  # 新增代码+DesktopGUICommandConsole：命令列表必须 token；如果没有这行，本地未授权网页可能读取终端命令摘要。
                return  # 新增代码+DesktopGUICommandConsole：门禁失败后退出；如果没有这行，会继续返回敏感命令状态。
            self._send_json(200, build_gui_command_console_payload(self.server.workspace))  # 新增代码+DesktopGUICommandConsole：返回复用 TaskRegistry 的命令控制台 payload；如果没有这行，前端拿不到命令事实源。
            return  # 新增代码+DesktopGUICommandConsole：响应完成后退出；如果没有这行，handler 可能继续匹配后续路由。
        if len(parts) == 5 and parts[:3] == ["v2", "gui", "commands"] and parts[4] == "tail":  # 新增代码+DesktopGUICommandConsole：匹配 /v2/gui/commands/{id}/tail；如果没有这行，长输出无法按命令刷新。
            if not self._guard_request(require_token=True):  # 新增代码+DesktopGUICommandConsole：tail 端点必须 token；如果没有这行，本地未授权网页可能读取终端输出。
                return  # 新增代码+DesktopGUICommandConsole：门禁失败后退出；如果没有这行，会继续返回输出内容。
            command_id = urllib.parse.unquote(parts[3])  # 新增代码+DesktopGUICommandConsole：URL 解码 command_id；如果没有这行，包含特殊字符的 id 会被错误查找。
            self._send_json(200, build_gui_command_tail_payload(self.server.workspace, command_id))  # 新增代码+DesktopGUICommandConsole：返回指定命令 tail payload；如果没有这行，前端 tail 刷新没有后端入口。
            return  # 新增代码+DesktopGUICommandConsole：响应完成后退出；如果没有这行，handler 可能继续匹配后续路由。
        if path == "/v2/gui/acceptance/scenarios":  # 新增代码+DesktopGUIAcceptance：识别验收场景清单端点；如果没有这行，AcceptancePanel 只能显示前端空壳。
            if not self._guard_request(require_token=True):  # 新增代码+DesktopGUIAcceptance：验收清单必须 token；如果没有这行，本地未授权网页可能读取验收 prompt 和证据摘要。
                return  # 新增代码+DesktopGUIAcceptance：门禁失败后退出；如果没有这行，会继续返回验收数据。
            self._send_json(200, build_gui_acceptance_scenarios_payload(self.server.workspace))  # 新增代码+DesktopGUIAcceptance：返回复用 acceptance_controller 的场景和最近证据；如果没有这行，前端拿不到验收事实源。
            return  # 新增代码+DesktopGUIAcceptance：响应完成后退出；如果没有这行，handler 可能继续匹配后续路由。
        if path == "/v2/gui/acceptance/runs":  # 新增代码+DesktopGUIAcceptance：识别验收运行记录端点；如果没有这行，GUI 无法按需刷新历史证据。
            if not self._guard_request(require_token=True):  # 新增代码+DesktopGUIAcceptance：运行证据必须 token；如果没有这行，本地未授权网页可能读取 result/events 路径。
                return  # 新增代码+DesktopGUIAcceptance：门禁失败后退出；如果没有这行，会继续返回运行证据。
            self._send_json(200, build_gui_acceptance_runs_payload(self.server.workspace))  # 新增代码+DesktopGUIAcceptance：返回验收 run 目录白名单摘要；如果没有这行，证据列表没有后端入口。
            return  # 新增代码+DesktopGUIAcceptance：响应完成后退出；如果没有这行，handler 可能继续匹配后续路由。
        if path == "/v2/gui/toolchain":  # 新增代码+DesktopGUIToolchain：识别 V2 工具链清单端点；如果没有这行，Toolchain 面板只能停留在前端假数据。
            if not self._guard_request(require_token=True):  # 新增代码+DesktopGUIToolchain：工具链清单必须 token；如果没有这行，本地未授权网页可能读取工具和权限元数据。
                return  # 新增代码+DesktopGUIToolchain：门禁失败后退出；如果没有这行，会继续返回工具清单。
            self._send_json(200, build_gui_toolchain_payload(self.server.workspace))  # 新增代码+DesktopGUIToolchain：返回复用内置工具目录生成的清单；如果没有这行，前端无法知道哪些 core 工具已接 GUI。
            return  # 新增代码+DesktopGUIToolchain：响应完成后退出；如果没有这行，handler 可能继续匹配后续路由。
        if path == "/v2/gui/planning":  # 新增代码+DesktopGUIPlanning：识别 V2 计划协作端点；如果没有这行，Planning 面板只能显示前端空壳。
            if not self._guard_request(require_token=True):  # 新增代码+DesktopGUIPlanning：todo/task/team 状态必须 token；如果没有这行，本地未授权网页可能读取任务目标和协作消息。
                return  # 新增代码+DesktopGUIPlanning：门禁失败后退出；如果没有这行，会继续返回计划协作状态。
            self._send_json(200, build_gui_planning_payload(self.server.workspace))  # 新增代码+DesktopGUIPlanning：返回复用 runtime registry 的只读 planning payload；如果没有这行，前端拿不到真实 todo/task/team 状态。
            return  # 新增代码+DesktopGUIPlanning：响应完成后退出；如果没有这行，handler 可能继续匹配后续路由。
        if path == "/v2/gui/memory/summary":  # 新增代码+DesktopGUIMemoryPanel：识别长期记忆摘要端点；如果没有这行，Memory 面板只能显示前端空壳。
            if not self._guard_request(require_token=True):  # 新增代码+DesktopGUIMemoryPanel：记忆摘要必须 token；如果没有这行，本地未授权网页可能读取任务进度摘要。
                return  # 新增代码+DesktopGUIMemoryPanel：门禁失败后退出；如果没有这行，会继续返回长期记忆状态。
            self._send_json(200, build_gui_memory_summary_payload(self.server.workspace))  # 新增代码+DesktopGUIMemoryPanel：返回 agent_memory 只读摘要；如果没有这行，前端拿不到 context/progress/bugs 状态。
            return  # 新增代码+DesktopGUIMemoryPanel：响应完成后退出；如果没有这行，handler 可能继续匹配后续路由。
        if path == "/v2/gui/prompt/status":  # 新增代码+DesktopGUIMemoryPanel：识别 prompt/token 状态端点；如果没有这行，GUI 无法显示上下文预算和报告工具可用性。
            if not self._guard_request(require_token=True):  # 新增代码+DesktopGUIMemoryPanel：prompt 状态必须 token；如果没有这行，本地未授权网页可能读取运行预算摘要。
                return  # 新增代码+DesktopGUIMemoryPanel：门禁失败后退出；如果没有这行，会继续返回 prompt 状态。
            self._send_json(200, build_gui_prompt_status_payload(self.server.workspace))  # 新增代码+DesktopGUIMemoryPanel：返回 prompt surface 和 token budget 状态；如果没有这行，前端无法解释上下文是否接近压缩。
            return  # 新增代码+DesktopGUIMemoryPanel：响应完成后退出；如果没有这行，handler 可能继续匹配后续路由。
        if path == "/v2/gui/notebook/status":  # 新增代码+DesktopGUIMemoryPanel：识别 Notebook 状态端点；如果没有这行，GUI 无法显示 notebook_read/edit 是否接入。
            if not self._guard_request(require_token=True):  # 新增代码+DesktopGUIMemoryPanel：Notebook 状态必须 token；如果没有这行，本地未授权网页可能读取 notebook 文件摘要。
                return  # 新增代码+DesktopGUIMemoryPanel：门禁失败后退出；如果没有这行，会继续返回 notebook 状态。
            self._send_json(200, build_gui_notebook_status_payload(self.server.workspace))  # 新增代码+DesktopGUIMemoryPanel：返回 notebook 工具和文件可见性摘要；如果没有这行，前端 notebook 区会收到 404。
            return  # 新增代码+DesktopGUIMemoryPanel：响应完成后退出；如果没有这行，handler 可能继续匹配后续路由。
        if path in {"/v2/gui/mcp/servers", "/v2/gui/mcp/resources", "/v2/gui/mcp/prompts"}:  # 新增代码+DesktopGUIMcpControl：识别 MCP 只读管理端点；如果没有这行，GUI 无法读取 MCP server/resource/prompt 状态。
            if not self._guard_request(require_token=True):  # 新增代码+DesktopGUIMcpControl：MCP 管理数据必须 token；如果没有这行，本地未授权网页可能读取外部 server 摘要。
                return  # 新增代码+DesktopGUIMcpControl：门禁失败后退出；如果没有这行，会继续返回 MCP 管理数据。
            collection_kind = path.rsplit("/", 1)[-1]  # 新增代码+DesktopGUIMcpControl：从路径末尾取 servers/resources/prompts；如果没有这行，三个路由需要重复代码。
            self._send_json(200, build_gui_mcp_collection_payload(collection_kind, self.server.workspace))  # 新增代码+DesktopGUIMcpControl：返回复用 runtime registry 的 MCP 只读 payload；如果没有这行，前端 MCP 页签会收到 404。
            return  # 新增代码+DesktopGUIMcpControl：响应完成后退出；如果没有这行，handler 可能继续匹配后续路由。
        ...
        if path == "/v2/gui/provider-settings/providers":  # 新增代码+ProviderSettings：识别 Provider 设置 catalog 端点；如果没有这行，设置页无法读取 provider 列表。
            if not self._guard_request(require_token=True):  # 新增代码+ProviderSettings：Provider 设置必须 token；如果没有这行，本机未授权网页可能读取模型和连接状态。
                return  # 新增代码+ProviderSettings：门禁失败后退出；如果没有这行，会继续返回敏感状态。
            self._send_json(200, build_provider_settings_payload(self.server.workspace))  # 新增代码+ProviderSettings：返回脱敏 provider catalog；如果没有这行，前端无法渲染提供商设置页。
            return  # 新增代码+ProviderSettings：响应完成后退出；如果没有这行，handler 可能继续匹配事件路由。
        if path == "/v2/gui/provider-settings/openai-models":  # 新增代码+OpenAIModelRegistry：识别 OpenAI 模型注册表端点；如果没有这行，前端无法单独刷新模型状态。
            if not self._guard_request(require_token=True):  # 新增代码+OpenAIModelRegistry：模型状态必须 token；如果没有这行，本机未授权网页可能读取账号模型状态。
                return  # 新增代码+OpenAIModelRegistry：门禁失败后退出；如果没有这行，会继续返回模型状态。
            settings = load_provider_settings(self.server.workspace)  # 新增代码+OpenAIModelRegistry：读取 provider settings 用于模型可见性；如果没有这行，openai-models 端点会忽略隐藏模型。
            self._send_json(200, build_openai_model_registry_payload(self.server.workspace, settings))  # 新增代码+OpenAIModelRegistry：返回不含 token/email 的模型注册表；如果没有这行，模型下拉只能读旧 catalog。
            return  # 新增代码+OpenAIModelRegistry：响应完成后退出；如果没有这行，handler 可能继续匹配 auth-attempt 路由。
        if path == "/v2/gui/provider-settings/auth-attempt/status":  # 新增代码+OpenAIAuthAttempt：识别 auth-attempt status 端点；如果没有这行，前端无法轮询 browser/headless 授权状态。
            if not self._guard_request(require_token=True):  # 新增代码+OpenAIAuthAttempt：status 必须 token；如果没有这行，本机未授权网页可能读取授权尝试状态。
                return  # 新增代码+OpenAIAuthAttempt：门禁失败后退出；如果没有这行，会继续返回状态。
            self._send_json(200, get_provider_auth_attempt_status(query.get("attempt_id", [""])[0]))  # 新增代码+OpenAIAuthAttempt：返回指定 attempt 状态；如果没有这行，等待页无法更新 pending/complete/expired。
            return  # 新增代码+OpenAIAuthAttempt：响应完成后退出；如果没有这行，handler 可能继续匹配事件路由。
    ...
    def do_POST(self) -> None:  # 新增代码+DesktopGUILifecycle：函数段开始，处理 GUI 生命周期 POST；如果没有这段，messages/cancel/retry/resume 全部不可用。
        ...
        try:  # 新增代码+DesktopGUILifecycle：统一捕获业务结构化错误；如果没有这行，每个分支都要重复 try/except。
            if path == "/v1/gui/messages":  # 新增代码+DesktopGUILifecycle：识别发送消息端点；如果没有这行，GUI 无法创建 turn。
                provider_source = body.get("providerId") if isinstance(body.get("providerId"), str) and body.get("providerId", "").strip() else body.get("provider_id")  # 修改代码+GuiToolchainReuse：优先读取前端 camelCase providerId，并兼容脚本 snake_case provider_id；如果没有这一行，外部控制器选择 OpenAI 会被丢弃。
                model_source = body.get("modelId") if isinstance(body.get("modelId"), str) and body.get("modelId", "").strip() else body.get("model_id")  # 修改代码+GuiToolchainReuse：优先读取前端 camelCase modelId，并兼容脚本 snake_case model_id；如果没有这一行，外部控制器选择模型会被丢弃。
                reasoning_source = body.get("reasoningEffort") if body.get("reasoningEffort") in GUI_REASONING_EFFORTS else body.get("reasoning_effort")  # 修改代码+GuiToolchainReuse：优先读取前端 camelCase reasoningEffort，并兼容脚本 snake_case reasoning_effort；如果没有这一行，只读验收无法证明推理档位传递。
                permission_source = body.get("permissionMode") if body.get("permissionMode") in GUI_PERMISSION_MODES else body.get("permission_mode")  # 修改代码+GuiToolchainReuse：优先读取前端 camelCase permissionMode，并兼容脚本 snake_case permission_mode；如果没有这一行，只读脚本会误回退 full_access。
                provider_id = provider_source if isinstance(provider_source, str) else None  # 修改代码+GuiToolchainReuse：只接受字符串 provider；如果没有这一行，恶意 JSON 对象可能进入模型路由字段。
                model_id = model_source if isinstance(model_source, str) else None  # 修改代码+GuiToolchainReuse：只接受字符串 model；如果没有这一行，坏请求可能污染模型路由字段。
                reasoning_effort = reasoning_source if reasoning_source in GUI_REASONING_EFFORTS else "high"  # 修改代码+GuiToolchainReuse：白名单推理强度；如果没有这一行，非法值可能传给真实模型 API。
                permission_mode = permission_source if permission_source in GUI_PERMISSION_MODES else "full_access"  # 修改代码+GuiToolchainReuse：白名单权限模式；如果没有这一行，非法权限值可能污染工具策略。
                payload = self.server.run_manager.start_message(str(body.get("conversation_id", "default")), str(body.get("prompt", "")), client_request_id=str(body.get("client_request_id", "")), provider_id=provider_id, model_id=model_id, reasoning_effort=str(reasoning_effort), permission_mode=str(permission_mode))  # 修改代码+DirectSSEPayload：提交 prompt 和模型路由字段给 run manager；如果没有这行，POST /messages 无法传递 provider/model 选择。
            ...
            if len(parts) == 5 and parts[:3] == ["v2", "gui", "commands"] and parts[4] == "stop":  # 新增代码+DesktopGUICommandConsole：匹配 /v2/gui/commands/{id}/stop；如果没有这行，命令停止按钮会收到 404。
                command_id = urllib.parse.unquote(parts[3])  # 新增代码+DesktopGUICommandConsole：URL 解码 command_id；如果没有这行，包含特殊字符的 id 会被错误查找。
                self._send_json(200, build_gui_command_stop_payload(self.server.workspace, command_id))  # 新增代码+DesktopGUICommandConsole：返回真实停止能力反馈；如果没有这行，GUI 无法诚实展示不可用原因。
                return  # 新增代码+DesktopGUICommandConsole：响应完成后退出；如果没有这行，handler 可能继续匹配其它 POST 路由。
            if path == "/v2/gui/acceptance/run":  # 新增代码+DesktopGUIAcceptance：匹配验收场景启动端点；如果没有这行，AcceptancePanel 的运行按钮会收到 404。
                self._send_json(200, build_gui_acceptance_run_payload(self.server.workspace, body))  # 新增代码+DesktopGUIAcceptance：把请求交给 acceptance_controller 薄适配层；如果没有这行，GUI 无法启动或 dry-run 验收。
                return  # 新增代码+DesktopGUIAcceptance：响应完成后退出；如果没有这行，handler 可能继续匹配其它 POST 路由。
            ...
            if len(parts) == 5 and parts[:3] == ["v2", "gui", "sessions"] and parts[4] == "archive":  # 新增代码+DesktopGUISessionSearch：匹配 /v2/gui/sessions/{session_id}/archive；如果没有这行，归档按钮没有真实后端路由。
                ...
                return  # 新增代码+DesktopGUISessionSearch：响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/provider-settings/auth-attempt/start":  # 新增代码+OpenAIAuthAttempt：匹配 browser/headless 授权尝试启动路由；如果没有这行，前端点 ChatGPT 登录方式会得到 404。
                self._send_json(200, start_provider_auth_attempt(self.server.workspace, body.get("provider_id"), body.get("auth_method_id")))  # 新增代码+OpenAIAuthAttempt：创建可轮询的授权尝试并返回确认码；如果没有这行，GUI 没有等待授权页面的数据来源。
                return  # 新增代码+OpenAIAuthAttempt：启动响应完成后退出；如果没有这行，handler 可能继续匹配 API key 保存路由。
            if path == "/v2/gui/provider-settings/auth-attempt/complete":  # 新增代码+OpenAIAuthAttempt：匹配测试驱动和未来回调使用的完成路由；如果没有这行，mock 授权无法变成已连接状态。
                self._send_json(200, complete_provider_auth_attempt(self.server.workspace, body.get("attempt_id")))  # 新增代码+OpenAIAuthAttempt：把授权尝试标记完成并写入无 token 的 mock 连接；如果没有这行，Provider 列表不会显示已连接。
                return  # 新增代码+OpenAIAuthAttempt：完成响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/provider-settings/auth-attempt/cancel":  # 新增代码+OpenAIAuthAttempt：匹配授权等待页取消路由；如果没有这行，用户关闭弹窗后 pending 尝试会残留。
                self._send_json(200, cancel_provider_auth_attempt(body.get("attempt_id")))  # 新增代码+OpenAIAuthAttempt：取消 pending 授权尝试并返回最终状态；如果没有这行，前端无法确认取消是否成功。
                return  # 新增代码+OpenAIAuthAttempt：取消响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/provider-settings/auth":  # 新增代码+ProviderSettings：匹配 Provider auth 写入路由；如果没有这行，连接按钮没有后端效果。
                self._send_json(200, set_provider_auth(self.server.workspace, body.get("provider_id"), body.get("auth_method_id"), body.get("fields", {})))  # 新增代码+ProviderSettings：保存 provider 密钥并返回脱敏 catalog；如果没有这行，连接状态不会持久。
                return  # 新增代码+ProviderSettings：响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/provider-settings/disconnect":  # 新增代码+ProviderSettings：匹配 Provider 断开路由；如果没有这行，断开按钮不会清理 secret。
                self._send_json(200, disconnect_provider(self.server.workspace, body.get("provider_id")))  # 新增代码+ProviderSettings：清理 provider auth 并返回脱敏 catalog；如果没有这行，旧密钥会残留。
                return  # 新增代码+ProviderSettings：响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/provider-settings/custom-provider":  # 新增代码+ProviderSettings：匹配自定义 Provider 保存路由；如果没有这行，自定义提供商表单无法落盘。
                self._send_json(200, save_custom_provider(self.server.workspace, body))  # 新增代码+ProviderSettings：保存自定义 provider 并返回脱敏 catalog；如果没有这行，新增 provider 不会显示。
                return  # 新增代码+ProviderSettings：响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/provider-settings/model-visibility":  # 新增代码+ProviderSettings：匹配模型可见性路由；如果没有这行，模型开关不会持久。
                self._send_json(200, set_model_visibility(self.server.workspace, body.get("provider_id"), body.get("model_id"), body.get("visible", True)))  # 新增代码+ProviderSettings：保存模型 visible 状态并返回 catalog；如果没有这行，模型页刷新后会丢状态。
                return  # 新增代码+ProviderSettings：响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/provider-settings/test-connection":  # 新增代码+ProviderSettings：匹配 Provider 连接测试路由；如果没有这行，设置页无法验证 key/base URL 是否可用。
                self._send_json(200, test_provider_connection(self.server.workspace, body.get("provider_id")))  # 新增代码+ProviderSettings：执行 metadata probe 并返回安全结果；如果没有这行，保存 key 会被误当成连接成功。
                return  # 新增代码+ProviderSettings：响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/harness/pause":  # 新增代码+DesktopGUIHarnessPanel：匹配 Harness 暂停请求；如果没有这行，前端无法得到结构化能力反馈。
                self._send_json(200, build_gui_harness_control_payload("pause", self.server.workspace))  # 修改代码+DesktopGUIHarnessControls：执行真实暂停并返回结果；如果没有这行，暂停按钮只会得到占位响应。
            ...
            if path == "/v2/gui/harness/resume":  # 新增代码+DesktopGUIHarnessPanel：匹配 Harness 恢复请求；如果没有这行，前端无法得到结构化能力反馈。
                self._send_json(200, build_gui_harness_control_payload("resume", self.server.workspace))  # 修改代码+DesktopGUIHarnessControls：执行真实恢复并返回结果；如果没有这行，恢复按钮只会得到占位响应。
                return  # 新增代码+DesktopGUIHarnessPanel：响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/harness/stop":  # 新增代码+DesktopGUIHarnessControls：匹配 Harness 停止请求；如果没有这行，前端停止按钮会收到 404。
                self._send_json(200, build_gui_harness_control_payload("stop", self.server.workspace))  # 新增代码+DesktopGUIHarnessControls：执行真实停止并返回结果；如果没有这行，停止按钮没有后端动作。
                return  # 新增代码+DesktopGUIHarnessControls：响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/harness/checkpoint":  # 新增代码+DesktopGUIHarnessControls：匹配 Harness 手动 checkpoint 请求；如果没有这行，前端 checkpoint 按钮会收到 404。
                self._send_json(200, build_gui_harness_control_payload("checkpoint", self.server.workspace))  # 新增代码+DesktopGUIHarnessControls：执行真实 checkpoint 并返回结果；如果没有这行，恢复点按钮没有后端动作。
                return  # 新增代码+DesktopGUIHarnessPanel：响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/browser/open":  # 新增代码+DesktopGUIBrowserWorkbench：匹配 Browser open 请求；如果没有这行，浏览器工作台的打开按钮会收到 404。
                self._send_json(200, build_gui_browser_action_payload("open", self.server.workspace, body))  # 新增代码+DesktopGUIBrowserWorkbench：记录打开 URL 意图但不绕过 agent/browser 权限；如果没有这行，GUI 无法把用户请求写入审计事件流。
                return  # 新增代码+DesktopGUIBrowserWorkbench：响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/browser/refresh-status":  # 新增代码+DesktopGUIBrowserWorkbench：匹配 Browser 状态刷新请求；如果没有这行，浏览器工作台刷新按钮会收到 404。
                self._send_json(200, build_gui_browser_action_payload("refresh-status", self.server.workspace, body))  # 新增代码+DesktopGUIBrowserWorkbench：刷新 Browser 快照并返回完整 workbench；如果没有这行，用户点击刷新后只能等轮询。
                return  # 新增代码+DesktopGUIBrowserWorkbench：响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/computer-use/request-access":  # 新增代码+DesktopGUIComputerUseWorkbench：匹配 Computer Use 申请访问请求；如果没有这行，GUI 权限按钮会收到 404。
                self._send_json(200, build_gui_computer_use_action_payload("request-access", self.server.workspace, body))  # 新增代码+DesktopGUIComputerUseWorkbench：调用薄适配层打开只读 observe 模式；如果没有这行，按钮只会是前端假状态。
                return  # 新增代码+DesktopGUIComputerUseWorkbench：响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/computer-use/observe":  # 新增代码+DesktopGUIComputerUseWorkbench：匹配 Computer Use 只读观察刷新请求；如果没有这行，观察按钮会收到 404。
                self._send_json(200, build_gui_computer_use_action_payload("observe", self.server.workspace, body))  # 新增代码+DesktopGUIComputerUseWorkbench：调用薄适配层刷新观察摘要；如果没有这行，用户看不到可审计观察反馈。
                return  # 新增代码+DesktopGUIComputerUseWorkbench：响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
            if path == "/v2/gui/computer-use/abort":  # 新增代码+DesktopGUIComputerUseWorkbench：匹配 Computer Use 中止请求；如果没有这行，急停按钮会收到 404。
                self._send_json(200, build_gui_computer_use_action_payload("abort", self.server.workspace, body))  # 新增代码+DesktopGUIComputerUseWorkbench：复用既有 stop 状态机停止 Computer Use；如果没有这行，GUI 无法复用原有急停路径。
                return  # 新增代码+DesktopGUIComputerUseWorkbench：响应完成后退出；如果没有这行，handler 可能继续匹配其它路由。
        ...
        except GuiProviderSettingsError as error:  # 新增代码+ProviderSettings：处理 Provider 设置结构化错误；如果没有这行，非法 provider 输入会变成 500。
            self._send_error(error.status, error.code, error.message)  # 新增代码+ProviderSettings：返回 Provider 设置错误；如果没有这行，前端无法显示具体校验失败。
            return  # 新增代码+ProviderSettings：错误响应后退出；如果没有这行，handler 可能继续发送第二个响应。
