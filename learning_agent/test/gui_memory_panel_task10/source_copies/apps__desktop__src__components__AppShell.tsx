...
import { createGuiClient, type GuiAcceptanceRunActionPayload, type GuiBrowserActionPayload, type GuiCommandActionPayload, type GuiComputerUseActionPayload, type GuiHarnessControlPayload } from "../api/guiClient"; // 修改代码+DesktopGUIAcceptanceShell：引入验收启动响应类型；如果没有这行，前端无法类型化验收运行结果。
...
export function AppShell(): JSX.Element { // 修改代码+DesktopToolTimeline：函数段开始，组织桌面 GUI 主壳；如果没有这段，各组件无法组合成 Codex 风格窗口。
  ...
  const [mcpPayload, setMcpPayload] = useState<Record<string, unknown>>({}); // 新增代码+DesktopGUIMcpPanel：保存 V2 MCP 管理总览；如果没有这行，右侧 MCP 页签无法显示 server/resource/prompt 状态。
  const [memoryPayload, setMemoryPayload] = useState<Record<string, unknown>>({}); // 新增代码+DesktopGUIMemoryPanel：保存 V2 memory/prompt/notebook 组合 payload；如果没有这行，右侧记忆页签会一直显示空态。
  const [planningPayload, setPlanningPayload] = useState<Record<string, unknown>>({}); // 新增代码+DesktopGUIPlanningPanel：保存 V2 planning 总览；如果没有这行，右侧计划页签无法显示 todo/task/team 状态。
  const [commandPayload, setCommandPayload] = useState<Record<string, unknown>>({}); // 新增代码+DesktopGUICommandConsoleShell：保存 V2 commands 总览；如果没有这行，右侧命令页签无法显示后台命令状态。
  const [acceptancePayload, setAcceptancePayload] = useState<Record<string, unknown>>({}); // 新增代码+DesktopGUIAcceptanceShell：保存 V2 acceptance 总览；如果没有这行，右侧验收页签无法显示场景和证据。
  ...
  const [commandActionPending, setCommandActionPending] = useState(false); // 新增代码+DesktopGUICommandConsoleShell：保存命令控制请求等待态；如果没有这行，停止按钮无法避免重复点击。
  const [lastCommandActionResult, setLastCommandActionResult] = useState<GuiCommandActionPayload | null>(null); // 新增代码+DesktopGUICommandConsoleShell：保存最近一次命令控制结果；如果没有这行，命令面板不会显示停止反馈。
  const [acceptanceActionPending, setAcceptanceActionPending] = useState(false); // 新增代码+DesktopGUIAcceptanceShell：保存验收启动请求等待态；如果没有这行，运行按钮无法避免重复点击。
  const [lastAcceptanceActionResult, setLastAcceptanceActionResult] = useState<GuiAcceptanceRunActionPayload | null>(null); // 新增代码+DesktopGUIAcceptanceShell：保存最近一次验收启动结果；如果没有这行，验收面板不会显示运行反馈。
  ...
  useEffect(() => { // 新增代码+DesktopGUISessions：副作用段开始，加载项目名和最近会话；如果没有这段，侧栏无法从真实 bridge 获取项目上下文。
    ...
    async function loadSidebarData(): Promise<void> { // 新增代码+DesktopGUISessions：函数段开始，集中读取 bootstrap 和 sessions；如果没有这段，侧栏数据加载逻辑会散落到渲染层。
      try { // 新增代码+DesktopGUISessions：捕获侧栏加载失败；如果没有这行，bridge 暂时不可用会让界面副作用抛错。
        const [bootstrapPayload, sessionsPayload, runtimePanelsPayload, toolchainPayloadResult, mcpPayloadResult, memorySummaryPayloadResult, promptStatusPayloadResult, notebookStatusPayloadResult, planningPayloadResult, commandPayloadResult, acceptancePayloadResult, healthPayloadResult, diagnosticsPayloadResult, harnessPayloadResult] = await Promise.all([activeClient.bootstrap(), activeClient.sessions(), activeClient.runtimePanels(), activeClient.toolchain(), activeClient.mcpServers(), activeClient.memorySummary(), activeClient.promptStatus(), activeClient.notebookStatus(), activeClient.planning(), activeClient.commands(), activeClient.acceptanceScenarios(), activeClient.health(), activeClient.diagnostics(), activeClient.harnessStatus()]); // 修改代码+DesktopGUIMemoryPanel：首屏同时读取工具链、MCP、记忆、计划、后台命令和验收总览；如果没有这行，记忆页签要等额外请求才有事实源。
        ...
        setMcpPayload(mcpPayloadResult); // 新增代码+DesktopGUIMcpPanel：保存 MCP 管理 payload；如果没有这行，MCP 页签会一直显示空态。
        setMemoryPayload({ memory: memorySummaryPayloadResult, prompt: promptStatusPayloadResult, notebook: notebookStatusPayloadResult }); // 新增代码+DesktopGUIMemoryPanel：组合记忆、prompt 和 notebook payload；如果没有这行，记忆页签拿不到三条 endpoint 的统一输入。
        setPlanningPayload(planningPayloadResult); // 新增代码+DesktopGUIPlanningPanel：保存计划协作 payload；如果没有这行，计划页签会一直显示空态。
        setCommandPayload(commandPayloadResult); // 新增代码+DesktopGUICommandConsoleShell：保存后台命令 payload；如果没有这行，命令页签会一直显示空态。
        setAcceptancePayload(acceptancePayloadResult); // 新增代码+DesktopGUIAcceptanceShell：保存验收控制中心 payload；如果没有这行，验收页签会一直显示空态。
  ...
  useEffect(() => { // 新增代码+DesktopGUIPlanningPanel：副作用段开始，定时刷新计划协作状态；如果没有这段，todo/task/team 面板会停留在首屏快照。
    if (guiClient === null) { // 新增代码+DesktopGUIPlanningPanel：没有 client 时跳过轮询；如果没有这行，缺少配置时会持续报网络错误。
      return undefined; // 新增代码+DesktopGUIPlanningPanel：返回空清理函数；如果没有这行，React 副作用返回值不清晰。
    } // 新增代码+DesktopGUIPlanningPanel：client 检查结束；如果没有这行，条件块语法不完整。
    const activeClient = guiClient; // 新增代码+DesktopGUIPlanningPanel：固定本次副作用使用的 client；如果没有这行，异步返回时 TypeScript 会担心 client 变成 null。
    let disposed = false; // 新增代码+DesktopGUIPlanningPanel：记录组件是否已卸载；如果没有这行，迟到响应可能更新已卸载组件。
    async function pollPlanning(): Promise<void> { // 新增代码+DesktopGUIPlanningPanel：函数段开始，执行一次 planning 状态刷新；如果没有这段，interval 没有复用任务。
      try { // 新增代码+DesktopGUIPlanningPanel：捕获轮询失败；如果没有这行，bridge 暂时不可用会打断副作用。
        const payload = await activeClient.planning(); // 新增代码+DesktopGUIPlanningPanel：请求后端 planning 状态；如果没有这行，计划页签没有实时事实。
        if (disposed) { // 新增代码+DesktopGUIPlanningPanel：检查组件是否已经卸载；如果没有这行，卸载后仍可能 setState。
          return; // 新增代码+DesktopGUIPlanningPanel：卸载后停止处理；如果没有这行，可能产生 React 状态更新警告。
        } // 新增代码+DesktopGUIPlanningPanel：卸载检查结束；如果没有这行，条件块语法不完整。
        setPlanningPayload(payload); // 新增代码+DesktopGUIPlanningPanel：保存最新 planning payload；如果没有这行，面板不会更新。
      } catch (error) { // 新增代码+DesktopGUIPlanningPanel：处理轮询异常；如果没有这行，断线会变成未捕获 Promise。
        console.warn("GUI bridge planning polling failed", error); // 新增代码+DesktopGUIPlanningPanel：把失败留给开发者诊断；如果没有这行，计划面板不刷新时难以排查。
      } // 新增代码+DesktopGUIPlanningPanel：异常处理结束；如果没有这行，try/catch 语法不完整。
    } // 新增代码+DesktopGUIPlanningPanel：函数段结束，pollPlanning 到此结束；如果没有边界说明，初学者不易看出轮询范围。
    void pollPlanning(); // 新增代码+DesktopGUIPlanningPanel：挂载后立即刷新一次；如果没有这行，用户要等到 interval 才看到状态。
    const intervalId = window.setInterval(() => { void pollPlanning(); }, 3000); // 新增代码+DesktopGUIPlanningPanel：每 3 秒刷新 planning 状态；如果没有这行，todo/task/team 变化不会持续可见。
    return () => { // 新增代码+DesktopGUIPlanningPanel：定义清理函数；如果没有这行，组件卸载时轮询计时器会泄漏。
      disposed = true; // 新增代码+DesktopGUIPlanningPanel：标记异步任务已失效；如果没有这行，迟到响应可能继续更新状态。
      window.clearInterval(intervalId); // 新增代码+DesktopGUIPlanningPanel：清理轮询计时器；如果没有这行，窗口关闭后仍可能保留定时任务。
    }; // 新增代码+DesktopGUIPlanningPanel：清理函数结束；如果没有这行，return 语法不完整。
  }, [guiClient]); // 新增代码+DesktopGUIPlanningPanel：按 client 变化重建 planning 轮询；如果没有这行，换 bridge 后会使用旧连接。

  useEffect(() => { // 新增代码+DesktopGUIMemoryPanel：副作用段开始，定时刷新记忆、prompt 和 notebook 状态；如果没有这段，记忆页签会停留在首屏快照。
    if (guiClient === null) { // 新增代码+DesktopGUIMemoryPanel：没有 bridge client 时跳过轮询；如果没有这行，离线状态会反复请求失败。
      return undefined; // 新增代码+DesktopGUIMemoryPanel：返回空清理函数；如果没有这行，React 副作用返回值不清楚。
    } // 新增代码+DesktopGUIMemoryPanel：client 检查结束；如果没有这行，条件块语法不完整。
    const activeClient = guiClient; // 新增代码+DesktopGUIMemoryPanel：固定本轮副作用使用的 client；如果没有这行，异步返回时可能读到新 client。
    let disposed = false; // 新增代码+DesktopGUIMemoryPanel：记录组件是否已卸载；如果没有这行，迟到响应可能更新已卸载组件。
    async function pollMemory(): Promise<void> { // 新增代码+DesktopGUIMemoryPanel：函数段开始，执行一次记忆状态刷新；如果没有这段，interval 没有复用任务。
      try { // 新增代码+DesktopGUIMemoryPanel：捕获轮询失败；如果没有这行，bridge 暂时不可用会打断副作用。
        const [memory, prompt, notebook] = await Promise.all([activeClient.memorySummary(), activeClient.promptStatus(), activeClient.notebookStatus()]); // 新增代码+DesktopGUIMemoryPanel：并行读取三条只读 endpoint；如果没有这行，记忆、预算和 notebook 状态无法同步刷新。
        if (disposed) { // 新增代码+DesktopGUIMemoryPanel：检查组件是否已经卸载；如果没有这行，卸载后仍可能 setState。
          return; // 新增代码+DesktopGUIMemoryPanel：卸载后停止处理；如果没有这行，可能产生 React 状态更新警告。
        } // 新增代码+DesktopGUIMemoryPanel：卸载检查结束；如果没有这行，条件块语法不完整。
        setMemoryPayload({ memory, prompt, notebook }); // 新增代码+DesktopGUIMemoryPanel：保存最新组合 payload；如果没有这行，面板不会更新。
      } catch (error) { // 新增代码+DesktopGUIMemoryPanel：处理轮询异常；如果没有这行，断线会变成未捕获 Promise。
        console.warn("GUI bridge memory polling failed", error); // 新增代码+DesktopGUIMemoryPanel：把失败留给开发者诊断；如果没有这行，记忆面板不刷新时难以排查。
      } // 新增代码+DesktopGUIMemoryPanel：异常处理结束；如果没有这行，try/catch 语法不完整。
    } // 新增代码+DesktopGUIMemoryPanel：函数段结束，pollMemory 到此结束；如果没有边界说明，初学者不易看出轮询范围。
    void pollMemory(); // 新增代码+DesktopGUIMemoryPanel：挂载后立即刷新一次；如果没有这行，用户要等到 interval 才看到记忆状态。
    const intervalId = window.setInterval(() => { void pollMemory(); }, 5000); // 新增代码+DesktopGUIMemoryPanel：每 5 秒刷新记忆状态；如果没有这行，progress/bugs 更新不会持续可见。
    return () => { // 新增代码+DesktopGUIMemoryPanel：定义清理函数；如果没有这行，组件卸载时轮询计时器会泄漏。
      disposed = true; // 新增代码+DesktopGUIMemoryPanel：标记异步任务已失效；如果没有这行，迟到响应可能继续更新状态。
      window.clearInterval(intervalId); // 新增代码+DesktopGUIMemoryPanel：清理轮询计时器；如果没有这行，窗口关闭后仍可能保留定时任务。
    }; // 新增代码+DesktopGUIMemoryPanel：清理函数结束；如果没有这行，return 语法不完整。
  }, [guiClient]); // 新增代码+DesktopGUIMemoryPanel：按 client 变化重建记忆轮询；如果没有这行，换 bridge 后会使用旧连接。

  useEffect(() => { // 新增代码+DesktopGUICommandConsoleShell：副作用段开始，定时刷新后台命令状态；如果没有这段，命令页签不会持续更新输出。
    if (guiClient === null) { // 新增代码+DesktopGUICommandConsoleShell：没有 bridge client 时跳过轮询；如果没有这行，离线状态会反复请求失败。
      return undefined; // 新增代码+DesktopGUICommandConsoleShell：返回空清理函数；如果没有这行，React 副作用返回值不清楚。
    } // 新增代码+DesktopGUICommandConsoleShell：client 检查结束；如果没有这行，条件块语法不完整。
    const activeClient = guiClient; // 新增代码+DesktopGUICommandConsoleShell：固定本轮副作用使用的 client；如果没有这行，异步返回时可能读到新 client。
    let disposed = false; // 新增代码+DesktopGUICommandConsoleShell：记录组件是否已卸载；如果没有这行，迟到响应可能更新已卸载组件。
    async function pollCommands(): Promise<void> { // 新增代码+DesktopGUICommandConsoleShell：函数段开始，执行一次后台命令状态刷新；如果没有这段，interval 没有复用任务。
      try { // 新增代码+DesktopGUICommandConsoleShell：捕获轮询失败；如果没有这行，bridge 暂时不可用会打断副作用。
        const payload = await activeClient.commands(); // 新增代码+DesktopGUICommandConsoleShell：请求后端 commands 状态；如果没有这行，命令页签没有实时事实。
        if (disposed) { // 新增代码+DesktopGUICommandConsoleShell：检查组件是否已经卸载；如果没有这行，卸载后仍可能 setState。
          return; // 新增代码+DesktopGUICommandConsoleShell：卸载后停止处理；如果没有这行，可能产生 React 状态更新警告。
        } // 新增代码+DesktopGUICommandConsoleShell：卸载检查结束；如果没有这行，条件块语法不完整。
        setCommandPayload(payload); // 新增代码+DesktopGUICommandConsoleShell：保存最新 commands payload；如果没有这行，面板不会更新。
      } catch (error) { // 新增代码+DesktopGUICommandConsoleShell：处理轮询异常；如果没有这行，断线会变成未捕获 Promise。
        console.warn("GUI bridge commands polling failed", error); // 新增代码+DesktopGUICommandConsoleShell：把失败留给开发者诊断；如果没有这行，命令面板不刷新时难以排查。
      } // 新增代码+DesktopGUICommandConsoleShell：异常处理结束；如果没有这行，try/catch 语法不完整。
    } // 新增代码+DesktopGUICommandConsoleShell：函数段结束，pollCommands 到此结束；如果没有边界说明，初学者不易看出轮询范围。
    void pollCommands(); // 新增代码+DesktopGUICommandConsoleShell：挂载后立即刷新一次；如果没有这行，用户要等到 interval 才看到命令状态。
    const intervalId = window.setInterval(() => { void pollCommands(); }, 3000); // 新增代码+DesktopGUICommandConsoleShell：每 3 秒刷新命令状态；如果没有这行，后台输出变化不会持续可见。
    return () => { // 新增代码+DesktopGUICommandConsoleShell：定义清理函数；如果没有这行，组件卸载时轮询计时器会泄漏。
      disposed = true; // 新增代码+DesktopGUICommandConsoleShell：标记异步任务已失效；如果没有这行，迟到响应可能继续更新状态。
      window.clearInterval(intervalId); // 新增代码+DesktopGUICommandConsoleShell：清理轮询计时器；如果没有这行，窗口关闭后仍可能保留定时任务。
    }; // 新增代码+DesktopGUICommandConsoleShell：清理函数结束；如果没有这行，return 语法不完整。
  }, [guiClient]); // 新增代码+DesktopGUICommandConsoleShell：按 client 变化重建 commands 轮询；如果没有这行，换 bridge 后会使用旧连接。
  ...
  async function handleCommandStop(commandId: string): Promise<void> { // 新增代码+DesktopGUICommandConsoleShell：函数段开始，处理后台命令停止请求；如果没有这段，命令页签停止按钮无法调用后端。
    if (commandId.length === 0) { // 新增代码+DesktopGUICommandConsoleShell：防御空命令 id；如果没有这行，按钮可能发送无意义 stop 请求。
      return; // 新增代码+DesktopGUICommandConsoleShell：空 id 直接结束；如果没有这行，后续会访问错误 endpoint。
    } // 新增代码+DesktopGUICommandConsoleShell：空 id 判断结束；如果没有这行，条件块语法不完整。
    if (guiClient === null) { // 新增代码+DesktopGUICommandConsoleShell：处理 bridge 未配置；如果没有这行，停止按钮会访问空 client。
      dispatchThread({ type: "message_added", message: { id: `local_command_stop_missing_bridge_${Date.now()}`, role: "system", text: "GUI bridge 尚未注入 baseUrl/token，暂时无法停止后台命令。", status: "failed" } }); // 新增代码+DesktopGUICommandConsoleShell：把配置缺失写到主线程；如果没有这行，用户不知道停止为什么无效。
      return; // 新增代码+DesktopGUICommandConsoleShell：没有 client 时停止流程；如果没有这行，会继续访问 null client。
    } // 新增代码+DesktopGUICommandConsoleShell：bridge 缺失分支结束；如果没有这行，条件块语法不完整。
    setCommandActionPending(true); // 新增代码+DesktopGUICommandConsoleShell：标记停止请求正在提交；如果没有这行，按钮可能被重复点击。
    try { // 新增代码+DesktopGUICommandConsoleShell：捕获停止请求失败；如果没有这行，后端异常会变成未捕获 Promise。
      const actionResult = await guiClient.stopCommand(commandId); // 新增代码+DesktopGUICommandConsoleShell：调用命令 stop endpoint；如果没有这行，停止按钮没有后端动作。
      setLastCommandActionResult(actionResult); // 新增代码+DesktopGUICommandConsoleShell：保存最近停止结果；如果没有这行，CommandPanel 不会显示本次反馈。
      const payload = await guiClient.commands(); // 新增代码+DesktopGUICommandConsoleShell：停止请求后刷新命令列表；如果没有这行，面板会停留在旧事实。
      setCommandPayload(payload); // 新增代码+DesktopGUICommandConsoleShell：把最新命令状态写入页面；如果没有这行，用户看不到请求后的结果。
      const failedStatuses = new Set(["failed", "unavailable", "unsupported", "not_found", "not_background_command"]); // 新增代码+DesktopGUICommandConsoleShell：集中定义主线程失败状态；如果没有这行，消息状态判断会散落硬编码。
      dispatchThread({ type: "message_added", message: { id: `local_command_stop_${commandId}_${Date.now()}`, role: "system", text: `后台命令停止：${actionResult.message}`, status: failedStatuses.has(actionResult.status) ? "failed" : "completed" } }); // 新增代码+DesktopGUICommandConsoleShell：把停止结果写到主线程；如果没有这行，用户只能在右侧栏找反馈。
    } catch (error) { // 新增代码+DesktopGUICommandConsoleShell：处理停止请求异常；如果没有这行，断线或 404 只会留在控制台。
      const message = `后台命令停止请求失败：${String(error)}`; // 新增代码+DesktopGUICommandConsoleShell：生成可见失败说明；如果没有这行，fallback 响应没有人话消息。
      const fallback: GuiCommandActionPayload = { ok: true, schema_version: 2, action: "stop", command_id: commandId, supported: false, status: "failed", message, safe_error: "后台命令停止请求失败。", command: {} }; // 新增代码+DesktopGUICommandConsoleShell：构造前端失败结果；如果没有这行，右侧命令面板不会显示失败反馈。
      setLastCommandActionResult(fallback); // 新增代码+DesktopGUICommandConsoleShell：保存失败结果到命令面板；如果没有这行，按钮失败后 UI 会像没发生过。
      dispatchThread({ type: "message_added", message: { id: `local_command_stop_error_${Date.now()}`, role: "system", text: message, status: "failed" } }); // 新增代码+DesktopGUICommandConsoleShell：把失败显示到主线程；如果没有这行，用户看不到请求失败原因。
    } finally { // 新增代码+DesktopGUICommandConsoleShell：无论成功失败都清理等待态；如果没有这行，失败后按钮会一直禁用。
      setCommandActionPending(false); // 新增代码+DesktopGUICommandConsoleShell：结束命令停止请求等待态；如果没有这行，停止按钮无法恢复点击。
    } // 新增代码+DesktopGUICommandConsoleShell：异常处理结束；如果没有这行，try/catch/finally 语法不完整。
  } // 新增代码+DesktopGUICommandConsoleShell：函数段结束，handleCommandStop 到此结束；如果没有这个边界，用户不容易看出命令控制范围。

  async function handleAcceptanceRun(scenarioId: string): Promise<void> { // 新增代码+DesktopGUIAcceptanceShell：函数段开始，处理验收场景运行请求；如果没有这段，验收页签运行按钮无法调用后端。
    if (scenarioId.length === 0) { // 新增代码+DesktopGUIAcceptanceShell：防御空场景 id；如果没有这行，按钮可能发送无意义 run 请求。
      return; // 新增代码+DesktopGUIAcceptanceShell：空 id 直接结束；如果没有这行，后续会访问错误 endpoint。
    } // 新增代码+DesktopGUIAcceptanceShell：空 id 判断结束；如果没有这行，条件块语法不完整。
    if (guiClient === null) { // 新增代码+DesktopGUIAcceptanceShell：处理 bridge 未配置；如果没有这行，运行按钮会访问空 client。
      dispatchThread({ type: "message_added", message: { id: `local_acceptance_run_missing_bridge_${Date.now()}`, role: "system", text: "GUI bridge 尚未注入 baseUrl/token，暂时无法启动验收。", status: "failed" } }); // 新增代码+DesktopGUIAcceptanceShell：把配置缺失写到主线程；如果没有这行，用户不知道运行为什么无效。
      return; // 新增代码+DesktopGUIAcceptanceShell：没有 client 时停止流程；如果没有这行，会继续访问 null client。
    } // 新增代码+DesktopGUIAcceptanceShell：bridge 缺失分支结束；如果没有这行，条件块语法不完整。
    setAcceptanceActionPending(true); // 新增代码+DesktopGUIAcceptanceShell：标记验收启动请求正在提交；如果没有这行，运行按钮可能被重复点击。
    try { // 新增代码+DesktopGUIAcceptanceShell：捕获启动请求失败；如果没有这行，后端异常会变成未捕获 Promise。
      const actionResult = await guiClient.runAcceptanceScenario(scenarioId); // 新增代码+DesktopGUIAcceptanceShell：调用验收 run endpoint；如果没有这行，运行按钮没有后端动作。
      setLastAcceptanceActionResult(actionResult); // 新增代码+DesktopGUIAcceptanceShell：保存最近运行结果；如果没有这行，AcceptancePanel 不会显示本次反馈。
      const payload = await guiClient.acceptanceScenarios(); // 新增代码+DesktopGUIAcceptanceShell：启动请求后刷新验收总览；如果没有这行，面板会停留在旧事实。
      setAcceptancePayload(payload); // 新增代码+DesktopGUIAcceptanceShell：把最新验收状态写入页面；如果没有这行，用户看不到请求后的结果。
      const failedStatuses = new Set(["failed", "unavailable", "unsupported", "not_found"]); // 新增代码+DesktopGUIAcceptanceShell：集中定义主线程失败状态；如果没有这行，消息状态判断会散落硬编码。
      dispatchThread({ type: "message_added", message: { id: `local_acceptance_run_${scenarioId}_${Date.now()}`, role: "system", text: `验收启动：${actionResult.message}`, status: failedStatuses.has(actionResult.status) ? "failed" : "completed" } }); // 新增代码+DesktopGUIAcceptanceShell：把启动结果写到主线程；如果没有这行，用户只能在右侧栏找反馈。
    } catch (error) { // 新增代码+DesktopGUIAcceptanceShell：处理启动请求异常；如果没有这行，断线或 404 只会留在控制台。
      const message = `验收启动请求失败：${String(error)}`; // 新增代码+DesktopGUIAcceptanceShell：生成可见失败说明；如果没有这行，fallback 响应没有人话消息。
      const fallback: GuiAcceptanceRunActionPayload = { ok: true, schema_version: 2, action: "run", scenario_id: scenarioId, status: "failed", supported: false, launched: false, message, safe_error: "验收启动请求失败。", run: {} }; // 新增代码+DesktopGUIAcceptanceShell：构造前端失败结果；如果没有这行，右侧验收面板不会显示失败反馈。
      setLastAcceptanceActionResult(fallback); // 新增代码+DesktopGUIAcceptanceShell：保存失败结果到验收面板；如果没有这行，按钮失败后 UI 会像没发生过。
      dispatchThread({ type: "message_added", message: { id: `local_acceptance_run_error_${Date.now()}`, role: "system", text: message, status: "failed" } }); // 新增代码+DesktopGUIAcceptanceShell：把失败显示到主线程；如果没有这行，用户看不到请求失败原因。
    } finally { // 新增代码+DesktopGUIAcceptanceShell：无论成功失败都清理等待态；如果没有这行，失败后按钮会一直禁用。
      setAcceptanceActionPending(false); // 新增代码+DesktopGUIAcceptanceShell：结束验收启动请求等待态；如果没有这行，运行按钮无法恢复点击。
    } // 新增代码+DesktopGUIAcceptanceShell：异常处理结束；如果没有这行，try/catch/finally 语法不完整。
  } // 新增代码+DesktopGUIAcceptanceShell：函数段结束，handleAcceptanceRun 到此结束；如果没有这个边界，用户不容易看出验收启动范围。
  ...
  return ( // 修改代码+DesktopToolTimeline：返回主界面结构；如果没有这行，组件不会输出任何 UI。
    <main className="app-shell"> {/* 修改代码+DesktopToolTimeline：定义三栏窗口网格容器；如果没有这行，侧栏、线程区和状态区无法稳定并排。 */}
      ...
      <StatusInspector events={statusState.events} browserProviderStatus={browserProviderStatus} runtimePanels={runtimePanels} toolchainPayload={toolchainPayload} mcpPayload={mcpPayload} memoryPayload={memoryPayload} planningPayload={planningPayload} commandPayload={commandPayload} acceptancePayload={acceptancePayload} harnessPayload={harnessPayload} bridgeBaseUrl={bridgeConfig?.baseUrl ?? ""} healthPayload={healthPayload} diagnosticsPayload={diagnosticsPayload} onProbeUnknownRoute={() => { void handleUnknownRouteProbe(); }} onPauseHarness={() => { void handleHarnessControl("pause"); }} onResumeHarness={() => { void handleHarnessControl("resume"); }} onStopHarness={() => { void handleHarnessControl("stop"); }} onCheckpointHarness={() => { void handleHarnessControl("checkpoint"); }} harnessControlPending={harnessControlPending} lastHarnessControlResult={lastHarnessControlResult ?? undefined} onStopCommand={(commandId) => { void handleCommandStop(commandId); }} commandActionPending={commandActionPending} lastCommandActionResult={lastCommandActionResult ?? undefined} onRunAcceptanceScenario={(scenarioId) => { void handleAcceptanceRun(scenarioId); }} acceptanceActionPending={acceptanceActionPending} lastAcceptanceActionResult={lastAcceptanceActionResult ?? undefined} onRefreshBrowserStatus={() => { void handleBrowserAction("refresh-status"); }} onOpenBrowserUrl={(url) => { void handleBrowserAction("open", url); }} browserActionPending={browserActionPending} lastBrowserActionResult={lastBrowserActionResult ?? undefined} onRequestComputerUseAccess={() => { void handleComputerUseAction("request-access"); }} onObserveComputerUse={() => { void handleComputerUseAction("observe"); }} onAbortComputerUse={() => { void handleComputerUseAction("abort"); }} computerUseActionPending={computerUseActionPending} lastComputerUseActionResult={lastComputerUseActionResult ?? undefined} /> {/* 修改代码+DesktopGUIMemoryPanel：把工具链、MCP、记忆、Planning、后台命令、验收、Browser、Harness 控制和 Computer Use 工作台按钮交给右侧检查器；如果没有这行，GUI 无法从可见界面查看记忆与验收门禁。 */}
