...
import { Activity, BookOpen, Boxes, Bug, ClipboardCheck, Globe2, ListChecks, Network, Settings, Terminal, Wrench, type LucideIcon } from "lucide-react"; // 修改代码+DesktopGUIMemoryPanel：引入记忆页签 BookOpen 图标；如果没有这行，记忆入口缺少可扫描符号。
...
import { AcceptancePanel } from "./AcceptancePanel"; // 新增代码+DesktopGUIAcceptancePanel：引入验收控制中心面板；如果没有这行，右侧检查器无法展示验收场景和证据。
...
import { MemoryPanel } from "./MemoryPanel"; // 新增代码+DesktopGUIMemoryPanel：引入记忆与预算面板；如果没有这行，右侧检查器无法展示 context/progress/bugs 和 token 状态。
...
type StatusInspectorProps = { // 修改代码+DesktopDiagnosticsPanel：类型段开始，定义右侧检查器 props；如果没有这段，调用方不知道要传哪些数据。
  ...
  memoryPayload?: Record<string, unknown>; // 新增代码+DesktopGUIMemoryPanel：保存 V2 memory/prompt/notebook 组合 payload；如果没有这行，记忆页签无法显示长任务上下文状态。
  ...
  acceptancePayload?: Record<string, unknown>; // 新增代码+DesktopGUIAcceptancePanel：保存 V2 acceptance payload；如果没有这行，验收页签无法显示场景和证据。
  ...
  onRunAcceptanceScenario?: (scenarioId: string) => void; // 新增代码+DesktopGUIAcceptancePanel：保存验收运行回调；如果没有这行，验收页签运行按钮无法调用后端。
  acceptanceActionPending?: boolean; // 新增代码+DesktopGUIAcceptancePanel：保存验收启动等待态；如果没有这行，运行按钮无法避免重复点击。
  lastAcceptanceActionResult?: Record<string, unknown>; // 新增代码+DesktopGUIAcceptancePanel：保存最近验收启动结果；如果没有这行，验收面板无法展示点击反馈。
...
type InspectorTabId = "status" | "tools" | "toolchain" | "mcp" | "memory" | "planning" | "commands" | "acceptance" | "browser" | "harness" | "settings" | "diagnostics"; // 修改代码+DesktopGUIMemoryPanel：限定右侧检查器页签 id 并加入记忆页；如果没有这行，记忆页无法被选中。
...
const inspectorTabs: InspectorTab[] = [ // 修改代码+DesktopDiagnosticsPanel：集中声明右侧五个页签；如果没有这段，用户无法在状态、工具、浏览器、设置和诊断之间切换。
  ...
  { id: "memory", label: "记忆", title: "记忆与预算", Icon: BookOpen }, // 新增代码+DesktopGUIMemoryPanel：声明记忆页签；如果没有这行，用户看不到 context、progress、bugs 和 token 状态。
  ...
  { id: "acceptance", label: "验收", title: "验收控制器", Icon: ClipboardCheck }, // 新增代码+DesktopGUIAcceptancePanel：声明验收页签；如果没有这行，用户看不到真实 GUI 验收场景和证据。
...
export function StatusInspector({ events, browserProviderStatus, runtimePanels = {}, toolchainPayload = {}, mcpPayload = {}, memoryPayload = {}, planningPayload = {}, commandPayload = {}, acceptancePayload = {}, harnessPayload = {}, bridgeBaseUrl = "", healthPayload = {}, diagnosticsPayload = {}, onProbeUnknownRoute, onPauseHarness, onResumeHarness, onStopHarness, onCheckpointHarness, harnessControlPending = false, lastHarnessControlResult = {}, onStopCommand, commandActionPending = false, lastCommandActionResult = {}, onRunAcceptanceScenario, acceptanceActionPending = false, lastAcceptanceActionResult = {}, onRequestComputerUseAccess, onObserveComputerUse, onAbortComputerUse, computerUseActionPending = false, lastComputerUseActionResult = {}, onRefreshBrowserStatus, onOpenBrowserUrl, browserActionPending = false, lastBrowserActionResult = {} }: StatusInspectorProps): JSX.Element { // 修改代码+DesktopGUIMemoryPanel：函数段开始，渲染右侧多页签检查器并接入记忆与预算控制中心；如果没有这段，记忆页签无法拿到 payload。
  ...
  return ( // 修改代码+DesktopDiagnosticsPanel：返回右侧检查器结构；如果没有这行，组件不会输出 UI。
    <aside className="status-inspector" aria-label="运行检查器"> {/* 修改代码+DesktopDiagnosticsPanel：右侧检查器容器；如果没有这一层，状态区域没有独立语义。 */}
      ...
      <div className="inspector-panel"> {/* 修改代码+DesktopDiagnosticsPanel：当前页签内容容器；如果没有这一层，面板内容没有独立滚动边界。 */}
        ...
        {activeTab === "memory" ? <MemoryPanel payload={memoryPayload} /> : null} {/* 新增代码+DesktopGUIMemoryPanel：记忆与预算页签；如果没有这一行，用户看不到 context、progress、bugs、prompt 和 notebook 状态。 */}
        ...
        {activeTab === "acceptance" ? <AcceptancePanel payload={acceptancePayload} onRunScenario={onRunAcceptanceScenario} actionPending={acceptanceActionPending} lastActionResult={lastAcceptanceActionResult} /> : null} {/* 新增代码+DesktopGUIAcceptancePanel：验收控制器页签；如果没有这一行，用户看不到验收场景、证据和运行入口。 */}
