...
describe("createGuiClient", () => { // 修改代码+DesktopRuntimePanelsClientTest：测试段开始，覆盖 GUI bridge 前端客户端；如果没有这段，后续改动容易漏掉 token header 或 endpoint。
  ...
  it("calls Browser workbench action and collection endpoints", async () => { // 新增代码+DesktopGUIBrowserWorkbenchClientTest：验证 Browser 工作台新接口；如果没有这段，刷新、记录打开和集合读取可能打错 endpoint。
    const fetchMock = vi.fn(async (url: string) => ({ // 新增代码+DesktopGUIBrowserWorkbenchClientTest：创建按 URL 返回 payload 的 fetch mock；如果没有这行，测试会依赖真实 bridge。
      ok: true, // 新增代码+DesktopGUIBrowserWorkbenchClientTest：模拟 HTTP 成功；如果没有这行，client 会进入失败分支。
      status: 200, // 新增代码+DesktopGUIBrowserWorkbenchClientTest：模拟成功状态码；如果没有这行，mock 响应形状不完整。
      json: async () => { // 新增代码+DesktopGUIBrowserWorkbenchClientTest：根据 URL 返回 JSON；如果没有这段，五个调用无法获得各自响应形状。
        if (url.endsWith("/tabs")) { // 新增代码+DesktopGUIBrowserWorkbenchClientTest：识别 tabs 集合请求；如果没有这行，tabs 响应无法单独断言。
          return { ok: true, schema_version: 2, kind: "tabs", status: "ready", safe_error: "", data: { tab_count: 1 }, browser: {} }; // 新增代码+DesktopGUIBrowserWorkbenchClientTest：返回 tabs payload；如果没有这行，client.browserTabs 没有数据。
        } // 新增代码+DesktopGUIBrowserWorkbenchClientTest：tabs 响应分支结束；如果没有这行，条件块语法不完整。
        if (url.endsWith("/console")) { // 新增代码+DesktopGUIBrowserWorkbenchClientTest：识别 console 集合请求；如果没有这行，console 响应无法单独断言。
          return { ok: true, schema_version: 2, kind: "console", status: "ready", safe_error: "", data: { latest_message: "console" }, browser: {} }; // 新增代码+DesktopGUIBrowserWorkbenchClientTest：返回 console payload；如果没有这行，client.browserConsole 没有数据。
        } // 新增代码+DesktopGUIBrowserWorkbenchClientTest：console 响应分支结束；如果没有这行，条件块语法不完整。
        if (url.endsWith("/network")) { // 新增代码+DesktopGUIBrowserWorkbenchClientTest：识别 network 集合请求；如果没有这行，network 响应无法单独断言。
          return { ok: true, schema_version: 2, kind: "network", status: "ready", safe_error: "", data: { latest_message: "network" }, browser: {} }; // 新增代码+DesktopGUIBrowserWorkbenchClientTest：返回 network payload；如果没有这行，client.browserNetwork 没有数据。
        } // 新增代码+DesktopGUIBrowserWorkbenchClientTest：network 响应分支结束；如果没有这行，条件块语法不完整。
        return { ok: true, schema_version: 2, action: url.endsWith("/open") ? "open" : "refresh-status", status: "recorded", message: "ok", safe_error: "", events_after_sequence: 0, browser: {} }; // 新增代码+DesktopGUIBrowserWorkbenchClientTest：返回 Browser 动作 payload；如果没有这行，open/refresh 方法没有响应。
      }, // 新增代码+DesktopGUIBrowserWorkbenchClientTest：json 函数结束；如果没有这行，mock 对象语法不完整。
    })) as unknown as typeof fetch; // 新增代码+DesktopGUIBrowserWorkbenchClientTest：把 mock 转成 fetch 类型；如果没有这行，TypeScript 不接受测试注入。
    const client = createGuiClient("http://127.0.0.1:8776", "test-token", fetchMock); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：创建带 token 的 GUI client；如果没有这行，测试没有被测对象。
    const open = await client.openBrowser("https://example.com"); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：执行 openBrowser 调用；如果没有这行，无法验证 open endpoint。
    const refresh = await client.refreshBrowserStatus(); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：执行刷新调用；如果没有这行，无法验证 refresh endpoint。
    const tabs = await client.browserTabs(); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：执行 tabs 调用；如果没有这行，无法验证 tabs endpoint。
    const consolePayload = await client.browserConsole(); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：执行 console 调用；如果没有这行，无法验证 console endpoint。
    const networkPayload = await client.browserNetwork(); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：执行 network 调用；如果没有这行，无法验证 network endpoint。
    expect(fetchMock).toHaveBeenNthCalledWith(1, "http://127.0.0.1:8776/v2/gui/browser/open", { method: "POST", headers: { "X-OpenHarness-Desktop-Token": "test-token", "Content-Type": "application/json" }, body: JSON.stringify({ url: "https://example.com" }) }); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：确认 open POST 形状；如果没有这行，URL 可能没有进入后端。
    expect(fetchMock).toHaveBeenNthCalledWith(2, "http://127.0.0.1:8776/v2/gui/browser/refresh-status", { method: "POST", headers: { "X-OpenHarness-Desktop-Token": "test-token", "Content-Type": "application/json" }, body: JSON.stringify({}) }); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：确认 refresh POST 形状；如果没有这行，刷新按钮可能打错路径。
    expect(fetchMock).toHaveBeenNthCalledWith(3, "http://127.0.0.1:8776/v2/gui/browser/tabs", { headers: { "X-OpenHarness-Desktop-Token": "test-token" } }); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：确认 tabs GET 形状；如果没有这行，tabs 读取可能漏 token。
    expect(fetchMock).toHaveBeenNthCalledWith(4, "http://127.0.0.1:8776/v2/gui/browser/console", { headers: { "X-OpenHarness-Desktop-Token": "test-token" } }); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：确认 console GET 形状；如果没有这行，console 读取可能漏 token。
    expect(fetchMock).toHaveBeenNthCalledWith(5, "http://127.0.0.1:8776/v2/gui/browser/network", { headers: { "X-OpenHarness-Desktop-Token": "test-token" } }); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：确认 network GET 形状；如果没有这行，network 读取可能漏 token。
    expect(open.action).toBe("open"); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：确认 open 响应透传；如果没有这行，动作结果可能丢失。
    expect(refresh.action).toBe("refresh-status"); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：确认 refresh 响应透传；如果没有这行，刷新结果可能丢失。
    expect(tabs.kind).toBe("tabs"); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：确认 tabs kind 透传；如果没有这行，集合类型可能错配。
    expect(consolePayload.kind).toBe("console"); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：确认 console kind 透传；如果没有这行，集合类型可能错配。
    expect(networkPayload.kind).toBe("network"); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：确认 network kind 透传；如果没有这行，集合类型可能错配。
  }); // 新增代码+DesktopGUIBrowserWorkbenchClientTest：Browser endpoint 测试结束；如果没有这行，测试块语法不完整。
  ...
  it("loads MCP management endpoints for the GUI control center", async () => { // 新增代码+DesktopGUIMcpClientTest：测试 MCP 管理 endpoint；如果没有这段，前端可能打错 MCP server/resource/prompt 路径。
    const fetchMock = vi.fn(async (url: string) => ({ // 新增代码+DesktopGUIMcpClientTest：创建按 URL 返回 MCP payload 的 fetch mock；如果没有这行，单元测试会依赖真实 bridge。
      ok: true, // 新增代码+DesktopGUIMcpClientTest：模拟 HTTP 成功；如果没有这行，client 会进入失败分支。
      status: 200, // 新增代码+DesktopGUIMcpClientTest：模拟成功状态码；如果没有这行，mock 响应形状不完整。
      json: async () => ({ ok: true, schema_version: 2, kind: url.endsWith("/resources") ? "resources" : url.endsWith("/prompts") ? "prompts" : "servers", status: "ready", workspace: "workspace", config_path: "mcp_servers.json", generated_at: "2026-06-27T00:00:00Z", server_count: 1, resource_count: 1, prompt_count: 1, servers: [{ name: "alpha" }], resources: [{ name: "Repo Docs" }], prompts: [{ name: "Review" }], status_degraded: false, safe_error: "", data: [] }), // 新增代码+DesktopGUIMcpClientTest：模拟 MCP 管理 payload；如果没有这行，client 方法没有可返回数据。
    })) as unknown as typeof fetch; // 新增代码+DesktopGUIMcpClientTest：把 mock 转成 fetch 类型；如果没有这行，TypeScript 不接受测试注入。
    const client = createGuiClient("http://127.0.0.1:8776", "test-token", fetchMock); // 新增代码+DesktopGUIMcpClientTest：创建带 token 的 GUI client；如果没有这行，测试没有被测对象。
    const servers = await client.mcpServers(); // 新增代码+DesktopGUIMcpClientTest：执行 MCP servers 请求；如果没有这行，无法验证 servers endpoint。
    const resources = await client.mcpResources(); // 新增代码+DesktopGUIMcpClientTest：执行 MCP resources 请求；如果没有这行，无法验证 resources endpoint。
    const prompts = await client.mcpPrompts(); // 新增代码+DesktopGUIMcpClientTest：执行 MCP prompts 请求；如果没有这行，无法验证 prompts endpoint。
    expect(fetchMock).toHaveBeenNthCalledWith(1, "http://127.0.0.1:8776/v2/gui/mcp/servers", { headers: { "X-OpenHarness-Desktop-Token": "test-token" } }); // 新增代码+DesktopGUIMcpClientTest：确认 servers GET 形状；如果没有这行，MCP 总览可能打错路径或漏 token。
    expect(fetchMock).toHaveBeenNthCalledWith(2, "http://127.0.0.1:8776/v2/gui/mcp/resources", { headers: { "X-OpenHarness-Desktop-Token": "test-token" } }); // 新增代码+DesktopGUIMcpClientTest：确认 resources GET 形状；如果没有这行，资源集合可能打错路径或漏 token。
    expect(fetchMock).toHaveBeenNthCalledWith(3, "http://127.0.0.1:8776/v2/gui/mcp/prompts", { headers: { "X-OpenHarness-Desktop-Token": "test-token" } }); // 新增代码+DesktopGUIMcpClientTest：确认 prompts GET 形状；如果没有这行，提示词集合可能打错路径或漏 token。
    expect(servers.server_count).toBe(1); // 新增代码+DesktopGUIMcpClientTest：确认 servers 响应透传数量；如果没有这行，前端可能拿不到总览字段。
    expect(resources.kind).toBe("resources"); // 新增代码+DesktopGUIMcpClientTest：确认 resources kind 透传；如果没有这行，集合类型可能错配。
    expect(prompts.kind).toBe("prompts"); // 新增代码+DesktopGUIMcpClientTest：确认 prompts kind 透传；如果没有这行，集合类型可能错配。
  }); // 新增代码+DesktopGUIMcpClientTest：MCP client 测试结束；如果没有这行，测试块语法不完整。

  it("loads Planning state for the GUI control center", async () => { // 新增代码+DesktopGUIPlanningClientTest：测试 planning endpoint；如果没有这段，前端可能打错计划协作路径。
    const fetchMock = vi.fn(async () => ({ // 新增代码+DesktopGUIPlanningClientTest：创建 planning 请求 mock；如果没有这行，单元测试会依赖真实 bridge。
      ok: true, // 新增代码+DesktopGUIPlanningClientTest：模拟 HTTP 成功；如果没有这行，client 会进入失败分支。
      status: 200, // 新增代码+DesktopGUIPlanningClientTest：模拟成功状态码；如果没有这行，mock 响应形状不完整。
      json: async () => ({ ok: true, schema_version: 2, workspace: "workspace", generated_at: "2026-06-27T00:00:00Z", reuse_module: "learning_agent.runtime.task_registry", tool_count: 2, available_tool_count: 2, tools: [{ name: "todo_read" }], todo_count: 1, todos: [{ content: "接入 planning GUI" }], task_count: 1, active_task_count: 1, tasks: [{ task_id: "task_a" }], peer_count: 1, active_peer_count: 1, peer_message_count: 1, pending_peer_message_count: 1, peers: [{ peer_id: "peer_a" }], peer_messages: [{ message_id: "message_a" }], status_degraded: false, safe_error: "" }), // 新增代码+DesktopGUIPlanningClientTest：模拟 planning payload；如果没有这行，client.planning 没有可返回数据。
    })) as unknown as typeof fetch; // 新增代码+DesktopGUIPlanningClientTest：把 mock 转成 fetch 类型；如果没有这行，TypeScript 不接受测试注入。
    const client = createGuiClient("http://127.0.0.1:8776", "test-token", fetchMock); // 新增代码+DesktopGUIPlanningClientTest：创建带 token 的 GUI client；如果没有这行，测试没有被测对象。
    const payload = await client.planning(); // 新增代码+DesktopGUIPlanningClientTest：执行 planning 请求；如果没有这行，无法验证 endpoint 已接入前端 client。
    expect(fetchMock).toHaveBeenCalledWith("http://127.0.0.1:8776/v2/gui/planning", { headers: { "X-OpenHarness-Desktop-Token": "test-token" } }); // 新增代码+DesktopGUIPlanningClientTest：确认请求命中 V2 planning endpoint 和 token header；如果没有这行，client 可能打错路径或漏传权限。
    expect(payload.todo_count).toBe(1); // 新增代码+DesktopGUIPlanningClientTest：确认 todo 数量透传；如果没有这行，计划面板可能拿不到计划规模。
    expect(payload.pending_peer_message_count).toBe(1); // 新增代码+DesktopGUIPlanningClientTest：确认待处理消息数量透传；如果没有这行，协作待办可能丢失。
  }); // 新增代码+DesktopGUIPlanningClientTest：planning client 测试结束；如果没有这行，测试块语法不完整。

  it("loads memory, prompt, and notebook status endpoints for the GUI control center", async () => { // 新增代码+DesktopGUIMemoryPanelClientTest：测试记忆、prompt 和 notebook endpoint；如果没有这段，前端可能打错长任务上下文路径。
    const fetchMock = vi.fn(async (url: string) => ({ // 新增代码+DesktopGUIMemoryPanelClientTest：创建按 URL 返回不同 payload 的 fetch mock；如果没有这行，测试会依赖真实 bridge。
      ok: true, // 新增代码+DesktopGUIMemoryPanelClientTest：模拟 HTTP 成功；如果没有这行，client 会进入失败分支。
      status: 200, // 新增代码+DesktopGUIMemoryPanelClientTest：模拟成功状态码；如果没有这行，mock 响应形状不完整。
      json: async () => { // 新增代码+DesktopGUIMemoryPanelClientTest：根据 URL 返回 JSON；如果没有这段，三个调用无法获得各自响应形状。
        if (url.endsWith("/memory/summary")) { // 新增代码+DesktopGUIMemoryPanelClientTest：识别 memory summary 请求；如果没有这行，记忆响应无法单独断言。
          return { ok: true, schema_version: 2, generated_at: "2026-06-27T00:00:00Z", workspace_name: "OpenHarness-main", reuse_module: "agent_memory", memory_root: "agent_memory", files: [{ id: "context", status: "ready" }], context_summary: { status: "ready" }, progress_summary: { status: "ready" }, bugs_summary: { status: "ready" }, experience_summary: { status: "missing" }, status_degraded: false, safe_error: "" }; // 新增代码+DesktopGUIMemoryPanelClientTest：返回 memory payload；如果没有这行，client.memorySummary 没有数据。
        } // 新增代码+DesktopGUIMemoryPanelClientTest：memory 响应分支结束；如果没有这行，条件块语法不完整。
        if (url.endsWith("/prompt/status")) { // 新增代码+DesktopGUIMemoryPanelClientTest：识别 prompt status 请求；如果没有这行，prompt 响应无法单独断言。
          return { ok: true, schema_version: 2, generated_at: "2026-06-27T00:00:00Z", workspace_name: "OpenHarness-main", reuse_module: "learning_agent.prompts.report_tools", tools: [{ name: "prompt_surface_report" }], tool_count: 2, available_tool_count: 2, context_budget: { max_messages: 18, max_chars: 24000 }, prompt_surface: {}, token_budget: {}, snapshot_summary: {}, status_degraded: false, safe_error: "" }; // 新增代码+DesktopGUIMemoryPanelClientTest：返回 prompt payload；如果没有这行，client.promptStatus 没有数据。
        } // 新增代码+DesktopGUIMemoryPanelClientTest：prompt 响应分支结束；如果没有这行，条件块语法不完整。
        return { ok: true, schema_version: 2, generated_at: "2026-06-27T00:00:00Z", workspace_name: "OpenHarness-main", reuse_module: "learning_agent.tools.notebook_tools", tools: [{ name: "notebook_read" }], tool_count: 2, available_tool_count: 2, read_only_first_pass: true, edit_exposed_in_gui: false, notebook_count: 1, notebooks: ["demo.ipynb"], scan_limited: false, status_degraded: false, safe_error: "" }; // 新增代码+DesktopGUIMemoryPanelClientTest：返回 notebook payload；如果没有这行，client.notebookStatus 没有数据。
      }, // 新增代码+DesktopGUIMemoryPanelClientTest：json 函数结束；如果没有这行，mock 对象语法不完整。
    })) as unknown as typeof fetch; // 新增代码+DesktopGUIMemoryPanelClientTest：把 mock 转成 fetch 类型；如果没有这行，TypeScript 不接受测试注入。
    const client = createGuiClient("http://127.0.0.1:8776", "test-token", fetchMock); // 新增代码+DesktopGUIMemoryPanelClientTest：创建带 token 的 GUI client；如果没有这行，测试没有被测对象。
    const memory = await client.memorySummary(); // 新增代码+DesktopGUIMemoryPanelClientTest：执行 memory summary 请求；如果没有这行，无法验证 endpoint 已接入前端 client。
    const prompt = await client.promptStatus(); // 新增代码+DesktopGUIMemoryPanelClientTest：执行 prompt status 请求；如果没有这行，无法验证 prompt endpoint。
    const notebook = await client.notebookStatus(); // 新增代码+DesktopGUIMemoryPanelClientTest：执行 notebook status 请求；如果没有这行，无法验证 notebook endpoint。
    expect(fetchMock).toHaveBeenNthCalledWith(1, "http://127.0.0.1:8776/v2/gui/memory/summary", { headers: { "X-OpenHarness-Desktop-Token": "test-token" } }); // 新增代码+DesktopGUIMemoryPanelClientTest：确认 memory GET 形状；如果没有这行，记忆总览可能打错路径或漏 token。
    expect(fetchMock).toHaveBeenNthCalledWith(2, "http://127.0.0.1:8776/v2/gui/prompt/status", { headers: { "X-OpenHarness-Desktop-Token": "test-token" } }); // 新增代码+DesktopGUIMemoryPanelClientTest：确认 prompt GET 形状；如果没有这行，prompt 状态可能打错路径或漏 token。
    expect(fetchMock).toHaveBeenNthCalledWith(3, "http://127.0.0.1:8776/v2/gui/notebook/status", { headers: { "X-OpenHarness-Desktop-Token": "test-token" } }); // 新增代码+DesktopGUIMemoryPanelClientTest：确认 notebook GET 形状；如果没有这行，notebook 状态可能打错路径或漏 token。
    expect(memory.context_summary.status).toBe("ready"); // 新增代码+DesktopGUIMemoryPanelClientTest：确认 Context 状态透传；如果没有这行，记忆面板可能拿不到 context 摘要。
    expect(prompt.context_budget.max_messages).toBe(18); // 新增代码+DesktopGUIMemoryPanelClientTest：确认上下文预算透传；如果没有这行，token 面板可能拿不到预算。
    expect(notebook.notebook_count).toBe(1); // 新增代码+DesktopGUIMemoryPanelClientTest：确认 notebook 数量透传；如果没有这行，notebook 面板可能拿不到文件规模。
  }); // 新增代码+DesktopGUIMemoryPanelClientTest：memory/prompt/notebook client 测试结束；如果没有这行，测试块语法不完整。
  ...
  it("calls background command console endpoints", async () => { // 新增代码+DesktopGUICommandConsoleClientTest：测试命令控制台三个 client 方法；如果没有这段，前端可能打错 commands/tail/stop 路径。
    const fetchMock = vi.fn(async (url: string) => ({ // 新增代码+DesktopGUICommandConsoleClientTest：创建按 URL 返回 payload 的 fetch mock；如果没有这行，测试会依赖真实 bridge。
      ok: true, // 新增代码+DesktopGUICommandConsoleClientTest：模拟 HTTP 成功；如果没有这行，client 会进入错误分支。
      status: 200, // 新增代码+DesktopGUICommandConsoleClientTest：模拟成功状态码；如果没有这行，response 形状不完整。
      json: async () => { // 新增代码+DesktopGUICommandConsoleClientTest：按 endpoint 返回不同 JSON；如果没有这段，三个方法无法区分响应形状。
        if (url.endsWith("/tail")) { // 新增代码+DesktopGUICommandConsoleClientTest：识别 tail 请求；如果没有这行，tail payload 无法单独断言。
          return { ok: true, schema_version: 2, command_id: "bg_test", status: "ready", tail: "line two", line_count: 1, next_offset: 8, command: {}, safe_error: "" }; // 新增代码+DesktopGUICommandConsoleClientTest：返回 tail 响应；如果没有这行，client.commandTail 没有 payload。
        } // 新增代码+DesktopGUICommandConsoleClientTest：tail 分支结束；如果没有这行，条件块语法不完整。
        if (url.endsWith("/stop")) { // 新增代码+DesktopGUICommandConsoleClientTest：识别 stop 请求；如果没有这行，stop payload 无法单独断言。
          return { ok: true, schema_version: 2, action: "stop", command_id: "bg_test", supported: false, status: "unavailable", message: "no live handle", safe_error: "", command: {} }; // 新增代码+DesktopGUICommandConsoleClientTest：返回停止响应；如果没有这行，client.stopCommand 没有 payload。
        } // 新增代码+DesktopGUICommandConsoleClientTest：stop 分支结束；如果没有这行，条件块语法不完整。
        return { ok: true, schema_version: 2, workspace: "H:/repo", generated_at: "2026-06-27T00:00:00Z", reuse_module: "learning_agent.runtime.task_registry", command_count: 1, running_command_count: 1, stop_supported_count: 0, commands: [{ command_id: "bg_test" }], status_degraded: false, safe_error: "" }; // 新增代码+DesktopGUICommandConsoleClientTest：返回命令列表响应；如果没有这行，client.commands 没有 payload。
      }, // 新增代码+DesktopGUICommandConsoleClientTest：json 函数结束；如果没有这行，mock 对象语法不完整。
    })) as unknown as typeof fetch; // 新增代码+DesktopGUICommandConsoleClientTest：把 mock 转成 fetch 类型；如果没有这行，TypeScript 不接受测试注入。
    const client = createGuiClient("http://127.0.0.1:8776", "test-token", fetchMock); // 新增代码+DesktopGUICommandConsoleClientTest：创建测试 client；如果没有这行，测试没有被测对象。
    const listPayload = await client.commands(); // 新增代码+DesktopGUICommandConsoleClientTest：执行命令列表请求；如果没有这行，无法验证 commands endpoint。
    const tailPayload = await client.commandTail("bg_test"); // 新增代码+DesktopGUICommandConsoleClientTest：执行 tail 请求；如果没有这行，无法验证 tail endpoint。
    const stopPayload = await client.stopCommand("bg_test"); // 新增代码+DesktopGUICommandConsoleClientTest：执行 stop 请求；如果没有这行，无法验证 stop endpoint。
    expect(fetchMock).toHaveBeenNthCalledWith(1, "http://127.0.0.1:8776/v2/gui/commands", { headers: { "X-OpenHarness-Desktop-Token": "test-token" } }); // 新增代码+DesktopGUICommandConsoleClientTest：确认列表 URL 和 token；如果没有这行，GET 路径错误不会被发现。
    expect(fetchMock).toHaveBeenNthCalledWith(2, "http://127.0.0.1:8776/v2/gui/commands/bg_test/tail", { headers: { "X-OpenHarness-Desktop-Token": "test-token" } }); // 新增代码+DesktopGUICommandConsoleClientTest：确认 tail URL 和 token；如果没有这行，tail 路径错误不会被发现。
    expect(fetchMock).toHaveBeenNthCalledWith(3, "http://127.0.0.1:8776/v2/gui/commands/bg_test/stop", { method: "POST", headers: { "X-OpenHarness-Desktop-Token": "test-token", "Content-Type": "application/json" }, body: JSON.stringify({}) }); // 新增代码+DesktopGUICommandConsoleClientTest：确认 stop POST 形状；如果没有这行，停止按钮可能缺 body 或 token。
    expect(listPayload.command_count).toBe(1); // 新增代码+DesktopGUICommandConsoleClientTest：确认列表 payload 透传；如果没有这行，命令总数可能丢失。
    expect(tailPayload.tail).toBe("line two"); // 新增代码+DesktopGUICommandConsoleClientTest：确认 tail payload 透传；如果没有这行，输出内容可能丢失。
    expect(stopPayload.status).toBe("unavailable"); // 新增代码+DesktopGUICommandConsoleClientTest：确认 stop 状态透传；如果没有这行，前端无法显示真实不可用原因。
  }); // 新增代码+DesktopGUICommandConsoleClientTest：命令控制台 client 测试结束；如果没有这行，测试块语法不完整。

  it("loads and starts acceptance controller scenarios", async () => { // 新增代码+DesktopGUIAcceptanceClientTest：测试验收控制器三个 client 方法；如果没有这段，验收页签可能打错 scenarios/runs/run 路径。
    const fetchMock = vi.fn(async (url: string) => ({ // 新增代码+DesktopGUIAcceptanceClientTest：创建按 URL 返回验收 payload 的 fetch mock；如果没有这行，测试会依赖真实 bridge。
      ok: true, // 新增代码+DesktopGUIAcceptanceClientTest：模拟 HTTP 成功；如果没有这行，client 会进入错误分支。
      status: 200, // 新增代码+DesktopGUIAcceptanceClientTest：模拟成功状态码；如果没有这行，response 形状不完整。
      json: async () => { // 新增代码+DesktopGUIAcceptanceClientTest：按 endpoint 返回不同 JSON；如果没有这段，三个方法无法区分响应形状。
        if (url.endsWith("/runs")) { // 新增代码+DesktopGUIAcceptanceClientTest：识别验收运行历史请求；如果没有这行，runs payload 无法单独断言。
          return { ok: true, schema_version: 2, workspace: "workspace", generated_at: "2026-06-27T00:00:00Z", reuse_module: "learning_agent.acceptance_controller", run_count: 1, runs: [{ id: "run_a" }], status_degraded: false, safe_error: "" }; // 新增代码+DesktopGUIAcceptanceClientTest：返回运行历史响应；如果没有这行，client.acceptanceRuns 没有 payload。
        } // 新增代码+DesktopGUIAcceptanceClientTest：runs 分支结束；如果没有这行，条件块语法不完整。
        if (url.endsWith("/run")) { // 新增代码+DesktopGUIAcceptanceClientTest：识别启动验收请求；如果没有这行，run action payload 无法单独断言。
          return { ok: true, schema_version: 2, action: "run", scenario_id: "smoke", status: "planned", supported: true, launched: false, message: "planned", safe_error: "", run: {} }; // 新增代码+DesktopGUIAcceptanceClientTest：返回 dry-run 启动响应；如果没有这行，client.runAcceptanceScenario 没有 payload。
        } // 新增代码+DesktopGUIAcceptanceClientTest：run 分支结束；如果没有这行，条件块语法不完整。
        return { ok: true, schema_version: 2, workspace: "workspace", generated_at: "2026-06-27T00:00:00Z", reuse_module: "learning_agent.acceptance_controller", controller: { controller_ps1_exists: true }, visible_gui_gate: true, scenario_count: 1, run_count: 0, safe_smoke_scenario_id: "smoke", scenarios: [{ id: "smoke" }], recent_runs: [], status_degraded: false, safe_error: "" }; // 新增代码+DesktopGUIAcceptanceClientTest：返回场景总览响应；如果没有这行，client.acceptanceScenarios 没有 payload。
      }, // 新增代码+DesktopGUIAcceptanceClientTest：json 函数结束；如果没有这行，mock 对象语法不完整。
    })) as unknown as typeof fetch; // 新增代码+DesktopGUIAcceptanceClientTest：把 mock 转成 fetch 类型；如果没有这行，TypeScript 不接受测试注入。
    const client = createGuiClient("http://127.0.0.1:8776", "test-token", fetchMock); // 新增代码+DesktopGUIAcceptanceClientTest：创建测试 client；如果没有这行，测试没有被测对象。
    const scenarios = await client.acceptanceScenarios(); // 新增代码+DesktopGUIAcceptanceClientTest：执行场景总览请求；如果没有这行，无法验证 scenarios endpoint。
    const runs = await client.acceptanceRuns(); // 新增代码+DesktopGUIAcceptanceClientTest：执行运行历史请求；如果没有这行，无法验证 runs endpoint。
    const action = await client.runAcceptanceScenario("smoke", true); // 新增代码+DesktopGUIAcceptanceClientTest：执行 dry-run 启动请求；如果没有这行，无法验证 POST body。
    expect(fetchMock).toHaveBeenNthCalledWith(1, "http://127.0.0.1:8776/v2/gui/acceptance/scenarios", { headers: { "X-OpenHarness-Desktop-Token": "test-token" } }); // 新增代码+DesktopGUIAcceptanceClientTest：确认 scenarios URL 和 token；如果没有这行，场景清单路径错误不会被发现。
    expect(fetchMock).toHaveBeenNthCalledWith(2, "http://127.0.0.1:8776/v2/gui/acceptance/runs", { headers: { "X-OpenHarness-Desktop-Token": "test-token" } }); // 新增代码+DesktopGUIAcceptanceClientTest：确认 runs URL 和 token；如果没有这行，运行历史路径错误不会被发现。
    expect(fetchMock).toHaveBeenNthCalledWith(3, "http://127.0.0.1:8776/v2/gui/acceptance/run", { method: "POST", headers: { "X-OpenHarness-Desktop-Token": "test-token", "Content-Type": "application/json" }, body: JSON.stringify({ scenario_id: "smoke", dry_run: true }) }); // 新增代码+DesktopGUIAcceptanceClientTest：确认启动 POST 形状；如果没有这行，运行按钮可能不传 scenario_id 或 dry_run。
    expect(scenarios.safe_smoke_scenario_id).toBe("smoke"); // 新增代码+DesktopGUIAcceptanceClientTest：确认安全 smoke 场景 id 透传；如果没有这行，GUI 可能无法优先展示安全验收入口。
    expect(runs.run_count).toBe(1); // 新增代码+DesktopGUIAcceptanceClientTest：确认运行历史数量透传；如果没有这行，历史统计可能丢失。
    expect(action.status).toBe("planned"); // 新增代码+DesktopGUIAcceptanceClientTest：确认启动状态透传；如果没有这行，前端无法显示 dry-run 计划结果。
  }); // 新增代码+DesktopGUIAcceptanceClientTest：验收控制器 client 测试结束；如果没有这行，测试块语法不完整。
  ...
  it("loads V2 health and diagnostics for settings panels", async () => { // ????+DesktopDiagnosticsClientTest??? V2 ??/????????????client ???? endpoint?
    const fetchMock = vi.fn(async (url: string) => ({ // ????+DesktopDiagnosticsClientTest???? URL ?? payload ? fetch mock??????????????? bridge?
      ...
      json: async () => { // ????+DesktopDiagnosticsClientTest??? URL ?? JSON???????????????????
        ...
        return { ok: true, schema_version: 2, backend_online: true, health: {}, status_degraded: false, safe_error: "", snapshot_summary: {}, trace_summary: {}, compact_status: {}, resume_report_summary: {}, health_status_summary: {}, lsp_diagnostics_summary: {}, repl_summary: {}, last_error: "", release_gate: { status: "passed" }, diagnostic_bundle: { copy_text: "{}" } }; // 修改代码+DesktopDiagnosticsToolSurfaceClientTest：模拟扩展后的 diagnostics payload；如果没有这行，client.diagnostics 类型不会覆盖 Trace/LSP/REPL 新字段。
