...
try:  # 新增代码+BrowserRuntimeStore: 优先从包路径导入浏览器 runtime store；若没有这行代码，真实工具执行无法写入 durable browser run。
    from learning_agent.browser.runtime_events import BROWSER_ACTION_COMPLETED, BROWSER_ACTION_FAILED, BROWSER_ACTION_STARTED  # 新增代码+BrowserRuntimeStore: 导入动作开始/完成/失败事件名；若没有这行代码，事件字符串会散落在 server 中。
    ...
    if import_error.name not in {"learning_agent", "learning_agent.browser", "learning_agent.browser.runtime_events", "learning_agent.browser.runtime_models", "learning_agent.browser.runtime_store"}:  # 新增代码+BrowserRuntimeStore: 只允许包路径缺失时 fallback；若没有这行代码，runtime 模块内部 bug 会被误吞。
        raise  # 新增代码+BrowserRuntimeStore: 重新抛出真正的内部导入错误；若没有这行代码，排查会被错误 fallback 误导。
    from browser.runtime_events import BROWSER_ACTION_COMPLETED, BROWSER_ACTION_FAILED, BROWSER_ACTION_STARTED  # 新增代码+BrowserRuntimeStore: 脚本模式导入动作事件名；若没有这行代码，直接运行 MCP server 时事件名不可用。
...
BROWSER_RETRYABLE_TOOLS = {"browser_open", "browser_snapshot", "browser_click", "browser_type", "browser_type_secret", "browser_press_key", "browser_wait", "browser_screenshot", "browser_tabs", "browser_console", "browser_network", "browser_upload_file", "browser_downloads", "browser_recover_page", "browser_visual_locate"}  # 修改代码+SecretInput: 允许复杂流程执行敏感输入工具；若没有这行代码，browser_flow_run 不能安全完成账号密码登录步骤
BROWSER_REPLAY_SAFE_TOOLS = {"browser_open", "browser_click", "browser_press_key", "browser_wait", "browser_screenshot", "browser_tabs", "browser_recover_page", "browser_visual_locate"}  # 新增代码+BrowserRuntime回放: 定义默认可安全回放的动作；若没有这行代码，回放可能误执行输入、上传或脚本等高风险动作
...
class BrowserAutomationServer:  # 新增代码+BrowserAutomation服务: 定义浏览器自动化 server 状态类；若省略: tools/call 无法复用浏览器状态
    ...
    def call(self, name: str, arguments: dict[str, Any]) -> str:  # 修改代码+BrowserAutomation分发: 工具方法统一返回文本结果；若省略: 协议层无法正确包装 MCP content 文本
        ...
        runtime_action = self._start_browser_runtime_action(name, arguments)  # 新增代码+BrowserRuntimeStore: 写入 browser_action_started 事件；若没有这行代码，状态生态看不到工具开始。
        try:  # 新增代码+BrowserRuntimeStore: 捕获成功和失败以便写 completed/failed 事件；若没有这行代码，durable event log 会缺少收尾。
            result = self._run_browser_tool_with_retries(name, dispatch[name], arguments)  # 修改代码+BrowserRuntimeStore: 所有工具仍进入既有重试和旧动作日志；若没有这行代码，回放、审计和失败恢复会退化。
        except Exception as error:  # 新增代码+BrowserRuntimeStore: 工具最终失败时进入失败持久化；若没有这行代码，失败动作不会写入 browser runtime store。
            self._fail_browser_runtime_action(runtime_action, error)  # 新增代码+BrowserRuntimeStore: 写入 browser_action_failed 事件；若没有这行代码，恢复管理器无法从 store 发现失败。
            ...
            self._complete_browser_runtime_action(runtime_action)  # 新增代码+BrowserRuntimeStore: 写入 browser_action_completed 事件；若没有这行代码，事件流不知道工具已完成。
    ...
    def _start_browser_runtime_action(self, tool_name: str, arguments: dict[str, Any]) -> BrowserAction | None:  # 新增代码+BrowserRuntimeStore: 写入 browser_action_started；若没有这行代码，工具开始事件不可审计。
        ...
        runtime_action = BrowserAction.create(run_id=browser_run.run_id, stage_id=tool_name, tool_name=tool_name, arguments=arguments, action_id=action_id)  # 新增代码+BrowserRuntimeStore: 创建脱敏动作对象；若没有这行代码，参数可能明文落盘。
        runtime_action.mark_started()  # 新增代码+BrowserRuntimeStore: 标记动作开始时间和状态；若没有这行代码，事件缺少 started_at_ms。
        self.browser_runtime_store.save_action(runtime_action, event_type=BROWSER_ACTION_STARTED)  # 新增代码+BrowserRuntimeStore: 保存 action 并追加 started 事件；若没有这行代码，状态生态看不到工具开始。
    ...
    def _complete_browser_runtime_action(self, runtime_action: BrowserAction | None) -> None:  # 新增代码+BrowserRuntimeStore: 写入 browser_action_completed；若没有这行代码，工具成功没有 durable 收尾事件。
        ...
        runtime_action.mark_completed()  # 新增代码+BrowserRuntimeStore: 标记动作完成；若没有这行代码，action 状态会停留在 running。
        self.browser_runtime_store.save_action(runtime_action, event_type=BROWSER_ACTION_COMPLETED)  # 新增代码+BrowserRuntimeStore: 保存 completed action 和事件；若没有这行代码，event log 无法证明工具完成。
    ...
    def _fail_browser_runtime_action(self, runtime_action: BrowserAction | None, error: Exception) -> None:  # 新增代码+BrowserRuntimeStore: 写入 browser_action_failed；若没有这行代码，工具失败没有 durable 证据。
        ...
        runtime_action.mark_failed(type(error).__name__, error_text)  # 新增代码+BrowserRuntimeStore: 标记失败类型和信息；若没有这行代码，恢复策略无法按错误分类。
        self.browser_runtime_store.save_action(runtime_action, event_type=BROWSER_ACTION_FAILED)  # 新增代码+BrowserRuntimeStore: 保存 failed action 和事件；若没有这行代码，event log 无法证明失败步骤。
    ...
    def browser_flow_run(self, arguments: dict[str, Any]) -> str:  # 新增代码+BrowserRuntime复杂流程: 按阶段执行多步浏览器流程；若没有这行代码，复杂网站任务无法一次性验收
        ...
        stage_lines: list[str] = []  # 新增代码+BrowserRuntime复杂流程: 准备保存每阶段摘要；若没有这行代码，最终报告没有阶段状态
        for index, stage in enumerate(stages, 1):  # 新增代码+BrowserRuntime复杂流程: 按顺序遍历阶段；若没有这行代码，流程无法保持网站步骤顺序
            if not isinstance(stage, dict):  # 新增代码+BrowserRuntime复杂流程: 校验单个阶段必须是对象；若没有这行代码，坏阶段会触发难懂错误
                raise RuntimeError(f"browser_flow_run 第 {index} 个 stage 必须是对象。")  # 新增代码+BrowserRuntime复杂流程: 指明坏阶段位置；若没有这行代码，用户难以修复输入
            stage_name = str(stage.get("name", f"stage-{index}") or f"stage-{index}")  # 新增代码+BrowserRuntime复杂流程: 读取阶段名称；若没有这行代码，失败时不知道是哪一步
            tool_name = str(stage.get("tool", "") or "").strip()  # 新增代码+BrowserRuntime复杂流程: 读取阶段要调用的工具；若没有这行代码，阶段没有动作
            tool_arguments = stage.get("arguments", {})  # 新增代码+BrowserRuntime复杂流程: 读取阶段工具参数；若没有这行代码，阶段无法传 URL、selector 等信息
            if tool_name in forbidden_tools or tool_name not in allowed_tools:  # 新增代码+BrowserRuntime复杂流程: 校验工具在安全允许范围内；若没有这行代码，流程可能调用危险工具
                raise RuntimeError(f"browser_flow_run 不允许阶段 {stage_name} 调用工具：{tool_name}")  # 新增代码+BrowserRuntime复杂流程: 用中文拒绝危险工具；若没有这行代码，安全边界不可见
            if not isinstance(tool_arguments, dict):  # 新增代码+BrowserRuntime复杂流程: 校验 arguments 必须是对象；若没有这行代码，call 会收到不可用参数
                raise RuntimeError(f"browser_flow_run 阶段 {stage_name} 的 arguments 必须是对象。")  # 新增代码+BrowserRuntime复杂流程: 指明坏参数位置；若没有这行代码，用户难以修复
            if tool_name == "browser_wait" and not any(key in tool_arguments for key in ("milliseconds", "selector", "text", "url_contains", "load_state")):  # 新增代码+BrowserRuntime流程容错: flow_run 内空等待阶段自动补一个短等待；若没有这行代码，模型少传 wait 参数会让复杂流程直接失败。
                tool_arguments = dict(tool_arguments)  # 新增代码+BrowserRuntime流程容错: 复制参数避免修改调用方原始对象；若没有这行代码，动作日志和上层参数可能被隐式污染。
                tool_arguments["milliseconds"] = 250  # 新增代码+BrowserRuntime流程容错: 使用 250ms 作为安全短等待；若没有这行代码，空 wait 阶段没有可执行条件。
            try:  # 新增代码+BrowserRuntime复杂流程: 捕获单阶段执行错误；若没有这行代码，流程无法返回阶段化失败摘要
                output = self.call(tool_name, tool_arguments)  # 新增代码+BrowserRuntime复杂流程: 复用公开工具入口执行阶段；若没有这行代码，阶段不会走重试和动作日志
                clipped_output, _ = clip_text(output, 600)  # 新增代码+BrowserRuntime复杂流程: 截断阶段输出；若没有这行代码，大页面快照会撑爆流程报告
                stage_lines.append(f"{index}. {stage_name}: success tool={tool_name} output={clipped_output}")  # 新增代码+BrowserRuntime复杂流程: 记录成功阶段；若没有这行代码，用户不知道哪些步骤完成
            except Exception as error:  # 新增代码+BrowserRuntime复杂流程: 捕获阶段失败；若没有这行代码，失败阶段没有可读报告
                stage_lines.append(f"{index}. {stage_name}: error tool={tool_name} error={error}")  # 新增代码+BrowserRuntime复杂流程: 记录失败阶段；若没有这行代码，用户不知道卡在哪里
                if not continue_on_error:  # 新增代码+BrowserRuntime复杂流程: 默认失败即停止；若没有这行代码，流程可能继续在错误页面上误操作
                    self.last_flow_summary = f"failed at {stage_name}"  # 新增代码+BrowserRuntime复杂流程: 保存失败摘要；若没有这行代码，状态工具看不到流程失败点
                    return "browser_flow_run 失败\n" + "\n".join(stage_lines)  # 新增代码+BrowserRuntime复杂流程: 返回失败报告；若没有这行代码，调用方收不到阶段状态
        self.last_flow_summary = f"completed {len(stages)} stages"  # 新增代码+BrowserRuntime复杂流程: 保存成功摘要；若没有这行代码，状态工具无法展示流程完成情况
        return "browser_flow_run 完成\n" + "\n".join(stage_lines)  # 新增代码+BrowserRuntime复杂流程: 返回完整阶段报告；若没有这行代码，复杂流程没有验收输出
    ...
    def browser_plugin_status(self, arguments: dict[str, Any]) -> str:  # 新增代码+BrowserRuntime插件兼容: 报告浏览器运行层兼容能力；若没有这行代码，其他 agent 无法判断可用功能
        ...
        return "\n".join([  # 新增代码+BrowserRuntime插件兼容: 返回多行机器可读状态；若没有这行代码，MCP 调用方无法解析
            ...
            "capabilities=visible_browser,page_recovery,visual_locate,coordinate_click,flow_run,action_log,replay,site_grant,retry",  # 修改代码+VisibleBrowser插件兼容: 输出可见浏览器能力；若没有这行代码，其他 agent 不知道可以做肉眼可见验收
    ...
    def browser_site_grant(self, arguments: dict[str, Any]) -> str:  # 新增代码+BrowserRuntime登录态安全: 管理真实 Chrome 站点级授权；若没有这行代码，登录态安全缺少 ClaudeCode 式 origin 边界
        ...
        if action == "grant":  # 新增代码+BrowserRuntime登录态安全: 授权 origin 分支；若没有这行代码，无法添加允许站点
            ...
            return f"browser_site_grant 成功\naction=grant\ngranted_origin={origin}\nsite_grants={len(self.site_grants)}"  # 新增代码+BrowserRuntime登录态安全: 返回授权结果；若没有这行代码，调用方不知道是否生效
        if action == "revoke":  # 新增代码+BrowserRuntime登录态安全: 撤销 origin 分支；若没有这行代码，错误授权无法移除
            ...
            return f"browser_site_grant 成功\naction=revoke\nrevoked_origin={origin}\nsite_grants={len(self.site_grants)}"  # 新增代码+BrowserRuntime登录态安全: 返回撤销结果；若没有这行代码，调用方不知道是否生效
