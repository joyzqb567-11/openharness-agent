import { AlertTriangle, ListChecks, Pause, Play, Save, Square } from "lucide-react"; // 修改代码+DesktopGUIHarnessControlsPanel：引入停止和 checkpoint 图标；如果没有这行，新增控制按钮缺少可扫描符号。
...
type HarnessPanelProps = { // 新增代码+DesktopGUIHarnessPanel：类型段开始，定义 HarnessPanel 入参；如果没有这段，调用方不知道要传哪些状态和回调。
  ...
  onStop?: () => void; // 新增代码+DesktopGUIHarnessControlsPanel：保存可选停止回调；如果没有这行，后端支持停止时按钮无法接线。
  onCheckpoint?: () => void; // 新增代码+DesktopGUIHarnessControlsPanel：保存可选 checkpoint 回调；如果没有这行，后端支持恢复点时按钮无法接线。
  controlPending?: boolean; // 新增代码+DesktopGUIHarnessPanel：保存控制请求是否进行中；如果没有这行，按钮无法避免重复点击。
  lastControlResult?: Record<string, unknown>; // 新增代码+DesktopGUIHarnessControlsPanel：保存最近一次控制结果；如果没有这行，用户点按钮后没有肉眼反馈。
...
export function HarnessPanel({ payload, onPause, onResume, onStop, onCheckpoint, controlPending = false, lastControlResult = {} }: HarnessPanelProps): JSX.Element { // 修改代码+DesktopGUIHarnessControlsPanel：函数段开始，渲染长任务 Harness 状态和真实控制；如果没有这段，右侧任务页签没有内容。
  ...
  const stopSupported = boolFrom(controls.stop_supported); // 新增代码+DesktopGUIHarnessControlsPanel：读取停止能力开关；如果没有这行，停止按钮可能误显示。
  const checkpointSupported = boolFrom(controls.checkpoint_supported); // 新增代码+DesktopGUIHarnessControlsPanel：读取 checkpoint 能力开关；如果没有这行，恢复点按钮可能误显示。
  const showControls = pauseSupported || resumeSupported || stopSupported || checkpointSupported; // 修改代码+DesktopGUIHarnessControlsPanel：计算是否显示控制区；如果没有这行，未支持能力也可能占空间。
  const controlResult = asRecord(lastControlResult); // 新增代码+DesktopGUIHarnessControlsPanel：读取最近控制结果对象；如果没有这行，反馈区会信任未知类型。
  const controlStatus = textFrom(controlResult.status, ""); // 新增代码+DesktopGUIHarnessControlsPanel：读取最近控制状态；如果没有这行，反馈无法显示 accepted/paused 等状态。
  const controlMessage = textFrom(controlResult.message, ""); // 新增代码+DesktopGUIHarnessControlsPanel：读取最近控制说明；如果没有这行，用户点击按钮后没有可读反馈。
  return ( // 新增代码+DesktopGUIHarnessPanel：返回面板 JSX；如果没有这行，组件不会输出 UI。
    <section className="harness-panel" aria-label="长任务 Harness"> {/* 新增代码+DesktopGUIHarnessPanel：面板语义容器；如果没有这一层，右侧任务区域缺少可访问标签。 */}
      ...
      {controlMessage ? <p className="harness-control-result"><ListChecks size={13} aria-hidden="true" /> {controlStatus ? `${controlStatus} · ` : ""}{controlMessage}</p> : null} {/* 新增代码+DesktopGUIHarnessControlsPanel：显示最近一次控制结果；如果没有这一行，用户点击按钮后没有肉眼反馈。 */}
      <div className="harness-goal"> {/* 新增代码+DesktopGUIHarnessPanel：当前目标容器；如果没有这一层，prompt 和步骤会混进列表。 */}
        ...
        {showControls ? ( // 新增代码+DesktopGUIHarnessPanel：仅在后端支持时渲染控制按钮；如果没有这行，未支持动作会误导用户。
          <div className="harness-controls"> {/* 新增代码+DesktopGUIHarnessPanel：控制按钮容器；如果没有这一层，按钮布局不稳定。 */}
            ...
            {stopSupported ? <button type="button" onClick={onStop} disabled={controlPending}><Square size={13} aria-hidden="true" /> 停止</button> : null} {/* 新增代码+DesktopGUIHarnessControlsPanel：停止按钮；如果没有这一行，后端支持停止时用户无法终止任务。 */}
            {checkpointSupported ? <button type="button" onClick={onCheckpoint} disabled={controlPending}><Save size={13} aria-hidden="true" /> Checkpoint</button> : null} {/* 新增代码+DesktopGUIHarnessControlsPanel：checkpoint 按钮；如果没有这一行，用户无法手动留下恢复点。 */}
