...
import json  # 新增代码+Stage15G: 读取 transcript JSONL 时需要解析 JSON；若没有这行代码，resume 无法从 events.jsonl 恢复事件对象。
from pathlib import Path  # 新增代码+Stage15A: 使用 Path 管理 session 目录和 events.jsonl；若没有这行代码，路径拼接会更容易出错。
from typing import Any  # 新增代码+Stage15G: 读取事件载荷时需要通用 JSON 类型；若没有这行代码，payload 类型边界不清楚。
...
def _safe_event_payload(raw_payload: Any) -> dict[str, Any]:  # 新增代码+Stage15G: 规范化 JSONL 中的 payload 字段；若没有这行代码，坏 transcript 会把非 dict 载荷传进 AgentEvent。
    return raw_payload if isinstance(raw_payload, dict) else {}  # 新增代码+Stage15G: 只有 dict 载荷才保留，否则回退空字典；若没有这行代码，resume 可能在异常旧数据上崩溃。


def read_transcript_events(events_path: Path) -> list[AgentEvent]:  # 新增代码+Stage15G: 从 events.jsonl 读回标准事件对象；若没有这行代码，session resume 只能保存不能恢复。
    path = Path(events_path)  # 新增代码+Stage15G: 规范化事件文件路径；若没有这行代码，调用方传字符串时后续 exists/read_text 不稳定。
    if not path.exists():  # 新增代码+Stage15G: 缺失 transcript 时返回空列表；若没有这行代码，列出或恢复不存在 session 会直接抛文件错误。
        return []  # 新增代码+Stage15G: 返回空事件列表；若没有这行代码，调用方需要自己捕获 FileNotFoundError。
    events: list[AgentEvent] = []  # 新增代码+Stage15G: 保存读回的事件对象；若没有这行代码，函数没有地方累计结果。
    for raw_line in path.read_text(encoding="utf-8").splitlines():  # 新增代码+Stage15G: 逐行读取 JSONL；若没有这行代码，无法处理多事件 transcript。
        line = raw_line.strip()  # 新增代码+Stage15G: 去掉空白；若没有这行代码，空行会进入 JSON 解析导致错误。
        if not line:  # 新增代码+Stage15G: 跳过空行；若没有这行代码，手动编辑后的空行会让 resume 失败。
            continue  # 新增代码+Stage15G: 继续读取下一行；若没有这行代码，空行无法被容错。
        parsed = json.loads(line)  # 新增代码+Stage15G: 解析单行 JSON；若没有这行代码，无法恢复事件字段。
        events.append(AgentEvent(event_type=str(parsed.get("event_type", "")), run_id=str(parsed.get("run_id", "")), sequence=int(parsed.get("sequence", 0)), session_id=str(parsed.get("session_id", "")), timestamp=str(parsed.get("timestamp", "")), payload=_safe_event_payload(parsed.get("payload", {}))))  # 新增代码+Stage15G: 重建 AgentEvent 对象；若没有这行代码，resume 只能拿到普通 dict 而不是统一事件类型。
    return events  # 新增代码+Stage15G: 返回完整事件列表；若没有这行代码，调用方拿不到恢复结果。
