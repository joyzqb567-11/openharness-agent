...
def render_computer_status(snapshot: dict[str, Any]) -> str:  # 新增代码+Phase51ComputerStatusUI: 函数段开始，渲染 `/computer status` 聚焦状态页；如果没有这段函数，终端状态会继续长而散。
    ...
    lines.append("Computer Native")  # 新增代码+Phase51ComputerStatusUI: 输出 native 能力区标题；如果没有这行代码，能力矩阵细节不易找到。
    for capability in _as_list(capability_matrix.get("capabilities", []))[:4]:  # 新增代码+Phase51ComputerStatusUI: 只展示前四条能力避免刷屏；如果没有这行代码，状态 UI 会被矩阵撑长。
        item = _as_dict(capability)  # 新增代码+Phase51ComputerStatusUI: 安全读取能力项；如果没有这行代码，坏 capability 会导致渲染崩溃。
        lines.append(f"- capability={_short_text(item.get('name', ''), 80)} available={_bool_token(item.get('available'))} enabled={_bool_token(item.get('enabled'))} reason={_short_text(item.get('reason', ''), 90)}")  # 新增代码+Phase51ComputerStatusUI: 输出单项能力摘要；如果没有这行代码，WGC/UIA/SendInput 细节不可见。
    ...
    lines.append("- risk=low command=/computer revoke <app> : remove terminal UI grant draft")  # 新增代码+Phase51ComputerStatusUI: 展示 revoke 命令；如果没有这行代码，用户不知道如何撤销草案。
