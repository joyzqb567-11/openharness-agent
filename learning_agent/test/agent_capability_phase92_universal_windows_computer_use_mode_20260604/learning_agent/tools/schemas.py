...
TOOL_SCHEMAS: list[dict[str, Any]] = [  # 作用: 告诉真实模型当前 agent 暴露了哪些工具
    ...
    {  # 新增代码+StatusTools: 第四十三个工具定义开始，暴露统一状态快照；若没有这行代码，模型无法像 ClaudeCode 一样主动查看 run/task/session 总状态
        "type": "function",  # 新增代码+StatusTools: OpenAI-compatible 工具类型固定写 function；若没有这行代码，模型接口不会识别这是可调用工具
        "function": {  # 新增代码+StatusTools: function 内部描述工具名称、说明和参数；若没有这行代码，状态快照工具缺少标准定义结构
            "name": "status_snapshot",  # 新增代码+StatusTools: 工具名用于读取统一状态快照；若没有这行代码，模型无法请求项目状态总览
            "description": "读取 learning_agent 的统一状态快照，包含 run、stage、task、event、queue、session 等可审计状态。",  # 新增代码+StatusTools: 告诉模型该工具用于状态总览；若没有这行代码，模型不知道何时调用它
            "parameters": {  # 新增代码+StatusTools: JSON Schema 描述状态快照参数；若没有这行代码，模型不知道如何选择 text/json 输出
                "type": "object",  # 新增代码+StatusTools: 参数必须是对象；若没有这行代码，工具调用结构不明确
                "properties": {  # 新增代码+StatusTools: 声明允许的参数字段；若没有这行代码，format 字段无法进入输出 schema
                    "format": {"type": "string", "enum": ["text", "json"], "description": "输出格式，text 适合终端阅读，json 适合外部 agent/SDK 解析。"},  # 新增代码+StatusTools: format 控制返回格式；若没有这行代码，模型只能拿到单一输出形态
                },  # 新增代码+StatusTools: properties 定义结束；若没有这行代码，参数 schema 不完整
                "required": [],  # 新增代码+StatusTools: 状态快照允许无参数调用；若没有这行代码，模型必须传无意义字段
                "additionalProperties": False,  # 新增代码+StatusTools: 禁止模型传入无关参数；若没有这行代码，状态工具参数会被污染
            },  # 新增代码+StatusTools: parameters 定义结束；若没有这行代码，Python 字典结构不完整
        },  # 新增代码+StatusTools: function 定义结束；若没有这行代码，工具定义结构不完整
    },  # 新增代码+StatusTools: status_snapshot 工具定义结束；若没有这行代码，工具列表语法不完整
    {  # 新增代码+StatusTools: 第四十四个工具定义开始，暴露持久任务状态查询；若没有这行代码，模型只能靠零散 task 工具拼状态
        "type": "function",  # 新增代码+StatusTools: OpenAI-compatible 工具类型固定写 function；若没有这行代码，模型接口不会识别这是可调用工具
        "function": {  # 新增代码+StatusTools: function 内部描述工具名称、说明和参数；若没有这行代码，任务状态工具缺少标准定义结构
            "name": "task_status",  # 新增代码+StatusTools: 工具名用于查询任务状态；若没有这行代码，模型无法快速查看持久任务
            "description": "按 task_id 或最近记录查询持久任务状态，用于恢复长任务、后台任务和阶段性验收。",  # 新增代码+StatusTools: 告诉模型该工具用于任务状态追踪；若没有这行代码，模型可能不知道它和 task_list 的区别
            "parameters": {  # 新增代码+StatusTools: JSON Schema 描述任务状态参数；若没有这行代码，模型不知道如何筛选任务
                "type": "object",  # 新增代码+StatusTools: 参数必须是对象；若没有这行代码，工具调用结构不明确
                "properties": {  # 新增代码+StatusTools: 声明允许的参数字段；若没有这行代码，task_id/max_results 无法进入输出 schema
                    "task_id": {"type": "string", "description": "可选任务 id；不传时返回最近任务。"},  # 新增代码+StatusTools: task_id 精确定位任务；若没有这行代码，模型无法查单个任务
                    "max_results": {"type": "integer", "description": "可选最大返回记录数，默认 10。"},  # 新增代码+StatusTools: max_results 控制输出长度；若没有这行代码，大量任务可能撑爆上下文
                },  # 新增代码+StatusTools: properties 定义结束；若没有这行代码，参数 schema 不完整
                "required": [],  # 新增代码+StatusTools: 任务状态允许无参数调用；若没有这行代码，模型无法直接查看最近任务
                "additionalProperties": False,  # 新增代码+StatusTools: 禁止模型传入无关参数；若没有这行代码，任务状态参数会被污染
            },  # 新增代码+StatusTools: parameters 定义结束；若没有这行代码，Python 字典结构不完整
        },  # 新增代码+StatusTools: function 定义结束；若没有这行代码，工具定义结构不完整
    },  # 新增代码+StatusTools: task_status 工具定义结束；若没有这行代码，工具列表语法不完整
    {  # 新增代码+StatusTools: 第四十五个工具定义开始，暴露最近 session 列表；若没有这行代码，模型不知道哪些会话可以恢复
        "type": "function",  # 新增代码+StatusTools: OpenAI-compatible 工具类型固定写 function；若没有这行代码，模型接口不会识别这是可调用工具
        "function": {  # 新增代码+StatusTools: function 内部描述工具名称、说明和参数；若没有这行代码，会话列表工具缺少标准定义结构
            "name": "session_list",  # 新增代码+StatusTools: 工具名用于列出最近 session；若没有这行代码，模型无法发现可 resume 会话
            "description": "列出最近可恢复 session，帮助长任务在中断后选择恢复点。",  # 新增代码+StatusTools: 告诉模型该工具用于会话发现；若没有这行代码，模型可能重新开始任务
            "parameters": {  # 新增代码+StatusTools: JSON Schema 描述 session_list 参数；若没有这行代码，模型不知道如何限制数量
                "type": "object",  # 新增代码+StatusTools: 参数必须是对象；若没有这行代码，工具调用结构不明确
                "properties": {  # 新增代码+StatusTools: 声明允许的参数字段；若没有这行代码，limit 无法进入输出 schema
                    "limit": {"type": "integer", "description": "可选最大返回 session 数，默认 10。"},  # 新增代码+StatusTools: limit 控制输出长度；若没有这行代码，大量会话可能撑爆上下文
                },  # 新增代码+StatusTools: properties 定义结束；若没有这行代码，参数 schema 不完整
                "required": [],  # 新增代码+StatusTools: 会话列表允许无参数调用；若没有这行代码，模型必须传无意义字段
                "additionalProperties": False,  # 新增代码+StatusTools: 禁止模型传入无关参数；若没有这行代码，会话列表参数会被污染
            },  # 新增代码+StatusTools: parameters 定义结束；若没有这行代码，Python 字典结构不完整
        },  # 新增代码+StatusTools: function 定义结束；若没有这行代码，工具定义结构不完整
    },  # 新增代码+StatusTools: session_list 工具定义结束；若没有这行代码，工具列表语法不完整
    {  # 新增代码+StatusTools: 第四十六个工具定义开始，暴露 session 恢复上下文；若没有这行代码，模型无法审计 resume loader 的真实输入
        "type": "function",  # 新增代码+StatusTools: OpenAI-compatible 工具类型固定写 function；若没有这行代码，模型接口不会识别这是可调用工具
        "function": {  # 新增代码+StatusTools: function 内部描述工具名称、说明和参数；若没有这行代码，会话恢复工具缺少标准定义结构
            "name": "session_resume",  # 新增代码+StatusTools: 工具名用于加载指定 session 恢复上下文；若没有这行代码，模型无法查看恢复内容
            "description": "加载指定 session 的恢复上下文，返回 summary、transcript_v2、compact boundary 与一致性检查结果。",  # 新增代码+StatusTools: 告诉模型该工具用于恢复审计；若没有这行代码，模型可能误以为只读 summary 即可恢复
            "parameters": {  # 新增代码+StatusTools: JSON Schema 描述 session_resume 参数；若没有这行代码，模型不知道必须传 session_id
                "type": "object",  # 新增代码+StatusTools: 参数必须是对象；若没有这行代码，工具调用结构不明确
                "properties": {  # 新增代码+StatusTools: 声明允许的参数字段；若没有这行代码，session_id 无法进入输出 schema
                    "session_id": {"type": "string", "description": "要加载恢复上下文的 session id。"},  # 新增代码+StatusTools: session_id 指定恢复对象；若没有这行代码，工具不知道读取哪次会话
                },  # 新增代码+StatusTools: properties 定义结束；若没有这行代码，参数 schema 不完整
                "required": ["session_id"],  # 新增代码+StatusTools: 恢复上下文必须指定 session_id；若没有这行代码，空调用会进入工具层再失败
                "additionalProperties": False,  # 新增代码+StatusTools: 禁止模型传入无关参数；若没有这行代码，会话恢复参数会被污染
            },  # 新增代码+StatusTools: parameters 定义结束；若没有这行代码，Python 字典结构不完整
        },  # 新增代码+StatusTools: function 定义结束；若没有这行代码，工具定义结构不完整
    },  # 新增代码+StatusTools: session_resume 工具定义结束；若没有这行代码，工具列表语法不完整
    {  # 新增代码+StatusTools: 第四十七个工具定义开始，暴露 compact/resume 状态；若没有这行代码，模型无法知道压缩边界是否存在
        "type": "function",  # 新增代码+StatusTools: OpenAI-compatible 工具类型固定写 function；若没有这行代码，模型接口不会识别这是可调用工具
        "function": {  # 新增代码+StatusTools: function 内部描述工具名称、说明和参数；若没有这行代码，compact 状态工具缺少标准定义结构
            "name": "compact_status",  # 新增代码+StatusTools: 工具名用于查看 compact 状态；若没有这行代码，模型无法主动审计压缩恢复边界
            "description": "查看 compact/resume 状态，可按 session_id 查询，也可列出最近 session 是否有 compact boundary。",  # 新增代码+StatusTools: 告诉模型该工具用于压缩恢复检查；若没有这行代码，模型可能忽略上下文是否被压缩
            "parameters": {  # 新增代码+StatusTools: JSON Schema 描述 compact_status 参数；若没有这行代码，模型不知道如何指定 session
                "type": "object",  # 新增代码+StatusTools: 参数必须是对象；若没有这行代码，工具调用结构不明确
                "properties": {  # 新增代码+StatusTools: 声明允许的参数字段；若没有这行代码，session_id 无法进入输出 schema
                    "session_id": {"type": "string", "description": "可选 session id；不传时返回最近 session 的 compact 状态。"},  # 新增代码+StatusTools: session_id 精确定位 compact 状态；若没有这行代码，模型无法查单个会话
                },  # 新增代码+StatusTools: properties 定义结束；若没有这行代码，参数 schema 不完整
                "required": [],  # 新增代码+StatusTools: compact 状态允许无参数调用；若没有这行代码，模型无法直接查看最近状态
                "additionalProperties": False,  # 新增代码+StatusTools: 禁止模型传入无关参数；若没有这行代码，compact 状态参数会被污染
            },  # 新增代码+StatusTools: parameters 定义结束；若没有这行代码，Python 字典结构不完整
        },  # 新增代码+StatusTools: function 定义结束；若没有这行代码，工具定义结构不完整
    },  # 新增代码+StatusTools: compact_status 工具定义结束；若没有这行代码，工具列表语法不完整
    {  # 新增代码+StatusTools: 第四十八个工具定义开始，暴露状态事件尾部读取；若没有这行代码，模型无法增量观察运行事件
        "type": "function",  # 新增代码+StatusTools: OpenAI-compatible 工具类型固定写 function；若没有这行代码，模型接口不会识别这是可调用工具
        "function": {  # 新增代码+StatusTools: function 内部描述工具名称、说明和参数；若没有这行代码，事件尾部工具缺少标准定义结构
            "name": "event_tail",  # 新增代码+StatusTools: 工具名用于读取状态事件尾部；若没有这行代码，模型无法按序查看状态事件
            "description": "读取统一状态事件日志尾部，支持 since_sequence 增量读取，方便外部 agent/SDK 观察运行。",  # 新增代码+StatusTools: 告诉模型该工具用于事件流观察；若没有这行代码，模型可能反复读取全量状态
            "parameters": {  # 新增代码+StatusTools: JSON Schema 描述 event_tail 参数；若没有这行代码，模型不知道如何增量读取
                "type": "object",  # 新增代码+StatusTools: 参数必须是对象；若没有这行代码，工具调用结构不明确
                "properties": {  # 新增代码+StatusTools: 声明允许的参数字段；若没有这行代码，limit/since_sequence 无法进入输出 schema
                    "limit": {"type": "integer", "description": "可选最大返回事件数，默认 50。"},  # 新增代码+StatusTools: limit 控制输出长度；若没有这行代码，大量事件可能撑爆上下文
                    "since_sequence": {"type": "integer", "description": "可选起始 sequence，只返回更大的事件。"},  # 新增代码+StatusTools: since_sequence 支持增量读取；若没有这行代码，外部 agent 只能反复拉全量事件
                },  # 新增代码+StatusTools: properties 定义结束；若没有这行代码，参数 schema 不完整
                "required": [],  # 新增代码+StatusTools: 事件尾部允许无参数调用；若没有这行代码，模型无法直接查看最近事件
                "additionalProperties": False,  # 新增代码+StatusTools: 禁止模型传入无关参数；若没有这行代码，事件读取参数会被污染
            },  # 新增代码+StatusTools: parameters 定义结束；若没有这行代码，Python 字典结构不完整
        },  # 新增代码+StatusTools: function 定义结束；若没有这行代码，工具定义结构不完整
    },  # 新增代码+StatusTools: event_tail 工具定义结束；若没有这行代码，工具列表语法不完整
    {  # 新增代码+StatusToolsV2: 新增 resume_report 工具定义开始；若没有这行代码，模型无法读取精简恢复审计报告
        "type": "function",  # 新增代码+StatusToolsV2: OpenAI-compatible 工具类型固定写 function；若没有这行代码，模型接口不会识别这是可调用工具
        "function": {  # 新增代码+StatusToolsV2: function 内部描述工具名称、说明和参数；若没有这行代码，恢复报告工具缺少标准定义结构
            "name": "resume_report",  # 新增代码+StatusToolsV2: 工具名用于读取精简恢复报告；若没有这行代码，模型无法请求 resume 安全状态
            "description": "读取指定 session 的精简恢复审计报告，包含 resume_state、坏 transcript 行、缺失 tool_result、tombstone 和 compact boundary。",  # 新增代码+StatusToolsV2: 告诉模型此工具用于判断能否安全继续；若没有这行代码，模型可能误用全量 session_resume
            "parameters": {  # 新增代码+StatusToolsV2: JSON Schema 描述 resume_report 参数；若没有这行代码，模型不知道必须传 session_id
                "type": "object",  # 新增代码+StatusToolsV2: 参数必须是对象；若没有这行代码，工具调用结构不明确
                "properties": {  # 新增代码+StatusToolsV2: 声明允许的参数字段；若没有这行代码，session_id 无法进入输出 schema
                    "session_id": {"type": "string", "description": "要审计恢复状态的 session id。"},  # 新增代码+StatusToolsV2: session_id 指定恢复报告目标；若没有这行代码，工具不知道读哪次会话
                },  # 新增代码+StatusToolsV2: properties 定义结束；若没有这行代码，参数 schema 不完整
                "required": ["session_id"],  # 新增代码+StatusToolsV2: 恢复报告必须指定 session_id；若没有这行代码，空调用会进入工具层再失败
                "additionalProperties": False,  # 新增代码+StatusToolsV2: 禁止模型传入无关参数；若没有这行代码，恢复报告参数会被污染
            },  # 新增代码+StatusToolsV2: parameters 定义结束；若没有这行代码，Python 字典结构不完整
        },  # 新增代码+StatusToolsV2: function 定义结束；若没有这行代码，工具定义结构不完整
    },  # 新增代码+StatusToolsV2: resume_report 工具定义结束；若没有这行代码，工具列表语法不完整
    {  # 新增代码+StatusToolsV2: 新增 run_status 工具定义开始；若没有这行代码，模型无法直接查看当前 run/turn 状态
        "type": "function",  # 新增代码+StatusToolsV2: OpenAI-compatible 工具类型固定写 function；若没有这行代码，模型接口不会识别这是可调用工具
        "function": {  # 新增代码+StatusToolsV2: function 内部描述工具名称、说明和参数；若没有这行代码，run 状态工具缺少标准定义结构
            "name": "run_status",  # 新增代码+StatusToolsV2: 工具名用于查询 run 状态；若没有这行代码，模型无法请求当前长期任务阶段
            "description": "读取当前或指定 harness run 的状态，并同时返回 current_turn，帮助模型判断长任务卡在哪一阶段。",  # 新增代码+StatusToolsV2: 告诉模型此工具用于阶段定位；若没有这行代码，模型可能只能读取完整状态页
            "parameters": {  # 新增代码+StatusToolsV2: JSON Schema 描述 run_status 参数；若没有这行代码，模型不知道如何指定 run_id
                "type": "object",  # 新增代码+StatusToolsV2: 参数必须是对象；若没有这行代码，工具调用结构不明确
                "properties": {  # 新增代码+StatusToolsV2: 声明允许的参数字段；若没有这行代码，run_id 无法进入输出 schema
                    "run_id": {"type": "string", "description": "可选 run id；不传时返回当前 run 和最近 run 摘要。"},  # 新增代码+StatusToolsV2: run_id 精确定位长期任务；若没有这行代码，模型无法查指定 run
                },  # 新增代码+StatusToolsV2: properties 定义结束；若没有这行代码，参数 schema 不完整
                "required": [],  # 新增代码+StatusToolsV2: run 状态允许无参数调用；若没有这行代码，模型无法直接查看当前 run
                "additionalProperties": False,  # 新增代码+StatusToolsV2: 禁止模型传入无关参数；若没有这行代码，run 状态参数会被污染
            },  # 新增代码+StatusToolsV2: parameters 定义结束；若没有这行代码，Python 字典结构不完整
        },  # 新增代码+StatusToolsV2: function 定义结束；若没有这行代码，工具定义结构不完整
    },  # 新增代码+StatusToolsV2: run_status 工具定义结束；若没有这行代码，工具列表语法不完整
    {  # 新增代码+StatusToolsV2: 新增 health_status 工具定义开始；若没有这行代码，模型无法自查健康风险
        "type": "function",  # 新增代码+StatusToolsV2: OpenAI-compatible 工具类型固定写 function；若没有这行代码，模型接口不会识别这是可调用工具
        "function": {  # 新增代码+StatusToolsV2: function 内部描述工具名称、说明和参数；若没有这行代码，健康状态工具缺少标准定义结构
            "name": "health_status",  # 新增代码+StatusToolsV2: 工具名用于查询健康状态；若没有这行代码，模型无法主动检查 warnings/verifier
            "description": "读取 health、resume、verifier 和 current_run 风险状态，帮助模型决定继续执行、等待用户还是停止。",  # 新增代码+StatusToolsV2: 告诉模型此工具用于长任务自查；若没有这行代码，模型可能忽略状态风险
            "parameters": {  # 新增代码+StatusToolsV2: JSON Schema 描述 health_status 参数；若没有这行代码，工具定义不完整
                "type": "object",  # 新增代码+StatusToolsV2: 参数必须是对象；若没有这行代码，工具调用结构不明确
                "properties": {},  # 新增代码+StatusToolsV2: 健康状态当前不需要参数；若没有这行代码，schema 缺少 properties 字段
                "required": [],  # 新增代码+StatusToolsV2: 健康状态允许无参数调用；若没有这行代码，模型必须传无意义字段
                "additionalProperties": False,  # 新增代码+StatusToolsV2: 禁止模型传入无关参数；若没有这行代码，健康状态参数会被污染
            },  # 新增代码+StatusToolsV2: parameters 定义结束；若没有这行代码，Python 字典结构不完整
        },  # 新增代码+StatusToolsV2: function 定义结束；若没有这行代码，工具定义结构不完整
        ...
        "function": {  # 新增代码+OSComputerUse: function 内部描述工具名称、说明和参数；若没有这行代码，Computer Use 动作工具缺少标准定义结构。
            ...
            "parameters": {  # 新增代码+OSComputerUse: JSON Schema 描述 computer_action 参数；若没有这行代码，模型不知道动作字段怎么传。
                ...
                "properties": {  # 新增代码+OSComputerUse: 声明允许的动作参数字段；若没有这行代码，模型无法构造合法调用。
                    "action": {"type": "string", "enum": ["screenshot", "move_mouse", "click", "double_click", "type_text", "press_key", "scroll"], "description": "要执行的桌面动作。"},  # 新增代码+OSComputerUse: 限定动作枚举；若没有这行代码，模型可能传入未知危险动作。
    ...
    {  # 新增代码+Phase49ComputerUseToolSurface: 新增 computer_use 兼容工具定义开始；如果没有这段代码，模型无法使用统一 Computer Use 工具入口。
        ...
        "function": {  # 新增代码+Phase49ComputerUseToolSurface: function 内部描述兼容工具名称、说明和参数；如果没有这行代码，工具定义结构不完整。
            ...
            "description": "兼容式 OS Computer Use 统一入口；operation=status/observe/action/mode/run_prompt，内部会转发到 computer_status、computer_observe、computer_action 或通用 Windows Computer Use mode，同样经过 controller、安全策略和 evidence。",  # 修改代码+Phase92UniversalComputerUse：说明兼容入口支持通用 mode；如果没有这行代码，模型不知道可以用 prompt 打开通用 Computer Use。
            "parameters": {  # 新增代码+Phase49ComputerUseToolSurface: JSON Schema 描述统一入口参数；如果没有这行代码，模型不知道 operation 和动作字段怎么传。
                ...
                "properties": {  # 新增代码+Phase49ComputerUseToolSurface: 声明兼容工具允许的参数字段；如果没有这行代码，operation/action/window 等字段无法进入 schema。
                    "operation": {"type": "string", "enum": ["status", "observe", "action", "mode", "run_prompt"], "description": "要执行的统一操作：status 查询状态，observe 只读观察，action 执行桌面动作，mode/run_prompt 用自然语言 prompt 进入通用 Windows Computer Use 模式。"},  # 修改代码+Phase92UniversalComputerUse：把 mode/run_prompt 加入枚举；如果没有这行代码，模型工具调用会被 schema 拦住。
                    "op": {"type": "string", "enum": ["status", "observe", "action", "mode", "run_prompt"], "description": "operation 的 ClaudeCode/MCP 风格别名。"},  # 修改代码+Phase92UniversalComputerUse：同步 op 别名枚举；如果没有这行代码，外部习惯用 op 的调用无法进入 mode。
                    "prompt": {"type": "string", "description": "operation=mode 或 run_prompt 时使用的自然语言任务；日志只保存哈希和长度。"},  # 新增代码+Phase92UniversalComputerUse：允许模型传入通用 mode prompt；如果没有这行代码，用户自然语言任务无法进入新运行时。
    ...
    {  # 新增代码+Phase49ComputerUseToolSurface: 新增 computer-use 兼容工具定义开始；如果没有这段代码，ClaudeCode 风格连字符工具名无法被兼容。
        ...
        "function": {  # 新增代码+Phase49ComputerUseToolSurface: function 内部描述连字符兼容工具；如果没有这行代码，工具定义结构不完整。
            ...
            "description": "ClaudeCode 风格 OS Computer Use 兼容入口；参数与 computer_use 相同，支持 status/observe/action/mode/run_prompt，内部仍走同一 controller 和通用 mode。",  # 修改代码+Phase92UniversalComputerUse：说明连字符工具也支持 mode；如果没有这行代码，ClaudeCode 风格调用不知道新能力。
            "parameters": {  # 新增代码+Phase49ComputerUseToolSurface: JSON Schema 描述连字符工具参数；如果没有这行代码，模型不知道如何调用该别名。
                ...
                "properties": {  # 新增代码+Phase49ComputerUseToolSurface: 声明允许参数字段；如果没有这行代码，operation 和动作字段无法进入 schema。
                    "operation": {"type": "string", "enum": ["status", "observe", "action", "mode", "run_prompt"], "description": "要执行的统一操作：status、observe、action、mode 或 run_prompt。"},  # 修改代码+Phase92UniversalComputerUse：把 mode/run_prompt 加入连字符工具枚举；如果没有这行代码，ClaudeCode 风格别名会拒绝新模式。
                    "op": {"type": "string", "enum": ["status", "observe", "action", "mode", "run_prompt"], "description": "operation 的短别名。"},  # 修改代码+Phase92UniversalComputerUse：同步 op 别名枚举；如果没有这行代码，MCP 风格短字段不能进入 mode。
                    "prompt": {"type": "string", "description": "operation=mode 或 run_prompt 时使用的自然语言任务；日志只保存哈希和长度。"},  # 新增代码+Phase92UniversalComputerUse：允许连字符别名传入通用 mode prompt；如果没有这行代码，外部 agent 无法用自然语言任务调用。
...
DYNAMIC_SKILL_CAPABILITY_PACKS: dict[str, tuple[str, ...]] = {  # 修改代码+长任务真实入口: 定义哪些动态 skill 读取后可以解锁对应工具能力包；若没有这行代码，read-based skill 只能加载提示词不能激活真实工具
    "file_operations": ("file_operations",),  # 新增代码+长任务真实入口: 文件操作 skill 读取后解锁旧版 read_file/write_file 兼容工具；若没有这行代码，依赖旧接口的模型无法按说明恢复文件工具
    "memory": ("memory",),  # 新增代码+长任务真实入口: 记忆 skill 读取后解锁 append_memory；若没有这行代码，模型读完记忆说明仍不能写入长期记忆
    "execution": ("execution",),  # 新增代码+长任务真实入口: 执行 skill 读取后解锁后台命令生命周期工具；若没有这行代码，真实终端只能看到 bash 而不能托管长任务进程
    "notebook": ("notebook",),  # 新增代码+长任务真实入口: Notebook skill 读取后解锁 notebook_read/notebook_edit；若没有这行代码，模型读完说明仍无法安全处理 .ipynb
    "mcp": ("mcp",),  # 新增代码+长任务真实入口: MCP skill 读取后解锁 MCP resource/prompt/stream 工具；若没有这行代码，MCP 说明书和真实 MCP 能力会脱节
    "browser_automation": ("browser_automation",),  # 修改代码+长任务真实入口: 普通浏览器 skill 只解锁普通浏览器自动化工具包；若没有这行代码，browser_open/click 等工具读完 skill 后仍不可见
    "real_chrome": ("real_chrome", "browser_automation"),  # 修改代码+长任务真实入口: 真实 Chrome skill 同时准备连接工具和后续页面操作工具；若没有这行代码，连接成功后仍缺少打开、点击、截图等动作工具
    "delegation": ("delegation",),  # 新增代码+长任务真实入口: 委派 skill 读取后解锁 task/team peer 工具；若没有这行代码，子任务只能留在内存旁路而无法从真实工具池进入
    "planning": ("planning",),  # 新增代码+长任务真实入口: 计划 skill 读取后解锁计划模式和工作区隔离工具；若没有这行代码，复杂任务确认流程无法从说明书进入执行层
    "diagnostics": ("diagnostics",),  # 新增代码+长任务真实入口: 诊断 skill 读取后解锁 LSP/REPL 诊断工具；若没有这行代码，模型读完诊断说明仍只能用普通 bash 粗查
    "long_running_work": ("execution", "long_running_work"),  # 新增代码+长任务真实入口: 长期工作 skill 同时解锁后台进程执行包和监控记录包；若没有这行代码，真实终端无法启动后台任务并留下可审计状态
    "prompt_architecture": ("prompt_architecture",),  # 新增代码+长任务真实入口: 提示词架构 skill 读取后解锁 prompt/token 报告工具；若没有这行代码，模型无法审计当前提示词和工具池预算
    "computer_use": ("computer_use",),  # 新增代码+OSComputerUse: Computer Use skill 读取后解锁桌面控制工具包；若没有这行代码，OS 桌面能力无法按包加载。
}  # 修改代码+长任务真实入口: 动态 skill 到能力包映射结束；若没有这行代码，Python 字典语法不完整
...
BUILTIN_TOOL_CAPABILITY_PACKS: dict[str, str] = {  # 新增代码+CapabilityPacks: 定义内置工具到能力包的映射；若没有这行代码，内置工具无法按包延迟加载
    ...
    "status_snapshot": "long_running_work",  # 新增代码+StatusTools: 把状态快照归入长期工作包；若没有这行代码，状态总览无法随长任务能力包加载
    "task_status": "long_running_work",  # 新增代码+StatusTools: 把任务状态查询归入长期工作包；若没有这行代码，长任务恢复时模型难以统一查看任务
    "session_list": "long_running_work",  # 新增代码+StatusTools: 把 session 列表归入长期工作包；若没有这行代码，中断恢复入口会和长期工作包脱节
    "session_resume": "long_running_work",  # 新增代码+StatusTools: 把 session 恢复上下文归入长期工作包；若没有这行代码，模型无法按包加载恢复审计能力
    "compact_status": "long_running_work",  # 新增代码+StatusTools: 把 compact 状态归入长期工作包；若没有这行代码，压缩恢复检查会被拆散
    "event_tail": "long_running_work",  # 新增代码+StatusTools: 把事件尾部读取归入长期工作包；若没有这行代码，外部观察事件流能力不会随包加载
