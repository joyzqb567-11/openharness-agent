...
class WindowsSendInputExecutor:  # 新增代码+Phase37WindowsSendInputExecutor: 定义 Windows SendInput 动作执行器；如果没有这个类，learning_agent 仍停留在旧 SetCursorPos/mouse_event 路径。
    ...
    def _requires_raw_text_for_sender(self) -> bool:  # 新增代码+ComputerUsePressureReadiness：函数段开始，判断底层实现是否真的需要原始文本；如果没有这段函数，type_text 要么永远丢字，要么把明文传给所有测试 sender。
        return bool(getattr(self.sendinput_impl, "requires_raw_text", False))  # 新增代码+ComputerUsePressureReadiness：只信任底层实现显式声明；如果没有这行代码，普通记录型 sender 可能意外拿到用户原文。
    # 新增代码+ComputerUsePressureReadiness：函数段结束，WindowsSendInputExecutor._requires_raw_text_for_sender 到此结束；如果没有这个边界说明，初学者不容易看出明文传递门禁范围。
    ...
    def _events_for_action(self, action: str, arguments: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any], WindowsSendInputActionResult | None]:  # 新增代码+Phase37WindowsSendInputExecutor: 函数段开始，把高层动作转成规范输入事件；如果没有这段函数，动作分支会和执行逻辑混在一起。
        ...
        event = {"type": "text", **summary}  # 修改代码+ComputerUsePressureReadiness：默认仍生成不含原文的文本事件；如果没有这行代码，普通自检和记录路径可能泄露用户输入。
        if self._requires_raw_text_for_sender():  # 新增代码+ComputerUsePressureReadiness：只有真实发送链路明确需要原文时才打开内存通道；如果没有这行代码，低层 Unicode 输入会继续收到空字符串。
            event["text"] = text  # 新增代码+ComputerUsePressureReadiness：把原文只交给声明需要它的底层实现；如果没有这行代码，WindowsSendInputLowLevelSender 无法真实逐字符输入。
