...
class WindowsComputerUseLaunchResolverPhase124Tests(unittest.TestCase):  # 新增代码+WindowsLaunchResolver：测试类开始，集中验证 Windows 多后端启动解析器；如果没有这个类，Phase124 行为没有回归保护。
    ...
    def test_phase108_uses_resolver_for_appx_candidate(self) -> None:  # 新增代码+WindowsLaunchResolver：函数段开始，验证 Phase108 已把 launch_plan 接到 resolver；如果没有这段测试，resolver 可能存在但主链路仍不用。
        phase108 = importlib.import_module("learning_agent.computer_use.generic_app_discovery")  # 新增代码+WindowsLaunchResolver：读取 Phase108 通用发现模块；如果没有这一行，测试无法检查真实接入点。
    ...
    def test_phase110_request_keeps_non_argv_backend_out_of_executable_slot(self) -> None:  # 新增代码+WindowsLaunchResolver：函数段开始，验证非 argv resolver 计划不会被 Phase110 回填成可执行名；如果没有这段测试，AppX 可能被未来代码误启动成短 exe。
        phase108 = importlib.import_module("learning_agent.computer_use.generic_app_discovery")  # 新增代码+WindowsLaunchResolver：读取 Phase108 模块；如果没有这一行，测试无法生成真实主链路报告。
        ...
        self.assertEqual(request.executable, "")  # 新增代码+WindowsLaunchResolver：断言非 argv 后端不填 executable；如果没有这一行，未来 Popen 路径可能误用 canonical_target。
        self.assertEqual(request.argv(), [""])  # 新增代码+WindowsLaunchResolver：断言 argv 不包含 calc.exe 之类假命令；如果没有这一行，非 argv 后端可能被误当 argv 计划。
        self.assertFalse(backend.phase110_safe_launch_request(request))  # 新增代码+WindowsLaunchResolver：断言当前 Phase110 不放行 AppX 到 Popen；如果没有这一行，真实桌面可能被错误后端触碰。
    # 新增代码+WindowsLaunchResolver：函数段结束，test_phase110_request_keeps_non_argv_backend_out_of_executable_slot 到此结束；如果没有这个边界说明，用户不容易看出 Phase110 防误接测试范围。
    ...
    def test_phase108_cli_line_exposes_resolver_backend_for_visible_terminal(self) -> None:  # 新增代码+WindowsLaunchResolver：函数段开始，验证真实终端 token 行暴露 resolver 后端；如果没有这段测试，可见验收只能靠隐藏 JSON 推断。
        phase108 = importlib.import_module("learning_agent.computer_use.generic_app_discovery")  # 新增代码+WindowsLaunchResolver：读取 Phase108 模块；如果没有这一行，测试无法调用 CLI 格式化器。
