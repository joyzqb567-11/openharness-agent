...
def build_generic_launch_request(phase108_report: GenericLaunchRequest | dict[str, Any], real_launch_authorized: bool = False, request_id: str = "phase110-generic-launch-request") -> GenericLaunchRequest:  # 新增代码+GenericLaunchBackendMaturity：函数段开始，把 Phase108 报告转换成唯一启动请求；如果没有这个函数，后端会直接吃松散字典。
    ...
    executable = str(plan.get("executable") or report.get("best_candidate_executable") or report.get("canonical_target") or "").strip() if str(plan.get("launch_backend") or "argv_no_shell") == "argv_no_shell" else ""  # 修改代码+WindowsLaunchResolver：只有 argv_no_shell 计划才允许回填 executable；如果没有这一行，AppX/shortcut 会被错误塞进 Popen 可执行槽。
