...
try:  # 新增代码+Phase95ControlledPhysicalSendInput：优先按包路径导入项目内组件；如果没有这段代码，单元测试和生产入口不能共享同一实现。
    from learning_agent.computer_use_mcp_v2.windows_runtime.authorized_real_dispatch import WindowsAuthorizedRealDispatchCandidate, run_phase94_authorized_real_dispatch_candidate_contract  # 新增代码+Phase95ControlledPhysicalSendInput：复用 Phase94 授权和目标复核；如果没有这行代码，Phase95 可能绕开已有安全门。
    from learning_agent.computer_use_mcp_v2.windows_runtime.persistent_grants import DEFAULT_PERSISTENT_GRANTS_ROOT  # 新增代码+Phase95ControlledPhysicalSendInput：复用项目现有 Computer Use 运行目录；如果没有这行代码，报告落点会散乱。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.runtime", "learning_agent.runtime.files"}:  # 新增代码+Phase95ControlledPhysicalSendInput：只对包前缀缺失做 fallback；如果没有这行代码，内部真实 bug 可能被误吞。
        raise  # 新增代码+Phase95ControlledPhysicalSendInput：重新抛出非路径类导入错误；如果没有这行代码，排查底层模块问题会很困难。
    from computer_use_mcp_v2.windows_runtime.authorized_real_dispatch import WindowsAuthorizedRealDispatchCandidate, run_phase94_authorized_real_dispatch_candidate_contract  # type: ignore  # 新增代码+Phase95ControlledPhysicalSendInput：脚本模式复用 Phase94；如果没有这行代码，bat 入口无法跑合同。
    from computer_use_mcp_v2.windows_runtime.persistent_grants import DEFAULT_PERSISTENT_GRANTS_ROOT  # type: ignore  # 新增代码+Phase95ControlledPhysicalSendInput：脚本模式复用默认目录；如果没有这行代码，报告目录无法稳定定位。
...
PHASE95_SUPPORTED_LOW_LEVEL_TYPES = ("set_foreground", "pause", "mouse_move", "mouse_down", "mouse_up", "mouse_wheel", "keyboard_text", "unicode_text", "key_down", "key_up")  # 修改代码+ControlledPhysicalPauseBridge：低层白名单加入通用 loop 的安全等待事件；如果没有 pause，set_foreground 后的稳定等待会被 Phase95 拒绝，导致 /computer maturity 误判受控物理后端没有接通。
...
PHASE95_SECURE_TEXT_KEYS = ("_secure_plaintext_text",)  # 修改代码+SecureTextChannel：定义只允许内存短通道使用的私有明文字段；如果没有这行代码，真实文本输入只能在泄露和失败之间二选一。
...
def _phase95_secure_text_from_event(event: dict[str, Any]) -> str | None:  # 修改代码+SecureTextChannel：函数段开始，从低层事件读取私有明文短通道；如果没有这段函数，真实文本后端无法安全拿到要输入的文字。
    for key in PHASE95_SECURE_TEXT_KEYS:  # 修改代码+SecureTextChannel：遍历允许的私有字段；如果没有这行代码，字段名会散落在多个位置。
        if key in event:  # 修改代码+SecureTextChannel：只在事件确实携带私有字段时读取；如果没有这行代码，缺字段和空文本无法区分。
            return str(event.get(key, ""))  # 修改代码+SecureTextChannel：返回短生命周期明文；如果没有这行代码，最后一跳真实 Unicode 输入仍没有字符来源。
    return None  # 修改代码+SecureTextChannel：没有私有字段时返回 None；如果没有这行代码，调用方无法决定是否拒绝真实文本输入。
# 修改代码+SecureTextChannel：函数段结束，_phase95_secure_text_from_event 到此结束；如果没有这个边界说明，初学者不容易看出私有文本读取范围。


def _phase95_backend_event(event: dict[str, Any], include_secure_text: bool = False) -> dict[str, Any]:  # 修改代码+SecureTextChannel：函数段开始，把安全事件转成后端事件并可按需注入明文；如果没有这段函数，真实 SendInput 后端可能不认识 kind。
    event_type = _phase95_event_type(event)  # 新增代码+Phase95ControlledPhysicalSendInput：读取兼容事件类型；如果没有这行代码，转换分支没有依据。
    blocked_keys = set(PHASE95_RAW_TEXT_KEYS) | set(PHASE95_SECURE_TEXT_KEYS)  # 修改代码+SecureTextChannel：同时剥离公开明文字段和私有短通道字段；如果没有这行代码，内部字段可能流入日志型后端。
    clean_event = {key: value for key, value in dict(event).items() if key not in blocked_keys}  # 修改代码+SecureTextChannel：复制并删除所有明文相关字段；如果没有这行代码，坏事件可能继续携带原文。
    ...
    if include_secure_text and event_type in {"keyboard_text", "unicode_text"}:  # 修改代码+SecureTextChannel：仅在真实文本后端需要时恢复明文字段；如果没有这行代码，文本输入无法最终落到 Windows。
        secure_text = _phase95_secure_text_from_event(event)  # 修改代码+SecureTextChannel：读取私有短通道明文；如果没有这行代码，后端会收到空文本。
        if secure_text is not None:  # 修改代码+SecureTextChannel：只有私有通道存在时才注入后端 text；如果没有这行代码，缺通道也可能被当成空输入成功。
            clean_event["text"] = secure_text  # 修改代码+SecureTextChannel：只把明文交给最后一跳后端；如果没有这行代码，真实 Unicode sender 无法逐字符输入。
    return clean_event  # 新增代码+Phase95ControlledPhysicalSendInput：返回安全后端事件；如果没有这行代码，sender 无法调用底层后端。
# 修改代码+SecureTextChannel：函数段结束，_phase95_backend_event 到此结束；如果没有这个边界说明，初学者不容易看出事件转换范围。
...
class WindowsControlledPhysicalSendInputSender:  # 新增代码+Phase95ControlledPhysicalSendInput：类段开始，定义受控物理 sender；如果没有这个类，Phase94 低层事件仍不能进入受控物理后端。
    requires_raw_text = True  # 修改代码+SecureTextChannel：声明受控物理 sender 能接收私有短通道文本；如果没有这行代码，dispatcher 不会给真实文本输入准备明文。
    accepts_secure_plaintext_text_channel = True  # 修改代码+SecureTextChannel：声明本 sender 只能通过私有字段接收明文；如果没有这行代码，dispatcher 会退回公开 text 字段并被 Phase95 拒绝。
    ...
    def _backend(self) -> Any | None:  # 新增代码+Phase95ControlledPhysicalSendInput：函数段开始，返回可用后端；如果没有这段函数，真实后端创建逻辑会散落。
        ...
        try:  # 新增代码+Phase95ControlledPhysicalSendInput：按需导入 Phase58 真实 sender；如果没有这行代码，Phase95 不能连接已有真实 SendInput 实现。
            from learning_agent.computer_use_mcp_v2.windows_runtime.real_sendinput_guard import WindowsSendInputLowLevelSender  # 新增代码+Phase95ControlledPhysicalSendInput：导入真实低层 sender；如果没有这行代码，启用真实后端时没有实现来源。
        except ModuleNotFoundError:  # 新增代码+Phase95ControlledPhysicalSendInput：兼容脚本模式导入；如果没有这行代码，bat 入口可能找不到包路径。
            from computer_use_mcp_v2.windows_runtime.real_sendinput_guard import WindowsSendInputLowLevelSender  # type: ignore  # 新增代码+Phase95ControlledPhysicalSendInput：脚本模式导入真实 sender；如果没有这行代码，脚本入口不能复用真实后端。
    ...
    def send_low_level(self, events: list[dict[str, Any]], enable_physical_dispatch: bool | None = None) -> dict[str, Any]:  # 新增代码+Phase95ControlledPhysicalSendInput：函数段开始，受控发送低层事件；如果没有这段函数，Phase94 不能把事件交到物理后端。
        ...
        backend_requires_raw_text = bool(getattr(backend, "requires_raw_text", False))  # 修改代码+SecureTextChannel：读取最后一跳是否必须拿到明文；如果没有这行代码，真实后端和记录后端会共用错误路径。
        missing_secure_text = backend_requires_raw_text and any(_phase95_event_type(event) in {"keyboard_text", "unicode_text"} and _phase95_secure_text_from_event(event) is None for event in event_list)  # 修改代码+SecureTextChannel：检查文本事件是否缺少私有短通道；如果没有这行代码，真实后端可能收到空文本或哈希。
        if missing_secure_text:  # 修改代码+SecureTextChannel：真实文本事件缺通道时拒绝；如果没有这行代码，隐私门禁和真实输入都会不可靠。
        ...
        backend_events = [_phase95_backend_event(event, include_secure_text=backend_requires_raw_text) for event in event_list]  # 修改代码+SecureTextChannel：按后端需求转换事件并只给真实文本后端恢复明文；如果没有这行代码，文本输入不是失败就是泄露。
