...
def resolve_windows_app_name_hint(target_app_hint: Any, catalog: list[dict[str, Any]] | None = None) -> str:  # 修改代码+WindowsAppInventory：函数段开始，在统一 inventory 内解析用户友好应用提示；如果没有这段函数，删除 app_names.py 后中文应用名会失去稳定解析入口。
...
__all__ = ["APP_INVENTORY_MAX_COUNT", "build_windows_app_inventory", "discover_windows_app_inventory_sources", "format_windows_app_inventory_for_model", "resolve_windows_app_name_hint", "sanitize_inventory_display_name"]  # 修改代码+WindowsAppInventory：限定公开 API 并暴露统一解析入口；如果没有这一行，通配导入可能找不到应用名解析能力或暴露内部 helper。
