...
def _desktop_launch_target_app_from_arguments(arguments: dict[str, Any]) -> str:  # 新增代码+FreshTargetRepeatedLaunchGate：函数段开始，抽取 launch_app 的目标应用名；如果没有这段函数，重复启动门禁会散落读取多个字段。
    return str(arguments.get("target_app") or arguments.get("app_name") or arguments.get("app") or arguments.get("target") or "").strip()  # 新增代码+FreshTargetRepeatedLaunchGate：按兼容字段读取目标应用；如果没有这一行，Notepad/notepad.exe 等入口可能漏判。
# 新增代码+FreshTargetRepeatedLaunchGate：函数段结束，_desktop_launch_target_app_from_arguments 到此结束；如果没有这个边界说明，用户不容易看出字段来源。


def _desktop_redundant_launch_target_ref(action_name: str, arguments: dict[str, Any], controller: Any) -> str:  # 新增代码+FreshTargetRepeatedLaunchGate：函数段开始，判断重复 launch_app 是否已有可用 target_ref；如果没有这段函数，模型会反复申请打开同一软件。
    if str(action_name or "").strip() != "launch_app":  # 新增代码+FreshTargetRepeatedLaunchGate：只处理打开应用动作；如果没有这一行，普通输入点击会被错误检查。
        return ""  # 新增代码+FreshTargetRepeatedLaunchGate：非 launch_app 不返回重复引用；如果没有这一行，函数缺少不适用出口。
    safe_arguments = arguments if isinstance(arguments, dict) else {}  # 新增代码+FreshTargetRepeatedLaunchGate：防御异常参数；如果没有这一行，坏输入会导致 get 调用崩溃。
    if bool(safe_arguments.get("explicit_existing_window_request") or safe_arguments.get("user_authorized_window") or safe_arguments.get("force_new_target") or safe_arguments.get("new_target_instance_required")):  # 新增代码+FreshTargetRepeatedLaunchGate：显式授权/强制新目标时不拦；如果没有这一行，合法多窗口或用户授权场景不可用。
        return ""  # 新增代码+FreshTargetRepeatedLaunchGate：返回空表示不做重复启动拦截；如果没有这一行，显式场景仍会被误挡。
    target_app = _desktop_launch_target_app_from_arguments(safe_arguments)  # 新增代码+FreshTargetRepeatedLaunchGate：抽取目标应用名；如果没有这一行，无法查注册表已有目标。
    if not target_app:  # 新增代码+FreshTargetRepeatedLaunchGate：没有目标应用名时不拦；如果没有这一行，未知动作可能被错误拒绝。
        return ""  # 新增代码+FreshTargetRepeatedLaunchGate：返回空表示没有重复目标证据；如果没有这一行，函数缺少空目标出口。
    target_registry = getattr(controller, "target_registry", None) if controller is not None else None  # 新增代码+FreshTargetRepeatedLaunchGate：读取 controller 的目标注册表；如果没有这一行，无法发现已有 target_ref。
    if target_registry is None or not hasattr(target_registry, "resolve_target_label"):  # 新增代码+FreshTargetRepeatedLaunchGate：旧 controller 或测试 fake 没有注册表时不拦；如果没有这一行，兼容路径会崩溃。
        return ""  # 新增代码+FreshTargetRepeatedLaunchGate：返回空表示无法判断重复；如果没有这一行，老环境会被误杀。
    resolved = target_registry.resolve_target_label(target_app)  # 新增代码+FreshTargetRepeatedLaunchGate：按应用标签查找已有目标；如果没有这一行，重复启动门禁没有事实来源。
    if bool(resolved.get("allowed")) and str(resolved.get("target_ref") or "").strip():  # 新增代码+FreshTargetRepeatedLaunchGate：已有有效 target_ref 时判定重复启动；如果没有这一行，模型会继续开同一应用。
        return str(resolved.get("target_ref") or "").strip()  # 新增代码+FreshTargetRepeatedLaunchGate：返回已有 target_ref；如果没有这一行，纠偏信息无法告诉模型该用哪个 ID。
    return ""  # 新增代码+FreshTargetRepeatedLaunchGate：没有已有目标时允许 launch_app 继续；如果没有这一行，函数没有默认出口。
# 新增代码+FreshTargetRepeatedLaunchGate：函数段结束，_desktop_redundant_launch_target_ref 到此结束；如果没有这个边界说明，用户不容易看出重复启动判断范围。


def _desktop_redundant_launch_rejection_text(action_name: str, arguments: dict[str, Any], controller: Any, record_observation: RecordObservation, record_runtime_trace: RecordRuntimeTrace) -> str | None:  # 新增代码+FreshTargetRepeatedLaunchGate：函数段开始，生成重复 launch_app 的权限前拒绝文本；如果没有这段函数，重复启动会先弹权限再被 controller 拒绝。
    target_ref = _desktop_redundant_launch_target_ref(action_name, arguments, controller)  # 新增代码+FreshTargetRepeatedLaunchGate：查找已有同应用 target_ref；如果没有这一行，拒绝文本没有判断依据。
    if not target_ref:  # 新增代码+FreshTargetRepeatedLaunchGate：没有重复目标时不拒绝；如果没有这一行，正常首次启动会被阻断。
        return None  # 新增代码+FreshTargetRepeatedLaunchGate：返回 None 表示继续正常动作流程；如果没有这一行，调用方无法区分放行和拒绝。
    report = {"ok": False, "decision": "existing_target_ref_must_be_used_before_relaunch", "action": action_name, "target_ref": target_ref, "reason": "同一应用已经有本轮 Computer Use 建立的 target_ref，后续应观察/操作该 target_ref，不应反复 launch_app。", "next_tools": ["mcp__computer-use__observe"], "next_required_action": "get_window_state"}  # 新增代码+FreshTargetRepeatedLaunchGate：构造模型可读纠偏报告；如果没有这一行，模型不知道该停止重复打开并使用哪个 target_ref。
    record_observation("computer_use_redundant_launch_rejected", report)  # 新增代码+FreshTargetRepeatedLaunchGate：记录重复启动拒绝事件；如果没有这一行，验收无法证明权限前门禁生效。
    record_runtime_trace("computer_redundant_launch_rejected", {"action": action_name, "target_ref": target_ref, "low_level_event_count": 0})  # 新增代码+FreshTargetRepeatedLaunchGate：写入 runtime trace 零事件证据；如果没有这一行，调试时看不到为什么没有继续弹权限。
    return "Computer Use 已拒绝重复打开同一目标应用：本轮已经有可用 target_ref，请改用该 target_ref 观察和操作窗口，不要重新 launch_app。\n" + json.dumps(report, ensure_ascii=False, indent=2)  # 新增代码+FreshTargetRepeatedLaunchGate：返回中文纠偏文本；如果没有这一行，模型会继续重复启动导致压力场景耗尽时间。
# 新增代码+FreshTargetRepeatedLaunchGate：函数段结束，_desktop_redundant_launch_rejection_text 到此结束；如果没有这个边界说明，用户不容易看出重复启动拒绝范围。
...
def computer_action(arguments: dict[str, Any], controller: Any, ask_permission: AskPermission, observe_before_action_rejection: ActionGate, agent_owned_launch_rejection: ActionGate, completion_signal_for_action: ActionGate, record_observation: RecordObservation, record_runtime_trace: RecordRuntimeTrace, record_image_artifacts: RecordImageArtifacts, emit_acceptance_event: AcceptanceEmitter) -> str:  # 新增代码+AgentPySplitPhase6: 定义动作工具适配入口；如果没有这行代码，agent.py 的薄包装无法委托真实动作流程。
    ...
    redundant_launch_rejection = _desktop_redundant_launch_rejection_text(action_name, arguments, controller, record_observation, record_runtime_trace)  # 新增代码+FreshTargetRepeatedLaunchGate：在权限前拒绝重复 launch_app；如果没有这一行，模型会反复弹权限打开同一软件。
    if redundant_launch_rejection is not None:  # 新增代码+FreshTargetRepeatedLaunchGate：判断重复启动门禁是否触发；如果没有这一行，拒绝文本会被忽略继续弹权限。
        return redundant_launch_rejection  # 新增代码+FreshTargetRepeatedLaunchGate：直接返回纠偏结果且不触碰桌面；如果没有这一行，压力场景会陷入打开/拒绝循环。
