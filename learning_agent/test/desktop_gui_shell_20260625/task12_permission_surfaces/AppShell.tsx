...
import { appendStatusEvents, initialStatusState, latestPermissionEvent, normalizeStatusEvent, type StatusEvent } from "../state/statusStore"; // 修改代码+DesktopGUIPermissions：引入状态时间线和最新权限事件工具；如果没有这行，权限弹窗无法从事件流找到请求。
    ...
    <main className="app-shell"> {/* 修改代码+DesktopToolTimeline：定义三栏窗口网格容器；如果没有这行，侧栏、线程区和状态区无法稳定并排。 */}
      ...
      <section className="thread-panel" aria-label="当前对话"> {/* 修改代码+DesktopToolTimeline：定义当前对话主区域；如果没有这行，读屏器和布局都缺少对话容器。 */}
        ...
        <div className="thread-content"> {/* 新增代码+DesktopGUIPermissions：包裹权限提示和消息视图；如果没有这行，banner 会破坏主面板三段布局。 */}
          ...
          <ThreadView messages={threadState.messages.length > 0 ? threadState.messages : undefined} events={statusState.events} /> {/* 修改代码+DesktopGUIPermissions：把消息和事件传给主线程视图；如果没有这行，工具卡片和权限上下文不会出现在对话流里。 */}
