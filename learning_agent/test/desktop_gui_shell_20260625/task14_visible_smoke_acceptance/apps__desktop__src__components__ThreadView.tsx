...
import { RefreshCcw, X } from "lucide-react"; // 新增代码+DesktopTurnActions：引入重试和取消图标；如果没有这行，生命周期按钮只能用文字，界面不够像成熟桌面工具。
...
const cancellableStatuses = new Set(["queued", "running", "needs_permission", "cancelling"]); // 新增代码+DesktopTurnActions：定义哪些消息可显示取消按钮；如果没有这行，UI 会把取消规则散落在 JSX 里。
const retryableStatuses = new Set(["completed", "failed", "cancelled"]); // 新增代码+DesktopTurnActions：定义哪些消息可显示重试按钮；如果没有这行，完成、失败、取消后的重试入口不稳定。

function canCancelMessage(message: ThreadMessage): boolean { // 新增代码+DesktopTurnActions：函数段开始，判断消息是否能取消；如果没有这段，ThreadView 需要在 JSX 里写复杂条件。
  return message.role === "assistant" && typeof message.turnId === "string" && cancellableStatuses.has(message.status ?? ""); // 新增代码+DesktopTurnActions：只有带 turnId 的运行中助手消息可取消；如果没有这行，用户消息或已完成消息可能误显示取消。
} // 新增代码+DesktopTurnActions：函数段结束，canCancelMessage 到此结束；如果没有这个边界，用户不容易看出取消按钮规则范围。

function canRetryMessage(message: ThreadMessage): boolean { // 新增代码+DesktopTurnActions：函数段开始，判断消息是否能重试；如果没有这段，ThreadView 需要在 JSX 里写复杂条件。
  return message.role === "assistant" && typeof message.turnId === "string" && retryableStatuses.has(message.status ?? ""); // 新增代码+DesktopTurnActions：只有带 turnId 的终态助手消息可重试；如果没有这行，运行中消息可能误显示重试。
} // 新增代码+DesktopTurnActions：函数段结束，canRetryMessage 到此结束；如果没有这个边界，用户不容易看出重试按钮规则范围。
...
type ThreadViewProps = { // 修改代码+DesktopToolTimeline：定义组件 props；如果没有这段，父组件无法传入真实消息和事件。
  ...
  onCancelTurn?: (turnId: string) => void; // 新增代码+DesktopTurnActions：允许父组件处理取消；如果没有这行，取消按钮只能显示但无法调用 bridge。
  onRetryTurn?: (turnId: string) => void; // 新增代码+DesktopTurnActions：允许父组件处理重试；如果没有这行，重试按钮只能显示但无法调用 bridge。
...
export function ThreadView({ messages = fallbackMessages, events = [], onCancelTurn, onRetryTurn }: ThreadViewProps): JSX.Element { // 修改代码+DesktopTurnActions：函数段开始，渲染对话消息、工具事件和生命周期动作；如果没有这段，主面板没有消息内容。
  ...
  return ( // 修改代码+DesktopToolTimeline：返回消息列表结构；如果没有这行，组件不会输出 UI。
    <div className="thread-body"> {/* 修改代码+DesktopToolTimeline：定义可滚动消息区域；如果没有这行，长对话无法独立滚动。 */}
      {messages.map((message) => { // 修改代码+DesktopTurnActions：遍历消息数组并计算动作状态；如果没有这行，真实对话历史无法渲染。
        const canCancel = canCancelMessage(message); // 新增代码+DesktopTurnActions：计算该消息是否显示取消按钮；如果没有这行，JSX 里会重复判断。
        const canRetry = canRetryMessage(message); // 新增代码+DesktopTurnActions：计算该消息是否显示重试按钮；如果没有这行，JSX 里会重复判断。
        return ( // 修改代码+DesktopTurnActions：返回单条消息结构；如果没有这行，map 回调没有 JSX 结果。
          <article className={`message ${message.role}-message`} key={message.id}> {/* 修改代码+DesktopToolTimeline：渲染单条消息；如果没有这行，用户看不到消息内容。 */}
            <div className="message-role"> {/* 修改代码+DesktopTurnActions：渲染消息角色、状态和动作行；如果没有这行，消息来源和按钮会混在正文里。 */}
              ...
              {canCancel && message.turnId ? <button className="message-action-button" type="button" title="取消本轮" aria-label="取消本轮" onClick={() => onCancelTurn?.(message.turnId ?? "")}><X aria-hidden={true} size={13} /></button> : null} {/* 新增代码+DesktopTurnActions：渲染取消按钮并把 turnId 交给父组件；如果没有这行，用户无法在 GUI 中触发后端 cancel。 */}
              {canRetry && message.turnId ? <button className="message-action-button" type="button" title="重试本轮" aria-label="重试本轮" onClick={() => onRetryTurn?.(message.turnId ?? "")}><RefreshCcw aria-hidden={true} size={13} /></button> : null} {/* 新增代码+DesktopTurnActions：渲染重试按钮并把 turnId 交给父组件；如果没有这行，用户无法在 GUI 中触发后端 retry。 */}
            </div> {/* 修改代码+DesktopTurnActions：消息角色和动作行结束；如果没有这行，JSX 结构不完整。 */}
        ...
        ); // 新增代码+DesktopTurnActions：map 回调返回结束；如果没有这行，JSX return 语法不完整。
      })} {/* 修改代码+DesktopTurnActions：消息遍历结束；如果没有这行，JSX 表达式不完整。 */}
