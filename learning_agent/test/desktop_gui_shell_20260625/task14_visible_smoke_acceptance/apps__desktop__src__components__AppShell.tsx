...
import { appendStatusEvents, initialStatusState, latestPermissionEvent, messageTextFromStatusEvent, normalizeStatusEvent, type StatusEvent } from "../state/statusStore"; // 修改代码+DesktopFailedEventText：引入状态时间线、权限事件和消息正文提取工具；如果没有这行，失败事件可能无法把 error 写回消息卡。
...
function permissionFromEvent(event: StatusEvent | null): PermissionViewModel | null { // 新增代码+DesktopGUIPermissions：函数段开始，把状态事件转成权限弹窗数据；如果没有这段，UI 要到处猜 payload 字段。
  ...
  return { requestId, turnId, appName, reason, riskSummary, answered }; // 新增代码+DesktopGUIPermissions：返回权限 view model；如果没有这行，调用方拿不到可渲染数据。
...
export function AppShell(): JSX.Element { // 修改代码+DesktopToolTimeline：函数段开始，组织桌面 GUI 主壳；如果没有这段，各组件无法组合成 Codex 风格窗口。
  ...
  function syncTurnEventsToThread(events: StatusEvent[]): void { // 新增代码+DesktopFastEventReplay：函数段开始，把已取得的生命周期事件重放到消息线程；如果没有这段代码，快速失败事件可能早于助手占位消息而丢失正文同步。
    for (const event of events) { // 新增代码+DesktopFastEventReplay：逐条处理事件；如果没有这行代码，多个连续事件只能处理其中一个。
      const status = statusFromEventType(event.event_type); // 新增代码+DesktopFastEventReplay：把事件类型转成 turn 状态；如果没有这行代码，消息卡无法知道要切到 failed/completed/cancelled。
      if (status === null || event.turn_id.length === 0) { // 新增代码+DesktopFastEventReplay：跳过非 turn 事件；如果没有这行代码，permission_answered 等审计事件可能错误改消息。
        continue; // 新增代码+DesktopFastEventReplay：无关事件继续下一条；如果没有这行代码，无关事件会落入 reducer。
      } // 新增代码+DesktopFastEventReplay：跳过判断结束；如果没有这行代码，条件块语法不完整。
      dispatchThread({ type: "turn_status_changed", turnId: event.turn_id, status, text: messageTextFromStatusEvent(event), errorMessage: errorFromEvent(event) }); // 修改代码+DesktopFailedEventText：把状态和非空 answer/error 正文同步进消息卡；如果没有这行代码，失败事件会只显示状态不显示原因。
    } // 新增代码+DesktopFastEventReplay：事件重放循环结束；如果没有这行代码，for 循环语法不完整。
  } // 新增代码+DesktopFastEventReplay：函数段结束，syncTurnEventsToThread 到此结束；如果没有这个边界，初学者不易看出事件重放范围。
    ...
    <main className="app-shell"> {/* 修改代码+DesktopToolTimeline：定义三栏窗口网格容器；如果没有这行，侧栏、线程区和状态区无法稳定并排。 */}
      ...
      <section className="thread-panel" aria-label="当前对话"> {/* 修改代码+DesktopToolTimeline：定义当前对话主区域；如果没有这行，读屏器和布局都缺少对话容器。 */}
        ...
        <div className="thread-content"> {/* 新增代码+DesktopGUIPermissions：包裹权限提示和消息视图；如果没有这行，banner 会破坏主面板三段布局。 */}
          {permissionDialogOpen && permissionRequest ? <PermissionBanner title="等待权限确认" detail={`${permissionRequest.appName}：${permissionRequest.riskSummary}`} /> : null} {/* 新增代码+DesktopGUIPermissions：渲染权限等待提示条；如果没有这行，用户可能只看到消息停在 needs_permission。 */}
      ...
      <PermissionDialog open={permissionDialogOpen} requestId={permissionRequest?.requestId ?? ""} appName={permissionRequest?.appName ?? ""} reason={permissionRequest?.reason ?? ""} riskSummary={permissionRequest?.riskSummary ?? ""} onApprove={() => { void handlePermissionDecision("approve"); }} onDeny={() => { void handlePermissionDecision("deny"); }} /> {/* 新增代码+DesktopGUIPermissions：渲染权限确认弹窗并提交后端决策；如果没有这行，用户无法在 GUI 中允许或拒绝。 */}
