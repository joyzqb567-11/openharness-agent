...
GUI_SCHEMA_VERSION = 1  # 修改代码+DesktopGUIBridge：固定 GUI bridge 协议版本；如果没有这行，前端无法做兼容判断。
GUI_TOKEN_HEADER = "X-OpenHarness-Desktop-Token"  # 修改代码+DesktopGUIBridge：固定桌面 bridge token header；如果没有这行，前后端认证字段会分裂。
...
class GuiRunManager:  # 新增代码+DesktopGUILifecycle：类段开始，管理 GUI turn 生命周期；如果没有这个类，handler 会堆满业务状态逻辑。
    def __init__(self, workspace: str | Path, answer_runner: GuiAnswerRunner | None = None) -> None:  # 新增代码+DesktopGUILifecycle：初始化 run manager；如果没有这段，server 无法管理 active turn 和 session。
        ...
        self.answer_runner = answer_runner or self._default_answer_runner  # 新增代码+DesktopGUILifecycle：保存可插拔回答 runner；如果没有这行，未来无法接真实 agent。
    ...
    def _load_state(self) -> None:  # 新增代码+DesktopGUILifecycle：函数段开始，从磁盘恢复 GUI 状态；如果没有这段，resume 只能在进程内有效。
        ...
        self.sessions = {item["session_id"]: GuiSession(session_id=item["session_id"], messages=[GuiMessage(**message) for message in item.get("messages", []) if isinstance(message, dict)], last_turn_id=str(item.get("last_turn_id", ""))) for item in raw.get("sessions", []) if isinstance(item, dict) and "session_id" in item}  # 新增代码+DesktopGUILifecycle：恢复 session 和消息；如果没有这行，resume 不会带回对话历史。
    ...
    def sessions_payload(self) -> dict[str, Any]:  # 新增代码+DesktopGUISessionList：函数段开始，生成 GUI 侧栏会话列表；如果没有这段代码，左侧“最近对话”不会显示桌面 GUI 自己创建的会话。
        with self.lock:  # 新增代码+DesktopGUISessionList：读取 sessions 时加锁；如果没有这行代码，后台 worker 写消息时列表可能读到半状态。
            sessions = []  # 新增代码+DesktopGUISessionList：创建侧栏条目数组；如果没有这行代码，后续无法累计多个会话。
            for session in reversed(list(self.sessions.values())):  # 新增代码+DesktopGUISessionList：按最近创建顺序倒序遍历会话；如果没有这行代码，最新对话可能沉到底部。
                first_user_text = next((message.text for message in session.messages if message.role == "user" and message.text), session.session_id)  # 新增代码+DesktopGUISessionList：用第一条用户消息做标题；如果没有这行代码，侧栏只能显示难懂的 session id。
                last_turn = self.turns.get(session.last_turn_id) if session.last_turn_id else None  # 新增代码+DesktopGUISessionList：读取最近 turn；如果没有这行代码，侧栏副标题无法显示状态。
                sessions.append({"session_id": session.session_id, "title": first_user_text[:60], "subtitle": last_turn.status if last_turn else "", "last_turn_id": session.last_turn_id, "message_count": len(session.messages)})  # 新增代码+DesktopGUISessionList：追加规范化会话条目；如果没有这行代码，前端 normalizeSidebarSessions 没有真实数据可渲染。
            ...
            return {"ok": True, "sessions": sessions, "resume": {"latest_session_id": latest_session_id, "latest_turn_id": latest_turn_id}}  # 新增代码+DesktopGUISessionList：返回稳定 sessions payload；如果没有这行代码，GET /v1/gui/sessions 无法驱动侧栏。
    # 新增代码+DesktopGUISessionList：函数段结束，sessions_payload 到此结束；如果没有边界说明，初学者不易看出它只负责 GUI 会话列表。
    ...
    def _permission_payload(self, permission: GuiPermissionRequest) -> dict[str, Any]:  # 新增代码+DesktopGUIPermissions：函数段开始，生成脱敏权限事件 payload；如果没有这段，请求和回答事件字段会不一致。
        return {"request_id": permission.request_id, "turn_id": permission.turn_id, "run_id": permission.run_id, "session_id": permission.session_id, "app_name": permission.app_name, "reason": permission.reason, "risk_summary": permission.risk_summary, "status": permission.status, "decision": permission.decision}  # 新增代码+DesktopGUIPermissions：只返回安全摘要字段；如果没有这行，GUI 可能把原始敏感参数或页面隐藏文本暴露出来。
    # 新增代码+DesktopGUIPermissions：函数段结束，_permission_payload 到此结束；如果没有这个边界，初学者不易看出权限 payload 脱敏范围。

    def record_permission_required(self, request_id: str, turn_id: str = "", app_name: str = "", reason: str = "", risk_summary: str = "") -> dict[str, Any]:  # 新增代码+DesktopGUIPermissions：函数段开始，把后端权限请求登记为 GUI 可见请求；如果没有这段，权限事件无法变成桌面弹窗和 needs_permission 状态。
        ...
        with self.lock:  # 新增代码+DesktopGUIPermissions：保护权限请求登记；如果没有这行，并发权限事件可能重复写同一 request。
            ...
            permission = GuiPermissionRequest(request_id=clean_request_id, turn_id=clean_turn_id, run_id=turn.run_id if turn else "", session_id=turn.session_id if turn else "", app_name=app_name.strip() or "OpenHarness", reason=reason.strip(), risk_summary=risk_summary.strip())  # 新增代码+DesktopGUIPermissions：创建权限请求对象；如果没有这行，后续 decision endpoint 没有可查状态。
    ...
    def decide_permission(self, request_id: str, turn_id: str = "", decision: str = "", reason: str = "") -> dict[str, Any]:  # 新增代码+DesktopGUIPermissions：函数段开始，处理 GUI 允许或拒绝意图；如果没有这段，POST /permissions/{id}/decision 没有业务实现。
        clean_request_id = request_id.strip()  # 新增代码+DesktopGUIPermissions：清理 request id；如果没有这行，路径里的空格会造成查找异常。
        clean_decision = decision.strip().lower()  # 新增代码+DesktopGUIPermissions：标准化决策文本；如果没有这行，Approve/APPROVE 等常见大小写会被误拒。
        if clean_decision not in {"approve", "deny"}:  # 新增代码+DesktopGUIPermissions：只允许 approve/deny；如果没有这行，前端可写入任意权限状态。
        ...
        with self.lock:  # 新增代码+DesktopGUIPermissions：保护权限决策写入；如果没有这行，并发点击可能同时通过重复检查。
            ...
            permission.decision_reason = reason.strip()  # 新增代码+DesktopGUIPermissions：保存回答原因；如果没有这行，审计缺少 GUI 侧解释。
            ...
            payload = {**self._permission_payload(permission), "approved": clean_decision == "approve", "decision_reason": permission.decision_reason}  # 新增代码+DesktopGUIPermissions：生成权限回答审计 payload；如果没有这行，事件缺少 approve 布尔值和理由。
    ...
    def _run_turn_worker(self, turn_id: str) -> None:  # 新增代码+DesktopGUILifecycle：函数段开始，后台推进 turn；如果没有这段，queued 无法进入 running/completed。
        ...
        with self.lock:  # 新增代码+DesktopGUILifecycle：锁住最终状态写入；如果没有这行，cancel 和 completed 可能同时发生。
            ...
            try:  # 新增代码+DesktopGUILifecycle：捕获 runner 失败；如果没有这行，后台线程异常不会变成 GUI 事件。
                if "__fail__" in turn.prompt:  # 新增代码+DesktopGUILifecycle：提供测试失败触发器；如果没有这行，failed 状态难以稳定测试。
                    raise RuntimeError("测试触发的 GUI turn 失败。")  # 新增代码+DesktopGUILifecycle：抛出可控失败；如果没有这行，失败路径无法覆盖。
                answer = self.answer_runner(turn.prompt, turn.session_id, turn.turn_id)  # 新增代码+DesktopGUILifecycle：调用回答 runner；如果没有这行，turn 没有最终回答。
                self._set_turn_status(turn, "completed", answer=answer)  # 新增代码+DesktopGUILifecycle：写入完成终态；如果没有这行，GUI 无法展示最终答案。
...
class DesktopGuiBridgeServer(http.server.ThreadingHTTPServer):  # 修改代码+DesktopGUIBridge：类段开始，保存 GUI bridge 共享状态；如果没有这个类，handler 无法访问 workspace/token/manager。
    def __init__(self, server_address: tuple[str, int], workspace: str | Path, token: str = "", answer_runner: GuiAnswerRunner | None = None) -> None:  # 修改代码+DesktopGUIBridge：初始化 server；如果没有这段，外部无法注入工作区和 runner。
        ...
        self.run_manager = GuiRunManager(self.workspace, answer_runner=answer_runner)  # 新增代码+DesktopGUILifecycle：创建生命周期 manager；如果没有这行，POST 生命周期端点没有业务实现。
...
class DesktopGuiBridgeHandler(http.server.BaseHTTPRequestHandler):  # 修改代码+DesktopGUIBridge：类段开始，处理 GUI bridge HTTP 请求；如果没有这个类，Electron 前端没有后端入口。
    ...
    def _send_error(self, status: int, code: str, message: str) -> None:  # 修改代码+DesktopGUIBridge：函数段开始，统一返回结构化错误；如果没有这段，前端要解析不同形状失败。
        self._send_json(status, {"ok": False, "code": code, "error": message})  # 修改代码+DesktopGUIBridge：返回不含本机路径的错误对象；如果没有这行，错误可能泄露文件系统细节。
    ...
    def _authorized(self) -> bool:  # 修改代码+DesktopGUIBridge：函数段开始，检查 GUI bridge token；如果没有这段，本机其他进程可直接读状态。
        return self.headers.get(GUI_TOKEN_HEADER, "") == self.server.token  # 修改代码+DesktopGUIBridge：比对启动 token；如果没有这行，认证字段不会生效。
...
def create_gui_bridge_server(workspace: str | Path, host: str = "127.0.0.1", port: int = 8776, token: str = "", answer_runner: GuiAnswerRunner | None = None) -> DesktopGuiBridgeServer:  # 修改代码+DesktopGUIBridge：函数段开始，创建 GUI bridge server；如果没有这段，CLI 和测试会重复构造逻辑。
    return DesktopGuiBridgeServer((host, port), workspace=workspace, token=token, answer_runner=answer_runner)  # 修改代码+DesktopGUIBridge：返回绑定 server；如果没有这行，调用方拿不到可 serve_forever 的对象。
