...
export function latestPermissionEvent(events: StatusEvent[]): StatusEvent | null { // 新增代码+DesktopGUIPermissions：函数段开始，查找最新权限相关事件；如果没有这段，AppShell 需要手写事件过滤逻辑。
  const permissionEventTypes = new Set(["permission_required", "permission_answered", "computer_use_permission_required", "gui_turn_needs_permission"]); // 新增代码+DesktopGUIPermissions：集中列出权限事件类型；如果没有这行，新增权限事件容易漏到 UI 外。
