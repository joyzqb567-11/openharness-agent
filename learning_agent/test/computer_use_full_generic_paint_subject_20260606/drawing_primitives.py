...
__all__ = ["DRAWING_PRIMITIVES_MARKER", "DRAWING_PRIMITIVES_MODEL", "DRAWING_PRIMITIVES_OK_TOKEN", "build_elephant_drag_plan", "build_pikachu_drag_plan", "expand_drag_path_to_low_level_events"]  # 修改代码+GenericPaintSubject：把大象计划加入公开 API；如果没有这一行，Paint 真实闭环无法按用户 prompt 调用大象计划。
