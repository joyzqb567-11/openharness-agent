...
try:  # 新增代码+PaintPikachuRealLoop：优先按包路径导入项目已有 Computer Use 组件；如果没有这一段，单元测试和真实 bat 入口无法共享实现。
    from learning_agent.computer_use.drawing_primitives import build_elephant_drag_plan, build_pikachu_drag_plan, expand_drag_path_to_low_level_events  # 修改代码+GenericPaintSubject：同时复用皮卡丘和大象拖拽 primitive；如果没有这一行，大象 prompt 会继续误走皮卡丘硬编码。
    ...
    from computer_use.drawing_primitives import build_elephant_drag_plan, build_pikachu_drag_plan, expand_drag_path_to_low_level_events  # type: ignore  # 修改代码+GenericPaintSubject：脚本模式同时复用皮卡丘和大象 primitive；如果没有这一行，bat 入口下大象仍会画成皮卡丘。
...
PAINT_SUPPORTED_DRAWING_SUBJECTS = ["pikachu", "elephant"]  # 新增代码+GenericPaintSubject：声明当前真实 Paint loop 明确支持的绘画对象；如果没有这一行，未知对象会被误以为也已经成熟支持。
...
class WindowsPaintPikachuRealExecutionLoop:  # 修改代码+GenericPaintSubject：类段开始，封装 full 模式 Paint 已支持对象真实执行流程；如果没有这个类，desktop_task_runtime 仍没有可注入真实闭环。
    ...
    def run_desktop_task(self, target_app: str, prompt: str) -> dict[str, Any]:  # 修改代码+GenericPaintSubject：函数段开始，执行 Paint 已支持对象真实桌面任务；如果没有这段函数，desktop_task_runtime 无法调用真实闭环。
        ...
        if subject not in PAINT_SUPPORTED_DRAWING_SUBJECTS:  # 新增代码+GenericPaintSubject：未知对象必须明确拒绝；如果没有这一行，agent 会再次假装支持任意绘画对象。
            return self._failure("paint_drawing_subject_not_supported", "supported_subjects=pikachu,elephant", {"drawing_subject": subject, "supported_drawing_subjects": list(PAINT_SUPPORTED_DRAWING_SUBJECTS), "prompt_length": len(str(prompt or ""))})  # 新增代码+GenericPaintSubject：返回对象不支持报告；如果没有这一行，用户不知道为什么没有打开 Paint 画图。
