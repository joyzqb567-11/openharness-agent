            ...
            for model_event in stream:  # 新增代码+DirectSseAdapter: 逐个消费模型事件；如果没有这行代码，GUI 收不到增量或完成。
                if model_event.event_type == "text_delta":  # 新增代码+DirectSseAdapter: 处理文本增量；如果没有这行代码，流式文字不会显示。
                    if not first_delta_seen:  # 新增代码+DirectSseAdapter: 首个 delta 到达时记录延迟；如果没有这行代码，诊断无法计算首包。
                        ...
                        first_delta_latency_ms = int((time.monotonic() - started_at) * 1000)  # 新增代码+DirectSseLatencyBudget：计算首个可见 delta 延迟；如果没有这行，预算判断没有输入。
                        route_payload["first_delta_latency_ms"] = first_delta_latency_ms  # 修改代码+DirectSseLatencyBudget：写入首包延迟毫秒；如果没有这行代码，性能面板没有依据。
                        route_payload["first_delta_budget_exceeded"] = first_delta_latency_ms > int(DIRECT_SSE_FIRST_DELTA_BUDGET_SECONDS * 1000)  # 新增代码+DirectSseLatencyBudget：标记首包是否超过 5 秒预算；如果没有这行，慢回复问题不会进入可见诊断。
