...
try:  # 新增代码+CompatSlimming：优先按包路径导入统一 Windows app resolver；如果没有这一段，Phase109 候选迁入后无法生成发现报告。
    from learning_agent.computer_use.windows_launch_resolver import build_launch_plan, resolve_windows_app_launch_target  # 新增代码+CompatSlimming：复用唯一应用发现和启动计划入口；如果没有这一行，generic_launch_backend 会重新分叉发现逻辑。
except ModuleNotFoundError as error:  # 新增代码+CompatSlimming：兼容 start_oauth_agent.bat 脚本式导入；如果没有这一段，真实可见终端可能因包名前缀不同失败。
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.windows_launch_resolver"}:  # 新增代码+CompatSlimming：只兜底包路径缺失；如果没有这一行，resolver 内部真实 bug 会被误吞。
        raise  # 新增代码+CompatSlimming：重新抛出真实内部错误；如果没有这一行，排查启动候选失败会很困难。
    from computer_use.windows_launch_resolver import build_launch_plan, resolve_windows_app_launch_target  # type: ignore  # 新增代码+CompatSlimming：脚本模式导入同一 resolver；如果没有这一行，bat 入口无法复用瘦身后的发现层。
