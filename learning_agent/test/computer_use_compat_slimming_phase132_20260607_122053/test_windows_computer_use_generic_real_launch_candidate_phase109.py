...
class WindowsComputerUseGenericRealLaunchCandidatePhase109Tests(unittest.TestCase):  # 新增代码+Phase109GenericRealLaunchCandidate：类段开始，集中验收 Phase109 通用真实启动候选；如果没有这个类，通用启动候选只会停在设计说明没有回归保护。
    def _phase109_module(self):  # 新增代码+Phase109GenericRealLaunchCandidate：函数段开始，动态读取 Phase109 模块；如果没有这段函数，模块缺失时测试会报难懂的导入异常。
        try:  # 新增代码+Phase109GenericRealLaunchCandidate：尝试加载即将实现的新模块；如果没有这行代码，红测不能表达“模块必须存在”的合同。
            return importlib.import_module("learning_agent.computer_use.generic_launch_backend")  # 新增代码+Phase109GenericRealLaunchCandidate：返回 Phase109 通用候选模块；如果没有这行代码，后续断言拿不到被测 API。
    ...
    def test_candidate_default_off_uses_phase108_without_per_app_patch(self) -> None:  # 新增代码+Phase109GenericRealLaunchCandidate：函数段开始，验证默认关闭候选复用 Phase108 且不需要逐应用补丁；如果没有这段测试，项目可能又退回每个 app 手写白名单。
        ...
        self.assertTrue(report["uses_phase108_generic_discovery"])  # 修改代码+CompatSlimming：断言旧 Phase109 合同仍兼容读取统一 resolver 发现结果；如果没有这行代码，后续实现可能绕回硬编码应用名。
    ...
    def test_interactive_launch_prints_phase109_default_off_candidate_after_full_mode(self) -> None:  # 新增代码+Phase109GenericRealLaunchCandidate：函数段开始，验证真实 `/computer launch` 会输出 Phase109 默认关闭证据；如果没有这段测试，模块合同可能没有接到用户命令入口。
        ...
        self.assertIn("WINDOWS_APP_LAUNCH_TARGET_READY", output)  # 修改代码+CompatSlimming：断言用户命令先输出统一 resolver marker；如果没有这行代码，可见终端验收会误以为还在走旧候选文件。
        self.assertIn("WINDOWS_APP_LAUNCH_TARGET_OK", output)  # 修改代码+CompatSlimming：断言统一 resolver 成功接收普通目标；如果没有这行代码，失败路径可能被误读。
        self.assertIn("interactive_target_resolved=true", output)  # 修改代码+CompatSlimming：断言交互入口已接入统一目标解析；如果没有这行代码，用户关心的通用性没有被证明。
        self.assertIn("safe_known_ordinary_app=true", output)  # 修改代码+CompatSlimming：断言普通应用被识别为安全目标；如果没有这行代码，设计可能回到逐应用授权。
        self.assertIn("real_launch_supported=false", output)  # 修改代码+CompatSlimming：断言当前普通目标默认不真实启动；如果没有这行代码，测试可能误导用户以为已经打开应用。
