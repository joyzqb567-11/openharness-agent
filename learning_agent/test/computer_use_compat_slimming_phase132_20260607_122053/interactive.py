...
try:  # 新增代码+AppSplit: 优先按包路径导入轮次格式化和验收事件 helper；若没有这行代码，包运行模式下交互层无法复用通用能力。
    ...
    if error.name not in {"learning_agent", "learning_agent.app", "learning_agent.app.chrome_status_renderer", "learning_agent.app.computer_status_renderer", "learning_agent.app.process_summary", "learning_agent.app.status_renderer", "learning_agent.browser_extension_host", "learning_agent.browser_extension_host.bridge_server", "learning_agent.browser_extension_host.manifest_installer", "learning_agent.computer_use", "learning_agent.computer_use.approval", "learning_agent.computer_use.abort_streaming_hooks", "learning_agent.computer_use.high_level_tools", "learning_agent.computer_use.controller_takeover", "learning_agent.computer_use.controller", "learning_agent.computer_use.lock", "learning_agent.computer_use.mode_session", "learning_agent.computer_use.native_diagnostics", "learning_agent.computer_use.persistent_grants", "learning_agent.computer_use.security_policy", "learning_agent.computer_use.session_context", "learning_agent.computer_use.session_runtime", "learning_agent.computer_use.terminal_grants", "learning_agent.core", "learning_agent.core.config", "learning_agent.observability", "learning_agent.observability.acceptance_events", "learning_agent.observability.run_records", "learning_agent.runtime", "learning_agent.runtime.command_queue", "learning_agent.runtime.status_snapshot", "learning_agent.sdk", "learning_agent.sdk.status"}:  # 修改代码+ProcessSummaryUX: 允许 process_summary 在脚本模式 fallback；若没有这行代码，start_oauth_agent.bat 下过程摘要导入会被误判为真实错误。
...
def _phase106_live_execution_classes() -> tuple[Any, Any]:  # 新增代码+Phase106InteractiveFullLaunch：函数段开始，延迟导入 Phase105/Universal gate；如果没有这段函数，脚本模式和包模式会重复导入分支。
    try:  # 新增代码+Phase106InteractiveFullLaunch：优先按 learning_agent 包路径导入；如果没有这行代码，项目根运行时无法加载标准模块。
        from learning_agent.computer_use.universal_live_execution import Phase105ControlledNotepadSmokeLaunchCandidate, UniversalWindowsLiveExecutionGate  # 新增代码+Phase106InteractiveFullLaunch：导入 Phase105 受控 Notepad bridge 和 Universal gate；如果没有这行代码，交互命令无法接到 Phase105。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.universal_live_execution"}:  # 新增代码+Phase106InteractiveFullLaunch：只兜底包路径缺失；如果没有这行代码，Universal gate 内部 bug 会被误吞。
            raise  # 新增代码+Phase106InteractiveFullLaunch：重新抛出真实内部错误；如果没有这行代码，排查 Phase106 失败会被错误 fallback 干扰。
        from computer_use.universal_live_execution import Phase105ControlledNotepadSmokeLaunchCandidate, UniversalWindowsLiveExecutionGate  # type: ignore  # 新增代码+Phase106InteractiveFullLaunch：脚本模式导入同一能力；如果没有这行代码，双击 bat 后 `/computer launch notepad` 不可用。
...
def _phase107_launch_target_tools() -> tuple[Any, Any]:  # 新增代码+Phase107InteractiveLaunchTarget：函数段开始，延迟导入 Phase107 目标解析器；如果没有这段函数，脚本模式下 `/computer launch` 可能找不到新模块。
    try:  # 新增代码+Phase107InteractiveLaunchTarget：优先按 learning_agent 包路径导入；如果没有这行代码，项目根运行时无法加载标准模块。
        from learning_agent.computer_use.windows_launch_resolver import phase107_cli_line, resolve_interactive_launch_target  # 修改代码+CompatSlimming：改为从统一 resolver 导入交互目标解析；如果没有这一行，删除旧 interactive_launch_target.py 后交互命令会断。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.windows_launch_resolver"}:  # 修改代码+CompatSlimming：只兜底 resolver 包路径缺失；如果没有这一行，resolver 内部 bug 会被误吞。
            raise  # 新增代码+Phase107InteractiveLaunchTarget：重新抛出真实内部错误；如果没有这行代码，排查 Phase107 会被 fallback 干扰。
        from computer_use.windows_launch_resolver import phase107_cli_line, resolve_interactive_launch_target  # type: ignore  # 修改代码+CompatSlimming：脚本模式从统一 resolver 导入交互解析；如果没有这一行，双击 bat 后新命令不可用。
...
def _phase108_generic_app_tools() -> tuple[Any, Any]:  # 新增代码+Phase108GenericAppDiscovery：函数段开始，延迟导入 Phase108 通用应用发现工具；如果没有这段函数，脚本模式下 `/computer launch obsidian` 这类普通未知应用无法走通用发现。
    try:  # 新增代码+Phase108GenericAppDiscovery：优先按 learning_agent 包路径导入；如果没有这行代码，项目根运行时无法加载标准模块。
        from learning_agent.computer_use.windows_launch_resolver import phase108_cli_line, resolve_generic_app_launch_target  # 修改代码+CompatSlimming：改为从统一 resolver 导入通用应用解析；如果没有这一行，删除旧 generic_app_discovery.py 后交互命令会断。
        ...
        from computer_use.windows_launch_resolver import phase108_cli_line, resolve_generic_app_launch_target  # type: ignore  # 修改代码+CompatSlimming：脚本模式从统一 resolver 导入通用应用解析；如果没有这一行，双击 bat 后通用发现不可用。
...
def _phase109_generic_real_launch_tools() -> tuple[Any, Any]:  # 新增代码+Phase109GenericRealLaunchCandidate：函数段开始，延迟导入 Phase109 通用真实启动候选工具；如果没有这段函数，脚本模式下 `/computer launch obsidian` 无法显示通用真实启动候选证据。
    try:  # 新增代码+Phase109GenericRealLaunchCandidate：优先按 learning_agent 包路径导入；如果没有这行代码，项目根运行时无法加载标准模块。
        from learning_agent.computer_use.generic_launch_backend import phase109_cli_line, prepare_phase109_generic_real_launch_candidate  # 修改代码+CompatSlimming：改为从通用启动后端导入真实启动候选；如果没有这一行，删除旧 generic_real_launch_candidate.py 后交互命令会断。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.generic_launch_backend"}:  # 修改代码+CompatSlimming：只兜底通用启动后端包路径缺失；如果没有这一行，后端内部 bug 会被误吞。
            raise  # 新增代码+Phase109GenericRealLaunchCandidate：重新抛出真实内部错误；如果没有这行代码，排查通用候选会被 fallback 干扰。
        from computer_use.generic_launch_backend import phase109_cli_line, prepare_phase109_generic_real_launch_candidate  # type: ignore  # 修改代码+CompatSlimming：脚本模式从通用启动后端导入候选工具；如果没有这一行，双击 bat 后通用候选不可用。
...
def _phase113_interactive_generic_launch_tools() -> tuple[Any, Any]:  # 新增代码+InteractiveGenericLaunchMaturity：函数段开始，延迟导入 Phase113 交互通用启动桥；如果没有这段函数，脚本模式下 `/computer launch obsidian` 无法显示通用后端接线证据。
    try:  # 新增代码+InteractiveGenericLaunchMaturity：优先按 learning_agent 包路径导入；如果没有这一行，项目根运行时无法加载标准模块。
        from learning_agent.computer_use.universal_live_execution import phase113_cli_line, run_phase113_interactive_generic_launch_bridge  # 新增代码+InteractiveGenericLaunchMaturity：导入 Phase113 桥和 token 格式化器；如果没有这一行，交互命令无法接到 Phase110 通用后端。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.universal_live_execution"}:  # 新增代码+InteractiveGenericLaunchMaturity：只兜底包路径缺失；如果没有这一行，Phase113 内部 bug 会被误吞。
            raise  # 新增代码+InteractiveGenericLaunchMaturity：重新抛出真实内部错误；如果没有这一行，排查通用后端接线会被 fallback 干扰。
        from computer_use.universal_live_execution import phase113_cli_line, run_phase113_interactive_generic_launch_bridge  # type: ignore  # 新增代码+InteractiveGenericLaunchMaturity：脚本模式导入同一桥；如果没有这一行，双击 bat 后 Phase113 不可用。
...
def _phase115_full_maturity_matrix_tools() -> tuple[Any, Any]:  # 新增代码+FullMaturityMatrix：函数段开始，延迟导入最终成熟矩阵工具；如果没有这段函数，脚本模式下 `/computer maturity` 可能找不到新模块。
    try:  # 新增代码+FullMaturityMatrix：优先按 learning_agent 包路径导入；如果没有这一行，项目根运行时无法加载标准模块。
        from learning_agent.computer_use.full_maturity_matrix import computer_use_full_maturity_cli_line, run_computer_use_full_maturity_matrix  # 新增代码+FullMaturityMatrix：导入矩阵生成函数和 token 格式化器；如果没有这一行，交互命令无法输出最终成熟矩阵。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.full_maturity_matrix"}:  # 新增代码+FullMaturityMatrix：只兜底包路径缺失；如果没有这一行，矩阵内部 bug 会被误吞。
            raise  # 新增代码+FullMaturityMatrix：重新抛出真实内部错误；如果没有这一行，排查 maturity 命令会被 fallback 干扰。
        from computer_use.full_maturity_matrix import computer_use_full_maturity_cli_line, run_computer_use_full_maturity_matrix  # type: ignore  # 新增代码+FullMaturityMatrix：脚本模式导入同一矩阵工具；如果没有这一行，双击 bat 后 `/computer maturity` 不可用。
...
def run_computer_terminal_command(workspace: Path, user_input: str) -> str:  # 新增代码+Phase31ComputerUseLockAbortEvidence: 函数段开始，执行 `/computer` 子命令；如果没有这段函数，真实用户无法在终端查看或触发桌面急停。
    ...
    if subcommand == "stop":  # 新增代码+Phase98UniversalComputerUseMode: 识别 `/computer stop` 模式停止命令；如果没有这行代码，用户无法用新模式入口终止后续动作。
        ...
        stop_result = {**dict(stop_result), "owned_resource_cleanup_completed": bool(abort_result.get("owned_resource_cleanup_completed", False)), "residual_owned_process": bool(abort_result.get("residual_owned_process", False))}  # 新增代码+InteractiveGenericLaunchMaturity：把 runtime owned resource 清理结果合并进 stop 面板；如果没有这一行，用户看不到 stop 是否完成自有资源收尾。
...
def _reset_computer_use_session_context_for_agent(agent: Any) -> None:  # 新增代码+ComputerUseSessionTargetGuard：函数段开始，清理上一轮 Computer Use 会话残留上下文；如果没有这段函数，旧窗口截图和旧桌面动作会继续进入新任务模型主循环。
    ...
    recorder = getattr(agent, "_record_observation", None)  # 新增代码+ComputerUseSessionTargetGuard：读取观察记录接口用于留下清理证据；如果没有这一行，真实终端排查不知道新会话是否做过隔离。
    if callable(recorder):  # 新增代码+ComputerUseSessionTargetGuard：只有 agent 支持记录时才写审计事件；如果没有这一行，轻量测试替身可能因为缺少方法而崩溃。
        recorder("computer_use_session_context_reset", {"old_computer_use_events_cleared": True, "old_computer_use_artifacts_cleared": True})  # 新增代码+ComputerUseSessionTargetGuard：记录本次 full 会话已清理旧桌面上下文；如果没有这一行，后续复盘无法确认隔离步骤发生过。
...
def _activate_computer_use_tool_pack_for_agent(agent: Any, user_input: str, computer_output: str) -> str:  # 新增代码+ModelLoopComputerUse：函数段开始，把终端 `/computer use` 模式和模型主循环工具池接上；如果没有这段函数，用户打开 full 模式后模型仍看不到 computer_use 工具。
    ...
    select_pack = getattr(agent, "_tool_search_select_pack", None)  # 新增代码+ModelLoopComputerUse：读取 agent 现有能力包加载接口；如果没有这一行，交互层只能硬编码工具名，后续工具架构变化会再次错接。
    if not callable(select_pack):  # 新增代码+ModelLoopComputerUse：防御测试替身或外部 agent 没有该接口；如果没有这一行，轻量 agent 会在 `/computer use` 命令后崩溃。
        return "Computer Use Tool Pack\n- model_loop_tools_loaded=false\n- reason=agent_missing_select_pack\n"  # 新增代码+ModelLoopComputerUse：返回可见失败原因；如果没有这一行，用户不知道 full 模式为何没有进入模型工具面。
    selection_result = str(select_pack("computer_use"))  # 新增代码+ModelLoopComputerUse：通过正式能力包接口加载 computer_use 工具；如果没有这一行，模型下一轮仍看不到 observe/action/use 这些通用桌面工具 schema。
    recorder = getattr(agent, "_record_observation", None)  # 新增代码+ModelLoopComputerUse：读取 observation 记录接口；如果没有这一行，本次模式到工具面的接线缺少审计证据。
    if callable(recorder):  # 新增代码+ModelLoopComputerUse：只有接口存在时才记录观察事件；如果没有这一行，测试替身可能因为缺少私有方法而失败。
        recorder("computer_use_tool_pack_loaded_from_terminal_command", {"command": normalized_command, "selection_result": selection_result[:1200]})  # 新增代码+ModelLoopComputerUse：记录命令和加载结果摘要；如果没有这一行，后续排查无法证明 `/computer use --full` 曾经扩展模型工具池。
...
def run_interactive_session(agent: Any, workspace: Path, visible_tools: list[str], max_turns: int | None, prompt_soft_token_limit: int) -> None:  # 新增代码+AppSplit: 运行用户可见交互式终端循环；若没有这行代码，main 仍要承载所有交互逻辑。
    ...
    while True:  # 新增代码+AppSplit: 开始命令行交互循环；若没有这行代码，agent 只能回答一次或直接退出。
        ...
        answer = agent.run(user_input, max_turns=max_turns, event_callback=process_summary_renderer.handle_event)  # 修改代码+ProcessSummaryUX: 调用 agent 主循环时订阅事件摘要；若没有这行代码，run_events 已存在但真实终端不会显示 Codex 风格过程提示。
