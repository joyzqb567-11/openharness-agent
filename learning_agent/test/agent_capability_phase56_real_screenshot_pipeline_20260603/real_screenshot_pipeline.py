...
try:  # 新增代码+Phase56RealScreenshotPipeline: 优先按包模式导入既有 Computer Use 组件；如果没有这行代码，unittest 和生产入口不能复用项目模块。
    from learning_agent.computer_use.evidence import ComputerUseEvidenceStore  # 新增代码+Phase56RealScreenshotPipeline: 导入 evidence store；如果没有这行代码，截图无法落盘并生成 image_result。
    from learning_agent.computer_use.helper_client import WindowObservationPayload  # 新增代码+Phase56RealScreenshotPipeline: 导入观察 payload；如果没有这行代码，pipeline 无法复用 Phase29/41 保存合同。
    from learning_agent.computer_use.native_helper import NativeWindowCaptureResult, Win32GdiWindowCaptureProvider, parse_hwnd_from_window  # 新增代码+Phase56RealScreenshotPipeline: 导入截图结果、GDI provider 和 hwnd 解析；如果没有这行代码，真实 Windows 截图链路无法工作。
    from learning_agent.computer_use.wgc_capture import WindowsGraphicsCaptureProvider  # 新增代码+Phase56RealScreenshotPipeline: 导入 WGC provider 合同；如果没有这行代码，Phase56 不能保持 WGC 优先策略。
    from learning_agent.computer_use.windows_backend import WindowsWindowInventoryProbe  # 新增代码+Phase56RealScreenshotPipeline: 导入真实窗口 inventory；如果没有这行代码，safe window smoke 无法定位自有 Notepad。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.evidence", "learning_agent.computer_use.helper_client", "learning_agent.computer_use.native_helper", "learning_agent.computer_use.wgc_capture", "learning_agent.computer_use.windows_backend"}:  # 新增代码+Phase56RealScreenshotPipeline: 只允许包路径缺失时 fallback；如果没有这行代码，真实内部 bug 会被误吞。
        raise  # 新增代码+Phase56RealScreenshotPipeline: 重新抛出非路径错误；如果没有这行代码，排查真实依赖问题会困难。
    from computer_use.evidence import ComputerUseEvidenceStore  # 新增代码+Phase56RealScreenshotPipeline: 脚本模式导入 evidence store；如果没有这行代码，bat 入口无法保存截图。
    from computer_use.helper_client import WindowObservationPayload  # 新增代码+Phase56RealScreenshotPipeline: 脚本模式导入 observation payload；如果没有这行代码，bat 入口无法构造证据输入。
    from computer_use.native_helper import NativeWindowCaptureResult, Win32GdiWindowCaptureProvider, parse_hwnd_from_window  # 新增代码+Phase56RealScreenshotPipeline: 脚本模式导入 GDI provider 和 hwnd 解析；如果没有这行代码，bat 入口无法截图。
    from computer_use.wgc_capture import WindowsGraphicsCaptureProvider  # 新增代码+Phase56RealScreenshotPipeline: 脚本模式导入 WGC provider；如果没有这行代码，bat 入口无法报告 WGC 优先链。
    from computer_use.windows_backend import WindowsWindowInventoryProbe  # 新增代码+Phase56RealScreenshotPipeline: 脚本模式导入窗口 inventory；如果没有这行代码，bat 入口无法定位安全窗口。
...
def run_phase56_real_screenshot_pipeline_contract(real_smoke: bool = True) -> dict[str, Any]:  # 新增代码+Phase56RealScreenshotPipeline: 函数段开始，执行 Phase56 合同自检；如果没有这段函数，CLI 和真实终端没有统一入口。
    ...
    with tempfile.TemporaryDirectory() as raw_dir:  # 新增代码+Phase56RealScreenshotPipeline: 创建临时 evidence 目录隔离 fake 合同；如果没有这行代码，自检会污染真实 evidence。
        ...
        from learning_agent.computer_use.native_helper_v2 import WindowsNativeHelperV2Worker  # 新增代码+Phase56RealScreenshotPipeline: 延迟导入 helper v2 避免模块循环；如果没有这行代码，合同无法证明 helper v2 capture 接入。
