...
class GuiProviderInfo:  # 新增代码+ProviderSettings：类段开始，描述 Provider 设置页一行数据；如果没有这个类，前端契约字段容易漂移。
    ...
    account_label: str = ""  # 新增代码+ComposerRouteControls：保存脱敏账号提示；如果没有这行，真实 OAuth 连接后用户不知道当前连接哪个 ChatGPT 账号。
    direct_route_status: str = ""  # 新增代码+ComposerRouteControls：保存 Direct SSE 路由状态；如果没有这行，设置页无法说明当前是否绕开 Codex CLI WebSocket。
    oauth_client_source: str = ""  # 新增代码+ComposerRouteControls：保存 OAuth client 来源；如果没有这行，真实 OAuth 调试无法区分操作者配置和 OpenCode 观察参考。
...
def load_provider_settings(workspace: str | Path) -> dict[str, Any]:  # 新增代码+ProviderSettings：函数段开始，安全读取 provider 设置；如果没有这段，catalog 无法合并用户配置。
    ...
    if not settings_path.exists():  # 新增代码+ProviderSettings：处理首次使用无配置；如果没有这行，首次打开设置页会报错。
        return {"auth": {}, "custom_providers": {}, "model_visibility": {}, "default_provider_id": "", "default_model_id": "", "selected_openai_account_id": "", "selected_openai_account_label": "", "selected_openai_model_id": ""}  # 修改代码+OpenAIDisconnect：返回包含 OpenAI 选择状态的空配置骨架；如果没有这行，disconnect 无法稳定清理选择字段。
    try:  # 新增代码+ProviderSettings：保护 JSON 解析；如果没有这行，坏文件会让 bridge 500。
        ...
        return {"auth": {}, "custom_providers": {}, "model_visibility": {}, "default_provider_id": "", "default_model_id": "", "selected_openai_account_id": "", "selected_openai_account_label": "", "selected_openai_model_id": ""}  # 修改代码+OpenAIDisconnect：损坏后使用含选择字段的空配置继续；如果没有这行，后续保存可能缺少稳定字段。
    if not isinstance(raw, dict):  # 新增代码+ProviderSettings：防御非对象 JSON；如果没有这行，数组或字符串会污染配置。
        return {"auth": {}, "custom_providers": {}, "model_visibility": {}, "default_provider_id": "", "default_model_id": "", "selected_openai_account_id": "", "selected_openai_account_label": "", "selected_openai_model_id": ""}  # 修改代码+OpenAIDisconnect：非对象视为含选择字段的空配置；如果没有这行，后续 get() 可能失败。
    return {"auth": raw.get("auth", {}) if isinstance(raw.get("auth", {}), dict) else {}, "custom_providers": raw.get("custom_providers", {}) if isinstance(raw.get("custom_providers", {}), dict) else {}, "model_visibility": raw.get("model_visibility", {}) if isinstance(raw.get("model_visibility", {}), dict) else {}, "default_provider_id": str(raw.get("default_provider_id", "")), "default_model_id": str(raw.get("default_model_id", "")), "selected_openai_account_id": str(raw.get("selected_openai_account_id", "")), "selected_openai_account_label": str(raw.get("selected_openai_account_label", "")), "selected_openai_model_id": str(raw.get("selected_openai_model_id", ""))}  # 修改代码+OpenAIDisconnect：只保留已知配置区块和 OpenAI 选择字段；如果没有这行，disconnect 会丢失或清不掉选择状态。
...
def _built_in_provider_catalog(workspace: str | Path, settings: dict[str, Any], secret_store: GuiProviderSecretStore) -> list[dict[str, Any]]:  # 修改代码+OpenAIModelRegistry：函数段开始，构造内置 provider catalog；如果没有这段，GET 路由没有核心数据。
    ...
    built_ins = [  # 新增代码+ProviderSettings：集中声明内置 provider 元数据；如果没有这段，display name 和模型会散落。
        ...
        ("openai", "OpenAI", "使用 ChatGPT Pro/Plus 或 API 密钥连接", _openai_auth_methods(), build_openai_model_catalog_rows(workspace, settings)),  # 修改代码+OpenAIModelRegistry：声明 OpenAI provider 三种连接方式和注册表模型；如果没有这行，模型菜单无法反映 probe/LKG 状态。
    ...
    for provider_id, display_name, description, auth_methods, models in built_ins:  # 新增代码+ProviderSettings：遍历内置 provider；如果没有这行，元数据不会变成 payload。
        ...
        if auth_type == "oauth_mock":  # 新增代码+OpenAIAuthAttempt：处理稳定 V1 的 mock OAuth 完成状态；如果没有这行，mock complete 后 catalog 仍显示未连接。
            providers.append(GuiProviderInfo(id=provider_id, display_name=display_name, kind="built_in", source="mock", connected=True, masked_key="Mock ChatGPT auth", auth_methods=auth_methods, description=description, models=models, account_label="Mock ChatGPT", direct_route_status="mock_only", oauth_client_source="mock"))  # 修改代码+ComposerRouteControls：加入 mock provider 并显式标记只可界面验收；如果没有这行，用户会误把 mock OAuth 当成真实 Direct SSE。
            continue  # 新增代码+OpenAIAuthAttempt：mock auth 不再读取 secret_ref；如果没有这行，后续 secret_store 逻辑会把 mock 状态当未连接。
        if auth_type == "oauth_real":  # 新增代码+OpenAIRealOAuthAttempt：处理真实 OAuth secret_refs 连接状态；如果没有这行，真实 OAuth 完成后 catalog 仍会显示未连接。
            grouped_refs = auth_record.get("secret_refs", {}) if isinstance(auth_record.get("secret_refs", {}), dict) else {}  # 新增代码+OpenAIRealOAuthAttempt：读取 OAuth token 引用组；如果没有这行，无法定位 access token。
            access_ref = str(grouped_refs.get("access_token", ""))  # 新增代码+OpenAIRealOAuthAttempt：读取 access token 引用；如果没有这行，连接状态无法判断。
            connected = bool(access_ref and secret_store.get_secret(access_ref))  # 新增代码+OpenAIRealOAuthAttempt：确认加密 store 中仍有 access token 才算连接；如果没有这行，孤儿 secret_refs 会误显示已连接。
            source = "oauth" if connected else "none"  # 新增代码+OpenAIRealOAuthAttempt：设置真实 OAuth 来源 badge；如果没有这行，UI 无法区分 API key 和 OAuth。
            account_label = str(auth_record.get("account_label", settings.get("selected_openai_account_label", ""))) if connected else ""  # 新增代码+ComposerRouteControls：读取已脱敏账号标签；如果没有这行，设置页无法提示当前 OAuth 账号。
            client_source = str(auth_record.get("oauth_client_source", "")) if connected else ""  # 新增代码+ComposerRouteControls：读取 OAuth client 来源；如果没有这行，真实 OAuth 来源诊断会丢失。
            account_status = str(auth_record.get("account_selection_status", ""))  # 新增代码+ComposerRouteControls：读取账号选择状态；如果没有这行，多账号阻断无法进入可见 UI。
            direct_route_status = "account_selection_required" if account_status == "account_selection_required" else ("direct_sse_ready" if connected else "disconnected")  # 新增代码+ComposerRouteControls：把真实 OAuth 状态映射为 Direct SSE 路由状态；如果没有这行，UI 无法区分可直连和账号待选。
            masked_key = f"ChatGPT OAuth ({account_label})" if connected and account_label else ("ChatGPT OAuth" if connected else "")  # 修改代码+ComposerRouteControls：返回不含 token 的 OAuth 摘要；如果没有这行，用户不知道连接方式但不能暴露密钥。
            providers.append(GuiProviderInfo(id=provider_id, display_name=display_name, kind="built_in", source=source, connected=connected, masked_key=masked_key, auth_methods=auth_methods, description=description, models=models, account_label=account_label, direct_route_status=direct_route_status, oauth_client_source=client_source))  # 修改代码+ComposerRouteControls：加入真实 OAuth provider payload 和低敏路由事实；如果没有这行，设置页无法刷新 Direct SSE 连接状态。
            continue  # 新增代码+OpenAIRealOAuthAttempt：真实 OAuth 不再读取旧 secret_ref；如果没有这行，后续旧逻辑会覆盖 connected 状态。
        ...
        providers.append(GuiProviderInfo(id=provider_id, display_name=display_name, kind="built_in", source=source, connected=connected, masked_key=masked_key, auth_methods=auth_methods, description=description, models=models, direct_route_status="api_key_route" if connected else ""))  # 修改代码+ComposerRouteControls：加入 provider payload 并标记 API key 路径；如果没有这行，设置页无法区分 API key 和 Direct SSE。
...
def _custom_provider_catalog(settings: dict[str, Any], secret_store: GuiProviderSecretStore) -> list[dict[str, Any]]:  # 修改代码+SecretStoreFactory：函数段开始，构造自定义 provider catalog；如果没有这段，保存的 OpenAI-compatible provider 不会显示。
    ...
    for provider_id, record in sorted(custom_providers.items()):  # 新增代码+ProviderSettings：稳定遍历自定义 provider；如果没有这行，自定义列表顺序会抖动。
        ...
        result.append(asdict(GuiProviderInfo(id=str(provider_id), display_name=str(record.get("display_name", record.get("name", provider_id))), kind="custom", source="custom", connected=connected, masked_key=secret_store.mask_secret(secret_ref) if connected else "", auth_methods=[_api_key_auth_method()], description="通过基础 URL 添加的 OpenAI 兼容提供商", models=models, direct_route_status="api_key_route" if connected else "")))  # 修改代码+ComposerRouteControls：加入自定义 provider payload 并标记 API key 兼容路径；如果没有这行，自定义 provider 路由状态会空白。
...
def build_provider_settings_payload(workspace: str | Path) -> dict[str, Any]:  # 新增代码+ProviderSettings：函数段开始，生成 Provider Settings V1 catalog；如果没有这段，bridge GET 路由没有稳定响应。
    ...
    secret_store = make_provider_secret_store(workspace)  # 修改代码+SecretStoreFactory：通过 factory 创建后端密钥存储；如果没有这行，真实 OAuth 无法使用 os_encrypted。
    providers = _built_in_provider_catalog(workspace, settings, secret_store) + _custom_provider_catalog(settings, secret_store)  # 修改代码+OpenAIModelRegistry：合并内置和自定义 provider；如果没有这行，catalog 不完整且 OpenAI 模型无法读取注册表。
    default_provider_id = str(settings.get("default_provider_id", ""))  # 新增代码+ComposerRouteControls：读取通用默认 provider；如果没有这行，后续默认模型选择只能从原始 dict 直接拼。
    default_model_id = str(settings.get("default_model_id", ""))  # 新增代码+ComposerRouteControls：读取通用默认模型；如果没有这行，底部模型下拉无法使用后端偏好。
    selected_openai_model_id = str(settings.get("selected_openai_model_id", ""))  # 新增代码+ComposerRouteControls：读取 OpenAI OAuth 当前模型；如果没有这行，OAuth 重连后无法优先恢复上次模型。
    payload_default_provider_id = "openai" if selected_openai_model_id else default_provider_id  # 修改代码+ComposerRouteControls：OpenAI 已选模型存在时优先配套 OpenAI provider；如果没有这行，前端可能把 OpenAI 模型配到其它 provider。
    payload_default_model_id = selected_openai_model_id or default_model_id  # 新增代码+ComposerRouteControls：优先暴露 OpenAI 当前模型；如果没有这行，真实 OAuth 连接后底部可能选不到上次成功模型。
    payload = {"ok": True, "schema_version": PROVIDER_SETTINGS_SCHEMA_VERSION, "secret_store": secret_store.info(), "providers": providers, "custom_provider_cta": {"id": "custom-provider-cta", "display_name": "自定义提供商", "description": "通过基础 URL 添加与 OpenAI 兼容的提供商。"}, "default_provider_id": payload_default_provider_id, "default_model_id": payload_default_model_id}  # 修改代码+ComposerRouteControls：构造完整 catalog payload 并带可用默认模型；如果没有这行，前端缺少 provider、CTA 和默认选择。
...
def sanitize_header_rows(rows: Any, secret_store: GuiProviderSecretStore, provider_id: str) -> str:  # 修改代码+SecretStoreFactory：函数段开始，把自定义 header 值存入 secret store；如果没有这段，header 值可能明文落进主配置。
...
def _append_secret_ref_once(secret_refs: list[str], value: Any) -> None:  # 新增代码+OpenAIDisconnect：函数段开始，向待删列表加入一个 secret_ref；如果没有这段，断开逻辑会重复写去重和空值判断。
    secret_ref = str(value or "").strip()  # 新增代码+OpenAIDisconnect：把未知输入收敛成引用字符串；如果没有这行，None 或数字会污染 secret 删除列表。
    if secret_ref and secret_ref not in secret_refs:  # 新增代码+OpenAIDisconnect：只保留非空且未加入的引用；如果没有这行，重复删除会让排查日志混乱。
        secret_refs.append(secret_ref)  # 新增代码+OpenAIDisconnect：加入待删引用；如果没有这行，目标 secret 不会被清理。
# 新增代码+OpenAIDisconnect：函数段结束，_append_secret_ref_once 到此结束；如果没有边界说明，初学者不易看出它只负责列表收集。


def _secret_refs_from_record(record: Any, provider_id: str, include_legacy_api_ref: bool) -> list[str]:  # 新增代码+OpenAIDisconnect：函数段开始，从 auth/custom 记录收集 secret 引用；如果没有这段，OAuth token 组断开时会漏删。
    secret_refs: list[str] = []  # 新增代码+OpenAIDisconnect：准备待删除引用列表；如果没有这行，后续无法累积 access/refresh/id token。
    if isinstance(record, dict):  # 新增代码+OpenAIDisconnect：只解析对象记录；如果没有这行，坏配置会让断开流程异常。
        _append_secret_ref_once(secret_refs, record.get("secret_ref", ""))  # 新增代码+OpenAIDisconnect：兼容旧 API key 单引用；如果没有这行，旧配置断开后 key 会残留。
        _append_secret_ref_once(secret_refs, record.get("headers_ref", ""))  # 新增代码+OpenAIDisconnect：兼容自定义 provider header 引用；如果没有这行，自定义 header secret 会残留。
        grouped_refs = record.get("secret_refs", {})  # 新增代码+OpenAIDisconnect：读取 OAuth token 引用组；如果没有这行，access/refresh/id token 无法批量清理。
        if isinstance(grouped_refs, dict):  # 新增代码+OpenAIDisconnect：只接受对象形式的 secret_refs；如果没有这行，坏类型会导致 values() 异常。
            for grouped_ref in grouped_refs.values():  # 新增代码+OpenAIDisconnect：遍历 access_token、refresh_token、id_token 等引用；如果没有这行，OAuth token 组不会被删除。
                _append_secret_ref_once(secret_refs, grouped_ref)  # 新增代码+OpenAIDisconnect：把每个 OAuth 引用加入待删列表；如果没有这行，真实 token 密文会残留。
    if include_legacy_api_ref and not secret_refs:  # 新增代码+OpenAIDisconnect：无记录时兼容默认 API key 引用；如果没有这行，旧版本默认引用可能清不掉。
        _append_secret_ref_once(secret_refs, safe_secret_ref(provider_id, "api_key"))  # 新增代码+OpenAIDisconnect：加入默认 API key 引用；如果没有这行，孤儿旧 key 可能残留。
    return secret_refs  # 新增代码+OpenAIDisconnect：返回去重后的引用列表；如果没有这行，调用方拿不到待删目标。
# 新增代码+OpenAIDisconnect：函数段结束，_secret_refs_from_record 到此结束；如果没有边界说明，初学者不易看出它只负责收集。


def _delete_secret_refs(secret_store: GuiProviderSecretStore, secret_refs: list[str]) -> None:  # 新增代码+OpenAIDisconnect：函数段开始，批量删除 secret 引用；如果没有这段，disconnect 会漏掉 OAuth token 组。
    for secret_ref in secret_refs:  # 新增代码+OpenAIDisconnect：遍历每一个待删引用；如果没有这行，批量删除不会执行。
        secret_store.delete_secret(secret_ref)  # 新增代码+OpenAIDisconnect：从当前 secret store 删除引用；如果没有这行，token 密文或 API key 会残留。
# 新增代码+OpenAIDisconnect：函数段结束，_delete_secret_refs 到此结束；如果没有边界说明，初学者不易看出它只负责删除。


def _clear_openai_selection(settings: dict[str, Any]) -> None:  # 新增代码+OpenAIDisconnect：函数段开始，清理 OpenAI 账号和模型选择；如果没有这段，断开后底部仍可能显示已连接模型。
    settings["selected_openai_account_id"] = ""  # 新增代码+OpenAIDisconnect：清空 OpenAI 账号 id；如果没有这行，重连前可能误用旧账号。
    settings["selected_openai_account_label"] = ""  # 新增代码+OpenAIDisconnect：清空脱敏账号标签；如果没有这行，UI 会误显示旧账号仍连接。
    settings["selected_openai_model_id"] = ""  # 新增代码+OpenAIDisconnect：清空 OpenAI 已选模型；如果没有这行，真实调用可能继续用断开前模型。
    if str(settings.get("default_provider_id", "")) == "openai":  # 新增代码+OpenAIDisconnect：检查默认 provider 是否指向 OpenAI；如果没有这行，断开其它 provider 时会误清默认值。
        settings["default_provider_id"] = ""  # 新增代码+OpenAIDisconnect：清空 OpenAI 默认 provider；如果没有这行，断开后底部仍可能默认选择 OpenAI。
    if str(settings.get("default_model_id", "")).startswith("gpt-"):  # 新增代码+OpenAIDisconnect：检查默认模型是否像 OpenAI 模型；如果没有这行，断开后旧 gpt 模型会继续显示。
        settings["default_model_id"] = ""  # 新增代码+OpenAIDisconnect：清空 OpenAI 默认模型；如果没有这行，后续发送可能误带旧模型。
# 新增代码+OpenAIDisconnect：函数段结束，_clear_openai_selection 到此结束；如果没有边界说明，初学者不易看出它只改选择状态。
...
def set_provider_auth(workspace: str | Path, provider_id: Any, auth_method_id: Any, fields: Any) -> dict[str, Any]:  # 新增代码+ProviderSettings：函数段开始，保存内置或自定义 provider 认证；如果没有这段，连接按钮没有后端效果。
    ...
    secret_store = make_provider_secret_store(workspace)  # 修改代码+SecretStoreFactory：通过 factory 创建密钥存储；如果没有这行，API key/真实 token 会绕过当前安全模式。
...
def disconnect_provider(workspace: str | Path, provider_id: Any) -> dict[str, Any]:  # 新增代码+ProviderSettings：函数段开始，断开 provider 并清理密钥；如果没有这段，断开按钮不会真正删除 secret。
    ...
    secret_store = make_provider_secret_store(workspace)  # 修改代码+SecretStoreFactory：通过 factory 创建密钥存储；如果没有这行，disconnect 无法删除 os_encrypted token。
    ...
    secret_refs = _secret_refs_from_record(auth_record, clean_provider_id, include_legacy_api_ref=True)  # 修改代码+OpenAIDisconnect：收集旧 secret_ref 和 OAuth secret_refs 组；如果没有这行，refresh/id token 会残留。
    _delete_secret_refs(secret_store, secret_refs)  # 修改代码+OpenAIDisconnect：删除认证记录关联的所有 secret；如果没有这行，断开后 raw key 或 token 密文仍在磁盘。
    ...
    if clean_provider_id in custom_providers and isinstance(custom_providers[clean_provider_id], dict):  # 新增代码+ProviderSettings：处理自定义 provider 记录；如果没有这行，自定义密钥可能残留。
        custom_secret_refs = _secret_refs_from_record(custom_providers[clean_provider_id], clean_provider_id, include_legacy_api_ref=False)  # 修改代码+OpenAIDisconnect：收集自定义 provider key/header 引用；如果没有这行，自定义 header secret 会残留。
        _delete_secret_refs(secret_store, custom_secret_refs)  # 修改代码+OpenAIDisconnect：删除自定义 provider 关联 secret；如果没有这行，自定义断开后仍有 raw key 或 header。
    if clean_provider_id == "openai":  # 新增代码+OpenAIDisconnect：只在断开 OpenAI 时清理 OpenAI 专用选择状态；如果没有这行，断开其它 provider 会误清 OpenAI。
        _clear_openai_selection(settings)  # 新增代码+OpenAIDisconnect：清理 OpenAI 账号和模型选择；如果没有这行，断开后仍可能用旧模型发请求。
...
def save_custom_provider(workspace: str | Path, payload: dict[str, Any]) -> dict[str, Any]:  # 新增代码+ProviderSettings：函数段开始，保存自定义 OpenAI-compatible provider；如果没有这段，添加自定义提供商没有后端效果。
    ...
    secret_store = make_provider_secret_store(workspace)  # 修改代码+SecretStoreFactory：通过 factory 创建密钥存储；如果没有这行，自定义 provider 会绕过当前安全模式。
...
def _headers_from_ref(secret_store: GuiProviderSecretStore, headers_ref: str) -> dict[str, str]:  # 修改代码+SecretStoreFactory：函数段开始，从 secret store 读取自定义 header；如果没有这段，探针无法使用用户配置 header。
...
def test_provider_connection(workspace: str | Path, provider_id: Any) -> dict[str, Any]:  # 新增代码+ProviderSettings：函数段开始，测试 provider 连接但不切换真实 runtime；如果没有这段，设置页无法区分“已保存 key”和“连接可用”。
    ...
    secret_store = make_provider_secret_store(workspace)  # 修改代码+SecretStoreFactory：通过 factory 创建 secret store；如果没有这行，探针无法读取 os_encrypted 或 dev_json 中的凭据。
