...
def _chrome_guide_line(installer_state: str) -> str:  # 新增代码+Phase12ChromeStatusGuide: 根据 native host 安装状态生成下一步建议；如果没有这段函数，/chrome 只能展示状态不能指导操作。
...
def render_chrome_status(snapshot: dict[str, Any]) -> str:  # 新增代码+ChromeTerminalUI: 渲染 `/chrome` 聚焦状态页；若没有这段代码，终端命令没有可复用输出逻辑。
    ...
    lines.append(_chrome_guide_line(str(native_host.get("installer_state", ""))))  # 新增代码+Phase12ChromeStatusGuide: 输出按状态计算的下一条命令；如果没有这行代码，/chrome 不能指导后续操作。
