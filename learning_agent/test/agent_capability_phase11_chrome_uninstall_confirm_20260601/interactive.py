...
def run_chrome_terminal_command(workspace: Path, user_input: str, registry_adapter: Any | None = None) -> str:  # 修改代码+Phase10ChromeInstallConfirm: 执行 `/chrome` 子命令并允许测试注入 fake registry；如果没有这段函数，终端动作无法被单元测试和真实终端复用。
    ...
    return f"Chrome Action\n- 不支持的 /chrome 子命令。\n- 可用命令：/chrome、/chrome install-preview [extension_id]、/chrome install-confirm <extension_id> {CHROME_INSTALL_CONFIRM_TOKEN}、/chrome repair、/chrome uninstall-preview、/chrome uninstall-confirm {CHROME_UNINSTALL_CONFIRM_TOKEN}\n"  # 修改代码+Phase11ChromeUninstallConfirm: 给未知命令返回包含确认卸载的清楚用法；如果没有这行代码，输错命令会被误发给模型或无响应。
