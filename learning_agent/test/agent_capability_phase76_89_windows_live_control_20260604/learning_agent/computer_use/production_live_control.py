...
class WindowsProductionComputerUseHostAdapter:
    ...
    def cleanup(self) -> dict[str, Any]:
        self.cleanup_events.extend(["release_keys", "release_mouse", "restore_clipboard", "clear_transient_locks"])  # 新增代码+Phase76-89：记录清理动作，缺少它会无法证明 Phase 85 达标。
        return {"cleanup_done": True, "events": list(self.cleanup_events), "abort_requested": self.abort_requested}  # 新增代码+Phase76-89：返回清理结果，缺少它会让终端验收看不到清理是否完成。
    # 新增代码+Phase76-89：函数段结束；真实清理器可以在这里接入 keyUp/mouseUp/锁释放。
...
def build_phase86_high_level_tool_surface() -> list[dict[str, Any]]:
    ...
    tool_names = ["status", "observe", "open_app", "focus_window", "click", "double_click", "drag", "type_text", "clipboard_paste", "press_key", "hotkey", "scroll", "wait_for", "zoom", "cleanup", "abort"]  # 新增代码+Phase76-89：列出高层工具名称，缺少它会让工具能力不完整。
    return [{"name": name, "stable": True, "requires_permission_gate": name not in {"status", "observe"}} for name in tool_names]  # 新增代码+Phase76-89：给每个工具标注稳定性和权限门，缺少它会让模型不知道哪些动作需要审批。
...
def _run_optional_phase58_safe_real_smoke(real_smoke: bool) -> dict[str, Any]:
    ...
    try:  # 新增代码+Phase76-89：真实 smoke 可能受桌面环境影响，缺少异常保护会让整个验收直接崩溃。
        from learning_agent.computer_use.real_sendinput_guard import run_phase58_real_sendinput_guard_contract  # 新增代码+Phase76-89：复用 Phase 58 已有安全窗口真实输入门禁，缺少它会重复造真实 SendInput 轮子。
