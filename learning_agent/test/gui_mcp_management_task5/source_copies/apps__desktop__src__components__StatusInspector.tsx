...
import { Activity, Boxes, Bug, Globe2, ListChecks, Network, Settings, Wrench, type LucideIcon } from "lucide-react"; // 修改代码+DesktopGUIMcpPanel：引入 MCP 页签 Network 图标；如果没有这行，MCP 管理入口会缺少可扫描符号。
...
type InspectorTabId = "status" | "tools" | "toolchain" | "mcp" | "browser" | "harness" | "settings" | "diagnostics"; // 修改代码+DesktopGUIMcpPanel：限定右侧检查器页签 id 并加入 MCP 页；如果没有这行，MCP 管理页无法被选中。
...
export function StatusInspector({ events, browserProviderStatus, runtimePanels = {}, toolchainPayload = {}, mcpPayload = {}, harnessPayload = {}, bridgeBaseUrl = "", healthPayload = {}, diagnosticsPayload = {}, onProbeUnknownRoute, onPauseHarness, onResumeHarness, onStopHarness, onCheckpointHarness, harnessControlPending = false, lastHarnessControlResult = {}, onRequestComputerUseAccess, onObserveComputerUse, onAbortComputerUse, computerUseActionPending = false, lastComputerUseActionResult = {}, onRefreshBrowserStatus, onOpenBrowserUrl, browserActionPending = false, lastBrowserActionResult = {} }: StatusInspectorProps): JSX.Element { // 修改代码+DesktopGUIMcpPanel：函数段开始，渲染右侧多页签检查器并接入 MCP、Browser、Harness 与 Computer Use；如果没有这段，MCP 页签无法拿到 payload。
