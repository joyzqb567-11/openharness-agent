...
class WindowsPaintPikachuRealExecutionLoop:  # 新增代码+PaintPikachuRealLoop：类段开始，封装 full 模式 Paint 皮卡丘真实执行流程；如果没有这个类，desktop_task_runtime 仍没有可注入真实闭环。
    ...
    def run_desktop_task(self, target_app: str, prompt: str) -> dict[str, Any]:  # 新增代码+PaintPikachuRealLoop：函数段开始，执行 Paint 皮卡丘真实桌面任务；如果没有这段函数，desktop_task_runtime 无法调用真实闭环。
        ...
        cleanup_report = self.launch_registry.cleanup_owned_processes(reason="paint_pikachu_real_loop_finished")  # 新增代码+FullNaturalUserFlowCleanup：截图取证后清理本轮启动的 Paint 进程；如果没有这一行，真实验收会把 agent 自己打开的 Paint 留在桌面。
