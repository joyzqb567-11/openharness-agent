...
class ComputerUseModeSessionStore:  # 新增代码+Phase98UniversalComputerUseMode：类段落开始，管理 Computer Use 模式 session；如果没有这个类，外部没有统一 API 打开/停止/查询模式。
    ...
    def open_full_mode(self, reason: str = "") -> dict[str, Any]:  # 新增代码+FullNaturalUserFlow：函数段落开始，给普通用户命令直接打开 full mode；如果没有这段函数，`/computer use --full` 只能继续走反直觉的动态 token 流程。
        opened = self._build_state("full", reason, DEFAULT_FULL_TTL_SECONDS, allow_full=True)  # 新增代码+FullNaturalUserFlow：从 store 内部受控路径构建 full 状态；如果没有这行代码，外部调用方可能绕开 full 的短 TTL 和安全字段。
        atomic_write_json(self.current_path, opened)  # 新增代码+FullNaturalUserFlow：原子写入 current.json 持久化 full 状态；如果没有这行代码，终端显示打开但后续动作层读不到授权。
        try:  # 新增代码+FullNaturalUserFlow：尝试清理旧 pending token 文件；如果没有这行代码，旧 token 文件会残留并让排查误以为仍需要确认。
            self.pending_full_path.unlink(missing_ok=True)  # 新增代码+FullNaturalUserFlow：删除旧的 full-confirm 待确认文件；如果没有这行代码，历史 token 可能被误拿来当成本轮入口。
        except OSError:  # 新增代码+FullNaturalUserFlow：容忍 pending 文件清理失败；如果没有这行代码，full 已打开时可能因为清理小问题被误报失败。
            pass  # 新增代码+FullNaturalUserFlow：忽略清理异常并保留已打开状态；如果没有这行代码，except 分支语法不完整。
        return dict(opened)  # 新增代码+FullNaturalUserFlow：返回 full 状态副本给终端渲染；如果没有这行代码，调用方拿不到 opened=true 和 ttl_seconds。
    # 新增代码+FullNaturalUserFlow：函数段落结束，ComputerUseModeSessionStore.open_full_mode 到此结束；如果没有这个边界说明，初学者不容易看出自然 full 打开范围。
