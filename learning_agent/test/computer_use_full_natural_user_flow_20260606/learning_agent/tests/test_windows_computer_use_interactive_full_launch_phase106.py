...
class WindowsComputerUseInteractiveFullLaunchPhase106Tests(unittest.TestCase):  # 新增代码+Phase106InteractiveFullLaunch：类段开始，集中验证用户交互命令如何接入 full-mode 受控启动；如果没有这个类，Phase106 缺少回归保护。
    ...
    def _confirm_full_mode(self, workspace: Path) -> None:  # 修改代码+FullNaturalUserFlow：函数段开始，用真实用户的一行命令打开 full mode；如果没有这段函数，测试会继续模拟不自然的动态 token 流程。
        full_output = run_computer_terminal_command(workspace, "/computer use --full")  # 修改代码+FullNaturalUserFlow：直接执行用户会输入的 full 命令；如果没有这行代码，launch 命令会缺少授权前置。
        self.assertIn("full_mode=true", full_output)  # 修改代码+FullNaturalUserFlow：断言 full mode 已真正打开；如果没有这行代码，后续启动测试的前置条件可能是假的。
