...
class WindowsComputerUseFullMaturityMatrixTests(unittest.TestCase):  # 新增代码+FullMaturityMatrix：类段开始，集中验收 `/computer use --full` 最终成熟矩阵；如果没有这个类，蓝图 Task7 没有自动化边界。
    ...
    def test_full_maturity_matrix_reports_required_fields(self) -> None:  # 新增代码+FullMaturityMatrix：函数段开始，验证结构化矩阵和 CLI 行都包含蓝图最终字段；如果没有这段测试，矩阵可能只输出部分阶段事实。
        ...
        self.assertTrue(report["paint_pikachu_visible_terminal_acceptance"])  # 新增代码+Task8Maturity：断言皮卡丘严格可见终端验收已入矩阵；如果没有这一行，最终成熟度会缺少真实终端证据。
        ...
        self.assertFalse(report["maturity_known_limit_real_desktop_execution"])  # 新增代码+Task8Maturity：断言真实无约束桌面执行尚未被声明成熟；如果没有这一行，项目会过度承诺“完全控制真实电脑”。
        ...
        visible_report = report["reports"]["paint_pikachu_visible_terminal_acceptance"]  # 新增代码+Task8Maturity：读取皮卡丘可见终端验收子报告；如果没有这一行，测试无法检查真实终端证据的细节。
        self.assertTrue(visible_report["completed"])  # 新增代码+Task8Maturity：断言 controller 真实可见终端运行已完成；如果没有这一行，旧证据文件存在但失败也可能被当成成熟。
        self.assertEqual(4, visible_report["prompt_lines_sent"])  # 新增代码+Task8Maturity：断言严格场景实际发出了 full、confirm、任务、stop 四行；如果没有这一行，未输入皮卡丘 prompt 的旧问题可能复发。
        self.assertTrue(visible_report["gui_action_count_regex_passed"])  # 新增代码+Task8Maturity：断言 GUI 动作数大于 0 的正则验收通过；如果没有这一行，空动作也可能冒充成功。
        self.assertTrue(visible_report["low_level_event_count_regex_passed"])  # 新增代码+Task8Maturity：断言底层鼠标事件数大于 0 的正则验收通过；如果没有这一行，只有高层文字报告也可能冒充成功。
