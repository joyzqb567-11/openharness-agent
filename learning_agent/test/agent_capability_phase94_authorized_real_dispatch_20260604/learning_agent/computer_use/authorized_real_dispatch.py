...
try:  # 新增代码+Phase94AuthorizedRealDispatch：优先按 learning_agent 包路径导入现有安全组件；如果没有这段代码，单元测试和生产入口不能共享同一套模块。
    from learning_agent.computer_use.persistent_grants import DEFAULT_PERSISTENT_GRANT_SESSION_ID, DEFAULT_PERSISTENT_GRANTS_ROOT, WindowsComputerUsePersistentGrantStore  # 新增代码+Phase94AuthorizedRealDispatch：复用 Phase60 持久授权事实源；如果没有这行代码，Phase94 会绕开已有授权门禁。
    from learning_agent.computer_use.real_app_safety_boundary import PHASE72_UNCONTROLLED_ACTIONS_EXPANDED, WindowsRealAppSafetyBoundary  # 新增代码+Phase94AuthorizedRealDispatch：复用 Phase72 真实应用安全边界；如果没有这行代码，危险窗口和未授权窗口会缺少统一拒绝。
    from learning_agent.computer_use.universal_live_execution import run_phase93_universal_live_execution_gate_contract  # 新增代码+Phase94AuthorizedRealDispatch：复用 Phase93 合同证明旧缺口；如果没有这行代码，Phase94 报告不能说明自己补的是哪一段差距。
    from learning_agent.computer_use.windows_backend import StaticWindowsWindowInventory  # 新增代码+Phase94AuthorizedRealDispatch：复用静态窗口 inventory 做目标复核；如果没有这行代码，测试只能依赖真实桌面窗口。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.runtime", "learning_agent.runtime.files"}:  # 新增代码+Phase94AuthorizedRealDispatch：只对包路径缺失做 fallback；如果没有这行代码，内部真实 bug 可能被错误吞掉。
        raise  # 新增代码+Phase94AuthorizedRealDispatch：重新抛出非路径类导入错误；如果没有这行代码，底层模块问题会被隐藏。
    from computer_use.persistent_grants import DEFAULT_PERSISTENT_GRANT_SESSION_ID, DEFAULT_PERSISTENT_GRANTS_ROOT, WindowsComputerUsePersistentGrantStore  # type: ignore  # 新增代码+Phase94AuthorizedRealDispatch：脚本模式复用 Phase60 授权 store；如果没有这行代码，bat 入口无法验证授权。
    from computer_use.real_app_safety_boundary import PHASE72_UNCONTROLLED_ACTIONS_EXPANDED, WindowsRealAppSafetyBoundary  # type: ignore  # 新增代码+Phase94AuthorizedRealDispatch：脚本模式复用 Phase72 安全边界；如果没有这行代码，bat 入口无法拦截危险窗口。
    from computer_use.universal_live_execution import run_phase93_universal_live_execution_gate_contract  # type: ignore  # 新增代码+Phase94AuthorizedRealDispatch：脚本模式复用 Phase93 合同；如果没有这行代码，bat 入口无法证明承接 Phase93。
    from computer_use.windows_backend import StaticWindowsWindowInventory  # type: ignore  # 新增代码+Phase94AuthorizedRealDispatch：脚本模式复用静态窗口 inventory；如果没有这行代码，合同无法离线复核目标。
