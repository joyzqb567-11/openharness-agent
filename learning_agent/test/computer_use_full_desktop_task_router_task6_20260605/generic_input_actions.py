...
try:  # 新增代码+VerifiedWindowActionsMaturity: 优先按包路径导入 Phase114 已验证窗口动作门禁；如果没有这段代码，Phase71 热键/滚轮/拖拽无法复用统一身份门禁。
    from learning_agent.computer_use.closed_loop_executor import phase114_verified_action_gate  # 新增代码+VerifiedWindowActionsMaturity: 导入统一动作身份门禁函数；如果没有这行代码，热键和鼠标事件会各自手写身份判断。
    ...
    from computer_use.closed_loop_executor import phase114_verified_action_gate  # type: ignore  # 新增代码+VerifiedWindowActionsMaturity: 脚本模式导入统一身份门禁；如果没有这行代码，bat 入口无法执行 Task6 输入门禁。

try:  # 新增代码+DrawingPrimitives：优先按包路径导入通用绘图拖拽展开器；如果没有这段代码，Phase71 会继续维护一套重复的 drag_path 展开逻辑。
    from learning_agent.computer_use.drawing_primitives import expand_drag_path_to_low_level_events  # 新增代码+DrawingPrimitives：导入统一拖拽路径到鼠标事件的转换函数；如果没有这一行，绘图 primitive 与通用输入层可能事件格式漂移。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.drawing_primitives"}:  # 新增代码+DrawingPrimitives：只兜底包路径缺失；如果没有这一行，drawing_primitives 内部真实 bug 会被误吞。
        raise  # 新增代码+DrawingPrimitives：重新抛出非路径类导入错误；如果没有这一行，排查绘图 primitive 失败会很困难。
    from computer_use.drawing_primitives import expand_drag_path_to_low_level_events  # type: ignore  # 新增代码+DrawingPrimitives：脚本模式导入同一转换函数；如果没有这一行，bat 入口无法复用 Task 6 拖拽 primitive。
