...
class WindowsComputerUseFullDesktopTaskRouterTests(unittest.TestCase):  # 新增代码+DesktopTaskAcceptance：类段开始，集中冻结 `/computer use --full` 桌面任务路由验收；如果没有这个类，unittest 不会组织这些回归用例。
    ...
    def test_agent_routes_paint_prompt_to_desktop_runtime_before_bash(self) -> None:  # 新增代码+DesktopTaskRuntimeRoute：函数段开始，验证 agent.run 在模型选择 bash 前拦截桌面任务；如果没有这个测试，Task 5 可能仍让模型生成 System.Drawing 绕路。
        ...
        with tempfile.TemporaryDirectory() as raw_dir:  # 新增代码+DesktopTaskRuntimeRoute：创建临时 workspace；如果没有这一行，测试会污染真实运行目录。
            ...
            agent.desktop_task_runtime = ComputerUseDesktopTaskRuntime.for_test(full_mode=True)  # 新增代码+DesktopTaskRuntimeRoute：注入已开启 full 的隔离运行时；如果没有这一行，测试无法稳定模拟用户已确认 full mode。
    ...
    def test_computer_use_full_command_still_uses_terminal_command_handler(self) -> None:  # 新增代码+DesktopTaskRuntimeRoute：函数段开始，验证 /computer 命令仍由终端命令处理器负责；如果没有这个测试，Task 5 可能误把 /computer use --full 当普通 prompt 路由。
        ...
        with tempfile.TemporaryDirectory() as raw_dir:  # 新增代码+DesktopTaskRuntimeRoute：创建临时 workspace；如果没有这一行，命令状态会污染项目目录。
            output = run_computer_terminal_command(Path(raw_dir), "/computer use --full")  # 新增代码+DesktopTaskRuntimeRoute：执行真实 full 请求命令；如果没有这一行，断言没有终端输出对象。
            self.assertIn("Computer Use Full Request", output)  # 新增代码+DesktopTaskRuntimeRoute：断言 full 请求仍返回终端命令面板；如果没有这一行，/computer 命令可能被普通 prompt 路由吃掉。
            self.assertIn("confirm_command=/computer use --full-confirm", output)  # 新增代码+DesktopTaskRuntimeRoute：断言二次确认命令仍可见；如果没有这一行，用户无法开启 full mode。
