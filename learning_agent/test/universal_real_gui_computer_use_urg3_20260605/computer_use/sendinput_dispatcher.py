...
class WindowsSendInputDispatcher:  # 新增代码+Phase47WindowsSendInputDispatcher: 类段开始，定义 Windows SendInput 调度器；如果没有这个类，高层事件无法展开成低层输入序列。
    ...
    def _expand_event(self, event: dict[str, Any]) -> list[dict[str, Any]]:  # 新增代码+Phase47WindowsSendInputDispatcher: 函数段开始，把一个规范事件展开成低层事件；如果没有这段函数，动作类型会散落在 send 里。
        ...
        if event_type == "move":  # 新增代码+Phase47WindowsSendInputDispatcher: 展开鼠标移动；如果没有这行代码，move_mouse 无法生成低层事件。
            return [{"type": "mouse_move", "x": _safe_int(event.get("x")), "y": _safe_int(event.get("y"))}]  # 新增代码+Phase47WindowsSendInputDispatcher: 返回鼠标移动事件；如果没有这行代码，低层 sender 不知道坐标。
        if event_type == "click":  # 新增代码+Phase47WindowsSendInputDispatcher: 展开单击；如果没有这行代码，click 无法生成按下/抬起。
            return [{"type": "mouse_move", "x": _safe_int(event.get("x")), "y": _safe_int(event.get("y"))}, {"type": "mouse_down", "button": str(event.get("button", "left"))}, {"type": "mouse_up", "button": str(event.get("button", "left"))}]  # 新增代码+Phase47WindowsSendInputDispatcher: 返回移动、按下、抬起序列；如果没有这行代码，点击不会完整。
        if event_type == "double_click":  # 新增代码+Phase47WindowsSendInputDispatcher: 展开双击；如果没有这行代码，double_click 无法生成两次点击。
            ...
            return [{"type": "mouse_move", "x": _safe_int(event.get("x")), "y": _safe_int(event.get("y"))}, {"type": "mouse_down", "button": button}, {"type": "mouse_up", "button": button}, {"type": "mouse_down", "button": button}, {"type": "mouse_up", "button": button}]  # 新增代码+Phase47WindowsSendInputDispatcher: 返回移动和两次按下/抬起；如果没有这行代码，双击无法表达。
        if event_type == "scroll":  # 新增代码+Phase47WindowsSendInputDispatcher: 展开滚轮；如果没有这行代码，scroll 无法生成低层滚轮事件。
            return [{"type": "mouse_move", "x": _safe_int(event.get("x")), "y": _safe_int(event.get("y"))}, {"type": "mouse_wheel", "delta": _safe_int(event.get("delta"))}]  # 新增代码+Phase47WindowsSendInputDispatcher: 返回移动和滚轮事件；如果没有这行代码，滚动位置和方向会丢失。
        if event_type == "drag_path":  # 新增代码+DrawingPrimitives：展开通用绘图拖拽路径；如果没有这一行，皮卡丘等绘图 primitive 无法进入低层鼠标事件。
            return expand_drag_path_to_low_level_events(list(event.get("points", [])))  # 新增代码+DrawingPrimitives：把 points 转成 mouse_move/down/up 序列；如果没有这一行，dispatcher 会把 drag_path 当未知事件空跑。
        if event_type == "key":  # 新增代码+Phase47WindowsSendInputDispatcher: 展开按键；如果没有这行代码，press_key 无法生成键盘事件。
            ...
            return [{"type": "key_down", "key": key}, {"type": "key_up", "key": key}] if key else []  # 新增代码+Phase47WindowsSendInputDispatcher: 返回按下/抬起；如果没有这行代码，按键不会完整。
        if event_type == "hotkey":  # 新增代码+URG3ActionDSL：展开通用组合键动作；如果没有这一行，URG-3 的 hotkey 只能退化成多个普通按键，真实语义不完整。
            ...
            return [{"type": "key_down", "key": key} for key in keys] + [{"type": "key_up", "key": key} for key in reversed(keys)] if keys else []  # 新增代码+URG3ActionDSL：先按下全部键再反向抬起；如果没有这一行，Ctrl+S 这类组合键无法被通用 DSL 表达。
        if event_type == "text":  # 新增代码+Phase47WindowsSendInputDispatcher: 展开文本输入摘要；如果没有这行代码，type_text 无法进入低层 sender。
            return [{"type": "unicode_text", "text_length": _safe_int(event.get("text_length")), "text_sha256_16": str(event.get("text_sha256_16", "")), "text_redacted": True}]  # 新增代码+Phase47WindowsSendInputDispatcher: 返回不含原文的 unicode 文本摘要；如果没有这行代码，文本可能泄露。
