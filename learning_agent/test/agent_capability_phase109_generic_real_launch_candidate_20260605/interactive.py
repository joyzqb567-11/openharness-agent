    ...
    if not bool(target_report.get("safe_known_ordinary_app", False)):  # 新增代码+Phase107InteractiveLaunchTarget：先拦截高风险或未知目标；如果没有这行代码，powershell/cmd 等可能进入 Universal gate。
        if bool(target_report.get("unknown_target_refused", False)) and not bool(target_report.get("high_risk_refused", False)):  # 新增代码+Phase108GenericAppDiscovery：只有 Phase107 未知且不是高风险时才交给通用发现；如果没有这行代码，未知普通应用仍会被硬编码白名单卡住。
            ...
            if bool(generic_report.get("passed", False)):  # 新增代码+Phase108GenericAppDiscovery：只有普通且安全的通用目标才进入默认关闭成功路径；如果没有这行代码，高风险或无候选目标可能被误当成成功。
                ...
                return {"marker": PHASE106_INTERACTIVE_FULL_LAUNCH_MARKER, "ok_token": PHASE106_INTERACTIVE_FULL_LAUNCH_OK_TOKEN, "passed": bool(full_mode_session_used), "target_app": str(generic_report.get("canonical_target", "") or target_app or ""), "decision": decision, "real_action_blocked_decision": blocked_decision, "real_launch_requested": False, "real_enable_gate_passed": False, "controlled_real_launch_gate_passed": False, "full_mode_session_used": full_mode_session_used, "controlled_launch_candidate_ready": False, "real_full_launch_attempted": False, "visible_window_verified": False, "cleanup_completed": False, "verified_window_cleanup_completed": False, "residual_owned_process": False, "real_desktop_touched": False, "uncontrolled_actions_expanded": False, "phase107_target_report": target_report, "phase108_discovery_report": generic_report, "phase109_candidate_report": phase109_report}  # 修改代码+Phase109GenericRealLaunchCandidate：返回未知普通 app 的默认关闭报告并附带 Phase109 通用真实启动候选；如果没有这行代码，Phase108 能发现目标但用户看不到统一身份验证与清理候选模型。
