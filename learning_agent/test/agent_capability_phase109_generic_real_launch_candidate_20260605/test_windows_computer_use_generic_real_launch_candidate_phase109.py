...
class WindowsComputerUseGenericRealLaunchCandidatePhase109Tests(unittest.TestCase):  # 新增代码+Phase109GenericRealLaunchCandidate：类段开始，集中验收 Phase109 通用真实启动候选；如果没有这个类，通用启动候选只会停在设计说明没有回归保护。
    ...
    def _confirm_full_mode(self, workspace: Path) -> None:  # 新增代码+Phase109GenericRealLaunchCandidate：函数段开始，用真实命令打开 full 模式；如果没有这段函数，交互测试会绕过用户确认流程。
        request_output = run_computer_terminal_command(workspace, "/computer use --full")  # 新增代码+Phase109GenericRealLaunchCandidate：先申请 full 模式动态确认 token；如果没有这行代码，后续确认命令没有合法 token。
        token = self._confirmation_token(request_output)  # 新增代码+Phase109GenericRealLaunchCandidate：从请求输出读取本轮 token；如果没有这行代码，测试无法跟随真实安全流程。
        confirm_output = run_computer_terminal_command(workspace, f"/computer use --full-confirm {token}")  # 新增代码+Phase109GenericRealLaunchCandidate：用真实命令确认 full 模式；如果没有这行代码，launch 命令应该仍被权限门拦截。
        self.assertIn("full_mode=true", confirm_output)  # 新增代码+Phase109GenericRealLaunchCandidate：断言 full 模式确实开启；如果没有这行代码，后续启动测试的前置状态可能是假的。
    # 新增代码+Phase109GenericRealLaunchCandidate：函数段结束，_confirm_full_mode 到此结束；如果没有这个边界说明，初学者不容易看出 full 确认范围。
