...
class ComputerUseModeCommandPhase98Tests(unittest.TestCase):  # 新增代码+Phase98UniversalComputerUseMode：类段开始，集中验证 Phase98 模式命令真实终端契约；如果没有这个类，Task3 的 `/computer` 接入没有红绿灯。
    ...
    def test_computer_full_user_command_clears_stale_abort_automatically(self) -> None:  # 新增代码+FullModeAutoClearAbort：函数段开始，验证普通用户打开 full 模式时不需要先输入 clear-abort；如果没有这个测试，真实验收会继续依赖开发者维护命令。
        with tempfile.TemporaryDirectory() as temp_dir:  # 新增代码+FullModeAutoClearAbort：创建临时 workspace；如果没有这行代码，abort 状态会污染真实项目 memory。
            workspace = Path(temp_dir)  # 新增代码+FullModeAutoClearAbort：保存同一 workspace 给 abort 和 full 共用；如果没有这行代码，full 命令读不到前一条 abort 状态。
            abort_output = run_computer_terminal_command(workspace, "/computer abort phase98-stale-abort")  # 新增代码+FullModeAutoClearAbort：先模拟上一轮留下急停状态；如果没有这行代码，测试不能证明 full 会自动恢复历史 abort。
            full_output = run_computer_terminal_command(workspace, "/computer use --full")  # 新增代码+FullModeAutoClearAbort：执行真实用户会输入的一行 full 命令；如果没有这行代码，自动清理入口没有被覆盖。
            status_output = run_computer_terminal_command(workspace, "/computer status")  # 新增代码+FullModeAutoClearAbort：读取状态面板确认 abort 已被清掉；如果没有这行代码，只看 full 输出可能漏掉持久状态。
        self.assertIn("abort_requested=true", abort_output)  # 新增代码+FullModeAutoClearAbort：确认前置急停确实写入；如果没有这行代码，测试可能因为没有 abort 而假通过。
        self.assert_contains_all(full_output, ["Computer Use Mode", "mode=full", "full_mode=true", "abort_auto_cleared=true"])  # 新增代码+FullModeAutoClearAbort：断言 full 输出说明已自动清理历史急停；如果没有这行代码，用户仍不知道系统做了恢复。
        self.assert_contains_all(status_output, ["Computer Use Mode", "mode=full", "abort:false"])  # 新增代码+FullModeAutoClearAbort：断言状态面板里 abort 已经为 false；如果没有这行代码，full 输出可能只是文字但没有清持久 flag。
    # 新增代码+FullModeAutoClearAbort：函数段结束，test_computer_full_user_command_clears_stale_abort_automatically 到此结束；如果没有这个边界说明，读者不容易看出自动清理测试范围。
