...
class UniversalStageVerifier:  # 新增代码+StageVerifier：类段开始，定义通用阶段验收器；如果没有这个类，阶段循环没有可靠完成判断。
    ...
    def _verify_text_stage(self, stage: StagePlan, after: Mapping[str, Any], execution_result: StageResult) -> StageResult:  # 新增代码+StageVerifier：函数段开始，验证文本阶段；如果没有这段函数，文本验收会依赖事件数。
        ...
        visible_text = str(after.get("visible_text", after.get("text_content", "")) or "").strip()  # 新增代码+StageVerifier：读取通用可见文本；如果没有这行代码，弱观察帧没有 requested_text 字段时无法验收。
    ...
    def _verify_drawing_stage(self, stage: StagePlan, after: Mapping[str, Any], execution_result: StageResult) -> StageResult:  # 新增代码+StageVerifier：函数段开始，验证绘图阶段；如果没有这段函数，绘图验收会只看鼠标事件。
        changed = _stage_verifier_truthy(after, ("canvas_changed_after_actions", "canvas_changed", "canvas_not_blank"))  # 新增代码+StageVerifier：读取画布变化证据；如果没有这行代码，空画布可能误过。
        structure = bool(after.get("visual_primitives") or after.get("visual_structure_present") or after.get("colored_region_count", 0))  # 新增代码+StageVerifier：读取视觉结构证据；如果没有这行代码，单条无意义线可能误过。
    ...
    def _verify_commit_stage(self, stage: StagePlan, after: Mapping[str, Any], execution_result: StageResult) -> StageResult:  # 新增代码+StageVerifier：函数段开始，验证资源提交阶段；如果没有这段函数，保存动作会缺完成证据。
        saved = _stage_verifier_truthy(after, ("saved_resource_exists", "save_dialog_completed", "resource_commit_verified", "file_saved", "document_saved"))  # 新增代码+StageVerifier：读取保存完成证据；如果没有这行代码，按 Ctrl+S 会被误算保存成功。
