...
try:  # 新增代码+Phase27ComputerUse: 包运行模式下导入窗口协议模型；如果没有这行代码，控制器无法使用强类型窗口引用。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.action_policy", "learning_agent.computer_use.audit", "learning_agent.computer_use.evidence", "learning_agent.computer_use.helper_client", "learning_agent.computer_use.lock", "learning_agent.computer_use.models", "learning_agent.computer_use.native_helper", "learning_agent.computer_use.windows_backend"}:  # 修改代码+Phase32WindowsNativeHelper: 允许 Phase32 native helper 在脚本模式下 fallback；如果没有这行代码，新模块路径缺失会阻断 start_oauth_agent.bat。
...
class WindowsComputerUseBackend:  # 新增代码+Phase8ProductionEdges: 提供显式启用后的 Windows OS 控制后端；如果没有这段代码，Computer Use 只能停留在占位状态。
    def __init__(self, inventory: Any | None = None, real_actions_enabled: bool = True, evidence_store: Any | None = None, observation_helper: Any | None = None) -> None:  # 修改代码+Phase29ComputerUse: 初始化 Windows 后端并接入证据仓库和观察 helper；如果没有这段代码，get_window_state 无法保存截图/UIA artifact。
...
class ComputerUseController:  # 新增代码+OSComputerUse: 在 agent 和后端之间提供安全控制层；若没有这段代码，真实桌面操作缺少统一确认和参数校验。
    ...
    def status(self) -> dict[str, Any]:  # 新增代码+OSComputerUse: 返回 Computer Use 当前状态；若没有这段代码，computer_status 工具没有实现来源。
        ...
        return {"tool": "computer_use", "actions": sorted(self.ALLOWED_ACTIONS), "observe_actions": sorted(self.OBSERVE_ACTIONS), "backend": backend_status, "audit": audit_status, "lock": lock_status, "owner_session_id": self.owner_session_id, "auto_acquire_lock": self.auto_acquire_lock}  # 修改代码+Phase30ComputerUseActionGate: 返回写动作、只读观察、后端、审计、锁摘要和自动取锁策略；如果没有这行代码，模型无法区分 observe、action 和 lock 状态。
