...
class _OpenAIRealOAuthCallbackHandler(http.server.BaseHTTPRequestHandler):  # 新增代码+OpenAIRealOAuthCallback：类段开始，处理浏览器访问 /auth/callback；如果没有这个类，OpenAI 授权返回后没人接收 code。
    ...
    def do_GET(self) -> None:  # 新增代码+OpenAIRealOAuthCallback：函数段开始，接收 OpenAI OAuth callback GET；如果没有这段，浏览器回调无法进入后端。
        ...
        if parsed.path == "/auth/callback/health":  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：先处理本机 owner 探针路径；如果没有这行，bridge 无法确认 localhost 是否真的路由到自己。
            query = urllib.parse.parse_qs(parsed.query)  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：解析探针 query；如果没有这行，无法读取本次 bridge 的 owner token。
            expected_owner = str(getattr(self.server, "owner_token", "") or "")  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：读取当前 server 的私有 owner token；如果没有这行，探针没有可比对的身份值。
            requested_owner = query.get("owner", [""])[0]  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：读取客户端带来的 owner token；如果没有这行，任何本机请求都可能通过探针。
            if requested_owner != expected_owner:  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：拒绝不匹配的 owner token；如果没有这行，其它进程也可能伪装成当前 listener。
                self._send_callback_page("Authorization Failed", "OpenHarness callback owner mismatch.", status=404)  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：返回可读失败页；如果没有这行，错误 owner 探针没有稳定响应。
                return  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：owner 不匹配时停止处理；如果没有这行，错误请求会继续进入 OAuth callback 分支。
            body = expected_owner.encode("ascii")  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：把 owner token 编成响应体；如果没有这行，启动方无法确认命中的就是当前 server。
            self.send_response(200)  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：返回探针成功状态；如果没有这行，urlopen 会把健康检查当失败。
            self.send_header("Content-Type", "text/plain; charset=ascii")  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：声明纯文本响应；如果没有这行，调试时不清楚响应格式。
            self.send_header("Content-Length", str(len(body)))  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：声明响应长度；如果没有这行，客户端可能等待连接关闭。
            self.end_headers()  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：结束响应头；如果没有这行，响应体无法写出。
            self.wfile.write(body)  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：写入 owner token；如果没有这行，启动方无法做内容比对。
            return  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：探针处理完立即返回；如果没有这行，探针会继续被当成未知 OAuth path。
...
def _probe_openai_oauth_callback_owner(callback_port: int, owner_token: str) -> None:  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：函数段开始，验证 localhost callback 实际命中当前 bridge；如果没有这段，多 bridge/IPv4-IPv6 错位会返回假成功。
    import urllib.error  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：导入 urllib 错误类型；如果没有这行，探针失败无法稳定归类。
    import urllib.request  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：导入本机 HTTP 请求工具；如果没有这行，无法验证浏览器将访问的 localhost 路径。
    quoted_owner = urllib.parse.quote(str(owner_token or ""), safe="")  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：编码 owner token 到 query；如果没有这行，特殊字符可能破坏健康检查 URL。
    probe_url = f"http://localhost:{callback_port}/auth/callback/health?owner={quoted_owner}"  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：构造和 OAuth redirect 同 host/port 的探针 URL；如果没有这行，检查不到真实浏览器路由。
    last_error: Exception | None = None  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：保存最后一次探针错误；如果没有这行，失败消息会丢失具体原因。
    for _ in range(10):  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：短轮询等待后台 listener ready；如果没有这行，线程刚启动时可能偶发误判失败。
        try:  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：保护单次 HTTP 探针；如果没有这行，第一次连接失败会直接结束。
            with urllib.request.urlopen(probe_url, timeout=0.3) as response:  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：请求 owner health endpoint；如果没有这行，无法证明 localhost 命中当前 listener。
                body = response.read().decode("ascii", errors="replace")  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：读取探针响应体；如果没有这行，无法和 owner token 比对。
            if body == owner_token:  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：确认响应就是当前 bridge token；如果没有这行，其它 listener 的 200 响应会被误判成功。
                return  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：探针通过后返回；如果没有这行，成功也会继续重试直到失败。
            last_error = RuntimeError("callback owner mismatch")  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：记录 owner 不匹配；如果没有这行，最终错误无法说明命中了错误进程。
        except urllib.error.HTTPError as error:  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：HTTP 错误说明端口已有服务但不是当前 health endpoint；如果没有这行，GUI 会在确定错路由时多等数秒。
            last_error = error  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：保存 HTTP 错误详情；如果没有这行，最终错误无法说明收到的是 404/403 等响应。
            break  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：HTTP 错误不再重试；如果没有这行，已经确认的其它 listener 会拖慢连接失败反馈。
        except (OSError, TimeoutError, urllib.error.URLError) as error:  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：捕获连接失败、超时和 HTTP 错误；如果没有这行，探针异常会泄露到底层。
            last_error = error  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：保存失败原因；如果没有这行，最终错误只能说未知失败。
        time.sleep(0.05)  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：给 listener 或 DNS 重试一点时间；如果没有这行，循环会忙等浪费 CPU。
    detail = str(last_error or "callback owner probe failed")[:160]  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：截断失败详情；如果没有这行，底层错误可能过长污染 GUI。
    raise GuiProviderSettingsError(503, "openai_oauth_callback_listener_unavailable", f"OpenAI OAuth callback listener 不属于当前 GUI bridge：{detail}")  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：返回结构化 listener 错误；如果没有这行，GUI 会继续给用户不可用链接。
# 新增代码+OpenAIRealOAuthCallbackOwnerProbe：函数段结束，_probe_openai_oauth_callback_owner 到此结束；如果没有边界说明，初学者不易看出它只做本机路由验证。


def _close_openai_oauth_callback_server(server: Any) -> None:  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：函数段开始，失败时尽力关闭刚创建的 callback server；如果没有这段，owner probe 失败会遗留监听线程。
    shutdown = getattr(server, "shutdown", None)  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：读取可选 shutdown 方法；如果没有这行，fake server 或真实 server 的清理路径无法统一。
    if callable(shutdown):  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：只在方法存在时调用；如果没有这行，测试 fake server 会因为缺少方法崩溃。
        shutdown()  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：停止 serve_forever；如果没有这行，真实 server 可能继续占用端口。
    close = getattr(server, "server_close", None)  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：读取可选 socket close 方法；如果没有这行，底层 socket 可能不释放。
    if callable(close):  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：只在方法存在时关闭；如果没有这行，fake server 清理会报错。
        close()  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：关闭 server socket；如果没有这行，下一次 OAuth start 可能仍然端口冲突。
# 新增代码+OpenAIRealOAuthCallbackOwnerProbe：函数段结束，_close_openai_oauth_callback_server 到此结束；如果没有边界说明，初学者不易看出它是失败清理工具。
...
def _ensure_openai_oauth_callback_server(workspace: str | Path, oauth_secret: OpenAIRealOAuthAttemptSecret) -> None:  # 新增代码+OpenAIRealOAuthCallback：函数段开始，确保本机 callback server 已监听；如果没有这段，OpenAI 返回 redirect_uri 时会连接失败。
    ...
    with _CALLBACK_SERVER_LOCK:  # 新增代码+OpenAIRealOAuthCallback：锁住 server 创建；如果没有这行，并发 start 会重复 bind。
        if callback_port in _CALLBACK_SERVERS:  # 新增代码+OpenAIRealOAuthCallback：复用同端口已启动 server；如果没有这行，第二次连接会因端口占用失败。
            server = _CALLBACK_SERVERS[callback_port]  # 修改代码+OpenAIRealOAuthCallbackOwnerProbe：取出当前进程记录的 server；如果没有这行，复用路径无法验证 listener owner。
            _probe_openai_oauth_callback_owner(callback_port, str(getattr(server, "owner_token", "")))  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：复用前确认 localhost 仍命中当前 server；如果没有这行，陈旧或错路由 listener 会继续返回假成功。
            return  # 修改代码+OpenAIRealOAuthCallbackOwnerProbe：复用验证通过后返回；如果没有这行，会尝试重复创建 server。
        try:  # 新增代码+OpenAIRealOAuthCallback：保护端口绑定；如果没有这行，端口占用会变成底层 OSError。
            ...
            server.owner_token = f"openharness_callback_owner_{uuid.uuid4().hex}"  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：给当前 listener 分配私有 owner token；如果没有这行，health probe 无法确认命中当前 bridge。
        ...
        try:  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：保护启动后的 owner 探针；如果没有这行，探针失败会遗留刚创建的 server。
            _probe_openai_oauth_callback_owner(callback_port, server.owner_token)  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：确认 redirect_uri host/port 实际命中当前 listener；如果没有这行，多 bridge 时会生成无效授权链接。
        except GuiProviderSettingsError:  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：处理 owner 探针失败；如果没有这行，错误 listener 会残留在 registry 外继续占端口。
            _close_openai_oauth_callback_server(server)  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：关闭探针失败的 server；如果没有这行，失败后重试可能仍端口冲突。
            raise  # 新增代码+OpenAIRealOAuthCallbackOwnerProbe：把结构化错误传回 start；如果没有这行，GUI 不知道 listener 不可信。
        _CALLBACK_SERVERS[callback_port] = server  # 修改代码+OpenAIRealOAuthCallbackOwnerProbe：仅在 owner 探针通过后记录 server；如果没有这行，错路由 server 会污染复用路径。
