...
try:  # 修改代码+GenericLaunchBackendMaturity：优先导入 Task 2 的通用启动后端；如果没有这段代码，Phase109 默认仍会停留在旧的本地记录替身。
    from learning_agent.computer_use.generic_launch_backend import Phase110RecordingGenericLaunchBackend  # 修改代码+GenericLaunchBackendMaturity：复用新的 Phase110 记录型通用后端；如果没有这一行，新后端不会成为真实候选链路的一部分。
except ModuleNotFoundError as error:  # 修改代码+GenericLaunchBackendMaturity：兼容 start_oauth_agent.bat 从 learning_agent 目录启动的脚本模式；如果没有这一行，真实可见终端可能因包路径不同导入失败。
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.generic_launch_backend"}:  # 修改代码+GenericLaunchBackendMaturity：只兜底包路径缺失；如果没有这一行，新后端内部 bug 可能被误吞。
        raise  # 修改代码+GenericLaunchBackendMaturity：重新抛出真实内部错误；如果没有这一行，排查通用启动后端会非常困难。
    from computer_use.generic_launch_backend import Phase110RecordingGenericLaunchBackend  # type: ignore  # 修改代码+GenericLaunchBackendMaturity：脚本模式下导入同一个 Phase110 后端；如果没有这一行，双击 bat 后 Phase109 不能复用通用后端。
try:  # 修改代码+TargetIdentityMaturity：优先导入 Task 3 目标身份绑定 helper；如果没有这一段，Phase109 只能报告旧的粗略窗口布尔值。
    from learning_agent.computer_use.target_identity import build_owned_target_identity, verify_owned_target_identity  # 修改代码+TargetIdentityMaturity：复用 pid、hwnd、标题哈希和漂移校验逻辑；如果没有这一行，启动候选不会真正绑定目标身份。
except ModuleNotFoundError as error:  # 修改代码+TargetIdentityMaturity：兼容 start_oauth_agent.bat 从 learning_agent 目录启动的脚本模式；如果没有这一行，真实终端入口可能导入失败。
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.target_identity"}:  # 修改代码+TargetIdentityMaturity：只允许包路径缺失时 fallback；如果没有这一行，target_identity 内部 bug 可能被误吞。
        raise  # 修改代码+TargetIdentityMaturity：重新抛出真实内部错误；如果没有这一行，目标身份绑定问题会很难定位。
    from computer_use.target_identity import build_owned_target_identity, verify_owned_target_identity  # type: ignore  # 修改代码+TargetIdentityMaturity：脚本模式下导入同一目标身份 helper；如果没有这一行，双击 bat 后 Phase109 报告缺少 Task 3 字段。
...
class Phase109RecordingGenericLaunchBackend:  # 新增代码+Phase109GenericRealLaunchCandidate：类段开始，定义记录型通用启动后端；如果没有这个类，合同测试可能需要真的打开用户本机应用才能证明最后一跳。
    ...
    def launch(self, phase108_report: dict[str, Any]) -> dict[str, Any]:  # 新增代码+Phase109GenericRealLaunchCandidate：函数段开始，记录一次通用启动请求但不打开真实应用；如果没有这段函数，显式启用路径没有安全替身。
        ...
        return {"ok": bool(executable), "backend": "phase109_recording_generic_launch_backend", "backend_launch_performed": bool(executable), "process_started": True, "process_id": process_id, "process_handle_id": f"phase109-process:{process_id}", "process_executable": executable, "process_identity_verified": bool(executable), "cleanup_registered": bool(executable), "owned_process_registered": bool(executable), "real_desktop_touched": False, "phase108_report": safe_report}  # 修改代码+TargetIdentityMaturity：返回带自有进程登记的记录型进程摘要；如果没有这一行，目标身份模块会把记录型进程当成用户已有进程拒绝。
...
class Phase109RecordingWindowIdentityProbe:  # 新增代码+Phase109GenericRealLaunchCandidate：类段开始，定义记录型窗口身份探针；如果没有这个类，启动后无法证明窗口属于刚才的目标进程。
    ...
    def wait_for_verified_window(self, process_report: dict[str, Any], phase108_report: dict[str, Any]) -> dict[str, Any]:  # 新增代码+Phase109GenericRealLaunchCandidate：函数段开始，等待并验证目标窗口身份；如果没有这段函数，后续自动化可能误操作别的窗口。
        ...
        hwnd = 109000 + process_id  # 修改代码+TargetIdentityMaturity：为记录型窗口生成稳定 hwnd；如果没有这一行，目标身份模块无法证明 pid 已绑定到窗口句柄。
        window = {"window_id": f"hwnd:{hwnd}", "hwnd": hwnd, "pid": process_id, "window_process_id": process_id, "process_name": executable, "window_executable": executable, "app_id": executable, "title_preview": f"{canonical_target} - Phase109 recording", "safe_to_target": same_target}  # 修改代码+TargetIdentityMaturity：构造带 pid/hwnd/进程名的记录型窗口身份；如果没有这一行，窗口探针不能进入 Task 3 的强身份校验。
        owned_identity = build_owned_target_identity(process_report, window)  # 修改代码+TargetIdentityMaturity：把启动进程和窗口绑定成自有目标身份；如果没有这一行，Phase109 报告没有结构化目标凭证。
        verification = verify_owned_target_identity(owned_identity, window)  # 修改代码+TargetIdentityMaturity：用当前窗口立即跑一次动作前校验；如果没有这一行，target_drift_blocks_action 字段没有真实来源。
        self.windows.append(dict(window))  # 新增代码+Phase109GenericRealLaunchCandidate：保存窗口身份副本；如果没有这行代码，测试无法确认探针生成过窗口。
        return {"ok": bool(same_target and verification.allowed), "visible_window_verified": bool(same_target and verification.allowed), "window_identity_verified": verification.window_identity_verified, "target_identity_verified": verification.target_identity_verified, "target_drift_blocks_action": verification.target_drift_blocks_action, "process_identity_verified": bool(process_report.get("process_identity_verified", False) and owned_identity.process_identity_verified), "real_desktop_touched": False, "window": window, "owned_target_identity": owned_identity.to_report(), "target_identity_verification": verification.to_report()}  # 修改代码+TargetIdentityMaturity：返回 Task 3 目标身份和漂移阻断摘要；如果没有这一行，上层无法知道是否应当停止动作。
...
class Phase109GenericRealLaunchCandidate:  # 新增代码+Phase109GenericRealLaunchCandidate：类段开始，定义通用真实启动候选编排器；如果没有这个类，发现、启动、窗口验证和清理会散落到交互层。
    def __init__(self, launch_backend: Any | None = None, window_probe: Any | None = None, cleanup_manager: Any | None = None) -> None:  # 新增代码+Phase109GenericRealLaunchCandidate：函数段开始，允许注入后端、窗口探针和清理器；如果没有这段函数，测试和未来真实后端无法复用同一编排逻辑。
        self.launch_backend = launch_backend if launch_backend is not None else Phase110RecordingGenericLaunchBackend()  # 修改代码+GenericLaunchBackendMaturity：默认使用 Phase110 通用记录型启动后端；如果没有这一行，Task 2 新后端会孤立存在，Phase109 仍旧不能代表成熟通用链路。
    ...
    def prepare(self, raw_target: Any | None = None, candidates: list[dict[str, Any]] | None = None, generic_report: dict[str, Any] | None = None, enable_real_launch: bool = False) -> dict[str, Any]:  # 新增代码+Phase109GenericRealLaunchCandidate：函数段开始，准备通用真实启动候选；如果没有这段函数，交互层无法从任意 app 发现结果生成候选模型。
        ...
        target_drift_blocks_action = bool(window_result.get("target_drift_blocks_action", False))  # 修改代码+TargetIdentityMaturity：读取窗口身份校验是否因漂移阻断；如果没有这一行，Phase109 顶层报告不能告诉动作循环是否停止。
        base_report.update({"decision": "generic_real_launch_candidate_recording_backend_verified", "passed": bool(backend_reaches and process_identity_verified and window_identity_verified and not target_drift_blocks_action and cleanup_completed and cleanup_result.get("residual_process_check_completed", False) and not cleanup_result.get("residual_owned_process", True) and not real_desktop_touched), "generic_real_launch_candidate_ready": True, "default_off_backend_not_called": False, "backend_launch_reaches_launcher": backend_reaches, "recording_backend_reaches_launcher": backend_reaches, "process_identity_verified": process_identity_verified, "window_identity_verified": window_identity_verified, "target_identity_verified": bool(window_result.get("target_identity_verified", False)), "target_drift_blocks_action": target_drift_blocks_action, "visible_window_verified": bool(window_result.get("visible_window_verified", False)), "cleanup_completed": cleanup_completed, "verified_window_cleanup_completed": bool(cleanup_result.get("verified_window_cleanup_completed", False)), "residual_process_check_completed": bool(cleanup_result.get("residual_process_check_completed", False)), "residual_owned_process": bool(cleanup_result.get("residual_owned_process", False)), "real_desktop_touched": real_desktop_touched, "launch_result": launch_result, "window_result": window_result, "cleanup_result": cleanup_result})  # 修改代码+TargetIdentityMaturity：合并显式记录路径报告并上浮漂移阻断字段；如果没有这一行，合同无法证明目标身份绑定已经串起来。
...
__all__ = ["PHASE109_GENERIC_REAL_LAUNCH_CANDIDATE_MARKER", "PHASE109_GENERIC_REAL_LAUNCH_CANDIDATE_MODEL", "PHASE109_GENERIC_REAL_LAUNCH_CANDIDATE_OK_TOKEN", "PHASE109_REAL_LAUNCH_DEFAULT_DISABLED", "PHASE109_UNCONTROLLED_ACTIONS_EXPANDED", "Phase109GenericRealLaunchCandidate", "Phase109RecordingCleanupManager", "Phase109RecordingGenericLaunchBackend", "Phase109RecordingWindowIdentityProbe", "Phase110RecordingGenericLaunchBackend", "main", "phase109_cli_line", "prepare_phase109_generic_real_launch_candidate", "run_phase109_generic_real_launch_candidate_contract"]  # 修改代码+GenericLaunchBackendMaturity：公开 Phase110 默认后端名称便于审计；如果没有这一行，后续测试和学习备份不容易确认 Phase109 已接入通用后端。
