...
class WindowsComputerUseFullDesktopTaskRouterTests(unittest.TestCase):  # 新增代码+DesktopTaskAcceptance：类段开始，集中冻结 `/computer use --full` 桌面任务路由验收；如果没有这个类，unittest 不会组织这些回归用例。
    ...
    def test_default_agent_full_mode_uses_universal_adapter_with_real_dispatch_shape(self) -> None:  # 修改代码+RealPhysicalFullMode：函数段开始，验证真实 agent 默认 full 路径会进入通用 adapter 并识别真实派发形状；如果没有这个测试，生产路径可能继续停在只观察不动作。
    ...
    # 修改代码+RealPhysicalFullMode：函数段结束，test_default_agent_full_mode_uses_universal_adapter_with_real_dispatch_shape 到此结束；如果没有这个边界说明，代码小白不容易看出默认真实派发接线测试范围。
