...
from learning_agent.core.actionability_state import (  # 修改代码+FreshTargetUserAction：集中导入 actionability 协议接口；如果没有这段导入，收敛器无法同时处理 pending 和用户动作阻断。
    get_last_actionability_block,  # 新增代码+FreshTargetUserAction：导入最近阻断读取函数；如果没有这行代码，旧窗口拒绝后下一轮模型前无法收束。
    get_pending_actionability,  # 修改代码+FreshTargetUserAction：保留 pending 读取函数；如果没有这行代码，真实动作下一步要求会失效。
    pending_actionability_argument_mismatch,  # 修改代码+FreshTargetUserAction：保留 pending 参数门禁；如果没有这行代码，target_ref 或 element_id 错误仍会执行。
    pending_actionability_message,  # 修改代码+FreshTargetUserAction：保留 pending 提示构造；如果没有这行代码，模型不知道 pending 下一步怎么做。
    record_actionability_from_tool_result,  # 修改代码+FreshTargetUserAction：保留工具结果记录入口；如果没有这行代码，工具 marker 无法影响后续轮次。
    should_block_tool_for_pending_actionability,  # 修改代码+FreshTargetUserAction：保留 pending 绕行阻断；如果没有这行代码，模型会继续读日志或写 fallback。
    should_block_tool_for_user_action_required,  # 新增代码+FreshTargetUserAction：导入用户动作阻断判断；如果没有这行代码，旧窗口拒绝后重复 open_application 不会被拦。
    user_action_required_message,  # 新增代码+FreshTargetUserAction：导入用户动作提示构造；如果没有这行代码，收敛器会重复拼脆弱中文提示。
)  # 修改代码+FreshTargetUserAction：actionability 导入结束；如果没有这行代码，Python 导入语法不完整。
...
DESKTOP_STAGE_FINAL_BACKUP_MARKERS = ("next desktop action", "下一步动作", "下一步桌面动作", "desktop_task_incomplete")  # 新增代码+DesktopStageFinalGate：定义未完成桌面任务的文本备份信号；如果没有这行代码，模型把 next action 当 final 时缺最后门禁。
DESKTOP_STAGE_SUCCESS_MARKERS = ("_OK", "desktop_task_completed=true", "完成", "已完成", "成功")  # 新增代码+DesktopStageFinalGate：定义可能宣称成功的文本信号；如果没有这行代码，未完成阶段配成功话术不容易被识别。
DESKTOP_STAGE_USER_REQUEST_MARKERS = ("请", "需要你", "用户", "手动", "关闭", "授权", "确认", "user", "manual", "close", "authorize")  # 新增代码+DesktopStageFinalGate：定义 needs_user 回答必须包含的用户动作提示；如果没有这行代码，needs_user 可能被伪装成成功。
...
@dataclass  # 新增代码+DesktopStageFinalGate：自动生成最终回答门禁决策初始化；如果没有这行代码，返回结构会变成松散 dict。
class FinalAnswerGateDecision:  # 新增代码+DesktopStageFinalGate：类段开始，表示最终回答是否允许输出；如果没有这个类，agent.py 无法区分放行和需要继续执行。
    allow_final_answer: bool  # 新增代码+DesktopStageFinalGate：标记是否允许最终回答；如果没有这行代码，调用方无法知道该输出还是重试。
    reason: str  # 新增代码+DesktopStageFinalGate：保存放行或阻断原因；如果没有这行代码，测试和日志无法解释门禁判断。
    injected_message: dict[str, str] | None = None  # 新增代码+DesktopStageFinalGate：保存给模型的补救提示；如果没有这行代码，阻断后下一轮模型不知道该继续工具还是请求用户。

    def to_dict(self) -> dict[str, Any]:  # 新增代码+DesktopStageFinalGate：函数段开始，把最终门禁决策转成 JSON；如果没有这段函数，事件 payload 需要重复拼字段。
        return {"allow_final_answer": self.allow_final_answer, "reason": self.reason, "injected_message": dict(self.injected_message) if self.injected_message is not None else None}  # 新增代码+DesktopStageFinalGate：返回稳定字段；如果没有这行代码，调试事件无法读取完整门禁结果。
    # 新增代码+DesktopStageFinalGate：函数段结束，FinalAnswerGateDecision.to_dict 到此结束；如果没有这个边界说明，初学者不容易看出序列化范围。
...
def _desktop_stage_bool(value: Any) -> bool:  # 新增代码+DesktopStageFinalGate：函数段开始，宽松解析阶段证据里的布尔值；如果没有这段函数，字符串 true/false 会被误判。
    if isinstance(value, str):  # 新增代码+DesktopStageFinalGate：处理 JSON/token 文本中的布尔值；如果没有这行代码，"false" 会因为非空字符串被当真。
        return value.strip().lower() in {"1", "true", "yes", "y"}  # 新增代码+DesktopStageFinalGate：只把明确真值文本视为真；如果没有这行代码，false 字符串会破坏门禁。
    return bool(value)  # 新增代码+DesktopStageFinalGate：其它类型使用 Python 真值；如果没有这行代码，数字和原生 bool 无法复用。
# 新增代码+DesktopStageFinalGate：函数段结束，_desktop_stage_bool 到此结束；如果没有这个边界说明，初学者不容易看出布尔解析范围。


def _desktop_stage_int(value: Any) -> int:  # 新增代码+DesktopStageFinalGate：函数段开始，安全读取阶段计数；如果没有这段函数，坏计数字段会让 final gate 崩溃。
    try:  # 新增代码+DesktopStageFinalGate：尝试转成整数；如果没有这行代码，字符串数字无法兼容。
        return int(value)  # 新增代码+DesktopStageFinalGate：返回整数值；如果没有这行代码，调用方无法比较 stage_count。
    except (TypeError, ValueError):  # 新增代码+DesktopStageFinalGate：捕获空值和非数字；如果没有这行代码，坏工具结果会中断 agent。
        return 0  # 新增代码+DesktopStageFinalGate：坏值按 0 处理；如果没有这行代码，缺字段无法保守处理。
# 新增代码+DesktopStageFinalGate：函数段结束，_desktop_stage_int 到此结束；如果没有这个边界说明，初学者不容易看出计数兜底范围。


def _extract_desktop_stage_evidence(payload: Any) -> dict[str, Any]:  # 新增代码+DesktopStageFinalGate：函数段开始，从任意工具结果里递归寻找阶段证据；如果没有这段函数，final gate 只能依赖脆弱文本。
    if isinstance(payload, dict):  # 新增代码+DesktopStageFinalGate：处理字典工具结果；如果没有这行代码，结构化 JSON 无法被识别。
        if any(key in payload for key in ("desktop_task_completed", "desktop_task_incomplete", "stage_count", "completed_stage_count", "needs_user")):  # 新增代码+DesktopStageFinalGate：命中阶段证据字段即返回；如果没有这行代码，门禁找不到新 stage runtime 的事实。
            return dict(payload)  # 新增代码+DesktopStageFinalGate：复制返回证据；如果没有这行代码，后续修改可能污染原工具结果。
        for value in payload.values():  # 新增代码+DesktopStageFinalGate：递归检查嵌套字典/list；如果没有这行代码，adapter_report 里的内层 loop_report 会被漏掉。
            nested = _extract_desktop_stage_evidence(value)  # 新增代码+DesktopStageFinalGate：读取嵌套证据；如果没有这行代码，递归不会推进。
            if nested:  # 新增代码+DesktopStageFinalGate：找到证据时停止；如果没有这行代码，后续值可能覆盖有效证据。
                return nested  # 新增代码+DesktopStageFinalGate：返回嵌套证据；如果没有这行代码，调用方拿不到结果。
    if isinstance(payload, list):  # 新增代码+DesktopStageFinalGate：处理列表工具结果；如果没有这行代码，多工具结果数组无法扫描。
        for item in payload:  # 新增代码+DesktopStageFinalGate：逐项扫描列表；如果没有这行代码，列表内部字典会被漏掉。
            nested = _extract_desktop_stage_evidence(item)  # 新增代码+DesktopStageFinalGate：递归读取列表项；如果没有这行代码，递归不会进入列表。
            if nested:  # 新增代码+DesktopStageFinalGate：找到证据时停止；如果没有这行代码，有效证据可能继续被忽略。
                return nested  # 新增代码+DesktopStageFinalGate：返回列表内证据；如果没有这行代码，调用方拿不到结果。
    return {}  # 新增代码+DesktopStageFinalGate：没有证据时返回空字典；如果没有这行代码，调用方需要处理 None。
# 新增代码+DesktopStageFinalGate：函数段结束，_extract_desktop_stage_evidence 到此结束；如果没有这个边界说明，初学者不容易看出证据提取范围。


def _desktop_stage_evidence_from_runtime(state: dict[str, Any]) -> dict[str, Any]:  # 新增代码+DesktopStageFinalGate：函数段开始，从 runtime_state 读取最近阶段证据；如果没有这段函数，final gate 入口会散落多个 key。
    evidence = state.get("computer_use_stage_evidence", {})  # 新增代码+DesktopStageFinalGate：优先读取 record_tool_result 沉淀的证据；如果没有这行代码，最终回答前看不到最近工具事实。
    return dict(evidence) if isinstance(evidence, dict) else {}  # 新增代码+DesktopStageFinalGate：返回字典证据或空字典；如果没有这行代码，坏状态会让门禁报错。
# 新增代码+DesktopStageFinalGate：函数段结束，_desktop_stage_evidence_from_runtime 到此结束；如果没有这个边界说明，初学者不容易看出状态读取范围。


def assess_final_answer_for_desktop_task(answer_text: str, runtime_state: dict[str, Any] | None = None) -> FinalAnswerGateDecision:  # 新增代码+DesktopStageFinalGate：函数段开始，判断 Computer Use 最终回答是否允许输出；如果没有这段函数，未完成桌面任务会继续被当 final。
    state = _runtime_dict(runtime_state)  # 新增代码+DesktopStageFinalGate：规范化运行时状态；如果没有这行代码，None 状态会崩溃。
    evidence = _desktop_stage_evidence_from_runtime(state)  # 新增代码+DesktopStageFinalGate：读取结构化阶段证据；如果没有这行代码，门禁只能看最终文本。
    answer_lower = str(answer_text or "").lower()  # 新增代码+DesktopStageFinalGate：准备小写最终回答文本；如果没有这行代码，英文备份信号大小写会漏判。
    backup_incomplete = any(marker.lower() in answer_lower for marker in DESKTOP_STAGE_FINAL_BACKUP_MARKERS)  # 新增代码+DesktopStageFinalGate：检查 next action/incomplete 文本信号；如果没有这行代码，模型把待执行语义当 final 无法阻断。
    if not evidence:  # 新增代码+DesktopStageFinalGate：没有阶段证据时不误伤普通任务；如果没有这行代码，非 Computer Use 回答可能被错误拦截。
        return FinalAnswerGateDecision(not backup_incomplete, "no_stage_evidence" if not backup_incomplete else "desktop_task_incomplete_text_marker", _system_message("收束提醒：最终回答包含未完成桌面动作信号，请继续调用 Computer Use 工具或明确说明 desktop_task_incomplete。") if backup_incomplete else None)  # 新增代码+DesktopStageFinalGate：无证据时只拦截明显未完成文本；如果没有这行代码，普通聊天会被阶段门禁污染。
    stage_count = _desktop_stage_int(evidence.get("stage_count", 0))  # 新增代码+DesktopStageFinalGate：读取阶段总数；如果没有这行代码，无法比较完成阶段数量。
    completed_count = _desktop_stage_int(evidence.get("completed_stage_count", 0))  # 新增代码+DesktopStageFinalGate：读取已完成阶段数；如果没有这行代码，虚假成功无法识别。
    task_completed = _desktop_stage_bool(evidence.get("desktop_task_completed", False))  # 新增代码+DesktopStageFinalGate：读取任务完成布尔值；如果没有这行代码，结构化完成状态不会生效。
    task_incomplete = _desktop_stage_bool(evidence.get("desktop_task_incomplete", False))  # 新增代码+DesktopStageFinalGate：读取任务未完成布尔值；如果没有这行代码，结构化未完成状态不会生效。
    needs_user = _desktop_stage_bool(evidence.get("needs_user", False))  # 新增代码+DesktopStageFinalGate：读取是否需要用户动作；如果没有这行代码，旧窗口/授权类阻断无法安全 final。
    success_claimed = any(marker.lower() in answer_lower for marker in DESKTOP_STAGE_SUCCESS_MARKERS)  # 新增代码+DesktopStageFinalGate：检查最终回答是否宣称成功；如果没有这行代码，未完成阶段配成功 marker 会漏过。
    if task_completed and (stage_count == 0 or completed_count >= stage_count):  # 新增代码+DesktopStageFinalGate：结构化完成且阶段数一致时放行；如果没有这行代码，成功任务会被错误阻断。
        return FinalAnswerGateDecision(True, "desktop_task_completed")  # 新增代码+DesktopStageFinalGate：返回完成放行；如果没有这行代码，调用方不知道可正常 final。
    if needs_user and any(marker.lower() in answer_lower for marker in DESKTOP_STAGE_USER_REQUEST_MARKERS):  # 新增代码+DesktopStageFinalGate：needs_user 只有回答请求用户动作时放行；如果没有这行代码，授权/关闭窗口场景会被迫无限工具循环。
        return FinalAnswerGateDecision(True, "needs_user_answer_requests_user_action")  # 新增代码+DesktopStageFinalGate：返回用户动作放行；如果没有这行代码，用户介入说明无法作为安全最终回答。
    if task_incomplete or backup_incomplete or (stage_count > 0 and completed_count < stage_count) or success_claimed:  # 新增代码+DesktopStageFinalGate：未完成、next action、阶段不足或虚假成功都阻断；如果没有这行代码，复杂桌面任务会早停。
        message = _system_message("收束提醒：Computer Use 阶段任务尚未完成，不能输出成功最终回答。请继续调用 Computer Use 工具完成剩余阶段；如果阶段结果是 needs_user，请明确请求用户关闭/授权目标窗口。")  # 新增代码+DesktopStageFinalGate：构造下一轮模型提示；如果没有这行代码，阻断后模型不知道该继续执行还是请求用户。
        return FinalAnswerGateDecision(False, "desktop_task_incomplete", message)  # 新增代码+DesktopStageFinalGate：返回未完成阻断；如果没有这行代码，final gate 不会生效。
    return FinalAnswerGateDecision(True, "stage_evidence_non_blocking")  # 新增代码+DesktopStageFinalGate：缺少阻断条件时放行；如果没有这行代码，边界情况无法返回。
# 新增代码+DesktopStageFinalGate：函数段结束，assess_final_answer_for_desktop_task 到此结束；如果没有这个边界说明，初学者不容易看出最终门禁范围。
...
def assess_before_model(task_state: TaskState, recent_messages: list[dict[str, Any]], runtime_state: dict[str, Any] | None = None) -> ConvergenceAssessment:  # 新增代码+ConvergenceController: 函数段开始，模型调用前评估是否需要收束提醒；如果没有这段函数，主循环无法提前注入任务状态。
    ...
    last_actionability_block = get_last_actionability_block(state)  # 新增代码+FreshTargetUserAction：读取最近一次用户动作阻断；如果没有这行代码，旧窗口拒绝后模型前不会被提醒停止工具。
    if last_actionability_block.get("block_class") == "user_action_required" and not _user_asked_for_debug_artifacts(task_state):  # 新增代码+FreshTargetUserAction：非调试任务遇到用户动作要求时要求最终回答；如果没有这行代码，模型会继续计划 open_application。
        message = _system_message(f"收束提醒：FreshTarget 已经拒绝默认接管旧窗口，请不要继续调用 Computer Use 工具，请直接最终回答用户关闭旧窗口或明确授权已有窗口。\\n{user_action_required_message(last_actionability_block)}\\n{task_state.to_model_summary()}")  # 新增代码+FreshTargetUserAction：注入停止工具并答复用户的提示；如果没有这行代码，模型不知道下一步应解释而不是重试。
        return ConvergenceAssessment(True, message, "fresh_target_user_action_requires_final_answer")  # 新增代码+FreshTargetUserAction：返回用户动作收束评估；如果没有这行代码，主循环不会把提醒传给模型。
...
def decide_tool_call(tool_call: Any, task_state: TaskState, runtime_state: dict[str, Any] | None = None) -> ToolCallDecision:  # 新增代码+ConvergenceController: 函数段开始，判断一个工具调用是否真实执行；如果没有这段函数，重复工具无法被收束。
    ...
    last_actionability_block = get_last_actionability_block(state)  # 新增代码+FreshTargetUserAction：读取最近一次用户动作阻断；如果没有这行代码，重复 launch 无法被旧窗口拒绝状态拦截。
    user_action_block_reason = should_block_tool_for_user_action_required(_tool_name(tool_call), _tool_arguments(tool_call), last_actionability_block)  # 新增代码+FreshTargetUserAction：判断本次工具是否违反用户动作要求；如果没有这行代码，模型会继续 open_application 或鼠标键盘动作。
    if user_action_block_reason and not _user_asked_for_debug_artifacts(task_state):  # 新增代码+FreshTargetUserAction：非调试任务中发现违规工具时硬阻断；如果没有这行代码，旧窗口状态下仍会真实执行工具。
        output = f"{user_action_required_message(last_actionability_block)}\\n当前工具调用已被阻断：{user_action_block_reason}\\n{task_state.to_model_summary()}"  # 新增代码+FreshTargetUserAction：构造合成工具输出闭合 tool_call；如果没有这行代码，模型收不到为什么被拦和该如何回答。
        message = _system_message("收束提醒：FreshTarget 已经要求用户关闭目标软件或明确授权已有窗口。请不要继续调用 Computer Use 工具，请直接最终回答用户。")  # 新增代码+FreshTargetUserAction：构造模型可见的终止提示；如果没有这行代码，模型可能继续寻找其他工具绕行。
        return ToolCallDecision(False, output, message, "fresh_target_user_action_required", signature)  # 新增代码+FreshTargetUserAction：返回用户动作阻断决策；如果没有这行代码，主循环会执行危险重复工具。
...
def record_tool_result(tool_call: Any, output_text: str, task_state: TaskState, runtime_state: dict[str, Any] | None = None) -> None:  # 新增代码+ConvergenceController: 函数段开始，记录工具结果用于后续无进展判断；如果没有这段函数，收束器只能看调用不能看结果。
    ...
    try:  # 新增代码+DesktopStageFinalGate：尝试把工具输出解析成 JSON；如果没有这行代码，stage loop 的结构化结果无法进入 final gate。
        parsed_output = json.loads(str(output_text or ""))  # 新增代码+DesktopStageFinalGate：解析工具输出文本；如果没有这行代码，只能依赖脆弱字符串匹配。
    except json.JSONDecodeError:  # 新增代码+DesktopStageFinalGate：处理普通文本工具结果；如果没有这行代码，非 JSON 输出会中断记录流程。
        parsed_output = {}  # 新增代码+DesktopStageFinalGate：普通文本按无结构化证据处理；如果没有这行代码，后续提取函数会收到未定义变量。
    stage_evidence = _extract_desktop_stage_evidence(parsed_output)  # 新增代码+DesktopStageFinalGate：从工具结果里寻找阶段证据；如果没有这行代码，最终回答前读不到桌面任务完成状态。
    if stage_evidence:  # 新增代码+DesktopStageFinalGate：只有找到阶段证据才写入状态；如果没有这行代码，空结果会覆盖已有有效证据。
        state["computer_use_stage_evidence"] = stage_evidence  # 新增代码+DesktopStageFinalGate：保存最近阶段证据；如果没有这行代码，assess_final_answer_for_desktop_task 无法工作。
...
__all__ = ["ConvergenceAssessment", "FinalAnswerGateDecision", "ToolCallDecision", "assess_before_model", "assess_final_answer_for_desktop_task", "decide_tool_call", "record_tool_result"]  # 修改代码+DesktopStageFinalGate：明确公开最终回答门禁接口；如果没有这行代码，agent.py 和测试无法稳定导入新门禁。
