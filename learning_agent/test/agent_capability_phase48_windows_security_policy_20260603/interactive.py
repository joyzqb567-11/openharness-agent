...
try:  # 新增代码+AppSplit: 优先按包路径导入轮次格式化和验收事件 helper；若没有这行代码，包运行模式下交互层无法复用通用能力。
    ...
    from learning_agent.computer_use.session_runtime import WindowsComputerUseSessionRuntime, format_phase40_runtime_action  # 新增代码+Phase40WindowsAbortCleanup: 导入 Phase40 会话运行时和终端格式化器；如果没有这行代码，/computer 无法显示 abort/cleanup 通知层。
    ...
    if error.name not in {"learning_agent", "learning_agent.app", "learning_agent.app.chrome_status_renderer", "learning_agent.app.status_renderer", "learning_agent.browser_extension_host", "learning_agent.browser_extension_host.bridge_server", "learning_agent.browser_extension_host.manifest_installer", "learning_agent.computer_use", "learning_agent.computer_use.approval", "learning_agent.computer_use.lock", "learning_agent.computer_use.native_diagnostics", "learning_agent.computer_use.security_policy", "learning_agent.computer_use.session_runtime", "learning_agent.core", "learning_agent.core.config", "learning_agent.observability", "learning_agent.observability.acceptance_events", "learning_agent.observability.run_records", "learning_agent.runtime", "learning_agent.runtime.command_queue", "learning_agent.runtime.status_snapshot", "learning_agent.sdk", "learning_agent.sdk.status"}:  # 修改代码+Phase48WindowsSecurityPolicy: 允许 computer_use.security_policy 在脚本模式 fallback；若没有这行代码，bat 入口下 /computer status 可能导入失败。
    ...
    from computer_use.session_runtime import WindowsComputerUseSessionRuntime, format_phase40_runtime_action  # 新增代码+Phase40WindowsAbortCleanup: 脚本模式导入 Phase40 会话运行时；如果没有这行代码，双击 bat 后 /computer cleanup 不可用。
...
def run_computer_terminal_command(workspace: Path, user_input: str) -> str:  # 新增代码+Phase31ComputerUseLockAbortEvidence: 函数段开始，执行 `/computer` 子命令；如果没有这段函数，真实用户无法在终端查看或触发桌面急停。
    ...
    if subcommand in {"status", ""}:  # 新增代码+Phase31ComputerUseLockAbortEvidence: 识别状态查询；如果没有这行代码，用户无法只读查看 Computer Use 状态。
        approval_lines = WindowsComputerUseApprovalModel(security_policy=WindowsComputerUseSecurityPolicy()).terminal_status_lines()  # 修改代码+Phase48WindowsSecurityPolicy: 生成带 Phase48 安全策略的审批终端摘要；如果没有这行代码，用户看不到 grant_classes 和高风险默认拒绝。
        runtime_lines = runtime.terminal_status_lines()  # 新增代码+Phase40WindowsAbortCleanup: 生成 runtime 摘要；如果没有这行代码，/computer status 看不到 notification 和 cleanup 状态。
        ...
        capability_lines = format_phase43_capability_matrix_lines(capability_report["matrix"])  # 新增代码+Phase43WindowsNativeCapabilityMatrix: 把矩阵报告渲染成终端行；如果没有这行代码，用户只能读复杂 JSON。
        return _format_computer_lock_action("status", lock_manager.status()) + "\n".join(approval_lines) + "\n" + "\n".join(runtime_lines) + "\n" + "\n".join(capability_lines) + "\n"  # 修改代码+Phase43WindowsNativeCapabilityMatrix: 返回锁、审批、runtime 和 native 能力矩阵；如果没有这行代码，状态 UI 会缺少 Phase43 原生能力总览。
    ...
    if subcommand == "cleanup":  # 新增代码+Phase40WindowsAbortCleanup: 识别 turn cleanup 命令；如果没有这行代码，用户无法从终端清理残留桌面锁。
        ...
        return format_phase40_runtime_action("cleanup", cleanup_result)  # 新增代码+Phase40WindowsAbortCleanup: 返回 cleanup runtime 面板；如果没有这行代码，真实终端看不到 cleanup_completed 和 lock_released。
    ...
    return "Computer Action\n- 不支持的 /computer 子命令。\n- 可用命令：/computer、/computer status、/computer abort <reason>、/computer clear-abort、/computer cleanup [session_id]、/computer notifications、/computer release [session_id]\n"  # 修改代码+Phase40WindowsAbortCleanup: 返回包含 cleanup/notifications 的用法；如果没有这行代码，输错 Phase40 命令时用户不知道怎么修正。
