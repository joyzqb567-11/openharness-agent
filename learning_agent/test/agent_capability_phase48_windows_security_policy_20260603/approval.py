...
try:  # 新增代码+Phase48WindowsSecurityPolicy: 优先按包路径导入 Phase48 细粒度安全策略；如果没有这行代码，approval 无法可选接入 observe/action/system_key/clipboard grant。
    from learning_agent.computer_use.security_policy import WindowsComputerUseSecurityPolicy  # 新增代码+Phase48WindowsSecurityPolicy: 导入安全策略类用于审批模型注入；如果没有这行代码，Phase48 策略只能孤立自检不能进入 controller 链路。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.security_policy"}:  # 新增代码+Phase48WindowsSecurityPolicy: 只允许包路径缺失时 fallback；如果没有这行代码，security_policy 内部真实 bug 会被误吞。
        raise  # 新增代码+Phase48WindowsSecurityPolicy: 重新抛出非路径类导入错误；如果没有这行代码，排查策略模块问题会很困难。
    from computer_use.security_policy import WindowsComputerUseSecurityPolicy  # 新增代码+Phase48WindowsSecurityPolicy: 脚本模式从本地包导入安全策略；如果没有这行代码，bat 入口无法加载 Phase48 审批策略。
