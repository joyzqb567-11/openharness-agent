  ...
  return ( // 新增代码+DesktopSettingsPanel：返回设置面板结构；如果没有这行，组件不会输出 UI。
    <section className="settings-panel" aria-label="设置"> {/* 新增代码+DesktopSettingsPanel：设置面板语义容器；如果没有这一层，读屏器无法识别设置区域。 */}
      ...
      <div className="settings-section"> {/* 新增代码+DesktopSettingsPanel：诊断目录区域容器；如果没有这一层，复制路径按钮缺少分组。 */}
        ...
        <button className="settings-copy-row" type="button" onClick={() => { copyText(model.logPath); }} title="复制日志目录"> {/* 新增代码+DesktopSettingsPanel：日志目录复制按钮；如果没有这行，用户无法一键复制日志路径。 */}
        ...
        <button className="settings-copy-row" type="button" onClick={() => { copyText(model.evidencePath); }} title="复制证据目录"> {/* 新增代码+DesktopSettingsPanel：证据目录复制按钮；如果没有这行，用户无法一键复制验收材料路径。 */}
