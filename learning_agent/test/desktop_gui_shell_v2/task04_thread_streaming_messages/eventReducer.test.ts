...
import { reduceGuiEventToThreadActions } from "../src/state/eventReducer"; // 新增代码+DesktopEventReducerTest：导入待实现的 V2 事件 reducer；如果没有这一行，测试无法驱动真实实现。
...
function makeEvent(eventType: string, payload: Record<string, unknown>, turnId = "turn_a"): StatusEvent { // 新增代码+DesktopEventReducerTest：helper 段开始，构造可复用状态事件；如果没有这段，每个测试都会重复大量样板字段。
  return { sequence: 1, event_type: eventType, session_id: "session_a", run_id: "run_a", turn_id: turnId, payload }; // 新增代码+DesktopEventReducerTest：返回最小合法 StatusEvent；如果没有这一行，eventReducer 输入不稳定。
