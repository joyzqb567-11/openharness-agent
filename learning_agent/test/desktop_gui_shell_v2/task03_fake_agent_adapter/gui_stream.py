...
_STATUS_TO_GUI_V2_KIND = {  # 新增代码+GuiV2Stream：映射段开始，把当前 status event 转成 V2 GUI kind；如果没有这段，前端 V2 状态机无法消费旧事件名。
    ...
    "turn_started": "turn_started",  # 新增代码+GuiAgentAdapter：原样透传 adapter 的 turn_started；如果没有这一项，V2 原生启动事件会被降级成 heartbeat。
    "message_delta": "message_delta",  # 新增代码+GuiAgentAdapter：原样透传 adapter 的 message_delta；如果没有这一项，流式文本会在 SSE/fallback 中消失。
    "message_completed": "message_completed",  # 新增代码+GuiAgentAdapter：原样透传 adapter 的 message_completed；如果没有这一项，V2 原生完成事件会被降级成 heartbeat。
    "tool_started": "tool_started",  # 新增代码+GuiAgentAdapter：原样透传 adapter 的 tool_started；如果没有这一项，TracePanel 无法消费真实工具起点。
    "tool_finished": "tool_finished",  # 新增代码+GuiAgentAdapter：原样透传 adapter 的 tool_finished；如果没有这一项，TracePanel 无法消费真实工具结果。
    "permission_requested": "permission_requested",  # 新增代码+GuiAgentAdapter：原样透传 adapter 的 permission_requested；如果没有这一项，真实 adapter 权限请求会丢失。
    "safety_refusal": "safety_refusal",  # 新增代码+GuiAgentAdapter：原样透传 adapter 的 safety_refusal；如果没有这一项，安全拒绝无法成为一等消息。
    "turn_failed": "turn_failed",  # 新增代码+GuiAgentAdapter：原样透传 adapter 的 turn_failed；如果没有这一项，adapter 失败会被降级成 heartbeat。
    "turn_cancelled": "turn_cancelled",  # 新增代码+GuiAgentAdapter：原样透传 adapter 的 turn_cancelled；如果没有这一项，adapter 取消会被降级成 heartbeat。
    "heartbeat": "heartbeat",  # 新增代码+GuiAgentAdapter：原样透传 adapter 的 heartbeat；如果没有这一项，adapter 保活会被当未知事件处理。
