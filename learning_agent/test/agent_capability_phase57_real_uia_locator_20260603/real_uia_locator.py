...
def _is_phase57_safe_window(window: dict[str, Any]) -> tuple[bool, str]:  # 新增代码+Phase57RealUiaLocator: 函数段开始，判断窗口是否允许读取 UIA 树；如果没有这段函数，Phase57 可能误读终端、Codex 或认证窗口。
    ...
    if "learningagent-phase57" in title:  # 修改代码+Phase57RealUiaLocator: 优先允许本阶段自建安全窗口标题；如果没有这行代码，WinForms 安全窗口的 PowerShell 进程名会被误判成终端风险。
        return True, "目标是 Phase57 自有安全窗口。"  # 修改代码+Phase57RealUiaLocator: 返回安全原因；如果没有这行代码，审计无法说明为什么允许 PowerShell 承载的测试窗体。
...
class Phase57DedicatedSafeWindowLauncher:  # 修改代码+Phase57RealUiaLocator: 类段开始，启动只属于 Phase57 的独立 WinForms 安全窗口；如果没有这个类，真实 UIA smoke 可能误读用户已有 Notepad 标签页。
    def __init__(self, marker_text: str = "LearningAgent Phase57 real UIA locator smoke") -> None:  # 新增代码+Phase57RealUiaLocator: 函数段开始，初始化安全窗口内容；如果没有这段函数，临时文件和进程状态没有归属。
    ...
    def launch(self) -> dict[str, Any]:  # 修改代码+Phase57RealUiaLocator: 函数段开始，创建临时 PowerShell WinForms 脚本并启动独立安全窗体；如果没有这段函数，真实 smoke 没有隔离 UIA 目标。
        ...
        title = f"LearningAgent-Phase57-RealUiaLocatorSmoke-{int(time.time() * 1000)}"  # 修改代码+Phase57RealUiaLocator: 设置每次唯一的独立安全窗体标题；如果没有这行代码，旧 Notepad 或旧测试窗体可能被误匹配。
