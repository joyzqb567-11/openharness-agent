...
def query_windows_app_inventory(query: Any = "", candidates: list[dict[str, Any]] | None = None, include_common: bool = True, max_count: int = 10) -> dict[str, Any]:  # 新增代码+ComputerDiscoverTool：函数段开始，按自然语言查询 Windows 应用候选；如果没有这段函数，Computer Use 缺少类似 ClaudeCode appNames 的 discover 工具入口。
    ...
    try:  # 新增代码+ComputerDiscoverTool：尝试解析返回数量；如果没有这一行，模型传入字符串 max_results 可能让工具崩溃。
        result_limit = max(1, min(int(max_count or 10), APP_INVENTORY_MAX_COUNT))  # 新增代码+ComputerDiscoverTool：把数量限制在 1 到总上限之间；如果没有这一行，结果可能为空或撑大上下文。
    except (TypeError, ValueError):  # 新增代码+ComputerDiscoverTool：处理非法数量；如果没有这一行，坏参数会变成 Python traceback。
        result_limit = 10  # 新增代码+ComputerDiscoverTool：非法数量回退到 10；如果没有这一行，模型需要额外修正简单参数错误。
    catalog = build_windows_app_inventory(candidates=candidates, include_common=include_common, max_count=APP_INVENTORY_MAX_COUNT)  # 新增代码+ComputerDiscoverTool：从统一 inventory 读取清洗后的候选；如果没有这一行，discover 会绕开正式应用清单。
    ...
    public_candidates = [_inventory_public_candidate(entry, score) for score, entry in scored[:result_limit]]  # 新增代码+ComputerDiscoverTool：转换并截断为模型可见候选；如果没有这一行，输出可能泄露内部字段或过长。
    return {"ok": True, "tool": "computer_discover", "marker": "PHASE123_COMPUTER_DISCOVER_READY", "model": "phase123_windows_app_inventory_query", "query": query_text, "normalized_query": normalized_query, "max_results": result_limit, "result_count": len(public_candidates), "candidates": public_candidates, "not_hard_whitelist": True, "next_step": "Use app_name with computer_action(action=launch_app), then continue observe-plan-act with screenshots and tool results."}  # 新增代码+ComputerDiscoverTool：返回结构化 discover 报告；如果没有这一行，模型无法稳定读取候选并进入后续真实主循环。
# 新增代码+ComputerDiscoverTool：函数段结束，query_windows_app_inventory 到此结束；如果没有这个边界说明，用户不容易看出 discover 查询入口范围。
...
__all__ = ["APP_INVENTORY_MAX_COUNT", "build_windows_app_inventory", "discover_windows_app_inventory_sources", "format_windows_app_inventory_for_model", "query_windows_app_inventory", "resolve_windows_app_name_hint", "sanitize_inventory_display_name"]  # 修改代码+ComputerDiscoverTool：限定公开 API 并暴露 discover 查询入口；如果没有这一行，通配导入可能找不到 computer_discover 需要的查询能力。
