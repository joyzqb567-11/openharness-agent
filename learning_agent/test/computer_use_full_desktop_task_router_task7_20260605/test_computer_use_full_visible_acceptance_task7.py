...
class ComputerUseFullVisibleAcceptanceTask7Tests(unittest.TestCase):  # 新增代码+DesktopTaskVisibleAcceptance：类段开始，集中验证 Task7 的场景和 controller 合同；如果没有这个类，unittest 不会组织这些门禁。
    def test_strict_scenario_uses_dynamic_full_mode_prompt_lines(self) -> None:  # 新增代码+DesktopTaskVisibleAcceptance：函数段开始，验证严格场景逐行输入 full 请求、动态确认、自然语言任务和 stop；如果没有这个测试，场景可能又退回硬编码 token。
        ...
        self.assertEqual(scenario["prompt_lines"][0], "/computer use --full")  # 新增代码+DesktopTaskVisibleAcceptance：断言第一行是真实请求 full mode；如果没有这一行，token 捕获没有事件来源。
        self.assertEqual(scenario["prompt_lines"][1], "/computer use --full-confirm ${confirmation_token}")  # 新增代码+DesktopTaskVisibleAcceptance：断言第二行使用动态 token 占位符；如果没有这一行，场景可能硬编码过期 token。
        self.assertEqual(scenario["prompt_lines"][2], "请使用本地电脑的画图软件画一个皮卡丘。")  # 新增代码+DesktopTaskVisibleAcceptance：断言第三行是真实用户自然语言桌面任务；如果没有这一行，验收可能只开 full mode 不执行任务。
        self.assertEqual(scenario["prompt_lines"][3], "/computer stop")  # 新增代码+DesktopTaskVisibleAcceptance：断言第四行执行清理；如果没有这一行，full mode 可能在验收后残留。
        self.assertEqual(scenario["capture_event_payload_regex"]["confirmation_token"], "confirmation_token=(FULL-[A-F0-9]+)")  # 新增代码+DesktopTaskVisibleAcceptance：断言 token 捕获规则稳定；如果没有这一行，controller 无法把本轮 token 放进 confirm 命令。
    ...
    # 新增代码+DesktopTaskVisibleAcceptance：函数段结束，test_strict_scenario_uses_dynamic_full_mode_prompt_lines 到此结束；如果没有这个边界说明，代码小白不容易看出严格场景测试范围。
    ...
    def test_controller_supports_dynamic_prompt_capture_and_regex_checks(self) -> None:  # 新增代码+DesktopTaskVisibleAcceptance：函数段开始，验证 controller 具备动态变量捕获、多轮输入和正则断言能力；如果没有这个测试，PowerShell 脚本可能被改回单 prompt。
        ...
        self.assertIn("Resolve-PromptLineVariables", controller_text)  # 新增代码+DesktopTaskVisibleAcceptance：断言 controller 有占位符替换函数；如果没有这一行，full-confirm 会收到未替换文本。
