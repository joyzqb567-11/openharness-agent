...
def _run_phase106_interactive_full_launch(workspace: Path, mode_sessions: ComputerUseModeSessionStore, target_app: str, real_launch: bool | None = None, allow_real_gate: bool | None = None, real_launch_candidate: Any | None = None) -> dict[str, Any]:  # 新增代码+Phase106InteractiveFullLaunch：函数段开始，执行 `/computer launch notepad` 的 Phase106 桥接；如果没有这段函数，用户命令无法接到 Phase105 受控启动链。
    target = str(target_app or "notepad").strip().lower() or "notepad"  # 新增代码+Phase106InteractiveFullLaunch：规范化用户输入的目标应用；如果没有这行代码，空参数或大小写会让命令判断不稳定。
    if target == "记事本":  # 新增代码+Phase106InteractiveFullLaunch：兼容中文 Notepad 叫法；如果没有这行代码，中文用户输入会被当成未知应用。
        target = "notepad"  # 新增代码+Phase106InteractiveFullLaunch：把中文目标映射为安全内部名；如果没有这行代码，后续 Phase105 notepad-only 判断无法通过。
    if target != "notepad":  # 新增代码+Phase106InteractiveFullLaunch：Phase106 只允许 Notepad 受控样本；如果没有这行代码，用户命令会过早扩张成任意应用控制。
        return {"marker": PHASE106_INTERACTIVE_FULL_LAUNCH_MARKER, "ok_token": PHASE106_INTERACTIVE_FULL_LAUNCH_OK_TOKEN, "passed": False, "target_app": target, "decision": "phase106_notepad_only_refused", "real_action_blocked_decision": "target_app_not_supported", "full_mode_session_used": False, "controlled_launch_candidate_ready": False, "controlled_real_launch_gate_passed": False, "real_full_launch_attempted": False, "visible_window_verified": False, "cleanup_completed": False, "verified_window_cleanup_completed": False, "residual_owned_process": False, "real_desktop_touched": False, "uncontrolled_actions_expanded": False}  # 新增代码+Phase106InteractiveFullLaunch：返回零副作用拒绝报告；如果没有这行代码，非 Notepad 请求可能触碰桌面。
