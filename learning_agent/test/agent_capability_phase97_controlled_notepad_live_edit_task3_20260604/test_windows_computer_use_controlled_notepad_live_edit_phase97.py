...
class Phase97WindowsNotepadLiveEditDriverTests(unittest.TestCase):  # 新增代码+Phase97ControlledNotepadLiveEdit：类段开始，专门验证 Task3 driver 安全边界；如果没有这个类，合同测试无法证明真实驱动行为。
    ...
    def test_driver_rechecks_target_before_save_and_stops_after_text_input(self) -> None:  # 新增代码+Phase97ControlledNotepadLiveEdit：函数段开始，验证保存前复核失败不再发送保存快捷键；如果没有这个测试，Ctrl+S 可能发到错误窗口。
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:  # 新增代码+Phase97ControlledNotepadLiveEdit：创建隔离目录；如果没有这一行，测试产物可能污染项目。
            report = self.make_driver([self.safe_window(), self.safe_window(), self.unsafe_window()], sender).run(run_root=Path(temporary_directory), expected_text="safe text", target_file=Path(temporary_directory) / "phase97_controlled_notepad_live_edit.txt")  # 新增代码+Phase97ControlledNotepadLiveEdit：运行保存前漂移场景；如果没有这一行，保存前复核没有事实来源。
        ...
        self.assertIn("unicode_text", event_types)  # 新增代码+Phase97ControlledNotepadLiveEdit：确认受控文本输入已经发送；如果没有这一行，测试不能证明停止点发生在输入之后。
    ...
    def test_driver_positive_path_sends_supported_save_events_and_sanitizes_report(self) -> None:  # 新增代码+Phase97ControlledNotepadLiveEdit：函数段开始，验证成功路径发送文本、支持形状保存事件、调用 verifier 并脱敏；如果没有这个测试，Task3 正向行为可能只有失败边界被覆盖。
        ...
        self.assertIn("unicode_text", event_types)  # 新增代码+Phase97ControlledNotepadLiveEdit：确认发送了受控文本事件；如果没有这一行，driver 可能只保存不输入。
    ...
    def test_real_low_level_sender_dispatches_phase97_ctrl_s_shape(self) -> None:  # 修改代码+Phase97ControlledNotepadLiveEdit：函数段开始，验证真实 sender 已支持 Phase97 的保存事件形状；如果没有这个测试，真实路径可能只在 fake sender 里成功。
        ...
        result = sender.send_low_level([{"type": "set_foreground", "hwnd": 9701}, {"type": "unicode_text", "text": "x"}, {"type": "key_down", "key": "ctrl"}, {"type": "key_down", "key": "s"}, {"type": "key_up", "key": "s"}, {"type": "key_up", "key": "ctrl"}])  # 修改代码+Phase97ControlledNotepadLiveEdit：发送 Phase97 真实事件形状给 sender；如果没有这一行，后续断言没有事实来源。
        ...
        self.assertEqual(sender.unicode_texts, ["x"])  # 修改代码+Phase97ControlledNotepadLiveEdit：确认文本分支仍然通过；如果没有这一行，修保存键可能破坏文本输入。
