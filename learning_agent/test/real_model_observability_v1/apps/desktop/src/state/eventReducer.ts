...
import { isModelCallEventType } from "../components/ModelCallStatus"; // 新增代码+RealModelObservability：引入模型调用事件白名单；如果没有这一行，正常模型阶段会被误报为 unsupported。
...
export function reduceGuiEventToThreadActions(event: StatusEvent): EventReduction { // 新增代码+DesktopEventReducer：函数段开始，把 GUI 事件转换为线程动作；如果没有这段，后端事件无法驱动 Codex 风格主线程。
  if (isModelCallEventType(event.event_type)) { // 新增代码+RealModelObservability：模型调用状态只供状态条消费；如果没有这一行，正常阶段事件会污染主消息 reducer。
    return ok([]); // 新增代码+RealModelObservability：返回无动作且无诊断；如果没有这一行，状态面板会把正常模型阶段当未知事件。
  } // 新增代码+RealModelObservability：模型调用状态分支结束；如果没有这一行，条件块语法不完整。
