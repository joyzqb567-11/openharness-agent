...
def test_direct_sse_adapter_emits_runtime_path_and_streaming_events(tmp_path: Path, monkeypatch) -> None:  # 新增代码+DirectSseAdapterTest: 函数段开始，验证 direct_sse 成功路径；如果没有这个测试，GUI 可能仍走 Codex CLI 或 fake，本段到断言结束。
    ...
    assert "model_call_started" in event_types  # 新增代码+RealModelObservabilityAdapterTest：断言真实模型调用开始事件可见；如果没有这行代码，Composer 状态条不知道请求已经离开前端。
    assert "model_first_delta" in event_types  # 新增代码+RealModelObservabilityAdapterTest：断言首个模型响应事件可见；如果没有这行代码，用户看不到首包延迟何时结束。
    assert "model_call_completed" in event_types  # 新增代码+RealModelObservabilityAdapterTest：断言模型调用完成事件可见；如果没有这行代码，状态条无法稳定收尾。
