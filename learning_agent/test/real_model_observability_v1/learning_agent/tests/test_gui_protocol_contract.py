...
class GuiV2ProtocolContractTest(unittest.TestCase):  # 新增代码+GuiV2ProtocolContractTest：测试类段开始，集中锁定桌面 GUI V2 协议形状；如果没有这个类，协议漂移不会被后端测试发现。
    ...
    def test_model_call_observability_events_are_protocol_events(self) -> None:  # 新增代码+RealModelObservabilityProtocolTest：函数段开始，验证真实模型调用阶段是一等 GUI 事件；如果没有这段测试，后端可能继续拒绝 model_call 状态。
        from learning_agent.app.gui_protocol import make_event  # 新增代码+RealModelObservabilityProtocolTest：导入事件 helper；如果没有这一行，测试无法证明协议白名单是否接受新事件。

        required_events = ("model_call_started", "model_call_status", "model_first_delta", "model_call_completed", "model_call_failed")  # 新增代码+RealModelObservabilityProtocolTest：列出前端状态条需要的五种事件；如果没有这一行，测试目标不清楚。
        for index, event_kind in enumerate(required_events, start=1):  # 新增代码+RealModelObservabilityProtocolTest：逐个事件生成样本；如果没有这一行，只测一个事件会漏掉其它阶段。
            event = make_event(event_kind, index, {"phase": "connecting", "model_id": "gpt-5.5"}, run_id="run_model", turn_id="turn_model")  # 新增代码+RealModelObservabilityProtocolTest：构造模型调用事件；如果没有这一行，后续断言没有具体样本。
            self.assertEqual(event_kind, event["kind"])  # 新增代码+RealModelObservabilityProtocolTest：确认事件类型被原样保留；如果没有这一行，前端无法按 kind 解释阶段。
            self.assertEqual("gpt-5.5", event["payload"]["model_id"])  # 新增代码+RealModelObservabilityProtocolTest：确认模型 id 被保留；如果没有这一行，GUI 状态条不知道当前调用模型。
    # 新增代码+RealModelObservabilityProtocolTest：函数段结束，test_model_call_observability_events_are_protocol_events 到此结束；如果没有这个边界说明，用户不容易看出模型事件合同范围。
