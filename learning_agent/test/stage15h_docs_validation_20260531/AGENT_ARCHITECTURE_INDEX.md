# learning_agent Stage 15 事件运行时架构索引

更新时间：2026-05-31

这份文档是 `learning_agent` 的修 bug 索引和维护地图。它描述 Stage 15 之后的当前新架构，帮助用户和后续 agent 快速判断“问题应该去哪个层、哪个模块查”，避免重新回到翻巨型入口文件的模式。

## 1. 当前结论

`learning_agent` 已经从“一个大文件堆所有能力”改造成分层 agent 工程。当前可见架构目标是：入口清晰、模型适配独立、工具目录独立、事件 transcript 独立、工具协议和执行器独立、MCP 生命周期独立、真实浏览器能力独立、观测验收独立、测试按模块拆分。

Stage 15 新增的运行时骨架：

- `core/events.py`：统一 agent 运行事件。
- `observability/transcript.py`：事件 JSONL 写入和读取。
- `models/base.py`：流式模型事件兼容层。
- `core/agent.py`：`run_events()` 事件流主循环，旧 `run()` 保持兼容。
- `tools/types.py`：Tool Protocol v3 元数据。
...
learning_agent/
  ...
  tools/                  内置工具 schema、工具目录、工具策略、权限 hook、执行器、并发编排、文件和 shell 工具实现。
  tests/                  当前唯一模块化测试套件。
...
- Tool Executor v2 看 `learning_agent/tools/executor.py`
- hook 事件看 `learning_agent/tools/hooks.py`
- 权限决策看 `learning_agent/tools/permissions.py`
- 安全并发批处理看 `learning_agent/tools/orchestrator.py`
...
- 运行 transcript 看 `learning_agent/observability/transcript.py`
...
- 工具协议失败看 `learning_agent/tests/test_tool_protocol.py`
