...
COMPUTER_USE_MCP_TOOL_NAMES = (  # 新增代码+ComputerUseMcpV2：函数段开始，按用户确认顺序列出唯一可见工具；如果没有这段常量，工具面容易多出旧接口。
    ...
    "desktop_task",  # 新增代码+DesktopTaskMcpTool: 完整自然语言桌面任务优先入口；如果没有这一项，模型只能反复手搓原子工具导致复杂任务慢且易中断。
...
def computer_use_mcp_tools(app_inventory_hint: str = "") -> list[dict[str, Any]]:  # 修改代码+ClaudeCodeDynamicToolsList：函数段开始，返回 v2 工具清单副本并允许 tools/list 注入安全应用提示；如果没有这段函数，server 和测试没有统一工具面入口。
    ...
    tools = [  # 新增代码+ComputerUseMcpV2：开始构造工具列表；如果没有这行代码，函数没有可返回内容。
        ...
        _tool("desktop_task", "优先用于完整自然语言桌面任务：自动进行通用阶段规划、阶段边界观察、批量执行和阶段验证；不要把复杂用户任务手动拆成 open_application/type/key，除非本工具要求人工处理。", {"prompt": {"type": "string"}, "target_hint": {"type": "string"}}, ["prompt"]),  # 新增代码+DesktopTaskMcpTool: 定义高层桌面任务 schema；如果没有这一行，真实终端模型看不到 Stage Runtime 主入口。
