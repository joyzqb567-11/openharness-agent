...
class WindowsComputerUseAppNamesPhase121Tests(unittest.TestCase):  # 新增代码+AppNames：测试类开始，集中验证 Windows appNames 风格应用清单；如果没有这个类，Phase121 缺少回归保护。
    ...
    def test_catalog_filter_removes_risky_noise_and_prompt_injection_names(self) -> None:  # 新增代码+AppNames：函数段开始，验证清单过滤危险项和提示注入；如果没有这段测试，应用枚举可能把危险或脏名字交给模型。
        ...
        catalog = module.filter_apps_for_model_description([  # 新增代码+AppNames：构造模拟本机应用清单；如果没有这一行，测试无法稳定复现多种候选。
            ...
            {"display_name": "Performance Monitor", "executable": "Performance Monitor.lnk", "source": "start_menu"},  # 修改代码+AppNames：加入真实日志里暴露出的性能监视器；如果没有这一行，系统监控工具仍可能污染普通应用清单。
            {"display_name": "Visual Studio Installer", "executable": "Visual Studio Installer.lnk", "source": "start_menu"},  # 修改代码+AppNames：加入真实日志里暴露出的安装器；如果没有这一行，安装维护入口会继续混入模型候选。
            {"display_name": "Git Bash", "executable": "Git Bash.lnk", "source": "start_menu"},  # 修改代码+AppNames：加入真实日志里暴露出的 bash 终端；如果没有这一行，开发终端入口可能继续暴露给模型。
        ...
        self.assertNotIn("Performance Monitor", names)  # 修改代码+AppNames：断言性能监视器被过滤；如果没有这一行，系统监控工具会继续出现在候选里。
        self.assertNotIn("Visual Studio Installer", names)  # 修改代码+AppNames：断言安装维护入口被过滤；如果没有这一行，模型可能把安装器当成应用本体。
        self.assertNotIn("Git Bash", names)  # 修改代码+AppNames：断言 bash 终端被过滤；如果没有这一行，终端类开发工具会继续进入普通候选。
