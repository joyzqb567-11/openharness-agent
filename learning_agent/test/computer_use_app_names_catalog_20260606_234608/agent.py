...
class LearningAgent:  # 作用: 教学版 agent 主类
    ...
    def _build_computer_use_full_model_loop_harness_message(self, user_input: str) -> str:  # 新增代码+ModelLoopSemanticPlanner：函数段开始，为 full 模式桌面任务生成极简模型主循环约束；如果没有这段函数，自然语言桌面任务会缺少“由模型选工具”的明确上下文。
        ...
        return "\n".join([  # 新增代码+ModelLoopSemanticPlanner：拼出短提示词块；如果没有这一行，模型不会获得结构化 Computer Use full 工作方式。
            ...
            app_catalog_context,  # 新增代码+AppNames：把清洗后的应用候选交给模型主循环；如果没有这一行，新增 app_names 模块会孤立存在，模型仍看不到可选软件清单。
    ...
    def _computer_use_full_app_catalog_context(self) -> str:  # 新增代码+AppNames：函数段开始，构造 full 模式模型可见应用候选清单；如果没有这段函数，应用发现无法接入模型主循环。
        try:  # 新增代码+AppNames：优先按包路径导入 app_names 能力；如果没有这一行，agent.py 会和应用清单模块形成硬依赖导致脚本模式不兼容。
            from learning_agent.computer_use.app_names import discover_windows_app_catalog, format_apps_for_model_description  # 新增代码+AppNames：导入应用枚举和格式化函数；如果没有这一行，主循环拿不到清洗后的候选提示。
        except ModuleNotFoundError as error:  # 新增代码+AppNames：兼容 start_oauth_agent.bat 从 learning_agent 目录启动的脚本模式；如果没有这一行，真实终端入口可能导入失败。
            if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.app_names"}:  # 新增代码+AppNames：只兜底包路径缺失；如果没有这一行，app_names 内部 bug 会被错误吞掉。
                raise  # 新增代码+AppNames：重新抛出真实内部错误；如果没有这一行，排查清单模块问题会变得困难。
            from computer_use.app_names import discover_windows_app_catalog, format_apps_for_model_description  # type: ignore  # 新增代码+AppNames：脚本模式导入同一能力；如果没有这一行，bat 入口无法给模型应用候选清单。
        catalog = discover_windows_app_catalog(include_common=True)  # 新增代码+AppNames：枚举并清洗本机应用候选；如果没有这一行，模型提示没有实际候选来源。
        return format_apps_for_model_description(catalog)  # 新增代码+AppNames：返回不含原始路径的模型提示文本；如果没有这一行，harness 无法拼接候选清单。
    # 新增代码+AppNames：函数段结束，_computer_use_full_app_catalog_context 到此结束；如果没有这个边界说明，用户不容易看出候选清单提示范围。
    ...
    def _computer_use_full_canonical_target_app_hint(self, target_app_hint: str) -> str:  # 新增代码+ModelLoopFirstStepLaunch：函数段开始，把脱敏应用提示转换成稳定启动别名；如果没有这段函数，模型会在“画图”和 mspaint 之间摇摆，作者意图是只规范应用入口而不解析绘图主体。
        ...
        try:  # 新增代码+AppNames：优先使用清洗后的应用候选清单解析启动名；如果没有这一行，函数会继续只靠局部硬编码字典。
            from learning_agent.computer_use.app_names import resolve_app_name_hint  # 新增代码+AppNames：导入 app_names 解析器；如果没有这一行，中文/英文别名无法共享清单逻辑。
        except ModuleNotFoundError as error:  # 新增代码+AppNames：兼容脚本模式导入；如果没有这一行，真实 bat 入口可能无法解析应用名。
            if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.app_names"}:  # 新增代码+AppNames：只兜底包路径缺失；如果没有这一行，内部错误会被误判为脚本模式问题。
                raise  # 新增代码+AppNames：重新抛出真实内部错误；如果没有这一行，清单解析 bug 会被掩盖。
            from computer_use.app_names import resolve_app_name_hint  # type: ignore  # 新增代码+AppNames：脚本模式导入同一解析器；如果没有这一行，start_oauth_agent.bat 入口无法使用清单解析。
        catalog_hint = resolve_app_name_hint(normalized_hint)  # 新增代码+AppNames：用候选清单解析规范 app_name；如果没有这一行，模型仍无法从清单别名得到 mspaint。
        if catalog_hint:  # 新增代码+AppNames：命中清单别名时优先使用；如果没有这一行，清单解析结果不会生效。
            return catalog_hint  # 新增代码+AppNames：返回清单给出的稳定 app_name；如果没有这一行，后续仍走旧硬编码路径。
