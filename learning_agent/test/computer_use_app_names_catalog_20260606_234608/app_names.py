...
APP_NAME_BLOCKED_EXACT_NAMES = {"run", "computer management", "event viewer", "disk cleanup", "remote desktop connection", "steps recorder", "memory diagnostics tool", "odbc data sources (32-bit)", "odbc data sources (64-bit)", "iscsi initiator", "dfrgui", "magnify", "narrator", "on-screen keyboard", "voiceaccess", "livecaptions", "performance monitor", "print management", "recoverydrive", "resource monitor", "system configuration", "system information", "task scheduler"}  # 修改代码+AppNames：按精确名称拦截真实开始菜单里的系统/辅助/管理入口；如果没有这一行，模型候选清单仍会混入不适合普通任务优先选择的系统工具。
APP_NAME_NOISE_TOKENS = ("helper", "agent", "service", "installer", "uninstaller", "uninstall", "updater", "update", "background", "runtime broker", "documentation", "release notes", "help", "卸载", "修复", "官网", "配置")  # 修改代码+AppNames：同时过滤英文后台/安装维护/帮助文档和中文卸载配置类快捷方式噪声；如果没有这一行，真实开始菜单里的维护入口会继续污染模型候选。
APP_NAME_HIGH_RISK_TOKENS = ("powershell", "pwsh", "cmd", "command prompt", "terminal", "bash", "wt", "regedit", "registry", "control", "settings", "mmc", "taskmgr", "administrator", "administrative tools", "credential", "password", "security", "firewall", "defender", "services", "wsl", "system tools", "windows tools")  # 修改代码+AppNames：补充真实日志暴露出的命令提示符、bash、管理工具和 WSL 等系统入口；如果没有这一行，模型仍可能把终端类工具当成普通桌面应用。
...
def filter_apps_for_model_description(installed: Any, include_common: bool = True, max_count: int = APP_CATALOG_MAX_COUNT) -> list[dict[str, Any]]:  # 新增代码+AppNames：函数段开始，过滤并去重模型可见候选；如果没有这段函数，模型仍会面对原始混乱应用列表。
    ...
    for item in raw_items:  # 新增代码+AppNames：逐个处理候选；如果没有这一行，函数无法过滤列表内容。
        ...
        if entry["display_name"].casefold() in APP_NAME_BLOCKED_EXACT_NAMES or entry["app_name"].casefold() in APP_NAME_BLOCKED_EXACT_NAMES:  # 修改代码+AppNames：用精确黑名单拦截 Run/Event Viewer 等短名字系统入口；如果没有这一行，短系统工具不容易靠子串风险词稳定过滤。
            continue  # 修改代码+AppNames：跳过精确命中的系统入口；如果没有这一行，黑名单只会定义但不会生效。
