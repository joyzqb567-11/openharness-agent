...
def _stage_planner_requested_text(text: str) -> str:  # 新增代码+UniversalStagePlanner：函数段开始，从通用文本任务里抽取要输入的正文；如果没有这段函数，批执行器会退回 sample text 或无法验收。
    ...
    for prefix in TEXT_PAYLOAD_PREFIXES:  # 新增代码+UniversalStagePlanner：遍历通用输入前缀；如果没有这行代码，中英文输入动词不能统一处理。
        index = lowered.rfind(prefix.lower())  # 新增代码+UniversalStagePlanner：取最后一次输入前缀位置；如果没有这行代码，像“输入到窗口里输入 hello”会抓到前半句说明。
        if index < 0:  # 新增代码+UniversalStagePlanner：跳过未命中的前缀；如果没有这行代码，负索引会截错文本。
            continue  # 新增代码+UniversalStagePlanner：继续尝试其它前缀；如果没有这行代码，一个前缀没命中就会提前失败。
        candidate = text[index + len(prefix):].strip(" ：:，,。.;；\"'`“”")  # 新增代码+UniversalStagePlanner：截取前缀后的正文候选；如果没有这行代码，正文会带着动词和标点。
        for stop in TEXT_PAYLOAD_STOPS:  # 新增代码+UniversalStagePlanner：逐个检查后续动作边界；如果没有这行代码，保存、拖动等后续步骤会混进正文。
            ...
            if stop_index > 0:  # 新增代码+UniversalStagePlanner：只在终止词出现在正文后方时切分；如果没有这行代码，开头标点可能误删全部内容。
                candidate = candidate[:stop_index].strip(" ：:，,。.;；\"'`“”")  # 新增代码+UniversalStagePlanner：切掉后续动作说明；如果没有这行代码，正文会包含任务控制语句。
        if 0 < len(candidate) <= 160:  # 新增代码+UniversalStagePlanner：接受非空且不过长的正文；如果没有这行代码，异常长 prompt 可能被误塞进 type_text。
...
class UniversalDesktopStagePlanner:  # 新增代码+UniversalStagePlanner：类段开始，定义通用桌面阶段规划器；如果没有这个类，full mode 无法拥有上层阶段计划。
    ...
    def _stages_for_kind(self, task_kind: str, text: str = "") -> list[StagePlan]:  # 新增代码+UniversalStagePlanner：函数段开始，按通用任务类型生成阶段；如果没有这段函数，阶段序列无法复用。
        ...
        if task_kind == "multi_app":  # 新增代码+UniversalStagePlanner：多窗口任务需要分别准备和写入；如果没有这行代码，多目标阶段无法生成。
            return [_stage_planner_stage("stage_001_prepare_source", "prepare_target", "target_1", "before_stage"), _stage_planner_stage("stage_002_probe_source", "probe_capabilities", "target_1", "before_stage"), _stage_planner_stage("stage_003_prepare_destination", "prepare_target", "target_2", "before_stage"), _stage_planner_stage("stage_004_probe_destination", "probe_capabilities", "target_2", "before_stage"), _stage_planner_stage("stage_005_read_source", "perform_content_work", "target_1", "before_and_after_stage"), _stage_planner_stage("stage_006_write_destination", "perform_content_work", "target_2", "before_and_after_stage"), _stage_planner_stage("stage_007_commit_destination", "commit_resource", "target_2", "before_and_after_stage"), _stage_planner_stage("stage_008_verify_result", "verify_result", "target_2", "after_stage")]  # 新增代码+UniversalStagePlanner：返回多目标阶段序列；如果没有这行代码，跨窗口整理任务无法表达。
