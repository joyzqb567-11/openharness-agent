...
try:  # 新增代码+Phase47WindowsSendInputDispatcher: 优先按包模式导入 Phase37 executor；如果没有这行代码，unittest 和生产入口无法复用现有动作层。
    from learning_agent.computer_use_mcp_v2.windows_runtime.sendinput_executor import WindowsSendInputExecutor  # 新增代码+Phase47WindowsSendInputDispatcher: 复用 executor 生成规范事件；如果没有这行代码，dispatcher 要重复处理高层动作参数。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.sendinput_executor"}:  # 新增代码+Phase47WindowsSendInputDispatcher: 只允许目标包路径缺失时 fallback；如果没有这行代码，真实内部 bug 会被误吞。
        raise  # 新增代码+Phase47WindowsSendInputDispatcher: 重新抛出非路径类导入错误；如果没有这行代码，排查 executor 内部错误会困难。
    from computer_use_mcp_v2.windows_runtime.sendinput_executor import WindowsSendInputExecutor  # 新增代码+Phase47WindowsSendInputDispatcher: 脚本模式导入 executor；如果没有这行代码，bat 入口无法运行自检。

try:  # 新增代码+DrawingPrimitives：优先按包路径导入通用 drag_path 展开器；如果没有这段代码，dispatcher 无法理解绘图 primitive 输出。
    from learning_agent.computer_use_mcp_v2.windows_runtime.drawing_primitives import expand_drag_path_to_low_level_events  # 新增代码+DrawingPrimitives：导入拖拽路径到底层鼠标事件的统一转换函数；如果没有这一行，SendInput 最后一跳无法执行 drag_path。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.drawing_primitives"}:  # 新增代码+DrawingPrimitives：只兜底包路径缺失；如果没有这一行，drawing primitive 内部 bug 会被误吞。
        raise  # 新增代码+DrawingPrimitives：重新抛出真实内部导入错误；如果没有这一行，排查 drag_path 展开问题会困难。
    from computer_use_mcp_v2.windows_runtime.drawing_primitives import expand_drag_path_to_low_level_events  # type: ignore  # 新增代码+DrawingPrimitives：脚本模式导入同一转换函数；如果没有这一行，bat 入口无法使用 drag_path。
...
def _phase47_hwnd_from_target(target: dict[str, Any]) -> int:  # 新增代码+ForegroundTargetRouting: 函数段开始，从目标身份中提取 Windows hwnd；如果没有这段函数，dispatcher 无法把 window_id 转成 SetForegroundWindow 需要的数字句柄。
    raw_hwnd = target.get("hwnd")  # 新增代码+ForegroundTargetRouting: 优先读取显式 hwnd 字段；如果没有这行代码，未来更标准的窗口字段无法直接使用。
    if raw_hwnd is None:  # 新增代码+ForegroundTargetRouting: 没有显式 hwnd 时回退读取 window_id；如果没有这行代码，当前真实 Paint 返回的 hwnd:xxxx 形式无法被识别。
        raw_hwnd = str(target.get("window_id", "")).replace("hwnd:", "", 1)  # 新增代码+ForegroundTargetRouting: 去掉 window_id 的 hwnd: 前缀；如果没有这行代码，int 转换会失败导致无法聚焦。
    return _safe_int(raw_hwnd)  # 新增代码+ForegroundTargetRouting: 把提取结果安全转成整数；如果没有这行代码，坏 hwnd 会抛异常而不是安全返回 0。
# 新增代码+ForegroundTargetRouting: 函数段结束，_phase47_hwnd_from_target 到此结束；如果没有这个边界说明，读者不容易看出 hwnd 提取范围。


def _phase47_foreground_event_from_events(events: list[dict[str, Any]]) -> dict[str, Any]:  # 新增代码+ForegroundTargetRouting: 函数段开始，从一组高层事件中生成前台聚焦事件；如果没有这段函数，鼠标事件可能在 Paint 未聚焦时被系统吃掉。
    for event in events:  # 新增代码+ForegroundTargetRouting: 按原始动作顺序查找第一个带 target 的事件；如果没有这行代码，无法从动作里找到窗口身份。
        target = _phase47_event_target(event)  # 新增代码+ForegroundTargetRouting: 读取当前事件绑定的目标身份；如果没有这行代码，函数不知道该聚焦哪个窗口。
        hwnd = _phase47_hwnd_from_target(target)  # 新增代码+ForegroundTargetRouting: 把目标身份转成 hwnd 数字；如果没有这行代码，SetForegroundWindow 没有可用参数。
        if hwnd > 0:  # 新增代码+ForegroundTargetRouting: 只在 hwnd 有效时生成聚焦事件；如果没有这行代码，无效句柄也会进入真实 Win32 API。
            return {"type": "set_foreground", "hwnd": hwnd, "target": target}  # 新增代码+ForegroundTargetRouting: 返回带 target 的前台聚焦事件；如果没有这行代码，send 无法把聚焦插到鼠标事件前。
    return {}  # 新增代码+ForegroundTargetRouting: 没有可用 hwnd 时返回空字典；如果没有这行代码，无窗口动作会异常退出。
# 新增代码+ForegroundTargetRouting: 函数段结束，_phase47_foreground_event_from_events 到此结束；如果没有这个边界说明，读者不容易看出自动聚焦生成范围。
...
class WindowsSendInputDispatcher:  # 新增代码+Phase47WindowsSendInputDispatcher: 类段开始，定义 Windows SendInput 调度器；如果没有这个类，高层事件无法展开成低层输入序列。
    def __init__(self, platform: str | None = None, enabled: bool = False, low_level_sender: Any | None = None, target_verifier: Any | None = None) -> None:  # 新增代码+Phase47WindowsSendInputDispatcher: 函数段开始，初始化平台、开关、低层 sender 和目标校验器；如果没有这段函数，dispatcher 无法安全注入和测试。
        ...
        self.requires_raw_text = bool(getattr(low_level_sender, "requires_raw_text", False))  # 新增代码+ComputerUsePressureReadiness：透传低层 sender 是否需要原始文本；如果没有这行代码，executor 不知道真实 Unicode 输入最后一跳需要明文。
        self.uses_secure_text_channel = bool(getattr(low_level_sender, "accepts_secure_plaintext_text_channel", False))  # 修改代码+SecureTextChannel：识别最后一跳是否支持私有明文短通道；如果没有这行代码，受控 sender 和普通 raw sender 会被错误混用。
    ...
    def send(self, events: list[dict[str, Any]]) -> dict[str, Any]:  # 新增代码+Phase47WindowsSendInputDispatcher: 函数段开始，接收 Phase37 规范事件并调度低层事件；如果没有这段函数，executor 无法使用 dispatcher。
        ...
        events_for_send = list(events or [])  # 新增代码+ForegroundTargetRouting: 固定本次要发送的高层事件快照；如果没有这行代码，自动聚焦和后续展开可能看到不同的事件列表。
        low_level_events: list[dict[str, Any]] = []  # 新增代码+Phase47WindowsSendInputDispatcher: 准备低层事件容器；如果没有这行代码，展开结果没有保存位置。
        foreground_event = _phase47_foreground_event_from_events(events_for_send)  # 新增代码+ForegroundTargetRouting: 根据窗口绑定事件生成 set_foreground 预备动作；如果没有这行代码，真实鼠标键盘可能仍发到终端或旧焦点窗口。
        if foreground_event:  # 新增代码+ForegroundTargetRouting: 只有存在有效 hwnd 时才插入聚焦事件；如果没有这行代码，无窗口动作会多出无意义事件。
            low_level_events.extend(self._expand_event(foreground_event))  # 新增代码+ForegroundTargetRouting: 把聚焦事件排在所有鼠标键盘事件之前；如果没有这行代码，Paint 画布可能打开但仍不接收拖拽。
        for event in events_for_send:  # 修改代码+ForegroundTargetRouting: 遍历固定快照里的规范事件；如果没有这行代码，dispatcher 不会处理任何动作。
    ...
    def _expand_event(self, event: dict[str, Any]) -> list[dict[str, Any]]:  # 新增代码+Phase47WindowsSendInputDispatcher: 函数段开始，把一个规范事件展开成低层事件；如果没有这段函数，动作类型会散落在 send 里。
        ...
        if event_type == "set_foreground":  # 新增代码+RealLaunchTargetSession：展开前台窗口聚焦事件；如果没有这一行，focus_window 无法真正把 agent 启动的窗口置前。
            return _phase47_attach_target([{"type": "set_foreground", "hwnd": _safe_int(event.get("hwnd"))}, {"type": "pause", "seconds": 0.08}], target)  # 修改代码+ForegroundTargetRouting: 返回聚焦后短暂停顿的低层事件；如果没有这一行，Paint 可能还没完成前台切换就收到第一条鼠标移动。
        ...
        if event_type == "text":  # 新增代码+Phase47WindowsSendInputDispatcher: 展开文本输入摘要；如果没有这行代码，type_text 无法进入低层 sender。
            unicode_event = {"type": "unicode_text", "text_length": _safe_int(event.get("text_length")), "text_sha256_16": str(event.get("text_sha256_16", "")), "text_redacted": True}  # 修改代码+ComputerUsePressureReadiness：先构造默认脱敏低层文本事件；如果没有这行代码，普通记录型 sender 可能拿到不该拿的原文。
            if self.requires_raw_text and "text" in event:  # 修改代码+SecureTextChannel：只有真实低层 sender 声明需要原文且上游已给出时才附加私有明文字段；如果没有这行代码，日志型 sender 和真实 sender 的边界会混在一起。
                secure_key = "_secure_plaintext_text" if self.uses_secure_text_channel else "text"  # 修改代码+SecureTextChannel：受控 sender 走私有字段，普通 raw sender 保持旧合同；如果没有这行代码，压力回归和生产门禁无法同时满足。
                unicode_event[secure_key] = str(event.get("text", ""))  # 修改代码+SecureTextChannel：按最后一跳能力注入短生命周期明文；如果没有这行代码，真实 Unicode 输入会继续发送空字符串。
            return _phase47_attach_target([unicode_event], target)  # 修改代码+ComputerUsePressureReadiness：返回按需含原文、默认脱敏的文本事件；如果没有这行代码，文本输入无法进入受控 sender 的目标验证。
...
def run_phase47_sendinput_dispatcher_contract() -> dict[str, Any]:  # 新增代码+Phase47WindowsSendInputDispatcher: 函数段开始，运行 Phase47 安全自检合同；如果没有这段函数，真实终端场景没有稳定命令入口。
    from learning_agent.computer_use_mcp_v2.windows_runtime.native_host import InProcessWindowsNativeHostClient, WindowsComputerUseNativeHost  # 新增代码+Phase47WindowsSendInputDispatcher: 延迟导入 native host 避免循环依赖；如果没有这行代码，自检无法证明 host action 集成。
