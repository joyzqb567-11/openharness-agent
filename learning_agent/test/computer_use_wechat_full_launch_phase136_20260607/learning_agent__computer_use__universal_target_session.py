...
try:  # 新增代码+URG2UniversalTargetSession：优先按 learning_agent 包路径导入已有通用能力；如果没有这一段，单测和生产入口无法共享同一实现。
    from learning_agent.computer_use.windows_launch_resolver import resolve_generic_app_launch_target  # 新增代码+URG2UniversalTargetSession：复用 Phase108 通用应用发现；如果没有这一行，URG-2 可能退回逐 app 白名单。
    from learning_agent.computer_use.generic_launch_backend import Phase110ProductionGenericLaunchBackend, run_generic_launch_backend  # 新增代码+RealLaunchTargetSession：复用 Phase110 真实启动后端；如果没有这一行，URG-2 会继续伪造 pid/hwnd。
    from learning_agent.computer_use.target_identity import build_owned_target_identity, verify_owned_target_identity  # 新增代码+URG2UniversalTargetSession：复用 Phase111 目标身份和漂移验证；如果没有这一行，URG-2 会重造不一致的身份算法。
    from learning_agent.computer_use.windows_backend import WindowsWindowInventoryProbe  # 新增代码+RealLaunchTargetSession：复用真实窗口 inventory；如果没有这一行，真实启动后无法按 pid 绑定窗口。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.windows_launch_resolver", "learning_agent.computer_use.generic_launch_backend", "learning_agent.computer_use.target_identity", "learning_agent.computer_use.windows_backend"}:  # 修改代码+RealLaunchTargetSession：兜底名单加入启动后端和窗口 inventory；如果没有这一行，bat 入口可能无法走真实启动链路。
        raise  # 新增代码+URG2UniversalTargetSession：重新抛出非路径类导入错误；如果没有这一行，排查真实依赖问题会很困难。
    from computer_use.windows_launch_resolver import resolve_generic_app_launch_target  # type: ignore  # 新增代码+URG2UniversalTargetSession：脚本模式导入通用发现；如果没有这一行，bat 入口不能解析普通目标。
    from computer_use.generic_launch_backend import Phase110ProductionGenericLaunchBackend, run_generic_launch_backend  # type: ignore  # 新增代码+RealLaunchTargetSession：脚本模式导入 Phase110 启动后端；如果没有这一行，真实终端入口无法启动普通应用。
    from computer_use.target_identity import build_owned_target_identity, verify_owned_target_identity  # type: ignore  # 新增代码+URG2UniversalTargetSession：脚本模式导入目标身份 helper；如果没有这一行，bat 入口不能防漂移。
    from computer_use.windows_backend import WindowsWindowInventoryProbe  # type: ignore  # 新增代码+RealLaunchTargetSession：脚本模式导入窗口 inventory；如果没有这一行，bat 入口无法找到启动窗口。
...
def _phase117_window_matches_target_hint(window: dict[str, Any], target_hint: str) -> bool:  # 新增代码+ProxyWindowBinding：函数段开始，判断窗口是否像本次目标应用；如果没有这段函数，启动 pid 不一致的微信窗口会被误判为找不到。函数到 return 结束。
    hint = _phase117_safe_text(target_hint).casefold()  # 新增代码+ProxyWindowBinding：清理目标提示并转小写；如果没有这一行，中文目标或大小写差异会影响匹配。
    if not hint:  # 新增代码+ProxyWindowBinding：没有目标提示时不能做代理匹配；如果没有这一行，任意窗口可能被误绑定。
        return False  # 新增代码+ProxyWindowBinding：空提示直接拒绝；如果没有这一行，代理匹配安全边界会过宽。
    if _phase117_safe_int(window.get("hwnd")) <= 0:  # 新增代码+ProxyWindowBinding：必须有真实窗口句柄；如果没有这一行，后台进程或空窗口也可能被当成可见目标。
        return False  # 新增代码+ProxyWindowBinding：无 hwnd 时拒绝；如果没有这一行，身份绑定仍会缺少可操作窗口。
    return any(hint in text or text in hint for text in _phase117_window_texts(window) if text)  # 新增代码+ProxyWindowBinding：目标提示与标题/进程名任一包含即命中；如果没有这一行，标题“微信”的代理窗口无法被绑定。
# 新增代码+ProxyWindowBinding：函数段结束，_phase117_window_matches_target_hint 到此结束；如果没有这个边界说明，用户不容易看出代理窗口匹配条件。


def _phase117_proxy_window_for_owned_process(window: dict[str, Any], launch_result: dict[str, Any], process_id: int) -> dict[str, Any]:  # 新增代码+ProxyWindowBinding：函数段开始，把代理窗口包装成可审计的 agent-owned 目标窗口；如果没有这段函数，pid 不一致会让身份模型拒绝微信窗口。函数到 return 结束。
...
class Phase117OwnedWindowProbe:  # 新增代码+RealLaunchTargetSession：类段开始，启动后按自有进程 pid 查找真实窗口；如果没有这个类，agent 只能启动进程却不能证明目标窗口属于自己。
    def __init__(self, inventory_probe: Any | None = None, poll_attempts: int = 30, poll_interval_seconds: float = 0.2) -> None:  # 新增代码+RealLaunchTargetSession：函数段开始，允许注入窗口 inventory 和轮询参数；如果没有这段函数，单测无法替换真实桌面依赖。
    ...
    def find_owned_window(self, launch_result: dict[str, Any], target_hint: str = "") -> dict[str, Any]:  # 新增代码+RealLaunchTargetSession：函数段开始，按启动结果中的 pid 找窗口；如果没有这段函数，旧用户窗口可能被误当 agent 窗口。
        ...
        for attempt_index in range(self.poll_attempts):  # 新增代码+RealLaunchTargetSession：有限轮询等待窗口出现；如果没有这一行，刚启动的 GUI 程序可能还没建窗就失败。
            ...
            for window in windows:  # 修改代码+ProxyWindowBinding：先遍历安全窗口候选做 pid 精确匹配；如果没有这一行，无法找到标准 agent-owned 目标窗口。
                if _phase117_safe_int(window.get("pid", window.get("window_process_id"))) == process_id:  # 新增代码+RealLaunchTargetSession：只接受 pid 与启动进程一致的窗口；如果没有这一行，用户已有同名窗口会被误用。
                    ...
                    return result  # 新增代码+RealLaunchTargetSession：返回 agent-owned 目标窗口；如果没有这一行，启动成功也无法进入动作层。
            for window in windows:  # 新增代码+ProxyWindowBinding：pid 精确匹配失败后再按目标标题/进程名查找代理窗口；如果没有这一行，微信这类多进程应用会启动但无法绑定窗口。
                if _phase117_window_matches_target_hint(window, target_hint):  # 新增代码+ProxyWindowBinding：检查窗口是否匹配本次目标提示；如果没有这一行，代理匹配会误用任意窗口。
                    return _phase117_proxy_window_for_owned_process(window, launch_result, process_id)  # 新增代码+ProxyWindowBinding：返回带代理审计字段的目标窗口；如果没有这一行，Phase111 仍会因 pid 不同拒绝。
...
class UniversalTargetSessionRuntime:  # 新增代码+URG2UniversalTargetSession：类段开始，管理通用目标 session 和动作前身份复核；如果没有这个类，目标凭证会散落在发现、启动和动作层。
    ...
    def _open_real_target_session(self, raw_target: Any, phase108_report: dict[str, Any], session_id: str, executable: str, user_authorized_window: bool = False) -> dict[str, Any]:  # 新增代码+RealLaunchTargetSession：函数段开始，真实启动应用并绑定 agent-owned 窗口；如果没有这段函数，full 模式会继续误用用户已打开窗口。
        ...
        target_window = dict(finder(launch_result, target_hint=str(phase108_report.get("canonical_target", raw_target))) if callable(finder) else {})  # 新增代码+RealLaunchTargetSession：按启动 pid 查找窗口；如果没有这一行，进程启动后没有窗口身份。
        ...
        session_ready = bool(phase108_report.get("passed") and launch_result.get("ok") and launch_result.get("process_started") and owned_identity.target_identity_verified and stable_verification.get("allowed"))  # 新增代码+RealLaunchTargetSession：汇总真实启动 session 门禁；如果没有这一行，启动失败或未绑定窗口也可能被当成功。
        return {"marker": PHASE117_UNIVERSAL_TARGET_SESSION_MARKER, "model": PHASE117_UNIVERSAL_TARGET_SESSION_MODEL, "session_id": session_id, "raw_target": str(raw_target or ""), "canonical_target": str(phase108_report.get("canonical_target", "")), "session_ready": session_ready, "generic_target_resolver_used": bool(phase108_report.get("dynamic_discovery_used")), "per_app_controller_required": False, "hardcoded_app_whitelist_required": False, "representative_apps_are_acceptance_only": True, "agent_owned_or_user_authorized_window": bool(owned_identity.target_identity_verified or user_authorized_window), "target_identity_bound": bool(owned_identity.target_identity_verified), "target_identity_rechecked_before_each_action": True, "target_drift_zero_events": bool(drift_verification.get("target_drift_blocks_action") and not drift_verification.get("allowed")), "stable_verification": stable_verification, "drift_verification": drift_verification, "target_window": target_window, "owned_target_identity": owned_identity.to_report(), "phase108_report": phase108_report, "launch_result": launch_result, "real_launch_performed": bool(launch_result.get("real_desktop_touched") or launch_result.get("process_started")), "backend_launch_performed": bool(launch_result.get("backend_launch_performed") or launch_result.get("backend_launch_reaches_launcher")), "process_started": bool(launch_result.get("process_started")), "owned_process_registered": bool(launch_result.get("owned_process_registered")), "visible_window_verified": bool(owned_identity.window_identity_verified), "low_level_event_count": 0, "real_desktop_touched": bool(launch_result.get("real_desktop_touched")), "actions_expanded": PHASE117_ACTIONS_EXPANDED}  # 新增代码+RealLaunchTargetSession：返回真实启动 session 报告；如果没有这一行，终端验收无法证明 agent 自己打开并绑定窗口。
