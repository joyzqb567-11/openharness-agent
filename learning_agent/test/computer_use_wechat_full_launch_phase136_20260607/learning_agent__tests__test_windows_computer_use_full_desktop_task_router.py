...
from learning_agent.computer_use.desktop_task_runtime import ComputerUseDesktopTaskRuntime, build_default_desktop_task_runtime, computer_use_full_desktop_task_runtime_cli_line  # 修改代码+RuntimeFactorySlimming：导入迁移后的默认 runtime 工厂；如果没有这一行，默认接线测试仍会把职责错误挂在 LearningAgent 主类上。
...
from learning_agent.core.messages import ToolCall  # 新增代码+ModelLoopFullRuntimeBridge：导入真实工具调用对象用于覆盖模型主循环的 computer_use 工具入口；如果没有这一行，测试只能绕过工具执行器而无法证明模型工具层接到了 full runtime。
...
class WindowsComputerUseFullDesktopTaskRouterTests(unittest.TestCase):  # 新增代码+DesktopTaskAcceptance：类段开始，集中冻结 `/computer use --full` 桌面任务路由验收；如果没有这个类，unittest 不会组织这些回归用例。
    ...
    def test_chinese_local_app_name_without_generic_app_word_routes_to_desktop_task(self) -> None:  # 新增代码+GenericLocalAppRouter：函数段开始，验证“本机电脑+应用名”但没有“软件/应用”泛词时仍进入 Computer Use；如果没有这段测试，打开微信这类真实用户说法会漏判。
        intent = classify_desktop_task("请帮我打开本机电脑微信。看到微信窗口后就结束，不要登录。")  # 新增代码+GenericLocalAppRouter：调用分类器处理真实中文微信打开请求；如果没有这一行，测试不会覆盖本轮失败根因。
        self.assertTrue(intent.is_desktop_task)  # 新增代码+GenericLocalAppRouter：断言该请求必须进入桌面 GUI 路线；如果没有这一行，full harness 不会注入。
        self.assertEqual("微信", intent.target_app_hint)  # 新增代码+GenericLocalAppRouter：断言目标应用提示提取为微信；如果没有这一行，首轮 launch_app 收窄无法知道目标。
        self.assertTrue(intent.requires_gui_actions)  # 新增代码+GenericLocalAppRouter：断言打开本机应用需要 GUI 动作；如果没有这一行，系统可能把它当普通聊天。
    # 新增代码+GenericLocalAppRouter：函数段结束，test_chinese_local_app_name_without_generic_app_word_routes_to_desktop_task 到此结束；如果没有这个边界说明，用户不容易看出微信自然表达回归范围。

    def test_chinese_local_app_name_before_software_suffix_routes_to_desktop_task(self) -> None:  # 新增代码+GenericLocalAppRouter：函数段开始，验证“本机电脑QQ软件”这类应用名加后缀的表达；如果没有这段测试，QQ/微信/钉钉等普通软件会继续依赖硬编码。
        intent = classify_desktop_task("请帮我打开本机电脑QQ软件，然后输入账户。")  # 新增代码+GenericLocalAppRouter：调用分类器处理真实 QQ 打开和输入请求；如果没有这一行，测试无法覆盖普通软件名加“软件”后缀。
        self.assertTrue(intent.is_desktop_task)  # 新增代码+GenericLocalAppRouter：断言该请求进入桌面 GUI 路线；如果没有这一行，full 模式无法给模型正确工具约束。
        self.assertEqual("qq", intent.target_app_hint)  # 新增代码+GenericLocalAppRouter：断言目标应用提示保留为 QQ 的规范小写形式；如果没有这一行，resolver/discover 可能只能收到泛化 local_app。
        self.assertTrue(intent.requires_gui_actions)  # 新增代码+GenericLocalAppRouter：断言输入账户属于 GUI 操作任务；如果没有这一行，键盘输入门禁不会启用。
    # 新增代码+GenericLocalAppRouter：函数段结束，test_chinese_local_app_name_before_software_suffix_routes_to_desktop_task 到此结束；如果没有这个边界说明，用户不容易看出普通软件后缀回归范围。
    ...
    def test_model_loop_computer_use_run_prompt_requires_observe_action_tools_instead_of_runtime_black_box(self) -> None:  # 修改代码+ModelLoopSemanticPlanner：函数段开始，验证 run_prompt 不再黑盒执行自然语言桌面任务；如果没有这个测试，模型工具层可能再次绕过主循环语义规划。
        import tempfile  # 修改代码+ModelLoopSemanticPlanner：导入临时目录工具隔离本测试 workspace；如果没有这一行，测试会把 full mode 状态写进用户真实项目目录。
        from pathlib import Path  # 修改代码+ModelLoopSemanticPlanner：导入 Path 构造 Windows 兼容路径；如果没有这一行，workspace 路径表达会退回脆弱字符串拼接。
        class ExplodingDesktopRuntime:  # 新增代码+ModelLoopSemanticPlanner：类段开始，若旧 runtime 被调用就立刻失败；如果没有这个类，测试无法证明黑盒 runtime 没有抢跑。
            def run_prompt(self, prompt: str, real_actions: bool = False) -> dict[str, object]:  # 新增代码+ModelLoopSemanticPlanner：提供旧 runtime 同名接口；如果没有这一行，agent.desktop_task_runtime 注入对象形状不完整。
                raise AssertionError(f"run_prompt should not execute semantic prompt in model-loop mode: {prompt} real={real_actions}")  # 新增代码+ModelLoopSemanticPlanner：旧黑盒路径被调用时明确失败；如果没有这一行，回归会静默通过。
            # 新增代码+ModelLoopSemanticPlanner：函数段结束，ExplodingDesktopRuntime.run_prompt 到此结束；如果没有这个边界说明，初学者不容易看出失败哨兵范围。
        # 新增代码+ModelLoopSemanticPlanner：类段结束，ExplodingDesktopRuntime 到此结束；如果没有这个边界说明，初学者不容易看出注入哨兵范围。
        with tempfile.TemporaryDirectory() as raw_dir:  # 修改代码+ModelLoopSemanticPlanner：创建一次性 workspace；如果没有这一行，测试之间会互相污染 Computer Use session 和证据目录。
            workspace = Path(raw_dir)  # 修改代码+ModelLoopSemanticPlanner：把临时目录转成 Path 对象；如果没有这一行，LearningAgent 构造参数不够清楚且后续路径拼接容易出错。
            agent = LearningAgent(model=ToolCallingFakeModel([]), workspace=workspace, ask_permission=lambda action: True, debug_enabled=False)  # 修改代码+ModelLoopSemanticPlanner：创建真实 agent 并自动允许测试权限；如果没有这一行，无法通过真实工具执行器复现模型主循环工具调用。
            agent.desktop_task_runtime = ExplodingDesktopRuntime()  # 新增代码+ModelLoopSemanticPlanner：注入会爆炸的旧 runtime；如果没有这一行，测试无法证明 run_prompt 没有进入黑盒执行器。
            output = agent._execute_tool(ToolCall(name="computer_use", arguments={"operation": "run_prompt", "prompt": "请使用本地电脑的画图软件画一棵树。", "real_actions": True}))  # 修改代码+ModelLoopSemanticPlanner：通过真实工具分发调用旧 run_prompt 入口；如果没有这一行，测试不能锁定兼容入口的降级行为。
            self.assertIn("model_loop_observe_action_required", output)  # 新增代码+ModelLoopSemanticPlanner：断言工具返回要求模型改用 observe/action；如果没有这一行，模型可能继续把 run_prompt 当作黑盒执行器。
            self.assertIn("computer_observe", output)  # 新增代码+ModelLoopSemanticPlanner：断言返回文本提示使用观察工具；如果没有这一行，模型难以自我纠正下一步。
            self.assertIn("computer_action", output)  # 新增代码+ModelLoopSemanticPlanner：断言返回文本提示使用动作工具；如果没有这一行，模型难以继续真实桌面闭环。
            self.assertNotIn("COMPUTER_USE_FULL_DESKTOP_TASK_ROUTER_OK", output)  # 修改代码+ModelLoopSemanticPlanner：断言旧桌面 runtime 成功 token 不再出现；如果没有这一行，黑盒 runtime 可能继续伪装成模型主循环。
            self.assertFalse(any(event.get("kind") == "computer_use_full_desktop_runtime" for event in agent.observation_events))  # 新增代码+ModelLoopSemanticPlanner：断言审计事件没有旧 runtime 记录；如果没有这一行，返回文本以外的副作用可能漏检。
    # 修改代码+ModelLoopSemanticPlanner：函数段结束，test_model_loop_computer_use_run_prompt_requires_observe_action_tools_instead_of_runtime_black_box 到此结束；如果没有这个边界说明，代码小白不容易看出 run_prompt 降级测试范围。
    ...
    def test_default_agent_runtime_injects_universal_desktop_adapter_not_paint_loop(self) -> None:  # 修改代码+UniversalDesktopAdapter：函数段开始，验证默认生产 runtime 接入通用桌面 adapter 而不是 Paint/Pikachu 专用闭环；如果没有这个测试，后续可能再次把通用入口误接到特例 loop 或空 loop。
        ...
        with tempfile.TemporaryDirectory() as raw_dir:  # 新增代码+UniversalLoopSlimming：创建一次性 workspace；如果没有这一行，测试可能污染用户真实 memory。
            ...
            self.assertFalse(hasattr(agent, "_desktop_task_runtime_for_current_run"))  # 新增代码+RuntimeFactorySlimming：断言默认 runtime 工厂不再挂在 LearningAgent 主类；如果没有这一行，旧兼容入口可能继续误导主循环设计。
            runtime = build_default_desktop_task_runtime(Path(raw_dir))  # 修改代码+RuntimeFactorySlimming：通过 computer_use 模块级工厂构造默认 runtime；如果没有这一行，测试无法验证迁移后的默认接线。
    ...
    def test_default_desktop_runtime_uses_universal_adapter_with_real_dispatch_shape(self) -> None:  # 修改代码+ModelLoopComputerUse：函数段开始，验证底层桌面 runtime 仍能进入通用 adapter 并识别真实派发形状；如果没有这个测试，移除 agent.run 抢跑后可能误删底层执行能力。
        ...
        with tempfile.TemporaryDirectory() as raw_dir:  # 新增代码+UniversalDesktopAdapter：创建一次性 workspace；如果没有这一行，full mode 状态会污染真实项目。
            ...
            self.assertFalse(hasattr(agent, "_desktop_task_runtime_for_current_run"))  # 新增代码+RuntimeFactorySlimming：断言旧主类 runtime 工厂不可用；如果没有这一行，测试仍可能默认旧路线存在。
            runtime = build_default_desktop_task_runtime(Path(raw_dir))  # 修改代码+RuntimeFactorySlimming：通过新模块级工厂取得默认 runtime；如果没有这一行，迁移后的真实派发形状无法被测试覆盖。
    ...
    def test_visual_semantic_planner_does_not_claim_tree_specific_planning_in_code(self) -> None:  # 修改代码+ModelLoopSemanticPlanner：函数段开始，验证“画树”不再被代码层声明成树专用 planner；如果没有这个测试，项目可能继续往对象特例里堆功能。
        from learning_agent.computer_use.universal_desktop_execution_loop import UniversalDesktopExecutionLoopAdapter  # 修改代码+ModelLoopSemanticPlanner：导入用户真实路径会使用的 adapter；如果没有这一行，只测底层函数无法证明入口边界。
        from learning_agent.computer_use.visual_semantic_planner import phase122_plan_visual_semantic_task  # 修改代码+ModelLoopSemanticPlanner：导入旧视觉 planner 用于确认它没有树专用动作；如果没有这一行，树特例回流无法被发现。
        adapter = UniversalDesktopExecutionLoopAdapter()  # 修改代码+ModelLoopSemanticPlanner：创建默认 adapter 但不触碰真实桌面；如果没有这一行，测试没有可调用的任务包装对象。
        prompt = "请使用本地电脑的画图软件画一棵树。"  # 修改代码+ModelLoopSemanticPlanner：使用本轮真实失败的用户 prompt；如果没有这一行，测试可能覆盖不到树语义外置规划问题。
        task = adapter._task_from_prompt("mspaint", prompt)  # 修改代码+ModelLoopSemanticPlanner：通过真实 adapter 抽取结构化任务；如果没有这一行，无法证明当前代码层是否仍在声明树专用语义。
        actions = phase122_plan_visual_semantic_task(task, {"target_window": {"app_id": "mspaintapp", "rect": {"left": 0, "top": 0, "right": 1200, "bottom": 800}}}, "test_tree_planner")  # 修改代码+ModelLoopSemanticPlanner：用安全观察帧运行旧 planner；如果没有这一行，断言没有真实动作列表可检查。
        roles = {str(action.get("semantic_role", "")) for action in actions if isinstance(action, dict)}  # 修改代码+ModelLoopSemanticPlanner：收集每个动作的语义角色；如果没有这一行，测试只能看动作数量而不知道是否出现树特例。
        self.assertEqual("generic", task["visual_subject_hint"])  # 修改代码+ModelLoopSemanticPlanner：断言代码层不再把树硬编码为专用主题；如果没有这一行，后续可能继续堆树/猫/象特例。
        self.assertFalse(task["visual_intent"]["known_subject"])  # 修改代码+ModelLoopSemanticPlanner：断言通用 adapter 不声明树是代码已知主题；如果没有这一行，成熟度报告会误导用户以为语义理解已完成。
        self.assertFalse(task["subject_specific_planning"])  # 修改代码+ModelLoopSemanticPlanner：断言任务报告不再声称对象专用规划；如果没有这一行，外置 planner 会继续冒充模型主循环能力。
        self.assertNotIn("tree_trunk", roles)  # 修改代码+ModelLoopSemanticPlanner：断言没有树干 primitive；如果没有这一行，代码层树特例会重新混入。
        self.assertNotIn("tree_canopy", roles)  # 修改代码+ModelLoopSemanticPlanner：断言没有树冠 primitive；如果没有这一行，代码层对象 planner 会继续膨胀。
        self.assertNotIn("tree_branches", roles)  # 修改代码+ModelLoopSemanticPlanner：断言没有树枝 primitive；如果没有这一行，树专用动作会继续绕过模型主循环。
        self.assertNotIn(prompt, str(task))  # 修改代码+ModelLoopSemanticPlanner：断言结构化任务不泄露完整原始 prompt；如果没有这一行，语义边界改造可能破坏隐私边界。
    # 修改代码+ModelLoopSemanticPlanner：函数段结束，test_visual_semantic_planner_does_not_claim_tree_specific_planning_in_code 到此结束；如果没有这个边界说明，代码小白不容易看出树特例清理测试范围。
