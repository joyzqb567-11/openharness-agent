...
from learning_agent.computer_use.universal_target_session import PHASE117_UNIVERSAL_TARGET_SESSION_MARKER, PHASE117_UNIVERSAL_TARGET_SESSION_OK_TOKEN, Phase117OwnedWindowProbe, UniversalTargetSessionRuntime, main, phase117_universal_target_session_cli_line, run_phase117_universal_target_session_contract  # 修改代码+ProxyWindowBinding：导入真实窗口探测器以测试代理进程窗口绑定；如果没有这一行，微信 pid 不一致场景没有单元护栏。
...
class _Phase117ProxyWindowInventory:  # 新增代码+ProxyWindowBinding：类段开始，模拟微信这类启动 pid 与可见窗口 pid 不一致的窗口清单；如果没有这个 fake，测试无法复现真实微信失败形态。
    def snapshot(self) -> dict[str, Any]:  # 新增代码+ProxyWindowBinding：函数段开始，返回一帧稳定窗口快照；如果没有这段函数，Phase117OwnedWindowProbe 没有可读窗口来源。
        return {"windows": [{"pid": 26164, "window_process_id": 26164, "hwnd": 42734532, "window_id": "hwnd:42734532", "process_name": "Weixin.exe", "app_id": "Weixin.exe", "title_preview": "微信", "title": "微信", "rect": {"left": 812, "top": 326, "right": 1108, "bottom": 714}}]}  # 新增代码+ProxyWindowBinding：返回与启动 pid 不同但标题命中目标的微信窗口；如果没有这一行，测试不会覆盖代理窗口绑定。
    # 新增代码+ProxyWindowBinding：函数段结束，_Phase117ProxyWindowInventory.snapshot 到此结束；如果没有这个边界说明，用户不容易看出 fake 快照范围。
# 新增代码+ProxyWindowBinding：类段结束，_Phase117ProxyWindowInventory 到此结束；如果没有这个边界说明，用户不容易看出代理窗口 fake 范围。
...
class UniversalTargetSessionTests(unittest.TestCase):  # 新增代码+URG2UniversalTargetSession：类段开始，集中验收 URG-2 目标 session 和身份防漂移；如果没有这个类，蓝图 URG-2 没有自动化护栏。
    ...
    def test_owned_window_probe_finds_proxy_window_by_target_title_when_pid_differs(self) -> None:  # 新增代码+ProxyWindowBinding：函数段开始，验证微信这类代理窗口 pid 不等于启动 pid 时仍能按目标标题绑定；如果没有这段测试，真实微信会启动进程但找不到窗口。
        probe = Phase117OwnedWindowProbe(inventory_probe=_Phase117ProxyWindowInventory(), poll_attempts=1, poll_interval_seconds=0.0)  # 新增代码+ProxyWindowBinding：创建只轮询一次的 fake 窗口探测器；如果没有这一行，测试会依赖真实桌面窗口。
        window = probe.find_owned_window({"process_id": 11720, "process_executable": "微信.lnk", "owned_process_registered": True}, target_hint="微信")  # 新增代码+ProxyWindowBinding：用启动 pid 和中文目标提示查找代理窗口；如果没有这一行，断言没有被测结果。
        self.assertEqual(42734532, window.get("hwnd"))  # 新增代码+ProxyWindowBinding：断言找到标题为微信的真实窗口句柄；如果没有这一行，pid 不一致仍可能返回空窗口。
        self.assertEqual("微信", window.get("title_preview"))  # 新增代码+ProxyWindowBinding：断言窗口标题来自目标应用；如果没有这一行，可能误绑定无关窗口。
        self.assertTrue(window.get("agent_owned_proxy_window"))  # 新增代码+ProxyWindowBinding：断言报告标记这是代理窗口绑定；如果没有这一行，审计时看不出为什么 pid 可以不同。
    # 新增代码+ProxyWindowBinding：函数段结束，test_owned_window_probe_finds_proxy_window_by_target_title_when_pid_differs 到此结束；如果没有这个边界说明，用户不容易看出代理窗口回归范围。
