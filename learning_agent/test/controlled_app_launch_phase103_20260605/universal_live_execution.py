...
class _Phase93ClosedLoopActor:  # 新增代码+Phase93UniversalLiveExecutionGate：定义 Phase93 闭环动作器；如果没有这行代码，Phase68 执行器没有 act 阶段。
    ...
    def __init__(self, window: dict[str, Any], safety_boundary: WindowsRealAppSafetyBoundary, grant_store: WindowsComputerUsePersistentGrantStore, mode_store: Any | None, session_id: str, control_runtime: WindowsGenericControlActionRuntime, input_runtime: WindowsGenericInputActionRuntime, request_real_actions: bool, controlled_launch_candidate: Any | None = None) -> None:  # 修改代码+Phase103ControlledAppLaunch：增加受控启动候选依赖；如果没有这行代码，full launch_app 仍只能停在 Phase102 记录假动作。
    ...
    def act(self, step: dict[str, Any], observation: dict[str, Any]) -> dict[str, Any]:  # 修改代码+Phase102FullModeExecutionGate：定义动作方法；如果没有这行代码，闭环执行器无法进入 action_class-aware act 阶段。
        ...
        if action_class == "launch_app":  # 新增代码+Phase102FullModeExecutionGate：识别 full 模式启动应用动作；如果没有这行代码，授权后仍会走点击/滚动旧路径。
            ...
            controlled_launch = launch_method(target_app, enable_real_launch=False) if callable(launch_method) else {"ok": True, "decision": "phase102_recording_only_without_phase103_candidate", "controlled_launch_candidate_ready": False, "safe_start_process_plan": False, "real_app_launch_default_disabled": True, "backend_launch_reaches_launcher": False, "real_dispatch_performed": False, "real_desktop_touched": False, "low_level_event_count": 0, "target_app": target_app}  # 新增代码+Phase103ControlledAppLaunch：默认关闭调用候选或回退 Phase102 记录；如果没有这行代码，full launch_app 无法兼容新旧路径。
            launch_recording = {"ok": True, "operation": "launch_app", "recording_only": True, "real_dispatch_performed": bool(controlled_launch.get("real_dispatch_performed", False)), "low_level_event_count": _phase93_low_level_count(controlled_launch), "target_app": target_app}  # 修改代码+Phase103ControlledAppLaunch：启动记录携带候选目标和零事件统计；如果没有这行代码，Phase102 旧字段和 Phase103 新字段无法同时被回归读取。
...
class UniversalWindowsLiveExecutionGate:  # 新增代码+Phase93UniversalLiveExecutionGate：定义公开运行时类；如果没有这行代码，测试和外部 agent 无法复用 Phase93 能力。
    ...
    def run_prompt(self, prompt: Any, request_real_actions: bool = False) -> dict[str, Any]:  # 新增代码+Phase93UniversalLiveExecutionGate：定义 prompt 运行入口；如果没有这行代码，外部 agent 只能调用低层零散组件。
        ...
        actor = _Phase93ClosedLoopActor(window, self.safety_boundary, self.grant_store, self.mode_store, session_id, self.control_runtime, self.input_runtime, bool(request_real_actions), self.controlled_launch_candidate)  # 修改代码+Phase103ControlledAppLaunch：创建动作器并注入受控启动候选；如果没有这行代码，full launch_app 不会进入 Phase103 路径。
