...
- 未完成门禁：真实可见 GUI OAuth 验收证据缺失，eal_codex_login_acceptance.json 与 direct_oauth_acceptance.json 当前不存在，不能声明真实模型连接开发完成。
- V1C 外部阻塞：缺少 OpenHarness 自有或明确授权的 OPENHARNESS_OPENAI_CLIENT_ID 以及用户肉眼完成的真实浏览器 OAuth/GUI 截图证据；代码层已禁止默认复用 OpenCode client id，这是有意的安全/产品边界。
- 过程问题：复制学习副本到较深目录时触发 Windows 路径长度失败，已改用短目录 learning_agent/test/v1c_source_copies 解决。

## 2026-06-26 V1A visible GUI real model acceptance passed
- V1A 缺口已关闭：真实 GUI provider connected、真实模型回答、事件流和 acceptance JSON 均已生成并通过断言。
- V1C 缺口仍存在：direct_oauth_evidence/direct_oauth_acceptance.json 仍缺失；没有明确的 OpenHarness 自有/授权 OPENHARNESS_OPENAI_CLIENT_ID 和用户完成 direct OAuth 浏览器回调证据，因此不能声明整份蓝图全部完成。

## 2026-06-26 V1C direct OAuth visible QA script added
- V1C 剩余阻塞仍是外部 OAuth 配置：没有 OpenHarness 自有或用户明确授权的 OPENHARNESS_OPENAI_CLIENT_ID，因此不能合法执行 direct OAuth 浏览器授权并生成 direct_oauth_acceptance.json。
- 过程修复：新增 PowerShell 脚本初次运行遇到 Windows PowerShell 5.1 无 BOM UTF-8 中文注释解析问题，已通过 PowerShell Set-Content -Encoding UTF8 重新编码解决。

## 2026-06-26 Provider test connection OAuth/Codex probe bug
- Closed risk: 真实 GUI 中 OpenAI provider 显示 `已连接 / Codex ChatGPT login`，但“测试连接”返回 `缺少密钥`。这会误导用户认为真实模型连接没有成功。
- Root cause: `test_provider_connection()` 的旧实现只支持 API key 和 OpenAI-compatible `/models` 探针，没有区分 `codex_cli` / `direct_oauth` 连接来源；OAuth/Codex 的非 API-key 状态被错误落入缺密钥分支。
- Fix: `codex_cli` 模式新增登录状态探针；`direct_oauth` 模式新增 `oauth_direct + token_ref` 记录探针；两条路径都不读取或返回 raw token。
- Verification: `python -m pytest learning_agent/tests/test_gui_provider_settings_contract.py -q` 为 7 passed；完整相关 Python suite 57 passed；真实 `computer-use` 窗口复测中 OpenAI 测试连接显示 `连接测试通过`，主聊天返回真实模型回答并出现 `message_delta/message_completed`。
- Remaining boundary: 这关闭的是 Codex CLI/Direct OAuth 已连接后测试按钮误报问题，不等于 direct OAuth 真实浏览器授权已完成；direct OAuth 仍需要明确 client id 和用户授权回调证据。

## 2026-06-26 OpenAI disconnect after Codex CLI login bug
- Closed risk: 用户在真实 OpenHarness Desktop GUI 中发现 OpenAI 连接后无法断开；点击“断开”后，OpenAI 仍可能因外部 Codex CLI 登录态再次显示为 `已连接`。
- Root cause: `disconnect_provider()` 旧逻辑只清理 OpenHarness 本地 auth/API-key 记录，没有为 `codex_cli` 这种外部登录源建立本地禁用覆盖层；catalog 重新加载时又读取 `CodexAuthBridge().login_status()`，所以断开状态被外部登录态覆盖。
- Fix: `codex_cli` 断开写入本地 `auth["openai"].type="disabled"`，catalog 先看 disabled 覆盖层再看 Codex CLI；direct OAuth 断开同步删除 token store；Codex CLI 重连时清除 disabled 覆盖层。
- Verification: Python provider/OAuth suite 60 passed；frontend vitest 72 tests passed；py_compile passed；真实 `computer-use` 窗口复测中 OpenAI 点击断开后显示 `+ 连接`，关闭设置再打开仍显示 `+ 连接`。
- Evidence: `learning_agent/test/provider_settings_v2_openai_connect/computer_use_visible_evidence/computer_use_visible_openai_disconnect_fix.json` 记录 `provider_connected_after=false`、`provider_source_after="none"`、`persisted_auth_type="disabled"`、`secret_leak_detected=false`。
- Boundary: 该修复只让 OpenHarness Desktop 本地 GUI 断开 OpenAI provider，不会执行全局 `codex logout`，因此不会破坏用户在其他终端或工具中的 Codex 登录状态。

## 2026-06-26 Composer model selection missing after OAuth connection
- Closed risk: OpenHarness Desktop 即使 OpenAI/OAuth provider 连接成功，主聊天底部仍没有模型选择、权限模式和推理强度入口，用户不知道到哪里选择大模型。
- Root cause: 旧 `Composer.tsx` 只有附件、textarea、发送/取消按钮；Provider Settings 的模型 catalog 没有传入 AppShell/Composer，`sendMessage()` 也只发送 prompt 和 conversation_id。
- Fix: Composer 改为 Codex 式双层工具条，AppShell 从 Provider Settings catalog 派生已连接 provider 的可见模型，并把 `provider_id/model_id/reasoning_effort/permission_mode` 透传到 bridge。
- Verification: 前端全量测试 75 passed；相关 Python GUI 合同 22 passed；lint/build/py_compile 通过；真实 `computer-use` 窗口截图和可访问树确认底部控件可见且可识别。
- Remaining boundary: 这关闭的是“GUI 无模型入口和上下文丢失”风险；模型工厂是否按 `model_id` 动态切换仍属于后续真实模型 runtime 路由增强。

## 2026-06-26 Composer selected model ignored by real model factories
- Closed risk: 用户在底部选择 GPT-5.5/GPT-4.1 等模型后，真实 OpenAI/ChatGPT OAuth 调用仍可能使用 `CODEX_MODEL` 或启动时固定 adapter，导致“看起来能选模型，实际没有按模型调用”。
- Root cause: `RealModelGuiAgentAdapter` 旧模型工厂合同没有接收 `GuiAgentRunRequest`，`_default_real_model_factory()` 和 `CodexCliChatModel.from_env()` 只读取环境变量；同时 `GuiRunManager` 在 auto 模式启动时未连接 provider 会把全局模式改成 fake，导致后续连接 OAuth 后仍 fake streaming。
- Fix: `RealModelGuiAgentAdapter` 增加请求感知工厂调用和 provider 感知连接门禁；direct OAuth 与 Codex CLI/OAuth 兼容两条工厂路径都优先读取 `request.model_id`；`GuiRunManager` auto 模式每轮重新检查 OpenAI provider 连接状态。
- Verification: 新增 CLI/OAuth 兼容路径红测初次失败于 `gpt-env-should-not-win` 覆盖 GUI 选择，修复后通过；相关 Python suite 62 passed、前端 Composer/Client/Provider store 25 passed、py_compile 和 desktop lint 均通过。
- Remaining boundary: 自动测试证明请求路由和模型对象选择正确；真实外部模型回答仍必须在可见 OpenHarness Desktop GUI 中用已登录 OpenAI/ChatGPT OAuth 状态做人工或 computer-use 验收后才能声明通过。

## 2026-06-26 OpenAI OAuth catalog offered unsupported gpt-4.1
- Closed risk: 真实 GUI 发送 `model_id=gpt-4.1` 后已经进入 `mode=real`，但 Codex ChatGPT 账号真实后端拒绝该模型，导致用户看到真实调用失败。
- Root cause: Provider Settings 早期把 OpenAI API key 场景的 `gpt-4.1` 复用到了 `codex_cli` / ChatGPT OAuth 场景；这两个场景模型权限不是同一套，不能共用静态模型列表。
- Fix: `gui_provider_settings.py` 增加 Codex OAuth 模型目录分流：`codex_cli` 和 `direct_oauth` 模式读取 `codex debug models` 可见模型，失败时使用已验证 fallback；默认 mock/API key 模式保留普通 OpenAI API 模型。
- Fix: `codex debug models` 读取固定 UTF-8 解码，关闭 Windows GBK 解码警告。
- Verification: `/v2/gui/provider-settings/providers` 在 `OPENHARNESS_OPENAI_AUTH_MODE=codex_cli` 下返回 `gpt-5.5/gpt-5.4/gpt-5.4-mini/gpt-5.3-codex-spark`，不再返回 `gpt-4.1`；真实可见 GUI 发送后事件流显示 `model_id=gpt-5.5`、`mode=real`、最终回答 `gpt-5.5`。
- Boundary: 这证明 OpenHarness Desktop 能按所选 Codex ChatGPT OAuth 模型真实调用；普通 OpenAI API key 模型集合仍需要单独的 API key 模型发现/配置策略。

## 2026-06-26 GUI adapter contract status.lock cleanup race
- Closed risk: 批量运行 GUI adapter contract 测试时，Windows 偶发在删除临时 workspace 的 `memory/status/status.lock` 时抛出 WinError 32，导致真实模型路由验收的后端测试组不稳定。
- Root cause: `GuiRunManager` 的 worker 是 daemon thread，测试只等到 `turn.status == "completed"`，但 worker 的 `finally` 还可能正在释放 `active_turn_id` 或刚退出 `FileLock`；`TemporaryDirectory` 立刻清理目录时会撞上 Windows 文件句柄。
- Fix: 测试新增 `_wait_for_manager_turn_idle()`，只在 turn 到达终态且 `manager.active_turn_id is None` 后退出临时目录，等待真实生命周期收尾而不是使用固定 sleep。
- Verification: `python -m pytest learning_agent/tests/test_gui_agent_adapter_contract.py -q` 为 11 passed；相关后端组合 `test_gui_provider_settings_contract.py test_gui_agent_adapter_contract.py test_gui_direct_oauth_model_factory.py test_gui_real_model_adapter.py` 为 34 passed。

## 2026-06-26 Real model reply appears blank for about 2 minutes
- Open risk: 用户在真实 OpenHarness Desktop GUI 发送 `你好` 后，界面长时间只显示“正在等待后端响应”，大约 2 分钟后才出现回答。
- Evidence: 事件流显示 `gui_turn_queued` 到 `turn_started` 基本同秒完成，但 `turn_started` 到首个 `message_delta` 约 122-128 秒；截图中的后端日志显示 `Responses WebSocket timed out` 和 `Falling back from WebSockets to HTTPS transport`。
- External cause: 当前机器的 Codex CLI ChatGPT OAuth WebSocket handshake timeout，随后 fallback 到 HTTPS；这是外部 Codex transport/网络状态问题，OpenHarness 不能直接把它伪装成低延迟。
- Internal cause: OpenHarness 的真实模型 adapter 仍用阻塞式 `model.chat(...)`，`CodexCliChatModel` 内部用 `subprocess.run(...)` 等完整结果，GUI 只能在完整结果后 fake streaming，所以用户看不到连接、timeout、fallback 或首 token 阶段。
- Fix plan: 已制定 `docs/superpowers/plans/2026-06-26-openharness-desktop-real-model-latency-v2.md`，按计划实现 streaming event contract、可观测 Codex CLI runner、Direct OAuth SSE 快路径、前端模型阶段显示和真实取消。
- Boundary: 在 Codex CLI WebSocket timeout 未解决且没有 Direct OAuth SSE 快路径时，V2 也不能承诺所有真实回答都几秒完成；必须至少做到 2 秒内可见状态、可取消、错误可读、真实 token 到达时立即显示。

## 2026-06-26 Real model cancel did not stop Codex CLI child process
- Closed risk: 真实 OpenHarness Desktop 窗口中点击底部红色取消按钮后，UI 发出了取消请求，但真实 Codex CLI 慢链路仍可能继续运行到完整完成，用户会以为取消无效。
- Evidence: 可见 GUI 复现时，点击取消后旧实现没有立刻出现 `gui_turn_cancelled`，随后 turn 仍完成并输出结果；这说明 bug 在后端 stream/subprocess 层，而不只是按钮显示问题。
- Root cause: `RealModelGuiAgentAdapter._run_streaming_model()` 旧逻辑只在 model generator 产出事件时检查 `is_cancelled()`；`CodexCliStreamingRunner.stream()` 自己没有接收外部 cancellation checker，因此当 Codex CLI 卡在 WebSocket timeout 或 HTTPS fallback 时，GUI 取消信号进不到真实子进程层。
- Fix: `CodexCliStreamingRunner.stream()` 新增可选 `is_cancelled` callback，并在每轮 poll 时检查；一旦取消，调用 `CodexCliProcessController.cancel()` 终止 Windows 子进程树并产出取消事件。`CodexCliChatModel.stream_chat()` 和 `RealModelGuiAgentAdapter` 已透传该 callback。
- Verification: Python 测试新增覆盖 runner 外部取消、model 透传取消 checker、GUI adapter 传入 cancellation checker；前端测试覆盖 running 状态下底部取消按钮渲染。真实 GUI 复测连续两次点击取消后，消息状态为 `cancelled`，事件顺序为 `gui_turn_cancel_requested` -> `gui_turn_cancelled`。
- Boundary: 该修复保证 OpenHarness 后端能及时终止自己启动的 Codex CLI 进程；如果外部 provider/CLI 本身已在远端排队，OpenHarness 只能停止本地等待和后续 UI 回写，不能取消远端已不可控的服务端计算。
