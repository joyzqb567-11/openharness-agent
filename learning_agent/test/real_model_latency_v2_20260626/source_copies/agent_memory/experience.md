...
## 2026-06-26 Provider/OAuth 经验：测试按钮不能复用单一 API-key 假设
- 经验：一个 provider 行显示“已连接”不代表所有后续按钮都自动理解该连接来源；catalog、probe、chat adapter 是三条不同链路，必须分别按 `source/auth_mode` 分流。
- 做法：API key 和 OpenAI-compatible 自定义 provider 可以用 `/models` 探针；Codex CLI 登录态应只检查 CLI login status；direct OAuth 应只检查非敏感 `oauth_direct + token_ref` 记录，不能读取或返回 raw token。
- 验收：真实 GUI 验收时要同时点击 provider “测试连接”和发送一条主聊天 prompt；前者证明 settings/probe 合同，后者证明 real adapter/model runtime。只看其中一个会漏掉“已连接但测试缺密钥”或“测试通过但聊天仍 fake”的问题。
## 2026-06-26 Provider/OAuth 经验：断开按钮必须断开“本地启用状态”，不能假设等于清除外部登录态
- 经验：`codex_cli` 这类连接来源是外部工具登录态，不是 OpenHarness 自己保存的 API key 或 OAuth token；如果断开按钮只删除本地 auth 记录，catalog 下一次读取外部登录态时会把 provider 重新显示为已连接。
- 做法：provider 状态机要把“外部账号仍然登录”和“OpenHarness Desktop 当前启用该 provider”拆成两层。OpenAI `codex_cli` 断开时应写入本地 `disabled` 覆盖层，不应该执行全局 `codex logout`。
- 做法：重新连接时必须显式清除 `disabled` 覆盖层，并且前端要处理 start attempt 立即返回 `complete` 的情况，马上刷新 catalog。
- 验收：这类 bug 不能只看后端断开接口返回成功；必须在真实 GUI 中点击断开、关闭设置、重新打开设置，确认 provider 没有被 catalog 复活，同时检查持久化文件里的 disabled 覆盖层。

## 2026-06-26 Real Model Latency 经验：取消按钮必须打到最底层等待点
- 经验：真实模型慢链路里的“取消”不是 UI 状态问题，而是运行时所有等待点的问题；如果 generator 只有在产出事件后才检查取消，那么 WebSocket timeout、HTTPS fallback、子进程等待都会让取消失效。
- 做法：GUI cancel signal 要作为 callback 传进 streaming model，再传进 subprocess/HTTP/SSE runner；runner 的 poll loop 每轮都要检查取消，并负责终止自己启动的 process tree 或关闭 stream。
- 验收：不能只看按钮变红、消息标记 cancelled 或接口返回成功；必须在真实可见 GUI 中点击取消，同时检查事件序列出现 `gui_turn_cancel_requested` -> `gui_turn_cancelled`，并用自动测试证明底层 runner 在外部取消时会终止等待。
- 边界：OpenHarness 可以取消本地等待和自己启动的子进程，不能保证远端 provider 已排队的计算一定被服务端撤销；因此最终文案要说“本地 turn 已取消并停止回写”，不要夸成“远端请求一定撤销”。
