...
Task 12 已明确桌面壳视觉边界：`layout.css` 的主网格必须在 1024x720 保持三栏，不能再用隐藏 `.status-inspector` 的方式解决紧凑窗口问题；右侧五个 tabs 要持续可见，tab 文字可截断但不能互相覆盖。Composer 高度由 `thread-panel` 的 `minmax(88px, 112px)` 和 `.composer`/`.composer-input` 的 min/max 控制，后续新增输入附件、工具卡或设置项时必须保持底部输入区稳定，不允许让页面外层滚动。

Task 12 的视觉验收证据位于 `learning_agent/test/desktop_gui_shell_v2/task12_visual_accessibility/`，包含真实 Electron 窗口截图、1280x800 截图、1024x720 截图和本次改动代码副本。后续修改布局时优先复用这组 smoke checklist，不要只用单元测试代替可见 GUI 验收。

Task 8 已建立项目/会话/搜索边界：`GuiRunManager` 现在是桌面会话事实源，`GuiSession` 包含 `title/archived/pinned/updated_at/messages/last_turn_id`。`/v2/gui/sessions` 默认隐藏归档会话，`include_archived=true` 用于归档视图；`/v2/gui/search?q=...` 默认搜索未归档会话，`include_archived=true` 用于归档过滤里的搜索。

前端 Task 8 数据流是：`createGuiClient().sessions()` 拉取 `/v2/gui/sessions`，`AppShell.tsx` 保存 `sidebarSessions` 和 `archivedCount`，`Sidebar.tsx` 只渲染项目、入口、active 会话和归档计数，不直接 fetch。搜索入口打开普通 `SearchPanel`，归档入口调用 `sessions(true)` 后只展示 `archived` 会话；在归档模式下继续输入关键词时，`searchSessions(query, true)` 会包含归档范围。

后续如果要新增 rename/pin/archive 的完整 UI 控件，应继续调用 `guiClient.renameSession()`、`guiClient.archiveSession()` 并在成功后复用 `refreshSidebarSessions()`，不要在 React state 里做永久假状态。`pin` 字段已经在 payload 和 Sidebar 标记中预留，但当前 Task 8 未新增 pin 写入 endpoint。

Task 10 已建立长任务 Harness GUI 边界：`learning_agent/app/gui_bridge.py` 中的 `build_gui_harness_status_payload(workspace)` 是 `/v2/gui/harness/status` 的唯一 payload 生成点，输出 `active_goal/queue/checkpoints/last_progress/blocked_reason/safe_error/controls`。该 payload 从 `build_status_snapshot(workspace)`、run stages、runtime command queue 和 `status_events` 中提取事实；读取失败时只返回安全错误文案，不泄露本机路径。
...
- OpenHarness Desktop 现在具备两条真实模型连接路径：默认稳定路径 codex_cli 通过官方 codex login/status + CodexCliChatModel；实验路径 direct_oauth 通过显式 client id、PKCE、localhost callback、OS encrypted token store + CodexOAuthChatModel。
- direct_oauth 仍是 experimental：必须设置 OPENHARNESS_OPENAI_AUTH_MODE=direct_oauth、OPENHARNESS_OPENAI_EXPERIMENTAL=1、OPENHARNESS_PROVIDER_SECRET_STORE=os_encrypted、OPENHARNESS_OPENAI_CLIENT_ID=<明确测试或自有 client id>，不得默认使用 OpenCode client id。
- renderer/provider catalog 只展示连接状态、provider source 和 masked label；raw access_token/refresh_token/id_token 不允许进入 renderer payload 或证据包。
- Provider “测试连接”必须按 provider source/auth mode 分流：API key/custom provider 才走 OpenAI-compatible `/models`；`codex_cli` 走 Codex CLI 登录状态；`direct_oauth` 走非敏感 `oauth_direct + token_ref` 记录检查。不能再让 OAuth/Codex 已连接状态落入 API key 缺失分支。
- Provider “断开”也必须按 provider source/auth mode 分流：API key/custom provider 删除本地 secret；`direct_oauth` 同时删除 token store；`codex_cli` 不能全局执行 `codex logout`，只能写入 OpenHarness 本地 `auth["openai"].type="disabled"` 覆盖层，让 GUI 显示断开且不影响用户的外部 Codex CLI 登录。
- Provider catalog 必须让本地 disabled 覆盖层优先于外部 Codex CLI 登录态；否则用户点击“断开”后，catalog 会再次读取 `codex auth status` 并把 OpenAI 复活为已连接。
- Provider 重新连接必须清除 disabled 覆盖层；`codex_cli` 重新连接如果 start attempt 立即返回 `complete`，前端要立刻刷新 provider catalog，不能只依赖 pending 轮询。

## 2026-06-26 Composer Model Toolbar Context
- OpenHarness Desktop 底部 Composer 现在是 Codex 式双层输入区：上层输入 prompt，下层包含添加入口、权限模式、模型选择、推理强度、语音入口和发送/取消按钮。
- 模型菜单不写死：`providerSettingsStore.composerModelOptionsForDisplay()` 只从已连接 provider 的可见模型生成菜单项；AppShell 首屏加载和设置弹窗关闭后都会刷新 provider catalog。
- 提交链路现在随 prompt 传递 `provider_id`、`model_id`、`reasoning_effort`、`permission_mode` 到 `/v1/gui/messages`；`GuiTurn`、`gui_turn_queued` 事件和 `GuiAgentRunRequest` 都保留这些上下文。
- 真实模型工厂现在会消费本轮 `GuiAgentRunRequest.model_id`：direct OAuth 路径创建 `CodexOAuthChatModel(model=request.model_id)`，Codex CLI/OAuth 兼容路径创建 `CodexCliChatModel(model=request.model_id)`；没有 GUI 选择时才回退 `CODEX_MODEL`。
- `GuiRunManager` 的 `auto` 模式现在允许“窗口先启动、后连接 OAuth、下一条消息切到 real adapter”，不会再因为启动时未连接而永久落回 fake streaming。
- OpenAI provider 的模型菜单必须按认证模式分流：`codex_cli` / `direct_oauth` 是 ChatGPT OAuth/Codex 模型集合，当前从 `codex debug models` 读取可见模型并回退 `gpt-5.5/gpt-5.4/gpt-5.4-mini/gpt-5.3-codex-spark`；默认 mock/API key 模式才使用普通 OpenAI API 模型 `gpt-4.1`。
- 真实 GUI 验收已证明底部 `GPT-5.5` 会随 prompt 提交为 `model_id=gpt-5.5`，后端事件进入 `turn_started.mode=real`，最终真实模型回答 `gpt-5.5`；后续不要再用 `gpt-4.1` 作为 ChatGPT OAuth 默认模型。

## 2026-06-26 Real Model Latency Context
- 当前真实 GUI 慢回复的已知事实：bridge 和 turn queue 很快，慢点在真实模型调用阶段；`turn_started` 到首个 `message_delta` 约 2 分钟。
- 真实外部原因：Codex CLI ChatGPT OAuth 链路在当前机器上 WebSocket handshake timeout，然后 fallback 到 HTTPS。
- OpenHarness 自身代码原因：后端用阻塞式 `chat()` 等完整结果，GUI adapter 再 fake streaming，导致用户在 2 分钟内看不到连接阶段、fallback 阶段或真实首 token。
- 下一步应按升级版 `docs/superpowers/plans/2026-06-26-openharness-desktop-real-model-latency-v2.md` 执行：先做 Task 0 baseline measurement，再做 streaming event contract、后端 phase、前端状态显示、真实取消，然后再做 Codex CLI discovery/observable runner；Direct OAuth SSE 必须走 gated probe，不允许未验证就声明真实低延迟 streaming。

## 2026-06-26 Real Model Latency V2 Runtime Boundary
- V2 之后，真实模型调用的事实边界是：GUI turn 可以很快进入 `turn_started`，随后必须持续输出 `model_call_*` 阶段事件，让用户知道当前是在准备请求、连接 transport、等待首个真实 delta、fallback、完成、失败还是取消。
- Codex CLI ChatGPT OAuth 路径当前是“可观测、可取消、真实最终回答”，不是“已证明 token-level streaming”。`codex exec --json` discovery 没证明 stdout 提供真实 token delta，因此不能把 CLI 路径包装成真实逐 token streaming。
- Direct OAuth SSE 是实验快路径，默认关闭；只有 gated probe 证明 endpoint/payload shape 可用，且 secret store 是 OS encrypted，才允许使用并声明低延迟 streaming。
- 取消必须从 GUI 按钮一路传到 model stream/subprocess 层。仅把消息状态改成 cancelled 不够；runner 必须终止自己启动的 Codex CLI process tree 或关闭 HTTP/SSE stream。
- 可见 GUI 验收证据统一放在 `learning_agent/test/real_model_latency_v2_20260626/visible_evidence/`；真实 OAuth 问答和 deterministic slow-turn 取消证据要分开描述，避免把本地慢任务误说成外部 OpenAI 取消验收。
