...
## 2026-06-26 目标长任务 ChatGPT OAuth real model V1C 收尾
- 已实现并验证：Codex CLI 官方登录桥、provider catalog source、auth attempt 轮询、RealModelGuiAgentAdapter、fake fallback 禁止、V1C DPAPI/fake token store、PKCE authorize URL、callback server、token exchange、direct OAuth provider integration、direct OAuth model factory、token refresh 回写、ChatGPT-Account-Id header、direct OAuth 验收断言脚本。
- 自动化验证记录：backend combined pytest 55 passed；desktop vitest 71 passed；desktop lint/build passed；py_compile passed；provider secret scan passed。
- 已按 AGENTS.md 复制学习副本到 learning_agent/test/v1c_source_copies；早先长路径副本目录 learning_agent/test/provider_settings_v2_openai_connect/v1c_direct_oauth_source_copies 中只有部分成功副本，完整副本以短路径目录为准。

- 更正：完整学习副本最终位置为 `learning_agent/test/provider_settings_v2_openai_connect/sc`；临时目录 `learning_agent/test/v1c_source_copies` 和半份长路径副本已清理，避免 secret scan 误报。

## 2026-06-26 V1A visible GUI real model acceptance passed
- 已启动真实可见 OpenHarness Desktop Electron 窗口，通过 CDP 控制真实 GUI，打开设置 -> 提供商，确认 OpenAI 显示 已连接 且来源为 Codex ChatGPT login。
- 已在主聊天通过 GUI 发送 prompt：Please answer in Chinese in one sentence: are you a real model connection or a simulated reply?，后端事件流返回 	urn_started、message_delta、message_completed，回答为中文真实模型回复。
- V1A 证据目录：learning_agent/test/provider_settings_v2_openai_connect/real_codex_login_evidence/。
- ssert_real_model_acceptance.ps1 已通过，ssert_no_provider_secret_leaks.ps1 已通过。

## 2026-06-26 V1C direct OAuth visible QA script added
- 已新增 learning_agent/test/provider_settings_v2_openai_connect/scripts/capture_direct_oauth_visual_qa.ps1，用于拿到明确 OPENHARNESS_OPENAI_CLIENT_ID 后启动真实可见 GUI direct OAuth 验收。
- 该脚本会强制要求 -ClientId 或 OPENHARNESS_OPENAI_CLIENT_ID，并设置 direct_oauth、experimental=1、os_encrypted、eal adapter、callback port，再引导用户完成真实浏览器 OAuth、截图和 direct_oauth_acceptance.json 生成。
- 已验证脚本入口在缺 client id 时按预期失败，说明 PowerShell 语法和路径可解析，失败点是有意的安全门禁。

## 2026-06-26 Computer Use visible GUI OAuth/model verification
- 已按用户提醒启用 `computer-use` 控制真实 OpenHarness Desktop 窗口，而不是只依赖 CDP 或日志。
- 真实 GUI 复现 bug：设置 -> 提供商里 OpenAI 显示 `已连接 / Codex ChatGPT login`，但点击“测试连接”显示 `缺少密钥`。
- Systematic debugging 根因：`test_provider_connection()` 只按 API key secret store 探针，没有为 `codex_cli` 和 `direct_oauth` 连接状态分支，导致 OAuth/Codex 已连接状态被 API key 缺失覆盖。
- 修复：新增 `probe_openai_codex_cli_login()` 与 `probe_openai_direct_oauth_record()`，OpenAI 在 `codex_cli` 模式走 Codex CLI 登录状态探针，在 `direct_oauth` 模式走非敏感 `token_ref` 记录探针；API key provider 仍走 OpenAI-compatible `/models`。
- 回归测试：`python -m pytest learning_agent/tests/test_gui_provider_settings_contract.py -q` 为 7 passed；完整相关 Python suite 为 57 passed；`npm --prefix apps/desktop test` 为 16 files / 71 tests passed；`python -m py_compile learning_agent/app/gui_provider_settings.py learning_agent/tests/test_gui_provider_settings_contract.py` 通过。
- 真实 GUI 复测：OpenAI provider 测试连接从 `缺少密钥` 变为 `连接测试通过`；主聊天通过真实窗口发送“请用一句话说明你是真实模型连接还是模拟回复。”，事件流出现 `turn_started/message_delta/message_completed`，回答“我是真实模型连接生成的回复，不是模拟回复。”。
- 证据 JSON：`learning_agent/test/provider_settings_v2_openai_connect/computer_use_visible_evidence/computer_use_visible_acceptance_after_probe_fix.json`，其中 `provider_source=codex_cli`、`probe_status=ok`、`real_adapter_seen=true`、`message_delta_seen=true`、`fake_streaming_detected=false`、`secret_leak_detected=false`。
- 学习副本已保存到主项目 `H:\codexworkplace\sofeware\OpenHarness-main\learning_agent\test\provider_settings_v2_openai_connect\sc\26_gui_provider_settings_probe_fix.py` 和 `27_test_gui_provider_settings_contract_probe_fix.py`。

## 2026-06-26 OpenAI codex_cli disconnect bugfix
- 用户真实 GUI 测试发现：OpenAI 已通过 Codex CLI 登录源连接后，点击“断开”无法真正断开，重新打开设置仍会显示 `已连接 / Codex ChatGPT login`。
- Systematic debugging 根因：provider catalog 会直接读取外部 `codex auth status`，而旧的 `disconnect_provider()` 只删除 OpenHarness 本地 `auth["openai"]` 和 API-key secret；对 `codex_cli` 来源来说，本地没有可删除的 token，所以断开后 catalog 又立刻从外部 Codex 登录态恢复为已连接。
- 修复：`disconnect_provider()` 在 `codex_cli` 模式下写入 OpenHarness 本地 `auth["openai"].type="disabled"` 覆盖层；catalog 看到该覆盖层时显示断开，不会全局执行 `codex logout`，避免误伤用户的 Codex CLI 登录。
- 修复：direct OAuth 断开现在同时删除 `default_oauth_token_store().delete_tokens("openai")`，避免只删 provider 记录但 token 文件仍残留。
- 修复：重新连接 Codex CLI 模式时，`start_provider_auth_attempt()` 会清除本地 disabled 覆盖层；前端 `handleStartAuthAttempt()` 现在处理 `start` 立即返回 `complete` 的情况，马上刷新 provider catalog，而不是等待不会发生的 pending 轮询。
- 回归测试：新增并通过 `test_openai_codex_cli_disconnect_persists_local_disabled_override`、`test_openai_direct_oauth_disconnect_deletes_token_store_record`、`test_codex_cli_reconnect_clears_local_disabled_override` 和前端 `authAttemptStartDecision` complete-refresh 测试。
- 自动验证：相关 Python provider/OAuth suite 为 60 passed；`python -m py_compile` 通过；`npm --prefix apps/desktop test` 为 16 files / 72 tests passed。
- 真实 GUI 验收：已用 `computer-use` 控制真实 OpenHarness Desktop 窗口，打开设置 -> 提供商，确认 OpenAI 断开前显示 `已连接 / Codex ChatGPT login / 断开`，点击断开后显示 `+ 连接`；关闭设置并重新打开后仍保持 `+ 连接`。
- 后端事实验证：断开后 `/v2/gui/provider-settings/providers` 返回 OpenAI `connected=false`、`source="none"`、`masked_key=""`；`memory/gui_provider_settings/providers.json` 持久化为 `type="disabled"`、`disabled_source="codex_cli"`。
- 真实 GUI 证据 JSON：`learning_agent/test/provider_settings_v2_openai_connect/computer_use_visible_evidence/computer_use_visible_openai_disconnect_fix.json`，其中 `ok=true`、`provider_connected_after=false`、`persisted_auth_type="disabled"`、`secret_leak_detected=false`。

## 2026-06-26 Composer Model Toolbar Implementation
- 已按用户截图补齐 OpenHarness Desktop 底部 Composer 控件：`+`、`完全访问` 权限菜单、模型菜单、推理强度菜单、语音按钮和发送按钮。
- 已实现数据链路：Provider Settings catalog -> `composerModelOptionsForDisplay()` -> AppShell state -> Composer select -> `guiClient.sendMessage()` -> `GuiRunManager.start_message()` -> `GuiTurn`/queued event/adapter request。
- 自动验证通过：`npm --prefix apps/desktop test` 为 16 files / 75 tests passed；相关 Python GUI suite 为 22 passed；`python -m py_compile learning_agent/app/gui_bridge.py learning_agent/app/gui_agent_adapter.py` 通过；`npm --prefix apps/desktop run lint` 通过；`npm --prefix apps/desktop run build` 通过。
- Scoped diff check 已对本次触及源码/测试路径通过；全仓 `git diff --check` 仍会被历史 `agent_memory` 尾随空格噪声淹没，不作为本次改动失败。
- 真实 GUI 验收：已用 `computer-use` 恢复并截图真实 `OpenHarness Desktop` Electron 窗口，底部可见 `+ / 完全访问 / GPT-4.1 / 超高 / 麦克风 / 发送`；可访问树识别 `权限模式`、`选择模型`、`推理强度`、`语音输入` 和 `发送，需要先输入内容`。
- 学习副本已保存到主项目 `H:\codexworkplace\sofeware\OpenHarness-main\learning_agent\test\composer_model_toolbar_20260626\`。

## 2026-06-26 Composer Real Model Routing Implementation
- 已补齐真实模型路由：`RealModelGuiAgentAdapter` 会把完整 `GuiAgentRunRequest` 传给模型工厂，连接门禁按本轮 `provider_id` 检查，模型工厂优先使用 GUI 底部选择的 `model_id`。
- direct OAuth 路径现在按所选模型创建 `CodexOAuthChatModel`；Codex CLI/OAuth 兼容路径现在按所选模型创建 `CodexCliChatModel`，`CODEX_MODEL` 只作为无 GUI 选择时的兜底。
- 已修复 `GuiRunManager` auto 模式启动时未连接就永久 fake 的问题：每轮发送前会重新检查 OpenAI provider 是否已通过 `codex_cli` 或 `direct_oauth_experimental` 连接。
- 新增红测先复现 CLI/OAuth 兼容路径忽略 `model_id` 的 bug，修复后定向合同测试 3 passed。
- 自动验证：相关 Python GUI/provider/OAuth suite 为 62 passed；`python -m py_compile learning_agent/app/gui_agent_adapter.py learning_agent/app/gui_bridge.py learning_agent/models/adapters.py` 通过；前端 Composer/Client/Provider store 测试为 25 passed；`npm --prefix apps/desktop run lint` 通过。
- 学习副本已保存到主项目 `H:\codexworkplace\sofeware\OpenHarness-main\learning_agent\test\composer_real_model_routing_20260626\`。
- 待完成门禁：还需要重启真实 OpenHarness Desktop 窗口并做肉眼可见 GUI 验收；若要证明“真实外部 OpenAI/ChatGPT OAuth 模型回答”，仍需要当前机器存在有效 OpenAI OAuth 连接和可用模型权限。

## 2026-06-26 Composer Real OpenAI OAuth Model Acceptance
- 已用真实 OpenHarness Desktop Electron 窗口验证底部 Composer 通过 OpenAI/ChatGPT OAuth 真实调用所选模型。
- Root cause follow-up：首次可见 GUI 验收中，底部提交 `model_id=gpt-4.1`，后端已进入 `mode=real`，但 Codex ChatGPT 账号返回 `The 'gpt-4.1' model is not supported when using Codex with a ChatGPT account.`；这证明调用链真实，但模型 catalog 不适配 ChatGPT OAuth。
- 修复：OpenAI provider 在 `codex_cli` / `direct_oauth` 模式下改为读取 `codex debug models` 的可见模型；读取失败时回退到 `gpt-5.5`、`gpt-5.4`、`gpt-5.4-mini`、`gpt-5.3-codex-spark`；默认 mock/API key 模式仍保留普通 OpenAI API 模型 `gpt-4.1`。
- 修复：`codex debug models` 子进程输出固定用 UTF-8 + replace 解码，避免 Windows GBK 环境在测试或 GUI provider 加载时产生后台解码警告。
- 自动验证：相关 Python GUI/provider/OAuth suite 为 63 passed；新增/相关 provider/model routing suite 为 34 passed；`python -m py_compile` 通过；前端 Composer/Client/Provider store 测试 25 passed；`npm --prefix apps/desktop run lint` 通过。
- 真实 GUI 验收：重启 bridge 后 `/v2/gui/provider-settings/providers` 返回 OpenAI `source=codex_cli`、`connected=true`，模型列表为 `gpt-5.5/gpt-5.4/gpt-5.4-mini/gpt-5.3-codex-spark`。
- 真实 GUI 验收：通过 `computer-use` 在可见窗口发送 prompt，事件 #62 `gui_turn_queued` 包含 `provider_id=openai`、`model_id=gpt-5.5`、`reasoning_effort=ultra`、`permission_mode=full-access`，事件 #64 `turn_started.mode=real`，最终 #67 `gui_turn_completed.answer=gpt-5.5`。
- 当前 OpenHarness Desktop 可见窗口底部显示 `完全访问 / GPT-5.5 / 超高`，主聊天可见最终模型回答 `gpt-5.5`。
- 后续稳定性修复：批量运行 `test_gui_agent_adapter_contract.py` 时，Windows 偶发在 `TemporaryDirectory` cleanup 删除 `memory/status/status.lock` 时报 WinError 32；根因是测试只等到 turn.status completed，未等 worker finally 释放 `active_turn_id`。已新增 `_wait_for_manager_turn_idle()` 等待终态且 manager 空闲，相关单文件测试 11 passed，相关后端组合 34 passed。

## 2026-06-26 Real Model Latency V2 Blueprint
- 用户在真实 OpenHarness Desktop GUI 中观察到“你好”回复很慢，截图显示后端日志包含 `Responses WebSocket timed out`、`Falling back from WebSockets to HTTPS transport`，并且 GUI 长时间停留在“正在等待后端响应”。
- 已用 systematic debugging 思路确认当前慢点不是前端输入或 bridge 排队：`gui_turn_queued` 到 `turn_started` 基本同秒完成，但 `turn_started` 到首个 `message_delta` 约 122-128 秒。
- 已确认核心代码根因：`CodexCliChatModel._run_codex_process()` 使用阻塞式 `subprocess.run(...)`；`RealModelGuiAgentAdapter.run()` 等 `model.chat(...)` 完整返回后才拆分文本做 fake streaming，因此 GUI 看不到真实连接阶段和首 token。
- 已制定 V2 治本蓝图：`docs/superpowers/plans/2026-06-26-openharness-desktop-real-model-latency-v2.md`。
- 蓝图重点：新增 `StreamingChatModel`/`ModelStreamEvent` 合同、让 adapter 输出真实模型阶段事件、Codex CLI 改为可观测 Popen runner、Direct OAuth SSE 走真实 token streaming、前端显示模型调用阶段、取消按钮真正中断后端子进程或 HTTP stream。
- 学习副本已同步到主项目：`H:\codexworkplace\sofeware\OpenHarness-main\learning_agent\test\real_model_latency_v2_20260626\2026-06-26-openharness-desktop-real-model-latency-v2.md`。

## 2026-06-26 Real Model Latency V2 Blueprint Upgrade
- 已按 Karpathy-style review 升级 `docs/superpowers/plans/2026-06-26-openharness-desktop-real-model-latency-v2.md`，计划从 10 个任务升级为 12 个执行任务。
- 新增 Task 0 baseline latency measurement，要求固化 `queued_to_started_ms`、`started_to_first_status_ms`、`started_to_first_delta_ms`、`started_to_completed_ms`，避免后续凭感觉判断“变快”。
- 新增有限状态机事件合同：`ModelStreamEvent` 必须包含 `phase`、`elapsed_ms`、`sequence`、`turn_id`、`provider_id`、`model_id`，以防取消后旧 turn 的 late delta 污染新 turn。
- Codex CLI 改为先做 event shape discovery，再实现 observable runner；如果 `codex exec --json` 不提供 token delta，只允许声明 transport/status 可观测，不允许声明 CLI true token streaming。
- Direct ChatGPT OAuth SSE 改为 gated fast path：只有显式 direct OAuth 配置和 capability probe 通过后，才允许实现并声明真实 SSE streaming。
- 取消逻辑补充 Windows process tree 行为、幂等取消、drain queue、late event ignore；diagnostics 明确为异步缓存，不能在发送 prompt 同步路径运行 `codex doctor`。
- 新增 secret redaction 自动门禁和真实 GUI 验收指标：`first_status_ms`、`first_transport_warning_ms`、`first_delta_ms`、`completion_ms`、`cancel_visible_ms`、`cancel_backend_stop_ms`。
- 升级版学习副本已覆盖到主项目：`H:\codexworkplace\sofeware\OpenHarness-main\learning_agent\test\real_model_latency_v2_20260626\2026-06-26-openharness-desktop-real-model-latency-v2.md`。
- 用户补充的真实 GUI 验收门禁已写入蓝图：验收必须使用肉眼可见的真实 GUI 界面，并使用 `computer-use:computer-use` 技能确认和验证；如发现 bug，必须使用 `superpowers:systematic-debugging` 定位根因、修复并重新测试，通过后才能继续下一个任务。

## 2026-06-26 Real Model Latency V2 Execution Completion
- 已逐项完成 `docs/superpowers/plans/2026-06-26-openharness-desktop-real-model-latency-v2.md`：baseline 证据、streaming contract、后端真实模型阶段事件、前端模型调用状态、真实取消、Codex CLI event shape discovery、observable runner、异步诊断、honest gating、Direct OAuth SSE gated probe、secret redaction gate 和真实 GUI 可见验收。
- 真实模型短问答证据：OpenHarness Desktop 可见窗口中选择 OpenAI/ChatGPT OAuth 模型 `GPT-5.5`，prompt `请只回答两个字：真实` 最终回答 `真实`；事件包含 `model_first_delta`、`message_delta`、`model_call_completed`、`gui_turn_completed`，耗时约 127.9 秒，GUI 诚实显示慢链路而不是 fake streaming。
- 真实 GUI 取消验收发现并修复关键 bug：前端红色取消按钮会进入 `gui_turn_cancel_requested`，但旧 `CodexCliStreamingRunner.stream()` 没有外部取消检查，Codex CLI WebSocket/HTTPS fallback 卡住时不会被及时终止。
- 修复后 `RealModelGuiAgentAdapter` 会把 GUI cancellation checker 透传给支持 `is_cancelled` 的 streaming model；`CodexCliChatModel.stream_chat()` 再传给 `CodexCliStreamingRunner.stream()`，runner 每轮 poll 都检查取消并调用 `CodexCliProcessController.cancel()` 终止子进程。
- 可见 GUI 复测已通过：真实 OpenHarness Desktop 窗口中连续两次提交 `__slow__` turn 并点击底部红色取消按钮，消息状态变为 `cancelled`，右侧事件出现 `gui_turn_cancel_requested` 后接 `gui_turn_cancelled`；截图位于 `learning_agent/test/real_model_latency_v2_20260626/visible_evidence/`。
- Direct OAuth SSE 快路径当前只实现 gated probe 与 payload 形状解析，不默认宣称可用；必须满足 `OPENHARNESS_OPENAI_AUTH_MODE=direct_oauth`、`OPENHARNESS_OPENAI_EXPERIMENTAL=1`、`OPENHARNESS_PROVIDER_SECRET_STORE=os_encrypted`、`OPENHARNESS_OPENAI_CLIENT_ID` 后，probe 通过才允许走真实低延迟 SSE。
