...
const openAiBrowserOnlyProvider: ProviderRowView = { id: "openai", displayName: "OpenAI", kind: "built_in", source: "none", connected: false, maskedKey: "", description: "使用 ChatGPT Pro/Plus 或 API 密钥连接", authMethods: [{ id: "chatgpt-browser", label: "ChatGPT Pro/Plus (browser)", enabled: true, status: "available", methodType: "oauth", mode: "auto", fields: [], helpText: "使用官方 Codex CLI 打开 ChatGPT OAuth 登录；OpenHarness 不读取 Codex token。", experimental: false }], models: [], primaryActionLabel: "+ 连接", primaryActionDisabled: false }; // 新增代码+CodexLoginDialogTest：定义单一官方 browser 方法 provider；如果没有这行，静态渲染无法直接进入 codex_cli_login 等待页。
...
const pendingCodexCliAuthAttempt = { attempt_id: "auth_attempt_codex", provider_id: "openai", auth_method_id: "chatgpt-browser", mode: "codex_cli_login", url: "", instructions: "在浏览器中完成官方 Codex/ChatGPT 登录；OpenHarness 不读取 Codex token。", display_code: "Complete authorization in your browser. This window will close automatically.", display_code_kind: "browser_instruction", display_code_copyable: false, status: "pending", message: "Codex login started.", created_at: 1, expires_at: 2 }; // 新增代码+CodexLoginDialogTest：定义官方 Codex CLI pending attempt fixture；如果没有这行，前端无法锁住空 URL 等待页行为。
...
describe("SettingsProvidersPanel", () => { // 新增代码+ProviderSettingsPanelTest：测试段开始，覆盖 Provider 列表和连接弹窗；如果没有这段，Task 6 没有自动验收。
  ...
  it("renders Codex CLI auth-attempt waiting state without requiring a link", () => { // 新增代码+CodexLoginDialogTest：测试段开始，验证官方 Codex 登录等待页可以没有 URL；如果没有这段，空链接会出现在真实 GUI 验收里。
    const markup = renderToStaticMarkup(<ProviderConnectDialog open={true} provider={openAiBrowserOnlyProvider} apiKey="" pending={false} errorMessage="" authAttempt={pendingCodexCliAuthAttempt} onApiKeyChange={vi.fn()} onClose={vi.fn()} onSubmit={vi.fn()} onStartAuthAttempt={vi.fn()} onCancelAuthAttempt={vi.fn()} />); // 新增代码+CodexLoginDialogTest：渲染官方 Codex pending attempt；如果没有这行，后续断言没有等待页输入。
    expect(markup).toContain("连接 OpenAI"); // 新增代码+CodexLoginDialogTest：确认连接标题可见；如果没有这行，等待页可能不在正确弹窗中。
    expect(markup).toContain("等待授权"); // 新增代码+CodexLoginDialogTest：确认 pending 状态可见；如果没有这行，用户不知道系统正在等待浏览器授权。
    expect(markup).toContain("Complete authorization in your browser"); // 新增代码+CodexLoginDialogTest：确认官方 Codex 浏览器提示可见；如果没有这行，用户不知道要回到浏览器完成登录。
    expect(markup).not.toContain("href=\"\""); // 新增代码+CodexLoginDialogTest：确认空 URL 不渲染成空链接；如果没有这行，用户会看到无法点击的错误链接。
    expect(markup).not.toContain("access_token"); // 新增代码+CodexLoginDialogTest：确认不渲染 access token 字段；如果没有这行，截图可能泄露敏感字段语义。
    expect(markup).not.toContain("refresh_token"); // 新增代码+CodexLoginDialogTest：确认不渲染 refresh token 字段；如果没有这行，长期凭据语义可能进入 UI。
  }); // 新增代码+CodexLoginDialogTest：官方 Codex 等待页测试结束；如果没有这行，测试块语法不完整。
