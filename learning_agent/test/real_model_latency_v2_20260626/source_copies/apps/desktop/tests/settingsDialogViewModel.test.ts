...
import { authAttemptPollDecision, authAttemptStartDecision, shouldPollAuthAttempt } from "../src/components/settings/SettingsDialog"; // 修改代码+OpenAIConnectImmediateCompleteTest：导入 auth-attempt 启动/轮询决策和开关 helper；如果没有这行，测试无法锁住 start 直接 complete 时刷新 provider catalog 的行为。
...
describe("provider settings dialog view model", () => { // 新增代码+ProviderSettingsDialogModelTest：测试段开始，覆盖设置弹窗需要的派生状态；如果没有这段，弹窗可能误显示 secret store 或 auth 状态。
  ...
  it("refreshes provider catalog when auth-attempt start returns complete immediately", () => { // 新增代码+OpenAIConnectImmediateCompleteTest：测试段开始，覆盖 start 请求直接 complete 的尾部；如果没有这段，已登录 Codex 用户重新连接后弹窗可能不刷新。
    const completeDecision = authAttemptStartDecision("complete"); // 新增代码+OpenAIConnectImmediateCompleteTest：计算 start 直接 complete 的动作；如果没有这行，测试没有被测结果。
    const pendingDecision = authAttemptStartDecision("pending"); // 新增代码+OpenAIConnectImmediateCompleteTest：计算 start pending 的动作；如果没有这行，无法确认等待态仍展示 attempt。
    expect(completeDecision.refreshProviderSettings).toBe(true); // 新增代码+OpenAIConnectImmediateCompleteTest：确认 complete 要立即刷新 provider catalog；如果没有这行，连接状态会停在旧列表。
    expect(completeDecision.keepAttemptVisibleBeforeRefresh).toBe(false); // 新增代码+OpenAIConnectImmediateCompleteTest：确认 complete 不先展示等待页；如果没有这行，弹窗可能停在已完成但不关闭。
    expect(pendingDecision.refreshProviderSettings).toBe(false); // 新增代码+OpenAIConnectImmediateCompleteTest：确认 pending 不立即刷新 catalog；如果没有这行，等待授权期间可能提前关闭弹窗。
    expect(pendingDecision.keepAttemptVisibleBeforeRefresh).toBe(true); // 新增代码+OpenAIConnectImmediateCompleteTest：确认 pending 仍显示等待页；如果没有这行，用户看不到授权进度。
  }); // 新增代码+OpenAIConnectImmediateCompleteTest：start immediate complete 测试结束；如果没有这行，测试块语法不完整。
  ...
  it("polls auth-attempt status only while a pending dialog is open with a client", () => { // 新增代码+OpenAIConnectPollTailTest：测试段开始，锁住轮询开关尾部行为；如果没有这段，关闭弹窗或失败终态可能继续轮询。
    expect(shouldPollAuthAttempt(true, true, { status: "pending" })).toBe(true); // 新增代码+OpenAIConnectPollTailTest：确认打开、有 client、pending 时才轮询；如果没有这行，等待页可能不会自动更新。
    expect(shouldPollAuthAttempt(false, true, { status: "pending" })).toBe(false); // 新增代码+OpenAIConnectPollTailTest：确认弹窗关闭时不轮询；如果没有这行，renderer 关闭 wizard 后可能残留 timer。
    expect(shouldPollAuthAttempt(true, false, { status: "pending" })).toBe(false); // 新增代码+OpenAIConnectPollTailTest：确认没有 client 时不轮询；如果没有这行，离线测试或 bridge 缺失会抛异常。
    expect(shouldPollAuthAttempt(true, true, null)).toBe(false); // 新增代码+OpenAIConnectPollTailTest：确认没有 attempt 时不轮询；如果没有这行，方法选择页会发空 status 请求。
    expect(shouldPollAuthAttempt(true, true, { status: "failed" })).toBe(false); // 新增代码+OpenAIConnectPollTailTest：确认 failed 终态停止轮询；如果没有这行，失败状态会不断请求后端。
    expect(shouldPollAuthAttempt(true, true, { status: "expired" })).toBe(false); // 新增代码+OpenAIConnectPollTailTest：确认 expired 终态停止轮询；如果没有这行，取消或过期后仍可能继续轮询。
    expect(shouldPollAuthAttempt(true, true, { status: "complete" })).toBe(false); // 新增代码+OpenAIConnectPollTailTest：确认 complete 终态停止轮询；如果没有这行，完成后可能重复刷新 catalog。
  }); // 新增代码+OpenAIConnectPollTailTest：轮询开关尾部测试结束；如果没有这行，测试块语法不完整。
