...
import { buildProviderSettingsViewModel, composerModelOptionsForDisplay, modelGroupsForDisplay, providerRowsForDisplay, redactedProviderPayload } from "../src/state/providerSettingsStore"; // 修改代码+ComposerModelToolbarTest：导入 Composer 模型选项派生函数；如果没有这行，底部模型菜单无法复用 provider catalog。
...
const providerPayload: PollutedProviderSettingsPayload = { // 修改代码+ProviderSettingsStoreTest：定义后端 provider settings fixture；如果没有这段，多个测试会重复构造 payload。
  ...
  schema_version: 4, // 修改代码+ModelFailureStateTest：模拟带 recent_failure 的 V4 schema；如果没有这行，前端测试无法覆盖最新 provider catalog 合同。
...
describe("providerSettingsStore", () => { // 修改代码+ProviderSettingsStoreTest：测试段开始，覆盖 provider settings view model；如果没有这段，设置页会直接消费后端原始 JSON。
  ...
  it("builds composer model options from visible connected provider models", () => { // 新增代码+ComposerModelToolbarTest：测试段开始，确认底部模型菜单来自 provider catalog；如果没有这段，Composer 可能只能显示硬编码模型。
    const viewModel = buildProviderSettingsViewModel(providerPayload); // 新增代码+ComposerModelToolbarTest：构建 provider view model；如果没有这行，模型选项没有输入。
    const options = composerModelOptionsForDisplay(viewModel); // 新增代码+ComposerModelToolbarTest：派生底部模型选项；如果没有这行，无法断言 Composer 数据源。
    expect(options).toEqual([{ id: "gpt-4.1", label: "GPT-4.1", providerId: "openai", providerName: "OpenAI", supportsTools: true, supportsVision: true, recentFailure: null }]); // 修改代码+ModelFailureStateTest：确认只返回已连接且可见的 OpenAI 模型并保留空失败状态；如果没有这行，未连接 provider 或隐藏模型可能混入菜单，失败字段也可能丢失。
  }); // 新增代码+ComposerModelToolbarTest：Composer 模型选项测试结束；如果没有这行，测试块语法不完整。

  it("keeps recent model failures in composer model options", () => { // 新增代码+ModelFailureStateTest：测试段开始，确认后端失败摘要能进入底部模型菜单；如果没有这段，模型不支持错误只能停留在后端日志。
    const failedPayload = JSON.parse(JSON.stringify(providerPayload)) as PollutedProviderSettingsPayload; // 新增代码+ModelFailureStateTest：复制 fixture 避免污染其他测试；如果没有这行，后续测试可能被失败状态影响。
    const openai = failedPayload.providers.find((provider) => provider.id === "openai"); // 新增代码+ModelFailureStateTest：找到 OpenAI provider；如果没有这行，测试无法精准注入 OpenAI OAuth 模型失败。
    if (openai === undefined) { // 新增代码+ModelFailureStateTest：保护 fixture 必须包含 OpenAI；如果没有这行，测试输入损坏时会出现难懂的空对象错误。
      throw new Error("missing OpenAI provider fixture"); // 新增代码+ModelFailureStateTest：明确失败原因；如果没有这行，维护者不知道是 fixture 缺失而不是 store 出错。
    } // 新增代码+ModelFailureStateTest：fixture 校验分支结束；如果没有这行，条件块语法不完整。
    openai.models = [{ id: "gpt-4.1", display_name: "GPT-4.1", provider_id: "openai", visible: true, supports_tools: true, supports_vision: true, recent_failure: { error_kind: "model_unsupported", message: "所选模型 GPT-4.1 当前 ChatGPT OAuth 账号不支持", failed_at: 1782480000 } }]; // 新增代码+ModelFailureStateTest：注入后端 recent_failure；如果没有这行，store 不会遇到真实失败摘要字段。
    const options = composerModelOptionsForDisplay(buildProviderSettingsViewModel(failedPayload)); // 新增代码+ModelFailureStateTest：派生 Composer 模型选项；如果没有这行，无法验证失败摘要是否走完整数据链。
    expect(options[0].recentFailure).toEqual({ errorKind: "model_unsupported", message: "所选模型 GPT-4.1 当前 ChatGPT OAuth 账号不支持", failedAt: 1782480000 }); // 新增代码+ModelFailureStateTest：确认 snake_case 失败摘要转成 camelCase；如果没有这行，Composer 下拉无法显示最近失败原因。
  }); // 新增代码+ModelFailureStateTest：最近失败派生测试结束；如果没有这行，测试块语法不完整。
