...
export type GuiV2EventKind = // 新增代码+GuiV2ProtocolTypes：类型段开始，声明 V2 GUI 事件 kind 联合类型；如果没有这段，前端状态机只能接受任意字符串。
  ...
  | "model_call_started" // 新增代码+RealModelLatencyV2：表示真实模型调用开始；如果没有这一项，前端协议无法识别调用起点。
  | "model_call_status" // 新增代码+RealModelLatencyV2：表示真实模型调用阶段更新；如果没有这一项，连接/fallback/取消状态无法类型化。
  | "model_first_delta" // 新增代码+RealModelLatencyV2：表示收到模型首个响应增量；如果没有这一项，前端无法识别 TTFT 成功信号。
  | "model_call_completed" // 新增代码+RealModelLatencyV2：表示真实模型调用完成；如果没有这一项，完成指标无法类型化。
  | "model_call_failed" // 新增代码+RealModelLatencyV2：表示真实模型调用失败；如果没有这一项，模型不可用错误无法类型化。
