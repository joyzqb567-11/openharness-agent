...
export type SendMessageOptions = { providerId?: string; modelId?: string; reasoningEffort?: string; permissionMode?: string }; // 新增代码+ComposerModelToolbar：定义随 prompt 发送的模型上下文；如果没有这行，client 只能提交文本而无法提交用户选择的模型/权限。
...
export function createGuiClient(baseUrl: string, bridgeToken: string, fetcher: FetchLike = fetch) { // 修改代码+DesktopRuntimePanelsClient：函数段开始，创建 GUI bridge 客户端；如果没有这段，渲染进程会到处手写 fetch。
  ...
  return { // 修改代码+DesktopRuntimePanelsClient：返回面向 UI 的 client 方法；如果没有这行，调用方无法使用封装能力。
    ...
    sendMessage(prompt: string, conversationId = "default", options: SendMessageOptions = {}): Promise<SendMessageResponse> { // 修改代码+ComposerModelToolbar：发送用户 prompt 并携带底部菜单上下文；如果没有这段，composer 无法创建带模型选择的后端 turn。
      const payload: Record<string, unknown> = { conversation_id: conversationId, prompt }; // 修改代码+ComposerModelToolbar：先放入旧协议必需字段；如果没有这行，后端不知道会话和 prompt。
      if (options.providerId) { payload.provider_id = options.providerId; } // 新增代码+ComposerModelToolbar：按需写入 provider_id；如果没有这行，bridge 无法知道模型来自哪个提供商。
      if (options.modelId) { payload.model_id = options.modelId; } // 新增代码+ComposerModelToolbar：按需写入 model_id；如果没有这行，后端无法记录或路由用户选中的模型。
      if (options.reasoningEffort) { payload.reasoning_effort = options.reasoningEffort; } // 新增代码+ComposerModelToolbar：按需写入推理强度；如果没有这行，后端无法知道用户选择的是低/中/高/超高。
      if (options.permissionMode) { payload.permission_mode = options.permissionMode; } // 新增代码+ComposerModelToolbar：按需写入权限模式；如果没有这行，后端无法审计本轮是否为完全访问。
      return postJson<SendMessageResponse>("/v1/gui/messages", payload); // 修改代码+ComposerModelToolbar：请求 messages endpoint；如果没有这行，用户输入和模型上下文不会进入 bridge。
