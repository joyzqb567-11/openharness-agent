...
export type ProviderModelFailureView = { errorKind: string; message: string; failedAt: number }; // 新增代码+ModelFailureState：定义前端模型最近失败 view model；如果没有这行，菜单和模型页无法共享失败摘要字段。

export type ProviderModelView = { id: string; displayName: string; providerId: string; visible: boolean; supportsTools: boolean; supportsVision: boolean; recentFailure: ProviderModelFailureView | null }; // 修改代码+ModelFailureState：定义前端模型 view model 并带最近失败；如果没有这行，Models 页字段名会散落且无法显示失败状态。

export type ProviderRowView = { id: string; displayName: string; kind: string; source: string; connected: boolean; maskedKey: string; description: string; authMethods: ProviderAuthMethodView[]; models: ProviderModelView[]; primaryActionLabel: string; primaryActionDisabled: boolean }; // 新增代码+ProviderSettingsStore：定义 provider 行 view model；如果没有这行，Provider 页按钮状态会散落在组件里。
...
export type ComposerModelOption = { id: string; label: string; providerId: string; providerName: string; supportsTools: boolean; supportsVision: boolean; recentFailure: ProviderModelFailureView | null }; // 修改代码+ModelFailureState：定义底部 Composer 模型选项并带最近失败；如果没有这行，设置页模型无法以稳定形状传给底部模型菜单。
...
function modelFailureView(model: GuiModelInfo): ProviderModelFailureView | null { // 新增代码+ModelFailureState：函数段开始，把后端最近失败摘要转成前端字段；如果没有这段，组件要理解 snake_case 失败结构。
  const failure = model.recent_failure; // 新增代码+ModelFailureState：读取后端 recent_failure 字段；如果没有这行，后续要重复访问 snake_case。
  if (!isRecord(failure) || !failure.error_kind) { // 新增代码+ModelFailureState：只接受带 error_kind 的对象；如果没有这行，空对象会被误当失败。
    return null; // 新增代码+ModelFailureState：没有有效失败时返回 null；如果没有这行，调用方无法区分无失败和空失败。
  } // 新增代码+ModelFailureState：有效性判断结束；如果没有这行，条件块语法不完整。
  return { errorKind: String(failure.error_kind), message: String(failure.message ?? ""), failedAt: Number(failure.failed_at ?? 0) }; // 新增代码+ModelFailureState：返回 camelCase 失败摘要；如果没有这行，菜单无法显示失败类型和说明。
} // 新增代码+ModelFailureState：函数段结束，modelFailureView 到此结束；如果没有这行，函数语法不完整。
...
function modelView(model: GuiModelInfo): ProviderModelView { // 新增代码+ProviderSettingsStore：函数段开始，把后端模型转成前端字段；如果没有这段，Models 页要理解 snake_case。
  return { id: model.id, displayName: model.display_name, providerId: model.provider_id, visible: Boolean(model.visible), supportsTools: Boolean(model.supports_tools), supportsVision: Boolean(model.supports_vision), recentFailure: modelFailureView(model) }; // 修改代码+ModelFailureState：返回模型 view model 并带最近失败；如果没有这行，模型字段名无法统一且失败标记会丢失。
...
function providerRowView(provider: GuiProviderInfo): ProviderRowView { // 新增代码+ProviderSettingsStore：函数段开始，把后端 provider 转成前端行；如果没有这段，Provider 页组件会重复转换。
  ...
  return { id: provider.id, displayName: provider.display_name, kind: provider.kind, source: provider.source, connected: Boolean(provider.connected), maskedKey: provider.masked_key, description: provider.description, authMethods, models, primaryActionLabel: label, primaryActionDisabled: label === "暂未支持" }; // 新增代码+ProviderSettingsStore：返回 provider 行 view model；如果没有这行，组件拿不到统一字段。
...
export function composerModelOptionsForDisplay(viewModel: ProviderSettingsViewModel): ComposerModelOption[] { // 新增代码+ComposerModelToolbar：函数段开始，把 Provider 设置模型派生成 Composer 菜单选项；如果没有这段，OAuth 连接后底部模型菜单仍没有真实数据源。
  return providerRowsForDisplay(viewModel).filter((provider) => provider.connected).flatMap((provider) => provider.models.filter((model) => model.visible).map((model) => ({ id: model.id, label: model.displayName, providerId: provider.id, providerName: provider.displayName, supportsTools: model.supportsTools, supportsVision: model.supportsVision, recentFailure: model.recentFailure }))); // 修改代码+ModelFailureState：只返回已连接 provider 的可见模型并保留失败摘要；如果没有这行，未连接或隐藏模型会误入底部菜单且失败标记会丢失。
} // 新增代码+ComposerModelToolbar：函数段结束，composerModelOptionsForDisplay 到此结束；如果没有这行，函数语法不完整。
