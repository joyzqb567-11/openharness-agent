...
import { isModelCallEventType } from "../components/ModelCallStatus"; // 新增代码+RealModelLatencyV2：引入模型调用事件白名单；如果没有这行，阶段事件会被误报为 unsupported。
...
function assistantMessage(event: StatusEvent, text: string, status: TurnStatus, kind?: ThreadMessageKind): ThreadMessage { // 修改代码+DesktopSafetyRefusal：函数段开始，构造标准助手消息并允许普通完成事件不覆盖已有拒绝语义；如果没有这段，完成、拒绝和错误事件会各自拼装不同形状。
  return { // 新增代码+DesktopEventReducer：返回助手消息对象；如果没有这行，函数没有输出。
    ...
    kind, // 修改代码+DesktopSafetyRefusal：只在调用方明确传入时写入消息语义；如果没有这行，拒绝和错误无法显示专门标签。
...
export function reduceGuiEventToThreadActions(event: StatusEvent): EventReduction { // 新增代码+DesktopEventReducer：函数段开始，把 GUI 事件转换为线程动作；如果没有这段，后端事件无法驱动 Codex 风格主线程。
  if (isModelCallEventType(event.event_type)) { // 新增代码+RealModelLatencyV2：模型调用阶段事件只给状态条使用；如果没有这行，右栏 diagnostics 会把正常阶段误报成 unsupported。
    return ok([]); // 新增代码+RealModelLatencyV2：不改主消息也不产生诊断；如果没有这行，连接/fallback 状态会污染对话 reducer。
  } // 新增代码+RealModelLatencyV2：模型调用事件分支结束；如果没有这行，条件块语法不完整。
