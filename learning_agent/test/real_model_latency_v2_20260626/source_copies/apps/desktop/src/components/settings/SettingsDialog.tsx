...
export function authAttemptStartDecision(status: ProviderAuthAttemptInfo["status"]): AuthAttemptPollDecision { // 新增代码+OpenAIConnectImmediateComplete：函数段开始，处理 start 请求直接返回终态的动作；如果没有这段，已登录 Codex 用户重新连接时不会立即刷新 provider catalog。
  return authAttemptPollDecision(status); // 新增代码+OpenAIConnectImmediateComplete：复用轮询终态决策；如果没有这行，start complete 和 status complete 可能产生两套不同 UI 行为。
} // 新增代码+OpenAIConnectImmediateComplete：函数段结束，authAttemptStartDecision 到此结束；如果没有这个边界，用户不容易看出它只桥接 start 响应。
...
export function SettingsDialog({ open, onClose, version = packageInfo.version, secretStoreWarning = "", guiClient }: SettingsDialogProps): JSX.Element | null { // 修改代码+ProviderSettingsPanel：函数段开始，渲染 OpenHarness Desktop 设置弹窗；如果没有这段，左下角设置入口没有成熟 GUI 外壳。
  ...
  async function handleStartAuthAttempt(authMethodId: string): Promise<void> { // 新增代码+OpenAIConnectDialog：函数段开始，启动 browser/headless auth-attempt；如果没有这段，OAuth 方法点击后没有后端生命周期。
    ...
    try { // 新增代码+OpenAIConnectDialog：捕获启动失败；如果没有这行，bridge 错误会变成未捕获 Promise。
      ...
      const decision = authAttemptStartDecision(payload.attempt.status); // 新增代码+OpenAIConnectImmediateComplete：根据 start 返回状态决定是否立即刷新；如果没有这行，complete 状态会停在不会轮询的等待页。
      if (decision.refreshProviderSettings) { // 新增代码+OpenAIConnectImmediateComplete：处理 start 直接完成的授权；如果没有这行，已登录 Codex 用户重新连接后 provider 行不会立刻更新。
        const nextPayload = await guiClient.providerSettings(); // 新增代码+OpenAIConnectImmediateComplete：读取最新 provider catalog；如果没有这行，后端已清 disabled 但前端仍显示旧断开状态。
        setProviderPayload(nextPayload); // 新增代码+OpenAIConnectImmediateComplete：刷新 provider 列表；如果没有这行，断开后的重连无法在 GUI 中立即可见。
        setConnectAuthAttempt(null); // 新增代码+OpenAIConnectImmediateComplete：清空完成的 attempt；如果没有这行，下次打开可能看到旧完成状态。
        setConnectProvider(null); // 新增代码+OpenAIConnectImmediateComplete：关闭连接弹窗；如果没有这行，用户会停在已完成等待页。
        return; // 新增代码+OpenAIConnectImmediateComplete：complete 已处理完成后退出；如果没有这行，后续会把 complete attempt 又写回弹窗。
      } // 新增代码+OpenAIConnectImmediateComplete：直接完成分支结束；如果没有这行，条件块语法不完整。
