...
import type { GuiProviderSettingsPayload } from "../api/guiProviderTypes"; // 新增代码+ComposerModelToolbar：引入 Provider Settings payload 类型；如果没有这行，AppShell 无法安全保存模型 catalog。
...
import { buildProviderSettingsViewModel, composerModelOptionsForDisplay } from "../state/providerSettingsStore"; // 新增代码+ComposerModelToolbar：引入 provider catalog 到 Composer 模型菜单的派生函数；如果没有这行，底部模型菜单会和设置页数据分裂。
...
import { Composer, type ComposerPermissionMode, type ComposerReasoningEffort, type ComposerSubmitOptions } from "./Composer"; // 修改代码+ComposerModelToolbar：引入底部输入组件和提交上下文类型；如果没有这行，AppShell 无法把模型/权限选择传给后端。
...
import { latestModelCallStatus } from "./ModelCallStatus"; // 新增代码+RealModelLatencyV2：引入最新模型调用状态选择器；如果没有这行，底部 Composer 无法显示真实调用阶段。
...
export function AppShell(): JSX.Element { // 修改代码+DesktopToolTimeline：函数段开始，组织桌面 GUI 主壳；如果没有这段，各组件无法组合成 Codex 风格窗口。
  ...
  const [composerProviderPayload, setComposerProviderPayload] = useState<GuiProviderSettingsPayload | null>(null); // 新增代码+ComposerModelToolbar：保存 Provider Settings catalog；如果没有这行，底部模型菜单没有真实 provider 数据源。
  const [selectedComposerModelId, setSelectedComposerModelId] = useState(""); // 新增代码+ComposerModelToolbar：保存底部当前选中模型；如果没有这行，用户切换模型后下一次渲染会丢失选择。
  const [composerReasoningEffort, setComposerReasoningEffort] = useState<ComposerReasoningEffort>("ultra"); // 新增代码+ComposerModelToolbar：保存推理强度；如果没有这行，超高/高/中/低菜单只能写死。
  const [composerPermissionMode, setComposerPermissionMode] = useState<ComposerPermissionMode>("full-access"); // 新增代码+ComposerModelToolbar：保存权限模式；如果没有这行，完全访问下拉不会影响提交上下文。
  ...
  const composerModelOptions = useMemo(() => composerProviderPayload === null ? [] : composerModelOptionsForDisplay(buildProviderSettingsViewModel(composerProviderPayload)), [composerProviderPayload]); // 新增代码+ComposerModelToolbar：从 provider catalog 派生底部模型选项；如果没有这行，OAuth 连接后的模型不会出现在输入区。
  const effectiveComposerModelId = selectedComposerModelId || composerModelOptions[0]?.id || ""; // 新增代码+ComposerModelToolbar：为空选择兜底第一个可用模型；如果没有这行，菜单显示和提交模型可能不同步。
  ...
  useEffect(() => { // 新增代码+ComposerModelToolbar：副作用段开始，保证模型列表变化后选中项仍然有效；如果没有这段，断开 provider 后底部可能保留不存在的模型。
    if (composerModelOptions.length === 0) { // 新增代码+ComposerModelToolbar：处理没有可选模型的情况；如果没有这行，空模型菜单仍可能保留旧选中值。
      if (selectedComposerModelId.length > 0) { // 新增代码+ComposerModelToolbar：只在确实有旧值时清空；如果没有这行，空模型状态会反复 setState。
        setSelectedComposerModelId(""); // 新增代码+ComposerModelToolbar：清空不存在的模型选择；如果没有这行，提交时可能带上过期 model_id。
      } // 新增代码+ComposerModelToolbar：旧模型清空判断结束；如果没有这行，条件块语法不完整。
      return; // 新增代码+ComposerModelToolbar：无模型时结束副作用；如果没有这行，后续会访问第一个模型。
    } // 新增代码+ComposerModelToolbar：空模型判断结束；如果没有这行，条件块语法不完整。
    if (!composerModelOptions.some((option) => option.id === selectedComposerModelId)) { // 新增代码+ComposerModelToolbar：检查当前选择是否还在可见模型中；如果没有这行，隐藏模型或断开连接后会继续被提交。
      setSelectedComposerModelId(composerModelOptions[0].id); // 新增代码+ComposerModelToolbar：自动选择第一个可用模型；如果没有这行，连接成功后用户仍看到空模型。
    } // 新增代码+ComposerModelToolbar：有效性修复结束；如果没有这行，条件块语法不完整。
  }, [composerModelOptions, selectedComposerModelId]); // 新增代码+ComposerModelToolbar：按模型列表和当前选择重新校准；如果没有这行，模型 catalog 变化后不会触发修正。
  ...
  useEffect(() => { // 新增代码+DesktopGUISessions：副作用段开始，加载项目名和最近会话；如果没有这段，侧栏无法从真实 bridge 获取项目上下文。
    ...
    async function loadSidebarData(): Promise<void> { // 新增代码+DesktopGUISessions：函数段开始，集中读取 bootstrap 和 sessions；如果没有这段，侧栏数据加载逻辑会散落到渲染层。
      try { // 新增代码+DesktopGUISessions：捕获侧栏加载失败；如果没有这行，bridge 暂时不可用会让界面副作用抛错。
        const [bootstrapPayload, sessionsPayload, runtimePanelsPayload, healthPayloadResult, diagnosticsPayloadResult, harnessPayloadResult, providerSettingsPayloadResult] = await Promise.all([activeClient.bootstrap(), activeClient.sessions(), activeClient.runtimePanels(), activeClient.health(), activeClient.diagnostics(), activeClient.harnessStatus(), activeClient.providerSettings()]); // 修改代码+ComposerModelToolbar：首屏同时读取 Provider Settings catalog；如果没有这行，底部模型菜单要等用户打开设置才有数据。
        ...
        setComposerProviderPayload(providerSettingsPayloadResult); // 新增代码+ComposerModelToolbar：保存 provider catalog 给底部模型菜单；如果没有这行，已连接模型不会出现在 composer。
  ...
  useEffect(() => { // 新增代码+ComposerModelToolbar：副作用段开始，在设置弹窗关闭后刷新 provider catalog；如果没有这段，OAuth 连接成功后底部模型菜单不会及时更新。
    if (guiClient === null || settingsOpen) { // 新增代码+ComposerModelToolbar：没有 client 或设置仍打开时跳过刷新；如果没有这行，弹窗内部编辑时会不断抢状态。
      return undefined; // 新增代码+ComposerModelToolbar：返回空清理函数；如果没有这行，React 副作用返回值不清晰。
    } // 新增代码+ComposerModelToolbar：刷新门禁结束；如果没有这行，条件块语法不完整。
    const activeClient = guiClient; // 新增代码+ComposerModelToolbar：固定本轮刷新使用的 client；如果没有这行，异步返回时 TypeScript 会担心 client 变化。
    let disposed = false; // 新增代码+ComposerModelToolbar：记录本轮刷新是否失效；如果没有这行，迟到响应可能更新旧界面。
    async function refreshComposerProviderPayload(): Promise<void> { // 新增代码+ComposerModelToolbar：函数段开始，读取最新 provider catalog；如果没有这段，刷新逻辑会散落到渲染层。
      try { // 新增代码+ComposerModelToolbar：捕获 provider catalog 刷新失败；如果没有这行，临时离线会抛出未处理 Promise。
        const payload = await activeClient.providerSettings(); // 新增代码+ComposerModelToolbar：请求 provider settings endpoint；如果没有这行，底部模型菜单没有最新事实源。
        if (!disposed) { // 新增代码+ComposerModelToolbar：只在本轮仍有效时更新状态；如果没有这行，关闭窗口后仍可能 setState。
          setComposerProviderPayload(payload); // 新增代码+ComposerModelToolbar：写入最新 catalog；如果没有这行，连接或断开 provider 后底部模型菜单不会变。
        } // 新增代码+ComposerModelToolbar：有效刷新写入结束；如果没有这行，条件块语法不完整。
      } catch (error) { // 新增代码+ComposerModelToolbar：处理刷新异常；如果没有这行，网络错误会泄露到控制台外的未捕获路径。
        console.warn("GUI provider catalog refresh failed", error); // 新增代码+ComposerModelToolbar：保留刷新失败诊断；如果没有这行，底部模型不更新时难以排查。
      } // 新增代码+ComposerModelToolbar：异常处理结束；如果没有这行，try/catch 语法不完整。
    } // 新增代码+ComposerModelToolbar：函数段结束，refreshComposerProviderPayload 到此结束；如果没有这个边界，初学者不容易看出刷新范围。
    void refreshComposerProviderPayload(); // 新增代码+ComposerModelToolbar：执行一次刷新；如果没有这行，定义好的函数不会运行。
    return () => { // 新增代码+ComposerModelToolbar：定义副作用清理逻辑；如果没有这行，迟到响应无法失效。
      disposed = true; // 新增代码+ComposerModelToolbar：标记本轮刷新失效；如果没有这行，旧响应可能覆盖新 catalog。
    }; // 新增代码+ComposerModelToolbar：清理函数结束；如果没有这行，return 语法不完整。
  }, [guiClient, settingsOpen]); // 新增代码+ComposerModelToolbar：按 client 和设置开关刷新；如果没有这行，设置关闭后不会更新模型菜单。
  ...
  async function handleCancelTurn(turnId: string): Promise<void> { // 新增代码+DesktopTurnActions：函数段开始，处理 GUI 取消按钮；如果没有这段，消息卡取消按钮不会调用后端 cancel。
    ...
    try { // 新增代码+DesktopTurnActions：捕获取消请求失败；如果没有这行，404/409 会变成未捕获 Promise。
      ...
      await refreshEventsAfter(response.events_after_sequence); // 新增代码+真实模型取消：取消后立刻补拉后端事件；如果没有这一行，cancel_requested/cancelled 只能等 1.5 秒轮询，达不到 500ms 可见目标。
  ...
  async function handleSubmit(prompt: string, options?: ComposerSubmitOptions): Promise<void> { // 修改代码+ComposerModelToolbar：函数段开始，处理用户提交 prompt 和底部菜单上下文；如果没有这段，Composer 的模型/权限选择不会进入后端。
    ...
    try { // 修改代码+DesktopToolTimeline：捕获提交失败；如果没有这行，busy 或断线会变成未捕获错误。
      const response = await guiClient.sendMessage(prompt, activeConversationId, options); // 修改代码+ComposerModelToolbar：向当前会话提交 prompt 和模型上下文；如果没有这行，用户选中模型后后端仍只收到文本。
  ...
  const latestComposerModelStatus = useMemo(() => latestModelCallStatus(statusState.events), [statusState.events]); // 新增代码+RealModelLatencyV2：从事件流中取最新模型调用阶段；如果没有这行，底部状态条无法跟随后端连接/fallback/首响应事件。
  ...
  return ( // 修改代码+DesktopToolTimeline：返回主界面结构；如果没有这行，组件不会输出任何 UI。
    <main className="app-shell"> {/* 修改代码+DesktopToolTimeline：定义三栏窗口网格容器；如果没有这行，侧栏、线程区和状态区无法稳定并排。 */}
      ...
      <section className="thread-panel" aria-label="当前对话"> {/* 修改代码+DesktopToolTimeline：定义当前对话主区域；如果没有这行，读屏器和布局都缺少对话容器。 */}
        ...
        <Composer isRunning={threadState.isRunning} activeTurnId={threadState.activeTurnId} modelOptions={composerModelOptions} selectedModelId={effectiveComposerModelId} reasoningEffort={composerReasoningEffort} permissionMode={composerPermissionMode} modelCallStatus={latestComposerModelStatus} onModelChange={setSelectedComposerModelId} onReasoningEffortChange={setComposerReasoningEffort} onPermissionModeChange={setComposerPermissionMode} onCancelActiveTurn={(turnId) => { void handleCancelTurn(turnId); }} onSubmit={(prompt, options) => { void handleSubmit(prompt, options); }} /> {/* 修改代码+RealModelLatencyV2：把模型、推理、权限、真实调用状态、取消和提交回调交给 composer；如果没有这行代码，底部输入区无法像 Codex 一样展示所选模型的真实调用进度。 */}
      ...
      <SettingsDialog guiClient={guiClient ?? undefined} open={settingsOpen} onClose={() => setSettingsOpen(false)} /> {/* 修改代码+ProviderSettingsPanel：把真实 GUI bridge client 传给设置弹窗；如果没有这行，Provider 设置页只能显示静态外壳而不能连接后端。 */}
