...
import { Activity, Bug, Globe2, ListChecks, Settings, Wrench, type LucideIcon } from "lucide-react"; // 修改代码+DesktopGUIHarnessPanel：引入任务页签图标；如果没有这行，Harness 页签会缺少可扫描符号。
...
import { latestModelCallStatus, ModelCallStatus } from "./ModelCallStatus"; // 新增代码+RealModelLatencyV2：引入模型调用状态摘要；如果没有这行，右侧状态页无法解释慢调用阶段。
...
type InspectorTabId = "status" | "tools" | "browser" | "harness" | "settings" | "diagnostics"; // 修改代码+DesktopGUIHarnessPanel：限定右侧检查器页签 id 并加入任务页；如果没有这行，Harness 页签无法被选中。
...
export function StatusInspector({ events, browserProviderStatus, runtimePanels = {}, harnessPayload = {}, bridgeBaseUrl = "", healthPayload = {}, diagnosticsPayload = {}, onProbeUnknownRoute, onPauseHarness, onResumeHarness, harnessControlPending = false }: StatusInspectorProps): JSX.Element { // 修改代码+DesktopUnknownRouteProbe：函数段开始，渲染右侧多页签检查器并接入 unknown route 探针；如果没有这段，用户看不到运行、工具、浏览器、任务、设置和诊断信息。
  ...
  const latestModelStatus = useMemo(() => latestModelCallStatus(events), [events]); // 新增代码+RealModelLatencyV2：缓存最新模型调用状态；如果没有这行，状态页和事件列表之间没有慢调用摘要。
      ...
      <div className="inspector-panel"> {/* 修改代码+DesktopDiagnosticsPanel：当前页签内容容器；如果没有这一层，面板内容没有独立滚动边界。 */}
        {activeTab === "status" ? ( // 修改代码+DesktopDiagnosticsPanel：按需渲染状态页签；如果没有这行，状态内容会一直占用空间。
          <section className="status-tab-panel" aria-label="运行状态"> {/* 修改代码+DesktopDiagnosticsPanel：状态面板；如果没有这一层，读屏器无法识别状态页内容。 */}
            ...
            <div className="status-model-call-summary"> {/* 新增代码+RealModelLatencyV2：在事件列表上方显示模型调用摘要；如果没有这行，用户要翻事件才能知道慢在哪里。 */}
              <ModelCallStatus status={latestModelStatus} /> {/* 新增代码+RealModelLatencyV2：渲染最新模型调用状态；如果没有这行，右侧状态页缺少连接/fallback/失败摘要。 */}
            </div> {/* 新增代码+RealModelLatencyV2：模型调用摘要容器结束；如果没有这行，JSX 结构不完整。 */}
        ...
        {activeTab === "harness" ? <HarnessPanel payload={harnessPayload} onPause={onPauseHarness} onResume={onResumeHarness} controlPending={harnessControlPending} /> : null} {/* 新增代码+DesktopGUIHarnessPanel：渲染长任务页签；如果没有这一行，任务面板无法显示 active goal、queue 和 checkpoint。 */}
