...
class RealModelGuiAgentAdapterTest(unittest.TestCase):  # 新增代码+真实模型GUI适配器测试：测试类段开始，锁定 GUI 到真实模型的最小合同；如果没有这个类，fake streaming 可能长期冒充真实模型。
    ...
    def test_real_adapter_emits_completed_events_with_fake_model(self) -> None:  # 新增代码+真实模型GUI适配器测试：测试函数开始，验证真实 adapter 能把模型文本变成 GUI 事件；如果没有这段，完成路径可能仍是 fake streaming。
        ...
        self.assertEqual(["turn_started", "model_call_started", "model_call_status", "model_first_delta", "message_delta", "message_completed", "model_call_completed"], [event["kind"] for event in events])  # 修改代码+真实模型延迟可观测性：确认 GUI 同时收到模型阶段和原有正文事件；如果没有这一行，慢请求可观测性或聊天正文可能回归。
    ...
    def test_real_adapter_passes_selected_gui_model_context_to_factory(self) -> None:  # 新增代码+ComposerRealModelRoutingTest：测试函数开始，验证真实 adapter 会把底部菜单选择交给模型工厂；如果没有这段，OAuth 已连接后仍可能只用环境变量里的固定模型。
        from learning_agent.app.gui_agent_adapter import GuiAgentRunRequest, RealModelGuiAgentAdapter  # 新增代码+ComposerRealModelRoutingTest：导入请求对象和真实 adapter；如果没有这一行，测试无法构造带模型上下文的真实轮次。

        events: list[dict[str, object]] = []  # 新增代码+ComposerRealModelRoutingTest：保存 adapter 事件；如果没有这一行，测试无法确认轮次最终完成。
        factory_requests: list[GuiAgentRunRequest] = []  # 新增代码+ComposerRealModelRoutingTest：记录模型工厂收到的请求；如果没有这一行，无法证明 provider/model/reasoning 没有丢失。
        fake_model = FakeChatModel(FakeModelMessage(text="按 GPT-5.5 回答"))  # 新增代码+ComposerRealModelRoutingTest：构造假模型返回；如果没有这一行，测试会访问真实 ChatGPT OAuth 后端。

        def factory(request: GuiAgentRunRequest) -> FakeChatModel:  # 新增代码+ComposerRealModelRoutingTest：函数段开始，模拟需要读取 GUI 上下文的模型工厂；如果没有这段，测试只能覆盖旧的无参工厂。
            factory_requests.append(request)  # 新增代码+ComposerRealModelRoutingTest：保存完整请求对象；如果没有这一行，断言无法检查用户选择的模型。
            return fake_model  # 新增代码+ComposerRealModelRoutingTest：返回假模型；如果没有这一行，adapter 没有可调用模型。
        # 新增代码+ComposerRealModelRoutingTest：函数段结束，factory 到此结束；如果没有边界说明，用户不容易看出这是测试替身。

        adapter = RealModelGuiAgentAdapter(model_factory=factory, connected_provider_reader=lambda: True)  # 新增代码+ComposerRealModelRoutingTest：注入上下文感知模型工厂；如果没有这一行，adapter 不能被隔离测试。
        request = GuiAgentRunRequest(session_id="session_real", turn_id="turn_real", run_id="run_real", prompt="你好", mode="real", provider_id="openai", model_id="gpt-5.5", reasoning_effort="ultra", permission_mode="full-access")  # 新增代码+ComposerRealModelRoutingTest：构造带底部菜单选择的真实请求；如果没有这一行，模型工厂没有上下文样本。
        result = adapter.run(request, events.append, lambda: False)  # 新增代码+ComposerRealModelRoutingTest：执行真实 adapter；如果没有这一行，测试没有行为。
        self.assertEqual("completed", result.status)  # 新增代码+ComposerRealModelRoutingTest：确认上下文工厂不应导致失败；如果没有这一行，TypeError 之类问题会被漏掉。
        self.assertEqual("按 GPT-5.5 回答", result.final_text)  # 新增代码+ComposerRealModelRoutingTest：确认仍返回模型文本；如果没有这一行，adapter 可能只传上下文但丢了回答。
        self.assertEqual(["gpt-5.5"], [item.model_id for item in factory_requests])  # 新增代码+ComposerRealModelRoutingTest：确认选中模型进入工厂；如果没有这一行，用户在下拉里选模型不会影响真实调用。
        self.assertEqual(["openai"], [item.provider_id for item in factory_requests])  # 新增代码+ComposerRealModelRoutingTest：确认 provider 进入工厂；如果没有这一行，后续多 provider 路由无法判断来源。
        self.assertEqual(["ultra"], [item.reasoning_effort for item in factory_requests])  # 新增代码+ComposerRealModelRoutingTest：确认推理强度进入工厂；如果没有这一行，超高/高/中/低选择无法进入 runtime。
        self.assertEqual(["full-access"], [item.permission_mode for item in factory_requests])  # 新增代码+ComposerRealModelRoutingTest：确认权限模式进入工厂；如果没有这一行，真实模型运行无法知道本轮权限边界。
    # 新增代码+ComposerRealModelRoutingTest：测试函数结束，test_real_adapter_passes_selected_gui_model_context_to_factory 到此结束；如果没有边界说明，用户不容易看出覆盖的是真实模型路由上下文。
    ...
    def test_real_adapter_cancel_before_model_call_emits_turn_cancelled(self) -> None:  # 新增代码+真实模型GUI适配器测试：测试函数开始，验证模型调用前取消不会访问模型；如果没有这段，用户取消后仍可能产生真实后端调用。
        ...
        self.assertEqual(["turn_started", "model_call_status", "turn_cancelled"], [event["kind"] for event in events])  # 修改代码+真实模型取消可观测性测试：确认启动、取消请求状态和取消终态都出现；如果没有这一行，旧测试会误把可见 cancel_requested 当成回归。
        self.assertEqual("cancel_requested", events[1]["payload"]["phase"])  # 新增代码+真实模型取消可观测性测试：确认新增中间事件表达的是取消请求而不是模型正文；如果没有这一行，可见状态可能被错误事件占位。
    ...
    def test_real_adapter_cancel_after_model_call_before_completion(self) -> None:  # 新增代码+真实模型GUI适配器测试：测试函数开始，验证模型返回后取消不会写完成；如果没有这段，用户取消可能被最终回答覆盖。
        ...
        self.assertEqual(["turn_started", "model_call_started", "model_call_status", "model_call_status", "turn_cancelled"], [event["kind"] for event in events])  # 修改代码+真实模型延迟可观测性：确认取消前可见模型调用状态且没有 message_completed；如果没有这一行，取消竞态或状态冒泡可能漏掉。
