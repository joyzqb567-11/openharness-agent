...
if ($ExistingRuntimeArtifactRoots.Count -gt 0) { # 新增代码+OpenAIConnectSecretGate：仅在存在运行产物目录时执行严格扫描；如果没有这行，空项目会误失败。
  $RuntimeSecretPattern = 'access_token|refresh_token|id_token|secret_ref|oauth-secret-test-value|unit-test-secret-value|sk-[A-Za-z0-9]{12,}|Authorization:\s*Bearer|Bearer\s+|["'']?api_key["'']?\s*[:=]|api_key.*sk-' # 修改代码+真实模型证据门禁：定义运行产物禁止出现的敏感字段和值形态；如果没有这行，OAuth 字段名、Bearer 文本和 api_key 字段无法被抓住。
