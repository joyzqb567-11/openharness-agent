...
新增文档+DirectOAuthEvidence：可以运行 `learning_agent/test/provider_settings_v2_openai_connect/scripts/capture_direct_oauth_visual_qa.ps1 -ClientId <OpenHarness-owned-or-explicit-test-client-id>` 来启动真实可见 GUI 验收；如果没有这行说明，后续拿到 client id 后仍需要重新摸索 direct OAuth 证据采集流程。

新增文档+DirectOAuthEvidence：该脚本会要求用户在真实浏览器中完成 OpenAI 授权，再从真实 OpenHarness Desktop GUI 截图并生成 `direct_oauth_acceptance.json`；如果没有这行说明，用户可能误以为脚本会伪造 OAuth 成功或绕过肉眼验收。
