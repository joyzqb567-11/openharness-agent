param(
  [string[]]$AdditionalRuntimeArtifactRoots = @()
)
# 修改代码+RealModelLatencySecretGate：参数段负责接收额外运行产物目录；如果没有这段，蓝图传入的 real_model_latency_v2 证据目录不能被脚本扫描。
...
$RuntimeArtifactRoots = @( # 新增代码+OpenAIConnectSecretGate：定义需要严格扫描的运行产物目录；如果没有这段，visual QA JSON/log 泄露字段名不会被单独拦截。
  ...
  "learning_agent/test/provider_settings_v2_openai_connect/visual_evidence", # 修改代码+真实模型证据门禁：扫描 OpenAI Connect 最终肉眼验收证据目录；如果没有这行，最终交付截图 JSON 可能绕过泄露门禁。
  "learning_agent/test/provider_settings_v2_openai_connect/real_codex_login_evidence", # 新增代码+真实模型证据门禁：扫描真实 Codex 登录证据包；如果没有这行，V1A 真实回答验收 JSON 里的 token 字段可能漏过。
  "learning_agent/test/real_model_latency_v2_20260626", # 新增代码+RealModelLatencySecretGate：扫描本轮真实模型延迟 V2 证据目录；如果没有这行，Task 10 运行产物可能绕过泄密门禁。
  "memory/gui_provider_settings", # 新增代码+真实模型证据门禁：扫描 GUI provider 设置运行目录；如果没有这行，真实 provider 状态可能把敏感字段写入 memory。
  "learning_agent/memory/gui_provider_settings" # 新增代码+真实模型证据门禁：兼容旧 learning_agent memory provider 设置目录；如果没有这行，历史运行目录中的敏感字段可能漏扫。
) # 新增代码+OpenAIConnectSecretGate：运行产物目录数组结束；如果没有这行，PowerShell 数组语法不完整。
$RuntimeArtifactRoots += $AdditionalRuntimeArtifactRoots # 新增代码+RealModelLatencySecretGate：把命令行传入的额外目录加入严格扫描；如果没有这行，蓝图里的目标路径参数没有实际效果。
...
if ($ExistingRuntimeArtifactRoots.Count -gt 0) { # 新增代码+OpenAIConnectSecretGate：仅在存在运行产物目录时执行严格扫描；如果没有这行，空项目会误失败。
  $RuntimeSecretPattern = 'access_token|refresh_token|id_token|authorization_code|secret_ref|oauth-secret-test-value|unit-test-secret-value|sk-[A-Za-z0-9]{12,}|Authorization:\s*Bearer|Bearer\s+|oauth/authorize\?response_type=code|localhost:1455/auth/callback\?code=|["'']?api_key["'']?\s*[:=]|api_key.*sk-' # 修改代码+RealModelLatencySecretGate：定义运行产物禁止出现的敏感字段、授权码 URL 和值形态；如果没有这行，OAuth code/callback 证据可能漏扫。
  ...
  if ($RuntimeSecretMatches.Count -gt 0) { # 新增代码+OpenAIConnectSecretGate：判断运行产物是否命中禁止模式；如果没有这行，visual QA JSON 泄露字段名不会失败。
    Write-Host "Provider secret leak scan failed: runtime visual evidence contains token, secret locator, authorization code, or raw test secret text." # 修改代码+RealModelLatencySecretGate：输出运行产物失败摘要；如果没有这行，用户不知道是证据文件污染。
...
Write-Host "No provider secret leaks detected" # 修改代码+RealModelLatencySecretGate：输出蓝图要求的成功文本；如果没有这行，最终验收命令无法直接匹配预期结果。
