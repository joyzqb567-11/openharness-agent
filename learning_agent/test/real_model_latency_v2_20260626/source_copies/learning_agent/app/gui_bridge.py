...
import os  # 新增代码+真实模型GUI桥接：读取 OPENHARNESS_GUI_MODEL_MODE 模式开关；如果没有这一行，桌面 GUI 无法从 fake 切换到真实模型路径。
...
from learning_agent.app.gui_agent_adapter import DefaultHarnessGuiAgentAdapter, FakeStreamingGuiAgentAdapter, GuiAgentAdapter, GuiAgentRunRequest, GuiAgentRunResult, RealModelGuiAgentAdapter  # 修改代码+真实模型GUI桥接：导入真实模型 adapter；如果没有这一行，real 模式无法接到 CodexCliChatModel 路径。
...
from learning_agent.app.gui_provider_settings import GuiProviderSettingsError, build_provider_settings_payload, disconnect_provider, record_model_failure, save_custom_provider, set_model_visibility, set_provider_auth, test_provider_connection  # 修改代码+ModelFailureState：导入 Provider 设置 helper、模型失败记录、mutation 和连接探针；如果没有这行，真实模型不支持错误无法写回设置页。
...
def _gui_model_mode() -> str:  # 新增代码+真实模型GUI桥接：函数段开始，读取桌面 GUI 模型模式；如果没有这段，real/fake 选择会散落在 manager 内。
    raw_value = os.environ.get("OPENHARNESS_GUI_MODEL_MODE")  # 修改代码+真实模型GUI桥接：读取原始环境变量并保留未设置状态；如果没有这一行，无法区分默认 auto 和显式 fake。
    if raw_value is None or not raw_value.strip():  # 新增代码+真实模型GUI桥接：未设置或空值进入自动模式；如果没有这一行，provider 已连接时主聊天仍会 fake。
        return "auto"  # 新增代码+真实模型GUI桥接：返回自动模式；如果没有这一行，已连接 provider 无法自动启用真实 adapter。
    raw_mode = raw_value.strip().casefold()  # 修改代码+真实模型GUI桥接：规范化显式模式值；如果没有这一行，大小写或空格会导致模式误判。
    if raw_mode == "auto":  # 修改代码+ComposerRealModelRouting：显式 auto 也保持自动模式；如果没有这一行，启动脚本设置 auto 会被误判为 fake。
        return "auto"  # 修改代码+ComposerRealModelRouting：返回自动模式；如果没有这一行，用户显式配置 auto 后 OAuth 已连接仍不会走真实模型。
    if raw_mode in {"real", "codex", "codex_cli"}:  # 新增代码+真实模型GUI桥接：识别允许的真实模型模式名；如果没有这一行，用户设置 real 后仍可能走 fake。
        return "real"  # 新增代码+真实模型GUI桥接：返回真实模式；如果没有这一行，后续无法创建真实 adapter。
    return "fake"  # 修改代码+真实模型GUI桥接：未知或显式 fake 一律回到 fake；如果没有这一行，拼错环境变量可能意外触发真实模型。
# 新增代码+真实模型GUI桥接：函数段结束，_gui_model_mode 到此结束；如果没有边界说明，用户不容易看出模式开关范围。
...
class GuiTurn:  # 新增代码+DesktopGUILifecycle：turn 类段开始，保存一次 GUI prompt 生命周期；如果没有这个类，cancel/retry/resume 无法共享状态。
    ...
    provider_id: str = ""  # 新增代码+ComposerModelToolbar：记录本轮 GUI 选择的 provider；如果没有这行，后端事件和 adapter 无法知道模型来源。
    model_id: str = ""  # 新增代码+ComposerModelToolbar：记录本轮 GUI 选择的模型；如果没有这行，真实模型路由无法消费用户选择。
    reasoning_effort: str = ""  # 新增代码+ComposerModelToolbar：记录本轮 GUI 选择的推理强度；如果没有这行，后端无法审计低/中/高/超高档位。
    permission_mode: str = ""  # 新增代码+ComposerModelToolbar：记录本轮 GUI 选择的权限模式；如果没有这行，后端无法知道用户是否选择完全访问。
...
class GuiRunManager:  # 新增代码+DesktopGUILifecycle：类段开始，管理 GUI turn 生命周期；如果没有这个类，handler 会堆满业务状态逻辑。
    def __init__(self, workspace: str | Path, answer_runner: GuiAnswerRunner | None = None, agent_adapter: GuiAgentAdapter | None = None) -> None:  # 修改代码+GuiAgentAdapter：初始化 run manager 并允许注入 V2 adapter；如果没有这个参数，bridge 无法从 fake adapter 平滑升级到真实 harness adapter。
        ...
        self.agent_adapter_mode = _gui_model_mode() if agent_adapter is None else "custom"  # 新增代码+真实模型GUI桥接：记录当前默认 adapter 模式；如果没有这一行，_run_adapter_turn 无法把请求标成 real。
        self.agent_adapter = agent_adapter or self._default_agent_adapter()  # 修改代码+AutoRealModelGate：按环境变量选择显式 fake 或真实门禁占位 adapter；如果没有这一行，auto 模式无法保留真实模型路由边界。
    ...
    def _real_model_agent_adapter(self) -> GuiAgentAdapter:  # 新增代码+AutoRealModelGate：函数段开始，集中创建真实模型 adapter；如果没有这段，auto、real 和后续动态路由会重复拼构造参数。
        return RealModelGuiAgentAdapter(connected_provider_reader=self._openai_real_provider_connected, model_failure_recorder=self._record_real_model_failure)  # 新增代码+AutoRealModelGate：注入 provider 门禁和模型失败记录器；如果没有这一行，未连接不会快速失败，模型不支持也不会写回菜单。
    # 新增代码+AutoRealModelGate：函数段结束，_real_model_agent_adapter 到此结束；如果没有边界说明，用户不容易看出真实 adapter 的统一创建点。

    def _default_agent_adapter(self) -> GuiAgentAdapter:  # 新增代码+真实模型GUI桥接：函数段开始，按模式创建默认 GUI adapter；如果没有这段，__init__ 会堆入真实/fake 分支细节。
        if self.agent_adapter_mode == "real":  # 修改代码+AutoRealModelGate：只有显式 real 在初始化时直接创建真实 adapter；如果没有这一行，real 模式会误走 smoke fake。
            return self._real_model_agent_adapter()  # 修改代码+AutoRealModelGate：复用统一真实 adapter 工厂；如果没有这一行，real 模式无法使用 provider 门禁和失败记录。
        if self.agent_adapter_mode != "auto":  # 新增代码+ComposerRealModelRouting：只有显式 fake 才固化为 fake；如果没有这一行，启动时未连接的 auto 会被永久降级。
            self.agent_adapter_mode = "fake"  # 修改代码+ComposerRealModelRouting：显式 fake 时固化为 fake；如果没有这一行，后续请求模式可能保留前端不认识的值。
        return FakeStreamingGuiAgentAdapter()  # 修改代码+AutoRealModelGate：只给显式 fake 或 auto 的内部占位保留 deterministic adapter；如果没有这一行，smoke 测试无法在无模型环境稳定运行。
    # 新增代码+真实模型GUI桥接：函数段结束，_default_agent_adapter 到此结束；如果没有边界说明，用户不容易看出 adapter 选择范围。

    def _openai_real_provider_connected(self, provider_id: str = "openai") -> bool:  # 修改代码+ComposerRealModelRouting：函数段开始，按 provider 读取是否具备真实模型连接来源；如果没有这段，真实模型模式会绕过设置页连接状态。
        clean_provider_id = str(provider_id or "openai").strip() or "openai"  # 新增代码+ComposerRealModelRouting：清理 provider id 并默认 OpenAI；如果没有这一行，空 provider 会破坏旧调用兼容。
        if clean_provider_id != "openai":  # 新增代码+ComposerRealModelRouting：当前 ChatGPT OAuth 真实路径只支持 OpenAI；如果没有这一行，非 OpenAI provider 可能被误判为已连接。
            return False  # 新增代码+ComposerRealModelRouting：非 OpenAI 一律不放行到 ChatGPT OAuth adapter；如果没有这一行，Google/OpenRouter 选择可能被发往 OpenAI。
        try:  # 新增代码+真实模型GUI桥接：保护 provider payload 构建；如果没有这一行，设置读取异常会让后台 worker 崩溃。
            payload = build_provider_settings_payload(self.workspace)  # 新增代码+真实模型GUI桥接：复用设置页事实源；如果没有这一行，真实模式会维护第二套连接判断。
        except GuiProviderSettingsError:  # 新增代码+真实模型GUI桥接：处理设置页已知错误；如果没有这一行，用户只会看到泛化线程失败。
            return False  # 新增代码+真实模型GUI桥接：provider 读取失败时禁止真实模型调用；如果没有这一行，失败状态可能被误判为已连接。
        providers = payload.get("providers", []) if isinstance(payload, dict) else []  # 新增代码+真实模型GUI桥接：安全读取 provider 列表；如果没有这一行，坏 payload 会导致类型异常。
        for provider in providers:  # 新增代码+真实模型GUI桥接：遍历所有 provider；如果没有这一行，无法找到 OpenAI 条目。
            if not isinstance(provider, dict):  # 新增代码+真实模型GUI桥接：跳过非 dict 条目；如果没有这一行，坏数据会让 get 调用报错。
                continue  # 新增代码+真实模型GUI桥接：继续看下一个条目；如果没有这一行，坏条目会中断整个判断。
            if provider.get("id") != clean_provider_id:  # 修改代码+ComposerRealModelRouting：只关心本轮 provider；如果没有这一行，Anthropic 等 provider 可能误打开 Codex CLI 模型路径。
                continue  # 新增代码+真实模型GUI桥接：跳过非 OpenAI；如果没有这一行，后续 source 判断会混淆提供商。
            provider_source = str(provider.get("source", ""))  # 修改代码+DirectOAuthModelFactory：读取连接来源；如果没有这一行，无法区分 codex_cli、direct_oauth、mock 和 API key。
            return bool(provider.get("connected")) and provider_source in {"codex_cli", "direct_oauth_experimental"}  # 修改代码+DirectOAuthModelFactory：codex_cli 和 direct OAuth 已连接都放行真实模型；如果没有这行代码，direct OAuth 登录成功后主聊天仍会 fake。
        return False  # 新增代码+真实模型GUI桥接：没有 OpenAI 条目时禁止真实模型；如果没有这一行，函数可能返回 None。
    # 新增代码+真实模型GUI桥接：函数段结束，_openai_real_provider_connected 到此结束；如果没有边界说明，用户不容易看出连接门禁范围。

    def _record_real_model_failure(self, provider_id: str, model_id: str, error_kind: str, message: str) -> None:  # 新增代码+ModelFailureState：函数段开始，把真实模型失败写回 provider 设置；如果没有这段，GUI 运行时无法标记最近失败模型。
        record_model_failure(self.workspace, provider_id, model_id, error_kind, message)  # 新增代码+ModelFailureState：调用 provider 设置记录器且只传低敏摘要；如果没有这行，设置页和底部菜单拿不到错误事实。
    # 新增代码+ModelFailureState：函数段结束，_record_real_model_failure 到此结束；如果没有边界说明，用户不容易看出它不读取 token。
    ...
    def _adapter_for_turn(self, turn: GuiTurn) -> GuiAgentAdapter:  # 新增代码+GuiAgentAdapter：函数段开始，选择本轮使用 fake adapter 还是真实 harness shell；如果没有这段，真实模式触发语义会散落在 worker 中。
        ...
        if self.agent_adapter_mode in {"auto", "real"}:  # 修改代码+AutoRealModelGate：auto/real 每一轮都交给真实 adapter 自己做连接门禁；如果没有这一行，未连接时会继续 fake streaming 假成功。
            return self._real_model_agent_adapter()  # 修改代码+AutoRealModelGate：为本轮创建真实模型 adapter；如果没有这一行，OAuth 后模型路由和未连接快速失败都会失效。
    ...
    def _run_adapter_turn(self, turn: GuiTurn) -> GuiAgentRunResult:  # 新增代码+GuiAgentAdapter：函数段开始，把 GUI turn 转交给 adapter；如果没有这段，worker 只能调用旧 answer_runner。
        ...
        request_mode = "real" if self.agent_adapter_mode in {"auto", "real"} or isinstance(adapter, (DefaultHarnessGuiAgentAdapter, RealModelGuiAgentAdapter)) else "fake"  # 修改代码+AutoRealModelGate：auto/real 统一标记真实请求模式；如果没有这一行，测试替身或动态真实 adapter 会被误标成 fake。
        request = GuiAgentRunRequest(session_id=turn.session_id, turn_id=turn.turn_id, run_id=turn.run_id, prompt=turn.prompt, mode=request_mode, provider_id=turn.provider_id, model_id=turn.model_id, reasoning_effort=turn.reasoning_effort, permission_mode=turn.permission_mode)  # 修改代码+ComposerModelToolbar：构造 adapter 请求对象并传入 GUI 模型上下文；如果没有这一行，真实 adapter 收不到用户在底部菜单里的选择。
    ...
    def start_message(self, conversation_id: str, prompt: str, client_request_id: str = "", retry_of: str = "", provider_id: str = "", model_id: str = "", reasoning_effort: str = "", permission_mode: str = "") -> dict[str, Any]:  # 修改代码+ComposerModelToolbar：函数段开始，提交 GUI prompt 和模型上下文；如果没有这段，POST /messages 无法创建带模型选择的 turn。
        ...
        clean_provider_id = provider_id.strip()[:120]  # 新增代码+ComposerModelToolbar：清理 provider_id 并限制长度；如果没有这行，坏输入可能污染事件流。
        clean_model_id = model_id.strip()[:160]  # 新增代码+ComposerModelToolbar：清理 model_id 并限制长度；如果没有这行，坏输入可能污染后端运行上下文。
        clean_reasoning_effort = reasoning_effort.strip()[:40]  # 新增代码+ComposerModelToolbar：清理推理强度字段；如果没有这行，事件流会直接保存未经约束的字符串。
        clean_permission_mode = permission_mode.strip()[:80]  # 新增代码+ComposerModelToolbar：清理权限模式字段；如果没有这行，权限审计字段可能被异常输入撑大。
    ...
    def retry_turn(self, turn_id: str) -> dict[str, Any]:  # 新增代码+DesktopGUILifecycle：函数段开始，重试指定 turn；如果没有这段，POST /retry 无法工作。
        ...
        response = self.start_message(old_turn.session_id, old_turn.prompt, client_request_id="", retry_of=old_turn.turn_id, provider_id=old_turn.provider_id, model_id=old_turn.model_id, reasoning_effort=old_turn.reasoning_effort, permission_mode=old_turn.permission_mode)  # 修改代码+ComposerModelToolbar：用旧 prompt 和旧模型上下文创建新 turn；如果没有这行，retry 会丢失用户原先选择的模型。
    ...
    def do_POST(self) -> None:  # 新增代码+DesktopGUILifecycle：函数段开始，处理 GUI 生命周期 POST；如果没有这段，messages/cancel/retry/resume 全部不可用。
        ...
        try:  # 新增代码+DesktopGUILifecycle：统一捕获业务结构化错误；如果没有这行，每个分支都要重复 try/except。
            if path == "/v1/gui/messages":  # 新增代码+DesktopGUILifecycle：识别发送消息端点；如果没有这行，GUI 无法创建 turn。
                payload = self.server.run_manager.start_message(str(body.get("conversation_id", "default")), str(body.get("prompt", "")), client_request_id=str(body.get("client_request_id", "")), provider_id=str(body.get("provider_id", "")), model_id=str(body.get("model_id", "")), reasoning_effort=str(body.get("reasoning_effort", "")), permission_mode=str(body.get("permission_mode", "")))  # 修改代码+ComposerModelToolbar：提交 prompt 和模型上下文给 run manager；如果没有这行，POST /messages 会丢失底部菜单选择。
