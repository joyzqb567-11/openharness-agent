...
GUI_V2_EVENT_KINDS = {  # 新增代码+GuiV2Protocol：集合段开始，列出 V2 主事件类型；如果没有这段，事件状态机没有协议白名单。
    ...
    "model_call_started",  # 新增代码+真实模型延迟可观测性：表示真实模型调用已开始；如果没有这一项，adapter 的模型诊断事件会被协议层拒绝。
    "model_call_status",  # 新增代码+真实模型延迟可观测性：表示真实模型连接、fallback、等待等中间状态；如果没有这一项，GUI 无法解释回复慢在哪里。
    "model_first_delta",  # 新增代码+真实模型延迟可观测性：表示首个模型文本增量已到达；如果没有这一项，首 token 延迟无法被前端单独展示。
    "model_call_completed",  # 新增代码+真实模型延迟可观测性：表示真实模型调用完成；如果没有这一项，状态面板无法收尾并展示总耗时。
    "model_call_failed",  # 新增代码+真实模型延迟可观测性：表示真实模型调用失败；如果没有这一项，模型层错误只能退化成泛化 turn_failed。
