...
DIRECT_OPENAI_AUTH_MODES = {"direct_oauth"}  # 新增代码+DirectOAuthConfig：集中定义 V1C 实验直连 OAuth 模式；如果没有这行代码，配置层无法把 direct_oauth 从稳定 mock/real 路径里单独拎出来。
REAL_OPENAI_AUTH_MODES = {"real_headless", "real_browser"} | DIRECT_OPENAI_AUTH_MODES  # 修改代码+DirectOAuthConfig：把 direct_oauth 纳入真实 OAuth 门禁集合；如果没有这行代码，direct_oauth 会被当作未知安全模式悄悄放过。
...
def build_openai_auth_config(env: Mapping[str, str] | None = None) -> OpenAIAuthConfig:  # 新增代码+OpenAIAuthConfig：函数段开始，构造 OpenAI Auth 配置摘要；如果没有这段，auth-attempt 无法统一读取安全门禁。
    ...
    if auth_mode in DIRECT_OPENAI_AUTH_MODES and reasons:  # 新增代码+DirectOAuthConfig：direct_oauth 缺安全条件时立刻硬失败；如果没有这行代码，实验直连可能回退成 mock 并误导用户。
        missing = ", ".join(reasons)  # 新增代码+DirectOAuthConfig：把缺失条件合并成可读文本；如果没有这行代码，错误消息无法告诉用户具体缺什么。
        message = f"OpenAI direct OAuth experimental mode blocked: {missing}. Required: OPENHARNESS_OPENAI_EXPERIMENTAL=1, OPENHARNESS_PROVIDER_SECRET_STORE=os_encrypted, OPENHARNESS_OPENAI_CLIENT_ID. OpenHarness will not default to an OpenCode client_id."  # 新增代码+DirectOAuthConfig：说明 direct OAuth 必须显式实验开关、OS 加密和自有 client id；如果没有这行代码，用户可能误以为可以直接借用 OpenCode 配置。
        raise OpenAIAuthConfigError("openai_direct_oauth_blocked", message)  # 新增代码+DirectOAuthConfig：抛出 direct OAuth 专用错误码；如果没有这行代码，前端无法区分实验门禁和普通真实 OAuth 门禁。
