...
from typing import Any, Callable, Iterable, Mapping  # 修改代码+DirectOAuthSseProbe：补充 Mapping 描述 probe 环境字典；若没有 Mapping，Direct OAuth SSE 门禁 helper 的类型契约不清楚。
...
try:  # 新增代码+CodexCliStreaming：包运行模式下导入 Codex CLI 可观测 runner；若没有这行代码，桌面 GUI 只能继续等待阻塞式 chat。
    from learning_agent.models.codex_cli_stream import CodexCliStreamingRunner, CodexProcessFactory  # 新增代码+CodexCliStreaming：导入 runner 和进程工厂类型；若没有这行代码，CodexCliChatModel 不能接入 Popen streaming。
    from learning_agent.models.streaming import ModelStreamEvent  # 新增代码+CodexCliStreaming：导入模型流事件类型；若没有这行代码，stream_chat 返回值没有统一协议。
except ModuleNotFoundError as error:  # 新增代码+CodexCliStreaming：捕获脚本模式下 learning_agent 包名不可用；若没有这行代码，start_oauth_agent.bat 可能导入失败。
    if error.name not in {"learning_agent", "learning_agent.models", "learning_agent.models.codex_cli_stream", "learning_agent.models.streaming"}:  # 新增代码+CodexCliStreaming：只对包路径缺失启用 fallback；若没有这行代码，runner 内部真实 bug 会被误吞。
        raise  # 新增代码+CodexCliStreaming：重新抛出真实导入错误；若没有这行代码，排查 streaming runner 问题会很困难。
    from models.codex_cli_stream import CodexCliStreamingRunner, CodexProcessFactory  # type: ignore  # 新增代码+CodexCliStreaming：脚本模式导入 runner；若没有这行代码，直接脚本启动无法使用可观测 CLI。
    from models.streaming import ModelStreamEvent  # type: ignore  # 新增代码+CodexCliStreaming：脚本模式导入事件类型；若没有这行代码，stream_chat 在 bat 入口没有返回协议。
...
class CodexCliChatModel:  # 作用: 通过本机 codex exec 调用 Codex/GPT-5.5 的旧桥接模型
    def __init__(  # 作用: 初始化 Codex CLI 桥接模型
        ...
        process_factory: CodexProcessFactory | None = None,  # 新增代码+CodexCliStreaming：可选 Popen/fake 进程工厂；若没有这行代码，stream_chat 单元测试只能启动真实 Codex CLI。
    ) -> None:
        ...
        self._process_factory = process_factory  # 新增代码+CodexCliStreaming：保存可观测 runner 的进程工厂；若没有这行代码，GUI 流式路径无法注入测试进程。
    ...
    def stream_chat(self, messages: list[dict[str, Any]], tools: list[dict[str, Any]], *, turn_id: str, provider_id: str, model_id: str, is_cancelled: Callable[[], bool] | None = None) -> Iterable[ModelStreamEvent]:  # 修改代码+CodexCliCancelBridge：函数段开始，给 GUI 提供可观测且可取消的 Codex CLI 事件流；若没有这段，桌面外壳仍只能阻塞等待最终回答。
        prompt = self._build_prompt(messages=messages, tools=tools)  # 新增代码+CodexCliStreaming：复用旧 chat 的 prompt 构造；若没有这行代码，streaming 路径和旧模型协议会漂移。
        with tempfile.TemporaryDirectory(prefix="learning_agent_codex_") as raw_dir:  # 新增代码+CodexCliStreaming：创建临时目录保存 schema 和最终输出；若没有这行代码，Codex CLI strict 输出没有落盘位置。
            temp_dir = Path(raw_dir)  # 新增代码+CodexCliStreaming：把临时目录字符串转为 Path；若没有这行代码，后续路径拼接会更脆弱。
            schema_path = temp_dir / "output_schema.json"  # 新增代码+CodexCliStreaming：定义输出 schema 文件路径；若没有这行代码，codex exec 无法读取结构化输出约束。
            output_path = temp_dir / "last_message.json"  # 新增代码+CodexCliStreaming：定义最终消息输出路径；若没有这行代码，无 token delta 时无法读取最终回答。
            schema_path.write_text(json.dumps(self._output_schema(tools=tools), ensure_ascii=False, indent=2), encoding="utf-8")  # 新增代码+CodexCliStreaming：写入当前工具池对应的 strict schema；若没有这行代码，streaming 路径会丢失工具输出协议。
            command = self._build_command(schema_path=schema_path, output_path=output_path)  # 新增代码+CodexCliStreaming：构造 codex exec 命令；若没有这行代码，runner 没有可执行命令。
            self.last_command = command  # 新增代码+CodexCliStreaming：记录最近命令用于测试和排查；若没有这行代码，模型选择是否生效不可审计。
            runner = CodexCliStreamingRunner(command=command, prompt=prompt, output_path=output_path, cwd=self._cwd, process_factory=self._process_factory, stdout_contains_token_deltas=self._codex_cli_stdout_contains_token_deltas(), parse_output=self._parse_model_message, timeout_seconds=self._timeout_seconds)  # 新增代码+CodexCliStreaming：创建可观测 CLI runner；若没有这行代码，Popen 状态、stderr warning 和最终文件 fallback 都不会进入 GUI。
            yield from runner.stream(turn_id=turn_id, provider_id=provider_id, model_id=model_id, is_cancelled=is_cancelled)  # 修改代码+CodexCliCancelBridge：透传 runner 事件流并传入 GUI 取消回调；若没有这行代码，adapter 收不到连接、fallback、delta、取消和完成事件。
    # 新增代码+CodexCliStreaming：函数段结束，CodexCliChatModel.stream_chat 到此结束；若没有边界说明，用户不容易看出 GUI 流式入口范围。
    ...
    @staticmethod  # 新增代码+CodexCliStreaming：该门禁不依赖实例状态；若没有这行代码，当前无 token delta 证据的 CLI 可能被误标为真流式。
    def _codex_cli_stdout_contains_token_deltas() -> bool:  # 新增代码+CodexCliStreaming：函数段开始，读取 stdout delta discovery 开关；若没有这段，未来验证 delta 后无法安全开启解析。
        raw_value = os.environ.get("OPENHARNESS_CODEX_CLI_STDOUT_DELTAS", "").strip().lower()  # 新增代码+CodexCliStreaming：读取显式环境变量；若没有这行代码，runner 无法区分默认禁用和人工开启。
        return raw_value in {"1", "true", "yes", "on"}  # 新增代码+CodexCliStreaming：只有明确开启才认为 stdout 有 token delta；若没有这行代码，普通 lifecycle JSON 可能被误当正文。
    # 新增代码+CodexCliStreaming：函数段结束，CodexCliChatModel._codex_cli_stdout_contains_token_deltas 到此结束；若没有边界说明，用户不容易看出 discovery 门禁范围。
...
DIRECT_OAUTH_SSE_PROBE_TRUE_VALUES = {"1", "true", "yes", "on"}  # 新增代码+DirectOAuthSseProbe：集中定义实验开关的真值；如果没有这行，多个函数会各自猜测哪些字符串算开启。
DIRECT_OAUTH_SSE_PROBE_REQUIRED_SECRET_STORE = "os_encrypted"  # 新增代码+DirectOAuthSseProbe：要求 direct OAuth token 使用系统加密存储；如果没有这行，probe 可能在不安全存储下误放行。


@dataclass(frozen=True)  # 新增代码+DirectOAuthSseProbe：把 probe 结果做成不可变数据对象；如果没有这行，调用方可能在记录证据时意外改写状态。
class CodexOAuthStreamingProbeResult:  # 新增代码+DirectOAuthSseProbe：类段开始，保存 Direct OAuth SSE fast path 的安全探测摘要；如果没有这个类，probe 只能返回散乱字符串。
    status: str  # 新增代码+DirectOAuthSseProbe：保存 machine-readable 状态；如果没有这行，后续门禁无法稳定判断 available/not_configured。
    message: str = ""  # 新增代码+DirectOAuthSseProbe：保存不含 token 的短说明；如果没有这行，人工查看证据时不知道为什么被拒绝。
    event_types: tuple[str, ...] = ()  # 新增代码+DirectOAuthSseProbe：保存已见 SSE 事件类型；如果没有这行，未知 payload 排查缺少形状证据。

    @property  # 新增代码+DirectOAuthSseProbe：属性段开始，让调用方用布尔值判断是否可用；如果没有这行，每个调用点都要比较状态字符串。
    def available(self) -> bool:  # 新增代码+DirectOAuthSseProbe：函数段开始，判断 probe 是否允许声明 fast path 可用；如果没有这段，UI 和文档容易把未验证链路误报为成功。
        return self.status == "available"  # 新增代码+DirectOAuthSseProbe：只有明确 available 才放行；如果没有这行，401/未知 payload 可能误被当成功。
    # 新增代码+DirectOAuthSseProbe：函数段结束，available 到此结束；如果没有这个边界说明，用户不容易看出布尔判断范围。

    def to_safe_dict(self) -> dict[str, object]:  # 新增代码+DirectOAuthSseProbe：函数段开始，返回可写入证据文件的脱敏字典；如果没有这段，调用方可能直接序列化包含原始流的对象。
        return {  # 新增代码+DirectOAuthSseProbe：开始构造安全摘要；如果没有这行，函数没有返回值。
            "status": self.status,  # 新增代码+DirectOAuthSseProbe：写入状态；如果没有这行，证据无法判断 probe 结论。
            "available": self.available,  # 新增代码+DirectOAuthSseProbe：写入布尔结论；如果没有这行，人工验收还要重复理解状态字符串。
            "message": self.message,  # 新增代码+DirectOAuthSseProbe：写入短说明；如果没有这行，失败原因不透明。
            "event_types": list(self.event_types),  # 新增代码+DirectOAuthSseProbe：写入事件类型列表而不是原始 payload；如果没有这行，无法安全排查 SSE 形状。
        }  # 新增代码+DirectOAuthSseProbe：安全摘要构造结束；如果没有这行，Python 字典语法不完整。
    # 新增代码+DirectOAuthSseProbe：函数段结束，to_safe_dict 到此结束；如果没有这个边界说明，用户不容易看出它不会返回 token。
# 新增代码+DirectOAuthSseProbe：类段结束，CodexOAuthStreamingProbeResult 到此结束；如果没有这个边界说明，用户不容易看出结果对象范围。


def _direct_oauth_probe_env_value(env: Mapping[str, str], name: str) -> str:  # 新增代码+DirectOAuthSseProbe：函数段开始，安全读取环境字典中的字符串值；如果没有这段，None 或非字符串测试值可能污染门禁判断。
    return str(env.get(name, "") or "").strip()  # 新增代码+DirectOAuthSseProbe：清理环境变量文本；如果没有这行，空格或 None 会让配置判断不稳定。
# 新增代码+DirectOAuthSseProbe：函数段结束，_direct_oauth_probe_env_value 到此结束；如果没有这个边界说明，用户不容易看出它只做读取清洗。


def direct_oauth_sse_probe_environment_ready(env: Mapping[str, str] | None = None) -> bool:  # 新增代码+DirectOAuthSseProbe：函数段开始，判断 direct OAuth SSE probe 是否具备安全前置条件；如果没有这段，fast path 可能在未授权或未加密环境被误启用。
    source_env = env if env is not None else os.environ  # 新增代码+DirectOAuthSseProbe：允许测试注入环境，真实运行默认读 os.environ；如果没有这行，测试会污染用户真实环境。
    auth_mode = _direct_oauth_probe_env_value(source_env, "OPENHARNESS_OPENAI_AUTH_MODE").lower()  # 新增代码+DirectOAuthSseProbe：读取认证模式；如果没有这行，无法确认当前是否 direct_oauth。
    experimental = _direct_oauth_probe_env_value(source_env, "OPENHARNESS_OPENAI_EXPERIMENTAL").lower()  # 新增代码+DirectOAuthSseProbe：读取实验开关；如果没有这行，未显式同意的 fast path 会被启用。
    secret_store = _direct_oauth_probe_env_value(source_env, "OPENHARNESS_PROVIDER_SECRET_STORE").lower()  # 新增代码+DirectOAuthSseProbe：读取密钥存储类型；如果没有这行，明文 token store 也可能过门禁。
    client_id = _direct_oauth_probe_env_value(source_env, "OPENHARNESS_OPENAI_CLIENT_ID")  # 新增代码+DirectOAuthSseProbe：读取 OpenHarness 自己的 OAuth client id；如果没有这行，代码可能回落到别的项目 client id。
    return auth_mode == "direct_oauth" and experimental in DIRECT_OAUTH_SSE_PROBE_TRUE_VALUES and secret_store == DIRECT_OAUTH_SSE_PROBE_REQUIRED_SECRET_STORE and bool(client_id)  # 新增代码+DirectOAuthSseProbe：全部安全条件满足才返回 True；如果没有这行，probe 门禁形同虚设。
# 新增代码+DirectOAuthSseProbe：函数段结束，direct_oauth_sse_probe_environment_ready 到此结束；如果没有这个边界说明，用户不容易看出四个前置条件。


def _codex_oauth_sse_probe_payloads(raw_stream: str) -> list[dict[str, Any]]:  # 新增代码+DirectOAuthSseProbe：函数段开始，把 SSE 文本解析为 JSON payload 列表；如果没有这段，probe 无法判断远端 payload 形状。
    payloads: list[dict[str, Any]] = []  # 新增代码+DirectOAuthSseProbe：准备保存已解析事件；如果没有这行，后续没有容器。
    for raw_line in raw_stream.splitlines():  # 新增代码+DirectOAuthSseProbe：逐行扫描 SSE 文本；如果没有这行，无法处理多事件流。
        line = raw_line.strip()  # 新增代码+DirectOAuthSseProbe：清理空白；如果没有这行，缩进或换行会影响 data 判断。
        if not line.startswith("data:"):  # 新增代码+DirectOAuthSseProbe：只处理 data 行；如果没有这行，event/id 行会被误当 JSON。
            continue  # 新增代码+DirectOAuthSseProbe：跳过非 data 行；如果没有这行，后续解析会报错。
        data = line.removeprefix("data:").strip()  # 新增代码+DirectOAuthSseProbe：取出 JSON payload；如果没有这行，json.loads 会看到 data 前缀。
        if not data or data == "[DONE]":  # 新增代码+DirectOAuthSseProbe：跳过空 payload 和结束标记；如果没有这行，[DONE] 会被当成坏 JSON。
            continue  # 新增代码+DirectOAuthSseProbe：继续读下一行；如果没有这行，结束标记会打断列表构造。
        try:  # 新增代码+DirectOAuthSseProbe：捕获单个坏事件；如果没有这行，未知兼容事件会让整个 probe 失败。
            payload = json.loads(data)  # 新增代码+DirectOAuthSseProbe：解析 JSON 事件；如果没有这行，无法读取 type/delta/response。
        except json.JSONDecodeError:  # 新增代码+DirectOAuthSseProbe：处理非 JSON data；如果没有这行，probe 会把注释事件当异常。
            continue  # 新增代码+DirectOAuthSseProbe：跳过坏事件继续收集；如果没有这行，后续好事件无法被看到。
        if isinstance(payload, dict):  # 新增代码+DirectOAuthSseProbe：只接受对象 payload；如果没有这行，列表或字符串事件会污染 shape 判断。
            payloads.append(payload)  # 新增代码+DirectOAuthSseProbe：保存安全的结构化 payload；如果没有这行，probe 永远看不到事件。
    return payloads  # 新增代码+DirectOAuthSseProbe：返回已解析事件列表；如果没有这行，调用方拿不到 probe 输入。
# 新增代码+DirectOAuthSseProbe：函数段结束，_codex_oauth_sse_probe_payloads 到此结束；如果没有这个边界说明，用户不容易看出它不返回原始流。


def _codex_oauth_sse_probe_event_types(payloads: Iterable[dict[str, Any]]) -> tuple[str, ...]:  # 新增代码+DirectOAuthSseProbe：函数段开始，从 payload 提取去重事件类型；如果没有这段，证据会缺少远端响应形状摘要。
    event_types: list[str] = []  # 新增代码+DirectOAuthSseProbe：准备按出现顺序保存类型；如果没有这行，无法稳定输出列表。
    for payload in payloads:  # 新增代码+DirectOAuthSseProbe：遍历已解析事件；如果没有这行，无法提取多个事件类型。
        event_type = str(payload.get("type", "") or "")  # 新增代码+DirectOAuthSseProbe：读取事件 type；如果没有这行，未知事件不能进入证据。
        if event_type and event_type not in event_types:  # 新增代码+DirectOAuthSseProbe：只记录非空且未出现过的类型；如果没有这行，证据会重复又嘈杂。
            event_types.append(event_type)  # 新增代码+DirectOAuthSseProbe：保存事件类型；如果没有这行，返回列表永远为空。
    return tuple(event_types)  # 新增代码+DirectOAuthSseProbe：返回不可变事件类型摘要；如果没有这行，调用方可能误改证据。
# 新增代码+DirectOAuthSseProbe：函数段结束，_codex_oauth_sse_probe_event_types 到此结束；如果没有这个边界说明，用户不容易看出它只暴露类型。


def _codex_oauth_completed_response_has_output(response: dict[str, Any]) -> bool:  # 新增代码+DirectOAuthSseProbe：函数段开始，判断 completed response 是否含可消费输出；如果没有这段，probe 无法识别非 delta 但已完成的合法流。
    output_text = response.get("output_text")  # 新增代码+DirectOAuthSseProbe：读取 Responses output_text；如果没有这行，纯文本完成事件无法判断。
    output = response.get("output")  # 新增代码+DirectOAuthSseProbe：读取 Responses output 数组；如果没有这行，native output 完成事件无法判断。
    return isinstance(output_text, str) or isinstance(output, list)  # 新增代码+DirectOAuthSseProbe：文本或 output 数组任一存在即认为 shape 可消费；如果没有这行，completed 事件会被误判未知。
# 新增代码+DirectOAuthSseProbe：函数段结束，_codex_oauth_completed_response_has_output 到此结束；如果没有这个边界说明，用户不容易看出它只检查 shape。


def probe_codex_oauth_sse_payload(raw_stream: str, env: Mapping[str, str] | None = None, http_status: int = 200) -> CodexOAuthStreamingProbeResult:  # 新增代码+DirectOAuthSseProbe：函数段开始，给 Direct OAuth SSE fast path 做安全可审计 probe；如果没有这段，项目会把未验证 SSE 当成成熟能力。
    payloads = _codex_oauth_sse_probe_payloads(raw_stream)  # 新增代码+DirectOAuthSseProbe：先解析事件 payload；如果没有这行，后续只能凭原始字符串猜测。
    event_types = _codex_oauth_sse_probe_event_types(payloads)  # 新增代码+DirectOAuthSseProbe：提取事件类型摘要；如果没有这行，失败证据没有 payload 形状。
    if not direct_oauth_sse_probe_environment_ready(env):  # 新增代码+DirectOAuthSseProbe：先检查 direct OAuth 安全前置条件；如果没有这行，未配置环境也可能被误判可用。
        return CodexOAuthStreamingProbeResult(status="not_configured", message="direct_oauth_sse_probe_requires_explicit_env", event_types=event_types)  # 新增代码+DirectOAuthSseProbe：返回未配置而不是失败；如果没有这行，用户会把未启用实验误会成远端坏了。
    if http_status == 401:  # 新增代码+DirectOAuthSseProbe：识别未授权状态；如果没有这行，登录过期会被当未知 payload。
        return CodexOAuthStreamingProbeResult(status="unavailable_unauthorized", message="http_401", event_types=event_types)  # 新增代码+DirectOAuthSseProbe：返回脱敏 401；如果没有这行，证据可能写入错误体里的敏感内容。
    if http_status == 403:  # 新增代码+DirectOAuthSseProbe：识别账号无权限状态；如果没有这行，模型/账号权限问题会被归类错误。
        return CodexOAuthStreamingProbeResult(status="unavailable_forbidden", message="http_403", event_types=event_types)  # 新增代码+DirectOAuthSseProbe：返回脱敏 403；如果没有这行，用户无法区分未授权和无权限。
    if http_status < 200 or http_status >= 300:  # 新增代码+DirectOAuthSseProbe：处理其它非成功 HTTP 状态；如果没有这行，500/429 也可能继续判断 payload。
        return CodexOAuthStreamingProbeResult(status="unavailable_http_status", message=f"http_{http_status}", event_types=event_types)  # 新增代码+DirectOAuthSseProbe：只记录状态码不记录响应体；如果没有这行，证据可能含服务端原文。
    for payload in payloads:  # 新增代码+DirectOAuthSseProbe：遍历已解析 SSE 事件；如果没有这行，无法识别真实 delta/done。
        event_type = str(payload.get("type", "") or "")  # 新增代码+DirectOAuthSseProbe：读取事件类型；如果没有这行，后续条件没有输入。
        if event_type == "response.output_text.delta" and isinstance(payload.get("delta"), str):  # 新增代码+DirectOAuthSseProbe：识别真实文本增量事件；如果没有这行，fast path 的核心证据不会被采信。
            return CodexOAuthStreamingProbeResult(status="available", message="output_text_delta_seen", event_types=event_types)  # 新增代码+DirectOAuthSseProbe：允许声明 SSE 可用；如果没有这行，已验证 delta 仍无法放行。
        if event_type == "response.output_text.done" and isinstance(payload.get("text"), str):  # 新增代码+DirectOAuthSseProbe：识别完整文本完成事件；如果没有这行，短回答无 delta 时会被误判失败。
            return CodexOAuthStreamingProbeResult(status="available", message="output_text_done_seen", event_types=event_types)  # 新增代码+DirectOAuthSseProbe：允许短流完成场景；如果没有这行，probe 对小响应过于苛刻。
        response = payload.get("response")  # 新增代码+DirectOAuthSseProbe：读取 completed 事件的 response 字段；如果没有这行，无法检查完整响应 shape。
        if event_type == "response.completed" and isinstance(response, dict) and _codex_oauth_completed_response_has_output(response):  # 新增代码+DirectOAuthSseProbe：识别可消费的 completed response；如果没有这行，非 delta 但完整的 SSE 会被误判未知。
            return CodexOAuthStreamingProbeResult(status="available", message="completed_response_seen", event_types=event_types)  # 新增代码+DirectOAuthSseProbe：允许 completed-only 合法流；如果没有这行，短响应无法进入 fast path。
    return CodexOAuthStreamingProbeResult(status="unavailable_unknown_payload", message="no_supported_sse_output_event", event_types=event_types)  # 新增代码+DirectOAuthSseProbe：未知 payload 默认不放行；如果没有这行，probe 可能把空流当成功。
# 新增代码+DirectOAuthSseProbe：函数段结束，probe_codex_oauth_sse_payload 到此结束；如果没有这个边界说明，用户不容易看出 fast path 的保守判断范围。
...
class CodexOAuthChatModel:  # 作用: 参考 opencode2 的 OpenAI OAuth/API 链路实现 GPT-5.5 模型适配器
    ...
    def __init__(  # 作用: 初始化 OAuth/API 直连模型适配器
        ...
        client_id: str | None = None,  # 新增代码+DirectOAuthClientId：允许 OpenHarness direct OAuth 注入自己的 OAuth client id；如果没有这行代码，refresh/token exchange 会误用 OpenCode client id。
    ) -> None:
        self._model = model  # 作用: 保存模型名
        ...
        self._client_id = str(client_id or self.CLIENT_ID).strip() or self.CLIENT_ID  # 新增代码+DirectOAuthClientId：保存本实例使用的 OAuth client id；如果没有这行代码，direct OAuth 和旧默认 client 无法安全区分。
    ...
    def _refresh_tokens(self, tokens: CodexOAuthTokens) -> CodexOAuthTokens:  # 作用: 使用 refresh_token 换新的 access_token
        payload = self._post_json(  # 作用: 请求 OAuth token 端点
            ...
            {  # 作用: 刷新请求体
                ...
                "client_id": self._client_id,  # 修改代码+DirectOAuthClientId：刷新 token 时使用本实例 client id；如果没有这行代码，OpenHarness direct OAuth token 可能用 OpenCode client id 刷新失败。
            },  # 作用: 请求体结束
        )  # 作用: HTTP POST 结束
    ...
    def _exchange_code_for_tokens(self, code: str, redirect_uri: str, code_verifier: str) -> dict[str, object]:  # 作用: 使用授权 code 换 access/refresh token
        return self._post_json(  # 作用: 发起 token exchange 请求
            ...
            {  # 作用: 授权码交换请求体
                ...
                "client_id": self._client_id,  # 修改代码+DirectOAuthClientId：授权码交换时使用本实例 client id；如果没有这行代码，direct OAuth code exchange 会和授权 URL 的 client id 不一致。
    ...
    def _build_authorize_url(self, redirect_uri: str, pkce_challenge: str, state: str) -> str:  # 修改代码+DirectOAuthClientId：函数段开始，使用本实例 client id 构造授权 URL；如果没有这段代码，direct OAuth 可能仍依赖 OpenCode client id。
        params = urllib.parse.urlencode(  # 作用: 编码 query 参数
            {  # 作用: 授权参数字典
                "response_type": "code",  # 作用: 使用授权码模式
                "client_id": self._client_id,  # 修改代码+DirectOAuthClientId：授权 URL 使用本实例 client id；如果没有这行代码，OpenHarness 无法使用自己的 OAuth client。
        ...
        return f"{self.ISSUER}/oauth/authorize?{params}"  # 修改代码+DirectOAuthClientId：返回当前 issuer 的完整授权 URL；如果没有这行代码，实例级 issuer/client 配置无法生效。
