...
function Test-ScenarioAssertions {  # 新增代码+AcceptanceController: 统一检查事件、最终回答预览和调试日志断言；若没有这行代码，每个场景会重复写判断逻辑
    ...
    $Passed = $StatesPassed -and $EventAnswerPassed -and $EventPayloadPassed -and $DebugPassed -and $MarkerPassed -and $PermissionCountPassed  # 修改代码+Phase24AcceptancePayload: 汇总场景是否通过时纳入事件 payload 断言；若没有这行代码，real_extension_e2e=false 仍可能只靠事件出现通过。
    return [ordered]@{ passed = $Passed; marker_passed = $MarkerPassed; permission_count_passed = $PermissionCountPassed; max_permission_sent_count = $MaxPermissionSentCount; permission_sent_count = $PermissionSentCount; log_updated = $LogUpdated; state_checks = $StateChecks; event_answer_checks = $EventAnswerChecks; event_payload_checks = $EventPayloadChecks; debug_log_checks = $DebugChecks; final_answer_preview = $FinalAnswerPreview; final_answer_text_length = $FinalAnswerText.Length; event_payload_text_length = $EventPayloadText.Length }  # 修改代码+Phase24AcceptancePayload: 返回完整断言报告并包含 payload 检查；若没有这行代码，失败时无法定位 true/false 字段缺失。
