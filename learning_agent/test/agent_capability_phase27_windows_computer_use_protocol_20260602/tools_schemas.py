...
TOOL_SCHEMAS: list[dict[str, Any]] = [  # 作用: 告诉真实模型当前 agent 暴露了哪些工具
    ...
    {  # 新增代码+OSComputerUse: 新增 computer_action 工具定义开始；若没有这行代码，模型无法请求 OS 桌面动作。
        ...
        "function": {  # 新增代码+OSComputerUse: function 内部描述工具名称、说明和参数；若没有这行代码，Computer Use 动作工具缺少标准定义结构。
            ...
            "description": "执行 OS 级 Computer Use 动作。该工具高风险，必须传 confirm_desktop_control=true，并且仍会经过用户权限确认。",  # 新增代码+OSComputerUse: 告诉模型显式确认和权限要求；若没有这行代码，模型可能误以为动作可自动执行。
            "parameters": {  # 新增代码+OSComputerUse: JSON Schema 描述 computer_action 参数；若没有这行代码，模型不知道动作字段怎么传。
                ...
                "properties": {  # 新增代码+OSComputerUse: 声明允许的动作参数字段；若没有这行代码，模型无法构造合法调用。
                    ...
                    "x": {"type": "integer", "description": "可选屏幕 x 坐标，move_mouse/click/double_click 可使用。"},  # 新增代码+OSComputerUse: 提供横坐标字段；若没有这行代码，鼠标动作无法定位目标。
                    "y": {"type": "integer", "description": "可选屏幕 y 坐标，move_mouse/click/double_click 可使用。"},  # 新增代码+OSComputerUse: 提供纵坐标字段；若没有这行代码，鼠标动作无法定位目标。
