...
try:  # 新增代码+DesktopTaskRuntime：优先使用包路径导入既有 Computer Use 组件；如果没有这段代码，项目根运行时无法复用现有模块。
    ...
    from learning_agent.computer_use.windows_launch_resolver import resolve_generic_app_launch_target  # 修改代码+CompatSlimming：从统一 resolver 读取通用应用解析；如果没有这一行，旧 discovery 文件删除后 runtime 会断。
    from learning_agent.computer_use.generic_launch_backend import prepare_phase109_generic_real_launch_candidate  # 修改代码+CompatSlimming：从通用启动后端读取真实启动候选；如果没有这一行，旧 candidate 文件删除后窗口绑定证据会断。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.desktop_task_acceptance", "learning_agent.computer_use.desktop_task_router", "learning_agent.computer_use.windows_launch_resolver", "learning_agent.computer_use.generic_launch_backend", "learning_agent.computer_use.mode_session", "learning_agent.computer_use.representative_e2e_matrix"}:  # 修改代码+CompatSlimming：只在新核心模块包路径缺失时兜底；如果没有这一行，内部真实 bug 会被错误吞掉。
    ...
    from computer_use.windows_launch_resolver import resolve_generic_app_launch_target  # type: ignore  # 修改代码+CompatSlimming：脚本模式从统一 resolver 读取通用应用解析；如果没有这一行，bat 入口会丢失泛化应用能力。
    from computer_use.generic_launch_backend import prepare_phase109_generic_real_launch_candidate  # type: ignore  # 修改代码+CompatSlimming：脚本模式从通用启动后端读取候选；如果没有这一行，bat 入口无法构造自有窗口证据。
...
def build_default_desktop_task_runtime(workspace: str | Path) -> ComputerUseDesktopTaskRuntime:  # 新增代码+RuntimeFactorySlimming：函数段开始，集中构造默认桌面任务 runtime；如果没有这段函数，默认 runtime 工厂会继续挂在 LearningAgent 主类里制造误接风险。
    try:  # 新增代码+RuntimeFactorySlimming：优先用包路径导入真实 sender 和通用 adapter；如果没有这一行，项目根运行时无法构造当前默认 Computer Use 闭环。
        from learning_agent.computer_use.controlled_physical_sendinput import WindowsControlledPhysicalSendInputSender  # 新增代码+RuntimeFactorySlimming：导入受控物理 sender；如果没有这一行，默认 runtime 无法经过 Phase95 安全门发送鼠标键盘。
        from learning_agent.computer_use.real_sendinput_guard import WindowsSendInputLowLevelSender  # 新增代码+RuntimeFactorySlimming：导入真实 Windows SendInput 后端；如果没有这一行，受控 sender 背后没有能触达本机桌面的低层实现。
        from learning_agent.computer_use.universal_desktop_execution_loop import UniversalDesktopExecutionLoopAdapter  # 新增代码+RuntimeFactorySlimming：导入通用桌面执行 adapter；如果没有这一行，默认 runtime 会退回旧黑盒或空闭环。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.controlled_physical_sendinput", "learning_agent.computer_use.real_sendinput_guard", "learning_agent.computer_use.universal_desktop_execution_loop"}:  # 新增代码+RuntimeFactorySlimming：只对包路径缺失做兜底；如果没有这一行，真实内部 bug 会被误吞。
            raise  # 新增代码+RuntimeFactorySlimming：重新抛出非路径类导入错误；如果没有这一行，排查 sender 或 adapter 内部问题会很困难。
        from computer_use.controlled_physical_sendinput import WindowsControlledPhysicalSendInputSender  # type: ignore  # 新增代码+RuntimeFactorySlimming：脚本模式导入同一受控 sender；如果没有这一行，真实可见终端入口无法构造默认 sender。
        from computer_use.real_sendinput_guard import WindowsSendInputLowLevelSender  # type: ignore  # 新增代码+RuntimeFactorySlimming：脚本模式导入同一 Windows SendInput 后端；如果没有这一行，bat 入口没有物理输入后端。
        from computer_use.universal_desktop_execution_loop import UniversalDesktopExecutionLoopAdapter  # type: ignore  # 新增代码+RuntimeFactorySlimming：脚本模式导入同一通用 adapter；如果没有这一行，bat 入口无法接入通用 observe-plan-act loop。
    ...
    return ComputerUseDesktopTaskRuntime(base_dir=runtime_root, real_execution_loop=real_execution_loop)  # 新增代码+RuntimeFactorySlimming：返回已接好通用 adapter 的 runtime；如果没有这一行，调用方拿不到迁移后的默认 runtime。
