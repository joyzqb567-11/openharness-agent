...
try:  # 新增代码+URG4ObservePlanActVerify：优先按 learning_agent 包路径导入已完成的 URG-1/3 能力；如果没有这段代码，单测和真实 bat 入口无法共享实现。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.universal_action_dsl", "learning_agent.computer_use.universal_real_observation"}:  # 新增代码+URG4ObservePlanActVerify：只兜底包路径缺失；如果没有这行代码，真实内部 bug 可能被误吞。
