...
try:  # 新增代码+URG2UniversalTargetSession：优先按 learning_agent 包路径导入已有通用能力；如果没有这一段，单测和生产入口无法共享同一实现。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.generic_app_discovery", "learning_agent.computer_use.target_identity"}:  # 新增代码+URG2UniversalTargetSession：只兜底包路径缺失；如果没有这一行，真实内部 bug 会被误吞。
...
class UniversalTargetSessionRuntime:  # 新增代码+URG2UniversalTargetSession：类段开始，管理通用目标 session 和动作前身份复核；如果没有这个类，目标凭证会散落在发现、启动和动作层。
    def __init__(self, resolver: Any | None = None) -> None:  # 新增代码+URG2UniversalTargetSession：函数段开始，允许注入 resolver 但默认复用 Phase108；如果没有这段函数，测试无法替换目标发现来源。
