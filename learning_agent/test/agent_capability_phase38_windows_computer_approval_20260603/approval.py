...
DEFAULT_GRANT_FLAGS: dict[str, bool] = {"clipboardRead": False, "clipboardWrite": False, "systemKeyCombos": False}  # 新增代码+Phase38WindowsComputerApproval: 定义 ClaudeCode 风格 grant flags 默认关闭；如果没有这行代码，危险权限可能没有清晰默认值。
...
class WindowsComputerUseApprovalModel:  # 新增代码+Phase38WindowsComputerApproval: 类段开始，保存 Windows Computer Use 会话授权状态；如果没有这个类，审批逻辑只能写成零散函数无法被 controller 注入。
    def __init__(self, session_id: str = "learning-agent-default-session", granted_apps: dict[str, dict[str, Any]] | None = None, grant_flags: dict[str, bool] | None = None) -> None:  # 新增代码+Phase38WindowsComputerApproval: 函数段开始，初始化审批模型；如果没有这段函数，模型无法保存会话授权和 grant flags。
    ...
    # 新增代码+Phase38WindowsComputerApproval: 函数段结束，__init__ 到此结束；如果没有这个边界说明，读者不容易看出初始化范围。

    def status(self) -> dict[str, Any]:  # 新增代码+Phase38WindowsComputerApproval: 函数段开始，返回机器可读审批状态；如果没有这段函数，controller.status 无法展示 approval 边界。
        return {"enabled": True, "approval_model": PHASE38_APPROVAL_CONTRACT, "approval_granted_app_count": len(self.granted_apps), "grant_flags": dict(self.grant_flags), "actions_expanded": PHASE38_ACTIONS_EXPANDED, "marker": PHASE38_WINDOWS_COMPUTER_APPROVAL_MARKER}  # 新增代码+Phase38WindowsComputerApproval: 返回 allowlist 数量、flags 和 marker；如果没有这行代码，状态页缺少审批摘要。
    # 新增代码+Phase38WindowsComputerApproval: 函数段结束，status 到此结束；如果没有这个边界说明，读者不容易看出状态输出范围。

    def terminal_status_lines(self) -> list[str]:  # 新增代码+Phase38WindowsComputerApproval: 函数段开始，生成 `/computer status` 终端可见摘要；如果没有这段函数，用户只能通过 JSON 理解审批状态。
        ...
        return ["Computer Use Approval", f"- approval_model={PHASE38_APPROVAL_CONTRACT}", f"- approval_granted_app_count={len(self.granted_apps)}", f"- grant_flags={flag_text}", f"- actions_expanded={_bool_token(PHASE38_ACTIONS_EXPANDED)}"]  # 新增代码+Phase38WindowsComputerApproval: 返回多行终端摘要；如果没有这行代码，真实终端验收看不到 Phase38 状态。
    # 新增代码+Phase38WindowsComputerApproval: 函数段结束，terminal_status_lines 到此结束；如果没有这个边界说明，读者不容易看出终端摘要范围。
    ...
    def evaluate(self, action: str, arguments: dict[str, Any]) -> dict[str, Any]:  # 新增代码+Phase38WindowsComputerApproval: 函数段开始，评估一次桌面动作是否被本会话授权；如果没有这段函数，controller 无法在后端执行前拦截。
        ...
        return {"allowed": True, "decision": "allowed_by_session_grant", "grant_id": grant_record.get("grant_id", ""), "grant_flags": active_flags, "target_summary": summary, "approval_model": PHASE38_APPROVAL_CONTRACT, "actions_expanded": PHASE38_ACTIONS_EXPANDED}  # 新增代码+Phase38WindowsComputerApproval: 返回允许结果并关联 grant_id；如果没有这行代码，controller 无法知道动作已被会话授权。
