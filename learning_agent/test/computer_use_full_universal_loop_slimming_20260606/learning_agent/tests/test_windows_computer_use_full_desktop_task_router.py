...
class WindowsComputerUseFullDesktopTaskRouterTests(unittest.TestCase):  # 新增代码+DesktopTaskAcceptance：类段开始，集中冻结 `/computer use --full` 桌面任务路由验收；如果没有这个类，unittest 不会组织这些回归用例。
    ...
    def test_default_agent_runtime_does_not_inject_paint_specific_loop(self) -> None:  # 新增代码+UniversalLoopSlimming：函数段开始，验证默认生产 runtime 不再注入 Paint/Pikachu 专用闭环；如果没有这个测试，后续可能再次把通用入口误接到特例 loop。
        ...
        with tempfile.TemporaryDirectory() as raw_dir:  # 新增代码+UniversalLoopSlimming：创建一次性 workspace；如果没有这一行，测试可能污染用户真实 memory。
            ...
            loop = getattr(runtime, "real_execution_loop", None)  # 新增代码+UniversalLoopSlimming：读取运行时真实执行闭环字段；如果没有这一行，断言无法知道默认路径是否接了特例 loop。
            self.assertIsNone(loop)  # 新增代码+UniversalLoopSlimming：断言默认生产路径不再接任何临时真实执行器；如果没有这一行，Paint/Pikachu loop 可能继续冒充通用 Computer Use。
    ...
    # 新增代码+UniversalLoopSlimming：函数段结束，test_default_agent_runtime_does_not_inject_paint_specific_loop 到此结束；如果没有这个边界说明，代码小白不容易看出默认接线门禁范围。
