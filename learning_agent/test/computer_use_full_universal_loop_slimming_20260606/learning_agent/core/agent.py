...
class LearningAgent:  # 作用: 教学版 agent 主类
    ...
    def _desktop_task_runtime_for_current_run(self) -> Any:  # 新增代码+DesktopTaskRuntimeRoute：函数段开始，取得当前 run 应使用的桌面任务 runtime；如果没有这段函数，测试无法注入 full-mode runtime。
        ...
        try:  # 修改代码+UniversalLoopSlimming：优先按包路径只导入通用桌面任务 runtime；如果没有这一行，项目根运行时无法构造诚实的未接线默认 runtime。
            ...
            if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.desktop_task_runtime"}:  # 修改代码+UniversalLoopSlimming：脚本模式兜底名单只保留通用 runtime；如果没有这一行，Paint/Pikachu 专用模块会继续被默认生产路径依赖。
        ...
        runtime = ComputerUseDesktopTaskRuntime(base_dir=runtime_root)  # 修改代码+UniversalLoopSlimming：默认生产路径先不注入任何 Paint/Pikachu 特例真实闭环；如果没有这一行，用户的通用 Computer Use prompt 会继续被误接到固定绘图控制器。
