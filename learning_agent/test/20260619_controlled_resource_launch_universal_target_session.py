...
def _phase117_attach_controlled_resource_to_phase108_report(phase108_report: dict[str, Any], controlled_resource_path: str = "") -> dict[str, Any]:  # 新增代码+ControlledResourceLaunch：函数段开始，把受控文档路径并入 Phase108 启动计划；如果没有这段函数，Notepad 会继续裸启动并恢复旧文档。
    report = dict(phase108_report or {})  # 新增代码+ControlledResourceLaunch：复制发现报告避免污染 resolver 原始返回；如果没有这一行，调用方共享的报告可能被意外修改。
    resource_path = str(controlled_resource_path or "").strip()  # 新增代码+ControlledResourceLaunch：清理受控资源路径；如果没有这一行，空白路径可能进入 argv。
    if not resource_path:  # 新增代码+ControlledResourceLaunch：没有受控资源时保持原报告；如果没有这一行，普通应用启动会被误改。
        return report  # 新增代码+ControlledResourceLaunch：返回原始启动计划副本；如果没有这一行，普通启动没有返回值。
    plan = dict(report.get("launch_plan", {}) or {})  # 新增代码+ControlledResourceLaunch：复制安全启动计划；如果没有这一行，后续写 arguments 会污染嵌套原对象。
    launch_backend = str(plan.get("launch_backend") or "argv_no_shell")  # 新增代码+ControlledResourceLaunch：读取 resolver 选择的启动后端；如果没有这一行，无法确认文件路径能否安全传入 argv。
    report["controlled_resource_path"] = resource_path  # 新增代码+ControlledResourceLaunch：把受控资源路径写到报告顶层；如果没有这一行，controller 和验收看不到目标文件。
    if launch_backend != "argv_no_shell":  # 新增代码+ControlledResourceLaunch：只有 argv 后端才能安全附加文件参数；如果没有这一行，AppX/shortcut 可能收到无效参数。
        report["resource_bound_launch"] = False  # 新增代码+ControlledResourceLaunch：声明本次不能执行资源绑定启动；如果没有这一行，失败原因会不清楚。
        report["resource_bound_launch_refusal_reason"] = "controlled_resource_requires_argv_launch_backend"  # 新增代码+ControlledResourceLaunch：记录拒绝原因；如果没有这一行，调试时不知道为什么路径没进 argv。
        return report  # 新增代码+ControlledResourceLaunch：返回带拒绝原因的报告；如果没有这一行，非 argv 应用会继续走错误路径。
    plan["arguments"] = [resource_path]  # 新增代码+ControlledResourceLaunch：把目标文件作为无 shell argv 参数传给应用；如果没有这一行，Notepad 仍会裸启动。
    plan["controlled_resource_path"] = resource_path  # 新增代码+ControlledResourceLaunch：在 launch_plan 内保存目标文件；如果没有这一行，Phase110 请求报告缺少资源证据。
    plan["resource_bound_launch"] = True  # 新增代码+ControlledResourceLaunch：标记启动计划已绑定资源；如果没有这一行，验收无法区分裸启动和文件启动。
    report["launch_plan"] = plan  # 新增代码+ControlledResourceLaunch：把更新后的计划写回报告；如果没有这一行，后端仍会读取旧空 arguments。
    report["resource_bound_launch"] = True  # 新增代码+ControlledResourceLaunch：在顶层声明资源绑定成功；如果没有这一行，controller 审计无法快速读取状态。
    return report  # 新增代码+ControlledResourceLaunch：返回带受控资源 argv 的 Phase108 报告；如果没有这一行，runtime 拿不到修正后的计划。
# 新增代码+ControlledResourceLaunch：函数段结束，_phase117_attach_controlled_resource_to_phase108_report 到此结束；如果没有这个边界说明，读者不容易看出资源路径注入范围。
...
class UniversalTargetSessionRuntime:  # 新增代码+URG2UniversalTargetSession：类段开始，管理通用目标 session 和动作前身份复核；如果没有这个类，目标凭证会散落在发现、启动和动作层。
    ...
    def open_target_session(self, raw_target: Any, candidates: list[dict[str, Any]] | None = None, user_authorized_window: bool = False, controlled_resource_path: str = "") -> dict[str, Any]:  # 修改代码+ControlledResourceLaunch：函数段开始，为任意普通目标建立通用 session 并可绑定文档资源；如果没有 controlled_resource_path，Notepad 无法从源头避开旧标签页恢复。
        ...
        phase108_report = _phase117_attach_controlled_resource_to_phase108_report(phase108_report, controlled_resource_path)  # 新增代码+ControlledResourceLaunch：把受控文档路径并入启动计划；如果没有这一行，Phase110 后端仍收到空 arguments。
        ...
        if self.enable_real_launch:  # 新增代码+RealLaunchTargetSession：真实启动模式走 Phase110 后端和窗口 probe；如果没有这一行，full 模式仍会生成假 pid/hwnd。
            return self._open_real_target_session(raw_target, phase108_report, session_id, executable, user_authorized_window, controlled_resource_path)  # 修改代码+ControlledResourceLaunch：返回真实启动 session 报告并携带受控资源；如果没有这一行，真实路径的报告会丢失目标文件。
    ...
    def _open_real_target_session(self, raw_target: Any, phase108_report: dict[str, Any], session_id: str, executable: str, user_authorized_window: bool = False, controlled_resource_path: str = "") -> dict[str, Any]:  # 修改代码+ControlledResourceLaunch：函数段开始，真实启动应用并可绑定受控文档资源；如果没有 controlled_resource_path 报告字段，验收看不出是否仍是裸启动。
