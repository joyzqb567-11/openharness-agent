...
def _proxy_window(launcher_pid: int = 111, window_pid: int = 222, hwnd: int = 333) -> dict:  # 新增代码+ProxyPidLeaseTest：函数段开始，模拟 Windows 启动器 pid 与真实窗口 pid 不一致的代理窗口；如果没有这段函数，Notepad 这类真实场景只能靠人工截图排查。
    return {  # 新增代码+ProxyPidLeaseTest：返回代理窗口事实；如果没有这一行，租约测试没有可复现的 pid 不一致样本。
        "app_id": "notepad.exe",  # 新增代码+ProxyPidLeaseTest：保存目标应用身份；如果没有这一行，测试无法证明修复是通用窗口绑定而不是空数据放行。
        "process_name": "notepad.exe",  # 新增代码+ProxyPidLeaseTest：保存真实窗口进程名；如果没有这一行，动作前验证缺少进程侧证据。
        "pid": launcher_pid,  # 新增代码+ProxyPidLeaseTest：保留启动器 pid 作为历史兼容字段；如果没有这一行，租约构建无法复现当前线上报告形状。
        "window_process_id": launcher_pid,  # 新增代码+ProxyPidLeaseTest：保留被代理后的窗口 pid 字段；如果没有这一行，旧身份模型不会认为首次租约可验证。
        "actual_window_process_id": window_pid,  # 新增代码+ProxyPidLeaseTest：保存真实 Notepad 窗口 pid；如果没有这一行，动作前无法把窗口绑定到真正可见窗口。
        "actual_process_name": "notepad.exe",  # 新增代码+ProxyPidLeaseTest：保存真实窗口进程名；如果没有这一行，报告看不出代理窗口对应哪个程序。
        "hwnd": hwnd,  # 新增代码+ProxyPidLeaseTest：保存真实窗口句柄；如果没有这一行，pid 不一致时无法靠 hwnd 做一对一绑定。
        "window_id": f"hwnd:{hwnd}",  # 新增代码+ProxyPidLeaseTest：保存协议窗口 id；如果没有这一行，target_ref 无法稳定回指窗口。
        "title_preview": "Untitled - Notepad",  # 新增代码+ProxyPidLeaseTest：保存窗口标题摘要；如果没有这一行，失败报告缺少用户可读证据。
        "agent_owned_proxy_window": True,  # 新增代码+ProxyPidLeaseTest：声明这是 agent 自启动后的代理窗口；如果没有这一行，普通 pid 漂移可能被误放行。
        "proxy_window_binding": {  # 新增代码+ProxyPidLeaseTest：保存代理绑定证据；如果没有这一行，验证函数无法区分可信代理和错窗漂移。
            "proxy_window_bound": True,  # 新增代码+ProxyPidLeaseTest：声明代理绑定成立；如果没有这一行，测试不能证明必须有绑定证据才放行。
            "launcher_pid": launcher_pid,  # 新增代码+ProxyPidLeaseTest：记录启动器 pid；如果没有这一行，启动事实和窗口事实无法关联。
            "window_pid": window_pid,  # 新增代码+ProxyPidLeaseTest：记录真实窗口 pid；如果没有这一行，动作前比较没有目标 pid。
            "window_hwnd": hwnd,  # 新增代码+ProxyPidLeaseTest：记录真实窗口 hwnd；如果没有这一行，同进程多窗口可能混淆。
        },  # 新增代码+ProxyPidLeaseTest：结束代理绑定证据；如果没有这一行，Python 字典语法不完整。
        "rect": {"left": 0, "top": 0, "right": 800, "bottom": 600},  # 新增代码+ProxyPidLeaseTest：保存窗口矩形；如果没有这一行，后续真实动作坐标缺少边界。
    }  # 新增代码+ProxyPidLeaseTest：结束代理窗口字典；如果没有这一行，Python 语法不完整。
# 新增代码+ProxyPidLeaseTest：函数段结束，_proxy_window 到此结束；如果没有这个边界说明，用户不容易看出代理 pid 夹具范围。
...
def test_agent_owned_proxy_window_allows_real_window_pid_after_nested_session_report() -> None:  # 新增代码+ProxyPidLeaseTest：函数段开始，验证嵌套 session 报告和真实窗口 pid 不一致时仍能绑定同一窗口；如果没有这段测试，Notepad 会在打开后第一下输入前被误判漂移。
    session_report = {  # 新增代码+ProxyPidLeaseTest：模拟 universal_target_session 返回给 controller 的嵌套报告；如果没有这一行，测试无法复现真实日志里 process_id 藏在 launch_result 内的问题。
        "ok": True,  # 新增代码+ProxyPidLeaseTest：声明 session 打开成功；如果没有这一行，租约构建缺少成功状态。
        "launch_result": {**_launch_result(pid=111), "process_executable": "notepad.exe"},  # 修改代码+ProxyPidLeaseTest：把真实目标进程事实放在嵌套字段里；如果没有这一行，测试会因为夹具进程名不匹配而偏离真实 bug。
        "proxy_window_bound": True,  # 新增代码+ProxyPidLeaseTest：声明代理窗口绑定已完成；如果没有这一行，后续放行证据不完整。
        "fresh_target_decision": "fresh_target_ready",  # 新增代码+ProxyPidLeaseTest：声明 FreshTarget 已通过；如果没有这一行，租约报告无法表达新窗口来源。
        "fresh_target_class": "fresh_agent_owned_window",  # 新增代码+ProxyPidLeaseTest：声明目标是 agent 自启动窗口；如果没有这一行，测试不能覆盖自有窗口路径。
        "fresh_target_identity_verified": True,  # 新增代码+ProxyPidLeaseTest：声明新鲜度身份已验证；如果没有这一行，租约可能缺少启动后策略证据。
    }  # 新增代码+ProxyPidLeaseTest：结束嵌套 session 报告；如果没有这一行，Python 字典语法不完整。
    lease = build_target_lease(  # 新增代码+ProxyPidLeaseTest：用真实报告形状构建租约；如果没有这一行，不能证明入口函数能吃真实 open_application 结果。
        session_id="session-proxy",  # 新增代码+ProxyPidLeaseTest：绑定测试会话；如果没有这一行，租约缺少归属。
        target_ref="cu-target-proxy-0001",  # 新增代码+ProxyPidLeaseTest：绑定一对一窗口 ID；如果没有这一行，动作前无法关联目标。
        origin="agent_owned_launch",  # 新增代码+ProxyPidLeaseTest：声明来源是 agent 自启动；如果没有这一行，验证会走错授权分支。
        launch_result=session_report,  # 新增代码+ProxyPidLeaseTest：传入嵌套 session 报告；如果没有这一行，回归测试不会覆盖真实 bug。
        target_window=_proxy_window(launcher_pid=111, window_pid=222, hwnd=333),  # 新增代码+ProxyPidLeaseTest：传入代理窗口事实；如果没有这一行，pid 不一致场景不存在。
        user_granted_existing_window=False,  # 新增代码+ProxyPidLeaseTest：声明不是用户授权旧窗口；如果没有这一行，测试无法证明无需旧窗口授权也可控新窗口。
    )  # 新增代码+ProxyPidLeaseTest：结束租约构建；如果没有这一行，Python 语法不完整。
    result = verify_target_lease_before_action(  # 新增代码+ProxyPidLeaseTest：执行写动作前验证；如果没有这一行，不能证明第一下输入不会被误拒。
        lease=lease,  # 新增代码+ProxyPidLeaseTest：传入代理租约；如果没有这一行，验证函数没有期望目标。
        current_window={**_window(pid=222, hwnd=333, title="Untitled - Notepad"), "process_name": "notepad.exe", "app_id": "notepad.exe"},  # 新增代码+ProxyPidLeaseTest：传入真实可见窗口 pid；如果没有这一行，测试无法模拟启动器 pid 与真实窗口 pid 不同。
        action="type_text",  # 新增代码+ProxyPidLeaseTest：选择真实输入类动作；如果没有这一行，测试可能绕过租约写动作门禁。
    )  # 新增代码+ProxyPidLeaseTest：结束动作前验证；如果没有这一行，Python 语法不完整。
    assert lease.lease_identity_verified is True  # 新增代码+ProxyPidLeaseTest：确认嵌套启动报告也能建立可信租约；如果没有这一行，真实 open_application 成功后仍会被判 target_lease_not_verified。
    assert result.allowed is True  # 新增代码+ProxyPidLeaseTest：确认真实窗口 pid 不同但 hwnd 绑定一致时允许动作；如果没有这一行，修复可能仍停在打开应用后不输入。
    assert result.decision == "target_lease_verified"  # 新增代码+ProxyPidLeaseTest：确认成功决策 token 稳定；如果没有这一行，controller 无法进入下一步动作。
    assert result.low_level_event_count == 0  # 新增代码+ProxyPidLeaseTest：确认验证本身零低层事件；如果没有这一行，门禁可能自己触碰桌面。
# 新增代码+ProxyPidLeaseTest：函数段结束，test_agent_owned_proxy_window_allows_real_window_pid_after_nested_session_report 到此结束；如果没有这个边界说明，用户不容易看出代理 pid 验证范围。
