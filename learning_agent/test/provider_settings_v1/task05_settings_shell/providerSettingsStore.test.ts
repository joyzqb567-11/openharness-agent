...
const providerPayload: PollutedProviderSettingsPayload = { // 修改代码+ProviderSettingsStoreTest：定义后端 provider settings fixture；如果没有这段，多个测试会重复构造 payload。
  ...
  schema_version: 2, // 修改代码+ProviderSettingsStoreTest：模拟 V2 schema；如果没有这行，版本字段缺失不会被发现。
  ...
  providers: [ // 修改代码+ProviderSettingsStoreTest：开始 provider 列表；如果没有这行，排序和分组没有输入。
    { id: "openrouter", display_name: "OpenRouter", kind: "built_in", source: "none", connected: false, masked_key: "", auth_methods: [{ id: "api-key", label: "API 密钥", enabled: true, status: "available", fields: ["secret"], help_text: "help" }], description: "OpenRouter", models: [] }, // 修改代码+ProviderSettingsStoreTest：模拟未连接 provider；如果没有这行，排序无法比较连接状态。
    { id: "openai", display_name: "OpenAI", kind: "built_in", source: "config", connected: true, masked_key: "sk••••xx", auth_methods: [{ id: "api-key", label: "API 密钥", enabled: true, status: "available", fields: ["secret"], help_text: "help" }], description: "OpenAI", api_key: "unit-test-secret-value", models: [{ id: "gpt-4.1", display_name: "GPT-4.1", provider_id: "openai", visible: true, supports_tools: true, supports_vision: true }] }, // 修改代码+ProviderSettingsStoreTest：模拟已连接 provider 且夹带危险字段；如果没有这行，脱敏测试不真实。
    { id: "github-copilot", display_name: "GitHub Copilot", kind: "built_in", source: "none", connected: false, masked_key: "", auth_methods: [{ id: "device-code", label: "设备码", enabled: false, status: "unsupported_v1", fields: [], help_text: "unsupported" }], description: "Copilot", models: undefined }, // 修改代码+ProviderSettingsStoreTest：模拟 unsupported auth 和缺 models；如果没有这行，禁用按钮和空数组兼容无法测试。
