...
class GuiBridgeSecurityContractTests(unittest.TestCase):  # 新增代码+DesktopGUISecurityTest: 测试类段开始，锁定 GUI bridge 安全合同；如果没有这个类，安全门禁不会被 unittest 执行。
    ...
    def test_alternate_loopback_vite_origin_can_load_provider_settings(self) -> None:  # 新增代码+ProviderSettingsVisualQA：测试方法开始，锁定备用 Vite 端口的 Provider 设置 CORS 合同；如果没有这段测试，可见 GUI 验收换端口时会再次加载失败。
        with tempfile.TemporaryDirectory() as directory:  # 新增代码+ProviderSettingsVisualQA：创建临时工作区；如果没有这行代码，provider 设置测试会污染真实 memory。
            server = self._start_server(Path(directory))  # 新增代码+ProviderSettingsVisualQA：启动测试 bridge；如果没有这行代码，预检和 provider catalog 请求没有真实目标。
            try:  # 新增代码+ProviderSettingsVisualQA：确保服务最终关闭；如果没有这行代码，失败时后台线程可能残留。
                origin = "http://127.0.0.1:5277"  # 新增代码+ProviderSettingsVisualQA：模拟这次视觉验收使用的备用 Vite 端口；如果没有这行代码，测试无法复现真实失败来源。
                options_request = urllib.request.Request(self._url(server, "/v2/gui/provider-settings/providers"), method="OPTIONS", headers={"Origin": origin, "Access-Control-Request-Method": "GET", "Access-Control-Request-Headers": "x-openharness-desktop-token"})  # 新增代码+ProviderSettingsVisualQA：构造浏览器读取 Provider catalog 前的预检；如果没有这行代码，真实浏览器失败路径不会被测试覆盖。
                with urllib.request.urlopen(options_request, timeout=5) as options_response:  # 新增代码+ProviderSettingsVisualQA：执行预检请求；如果没有这行代码，测试无法确认浏览器会允许后续 GET。
                    self.assertEqual(options_response.status, 204)  # 新增代码+ProviderSettingsVisualQA：确认预检成功；如果没有这行断言，403 回归可能被忽略。
                    self.assertEqual(options_response.getheader("Access-Control-Allow-Origin"), origin)  # 新增代码+ProviderSettingsVisualQA：确认 bridge 精确回写当前本机来源；如果没有这行断言，浏览器仍可能拦截响应。
                catalog_request = urllib.request.Request(self._url(server, "/v2/gui/provider-settings/providers"), headers={"X-OpenHarness-Desktop-Token": "test-token", "Origin": origin})  # 新增代码+ProviderSettingsVisualQA：构造带 token 的 Provider catalog 请求；如果没有这行代码，无法验证业务响应也能被前端读取。
                with urllib.request.urlopen(catalog_request, timeout=5) as catalog_response:  # 新增代码+ProviderSettingsVisualQA：执行 Provider catalog GET；如果没有这行代码，测试只覆盖预检不覆盖真实读取。
                    payload = json.loads(catalog_response.read().decode("utf-8"))  # 新增代码+ProviderSettingsVisualQA：解析 provider catalog JSON；如果没有这行代码，无法确认返回的是业务数据。
                    self.assertEqual(catalog_response.getheader("Access-Control-Allow-Origin"), origin)  # 新增代码+ProviderSettingsVisualQA：确认业务响应携带允许来源；如果没有这行断言，fetch 会拿不到响应体。
                self.assertGreaterEqual(len(payload["providers"]), 5)  # 新增代码+ProviderSettingsVisualQA：确认 Provider 列表真实返回；如果没有这行断言，空 payload 也可能误过。
            finally:  # 新增代码+ProviderSettingsVisualQA：清理 server；如果没有这行代码，后台线程可能残留。
                server.shutdown()  # 新增代码+ProviderSettingsVisualQA：停止 HTTP server；如果没有这行代码，测试进程可能等待。
                server.server_close()  # 新增代码+ProviderSettingsVisualQA：释放 socket；如果没有这行代码，Windows 端口可能残留占用。
    # 新增代码+ProviderSettingsVisualQA：测试方法结束；如果没有这个边界说明，用户不容易看出本测试专门覆盖可视化验收端口回归。
