...
try:  # 新增代码+Phase108GenericAppDiscovery：优先按标准包路径导入 Phase69 启动计划；如果没有这行代码，项目根运行时无法复用已有安全计划。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.app_window_control"}:  # 新增代码+Phase108GenericAppDiscovery：只兜底包路径缺失；如果没有这行代码，Phase69 内部错误会被误吞。
...
def _phase108_candidate_from_mapping(candidate: dict[str, Any], default_source: str) -> dict[str, Any]:  # 新增代码+Phase108GenericAppDiscovery：函数段开始，把外部候选规范成统一字段；如果没有这段函数，测试注入、开始菜单扫描和 fallback 形状会不一致。
    ...
    executable_source = candidate.get("executable") or candidate.get("target") or candidate.get("path") or display_name  # 新增代码+Phase108GenericAppDiscovery：选择可执行名来源；如果没有这行代码，只有 display_name 的候选无法生成计划。
...
def discover_generic_app_candidates(query: Any, candidates: list[dict[str, Any]] | None = None) -> list[dict[str, Any]]:  # 新增代码+Phase108GenericAppDiscovery：函数段开始，发现与查询匹配的普通应用候选；如果没有这段函数，Phase108 只能硬编码单个目标。
    ...
    if candidates is not None:  # 新增代码+Phase108GenericAppDiscovery：优先使用测试或上层注入的候选源；如果没有这行代码，单元测试无法稳定模拟不同本机应用。
        normalized_candidates = [_phase108_candidate_from_mapping(candidate, "injected") for candidate in candidates]  # 新增代码+Phase108GenericAppDiscovery：规范注入候选列表；如果没有这行代码，候选字段形状不统一。
    ...
    discovered = _phase108_discover_start_menu_candidates(normalized_query)  # 新增代码+Phase108GenericAppDiscovery：从本机开始菜单做只读发现；如果没有这行代码，真实命令不会尝试发现已安装应用。
