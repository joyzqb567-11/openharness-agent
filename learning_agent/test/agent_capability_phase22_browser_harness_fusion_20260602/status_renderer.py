...
def render_status_snapshot(snapshot: dict[str, Any]) -> str:  # 新增代码+StatusRenderer: 渲染统一状态快照；若没有这行代码，终端/工具/CLI 会各写一套文本。
    ...
    latest_browser_harness = _as_dict(latest_browser_run.get("harness", {}))  # 新增代码+Phase22BrowserHarnessFusion: 读取最近 browser run 的 harness 摘要；如果没有这行代码，/status 无法展示浏览器任务对应的长任务证据。
    latest_browser_verifier = _as_dict(latest_browser_harness.get("latest_verifier", {}))  # 新增代码+Phase22BrowserHarnessFusion: 读取最近 verifier 摘要；如果没有这行代码，/status 只能显示 run id 不能显示验收是否通过。
    latest_action_evidence = _as_dict(latest_browser_harness.get("latest_action_evidence", {}))  # 新增代码+Phase22BrowserHarnessFusion: 读取最近 action 证据；如果没有这行代码，/status 看不到哪步浏览器动作已进入 harness。
    if latest_browser_harness:  # 新增代码+Phase22BrowserHarnessFusion: 只有存在 harness 投影时才显示链接；如果没有这行代码，空状态会显示误导性的空 harness 字段。
        lines.append(f"- harness_run_id={latest_browser_harness.get('run_id', '')} harness_verifier_passed={str(bool(latest_browser_verifier.get('passed', False))).lower()} harness_action_evidence_count={latest_browser_harness.get('action_evidence_count', 0)} harness_latest_action={_short_text(latest_action_evidence.get('tool_name', ''), 80)}")  # 新增代码+Phase22BrowserHarnessFusion: 展示 browser 到 harness 的验收链接；如果没有这行代码，用户仍要手工跨目录查 verifier 和 action 证据。
