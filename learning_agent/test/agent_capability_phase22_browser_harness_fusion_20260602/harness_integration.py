...
class BrowserHarnessMirror:  # 新增代码+BrowserHarnessStage11: 把 browser runtime 生命周期镜像成 harness 生命周期；若没有这个类，接线会散落在 server 各处。
    ...
    def append_action_evidence(self, run_id: str, action: Any) -> None:  # 新增代码+Phase22BrowserHarnessFusion: 把单个浏览器动作镜像成 verifier 友好的 harness 事件；如果没有这段函数，浏览器 action 仍只存在 browser runtime 旁路里。
        if not run_id:  # 新增代码+Phase22BrowserHarnessFusion: 没有 run id 时跳过同步；如果没有这行代码，空 run 会污染 harness events 目录。
            return  # 新增代码+Phase22BrowserHarnessFusion: 直接返回表示没有可同步目标；如果没有这行代码，后续 read_events 会读取空路径。
        action_payload = action.to_dict() if hasattr(action, "to_dict") else dict(action or {})  # 新增代码+Phase22BrowserHarnessFusion: 兼容 BrowserAction 对象和 dict；如果没有这行代码，生产回调和测试对象不能共用入口。
        action_id = str(action_payload.get("action_id", "") or "")  # 新增代码+Phase22BrowserHarnessFusion: 读取 action id 用于去重；如果没有这行代码，重复 resume 同步无法识别同一动作。
        if not action_id:  # 新增代码+Phase22BrowserHarnessFusion: 缺 action id 的证据不写入；如果没有这行代码，去重语义会变得不可靠。
            return  # 新增代码+Phase22BrowserHarnessFusion: 跳过坏 action 证据；如果没有这行代码，状态页可能显示无法追踪的空 action。
        existing = self.store.read_events(run_id)  # 新增代码+Phase22BrowserHarnessFusion: 读取已有 harness 事件用于去重；如果没有这行代码，同一已完成 action 会在恢复时重复写入。
        if any(event.get("event_type") == "browser_action_evidence" and event.get("payload", {}).get("action_id") == action_id for event in existing):  # 新增代码+Phase22BrowserHarnessFusion: 检查同一 action 是否已经同步；如果没有这行代码，resume/retry 会重复展示已完成动作。
            return  # 新增代码+Phase22BrowserHarnessFusion: 已同步则直接返回；如果没有这行代码，action evidence count 会被重复计算。
        payload = {"action_id": action_id, "stage_id": str(action_payload.get("stage_id", "") or ""), "tool_name": str(action_payload.get("tool_name", "") or ""), "status": str(action_payload.get("status", "") or ""), "observation_id": str(action_payload.get("observation_id", "") or ""), "verifier_friendly": True}  # 新增代码+Phase22BrowserHarnessFusion: 构造短证据 payload；如果没有这行代码，harness 事件会包含过多参数或缺少 verifier 可读字段。
        self.store.append_event(run_id, "browser_action_evidence", payload)  # 新增代码+Phase22BrowserHarnessFusion: 写入 action 级 harness 证据事件；如果没有这行代码，状态快照无法暴露浏览器动作证据链。
