...
class BrowserAutomationServer:  # 新增代码+BrowserAutomation服务: 定义浏览器自动化 server 状态类；若省略: tools/call 无法复用浏览器状态
    def __init__(self, workspace: Path) -> None:  # 新增代码+BrowserAutomation服务: 初始化 server 工作区和运行状态；若省略: 后续工具没有共享状态
        ...
        self.chrome_native_host_installer = ChromeNativeHostInstaller(self.workspace / "learning_agent" / "memory" / "chrome_native_host")  # 新增代码+Phase1Installer: 创建 native host 安装器；如果没有这行代码，browser_extension_install/status 无法管理安装生命周期。
