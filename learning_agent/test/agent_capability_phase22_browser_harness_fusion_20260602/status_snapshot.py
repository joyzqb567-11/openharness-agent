...
def _browser_harness_snapshot(store: HarnessStore, run_id: str) -> dict[str, Any]:  # 新增代码+BrowserHarnessStage11: 读取 browser run 对应的 harness 摘要；若没有这行代码，browser.runs 不能直接展示 verifier。
    ...
    action_evidence_events = [event for event in events if str(event.get("event_type", "")) == "browser_action_evidence"]  # 新增代码+Phase22BrowserHarnessFusion: 过滤浏览器动作证据事件；如果没有这行代码，状态快照无法显示 action 级证据数量。
    ...
    latest_action_evidence = action_evidence_events[-1].get("payload", {}) if action_evidence_events else {}  # 新增代码+Phase22BrowserHarnessFusion: 取最近浏览器动作证据；如果没有这行代码，/status 和 /chrome 无法显示最近已验收动作。
    ...
    payload["action_evidence_count"] = len(action_evidence_events)  # 新增代码+Phase22BrowserHarnessFusion: 附加 action 证据数量；如果没有这行代码，状态页无法判断浏览器动作证据链是否存在。
    payload["latest_action_evidence"] = latest_action_evidence if isinstance(latest_action_evidence, dict) else {}  # 新增代码+Phase22BrowserHarnessFusion: 附加最近 action 证据摘要；如果没有这行代码，用户不知道最近验收的是哪步浏览器动作。
...
def _chrome_extension_bridge_path(root: Path) -> Path:  # 新增代码+ChromeExtensionStage8: 按 workspace 形态定位插件 bridge 状态文件；若没有这行代码，CLI/API/SDK 会读错插件状态。
    ...
    if root.name.lower() == "learning_agent":  # 新增代码+Phase13ChromePairingGuide: 识别真实 bat 入口传入的 learning_agent 工作区；如果没有这行代码，bridge 路径会变成 learning_agent/learning_agent。
        return direct_path  # 新增代码+Phase13ChromePairingGuide: 包目录工作区直接读取 memory/chrome_extension_bridge.json；如果没有这行代码，真实配对状态会被漏读。
...
def _load_browser_provider_status(root: Path, browser_runtime: dict[str, Any]) -> dict[str, Any]:  # 新增代码+ChromeExtensionStage8: 聚合 provider、插件、native host、tab、权限和最近证据；若没有这行代码，Stage 8 状态生态仍是旁路。
    ...
    pairing = bridge_state.get("pairing", {}) if isinstance(bridge_state.get("pairing", {}), dict) else {}  # 新增代码+Phase13ChromePairingGuide: 读取脱敏 pairing 摘要；如果没有这行代码，/chrome 看不到 device/session。
    allowed_origins = pairing.get("allowed_origins", []) if isinstance(pairing.get("allowed_origins", []), list) else []  # 新增代码+Phase13ChromePairingGuide: 读取允许 origin 列表；如果没有这行代码，配对边界无法显示。
    paired = bool(pairing.get("device_id") or pairing.get("session_id") or pairing.get("extension_id"))  # 新增代码+Phase13ChromePairingGuide: 判断是否完成配对；如果没有这行代码，用户只能靠 connected 猜测。
