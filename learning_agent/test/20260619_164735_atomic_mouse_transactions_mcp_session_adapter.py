...
SPLIT_MOUSE_TRANSACTION_TOOL_NAMES = {"left_mouse_down", "left_mouse_up"}  # 新增代码+AtomicMouseTransaction：集中保存禁止 agent-side 直接执行的左键半事务工具名；如果没有这一行，旧桥接路径可能绕过 tools/list 重新触碰真实桌面。
...
def _reject_split_mouse_transaction(tool_name: str) -> dict[str, Any] | None:  # 新增代码+AtomicMouseTransaction：函数段开始，拒绝左键按下/释放这类不能跨轮次保证完整性的半事务；如果没有这段函数，旧模型记住工具名后仍可能绕过原子拖拽。
    normalized_name = str(tool_name or "").strip()  # 新增代码+AtomicMouseTransaction：清理工具名用于集合判断；如果没有这一行，带空格的旧工具名可能绕过拒绝。
    if normalized_name not in SPLIT_MOUSE_TRANSACTION_TOOL_NAMES:  # 新增代码+AtomicMouseTransaction：只拦截明确的左键半事务工具；如果没有这一行，普通 click/type/key 会被误拒绝。
        return None  # 新增代码+AtomicMouseTransaction：非半事务工具继续原有路径；如果没有这一行，调用方无法区分允许和拒绝。
    return _json_result(normalized_name, False, {"reason": "左键拖拽必须使用 left_click_drag 这个单次原子事务工具，不能拆成 left_mouse_down/left_mouse_up。", "replacement_tool": "left_click_drag", "desktop_action_performed": False, "low_level_event_count": 0}, error_class="desktop_atomic_drag_required")  # 新增代码+AtomicMouseTransaction：返回零事件拒绝并给出替代工具；如果没有这一行，adapter 会继续进入 controller 并可能造成半拖拽状态。
# 新增代码+AtomicMouseTransaction：函数段结束，_reject_split_mouse_transaction 到此结束；如果没有这个边界说明，用户不容易看出鼠标事务硬拒绝范围。
...
class ComputerUseMcpSessionAdapter:  # 新增代码+McpSessionAdapter: 函数段开始，执行 MCP 原子工具并复用 agent 旧能力；如果没有这个类，stdio MCP server 无法拿到 agent 回调。
    ...
    def call_atomic_tool(self, tool_name: str, arguments: dict[str, Any] | None = None) -> dict[str, Any]:  # 修改代码+ClaudeCodeParity: 函数段开始，公开执行单个 MCP 原子工具并让新增 parity 工具进入 action 映射；如果没有这段函数，agent_adapter 无法调用这层绑定。
        ...
        split_mouse_rejection = _reject_split_mouse_transaction(normalized_tool_name)  # 新增代码+AtomicMouseTransaction：在所有写动作前拒绝左键半事务工具；如果没有这一行，旧桥接方法仍可能调用 controller。
        if split_mouse_rejection is not None:  # 新增代码+AtomicMouseTransaction：判断是否命中左键半事务拒绝；如果没有这一行，拒绝结果不会生效。
            return split_mouse_rejection  # 新增代码+AtomicMouseTransaction：返回零事件拒绝且不触碰 controller；如果没有这一行，拆分拖拽仍可能在真实桌面执行。
