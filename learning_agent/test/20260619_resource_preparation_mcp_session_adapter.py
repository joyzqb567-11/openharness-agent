...
def _is_safe_new_blank_resource_preparation_action(legacy_arguments: dict[str, Any], arguments: dict[str, Any], action_window: dict[str, Any], resource_freshness: dict[str, Any]) -> bool:  # 新增代码+ResourcePreparation：函数段开始，判断当前旧资源阻断下是否允许一个受控新建空白动作；如果没有这段函数，Ctrl+N 会继续被硬挡或被过度放开。
    ...
    if not _arguments_request_new_blank_resource(arguments, legacy_arguments):  # 新增代码+ResourcePreparation：必须有明确新建空白意图；如果没有这一行，模型误按 Ctrl+N 也会被放行。
        return False  # 新增代码+ResourcePreparation：意图不明确时拒绝；如果没有这一行，安全策略会过宽。
    ...
    return _window_has_agent_owned_launch_context(action_window)  # 新增代码+ResourcePreparation：只允许 agent 新启动/绑定的窗口执行准备动作；如果没有这一行，用户已有旧窗口可能被误操作。
