...
class ObservabilityAcceptanceTests(LearningAgentTestBase):  # Stage14: unittest discovers this concrete modular test class.
    ...
    def test_wechat_full_acceptance_uses_structured_computer_action_event(self) -> None:  # 新增代码+StructuredComputerActionAcceptance: 函数段开始，验证微信真实启动场景不再靠开发调试日志验收；若没有这段测试，产品关闭调试日志后场景会失明。本段到最后一个断言结束。
        scenario_path = (TEST_ROOT / "acceptance_controller" / "scenarios" / "computer_use_full_open_wechat_probe.json")  # 新增代码+StructuredComputerActionAcceptance: 定位微信 full 模式真实验收场景；若没有这行代码，测试无法读取当前用户关心的场景。
        scenario = json.loads(scenario_path.read_text(encoding="utf-8"))  # 新增代码+StructuredComputerActionAcceptance: 读取 UTF-8 场景 JSON；若没有这行代码，中文微信断言和结构字段无法被检查。
        debug_log_text = "\n".join(scenario.get("debug_log_contains", []))  # 新增代码+StructuredComputerActionAcceptance: 合并开发调试日志断言；若没有这行代码，测试无法确认旧依赖是否已经清空。
        event_payload_text = "\n".join(scenario.get("event_payload_contains", []))  # 新增代码+StructuredComputerActionAcceptance: 合并正式事件断言；若没有这行代码，测试无法确认真实证据迁到了 JSONL 事件。
        self.assertEqual(debug_log_text, "")  # 新增代码+StructuredComputerActionAcceptance: 要求微信场景不再依赖 latest_run_readable.md；若没有这行代码，调试日志关闭后仍可能假成熟。
        self.assertIn("computer_action_result", event_payload_text)  # 新增代码+StructuredComputerActionAcceptance: 要求场景检查正式动作结果事件；若没有这行代码，launch_app 真实结果不会进入产品级验收。
        self.assertIn('"real_launch_performed":true', event_payload_text)  # 新增代码+StructuredComputerActionAcceptance: 要求真实启动证据来自事件 payload；若没有这行代码，只发现候选也可能误通过。
        self.assertIn('"visible_window_verified":true', event_payload_text)  # 新增代码+StructuredComputerActionAcceptance: 要求可见窗口验证来自事件 payload；若没有这行代码，后台启动或代理进程失败可能误通过。
        self.assertIn('"launch_backend":"start_menu_shortcut"', event_payload_text)  # 新增代码+StructuredComputerActionAcceptance: 要求启动后端进入正式事件；若没有这行代码，后续无法判断到底是不是开始菜单快捷方式真实启动。
        self.assertIn('"display_name":"微信"', event_payload_text)  # 新增代码+StructuredComputerActionAcceptance: 要求目标显示名进入正式事件；若没有这行代码，打开错应用也可能只靠文本“微信”误过。
    def test_computer_action_acceptance_payload_extracts_launch_evidence(self) -> None:  # 新增代码+StructuredComputerActionAcceptance: 函数段开始，验证 agent 能从动作结果结构里抽取产品级验收字段；若没有这段测试，事件可能只写一大段不可断言文本。本段到最后一个断言结束。
        result_data = {"target_app": "微信", "real_desktop_touched": True, "real_launch_performed": True, "visible_window_verified": True, "target_window": {"window_id": "hwnd:1"}, "low_level_event_count": 0, "image_result_count": 1, "session": {"phase108_report": {"launch_plan": {"launch_backend": "start_menu_shortcut", "display_name": "微信"}}}}  # 新增代码+StructuredComputerActionAcceptance: 构造接近真实 launch_app 返回值的数据；若没有这行代码，测试无法覆盖微信代理窗口和开始菜单后端证据。
        payload = LearningAgent._computer_action_acceptance_payload_from_result_data("launch_app", True, "ok", result_data)  # 新增代码+StructuredComputerActionAcceptance: 调用即将提供的正式 payload 抽取函数；若没有这行代码，测试无法证明主循环动作结果会变成结构化事件。
        self.assertEqual(payload["state_model"], "computer_action_structured_evidence")  # 新增代码+StructuredComputerActionAcceptance: 断言 payload 声明自己是结构化证据模型；若没有这行代码，消费者无法区分普通文本摘要和验收证据。
        self.assertEqual(payload["tool_name"], "computer_action")  # 新增代码+StructuredComputerActionAcceptance: 断言工具名稳定；若没有这行代码，controller 无法按工具类型筛选证据。
        self.assertEqual(payload["action"], "launch_app")  # 新增代码+StructuredComputerActionAcceptance: 断言动作名稳定；若没有这行代码，后续点击/输入动作可能被误当成启动证据。
        self.assertTrue(payload["real_launch_performed"])  # 新增代码+StructuredComputerActionAcceptance: 断言真实启动字段被抽取；若没有这行代码，发现候选但未启动也可能无法被识别。
        self.assertTrue(payload["visible_window_verified"])  # 新增代码+StructuredComputerActionAcceptance: 断言可见窗口字段被抽取；若没有这行代码，用户看不到窗口时仍可能误判成功。
        self.assertEqual(payload["launch_backend"], "start_menu_shortcut")  # 新增代码+StructuredComputerActionAcceptance: 断言启动后端来自嵌套 launch_plan；若没有这行代码，多后端 resolver 的真实选择会丢失。
        self.assertEqual(payload["display_name"], "微信")  # 新增代码+StructuredComputerActionAcceptance: 断言应用显示名来自嵌套 launch_plan；若没有这行代码，中文应用验收会退回脆弱日志 contains。
        self.assertTrue(payload["target_window_present"])  # 新增代码+StructuredComputerActionAcceptance: 断言目标窗口存在被折成布尔字段；若没有这行代码，controller 不容易判断窗口绑定是否完成。
        self.assertTrue(payload["real_desktop_touched"])  # 新增代码+StructuredComputerActionAcceptance: 断言真实桌面触碰字段保留；若没有这行代码，模拟路径和真实路径会再次混在一起。
