...
_DANGEROUS_PERMISSION_NOTICE_PRINTED = False  # 修改代码+PermissionNoticeNoiseControl: 记录危险调试提示是否已在当前进程打印；若没有这行代码，Computer Use 每次鼠标动作授权都会重复刷屏。
...
class LearningAgent:  # 作用: 教学版 agent 主类
    ...
    def run_events(self, user_input: str, max_turns: int | None = None):  # 新增代码+Stage15C: 新增事件流主循环；若没有这行代码，UI、HTTP bridge 和 transcript 无法观察运行过程。
        ...
        try:  # 新增代码+Stage15C: 捕获主循环异常并转成 run_failed 事件；若没有这行代码，事件消费者会看到生成器直接崩溃。
            ...
            while max_turns is None or turn < max_turns:  # 新增代码+Stage15C: 保留可配置轮次循环；若没有这行代码，agent 无法继续多轮工具调用。
                ...
                if any(self._is_computer_use_tool_name(name) for name in tool_names):  # 新增代码+RuntimeTrace：只有模型本轮看见 Computer Use 工具时才记录主循环请求；如果没有这行代码，trace 会被普通开发任务噪音污染。
                    self._record_computer_use_runtime_trace("model_request", {"turn": turn, "turn_id": current_turn_id, "tool_names": tool_names, "message_count": len(messages), "desktop_task_active": bool(self.desktop_task_context.get("active", False))})  # 新增代码+RuntimeTrace：记录模型请求阶段的工具 schema 摘要；如果没有这行代码，无法证明 Computer Use 工具真的暴露给模型主循环。
                ...
                if not model_message.tool_calls:  # 新增代码+Stage15C: 无工具调用时进入最终回答路径；若没有这行代码，直接回答会被误当成工具循环。
                    ...
                    final_answer_text = self._computer_use_full_final_answer_with_evidence_summary(model_message.text)  # 新增代码+ComputerUseFinalEvidenceSummary：在最终出口补齐真实桌面证据摘要；如果没有这一行，模型短答“已完成。”会丢掉 screenshot、real_desktop_touched 和 low_level_event_count。
                    self._write_debug_event(run_id=run_id, event="final_answer", payload={"text": final_answer_text}, turn=turn)  # 修改代码+ComputerUseFinalEvidenceSummary：debug 日志记录补强后的最终回答；如果没有这一行，真实验收失败时日志仍看不到用户实际收到的摘要。
                ...
                for tool_call in model_message.tool_calls:  # 修改代码+Stage15F: 先按原顺序发出本批工具开始事件；若没有这行代码，并发执行时 UI 无法看到稳定开始序列。
                    ...
                    if self._is_computer_use_tool_name(tool_call.name):  # 新增代码+RuntimeTrace：只记录 Computer Use 工具调用；如果没有这行代码，普通工具调用会污染桌面运行证据。
                        ...
                        self._record_computer_use_runtime_trace("tool_call", {"turn": turn, "turn_id": current_turn_id, "tool_name": tool_call.name, "call_id": tool_call.call_id, "action": trace_arguments.get("action", ""), "operation": trace_arguments.get("operation", ""), "query": trace_arguments.get("query", "")})  # 新增代码+RuntimeTrace：记录模型选择了哪个 Computer Use 工具；如果没有这行代码，后续无法区分模型看见工具和模型实际调用工具。
                ...
                for tool_call, tool_output in zip(model_message.tool_calls, tool_outputs):  # 修改代码+Stage15F: 按原 tool_call 顺序发出完成事件和回填消息；若没有这行代码，并发结果可能乱序。
                    ...
                    if self._is_computer_use_tool_name(tool_call.name):  # 新增代码+RuntimeTrace：只记录 Computer Use 工具结果；如果没有这行代码，普通工具结果会污染桌面主循环证据。
                        tool_output_lower = tool_output.lower()  # 新增代码+RuntimeTrace：生成小写文本用于低成本识别截图证据关键词；如果没有这行代码，截图标记匹配会受大小写影响。
                        self._record_computer_use_runtime_trace("tool_result", {"turn": turn, "turn_id": current_turn_id, "tool_name": tool_call.name, "call_id": tool_call.call_id, "raw_output_chars": len(tool_output), "context_output_chars": len(context_tool_output), "screenshot_returned_to_model": "image_result_count" in tool_output_lower or "screenshot" in tool_output_lower or "artifact_path" in tool_output_lower})  # 新增代码+RuntimeTrace：记录 Computer Use 工具结果回流摘要；如果没有这行代码，无法证明工具输出已经回到模型循环。
    ...
    def _build_computer_use_full_model_loop_harness_message(self, user_input: str) -> str:  # 新增代码+ModelLoopSemanticPlanner：函数段开始，为 full 模式桌面任务生成极简模型主循环约束；如果没有这段函数，自然语言桌面任务会缺少“由模型选工具”的明确上下文。
        ...
        loaded_computer_tools = {"computer_use", "computer_observe", "computer_discover", "computer_action"} & set(getattr(self, "loaded_tool_names", set()))  # 修改代码+ComputerDiscoverTool：确认 full 命令已经把发现、观察、动作工具放入当前工具池；如果没有这一行，模型会收到规则但看不到 discover。
        if not {"computer_use", "computer_observe", "computer_discover", "computer_action"}.issubset(loaded_computer_tools):  # 修改代码+ComputerDiscoverTool：四个核心工具不齐时不注入 full harness；如果没有这一行，半加载状态会误导模型调用不存在的 discover。
        ...
        discovery_only = self._computer_use_full_user_requested_discovery_only(user_input)  # 新增代码+ComputerDiscoverTool：判断用户是否只想查找 app_name 且不要打开软件；如果没有这一行，查询类任务会被首轮 launch_app 规则误导。
        desktop_task_context["discovery_only"] = bool(discovery_only)  # 新增代码+ComputerDiscoverTool：把 discovery-only 意图写回桌面上下文供工具收窄逻辑使用；如果没有这一行，第 0 轮仍可能只暴露 computer_action。
        app_catalog_context = "本轮是应用候选发现任务：不要直接从提示词里的内置候选清单回答，必须先调用 `computer_discover`，query 使用用户目标应用或原始查询词；如果用户说不要打开或操作软件，禁止调用 `launch_app`。" if discovery_only else self._computer_use_full_app_catalog_context()  # 修改代码+ComputerDiscoverTool：发现类任务改为工具驱动而不是提示词直接给答案；如果没有这一行，真实主循环会跳过 computer_discover。
        return "\n".join([  # 新增代码+ModelLoopSemanticPlanner：拼出短提示词块；如果没有这一行，模型不会获得结构化 Computer Use full 工作方式。
            ...
            app_catalog_context,  # 修改代码+WindowsAppInventory：把统一 inventory 的应用候选交给模型主循环；如果没有这一行，模型仍只能靠猜测 app 名而不是从清单里选择。
            ...
            "如果用户只是查询本机应用是否可用、要哪个 app_name、或明确不要打开/操作软件，第一步必须调用 `computer_discover` 并基于工具结果回答。",  # 新增代码+ComputerDiscoverTool：把应用发现作为主循环工具步骤；如果没有这一行，模型可能再次直接从候选提示里回答。
            "如果 computer_discover 返回唯一高置信候选，并且用户目标是打开或操作本机应用，下一轮必须使用该候选的 app_name 调用 `computer_action`，action=launch_app；不要把“发现到候选”当成最终完成。",  # 新增代码+WeChatLaunchRegression：约束 discover 后必须进入真实启动；如果没有这一行，微信这类应用可能被发现后模型直接结束而没有打开软件。
            "从 computer_discover 复制候选字段时必须逐字复制 app_name 或规范启动应用名，不要追加 .exe、.lnk、.pdf、问号、说明文字或其它后缀；需要保留启动标识时放在 target 字段。",  # 新增代码+WeChatLaunchRegression：阻止模型把“微信”改写成“微信.pdf???”这类脏启动名；如果没有这一行，后端虽可兜底但日志和后续工具选择会被污染。
            ...
            "每次观察后都要先判断当前画面是否服务于用户本轮目标；观察到当前画面与用户本轮目标不一致时，必须先纠偏到本轮目标状态，例如新建、清空、导航、重新聚焦或选择正确窗口，然后再继续操作。",  # 新增代码+ComputerUseSessionTargetGuard：把观察后的目标一致性检查写成通用硬流程；如果没有这一行，模型可能把旧窗口、旧文档、旧网页或旧画布当成当前任务上下文继续加工。
            ...
            "不要主动添加用户未要求的对象、背景、装饰或新主题；当截图已经能表达用户要求的目标时，直接最终回答，不要为了“丰富画面”继续加笔。",  # 新增代码+ComputerUseGoalLock: 禁止模型把当前任务扩展成未请求对象或装饰；如果没有这一行，简单画树任务可能继续漂移成火箭、星星、月亮等新主题。
    ...
    def _computer_use_full_user_requested_discovery_only(self, user_input: str) -> bool:  # 新增代码+ComputerDiscoverTool：函数段开始，判断用户是否只想发现应用候选而不是打开应用；如果没有这段函数，查 app_name 任务会被当成 GUI 操作任务。
        text = str(user_input or "").casefold()  # 新增代码+ComputerDiscoverTool：把用户输入转成大小写无关文本；如果没有这一行，英文 app_name 查询大小写会影响判断。
        discovery_markers = ("查找", "查询", "是否可用", "可用", "哪个 app_name", "app_name", "候选", "discover", "find app", "which app")  # 新增代码+ComputerDiscoverTool：定义发现类任务关键词；如果没有这一行，函数无法识别用户只是想找应用名。
        no_launch_markers = ("不要打开", "不打开", "不要启动", "不启动", "不要操作", "不操作", "不要控制", "不控制", "without launching", "do not launch", "do not open")  # 新增代码+ComputerDiscoverTool：定义禁止打开/操作关键词；如果没有这一行，发现任务可能仍被当成启动任务。
        has_discovery_intent = any(marker in text for marker in discovery_markers)  # 新增代码+ComputerDiscoverTool：检查用户是否表达应用发现意图；如果没有这一行，函数只能看禁止打开而无法判断任务类型。
        has_no_launch_intent = any(marker in text for marker in no_launch_markers)  # 新增代码+ComputerDiscoverTool：检查用户是否明确不要打开或操作；如果没有这一行，函数可能误把普通“打开画图”当成发现任务。
        return bool(has_discovery_intent and has_no_launch_intent)  # 新增代码+ComputerDiscoverTool：只有同时满足发现和不打开才进入 discovery-only；如果没有这一行，函数没有最终判断结果。
    # 新增代码+ComputerDiscoverTool：函数段结束，_computer_use_full_user_requested_discovery_only 到此结束；如果没有这个边界说明，用户不容易看出发现类意图识别范围。

    def _computer_use_full_app_catalog_context(self) -> str:  # 修改代码+WindowsAppInventory：函数段开始，构造 full 模式模型可见应用候选清单；如果没有这段函数，应用发现无法接入模型主循环。
        try:  # 修改代码+WindowsAppInventory：优先按包路径导入统一 inventory；如果没有这一行，agent.py 无法读取唯一应用清单入口。
            from learning_agent.computer_use.windows_app_inventory import build_windows_app_inventory, format_windows_app_inventory_for_model  # 修改代码+WindowsAppInventory：导入统一枚举和模型提示格式化函数；如果没有这一行，主循环拿不到清洗后的候选提示。
        except ModuleNotFoundError as error:  # 修改代码+WindowsAppInventory：兼容 start_oauth_agent.bat 从 learning_agent 目录启动的脚本模式；如果没有这一行，真实终端入口可能导入失败。
            if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.windows_app_inventory"}:  # 修改代码+WindowsAppInventory：只兜底包路径缺失；如果没有这一行，inventory 内部 bug 会被错误吞掉。
                raise  # 修改代码+WindowsAppInventory：重新抛出真实内部错误；如果没有这一行，排查清单模块问题会变得困难。
            from computer_use.windows_app_inventory import build_windows_app_inventory, format_windows_app_inventory_for_model  # type: ignore  # 修改代码+WindowsAppInventory：脚本模式导入同一能力；如果没有这一行，bat 入口无法给模型应用候选清单。
        catalog = build_windows_app_inventory(include_common=True)  # 修改代码+WindowsAppInventory：枚举并清洗本机应用候选；如果没有这一行，模型提示没有实际候选来源。
        return format_windows_app_inventory_for_model(catalog)  # 修改代码+WindowsAppInventory：返回不含原始路径的模型提示文本；如果没有这一行，harness 无法拼接候选清单。
    # 修改代码+WindowsAppInventory：函数段结束，_computer_use_full_app_catalog_context 到此结束；如果没有这个边界说明，用户不容易看出候选清单提示范围。
    ...
    def _computer_use_full_canonical_target_app_hint(self, target_app_hint: str) -> str:  # 新增代码+ModelLoopFirstStepLaunch：函数段开始，把脱敏应用提示转换成稳定启动别名；如果没有这段函数，模型会在“画图”和 mspaint 之间摇摆，作者意图是只规范应用入口而不解析绘图主体。
        ...
        try:  # 修改代码+WindowsAppInventory：优先使用统一 inventory 解析启动名；如果没有这一行，函数会继续只靠局部硬编码字典。
            from learning_agent.computer_use.windows_app_inventory import resolve_windows_app_name_hint  # 修改代码+WindowsAppInventory：导入统一应用名解析器；如果没有这一行，中文/英文别名无法共享清单逻辑。
        except ModuleNotFoundError as error:  # 修改代码+WindowsAppInventory：兼容脚本模式导入；如果没有这一行，真实 bat 入口可能无法解析应用名。
            if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.windows_app_inventory"}:  # 修改代码+WindowsAppInventory：只兜底包路径缺失；如果没有这一行，内部错误会被误判为脚本模式问题。
                raise  # 修改代码+WindowsAppInventory：重新抛出真实内部错误；如果没有这一行，清单解析 bug 会被掩盖。
            from computer_use.windows_app_inventory import resolve_windows_app_name_hint  # type: ignore  # 修改代码+WindowsAppInventory：脚本模式导入同一解析器；如果没有这一行，start_oauth_agent.bat 入口无法使用清单解析。
        catalog_hint = resolve_windows_app_name_hint(normalized_hint)  # 修改代码+WindowsAppInventory：用统一候选清单解析规范 app_name；如果没有这一行，模型仍无法从清单别名得到 mspaint。
        if catalog_hint:  # 修改代码+WindowsAppInventory：命中清单别名时优先使用；如果没有这一行，清单解析结果不会生效。
            return catalog_hint  # 修改代码+WindowsAppInventory：返回清单给出的稳定 app_name；如果没有这一行，后续仍走旧硬编码路径。
    ...
    def _computer_use_full_initial_launch_required(self, turn: int) -> bool:  # 新增代码+ModelLoopFirstStepLaunch：函数段开始，判断当前模型轮次是否必须先让模型选择 launch_app；如果没有这段函数，首轮工具收窄逻辑会散落在 run_events 主循环里。
        ...
        if bool(desktop_task_context.get("discovery_only", False)):  # 新增代码+ComputerDiscoverTool：发现类任务不强制启动应用；如果没有这一行，用户明确不要打开软件时第 0 轮仍会只剩 launch_app。
            return False  # 新增代码+ComputerDiscoverTool：返回不要求 launch_app；如果没有这一行，模型无法先调用 computer_discover。
    ...
    def _scoped_tool_schemas_for_model_turn(self, tools: list[dict[str, Any]], turn: int) -> list[dict[str, Any]]:  # 新增代码+ModelLoopFirstStepLaunch：函数段开始，按当前轮次收窄模型可见工具；如果没有这段函数，模型首轮会继续在大工具面里超时。
        ...
        for tool_schema in tools:  # 修改代码+ComputerDiscoverTool：遍历当前工具池以保留发现和启动工具；如果没有这一行，函数无法找到 computer_discover/computer_action schema。
            ...
            if tool_name in {"computer_discover", "computer_action"}:  # 修改代码+ComputerDiscoverTool：首轮保留应用发现和启动动作入口；如果没有这一行，模型无法先发现候选或启动目标应用。
                scoped_tools.append(tool_schema)  # 修改代码+ComputerDiscoverTool：加入 discover/action schema；如果没有这一行，模型首轮看不到必要的发现或启动工具。
    ...
    def _is_computer_use_tool_name(self, tool_name: str) -> bool:  # 新增代码+RuntimeTrace：函数段开始，判断一个工具名是否属于 Computer Use 工具面；如果没有这段函数，trace 过滤规则会散落在主循环多个位置。
        normalized_name = str(tool_name).strip()  # 新增代码+RuntimeTrace：清理工具名两端空白；如果没有这行代码，异常空白会导致 Computer Use 工具漏记。
        return normalized_name in {"computer_status", "computer_observe", "computer_discover", "computer_action", "computer_use", "computer-use", "computer_use_mode"} or normalized_name.startswith("computer_") or normalized_name.startswith("computer-")  # 新增代码+RuntimeTrace：匹配当前工具面和兼容命名；如果没有这行代码，模型调用过 Computer Use 也可能不进 trace。
    # 新增代码+RuntimeTrace：函数段结束，_is_computer_use_tool_name 到此结束；如果没有这个边界说明，用户不容易看出工具名过滤范围。

    def _record_computer_use_runtime_trace(self, phase: str, payload: dict[str, Any]) -> None:  # 新增代码+RuntimeTrace：函数段开始，记录 Computer Use 主循环运行时证据；如果没有这段函数，模型请求、工具调用和工具结果无法形成同一条证据链。
        if not hasattr(self, "computer_use_runtime_trace_events"):  # 修改代码+CompatSlimming：兼容测试或旧桥接用 __new__ 绕过初始化的对象；如果没有这一行，Computer Use 兼容分发会因缺 trace 列表崩溃。
            self.computer_use_runtime_trace_events = []  # 修改代码+CompatSlimming：惰性创建 trace 事件列表；如果没有这一行，后续 append 没有可写目标。
        event = {  # 新增代码+RuntimeTrace：构造固定结构的 trace 事件；如果没有这行代码，不同记录点会输出不兼容字段。
            "time": time.strftime("%Y-%m-%d %H:%M:%S"),  # 新增代码+RuntimeTrace：保存 trace 事件时间；如果没有这行代码，排查时无法判断模型请求和工具结果先后。
            "phase": str(phase),  # 新增代码+RuntimeTrace：保存阶段名，例如 model_request、tool_call、tool_result；如果没有这行代码，汇总函数无法分类。
            "payload": copy.deepcopy(payload),  # 新增代码+RuntimeTrace：深拷贝脱敏摘要载荷；如果没有这行代码，后续代码改动 payload 会污染历史证据。
        }  # 新增代码+RuntimeTrace：trace 事件字典结束；如果没有这行代码，Python 语法不完整。
        self.computer_use_runtime_trace_events.append(event)  # 新增代码+RuntimeTrace：把事件放入 Computer Use 专用 trace；如果没有这行代码，report 没有数据来源。
        self.computer_use_runtime_trace_events = self.computer_use_runtime_trace_events[-300:]  # 新增代码+RuntimeTrace：限制最多保留最近 300 条避免长任务内存无限增长；如果没有这行代码，真实桌面长循环会堆积过多事件。
        self._record_observation("computer_use_runtime_trace", event)  # 新增代码+RuntimeTrace：同步写入通用 observation 方便旧调试入口查看；如果没有这行代码，latest_run_readable 看不到 trace 入口。
    # 新增代码+RuntimeTrace：函数段结束，_record_computer_use_runtime_trace 到此结束；如果没有这个边界说明，用户不容易看出 trace 记录范围。

    def _computer_use_runtime_trace_report(self) -> dict[str, Any]:  # 新增代码+RuntimeTrace：函数段开始，汇总当前 agent 的 Computer Use 运行时证据；如果没有这段函数，用户无法查询主循环实际走过哪些工具。
        events = list(getattr(self, "computer_use_runtime_trace_events", []))  # 新增代码+RuntimeTrace：读取 trace 事件快照；如果没有这行代码，汇总过程可能被后续追加事件影响。
        computer_tool_names_seen: set[str] = set()  # 新增代码+RuntimeTrace：保存模型请求阶段看到过的 Computer Use 工具名；如果没有这行代码，schema 可见性无法汇总。
        tools_called: set[str] = set()  # 新增代码+RuntimeTrace：保存模型实际调用过的 Computer Use 工具名；如果没有这行代码，schema 暴露和真实调用会混在一起。
        direct_tool_phases: set[str] = set()  # 新增代码+RuntimeTrace：保存工具内部补充记录过的阶段；如果没有这行代码，observe/discover/action 内部路径无法单独查看。
        model_request_seen = False  # 新增代码+RuntimeTrace：默认尚未看到模型请求；如果没有这行代码，空 trace 可能被误判为真。
        tool_call_seen = False  # 新增代码+RuntimeTrace：默认尚未看到工具调用；如果没有这行代码，report 无法给出稳定布尔字段。
        tool_result_seen = False  # 新增代码+RuntimeTrace：默认尚未看到工具结果；如果没有这行代码，report 无法判断结果是否回流模型。
        screenshot_returned_to_model = False  # 新增代码+RuntimeTrace：默认未观察到截图回流标记；如果没有这行代码，视觉输入状态没有汇总字段。
        for event in events:  # 新增代码+RuntimeTrace：逐条扫描 trace 事件；如果没有这行代码，汇总字段没有计算过程。
            phase = str(event.get("phase", ""))  # 新增代码+RuntimeTrace：读取当前事件阶段；如果没有这行代码，后续无法按阶段判断。
            payload = event.get("payload", {})  # 新增代码+RuntimeTrace：读取当前事件载荷；如果没有这行代码，工具名和截图标记无法提取。
            payload_dict = payload if isinstance(payload, dict) else {}  # 新增代码+RuntimeTrace：防御异常载荷类型；如果没有这行代码，坏事件会让 report 崩溃。
            if phase == "model_request":  # 新增代码+RuntimeTrace：识别模型请求阶段；如果没有这行代码，schema 可见性不会被记录。
                model_request_seen = True  # 新增代码+RuntimeTrace：标记模型请求已出现；如果没有这行代码，report 会错误显示模型没有请求。
                for tool_name in payload_dict.get("tool_names", []):  # 新增代码+RuntimeTrace：遍历模型可见工具名；如果没有这行代码，无法收集 Computer Use schema。
                    tool_name_text = str(tool_name)  # 新增代码+RuntimeTrace：把工具名转成字符串；如果没有这行代码，异常类型会影响集合去重。
                    if self._is_computer_use_tool_name(tool_name_text):  # 新增代码+RuntimeTrace：只保留 Computer Use 工具；如果没有这行代码，普通 read/bash 工具会污染 report。
                        computer_tool_names_seen.add(tool_name_text)  # 新增代码+RuntimeTrace：记录模型可见 Computer Use 工具；如果没有这行代码，tool_schema_visible 无法判断。
            if phase == "tool_call":  # 新增代码+RuntimeTrace：识别工具调用阶段；如果没有这行代码，模型实际调用过工具不会被汇总。
                tool_name_text = str(payload_dict.get("tool_name", ""))  # 新增代码+RuntimeTrace：读取被调用工具名；如果没有这行代码，工具调用集合没有元素来源。
                if self._is_computer_use_tool_name(tool_name_text):  # 新增代码+RuntimeTrace：只统计 Computer Use 工具调用；如果没有这行代码，普通工具会误算到桌面能力里。
                    tool_call_seen = True  # 新增代码+RuntimeTrace：标记模型实际调用过 Computer Use 工具；如果没有这行代码，report 会漏掉调用事实。
                    tools_called.add(tool_name_text)  # 新增代码+RuntimeTrace：记录工具名；如果没有这行代码，用户看不到具体调用了 discover 还是 action。
            if phase == "tool_result":  # 新增代码+RuntimeTrace：识别工具结果阶段；如果没有这行代码，结果回流不会被汇总。
                tool_name_text = str(payload_dict.get("tool_name", ""))  # 新增代码+RuntimeTrace：读取结果所属工具名；如果没有这行代码，工具结果集合没有归属。
                if self._is_computer_use_tool_name(tool_name_text):  # 新增代码+RuntimeTrace：只统计 Computer Use 工具结果；如果没有这行代码，普通工具结果会污染桌面 trace。
                    tool_result_seen = True  # 新增代码+RuntimeTrace：标记 Computer Use 工具结果已回流；如果没有这行代码，report 无法证明模型拿到了工具结果。
                    tools_called.add(tool_name_text)  # 新增代码+RuntimeTrace：把有结果的工具也纳入调用集合；如果没有这行代码，只记录调用时中断会漏算。
                    screenshot_returned_to_model = screenshot_returned_to_model or bool(payload_dict.get("screenshot_returned_to_model", False))  # 新增代码+RuntimeTrace：累计截图回流标记；如果没有这行代码，视觉输入证据不会出现在 report。
            if phase.startswith("computer_"):  # 新增代码+RuntimeTrace：识别工具内部补充阶段；如果没有这行代码，discover/observe/action 内部 trace 无法汇总。
                direct_tool_phases.add(phase)  # 新增代码+RuntimeTrace：保存内部阶段名；如果没有这行代码，真实工具内部是否执行过不可见。
        return {  # 新增代码+RuntimeTrace：返回结构化汇总报告；如果没有这行代码，调用方拿不到可断言结果。
            "runtime_trace_available": True,  # 新增代码+RuntimeTrace：声明 runtime trace 能力存在；如果没有这行代码，外部无法区分旧 agent 和新 agent。
            "trace_event_count": len(events),  # 新增代码+RuntimeTrace：返回事件数量；如果没有这行代码，空 trace 不容易被发现。
            "model_request_seen": model_request_seen,  # 新增代码+RuntimeTrace：返回模型请求是否被记录；如果没有这行代码，主循环入口不可验证。
            "tool_schema_visible": bool(computer_tool_names_seen),  # 新增代码+RuntimeTrace：返回 Computer Use schema 是否对模型可见；如果没有这行代码，工具暴露状态不可验证。
            "tool_call_seen": tool_call_seen,  # 新增代码+RuntimeTrace：返回模型是否调用 Computer Use 工具；如果没有这行代码，模型主动选择工具不可验证。
            "tool_result_seen": tool_result_seen,  # 新增代码+RuntimeTrace：返回工具结果是否回流；如果没有这行代码，observe-plan-act loop 是否闭合不可验证。
            "screenshot_returned_to_model": screenshot_returned_to_model,  # 新增代码+RuntimeTrace：返回是否观察到截图回流；如果没有这行代码，视觉输入路径没有统一字段。
            "computer_tool_names_seen": sorted(computer_tool_names_seen),  # 新增代码+RuntimeTrace：返回模型看见过的 Computer Use 工具名；如果没有这行代码，用户只能看到布尔值不能定位缺哪一个。
            "tools_called": sorted(tools_called),  # 新增代码+RuntimeTrace：返回模型实际调用过的 Computer Use 工具名；如果没有这行代码，瘦身时仍无法区分暴露和使用。
            "direct_tool_phases": sorted(direct_tool_phases),  # 新增代码+RuntimeTrace：返回工具内部记录阶段；如果没有这行代码，内部 discover/observe/action 路径不可见。
        }  # 新增代码+RuntimeTrace：报告字典结束；如果没有这行代码，Python 语法不完整。
    # 新增代码+RuntimeTrace：函数段结束，_computer_use_runtime_trace_report 到此结束；如果没有这个边界说明，用户不容易看出汇总函数范围。
    ...
    def _computer_status(self, arguments: dict[str, Any]) -> str:  # 新增代码+OSComputerUse: 实现 computer_status 只读工具；若没有这段代码，模型无法查看 OS 桌面控制后端状态。
        ...
        self._record_computer_use_runtime_trace("computer_status", {"ok": bool(status.get("ok", False)), "backend": status.get("backend", ""), "real_desktop_touched": bool(status.get("real_desktop_touched", False))})  # 新增代码+RuntimeTrace：记录状态工具内部执行摘要；如果没有这行代码，runtime trace 只能看到模型调用而看不到工具内部状态。
    ...
    def _computer_observe(self, arguments: dict[str, Any]) -> str:  # 新增代码+Phase27ComputerUse: 实现 computer_observe 只读工具；如果没有这段代码，模型无法在动作前列出窗口或读取窗口状态。
        ...
        self._record_computer_use_runtime_trace("computer_observe", {"action": action_name, "ok": bool(result.ok), "message": result.message, "image_result_count": result.data.get("image_result_count", 0) if isinstance(result.data, dict) else 0})  # 新增代码+RuntimeTrace：记录 observe 工具内部执行摘要；如果没有这行代码，截图观察是否发生只能从长文本里猜。
    ...
    def _computer_discover(self, arguments: dict[str, Any]) -> str:  # 新增代码+ComputerDiscoverTool：函数段开始，实现 Computer Use 只读应用发现工具；如果没有这段函数，schema 中的 computer_discover 只是空壳。
        try:  # 新增代码+ComputerDiscoverTool：优先按包模式导入统一 Windows 应用清单查询函数；如果没有这行代码，正常包运行时无法查询应用候选。
            from learning_agent.computer_use.windows_app_inventory import query_windows_app_inventory  # 新增代码+ComputerDiscoverTool：导入正式 inventory 查询入口；如果没有这行代码，discover 会绕开统一应用清单。
        except ModuleNotFoundError as error:  # 新增代码+ComputerDiscoverTool：兼容 start_oauth_agent.bat 脚本模式；如果没有这行代码，真实终端入口可能因为包名前缀失败。
            if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.windows_app_inventory"}:  # 新增代码+ComputerDiscoverTool：只对目标包路径缺失做 fallback；如果没有这行代码，模块内部真实 bug 会被误吞。
                raise  # 新增代码+ComputerDiscoverTool：重新抛出非预期导入错误；如果没有这行代码，排查 inventory 内部问题会很困难。
            from computer_use.windows_app_inventory import query_windows_app_inventory  # 新增代码+ComputerDiscoverTool：脚本模式下导入同一个查询入口；如果没有这行代码，bat 入口无法执行 discover。
        query_text = str(arguments.get("query", "")).strip()  # 新增代码+ComputerDiscoverTool：读取模型提供的应用查询词；如果没有这行代码，工具不知道要查哪个应用。
        max_results = arguments.get("max_results", arguments.get("limit"))  # 修改代码+FullOrdinaryAppInventory：只在模型显式传 max_results 或 limit 时限制返回候选；如果没有这一行，computer_discover 会继续默认隐藏第 11 个以后的普通应用。
        include_common = bool(arguments.get("include_common", True))  # 新增代码+ComputerDiscoverTool：读取是否包含公共兜底候选；如果没有这行代码，枚举失败时 Paint 等基础应用可能缺少入口。
        report = query_windows_app_inventory(query=query_text, include_common=include_common, max_count=max_results)  # 新增代码+ComputerDiscoverTool：调用统一应用清单查询；如果没有这行代码，discover 不会返回真实清洗候选。
        self._record_observation("computer_use_discover", {"ok": bool(report.get("ok", False)), "query": report.get("query", ""), "result_count": report.get("result_count", 0), "candidates": report.get("candidates", [])})  # 新增代码+ComputerDiscoverTool：把应用发现结果写入观察流；如果没有这行代码，真实终端调试无法复盘模型看到了哪些应用候选。
        self._record_computer_use_runtime_trace("computer_discover", {"ok": bool(report.get("ok", False)), "query": report.get("query", ""), "result_count": report.get("result_count", 0)})  # 新增代码+RuntimeTrace：记录 discover 工具内部执行摘要；如果没有这行代码，运行时无法证明主循环 discover 使用的是统一 inventory 路线。
        return json.dumps(report, ensure_ascii=False, indent=2)  # 新增代码+ComputerDiscoverTool：返回中文友好的 JSON；如果没有这行代码，模型无法稳定读取候选 app_name 和 launch_kind。
    # 新增代码+ComputerDiscoverTool：函数段结束，_computer_discover 到此结束；如果没有这个边界说明，用户不容易看出应用发现处理器范围。
    ...
    def _computer_use_full_completion_action_threshold(self) -> int:  # 新增代码+ComputerUseGoalLock: 函数段开始，按任务类型选择真实动作收束阈值；如果没有这段函数，简单绘图会等太久才停止，复杂任务又不能保留更宽松空间。
        desktop_task_context = getattr(self, "desktop_task_context", {})  # 新增代码+ComputerUseGoalLock: 读取当前桌面任务上下文；如果没有这行代码，阈值无法知道本轮是不是 Paint 绘图。
        if not isinstance(desktop_task_context, dict):  # 新增代码+ComputerUseGoalLock: 防御异常上下文形状；如果没有这行代码，坏状态可能让完成门崩溃。
            return 12  # 新增代码+ComputerUseGoalLock: 上下文异常时使用原保守阈值；如果没有这行代码，异常状态会导致没有返回值。
        task_goal = str(desktop_task_context.get("task_goal", "") or "").strip()  # 新增代码+ComputerUseGoalLock: 读取脱敏任务目标码；如果没有这行代码，函数无法区分简单绘图和其他 GUI 操作。
        if task_goal == "draw_with_local_paint":  # 新增代码+ComputerUseGoalLock: Paint 绘图任务使用更早收束阈值；如果没有这行代码，简单树/猫/房子任务会给模型过多漂移空间。
            return 6  # 新增代码+ComputerUseGoalLock: 六次真实绘制动作后要求模型观察并最终回答；如果没有这行代码，模型可能继续过度美化或改题。
        return 12  # 新增代码+ComputerUseGoalLock: 其他 GUI 任务保留原阈值；如果没有这行代码，复杂非绘图任务可能被过早截断。
    # 新增代码+ComputerUseGoalLock: 函数段结束，_computer_use_full_completion_action_threshold 到此结束；如果没有这个边界说明，读者不容易看出阈值来源。

    def _computer_use_full_final_answer_summary_context_active(self) -> bool:  # 新增代码+ComputerUseFinalEvidenceSummary：函数段开始，判断当前最终回答是否属于 full GUI 桌面任务；如果没有这段函数，普通文本回答可能被旧桌面证据错误污染。
        desktop_task_context = getattr(self, "desktop_task_context", {})  # 新增代码+ComputerUseFinalEvidenceSummary：读取当前 run_events 设置的桌面任务上下文；如果没有这一行，最终出口不知道本轮是不是 Computer Use 任务。
        if not isinstance(desktop_task_context, dict):  # 新增代码+ComputerUseFinalEvidenceSummary：防御异常上下文形状；如果没有这一行，外部注入坏状态可能让最终回答崩溃。
            return False  # 新增代码+ComputerUseFinalEvidenceSummary：上下文坏掉时不补摘要；如果没有这一行，普通回答可能被误判为桌面任务。
        return bool(desktop_task_context.get("active", False)) and bool(desktop_task_context.get("requires_gui_actions", False))  # 新增代码+ComputerUseFinalEvidenceSummary：只给需要 GUI 动作的桌面任务补强；如果没有这一行，解释类任务会得到无关证据摘要。
    # 新增代码+ComputerUseFinalEvidenceSummary：函数段结束，_computer_use_full_final_answer_summary_context_active 到此结束；如果没有这个边界说明，读者不容易看出补强适用范围。

    def _computer_use_full_extract_window_summary(self, data: dict[str, Any]) -> dict[str, str]:  # 新增代码+ComputerUseFinalEvidenceSummary：函数段开始，从工具结果中提取窗口身份；如果没有这段函数，最终回答无法说明真实控制的是哪个应用窗口。
        for key in ("target_window", "window"):  # 新增代码+ComputerUseFinalEvidenceSummary：按常见字段顺序查找窗口摘要；如果没有这一行，不同后端返回字段会被漏掉。
            candidate = data.get(key, {})  # 新增代码+ComputerUseFinalEvidenceSummary：读取候选窗口字典；如果没有这一行，后续无法判断 app_id/window_id。
            if not isinstance(candidate, dict):  # 新增代码+ComputerUseFinalEvidenceSummary：跳过非字典窗口字段；如果没有这一行，字符串窗口字段会触发属性错误。
                continue  # 新增代码+ComputerUseFinalEvidenceSummary：继续检查下一个字段；如果没有这一行，一个坏字段会终止提取。
            app_id = str(candidate.get("app_id", "") or candidate.get("process_name", "") or "").strip()  # 新增代码+ComputerUseFinalEvidenceSummary：提取 app_id 或进程名；如果没有这一行，最终摘要无法指出 mspaintapp 等身份。
            window_id = str(candidate.get("window_id", "") or candidate.get("hwnd", "") or "").strip()  # 新增代码+ComputerUseFinalEvidenceSummary：提取窗口 id；如果没有这一行，用户无法区分是否绑定到具体窗口。
            title = str(candidate.get("title_preview", "") or candidate.get("title", "") or "").strip()  # 新增代码+ComputerUseFinalEvidenceSummary：提取窗口标题预览；如果没有这一行，最终摘要缺少可读窗口线索。
            if app_id or window_id or title:  # 新增代码+ComputerUseFinalEvidenceSummary：只有拿到至少一个有效字段才返回；如果没有这一行，空窗口会覆盖已有证据。
                return {"app_id": app_id, "window_id": window_id, "title": title}  # 新增代码+ComputerUseFinalEvidenceSummary：返回标准化窗口摘要；如果没有这一行，调用方需要重复处理字段差异。
        return {}  # 新增代码+ComputerUseFinalEvidenceSummary：没有窗口证据时返回空字典；如果没有这一行，函数可能隐式返回 None 让调用方判断变复杂。
    # 新增代码+ComputerUseFinalEvidenceSummary：函数段结束，_computer_use_full_extract_window_summary 到此结束；如果没有这个边界说明，读者不容易看出窗口证据提取范围。

    def _computer_use_full_extract_screenshot_path(self, data: dict[str, Any]) -> str:  # 新增代码+ComputerUseFinalEvidenceSummary：函数段开始，从工具结果中提取截图路径；如果没有这段函数，最终回答只能说有截图却不能给出证据位置。
        direct_path = str(data.get("screenshot_path", "") or "").strip()  # 新增代码+ComputerUseFinalEvidenceSummary：优先读取顶层截图路径；如果没有这一行，observe 返回的最终截图路径会被漏掉。
        if direct_path:  # 新增代码+ComputerUseFinalEvidenceSummary：如果顶层路径存在就直接使用；如果没有这一行，后续嵌套检查可能覆盖更准确的最终截图。
            return direct_path  # 新增代码+ComputerUseFinalEvidenceSummary：返回最直接的截图路径；如果没有这一行，调用方拿不到路径。
        state = data.get("state", {}) if isinstance(data.get("state", {}), dict) else {}  # 新增代码+ComputerUseFinalEvidenceSummary：读取状态块里的截图字段；如果没有这一行，某些 observe 后端的截图路径会被漏掉。
        state_path = str(state.get("screenshot_path", "") or "").strip()  # 新增代码+ComputerUseFinalEvidenceSummary：提取状态块截图路径；如果没有这一行，内嵌路径无法进入最终摘要。
        if state_path:  # 新增代码+ComputerUseFinalEvidenceSummary：如果状态块路径存在就使用；如果没有这一行，函数会继续不必要地查找嵌套证据。
            return state_path  # 新增代码+ComputerUseFinalEvidenceSummary：返回状态块截图路径；如果没有这一行，最终摘要缺少图片位置。
        for evidence_key in ("after_evidence", "before_evidence"):  # 新增代码+ComputerUseFinalEvidenceSummary：按最终优先顺序查找动作前后截图；如果没有这一行，action 自带截图不会进入最终摘要。
            evidence = data.get(evidence_key, {})  # 新增代码+ComputerUseFinalEvidenceSummary：读取某个动作证据块；如果没有这一行，无法访问 image_results。
            if not isinstance(evidence, dict):  # 新增代码+ComputerUseFinalEvidenceSummary：跳过异常证据块；如果没有这一行，坏数据会让最终回答崩溃。
                continue  # 新增代码+ComputerUseFinalEvidenceSummary：继续检查下一个证据块；如果没有这一行，异常块会中断路径提取。
            evidence_path = str(evidence.get("screenshot_path", "") or "").strip()  # 新增代码+ComputerUseFinalEvidenceSummary：先读证据块顶层路径；如果没有这一行，直接保存的动作截图路径会被漏掉。
            if evidence_path:  # 新增代码+ComputerUseFinalEvidenceSummary：证据块路径存在时使用；如果没有这一行，函数会继续查找较弱路径。
                return evidence_path  # 新增代码+ComputerUseFinalEvidenceSummary：返回动作证据路径；如果没有这一行，最终摘要无法引用动作截图。
            image_results = evidence.get("image_results", [])  # 新增代码+ComputerUseFinalEvidenceSummary：读取模型可见图片块列表；如果没有这一行，PNG artifact 路径无法被发现。
            if not isinstance(image_results, list):  # 新增代码+ComputerUseFinalEvidenceSummary：防御 image_results 不是列表；如果没有这一行，遍历字符串会产生错误路径。
                continue  # 新增代码+ComputerUseFinalEvidenceSummary：跳过异常图片列表；如果没有这一行，坏图片结构会污染最终摘要。
            for image_result in reversed(image_results):  # 新增代码+ComputerUseFinalEvidenceSummary：优先使用最新图片块；如果没有这一行，最终摘要可能指向过早截图。
                if not isinstance(image_result, dict):  # 新增代码+ComputerUseFinalEvidenceSummary：跳过异常图片项；如果没有这一行，非字典图片项会触发属性错误。
                    continue  # 新增代码+ComputerUseFinalEvidenceSummary：继续检查前一个图片项；如果没有这一行，坏图片项会中断提取。
                artifact_path = str(image_result.get("artifact_path", "") or "").strip()  # 新增代码+ComputerUseFinalEvidenceSummary：读取图片 artifact 路径；如果没有这一行，最终摘要无法引用已落盘截图。
                if artifact_path:  # 新增代码+ComputerUseFinalEvidenceSummary：找到路径后返回；如果没有这一行，空路径可能覆盖真实证据。
                    return artifact_path  # 新增代码+ComputerUseFinalEvidenceSummary：返回图片 artifact 路径；如果没有这一行，调用方拿不到截图位置。
        return ""  # 新增代码+ComputerUseFinalEvidenceSummary：没有截图路径时返回空字符串；如果没有这一行，调用方无法稳定判断是否有路径。
    # 新增代码+ComputerUseFinalEvidenceSummary：函数段结束，_computer_use_full_extract_screenshot_path 到此结束；如果没有这个边界说明，读者不容易看出截图路径提取范围。

    def _computer_use_full_collect_final_answer_evidence(self) -> dict[str, Any]:  # 新增代码+ComputerUseFinalEvidenceSummary：函数段开始，汇总当前 full 会话的真实桌面证据；如果没有这段函数，最终回答只能依赖模型自觉复述工具结果。
        summary: dict[str, Any] = {"available": False, "target_app": "", "app_id": "", "window_id": "", "title": "", "screenshot": False, "screenshot_path": "", "real_desktop_touched": False, "low_level_event_count": 0, "successful_action_count": 0}  # 新增代码+ComputerUseFinalEvidenceSummary：初始化证据摘要结构；如果没有这一行，各字段会散落导致最终格式不稳定。
        if not self._computer_use_full_final_answer_summary_context_active():  # 新增代码+ComputerUseFinalEvidenceSummary：确认本轮确实是 full GUI 桌面任务；如果没有这一行，普通聊天也可能追加过期桌面证据。
            return summary  # 新增代码+ComputerUseFinalEvidenceSummary：非桌面任务直接返回空摘要；如果没有这一行，后续会误扫旧 observation。
        for event in self._computer_use_full_recent_events_after_mode_open()[-120:]:  # 新增代码+ComputerUseFinalEvidenceSummary：只扫描当前 full 会话最近 120 条事件；如果没有这一行，长期任务会变慢且可能混入旧证据。
            if event.get("kind") not in {"computer_use_action", "computer_use_observe"}:  # 新增代码+ComputerUseFinalEvidenceSummary：只汇总真实桌面动作和观察；如果没有这一行，状态/日志事件会污染最终摘要。
                continue  # 新增代码+ComputerUseFinalEvidenceSummary：跳过无关事件；如果没有这一行，后续读取 payload 会遇到不同结构。
            payload = event.get("payload", {})  # 新增代码+ComputerUseFinalEvidenceSummary：读取 observation 载荷；如果没有这一行，无法判断 ok 和 data。
            if not isinstance(payload, dict) or not bool(payload.get("ok", False)):  # 新增代码+ComputerUseFinalEvidenceSummary：只接受成功的 Computer Use 结果；如果没有这一行，失败动作也会被当成完成证据。
                continue  # 新增代码+ComputerUseFinalEvidenceSummary：跳过失败或异常载荷；如果没有这一行，坏事件可能误导最终回答。
            data = payload.get("data", {})  # 新增代码+ComputerUseFinalEvidenceSummary：读取控制器返回的结构化 data；如果没有这一行，窗口、截图和低层事件都无从提取。
            if not isinstance(data, dict):  # 新增代码+ComputerUseFinalEvidenceSummary：防御 data 不是字典；如果没有这一行，字符串工具结果会让摘要崩溃。
                continue  # 新增代码+ComputerUseFinalEvidenceSummary：跳过异常 data；如果没有这一行，最终回答出口可能被坏工具结果打断。
            summary["available"] = True  # 新增代码+ComputerUseFinalEvidenceSummary：标记已经找到本轮真实桌面证据；如果没有这一行，后续即使提取到字段也不会追加摘要。
            target_app = str(data.get("target_app", "") or data.get("app_name", "") or data.get("app", "") or "").strip()  # 新增代码+ComputerUseFinalEvidenceSummary：提取目标应用名；如果没有这一行，最终摘要可能缺少 mspaint 等用户可读 app。
            if target_app:  # 新增代码+ComputerUseFinalEvidenceSummary：只有非空应用名才覆盖摘要；如果没有这一行，空字段会抹掉已有应用名。
                summary["target_app"] = target_app  # 新增代码+ComputerUseFinalEvidenceSummary：保存最新有效目标应用名；如果没有这一行，最终回答无法说明控制了哪个软件。
            window_summary = self._computer_use_full_extract_window_summary(data)  # 新增代码+ComputerUseFinalEvidenceSummary：从 data 提取窗口身份；如果没有这一行，app_id/window_id 证据无法进入摘要。
            if window_summary.get("app_id"):  # 新增代码+ComputerUseFinalEvidenceSummary：如果有 app_id 就保存；如果没有这一行，mspaintapp:pid 这类硬身份会丢失。
                summary["app_id"] = window_summary["app_id"]  # 新增代码+ComputerUseFinalEvidenceSummary：记录 app_id；如果没有这一行，验收和排查无法确认目标进程。
            if window_summary.get("window_id"):  # 新增代码+ComputerUseFinalEvidenceSummary：如果有 window_id 就保存；如果没有这一行，最终摘要缺少窗口绑定证据。
                summary["window_id"] = window_summary["window_id"]  # 新增代码+ComputerUseFinalEvidenceSummary：记录 window_id；如果没有这一行，无法区分 agent-owned 窗口和旧窗口。
            if window_summary.get("title"):  # 新增代码+ComputerUseFinalEvidenceSummary：如果有标题预览就保存；如果没有这一行，用户可读窗口线索会缺失。
                summary["title"] = window_summary["title"]  # 新增代码+ComputerUseFinalEvidenceSummary：记录窗口标题；如果没有这一行，最终回答不够可理解。
            dispatch = data.get("dispatch", {}) if isinstance(data.get("dispatch", {}), dict) else {}  # 新增代码+ComputerUseFinalEvidenceSummary：读取底层调度摘要；如果没有这一行，low_level_event_count 可能从嵌套字段漏掉。
            low_level_count = int(dispatch.get("low_level_event_count", data.get("low_level_event_count", 0)) or 0)  # 新增代码+ComputerUseFinalEvidenceSummary：提取本次低层输入事件数；如果没有这一行，真实鼠标键盘证据无法量化。
            if low_level_count > 0:  # 新增代码+ComputerUseFinalEvidenceSummary：只有正数才累加；如果没有这一行，空动作会影响真实触碰判断。
                summary["low_level_event_count"] = int(summary["low_level_event_count"]) + low_level_count  # 新增代码+ComputerUseFinalEvidenceSummary：累加低层输入事件数；如果没有这一行，多次动作的真实证据会被低估。
                summary["successful_action_count"] = int(summary["successful_action_count"]) + 1  # 新增代码+ComputerUseFinalEvidenceSummary：累加成功真实动作数；如果没有这一行，最终摘要无法说明动作规模。
            if bool(data.get("real_desktop_touched", False)) or bool(data.get("real_input_enabled", False)) or low_level_count > 0:  # 新增代码+ComputerUseFinalEvidenceSummary：判断是否真实触碰桌面；如果没有这一行，最终回答无法区分模拟和物理输入。
                summary["real_desktop_touched"] = True  # 新增代码+ComputerUseFinalEvidenceSummary：保存真实桌面触碰标记；如果没有这一行，验收字段 real_desktop_touched 会缺失。
            if self._computer_use_data_has_model_visible_image(data) or bool(data.get("screenshot_captured", False)) or bool(data.get("screenshot_path", "")):  # 新增代码+ComputerUseFinalEvidenceSummary：判断是否存在截图/图片证据；如果没有这一行，最终回答可能漏掉模型已观察屏幕的事实。
                summary["screenshot"] = True  # 新增代码+ComputerUseFinalEvidenceSummary：标记截图证据存在；如果没有这一行，验收字段 screenshot 会缺失。
            screenshot_path = self._computer_use_full_extract_screenshot_path(data)  # 新增代码+ComputerUseFinalEvidenceSummary：提取可引用截图路径；如果没有这一行，最终摘要只能泛泛说有截图。
            if screenshot_path:  # 新增代码+ComputerUseFinalEvidenceSummary：只有找到路径时才覆盖；如果没有这一行，空路径会抹掉已有最终截图。
                summary["screenshot_path"] = screenshot_path  # 新增代码+ComputerUseFinalEvidenceSummary：保存最新截图路径；如果没有这一行，用户无法定位证据图。
        if not summary["target_app"] and "mspaint" in str(summary["app_id"]).lower():  # 新增代码+ComputerUseFinalEvidenceSummary：从 app_id 反推 Paint 应用名；如果没有这一行，真实 mspaintapp:pid 可能不满足 mspaint 验收字段。
            summary["target_app"] = "mspaint"  # 新增代码+ComputerUseFinalEvidenceSummary：补全标准目标应用名；如果没有这一行，最终回答可能只含 mspaintapp 而没有 mspaint。
        return summary  # 新增代码+ComputerUseFinalEvidenceSummary：返回最终证据摘要；如果没有这一行，最终回答补强逻辑拿不到数据。
    # 新增代码+ComputerUseFinalEvidenceSummary：函数段结束，_computer_use_full_collect_final_answer_evidence 到此结束；如果没有这个边界说明，读者不容易看出证据汇总范围。

    def _computer_use_full_final_answer_with_evidence_summary(self, answer: str) -> str:  # 新增代码+ComputerUseFinalEvidenceSummary：函数段开始，给 full 桌面任务最终回答追加真实证据摘要；如果没有这段函数，模型短答会继续让终端体验和验收结果失真。
        summary = self._computer_use_full_collect_final_answer_evidence()  # 新增代码+ComputerUseFinalEvidenceSummary：读取本轮真实桌面证据；如果没有这一行，函数无法知道是否需要补强。
        if not bool(summary.get("available", False)):  # 新增代码+ComputerUseFinalEvidenceSummary：没有证据时不改答案；如果没有这一行，普通回答会被追加空摘要。
            return answer  # 新增代码+ComputerUseFinalEvidenceSummary：直接返回原答案；如果没有这一行，无证据场景会继续格式化无意义内容。
        required_tokens = ["computer_use", "screenshot", "real_desktop_touched", "low_level_event_count"]  # 新增代码+ComputerUseFinalEvidenceSummary：列出真实终端验收和用户理解需要的关键字段；如果没有这一行，缺字段判断会散落且不稳定。
        normalized_answer = answer.lower()  # 新增代码+ComputerUseFinalEvidenceSummary：把答案转小写用于包含判断；如果没有这一行，大小写差异会导致重复追加摘要。
        app_token = str(summary.get("target_app", "") or summary.get("app_id", "") or "").lower()  # 新增代码+ComputerUseFinalEvidenceSummary：读取应用字段用于判断答案是否已经提到目标软件；如果没有这一行，已包含 mspaint 的答案也可能被重复补强。
        missing_required = any(token not in normalized_answer for token in required_tokens) or (bool(app_token) and app_token not in normalized_answer)  # 新增代码+ComputerUseFinalEvidenceSummary：判断模型答案是否缺少关键证据；如果没有这一行，所有 Computer Use 答案都会重复追加摘要。
        if not missing_required:  # 新增代码+ComputerUseFinalEvidenceSummary：如果模型已经完整复述证据就不再追加；如果没有这一行，成熟模型回答会被冗余污染。
            return answer  # 新增代码+ComputerUseFinalEvidenceSummary：保持原答案；如果没有这一行，完整答案也会变得啰嗦。
        target_app = str(summary.get("target_app", "") or "unknown_app")  # 新增代码+ComputerUseFinalEvidenceSummary：准备最终摘要中的应用名；如果没有这一行，格式化时可能出现空白应用。
        app_id = str(summary.get("app_id", "") or "unknown_app_id")  # 新增代码+ComputerUseFinalEvidenceSummary：准备最终摘要中的 app_id；如果没有这一行，窗口身份为空时格式不稳定。
        window_id = str(summary.get("window_id", "") or "unknown_window_id")  # 新增代码+ComputerUseFinalEvidenceSummary：准备最终摘要中的 window_id；如果没有这一行，窗口身份行缺少兜底。
        screenshot_path = str(summary.get("screenshot_path", "") or "not_recorded")  # 新增代码+ComputerUseFinalEvidenceSummary：准备最终摘要中的截图路径；如果没有这一行，截图字段缺少可读兜底。
        real_desktop_touched = "true" if bool(summary.get("real_desktop_touched", False)) else "false"  # 新增代码+ComputerUseFinalEvidenceSummary：把真实触碰布尔值格式化成稳定 token；如果没有这一行，验收无法稳定匹配 real_desktop_touched=true。
        low_level_event_count = int(summary.get("low_level_event_count", 0) or 0)  # 新增代码+ComputerUseFinalEvidenceSummary：把低层事件数转成整数；如果没有这一行，None 或字符串会让最终格式不稳定。
        successful_action_count = int(summary.get("successful_action_count", 0) or 0)  # 新增代码+ComputerUseFinalEvidenceSummary：读取成功动作数；如果没有这一行，用户看不到本轮真实动作规模。
        evidence_lines = [  # 新增代码+ComputerUseFinalEvidenceSummary：开始构造可读证据摘要行；如果没有这一行，最终回答无法稳定包含验收字段。
            "",  # 新增代码+ComputerUseFinalEvidenceSummary：追加空行把模型回答和证据摘要隔开；如果没有这一行，短答和摘要会黏在一起难读。
            "Computer Use 真实执行摘要：",  # 新增代码+ComputerUseFinalEvidenceSummary：用中文标题说明下面是证据不是模型内心思考；如果没有这一行，用户不容易区分结论和证据。
            f"- tool_path=computer_use; target_app={target_app}; app_id={app_id}; window_id={window_id}",  # 新增代码+ComputerUseFinalEvidenceSummary：输出工具路径和目标窗口身份；如果没有这一行，验收和用户都无法确认真实应用链路。
            f"- screenshot={str(bool(summary.get('screenshot', False))).lower()}; screenshot_path={screenshot_path}",  # 新增代码+ComputerUseFinalEvidenceSummary：输出截图证据字段；如果没有这一行，最终回答仍可能看不出模型是否观察过屏幕。
            f"- real_desktop_touched={real_desktop_touched}; low_level_event_count={low_level_event_count}; successful_action_count={successful_action_count}",  # 新增代码+ComputerUseFinalEvidenceSummary：输出真实桌面触碰和低层事件数量；如果没有这一行，模拟/真实动作无法在最终回答里区分。
        ]  # 新增代码+ComputerUseFinalEvidenceSummary：证据摘要行列表结束；如果没有这一行，Python 列表语法不完整。
        return answer.rstrip() + "\n".join(evidence_lines)  # 新增代码+ComputerUseFinalEvidenceSummary：把原始答案和证据摘要合并返回；如果没有这一行，补强内容不会进入终端/controller 最终输出。
    # 新增代码+ComputerUseFinalEvidenceSummary：函数段结束，_computer_use_full_final_answer_with_evidence_summary 到此结束；如果没有这个边界说明，读者不容易看出最终回答补强范围。
    ...
    def _computer_use_full_completion_signal_for_action(self, action_name: str, arguments: dict[str, Any]) -> str | None:  # 新增代码+ComputerUseCompletionGate: 函数段开始，在重复真实绘图前给模型最终回答信号；如果没有这段函数，真实 Paint 已经画出图后模型仍可能一直加笔。
        ...
        threshold = self._computer_use_full_completion_action_threshold()  # 修改代码+ComputerUseGoalLock: 根据任务类型选择收束阈值；如果没有这行代码，简单 Paint 绘图仍会等到过多动作后才停止。
    ...
    @staticmethod  # 新增代码+StructuredComputerActionAcceptance: 把动作结果证据抽取做成静态方法；如果没有这行代码，单元测试必须启动完整 agent 才能验证验收字段。
    def _computer_action_acceptance_payload_from_result_data(action_name: str, result_ok: bool, result_message: str, result_data: Any) -> dict[str, Any]:  # 新增代码+StructuredComputerActionAcceptance: 函数段开始，从 computer_action 结果数据生成正式 acceptance payload；如果没有这段函数，真实启动证据只能留在调试日志里。本段到 return 结束。
        data = result_data if isinstance(result_data, dict) else {}  # 新增代码+StructuredComputerActionAcceptance: 只接受字典结果数据；如果没有这行代码，后端返回异常类型时会把事件抽取逻辑撞崩。
        session = data.get("session") if isinstance(data.get("session"), dict) else {}  # 新增代码+StructuredComputerActionAcceptance: 读取通用 Computer Use session 子报告；如果没有这行代码，launch resolver 的嵌套证据会丢失。
        phase108_report = session.get("phase108_report") if isinstance(session.get("phase108_report"), dict) else {}  # 新增代码+StructuredComputerActionAcceptance: 读取应用发现和启动 resolver 报告；如果没有这行代码，start_menu_shortcut 等后端证据拿不到。
        launch_plan = phase108_report.get("launch_plan") if isinstance(phase108_report.get("launch_plan"), dict) else {}  # 新增代码+StructuredComputerActionAcceptance: 读取最终启动计划；如果没有这行代码，显示名和启动后端只能靠脆弱字符串搜索。
        target_window = data.get("target_window") if isinstance(data.get("target_window"), dict) else session.get("target_window") if isinstance(session.get("target_window"), dict) else {}  # 新增代码+StructuredComputerActionAcceptance: 读取已绑定目标窗口；如果没有这行代码，验收无法判断是否真的找到可见窗口。
        launch_backend = str(launch_plan.get("launch_backend") or phase108_report.get("launch_backend") or data.get("launch_backend") or "")  # 新增代码+StructuredComputerActionAcceptance: 抽取真实启动后端；如果没有这行代码，场景无法区分开始菜单、App Paths 或 shell fallback。
        display_name = str(launch_plan.get("display_name") or phase108_report.get("display_name") or phase108_report.get("best_candidate_name") or data.get("target_app") or "")  # 新增代码+StructuredComputerActionAcceptance: 抽取用户可识别应用名；如果没有这行代码，微信这类中文应用会退回调试日志断言。
        target_app = str(data.get("target_app") or session.get("target_app") or display_name)  # 新增代码+StructuredComputerActionAcceptance: 统一输出目标应用字段；如果没有这行代码，消费者需要自己猜 display_name 是否就是目标。
        real_launch_performed = bool(data.get("real_launch_performed") or session.get("real_launch_performed") or phase108_report.get("real_launch_performed") or phase108_report.get("backend_launch_performed"))  # 新增代码+StructuredComputerActionAcceptance: 汇总真实启动完成信号；如果没有这行代码，发现候选但未打开应用也可能误过。
        visible_window_verified = bool(data.get("visible_window_verified") or session.get("visible_window_verified") or phase108_report.get("visible_window_verified") or bool(target_window))  # 新增代码+StructuredComputerActionAcceptance: 汇总可见窗口验证信号；如果没有这行代码，启动进程但窗口不可见也可能被当成功。
        real_desktop_touched = bool(data.get("real_desktop_touched") or session.get("real_desktop_touched") or real_launch_performed or visible_window_verified)  # 新增代码+StructuredComputerActionAcceptance: 汇总真实桌面触达信号；如果没有这行代码，验收无法区分真实桌面路径和纯模拟路径。
        low_level_event_count = int(data.get("low_level_event_count", 0) or 0)  # 新增代码+StructuredComputerActionAcceptance: 输出低层鼠标键盘事件数量；如果没有这行代码，点击输入类动作缺少可量化证据。
        image_result_count = int(data.get("image_result_count", 0) or 0)  # 新增代码+StructuredComputerActionAcceptance: 输出截图结果数量；如果没有这行代码，后续无法判断动作后是否带回视觉证据。
        return {"state_model": "computer_action_structured_evidence", "tool_name": "computer_action", "action": action_name, "ok": bool(result_ok), "message": result_message, "target_app": target_app, "real_desktop_touched": real_desktop_touched, "real_launch_performed": real_launch_performed, "visible_window_verified": visible_window_verified, "launch_backend": launch_backend, "display_name": display_name, "target_window_present": bool(target_window), "low_level_event_count": low_level_event_count, "image_result_count": image_result_count}  # 新增代码+StructuredComputerActionAcceptance: 返回正式 JSONL 可断言 payload；如果没有这行代码，controller 只能继续读取开发调试日志。

    def _emit_computer_action_acceptance_event(self, action_name: str, result: Any) -> None:  # 新增代码+StructuredComputerActionAcceptance: 函数段开始，把 computer_action 结果写入正式 acceptance event；如果没有这段函数，真实动作结果不会进入产品级验收 JSONL。本段到 emit 调用结束。
        payload = self._computer_action_acceptance_payload_from_result_data(action_name, bool(getattr(result, "ok", False)), str(getattr(result, "message", "")), getattr(result, "data", {}))  # 新增代码+StructuredComputerActionAcceptance: 从控制器结果抽取稳定字段；如果没有这行代码，事件只能包含难解析的原始文本。
        emit_acceptance_event("computer_action_result", payload)  # 新增代码+StructuredComputerActionAcceptance: 写入正式动作结果事件；如果没有这行代码，controller 无法在无调试日志的产品模式验证真实启动。
    ...
    def _computer_action(self, arguments: dict[str, Any]) -> str:  # 新增代码+OSComputerUse: 实现 computer_action 高风险动作工具；若没有这段代码，schema 中的桌面动作无法真正分发。
        ...
        self._record_computer_use_runtime_trace("computer_action", {"action": action_name, "ok": bool(result.ok), "message": result.message, "low_level_event_count": result.data.get("low_level_event_count", 0) if isinstance(result.data, dict) else 0, "image_result_count": result.data.get("image_result_count", 0) if isinstance(result.data, dict) else 0})  # 新增代码+RuntimeTrace：记录 action 工具内部执行摘要；如果没有这行代码，真实鼠标键盘动作证据只能散落在 controller 文本里。
        self._emit_computer_action_acceptance_event(action_name, result)  # 新增代码+StructuredComputerActionAcceptance: 把真实动作结果写入正式验收事件；如果没有这行代码，微信打开成功只能靠 debug_log_contains 证明。
    ...
    def _computer_use_compat(self, arguments: dict[str, Any]) -> str:  # 新增代码+Phase49ComputerUseToolSurface: 定义兼容工具入口；如果没有这行代码，executor 映射到 agent._computer_use_compat 时会直接报错。
        ...
        self._record_computer_use_runtime_trace("computer_use_compat_dispatch", {"ok": bool(dispatch.get("ok", False)), "target_tool": dispatch.get("target_tool", ""), "operation": dict(audit_dispatch.get("arguments", {}) or {}).get("operation", "") if isinstance(audit_dispatch.get("arguments", {}), dict) else ""})  # 新增代码+RuntimeTrace：记录兼容工具分发目标；如果没有这行代码，后续无法确认 computer_use/computer-use 是否只是薄兼容层。
        ...
        if target_tool == "computer_discover":  # 新增代码+ComputerDiscoverTool：分发 discover 到只读应用发现工具；如果没有这行代码，兼容入口 operation=discover 会落到未知目标。
            return self._computer_discover(target_arguments)  # 新增代码+ComputerDiscoverTool：调用正式 discover 工具；如果没有这行代码，兼容入口会出现第二套或不可用的应用发现逻辑。
...
def _should_print_dangerous_permission_notice_once() -> bool:  # 修改代码+PermissionNoticeNoiseControl: 函数段开始，判断危险调试提示本进程是否还需要打印；若没有这段代码，长 Computer Use 任务每次自动授权都会重复刷屏。本段只控制人类可见提示，不影响审计事件，段落到 return 结束。
    global _DANGEROUS_PERMISSION_NOTICE_PRINTED  # 修改代码+PermissionNoticeNoiseControl: 声明要更新模块级去重标记；若没有这行代码，函数内赋值只会创建局部变量导致去重失效。
    if _DANGEROUS_PERMISSION_NOTICE_PRINTED:  # 修改代码+PermissionNoticeNoiseControl: 检查危险提示是否已经打印过；若没有这行代码，无法区分第一次和后续自动授权。
        return False  # 修改代码+PermissionNoticeNoiseControl: 已打印过则不再显示；若没有这行代码，终端会继续出现重复危险提示。
    _DANGEROUS_PERMISSION_NOTICE_PRINTED = True  # 修改代码+PermissionNoticeNoiseControl: 标记危险提示已经显示；若没有这行代码，下一次权限请求仍会再次打印。
    return True  # 修改代码+PermissionNoticeNoiseControl: 首次进入时允许打印；若没有这行代码，用户完全看不到当前处于危险调试模式。
...
def ask_permission_from_terminal_customer_mode(action: str, show_progress: bool = True) -> bool:  # 修改代码+真实浏览器客户模式: 客户可见终端默认权限入口，允许 JSON 模式关闭进度输出；若没有这行代码，main() 只能继续使用原人工确认函数或污染 JSON 输出
    ...
    if dangerously_skip_permissions_enabled():  # 新增代码+危险调试权限: 危险模式优先于客户白名单，所有权限都自动允许；若没有这行代码，非 MCP 启动权限仍可能弹出 y/N
        ...
        if show_progress and _should_print_dangerous_permission_notice_once():  # 修改代码+PermissionNoticeNoiseControl: 只在真实人类终端首次展示危险模式提示；若没有这行代码，桌面循环会反复打印同一句授权提示。
            print("Agent > 危险调试模式已开启，本次权限请求已自动允许。", flush=True)  # 修改代码+PermissionNoticeNoiseControl: 明确告诉用户当前是全放开调试模式且只提醒一次；若没有这行代码，用户可能不知道权限已自动放开。
