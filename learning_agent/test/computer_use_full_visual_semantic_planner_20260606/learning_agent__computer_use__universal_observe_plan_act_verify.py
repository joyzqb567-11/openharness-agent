...
try:  # 新增代码+URG4ObservePlanActVerify：优先按 learning_agent 包路径导入已完成的 URG-1/3 能力；如果没有这段代码，单测和真实 bat 入口无法共享实现。
    from learning_agent.computer_use.universal_action_dsl import UniversalActionDslRuntime, run_phase118_universal_action_dsl_contract  # 新增代码+URG4ObservePlanActVerify：导入 URG-3 动作 DSL 和安全合同；如果没有这行代码，闭环会绕过已验收的动作桥。
    from learning_agent.computer_use.universal_real_observation import UniversalRealObservationFrameRuntime  # 新增代码+URG4ObservePlanActVerify：导入 URG-1 ObservationFrame runtime 类型；如果没有这行代码，闭环和观察帧之间没有明确组合关系。
    from learning_agent.computer_use.visual_semantic_planner import phase122_plan_visual_semantic_task  # 新增代码+VisualSemanticPlanner：导入语义视觉规划入口；如果没有这一行，Phase120 planner 仍无法把 house 等意图变成屋顶墙体门窗 primitive。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.universal_action_dsl", "learning_agent.computer_use.universal_real_observation", "learning_agent.computer_use.visual_semantic_planner"}:  # 修改代码+VisualSemanticPlanner：只兜底包路径缺失并允许脚本模式导入语义 planner；如果没有这一行，bat 入口可能因为包前缀无法找到新模块。
        raise  # 新增代码+URG4ObservePlanActVerify：重新抛出非路径类导入错误；如果没有这行代码，排查真实依赖问题会变困难。
    from computer_use.universal_action_dsl import UniversalActionDslRuntime, run_phase118_universal_action_dsl_contract  # type: ignore  # 新增代码+URG4ObservePlanActVerify：脚本模式导入 URG-3 动作桥；如果没有这行代码，bat 入口无法执行闭环。
    from computer_use.universal_real_observation import UniversalRealObservationFrameRuntime  # type: ignore  # 新增代码+URG4ObservePlanActVerify：脚本模式导入 URG-1 观察帧类型；如果没有这行代码，bat 入口缺少观察层引用。
    from computer_use.visual_semantic_planner import phase122_plan_visual_semantic_task  # type: ignore  # 新增代码+VisualSemanticPlanner：脚本模式导入语义视觉规划入口；如果没有这一行，真实终端入口不会使用 house primitive。
...
class Phase119RecordingObservationRuntime:  # 新增代码+URG4ObservePlanActVerify：类段开始，提供安全记录型观察帧 runtime；如果没有这个类，合同自检会触碰真实屏幕状态。
    ...
    def observe(self, target_hint: str = "", real_desktop_touched: bool = False) -> dict[str, Any]:  # 新增代码+URG4ObservePlanActVerify：函数段开始，生成兼容 URG-1 的安全观察帧；如果没有这段函数，闭环没有观察证据来源。
...
class Phase120VisualTaskPlanner(Phase119GenericPlanner):  # 新增代码+VisualPlannerLoop：类段开始，定义接入通用 loop 的视觉任务规划器；如果没有这个类，自然语言绘图仍只能靠静态 actions 或对象特例。
    ...
    def plan(self, task: dict[str, Any], observation_frame: dict[str, Any]) -> list[dict[str, Any]]:  # 新增代码+VisualPlannerLoop：函数段开始，根据自然语言视觉任务和观察帧生成可执行动作；如果没有这段函数，observe 到 act 之间仍没有真实规划。
        ...
        visual_requested = bool(task.get("visual_planning_requested") or task.get("goal") == "natural_language_desktop_task")  # 新增代码+VisualPlannerLoop：判断本任务是否需要视觉规划；如果没有这一行，所有旧样本都会被误改成绘图动作。
...
class UniversalObservePlanActVerifyLoop:  # 新增代码+URG4ObservePlanActVerify：类段开始，组合观察、规划、动作、验证和清理的通用闭环；如果没有这个类，URG-4 只会是一组散落 helper。
    ...
    def _observe(self, target: str) -> dict[str, Any]:  # 新增代码+URG4ObservePlanActVerify：函数段开始，统一调用观察 runtime；如果没有这段函数，任务级和动作级观察会重复散落。
        return dict(self.observation_runtime.observe(target_hint=target, real_desktop_touched=self.real_dispatch_performed))  # 新增代码+URG4ObservePlanActVerify：返回观察帧副本；如果没有这行代码，外部观察对象可能被后续步骤污染。
    ...
    def run_task(self, task: dict[str, Any]) -> dict[str, Any]:  # 新增代码+URG4ObservePlanActVerify：函数段开始，执行一个自然语言任务对应的通用闭环；如果没有这段函数，URG-4 没有可复用入口。
        ...
        for attempt_index in range(self.max_retries + 1):  # 新增代码+URG4ObservePlanActVerify：按首试加有限重试执行；如果没有这行代码，失败任务没有 bounded retry。
            before_frame = self._observe(target)  # 新增代码+URG4ObservePlanActVerify：任务尝试开始前观察；如果没有这行代码，planner 没有当前视觉/UIA 输入。
            ...
            for action in actions:  # 新增代码+URG4ObservePlanActVerify：逐个处理通用 DSL 动作；如果没有这行代码，动作序列无法执行。
                action_before = self._observe(target)  # 新增代码+URG4ObservePlanActVerify：动作前观察目标；如果没有这行代码，无法证明每个动作前重新观察。
                ...
                action_after = self._observe(target)  # 新增代码+URG4ObservePlanActVerify：动作后再次观察目标；如果没有这行代码，verify 没有动作后证据。
            ...
            after_frame = self._observe(target)  # 新增代码+URG4ObservePlanActVerify：整轮动作完成后观察；如果没有这行代码，验证器缺少最终状态。
...
__all__ = ["PHASE119_UNIVERSAL_OBSERVE_PLAN_ACT_VERIFY_MARKER", "PHASE119_UNIVERSAL_OBSERVE_PLAN_ACT_VERIFY_MODEL", "PHASE119_UNIVERSAL_OBSERVE_PLAN_ACT_VERIFY_OK_TOKEN", "PHASE119_REAL_DISPATCH_PERFORMED", "PHASE120_VISUAL_TASK_PLANNER_MODEL", "Phase119GenericPlanner", "Phase119GenericVerifier", "Phase119RecordingObservationRuntime", "Phase120VisualTaskPlanner", "UniversalObservePlanActVerifyLoop", "main", "phase119_universal_loop_cli_line", "run_phase119_universal_loop_contract"]  # 修改代码+VisualPlannerLoop：公开视觉规划器模型名和类；如果没有这一行，adapter 和测试无法稳定导入新 planner。
