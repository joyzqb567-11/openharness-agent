...
try:  # 新增代码+Phase62HighLevelTools: 优先按包路径导入 Computer Use 既有安全链；如果没有这段代码，unittest 和生产入口无法复用项目模块。
    from learning_agent.computer_use.abort_streaming_hooks import WindowsComputerUseAbortStreamingHooks  # 新增代码+Phase62HighLevelTools: 复用 Phase61 abort/cleanup/streaming hooks；如果没有这行代码，高层写动作无法最后一跳急停。
    from learning_agent.computer_use.lock import ComputerUseLockManager, phase30_lock_timestamp  # 新增代码+Phase62HighLevelTools: 复用 durable 桌面锁和时间戳；如果没有这行代码，写动作无法串行化。
    from learning_agent.computer_use.persistent_grants import DEFAULT_PERSISTENT_GRANT_SESSION_ID, WindowsComputerUsePersistentGrantStore  # 新增代码+Phase62HighLevelTools: 复用 Phase60 持久授权；如果没有这行代码，高层写动作会绕过审批。
    from learning_agent.computer_use.real_sendinput_guard import Phase58RecordingLowLevelSender, Phase58StaticSafeWindowObserver, WindowsRealSendInputGuardRuntime  # 新增代码+Phase62HighLevelTools: 复用 Phase58 目标守卫和低层 sender；如果没有这行代码，高层 click/type 会另开危险实现。
    from learning_agent.computer_use.real_uia_locator import SemanticControlLocator  # 新增代码+Phase62HighLevelTools: 复用 Phase57 语义控件定位器；如果没有这行代码，高层 find_control 会重复造 locator。
    from learning_agent.computer_use.windows_backend import StaticWindowsWindowInventory  # 新增代码+Phase62HighLevelTools: 复用静态 inventory 让合同不触碰真实桌面；如果没有这行代码，单测可能依赖用户窗口。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.abort_streaming_hooks", "learning_agent.computer_use.lock", "learning_agent.computer_use.persistent_grants", "learning_agent.computer_use.real_sendinput_guard", "learning_agent.computer_use.real_uia_locator", "learning_agent.computer_use.windows_backend", "learning_agent.core", "learning_agent.core.messages", "learning_agent.runtime", "learning_agent.runtime.files", "learning_agent.tools", "learning_agent.tools.streaming_executor"}:  # 新增代码+Phase62HighLevelTools: 只允许包路径缺失时 fallback；如果没有这行代码，内部真实 bug 会被误吞。
        raise  # 新增代码+Phase62HighLevelTools: 重新抛出非路径类导入错误；如果没有这行代码，排查底层模块会很困难。
    from computer_use.abort_streaming_hooks import WindowsComputerUseAbortStreamingHooks  # 新增代码+Phase62HighLevelTools: 脚本模式导入 Phase61 hooks；如果没有这行代码，bat 入口无法显示高层 streaming 集成。
    from computer_use.lock import ComputerUseLockManager, phase30_lock_timestamp  # 新增代码+Phase62HighLevelTools: 脚本模式导入锁管理器；如果没有这行代码，bat 入口高层写动作无法串行。
    from computer_use.persistent_grants import DEFAULT_PERSISTENT_GRANT_SESSION_ID, WindowsComputerUsePersistentGrantStore  # 新增代码+Phase62HighLevelTools: 脚本模式导入持久授权；如果没有这行代码，bat 入口无法评估 grant。
    from computer_use.real_sendinput_guard import Phase58RecordingLowLevelSender, Phase58StaticSafeWindowObserver, WindowsRealSendInputGuardRuntime  # 新增代码+Phase62HighLevelTools: 脚本模式导入 Phase58 安全链；如果没有这行代码，高层写动作无法执行合同。
    from computer_use.real_uia_locator import SemanticControlLocator  # 新增代码+Phase62HighLevelTools: 脚本模式导入语义 locator；如果没有这行代码，find_control 不可用。
    from computer_use.windows_backend import StaticWindowsWindowInventory  # 新增代码+Phase62HighLevelTools: 脚本模式导入静态 inventory；如果没有这行代码，合同自检会触碰真实桌面。
