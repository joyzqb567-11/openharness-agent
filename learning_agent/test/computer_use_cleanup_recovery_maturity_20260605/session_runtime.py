...
try:  # 新增代码+Phase40WindowsAbortCleanup: 优先按包模式导入锁和文件工具；如果没有这行代码，正常 `python -m` 入口无法复用既有 durable 文件能力。
    ...
    from learning_agent.computer_use.owned_resource_registry import OwnedResourceRegistry  # 修改代码+CleanupRecoveryMaturity：导入 Task 4 自有资源注册表；如果没有这一行，stop/abort 只能清锁不能清理本 agent 启动的资源。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.audit", "learning_agent.computer_use.lock", "learning_agent.computer_use.owned_resource_registry", "learning_agent.runtime", "learning_agent.runtime.files"}:  # 修改代码+CleanupRecoveryMaturity：允许 owned_resource_registry 在脚本模式 fallback；如果没有这一行，bat 入口下 Task 4 registry 导入会失败。
    ...
    from computer_use.owned_resource_registry import OwnedResourceRegistry  # type: ignore  # 修改代码+CleanupRecoveryMaturity：脚本模式导入同一自有资源注册表；如果没有这一行，双击 bat 后 stop/abort 不能调用 registry。
...
class WindowsComputerUseSessionRuntime:  # 新增代码+Phase40WindowsAbortCleanup: 定义 Windows Computer Use 会话运行时；如果没有这个类，abort、cleanup 和通知会继续散落在终端命令和锁管理器里。
    ...
    def request_global_abort(self, reason: str, source: str = "runtime") -> dict[str, Any]:  # 新增代码+Phase40WindowsAbortCleanup: 函数段开始，请求全局 abort 并记录通知；如果没有这段函数，abort 只有底层 flag 而没有 ClaudeCode 风格的用户可见事件。
        ...
        return {"model": self.model, "marker": PHASE40_WINDOWS_ABORT_CLEANUP_MARKER, "abort_requested": bool(status.get("abort_requested")), "status": status, "owned_resource_cleanup": owned_cleanup, "owned_resource_cleanup_completed": bool(owned_cleanup.get("cleanup_completed", False)), "residual_owned_process": bool(owned_cleanup.get("residual_owned_process", False)), "notification": notification, "notification_count": len(notifications), "actions_expanded": PHASE40_ACTIONS_EXPANDED}  # 修改代码+CleanupRecoveryMaturity: 返回结构化 abort 结果并上浮自有资源清理状态；如果没有这一行，终端和测试看不到急停是否收尾资源。
