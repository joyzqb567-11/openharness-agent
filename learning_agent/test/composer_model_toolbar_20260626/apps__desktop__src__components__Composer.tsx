...
export type ComposerModelOption = { id: string; label: string; providerId: string; providerName: string; supportsTools: boolean; supportsVision: boolean }; // 新增代码+ComposerModelToolbar：定义底部模型选项；如果没有这行，Provider 设置页的模型无法安全传给 Composer。
...
export function composerModelMenuLabel(options: ComposerModelOption[], selectedModelId = ""): string { // 新增代码+ComposerModelToolbar：函数段开始，计算模型菜单显示文案；如果没有这段，空状态和选中状态无法被单测锁住。
  return selectedComposerModelOption(options, selectedModelId)?.label ?? "选择模型"; // 新增代码+ComposerModelToolbar：显示选中模型名或清晰空状态；如果没有这行，未连接 provider 时用户看不懂模型区域。
      ...
      <div className="composer-frame"> {/* 新增代码+ComposerModelToolbar：定义 Codex 式圆角输入框外壳；如果没有这行，底部控件会分散成普通表单。 */}
        ...
        <div className="composer-toolbar"> {/* 新增代码+ComposerModelToolbar：定义底部工具条；如果没有这行，权限、模型、推理和语音入口没有容器。 */}
          ...
          <div className="composer-toolbar-right"> {/* 新增代码+ComposerModelToolbar：定义右侧工具组；如果没有这行，模型、推理、语音和发送无法靠右排列。 */}
            <label className="composer-select-shell composer-model-shell" title={activeModel ? `${activeModel.providerName} · ${activeModel.label}` : "请先在设置里连接提供商"}> {/* 新增代码+ComposerModelToolbar：渲染模型下拉外壳；如果没有这行，用户无法在 OAuth 后选择模型。 */}
              <select data-testid="composer-model-select" aria-label="选择模型" value={activeModelId} disabled={modelOptions.length === 0} onChange={(event) => onModelChange?.(event.currentTarget.value)}> {/* 新增代码+ComposerModelToolbar：渲染模型 select；如果没有这行，provider catalog 模型无法被用户选择。 */}
                {modelOptions.length === 0 ? <option value="">选择模型</option> : modelOptions.map((option) => <option key={`${option.providerId}:${option.id}`} value={option.id}>{option.label}</option>)} {/* 新增代码+ComposerModelToolbar：渲染模型空态或真实模型列表；如果没有这行，未连接和已连接两种状态都不可见。 */}
