...
export type ProviderModelView = { id: string; displayName: string; providerId: string; visible: boolean; supportsTools: boolean; supportsVision: boolean }; // 新增代码+ProviderSettingsStore：定义前端模型 view model；如果没有这行，Models 页字段名会散落。
...
export type ComposerModelOption = { id: string; label: string; providerId: string; providerName: string; supportsTools: boolean; supportsVision: boolean }; // 新增代码+ComposerModelToolbar：定义底部 Composer 模型选项；如果没有这行，设置页模型无法以稳定形状传给底部模型菜单。
...
function modelView(model: GuiModelInfo): ProviderModelView { // 新增代码+ProviderSettingsStore：函数段开始，把后端模型转成前端字段；如果没有这段，Models 页要理解 snake_case。
  return { id: model.id, displayName: model.display_name, providerId: model.provider_id, visible: Boolean(model.visible), supportsTools: Boolean(model.supports_tools), supportsVision: Boolean(model.supports_vision) }; // 新增代码+ProviderSettingsStore：返回模型 view model；如果没有这行，模型字段名无法统一。
