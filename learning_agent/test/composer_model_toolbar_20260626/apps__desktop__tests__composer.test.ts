...
import { canSubmitComposerDraft, Composer, composerButtonState, composerKeyIntent, composerModelMenuLabel, submitComposerDraft } from "../src/components/Composer"; // 修改代码+ComposerModelToolbarTest：导入 Composer 和模型菜单 helper；如果没有这行，测试无法锁定底部模型选择入口。
...
describe("Composer input rules", () => { // 新增代码+DesktopComposerV2Test：测试段开始，覆盖底部输入体验；如果没有这段，中文多行和运行态容易回归。
  ...
  it("renders Codex-style bottom controls for access, model, reasoning, voice, and send", () => { // 新增代码+ComposerModelToolbarTest：测试底部工具条必须有 Codex 风格关键入口；如果没有这段，OAuth 连接后用户仍找不到模型选择。
    const markup = renderToStaticMarkup(createElement(Composer, { modelOptions: [{ id: "gpt-4.1", label: "GPT-4.1", providerId: "openai", providerName: "OpenAI", supportsTools: true, supportsVision: true }], selectedModelId: "gpt-4.1" })); // 修改代码+ComposerModelToolbarTest：用 createElement 渲染带 OpenAI 模型的 Composer；如果没有这行，.ts 文件里的 JSX 会导致测试语法错误。
  ...
  it("uses the selected model label and falls back to a clear empty state", () => { // 新增代码+ComposerModelToolbarTest：测试模型按钮文案规则；如果没有这段，空模型或选中模型显示可能退化。
    const options = [{ id: "gpt-4.1", label: "GPT-4.1", providerId: "openai", providerName: "OpenAI", supportsTools: true, supportsVision: true }]; // 新增代码+ComposerModelToolbarTest：准备一个 OpenAI 模型选项；如果没有这行，helper 没有输入。
