...
class GuiBridgeLifecycleContractTests(unittest.TestCase):  # 新增代码+DesktopGUILifecycleTest：测试类段开始，锁定 GUI turn 生命周期；如果没有这个类，unittest 不会执行这些合同。
    ...
    def test_message_request_records_composer_model_context(self) -> None:  # 新增代码+ComposerModelToolbarTest：测试方法开始，确认底部模型和权限选择进入 bridge 事件；如果没有这段，Composer 下拉可能只是前端装饰。
        with tempfile.TemporaryDirectory() as directory:  # 新增代码+ComposerModelToolbarTest：创建隔离工作区；如果没有这行，测试会污染真实 memory。
            server = self._start_server(Path(directory))  # 新增代码+ComposerModelToolbarTest：启动带 token 的 GUI bridge；如果没有这行，测试无法走真实 HTTP 生命周期。
            try:  # 新增代码+ComposerModelToolbarTest：确保 server 最终关闭；如果没有这行，失败时端口可能残留。
                first = self._post_json(server, "/v1/gui/messages", {"conversation_id": "default", "prompt": "请使用选中的模型回答", "provider_id": "openai", "model_id": "gpt-4.1", "reasoning_effort": "high", "permission_mode": "full-access"})  # 新增代码+ComposerModelToolbarTest：发送带模型上下文的 GUI prompt；如果没有这行，后端合同没有真实输入。
                turn_id = str(first["turn_id"])  # 新增代码+ComposerModelToolbarTest：保存 turn id；如果没有这行，后续无法定位 queued 事件。
                queued_event = self._wait_for_event(server, "gui_turn_queued", turn_id)  # 新增代码+ComposerModelToolbarTest：等待 queued 事件；如果没有这行，无法确认 bridge 是否记录本轮模型上下文。
            finally:  # 新增代码+ComposerModelToolbarTest：清理 server；如果没有这行，测试失败会留下后台服务。
                server.shutdown()  # 新增代码+ComposerModelToolbarTest：停止 serve_forever；如果没有这行，测试进程可能挂住。
                server.server_close()  # 新增代码+ComposerModelToolbarTest：释放 socket；如果没有这行，Windows 端口可能短时间占用。
        self.assertEqual(queued_event["payload"]["provider_id"], "openai")  # 新增代码+ComposerModelToolbarTest：确认 provider 选择进入事件；如果没有这行，后续 runtime 无法知道用户选了哪个 provider。
        self.assertEqual(queued_event["payload"]["model_id"], "gpt-4.1")  # 新增代码+ComposerModelToolbarTest：确认模型选择进入事件；如果没有这行，OAuth 连接后仍没有可追踪模型选择。
        self.assertEqual(queued_event["payload"]["reasoning_effort"], "high")  # 新增代码+ComposerModelToolbarTest：确认推理强度进入事件；如果没有这行，推理菜单无法影响后续执行。
        self.assertEqual(queued_event["payload"]["permission_mode"], "full-access")  # 新增代码+ComposerModelToolbarTest：确认权限模式进入事件；如果没有这行，完全访问下拉无法被后端审计。
    # 新增代码+ComposerModelToolbarTest：测试方法结束，底部模型上下文合同到此结束；如果没有边界说明，初学者不易看出覆盖范围。
