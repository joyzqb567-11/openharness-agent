...
class GuiRunManager:  # 新增代码+DesktopGUILifecycle：类段开始，管理 GUI turn 生命周期；如果没有这个类，handler 会堆满业务状态逻辑。
    ...
    def _default_agent_adapter(self) -> GuiAgentAdapter:  # 新增代码+真实模型GUI桥接：函数段开始，按模式创建默认 GUI adapter；如果没有这段，__init__ 会堆入真实/fake 分支细节。
        if self.agent_adapter_mode == "real" or (self.agent_adapter_mode == "auto" and self._openai_real_provider_connected()):  # 修改代码+真实模型GUI桥接：显式 real 或 auto 且 provider 已连接时启用真实模型；如果没有这一行，登录成功后主聊天仍会 fake。
        ...
        self.agent_adapter_mode = "fake"  # 新增代码+真实模型GUI桥接：auto 未连接或显式 fake 时固化为 fake；如果没有这一行，后续请求模式可能保留 auto 这种前端不认识的值。
    ...
    def _openai_real_provider_connected(self) -> bool:  # 修改代码+DirectOAuthModelFactory：函数段开始，读取 OpenAI provider 是否具备真实模型连接来源；如果没有这段，真实模型模式会绕过设置页连接状态。
        ...
        for provider in providers:  # 新增代码+真实模型GUI桥接：遍历所有 provider；如果没有这一行，无法找到 OpenAI 条目。
            ...
            if provider.get("id") != "openai":  # 新增代码+真实模型GUI桥接：只关心 OpenAI provider；如果没有这一行，Anthropic 等 provider 可能误打开 Codex CLI 模型路径。
    ...
    def _run_adapter_turn(self, turn: GuiTurn) -> GuiAgentRunResult:  # 新增代码+GuiAgentAdapter：函数段开始，把 GUI turn 转交给 adapter；如果没有这段，worker 只能调用旧 answer_runner。
        ...
        request_mode = "real" if isinstance(adapter, DefaultHarnessGuiAgentAdapter) or self.agent_adapter_mode == "real" else "fake"  # 修改代码+真实模型GUI桥接：根据真实 adapter 或旧 harness shell 标记请求模式；如果没有这一行，real 模式事件会被误标成 fake。
