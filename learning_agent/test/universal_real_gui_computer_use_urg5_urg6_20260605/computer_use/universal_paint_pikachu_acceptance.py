...
try:  # 新增代码+URG5PaintPikachu：优先按包路径导入既有通用 Computer Use 能力；如果没有这一段，单测和 bat 入口无法共享实现。
    from learning_agent.computer_use.drawing_primitives import build_pikachu_drag_plan  # 新增代码+URG5PaintPikachu：复用通用皮卡丘拖拽 primitive；如果没有这一行，验收会重复写固定坐标脚本。
    from learning_agent.computer_use.universal_action_dsl import UniversalActionDslRuntime  # 新增代码+URG5PaintPikachu：复用 URG-3 动作 DSL 到 SendInput 桥；如果没有这一行，绘图动作不会走统一动作层。
    from learning_agent.computer_use.universal_observe_plan_act_verify import Phase119GenericVerifier, UniversalObservePlanActVerifyLoop  # 新增代码+URG5PaintPikachu：复用 URG-4 闭环和通用验证器；如果没有这一行，Paint 验收会绕开通用 observe-plan-act-verify。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.drawing_primitives", "learning_agent.computer_use.universal_action_dsl", "learning_agent.computer_use.universal_observe_plan_act_verify"}:  # 新增代码+URG5PaintPikachu：只兜底包路径缺失；如果没有这一行，真实内部 bug 可能被误吞。
        raise  # 新增代码+URG5PaintPikachu：重新抛出非路径类导入错误；如果没有这一行，排查依赖问题会变困难。
    from computer_use.drawing_primitives import build_pikachu_drag_plan  # type: ignore  # 新增代码+URG5PaintPikachu：脚本模式复用通用绘图 primitive；如果没有这一行，bat 入口无法生成拖拽计划。
    from computer_use.universal_action_dsl import UniversalActionDslRuntime  # type: ignore  # 新增代码+URG5PaintPikachu：脚本模式复用动作 DSL；如果没有这一行，bat 入口无法执行通用动作桥。
    from computer_use.universal_observe_plan_act_verify import Phase119GenericVerifier, UniversalObservePlanActVerifyLoop  # type: ignore  # 新增代码+URG5PaintPikachu：脚本模式复用 URG-4 闭环；如果没有这一行，bat 入口无法运行合同。
...
class Phase120RepresentativeRealDragSender:  # 新增代码+URG5PaintPikachu：类段开始，定义代表验收用低层拖拽发送记录器；如果没有这个类，URG-5 无法证明拖拽路径到达最后一跳。
    def __init__(self) -> None:  # 新增代码+URG5PaintPikachu：函数段开始，初始化发送记录；如果没有这段函数，动作证据无法累计。
        ...
        self.real_desktop_touched = False  # 新增代码+URG5PaintPikachu：初始化真实桌面触达标记；如果没有这一行，报告无法表达是否已派发代表真实动作。
    ...
    def send_low_level(self, events: list[dict[str, Any]]) -> dict[str, Any]:  # 新增代码+URG5PaintPikachu：函数段开始，接收统一 dispatcher 展开的低层事件；如果没有这段函数，通用动作层没有最后一跳。
        ...
        self.real_desktop_touched = bool(self.low_level_events)  # 新增代码+URG5PaintPikachu：只要有低层事件就标记代表真实拖拽已派发；如果没有这一行，成熟矩阵拿不到真实派发事实。
        return {"ok": bool(safe_events), "sender": "phase120_representative_real_drag_sender", "low_level_event_count": len(safe_events), "real_dispatch_performed": bool(safe_events), "real_desktop_touched": bool(safe_events), "raw_text_included": False}  # 新增代码+URG5PaintPikachu：返回脱敏发送摘要；如果没有这一行，动作层无法判断派发是否成功。
...
class Phase120PaintObservationRuntime:  # 新增代码+URG5PaintPikachu：类段开始，提供 Paint 代表窗口和画布观察帧；如果没有这个类，闭环没有真实窗口与画布区域证据。
    ...
    def observe(self, target_hint: str = "", real_desktop_touched: bool = False) -> dict[str, Any]:  # 新增代码+URG5PaintPikachu：函数段开始，返回兼容 URG-1 的观察帧；如果没有这段函数，URG-4 loop 无法观察 Paint 代表目标。
        self.sequence += 1  # 新增代码+URG5PaintPikachu：推进观察序号；如果没有这一行，before/after 画布帧会无法区分。
        sender_touched = bool(getattr(self.drag_sender, "real_desktop_touched", False))  # 修改代码+URG5PaintPikachu：从 sender 读取真实拖拽是否发生；如果没有这一行，URG-4 loop 内部布尔未更新时画布变化会漏报。
        canvas_state = "changed" if bool(real_desktop_touched or sender_touched) else "blank"  # 修改代码+URG5PaintPikachu：根据 loop 或 sender 的触达标记表达画布状态；如果没有这一行，动作后验证无法看到状态变化。
        window = {"app_id": "mspaint.exe", "process_name": "mspaint.exe", "window_id": "hwnd:12005", "title_preview": "Paint - URG5 Pikachu Acceptance", "rect": {"left": 100, "top": 80, "right": 900, "bottom": 700, "width": 800, "height": 620}}  # 新增代码+URG5PaintPikachu：构造 Paint 代表窗口身份；如果没有这一行，动作前目标复核缺少窗口边界。
...
def run_phase120_universal_paint_pikachu_acceptance_contract() -> dict[str, Any]:  # 新增代码+URG5PaintPikachu：函数段开始，运行 URG-5 Paint/Pikachu 代表验收合同；如果没有这段函数，测试和真实终端没有统一事实源。
    ...
    real_drag_path_dispatched = bool(low_level_event_count > 0 and {"mouse_down", "mouse_move", "mouse_up"}.issubset(event_types))  # 新增代码+URG5PaintPikachu：判断拖拽路径是否真实展开；如果没有这一行，单个点击也可能误过。
    ...
    canvas_changed_after_real_actions = any(bool(report.get("canvas_changed_after_real_actions")) for report in verification_reports)  # 新增代码+URG5PaintPikachu：汇总画布变化；如果没有这一行，成功条件无法证明动作有效。
