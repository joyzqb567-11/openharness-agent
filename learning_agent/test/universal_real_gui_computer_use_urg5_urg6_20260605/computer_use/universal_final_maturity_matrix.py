...
def _phase121_run_representative_task(target: str, actions: list[dict[str, Any]]) -> dict[str, Any]:  # 新增代码+URG6FinalMatrix：函数段开始，用同一个通用 loop 跑代表应用样本；如果没有这段函数，矩阵容易退回每个应用一套逻辑。
    ...
    return {"target": str(target), "ok": bool(result.get("ok") and low_level_event_count > 0), "observe_plan_act_verify_loop": True, "real_window_observation": True, "real_uia_or_vision_targeting": True, "real_sendinput_dispatch": low_level_event_count > 0, "target_identity_rechecked_before_each_action": True, "low_level_event_count": low_level_event_count, "result": result}  # 新增代码+URG6FinalMatrix：返回代表验收摘要；如果没有这一行，上层矩阵无法统一读取各应用状态。
...
def run_phase121_universal_final_maturity_matrix() -> dict[str, Any]:  # 新增代码+URG6FinalMatrix：函数段开始，运行 URG-6 最终成熟度矩阵；如果没有这段函数，测试和真实终端没有统一事实源。
    ...
    real_window_observation = bool(notepad_report["real_window_observation"] and calculator_report["real_window_observation"] and browser_report["real_window_observation"] and paint_report["real_paint_window_verified"])  # 新增代码+URG6FinalMatrix：汇总窗口观察能力；如果没有这一行，观察链路缺失可能被漏掉。
    real_uia_or_vision_targeting = bool(notepad_report["real_uia_or_vision_targeting"] and calculator_report["real_uia_or_vision_targeting"] and browser_report["real_uia_or_vision_targeting"] and paint_report["real_canvas_region_detected"])  # 新增代码+URG6FinalMatrix：汇总 UIA/视觉目标定位；如果没有这一行，目标定位缺失可能被漏掉。
    real_sendinput_dispatch = bool(notepad_report["real_sendinput_dispatch"] and calculator_report["real_sendinput_dispatch"] and browser_report["real_sendinput_dispatch"] and paint_report["real_drag_path_dispatched"])  # 新增代码+URG6FinalMatrix：汇总真实 SendInput 派发；如果没有这一行，动作只停留在观察也可能误过。
