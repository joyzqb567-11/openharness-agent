# Active Plan: Phase92 Universal Windows Computer Use Mode

> 当前最新蓝图以 `docs/superpowers/specs/2026-06-04-phase92-universal-windows-computer-use-mode-blueprint.md` 为准。当前实施计划以 `docs/superpowers/plans/2026-06-04-phase92-universal-windows-computer-use-mode.md` 为准。该计划是对 Phase90/91 路线的架构纠偏：代表性应用只作为验收样本，主架构必须是一个通用 Windows Computer Use Runtime。

## Active Success Criteria

- [x] 明确拒绝“每个软件一个专用控制器”的路线。
- [x] 明确 Notepad/Paint/Explorer/Browser 只作为代表性验收样本。
- [x] 规划一个单一 `UniversalWindowsComputerUseRuntime`。
- [x] 规划 prompt -> observe -> plan -> act -> verify -> recover 通用闭环。
- [x] 规划 `computer_use` 兼容入口支持 `operation=mode` 或 `operation=run_prompt`。
- [x] 保留高风险确认、授权、急停、目标复验、证据链和默认真实动作关闭。
- [x] 生成 Phase92 产品蓝图和实施计划。
- [x] 实现一个单一 `UniversalWindowsComputerUseRuntime`，组合观察融合、prompt 规划、通用动作层、安全边界和生产 host adapter。
- [x] `computer_use` / `computer-use` 兼容入口已支持 `operation=mode` 和 `operation=run_prompt`。
- [x] Phase92 自动化测试、Windows Computer Use 回归、编译检查和真实可见终端验收已通过。

## Active Phases

- [x] Phase92 Task 1：Red Test For Universal Runtime。
- [x] Phase92 Task 2：Implement Universal Runtime Contract。
- [x] Phase92 Task 3：Add Computer Use Mode Tool Route。
- [x] Phase92 Task 4：Add Acceptance Scenario。
- [x] Phase92 Task 5：Verification。
- [x] Phase92 Task 6：Records And Learning Backup。

## Active Stop Conditions

- 未经用户明确确认，不安装 OCR、视觉模型、驱动、系统服务、全局 hook。
- 未经用户明确确认，不修改 Windows 注册表、UAC、安全策略、系统设置或默认应用。
- 遇到密码、验证码、支付、登录 token、API key、管理员、安全窗口、终端危险命令或隐私数据时必须停止并请求确认。
- 未完成自动化测试和 `learning_agent/start_oauth_agent.bat` 真实可见终端交互验收时，不能声明 Phase92 开发完成。

---

# Active Plan: Phase65-75 Humanlike Windows Operator

> 当前最新蓝图以 `docs/superpowers/specs/2026-06-03-humanlike-windows-operator-blueprint.md` 为准。当前实施计划以 `docs/superpowers/plans/2026-06-03-phase65-75-humanlike-windows-operator.md` 为准。该计划已纳入用户新增最终标准：做到拟人级 Windows Computer Use，用户能手动操作的普通本机应用，`learning_agent` 的 agent 也要能通过 prompt 观察、理解、执行、校验和恢复。

## Active Success Criteria

- [x] 把“拟人操作所有普通本机应用”写成 Phase65-75 最终目标。
- [x] 明确不走“每个应用一个脚本”的路线，而走通用观察、规划、动作、校验、恢复闭环。
- [x] 明确代表性真实应用 E2E 用于证明通用性，不要求每个应用单独 E2E。
- [x] 把真实 `mspaint.exe` 画图软件绘制简化皮卡丘/黄色卡通电气鼠加入 Phase74 代表性场景。
- [x] 明确 Paint 场景必须通过真实鼠标键盘操作画图软件，不能直接生成图片文件作弊。
- [x] 明确 Phase75 最终 token 必须包含 `mspaint_pikachu_scenario=true`、`real_paint_app_control=true`、`humanlike_drawing_actions=true`、`direct_image_file_cheat=false`。
- [x] 生成 Phase65-75 正式实施计划：`docs/superpowers/plans/2026-06-03-phase65-75-humanlike-windows-operator.md`。
- [x] Phase75 最终 Humanlike Windows Operator 矩阵已完成；下一步若继续推进，应另起 live smoke / 生产级真实调度计划。

## Active Phases

- [x] Phase65: Universal Operator Contract。
- [x] Phase66: Observation Fusion。
- [x] Phase67: Prompt Task Planner。
- [x] Phase68: Closed-Loop Executor。
- [x] Phase69: App Launch, Focus, And Window Switching。
- [x] Phase70: Generic Click, Type, And Control Actions。
- [x] Phase71: Generic Hotkey, Menu, Scroll, And Drag。
- [x] Phase72: Safety Boundary For Real Apps。
- [x] Phase73: App Memory And Self-Learning。
- [x] Phase74: Representative Real App E2E Matrix, including Paint Pikachu。
- [x] Phase75: Humanlike Windows Operator Final Matrix。

## Active Stop Conditions

- 需要安装 OCR、视觉模型、驱动、全局 hook 或系统服务时必须停下请求确认。
- 需要修改 Windows 设置、注册表、UAC、安全策略或默认应用时必须停下请求确认。
- 遇到密码、验证码、支付、登录 token、管理员、安全窗口、终端危险命令或真实用户隐私数据时必须停下请求确认。
- 如果无法完成 `learning_agent/start_oauth_agent.bat` 真实可见终端交互验收，不能声明开发完成。
...
- [x] Phase 61：Global abort, hotkey, cleanup, and streaming hooks。
- [x] Phase 62：High-level Computer Tool API and streaming integration。
- [x] Phase 63：External agent controller takeover and debug surface。
- [x] Phase 64：Final parity plus production matrix。
...
# 当前任务：Phase76-89 Windows Computer Use Production Live-Control（2026-06-04）

## 任务目标

补齐 ClaudeCode Computer Use 对齐后剩余约 35% 的生产闭环差距，把 Windows Computer Use 组织成统一 Host Adapter、观察融合、坐标模型、SendInput 门禁、剪贴板保护、应用启动计划、权限哨兵、全局中止、轮次清理、高层工具面、observe-act-verify 闭环、代表性 E2E 矩阵和真实可见终端验收。

## 阶段状态

- [x] Phase76 ClaudeCode 对齐矩阵。
- [x] Phase77 统一 Windows Host Adapter。
- [x] Phase78 观察融合合同。
- [x] Phase79 坐标和显示模型。
- [x] Phase80 SendInput 生产门禁。
- [x] Phase81 剪贴板保存、验证、恢复。
- [x] Phase82 应用启动和聚焦计划。
- [x] Phase83 allowlist、sentinel、权限门。
- [x] Phase84 全局中止信号。
- [x] Phase85 轮次清理合同。
- [x] Phase86 高层工具面。
- [x] Phase87 observe-act-verify 闭环。
- [x] Phase88 代表性 E2E 矩阵，包含 Paint 皮卡丘。
- [x] Phase89 真实可见终端验收场景。

## 验收结果

- [x] `py_compile` 通过。
- [x] Focused Phase76-89 tests: 4 OK。
- [x] Phase75 + Phase76-89 regression: 7 OK。
- [x] Windows Computer Use tests: 205 OK。
- [x] `python -m compileall learning_agent` 通过。
- [x] CLI self-check 打印固定 Phase76-89 成功 token。
- [x] 真实可见 `start_oauth_agent.bat` 终端验收通过：`learning_agent/acceptance_controller/runs/agent_capability_phase76_89_windows_live_control-20260604_092149/result.json`。

## 当前边界

Phase76-89 是安全合同模式下的生产闭环，不扩张无保护动作面，不打开 Paint live，不绕过权限门。真实任意应用控制必须继续走授权、目标识别、中止、清理和真实可见终端验收。

# 当前任务：Phase90 Windows Live App Dispatcher（2026-06-04）

## 任务目标

新增受控 live app dispatcher，把 Phase60 持久授权、Phase69 应用启动/聚焦、Phase71 输入事件、Phase72 安全边界、Phase74 Paint 皮卡丘计划和 Phase58 安全窗口真实 smoke 路径串成一个可复用入口。

## 阶段状态

- [x] 读取 Phase58/60/69/71/72/74/76-89 相关源码。
- [x] 先写 Phase90 失败测试并确认红灯。
- [x] 实现 `WindowsLiveAppDispatcher`。
- [x] 接入包级公开导出。
- [x] 新增真实可见终端验收场景。
- [x] 更新 agent_memory 和任务计划。
- [x] 备份新增/修改文件到 `learning_agent/test/`。

## 验收结果

- [x] TDD red：`ModuleNotFoundError: No module named 'learning_agent.computer_use.live_app_dispatcher'`。
- [x] `py_compile` 通过。
- [x] Focused Phase90 tests: 4 OK。
- [x] Phase76-90 regression: 8 OK。
- [x] Windows Computer Use tests: 209 OK。
- [x] `python -m compileall learning_agent` 通过。
- [x] CLI self-check 打印固定 Phase90 成功 token。
- [x] 真实可见 `start_oauth_agent.bat` 终端验收通过：`learning_agent/acceptance_controller/runs/agent_capability_phase90_live_app_dispatcher-20260604_093939/result.json`。

## 当前边界

Phase90 已经有受控 live app dispatcher 路径，但默认仍是 recording/safe-contract dispatch，不做无保护任意应用裸控。下一阶段若继续推进，应做显式授权下的真实 Notepad 或 Paint live smoke。

## 2026-06-04 Phase93 Universal Live Execution Gate Follow-up

Status: implementation code added; automated checks passed; final visible-terminal gate retested and passed.

Completed:
- Implemented a universal live execution gate on top of Phase92.
- Added tests, package export, CLI token line, and acceptance scenario.
- Verified Phase93 focused tests, Phase92+Phase93 regression, Windows Computer Use discovery, source-directory compileall, and escalated CLI self-check.

Resolved issue:
- The first required real visible `start_oauth_agent.bat` acceptance scenario failed because the agent returned `OAuth state 不匹配` before tool execution.
- After the OAuth/model chain repair, the scenario passed at `learning_agent/acceptance_controller/runs/agent_capability_phase93_universal_live_execution_gate-20260604_203855/result.json`.

Next required step:
- Use Phase93 as the accepted universal gate baseline for future physical dispatch expansion.
- Keep explicit authorization, target identity, abort, cleanup, and visible terminal proof before enabling any real desktop input path.

# 当前任务：Phase94 Authorized Real Dispatch Candidate（2026-06-04）

## 任务目标

在 Phase93 通用门禁之后补上授权后低层派发候选层，证明安全授权目标可以产生低层事件并到达注入 sender，同时保持默认不做真实物理桌面输入。

## 阶段状态

- [x] 读取 Phase60/72/93/28 等相关源码。
- [x] 先写 Phase94 失败测试并确认红灯。
- [x] 实现 `WindowsAuthorizedRealDispatchCandidate`。
- [x] 接入包级公开导出。
- [x] 新增真实可见终端验收场景。
- [x] 更新 agent_memory 和任务计划。
- [x] 备份新增/修改文件到 `learning_agent/test/`。
- [x] 完成真实可见 `start_oauth_agent.bat` 终端验收。

## 验收结果

- [x] TDD red：`ModuleNotFoundError: No module named 'learning_agent.computer_use.authorized_real_dispatch'`。
- [x] Focused Phase94 tests: 4 OK。
- [x] Phase93 + Phase94 regression: 8 OK。
- [x] Phase90-94 discovery: 20 OK。
- [x] Windows Computer Use tests: 225 OK。
- [x] Source compile check: `python -m compileall -q learning_agent\computer_use learning_agent\tests`。
- [x] CLI self-check 打印固定 Phase94 成功 token。
- [x] Scenario JSON validation 通过。
- [x] 真实可见 `start_oauth_agent.bat` 终端验收通过：`learning_agent/acceptance_controller/runs/agent_capability_phase94_authorized_real_dispatch_candidate-20260604_210234/result.json`。

## 当前边界

Phase94 已经把 Phase93 的 recording-only 缺口推进到“授权后低层 sender 候选桥”，但默认 sender 仍是 recording-only，验收明确记录 `real_dispatch_performed=false`。下一阶段如果继续推进真实物理输入，必须继续保留显式授权、目标复核、高风险窗口拒绝、abort、cleanup、脱敏证据和真实可见终端验收。
---

# Current Task: Universal Real GUI Computer Use Implementation (2026-06-05)

## URG-4 Current Status

- [x] Red test added for missing universal observe-plan-act-verify loop module.
- [x] `UniversalObservePlanActVerifyLoop` implemented as a composition layer over observation, target session, action DSL, verifier, bounded retry, and cleanup.
- [x] Representative Notepad, Calculator, and Paint contract samples run through one loop with no per-app controller.
- [x] Learning backup created under `learning_agent/test/universal_real_gui_computer_use_urg4_20260605/`.
- [x] Adjacent regression, compile checks, scenario validation, and Windows Computer Use discovery passed in this session.
- [x] Visible-terminal acceptance passed through `start_oauth_agent.bat` and `controller.ps1`.

## Task Goal

Build `learning_agent` `/computer use --full` from the current recording-mode desktop task proof into a universal real GUI Computer Use loop that can observe, target, act, verify, and clean up ordinary Windows applications without per-app controllers.

## Execution Plan

- [x] Write the formal root-cause blueprint: `docs/superpowers/specs/2026-06-05-universal-real-gui-computer-use-blueprint.md`.
- [x] Write the implementation plan: `docs/superpowers/plans/2026-06-05-universal-real-gui-computer-use.md`.
- [x] URG-1 Real ObservationFrame.
- [x] URG-2 Universal target session and identity guard.
- [x] URG-3 Generic real action DSL to SendInput.
- [x] URG-4 Observe-plan-act-verify loop.
- [x] URG-5 Paint/Pikachu real representative acceptance.
- [x] URG-6 Cross-app final maturity matrix.

## Success Criteria

- The final matrix emits `UNIVERSAL_REAL_GUI_COMPUTER_USE_READY`.
- `per_app_controller_required=false`.
- `hardcoded_app_whitelist_required=false`.
- `real_desktop_execution_mature=true`.
- Paint, Notepad, Calculator, and Browser or another ordinary app are acceptance samples only and all route through the same universal runtime.
- Automated tests, `py_compile`, visible-terminal `start_oauth_agent.bat` acceptance, evidence backup, and `agent_memory` updates pass before any stage is called complete.

## Current Boundary

URG-1 through URG-6 are now implemented and accepted for this blueprint's contract path. The current verified final matrix emits `UNIVERSAL_REAL_GUI_COMPUTER_USE_READY`, `per_app_controller_required=false`, `hardcoded_app_whitelist_required=false`, and `real_desktop_execution_mature=true`.
...
- URG-6 visible terminal final matrix acceptance passed at `learning_agent/acceptance_controller/runs/agent_capability_universal_real_gui_final_matrix-20260605_223813/result.json`.
- Windows Computer Use discovery passed with 381 tests.
