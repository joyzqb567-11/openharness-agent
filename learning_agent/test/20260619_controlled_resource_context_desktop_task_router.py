...
_CONTROLLED_RESOURCE_TEXT_FILENAME_RE = re.compile(r"(?<![A-Za-z0-9_.\\/-])([A-Za-z0-9][A-Za-z0-9 _.-]{0,80}\.txt)(?![A-Za-z0-9_.\\/-])", re.IGNORECASE)  # 新增代码+ControlledResourceContext：匹配 1.txt 这类安全文本资源名；如果没有这一行，分类器无法从真实 prompt 脱敏保留保存目标。
...
class DesktopTaskIntent:  # 新增代码+DesktopTaskRouter：定义桌面任务意图结果类；如果没有这一行，项目没有 Task 2 要求的 DesktopTaskIntent 输出类型。
    ...
    controlled_resource_name: str = ""  # 新增代码+ControlledResourceContext：记录脱敏后的受控文件名例如 1.txt；如果没有这一字段，模型压缩 reason 后启动层会丢失目标资源。
    controlled_resource_location_hint: str = ""  # 新增代码+ControlledResourceContext：记录脱敏后的位置提示例如 desktop；如果没有这一字段，controller 不能安全判断是否拼到桌面。
    ...
    def to_dict(self) -> Dict[str, object]:  # 新增代码+DesktopTaskRouter：函数段开始，把意图结果转成公开字典；如果没有这个函数，调用方可能直接打印对象并误以为包含完整上下文，作者意图是提供稳定、脱敏、可测试的公开摘要，本函数与测试中的 raw prompt 泄漏检查配合使用，函数段到 return 结束为止。
        return {  # 新增代码+DesktopTaskRouter：返回只含稳定字段的字典；如果没有这一行，调用方就拿不到可序列化结果。
            ...
            "controlled_resource_name": self.controlled_resource_name,  # 新增代码+ControlledResourceContext：公开脱敏受控文件名；如果没有这一项，host adapter 无法把 1.txt 传给启动层。
            "controlled_resource_location_hint": self.controlled_resource_location_hint,  # 新增代码+ControlledResourceContext：公开脱敏受控位置；如果没有这一项，启动层不知道文件名对应桌面还是普通文本。
...
def _controlled_resource_location_hint(text: str) -> str:  # 新增代码+ControlledResourceContext：函数段开始，判断 prompt 是否明确把文件资源放在桌面；如果没有这段函数，1.txt 会缺少可安全拼路径的位置边界，函数段到 return 结束为止。
    if "桌面" in text or _contains_english_term(text, ("desktop",)):  # 新增代码+ControlledResourceContext：只接受明确桌面语义；如果没有这一行，普通提到 1.txt 的聊天也可能被错误绑定到桌面。
        return "desktop"  # 新增代码+ControlledResourceContext：返回稳定英文位置码；如果没有这一行，跨模块传递会混用中文自然语言。
    return ""  # 新增代码+ControlledResourceContext：没有明确桌面语义时返回空；如果没有这一行，调用方无法保守降级。
# 新增代码+ControlledResourceContext：函数段结束，_controlled_resource_location_hint 到此结束；如果没有这个边界说明，用户不容易看出位置提取范围。


def _controlled_resource_name_from_text(text: str) -> str:  # 新增代码+ControlledResourceContext：函数段开始，从 prompt 中提取安全 txt 文件名；如果没有这段函数，真实 prompt 里的 1.txt 不能脱敏传递，函数段到 return 结束为止。
    matches = [match.group(1).strip() for match in _CONTROLLED_RESOURCE_TEXT_FILENAME_RE.finditer(text)]  # 新增代码+ControlledResourceContext：收集所有安全文本文件名；如果没有这一行，分类器无法定位最后的保存目标。
    if not matches:  # 新增代码+ControlledResourceContext：处理没有匹配文件名的情况；如果没有这一行，下面访问最后一个元素会崩溃。
        return ""  # 新增代码+ControlledResourceContext：没有文件名时返回空；如果没有这一行，调用方无法保守保持普通应用启动。
    return matches[-1]  # 新增代码+ControlledResourceContext：使用最后一个 txt 名称作为最终资源；如果没有这一行，提示词同时提旧文件和新文件时可能选错目标。
# 新增代码+ControlledResourceContext：函数段结束，_controlled_resource_name_from_text 到此结束；如果没有这个边界说明，用户不容易看出文件名提取范围。


def _controlled_resource_fields_for_text(text: str) -> tuple[str, str]:  # 新增代码+ControlledResourceContext：函数段开始，组合受控资源名和位置提示；如果没有这段函数，正向分类分支要重复资源提取逻辑，函数段到 return 结束为止。
    location_hint = _controlled_resource_location_hint(text)  # 新增代码+ControlledResourceContext：先提取位置提示；如果没有这一行，文件名会在没有桌面语义时被过度使用。
    resource_name = _controlled_resource_name_from_text(text) if location_hint else ""  # 新增代码+ControlledResourceContext：只有明确桌面位置时才保留文件名；如果没有这一行，普通 1.txt 文本会扩大为桌面资源。
    return resource_name, location_hint if resource_name else ""  # 新增代码+ControlledResourceContext：只有文件名存在才返回位置；如果没有这一行，空文件名可能带着 desktop 误导下游。
# 新增代码+ControlledResourceContext：函数段结束，_controlled_resource_fields_for_text 到此结束；如果没有这个边界说明，用户不容易看出资源字段生成范围。
...
def classify_desktop_task(prompt: str) -> DesktopTaskIntent:  # 修改代码+DesktopTaskRouter：函数段开始，按保守关键词组合把自然语言 prompt 分类为桌面 GUI 任务或普通任务；如果没有这个函数，Task 2 没有实际入口，作者意图是先为 Task 3+ 提供脱敏、可测、误报受控的路由信号，本函数与 DesktopTaskIntent、关键词表和测试文件配合使用，函数段到最后 return 结束为止。
    ...
    if is_paint_gui_task or is_generic_local_gui_task:  # 新增代码+DesktopTaskRouter：只在正向组合成立时进入桌面任务；如果没有这一行，分类器无法把正负场景分开。
        controlled_resource_name, controlled_resource_location_hint = _controlled_resource_fields_for_text(text)  # 新增代码+ControlledResourceContext：在确认是桌面任务后提取脱敏资源提示；如果没有这一行，真实 prompt 的 1.txt 仍无法传到启动层。
        return DesktopTaskIntent(  # 新增代码+DesktopTaskRouter：返回桌面任务意图对象；如果没有这一行，正向命中无法生成结构化结果。
            ...
            controlled_resource_name=controlled_resource_name,  # 新增代码+ControlledResourceContext：写入脱敏受控文件名；如果没有这一项，host adapter 只能依赖模型后续 reason 是否碰巧复述文件名。
            controlled_resource_location_hint=controlled_resource_location_hint,  # 新增代码+ControlledResourceContext：写入脱敏受控位置；如果没有这一项，controller 无法安全拼出 Desktop 路径。
