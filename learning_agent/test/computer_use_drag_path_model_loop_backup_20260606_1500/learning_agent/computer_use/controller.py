...
class ComputerUseController:  # 新增代码+OSComputerUse: 在 agent 和后端之间提供安全控制层；若没有这段代码，真实桌面操作缺少统一确认和参数校验。
    ALLOWED_ACTIONS: set[str] = {"screenshot", "move_mouse", "click", "double_click", "drag_path", "type_text", "press_key", "scroll"}  # 修改代码+GenericDragPathToolSurface: 允许通用连续拖拽路径进入真实 controller；如果没有这行代码，模型画任意图形时仍会被顶层白名单拒绝。
