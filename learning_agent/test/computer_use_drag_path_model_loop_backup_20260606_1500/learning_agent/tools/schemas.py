        ...
        "function": {  # 新增代码+OSComputerUse: function 内部描述工具名称、说明和参数；若没有这行代码，Computer Use 动作工具缺少标准定义结构。
            ...
            "parameters": {  # 新增代码+OSComputerUse: JSON Schema 描述 computer_action 参数；若没有这行代码，模型不知道动作字段怎么传。
                ...
                "properties": {  # 新增代码+OSComputerUse: 声明允许的动作参数字段；若没有这行代码，模型无法构造合法调用。
                    "action": {"type": "string", "enum": ["screenshot", "move_mouse", "click", "double_click", "drag_path", "type_text", "press_key", "scroll"], "description": "要执行的桌面动作；drag_path 用于按住鼠标沿 points 连续拖拽，例如在画图软件里画线。"},  # 修改代码+GenericDragPathToolSurface: 暴露通用拖拽动作给模型主循环；如果没有这行代码，模型看不到可用于任意绘图的连续鼠标动作。
