...
class WindowsComputerUseNativeHost:  # 新增代码+Phase44WindowsNativeHost: 类段开始，定义进程内 Windows native host；如果没有这个类，主 agent 没有独立 host 边界可调用。
    def __init__(self, helper: Any | None = None, platform: str | None = None, host_id: str = "windows-computer-use-native-host", real_actions_enabled: bool = False, screenshot_runtime: Any | None = None) -> None:  # 修改代码+Phase45WindowsScreenshotRuntime: 函数段开始，初始化 helper、平台、动作开关和可选截图运行时；如果没有这段函数，host 无法接入 Phase45 真实截图链路。
    ...
    # 修改代码+Phase45WindowsScreenshotRuntime: 函数段结束，WindowsComputerUseNativeHost.__init__ 到此结束；如果没有这个边界说明，读者不容易看出初始化范围。
