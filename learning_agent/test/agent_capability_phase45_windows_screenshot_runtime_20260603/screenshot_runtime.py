...
try:  # 新增代码+Phase45WindowsScreenshotRuntime: 优先按包模式导入截图、evidence 和 provider 合同；如果没有这行代码，unittest 和生产入口不能复用已有模块。
    from learning_agent.computer_use.evidence import ComputerUseEvidenceStore  # 新增代码+Phase45WindowsScreenshotRuntime: 导入 evidence store；如果没有这行代码，截图 bytes 无法落盘成 artifact。
    from learning_agent.computer_use.helper_client import WindowObservationPayload  # 新增代码+Phase45WindowsScreenshotRuntime: 导入统一 observation payload；如果没有这行代码，runtime 无法复用 Phase29/41 证据链。
    from learning_agent.computer_use.native_helper import NativeWindowCaptureResult, Win32GdiWindowCaptureProvider, parse_hwnd_from_window  # 新增代码+Phase45WindowsScreenshotRuntime: 导入截图结果、GDI fallback 和 hwnd 解析；如果没有这行代码，runtime 无法连接真实 Win32 截图能力。
    from learning_agent.computer_use.wgc_capture import WindowsGraphicsCaptureProvider  # 新增代码+Phase45WindowsScreenshotRuntime: 导入 WGC provider 合同；如果没有这行代码，runtime 无法表达 WGC 优先、GDI 保底的生产顺序。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.evidence", "learning_agent.computer_use.helper_client", "learning_agent.computer_use.native_helper", "learning_agent.computer_use.wgc_capture"}:  # 新增代码+Phase45WindowsScreenshotRuntime: 只允许目标包路径缺失时 fallback；如果没有这行代码，真实内部 bug 会被误吞。
        raise  # 新增代码+Phase45WindowsScreenshotRuntime: 重新抛出非路径类导入错误；如果没有这行代码，排查 provider 内部错误会困难。
    from computer_use.evidence import ComputerUseEvidenceStore  # 新增代码+Phase45WindowsScreenshotRuntime: 脚本模式导入 evidence store；如果没有这行代码，bat 入口无法保存截图 artifact。
    from computer_use.helper_client import WindowObservationPayload  # 新增代码+Phase45WindowsScreenshotRuntime: 脚本模式导入 observation payload；如果没有这行代码，bat 入口无法构造 evidence 输入。
    from computer_use.native_helper import NativeWindowCaptureResult, Win32GdiWindowCaptureProvider, parse_hwnd_from_window  # 新增代码+Phase45WindowsScreenshotRuntime: 脚本模式导入 GDI fallback 和 hwnd 解析；如果没有这行代码，bat 入口无法使用 Win32 截图保底能力。
    from computer_use.wgc_capture import WindowsGraphicsCaptureProvider  # 新增代码+Phase45WindowsScreenshotRuntime: 脚本模式导入 WGC provider；如果没有这行代码，bat 入口无法报告 WGC 优先链路。
...
def run_phase45_screenshot_runtime_contract() -> dict[str, Any]:  # 新增代码+Phase45WindowsScreenshotRuntime: 函数段开始，运行安全截图运行时自检；如果没有这段函数，真实终端场景没有稳定命令入口。
    from learning_agent.computer_use.native_host import InProcessWindowsNativeHostClient, WindowsComputerUseNativeHost  # 新增代码+Phase45WindowsScreenshotRuntime: 延迟导入 native host 避免循环依赖；如果没有这行代码，自检无法证明 host capture 接入。
