...
def normalize_window_record(raw_window: dict[str, Any], captured_at: str) -> tuple[dict[str, Any] | None, str]:  # 新增代码+Phase28ComputerUse: 定义窗口归一化函数；如果没有这行代码，raw Win32 记录无法直接给模型。
    title = clean_protocol_text(raw_window.get("title", raw_window.get("title_preview", "")))  # 新增代码+Phase28ComputerUse: 读取并清理窗口标题；如果没有这行代码，空白和换行标题会污染输出。
    ...
    record = {"app_id": app_id, "window_id": window_id, "title_preview": title, "process_path_hash": process_hash, "captured_at": captured_at, "rect": rect, "pid": pid, "class_name": class_name, "safe_to_target": True}  # 新增代码+Phase28ComputerUse: 返回标准窗口记录；如果没有这行代码，observe 结果缺少稳定字段。
