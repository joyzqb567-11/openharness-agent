...
$ScenarioEnvironmentSetCommands = @()  # 新增代码+OAuthNativeVisibleTerminalEnv: 初始化场景环境变量 set 命令列表；如果没有这一行，scenario.environment 无法传给真实可见终端。
if ($Scenario.environment) {  # 新增代码+OAuthNativeVisibleTerminalEnv: 只有场景声明 environment 时才处理；如果没有这一行，旧场景缺字段会被误访问。
    foreach ($Property in $Scenario.environment.PSObject.Properties) {  # 新增代码+OAuthNativeVisibleTerminalEnv: 遍历 JSON environment 对象里的每个变量；如果没有这一行，只能支持写死的 CODEX_OAUTH_NATIVE_TOOLS。
        $EnvironmentName = [string]$Property.Name  # 新增代码+OAuthNativeVisibleTerminalEnv: 读取环境变量名；如果没有这一行，后续无法校验变量名。
        $EnvironmentValue = [string]$Property.Value  # 新增代码+OAuthNativeVisibleTerminalEnv: 读取环境变量值；如果没有这一行，启动命令无法设置变量。
        if ($EnvironmentName -notmatch '^[A-Za-z_][A-Za-z0-9_]*$') {  # 新增代码+OAuthNativeVisibleTerminalEnv: 只允许常规环境变量名；如果没有这一行，场景可能注入额外 cmd 命令。
            throw "场景 environment 包含非法变量名：$EnvironmentName"  # 新增代码+OAuthNativeVisibleTerminalEnv: 遇到非法变量名立即失败；如果没有这一行，风险配置会静默进入终端命令。
        }  # 新增代码+OAuthNativeVisibleTerminalEnv: 结束变量名校验；如果没有这一行，PowerShell 语法不完整。
        if ($EnvironmentValue.Contains('"')) {  # 新增代码+OAuthNativeVisibleTerminalEnv: 拒绝双引号，避免破坏 cmd set 语法；如果没有这一行，变量值可能截断命令。
            throw "场景 environment 变量 $EnvironmentName 的值不能包含双引号。"  # 新增代码+OAuthNativeVisibleTerminalEnv: 给出清楚的值错误；如果没有这一行，排查 cmd 启动失败会很困难。
        }  # 新增代码+OAuthNativeVisibleTerminalEnv: 结束变量值校验；如果没有这一行，PowerShell 语法不完整。
        $ScenarioEnvironmentSetCommands += "set `"$EnvironmentName=$EnvironmentValue`""  # 新增代码+OAuthNativeVisibleTerminalEnv: 生成 cmd 的 set 命令；如果没有这一行，真实终端不会收到环境变量。
    }  # 新增代码+OAuthNativeVisibleTerminalEnv: 结束 environment 遍历；如果没有这一行，PowerShell 语法不完整。
}  # 新增代码+OAuthNativeVisibleTerminalEnv: 结束 environment 读取；如果没有这一行，PowerShell 语法不完整。
$ScenarioEnvironmentCommandPrefix = ""  # 新增代码+OAuthNativeVisibleTerminalEnv: 初始化启动命令环境变量前缀；如果没有这一行，后续拼接可能引用未定义变量。
if ($ScenarioEnvironmentSetCommands.Count -gt 0) {  # 新增代码+OAuthNativeVisibleTerminalEnv: 只有存在环境变量时才拼接前缀；如果没有这一行，旧场景会多出无意义命令。
    $ScenarioEnvironmentCommandPrefix = ($ScenarioEnvironmentSetCommands -join " && ") + " && "  # 新增代码+OAuthNativeVisibleTerminalEnv: 把多条 set 命令串到 start_bat 前；如果没有这一行，环境变量不会在同一个 cmd 会话生效。
}  # 新增代码+OAuthNativeVisibleTerminalEnv: 结束前缀拼接；如果没有这一行，PowerShell 语法不完整。
...
$AlternateSuccessPaths = @()  # 新增代码+AcceptanceAlternatePath：初始化可选成功分支列表；如果没有这行代码，dirty-state 安全拒绝无法作为合格验收路径表达。
if ($Scenario.alternate_success_paths) {  # 新增代码+AcceptanceAlternatePath：只有场景声明备用分支时才读取；如果没有这行代码，旧场景缺字段会被当成可遍历对象。
    $AlternateSuccessPaths = @($Scenario.alternate_success_paths)  # 新增代码+AcceptanceAlternatePath：保存场景里的 OR 成功分支；如果没有这行代码，控制器仍只能做单一路径 AND 断言。
}  # 新增代码+AcceptanceAlternatePath：结束备用分支读取；如果没有这行代码，PowerShell 语法不完整。
...
function Activate-AgentWindow {  # 修改代码+TerminalFocusGuard: 聚焦并验证本轮真实终端窗口；若没有这行代码，键盘输入可能继续发到 Photoshop、浏览器或 Codex 窗口
    ...
    for ($FocusAttempt = 0; $FocusAttempt -lt 4; $FocusAttempt = $FocusAttempt + 1) {  # 新增代码+TerminalFocusGuard: 最多尝试四次聚焦和验证；若没有这行代码，一次前台锁失败就会误伤验收
        ...
        $TerminalWindow.Refresh()  # 修改代码+TerminalFocusGuard: 调用 Win32 聚焦前刷新窗口进程对象；若没有这行代码，刚启动或被 Windows Terminal 接管的窗口句柄可能仍是旧的空值
        $Handle = $TerminalWindow.MainWindowHandle  # 修改代码+TerminalFocusGuard: 读取刷新后的窗口句柄用于 Win32 聚焦；若没有这行代码，后续 API 没有目标
        if ($Handle -eq [IntPtr]::Zero) {  # 修改代码+TerminalFocusGuard: 二次检查窗口句柄是否为空；若没有这行代码，空句柄会传给 ShowWindowAsync 并让真实终端验收崩溃
            Start-Sleep -Milliseconds 300  # 修改代码+TerminalFocusGuard: 句柄暂时为空时短暂等待窗口创建完成；若没有这行代码，控制器会在窗口刚出现但句柄未就绪时过早失败
            $TerminalWindow = Get-AgentWindowProcess  # 修改代码+TerminalFocusGuard: 重新枚举目标终端窗口；若没有这行代码，Windows Terminal 外壳窗口接管后仍会继续使用旧 cmd 进程对象
            continue  # 修改代码+TerminalFocusGuard: 跳过本轮 Win32 调用并进入下一次聚焦尝试；若没有这行代码，后续代码仍会使用空句柄
        }  # 修改代码+TerminalFocusGuard: 结束空句柄二次保护；若没有这行代码，PowerShell 语法不完整
...
function Clear-TerminalInputLine {  # 新增代码+PromptRetryLineClear：函数段开始，重试 prompt 前清空真实终端当前输入行；若没有这段函数，第一次回车未生效时第二次 prompt 会拼接成 `/computer maturity/computer maturity` 这类非法命令。
    if (-not (Activate-AgentWindow)) {  # 新增代码+PromptRetryLineClear：清空前确认目标终端仍是前台；若没有这一行，清空快捷键可能误发到用户其他窗口。
        throw "无法聚焦真实终端窗口，停止清空输入行。"  # 新增代码+PromptRetryLineClear：聚焦失败时立刻停止；若没有这一行，controller 可能污染非目标应用。
    }  # 新增代码+PromptRetryLineClear：结束聚焦保护分支；若没有这一行，PowerShell 语法不完整。
    [System.Windows.Forms.SendKeys]::SendWait("^a")  # 新增代码+PromptRetryLineClear：选择当前输入行里已有的 prompt 文本；若没有这一行，后续 Backspace 只能删除一个字符，重复 prompt 仍会残留。
    Start-Sleep -Milliseconds 120  # 新增代码+PromptRetryLineClear：给终端一点时间完成选择动作；若没有这一行，Backspace 可能早于 Ctrl+A 生效。
    [System.Windows.Forms.SendKeys]::SendWait("{BACKSPACE}")  # 新增代码+PromptRetryLineClear：删除被选中的旧输入内容；若没有这一行，重试时新 prompt 仍会追加到旧 prompt 后面。
    Start-Sleep -Milliseconds 180  # 新增代码+PromptRetryLineClear：等待输入行完成清空后再继续粘贴；若没有这一行，下一次粘贴可能和清空动作交错。
}  # 新增代码+PromptRetryLineClear：函数段结束，Clear-TerminalInputLine 到此结束；若没有这个边界说明，初学者不容易看出清空输入行的范围。
...
function Test-ScenarioAssertions {  # 新增代码+AcceptanceController: 统一检查事件、最终回答预览和调试日志断言；若没有这行代码，每个场景会重复写判断逻辑
    ...
    $DefaultContentPassed = $EventAnswerPassed -and $DebugPassed -and $MarkerPassed  # 新增代码+AcceptanceAlternatePath：汇总原有单一路径内容断言；如果没有这行代码，加入 OR 分支后无法保留旧场景行为。
    $AlternateSuccessChecks = [ordered]@{}  # 新增代码+AcceptanceAlternatePath：准备保存每个备用成功分支的检查结果；如果没有这行代码，result.json 看不出哪条分支通过或失败。
    $AnyAlternateSuccessPassed = $false  # 新增代码+AcceptanceAlternatePath：默认没有备用分支通过；如果没有这行代码，后续 OR 汇总可能使用未初始化变量。
    $AlternateIndex = 0  # 新增代码+AcceptanceAlternatePath：初始化备用分支序号；如果没有这行代码，匿名分支无法生成稳定名称。
    foreach ($Path in $AlternateSuccessPaths) {  # 新增代码+AcceptanceAlternatePath：遍历场景声明的每条备用分支；如果没有这行代码，dirty-state 安全拒绝无法单独验收。
        if ($null -eq $Path) {  # 新增代码+AcceptanceAlternatePath：跳过空分支配置；如果没有这行代码，场景里的空值会触发属性访问错误。
            continue  # 新增代码+AcceptanceAlternatePath：继续处理下一条分支；如果没有这行代码，空分支会污染验收结果。
        }  # 新增代码+AcceptanceAlternatePath：结束空分支判断；如果没有这行代码，PowerShell 语法不完整。
        $AlternateIndex += 1  # 新增代码+AcceptanceAlternatePath：递增分支序号；如果没有这行代码，多个匿名分支会重名。
        $PathName = [string]$Path.name  # 新增代码+AcceptanceAlternatePath：读取分支名称；如果没有这行代码，结果报告无法标明通过的是哪条路径。
        if ([string]::IsNullOrWhiteSpace($PathName)) {  # 新增代码+AcceptanceAlternatePath：检查分支是否没有名字；如果没有这行代码，空键可能覆盖其他分支结果。
            $PathName = "alternate_success_path_$AlternateIndex"  # 新增代码+AcceptanceAlternatePath：给匿名分支生成稳定名称；如果没有这行代码，result.json 的 OR 分支不可读。
        }  # 新增代码+AcceptanceAlternatePath：结束分支名称兜底；如果没有这行代码，PowerShell 语法不完整。
        $PathAnswerContains = @(@($Path.event_answer_contains) | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) })  # 新增代码+AcceptanceAlternatePath：读取该分支要求的最终回答文本；如果没有这行代码，安全拒绝分支无法要求用户可见拒绝原因。
        $PathDebugContains = @(@($Path.debug_log_contains) | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) })  # 新增代码+AcceptanceAlternatePath：读取该分支要求的调试日志文本；如果没有这行代码，分支无法验证底层零事件证据。
        $PathPayloadContains = @(@($Path.event_payload_contains) | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) })  # 新增代码+AcceptanceAlternatePath：读取该分支要求的事件 payload 文本；如果没有这行代码，分支无法检查工具事件里的结构化证据。
        $PathMarker = [string]$Path.success_marker  # 新增代码+AcceptanceAlternatePath：读取该分支自己的成功标记；如果没有这行代码，成功分支和安全分支无法使用不同 marker。
        $PathAnswerChecks = [ordered]@{}  # 新增代码+AcceptanceAlternatePath：准备记录该分支回答文本检查；如果没有这行代码，失败时不知道缺哪一句。
        foreach ($Text in $PathAnswerContains) {  # 新增代码+AcceptanceAlternatePath：逐项检查该分支最终回答文本；如果没有这行代码，分支文本门槛不会生效。
            $PathAnswerChecks[[string]$Text] = $FinalAnswerText.Contains([string]$Text)  # 新增代码+AcceptanceAlternatePath：记录回答文本是否出现；如果没有这行代码，安全拒绝可能只靠日志假通过。
        }  # 新增代码+AcceptanceAlternatePath：结束分支回答文本检查；如果没有这行代码，PowerShell 语法不完整。
        $PathDebugChecks = [ordered]@{}  # 新增代码+AcceptanceAlternatePath：准备记录该分支调试日志检查；如果没有这行代码，日志证据缺失不可诊断。
        foreach ($Text in $PathDebugContains) {  # 新增代码+AcceptanceAlternatePath：逐项检查该分支调试日志文本；如果没有这行代码，分支无法验证真实工具输出。
            $PathDebugChecks[[string]$Text] = ($LogUpdated -and $LogText.Contains([string]$Text))  # 新增代码+AcceptanceAlternatePath：记录本轮日志是否包含目标文本；如果没有这行代码，旧日志可能误导验收。
        }  # 新增代码+AcceptanceAlternatePath：结束分支调试日志检查；如果没有这行代码，PowerShell 语法不完整。
        $PathPayloadChecks = [ordered]@{}  # 新增代码+AcceptanceAlternatePath：准备记录该分支事件 payload 检查；如果没有这行代码，结构化事件缺失不可定位。
        foreach ($Text in $PathPayloadContains) {  # 新增代码+AcceptanceAlternatePath：逐项检查该分支事件 payload 文本；如果没有这行代码，分支无法约束事件证据。
            $PathPayloadChecks[[string]$Text] = $EventPayloadText.Contains([string]$Text)  # 新增代码+AcceptanceAlternatePath：记录事件 payload 是否包含目标文本；如果没有这行代码，工具层 evidence 无法参与 OR 分支。
        }  # 新增代码+AcceptanceAlternatePath：结束分支事件 payload 检查；如果没有这行代码，PowerShell 语法不完整。
        $PathAnswerPassed = -not ($PathAnswerChecks.Values -contains $false)  # 新增代码+AcceptanceAlternatePath：汇总该分支回答文本是否通过；如果没有这行代码，无法判断安全拒绝说明是否完整。
        $PathDebugPassed = -not ($PathDebugChecks.Values -contains $false)  # 新增代码+AcceptanceAlternatePath：汇总该分支调试日志是否通过；如果没有这行代码，底层证据不会影响分支结果。
        $PathPayloadPassed = -not ($PathPayloadChecks.Values -contains $false)  # 新增代码+AcceptanceAlternatePath：汇总该分支事件 payload 是否通过；如果没有这行代码，事件证据不会影响分支结果。
        $PathMarkerPassed = $true  # 新增代码+AcceptanceAlternatePath：默认分支未声明 marker 时通过；如果没有这行代码，纯文本安全分支无法表达。
        if (-not [string]::IsNullOrWhiteSpace($PathMarker)) {  # 新增代码+AcceptanceAlternatePath：如果分支声明了 marker；如果没有这行代码，marker 字段不会生效。
            $PathMarkerPassed = $FinalAnswerText.Contains($PathMarker) -or ($LogUpdated -and $LogText.Contains($PathMarker))  # 新增代码+AcceptanceAlternatePath：检查 marker 是否出现在最终回答或本轮日志；如果没有这行代码，分支 marker 无法验收。
        }  # 新增代码+AcceptanceAlternatePath：结束分支 marker 检查；如果没有这行代码，PowerShell 语法不完整。
        $PathPassed = $PathAnswerPassed -and $PathDebugPassed -and $PathPayloadPassed -and $PathMarkerPassed  # 新增代码+AcceptanceAlternatePath：汇总该备用分支是否通过；如果没有这行代码，OR 逻辑没有单分支结果。
        $AlternateSuccessChecks[$PathName] = [ordered]@{ passed = $PathPassed; marker_passed = $PathMarkerPassed; event_answer_checks = $PathAnswerChecks; debug_log_checks = $PathDebugChecks; event_payload_checks = $PathPayloadChecks }  # 新增代码+AcceptanceAlternatePath：把分支结果写入报告；如果没有这行代码，失败时无法复盘分支条件。
        if ($PathPassed) {  # 新增代码+AcceptanceAlternatePath：检查当前分支是否通过；如果没有这行代码，OR 汇总不会被置为成功。
            $AnyAlternateSuccessPassed = $true  # 新增代码+AcceptanceAlternatePath：记录至少一个备用分支通过；如果没有这行代码，dirty-state 安全拒绝仍会被总结果判失败。
        }  # 新增代码+AcceptanceAlternatePath：结束当前分支通过判断；如果没有这行代码，PowerShell 语法不完整。
    }  # 新增代码+AcceptanceAlternatePath：结束备用分支遍历；如果没有这行代码，PowerShell 语法不完整。
    $ContentPassed = $DefaultContentPassed -or $AnyAlternateSuccessPassed  # 新增代码+AcceptanceAlternatePath：原有成功路径和备用路径任一通过即可满足内容门槛；如果没有这行代码，OR 分支不会参与总验收。
    $Passed = $StatesPassed -and $EventPayloadPassed -and $EventPayloadRegexPassed -and $PermissionCountPassed -and $ContentPassed  # 修改代码+AcceptanceAlternatePath：汇总场景是否通过时支持原路径或备用路径；如果没有这行代码，安全拒绝分支无法通过真实验收。
    return [ordered]@{ passed = $Passed; marker_passed = $MarkerPassed; permission_count_passed = $PermissionCountPassed; max_permission_sent_count = $MaxPermissionSentCount; permission_sent_count = $PermissionSentCount; log_updated = $LogUpdated; state_checks = $StateChecks; event_answer_checks = $EventAnswerChecks; event_payload_checks = $EventPayloadChecks; event_payload_regex_checks = $EventPayloadRegexChecks; debug_log_checks = $DebugChecks; alternate_success_checks = $AlternateSuccessChecks; default_content_passed = $DefaultContentPassed; alternate_content_passed = $AnyAlternateSuccessPassed; content_passed = $ContentPassed; final_answer_preview = $FinalAnswerPreview; final_answer_text_length = $FinalAnswerText.Length; event_payload_text_length = $EventPayloadText.Length }  # 修改代码+AcceptanceAlternatePath：返回完整断言报告并包含 OR 分支结果；如果没有这行代码，失败时无法定位是主路径还是安全路径缺证据。
...
$CmdLine = "/k title $WindowTitle && cd /d `"$RepoRoot`" && set `"LEARNING_AGENT_ACCEPTANCE_EVENT_LOG=$EventLog`" && $ScenarioEnvironmentCommandPrefix`call `"$BatPath`""  # 修改代码+OAuthNativeVisibleTerminalEnv: 构造 cmd 命令时插入场景环境变量；如果没有这一行，CODEX_OAUTH_NATIVE_TOOLS=1 不会传进真实可见终端。
...
while ((Get-Date) -lt $Deadline) {  # 新增代码+AcceptanceController: 主循环按事件驱动授权、输入和断言；若没有这行代码，控制器无法自动完成验收
    ...
    if ($MultiPromptEnabled) {  # 新增代码+DesktopTaskVisibleAcceptance: 多轮场景使用逐行输入推进；若没有这行代码，full request、confirm、自然语言任务会被错误合并
        ...
        $PromptLineAcknowledgedByReady = $false  # 新增代码+NaturalPromptAckGate：初始化 slash 命令的 ready 确认标记；若没有这一行，自然语言任务可能被额外 ready 事件误判为已完成。
        if ($PromptLineInFlightIndex -ge 0) {  # 新增代码+NaturalPromptNoRetry: 只有存在飞行中的 prompt 行才检查自然语言接收事件；若没有这行代码，未发送任何行时会访问无效序号。
            ...
            $PromptLineAcknowledgedByReady = ($InFlightPromptText.TrimStart().StartsWith("/") -and ($ReadyCount -gt ($PromptLineInFlightIndex + 1)))  # 修改代码+NaturalPromptAckGate：只有 slash 命令才允许用下一次 ready 作为确认；若没有这一行，QQ 这类自然语言任务会被未相关 ready 事件提前跳过。
        }  # 新增代码+NaturalPromptNoRetry: 结束自然语言接收检查；若没有这行代码，PowerShell 语法不完整。
        if (($PromptLineInFlightIndex -ge 0) -and ($PromptLineAcknowledgedByUserPromptReceived -or $PromptLineAcknowledgedByReady)) {  # 修改代码+NaturalPromptAckGate：自然语言只认 user_prompt_received，slash 命令只认下一次 ready；若没有这一行，controller 会在任务没进入模型主循环时提前发送 /computer stop。
        ...
        if ($LineToSend -ge 0) {  # 新增代码+DesktopTaskVisibleAcceptance: 只有本轮确定可发送时才进入输入；若没有这行代码，可能发送空 prompt
            ...
            if ([bool]$ResolvedPrompt.ready) {  # 新增代码+DesktopTaskVisibleAcceptance: 确认所有变量都已就绪；若没有这行代码，controller 可能在 token 未出现时抢跑
                if ($RetryCurrentLine) {  # 新增代码+PromptRetryLineClear：只有重发同一行 prompt 时才先清空当前输入行；若没有这一行，正常首次输入会被多余快捷键打扰。
                    Clear-TerminalInputLine  # 新增代码+PromptRetryLineClear：清掉可能残留在终端输入行里的旧 prompt；若没有这一行，重试会继续拼接成非法命令。
                }  # 新增代码+PromptRetryLineClear：结束多轮重试清空分支；若没有这一行，PowerShell 语法不完整。
        ...
        if ((-not $PromptReceived) -and ($ReadyCount -gt 0) -and ((-not $PromptSent) -or $ShouldRetryPrompt)) {  # 新增代码+AcceptanceController: ready 后发送或重发 prompt；若没有这行代码，输入时机仍然不稳定
            if ($ShouldRetryPrompt) {  # 新增代码+PromptRetryLineClear：单 prompt 模式只有重试时才清空当前输入行；若没有这一行，首次输入前可能多发无意义快捷键。
                Clear-TerminalInputLine  # 新增代码+PromptRetryLineClear：删除第一次未提交成功时残留的旧 prompt；若没有这一行，第二次粘贴会追加到旧 prompt 后面。
            }  # 新增代码+PromptRetryLineClear：结束单 prompt 重试清空分支；若没有这一行，PowerShell 语法不完整。
