...
function directRouteLabel(status: string): string { // 新增代码+ComposerRouteControls：函数段开始，把 Direct SSE 路由状态转成安全文案；如果没有这段，UI 只能显示机器状态码。
  if (status === "direct_sse_ready") { // 新增代码+ComposerRouteControls：识别 Direct SSE 已就绪；如果没有这行，真实 OAuth 连接成功后缺少明确路由反馈。
    return "Direct ChatGPT OAuth SSE 已就绪"; // 新增代码+ComposerRouteControls：返回就绪文案；如果没有这行，用户不知道当前绕开了 Codex CLI WebSocket。
  } // 新增代码+ComposerRouteControls：Direct SSE 就绪分支结束；如果没有这行，条件块语法不完整。
  if (status === "mock_only") { // 新增代码+ComposerRouteControls：识别 mock 授权状态；如果没有这行，测试连接可能被误解成真实模型可用。
    return "Mock 授权，仅用于界面验收"; // 新增代码+ComposerRouteControls：返回 mock 文案；如果没有这行，用户会误以为已经真实连接。
  } // 新增代码+ComposerRouteControls：mock 分支结束；如果没有这行，条件块语法不完整。
  if (status === "api_key_route") { // 新增代码+ComposerRouteControls：识别 API key 路径；如果没有这行，API key 连接无法和 OAuth SSE 区分。
    return "API Key 路径"; // 新增代码+ComposerRouteControls：返回 API key 文案；如果没有这行，设置页路由信息不完整。
  } // 新增代码+ComposerRouteControls：API key 分支结束；如果没有这行，条件块语法不完整。
  return status.length > 0 ? status : ""; // 新增代码+ComposerRouteControls：未知状态只显示非空安全文本；如果没有这行，空状态会制造噪音。
} // 新增代码+ComposerRouteControls：函数段结束，directRouteLabel 到此结束；如果没有这行，函数语法不完整。
...
export function SettingsProvidersPanel({ viewModel, loading, errorMessage, busyProviderId, probeResults, onRetry, onConnectProvider, onDisconnectProvider, onTestProvider, onOpenCustomProvider }: SettingsProvidersPanelProps): JSX.Element { // 新增代码+ProviderSettingsPanel：函数段开始，渲染 Provider Settings 列表；如果没有这段，设置弹窗无法管理大模型提供商。
  ...
  return ( // 新增代码+ProviderSettingsPanel：返回 provider 列表结构；如果没有这行，组件不会输出 UI。
    <div className="settings-provider-list"> {/* 新增代码+ProviderSettingsPanel：渲染 provider 列表容器；如果没有这层，行间距和滚动无法统一。 */}
        ...
        return ( // 新增代码+ProviderSettingsPanel：返回单个 provider 行；如果没有这行，map 不会输出元素。
          <section className="settings-provider-row" key={provider.id}> {/* 新增代码+ProviderSettingsPanel：渲染 provider 行容器；如果没有这层，图标、文本和按钮无法稳定对齐。 */}
            ...
            <div className="settings-provider-copy"> {/* 新增代码+ProviderSettingsPanel：渲染 provider 文案区；如果没有这层，标题和描述会散落。 */}
              ...
              {provider.connected && provider.accountLabel.length > 0 ? <small>账号：{provider.accountLabel}</small> : null} {/* 新增代码+ComposerRouteControls：显示 OAuth 脱敏账号摘要；如果没有这行，用户无法确认当前连接的是哪个 ChatGPT 账号。 */}
              {provider.connected && directRouteLabel(provider.directRouteStatus).length > 0 ? <small>路由：{directRouteLabel(provider.directRouteStatus)}</small> : null} {/* 新增代码+ComposerRouteControls：显示 Direct SSE 路由状态；如果没有这行，用户不知道当前连接是否可直连真实模型。 */}
              {provider.connected && provider.oauthClientSource.length > 0 ? <small>OAuth client：{provider.oauthClientSource}</small> : null} {/* 新增代码+ComposerRouteControls：显示 OAuth client 来源；如果没有这行，真实 OAuth 调试缺少低敏配置线索。 */}
      ...
      <button aria-label="添加自定义提供商" className="settings-provider-row settings-provider-custom-row" data-testid="provider-custom-cta" type="button" onClick={onOpenCustomProvider}> {/* 修改代码+CustomProviderDialog：渲染虚拟自定义 provider 入口并提供可访问标签；如果没有这行，用户无法添加兼容 provider，读屏器也不容易理解入口。 */}
