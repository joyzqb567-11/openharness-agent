  ...
  return ( // 修改代码+DesktopToolTimeline：返回主界面结构；如果没有这行，组件不会输出任何 UI。
    <main className="app-shell"> {/* 修改代码+DesktopToolTimeline：定义三栏窗口网格容器；如果没有这行，侧栏、线程区和状态区无法稳定并排。 */}
      ...
      <section className="thread-panel" aria-label="当前对话"> {/* 修改代码+DesktopToolTimeline：定义当前对话主区域；如果没有这行，读屏器和布局都缺少对话容器。 */}
        ...
        <Composer isRunning={threadState.isRunning} activeTurnId={threadState.activeTurnId} selectedProviderId={composerRoute.selectedProviderId} selectedModelId={composerRoute.selectedModelId} reasoningEffort={composerRoute.reasoningEffort} permissionMode={composerRoute.permissionMode} modelOptions={composerModelOptions} onModelChange={(providerId, modelId) => setComposerRoute((current) => ({ ...current, selectedProviderId: providerId, selectedModelId: modelId }))} onReasoningEffortChange={(value) => setComposerRoute((current) => ({ ...current, reasoningEffort: value }))} onPermissionModeChange={(value) => setComposerRoute((current) => ({ ...current, permissionMode: value }))} onCancelActiveTurn={(turnId) => { void handleCancelTurn(turnId); }} onSubmit={(payload) => { void handleSubmit(payload); }} /> {/* 修改代码+ComposerRouteControls：把运行态、活动 turn、模型选项、路由、权限、推理、取消回调和提交回调交给 composer；如果没有这行代码，底部输入区无法像 Codex 一样选择真实模型。 */}
