...
export type GuiModelInfo = { // 新增代码+ProviderSettingsTypes：类型段开始，描述后端模型信息；如果没有这段，模型页无法类型化 provider 分组。
  ...
  state?: string; // 新增代码+ComposerRouteControls：模型探针状态；如果没有这行，前端无法显示 available 或 not_supported_for_account。
  source?: string; // 新增代码+ComposerRouteControls：模型来源层；如果没有这行，前端无法解释模型来自静态、探针还是 last-known-good。
  message?: string; // 新增代码+ComposerRouteControls：模型安全状态说明；如果没有这行，模型不可用原因没有展示位置。
...
export type GuiProviderInfo = { // 新增代码+ProviderSettingsTypes：类型段开始，描述后端 provider 行；如果没有这段，Provider 设置页只能消费不透明对象。
  ...
  account_label?: string; // 新增代码+ComposerRouteControls：安全账号摘要；如果没有这行，OAuth 连接后 UI 只能显示泛化 ChatGPT OAuth。
  direct_route_status?: string; // 新增代码+ComposerRouteControls：Direct SSE 路由状态；如果没有这行，用户不知道当前是否走直接 ChatGPT OAuth SSE。
  oauth_client_source?: string; // 新增代码+ComposerRouteControls：OAuth client 来源；如果没有这行，调试真实 OAuth 配置时缺少低敏事实。
...
export type ProviderAuthAttemptInfo = { // 新增代码+OpenAIConnectTypes：类型段开始，描述一次 provider 授权尝试；如果没有这段，前端只能把等待页数据当不透明对象。
  ...
  mode: string; // 新增代码+OpenAIConnectTypes：保存 mock/real 模式摘要；如果没有这行，GUI 无法提示当前只是 mock。
