...
export function Composer({ isRunning = false, activeTurnId = null, selectedProviderId = "", selectedModelId = "", reasoningEffort = "high", permissionMode = "full_access", modelOptions = [], onModelChange, onReasoningEffortChange, onPermissionModeChange, onCancelActiveTurn, onSubmit }: ComposerProps): JSX.Element { // 修改代码+ComposerRouteControls：函数段开始，渲染底部输入工具条、模型选择、推理强度和权限模式；如果没有这段，用户看不到发送 prompt 或选择真实模型的入口。
