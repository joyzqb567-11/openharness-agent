...
const providerRows: ProviderRowView[] = [ // 新增代码+ProviderSettingsPanelTest：定义 provider 行 fixture；如果没有这段，列表状态没有输入。
  { id: "openai", displayName: "OpenAI", kind: "built_in", source: "oauth", connected: true, maskedKey: "ChatGPT OAuth (jo***@example.com)", accountLabel: "jo***@example.com", directRouteStatus: "direct_sse_ready", oauthClientSource: "operator_configured", description: "使用 ChatGPT Pro/Plus 或 API 密钥连接", authMethods: [{ id: "api-key", label: "API 密钥", enabled: true, status: "available", methodType: "api", mode: "form", fields: ["secret"], helpText: "help", experimental: false }], models: [], primaryActionLabel: "断开", primaryActionDisabled: false }, // 修改代码+ComposerRouteControlsTest：模拟已连接 OAuth provider 并带账号/Direct SSE 状态；如果没有这行，断开和路由状态无法验证。
  { id: "github-copilot", displayName: "GitHub Copilot", kind: "built_in", source: "none", connected: false, maskedKey: "", accountLabel: "", directRouteStatus: "", oauthClientSource: "", description: "使用 Copilot 或 API 密钥连接", authMethods: [{ id: "device-code", label: "设备码", enabled: false, status: "unsupported_v1", methodType: "oauth", mode: "auto", fields: [], helpText: "unsupported", experimental: true }], models: [], primaryActionLabel: "暂未支持", primaryActionDisabled: true }, // 修改代码+ComposerRouteControlsTest：模拟 V1 未支持 provider 并补齐新状态字段；如果没有这行，禁用按钮无法验证。
  { id: "openrouter", displayName: "OpenRouter", kind: "built_in", source: "none", connected: false, maskedKey: "", accountLabel: "", directRouteStatus: "", oauthClientSource: "", description: "使用 OpenRouter 账号或 API 密钥连接", authMethods: [{ id: "api-key", label: "API 密钥", enabled: true, status: "available", methodType: "api", mode: "form", fields: ["secret"], helpText: "help", experimental: false }], models: [], primaryActionLabel: "+ 连接", primaryActionDisabled: false }, // 修改代码+ComposerRouteControlsTest：模拟可连接 provider 并补齐新状态字段；如果没有这行，连接按钮无法验证。
...
const openAiPickerProvider: ProviderRowView = { id: "openai", displayName: "OpenAI", kind: "built_in", source: "none", connected: false, maskedKey: "", accountLabel: "", directRouteStatus: "", oauthClientSource: "", description: "使用 ChatGPT Pro/Plus 或 API 密钥连接", authMethods: [{ id: "chatgpt-browser", label: "ChatGPT Pro/Plus (browser)", enabled: true, status: "mock_available", methodType: "oauth", mode: "auto", fields: [], helpText: "browser", experimental: true }, { id: "chatgpt-headless", label: "ChatGPT Pro/Plus (headless)", enabled: true, status: "mock_available", methodType: "oauth", mode: "auto", fields: [], helpText: "headless", experimental: true }, { id: "api-key", label: "API 密钥", enabled: true, status: "available", methodType: "api", mode: "form", fields: ["secret"], helpText: "help", experimental: false }], models: [], primaryActionLabel: "+ 连接", primaryActionDisabled: false }; // 修改代码+ComposerRouteControlsTest：定义 OpenAI 三方法连接 fixture 并补齐路由状态字段；如果没有这行，方法选择器无法被独立测试。
const openAiHeadlessOnlyProvider: ProviderRowView = { id: "openai", displayName: "OpenAI", kind: "built_in", source: "none", connected: false, maskedKey: "", accountLabel: "", directRouteStatus: "", oauthClientSource: "", description: "使用 ChatGPT Pro/Plus 或 API 密钥连接", authMethods: [{ id: "chatgpt-headless", label: "ChatGPT Pro/Plus (headless)", enabled: true, status: "mock_available", methodType: "oauth", mode: "auto", fields: [], helpText: "headless", experimental: true }], models: [], primaryActionLabel: "+ 连接", primaryActionDisabled: false }; // 修改代码+ComposerRouteControlsTest：定义单一 headless 方法 provider 并补齐路由状态字段；如果没有这行，静态渲染很难直接进入等待授权步骤。
...
describe("SettingsProvidersPanel", () => { // 新增代码+ProviderSettingsPanelTest：测试段开始，覆盖 Provider 列表和连接弹窗；如果没有这段，Task 6 没有自动验收。
  it("renders provider rows with connection actions and safe probe labels", () => { // 新增代码+ProviderSettingsPanelTest：测试段开始，验证 provider 列表视觉文案；如果没有这段，列表状态可能退化。
    ...
    expect(markup).toContain("账号：jo***@example.com"); // 新增代码+ComposerRouteControlsTest：确认 OAuth 脱敏账号显示；如果没有这行，用户无法确认当前 ChatGPT 账号。
    expect(markup).toContain("Direct ChatGPT OAuth SSE 已就绪"); // 新增代码+ComposerRouteControlsTest：确认 Direct SSE 路由状态显示；如果没有这行，用户不知道当前是否绕过 Codex CLI WebSocket。
    expect(markup).toContain("OAuth client：operator_configured"); // 新增代码+ComposerRouteControlsTest：确认 OAuth client 来源显示；如果没有这行，真实 OAuth 调试缺少来源线索。
