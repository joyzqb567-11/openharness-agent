...
const viewModel: ProviderSettingsViewModel = { // 新增代码+SettingsModelsPanelTest：定义模型面板 fixture；如果没有这段，分组和 switch 测试没有输入。
  ...
  providers: [ // 新增代码+SettingsModelsPanelTest：声明 provider 列表；如果没有这行，模型分组没有来源。
    { id: "custom-gateway", displayName: "Custom Gateway", kind: "custom", source: "custom", connected: false, maskedKey: "", accountLabel: "", directRouteStatus: "", oauthClientSource: "", description: "自定义", authMethods: [], primaryActionLabel: "+ 连接", primaryActionDisabled: false, models: [{ id: "custom-1", displayName: "Custom One", providerId: "custom-gateway", visible: false, supportsTools: false, supportsVision: false, state: "unknown", source: "static_known_models", message: "" }] }, // 修改代码+ComposerRouteControlsTest：放入未连接 provider 并补齐账号/路由/模型状态字段；如果没有这行，连接排序和模型状态测试无法编译。
    { id: "openai", displayName: "OpenAI", kind: "built_in", source: "config", connected: true, maskedKey: "sk-***", accountLabel: "jo***@example.com", directRouteStatus: "direct_sse_ready", oauthClientSource: "operator_configured", description: "OpenAI", authMethods: [], primaryActionLabel: "断开", primaryActionDisabled: false, models: [{ id: "gpt-4.1", displayName: "GPT 4.1", providerId: "openai", visible: true, supportsTools: true, supportsVision: true, state: "available", source: "probe_available_models", message: "" }] }, // 修改代码+ComposerRouteControlsTest：放入已连接 provider 并补齐 Direct SSE 状态；如果没有这行，已连接优先排序和探针文案无法验证。
...
describe("SettingsModelsPanel", () => { // 新增代码+SettingsModelsPanelTest：测试组开始；如果没有这段，Task 8 没有自动验收。
  it("renders grouped models with connected providers first", () => { // 新增代码+SettingsModelsPanelTest：测试分组和字段文案；如果没有这段，模型页可能丢字段或排序错误。
    ...
    expect(markup).toContain("可用"); // 新增代码+ComposerRouteControlsTest：确认模型探针状态可见；如果没有这行，用户看不到模型是否真实可用。
    expect(markup).toContain("探针发现"); // 新增代码+ComposerRouteControlsTest：确认模型来源可见；如果没有这行，用户不知道模型为什么出现在菜单里。
