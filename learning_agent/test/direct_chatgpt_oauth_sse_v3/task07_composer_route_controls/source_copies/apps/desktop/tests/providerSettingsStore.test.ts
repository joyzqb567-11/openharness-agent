...
const providerPayload: PollutedProviderSettingsPayload = { // 修改代码+ProviderSettingsStoreTest：定义后端 provider settings fixture；如果没有这段，多个测试会重复构造 payload。
  ...
  providers: [ // 修改代码+ProviderSettingsStoreTest：开始 provider 列表；如果没有这行，排序和分组没有输入。
    ...
    { id: "openai", display_name: "OpenAI", kind: "built_in", source: "oauth", connected: true, masked_key: "ChatGPT OAuth (jo***@example.com)", account_label: "jo***@example.com", direct_route_status: "direct_sse_ready", oauth_client_source: "operator_configured", auth_methods: openAiAuthMethods, description: "OpenAI", api_key: "unit-test-secret-value", secret_ref: "secret-ref-should-not-render", models: [{ id: "gpt-4.1", display_name: "GPT-4.1", provider_id: "openai", visible: true, supports_tools: true, supports_vision: true, state: "available", source: "probe_available_models", message: "" }] }, // 修改代码+ComposerRouteControlsTest：模拟已连接 OpenAI OAuth 且夹带危险字段和模型状态；如果没有这行，脱敏、路由状态和方法元数据测试不真实。
...
describe("providerSettingsStore", () => { // 修改代码+ProviderSettingsStoreTest：测试段开始，覆盖 provider settings view model；如果没有这段，设置页会直接消费后端原始 JSON。
  ...
  it("sorts providers, keeps custom CTA separate, and groups models", () => { // 修改代码+ProviderSettingsStoreTest：测试段开始，验证排序、CTA 和模型分组；如果没有这段，Provider/Models 页会出现顺序和空状态错误。
    ...
    expect(rows[0].accountLabel).toBe("jo***@example.com"); // 新增代码+ComposerRouteControlsTest：确认脱敏账号进入 view model；如果没有这行，设置页会丢失 OAuth 账号提示。
    expect(rows[0].directRouteStatus).toBe("direct_sse_ready"); // 新增代码+ComposerRouteControlsTest：确认 Direct SSE 状态进入 view model；如果没有这行，设置页无法显示真实路由。
    expect(groups[0].models[0].state).toBe("available"); // 新增代码+ComposerRouteControlsTest：确认模型探针状态进入 view model；如果没有这行，模型页无法解释可用性。
