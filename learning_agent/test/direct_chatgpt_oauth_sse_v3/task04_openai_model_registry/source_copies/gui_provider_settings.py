...
def _built_in_provider_catalog(workspace: str | Path, settings: dict[str, Any], secret_store: GuiProviderSecretStore) -> list[dict[str, Any]]:  # 修改代码+OpenAIModelRegistry：函数段开始，构造内置 provider catalog；如果没有这段，GET 路由没有核心数据。
    ...
    for provider_id, display_name, description, auth_methods, models in built_ins:  # 新增代码+ProviderSettings：遍历内置 provider；如果没有这行，元数据不会变成 payload。
        ...
        if auth_type == "oauth_real":  # 新增代码+OpenAIRealOAuthAttempt：处理真实 OAuth secret_refs 连接状态；如果没有这行，真实 OAuth 完成后 catalog 仍会显示未连接。
            ...
            masked_key = "ChatGPT OAuth" if connected else ""  # 新增代码+OpenAIRealOAuthAttempt：返回不含 token 的连接摘要；如果没有这行，用户不知道已通过 OAuth 连接。
            providers.append(GuiProviderInfo(id=provider_id, display_name=display_name, kind="built_in", source=source, connected=connected, masked_key=masked_key, auth_methods=auth_methods, description=description, models=models))  # 新增代码+OpenAIRealOAuthAttempt：加入真实 OAuth provider payload；如果没有这行，设置页无法刷新连接状态。
