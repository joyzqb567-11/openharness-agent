...
class GuiRunManager:  # 新增代码+DesktopGUILifecycle：类段开始，管理 GUI turn 生命周期；如果没有这个类，handler 会堆满业务状态逻辑。
    ...
    def _run_adapter_turn(self, turn: GuiTurn) -> GuiAgentRunResult:  # 新增代码+GuiAgentAdapter：函数段开始，把 GUI turn 转交给 adapter；如果没有这段，worker 只能调用旧 answer_runner。
        ...
        request = GuiAgentRunRequest(session_id=turn.session_id, turn_id=turn.turn_id, run_id=turn.run_id, prompt=turn.prompt, mode="real" if isinstance(adapter, DefaultHarnessGuiAgentAdapter) else "fake", provider_id=turn.provider_id, model_id=turn.model_id, reasoning_effort=turn.reasoning_effort, permission_mode=turn.permission_mode)  # 修改代码+DirectSSEPayload：构造 adapter 请求并带上模型/权限选择；如果没有这一行，direct SSE adapter 收不到前端选择的模型和运行参数。
