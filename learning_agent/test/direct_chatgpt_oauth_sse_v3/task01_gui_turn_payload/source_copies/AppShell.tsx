...
import { Composer, type ComposerPermissionMode, type ComposerReasoningEffort, type ComposerSubmitPayload } from "./Composer"; // 修改代码+DirectSSEPayload：引入底部输入组件和结构化提交类型；如果没有这行，用户没有输入 prompt 的入口且无法类型化模型路由字段。
...
export function AppShell(): JSX.Element { // 修改代码+DesktopToolTimeline：函数段开始，组织桌面 GUI 主壳；如果没有这段，各组件无法组合成 Codex 风格窗口。
  ...
  const [selectedProviderId] = useState("openai"); // 新增代码+DirectSSEPayload：暂存当前 provider 选择，Task 7 会接真实下拉；如果没有这行，Task 1 无法证明 OpenAI 选择能进入后端。
  const [selectedModelId] = useState("gpt-5.5"); // 新增代码+DirectSSEPayload：暂存当前模型选择，Task 7 会接真实下拉；如果没有这行，Task 1 无法证明 gpt-5.5 能进入后端。
  const [reasoningEffort] = useState<ComposerReasoningEffort>("ultra"); // 新增代码+DirectSSEPayload：暂存推理强度，Task 7 会接底部菜单；如果没有这行，超高/低/中/高选择无法进入后端。
  const [permissionMode] = useState<ComposerPermissionMode>("full_access"); // 新增代码+DirectSSEPayload：暂存权限模式，Task 7 会接底部权限菜单；如果没有这行，完全访问选择无法进入后端。
  ...
  return ( // 修改代码+DesktopToolTimeline：返回主界面结构；如果没有这行，组件不会输出任何 UI。
    <main className="app-shell"> {/* 修改代码+DesktopToolTimeline：定义三栏窗口网格容器；如果没有这行，侧栏、线程区和状态区无法稳定并排。 */}
      ...
      <section className="thread-panel" aria-label="当前对话"> {/* 修改代码+DesktopToolTimeline：定义当前对话主区域；如果没有这行，读屏器和布局都缺少对话容器。 */}
        ...
        <Composer isRunning={threadState.isRunning} activeTurnId={threadState.activeTurnId} selectedProviderId={selectedProviderId} selectedModelId={selectedModelId} reasoningEffort={reasoningEffort} permissionMode={permissionMode} onCancelActiveTurn={(turnId) => { void handleCancelTurn(turnId); }} onSubmit={(payload) => { void handleSubmit(payload); }} /> {/* 修改代码+DirectSSEPayload：把运行态、活动 turn、模型路由、取消回调和提交回调交给 composer；如果没有这行代码，底部输入区无法稳定触发发送或取消生命周期。 */}
