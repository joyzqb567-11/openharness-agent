...
def _built_in_provider_catalog(settings: dict[str, Any], secret_store: GuiProviderSecretStore) -> list[dict[str, Any]]:  # 修改代码+SecretStoreFactory：函数段开始，构造内置 provider catalog；如果没有这段，GET 路由没有核心数据。
    ...
    built_ins = [  # 新增代码+ProviderSettings：集中声明内置 provider 元数据；如果没有这段，display name 和模型会散落。
        ...
        ("openai", "OpenAI", "使用 ChatGPT Pro/Plus 或 API 密钥连接", _openai_auth_methods(), [_model("openai", "gpt-4.1", "GPT-4.1", _visibility(settings, "openai", "gpt-4.1", True), True, True)]),  # 修改代码+OpenAIConnectCatalog：声明 OpenAI provider 三种连接方式；如果没有这行，最常用 provider 仍只有单 API key 入口。
