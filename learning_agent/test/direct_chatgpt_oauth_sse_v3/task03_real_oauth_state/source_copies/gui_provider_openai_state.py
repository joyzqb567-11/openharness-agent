...
def _openai_visible_models(settings: dict[str, Any]) -> list[str]:  # 新增代码+OpenAIProviderState：函数段开始，从设置里收集 OpenAI 可见模型；如果没有这段，runtime state 只能返回空模型。
    visibility = settings.get("model_visibility", {}) if isinstance(settings.get("model_visibility", {}), dict) else {}  # 新增代码+OpenAIProviderState：读取模型可见性覆盖；如果没有这行，隐藏模型状态无法应用。
    candidates = ["gpt-5.5", "gpt-5.4", "gpt-5.4-mini", "gpt-5.3-codex-spark", "gpt-4.1"]  # 新增代码+OpenAIProviderState：提供本地候选模型；如果没有这行，连接后模型菜单没有基础列表。
    return [model_id for model_id in candidates if bool(visibility.get(f"openai:{model_id}", True))]  # 新增代码+OpenAIProviderState：只返回未隐藏模型；如果没有这行，用户隐藏的模型仍会显示。
...
def build_openai_provider_runtime_state(workspace: str | Path) -> OpenAIProviderRuntimeState:  # 新增代码+OpenAIProviderState：函数段开始，构造安全 OpenAI runtime state；如果没有这段，前端无法显示真实连接诊断。
    ...
    state = OpenAIProviderRuntimeState(provider_id="openai", auth_type=auth_type, runtime="direct_sse", direct_route_status=_direct_route_status(auth_type, auth_record), account_id=account_id or None, account_label=account_label, selected_model_id=selected_model or None, available_models=_openai_visible_models(settings), last_known_good_models=[], oauth_client_source=str(auth_record.get("oauth_client_source", "")), needs_reconnect=auth_type == "", updated_at=_utc_now_iso())  # 新增代码+OpenAIProviderState：组装不含 token/secret_ref 的状态对象；如果没有这行，前端只能读取危险原始 settings。
