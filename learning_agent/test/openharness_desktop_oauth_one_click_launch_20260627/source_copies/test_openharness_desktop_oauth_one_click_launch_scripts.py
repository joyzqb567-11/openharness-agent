...
def test_oauth_launcher_sets_real_openai_runtime_contract() -> None:  # 新增代码+DesktopOAuthOneClickLaunchTest：测试段开始，锁定真实 OAuth 启动环境；如果没有这段测试，一键脚本可能退回 mock 或 API key 单一路径。
    ...
    assert "OPENHARNESS_GUI_AGENT_RUNTIME" in launcher_text and "real" in launcher_text  # 新增代码+RealHarnessOneClickLaunchTest：确认一键 GUI 会打开真实 agent harness adapter；如果没有这一行，`__real_harness__` 验收会稳定失败。
    assert "OPENHARNESS_GUI_AGENT_DEFAULT_MODE" in launcher_text and "agent" in launcher_text  # 新增代码+GuiToolchainReuseTest：确认一键 GUI 默认进入真实 Agent Mode；如果没有这一行，普通 prompt 仍要靠隐藏标记才能接入 core agent。
