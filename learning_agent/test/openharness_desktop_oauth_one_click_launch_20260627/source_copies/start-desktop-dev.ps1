...
$EvidenceFolder = if ($env:OPENHARNESS_DESKTOP_LAUNCH_LOG_DIR) { $env:OPENHARNESS_DESKTOP_LAUNCH_LOG_DIR } else { Join-Path $Workspace "learning_agent\test\desktop_gui_shell_v2\launch_logs" } # 修改代码+DesktopOAuthOneClickLaunch：允许一键启动脚本把日志放到临时目录；如果没有这行，用户每次启动 GUI 都会在仓库里制造未跟踪日志。
