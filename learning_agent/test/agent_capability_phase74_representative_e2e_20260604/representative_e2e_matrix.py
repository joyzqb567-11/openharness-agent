...
from learning_agent.computer_use.app_window_control import build_launch_plan  # 新增代码+Phase74RepresentativeE2E: 复用 Phase69 安全启动计划；如果没有这行代码，代表性场景会重复实现应用启动边界。
from learning_agent.computer_use.generic_input_actions import build_drag_path, build_hotkey_events, build_menu_sequence  # 新增代码+Phase74RepresentativeE2E: 复用 Phase71 通用输入事件构建器；如果没有这行代码，画图和菜单动作会变成手写脆弱协议。
...
class WindowsRepresentativeE2EMatrix:  # 新增代码+Phase74RepresentativeE2E: 类段开始，组织代表性真实应用 E2E 场景；如果没有这个类，Notepad、Explorer、Browser、窗口和 Paint 验收会散落。
    ...
    def build_explorer_scenario(self, real_smoke: bool = False) -> dict[str, Any]:  # 新增代码+Phase74RepresentativeE2E: 函数段开始，构建资源管理器文件浏览代表场景；如果没有这个函数，矩阵缺少文件系统 UI 应用验收。
        ...
        payload = {"scenario_id": "explorer_controlled_folder", "target_process": "explorer.exe", "launch_plan": launch_plan, "controlled_folder": str(controlled_folder), "operations": ["launch_app", "focus_window", "open_controlled_folder", "select_controlled_file", "verify_path_inside_root"], "real_smoke_requested": bool(real_smoke), "real_visible_app_invoked": False, "scenario_passed": bool(launch_plan.get("safe_to_launch") and launch_plan.get("executable") == "explorer.exe" and str(controlled_folder).startswith(str(self.base_dir)))}  # 新增代码+Phase74RepresentativeE2E: 汇总资源管理器场景证据；如果没有这行代码，报告无法证明文件浏览类应用可覆盖且不越界。
    ...
    def build_browser_scenario(self, real_smoke: bool = False) -> dict[str, Any]:  # 新增代码+Phase74RepresentativeE2E: 函数段开始，构建浏览器安全页面代表场景；如果没有这个函数，矩阵缺少网页类应用验收。
        ...
        payload = {"scenario_id": "browser_safe_blank_page", "target_process": "msedge.exe", "launch_plan": launch_plan, "safe_page": str(safe_page), "operations": ["launch_app", "open_about_blank_or_controlled_page", "type_url", "verify_page_title"], "reads_private_profile": False, "cookies_read": False, "tokens_read": False, "uses_private_profile_content": False, "real_smoke_requested": bool(real_smoke), "real_visible_app_invoked": False, "scenario_passed": bool(launch_plan.get("safe_to_launch") and launch_plan.get("executable") == "msedge.exe" and not launch_plan.get("changes_system_settings"))}  # 新增代码+Phase74RepresentativeE2E: 汇总浏览器场景证据；如果没有这行代码，报告无法证明网页类应用可覆盖且不读隐私。
