...
class LearningAgent:  # 作用: 教学版最小 agent 主类
    ...
    def _desktop_task_runtime_for_current_run(self) -> Any:  # 新增代码+DesktopTaskRuntimeRoute：函数段开始，取得当前 run 应使用的桌面任务 runtime；如果没有这段函数，测试无法注入 full-mode runtime。
        ...
        try:  # 修改代码+PaintPikachuRealLoop：优先按包路径导入 Task 4 runtime 和真实 Paint 闭环；如果没有这一行，项目根运行时无法使用新模块。
            ...
            from learning_agent.computer_use.paint_pikachu_real_loop import WindowsPaintPikachuRealExecutionLoop  # 新增代码+PaintPikachuRealLoop：导入受控 Paint 皮卡丘真实执行闭环；如果没有这一行，full prompt 仍会停在“真实闭环未接线”。
        except ModuleNotFoundError as error:  # 新增代码+DesktopTaskRuntimeRoute：兼容脚本模式包名前缀缺失；如果没有这一行，真实 bat 入口可能导入失败。
            if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.desktop_task_runtime", "learning_agent.computer_use.paint_pikachu_real_loop"}:  # 修改代码+PaintPikachuRealLoop：把真实 Paint 闭环纳入脚本模式兜底名单；如果没有这一行，bat 入口可能把正常包路径缺失当成真实错误。
            ...
            from computer_use.paint_pikachu_real_loop import WindowsPaintPikachuRealExecutionLoop  # type: ignore  # 新增代码+PaintPikachuRealLoop：脚本模式导入同一真实 Paint 闭环；如果没有这一行，bat 入口无法把 prompt 接到真实 mspaint。
        ...
        real_loop = WindowsPaintPikachuRealExecutionLoop(base_dir=runtime_root / "paint_pikachu_real_loop")  # 新增代码+PaintPikachuRealLoop：创建受控 Paint 皮卡丘真实闭环；如果没有这一行，full-confirm 后仍不会真实打开画图软件。
        runtime = ComputerUseDesktopTaskRuntime(base_dir=runtime_root, real_execution_loop=real_loop)  # 修改代码+PaintPikachuRealLoop：把真实闭环注入默认桌面任务 runtime；如果没有这一行，用户 prompt 只能得到未接线拒绝。
