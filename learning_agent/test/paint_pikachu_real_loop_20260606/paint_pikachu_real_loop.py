"""受控 Paint 皮卡丘真实执行闭环。"""  # 新增代码+PaintPikachuRealLoop：说明本文件只负责 full 模式下的受控 Paint 绘图真实路径；如果没有这一行，读者容易把它误解成通用无限桌面控制器。
...
try:  # 新增代码+PaintPikachuRealLoop：优先按包路径导入项目已有 Computer Use 组件；如果没有这一段，单元测试和真实 bat 入口无法共享实现。
    from learning_agent.computer_use.drawing_primitives import build_pikachu_drag_plan, expand_drag_path_to_low_level_events  # 新增代码+PaintPikachuRealLoop：复用通用皮卡丘拖拽 primitive；如果没有这一行，真实路径可能退回脚本生成图片。
    from learning_agent.computer_use.generic_app_discovery import resolve_generic_app_launch_target  # 新增代码+PaintPikachuRealLoop：复用通用应用发现和高风险拒绝；如果没有这一行，Paint 启动会变成新白名单补丁。
    from learning_agent.computer_use.generic_launch_backend import Phase110OwnedProcessRegistry, Phase110ProductionGenericLaunchBackend, run_generic_launch_backend  # 新增代码+PaintPikachuRealLoop：复用 argv/no-shell 真实启动后端；如果没有这一行，启动 Paint 可能绕开已有安全后端。
    from learning_agent.computer_use.real_sendinput_guard import WindowsSendInputLowLevelSender  # 新增代码+PaintPikachuRealLoop：复用真实 Windows SendInput sender；如果没有这一行，拖拽事件无法触达真实桌面。
    from learning_agent.computer_use.screenshot_runtime import WindowsScreenshotCaptureRuntime  # 新增代码+PaintVisualGuard：复用项目现有窗口截图 runtime 做动作后视觉验真；如果没有这一行，Paint loop 只能继续相信自报字段。
    from learning_agent.computer_use.windows_backend import WindowsWindowInventoryProbe  # 新增代码+PaintPikachuRealLoop：复用只读窗口枚举；如果没有这一行，动作前无法找到并复核 Paint 窗口。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.drawing_primitives", "learning_agent.computer_use.generic_app_discovery", "learning_agent.computer_use.generic_launch_backend", "learning_agent.computer_use.real_sendinput_guard", "learning_agent.computer_use.screenshot_runtime", "learning_agent.computer_use.windows_backend"}:  # 修改代码+PaintVisualGuard：允许脚本模式兜底截图 runtime 路径；如果没有这一行，bat 入口会把正常包路径差异误当内部错误。
        raise  # 新增代码+PaintPikachuRealLoop：重新抛出非路径类导入错误；如果没有这一行，排查真实控制链问题会很困难。
    from computer_use.drawing_primitives import build_pikachu_drag_plan, expand_drag_path_to_low_level_events  # type: ignore  # 新增代码+PaintPikachuRealLoop：脚本模式复用同一绘图 primitive；如果没有这一行，bat 入口无法生成拖拽路径。
    from computer_use.generic_app_discovery import resolve_generic_app_launch_target  # type: ignore  # 新增代码+PaintPikachuRealLoop：脚本模式复用通用应用发现；如果没有这一行，bat 入口会失去高风险拒绝和安全计划。
    from computer_use.generic_launch_backend import Phase110OwnedProcessRegistry, Phase110ProductionGenericLaunchBackend, run_generic_launch_backend  # type: ignore  # 新增代码+PaintPikachuRealLoop：脚本模式复用真实启动后端；如果没有这一行，bat 入口无法安全启动 mspaint。
    from computer_use.real_sendinput_guard import WindowsSendInputLowLevelSender  # type: ignore  # 新增代码+PaintPikachuRealLoop：脚本模式复用真实 SendInput sender；如果没有这一行，bat 入口无法发送鼠标拖拽。
    from computer_use.screenshot_runtime import WindowsScreenshotCaptureRuntime  # type: ignore  # 新增代码+PaintVisualGuard：脚本模式复用同一截图 runtime；如果没有这一行，start_oauth_agent.bat 下不能做视觉验真。
    from computer_use.windows_backend import WindowsWindowInventoryProbe  # type: ignore  # 新增代码+PaintPikachuRealLoop：脚本模式复用窗口枚举；如果没有这一行，bat 入口无法复核 Paint 窗口。
...
def _paint_loop_visual_canvas_report(window: dict[str, Any], canvas: dict[str, int], screenshot_result: dict[str, Any]) -> dict[str, Any]:  # 新增代码+PaintVisualGuard：函数段开始，根据动作后截图判断 Paint 画布是否真的出现笔迹；如果没有这个函数，低层事件数会继续冒充真实绘图结果。
    ...
    try:  # 新增代码+PaintVisualGuard：捕获图片读取和像素解析异常；如果没有这一行，坏截图会让整个 agent 崩溃。
        ...
        recognizable = bool(non_background >= 350 and region_counts["upper"] >= 30 and region_counts["middle"] >= 120 and region_counts["face"] >= 40 and region_counts["tail"] >= 20)  # 新增代码+PaintRecognizableVisualGuard：要求总笔迹和耳朵/主体/脸/尾巴区域都达标；如果没有这一行，几条断线仍会被误判成熟。
        base_report["canvas_changed_after_actions"] = recognizable  # 修改代码+PaintRecognizableVisualGuard：只有可识别分布达标才承认画布变化；如果没有这一行，空白或残缺图形会继续通过。
        base_report["pikachu_visual_elements"] = recognizable  # 修改代码+PaintRecognizableVisualGuard：代表性皮卡丘元素必须和严格视觉门禁一致；如果没有这一行，字段会再次自报成功。
        base_report["visual_guard_reason"] = "recognizable_pikachu_regions_present" if recognizable else "pikachu_regions_incomplete"  # 修改代码+PaintRecognizableVisualGuard：记录是否具备可识别皮卡丘区域；如果没有这一行，失败时不知道缺的是画布变化还是结构分布。
...
class WindowsPaintPikachuRealExecutionLoop:  # 新增代码+PaintPikachuRealLoop：类段开始，封装 full 模式 Paint 皮卡丘真实执行流程；如果没有这个类，desktop_task_runtime 仍没有可注入真实闭环。
    ...
    def run_desktop_task(self, target_app: str, prompt: str) -> dict[str, Any]:  # 新增代码+PaintPikachuRealLoop：函数段开始，执行 Paint 皮卡丘真实桌面任务；如果没有这段函数，desktop_task_runtime 无法调用真实闭环。
        ...
        canvas = _paint_loop_canvas_rect(window)  # 新增代码+PaintPikachuRealLoop：估算 Paint 画布区域；如果没有这一行，皮卡丘路径没有坐标范围。
        plan = build_pikachu_drag_plan(canvas)  # 新增代码+PaintPikachuRealLoop：生成皮卡丘拖拽计划；如果没有这一行，真实任务没有要画的图形。
        ...
        if len(events) <= 1:  # 新增代码+PaintPikachuRealLoop：检查除聚焦外是否有足够绘图事件；如果没有这一行，空计划可能误触成功。
            return self._failure("paint_drag_plan_empty", "pikachu_drag_plan_has_no_events", {"launch_report": launch, "target_window": window, "drawing_plan": plan})  # 新增代码+PaintPikachuRealLoop：返回空计划失败；如果没有这一行，绘图 primitive 错误难排查。
        ...
        visual_report = _paint_loop_visual_canvas_report(window, canvas, screenshot_result)  # 新增代码+PaintVisualGuard：用截图像素判断画布是否变化；如果没有这一行，空白画布会继续通过。
        ...
        ok = bool(dispatch_ok and visual_ok)  # 修改代码+PaintVisualGuard：只有事件发送和画布变化都成立才成功；如果没有这一行，用户截图里的空白 Paint 会再次被误报。
        decision = "paint_pikachu_real_execution_finished" if ok else ("paint_pikachu_visual_verification_failed" if dispatch_ok else "paint_pikachu_sendinput_failed")  # 修改代码+PaintVisualGuard：区分发送失败和视觉失败；如果没有这一行，排查不知道卡在输入还是画布。
        return {"ok": ok, "marker": PAINT_PIKACHU_REAL_LOOP_MARKER, "ok_token": PAINT_PIKACHU_REAL_LOOP_OK if ok else "", "model": PAINT_PIKACHU_REAL_LOOP_MODEL, "decision": decision, "target_app": "mspaint", "prompt_length": len(str(prompt or "")), "computer_use_gui_route_used": ok, "owned_window_verified": True, "gui_action_count": _paint_loop_safe_int(plan.get("gui_action_count", 0)), "low_level_event_count": low_level_count, "real_dispatch_performed": dispatch_ok, "real_desktop_touched": dispatch_ok, "post_action_screenshot_exists": bool(visual_report.get("post_action_screenshot_exists", False)), "post_action_visual_evidence_path": str(visual_report.get("post_action_visual_evidence_path", "")), "canvas_changed_after_actions": visual_ok, "pikachu_visual_elements": bool(visual_report.get("pikachu_visual_elements", False)), "visual_non_background_pixel_count": _paint_loop_safe_int(visual_report.get("visual_non_background_pixel_count", 0)), "visual_guard_reason": str(visual_report.get("visual_guard_reason", "")), "post_action_screenshot_result": visual_report.get("post_action_screenshot_result", {}), "forbidden_script_generation_used": False, "bash_final_artifact_route_used": False, "forbidden_script_artifact_route_blocked": True, "target_window": {"window_id": window.get("window_id"), "hwnd": window.get("hwnd"), "pid": window.get("pid"), "title_preview": window.get("title_preview")}, "canvas_rect": canvas, "drawing_plan": {"drag_path_count": plan.get("drag_path_count"), "colors": plan.get("colors"), "elements": plan.get("elements")}, "launch_report": launch, "send_result": send_result}  # 修改代码+PaintVisualGuard：返回带真实截图和像素门禁的执行报告；如果没有这一行，runtime 无法向终端证明 Paint 画布真的改变。
