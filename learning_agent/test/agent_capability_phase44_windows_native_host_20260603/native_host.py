...
class WindowsComputerUseNativeHost:  # 新增代码+Phase44WindowsNativeHost: 类段开始，定义进程内 Windows native host；如果没有这个类，主 agent 没有独立 host 边界可调用。
    def __init__(self, helper: Any | None = None, platform: str | None = None, host_id: str = "windows-computer-use-native-host", real_actions_enabled: bool = False) -> None:  # 新增代码+Phase44WindowsNativeHost: 函数段开始，初始化 helper、平台和动作开关；如果没有这段函数，host 无法保存安全默认值。
    ...
    # 新增代码+Phase44WindowsNativeHost: 函数段结束，WindowsComputerUseNativeHost.__init__ 到此结束；如果没有这个边界说明，读者不容易看出初始化范围。
    ...
    def status(self) -> dict[str, Any]:  # 新增代码+Phase44WindowsNativeHost: 函数段开始，返回 host 状态；如果没有这段函数，client 和 /computer 无法查看协议能力。
        ...
        return {"marker": PHASE44_WINDOWS_NATIVE_HOST_MARKER, "host_id": self.host_id, "protocol": PHASE44_NATIVE_HOST_PROTOCOL, "platform": self.platform, "started": bool(self.started), "healthy": bool(self.started), "supported_messages": list(PHASE44_SUPPORTED_MESSAGES), "helper": dict(helper_status) if isinstance(helper_status, dict) else {}, "safe_observe_only": True, "real_actions_enabled": bool(self.real_actions_enabled), "actions_expanded": PHASE44_ACTIONS_EXPANDED}  # 新增代码+Phase44WindowsNativeHost: 返回完整状态；如果没有这行代码，健康检查和能力展示没有统一事实源。
    # 新增代码+Phase44WindowsNativeHost: 函数段结束，WindowsComputerUseNativeHost.status 到此结束；如果没有这个边界说明，读者不容易看出状态范围。
