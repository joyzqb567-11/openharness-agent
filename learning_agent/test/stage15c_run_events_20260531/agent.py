...
try:  # 修改代码+ToolsSplit: 优先从新 tools 包导入工具类型和目录构建函数；若没有这行代码，learning_agent.py 会继续承载工具元数据实现。
    ...
    if error.name not in {"learning_agent", "learning_agent.tools", "learning_agent.tools.atom_tools", "learning_agent.tools.catalog", "learning_agent.tools.executor", "learning_agent.tools.pool", "learning_agent.tools.result_storage", "learning_agent.tools.types"}:  # 修改代码+ResultStorageSplit: 只允许目标包路径缺失时 fallback；若没有这行代码，tools 内部真实 bug 会被误吞。
