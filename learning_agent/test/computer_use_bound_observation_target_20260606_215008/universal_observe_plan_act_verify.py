...
try:  # 新增代码+URG4ObservePlanActVerify：优先按 learning_agent 包路径导入已完成的 URG-1/3 能力；如果没有这段代码，单测和真实 bat 入口无法共享实现。
    ...
    from learning_agent.computer_use.natural_language_semantic_planner import phase123_plan_semantic_desktop_task  # 新增代码+GenericSemanticPlanner：导入通用自然语言语义规划入口；如果没有这一行，非绘图任务会继续被误塞进视觉画线 planner。
    ...
    from computer_use.natural_language_semantic_planner import phase123_plan_semantic_desktop_task  # type: ignore  # 新增代码+GenericSemanticPlanner：脚本模式导入通用自然语言语义规划入口；如果没有这一行，真实终端入口无法处理打开应用等非绘图任务。
