...
class UniversalObservePlanActVerifyLoopTests(unittest.TestCase):  # 新增代码+URG4ObservePlanActVerify：测试类段开始，集中验证 URG-4 observe-plan-act-verify 通用闭环；如果没有这个类，闭环行为不会有自动化护栏。
    ...
    def test_loop_passes_bound_target_window_to_observation_runtime(self) -> None:  # 新增代码+BoundObservationTarget：函数段开始，验证 observe 层必须绑定 agent-owned 目标窗口；如果没有这个测试，真实桌面有旧 Paint 时观察层会看错窗口。
        class RecordingObservationRuntime:  # 新增代码+BoundObservationTarget：类段开始，记录每次 observe 收到的 target_window；如果没有这个类，测试无法证明观察层参数是否正确传递。
            def __init__(self) -> None:  # 新增代码+BoundObservationTarget：函数段开始，初始化观察记录；如果没有这段函数，测试无法保存每次观察收到的窗口。
                self.seen_target_windows: list[dict[str, object]] = []  # 新增代码+BoundObservationTarget：保存所有 observe 收到的目标窗口；如果没有这一行，最终无法断言是否绑定到正确窗口。
            # 新增代码+BoundObservationTarget：函数段结束，RecordingObservationRuntime.__init__ 到此结束；如果没有这个边界说明，初学者不容易看出记录范围。
            def observe(self, target_hint: str = "", real_desktop_touched: bool = False, target_window: dict[str, object] | None = None) -> dict[str, object]:  # 新增代码+BoundObservationTarget：实现带 target_window 的观察接口；如果没有这一行，loop 无法把已绑定窗口交给观察层。
                copied_window = dict(target_window or {})  # 新增代码+BoundObservationTarget：复制收到的目标窗口；如果没有这一行，后续 session 改动可能污染记录。
                self.seen_target_windows.append(copied_window)  # 新增代码+BoundObservationTarget：记录本次观察窗口；如果没有这一行，测试无法验证传参。
                return {"real_observation_frame": True, "target_hint": target_hint, "target_window": copied_window, "real_desktop_touched": real_desktop_touched}  # 新增代码+BoundObservationTarget：返回 verifier 可接受的观察帧；如果没有这一行，loop 会因缺观察帧失败。
            # 新增代码+BoundObservationTarget：函数段结束，RecordingObservationRuntime.observe 到此结束；如果没有这个边界说明，初学者不容易看出观察 fake 范围。
        # 新增代码+BoundObservationTarget：类段结束，RecordingObservationRuntime 到此结束；如果没有这个边界说明，初学者不容易看出观察记录器范围。
        class BoundTargetActionRuntime:  # 新增代码+BoundObservationTarget：类段开始，提供固定 agent-owned 目标窗口；如果没有这个类，loop 没有 session.target_window 可传给观察层。
            def open_target_session(self, target: str, candidates: list[dict[str, object]] | None = None) -> dict[str, object]:  # 新增代码+BoundObservationTarget：实现目标 session 创建接口；如果没有这一行，UniversalObservePlanActVerifyLoop 无法启动任务。
                _ = candidates  # 新增代码+BoundObservationTarget：本 fake 不需要候选列表；如果没有这一行，读者可能误以为参数遗漏。
                return {"session_id": "bound-observation-session", "target": target, "target_window": {"window_id": "hwnd:bound-123", "app_id": "mspaint.exe", "title_preview": "Bound Fake Paint"}}  # 新增代码+BoundObservationTarget：返回稳定绑定窗口；如果没有这一行，观察层没有目标身份可接收。
            # 新增代码+BoundObservationTarget：函数段结束，BoundTargetActionRuntime.open_target_session 到此结束；如果没有这个边界说明，初学者不容易看出 session fake 范围。
            def dispatch(self, session: dict[str, object], action: dict[str, object], current_window: dict[str, object] | None = None) -> dict[str, object]:  # 新增代码+BoundObservationTarget：实现动作分发接口；如果没有这一行，loop 无法执行任务动作。
                _ = session  # 新增代码+BoundObservationTarget：本 fake 不需要读取完整 session；如果没有这一行，读者可能误以为参数遗漏。
                _ = action  # 新增代码+BoundObservationTarget：本 fake 不需要动作内容；如果没有这一行，读者可能误以为参数遗漏。
                _ = current_window  # 新增代码+BoundObservationTarget：本 fake 不需要重复验证窗口；如果没有这一行，读者可能误以为参数遗漏。
                return {"ok": True, "low_level_event_count": 1, "real_dispatch_performed": False, "real_desktop_touched": False}  # 新增代码+BoundObservationTarget：返回安全成功动作；如果没有这一行，verifier 没有成功动作结果。
            # 新增代码+BoundObservationTarget：函数段结束，BoundTargetActionRuntime.dispatch 到此结束；如果没有这个边界说明，初学者不容易看出分发 fake 范围。
        # 新增代码+BoundObservationTarget：类段结束，BoundTargetActionRuntime 到此结束；如果没有这个边界说明，初学者不容易看出动作 fake 范围。
        observation_runtime = RecordingObservationRuntime()  # 新增代码+BoundObservationTarget：创建记录观察 runtime；如果没有这一行，测试没有观察参数来源。
        loop = UniversalObservePlanActVerifyLoop(observation_runtime=observation_runtime, action_runtime=BoundTargetActionRuntime(), max_retries=0)  # 新增代码+BoundObservationTarget：把记录观察和固定目标动作层注入 loop；如果没有这一行，测试会退回默认录制实现。
        result = loop.run_task({"target": "mspaint", "goal": "bound observation", "actions": [{"type": "click_point", "x": 1, "y": 1}]})  # 新增代码+BoundObservationTarget：执行一次普通动作任务；如果没有这一行，observe 不会被调用。
        self.assertTrue(result["ok"])  # 新增代码+BoundObservationTarget：确认测试任务本身成功；如果没有这一行，后续窗口断言可能掩盖 loop 基础失败。
        self.assertGreaterEqual(len(observation_runtime.seen_target_windows), 3)  # 新增代码+BoundObservationTarget：断言任务前、动作前、动作后等多次观察都发生；如果没有这一行，单次传参不能证明闭环全程绑定。
        self.assertTrue(all(window.get("window_id") == "hwnd:bound-123" for window in observation_runtime.seen_target_windows))  # 新增代码+BoundObservationTarget：断言每次观察都收到绑定窗口；如果没有这一行，真实观察层仍可能看旧窗口。
    # 新增代码+BoundObservationTarget：函数段结束，test_loop_passes_bound_target_window_to_observation_runtime 到此结束；如果没有这个边界说明，初学者不容易看出绑定观察测试范围。
    ...
    def test_visual_planner_routes_open_app_semantics_without_drag_paths(self) -> None:  # 新增代码+GenericSemanticPlanner：函数段开始，验证打开应用类任务不会被视觉绘图兜底接管；如果没有这个测试，Notepad 任务可能又生成错误笔画。
        planner = Phase120VisualTaskPlanner()  # 新增代码+GenericSemanticPlanner：创建当前默认 planner；如果没有这一行，测试无法覆盖生产 observe-plan-act 规划器。
        frame = {"target_window": {"app_id": "notepad.exe", "rect": {"left": 20, "top": 20, "right": 900, "bottom": 700}}}  # 新增代码+GenericSemanticPlanner：构造 Notepad 观察帧；如果没有这一行，通用 planner 没有目标窗口上下文。
        task = {"target": "notepad", "goal": "natural_language_desktop_task", "visual_planning_requested": False, "semantic_intent": {"task_kind": "open_app", "target_app": "notepad", "raw_prompt_included": False}, "prompt_signature": "open-notepad"}  # 新增代码+GenericSemanticPlanner：构造脱敏打开应用任务；如果没有这一行，planner 不知道这是非绘图任务。
        actions = planner.plan(task, frame)  # 新增代码+GenericSemanticPlanner：运行 planner 得到动作序列；如果没有这一行，后续断言没有事实来源。
        semantic_roles = {str(action.get("semantic_role", "")) for action in actions}  # 新增代码+GenericSemanticPlanner：收集语义角色；如果没有这一行，无法确认走的是打开应用分支。
        self.assertIn("open_app_focus_target", semantic_roles)  # 新增代码+GenericSemanticPlanner：要求先聚焦目标应用窗口；如果没有这一行，打开应用后可能没有状态确认动作。
        self.assertIn("open_app_post_observe", semantic_roles)  # 新增代码+GenericSemanticPlanner：要求聚焦后继续观察；如果没有这一行，observe-plan-act loop 不会给后续纠偏留下状态证据。
        self.assertFalse(any(action.get("type") == "drag_path" for action in actions))  # 新增代码+GenericSemanticPlanner：禁止打开应用任务生成拖拽绘图；如果没有这一行，错误画线仍可能被当成通用执行。
    # 新增代码+GenericSemanticPlanner：函数段结束，test_visual_planner_routes_open_app_semantics_without_drag_paths 到此结束；如果没有这个边界说明，代码小白不容易看出非绘图语义测试范围。
