...
TOP_LEVEL_CODE_TOOL_NAMES = {"read", "write", "edit", "bash", "tool_search"}  # 修改代码+ToolSearchBash常驻：列出普通代码开发模式首轮保留的 ClaudeCode 对齐入口；如果没有这行代码，代码层面的工具边界说明会继续漏掉命令执行和动态工具发现。
...
COMPUTER_USE_OPERATION_RAW_TOOL_NAMES = {"request_access", "desktop_task", "screenshot", "observe", "cursor_position", "mouse_move", "left_click", "double_click", "right_click", "middle_click", "triple_click", "left_mouse_down", "left_mouse_up", "type", "key", "hold_key", "scroll", "left_click_drag", "zoom", "wait", "read_clipboard", "write_clipboard", "open_application", "list_granted_applications", "computer_batch"}  # 修改代码+DesktopTaskMcpTool: 列出 operation/debug 模式应暴露的 ClaudeCode parity Computer Use 工具并加入高层 desktop_task；如果没有 desktop_task，模型会继续被迫走 open_application/type/key 原子链。
...
def tool_scope_policy_decision(agent: Any, tool: Any, base_decision: ToolPolicyDecision) -> ToolPolicyDecision:  # 新增代码+ClaudeCodeToolSurface：定义 scope 策略入口；如果没有这行代码，catalog、search、executor 无法复用同一隔离规则。
    ...
    if mode in {TOOL_SCOPE_CODE_DEVELOPMENT, TOOL_SCOPE_COMPUTER_USE_SOURCE_DEVELOPMENT}:  # 新增代码+ClaudeCodeToolSurface：普通开发和 Computer Use 源码开发都不应暴露真实桌面动作；如果没有这行代码，开发源码时可能误点真实鼠标。
        ...
        return base_decision  # 修改代码+ToolSearchBash常驻：代码模式允许 read/write/edit/bash/tool_search 等基础开发工具沿用原有策略；如果没有这一行，命令执行和动态工具发现会被 scope 层误拦。
