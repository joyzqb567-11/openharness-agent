...
class WindowsGenericControlActionRuntime:  # 新增代码+Phase70GenericControlActions: 类段开始，提供通用 query 点击、query 输入和视觉点点击 runtime；如果没有这个类，Phase68 闭环执行器不能把用户意图落到控件动作。
    ...
    def click_by_query(self, window: dict[str, Any], observation: dict[str, Any], query: Any) -> dict[str, Any]:  # 新增代码+Phase70GenericControlActions: 函数段开始，按自然语言或结构化 query 点击控件；如果没有这个函数，用户 prompt 无法直接变成通用按钮点击。
        located_context = self._locate_control(observation, query)  # 新增代码+Phase70GenericControlActions: 先用 Phase57 locator 查找控件；如果没有这行代码，点击可能落到错误目标。
        ...
        if not bool(located.get("matched")):  # 新增代码+Phase70GenericControlActions: 检查是否找到控件；如果没有这行代码，未命中时仍可能点击默认坐标。
            return self._refusal(window, observation, "click_by_query", "control_not_found", located_context)  # 新增代码+Phase70GenericControlActions: 返回零事件拒绝；如果没有这行代码，找不到控件会产生真实副作用风险。
    ...
    def type_by_query(self, window: dict[str, Any], observation: dict[str, Any], query: Any, text: str) -> dict[str, Any]:  # 新增代码+Phase70GenericControlActions: 函数段开始，按自然语言或结构化 query 输入文本；如果没有这个函数，用户 prompt 无法直接变成通用输入。
        located_context = self._locate_control(observation, query, preferred_role="Edit")  # 新增代码+Phase70GenericControlActions: 先用 Phase57 locator 查找输入控件；如果没有这行代码，文本可能输入到按钮或标签。
        located = dict(located_context.get("located", {}) or {})  # 新增代码+Phase70GenericControlActions: 取出定位结果；如果没有这行代码，后续判断无法读取 matched。
        if not bool(located.get("matched")):  # 新增代码+Phase70GenericControlActions: 检查是否找到输入控件；如果没有这行代码，未命中时仍可能输入到当前焦点。
            return self._refusal(window, observation, "type_by_query", "control_not_found", located_context)  # 新增代码+Phase70GenericControlActions: 返回零事件拒绝；如果没有这行代码，找不到输入框会产生真实副作用风险。
    ...
    def click_by_visual_point(self, window: dict[str, Any], point: dict[str, Any], reason: str) -> dict[str, Any]:  # 新增代码+Phase70GenericControlActions: 函数段开始，按视觉点执行兜底点击；如果没有这个函数，画图画布等无 UIA 控件区域无法操作。
