...
class WindowsComputerUseFullDesktopTaskRouterTests(unittest.TestCase):  # 新增代码+DesktopTaskAcceptance：类段开始，集中冻结 `/computer use --full` 桌面任务路由验收；如果没有这个类，unittest 不会组织这些回归用例。
    ...
    def test_model_loop_computer_use_run_prompt_uses_full_desktop_runtime_when_full_mode_is_open(self) -> None:  # 新增代码+ModelLoopFullRuntimeBridge：函数段开始，验证模型主循环里的 computer_use(run_prompt) 会进入 full desktop runtime；如果没有这个测试，工具层可能继续接到 Phase92 预览拒绝层而不被发现。
        import tempfile  # 新增代码+ModelLoopFullRuntimeBridge：导入临时目录工具隔离本测试 workspace；如果没有这一行，测试会把 full mode 状态写进用户真实项目目录。
        from pathlib import Path  # 新增代码+ModelLoopFullRuntimeBridge：导入 Path 构造 Windows 兼容路径；如果没有这一行，workspace 路径表达会退回脆弱字符串拼接。
        with tempfile.TemporaryDirectory() as raw_dir:  # 新增代码+ModelLoopFullRuntimeBridge：创建一次性 workspace；如果没有这一行，测试之间会互相污染 Computer Use session 和证据目录。
            workspace = Path(raw_dir)  # 新增代码+ModelLoopFullRuntimeBridge：把临时目录转成 Path 对象；如果没有这一行，LearningAgent 构造参数不够清楚且后续路径拼接容易出错。
            agent = LearningAgent(model=ToolCallingFakeModel([]), workspace=workspace, ask_permission=lambda action: True, debug_enabled=False)  # 新增代码+ModelLoopFullRuntimeBridge：创建真实 agent 并自动允许测试权限；如果没有这一行，无法通过真实工具执行器复现模型主循环工具调用。
            runtime = ComputerUseDesktopTaskRuntime.for_test(full_mode=True)  # 新增代码+ModelLoopFullRuntimeBridge：创建已确认 full mode 的隔离 desktop runtime；如果没有这一行，真实动作分支会被 full mode 门禁正确拒绝而无法验证接线。
            runtime.real_execution_loop = FakeRealExecutionLoopForAgentRoute()  # 新增代码+ModelLoopFullRuntimeBridge：注入安全 fake 真实闭环避免单元测试触碰用户桌面；如果没有这一行，测试要么真实移动鼠标键盘，要么因为没有真实闭环而失败。
            agent.desktop_task_runtime = runtime  # 新增代码+ModelLoopFullRuntimeBridge：把 full runtime 注入 agent 供 computer_use 工具复用；如果没有这一行，工具入口会构造默认生产 runtime 并可能触发现实桌面风险。
            output = agent._execute_tool(ToolCall(name="computer_use", arguments={"operation": "run_prompt", "prompt": "请使用本地电脑的画图软件画一棵树。", "real_actions": True}))  # 新增代码+ModelLoopFullRuntimeBridge：通过真实工具分发调用模型会用的 computer_use(run_prompt)；如果没有这一行，测试不能证明工具层已经从 Phase92 改接 full runtime。
            self.assertIn("COMPUTER_USE_FULL_DESKTOP_TASK_ROUTER_OK", output)  # 新增代码+ModelLoopFullRuntimeBridge：断言 full desktop runtime 成功 token 出现；如果没有这一行，测试可能只证明返回了任意 JSON。
            self.assertIn("agent_route_real_execution_loop_used", output)  # 新增代码+ModelLoopFullRuntimeBridge：断言请求确实进入注入的真实执行闭环；如果没有这一行，Phase92 或录制路径也可能伪装成成功。
            self.assertIn("real_desktop_touched", output)  # 新增代码+ModelLoopFullRuntimeBridge：断言输出保留真实桌面触达证据字段；如果没有这一行，用户无法核对是否真的走了 GUI 动作链。
            self.assertNotIn("PHASE92_UNIVERSAL_WINDOWS_COMPUTER_USE_READY", output)  # 新增代码+ModelLoopFullRuntimeBridge：断言工具入口不再落回 Phase92 通用预览层；如果没有这一行，旧接错接口会继续复发。
            self.assertNotIn("real_actions_require_explicit_controller_gate", output)  # 新增代码+ModelLoopFullRuntimeBridge：断言旧 Phase92 真实动作拒绝码没有出现；如果没有这一行，真实执行仍可能被预览门禁拦住。
    # 新增代码+ModelLoopFullRuntimeBridge：函数段结束，test_model_loop_computer_use_run_prompt_uses_full_desktop_runtime_when_full_mode_is_open 到此结束；如果没有这个边界说明，代码小白不容易看出模型工具接线测试范围。
