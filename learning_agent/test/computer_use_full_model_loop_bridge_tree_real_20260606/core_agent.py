...
class LearningAgent:  # 作用: 教学版 agent 主类
    ...
    def _computer_use_mode(self, arguments: dict[str, Any]) -> str:  # 新增代码+Phase92UniversalComputerUse：定义通用 mode 私有工具处理器；如果没有这行代码，Phase92 target_tool 没有 agent 侧执行入口。
        ...
        if real_actions:  # 新增代码+ModelLoopFullRuntimeBridge：模型主循环请求真实桌面动作时改走 full desktop runtime；如果没有这一行，computer_use(run_prompt) 会继续落到 Phase92 预览拒绝层。
            desktop_runtime = self._desktop_task_runtime_for_current_run()  # 新增代码+ModelLoopFullRuntimeBridge：复用已有注入或默认生产 runtime；如果没有这一行，工具入口会重建另一套执行链并再次造成接口错接。
            report = desktop_runtime.run_prompt(prompt_text, real_actions=True)  # 新增代码+ModelLoopFullRuntimeBridge：把用户任务交给通用 observe-plan-act desktop runtime 执行；如果没有这一行，真实鼠标键盘和截图闭环不会被触发。
            self._record_observation("computer_use_full_desktop_runtime", {"ok": bool(report.get("passed", report.get("ok", False))), "prompt_text_included": False, "real_actions_requested": True, "computer_use_gui_route_used": bool(report.get("computer_use_gui_route_used", False)), "real_desktop_touched": bool(report.get("real_desktop_touched", False)), "low_level_event_count": int(report.get("low_level_event_count", 0) or 0)})  # 新增代码+ModelLoopFullRuntimeBridge：记录脱敏执行摘要供事后审计；如果没有这一行，真实终端失败时无法从事件链确认工具层是否进入 full runtime。
            return self._format_desktop_task_runtime_answer(report)  # 新增代码+ModelLoopFullRuntimeBridge：返回和桌面 runtime 一致的可验收文本；如果没有这一行，模型读不到稳定 token、decision 和 report_json。
