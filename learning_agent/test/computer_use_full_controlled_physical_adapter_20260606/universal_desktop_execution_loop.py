...
class UniversalDesktopExecutionLoopAdapter:  # 新增代码+UniversalDesktopAdapter：类段开始，把 desktop_task_runtime 需要的 run_desktop_task 接口桥接到通用闭环；如果没有这个类，默认 full 模式只能保持空 loop 或误接特例 loop。
    def __init__(self, loop: Any | None = None, observation_runtime: Any | None = None, use_real_observation: bool = True, controlled_physical_sender: Any | None = None) -> None:  # 修改代码+ControlledPhysicalAdapter：函数段开始，允许注入受控物理 sender；如果没有这段函数，Phase95 最后一跳无法接入 /computer use --full 主链路。
