...
class WindowsComputerUseFullMaturityMatrixTests(unittest.TestCase):  # 新增代码+FullMaturityMatrix：类段开始，集中验收 `/computer use --full` 最终成熟矩阵；如果没有这个类，蓝图 Task7 没有自动化边界。
    ...
    def _required_current_architecture_tokens(self) -> list[str]:  # 新增代码+ModelLoopMaturityMatrix：函数段开始，列出当前主循环 Computer Use 架构事实 token；如果没有这段函数，旧 Paint/Pikachu 字段会继续污染成熟度判断。
        return [  # 新增代码+ModelLoopMaturityMatrix：返回稳定 token 列表供多个测试复用；如果没有这一行，CLI 和结构化报告断言会分散漂移。
            ...
            "preloop_desktop_runtime_method_removed=true",  # 新增代码+CompatSlimmingCore：要求矩阵证明旧的模型前桌面任务 runtime 入口已被删除；如果没有这一行，旧入口残留可能再次被误接成主线。
            "agent_runtime_factory_method_removed=true",  # 新增代码+RuntimeFactorySlimming：要求矩阵证明默认 runtime 工厂已从 LearningAgent 主类移除；如果没有这一行，主类历史包袱可能继续误导后续开发。
            "default_desktop_task_runtime_factory_available=true",  # 新增代码+RuntimeFactorySlimming：要求矩阵证明迁移后的模块级默认 runtime 工厂存在；如果没有这一行，删除旧工厂后可能没有新入口承接验收职责。
    ...
    def _required_runtime_reachability_tokens(self) -> list[str]:  # 新增代码+RuntimeReachabilityMaturity：函数段开始，列出运行时可达性审计必须在终端展示的 token；如果没有这段函数，86 个文件会继续被误当成全部核心运行时代码。
        return [  # 新增代码+RuntimeReachabilityMaturity：返回稳定 token 列表给 CLI、命令测试和真实终端场景复用；如果没有这一行，运行时可达性字段容易只留在 JSON 里。
            ...
            "legacy_fifty_app_inventory_limit_removed=true",  # 新增代码+FullOrdinaryAppInventory：要求矩阵证明旧 50 候选上限已经移除；如果没有这一行，用户新装应用仍可能排在第 51 个以后不可见。
            "ordinary_launchable_apps_unlimited_inventory=true",  # 新增代码+FullOrdinaryAppInventory：要求矩阵证明普通可启动应用库存默认全量；如果没有这一行，普通应用放开能力没有终端验收证据。
            "computer_discover_default_unlimited_ordinary_apps=true",  # 新增代码+FullOrdinaryAppInventory：要求矩阵证明 discover 工具入口省略数量时不限制；如果没有这一行，agent 层默认 10 回归不会被发现。
            "computer_discover_schema_default_unlimited_ordinary_apps=true",  # 新增代码+FullOrdinaryAppInventory：要求矩阵证明 schema 告诉模型默认全量；如果没有这一行，模型仍会被旧最大 50 提示误导。
            "appx_aumid_inventory_discovery_available=true",  # 新增代码+AppLaunchInventoryClosure：要求矩阵证明 AppX/AUMID 已进入普通应用发现层；如果没有这一行，商店应用发现链路断开时测试不会失败。
            "start_menu_shortcut_inventory_preserved=true",  # 新增代码+AppLaunchInventoryClosure：要求矩阵证明开始菜单 shortcut 保留 .lnk 启动身份；如果没有这一行，shortcut 被误当 exe 的回归不会被测试发现。
            "resolver_non_argv_backends_supported=true",  # 新增代码+AppLaunchInventoryClosure：要求矩阵证明 resolver 和真实后端支持 AppX/shortcut；如果没有这一行，发现层与启动层断开时测试不会失败。
    ...
    def test_full_maturity_matrix_reports_required_fields(self) -> None:  # 新增代码+FullMaturityMatrix：函数段开始，验证结构化矩阵和 CLI 行都包含蓝图最终字段；如果没有这段测试，矩阵可能只输出部分阶段事实。
        ...
        self.assertTrue(report["preloop_desktop_runtime_method_removed"])  # 新增代码+CompatSlimmingCore：断言旧的模型前桌面任务 runtime 入口已经从顶层矩阵消失；如果没有这一行，后续只删调用不删函数也会误通过。
        self.assertTrue(report["agent_runtime_factory_method_removed"])  # 新增代码+RuntimeFactorySlimming：断言旧默认 runtime 工厂已从 LearningAgent 主类移除；如果没有这一行，主类仍可能残留兼容路线。
        self.assertTrue(report["default_desktop_task_runtime_factory_available"])  # 新增代码+RuntimeFactorySlimming：断言新模块级默认 runtime 工厂存在；如果没有这一行，测试无法证明迁移后仍有受控构造入口。
        ...
        self.assertTrue(report["legacy_fifty_app_inventory_limit_removed"])  # 新增代码+FullOrdinaryAppInventory：断言旧 50 候选上限已经移除；如果没有这一行，库存层回归旧截断不会让测试失败。
        self.assertTrue(report["ordinary_launchable_apps_unlimited_inventory"])  # 新增代码+FullOrdinaryAppInventory：断言普通应用库存默认全量；如果没有这一行，第 51 个以后应用不可见的问题可能复发。
        self.assertTrue(report["computer_discover_default_unlimited_ordinary_apps"])  # 新增代码+FullOrdinaryAppInventory：断言 discover 执行入口默认不限制；如果没有这一行，agent 层默认 10 回归不会被捕捉。
        self.assertTrue(report["computer_discover_schema_default_unlimited_ordinary_apps"])  # 新增代码+FullOrdinaryAppInventory：断言 schema 默认全量；如果没有这一行，模型可能继续按旧文案限制候选。
        self.assertTrue(report["appx_aumid_inventory_discovery_available"])  # 新增代码+AppLaunchInventoryClosure：断言顶层报告显示 AppX/AUMID 发现可用；如果没有这一行，商店应用发现回归不会让顶层测试失败。
        self.assertTrue(report["start_menu_shortcut_inventory_preserved"])  # 新增代码+AppLaunchInventoryClosure：断言顶层报告显示 shortcut 身份被保留；如果没有这一行，开始菜单快捷方式回归 exe 误判不会被发现。
        self.assertTrue(report["resolver_non_argv_backends_supported"])  # 新增代码+AppLaunchInventoryClosure：断言顶层报告显示非 argv 后端被 resolver 支持；如果没有这一行，启动后端断开不会被发现。
        ...
        model_loop_report = report["reports"]["model_loop_architecture"]  # 新增代码+CompatSlimmingCore：读取模型主循环架构子报告；如果没有这一行，旧预执行入口删除事实只能看顶层扁平字段。
        self.assertTrue(model_loop_report["preloop_desktop_runtime_method_removed"])  # 新增代码+CompatSlimmingCore：断言子报告也确认旧预执行函数已删除；如果没有这一行，顶层字段可能被打印逻辑伪造。
        self.assertTrue(model_loop_report["source_report"]["preloop_desktop_runtime_method_removed"])  # 新增代码+CompatSlimmingCore：断言 AST 源码自检也确认旧函数不存在；如果没有这一行，矩阵无法证明结论来自真实源码读取。
        self.assertTrue(model_loop_report["agent_runtime_factory_method_removed"])  # 新增代码+RuntimeFactorySlimming：断言子报告确认旧默认 runtime 工厂已从 agent 主类移除；如果没有这一行，顶层字段可能和源码事实漂移。
        self.assertTrue(model_loop_report["default_desktop_task_runtime_factory_available"])  # 新增代码+RuntimeFactorySlimming：断言子报告确认新模块级工厂存在；如果没有这一行，迁移后工厂缺失不会被发现。
        self.assertTrue(model_loop_report["source_report"]["agent_runtime_factory_method_removed"])  # 新增代码+RuntimeFactorySlimming：断言 AST 源码自检确认 agent 主类没有旧工厂；如果没有这一行，结论可能不是来自源码读取。
        self.assertTrue(model_loop_report["source_report"]["default_desktop_task_runtime_factory_available"])  # 新增代码+RuntimeFactorySlimming：断言 AST 源码自检确认新工厂函数存在；如果没有这一行，矩阵无法证明职责迁移完成。
        ...
        self.assertTrue(reachability_report["legacy_fifty_app_inventory_limit_removed"])  # 新增代码+FullOrdinaryAppInventory：断言可达性子报告也检查旧 50 上限；如果没有这一行，顶层字段可能不是来自源码审计。
        self.assertTrue(reachability_report["ordinary_launchable_apps_unlimited_inventory"])  # 新增代码+FullOrdinaryAppInventory：断言子报告确认普通应用默认全量；如果没有这一行，maturity 子报告缺少直接证据。
        self.assertTrue(reachability_report["computer_discover_default_unlimited_ordinary_apps"])  # 新增代码+FullOrdinaryAppInventory：断言子报告确认 discover 入口默认全量；如果没有这一行，主循环入口事实无法锁住。
        self.assertTrue(reachability_report["computer_discover_schema_default_unlimited_ordinary_apps"])  # 新增代码+FullOrdinaryAppInventory：断言子报告确认 schema 默认全量；如果没有这一行，模型可见说明的回归不会报警。
        self.assertTrue(reachability_report["appx_aumid_inventory_discovery_available"])  # 新增代码+AppLaunchInventoryClosure：断言子报告确认 AppX/AUMID 发现来自源码审计；如果没有这一行，顶层字段可能不是事实来源。
        self.assertTrue(reachability_report["start_menu_shortcut_inventory_preserved"])  # 新增代码+AppLaunchInventoryClosure：断言子报告确认 shortcut 身份保留来自源码审计；如果没有这一行，.lnk 后端闭环可能只停在口头判断。
        self.assertTrue(reachability_report["resolver_non_argv_backends_supported"])  # 新增代码+AppLaunchInventoryClosure：断言子报告确认 resolver 与真实后端支持非 argv 启动；如果没有这一行，启动链路断点不会进入可达性门禁。
