...
PHASE95_SUPPORTED_LOW_LEVEL_TYPES = ("mouse_move", "mouse_down", "mouse_up", "mouse_wheel", "keyboard_text", "unicode_text", "key_down", "key_up")  # 新增代码+Phase95ControlledPhysicalSendInput：定义低层事件白名单；如果没有这行代码，未知事件可能进入物理后端。
...
class WindowsControlledPhysicalSendInputSender:  # 新增代码+Phase95ControlledPhysicalSendInput：类段开始，定义受控物理 sender；如果没有这个类，Phase94 低层事件仍不能进入受控物理后端。
    ...
    def send_low_level(self, events: list[dict[str, Any]], enable_physical_dispatch: bool | None = None) -> dict[str, Any]:  # 新增代码+Phase95ControlledPhysicalSendInput：函数段开始，受控发送低层事件；如果没有这段函数，Phase94 不能把事件交到物理后端。
        ...
        real_desktop_touched = bool(backend_result.get("real_desktop_touched") or backend_result.get("real_dispatch_performed"))  # 新增代码+Phase95ControlledPhysicalSendInput：仅以后端明确声明判断真实桌面副作用；如果没有这行代码，fake 后端会被误认为真实控制。
