        ...
        "function": {  # 新增代码+Phase49ComputerUseToolSurface: function 内部描述兼容工具名称、说明和参数；如果没有这行代码，工具定义结构不完整。
            ...
            "parameters": {  # 新增代码+Phase49ComputerUseToolSurface: JSON Schema 描述统一入口参数；如果没有这行代码，模型不知道 operation 和动作字段怎么传。
                ...
                "properties": {  # 新增代码+Phase49ComputerUseToolSurface: 声明兼容工具允许的参数字段；如果没有这行代码，operation/action/window 等字段无法进入 schema。
                    "operation": {"type": "string", "enum": ["status", "observe", "action"], "description": "要执行的统一操作：status 查询状态，observe 只读观察，action 执行一个明确桌面动作；自然语言语义规划由模型主循环完成。"},  # 修改代码+ModelLoopSemanticPlanner：从模型可见枚举移除 mode/run_prompt；如果没有这行代码，模型可能继续绕过主循环语义规划。
                    "op": {"type": "string", "enum": ["status", "observe", "action"], "description": "operation 的 ClaudeCode/MCP 风格别名；仅用于 status、observe、action。"},  # 修改代码+ModelLoopSemanticPlanner：同步收窄 op 别名枚举；如果没有这行代码，别名字段仍会诱导旧 run_prompt 路线。
        ...
        "function": {  # 新增代码+Phase49ComputerUseToolSurface: function 内部描述连字符兼容工具；如果没有这行代码，工具定义结构不完整。
            ...
            "parameters": {  # 新增代码+Phase49ComputerUseToolSurface: JSON Schema 描述连字符工具参数；如果没有这行代码，模型不知道如何调用该别名。
                ...
                "properties": {  # 新增代码+Phase49ComputerUseToolSurface: 声明允许参数字段；如果没有这行代码，operation 和动作字段无法进入 schema。
                    "operation": {"type": "string", "enum": ["status", "observe", "action"], "description": "要执行的统一操作：status、observe 或 action；自然语言任务由模型主循环拆成多步工具调用。"},  # 修改代码+ModelLoopSemanticPlanner：从连字符工具可见枚举移除旧 prompt 入口；如果没有这行代码，ClaudeCode 风格别名仍会暴露黑盒路线。
                    "op": {"type": "string", "enum": ["status", "observe", "action"], "description": "operation 的短别名；仅用于 status、observe、action。"},  # 修改代码+ModelLoopSemanticPlanner：同步收窄 op 别名；如果没有这行代码，短字段仍可能诱导旧路线。
