    ...
    def test_computer_use_full_desktop_prompt_adds_model_loop_harness_and_uses_tools(self) -> None:  # 新增代码+ModelLoopSemanticPlanner：函数段开始，验证 full 模式后的自然语言桌面任务在模型主循环里规划；如果没有这个测试，语义任务可能再次被 Python runtime 抢跑。
        ...
        class RecordingDesktopLoopModel:  # 新增代码+ModelLoopSemanticPlanner：类段开始，用假模型记录消息和工具并主动选择 observe；如果没有这个类，测试无法证明模型拿到了语义规划上下文。
            ...
            def chat(self, messages: list[dict[str, object]], tools: list[dict[str, object]]) -> ModelMessage:  # 新增代码+ModelLoopSemanticPlanner：实现模型 chat 接口；如果没有这一行，LearningAgent.run 无法驱动 fake 模型。
                ...
                if self.index == 0:  # 新增代码+ModelLoopSemanticPlanner：第一轮模拟模型选择观察屏幕；如果没有这一行，测试无法覆盖 observe-plan-act 的第一步。
                    self.index += 1  # 新增代码+ModelLoopSemanticPlanner：推进到下一轮最终回答；如果没有这一行，fake 模型会无限调用同一个工具。
                    return ModelMessage(text="", tool_calls=[ToolCall(name="computer_use", arguments={"operation": "observe", "action": "list_windows"})])  # 新增代码+ModelLoopSemanticPlanner：让模型自己调用 computer_use observe；如果没有这一行，测试不能证明语义规划在模型工具循环里发生。
        ...
        with tempfile.TemporaryDirectory() as raw_dir:  # 新增代码+ModelLoopSemanticPlanner：创建临时 workspace 隔离 memory/debug；如果没有这一行，测试会污染真实项目运行目录。
            ...
            self.assertIn("computer_use", model.received_tool_names[0])  # 新增代码+ModelLoopSemanticPlanner：断言统一 Computer Use 工具首轮可见；如果没有这一行，模型不能像 ClaudeCode 一样自主选工具。
            self.assertIn("computer_observe", model.received_tool_names[0])  # 新增代码+ModelLoopSemanticPlanner：断言观察工具首轮可见；如果没有这一行，模型无法先观察屏幕状态。
            self.assertIn("computer_action", model.received_tool_names[0])  # 新增代码+ModelLoopSemanticPlanner：断言动作工具首轮可见；如果没有这一行，模型无法执行鼠标键盘动作。
            ...
            self.assertTrue(any(event.get("kind") == "computer_use_observe" for event in agent.observation_events))  # 新增代码+ModelLoopSemanticPlanner：断言模型选择的 observe 工具真的执行；如果没有这一行，只看回答无法证明工具循环跑过。
