...
class ComputerUseModeSessionPhase98Tests(unittest.TestCase):  # 新增代码+Phase98UniversalComputerUseMode：类段开始，集中验证 /computer use 的权限模式状态；如果没有这个类，Phase98 的核心状态没有门禁。
    ...
    def test_full_mode_still_blocks_required_dangerous_targets(self) -> None:  # 新增代码+Phase100FullComputerUseMode：函数段开始，验证 full 模式仍拦截密码、令牌、私钥和验证码目标；如果没有这段测试，最高权限模式可能绕过危险窗口保护。
        with tempfile.TemporaryDirectory() as temp_dir:  # 新增代码+Phase100FullComputerUseMode：创建隔离目录；如果没有这行代码，full 危险目标测试会污染真实 session 状态。
            store = ComputerUseModeSessionStore(base_dir=Path(temp_dir))  # 新增代码+Phase100FullComputerUseMode：创建模式 store；如果没有这行代码，测试无法生成并确认 full 模式。
            request = store.request_full_mode(reason="用户请求 full mode 安全边界测试")  # 新增代码+Phase100FullComputerUseMode：先请求 full 确认 token；如果没有这行代码，后续无法按真实流程进入 full。
            confirmed = store.confirm_full_mode(request["confirmation_token"], reason="用户二次确认 full mode 安全边界测试")  # 新增代码+Phase100FullComputerUseMode：用 token 真实确认 full；如果没有这行代码，测试只能证明 normal 模式拦截而不是 full 模式拦截。
            status = store.status()  # 新增代码+Phase100FullComputerUseMode：读取确认后的状态；如果没有这行代码，测试无法证明当前确实处于 full 模式。
            blocked_decisions = (  # 新增代码+Phase100FullComputerUseMode：收集四类危险目标的评估结果；如果没有这段代码，full 模式危险目标覆盖会分散且容易漏项。
                store.evaluate_action({"title_preview": "password vault"}, "click"),  # 新增代码+Phase100FullComputerUseMode：评估 password 目标；如果没有这行代码，密码窗口在 full 模式下的拦截没有证据。
                store.evaluate_action({"title_preview": "token settings"}, "click"),  # 新增代码+Phase100FullComputerUseMode：评估 token 目标；如果没有这行代码，令牌窗口在 full 模式下的拦截没有证据。
                store.evaluate_action({"title_preview": "private key file"}, "click"),  # 新增代码+Phase100FullComputerUseMode：评估 private key 目标；如果没有这行代码，私钥窗口在 full 模式下的拦截没有证据。
                store.evaluate_action({"title_preview": "captcha challenge"}, "click"),  # 新增代码+Phase100FullComputerUseMode：评估 captcha 目标；如果没有这行代码，验证码窗口在 full 模式下的拦截没有证据。
            )  # 新增代码+Phase100FullComputerUseMode：结束危险目标结果集合；如果没有这行代码，Python 无法关闭元组。
        self.assertTrue(confirmed["full_mode"])  # 新增代码+Phase100FullComputerUseMode：断言确认结果显示 full；如果没有这行代码，测试可能在未提权状态下误以为覆盖了 full。
        self.assertTrue(status["full_mode"])  # 新增代码+Phase100FullComputerUseMode：断言持久化状态确实是 full；如果没有这行代码，store 落盘状态错误不会被发现。
        for decision in blocked_decisions:  # 新增代码+Phase100FullComputerUseMode：逐个检查危险目标结果；如果没有这行代码，四类危险目标无法共享同一套稳定断言。
            self.assertFalse(decision["allowed"])  # 新增代码+Phase100FullComputerUseMode：断言危险目标不允许执行；如果没有这行代码，full 模式可能对敏感目标放行动作。
            self.assertEqual(decision["decision"], "dangerous_target_blocked")  # 新增代码+Phase100FullComputerUseMode：断言拒绝原因码稳定；如果没有这行代码，调用方无法准确提示危险目标被拦截。
            self.assertEqual(decision["low_level_event_count"], 0)  # 新增代码+Phase100FullComputerUseMode：断言拒绝时没有低层事件；如果没有这行代码，安全拒绝可能仍触发真实输入。
    # 新增代码+Phase100FullComputerUseMode：函数段结束，test_full_mode_still_blocks_required_dangerous_targets 到此结束；如果没有这个边界说明，读者不容易看出 full 危险目标回归测试范围。
