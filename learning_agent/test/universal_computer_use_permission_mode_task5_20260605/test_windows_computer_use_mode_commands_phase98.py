...
class ComputerUseModeCommandPhase98Tests(unittest.TestCase):  # 新增代码+Phase98UniversalComputerUseMode：类段开始，集中验证 Phase98 模式命令真实终端契约；如果没有这个类，Task3 的 `/computer` 接入没有红绿灯。
    ...
    def test_computer_full_request_requires_confirmation_and_confirm_activates_full(self) -> None:  # 修改代码+Phase100FullComputerUseMode：函数段开始，验证 full 请求必须显示高风险并且 token 确认后才激活；如果没有这段测试，高风险 full 可能被一步打开或缺少风险提示。
        with tempfile.TemporaryDirectory() as temp_dir:  # 新增代码+Phase98UniversalComputerUseMode：创建临时 workspace；如果没有这行代码，full token 会污染真实状态。
            ...
            request_output = run_computer_terminal_command(workspace, "/computer use --full")  # 新增代码+Phase98UniversalComputerUseMode：执行 full 请求命令；如果没有这行代码，测试无法证明 full 不会直接打开。
            token_match = re.search(r"/computer use --full-confirm (FULL-[A-F0-9]+)", request_output)  # 新增代码+Phase98UniversalComputerUseMode：从输出里提取确认 token；如果没有这行代码，测试无法继续验证确认路径。
            self.assertIsNotNone(token_match)  # 新增代码+Phase98UniversalComputerUseMode：断言输出包含 confirm 命令和 token；如果没有这行代码，用户可能拿不到下一步命令。
            confirm_output = run_computer_terminal_command(workspace, f"/computer use --full-confirm {token_match.group(1)}")  # 新增代码+Phase98UniversalComputerUseMode：执行二次确认命令；如果没有这行代码，full 激活路径不会被覆盖。
        self.assert_contains_all(request_output, ["Computer Use Full Request", "strong_confirmation_required=true", "risk=high", "full_mode=false", "opened=false", "low_level_event_count=0"])  # 修改代码+Phase100FullComputerUseMode：断言 full 请求明确显示高风险且没有提权或低层事件；如果没有这行代码，用户可能看不到 full 是高风险入口。
        self.assert_contains_all(confirm_output, ["Computer Use Mode", "mode=full", "full_mode=true", "opened=true", "ttl_seconds=", "per_app_allowlist_required=false"])  # 修改代码+Phase100FullComputerUseMode：断言 confirm 后进入 full、显示短 TTL 且不恢复应用白名单要求；如果没有这行代码，full 输出可能退回错误的 per-app 设计。
    # 修改代码+Phase100FullComputerUseMode：函数段结束，test_computer_full_request_requires_confirmation_and_confirm_activates_full 到此结束；如果没有这个边界说明，读者不容易看出 full 请求/确认测试范围。
