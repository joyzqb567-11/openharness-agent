...
try:  # 新增代码+UniversalDesktopAdapter：优先按包路径导入现有通用闭环；如果没有这段代码，项目根目录运行时无法复用已完成的 Phase119 能力。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.universal_observe_plan_act_verify", "learning_agent.computer_use.universal_real_observation"}:  # 修改代码+RealObservationAdapter：脚本模式兜底名单加入真实观察模块；如果没有这一行，bat 入口可能因新观察层路径缺失而失败。
...
class UniversalDesktopExecutionLoopAdapter:  # 新增代码+UniversalDesktopAdapter：类段开始，把 desktop_task_runtime 需要的 run_desktop_task 接口桥接到通用闭环；如果没有这个类，默认 full 模式只能保持空 loop 或误接特例 loop。
    def __init__(self, loop: Any | None = None, observation_runtime: Any | None = None, use_real_observation: bool = True) -> None:  # 修改代码+RealObservationAdapter：函数段开始，允许默认接真实只读观察，也允许测试显式注入；如果没有这段函数，adapter 无法把“眼睛”和“手脚”分层升级。
        ...
        default_loop = UniversalObservePlanActVerifyLoop(observation_runtime=real_observation_runtime, max_retries=0) if use_real_observation else UniversalObservePlanActVerifyLoop(max_retries=0)  # 修改代码+RealObservationAdapter：默认把通用闭环接到真实只读 ObservationFrame；如果没有这一行，/computer use --full 没有真实屏幕和窗口事实输入。
