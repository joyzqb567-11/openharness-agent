...
def computer_use_observe_before_action_rejection(action_name: str, arguments: dict[str, Any], desktop_task_context: Any, observation_events: Any, record_observation: RecordObservation) -> str | None:  # 新增代码+AgentPySplitPhase4: 定义观察门禁拒绝入口；如果没有这行代码，agent.py 的旧函数没有新模块可以调用。
    ...
    actionability_lines = [  # 新增代码+ObserveBeforeActionPending：构造 actionability 协议行；如果没有这段代码，盲动拒绝不会被收束器识别为必须继续 observe。
        "OPENHARNESS_DESKTOP_ACTION_REQUIRED",  # 新增代码+ObserveBeforeActionPending：输出桌面下一步动作 marker；如果没有这一行，record_actionability_from_tool_result 不会保存 pending。
        "actionability_kind=desktop_observe_before_action",  # 新增代码+ObserveBeforeActionPending：说明 pending 来源是动作前观察门；如果没有这一行，调试时看不出为什么要 observe。
        "next_required_tool=observe",  # 新增代码+ObserveBeforeActionPending：要求下一步调用 observe 工具；如果没有这一行，模型可能继续盲目按键或直接结束。
        "next_allowed_tools=mcp__computer-use__observe,observe",  # 新增代码+ObserveBeforeActionPending：同时兼容 MCP 完整名和短名；如果没有这一行，工具名归一失败时会误拦正确 observe。
        "next_required_action=get_window_state",  # 新增代码+ObserveBeforeActionPending：要求 observe 读取目标窗口状态；如果没有这一行，模型可能调用无关观察动作。
        "next_required_reason=observe_before_action_required",  # 新增代码+ObserveBeforeActionPending：保存稳定原因码；如果没有这一行，验收日志难以统计这类恢复路径。
    ]  # 新增代码+ObserveBeforeActionPending：actionability 协议行结束；如果没有这一行，Python 列表语法不完整。
    return "Computer Use full 模式已拒绝盲目桌面动作：请先观察屏幕和目标窗口，再执行鼠标键盘动作。\n请先调用 mcp__computer-use__observe，action=get_window_state，并使用返回的截图继续规划。\n" + "\n".join(actionability_lines) + "\n" + json.dumps(report, ensure_ascii=False, indent=2)  # 修改代码+ObserveBeforeActionPending：返回中文说明、pending marker 和结构化 JSON 给模型自我纠正；如果没有这行代码，模型可能继续重复盲动或用 Done 假结束。
...
def computer_use_agent_owned_launch_rejection(action_name: str, arguments: dict[str, Any], desktop_task_context: Any, controller: Any, record_observation: RecordObservation) -> str | None:  # 新增代码+AgentPySplitPhase4: 定义先启动门禁拒绝入口；如果没有这行代码，agent.py 的旧函数没有新模块可以调用。
    ...
    if computer_use_has_agent_owned_launch_target(controller):  # 新增代码+AgentPySplitPhase4: 已经有 launch_app 绑定窗口时放行；如果没有这行代码，成功打开软件后仍无法操作。
        safe_arguments = arguments if isinstance(arguments, dict) else {}  # 新增代码+FreshTargetTargetRefGate: 读取动作参数字典；如果没有这一行，坏参数会让 target_ref 门禁崩溃。
        raw_window = safe_arguments.get("window") if isinstance(safe_arguments.get("window"), dict) else {}  # 新增代码+FreshTargetTargetRefGate: 检查模型是否携带了观察到的窗口对象；如果没有这一行，旧窗口对象可能绕过一对一 ID。
        target_ref_text = str(safe_arguments.get("target_ref") or "").strip()  # 新增代码+FreshTargetTargetRefGate: 读取动作是否显式携带 target_ref；如果没有这一行，无法判断动作是否绑定正确窗口 ID。
        target_registry = getattr(controller, "target_registry", None)  # 新增代码+FreshTargetTargetRefGate: 读取 controller 的目标注册表；如果没有这一行，多目标数量无法判断。
        try:  # 新增代码+FreshTargetTargetRefGate: 保护注册表计数读取；如果没有这一行，测试 fake 或旧 controller 会因缺方法崩溃。
            active_target_count = int(target_registry.target_count()) if target_registry is not None and hasattr(target_registry, "target_count") else 1  # 新增代码+FreshTargetTargetRefGate: 获取有效目标数量；如果没有这一行，多应用任务缺少歧义判断。
        except Exception:  # 新增代码+FreshTargetTargetRefGate: 捕获异常保持门禁保守可用；如果没有这一行，异常会中断真实终端主循环。
            active_target_count = 1  # 新增代码+FreshTargetTargetRefGate: 异常时按单目标兼容处理；如果没有这一行，老测试或旧 controller 可能被误杀。
        if not target_ref_text and (bool(raw_window) or active_target_count > 1):  # 新增代码+FreshTargetTargetRefGate: raw window 或多目标缺 target_ref 时拒绝；如果没有这一行，模型可能把动作打到观察到的旧窗口。
            report = {"ok": False, "decision": "target_ref_required_for_bound_window_action", "action": action_name, "reason": "Computer Use 写动作必须携带 launch_app 返回的 target_ref，不能只靠窗口对象或当前前台窗口。", "required_argument": "target_ref", "active_target_count": active_target_count, "has_raw_window": bool(raw_window), "next_tools": ["mcp__computer-use__observe"], "recovery": "请使用 launch_app/open_application 返回的 target_ref 重新观察并执行下一步动作。"}  # 新增代码+FreshTargetTargetRefGate: 构造零权限前置拒绝报告；如果没有这一行，模型不知道要补 target_ref 而不是继续重试。
            record_observation("computer_use_target_ref_required_for_bound_window_action", report)  # 新增代码+FreshTargetTargetRefGate: 记录拒绝事件；如果没有这一行，验收日志无法证明权限前就拦住了旧窗口动作。
            return "Computer Use full 模式已拒绝缺少 target_ref 的窗口写动作：请使用 launch_app 返回的 target_ref 一对一绑定目标窗口；不要只传观察到的 window 对象，也不要在多目标任务中省略 target_ref。\n" + json.dumps(report, ensure_ascii=False, indent=2)  # 新增代码+FreshTargetTargetRefGate: 返回中文纠偏说明；如果没有这一行，模型可能继续对旧窗口或歧义窗口请求权限。
    ...
    report = {"ok": False, "decision": "agent_owned_launch_required", "action": action_name, "reason": "full 本机应用任务必须先用 launch_app 打开并绑定 FreshTarget/TargetLease，不能默认操作用户旧窗口。", "next_tools": ["mcp__computer-use__open_application"], "required_action": "launch_app", "target_app_hint": target_app_hint, "fresh_target_policy": "如果目标软件已打开，默认要求用户手动关闭后重试；只有用户明确授权已有窗口时才能绑定 user_granted_existing_window。", "target_ref_policy": "launch_app 成功后每个窗口都有 target_ref；多应用或多窗口任务的后续写动作必须带正确 target_ref。", "argument_preview": {"has_window": isinstance(arguments.get("window"), dict), "has_coordinates": "x" in arguments or "y" in arguments, "has_points": bool(arguments.get("points"))}}  # 修改代码+FreshTargetGateText: 构造模型可读纠偏报告并写明 FreshTarget/target_ref 规则；如果没有这行代码，模型不知道应先打开软件而不是继续点击旧窗口。
    ...
    return "Computer Use full 模式已拒绝操作未绑定目标窗口：请先调用 mcp__computer-use__open_application，action=launch_app，传入 app_name/target_app 建立 FreshTarget/TargetLease；如果软件已经打开，请让用户关闭旧窗口或明确授权已有窗口；后续多目标动作必须携带对应 target_ref。\n" + json.dumps(report, ensure_ascii=False, indent=2)  # 修改代码+FreshTargetGateText: 返回中文说明和 JSON 给模型纠偏；如果没有这行代码，模型可能继续对旧窗口或错误窗口做动作。
