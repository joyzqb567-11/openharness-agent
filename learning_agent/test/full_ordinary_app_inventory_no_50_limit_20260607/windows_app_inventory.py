...
def query_windows_app_inventory(query: Any = "", candidates: list[dict[str, Any]] | None = None, include_common: bool = True, max_count: int | None = None) -> dict[str, Any]:  # 修改代码+FullOrdinaryAppInventory：函数段开始，按自然语言查询全量 Windows 普通应用候选；如果没有这段函数，Computer Use 缺少类似 ClaudeCode appNames 的 discover 工具入口。
    ...
    return {"ok": True, "tool": "computer_discover", "marker": "PHASE123_COMPUTER_DISCOVER_READY", "model": "phase123_windows_app_inventory_query", "query": query_text, "normalized_query": normalized_query, "max_results": result_limit, "result_count": len(public_candidates), "total_match_count": len(scored), "unlimited_results": result_limit is None, "candidates": public_candidates, "not_hard_whitelist": True, "next_step": "Use app_name with computer_action(action=launch_app), then continue observe-plan-act with screenshots and tool results."}  # 修改代码+FullOrdinaryAppInventory：返回结构化 discover 报告并声明是否无限制；如果没有这一行，模型无法稳定读取候选 app_name 和 launch_kind。
