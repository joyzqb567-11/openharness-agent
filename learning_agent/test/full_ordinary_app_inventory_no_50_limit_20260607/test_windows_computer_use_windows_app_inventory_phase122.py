...
import tempfile  # 修改代码+WindowsAppInventory：导入临时目录工具，用于隔离主循环 harness 测试；如果没有这一行，测试可能把 agent 运行文件写进真实项目目录。
import unittest  # 新增代码+WindowsAppInventory：导入 unittest 测试框架；如果没有这一行，本文件不会被测试运行器识别。
from pathlib import Path  # 修改代码+WindowsAppInventory：导入 Path 处理临时工作区路径；如果没有这一行，Windows 路径拼接容易出错。

from learning_agent.core.agent import LearningAgent, ModelMessage, ToolCallingFakeModel  # 修改代码+WindowsAppInventory：导入主 agent 和假模型，验证 inventory 已接入模型主循环；如果没有这一行，只能测孤立模块，无法防止主循环继续走旧接口。
...
class WindowsComputerUseWindowsAppInventoryPhase122Tests(unittest.TestCase):  # 新增代码+WindowsAppInventory：测试类开始，验证 Windows 版 ClaudeCode app inventory 设计；如果没有这个类，多源枚举架构没有回归保护。
    ...
    def test_inventory_keeps_all_ordinary_apps_without_fifty_item_cap(self) -> None:  # 新增代码+FullOrdinaryAppInventory：函数段开始，验证普通应用库存不再被 50 个候选截断；如果没有这段测试，新安装在排序后面的普通应用会继续被 discover 漏掉。
        module = self._inventory_module()  # 新增代码+FullOrdinaryAppInventory：读取当前 inventory 模块；如果没有这一行，测试无法调用真实库存构建和查询入口。
        candidates = [  # 新增代码+FullOrdinaryAppInventory：准备超过旧 50 上限的普通应用候选；如果没有这一行，测试不能复现第 51 个以后被截断的问题。
            {"display_name": f"Ordinary App {index:02d}", "launch_id": f"ordinary-app-{index:02d}.exe", "source": "start_menu", "launch_kind": "exe"}  # 新增代码+FullOrdinaryAppInventory：构造安全普通应用候选；如果没有这一行，库存没有足够样本证明无上限。
            for index in range(1, 61)  # 新增代码+FullOrdinaryAppInventory：生成 60 个候选以超过旧 50 上限；如果没有这一行，旧截断缺陷不会暴露。
        ]  # 新增代码+FullOrdinaryAppInventory：候选列表结束；如果没有这一行，Python 列表语法不完整。
        inventory = module.build_windows_app_inventory(candidates, include_common=False)  # 新增代码+FullOrdinaryAppInventory：构建完整普通应用库存；如果没有这一行，测试无法确认库存层是否仍被截断。
        report = module.query_windows_app_inventory("Ordinary App 60", candidates=candidates, include_common=False, max_count=3)  # 新增代码+FullOrdinaryAppInventory：从全量库存查询排序末尾应用；如果没有这一行，discover 层是否使用全量库存不可见。
        self.assertEqual(60, len(inventory))  # 新增代码+FullOrdinaryAppInventory：断言普通应用库存保留全部 60 个；如果没有这一行，旧 50 上限可能继续存在。
        self.assertEqual("Ordinary App 60", report["candidates"][0]["display_name"])  # 新增代码+FullOrdinaryAppInventory：断言第 60 个普通应用能被 discover 查到；如果没有这一行，库存全量但查询仍截断的问题会漏掉。
    # 新增代码+FullOrdinaryAppInventory：函数段结束，test_inventory_keeps_all_ordinary_apps_without_fifty_item_cap 到此结束；如果没有这个边界说明，用户不容易看出无上限库存测试范围。
    def test_inventory_resolves_user_app_hint_inside_unified_inventory(self) -> None:  # 修改代码+WindowsAppInventory：函数段开始，验证应用名解析已经归入统一 inventory；如果没有这段测试，中文别名解析可能没人保护。
    ...
    # 修改代码+WindowsAppInventory：函数段结束，test_inventory_resolves_user_app_hint_inside_unified_inventory 到此结束；如果没有这个边界说明，用户不容易看出解析迁移测试范围。

    def test_full_model_loop_harness_uses_windows_app_inventory_directly(self) -> None:  # 修改代码+WindowsAppInventory：函数段开始，验证 full 主循环直接接入统一 inventory；如果没有这段测试，主循环可能悄悄失去应用清单。
        with tempfile.TemporaryDirectory() as temp_dir:  # 修改代码+WindowsAppInventory：创建临时 agent 工作区；如果没有这一行，测试可能污染真实 memory 或 workspace 文件。
            agent = LearningAgent(ToolCallingFakeModel([ModelMessage(text="ok")]), Path(temp_dir), ask_permission=lambda _reason: True, debug_enabled=False)  # 修改代码+WindowsAppInventory：创建不会调用真实模型的 agent；如果没有这一行，测试会变成昂贵且不稳定的在线模型测试。
            agent.loaded_tool_names.update({"computer_use", "computer_observe", "computer_discover", "computer_action"})  # 修改代码+ComputerDiscoverTool：模拟用户开启 `/computer use --full` 后四件核心工具已加载；如果没有这一行，harness 会按保护逻辑跳过 Computer Use 规则。
            agent.desktop_task_context = {"active": True, "requires_gui_actions": True, "target_app_hint": "画图", "task_goal": "draw_with_local_paint"}  # 修改代码+WindowsAppInventory：模拟真实自然语言桌面任务上下文；如果没有这一行，主循环不会注入应用候选清单。
            harness = agent._build_computer_use_full_model_loop_harness_message("请使用本地电脑的画图软件画一棵树。")  # 修改代码+WindowsAppInventory：生成 full 模型主循环提示；如果没有这一行，测试无法确认模型实际会看到 inventory 候选。
        self.assertIn("Available desktop application candidates", harness)  # 修改代码+WindowsAppInventory：断言主循环提示包含应用候选标题；如果没有这一行，模型可能仍只能凭空猜 app 名。
        self.assertIn("not a hard whitelist", harness)  # 修改代码+WindowsAppInventory：断言应用清单只是提示而非硬白名单；如果没有这一行，设计可能退回限制用户的白名单。
        self.assertIn("app_name=mspaint", harness)  # 修改代码+WindowsAppInventory：断言中文画图提示被规范成 mspaint；如果没有这一行，模型可能继续把中文名传给启动器。
    # 修改代码+WindowsAppInventory：函数段结束，test_full_model_loop_harness_uses_windows_app_inventory_directly 到此结束；如果没有这个边界说明，用户不容易看出主循环接线测试范围。
