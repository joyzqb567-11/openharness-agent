...
def test_open_application_reuses_request_access_context_for_controlled_resource(monkeypatch) -> None:  # 新增代码+ControlledResourceLaunchTest：函数段开始，验证 open_application 会复用 request_access 的完整任务上下文；如果没有这段测试，模型短 reason 会再次丢掉桌面 1.txt。
    controller = _RegistryBackedController()  # 新增代码+ControlledResourceLaunchTest：创建轻量 controller 替身；如果没有这一行，adapter 没有可传给底层执行的对象。
    captured_arguments: dict = {}  # 新增代码+ControlledResourceLaunchTest：保存底层执行收到的启动参数；如果没有这一行，测试无法确认 session_task_context 是否传下去。

    def _fake_internal_execute_desktop_action(arguments, *_args, **_kwargs):  # 新增代码+ControlledResourceLaunchTest：函数段开始，替换真实桌面启动避免打开用户应用；如果没有这段函数，单元测试会真的启动 Notepad。
        captured_arguments.update(dict(arguments))  # 新增代码+ControlledResourceLaunchTest：记录 adapter 交给 controller 的参数；如果没有这一行，断言没有事实来源。
        return '{"ok": true, "message": "captured", "data": {}}'  # 新增代码+ControlledResourceLaunchTest：返回 fake 成功文本；如果没有这一行，adapter 包装流程无法继续。
    # 新增代码+ControlledResourceLaunchTest：函数段结束，_fake_internal_execute_desktop_action 到此结束；如果没有这个边界说明，用户不容易看出 fake 执行范围。

    monkeypatch.setattr(adapter_module.computer_use_internal_adapter_tools, "internal_execute_desktop_action", _fake_internal_execute_desktop_action)  # 新增代码+ControlledResourceLaunchTest：替换底层执行函数；如果没有这一行，测试会触碰真实桌面。
    state = ComputerUseMcpSessionState()  # 新增代码+ControlledResourceLaunchTest：创建会话状态；如果没有这一行，adapter 无法保存授权上下文。
    state.grants["last_request_arguments"] = {"reason": "请打开本地真实记事本，输入 hello everyone，最后保存文件名为 1.txt 到本地电脑桌面。"}  # 新增代码+ControlledResourceLaunchTest：模拟 request_access 已保存完整用户任务；如果没有这一行，open_application 没有上下文可复用。
    adapter = ComputerUseMcpSessionAdapter(controller=controller, state=state)  # 新增代码+ControlledResourceLaunchTest：构造真实 adapter；如果没有这一行，测试不会覆盖生产映射路径。
    result = adapter.call_atomic_tool("open_application", {"app_name": "notepad", "reason": "打开一个新的真实记事本窗口。"})  # 新增代码+ControlledResourceLaunchTest：模拟模型启动应用时 reason 没有文件名；如果没有这一行，真实失败条件不会被覆盖。
    assert result["ok"] is True  # 新增代码+ControlledResourceLaunchTest：确认 fake 启动路径成功；如果没有这一行，后续参数断言可能掩盖包装失败。
    assert "1.txt" in captured_arguments["session_task_context"]  # 新增代码+ControlledResourceLaunchTest：确认 adapter 把授权任务上下文补给 controller；如果没有这一行，controller 无法做受控资源启动。
# 新增代码+ControlledResourceLaunchTest：函数段结束，test_open_application_reuses_request_access_context_for_controlled_resource 到此结束；如果没有这个边界说明，用户不容易看出 adapter 上下文兜底范围。
...
def test_safe_new_blank_shortcut_runs_on_restored_resource_then_blocks_typing_until_observe(monkeypatch) -> None:  # 新增代码+ResourcePreparationTest：函数段开始，验证旧资源窗口里只允许明确新建空白资源动作，且新建后必须先观察；如果没有这段测试，修复可能放开直接输入旧文档的风险。
    ...
    def _fake_internal_observe_desktop(arguments, *_args, **_kwargs):  # 修改代码+ResourcePreparationAutoObserveTest：函数段开始，模拟自动观察仍看到旧文档；如果没有这段函数，单测可能读取真实桌面。
        observed_payload = {"action": "get_window_state", "state": {"window": controller.window}}  # 修改代码+ResourcePreparationAutoObserveTest：返回旧资源窗口状态；如果没有这一行，不能验证自动观察失败时仍阻断输入。
        return "mcp__computer-use__observe 成功：Windows 只读窗口状态已返回。\n数据：" + repr(observed_payload) + "\nComputer Use Image Results\n- image_result_count=0"  # 修改代码+ResourcePreparationAutoObserveTest：返回生产格式 observe 文本；如果没有这一行，adapter 解析不到窗口事实。
    # 修改代码+ResourcePreparationAutoObserveTest：函数段结束，_fake_internal_observe_desktop 到此结束；如果没有这个边界说明，用户不容易看出 fake observe 范围。
    ...
    monkeypatch.setattr(adapter_module.computer_use_internal_adapter_tools, "internal_observe_desktop", _fake_internal_observe_desktop)  # 修改代码+ResourcePreparationAutoObserveTest：替换底层 observe；如果没有这一行，测试会触碰真实屏幕。
...
def test_ctrl_n_is_allowed_as_preparation_even_when_reason_lacks_blank_words(monkeypatch) -> None:  # 新增代码+ResourcePreparationReasonFallbackTest：函数段开始，复现真实验收里模型只说按快捷键而没有写“新建空白文档”的情况；如果没有这段测试，Ctrl+N 会继续被旧资源门禁误挡。
    controller = _RestoredResourceController()  # 新增代码+ResourcePreparationReasonFallbackTest：创建一个 agent 自己启动但恢复旧文档的窗口；如果没有这一行，测试没有真实失败里的旧资源事实。
    executed_actions: list[dict] = []  # 新增代码+ResourcePreparationReasonFallbackTest：记录到底有没有真实动作穿过 adapter；如果没有这一行，测试无法证明只有安全 Ctrl+N 被放行。

    def _fake_internal_execute_desktop_action(arguments, *_args, **_kwargs):  # 新增代码+ResourcePreparationReasonFallbackTest：函数段开始，替换真实桌面动作，避免单测真的操作用户电脑；如果没有这段函数，测试可能会真实按键。
        executed_actions.append(dict(arguments))  # 新增代码+ResourcePreparationReasonFallbackTest：保存底层动作参数；如果没有这一行，断言看不到 adapter 最终传了什么。
        return '{"ok": true, "message": "executed", "data": {}}'  # 新增代码+ResourcePreparationReasonFallbackTest：返回成功 JSON，模拟 Ctrl+N 已经执行；如果没有这一行，adapter 会把 fake 动作当失败。
    # 新增代码+ResourcePreparationReasonFallbackTest：函数段结束，_fake_internal_execute_desktop_action 到此结束；如果没有这个边界说明，用户不容易看出 fake 执行范围。

    monkeypatch.setattr(adapter_module.computer_use_internal_adapter_tools, "internal_execute_desktop_action", _fake_internal_execute_desktop_action)  # 新增代码+ResourcePreparationReasonFallbackTest：只替换底层执行函数；如果没有这一行，测试会触碰真实桌面。
    blocked_resource_freshness = {"allowed": False, "decision": "restored_existing_resource_requires_new_blank_or_authorization", "document_like": True, "title_preview": controller.window["title_preview"], "app_id": "notepad.exe", "process_name": "notepad.exe", "low_level_event_count": 0}  # 新增代码+ResourcePreparationReasonFallbackTest：模拟 observe/action 已确认当前是恢复旧资源；如果没有这一行，Ctrl+N 不会处在真实失败门禁下。
    state = ComputerUseMcpSessionState(last_observed_window=dict(controller.window), last_resource_freshness=blocked_resource_freshness)  # 新增代码+ResourcePreparationReasonFallbackTest：把旧资源状态写进会话；如果没有这一行，adapter 不会执行 ResourcePreparation 判断。
    adapter = ComputerUseMcpSessionAdapter(controller=controller, state=state)  # 新增代码+ResourcePreparationReasonFallbackTest：创建真实 adapter；如果没有这一行，测试不会覆盖生产调用路径。
    result = adapter.call_atomic_tool("key", {"key": "CTRL+N", "reason": "按快捷键创建一个新的编辑页"})  # 新增代码+ResourcePreparationReasonFallbackTest：模拟真实模型常见的简短 reason；如果没有这一行，无法复现这次验收里的误挡风险。

    assert result["ok"] is True  # 新增代码+ResourcePreparationReasonFallbackTest：确认 Ctrl+N 在旧资源恢复状态下被允许；如果没有这一行，回归测试不能证明误挡已消失。
    assert executed_actions[0]["key"] == "CTRL+N"  # 新增代码+ResourcePreparationReasonFallbackTest：确认放行的是唯一安全的新建快捷键；如果没有这一行，其他危险按键可能被误放行。
    assert adapter.state.resource_preparation_pending["key_combo"] == "CTRL+N"  # 新增代码+ResourcePreparationReasonFallbackTest：确认执行后进入必须 observe 的准备状态；如果没有这一行，模型可能跳过观察直接输入。
# 新增代码+ResourcePreparationReasonFallbackTest：函数段结束，test_ctrl_n_is_allowed_as_preparation_even_when_reason_lacks_blank_words 到此结束；如果没有这个边界说明，用户不容易看出 reason 兜底测试范围。
...
def test_pending_preparation_auto_observes_blank_before_next_action(monkeypatch) -> None:  # 新增代码+ResourcePreparationAutoObserveTest：函数段开始，验证模型忘记显式 observe 时底层会先自动只读观察再决定是否放行动作；如果没有这段测试，真实 agent 会反复卡在 observe-required。
    controller = _RestoredResourceController()  # 新增代码+ResourcePreparationAutoObserveTest：创建恢复旧文档的目标窗口；如果没有这一行，测试没有旧资源恢复起点。
    blank_window = dict(controller.window)  # 新增代码+ResourcePreparationAutoObserveTest：复制同一个窗口身份；如果没有这一行，不能模拟同一 hwnd 经 Ctrl+N 后变成空白资源。
    blank_window["title_preview"] = "Untitled - Notepad"  # 新增代码+ResourcePreparationAutoObserveTest：把自动观察结果设为空白标题；如果没有这一行，ResourcePreparation 不能确认新资源。
    executed_actions: list[dict] = []  # 新增代码+ResourcePreparationAutoObserveTest：记录真实动作穿透顺序；如果没有这一行，测试无法确认只有 Ctrl+N 和输入被执行。
    observe_calls: list[dict] = []  # 新增代码+ResourcePreparationAutoObserveTest：记录自动 observe 是否发生；如果没有这一行，测试无法证明底层不再依赖模型自觉观察。

    def _fake_internal_observe_desktop(arguments, *_args, **_kwargs):  # 新增代码+ResourcePreparationAutoObserveTest：函数段开始，替换真实 observe 为可控空白窗口观察；如果没有这段函数，单测会读取用户真实屏幕。
        observe_calls.append(dict(arguments))  # 新增代码+ResourcePreparationAutoObserveTest：保存 observe 参数；如果没有这一行，断言看不到自动观察是否绑定同一窗口。
        observed_payload = {"action": "get_window_state", "state": {"window": blank_window}}  # 新增代码+ResourcePreparationAutoObserveTest：构造生产格式的窗口状态；如果没有这一行，adapter 解析不到空白资源。
        return "mcp__computer-use__observe 成功：Windows 只读窗口状态已返回。\n数据：" + repr(observed_payload) + "\nComputer Use Image Results\n- image_result_count=0"  # 新增代码+ResourcePreparationAutoObserveTest：返回 legacy observe 文本；如果没有这一行，测试不覆盖真实解析链。
    # 新增代码+ResourcePreparationAutoObserveTest：函数段结束，_fake_internal_observe_desktop 到此结束；如果没有这个边界说明，用户不容易看出 fake observe 范围。

    def _fake_internal_execute_desktop_action(arguments, *_args, **_kwargs):  # 新增代码+ResourcePreparationAutoObserveTest：函数段开始，替换真实键鼠执行；如果没有这段函数，单测可能真的输入文字。
        executed_actions.append(dict(arguments))  # 新增代码+ResourcePreparationAutoObserveTest：记录底层动作；如果没有这一行，无法检查自动 observe 后是否继续原动作。
        return '{"ok": true, "message": "executed", "data": {}}'  # 新增代码+ResourcePreparationAutoObserveTest：返回 fake 成功；如果没有这一行，adapter 无法包装成功动作。
    # 新增代码+ResourcePreparationAutoObserveTest：函数段结束，_fake_internal_execute_desktop_action 到此结束；如果没有这个边界说明，用户不容易看出 fake action 范围。

    monkeypatch.setattr(adapter_module.computer_use_internal_adapter_tools, "internal_observe_desktop", _fake_internal_observe_desktop)  # 新增代码+ResourcePreparationAutoObserveTest：替换底层 observe；如果没有这一行，测试会依赖真实桌面。
    monkeypatch.setattr(adapter_module.computer_use_internal_adapter_tools, "internal_execute_desktop_action", _fake_internal_execute_desktop_action)  # 新增代码+ResourcePreparationAutoObserveTest：替换底层 action；如果没有这一行，测试会触碰真实窗口。
    blocked_resource_freshness = {"allowed": False, "decision": "restored_existing_resource_requires_new_blank_or_authorization", "document_like": True, "title_preview": controller.window["title_preview"], "app_id": "notepad.exe", "process_name": "notepad.exe", "low_level_event_count": 0}  # 新增代码+ResourcePreparationAutoObserveTest：准备旧资源阻断状态；如果没有这一行，Ctrl+N 不会进入准备状态。
    state = ComputerUseMcpSessionState(last_observed_window=dict(controller.window), last_resource_freshness=blocked_resource_freshness)  # 新增代码+ResourcePreparationAutoObserveTest：模拟 observe 已发现旧资源；如果没有这一行，测试无法复现真实验收。
    adapter = ComputerUseMcpSessionAdapter(controller=controller, state=state)  # 新增代码+ResourcePreparationAutoObserveTest：创建真实 adapter；如果没有这一行，测试不会覆盖生产调用路径。
    new_result = adapter.call_atomic_tool("key", {"key": "CTRL+N", "reason": "创建一个新的编辑页"})  # 新增代码+ResourcePreparationAutoObserveTest：执行受控新建动作；如果没有这一行，pending 状态不会建立。
    type_result = adapter.call_atomic_tool("type", {"text": "hello everyone", "reason": "输入用户要求的新内容"})  # 新增代码+ResourcePreparationAutoObserveTest：模型忘记显式 observe 直接输入；如果没有这一行，真实卡死点不会被测到。

    assert new_result["ok"] is True  # 新增代码+ResourcePreparationAutoObserveTest：确认 Ctrl+N 本身被允许；如果没有这一行，测试前提不成立。
    assert type_result["ok"] is True  # 新增代码+ResourcePreparationAutoObserveTest：确认自动 observe 看到空白后继续输入；如果没有这一行，真实 agent 仍会卡在 observe-required。
    assert len(observe_calls) == 1  # 新增代码+ResourcePreparationAutoObserveTest：确认底层补了一次只读观察；如果没有这一行，可能只是错误放行输入。
    assert [item["action"] for item in executed_actions] == ["press_key", "type_text"]  # 新增代码+ResourcePreparationAutoObserveTest：确认动作顺序是先新建再输入；如果没有这一行，可能夹带错误动作。
    assert adapter.state.resource_preparation_pending == {}  # 新增代码+ResourcePreparationAutoObserveTest：确认自动观察成功后清空 pending；如果没有这一行，后续动作仍会被卡住。
# 新增代码+ResourcePreparationAutoObserveTest：函数段结束，test_pending_preparation_auto_observes_blank_before_next_action 到此结束；如果没有这个边界说明，用户不容易看出自动 observe 回归范围。
