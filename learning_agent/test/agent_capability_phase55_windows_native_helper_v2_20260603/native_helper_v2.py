...
class WindowsNativeHelperV2Worker:  # 新增代码+Phase55WindowsNativeHelperV2: 类段开始，定义运行在子进程里的 worker；如果没有这个类，helper v2 没有真正执行协议的一端。
    def __init__(self, real_actions_enabled: bool = False) -> None:  # 新增代码+Phase55WindowsNativeHelperV2: 函数段开始，初始化 worker 状态；如果没有这段函数，worker 不知道自己 pid、启动时间和安全开关。
    ...
    def handle(self, message: dict[str, Any]) -> dict[str, Any]:  # 新增代码+Phase55WindowsNativeHelperV2: 函数段开始，处理单条 JSON 请求；如果没有这段函数，worker 无法响应 client。
        ...
        if op == "capture_window":  # 新增代码+Phase55WindowsNativeHelperV2: 处理 capture_window 请求；如果没有这行代码，截图协议缺项。
            return _ok_response(op, {"captured": False, "artifact_path": "", "screenshot_bytes_included": False, "reason": "Phase55 helper v2 协议已就绪，真实截图由 Phase56 接入。"}, request_id=request_id)  # 新增代码+Phase55WindowsNativeHelperV2: 返回不含原始 bytes 的截图摘要；如果没有这行代码，JSON 响应可能泄露大二进制。
        if op == "read_uia_tree":  # 新增代码+Phase55WindowsNativeHelperV2: 处理 read_uia_tree 请求；如果没有这行代码，UIA 树协议缺项。
            return _ok_response(op, {"tree": {"role": "root", "children": []}, "raw_text_included": False, "reason": "Phase55 helper v2 协议已就绪，真实 UIA 树由 Phase57 接入。"}, request_id=request_id)  # 新增代码+Phase55WindowsNativeHelperV2: 返回脱敏 UIA 占位树；如果没有这行代码，UIA 原文可能越过脱敏边界。
...
class WindowsNativeHelperV2Client:  # 新增代码+Phase55WindowsNativeHelperV2: 类段开始，定义主进程侧 helper client；如果没有这个类，agent 无法安全启动和调用子进程 helper。
    ...
    def _command(self) -> list[str]:  # 新增代码+Phase55WindowsNativeHelperV2: 函数段开始，生成 worker 启动命令；如果没有这段函数，启动路径会散落在 start 里。
        return [sys.executable, "-m", "learning_agent.computer_use.native_helper_v2", "--worker"]  # 新增代码+Phase55WindowsNativeHelperV2: 使用当前解释器启动包模块 worker；如果没有这行代码，子进程可能用错 Python 环境。
