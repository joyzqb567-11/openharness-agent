...
$EvidenceDir = Join-Path $Workspace "learning_agent\test\provider_settings_v2_openai_connect\task08_visual_qa" # 新增代码+OpenAIConnectVisualQA：定位 OpenAI connect 截图证据目录；如果没有这行，Task 8 图片没有固定位置。
$DriverPath = Join-Path $ScriptDir "openai_connect_visual_qa_driver.mjs" # 新增代码+OpenAIConnectVisualQA：定位 OpenAI connect Node CDP driver；如果没有这行，PowerShell 无法调用自动截图逻辑。
...
try { # 新增代码+ProviderSettingsVisualQA：启动和验收主流程开始；如果没有这段，finally 无法可靠清理。
  ...
  node $DriverPath "--cdpPort=$CdpPort" "--evidenceDir=$EvidenceDir" "--bridgeUrl=$BridgeUrl" "--token=$Token" # 修改代码+OpenAIConnectVisualQA：运行 Node CDP driver 并传入 bridge URL/token；如果没有这行，driver 无法模拟 mock 授权完成。
