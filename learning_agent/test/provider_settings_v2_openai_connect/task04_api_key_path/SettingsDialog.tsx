...
import type { CustomProviderRequest, GuiProviderSettingsPayload, ProviderConnectionProbePayload } from "../../api/guiProviderTypes"; // 修改代码+CustomProviderDialog：导入 provider settings bridge 合同和自定义 provider 请求；如果没有这行，弹窗容器无法类型化保存流程。
      ...
      <section className="settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-dialog-title"> {/* 修改代码+ProviderSettingsPanel：渲染设置对话框语义容器；如果没有这行，读屏器无法识别模态弹窗。 */}
        ...
        <section className="settings-dialog-main"> {/* 修改代码+ProviderSettingsPanel：渲染右侧内容区；如果没有这层，页签内容没有稳定布局。 */}
          ...
          <div className="settings-dialog-content" role="tabpanel"> {/* 修改代码+ProviderSettingsPanel：渲染当前页签内容容器；如果没有这层，内容区无法稳定滚动。 */}
            {panelContent(activeTab, providerViewModel, providerLoading, providerError, busyProviderId, probeResults, modelPendingKeyValue, modelError, () => { void refreshProviderSettings(); }, (provider) => { setConnectProvider(provider); setConnectError(""); }, (provider) => { void handleDisconnectProvider(provider); }, (provider) => { void handleTestProvider(provider); }, () => { setCustomProviderOpen(true); setCustomProviderError(""); }, (providerId, modelId, visible) => { void handleToggleModelVisibility(providerId, modelId, visible); })} {/* 修改代码+SettingsModelsPanel：渲染当前页签内容并注入 provider、自定义 provider 和模型可见性操作；如果没有这行，模型开关无法调用 bridge。 */}
          </div> {/* 修改代码+ProviderSettingsPanel：内容容器结束；如果没有这行，JSX 结构不完整。 */}
        </section> {/* 修改代码+ProviderSettingsPanel：右侧内容区结束；如果没有这行，JSX 结构不完整。 */}
        <ProviderConnectDialog open={connectProvider !== null} provider={connectProvider} apiKey={connectApiKey} pending={connectPending} errorMessage={connectError} onApiKeyChange={setConnectApiKey} onClose={() => { setConnectProvider(null); setConnectApiKey(""); setConnectError(""); }} onSubmit={(authMethodId) => { void handleConnectSubmit(authMethodId); }} /> {/* 修改代码+OpenAIConnectApiKey：渲染受控连接弹窗并接收选中的认证方式；如果没有这行，连接按钮无法按新方法 id 提交 API key。 */}
