...
from learning_agent.app.gui_provider_settings import GuiProviderSettingsError, clear_provider_disabled_override, load_provider_settings, save_provider_settings  # 修改代码+ProviderReconnect：复用 provider 设置读写、disabled 清理和结构化错误；如果没有这行，Codex CLI 断开后重新登录不会恢复 OpenAI provider。
