...
export function SettingsDialog({ open, onClose, version = packageInfo.version, secretStoreWarning = "", guiClient }: SettingsDialogProps): JSX.Element | null { // 修改代码+ProviderSettingsPanel：函数段开始，渲染 OpenHarness Desktop 设置弹窗；如果没有这段，左下角设置入口没有成熟 GUI 外壳。
  ...
  useEffect(() => { // 新增代码+OpenAIConnectDialog：副作用段开始，轮询当前 auth-attempt 状态；如果没有这段，等待授权页不会自动更新。
    if (!open || guiClient === undefined || connectAuthAttempt === null || connectAuthAttempt.status !== "pending") { // 新增代码+OpenAIConnectDialog：只在打开、有 client、有 pending attempt 时轮询；如果没有这行，关闭弹窗也会继续请求。
    ...
    const activeClient = guiClient; // 新增代码+OpenAIConnectDialog：固定本轮轮询使用的 client；如果没有这行，异步回调里 client 类型可能变成 undefined。
    const activeAttemptId = connectAuthAttempt.attempt_id; // 新增代码+OpenAIConnectDialog：固定本轮轮询的 attempt id；如果没有这行，异步回调可能读到过期状态。
    ...
    async function pollAuthAttempt(): Promise<void> { // 新增代码+OpenAIConnectDialog：函数段开始，执行一次 auth-attempt 状态刷新；如果没有这段，轮询逻辑会散落在定时器里。
      try { // 新增代码+OpenAIConnectDialog：捕获 status 请求失败；如果没有这行，网络错误会变成未捕获 Promise。
        const payload = await activeClient.providerAuthAttemptStatus(activeAttemptId); // 新增代码+OpenAIConnectDialog：读取后端 auth-attempt 状态；如果没有这行，等待页没有事实源。
        ...
        setConnectAuthAttempt(payload.attempt); // 新增代码+OpenAIConnectDialog：保存最新 attempt 状态；如果没有这行，状态文案不会变化。
        if (payload.attempt.status === "complete") { // 新增代码+OpenAIConnectDialog：处理授权完成状态；如果没有这行，完成后 provider 列表不会刷新。
          const nextPayload = await activeClient.providerSettings(); // 新增代码+OpenAIConnectDialog：完成后重新读取 provider catalog；如果没有这行，已连接状态不会显示。
        ...
        } else if (payload.attempt.status === "failed" || payload.attempt.status === "expired") { // 新增代码+OpenAIConnectDialog：处理失败或过期状态；如果没有这行，终态可能继续显示等待。
          setConnectError(safeProviderError("授权未完成")); // 新增代码+OpenAIConnectDialog：显示安全失败文案；如果没有这行，用户不知道 flow 已结束。
        } // 新增代码+OpenAIConnectDialog：终态处理结束；如果没有这行，条件块语法不完整。
