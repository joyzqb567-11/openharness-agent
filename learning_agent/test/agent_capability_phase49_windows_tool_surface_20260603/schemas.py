...
TOOL_SCHEMAS: list[dict[str, Any]] = [  # 作用: 告诉真实模型当前 agent 暴露了哪些工具
    ...
    {  # 新增代码+Phase49ComputerUseToolSurface: 新增 computer_use 兼容工具定义开始；如果没有这段代码，模型无法使用统一 Computer Use 工具入口。
        ...
        "function": {  # 新增代码+Phase49ComputerUseToolSurface: function 内部描述兼容工具名称、说明和参数；如果没有这行代码，工具定义结构不完整。
            ...
            "description": "兼容式 OS Computer Use 统一入口；operation=status/observe/action，内部会转发到 computer_status、computer_observe 或 computer_action，同样经过 controller、安全策略和 evidence。",  # 新增代码+Phase49ComputerUseToolSurface: 说明该工具只是兼容适配层；如果没有这行代码，模型可能以为它绕过旧安全链路。
            "parameters": {  # 新增代码+Phase49ComputerUseToolSurface: JSON Schema 描述统一入口参数；如果没有这行代码，模型不知道 operation 和动作字段怎么传。
                ...
                "properties": {  # 新增代码+Phase49ComputerUseToolSurface: 声明兼容工具允许的参数字段；如果没有这行代码，operation/action/window 等字段无法进入 schema。
                    "operation": {"type": "string", "enum": ["status", "observe", "action"], "description": "要执行的统一操作：status 查询状态，observe 只读观察，action 执行桌面动作。"},  # 新增代码+Phase49ComputerUseToolSurface: 限定统一入口操作枚举；如果没有这行代码，兼容工具可能变成任意命令通道。
                    "op": {"type": "string", "enum": ["status", "observe", "action"], "description": "operation 的 ClaudeCode/MCP 风格别名。"},  # 新增代码+Phase49ComputerUseToolSurface: 兼容 op 字段；如果没有这行代码，外部习惯用 op 的调用无法转发。
    ...
    {  # 新增代码+Phase49ComputerUseToolSurface: 新增 computer-use 兼容工具定义开始；如果没有这段代码，ClaudeCode 风格连字符工具名无法被兼容。
        ...
        "function": {  # 新增代码+Phase49ComputerUseToolSurface: function 内部描述连字符兼容工具；如果没有这行代码，工具定义结构不完整。
            ...
            "description": "ClaudeCode 风格 OS Computer Use 兼容入口；参数与 computer_use 相同，内部仍转发到旧三工具和同一 controller。",  # 新增代码+Phase49ComputerUseToolSurface: 说明连字符工具是别名；如果没有这行代码，模型可能误以为它是独立实现。
            "parameters": {  # 新增代码+Phase49ComputerUseToolSurface: JSON Schema 描述连字符工具参数；如果没有这行代码，模型不知道如何调用该别名。
                ...
                "properties": {  # 新增代码+Phase49ComputerUseToolSurface: 声明允许参数字段；如果没有这行代码，operation 和动作字段无法进入 schema。
                    "operation": {"type": "string", "enum": ["status", "observe", "action"], "description": "要执行的统一操作：status、observe 或 action。"},  # 新增代码+Phase49ComputerUseToolSurface: 限定操作枚举；如果没有这行代码，连字符工具可能变成任意通道。
                    "op": {"type": "string", "enum": ["status", "observe", "action"], "description": "operation 的短别名。"},  # 新增代码+Phase49ComputerUseToolSurface: 兼容 op 字段；如果没有这行代码，MCP 风格调用不能用短字段。
