...
try:  # 修改代码+CoreSplit: 优先从新 core 包导入配置与消息结构；若没有这行代码，learning_agent.py 会继续承载过多基础定义。
    ...
    if error.name not in {"learning_agent", "learning_agent.core", "learning_agent.core.config", "learning_agent.core.events", "learning_agent.core.messages"}:  # 修改代码+Stage15C: 允许脚本模式 fallback 覆盖 core.events；若没有这行代码，直接运行时事件模块会被误判成真实错误。
...
try:  # 修改代码+ToolsSplit: 优先从新 tools 包导入工具类型和目录构建函数；若没有这行代码，learning_agent.py 会继续承载工具元数据实现。
    ...
    if error.name not in {"learning_agent", "learning_agent.tools", "learning_agent.tools.atom_tools", "learning_agent.tools.catalog", "learning_agent.tools.executor", "learning_agent.tools.hooks", "learning_agent.tools.pool", "learning_agent.tools.result_storage", "learning_agent.tools.types"}:  # 修改代码+Stage15E: 只允许目标包路径缺失时 fallback；若没有这行代码，tools 内部真实 bug 会被误吞。
...
class LearningAgent:  # 作用: 教学版最小 agent 主类
    ...
    def run(self, user_input: str, max_turns: int | None = None) -> str:  # 修改代码+可配置轮次: 默认不再写死 8 轮，允许配置层传入数字或 None；若省略: learning_agent 会继续和 Claude Code 的可配置策略不一致
        ...
        while max_turns is None or turn < max_turns:  # 修改代码+可配置轮次: 只有配置了数字上限时才按轮次停止；若省略: 默认仍会被硬编码轮次限制
            ...
            for tool_call in model_message.tool_calls:  # 作用: 遍历模型请求的每个工具调用
                ...
                tool_output = self._execute_tool(tool_call)  # 作用: 执行工具并得到结果文本
    ...
    def run_events(self, user_input: str, max_turns: int | None = None):  # 新增代码+Stage15C: 新增事件流主循环；若没有这行代码，UI、HTTP bridge 和 transcript 无法观察运行过程。
        ...
        try:  # 新增代码+Stage15C: 捕获主循环异常并转成 run_failed 事件；若没有这行代码，事件消费者会看到生成器直接崩溃。
            ...
            while max_turns is None or turn < max_turns:  # 新增代码+Stage15C: 保留可配置轮次循环；若没有这行代码，agent 无法继续多轮工具调用。
                ...
                for tool_call in model_message.tool_calls:  # 新增代码+Stage15C: 遍历模型请求的工具调用；若没有这行代码，模型无法驱动工具。
                    ...
                    tool_output = self._execute_tool(tool_call)  # 新增代码+Stage15C: 执行工具并得到原始结果；若没有这行代码，tool loop 失效。
