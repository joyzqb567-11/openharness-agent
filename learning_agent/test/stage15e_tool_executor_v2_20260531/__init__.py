...
try:  # 修改代码+ToolsSplit: 包运行模式下从正式包路径重导出工具 API；若没有这行代码，测试和外部包导入无法走稳定路径。
    ...
    if error.name not in {"learning_agent", "learning_agent.tools", "learning_agent.tools.catalog", "learning_agent.tools.hooks", "learning_agent.tools.permissions", "learning_agent.tools.types"}:  # 修改代码+Stage15E: 只允许目标包路径缺失时 fallback；若没有这行代码，tools 内部真实 bug 会被误吞。
...
__all__ = ["AgentTool", "ToolHookEvent", "ToolHookManager", "ToolPermissionDecision", "agent_tool_from_schema", "build_builtin_tool_catalog", "builtin_tool_capability_pack", "decide_tool_permission"]  # 修改代码+Stage15E: 明确 tools 包公开 API 包含 hook 和权限协议；若没有这行代码，后续重构时容易无意暴露或遗漏核心 helper。
