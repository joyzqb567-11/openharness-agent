...
class LearningAgent:  # 作用: 教学版 agent 主类
    ...
    def _computer_use_image_blocks_from_tool_output(self, output: str) -> list[dict[str, Any]]:  # 新增代码+ComputerUseVisionLoop: 定义图片块提取函数；如果没有这行代码，调用方无法把工具输出转换为多模态块。
        ...
        for spec in image_specs:  # 新增代码+ComputerUseVisionLoop: 遍历每个图片引用；如果没有这行代码，多张截图无法处理。
            ...
            try:  # 新增代码+ComputerUseVisionLoop: 捕获图片读取异常；如果没有这行代码，权限或并发写入问题会让 agent 主循环崩溃。
                image_bytes = artifact_path.read_bytes()  # 新增代码+ComputerUseVisionLoop: 读取截图原始字节；如果没有这行代码，无法生成 base64 data URL。
            except OSError as error:  # 新增代码+ComputerUseVisionLoop: 处理文件读取失败；如果没有这行代码，底层 OSError 会直接冒泡。
                self._record_observation("computer_use_image_read_failed", {"artifact_path": str(artifact_path), "error": str(error)})  # 新增代码+ComputerUseVisionLoop: 记录读取失败证据；如果没有这行代码，排查时不知道哪张图没读到。
                continue  # 新增代码+ComputerUseVisionLoop: 跳过失败图片继续其它图片；如果没有这行代码，局部失败会影响整轮模型。
            if not image_bytes or len(image_bytes) > max_image_bytes:  # 新增代码+ComputerUseVisionLoop: 拒绝空图或过大图；如果没有这行代码，模型可能收到无效或过大的图片块。
                self._record_observation("computer_use_image_skipped_for_model", {"artifact_path": str(artifact_path), "byte_count": len(image_bytes), "max_image_bytes": max_image_bytes})  # 新增代码+ComputerUseVisionLoop: 记录跳过原因；如果没有这行代码，用户不知道为什么某张截图没进模型。
                continue  # 新增代码+ComputerUseVisionLoop: 跳过当前图片；如果没有这行代码，后续仍会把异常图片编码进上下文。
            mime_type = self._computer_use_model_image_mime_type(spec.get("mime_type"), artifact_path)  # 新增代码+ComputerUseVisionLoop: 规范化 MIME 类型；如果没有这行代码，data URL 可能缺少可解码类型。
