...
try:  # 新增代码+AppSplit: 优先按包路径导入轮次格式化和验收事件 helper；若没有这行代码，包运行模式下交互层无法复用通用能力。
    ...
    if error.name not in {"learning_agent", "learning_agent.app", "learning_agent.app.chrome_status_renderer", "learning_agent.app.computer_status_renderer", "learning_agent.app.status_renderer", "learning_agent.browser_extension_host", "learning_agent.browser_extension_host.bridge_server", "learning_agent.browser_extension_host.manifest_installer", "learning_agent.computer_use", "learning_agent.computer_use.approval", "learning_agent.computer_use.abort_streaming_hooks", "learning_agent.computer_use.controller", "learning_agent.computer_use.lock", "learning_agent.computer_use.native_diagnostics", "learning_agent.computer_use.persistent_grants", "learning_agent.computer_use.security_policy", "learning_agent.computer_use.session_context", "learning_agent.computer_use.session_runtime", "learning_agent.computer_use.terminal_grants", "learning_agent.core", "learning_agent.core.config", "learning_agent.observability", "learning_agent.observability.acceptance_events", "learning_agent.observability.run_records", "learning_agent.runtime", "learning_agent.runtime.command_queue", "learning_agent.runtime.status_snapshot", "learning_agent.sdk", "learning_agent.sdk.status"}:  # 修改代码+Phase61AbortStreamingHooks: 允许 abort_streaming_hooks 在脚本模式 fallback；若没有这行代码，bat 入口下 /computer abort-hooks 读不到中断钩子。
...
def run_computer_terminal_command(workspace: Path, user_input: str) -> str:  # 新增代码+Phase31ComputerUseLockAbortEvidence: 函数段开始，执行 `/computer` 子命令；如果没有这段函数，真实用户无法在终端查看或触发桌面急停。
    ...
    if subcommand in {"status", ""}:  # 新增代码+Phase31ComputerUseLockAbortEvidence: 识别状态查询；如果没有这行代码，用户无法只读查看 Computer Use 状态。
        ...
        status_snapshot = {"lock": lock_manager.status(), "approval": approval_model.status(), "runtime": runtime.status(), "recovery": runtime.action_journal(limit=3), "terminal_grants": terminal_grants.status(), "persistent_grants": persistent_grants.status(DEFAULT_SESSION_CONTEXT_ID), "abort_streaming_hooks": abort_streaming_hooks.status(), "session_context": session_context_store.status(DEFAULT_SESSION_CONTEXT_ID), "capability_matrix": capability_report["matrix"]}  # 修改代码+Phase61AbortStreamingHooks: 组装 renderer 快照并加入中断钩子；如果没有这行代码，/computer status 看不到 hotkey 降级、stream 事件和 cleanup hook。
    ...
    return "Computer Action\n- 不支持的 /computer 子命令。\n- 可用命令：/computer、/computer status、/computer parity、/computer native-gate、/computer abort-hooks、/computer observe [action]、/computer grant <app> [flags]、/computer approve <app> [flags] ttl=60 scope=click、/computer grants、/computer deny <app>、/computer revoke <app>、/computer abort <reason>、/computer clear-abort、/computer cleanup [session_id]、/computer recover、/computer journal、/computer notifications、/computer release [session_id]\n"  # 修改代码+Phase61AbortStreamingHooks: 返回包含 abort-hooks 的用法；如果没有这行代码，输错 Phase61 命令时用户不知道中断钩子入口。
