...
class LearningAgent:  # 作用: 教学版 agent 主类
    ...
    def _desktop_task_runtime_for_current_run(self) -> Any:  # 新增代码+DesktopTaskRuntimeRoute：函数段开始，取得当前 run 应使用的桌面任务 runtime；如果没有这段函数，测试无法注入 full-mode runtime。
        ...
        runtime = ComputerUseDesktopTaskRuntime(base_dir=runtime_root, real_execution_loop=UniversalDesktopExecutionLoopAdapter())  # 修改代码+UniversalDesktopAdapter：默认生产路径注入通用桌面执行 adapter 而不是 Paint/Pikachu 特例；如果没有这一行，用户的通用 Computer Use prompt 会继续停在未接线拒绝状态。
