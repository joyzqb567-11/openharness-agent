...
class WindowsComputerUseFullDesktopTaskRouterTests(unittest.TestCase):  # 新增代码+DesktopTaskAcceptance：类段开始，集中冻结 `/computer use --full` 桌面任务路由验收；如果没有这个类，unittest 不会组织这些回归用例。
    ...
    def test_runtime_real_actions_uses_injected_real_execution_loop(self) -> None:  # 新增代码+源码复核门禁：函数段开始，验证 real_actions=True 进入可注入真实执行闭环；如果没有这个测试，运行时会继续直接拒绝真实动作。
        class FakeRealExecutionLoop:  # 新增代码+源码复核门禁：类段开始，模拟已经安全验证过的真实执行闭环；如果没有这个类，单测只能触碰真实桌面。
            def run_desktop_task(self, target_app: str, prompt: str) -> dict[str, Any]:  # 新增代码+源码复核门禁：函数段开始，返回真实执行形状的报告；如果没有这段函数，desktop runtime 没有可注入接口。
                return {"ok": True, "decision": "real_desktop_task_executed_by_injected_loop", "target_app": target_app, "prompt_length": len(prompt), "computer_use_gui_route_used": True, "owned_window_verified": True, "gui_action_count": 1, "low_level_event_count": 3, "real_dispatch_performed": True, "real_desktop_touched": True, "recording_mode": False, "post_action_screenshot_exists": True, "forbidden_script_generation_used": False, "bash_final_artifact_route_used": False, "forbidden_script_artifact_route_blocked": True}  # 新增代码+源码复核门禁：返回脱敏真实执行摘要；如果没有这一行，运行时无法汇总真实动作事实。
    ...
    def test_agent_routes_paint_prompt_to_desktop_runtime_before_bash(self) -> None:  # 新增代码+DesktopTaskRuntimeRoute：函数段开始，验证 agent.run 在模型选择 bash 前拦截桌面任务；如果没有这个测试，Task 5 可能仍让模型生成 System.Drawing 绕路。
        ...
        class BashBypassFakeModel:  # 新增代码+DesktopTaskRuntimeRoute：类段开始，定义会输出脚本绕路痕迹的离线假模型；如果没有这个类，测试无法证明模型路径没有被使用。
            def chat(self, messages: list[dict[str, object]], tools: list[dict[str, object]]) -> ModelMessage:  # 新增代码+DesktopTaskRuntimeRoute：模拟模型 chat 接口；如果没有这一行，LearningAgent 无法调用假模型。
                ...
                return ModelMessage(text="System.Drawing would have generated a fake Paint artifact.")  # 新增代码+DesktopTaskRuntimeRoute：返回危险脚本路线文本；如果没有这一行，未拦截时测试不会稳定失败。
        # 新增代码+DesktopTaskRuntimeRoute：类段结束，BashBypassFakeModel 到此结束；如果没有这个边界说明，代码小白不容易看出假模型范围。
        ...
        with tempfile.TemporaryDirectory() as raw_dir:  # 新增代码+DesktopTaskRuntimeRoute：创建临时 workspace；如果没有这一行，测试会污染真实运行目录。
            ...
            agent = LearningAgent(model=BashBypassFakeModel(), workspace=workspace, ask_permission=lambda action: True, debug_enabled=False)  # 新增代码+DesktopTaskRuntimeRoute：创建真实 agent 但使用离线假模型；如果没有这一行，测试没有 agent.run 主体。
            ...
            self.assertIn("COMPUTER_USE_FULL_DESKTOP_TASK_ROUTER_OK", answer)  # 新增代码+DesktopTaskRuntimeRoute：断言返回运行时成功 token；如果没有这一行，agent 可能仍走普通模型答案。
            self.assertIn("computer_use_gui_route_used=true", answer)  # 新增代码+DesktopTaskRuntimeRoute：断言最终答案明示 GUI 路由；如果没有这一行，脚本和 GUI 路由无法区分。
            self.assertNotIn("System.Drawing", answer)  # 新增代码+DesktopTaskRuntimeRoute：断言危险模型文本没有成为最终答案；如果没有这一行，之前失败根因可能再次溜过。
    # 新增代码+DesktopTaskRuntimeRoute：函数段结束，test_agent_routes_paint_prompt_to_desktop_runtime_before_bash 到此结束；如果没有这个边界说明，代码小白不容易看出 agent.run 拦截测试范围。
    ...
    def test_default_agent_full_mode_uses_universal_adapter_without_claiming_real_dispatch(self) -> None:  # 新增代码+UniversalDesktopAdapter：函数段开始，验证真实 agent 默认 full 路径会进入通用 adapter 但不冒充物理桌面成功；如果没有这个测试，后续可能又把录制事件当成熟真实执行。
        ...
        prompt = "\u8bf7\u4f7f\u7528\u672c\u5730\u7535\u8111\u7684\u753b\u56fe\u8f6f\u4ef6\u753b\u4e00\u53ea\u732b\u3002"  # 新增代码+UniversalDesktopAdapter：使用真实用户会输入的“画猫”中文 prompt；如果没有这一行，测试无法覆盖非皮卡丘任务。
        with tempfile.TemporaryDirectory() as raw_dir:  # 新增代码+UniversalDesktopAdapter：创建一次性 workspace；如果没有这一行，full mode 状态会污染真实项目。
            ...
            answer = agent.run(prompt, max_turns=1)  # 新增代码+UniversalDesktopAdapter：通过 agent.run 执行自然语言桌面任务；如果没有这一行，测试只能检查静态对象而无法验证用户路径。
            ...
            self.assertIn("real_dispatch_performed\": false", answer)  # 新增代码+UniversalDesktopAdapter：断言当前没有物理派发；如果没有这一行，录制 sender 可能被误报成真实桌面动作。
            self.assertIn("real_desktop_touched=false", answer)  # 新增代码+UniversalDesktopAdapter：断言可见 token 行也显示未触碰真实桌面；如果没有这一行，终端验收可能误判副作用。
            self.assertIn("universal_desktop_execution_loop_connected_without_real_dispatch", answer)  # 新增代码+UniversalDesktopAdapter：断言失败原因码稳定且诚实；如果没有这一行，用户无法知道下一步缺的是物理派发。
            self.assertNotIn("COMPUTER_USE_FULL_DESKTOP_TASK_ROUTER_OK", answer)  # 新增代码+UniversalDesktopAdapter：断言未真实派发时不输出成功 token；如果没有这一行，未成熟状态可能被自动验收误判通过。
    ...
    # 新增代码+UniversalDesktopAdapter：函数段结束，test_default_agent_full_mode_uses_universal_adapter_without_claiming_real_dispatch 到此结束；如果没有这个边界说明，代码小白不容易看出默认用户路径门禁范围。
