...
class DefaultHarnessGuiAgentAdapter:  # 新增代码+GuiAgentAdapter：类段开始，真实 harness adapter 的 feature-flagged shell；如果没有这个类，调用方无法显式请求真实模式并得到稳定失败。
    def __init__(self, enabled: bool = False, model_factory: Any | None = None, permission_requester: Any | None = None, max_turns: int = 1, allowed_tool_names: set[str] | None = None) -> None:  # 修改代码+RealHarnessReadOnlyTools：函数段开始，保存真实 adapter 开关、权限回调和工具白名单；如果没有这段，真实入口无法安全委托到 core agent。
        ...
        self.allowed_tool_names = set() if allowed_tool_names is None else set(allowed_tool_names)  # 新增代码+RealHarnessReadOnlyTools：保存本轮允许暴露给 core 的工具名；如果没有这一行，Phase 3 无法只开放读工具。
    ...
    def run(self, request: GuiAgentRunRequest, emit_event: GuiEventEmitter, is_cancelled: GuiCancelChecker) -> GuiAgentRunResult:  # 修改代码+RealHarnessAdapter：函数段开始，禁用时稳定失败，启用时委托真实 core adapter；如果没有这段，显式真实模式没有入口。
        ...
        delegate = RealHarnessGuiAgentAdapter(enabled=True, model_factory=self.model_factory, permission_requester=self.permission_requester, max_turns=self.max_turns, allowed_tool_names=self.allowed_tool_names)  # 修改代码+RealHarnessReadOnlyTools：创建真实委托对象并传递工具白名单；如果没有这一行，只读工具轨迹无法进入 core agent。
