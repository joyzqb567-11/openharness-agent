...
GUI_V2_EVENT_KINDS = {  # 新增代码+GuiV2Protocol：集合段开始，列出 V2 主事件类型；如果没有这段，事件状态机没有协议白名单。
    ...
    "model_call_started",  # 新增代码+真实模型状态条：表示真实模型调用已经开始；如果没有这一项，前端无法把“已提交”和“正在连模型”区分开。
    "model_call_status",  # 新增代码+真实模型状态条：表示真实模型调用中间状态；如果没有这一项，WebSocket 超时、HTTPS fallback 等状态无法进入 GUI。
    "model_first_delta",  # 新增代码+真实模型状态条：表示真实模型首个 delta 已到达；如果没有这一项，慢回复排查没有首字延迟锚点。
    "model_call_completed",  # 新增代码+真实模型状态条：表示真实模型调用完成；如果没有这一项，底部状态条无法显示真实完成态。
    "model_call_failed",  # 新增代码+真实模型状态条：表示真实模型调用失败；如果没有这一项，失败只能靠泛化 request_failed 事件间接判断。
    ...
    "agent_diagnostic",  # 新增代码+RealHarnessAdapter：表示真实 core agent 的非终态诊断事件；如果没有这一项，task_state/compact 等 AgentEvent 会被协议门禁拒绝。
