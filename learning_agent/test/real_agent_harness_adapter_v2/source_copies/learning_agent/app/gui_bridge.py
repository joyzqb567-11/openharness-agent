...
class GuiRunManager:  # 新增代码+DesktopGUILifecycle：类段开始，管理 GUI turn 生命周期；如果没有这个类，handler 会堆满业务状态逻辑。
    ...
    def _adapter_for_turn(self, turn: GuiTurn) -> GuiAgentAdapter:  # 新增代码+GuiAgentAdapter：函数段开始，选择本轮使用 fake adapter 还是真实 harness shell；如果没有这段，真实模式触发语义会散落在 worker 中。
        ...
        read_only_enabled = os.environ.get("OPENHARNESS_GUI_AGENT_READ_ONLY_TOOLS", "").strip().casefold() in {"1", "true", "yes", "on", "enabled", "read_only"} or "__read_only_tool__" in turn.prompt or turn.permission_mode == "read_only"  # 新增代码+RealHarnessReadOnlyTools：识别只读工具开放条件；如果没有这行，Phase 3 无法让真实 agent 产生安全工具轨迹。
        allowed_tool_names = {"read_file", "list_dir"} if read_only_enabled else set()  # 新增代码+RealHarnessReadOnlyTools：只允许安全读文件和列目录；如果没有这行，默认真实模式可能暴露写入或系统工具。
        max_turns = 2 if read_only_enabled else 1  # 新增代码+GuiModelFactoryVisibleToolTrace：只读工具烟测需要“模型请求工具”和“工具后回答”两轮；如果没有这一行，GUI 验收会停在工具结果前无法完成。
        if real_requested:  # 修改代码+RealHarnessAdapter：只在显式请求真实路径时返回 harness shell；如果没有这一行，普通 fake 基线会被破坏。
            return DefaultHarnessGuiAgentAdapter(enabled=real_enabled, permission_requester=self._ask_permission_for_adapter, max_turns=max_turns, allowed_tool_names=allowed_tool_names)  # 修改代码+GuiModelFactoryVisibleToolTrace：把真实 adapter 接到 GUI 权限回调、两轮上限和只读工具白名单；如果没有 max_turns，只读工具可见烟测不会产生最终回答。
    ...
    def do_POST(self) -> None:  # 新增代码+DesktopGUILifecycle：函数段开始，处理 GUI 生命周期 POST；如果没有这段，messages/cancel/retry/resume 全部不可用。
        ...
        try:  # 新增代码+DesktopGUILifecycle：统一捕获业务结构化错误；如果没有这行，每个分支都要重复 try/except。
            if path == "/v1/gui/messages":  # 新增代码+DesktopGUILifecycle：识别发送消息端点；如果没有这行，GUI 无法创建 turn。
                provider_id = body.get("providerId") if isinstance(body.get("providerId"), str) else None  # 新增代码+DirectSSEPayload：读取前端 camelCase providerId；如果没有这行，OpenAI 选择不会进入后端。
                model_id = body.get("modelId") if isinstance(body.get("modelId"), str) else None  # 新增代码+DirectSSEPayload：读取前端 camelCase modelId；如果没有这行，模型下拉选择不会进入后端。
                reasoning_effort = body.get("reasoningEffort") if body.get("reasoningEffort") in GUI_REASONING_EFFORTS else "high"  # 新增代码+DirectSSEPayload：读取并白名单推理强度；如果没有这行，非法推理强度可能污染真实请求。
                permission_mode = body.get("permissionMode") if body.get("permissionMode") in GUI_PERMISSION_MODES else "full_access"  # 新增代码+DirectSSEPayload：读取并白名单权限模式；如果没有这行，非法权限值可能污染后端权限语义。
