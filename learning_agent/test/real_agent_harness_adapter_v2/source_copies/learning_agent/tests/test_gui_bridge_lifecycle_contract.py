...
class GuiBridgeLifecycleContractTests(unittest.TestCase):  # 新增代码+DesktopGUILifecycleTest：测试类段开始，锁定 GUI turn 生命周期；如果没有这个类，unittest 不会执行这些合同。
    ...
    def test_message_endpoint_accepts_snake_case_route_fields(self) -> None:  # 新增代码+GuiToolchainReuseTest：测试方法开始，确认脚本或外部控制器用 snake_case 也能传入模型和权限字段；如果没有这段，GUI 工具链路验收脚本会悄悄回退 full_access。
        with tempfile.TemporaryDirectory() as directory:  # 新增代码+GuiToolchainReuseTest：创建隔离工作区；如果没有这行代码，测试会污染真实项目 memory。
            server = self._start_server(Path(directory))  # 新增代码+GuiToolchainReuseTest：启动带 token 的 GUI bridge；如果没有这行代码，测试无法走真实 HTTP 入口。
            try:  # 新增代码+GuiToolchainReuseTest：保护 server 清理；如果没有这行代码，断言失败时后台端口可能残留。
                response = self._post_json(server, "/v1/gui/messages", {"conversation_id": "default", "prompt": "验证 snake_case 路由字段", "provider_id": "openai", "model_id": "gpt-5.5", "reasoning_effort": "ultra", "permission_mode": "read_only"})  # 新增代码+GuiToolchainReuseTest：发送 snake_case 路由字段；如果没有这一行，无法复现命令行验收脚本的真实请求形状。
                turn = server.run_manager.turns[str(response["turn_id"])]  # 新增代码+GuiToolchainReuseTest：读取刚创建的 turn；如果没有这一行，只看响应无法知道字段是否进入后端状态。
            finally:  # 新增代码+GuiToolchainReuseTest：清理 server；如果没有这行代码，测试失败会留下后台服务。
                server.shutdown()  # 新增代码+GuiToolchainReuseTest：停止 serve_forever；如果没有这行代码，测试进程可能挂住。
                server.server_close()  # 新增代码+GuiToolchainReuseTest：释放 socket；如果没有这行代码，Windows 端口可能短时间占用。
        self.assertEqual(turn.provider_id, "openai")  # 新增代码+GuiToolchainReuseTest：确认 snake_case provider_id 被接收；如果没有这一行，外部脚本选择 provider 会被丢弃。
        self.assertEqual(turn.model_id, "gpt-5.5")  # 新增代码+GuiToolchainReuseTest：确认 snake_case model_id 被接收；如果没有这一行，外部脚本选择模型会被丢弃。
        self.assertEqual(turn.reasoning_effort, "ultra")  # 新增代码+GuiToolchainReuseTest：确认 snake_case reasoning_effort 被接收；如果没有这一行，验收脚本无法验证推理档位传递。
        self.assertEqual(turn.permission_mode, "read_only")  # 新增代码+GuiToolchainReuseTest：确认 snake_case permission_mode 被接收；如果没有这一行，只读验收会误走 full_access。
    # 新增代码+GuiToolchainReuseTest：测试方法结束，test_message_endpoint_accepts_snake_case_route_fields 到此结束；如果没有边界说明，用户不容易看出这是 HTTP 字段兼容合同。
