...
def _phase107_launch_target_tools() -> tuple[Any, Any]:  # 新增代码+Phase107InteractiveLaunchTarget：函数段开始，延迟导入 Phase107 目标解析器；如果没有这段函数，脚本模式下 `/computer launch` 可能找不到新模块。
    try:  # 新增代码+Phase107InteractiveLaunchTarget：优先按 learning_agent 包路径导入；如果没有这行代码，项目根运行时无法加载标准模块。
        from learning_agent.computer_use.interactive_launch_target import phase107_cli_line, resolve_interactive_launch_target  # 新增代码+Phase107InteractiveLaunchTarget：导入解析函数和 token 行格式化器；如果没有这行代码，交互命令无法输出 Phase107 证据。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.interactive_launch_target"}:  # 新增代码+Phase107InteractiveLaunchTarget：只兜底包路径缺失；如果没有这行代码，解析器内部 bug 会被误吞。
            raise  # 新增代码+Phase107InteractiveLaunchTarget：重新抛出真实内部错误；如果没有这行代码，排查 Phase107 会被 fallback 干扰。
        from computer_use.interactive_launch_target import phase107_cli_line, resolve_interactive_launch_target  # type: ignore  # 新增代码+Phase107InteractiveLaunchTarget：脚本模式导入同一解析器；如果没有这行代码，双击 bat 后新命令不可用。
...
def _phase108_generic_app_tools() -> tuple[Any, Any]:  # 新增代码+Phase108GenericAppDiscovery：函数段开始，延迟导入 Phase108 通用应用发现工具；如果没有这段函数，脚本模式下 `/computer launch obsidian` 这类普通未知应用无法走通用发现。
    try:  # 新增代码+Phase108GenericAppDiscovery：优先按 learning_agent 包路径导入；如果没有这行代码，项目根运行时无法加载标准模块。
        from learning_agent.computer_use.generic_app_discovery import phase108_cli_line, resolve_generic_app_launch_target  # 新增代码+Phase108GenericAppDiscovery：导入通用解析函数和 token 格式化器；如果没有这行代码，交互命令无法输出 Phase108 证据。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.generic_app_discovery"}:  # 新增代码+Phase108GenericAppDiscovery：只兜底包路径缺失；如果没有这行代码，Phase108 内部 bug 会被误吞。
            raise  # 新增代码+Phase108GenericAppDiscovery：重新抛出真实内部错误；如果没有这行代码，排查通用发现会被 fallback 干扰。
        from computer_use.generic_app_discovery import phase108_cli_line, resolve_generic_app_launch_target  # type: ignore  # 新增代码+Phase108GenericAppDiscovery：脚本模式导入同一发现工具；如果没有这行代码，双击 bat 后通用发现不可用。
...
def _phase109_generic_real_launch_tools() -> tuple[Any, Any]:  # 新增代码+Phase109GenericRealLaunchCandidate：函数段开始，延迟导入 Phase109 通用真实启动候选工具；如果没有这段函数，脚本模式下 `/computer launch obsidian` 无法显示通用真实启动候选证据。
    try:  # 新增代码+Phase109GenericRealLaunchCandidate：优先按 learning_agent 包路径导入；如果没有这行代码，项目根运行时无法加载标准模块。
        from learning_agent.computer_use.generic_real_launch_candidate import phase109_cli_line, prepare_phase109_generic_real_launch_candidate  # 新增代码+Phase109GenericRealLaunchCandidate：导入候选准备函数和 token 格式化器；如果没有这行代码，交互命令无法输出 Phase109 证据。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.generic_real_launch_candidate"}:  # 新增代码+Phase109GenericRealLaunchCandidate：只兜底包路径缺失；如果没有这行代码，Phase109 内部 bug 会被误吞。
            raise  # 新增代码+Phase109GenericRealLaunchCandidate：重新抛出真实内部错误；如果没有这行代码，排查通用候选会被 fallback 干扰。
        from computer_use.generic_real_launch_candidate import phase109_cli_line, prepare_phase109_generic_real_launch_candidate  # type: ignore  # 新增代码+Phase109GenericRealLaunchCandidate：脚本模式导入同一候选工具；如果没有这行代码，双击 bat 后通用候选不可用。
...
def _run_phase106_interactive_full_launch(workspace: Path, mode_sessions: ComputerUseModeSessionStore, target_app: str, real_launch: bool | None = None, allow_real_gate: bool | None = None, real_launch_candidate: Any | None = None) -> dict[str, Any]:  # 新增代码+Phase106InteractiveFullLaunch：函数段开始，执行 `/computer launch notepad` 的 Phase106 桥接；如果没有这段函数，用户命令无法接到 Phase105 受控启动链。
    ...
    if not bool(target_report.get("real_launch_supported", False)):  # 新增代码+Phase107InteractiveLaunchTarget：识别普通但暂不支持真实启动的目标；如果没有这行代码，calc/mspaint 可能误走 Notepad smoke。
        ...
        decision = "phase107_target_recognized_real_launch_not_enabled_yet" if full_mode_session_used else "blocked_by_mode_session"  # 新增代码+Phase107InteractiveLaunchTarget：生成普通目标默认关闭或 mode 拦截原因；如果没有这行代码，用户不知道为何没启动。
        ...
        return {"marker": PHASE106_INTERACTIVE_FULL_LAUNCH_MARKER, "ok_token": PHASE106_INTERACTIVE_FULL_LAUNCH_OK_TOKEN, "passed": bool(full_mode_session_used), "target_app": target, "decision": decision, "real_action_blocked_decision": blocked_decision, "real_launch_requested": False, "real_enable_gate_passed": False, "controlled_real_launch_gate_passed": False, "full_mode_session_used": full_mode_session_used, "controlled_launch_candidate_ready": False, "real_full_launch_attempted": False, "visible_window_verified": False, "cleanup_completed": False, "verified_window_cleanup_completed": False, "residual_owned_process": False, "real_desktop_touched": False, "uncontrolled_actions_expanded": False, "phase107_target_report": target_report}  # 新增代码+Phase107InteractiveLaunchTarget：返回普通目标默认不启动报告；如果没有这行代码，Phase107 不能安全支持 calc/mspaint 识别。
...
def phase107_main(argv: list[str] | None = None) -> int:  # 新增代码+Phase107InteractiveLaunchTarget：函数段开始，提供 Phase107 真实可见终端验收入口；如果没有这段函数，目标解析只能靠单元测试不能满足真实终端门禁。
    ...
    high_risk_refused = bool("decision=phase107_high_risk_target_refused" in powershell_output and "high_risk_refused=true" in powershell_output)  # 新增代码+Phase107InteractiveLaunchTarget：汇总 powershell 是否被高风险拒绝；如果没有这行代码，最终 token 无法表达安全拒绝。
...
def phase108_main(argv: list[str] | None = None) -> int:  # 新增代码+Phase108GenericAppDiscovery：函数段开始，提供 Phase108 真实可见终端验收入口；如果没有这段函数，通用发现只能靠单元测试不能满足真实终端门禁。
    ...
    generic_target_default_off = bool(ok_token in generic_output and "dynamic_discovery_used=true" in generic_output and "hardcoded_app_whitelist_required=false" in generic_output and "per_app_patch_required=false" in generic_output and "generic_target_default_off=true" in generic_output and "real_full_launch_attempted=false" in generic_output and "real_desktop_touched=false" in generic_output)  # 修改代码+Phase108GenericAppDiscovery：汇总任意通用普通应用是否默认关闭真实启动；如果没有这行代码，最终 token 无法同时覆盖已安装发现和 fallback 发现。
    high_risk_refused = bool("decision=phase107_high_risk_target_refused" in powershell_output and "high_risk_refused=true" in powershell_output)  # 新增代码+Phase108GenericAppDiscovery：汇总 powershell 是否仍被高风险拒绝；如果没有这行代码，通用发现可能被误解成放开所有系统工具。
...
def phase109_main(argv: list[str] | None = None) -> int:  # 新增代码+Phase109GenericRealLaunchCandidate：函数段开始，提供 Phase109 真实可见终端验收入口；如果没有这段函数，通用真实启动候选只能靠单元测试不能满足真实终端门禁。
    ...
    generic_candidate_default_off = bool(ok_token in generic_output and "PHASE109_GENERIC_REAL_LAUNCH_CANDIDATE_READY" in generic_output and "generic_real_launch_candidate_ready=true" in generic_output and "uses_phase108_generic_discovery=true" in generic_output and "hardcoded_app_whitelist_required=false" in generic_output and "per_app_patch_required=false" in generic_output and "real_launch_default_disabled=true" in generic_output and "real_desktop_touched=false" in generic_output)  # 新增代码+Phase109GenericRealLaunchCandidate：汇总真实用户命令是否输出 Phase109 默认关闭候选；如果没有这行代码，最终 token 不能证明交互入口已接入。
    ...
    high_risk_refused = bool("decision=phase107_high_risk_target_refused" in powershell_output and "high_risk_refused=true" in powershell_output)  # 新增代码+Phase109GenericRealLaunchCandidate：汇总 powershell 是否仍被高风险拒绝；如果没有这行代码，通用候选可能被误解成放开所有系统工具。
    ...
    print(f"{marker}{final_ok} generic_real_launch_candidate_ready={_phase98_bool(generic_candidate_default_off)} uses_phase108_generic_discovery={_phase98_bool(generic_candidate_default_off)} hardcoded_app_whitelist_required=false per_app_patch_required=false real_launch_default_disabled=true default_off_backend_not_called=true recording_backend_reaches_launcher={_phase98_bool(recording_identity_cleanup_ready)} process_identity_verified={_phase98_bool(recording_identity_cleanup_ready)} window_identity_verified={_phase98_bool(recording_identity_cleanup_ready)} cleanup_completed={_phase98_bool(recording_identity_cleanup_ready)} residual_process_check_completed={_phase98_bool(recording_identity_cleanup_ready)} residual_owned_process=false high_risk_refused={_phase98_bool(high_risk_refused)} real_desktop_touched=false uncontrolled_actions_expanded=false")  # 新增代码+Phase109GenericRealLaunchCandidate：打印最终固定 token 行；如果没有这行代码，真实终端最终回答缺少稳定可复制锚点。
...
def phase115_main(argv: list[str] | None = None) -> int:  # 新增代码+FullMaturityMatrix：函数段开始，提供最终真实可见终端验收入口；如果没有这段函数，Task8 场景需要复制脆弱的内联命令序列。
    ...
    ordinary_default_off = bool("PHASE108_GENERIC_APP_DISCOVERY_READY" in ordinary_output and "dynamic_discovery_used=true" in ordinary_output and "hardcoded_app_whitelist_required=false" in ordinary_output and "per_app_patch_required=false" in ordinary_output and "generic_target_default_off=true" in ordinary_output and "real_full_launch_attempted=false" in ordinary_output and "real_desktop_touched=false" in ordinary_output)  # 新增代码+FullMaturityMatrix：汇总普通 app 默认关闭路径是否通过；如果没有这一行，最终 token 可能只证明矩阵不证明交互命令。
