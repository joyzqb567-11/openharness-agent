...
import { createGuiClient, type GuiBrowserActionPayload, type GuiCommandActionPayload, type GuiComputerUseActionPayload, type GuiHarnessControlPayload } from "../api/guiClient"; // 修改代码+DesktopGUICommandConsoleShell：引入后台命令控制响应类型；如果没有这行，前端无法类型化停止命令结果。
  ...
  useEffect(() => { // 新增代码+DesktopGUISessions：副作用段开始，加载项目名和最近会话；如果没有这段，侧栏无法从真实 bridge 获取项目上下文。
    ...
    async function loadSidebarData(): Promise<void> { // 新增代码+DesktopGUISessions：函数段开始，集中读取 bootstrap 和 sessions；如果没有这段，侧栏数据加载逻辑会散落到渲染层。
      try { // 新增代码+DesktopGUISessions：捕获侧栏加载失败；如果没有这行，bridge 暂时不可用会让界面副作用抛错。
        const [bootstrapPayload, sessionsPayload, runtimePanelsPayload, toolchainPayloadResult, mcpPayloadResult, planningPayloadResult, commandPayloadResult, healthPayloadResult, diagnosticsPayloadResult, harnessPayloadResult] = await Promise.all([activeClient.bootstrap(), activeClient.sessions(), activeClient.runtimePanels(), activeClient.toolchain(), activeClient.mcpServers(), activeClient.planning(), activeClient.commands(), activeClient.health(), activeClient.diagnostics(), activeClient.harnessStatus()]); // 修改代码+DesktopGUICommandConsoleShell：首屏同时读取工具链、MCP、计划和后台命令总览；如果没有这行，命令页签要等额外请求才有事实源。
