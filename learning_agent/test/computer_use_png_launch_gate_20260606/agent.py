...
class LearningAgent:  # 作用: 教学版 agent 主类
    ...
    def run(self, user_input: str, max_turns: int | None = None) -> str:  # 修改代码+可配置轮次: 默认不再写死 8 轮，允许配置层传入数字或 None；若省略: learning_agent 会继续和 Claude Code 的可配置策略不一致
        ...
        return run_agent_with_harness_session(self, user_input, max_turns=max_turns)  # 新增代码+HarnessSessionRuntime: 保持 run() 返回文本但底层走 run_events+harness 持久化；若没有这行代码，真实终端不会创建 harness run。
    ...
    def _build_computer_use_full_model_loop_harness_message(self, user_input: str) -> str:  # 新增代码+ModelLoopSemanticPlanner：函数段开始，为 full 模式桌面任务生成极简模型主循环约束；如果没有这段函数，自然语言桌面任务会缺少“由模型选工具”的明确上下文。
        ...
        return "\n".join([  # 新增代码+ModelLoopSemanticPlanner：拼出短提示词块；如果没有这一行，模型不会获得结构化 Computer Use full 工作方式。
            ...
            f"脱敏目标应用提示：{target_app_hint or '未知'}；脱敏任务类型提示：{task_goal or '未知'}。",  # 新增代码+ModelLoopSemanticPlanner：提供分类器已有的非原文线索；如果没有这一行，模型少一个启动目标应用的安全提示。
    ...
    def _computer_use_has_recent_model_visible_observation(self) -> bool:  # 新增代码+ObserveBeforeActionGate: 函数段开始，检查主循环是否已经得到可回灌模型的真实截图观察；如果没有这段函数，动作层无法知道模型是否真正看过屏幕。
        for event in reversed(self.observation_events[-50:]):  # 新增代码+ObserveBeforeActionGate: 从最近观察事件倒序查找，避免旧截图长期误放行；如果没有这行代码，动作可能依赖过期屏幕状态。
            if event.get("kind") != "computer_use_observe":  # 新增代码+ObserveBeforeActionGate: 只接受 Computer Use 观察事件；如果没有这行代码，普通 read/bash 事件可能误当成桌面观察。
                continue  # 新增代码+ObserveBeforeActionGate: 跳过非桌面观察事件；如果没有这行代码，后续字段读取会混入无关结构。
            payload = event.get("payload", {})  # 新增代码+ObserveBeforeActionGate: 读取观察事件载荷；如果没有这行代码，无法判断观察是否成功和是否有图。
            if not isinstance(payload, dict) or not bool(payload.get("ok", False)):  # 新增代码+ObserveBeforeActionGate: 只接受成功观察；如果没有这行代码，失败的 get_window_state 也可能放行动作。
                continue  # 新增代码+ObserveBeforeActionGate: 跳过失败或异常观察；如果没有这行代码，坏观察会污染门禁判断。
            data = payload.get("data", {})  # 新增代码+ObserveBeforeActionGate: 读取工具返回的结构化数据；如果没有这行代码，无法查 image_results 和 state。
            if not isinstance(data, dict):  # 新增代码+ObserveBeforeActionGate: 防御异常 data 类型；如果没有这行代码，字符串结果会触发属性错误。
                continue  # 新增代码+ObserveBeforeActionGate: 跳过异常 data；如果没有这行代码，门禁可能崩溃。
            state = data.get("state", {}) if isinstance(data.get("state", {}), dict) else {}  # 新增代码+ObserveBeforeActionGate: 读取嵌套窗口状态；如果没有这行代码，部分后端只在 state 内暴露截图字段时会误判无图。
            image_result_count = int(data.get("image_result_count", 0) or state.get("image_result_count", 0) or 0)  # 新增代码+ObserveBeforeActionGate: 统计模型可见图片块数量；如果没有这行代码，只有路径没有图片区块的观察会被误放行。
            screenshot_captured = bool(data.get("screenshot_captured", False) or state.get("screenshot_captured", False))  # 新增代码+ObserveBeforeActionGate: 检查真实截图是否捕获；如果没有这行代码，空 image_result 计数异常可能误放行。
            if image_result_count > 0 and screenshot_captured:  # 新增代码+ObserveBeforeActionGate: 同时要求有截图和可见图片区块；如果没有这行代码，模型可能仍只拿到窗口列表文字。
                return True  # 新增代码+ObserveBeforeActionGate: 找到有效观察后放行动作；如果没有这行代码，已经看过屏幕的正常动作会被一直拦住。
        return False  # 新增代码+ObserveBeforeActionGate: 最近没有有效截图观察时拒绝动作；如果没有这行代码，函数缺少明确失败结果。
    # 新增代码+ObserveBeforeActionGate: 函数段结束，_computer_use_has_recent_model_visible_observation 到此结束；如果没有这个边界说明，用户不容易看出“看过屏幕”的判定标准。
