...
try:  # 新增代码+Phase27ComputerUse: 包运行模式下导入窗口协议模型；如果没有这行代码，控制器无法使用强类型窗口引用。
    ...
    from learning_agent.computer_use.evidence import ComputerUseEvidenceStore, collect_image_result_blocks, format_image_result_lines  # 修改代码+Phase41WindowsImageResults: 导入证据仓库和图片结果格式化 helper；如果没有这行代码，controller 无法把截图 artifact 转成模型可见图片区。
    ...
    from learning_agent.computer_use.native_diagnostics import build_phase43_native_capability_matrix  # 新增代码+Phase43WindowsNativeCapabilityMatrix: 导入 Phase43 原生能力矩阵构建器；如果没有这行代码，Windows 后端 status 无法展示统一能力矩阵。
    ...
    from learning_agent.computer_use.real_sendinput_guard import WindowsSendInputLowLevelSender  # 新增代码+RealComputerActionExecutor: 导入真实低层 Windows 输入 sender；如果没有这行代码，默认真实后端只能创建合同层，无法真正触发鼠标键盘。
    from learning_agent.computer_use.sendinput_dispatcher import WindowsSendInputDispatcher  # 新增代码+RealComputerActionExecutor: 导入 Phase47 通用 dispatcher；如果没有这行代码，高层 click/type/key 事件不会展开成低层 SendInput 序列。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.action_policy", "learning_agent.computer_use.audit", "learning_agent.computer_use.evidence", "learning_agent.computer_use.helper_client", "learning_agent.computer_use.lock", "learning_agent.computer_use.models", "learning_agent.computer_use.native_diagnostics", "learning_agent.computer_use.native_helper", "learning_agent.computer_use.real_sendinput_guard", "learning_agent.computer_use.sendinput_dispatcher", "learning_agent.computer_use.sendinput_executor", "learning_agent.computer_use.windows_backend"}:  # 修改代码+RealComputerActionExecutor: 允许新增真实输入 sender 和 dispatcher 在脚本模式 fallback；如果没有这行代码，start_oauth_agent.bat 会把包路径差异误当成内部 bug。
    ...
    from computer_use.evidence import ComputerUseEvidenceStore, collect_image_result_blocks, format_image_result_lines  # 修改代码+Phase41WindowsImageResults: 脚本模式导入图片结果 helper；如果没有这行代码，bat 入口无法输出 Phase41 图片区块。
    ...
    from computer_use.native_diagnostics import build_phase43_native_capability_matrix  # 新增代码+Phase43WindowsNativeCapabilityMatrix: 脚本模式导入 Phase43 能力矩阵；如果没有这行代码，start_oauth_agent.bat 无法显示矩阵状态。
    ...
    from computer_use.real_sendinput_guard import WindowsSendInputLowLevelSender  # type: ignore  # 新增代码+RealComputerActionExecutor: 脚本模式导入真实低层 sender；如果没有这行代码，bat 入口的真实动作后端没有物理输入实现。
    from computer_use.sendinput_dispatcher import WindowsSendInputDispatcher  # type: ignore  # 新增代码+RealComputerActionExecutor: 脚本模式导入通用 dispatcher；如果没有这行代码，bat 入口无法把高层动作展开到底层事件。
...
def _build_real_windows_action_executor(platform: str | None = None) -> WindowsSendInputExecutor:  # 新增代码+RealComputerActionExecutor: 函数段开始，构造默认真实 Windows 输入执行链；如果没有这段函数，真实后端只能停在 implementation_available=false 的合同层。
    ...
    dispatcher = WindowsSendInputDispatcher(platform=current_platform, enabled=True, low_level_sender=low_level_sender)  # 新增代码+RealComputerActionExecutor: 把低层 sender 接到 Phase47 通用 dispatcher；如果没有这行代码，高层 click/key/text 事件不会变成低层 SendInput 序列。
...
def _build_default_target_session_runtime(environ: dict[str, str] | None = None) -> Any:  # 新增代码+ModelLoopLaunchAppTool: 函数段开始，构造 launch_app 使用的通用目标 session；如果没有这段函数，controller 会重新发明一套应用启动逻辑。
    ...
    try:  # 新增代码+ModelLoopLaunchAppTool: 优先按包模式导入通用目标 session；如果没有这行代码，正常包运行时无法复用 Phase117 目标绑定能力。
        from learning_agent.computer_use.universal_target_session import UniversalTargetSessionRuntime  # 新增代码+ModelLoopLaunchAppTool: 读取现有通用 session runtime；如果没有这行代码，controller 会绕开已有防漂移和自有窗口绑定。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.universal_target_session"}:  # 新增代码+ModelLoopLaunchAppTool: 只对目标包路径缺失做 fallback；如果没有这行代码，模块内部真实错误会被误吞。
            raise  # 新增代码+ModelLoopLaunchAppTool: 重新抛出非路径类错误；如果没有这行代码，排查启动 runtime bug 会很困难。
        from computer_use.universal_target_session import UniversalTargetSessionRuntime  # type: ignore  # 新增代码+ModelLoopLaunchAppTool: 脚本模式导入同一个 runtime；如果没有这行代码，bat 入口无法执行 launch_app。
...
class ComputerUseController:  # 新增代码+OSComputerUse: 在 agent 和后端之间提供安全控制层；若没有这段代码，真实桌面操作缺少统一确认和参数校验。
    ...
    def _launch_app_target_from_arguments(self, arguments: dict[str, Any]) -> str:  # 修改代码+ModelLoopLaunchAppTool: 函数段开始，从动作参数里提取干净应用名；如果没有这段函数，模型不同字段习惯会导致启动目标丢失。
        for field_name in ("app_name", "target_app", "app", "target", "text"):  # 新增代码+ModelLoopLaunchAppTool: 按常见字段顺序查找应用名；如果没有这行代码，模型只能使用一个固定字段才会成功。
    ...
    def _execute_launch_app_action(self, action: str, arguments: dict[str, Any]) -> ComputerUseActionResult:  # 新增代码+ModelLoopLaunchAppTool: 函数段开始，执行模型主循环的启动应用动作；如果没有这段函数，launch_app 会被错误交给鼠标键盘后端。
        ...
        session_report = dict(open_method(target_app, user_authorized_window=bool(arguments.get("user_authorized_window", False))))  # 新增代码+ModelLoopLaunchAppTool: 调用现有通用目标 session 建立/真实启动路径；如果没有这行代码，launch_app 不会打开并绑定应用窗口。
        launch_ok = bool(session_report.get("session_ready"))  # 新增代码+ModelLoopLaunchAppTool: 读取 session 是否就绪；如果没有这行代码，后续无法统一判断启动成功。
        if launch_ok:  # 新增代码+ModelLoopLaunchAppTool: 只在 session 就绪时更新 active target；如果没有这行代码，失败启动也可能覆盖正确窗口。
            self.active_agent_owned_target_window = dict(session_report.get("target_window", {}) or {})  # 新增代码+ModelLoopLaunchAppTool: 保存本次 agent-owned target_window；如果没有这行代码，后续漂移门禁没有基准。
        data = {"action": action, "action_class": "launch_app", "target_app": target_app, "session": session_report, "target_window": dict(session_report.get("target_window", {}) or {}), "real_desktop_touched": bool(session_report.get("real_desktop_touched")), "low_level_event_count": int(session_report.get("low_level_event_count", 0) or 0)}  # 新增代码+ModelLoopLaunchAppTool: 生成模型可读启动结果；如果没有这行代码，模型不知道下一步该观察哪个窗口。
