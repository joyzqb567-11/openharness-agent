    ...
    if not bool(target_report.get("safe_known_ordinary_app", False)):  # 新增代码+Phase107InteractiveLaunchTarget：先拦截高风险或未知目标；如果没有这行代码，powershell/cmd 等可能进入 Universal gate。
        if bool(target_report.get("unknown_target_refused", False)) and not bool(target_report.get("high_risk_refused", False)):  # 新增代码+Phase108GenericAppDiscovery：只有 Phase107 未知且不是高风险时才交给通用发现；如果没有这行代码，未知普通应用仍会被硬编码白名单卡住。
            ...
            if bool(generic_report.get("passed", False)):  # 新增代码+Phase108GenericAppDiscovery：只有普通且安全的通用目标才进入默认关闭成功路径；如果没有这行代码，高风险或无候选目标可能被误当成成功。
                ...
                mode_status = mode_sessions.status()  # 新增代码+Phase108GenericAppDiscovery：读取当前 Computer Use full 模式状态；如果没有这行代码，未知普通应用会绕过 `/computer use --full-confirm`。
...
def _phase106_confirmation_token_from_output(output: str) -> str:  # 新增代码+Phase106InteractiveFullLaunch：函数段开始，从 full 请求输出提取 token；如果没有这段函数，phase106_main 无法自动跑完整用户命令链。
    for line in str(output or "").splitlines():  # 新增代码+Phase106InteractiveFullLaunch：逐行扫描终端输出；如果没有这行代码，多行面板无法解析。
        if line.startswith("- confirmation_token="):  # 新增代码+Phase106InteractiveFullLaunch：识别确认 token 行；如果没有这行代码，可能误读其他字段。
            return line.split("=", 1)[1].strip()  # 新增代码+Phase106InteractiveFullLaunch：返回 token 文本；如果没有这行代码，full-confirm 命令无法构造。
    return ""  # 新增代码+Phase106InteractiveFullLaunch：找不到 token 时返回空值；如果没有这行代码，验收入口会抛难懂异常。
# 新增代码+Phase106InteractiveFullLaunch：函数段结束，_phase106_confirmation_token_from_output 到此结束；如果没有这个边界说明，初学者不容易看出 token 解析范围。
...
def run_computer_terminal_command(workspace: Path, user_input: str) -> str:  # 新增代码+Phase31ComputerUseLockAbortEvidence: 函数段开始，执行 `/computer` 子命令；如果没有这段函数，真实用户无法在终端查看或触发桌面急停。
    ...
    if subcommand in {"launch", "open"}:  # 新增代码+Phase106InteractiveFullLaunch：识别 `/computer launch notepad` 用户启动命令；如果没有这行代码，Phase105 能力只能通过合同函数调用，真实用户命令无法触发。
        ...
        phase106_report = _run_phase106_interactive_full_launch(Path(workspace), mode_sessions, target_app)  # 新增代码+Phase106InteractiveFullLaunch：复用当前 workspace 和 mode session 运行受控启动桥；如果没有这行代码，命令不会经过 full-confirm 状态。
...
def phase106_main(argv: list[str] | None = None) -> int:  # 新增代码+Phase106InteractiveFullLaunch：函数段开始，提供真实可见终端一键验收入口；如果没有这段函数，Phase106 验收需要人工复制动态 token，容易出错。
    ...
    full_request_output = run_computer_terminal_command(workspace, "/computer use --full")  # 新增代码+Phase106InteractiveFullLaunch：第一步按真实用户命令申请 full token；如果没有这行代码，验收会绕过高风险二次确认。
    confirmation_token = _phase106_confirmation_token_from_output(full_request_output)  # 新增代码+Phase106InteractiveFullLaunch：从输出里读取动态 token；如果没有这行代码，下一步无法确认 full mode。
    print(full_request_output, end="")  # 新增代码+Phase106InteractiveFullLaunch：把 full 请求输出打印到真实终端；如果没有这行代码，用户看不到强确认证据。
    if not confirmation_token:  # 新增代码+Phase106InteractiveFullLaunch：检查 token 是否成功解析；如果没有这行代码，空 token 会导致后续失败原因不清楚。
        print(f"{PHASE106_INTERACTIVE_FULL_LAUNCH_MARKER} token_parse_failed=true")  # 新增代码+Phase106InteractiveFullLaunch：打印稳定失败 marker；如果没有这行代码，验收器无法定位失败原因。
        return 1  # 新增代码+Phase106InteractiveFullLaunch：用失败退出码终止；如果没有这行代码，token 缺失也可能继续跑后续命令。
    full_confirm_output = run_computer_terminal_command(workspace, f"/computer use --full-confirm {confirmation_token}")  # 新增代码+Phase106InteractiveFullLaunch：第二步按真实用户命令确认 full；如果没有这行代码，启动命令必须被 mode gate 拦截。
    print(full_confirm_output, end="")  # 新增代码+Phase106InteractiveFullLaunch：打印 full 确认结果；如果没有这行代码，用户看不到 full_mode=true 证据。
...
def phase107_main(argv: list[str] | None = None) -> int:  # 新增代码+Phase107InteractiveLaunchTarget：函数段开始，提供 Phase107 真实可见终端验收入口；如果没有这段函数，目标解析只能靠单元测试不能满足真实终端门禁。
    ...
    full_request_output = run_computer_terminal_command(workspace, "/computer use --full")  # 新增代码+Phase107InteractiveLaunchTarget：先按真实命令申请 full token；如果没有这行代码，calc 启动默认关闭路径会缺少授权前置。
    confirmation_token = _phase106_confirmation_token_from_output(full_request_output)  # 新增代码+Phase107InteractiveLaunchTarget：解析动态 full token；如果没有这行代码，无法自动执行 full-confirm。
    print(full_request_output, end="")  # 新增代码+Phase107InteractiveLaunchTarget：打印 full 请求证据；如果没有这行代码，真实终端看不到强确认流程。
    if not confirmation_token:  # 新增代码+Phase107InteractiveLaunchTarget：检查 token 是否存在；如果没有这行代码，空 token 会造成后续失败难排查。
        print(f"{marker} token_parse_failed=true")  # 新增代码+Phase107InteractiveLaunchTarget：打印稳定失败 marker；如果没有这行代码，验收器无法定位失败。
        return 1  # 新增代码+Phase107InteractiveLaunchTarget：失败退出；如果没有这行代码，缺 token 仍可能继续运行。
    full_confirm_output = run_computer_terminal_command(workspace, f"/computer use --full-confirm {confirmation_token}")  # 新增代码+Phase107InteractiveLaunchTarget：用真实命令确认 full；如果没有这行代码，launch calc 应被 mode gate 拦截。
    print(full_confirm_output, end="")  # 新增代码+Phase107InteractiveLaunchTarget：打印 full 确认证据；如果没有这行代码，真实终端看不到 full_mode=true。
...
def phase108_main(argv: list[str] | None = None) -> int:  # 新增代码+Phase108GenericAppDiscovery：函数段开始，提供 Phase108 真实可见终端验收入口；如果没有这段函数，通用发现只能靠单元测试不能满足真实终端门禁。
    ...
    full_request_output = run_computer_terminal_command(workspace, "/computer use --full")  # 新增代码+Phase108GenericAppDiscovery：先按真实命令申请 full token；如果没有这行代码，generic launch 默认关闭路径缺少授权前置。
    confirmation_token = _phase106_confirmation_token_from_output(full_request_output)  # 新增代码+Phase108GenericAppDiscovery：解析动态 full token；如果没有这行代码，无法自动执行 full-confirm。
    print(full_request_output, end="")  # 新增代码+Phase108GenericAppDiscovery：打印 full 请求证据；如果没有这行代码，真实终端看不到强确认流程。
    if not confirmation_token:  # 新增代码+Phase108GenericAppDiscovery：检查 token 是否存在；如果没有这行代码，空 token 会造成后续失败难排查。
        print(f"{marker} token_parse_failed=true")  # 新增代码+Phase108GenericAppDiscovery：打印稳定失败 marker；如果没有这行代码，验收器无法定位失败。
        return 1  # 新增代码+Phase108GenericAppDiscovery：失败退出；如果没有这行代码，缺 token 仍可能继续运行。
    full_confirm_output = run_computer_terminal_command(workspace, f"/computer use --full-confirm {confirmation_token}")  # 新增代码+Phase108GenericAppDiscovery：用真实命令确认 full；如果没有这行代码，launch obsidian 应被 mode gate 拦截。
    print(full_confirm_output, end="")  # 新增代码+Phase108GenericAppDiscovery：打印 full 确认证据；如果没有这行代码，真实终端看不到 full_mode=true。
...
def phase109_main(argv: list[str] | None = None) -> int:  # 新增代码+Phase109GenericRealLaunchCandidate：函数段开始，提供 Phase109 真实可见终端验收入口；如果没有这段函数，通用真实启动候选只能靠单元测试不能满足真实终端门禁。
    ...
    full_request_output = run_computer_terminal_command(workspace, "/computer use --full")  # 新增代码+Phase109GenericRealLaunchCandidate：先按真实命令申请 full token；如果没有这行代码，generic launch 默认关闭路径缺少授权前置。
    confirmation_token = _phase106_confirmation_token_from_output(full_request_output)  # 新增代码+Phase109GenericRealLaunchCandidate：解析动态 full token；如果没有这行代码，无法自动执行 full-confirm。
    print(full_request_output, end="")  # 新增代码+Phase109GenericRealLaunchCandidate：打印 full 请求证据；如果没有这行代码，真实终端看不到强确认流程。
    if not confirmation_token:  # 新增代码+Phase109GenericRealLaunchCandidate：检查 token 是否存在；如果没有这行代码，确认失败会继续误跑后续 launch。
        print(f"{marker} generic_real_launch_candidate_ready=false uses_phase108_generic_discovery=false hardcoded_app_whitelist_required=true per_app_patch_required=true real_launch_default_disabled=true default_off_backend_not_called=false recording_backend_reaches_launcher=false process_identity_verified=false window_identity_verified=false cleanup_completed=false residual_process_check_completed=false residual_owned_process=true high_risk_refused=false real_desktop_touched=false uncontrolled_actions_expanded=false")  # 新增代码+Phase109GenericRealLaunchCandidate：打印失败锚点；如果没有这行代码，真实终端验收器可能等不到 marker。
        return 1  # 新增代码+Phase109GenericRealLaunchCandidate：token 缺失时返回失败；如果没有这行代码，失败也可能被自动化当成功。
    full_confirm_output = run_computer_terminal_command(workspace, f"/computer use --full-confirm {confirmation_token}")  # 新增代码+Phase109GenericRealLaunchCandidate：用真实命令确认 full；如果没有这行代码，launch obsidian 应被 mode gate 拦截。
    print(full_confirm_output, end="")  # 新增代码+Phase109GenericRealLaunchCandidate：打印 full 确认证据；如果没有这行代码，真实终端看不到 full_mode=true。
...
def phase115_main(argv: list[str] | None = None) -> int:  # 新增代码+FullMaturityMatrix：函数段开始，提供最终真实可见终端验收入口；如果没有这段函数，Task8 场景需要复制脆弱的内联命令序列。
    ...
    full_request_output = run_computer_terminal_command(workspace, "/computer use --full")  # 新增代码+FullMaturityMatrix：第二步按真实用户命令申请 full token；如果没有这一行，后续 launch 会缺少授权前置。
    confirmation_token = _phase106_confirmation_token_from_output(full_request_output)  # 新增代码+FullMaturityMatrix：解析动态 full token；如果没有这一行，无法自动执行 full-confirm。
    print(full_request_output, end="")  # 新增代码+FullMaturityMatrix：打印 full 请求输出；如果没有这一行，真实终端看不到强确认和矩阵预览。
    if not confirmation_token:  # 新增代码+FullMaturityMatrix：检查 token 是否存在；如果没有这一行，空 token 会导致后续失败原因不清楚。
        print(f"{maturity_line} token_parse_failed=true")  # 新增代码+FullMaturityMatrix：打印稳定失败锚点；如果没有这一行，验收器可能等不到最终 marker。
        return 1  # 新增代码+FullMaturityMatrix：token 缺失时失败退出；如果没有这一行，失败也可能继续误跑后续命令。
    full_confirm_output = run_computer_terminal_command(workspace, f"/computer use --full-confirm {confirmation_token}")  # 新增代码+FullMaturityMatrix：第三步按真实用户命令确认 full；如果没有这一行，普通 launch 应被 mode gate 拦截。
    print(full_confirm_output, end="")  # 新增代码+FullMaturityMatrix：打印 full 确认输出；如果没有这一行，真实终端看不到 full_mode=true。
