...
def run_computer_terminal_command(workspace: Path, user_input: str) -> str:  # 新增代码+Phase31ComputerUseLockAbortEvidence: 函数段开始，执行 `/computer` 子命令；如果没有这段函数，真实用户无法在终端查看或触发桌面急停。
    ...
    return "Computer Action\n- 不支持的 /computer 子命令。\n- 可用命令：/computer、/computer status、/computer abort <reason>、/computer clear-abort、/computer cleanup [session_id]、/computer recover、/computer journal、/computer notifications、/computer release [session_id]\n"  # 修改代码+Phase50WindowsRecovery: 返回包含 recover/journal 的用法；如果没有这行代码，输错 Phase50 命令时用户不知道怎么修正。
