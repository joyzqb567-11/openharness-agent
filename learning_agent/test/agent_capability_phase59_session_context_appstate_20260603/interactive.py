...
try:  # 新增代码+AppSplit: 优先按包路径导入轮次格式化和验收事件 helper；若没有这行代码，包运行模式下交互层无法复用通用能力。
    ...
    if error.name not in {"learning_agent", "learning_agent.app", "learning_agent.app.chrome_status_renderer", "learning_agent.app.computer_status_renderer", "learning_agent.app.status_renderer", "learning_agent.browser_extension_host", "learning_agent.browser_extension_host.bridge_server", "learning_agent.browser_extension_host.manifest_installer", "learning_agent.computer_use", "learning_agent.computer_use.approval", "learning_agent.computer_use.controller", "learning_agent.computer_use.lock", "learning_agent.computer_use.native_diagnostics", "learning_agent.computer_use.security_policy", "learning_agent.computer_use.session_context", "learning_agent.computer_use.session_runtime", "learning_agent.computer_use.terminal_grants", "learning_agent.core", "learning_agent.core.config", "learning_agent.observability", "learning_agent.observability.acceptance_events", "learning_agent.observability.run_records", "learning_agent.runtime", "learning_agent.runtime.command_queue", "learning_agent.runtime.status_snapshot", "learning_agent.sdk", "learning_agent.sdk.status"}:  # 修改代码+Phase59SessionContextAppState: 允许 session_context 在脚本模式 fallback；若没有这行代码，bat 入口下 /computer status 读不到统一 session context。
...
def run_computer_terminal_command(workspace: Path, user_input: str) -> str:  # 新增代码+Phase31ComputerUseLockAbortEvidence: 函数段开始，执行 `/computer` 子命令；如果没有这段函数，真实用户无法在终端查看或触发桌面急停。
    ...
    if subcommand in {"status", ""}:  # 新增代码+Phase31ComputerUseLockAbortEvidence: 识别状态查询；如果没有这行代码，用户无法只读查看 Computer Use 状态。
        ...
        status_snapshot = {"lock": lock_manager.status(), "approval": approval_model.status(), "runtime": runtime.status(), "recovery": runtime.action_journal(limit=3), "terminal_grants": terminal_grants.status(), "session_context": session_context_store.status(DEFAULT_SESSION_CONTEXT_ID), "capability_matrix": capability_report["matrix"]}  # 修改代码+Phase59SessionContextAppState: 组装 renderer 快照并加入统一 session context；如果没有这行代码，/computer status 看不到 Phase59 的 allowed_apps/grants/display/screenshot/cleanup 事实源。
        return render_computer_status(status_snapshot)  # 修改代码+Phase51ComputerStatusUI: 返回紧凑 `/computer` 状态面板；如果没有这行代码，Phase51 renderer 不会进入真实终端入口。
    ...
    if subcommand == "revoke":  # 新增代码+Phase51ComputerStatusUI: 识别终端授权草案撤销命令；如果没有这行代码，用户无法从状态面板收回 grant 草案。
        ...
        revoke_result = terminal_grants.revoke_app(app_key)  # 新增代码+Phase51ComputerStatusUI: 从草案 store 删除 app；如果没有这行代码，revoke 命令不会改变状态。
        return f"Computer Revoke\n- revoked={str(bool(revoke_result.get('revoked', False))).lower()}\n- app_key={revoke_result.get('app_key', '')}\n- grant_scope={revoke_result.get('grant_scope', '')}\n- actions_expanded={str(bool(revoke_result.get('actions_expanded', False))).lower()}\n"  # 新增代码+Phase51ComputerStatusUI: 返回稳定 revoke 结果；如果没有这行代码，用户看不到撤销是否成功。
    ...
    return "Computer Action\n- 不支持的 /computer 子命令。\n- 可用命令：/computer、/computer status、/computer parity、/computer native-gate、/computer observe [action]、/computer grant <app> [flags]、/computer revoke <app>、/computer abort <reason>、/computer clear-abort、/computer cleanup [session_id]、/computer recover、/computer journal、/computer notifications、/computer release [session_id]\n"  # 修改代码+Phase54WindowsNativeRealityGate: 返回包含 native-gate 的用法；如果没有这行代码，输错 Phase54 命令时用户不知道怎么查看原生依赖门禁。
