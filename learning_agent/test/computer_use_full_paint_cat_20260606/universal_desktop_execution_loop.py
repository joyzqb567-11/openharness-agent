...
class UniversalDesktopExecutionLoopAdapter:  # 新增代码+UniversalDesktopAdapter：类段开始，把 desktop_task_runtime 需要的 run_desktop_task 接口桥接到通用闭环；如果没有这个类，默认 full 模式只能保持空 loop 或误接特例 loop。
    def __init__(self, loop: Any | None = None, observation_runtime: Any | None = None, use_real_observation: bool = True, controlled_physical_sender: Any | None = None, enable_supported_paint_drawing: bool = False, supported_paint_drawing_loop: Any | None = None) -> None:  # 修改代码+FullPaintDrawingBridge：函数段开始，允许生产路径显式接入已支持 Paint 绘图真实闭环；如果没有这段参数，用户 full 路径仍不会打开真实 Paint。
        ...
        controlled_action_runtime = UniversalActionDslRuntime(low_level_sender=controlled_physical_sender) if controlled_physical_sender is not None else None  # 新增代码+ControlledPhysicalAdapter：配置受控 sender 时创建专用动作 runtime；如果没有这一行，受控 sender 参数不会真正被 loop 使用。
