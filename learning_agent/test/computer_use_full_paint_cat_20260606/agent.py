...
class LearningAgent:  # 作用: 教学版 agent 主类
    ...
    def _desktop_task_runtime_for_current_run(self) -> Any:  # 新增代码+DesktopTaskRuntimeRoute：函数段开始，取得当前 run 应使用的桌面任务 runtime；如果没有这段函数，测试无法注入 full-mode runtime。
        ...
        runtime = ComputerUseDesktopTaskRuntime(base_dir=runtime_root, real_execution_loop=UniversalDesktopExecutionLoopAdapter(enable_supported_paint_drawing=True))  # 修改代码+FullPaintDrawingBridge：默认生产路径显式开启受支持 Paint 绘图真实桥；如果没有这一行，用户输入画猫仍不会打开真实 Paint。
