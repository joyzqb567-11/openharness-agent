...
try:  # 修改代码+BrowserSplit: 优先从 browser 包导入真实浏览器 helper；若没有这行代码，learning_agent.py 会继续承载浏览器意图和授权细节。
    from learning_agent.browser.harness import build_real_browser_task_harness_message  # 修改代码+BrowserSplit: 包运行模式下导入真实浏览器任务 harness 构造器；若没有这行代码，自然短 prompt 约束仍要写在主类里。
    from learning_agent.browser.intent import detect_real_browser_information_task, detect_real_chrome_intent, independent_browser_tool_names, real_chrome_request_blocks_independent_browser  # 修改代码+BrowserSplit: 包运行模式下导入真实浏览器意图与阻断 helper；若没有这行代码，工具池策略无法复用 browser 层。
    from learning_agent.browser.permissions import customer_mode_can_auto_approve_terminal_permission, real_browser_customer_auto_approve_reason, real_browser_customer_mode_active, real_browser_customer_progress_message  # 修改代码+BrowserSplit: 包运行模式下导入客户模式授权 helper；若没有这行代码，MCP 和终端权限仍会重复实现白名单。
    ...
    from browser.harness import build_real_browser_task_harness_message  # 修改代码+BrowserSplit: 脚本模式下导入真实浏览器任务 harness 构造器；若没有这行代码，bat 入口无法复用新 browser 层。
    from browser.intent import detect_real_browser_information_task, detect_real_chrome_intent, independent_browser_tool_names, real_chrome_request_blocks_independent_browser  # 修改代码+BrowserSplit: 脚本模式下导入真实浏览器意图与阻断 helper；若没有这行代码，直接运行时工具策略会找不到 helper。
    from browser.permissions import customer_mode_can_auto_approve_terminal_permission, real_browser_customer_auto_approve_reason, real_browser_customer_mode_active, real_browser_customer_progress_message  # 修改代码+BrowserSplit: 脚本模式下导入客户模式授权 helper；若没有这行代码，直接运行时客户模式会回退到主文件重复逻辑。
...
class LearningAgent:  # 作用: 教学版最小 agent 主类
    def __init__(  # 修改代码: 初始化参数增加调试日志配置，但保持旧调用方式仍然可用
    ...
    def run(self, user_input: str, max_turns: int | None = None) -> str:  # 修改代码+可配置轮次: 默认不再写死 8 轮，允许配置层传入数字或 None；若省略: learning_agent 会继续和 Claude Code 的可配置策略不一致
        ...
        desktop_task_answer = self._desktop_task_runtime_answer_from_prompt(user_input)  # 新增代码+DesktopTaskRuntimeRoute：在进入普通模型 loop 前尝试拦截自然语言桌面任务；如果没有这一行，模型仍可能选择 bash 生成最终图片绕路。
        if desktop_task_answer is not None:  # 新增代码+DesktopTaskRuntimeRoute：只有确认为桌面任务时才提前返回；如果没有这一行，普通开发/聊天任务会被错误截断。
            return desktop_task_answer  # 新增代码+DesktopTaskRuntimeRoute：返回 Computer Use runtime 的稳定 token 答案；如果没有这一行，桌面任务仍会进入普通模型工具循环。
        if self._stop_requested():  # 新增代码+AsyncTask: 在构造新一轮上下文前检查后台取消信号；若省略: 已取消的子 agent 仍会继续发起模型请求
            return "任务已停止：收到取消请求。"  # 新增代码+AsyncTask: 取消时直接返回可读状态；若省略: task_stop 后子 agent 仍可能继续执行
        self.real_chrome_requested = self._detect_real_chrome_intent(user_input)  # 新增代码+RealChromeWorkflow: 每轮从用户原话识别是否要求真实桌面 Chrome；若没有这行代码，策略层无法阻止误走独立 Chromium
        self.real_browser_information_task_requested = self._detect_real_browser_information_task(user_input)  # 新增代码+真实浏览器客户模式: 每轮识别是否可启用公开查询白名单自动授权；若没有这行代码，客户模式会缺少任务级边界
        self._maybe_confirm_plan_from_user_input(user_input)  # 新增代码+PlanModeGate: 用户明确确认计划后解除 awaiting_confirmation 闸门；若没有这行代码，exit_plan_mode 后副作用工具会被永久锁住
        run_id = self._new_debug_run_id()  # 新增代码: 为本轮用户请求生成一个调试编号，便于在日志中串起同一轮流程
        self._write_debug_event(run_id=run_id, event="user_input", payload={"text": user_input})  # 新增代码: 记录用户原始输入，便于回放问题从哪里开始
        messages = self._build_initial_messages(user_input)  # 作用: 构造本轮对话初始消息
        initial_tools = self._available_tool_schemas()  # 修改代码+ToolPolicyV2: 仅为初始日志读取一次当前工具池；若没有这行代码，调试日志缺少第一轮请求前的工具快照
        self._write_debug_event(run_id=run_id, event="initial_messages", payload={"messages": messages, "tool_names": self._tool_schema_names(initial_tools)})  # 修改代码+ToolPolicyV2: 记录初始上下文和初始工具名；若没有这行代码，select 后每轮重算 tools 时初始日志会失去参照
        turn = 0  # 修改代码+可配置轮次: 显式维护轮次编号，支持 max_turns=None 的无固定上限循环；若省略: for range 仍要求固定数字上限
        final_answer_retry_count = 0  # 新增代码+最终答案完整性: 记录本轮最终答案因缺用户要求标题而自动重写的次数；若没有这行代码，重试可能无限循环
        while max_turns is None or turn < max_turns:  # 修改代码+可配置轮次: 只有配置了数字上限时才按轮次停止；若省略: 默认仍会被硬编码轮次限制
            if self._stop_requested():  # 新增代码+AsyncTask: 每轮模型调用前检查取消信号；若省略: task_stop 后可能还会继续消耗模型调用
                return "任务已停止：收到取消请求。"  # 新增代码+AsyncTask: 在安全边界停止子 agent；若省略: 后台任务可能继续推进到下一轮工具循环
            tools = self._available_tool_schemas()  # 修改代码+ToolPolicyV2: 每轮模型请求前重新计算工具池；若没有这行代码，上一批 select 合并后的 deferred 工具不会进入下一轮 schema
            self._write_debug_event(run_id=run_id, event="model_request", payload={"messages": messages, "tool_names": self._tool_schema_names(tools)}, turn=turn)  # 修改代码+ToolPolicyV2: 记录即将发给模型的 messages 和本轮重新计算的工具列表；若没有这行代码，调试日志无法证明 select 后 schema 已刷新
            model_message = self.model.chat(messages, tools)  # 修改代码+ToolPolicyV2: 把本轮重新计算的工具列表传给模型；若没有这行代码，动态加载的 MCP 工具不会在下一轮变得可见
            self._write_debug_event(run_id=run_id, event="model_response", payload=self._model_message_to_debug_dict(model_message), turn=turn)  # 新增代码: 记录模型返回的文本和 tool_calls
            messages.append(self._assistant_message_to_dict(model_message))  # 作用: 把模型消息加入历史
            if self._stop_requested():  # 新增代码+AsyncTask: 模型返回后、执行工具前再次检查取消信号；若省略: 已取消子任务仍可能执行写入或外部副作用工具
                return "任务已停止：收到取消请求。"  # 新增代码+AsyncTask: 防止取消后的工具调用继续发生；若省略: task_stop 的安全意义会变弱
            if not model_message.tool_calls:  # 作用: 如果模型没有请求工具
                retry_message = self._final_answer_retry_message(user_input, model_message.text)  # 新增代码+最终答案完整性: 检查最终回答是否漏掉用户明确要求的 Markdown 标题；若没有这行代码，复合任务可能提前返回不完整答案
                can_retry_final_answer = retry_message and final_answer_retry_count < 1 and (max_turns is None or turn + 1 < max_turns)  # 新增代码+最终答案完整性: 只允许自动重写一次且保留至少一轮预算；若没有这行代码，短轮次任务可能越过安全上限或陷入循环
                if can_retry_final_answer:  # 新增代码+最终答案完整性: 如果可以安全重写最终答案；若没有这行代码，下面的修正消息不会进入模型
                    final_answer_retry_count += 1  # 新增代码+最终答案完整性: 消耗一次重写机会；若没有这行代码，模型持续漏标题时会被反复要求重写
                    self._write_debug_event(run_id=run_id, event="final_answer_retry", payload={"message": retry_message}, turn=turn)  # 新增代码+最终答案完整性: 记录自动重写原因；若没有这行代码，调试日志无法解释为什么多了一次模型调用
                    messages.append({"role": "user", "content": retry_message})  # 新增代码+最终答案完整性: 把可恢复的格式修正请求放回对话；若没有这行代码，模型没有机会基于已有工具结果补全攻略
                    turn += 1  # 新增代码+最终答案完整性: 自动重写也计入模型轮次；若没有这行代码，重试不会消耗 max_turns 预算
                    continue  # 新增代码+最终答案完整性: 继续下一轮模型调用而不是返回不完整答案；若没有这行代码，重试消息添加后仍会提前结束
                self._write_debug_event(run_id=run_id, event="final_answer", payload={"text": model_message.text}, turn=turn)  # 新增代码: 记录最终回答，便于观察本轮如何结束
                return model_message.text  # 作用: 直接返回最终回答
            self.defer_tool_select_until_next_turn = True  # 新增代码+ToolPolicyV2: 进入当前模型消息的一批 tool_calls 处理期，让 select 先进入 pending；若没有这行代码，同批后续工具会立刻看到 loaded 状态
            for tool_call in model_message.tool_calls:  # 修改代码+Stage15F: 先按原顺序记录本批工具调用；若没有这行代码，并发执行时调试日志会缺少稳定开始顺序。
                self._write_debug_event(run_id=run_id, event="tool_call", payload=self._tool_call_to_debug_dict(tool_call), turn=turn)  # 新增代码: 记录工具名、参数和 call_id，便于观察模型选择了什么工具
            tool_outputs = execute_tool_calls_from_orchestrator(self, model_message.tool_calls)  # 新增代码+Stage15F: 通过安全编排器批量执行工具；若没有这行代码，只读工具无法按元数据并发提速。
            for tool_call, tool_output in zip(model_message.tool_calls, tool_outputs):  # 修改代码+Stage15F: 按原 tool_call 顺序回填工具结果；若没有这行代码，并发完成顺序可能污染模型上下文。
                context_tool_output = self._offload_tool_output_if_needed(tool_call, tool_output)  # 新增代码+ResultPersistence: 大工具结果先落盘再把摘要放回上下文；若没有这行代码，长输出会继续撑爆模型上下文
                self._write_debug_event(run_id=run_id, event="tool_result", payload={"tool_name": tool_call.name, "call_id": tool_call.call_id, "output": context_tool_output, "raw_output_chars": len(tool_output)}, turn=turn)  # 修改代码+ResultPersistence: 记录送回模型的摘要和原始长度；若没有这行代码，审计时无法知道是否发生过落盘压缩
                messages.append(self._tool_result_to_dict(tool_call, context_tool_output))  # 修改代码+ResultPersistence: 把压缩后的工具结果放回历史；若没有这行代码，大输出仍会进入后续模型请求
                if self._stop_requested():  # 新增代码+AsyncTask: 每个工具执行后检查取消信号；若省略: 一批工具调用中后续工具可能在取消后继续执行
                    self.defer_tool_select_until_next_turn = False  # 新增代码+ToolPolicyV2: 停止前关闭批次延迟标记；若没有这行代码，后续复用同一 agent 直接 select 会被错误延迟
                    return "任务已停止：收到取消请求。"  # 新增代码+AsyncTask: 让后台子 agent 在工具边界尽快停止；若省略: task_stop 只能等完整循环结束才生效
            self.defer_tool_select_until_next_turn = False  # 新增代码+ToolPolicyV2: 当前批次 tool_calls 已处理完，关闭延迟 select 标记；若没有这行代码，后续普通直接 select 也会被错误放进 pending
            self._commit_pending_loaded_tool_names()  # 新增代码+ToolPolicyV2: 批次结束后把 pending select 合并到 loaded_tool_names；若没有这行代码，下一轮工具池仍看不到刚 select 的 deferred 工具
            turn += 1  # 新增代码+可配置轮次: 工具调用处理完后进入下一轮模型调用；若省略: while 循环会卡在同一轮编号并在有限上限下无法推进
        safety_message = "已经达到最大工具循环次数，为了安全我先停止。请把任务拆小一点再试。"  # 修改代码: 把安全停止提示先保存到变量，便于同时记录日志和返回
    ...
    def _desktop_task_runtime_answer_from_prompt(self, user_input: str) -> str | None:  # 新增代码+DesktopTaskRuntimeRoute：函数段开始，把普通 prompt 分流到桌面任务 runtime；如果没有这段函数，run() 无法在模型前执行治本路由。
        ...
        report = runtime.run_prompt(normalized_input, real_actions=False)  # 新增代码+DesktopTaskRuntimeRoute：以录制模式运行桌面任务；如果没有这一行，Task 5 只能分类不能生成 runtime 证据。
    ...
    def _desktop_task_runtime_for_current_run(self) -> Any:  # 新增代码+DesktopTaskRuntimeRoute：函数段开始，取得当前 run 应使用的桌面任务 runtime；如果没有这段函数，测试无法注入 full-mode runtime。
        ...
        try:  # 新增代码+DesktopTaskRuntimeRoute：优先按包路径导入 Task 4 runtime；如果没有这一行，项目根运行时无法使用新模块。
            ...
            if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.desktop_task_runtime"}:  # 新增代码+DesktopTaskRuntimeRoute：只对目标路径缺失兜底；如果没有这一行，runtime 内部 bug 会被误吞。
        ...
        runtime = ComputerUseDesktopTaskRuntime(base_dir=runtime_root)  # 新增代码+DesktopTaskRuntimeRoute：创建默认桌面任务 runtime；如果没有这一行，普通真实 run 没有 runtime 实例。
    ...
    def _format_desktop_task_runtime_answer(self, report: dict[str, Any]) -> str:  # 新增代码+DesktopTaskRuntimeRoute：函数段开始，把 runtime 报告转成最终回答文本；如果没有这段函数，run() 返回会缺少稳定可验收格式。
        ...
        guidance_line = "请先运行 /computer use --full 并按提示执行 /computer use --full-confirm <token>。" if str(report.get("decision", "")) == "computer_use_full_mode_required" else "桌面任务已交给 Computer Use runtime 录制模式证据链。"  # 新增代码+DesktopTaskRuntimeRoute：根据 full mode 门禁给出下一步或成功说明；如果没有这一行，用户不知道失败时该怎么开启权限。
    ...
    def run_events(self, user_input: str, max_turns: int | None = None):  # 新增代码+Stage15C: 新增事件流主循环；若没有这行代码，UI、HTTP bridge 和 transcript 无法观察运行过程。
        ...
        def emit(event_type: str, payload: dict[str, Any]) -> AgentEvent:  # 新增代码+Stage15C: 定义统一事件创建和落盘 helper；若没有这行代码，主循环每处都要重复写事件逻辑。
            nonlocal sequence  # 新增代码+Stage15C: 允许内部 helper 推进外层顺序号；若没有这行代码，事件序号不会递增。
        ...
        try:  # 新增代码+Stage15C: 捕获主循环异常并转成 run_failed 事件；若没有这行代码，事件消费者会看到生成器直接崩溃。
            ...
            while max_turns is None or turn < max_turns:  # 新增代码+Stage15C: 保留可配置轮次循环；若没有这行代码，agent 无法继续多轮工具调用。
                ...
                tools = self._available_tool_schemas()  # 新增代码+Stage15C: 每轮重新计算工具池；若没有这行代码，deferred 工具 select 后不会生效。
                ...
                yield emit("model_message_completed", {"turn": turn, "message": model_payload})  # 新增代码+Stage15C: 发出模型完成事件；若没有这行代码，transcript 无法记录模型结果。
                ...
                for tool_call, tool_output in zip(model_message.tool_calls, tool_outputs):  # 修改代码+Stage15F: 按原 tool_call 顺序发出完成事件和回填消息；若没有这行代码，并发结果可能乱序。
                    ...
                    yield emit("tool_call_completed", {"turn": turn, **result_payload})  # 新增代码+Stage15C: 发出工具完成事件；若没有这行代码，transcript 无法恢复工具结果。
                    messages.append(self._tool_result_to_dict(tool_call, context_tool_output))  # 新增代码+Stage15C: 把工具结果放回消息历史；若没有这行代码，下一轮模型不知道工具结果。
    ...
    def _mcp_tool_risk_summary(self, tool_call: ToolCall) -> tuple[str, str]:  # 新增代码+权限分级: 根据 MCP 工具名生成授权风险摘要；若省略: 用户只能看到模糊外部调用而无法区分搜索、写入、删除或命令风险
        ...
        if "browser_automation" in tool_name and (tool_name.endswith("__browser_open") or tool_name.endswith("__browser_wait") or tool_name.endswith("__browser_screenshot")):  # 新增代码+BrowserAutomation风险: 识别打开网页、等待和截图这类中等风险浏览器工具；若省略: 打开外部网页或保存截图的副作用不够清楚
    ...
    def _real_browser_customer_auto_approve_reason(self, tool_call: ToolCall) -> str:  # 新增代码+真实浏览器客户模式: 给白名单真实浏览器工具返回自动授权原因；若没有这行代码，_execute_mcp_tool 无法区分客户模式和普通高风险权限
        return real_browser_customer_auto_approve_reason(tool_call.name, tool_call.arguments, customer_mode_active=self._real_browser_customer_mode_active())  # 修改代码+BrowserSplit: 委托 browser.permissions 返回客户模式自动授权原因；若没有这行代码，工具白名单仍会留在主类内部。
    ...
    def _task_output(self, arguments: dict[str, Any]) -> str:  # 新增代码+TaskLifecycle: 执行 task_output 工具并读取任务记录；若省略: 模型无法二次查询子任务结果
        ...
        if task_record is None:  # 新增代码+TaskLifecycle: 处理未知 task_id；若省略: 访问 None 会抛异常
            return f"task_output 失败：未知 task_id：{task_id}"  # 新增代码+TaskLifecycle: 返回清楚未知任务错误；若省略: 模型难以判断是否需要重新创建任务
    ...
    def _task_stop(self, arguments: dict[str, Any]) -> str:  # 新增代码+TaskLifecycle: 执行 task_stop 工具并更新任务状态；若省略: 模型无法表达停止子任务的请求
        ...
        if task_record is None:  # 新增代码+TaskLifecycle: 处理未知 task_id；若省略: 访问 None 会抛异常
            return f"task_stop 失败：未知 task_id：{task_id}"  # 新增代码+TaskLifecycle: 返回清楚未知任务错误；若省略: 模型难以判断是否需要重新创建任务
    ...
    def _task_list(self, arguments: dict[str, Any]) -> str:  # 新增代码+TaskManagement: 执行 task_list 工具并返回子任务总览；若省略: 模型无法查看多个子任务的统一列表
        ...
        all_records = list(self.task_runs.values())  # 新增代码+TaskManagement: 把任务记录字典转成稳定列表；若省略: 后续无法统一筛选和截断
        ...
        for index, task_record in enumerate(visible_records, start=1):  # 新增代码+TaskManagement: 逐条格式化任务摘要；若省略: 任务记录无法变成人类和模型可读文本
            ...
            lines.append(f"{index}. task_id={task_record.task_id} status={task_record.status} background={'true' if task_record.background else 'false'} label={task_record.label or '(无标签)'}")  # 新增代码+TaskManagement: 输出任务 id、状态、后台标记和标签；若省略: 模型无法快速选择目标任务
    ...
    def _task_get(self, arguments: dict[str, Any]) -> str:  # 新增代码+TaskManagement: 执行 task_get 工具并返回单个子任务详情；若省略: 模型无法读取任务管理元信息
        ...
        if task_record is None:  # 新增代码+TaskManagement: 处理未知 task_id；若省略: 访问 None 会抛异常
            return f"task_get 失败：未知 task_id：{task_id}"  # 新增代码+TaskManagement: 返回清楚未知任务错误；若省略: 模型难以判断是否需要先 task_list
        max_chars = self._parse_max_chars(arguments.get("max_chars"))  # 新增代码+TaskManagement: 解析最大返回字符数；若省略: 长输出可能撑爆上下文
        output_text = task_record.output or task_record.error or "(暂无输出)"  # 新增代码+TaskManagement: 选择任务输出、错误或空输出占位；若省略: 未完成任务会返回空白难以理解
        ...
        lines.append(f"label={task_record.label or '(无标签)'}")  # 新增代码+TaskManagement: 输出任务短标签；若省略: task_update 的 label 无法被读回
        lines.append(f"notes={task_record.notes or '(无备注)'}")  # 新增代码+TaskManagement: 输出任务备注；若省略: task_update 的 notes 无法被读回
        ...
        lines.append(f"stop_requested={'true' if task_record.stop_requested else 'false'}")  # 新增代码+TaskManagement: 输出是否收到停止请求；若省略: 取消状态难以审计
    ...
    def _task_update(self, arguments: dict[str, Any]) -> str:  # 新增代码+TaskManagement: 执行 task_update 工具并更新任务管理元信息；若省略: 模型无法为任务补标签或备注
        ...
        if task_record is None:  # 新增代码+TaskManagement: 处理未知 task_id；若省略: 访问 None 会抛异常
            return f"task_update 失败：未知 task_id：{task_id}"  # 新增代码+TaskManagement: 返回清楚未知任务错误；若省略: 模型难以判断是否需要先 task_list
    ...
    def _send_message(self, arguments: dict[str, Any]) -> str:  # 新增代码+TeamCommunication: 执行 send_message 工具并把消息放入 peer inbox；若省略: peer 之间无法建立教学版通信记录
        ...
        if peer_record is None:  # 新增代码+TeamCommunication: 处理未知 peer_id；若省略: 访问 None 会抛异常
            return f"send_message 失败：未知 peer_id：{peer_id}，请先调用 team_create 或 list_peers。"  # 新增代码+TeamCommunication: 返回可恢复建议；若省略: 模型不知道该先创建或查看 peer
        message = str(arguments.get("message", "") or "").strip()  # 新增代码+TeamCommunication: 读取并清理消息正文；若省略: inbox 可能收到空消息
    ...
    def _list_peers(self, arguments: dict[str, Any]) -> str:  # 新增代码+TeamCommunication: 执行 list_peers 工具并返回 peer 总览；若省略: 模型无法查看当前团队成员
        ...
        all_peers = list(self.team_peers.values())  # 新增代码+TeamCommunication: 把 peer 登记表转成稳定列表；若省略: 后续无法统一筛选和截断
        ...
        for index, peer in enumerate(visible_peers, start=1):  # 新增代码+TeamCommunication: 逐条格式化 peer 摘要；若省略: peer 记录无法变成人类和模型可读文本
            ...
            bound_task_record = self.task_runs.get(peer.bound_task_id) if peer.bound_task_id else None  # 新增代码+TeamTaskBinding: 从任务表读取绑定 task 当前记录；若省略: team 视图无法动态反映 task 状态
    ...
    def _read_peer_messages(self, arguments: dict[str, Any]) -> str:  # 新增代码+TeamCommunicationLifecycle: 执行 read_peer_messages 工具并读取 peer inbox；若省略: 模型无法查看 peer 收到的消息详情
        ...
        if peer_record is None:  # 新增代码+TeamCommunicationLifecycle: 处理未知 peer_id；若省略: 访问 None 会抛异常
            return f"read_peer_messages 失败：未知 peer_id：{peer_id}，请先调用 team_create 或 list_peers。"  # 新增代码+TeamCommunicationLifecycle: 返回可恢复建议；若省略: 模型不知道该先创建或查看 peer
    ...
    def _ack_peer_message(self, arguments: dict[str, Any]) -> str:  # 新增代码+TeamCommunicationLifecycle: 执行 ack_peer_message 工具并确认一条消息；若省略: peer 消息无法标记已处理
        ...
        peer_record = self.team_peers.get(peer_id)  # 新增代码+TeamCommunicationLifecycle: 从登记表查找目标 peer；若省略: 工具无法定位 inbox
        if peer_record is None:  # 新增代码+TeamCommunicationLifecycle: 处理未知 peer_id；若省略: 访问 None 会抛异常
            return f"ack_peer_message 失败：未知 peer_id：{peer_id}，请先调用 team_create 或 list_peers。"  # 新增代码+TeamCommunicationLifecycle: 返回可恢复建议；若省略: 模型不知道该先创建或查看 peer
    ...
    def _team_delete(self, arguments: dict[str, Any]) -> str:  # 新增代码+TeamCommunicationLifecycle: 执行 team_delete 工具并删除教学版 peer；若省略: peer 记录无法回收
        ...
        if peer_record is None:  # 新增代码+TeamCommunicationLifecycle: 处理未知 peer_id；若省略: 删除不存在目标会被误认为成功
            return f"team_delete 失败：未知 peer_id：{peer_id}，请先调用 list_peers。"  # 新增代码+TeamCommunicationLifecycle: 返回可恢复建议；若省略: 模型难以判断目标是否已经被删
    ...
    def _team_start_task(self, arguments: dict[str, Any]) -> str:  # 新增代码+TeamTaskBinding: 执行 team_start_task 并把 peer 绑定到后台 task；若省略: team 成员无法承接真实可查询子任务
        ...
        if peer_record is None:  # 新增代码+TeamTaskBinding: 处理未知 peer_id；若省略: 访问 None 会抛异常
            return f"team_start_task 失败：未知 peer_id：{peer_id}，请先调用 team_create 或 list_peers。"  # 新增代码+TeamTaskBinding: 返回可恢复建议；若省略: 模型不知道该先创建或查看 peer
        ...
        existing_task = self.task_runs.get(peer_record.bound_task_id) if peer_record.bound_task_id else None  # 新增代码+TeamTaskBinding: 查找该 peer 已绑定的旧 task；若省略: 可能给同一 peer 重复启动多个未完成任务
