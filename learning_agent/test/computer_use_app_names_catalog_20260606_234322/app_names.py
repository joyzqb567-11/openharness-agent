...
APP_NAME_BLOCKED_EXACT_NAMES = {"run", "computer management", "event viewer", "disk cleanup", "remote desktop connection", "steps recorder", "memory diagnostics tool", "odbc data sources (32-bit)", "odbc data sources (64-bit)", "iscsi initiator", "dfrgui", "magnify", "narrator", "on-screen keyboard", "voiceaccess", "livecaptions"}  # 修改代码+AppNames：按精确名称拦截真实开始菜单里的系统/辅助/管理入口；如果没有这一行，模型候选清单仍会混入不适合普通任务优先选择的系统工具。
APP_NAME_NOISE_TOKENS = ("helper", "agent", "service", "uninstaller", "uninstall", "updater", "update", "background", "runtime broker", "卸载", "修复", "官网")  # 修改代码+AppNames：同时过滤英文后台组件和中文卸载/修复/官网快捷方式噪声；如果没有这一行，真实开始菜单里的卸载器会继续污染模型候选。
APP_NAME_HIGH_RISK_TOKENS = ("powershell", "pwsh", "cmd", "command prompt", "terminal", "wt", "regedit", "registry", "control", "settings", "mmc", "taskmgr", "administrator", "administrative tools", "credential", "password", "security", "firewall", "defender", "services", "wsl", "system tools", "windows tools")  # 修改代码+AppNames：补充真实日志暴露出的命令提示符、管理工具和 WSL 等系统入口；如果没有这一行，模型仍可能把终端类工具当成普通桌面应用。
