...
def test_batch_executor_blocks_when_low_level_event_limit_is_exceeded() -> None:  # 新增代码+BatchEventLimitTest：函数段开始，验证批执行超过事件预算时停止；如果没有这个测试，异常批会继续发真实鼠标键盘事件。
    runtime = FakeActionRuntime(FakeTargetRuntime())  # 新增代码+BatchEventLimitTest：创建稳定 fake runtime；如果没有这行代码，执行器没有动作依赖。
    actions = ({"type": "focus_window"}, {"type": "type_text"}, {"type": "press_key", "key": "ENTER"})  # 新增代码+BatchEventLimitTest：准备三步动作；如果没有这行代码，无法证明超限后剩余动作被停止。
    batch = ActionBatch(batch_id="batch-limit", batch_kind="text_entry_batch", target_ref="target-a", actions=actions, guardrails={"stage_id": "stage-limit", "max_low_level_events": 3})  # 新增代码+BatchEventLimitTest：构造低事件预算批；如果没有这行代码，执行器不会触发超限拒绝。
    result = UniversalActionBatchExecutor(runtime).execute_batch({"target_ref": "target-a", "target_window": {"hwnd": 6}}, batch)  # 新增代码+BatchEventLimitTest：执行低预算批；如果没有这行代码，无法读取执行结果。
    assert result.status == "blocked"  # 新增代码+BatchEventLimitTest：确认超限返回阻断；如果没有这行代码，异常事件可能被误报完成。
    assert result.evidence["decision"] == "batch_event_limit_exceeded"  # 新增代码+BatchEventLimitTest：确认原因码明确；如果没有这行代码，日志无法解释为何停止。
    assert len(runtime.dispatched) == 2  # 新增代码+BatchEventLimitTest：确认第三步未执行；如果没有这行代码，事件预算可能没有真正截停。
