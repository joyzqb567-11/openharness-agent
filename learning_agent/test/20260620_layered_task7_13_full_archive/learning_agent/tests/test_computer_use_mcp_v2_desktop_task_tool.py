...
class FakeRealDesktopTaskLoop:  # 新增代码+DesktopTaskExplicitToolTest: 类段开始，模拟已接 Stage Runtime 的真实执行闭环；如果没有这个 fake，单测会启动真实桌面软件。
    ...
    def run_desktop_task(self, target_app: str, prompt: str) -> dict[str, object]:  # 新增代码+DesktopTaskExplicitToolTest: 函数段开始，返回完整成功阶段证据；如果没有这段函数，runtime 无法模拟真实 Stage Loop。
        ...
        return {  # 新增代码+DesktopTaskExplicitToolTest: 返回真实路径需要的低敏证据；如果没有这一行，_real_execution_report 会判为失败。
            ...
            "stage_count": 5,  # 新增代码+LayeredRuntimeEvidenceTest：让 fake 闭环暴露阶段总数；如果没有这一行，desktop_task 外层的 verifier_state 没有可测输入。
            "completed_stage_count": 5,  # 新增代码+LayeredRuntimeEvidenceTest：让 fake 闭环暴露已完成阶段数；如果没有这一行，测试无法证明任务完整完成状态会透传。
            "stage_results": [{"stage_id": "verify_result", "status": "completed", "evidence": {"decision": "all_success_criteria_met"}}],  # 新增代码+LayeredRuntimeEvidenceTest：让 fake 闭环暴露验证层阶段结果；如果没有这一行，外层报告可能丢失每阶段判断而测试发现不了。
            "desktop_task_plan": {"layer_skill_metadata": {"intent_understanding": {"layer_name": "intent_understanding"}, "stage_planning": {"layer_name": "stage_planning"}}},  # 新增代码+LayeredRuntimeEvidenceTest：让 fake 闭环暴露分层 skill 元数据；如果没有这一行，测试无法证明独立 skill/prompt 接线可见。
            "reflection_learning_layer_used": True,  # 新增代码+LayeredRuntimeEvidenceTest：让 fake 闭环暴露反思学习层参与；如果没有这一行，外层报告可能永远看不到失败学习闭环。
...
def test_desktop_task_runtime_force_entry_bypasses_legacy_not_desktop_classifier() -> None:  # 新增代码+DesktopTaskExplicitToolTest: 函数段开始，验证显式 desktop_task 入口不会再被旧分类器误判挡住；如果没有这段测试，乱码中文 prompt 会回归 not_desktop_task。
    ...
    assert report["layered_runtime_used"] is True  # 新增代码+LayeredRuntimeEvidenceTest：断言外层报告确认进入通用分层闭环；如果没有这一行，回归到旧黑箱路线也不会失败。
    assert report["layer_skill_metadata"]["intent_understanding"]["layer_name"] == "intent_understanding"  # 新增代码+LayeredRuntimeEvidenceTest：断言意图理解层元数据被透传；如果没有这一行，独立 skill 接线丢失不会被测试发现。
    assert report["verifier_state"]["stage_count"] == 5  # 新增代码+LayeredRuntimeEvidenceTest：断言验证状态包含总阶段数；如果没有这一行，部分执行被误判完成的风险无法暴露。
    assert report["verifier_state"]["completed_stage_count"] == 5  # 新增代码+LayeredRuntimeEvidenceTest：断言验证状态包含已完成阶段数；如果没有这一行，报告无法解释是否真正完成全部阶段。
    assert report["verifier_state"]["stage_results"][0]["stage_id"] == "verify_result"  # 新增代码+LayeredRuntimeEvidenceTest：断言每阶段验证结果进入外层报告；如果没有这一行，真实失败阶段会被吞掉。
    assert report["reflection_learning_layer_used"] is True  # 新增代码+LayeredRuntimeEvidenceTest：断言反思学习层参与状态被透传；如果没有这一行，重复失败学习闭环不可见。
