...
class ComputerUseDesktopTaskRuntime:  # 新增代码+DesktopTaskRuntime：类段开始，编排自然语言桌面任务到 Computer Use GUI 证据链；如果没有这个类，/computer use --full 只有权限状态而没有任务运行时。
    ...
    def _real_execution_report(self, report: dict[str, Any], prompt: str, target_app: str) -> dict[str, Any]:  # 新增代码+源码复核门禁：函数段开始，调用注入的真实执行闭环并汇总报告；如果没有这段函数，real_actions=True 无法从硬拒绝推进到受控真实路径。
        ...
        plan_payload = loop_report.get("desktop_task_plan", {}) if isinstance(loop_report.get("desktop_task_plan", {}), dict) else {}  # 新增代码+LayeredRuntimeEvidence：安全取出分层计划报告；如果没有这一行，desktop_task 外层无法知道意图层和规划层是否真的参与。
        stage_results_payload = loop_report.get("stage_results", [])  # 新增代码+LayeredRuntimeEvidence：先取出阶段验证结果原始值；如果没有这一行，后面无法把验证层状态透传给终端和验收器。
        stage_results = stage_results_payload if isinstance(stage_results_payload, list) else []  # 新增代码+LayeredRuntimeEvidence：把坏格式阶段结果降级成空列表；如果没有这一行，异常返回值会污染 desktop_task 报告。
        layer_skill_metadata = plan_payload.get("layer_skill_metadata", {})  # 新增代码+LayeredRuntimeEvidence：读取计划中的分层 skill 元数据；如果没有这一行，报告无法证明独立 prompt/skill 已加载。
        report["layered_runtime_used"] = bool(loop_report.get("universal_stage_task_loop_used", False))  # 新增代码+LayeredRuntimeEvidence：标记是否进入通用分阶段 Computer Use 闭环；如果没有这一行，用户会分不清旧路线和新路线。
        report["layer_skill_metadata"] = dict(layer_skill_metadata) if isinstance(layer_skill_metadata, dict) else {}  # 新增代码+LayeredRuntimeEvidence：透传可序列化的分层 skill 元数据；如果没有这一行，验收时看不到理解、规划、观察、验证等独立层是否接线。
        report["verifier_state"] = {  # 新增代码+LayeredRuntimeEvidence：函数段内建立验证层汇总对象；如果没有这一段，失败时只能看到最终失败而看不到卡在哪个阶段。
            "stage_results": stage_results,  # 新增代码+LayeredRuntimeEvidence：保留每个阶段的验证结果；如果没有这一行，复杂桌面任务无法追踪具体失败阶段。
            "stage_count": _desktop_task_runtime_safe_int(loop_report.get("stage_count", 0)),  # 新增代码+LayeredRuntimeEvidence：记录计划阶段总数；如果没有这一行，无法判断任务是否只执行了一部分。
            "completed_stage_count": _desktop_task_runtime_safe_int(loop_report.get("completed_stage_count", 0)),  # 新增代码+LayeredRuntimeEvidence：记录已完成阶段数；如果没有这一行，final gate 难以解释为什么不能宣称完成。
            "desktop_task_completed": bool(loop_report.get("desktop_task_completed", False)),  # 新增代码+LayeredRuntimeEvidence：记录整项桌面任务是否完成；如果没有这一行，外层只能猜测 loop 的结束状态。
            "desktop_task_incomplete": bool(loop_report.get("desktop_task_incomplete", False)),  # 新增代码+LayeredRuntimeEvidence：记录是否被明确判定未完成；如果没有这一行，未完成任务容易被包装成普通失败。
        }  # 新增代码+LayeredRuntimeEvidence：验证层汇总对象到此结束；如果没有这一行，Python 字典结构不完整。
        report["reflection_learning_layer_used"] = bool(loop_report.get("reflection_learning_layer_used", False))  # 新增代码+LayeredRuntimeEvidence：标记反思学习层是否参与失败分析；如果没有这一行，重复问题无法在验收报告里看到学习闭环。
