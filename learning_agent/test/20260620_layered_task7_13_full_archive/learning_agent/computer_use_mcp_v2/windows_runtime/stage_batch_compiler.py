...
def _stage_compiler_write_guardrails(stage: StagePlan, capability: str, max_low_level_events: int) -> dict[str, Any]:  # 新增代码+BatchCompilerEventLimit：函数段开始，生成写批通用安全约束；如果没有这段函数，各写批的事件上限和阶段证据会散落。
    return {"stage_id": stage.stage_id, "stage_kind": stage.stage_kind, "capability": capability, "max_low_level_events": max_low_level_events}  # 新增代码+BatchCompilerEventLimit：返回阶段、能力和事件预算；如果没有这行代码，执行器无法按批拒绝异常事件规模。
# 新增代码+BatchCompilerEventLimit：函数段结束，_stage_compiler_write_guardrails 到此结束；如果没有这个边界说明，用户不容易看出写批安全约束来源。
...
def compile_stage_to_batch(stage: StagePlan, observation_frame: Mapping[str, Any], capability_profile: AppCapabilityProfile) -> ActionBatch:  # 新增代码+StageBatchCompiler：函数段开始，把通用阶段编译成一个动作批；如果没有这段函数，阶段内无法批量执行。
    ...
    if stage.stage_kind == "commit_resource":  # 新增代码+StageBatchCompiler：提交资源阶段走通用保存批；如果没有这行代码，保存任务可能早停。
        return ActionBatch(batch_id=f"{stage.stage_id}_save", batch_kind="file_save_batch", target_ref=stage.target_ref, actions=_stage_compiler_save_actions(), guardrails={**_stage_compiler_write_guardrails(stage, "file_save_surface", 40), "no_direct_file_write": True, "has_file_save_surface": bool(capability_profile.has_file_save_surface), "supports_keyboard_shortcuts_likely": bool(capability_profile.supports_keyboard_shortcuts_likely)})  # 修改代码+BatchCompilerEventLimit：返回带事件预算和保存能力证据的应用内保存批；如果没有这行代码，保存阶段可能绕过 GUI 或无限事件执行。
    ...
    if capability_profile.has_text_input:  # 新增代码+StageBatchCompiler：文本能力优先编译文本批；如果没有这行代码，文本输入任务无法批量完成。
        return ActionBatch(batch_id=f"{stage.stage_id}_text", batch_kind="text_entry_batch", target_ref=stage.target_ref, actions=_stage_compiler_text_actions(stage), guardrails=_stage_compiler_write_guardrails(stage, "text_input", 80))  # 修改代码+BatchCompilerEventLimit：返回带事件预算的文本输入批；如果没有这行代码，文本批无法被执行层按规模约束。
    if capability_profile.has_canvas_like_region:  # 新增代码+StageBatchCompiler：画布能力编译指针路径批；如果没有这行代码，图形任务无法批量执行。
        return ActionBatch(batch_id=f"{stage.stage_id}_pointer", batch_kind="pointer_path_batch", target_ref=stage.target_ref, actions=_stage_compiler_drawing_actions(observation_frame), guardrails=_stage_compiler_write_guardrails(stage, "canvas_like_region", 200))  # 修改代码+BatchCompilerEventLimit：返回带事件预算的多路径批；如果没有这行代码，复杂图形拖拽异常时无法截停。
    if capability_profile.has_browser_navigation_surface:  # 新增代码+StageBatchCompiler：导航能力编译导航批；如果没有这行代码，导航任务无法泛化。
        return ActionBatch(batch_id=f"{stage.stage_id}_navigation", batch_kind="navigation_batch", target_ref=stage.target_ref, actions=_stage_compiler_navigation_actions(stage), guardrails=_stage_compiler_write_guardrails(stage, "navigation_surface", 80))  # 修改代码+BatchCompilerEventLimit：返回带事件预算的导航批；如果没有这行代码，导航输入异常时无法被执行层截停。
