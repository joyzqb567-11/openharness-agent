...
from .observation_fact_layer import build_observation_facts  # 新增代码+LayeredObservationLoop：导入结构化观察事实层；如果没有这行代码，stage loop 会继续直接把原始观察帧交给 planner/verifier。
from .reflection_learning_layer import ReflectionLearningLayer  # 新增代码+ReflectionLearningLoop：导入反思学习层；如果没有这行代码，needs_repair 后只能盲目重试同一动作。
...
class UniversalStageTaskLoop:  # 新增代码+StageTaskLoop：类段开始，通用阶段级桌面任务循环；如果没有这个类，复杂 Computer Use 会继续由 primitive 循环驱动。
    def __init__(self, planner: Any | None = None, observation_runtime: Any | None = None, target_runtime: Any | None = None, capability_profile_builder: Callable[[Mapping[str, Any]], AppCapabilityProfile] | None = None, batch_compiler: Callable[[StagePlan, Mapping[str, Any], AppCapabilityProfile], Any] | None = None, batch_executor: Any | None = None, stage_verifier: Any | None = None, max_stage_repairs: int = 1, action_runtime: Any | None = None, reflection_layer: Any | None = None) -> None:  # 修改代码+ReflectionLearningLoop：函数段开始，注入所有阶段运行依赖和反思层；如果没有这段函数，测试和生产无法替换观察/执行/学习层。
        ...
        self.reflection_layer = reflection_layer if reflection_layer is not None else ReflectionLearningLayer()  # 新增代码+ReflectionLearningLoop：保存反思学习层；如果没有这行代码，needs_repair 缺少失败分类和修复策略。
    ...
    def _facts_for_observation(self, frame: Mapping[str, Any]) -> dict[str, Any]:  # 新增代码+LayeredObservationLoop：函数段开始，把原始观察转换成结构化 facts 字典；如果没有这段函数，后续层无法稳定消费观察结果。
        if not frame:  # 新增代码+LayeredObservationLoop：空观察直接返回空 facts；如果没有这行代码，未观察阶段会生成误导性 unknown facts。
            return {}  # 新增代码+LayeredObservationLoop：返回空字典；如果没有这行代码，调用方会收到无意义 facts。
        return build_observation_facts(frame).to_dict()  # 新增代码+LayeredObservationLoop：返回 JSON 安全观察事实；如果没有这行代码，planner/verifier 仍要理解原始截图/UIA。
    # 新增代码+LayeredObservationLoop：函数段结束，UniversalStageTaskLoop._facts_for_observation 到此结束；如果没有这个边界说明，用户不容易看出观察事实转换范围。
    ...
    def run_desktop_task(self, prompt: str, target_hint: str = "") -> dict[str, Any]:  # 新增代码+StageTaskLoop：函数段开始，运行完整通用桌面任务；如果没有这段函数，adapter 无法调用阶段运行时。
        ...
        for stage in plan.stages:  # 新增代码+StageTaskLoop：按顺序执行阶段；如果没有这行代码，任务不会推进。
            ...
            while True:  # 新增代码+StageTaskLoop：允许阶段有限修复重跑；如果没有这行代码，needs_repair 无法局部重试。
                ...
                before_facts = self._facts_for_observation(before_frame)  # 新增代码+LayeredObservationLoop：把前置观察转成 facts；如果没有这行代码，批编译器会继续读原始观察。
                compiler_input = before_facts or before_frame  # 新增代码+LayeredObservationLoop：优先使用 facts，空观察时保留旧输入；如果没有这行代码，旧测试 fake 观察可能失效。
                profile = self._profile_for_stage(plan, compiler_input)  # 修改代码+LayeredObservationLoop：从 facts 构建能力画像；如果没有这行代码，ObservationFacts 到 ActionBatch 链路不会生效。
                batch = self.batch_compiler(stage, compiler_input, profile)  # 修改代码+LayeredObservationLoop：把阶段和 facts 编译为动作批；如果没有这行代码，编译仍依赖原始帧。
                ...
                after_facts = self._facts_for_observation(after_frame)  # 新增代码+LayeredObservationLoop：把后置观察转成 facts；如果没有这行代码，verifier 会继续读原始观察。
                verified = self.stage_verifier.verify_stage(plan, stage, before_facts or before_frame, after_facts or after_frame, execution_result)  # 修改代码+LayeredObservationLoop：用结构化 facts 验证阶段；如果没有这行代码，观察层准确率提升无法进入验证。
                if verified.status == "needs_repair":  # 新增代码+ReflectionLearningLoop：只有阶段未完成时才触发反思；如果没有这行代码，成功阶段会产生无意义学习记录。
                    reflection = self.reflection_layer.reflect_stage_failure(stage.stage_id, verified, after_facts or after_frame, execution_result)  # 新增代码+ReflectionLearningLoop：生成失败分类和修复策略；如果没有这行代码，重试仍会重复同样错误。
                    verified = StageResult(status=verified.status, stage_id=verified.stage_id, evidence={**dict(verified.evidence), "reflection": reflection.to_dict()}, message=verified.message)  # 新增代码+ReflectionLearningLoop：把反思结果附到阶段证据；如果没有这行代码，acceptance 看不到失败原因和修复建议。
