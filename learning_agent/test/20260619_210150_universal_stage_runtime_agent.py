...
try:  # 修改代码+CoreSplit: 优先从新 core 包导入配置与消息结构；若没有这行代码，learning_agent.py 会继续承载过多基础定义。
    from learning_agent.core.actionability_recovery import ActionabilityRecoveryPlan, build_desktop_observe_recovery_plan, record_recovery_attempt  # 新增代码+AutoObserveRecovery：导入桌面 observe 自动恢复计划；如果没有这一行，主循环只能继续依赖模型自己重试 observe。
    from learning_agent.core.actionability_state import get_pending_actionability  # 新增代码+AutoObserveRecovery：导入 pending 读取函数；如果没有这一行，主循环无法知道工具结果是否刚创建 observe-required 状态。
    ...
    if error.name not in {"learning_agent", "learning_agent.core", "learning_agent.core.actionability_recovery", "learning_agent.core.actionability_state", "learning_agent.core.config", "learning_agent.core.events", "learning_agent.core.message_builders", "learning_agent.core.messages", "learning_agent.core.plan_runtime", "learning_agent.core.run_helpers", "learning_agent.core.session"}:  # 修改代码+AutoObserveRecovery：允许脚本模式 fallback 覆盖新增 actionability 恢复模块；若没有这一行，bat 入口路径差异会被误判成真实错误。
        raise  # 修改代码+CoreSplit: 重新抛出非路径问题；若没有这行代码，真正的导入错误会被伪装成脚本模式问题。
    from core.actionability_recovery import ActionabilityRecoveryPlan, build_desktop_observe_recovery_plan, record_recovery_attempt  # 新增代码+AutoObserveRecovery：脚本模式下导入自动恢复计划；如果没有这一行，start_oauth_agent.bat 无法执行自动 observe 恢复。
    from core.actionability_state import get_pending_actionability  # 新增代码+AutoObserveRecovery：脚本模式下导入 pending 读取函数；如果没有这一行，bat 入口无法判断 observe-required pending。
...
try:  # 新增代码+OAuthNativeCompatPermissionExport: 包运行模式下从 app 层导入终端权限函数并在 core.agent 旧入口重新导出；如果没有这段代码，旧测试和旧调用方从 learning_agent.core.agent 导入权限函数会继续失败。
    from learning_agent.app.terminal_permissions import ask_permission_from_terminal, ask_permission_from_terminal_customer_mode, build_permission_event_payload  # 新增代码+OAuthNativeCompatPermissionExport: 复用真实 app 层权限实现；如果没有这一行，兼容导出会复制逻辑并让权限规则分叉。
except ModuleNotFoundError as error:  # 新增代码+OAuthNativeCompatPermissionExport: 兼容 start_oauth_agent.bat 或脚本模式下包名前缀不可用；如果没有这一行，直接脚本入口可能因为导入路径不同而失败。
    if error.name not in {"learning_agent", "learning_agent.app", "learning_agent.app.terminal_permissions"}:  # 新增代码+OAuthNativeCompatPermissionExport: 只允许目标包路径缺失时 fallback；如果没有这一行，terminal_permissions 内部真实 bug 会被误吞。
        raise  # 新增代码+OAuthNativeCompatPermissionExport: 非路径问题继续抛出；如果没有这一行，排查权限模块内部错误会变困难。
    from app.terminal_permissions import ask_permission_from_terminal, ask_permission_from_terminal_customer_mode, build_permission_event_payload  # 新增代码+OAuthNativeCompatPermissionExport: 脚本模式下从同级 app 层导入真实权限实现；如果没有这一行，core.agent 旧入口在 bat 场景仍不能导出权限函数。
...
class LearningAgent:  # 作用: 教学版 agent 主类
    def __init__(  # 修改代码+DesktopTaskPolicy：函数段开始，初始化参数保持旧调用方式并补充桌面任务策略上下文；如果没有这个函数段，agent 无法保存模型、workspace、权限和 Task 3 的 active 状态，作者意图是让 `_bash_atom` 能知道当前是否处于桌面任务，本段与 `_bash_atom` 配合到初始化字段结束。
        ...
        self.computer_use_turn_cleanup_runtime = None  # 修改代码+OwnedLaunchCleanup：先预留 Computer Use cleanup runtime 缓存；如果没有这一行，controller 初始化时无法创建和 stop 共用的运行时对象。
        computer_use_cleanup_runtime = self._computer_use_cleanup_runtime()  # 新增代码+OwnedLaunchCleanup：创建 launch_app 与 /computer stop 共用的 session runtime；如果没有这一行，启动登记和回合清理会继续分裂。
        computer_use_lock_manager = getattr(computer_use_cleanup_runtime, "lock_manager", None)  # 新增代码+OwnedLaunchCleanup：复用 cleanup runtime 的同一个桌面锁管理器；如果没有这一行，request_access、launch_app 和 stop 可能各用一套锁状态。
        computer_use_session_id = str(getattr(computer_use_cleanup_runtime, "session_id", "learning-agent-default-session") or "learning-agent-default-session")  # 新增代码+OwnedLaunchCleanup：复用 cleanup runtime 的会话 id；如果没有这一行，资源登记和 stop 可能按不同 owner 匹配。
        self.computer_use_controller = ComputerUseController(lock_manager=computer_use_lock_manager, owner_session_id=computer_use_session_id, auto_acquire_lock=True, session_runtime=computer_use_cleanup_runtime)  # 修改代码+OwnedLaunchCleanup：把同一 lock/session/registry 传给 controller；如果没有这一行，launch_app 登记的自有进程不会被 stop 稳定清理。
    ...
    def _desktop_task_policy_context_from_prompt(self, user_input: str) -> dict[str, Any]:  # 新增代码+DesktopTaskPolicy：函数段开始，把用户自然语言 prompt 转成脱敏桌面任务策略上下文；如果没有这段函数，run_events 无法在真实模型工具循环前自动设置 active，作者意图是复用 Task 2 分类器而不保存原始 prompt，本函数与 run_events 和 _bash_atom 配合到 return 结束。
        ...
        context = {  # 修改代码+ComputerUseMcpV2SchemaNarrowingFix：先构造本轮脱敏上下文字典，后面再按已打开的 v2 会话补充字段；如果没有这一行，无法安全合并 prompt 分类结果和 terminal session 状态。
            ...
            "controlled_resource_name": str(getattr(intent, "controlled_resource_name", "") or ""),  # 新增代码+ControlledResourceContext：保存分类器提取的脱敏受控文件名；如果没有这一项，v2 MCP adapter 看不到 1.txt 这类保存目标。
            "controlled_resource_location_hint": str(getattr(intent, "controlled_resource_location_hint", "") or ""),  # 新增代码+ControlledResourceContext：保存分类器提取的脱敏资源位置；如果没有这一项，启动层不知道文件名是否对应桌面。
    ...
    def _build_synthetic_recovery_tool_call(self, recovery_plan: ActionabilityRecoveryPlan) -> ToolCall:  # 新增代码+AutoObserveRecovery：函数段开始，把恢复计划转成协议内 ToolCall；如果没有这段函数，主循环会用散落字典构造自动 observe。
        return ToolCall(name=recovery_plan.tool_name, arguments=dict(recovery_plan.arguments), call_id=f"call_auto_observe_{secrets.token_hex(8)}")  # 新增代码+AutoObserveRecovery：创建带稳定前缀的合成工具调用；如果没有这一行，自动 observe 的 tool result 无法和 assistant tool_call 配对。
        # 新增代码+AutoObserveRecovery：函数段结束，_build_synthetic_recovery_tool_call 到此结束；如果没有这个边界说明，用户不容易看出合成工具调用范围。

    def _append_synthetic_recovery_tool_call_message(self, messages: list[dict[str, Any]], recovery_tool_call: ToolCall) -> None:  # 新增代码+AutoObserveRecovery：函数段开始，把合成 tool_call 放进模型消息历史；如果没有这段函数，后续 tool result 会缺少前置 assistant tool_call。
        synthetic_message = ModelMessage(text="", tool_calls=[recovery_tool_call])  # 新增代码+AutoObserveRecovery：构造只有工具调用的 assistant 消息；如果没有这一行，消息构造器没有输入对象。
        messages.append(message_builders_from_core.assistant_message_to_dict(synthetic_message))  # 新增代码+AutoObserveRecovery：按 OpenAI 工具协议追加 assistant tool_call；如果没有这一行，自动 observe 的工具结果会破坏协议配对。
        # 新增代码+AutoObserveRecovery：函数段结束，_append_synthetic_recovery_tool_call_message 到此结束；如果没有这个边界说明，用户不容易看出协议配对范围。
    ...
    def run_events(self, user_input: str, max_turns: int | None = None, conversation_history: Sequence[Mapping[str, Any]] | None = None):  # 修改代码+交互上下文: 事件流主循环接收可选历史消息；若没有这行代码，run() 透传的终端历史无法进入模型消息。
        ...
        from learning_agent.core.convergence_controller import assess_before_model, assess_final_answer_for_desktop_task, decide_tool_call, record_tool_result  # 修改代码+DesktopStageFinalGate：引入最终回答前桌面阶段门禁；若没有这行代码，Computer Use 未完成阶段仍可能被模型当 final。
        ...
        try:  # 新增代码+Stage15C: 捕获主循环异常并转成 run_failed 事件；若没有这行代码，事件消费者会看到生成器直接崩溃。
            ...
            while max_turns is None or turn < max_turns:  # 新增代码+Stage15C: 保留可配置轮次循环；若没有这行代码，agent 无法继续多轮工具调用。
                ...
                if not model_message.tool_calls:  # 新增代码+Stage15C: 无工具调用时进入最终回答路径；若没有这行代码，直接回答会被误当成工具循环。
                    ...
                    desktop_final_gate = assess_final_answer_for_desktop_task(final_answer_text, compact_runtime_state)  # 新增代码+DesktopStageFinalGate：最终输出前检查 Computer Use 阶段完成证据；如果没有这行代码，未完成桌面任务会被当作最终回答。
                    can_retry_desktop_final = bool(not desktop_final_gate.allow_final_answer and final_answer_retry_count < 1 and (max_turns is None or turn + 1 < max_turns))  # 新增代码+DesktopStageFinalGate：判断是否还有一轮修正预算；如果没有这行代码，阻断后可能无限重试或完全不重试。
                    if can_retry_desktop_final:  # 新增代码+DesktopStageFinalGate：阶段门禁阻断且还有预算时进入补救轮；如果没有这行代码，模型无法继续完成剩余桌面阶段。
                        final_answer_retry_count += 1  # 新增代码+DesktopStageFinalGate：消耗一次最终回答补救机会；如果没有这行代码，桌面门禁可能反复重试。
                        retry_message = desktop_final_gate.injected_message or {"role": "system", "content": "Computer Use 阶段任务尚未完成，请继续执行桌面工具或请求用户动作。"}  # 新增代码+DesktopStageFinalGate：读取门禁提示并提供兜底；如果没有这行代码，下一轮模型不知道为什么不能 final。
                        debug_event_writer(run_id=run_id, event="desktop_stage_final_gate_retry", payload=desktop_final_gate.to_dict(), turn=turn)  # 新增代码+DesktopStageFinalGate：记录桌面 final gate 重试事件；如果没有这行代码，排查早停问题缺日志。
                        yield emit("desktop_stage_final_gate_retry", {"turn": turn, "decision": desktop_final_gate.to_dict()})  # 新增代码+DesktopStageFinalGate：把门禁重试发到事件流；如果没有这行代码，acceptance controller 无法观察阻断原因。
                        messages.append(retry_message)  # 新增代码+DesktopStageFinalGate：把门禁提示注入模型上下文；如果没有这行代码，下一轮模型不会收到继续执行要求。
                        turn += 1  # 新增代码+DesktopStageFinalGate：补救轮计入总轮次；如果没有这行代码，max_turns 预算会失效。
                        continue  # 新增代码+DesktopStageFinalGate：进入下一轮模型请求；如果没有这行代码，仍会落入保存最终回答。
                ...
                real_tool_call_ids = {real_tool_call.call_id for real_tool_call in real_tool_calls}  # 新增代码+AutoObserveRecovery：记录真实执行过的工具 call_id；如果没有这一行，被收敛器阻断的合成输出也可能误触发自动 observe。
                for tool_call in model_message.tool_calls:  # 修改代码+ContextCompactRepairRuntime: 按原 tool_call 顺序发出完成事件和回填消息；若没有这行代码，并发和合成结果可能乱序。
                    ...
                    pending_actionability = get_pending_actionability(compact_runtime_state)  # 新增代码+AutoObserveRecovery：读取刚由工具结果写入的 pending；如果没有这一行，主循环不知道是否需要自动补 observe。
                    recovery_plan = build_desktop_observe_recovery_plan(pending_actionability, compact_runtime_state)  # 新增代码+AutoObserveRecovery：把 pending 转成可执行恢复计划；如果没有这一行，observe-before-action 仍要靠模型下一轮自觉执行。
                    if tool_call.call_id in real_tool_call_ids and is_computer_use_tool_name(tool_call.name) and recovery_plan.should_recover:  # 新增代码+AutoObserveRecovery：只对真实 Computer Use 工具结果执行自动恢复；如果没有这一行，阻断输出或非桌面工具可能误触发 observe。
                        record_recovery_attempt(compact_runtime_state, recovery_plan)  # 新增代码+AutoObserveRecovery：先记录恢复尝试以阻止无限循环；如果没有这一行，observe 失败后可能重复自动补 observe。
                        recovery_tool_call = self._build_synthetic_recovery_tool_call(recovery_plan)  # 新增代码+AutoObserveRecovery：构造合成 observe 工具调用；如果没有这一行，主循环无法执行恢复计划。
                        self._append_synthetic_recovery_tool_call_message(messages, recovery_tool_call)  # 新增代码+AutoObserveRecovery：先写入 assistant tool_call 以保持协议配对；如果没有这一行，后续 tool result 会成为孤立消息。
                        recovery_tool_payload = debug_formatting_from_observability.tool_call_to_debug_dict(recovery_tool_call)  # 新增代码+AutoObserveRecovery：构造恢复工具调试载荷；如果没有这一行，日志和事件看不到自动 observe 参数。
                        session_tool_calls.append(copy.deepcopy(recovery_tool_payload))  # 新增代码+AutoObserveRecovery：把自动 observe 也写入会话摘要；如果没有这一行，resume 时会丢失恢复工具历史。
                        computer_use_used_this_run = True  # 新增代码+AutoObserveRecovery：标记本轮使用过 Computer Use；如果没有这一行，只有自动 observe 的场景可能漏 cleanup。
                        computer_use_trace_recorder("tool_call", {"turn": turn, "turn_id": current_turn_id, "tool_name": recovery_tool_call.name, "call_id": recovery_tool_call.call_id, "action": recovery_plan.arguments.get("action", ""), "operation": "", "query": "", "auto_recovery": True, "recovery_reason": recovery_plan.reason, "target_ref": recovery_plan.arguments.get("target_ref", ""), "recovery_attempt": 1})  # 新增代码+AutoObserveRecovery：记录自动 observe 工具调用 trace；如果没有这一行，acceptance 无法证明恢复动作来自运行时。
                        debug_event_writer(run_id=run_id, event="computer_use_auto_recovery_observe_started", payload={"tool_call": recovery_tool_payload, "recovery_plan": recovery_plan.__dict__}, turn=turn)  # 新增代码+AutoObserveRecovery：写入自动恢复开始 debug 日志；如果没有这一行，失败排查只能猜测恢复是否启动。
                        yield emit("computer_use_auto_recovery_observe", {"turn": turn, "turn_id": current_turn_id, "phase": "started", "tool_call": recovery_tool_payload, "recovery_reason": recovery_plan.reason, "target_ref": recovery_plan.arguments.get("target_ref", "")})  # 新增代码+AutoObserveRecovery：广播自动恢复开始事件；如果没有这一行，acceptance controller 看不到恢复阶段。
                        yield emit("tool_use_seen", {"turn": turn, "turn_id": current_turn_id, "tool_call": recovery_tool_payload})  # 新增代码+AutoObserveRecovery：复用标准工具调用可见事件；如果没有这一行，状态生态无法统一观察自动 observe。
                        yield emit("tool_call_started", {"turn": turn, "tool_call": recovery_tool_payload})  # 新增代码+AutoObserveRecovery：复用标准工具开始事件；如果没有这一行，终端和 transcript 会漏掉自动 observe 执行。
                        recovery_outputs = execute_tool_calls_from_orchestrator(self, [recovery_tool_call])  # 新增代码+AutoObserveRecovery：通过同一个工具编排器执行 observe；如果没有这一行，自动恢复会绕开现有工具生命周期。
                        recovery_tool_output = recovery_outputs[0] if recovery_outputs else ""  # 新增代码+AutoObserveRecovery：读取 observe 输出文本；如果没有这一行，空结果列表会导致索引错误。
                        recovery_context_output = self._offload_tool_output_if_needed(recovery_tool_call, recovery_tool_output)  # 新增代码+AutoObserveRecovery：按普通工具规则处理长 observe 输出；如果没有这一行，截图和 UIA 结果可能撑爆上下文。
                        record_tool_result(recovery_tool_call, recovery_context_output, task_state, compact_runtime_state)  # 新增代码+AutoObserveRecovery：把自动 observe 结果写入 actionability 和任务状态；如果没有这一行，pending 不会在 observe 成功后清理。
                        task_state.save_json(task_state_path)  # 新增代码+AutoObserveRecovery：保存自动 observe 后的任务状态；如果没有这一行，中断恢复会丢失恢复证据。
                        recovery_result_payload = {"tool_name": recovery_tool_call.name, "call_id": recovery_tool_call.call_id, "output": recovery_context_output, "raw_output_chars": len(recovery_tool_output), "auto_recovery": True}  # 新增代码+AutoObserveRecovery：构造自动 observe 工具结果载荷；如果没有这一行，事件和 debug 字段会不一致。
                        recovery_output_lower = recovery_tool_output.lower()  # 新增代码+AutoObserveRecovery：生成小写文本用于截图证据识别；如果没有这一行，trace 匹配会受大小写影响。
                        computer_use_trace_recorder("tool_result", {"turn": turn, "turn_id": current_turn_id, "tool_name": recovery_tool_call.name, "call_id": recovery_tool_call.call_id, "raw_output_chars": len(recovery_tool_output), "context_output_chars": len(recovery_context_output), "screenshot_returned_to_model": "image_result_count" in recovery_output_lower or "screenshot" in recovery_output_lower or "artifact_path" in recovery_output_lower, "auto_recovery": True, "recovery_reason": recovery_plan.reason, "target_ref": recovery_plan.arguments.get("target_ref", "")})  # 新增代码+AutoObserveRecovery：记录自动 observe 结果 trace；如果没有这一行，最终证据无法说明截图是否回到模型。
                        session_tool_results.append(copy.deepcopy(recovery_result_payload))  # 新增代码+AutoObserveRecovery：把自动 observe 结果写入会话摘要；如果没有这一行，resume 摘要缺少恢复结果。
                        debug_event_writer(run_id=run_id, event="tool_result", payload=recovery_result_payload, turn=turn)  # 新增代码+AutoObserveRecovery：复用标准工具结果 debug 日志；如果没有这一行，排查工具输出会漏掉自动 observe。
                        yield emit("tool_call_completed", {"turn": turn, **recovery_result_payload})  # 新增代码+AutoObserveRecovery：复用标准工具完成事件；如果没有这一行，transcript 无法恢复自动 observe 结果。
                        yield emit("tool_result_seen", {"turn": turn, "turn_id": current_turn_id, **recovery_result_payload})  # 新增代码+AutoObserveRecovery：复用标准工具结果事件；如果没有这一行，SDK/API 看不到自动 observe 输出。
                        turn_ledger.update_status(session_id=session_id, turn_id=current_turn_id, status="tool_result_recorded", checkpoint_uuid=last_transcript_uuid, metadata={"last_tool_name": recovery_tool_call.name, "last_tool_call_id": recovery_tool_call.call_id, "auto_recovery": True})  # 新增代码+AutoObserveRecovery：把自动 observe checkpoint 写入账本；如果没有这一行，中断后可能重复恢复工具。
                        messages.extend(message_builders_from_core.tool_result_messages_to_dicts(recovery_tool_call, recovery_context_output, lambda image_tool_call, image_output: build_computer_use_image_message_from_tool_output(tool_name=image_tool_call.name, output=image_output, record_observation=run_helpers_from_core.observation_recorder(self)), image_source_output=recovery_tool_output))  # 新增代码+AutoObserveRecovery：把自动 observe 文本和截图回填模型上下文；如果没有这一行，下一轮模型仍看不到恢复后的屏幕。
                        debug_event_writer(run_id=run_id, event="computer_use_auto_recovery_observe_completed", payload={"tool_result": recovery_result_payload, "recovery_plan": recovery_plan.__dict__}, turn=turn)  # 新增代码+AutoObserveRecovery：写入自动恢复完成 debug 日志；如果没有这一行，排查无法确认恢复是否结束。
                        yield emit("computer_use_auto_recovery_observe", {"turn": turn, "turn_id": current_turn_id, "phase": "completed", "tool_result": recovery_result_payload, "recovery_reason": recovery_plan.reason, "target_ref": recovery_plan.arguments.get("target_ref", "")})  # 新增代码+AutoObserveRecovery：广播自动恢复完成事件；如果没有这一行，acceptance 无法证明恢复闭环。
    ...
    def _final_answer_retry_message(self, user_input: str, answer_text: str) -> str | None:  # 新增代码+最终答案完整性: 生成“最终回答缺少用户指定 Markdown 标题”时的单次重写提示；若没有这行代码，run() 会调用不存在的方法并导致 HTTP bridge 真实测试 500
        if not answer_text.strip():  # 新增代码+OAuthNativeEmptyFinalRetry: 空最终答案不能算任务完成；如果没有这一行，Responses native reasoning-only 结果会被终端打印成空白成功。
            return "你的最终回答是空的。请基于上文已有工具结果重写最终回答，不要重新调用工具，必须回答用户最后一条消息的要求。"  # 新增代码+OAuthNativeEmptyFinalRetry: 给模型一次只写最终答案的恢复机会；如果没有这一行，真实 OAuth native 工具验收会卡在空输出。
    ...
    def _tool_catalog(self) -> list[AgentTool]:  # 新增代码+OAuthNativeCompatToolCatalog: 保留 LearningAgent 旧私有入口并转发到 catalog_runtime；如果没有这段函数，旧测试和旧调用方会因为工具目录方法消失而中断。
        return catalog_runtime_from_tools.tool_catalog(self)  # 新增代码+OAuthNativeCompatToolCatalog: 真实工具目录仍由 catalog_runtime 统一生成；如果没有这一行，兼容入口没有返回值，无法证明工具只是隐藏而不是丢失。

    def _available_tool_schemas(self) -> list[dict[str, Any]]:  # 新增代码+OAuthNativeCompatToolCatalog: 保留 Chat Completions 风格工具 schema 旧私有入口；如果没有这段函数，旧测试无法读取“当前会发给模型的工具列表”。
        return catalog_runtime_from_tools.available_tool_schemas(self)  # 新增代码+OAuthNativeCompatToolCatalog: 委托 filteredTools 后的真实 schema 生成逻辑；如果没有这一行，兼容入口会绕开新的过滤和动态加载规则。

    def _available_responses_tool_schemas(self) -> list[dict[str, Any]]:  # 新增代码+OAuthNativeCompatToolCatalog: 保留 Responses API 风格工具 schema 旧私有入口；如果没有这段函数，后续调试 native tools 时缺少统一查看入口。
        return catalog_runtime_from_tools.available_responses_tool_schemas(self)  # 新增代码+OAuthNativeCompatToolCatalog: 复用 toolToAPISchema 对应的 Responses schema 生成逻辑；如果没有这一行，native 工具链路会和 catalog_runtime 分叉。

    def _tool_schema_names(self, tools: list[dict[str, Any]] | None = None) -> list[str]:  # 新增代码+OAuthNativeCompatToolCatalog: 保留旧测试常用的工具名提取入口；如果没有这段函数，测试必须重复理解两种 schema 格式。
        return catalog_runtime_from_tools.tool_schema_names(self, tools)  # 新增代码+OAuthNativeCompatToolCatalog: 统一交给 catalog_runtime 识别 function/tools 名称；如果没有这一行，工具名提取规则会散落在测试和运行时代码里。
