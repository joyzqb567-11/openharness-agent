...
from learning_agent.computer_use_mcp_v2.windows_runtime.intent_understanding_layer import understand_desktop_intent  # 新增代码+LayeredIntentPlanner：导入独立意图理解层；如果没有这行代码，planner 会继续重复解析 prompt。
from learning_agent.computer_use_mcp_v2.windows_runtime.layer_contracts import IntentUnderstandingResult  # 新增代码+LayeredIntentPlanner：导入 intent 契约类型；如果没有这行代码，planner 输入边界不清楚。
...
TEXT_PAYLOAD_PREFIXES = ("准确文本", "exact text", "输入", "键入", "写入", "填写", "type", "write", "enter")  # 修改代码+TextPayloadExtraction：补充修复轮常见的“准确文本/exact text”前缀；如果没有这行代码，desktop_task 修复提示会丢失要输入的正文。
...
TEXT_PAYLOAD_LEADING_NOISE = ("exactly", "文本", "text", "内容", "正文")  # 修改代码+TextPayloadExtraction：补充“输入文本/type text”里的说明性噪声词；如果没有这行代码，应用里会被输入多余的“文本/text”标签。
...
def _stage_planner_stage(stage_id: str, stage_kind: str, target_ref: str, observation_policy: str, verifier_kind: str = "", verifier_extra: Mapping[str, Any] | None = None, batch_intent: Mapping[str, Any] | None = None) -> StagePlan:  # 修改代码+LayeredStagePlanner：函数段开始，构造带分层契约的通用阶段；如果没有 batch_intent，批编译器只能粗糙猜动作。
    ...
    input_contract = {"requires_target_ref": bool(target_ref), "requires_observation_facts": observation_policy != "none"}  # 新增代码+LayeredStagePlanner：声明阶段输入要求；如果没有这行代码，执行层无法知道何时必须先观察。
    output_contract = {"expected_stage_kind": stage_kind, "target_ref": target_ref}  # 新增代码+LayeredStagePlanner：声明阶段输出范围；如果没有这行代码，verifier 缺少阶段目标上下文。
    verification_contract = {"verifier_kind": verifier.get("verifier_kind", stage_kind), **dict(verifier_extra or {})}  # 新增代码+LayeredStagePlanner：声明验证契约；如果没有这行代码，verification layer 只能读旧 verifier。
    return StagePlan(stage_id=stage_id, stage_kind=stage_kind, target_ref=target_ref, observation_policy=observation_policy, input_contract=input_contract, output_contract=output_contract, batch_intent=dict(batch_intent or {"batch_goal": stage_kind}), verification_contract=verification_contract, verifier=verifier, repair_policy=repair_policy)  # 修改代码+LayeredStagePlanner：返回带分层契约的阶段计划；如果没有这些字段，阶段质量无法被测试和审计。
...
class UniversalDesktopStagePlanner:  # 新增代码+UniversalStagePlanner：类段开始，定义通用桌面阶段规划器；如果没有这个类，full mode 无法拥有上层阶段计划。
    ...
    def plan(self, prompt: str, semantic_intent: Mapping[str, Any] | None = None) -> DesktopTaskPlan:  # 新增代码+UniversalStagePlanner：函数段开始，把用户 prompt 转成通用阶段计划；如果没有这段函数，任务会继续由 primitive 循环驱动。
        ...
        intent = understand_desktop_intent(text, semantic_intent)  # 新增代码+LayeredIntentPlanner：先通过独立意图层提取结构化结果；如果没有这行代码，planner 会重复低质量解析 prompt。
        task_kind = intent.task_kind or _stage_planner_task_kind(text, semantic_intent)  # 修改代码+LayeredIntentPlanner：优先使用 intent 层类型并保留旧兜底；如果没有这行代码，旧测试和新层无法兼容。
        targets = self._targets_for_intent(intent)  # 新增代码+LayeredIntentPlanner：从 intent 生成目标描述；如果没有这行代码，多目标所有权无法复用意图层结果。
        stages = self._stages_for_kind(task_kind, text, intent)  # 修改代码+LayeredIntentPlanner：生成带 intent 的阶段序列；如果没有 intent，正文和新资源要求会重复解析。
        criteria = tuple(intent.success_criteria or self._success_criteria_for_kind(task_kind, text))  # 修改代码+LayeredIntentPlanner：优先使用意图层成功标准；如果没有这行代码，verifier 不能复用 intent 输出。
        resources = tuple(intent.artifact_requests or self._resources_for_prompt(text))  # 修改代码+LayeredIntentPlanner：优先使用意图层产物请求；如果没有这行代码，文件名和位置可能丢失。
        return DesktopTaskPlan(objective=(intent.objective or text[:160]), task_kind=task_kind, targets=tuple(targets), resources=tuple(resources), success_criteria=tuple(criteria), stages=tuple(stages), prompt_signature=_stage_planner_signature(text), layer_skill_metadata={"intent_understanding": dict(intent.layer_skill_metadata), "stage_planner_model": self.model})  # 修改代码+LayeredIntentPlanner：返回带层元数据的完整计划；如果没有这行代码，desktop_task evidence 看不到 layered runtime。
    ...
    def _targets_for_intent(self, intent: IntentUnderstandingResult) -> list[dict[str, Any]]:  # 新增代码+LayeredIntentPlanner：函数段开始，从 intent 输出构造 planner 目标；如果没有这段函数，意图层 target_ref 无法进入阶段计划。
        if intent.required_targets:  # 新增代码+LayeredIntentPlanner：优先使用意图层目标需求；如果没有这行代码，多应用目标角色会丢失。
            targets: list[dict[str, Any]] = []  # 新增代码+LayeredIntentPlanner：初始化目标列表；如果没有这行代码，无法逐项转换。
            for index, target in enumerate(intent.required_targets, start=1):  # 新增代码+LayeredIntentPlanner：遍历 intent 目标；如果没有这行代码，只能处理第一个目标。
                target_ref = str(target.get("target_ref", f"target_{index}") or f"target_{index}")  # 新增代码+LayeredIntentPlanner：读取或生成 target_ref；如果没有这行代码，阶段缺目标键。
                targets.append(_stage_planner_target(target_ref, str(target.get("target_role", "primary") or "primary"), str(target.get("target_hint", target_ref) or target_ref)))  # 新增代码+LayeredIntentPlanner：构造通用目标描述；如果没有这行代码，DesktopTaskPlan.targets 形状会漂移。
            return targets  # 新增代码+LayeredIntentPlanner：返回转换后的目标；如果没有这行代码，调用方拿不到 intent 目标。
        return self._targets_for_kind(intent.task_kind)  # 新增代码+LayeredIntentPlanner：没有 intent 目标时回退旧逻辑；如果没有这行代码，未知输入会缺目标。
    # 新增代码+LayeredIntentPlanner：函数段结束，UniversalDesktopStagePlanner._targets_for_intent 到此结束；如果没有这个边界说明，用户不容易看出目标映射范围。

    def _stages_for_kind(self, task_kind: str, text: str = "", intent: IntentUnderstandingResult | None = None) -> list[StagePlan]:  # 修改代码+LayeredIntentPlanner：函数段开始，按 intent 和通用类型生成阶段；如果没有 intent，正文和授权信息会重复解析。
        ...
        commit_needed = bool(task_kind in {"text_entry", "drawing"} or (intent and intent.artifact_requests))  # 修改代码+LayeredIntentPlanner：文本、绘图或显式产物请求需要保存提交；如果没有这行代码，intent 中的保存要求可能丢失。
        prepare_extra = {"fresh_resource_required": bool((intent.requires_fresh_resource if intent else False) or task_kind in {"text_entry", "drawing"})} if task_kind in {"text_entry", "drawing", "multi_app"} else {}  # 修改代码+LayeredIntentPlanner：优先使用 intent 新资源要求；如果没有这行代码，显式旧窗口授权无法影响准备阶段。
        requested_text = (intent.content_payloads[0] if intent and intent.content_payloads else _stage_planner_requested_text(text))  # 新增代码+LayeredIntentPlanner：优先使用意图层抽取的正文；如果没有这行代码，文本内容会被 planner 重复误抽。
        perform_extra = {"requested_text": requested_text} if task_kind == "text_entry" else {}  # 修改代码+LayeredIntentPlanner：为文本阶段附带正文；如果没有这行代码，批执行器不知道要输入什么。
        stages = [_stage_planner_stage("stage_001_prepare_target", "prepare_target", "target_1", "before_stage", verifier_extra=prepare_extra, batch_intent={"batch_goal": "focus_and_prepare_fresh_resource"}), _stage_planner_stage("stage_002_probe_capabilities", "probe_capabilities", "target_1", "before_stage", batch_intent={"batch_goal": "observe_capabilities"}), _stage_planner_stage("stage_003_perform_content_work", "perform_content_work", "target_1", "before_and_after_stage", verifier_extra=perform_extra, batch_intent={"batch_goal": "perform_stage_content_work", "task_kind": task_kind})]  # 修改代码+LayeredIntentPlanner：构造带批意图的基础阶段；如果没有这行代码，阶段计划缺 batch_intent 质量字段。
