...
def build_capability_profile(observation_frame: Mapping[str, Any] | None) -> AppCapabilityProfile:  # 新增代码+CapabilityProfile：函数段开始，从观察帧生成通用能力画像；如果没有这段函数，上层 planner 仍会靠应用名猜。
    ...
    embedded_profile = frame.get("capability_profile", {}) if isinstance(frame.get("capability_profile", {}), Mapping) else {}  # 新增代码+ObservationFactsProfile：读取 ObservationFacts 内嵌能力画像；如果没有这行代码，stage loop 使用 facts 后能力会退回 unknown。
    if embedded_profile:  # 新增代码+ObservationFactsProfile：优先使用结构化 facts 里的画像；如果没有这行代码，facts 到 compiler 的链路会被重复解析覆盖。
        return AppCapabilityProfile(has_text_input=bool(embedded_profile.get("has_text_input", False)), has_canvas_like_region=bool(embedded_profile.get("has_canvas_like_region", False)), has_menu_bar=bool(embedded_profile.get("has_menu_bar", False)), has_toolbar=bool(embedded_profile.get("has_toolbar", False)), has_file_save_surface=bool(embedded_profile.get("has_file_save_surface", False)), has_browser_navigation_surface=bool(embedded_profile.get("has_browser_navigation_surface", False)), has_grid_or_table=bool(embedded_profile.get("has_grid_or_table", False)), has_modal_dialog=bool(embedded_profile.get("has_modal_dialog", False)), supports_keyboard_shortcuts_likely=bool(embedded_profile.get("supports_keyboard_shortcuts_likely", False)), single_instance_suspected=bool(embedded_profile.get("single_instance_suspected", False)), unknown_capabilities=bool(embedded_profile.get("unknown_capabilities", False)), evidence=tuple(str(item) for item in list(embedded_profile.get("evidence", []) or []) if str(item)))  # 新增代码+ObservationFactsProfile：从 facts 恢复能力画像；如果没有这行代码，批编译不能消费 observation_fact_layer 输出。
