...
class UniversalStageVerifier:  # 新增代码+StageVerifier：类段开始，定义通用阶段验收器；如果没有这个类，阶段循环没有可靠完成判断。
    ...
    def _verify_text_stage(self, stage: StagePlan, after: Mapping[str, Any], execution_result: StageResult) -> StageResult:  # 新增代码+StageVerifier：函数段开始，验证文本阶段；如果没有这段函数，文本验收会依赖事件数。
        ...
        blob = _stage_verifier_blob(after)  # 新增代码+StageVerifier：压缩观察文本；如果没有这行代码，嵌套 UIA 文本无法匹配。
        visible_text = str(after.get("visible_text_summary", after.get("visible_text", after.get("text_content", ""))) or "").strip()  # 修改代码+ObservationFactsVerifier：优先读取结构化可见文本摘要；如果没有这行代码，ObservationFacts 文本结果不会进入验证。
        matched_without_requested = bool(not requested and visible_text and execution_result.status == "completed")  # 新增代码+StageVerifier：无明确正文时允许非空可见文本作为弱完成证据；如果没有这行代码，通用文本任务会卡在字段缺失上。
        matched = bool(after.get("requested_text_visible", False) or after.get("text_content_matches", False) or (requested and requested in blob) or matched_without_requested)  # 新增代码+StageVerifier：判断文本是否可见或匹配；如果没有这行代码，文本完成无法证明。
    ...
    def _verify_drawing_stage(self, stage: StagePlan, after: Mapping[str, Any], execution_result: StageResult) -> StageResult:  # 新增代码+StageVerifier：函数段开始，验证绘图阶段；如果没有这段函数，绘图验收会只看鼠标事件。
        visual_summary = str(after.get("visual_change_summary", "") or "").lower()  # 新增代码+ObservationFactsVerifier：读取结构化视觉变化摘要；如果没有这行代码，facts 中的视觉变化不会参与验证。
        changed = _stage_verifier_truthy(after, ("canvas_changed_after_actions", "canvas_changed", "canvas_not_blank")) or "canvas_changed" in visual_summary or "changed" in visual_summary  # 修改代码+ObservationFactsVerifier：结合 facts 摘要判断画布变化；如果没有这行代码，绘图 facts 会被误判未完成。
        structure = bool(after.get("visual_primitives") or after.get("visual_structure_present") or after.get("colored_region_count", 0) or "structure" in visual_summary or "color" in visual_summary)  # 修改代码+ObservationFactsVerifier：结合 facts 摘要判断视觉结构；如果没有这行代码，结构化观察无法通过绘图验证。
        if changed and structure:  # 新增代码+StageVerifier：画布变化和结构同时满足才完成；如果没有这行代码，复杂绘图质量门禁会过低。
    ...
    def _verify_commit_stage(self, stage: StagePlan, after: Mapping[str, Any], execution_result: StageResult) -> StageResult:  # 新增代码+StageVerifier：函数段开始，验证资源提交阶段；如果没有这段函数，保存动作会缺完成证据。
        save_dialog_state = after.get("save_dialog_state", {}) if isinstance(after.get("save_dialog_state", {}), Mapping) else {}  # 新增代码+ObservationFactsVerifier：读取结构化保存对话框状态；如果没有这行代码，facts 中的保存完成不会参与验证。
        saved = _stage_verifier_truthy(after, ("saved_resource_exists", "save_dialog_completed", "resource_commit_verified", "file_saved", "document_saved")) or bool(save_dialog_state.get("completed", False))  # 修改代码+ObservationFactsVerifier：结合 facts 判断保存完成；如果没有这行代码，保存 facts 会被误判缺证据。
