...
from learning_agent.app.interactive import is_chrome_terminal_command, run_chrome_terminal_command  # 修改代码+Phase13ChromePairingGuide: 导入 /chrome 命令识别和执行入口；如果没有这行代码，测试无法锁定真实终端逻辑。
...
class ChromeTerminalSubcommandsPhase9Tests(unittest.TestCase):  # 新增代码+Phase9ChromeTerminalActions: 定义 Phase 9 的 /chrome 子命令测试集合；如果没有这段测试，终端动作可能只停留在文案提示。
    ...
    def test_duplicate_chrome_command_text_is_treated_as_chrome_status(self) -> None:  # 新增代码+Phase13ChromePairingGuide: 验证真实终端重复输入 /chrome/chrome 仍显示状态；如果没有这段测试，控制器或用户误粘贴会进入模型调用。
        with tempfile.TemporaryDirectory() as temp_dir:  # 新增代码+Phase13ChromePairingGuide: 创建临时工作区；如果没有这行代码，测试会污染真实 memory。
            text = run_chrome_terminal_command(Path(temp_dir), "/chrome/chrome")  # 新增代码+Phase13ChromePairingGuide: 模拟重复粘连的 /chrome 命令；如果没有这行代码，无法复现真实验收失败。
            self.assertIn("Chrome Status", text)  # 新增代码+Phase13ChromePairingGuide: 重复命令也必须返回状态页；如果没有这行断言，误粘贴会被当成模型 prompt。
            self.assertIn("Chrome Guide", text)  # 新增代码+Phase13ChromePairingGuide: 状态页必须保留向导区；如果没有这行断言，Phase 13 验收仍可能看不到新能力。

    def test_interactive_loop_recognizes_duplicate_chrome_command_text(self) -> None:  # 新增代码+Phase13ChromePairingGuide: 验证交互循环会拦截重复 /chrome/chrome；如果没有这段测试，真实终端会把它发给模型。
        self.assertTrue(is_chrome_terminal_command("/chrome/chrome"))  # 新增代码+Phase13ChromePairingGuide: 重复粘连命令应视为 /chrome；如果没有这行断言，验收控制器重复输入会失败。
        self.assertTrue(is_chrome_terminal_command("/chrome install-preview"))  # 新增代码+Phase13ChromePairingGuide: 正常子命令仍应识别；如果没有这行断言，已有功能可能被误伤。
        self.assertFalse(is_chrome_terminal_command("/status"))  # 新增代码+Phase13ChromePairingGuide: 非 Chrome 命令不能被误拦截；如果没有这行断言，其他终端命令会被错误处理。
    ...
    def test_uninstall_confirm_refuses_without_explicit_confirmation_token(self) -> None:  # 新增代码+Phase11ChromeUninstallConfirm: 验证真实卸载必须强确认；如果没有这段测试，用户可能误删 Windows registry。
        with tempfile.TemporaryDirectory() as temp_dir:  # 新增代码+Phase11ChromeUninstallConfirm: 创建临时工作区；如果没有这行代码，测试会污染真实文件。
            registry = MemoryNativeHostRegistryAdapter()  # 新增代码+Phase11ChromeUninstallConfirm: 用内存 registry 观察是否误删；如果没有这行代码，测试无法证明拒绝路径无副作用。
            run_chrome_terminal_command(Path(temp_dir), "/chrome install-confirm abcdefghijklmnopabcdefghijklmnop I_UNDERSTAND_WRITE_REGISTRY", registry_adapter=registry)  # 新增代码+Phase11ChromeUninstallConfirm: 先用 fake registry 注册数据；如果没有这行代码，后续无法证明拒绝时没有删除。
            text = run_chrome_terminal_command(Path(temp_dir), "/chrome uninstall-confirm", registry_adapter=registry)  # 新增代码+Phase11ChromeUninstallConfirm: 少传确认 token 执行卸载；如果没有这行代码，无法覆盖拒绝路径。
            self.assertIn("需要显式确认 token", text)  # 新增代码+Phase11ChromeUninstallConfirm: 输出必须说明缺 token；如果没有这行断言，用户不知道为什么没有卸载。
            self.assertIn("I_UNDERSTAND_DELETE_REGISTRY", text)  # 新增代码+Phase11ChromeUninstallConfirm: 输出必须给出删除确认 token；如果没有这行断言，用户不知道如何明确授权。
            self.assertEqual(len(registry.values), 4)  # 新增代码+Phase11ChromeUninstallConfirm: 拒绝路径不得删除 fake registry；如果没有这行断言，安全门可能只是文案。

    def test_uninstall_confirm_deletes_registry_only_after_confirmation_token(self) -> None:  # 新增代码+Phase11ChromeUninstallConfirm: 验证带 token 后才删除 registry；如果没有这段测试，真实回滚链路可能不可用。
        with tempfile.TemporaryDirectory() as temp_dir:  # 新增代码+Phase11ChromeUninstallConfirm: 创建隔离工作区；如果没有这行代码，manifest 会写入真实项目。
            registry = MemoryNativeHostRegistryAdapter()  # 新增代码+Phase11ChromeUninstallConfirm: 使用 fake registry；如果没有这行代码，测试会有系统副作用。
            run_chrome_terminal_command(Path(temp_dir), "/chrome install-confirm abcdefghijklmnopabcdefghijklmnop I_UNDERSTAND_WRITE_REGISTRY", registry_adapter=registry)  # 新增代码+Phase11ChromeUninstallConfirm: 先写入 fake registry；如果没有这行代码，卸载测试没有可删除对象。
            text = run_chrome_terminal_command(Path(temp_dir), "/chrome uninstall-confirm I_UNDERSTAND_DELETE_REGISTRY", registry_adapter=registry)  # 新增代码+Phase11ChromeUninstallConfirm: 带 token 执行确认卸载；如果没有这行代码，无法证明删除路径可用。
            self.assertIn("browser_extension_uninstall_confirm", text)  # 新增代码+Phase11ChromeUninstallConfirm: 输出必须标明确认卸载；如果没有这行断言，用户无法区分 preview 和 confirm。
            self.assertIn("dry_run=false", text)  # 新增代码+Phase11ChromeUninstallConfirm: 输出必须明确这次不是预览；如果没有这行断言，真实删除边界不可见。
            self.assertEqual(registry.values, {})  # 新增代码+Phase11ChromeUninstallConfirm: token 正确时应删除所有 fake registry；如果没有这行断言，回滚可能留下残留项。
