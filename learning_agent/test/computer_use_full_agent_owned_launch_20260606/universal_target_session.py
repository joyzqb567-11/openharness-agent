...
try:  # 新增代码+URG2UniversalTargetSession：优先按 learning_agent 包路径导入已有通用能力；如果没有这一段，单测和生产入口无法共享同一实现。
    from learning_agent.computer_use.generic_app_discovery import resolve_generic_app_launch_target  # 新增代码+URG2UniversalTargetSession：复用 Phase108 通用应用发现；如果没有这一行，URG-2 可能退回逐 app 白名单。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.generic_app_discovery", "learning_agent.computer_use.generic_launch_backend", "learning_agent.computer_use.target_identity", "learning_agent.computer_use.windows_backend"}:  # 修改代码+RealLaunchTargetSession：兜底名单加入启动后端和窗口 inventory；如果没有这一行，bat 入口可能无法走真实启动链路。
        raise  # 新增代码+URG2UniversalTargetSession：重新抛出非路径类导入错误；如果没有这一行，排查真实依赖问题会很困难。
    from computer_use.generic_app_discovery import resolve_generic_app_launch_target  # type: ignore  # 新增代码+URG2UniversalTargetSession：脚本模式导入通用发现；如果没有这一行，bat 入口不能解析普通目标。
...
class Phase117OwnedWindowProbe:  # 新增代码+RealLaunchTargetSession：类段开始，启动后按自有进程 pid 查找真实窗口；如果没有这个类，agent 只能启动进程却不能证明目标窗口属于自己。
    ...
    def find_owned_window(self, launch_result: dict[str, Any], target_hint: str = "") -> dict[str, Any]:  # 新增代码+RealLaunchTargetSession：函数段开始，按启动结果中的 pid 找窗口；如果没有这段函数，旧用户窗口可能被误当 agent 窗口。
        ...
        for attempt_index in range(self.poll_attempts):  # 新增代码+RealLaunchTargetSession：有限轮询等待窗口出现；如果没有这一行，刚启动的 GUI 程序可能还没建窗就失败。
            ...
            for window in _phase117_snapshot_windows(snapshot):  # 新增代码+RealLaunchTargetSession：遍历安全窗口候选；如果没有这一行，无法找到目标窗口。
