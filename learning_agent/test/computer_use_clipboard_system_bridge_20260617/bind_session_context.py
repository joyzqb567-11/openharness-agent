...
from ..windows_runtime.system_clipboard import WindowsClipboardBackend  # 新增代码+ClipboardSystemBridge：导入 Windows 系统剪贴板后端；如果没有这一行，agent-side context 默认只能使用旧内存剪贴板。
from ..windows_runtime.coordinates import claudecode_display_resolved_for_apps_key  # 新增代码+ClaudeCodeLifecycleParity：导入 displayResolvedForApps key helper；如果没有这一行，绑定层无法从 rich records 恢复 ClaudeCode string key。
...
def _lock_callbacks_from_runtime(runtime: Any) -> dict[str, Any]:  # 新增代码+ClaudeCodeLockParity：函数段开始，从 session runtime 构造 context 锁回调；如果没有这段函数，bind_session_context 会充满重复 lambda 和字段判断。
    ...
    if lock_manager is None:  # 新增代码+ClaudeCodeLockParity：没有 lock manager 时不绑定真实回调；如果没有这行代码，后续 getattr None 会崩溃。
        def cleanup_after_turn_without_lock(reason: str) -> dict[str, Any]:  # 新增代码+ClaudeCodeLifecycleParity：函数段开始，轻量 runtime 没有 lock_manager 时仍可调用 cleanup_turn；如果没有这段函数，wrapper fake 或诊断 runtime 会丢失完整 cleanup。
            cleanup_method = getattr(runtime, "cleanup_turn", None)  # 新增代码+ClaudeCodeLifecycleParity：读取 runtime cleanup 方法；如果没有这行代码，轻量 runtime 无法收尾。
            if callable(cleanup_method):  # 新增代码+ClaudeCodeLifecycleParity：只有 cleanup 方法存在时才调用；如果没有这行代码，非标准 runtime 会崩溃。
                return dict(cleanup_method(reason=str(reason)) or {})  # 新增代码+ClaudeCodeLifecycleParity：执行完整 cleanup 并返回报告；如果没有这行代码，wrapper 无法保留 lifecycle 字段。
            return {"cleanup_completed": True, "lock_released": False, "reason": str(reason), "lock_backend": "unavailable"}  # 新增代码+ClaudeCodeLifecycleParity：无 cleanup 方法时安全 no-op；如果没有这行代码，fallback 结果字段不稳定。
        # 新增代码+ClaudeCodeLifecycleParity：函数段结束，cleanup_after_turn_without_lock 到此结束；如果没有这个边界说明，用户不容易看出轻量 cleanup 范围。
        def register_global_escape_abort_without_lock(tool_name: str) -> dict[str, Any]:  # 新增代码+ClaudeCodeLifecycleParity：函数段开始，轻量 runtime 仍可暴露 Esc 注册；如果没有这段函数，fake/runtime 无锁场景无法测试 Esc 接线。
            method = getattr(runtime, "register_global_escape_abort", None)  # 新增代码+ClaudeCodeLifecycleParity：读取 runtime Esc 注册方法；如果没有这行代码，非标准 runtime 会直接崩溃。
            return dict(method(tool_name) or {}) if callable(method) else {"global_hotkey_registered": False, "reason": "lock_manager_missing"}  # 新增代码+ClaudeCodeLifecycleParity：调用注册或返回稳定缺失报告；如果没有这行代码，debug 字段不稳定。
        # 新增代码+ClaudeCodeLifecycleParity：函数段结束，register_global_escape_abort_without_lock 到此结束；如果没有这个边界说明，用户不容易看出轻量 Esc 注册范围。
        def mark_expected_escape_without_lock(reason: str, count: int) -> dict[str, Any]:  # 新增代码+ClaudeCodeLifecycleParity：函数段开始，轻量 runtime 仍可声明计划内 Escape；如果没有这段函数，测试无法覆盖 expected Esc 接线。
            method = getattr(runtime, "mark_expected_escape", None)  # 新增代码+ClaudeCodeLifecycleParity：读取 runtime expected Escape 方法；如果没有这行代码，非标准 runtime 会直接崩溃。
            return dict(method(reason=reason, count=count) or {}) if callable(method) else {"expected_escape_count": 0, "reason": "lock_manager_missing"}  # 新增代码+ClaudeCodeLifecycleParity：调用声明或返回稳定缺失报告；如果没有这行代码，调用方无法审计能力。
        # 新增代码+ClaudeCodeLifecycleParity：函数段结束，mark_expected_escape_without_lock 到此结束；如果没有这个边界说明，用户不容易看出轻量 expected Escape 范围。
        return {"computer_use_session_id": session_id, "cleanup_after_turn": cleanup_after_turn_without_lock, "register_global_escape_abort": register_global_escape_abort_without_lock, "mark_expected_escape": mark_expected_escape_without_lock}  # 修改代码+ClaudeCodeLifecycleParity：返回 session id 和轻量 lifecycle 回调；如果没有这行代码，wrapper cleanup 会回退成 release-only no-op。
    ...
    def mark_expected_escape(reason: str, count: int) -> dict[str, Any]:  # 新增代码+ClaudeCodeLifecycleParity：函数段开始，绑定模型计划内 Escape 声明；如果没有这段函数，动作层未来无法声明模型发出的 Esc。
        method = getattr(runtime, "mark_expected_escape", None)  # 新增代码+ClaudeCodeLifecycleParity：读取 runtime 计划 Escape 方法；如果没有这一行，非标准 runtime 会直接崩溃。
    ...
    return {"computer_use_session_id": session_id, "check_computer_use_lock": check_lock, "acquire_computer_use_lock": acquire_lock, "release_computer_use_lock": release_lock, "cleanup_after_turn": cleanup_after_turn, "is_lock_held_locally": is_lock_held_locally, "register_global_escape_abort": register_global_escape_abort, "mark_expected_escape": mark_expected_escape}  # 修改代码+ClaudeCodeLifecycleParity：返回 context 可直接展开的锁和 Esc 回调；如果没有这一行，绑定层无法把 runtime 生命周期接到 v2 context。
# 新增代码+ClaudeCodeLockParity：函数段结束，_lock_callbacks_from_runtime 到此结束；如果没有这个边界说明，用户不容易看出锁回调构造范围。


def _display_state_from_agent(agent: Any) -> dict[str, Any]:  # 新增代码+ClaudeCodeDisplayParity：函数段开始，从 agent 读取可选 display 初始状态；如果没有这段函数，context 每次新建都会丢失主循环已知的屏幕状态。
    selected_display_id = str(getattr(agent, "computer_use_selected_display_id", "") or "")  # 新增代码+ClaudeCodeDisplayParity：读取 agent 保存的 selected display；如果没有这行代码，多轮主循环无法把屏幕选择传给 v2 context。
    display_pinned_by_model = bool(getattr(agent, "computer_use_display_pinned_by_model", False))  # 新增代码+ClaudeCodeDisplayParity：读取 agent 保存的临时 pin 状态；如果没有这行代码，ClaudeCode displayPinnedByModel 不能从主循环恢复。
    resolved_for_apps = getattr(agent, "computer_use_display_resolved_for_apps", [])  # 新增代码+ClaudeCodeDisplayParity：读取 agent 保存的 app-display 解析；如果没有这行代码，已解析应用落屏信息不会进入 context。
    raw_resolved_key = getattr(agent, "computer_use_display_resolved_for_apps_key", "")  # 新增代码+ClaudeCodeLifecycleParity：读取 agent 保存的 ClaudeCode string key；如果没有这行代码，context 重建后 key 可能丢失。
    screenshot_dims = getattr(agent, "computer_use_last_screenshot_dims", {})  # 新增代码+ClaudeCodeDisplayParity：读取 agent 保存的最近截图尺寸；如果没有这行代码，lastScreenshotDims 在 context 重建时会丢失。
    safe_resolved_for_apps = [dict(item) for item in resolved_for_apps if isinstance(item, dict)] if isinstance(resolved_for_apps, list) else []  # 新增代码+ClaudeCodeDisplayParity：复制合法解析记录；如果没有这行代码，坏字段类型或共享引用可能污染 context。
    resolved_key_from_legacy_field = str(resolved_for_apps or "") if isinstance(resolved_for_apps, str) else ""  # 新增代码+ClaudeCodeLifecycleParity：兼容旧字段直接保存 string key；如果没有这行代码，key-only 状态会被当成无效 records 丢弃。
    safe_resolved_key = str(raw_resolved_key or resolved_key_from_legacy_field or claudecode_display_resolved_for_apps_key(safe_resolved_for_apps))  # 新增代码+ClaudeCodeLifecycleParity：优先保存显式 key，其次兼容旧字段，再由 records 生成；如果没有这行代码，context 无法稳定恢复 display key。
    safe_screenshot_dims = dict(screenshot_dims) if isinstance(screenshot_dims, dict) else {}  # 新增代码+ClaudeCodeDisplayParity：复制合法截图尺寸；如果没有这行代码，坏字段类型会让 context 初始化失败。
    return {"selected_display_id": selected_display_id, "display_pinned_by_model": display_pinned_by_model, "display_resolved_for_apps": safe_resolved_for_apps, "display_resolved_for_apps_key": safe_resolved_key, "last_screenshot_dims": safe_screenshot_dims}  # 修改代码+ClaudeCodeLifecycleParity：返回可展开到 dataclass 的 display 字段和 key；如果没有这行代码，绑定层无法保持初始化代码简洁。
# 新增代码+ClaudeCodeDisplayParity：函数段结束，_display_state_from_agent 到此结束；如果没有这个边界说明，用户不容易看出 display 初始状态读取范围。


def _disabled_callbacks_from_agent(agent: Any) -> dict[str, Any]:  # 新增代码+ClaudeCodeLifecycleParity：函数段开始，从 agent 生成 disabled 判断回调；如果没有这段函数，tools/list 无法复用主循环禁用状态。
    def is_computer_use_disabled() -> bool:  # 新增代码+ClaudeCodeLifecycleParity：函数段开始，判断 Computer Use 模式是否禁用；如果没有这段函数，mode disabled 只能靠静态字段且无法动态变化。
        method = getattr(agent, "is_computer_use_disabled", None)  # 新增代码+ClaudeCodeLifecycleParity：优先读取 agent 方法；如果没有这一行，动态禁用判断无法接入。
        if callable(method):  # 新增代码+ClaudeCodeLifecycleParity：确认方法可调用；如果没有这一行，非函数字段会被误调用。
            return bool(method())  # 新增代码+ClaudeCodeLifecycleParity：返回动态禁用状态；如果没有这一行，tools/list 看不到最新模式状态。
        return bool(getattr(agent, "computer_use_mcp_v2_disabled", getattr(agent, "computer_use_disabled", False)))  # 新增代码+ClaudeCodeLifecycleParity：回退读取常见布尔字段；如果没有这一行，轻量 fake agent 和旧主循环字段无法表达禁用。
    # 新增代码+ClaudeCodeLifecycleParity：函数段结束，is_computer_use_disabled 到此结束；如果没有这个边界说明，用户不容易看出模式禁用判断范围。
    def is_computer_use_context_disabled() -> bool:  # 新增代码+ClaudeCodeLifecycleParity：函数段开始，判断当前上下文是否禁用 Computer Use；如果没有这段函数，单轮上下文禁用无法进入 tools/list。
        method = getattr(agent, "is_computer_use_context_disabled", None)  # 新增代码+ClaudeCodeLifecycleParity：优先读取 agent 上下文禁用方法；如果没有这一行，动态上下文状态无法接入。
        if callable(method):  # 新增代码+ClaudeCodeLifecycleParity：确认方法可调用；如果没有这一行，非函数字段会被误调用。
            return bool(method())  # 新增代码+ClaudeCodeLifecycleParity：返回动态上下文禁用状态；如果没有这一行，tools/list 看不到最新上下文状态。
        return bool(getattr(agent, "computer_use_mcp_v2_context_disabled", getattr(agent, "computer_use_context_disabled", False)))  # 新增代码+ClaudeCodeLifecycleParity：回退读取常见布尔字段；如果没有这一行，轻量 fake agent 和旧主循环字段无法表达单轮禁用。
    # 新增代码+ClaudeCodeLifecycleParity：函数段结束，is_computer_use_context_disabled 到此结束；如果没有这个边界说明，用户不容易看出上下文禁用判断范围。
    return {"is_computer_use_disabled": is_computer_use_disabled, "is_computer_use_context_disabled": is_computer_use_context_disabled}  # 新增代码+ClaudeCodeLifecycleParity：返回可展开到 context 的禁用回调；如果没有这一行，绑定层无法保持 context 构造简洁。
# 新增代码+ClaudeCodeLifecycleParity：函数段结束，_disabled_callbacks_from_agent 到此结束；如果没有这个边界说明，用户不容易看出禁用回调构造范围。
...
def bind_session_context(agent: Any) -> ComputerUseMcpV2Context:  # 修改代码+ClaudeCodeLockParity：函数段开始，从 agent 构建或复用 v2 context；如果没有这段函数，agent-side wrapper 会重复拼装回调并丢失状态。
    ...
    display_state = _display_state_from_agent(agent)  # 新增代码+ClaudeCodeDisplayParity：读取 agent 上已有 display state；如果没有这行代码，新 context 无法带入 selectedDisplayId/lastScreenshotDims。
    disabled_callbacks = _disabled_callbacks_from_agent(agent)  # 新增代码+ClaudeCodeLifecycleParity：读取 agent 禁用状态回调；如果没有这行代码，standalone tools/list 不会对齐 disabled 空工具列表。
    ...
    explicit_clipboard_backend = getattr(agent, "computer_use_mcp_v2_clipboard_backend", None)  # 新增代码+ClipboardSystemBridge：读取测试或宿主显式注入的剪贴板后端；如果没有这一行，自动化测试无法避免触碰真实系统剪贴板。
    clipboard_backend = explicit_clipboard_backend or WindowsClipboardBackend()  # 新增代码+ClipboardSystemBridge：没有显式后端时绑定 Windows 系统剪贴板；如果没有这一行，真实 agent-side 工具仍会停留在 context-only 剪贴板。
    context = ComputerUseMcpV2Context(host=host, ask_permission=ask_permission, record_observation=record_observation, record_runtime_trace=record_runtime_trace, emit_acceptance_event=emit_acceptance, clipboard_backend=clipboard_backend, **lock_callbacks, **display_state, **disabled_callbacks)  # 修改代码+ClipboardSystemBridge：创建绑定主循环能力、锁生命周期、display 状态、disabled 判断和系统剪贴板的 v2 context；如果没有这一行，v2 工具无法共享真实剪贴板后端。
