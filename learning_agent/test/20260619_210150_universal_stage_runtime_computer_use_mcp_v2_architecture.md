...
## 2026-06-19 Universal Stage Planner / Observer / Batch Executor

OpenHarness 的 Computer Use full 模式现在采用上下两层设计。下层继续沿用 ClaudeCode 风格的运行时治理：权限申请、桌面控制锁、目标窗口所有权、动作安全门禁、turn cleanup、截图和底层 SendInput/UIA 执行都保持在 `computer_use_mcp_v2/windows_runtime` 内统一管理。上层新增通用阶段运行时，负责把用户意图转成可验收的 `DesktopTaskPlan`，再按阶段观察、批量执行和验证。

新的上层核心对象包括 `DesktopTaskPlan`、`StagePlan`、`ActionBatch`、`StageResult` 和 `DesktopTaskRunState`。任务先被拆成准备目标、探测能力、执行内容、提交资源、验证结果等通用阶段，再由每个阶段编译出绑定同一个 `target_ref` 的动作批次，避免每个按键或每一笔鼠标路径都重新回到模型规划。

能力判断不依赖单个应用名。`capability_profile.py` 会从观察帧抽象出文本输入、画布、菜单、工具栏、保存界面、浏览导航、弹窗和未知能力。代表性软件可以出现在 acceptance fixture、别名发现和日志证据里，但生产运行时不能因为 Notepad、Paint、浏览器、微信或 WPS 等名字走专用控制器。

执行策略从原子动作循环升级为阶段边界观察加阶段批执行。默认只在阶段前后观察：准备和探测阶段确认目标，内容阶段一次性执行一批输入、拖动或导航动作，提交阶段执行保存或提交命令，验证阶段读取观察证据。高风险或破坏性操作仍可升级为更细粒度观察。

`UniversalStageTaskLoop` 是新的真实 full-mode 主路径。`UniversalDesktopExecutionLoopAdapter` 会优先调用它，并在结果中输出 `universal_stage_task_loop_used`、`desktop_task_plan_created`、`stage_count`、`completed_stage_count`、`desktop_task_completed`、`desktop_task_incomplete`、`stage_boundary_observation_used`、`batch_execution_used`、`primitive_action_count` 和 `low_level_event_count`。旧的 `UniversalObservePlanActVerifyLoop` 保留为兼容路径，不再作为复杂本地软件任务的主要收敛方式。

多窗口任务通过 `target_ref` 一对一绑定表达。每个写动作批次只能绑定一个目标窗口；切换窗口必须经过观察或焦点验证；如果发现用户已有窗口，默认不接管，除非用户明确授权使用已有窗口。单实例应用、未知应用、多进程应用都走同一套 target session、FreshTarget、TargetLease 和用户授权规则。

最终回答增加桌面任务完成门禁。`convergence_controller.py` 会从工具结果和运行时状态提取阶段证据；如果 `desktop_task_incomplete=true`、`completed_stage_count < stage_count`、出现 `Next desktop action`，或保存/目标漂移验证失败，则阻止模型把未完成桌面任务说成完成。这个门禁只是兜底，真正的主方案是阶段规划、阶段观察、批量执行和阶段验证。
...
本轮补齐 ClaudeCode lifecycle parity 蓝图中的四个剩余协议点，仍然保持“Windows runtime 不伪装成 macOS native package”的边界。

1. Esc acquire lifecycle：`ComputerUseMcpV2Context` 新增 `register_global_escape_abort` 和 `mark_expected_escape` 回调；`runtime.py` 在非 defers-lock 工具 acquire 成功后注册全局 Esc 急停，并把注册结果写入 `debug.escape_abort`；`WindowsComputerUseSessionRuntime` 持有 `GlobalEscapeAbortController`，绑定层把该能力传给 v2 context。
2. 完整 turn cleanup：`WindowsComputerUseSessionRuntime.cleanup_turn()` 统一委托 `run_turn_cleanup()`，覆盖 transient input release、hidden host/app restore、Esc hook unregister、lock release、abort clear，再合并 owned resource cleanup 和 notification。wrapper 仍通过 `cleanup_computer_use_mcp_v2_turn(context)` 复用同一绑定 context。
3. `tools/list` disabled：standalone `claudecode_bridge/mcpServer.py` 在 `tools/list` 入口先读取 `is_computer_use_disabled()` 和 `is_computer_use_context_disabled()`。任一为 true 或判断异常时失败关闭，返回 `{"tools": []}`，并记录 `computer_use_tools_list_app_inventory` trace，状态为 `disabled`。
4. `displayResolvedForApps` key：外部 `payload.displayState.displayResolvedForApps` 现在是 ClaudeCode-style string key，例如 `alpha.exe,zeta.exe`；OpenHarness rich records 保留在 `displayResolvedForAppsRecords`，并继续存入 context 的 `display_resolved_for_apps`，用于 Windows 多屏和窗口诊断。
...
- `learning_agent/tests/test_computer_use_mcp_v2_bridge_wrapper.py`
