...
PHASE37_SUPPORTED_ACTIONS = ("click", "double_click", "move_mouse", "press_key", "scroll", "type_text")  # 新增代码+Phase37WindowsSendInputExecutor: 集中列出 SendInput 执行器覆盖动作；如果没有这行代码，支持范围会散落在多个分支里。
...
class WindowsSendInputExecutor:  # 新增代码+Phase37WindowsSendInputExecutor: 定义 Windows SendInput 动作执行器；如果没有这个类，learning_agent 仍停留在旧 SetCursorPos/mouse_event 路径。
    ...
    def status(self) -> dict[str, Any]:  # 新增代码+Phase37WindowsSendInputExecutor: 函数段开始，返回执行器状态；如果没有这段函数，/computer status 无法看到 SendInput 缺口。
        ...
        return {"backend": "windows_sendinput", "contract": PHASE37_SENDINPUT_CONTRACT, "contract_ready": True, "available": real_input_enabled, "platform": self.platform, "platform_supported": platform_supported, "enabled": self.enabled, "implementation_available": implementation_available, "real_input_enabled": real_input_enabled, "real_input_default": False, "safe_gate_required": True, "forbidden_targets_enforced_upstream": True, "actions_supported": list(PHASE37_SUPPORTED_ACTIONS), "reason": reason}  # 新增代码+Phase37WindowsSendInputExecutor: 返回机器可读状态；如果没有这行代码，测试和状态页无法验证执行器边界。
