...
class WindowsComputerUseSendInputDispatcherPhase47Tests(unittest.TestCase):  # 新增代码+Phase47WindowsSendInputDispatcher: 定义 Phase47 测试集合；如果没有这个类，unittest 不会组织 SendInput dispatcher 测试。
    ...
    def test_window_bound_drag_path_focuses_target_before_mouse_events(self) -> None:  # 新增代码+ForegroundTargetRouting: 函数段开始，验证绑定窗口的拖拽路径会先把目标窗口置前；如果没有这个测试，Paint 可能真实打开但鼠标事件落不到画布。
        verifier = Phase47TargetVerifier(ok=True)  # 新增代码+ForegroundTargetRouting: 创建通过的目标校验器；如果没有这行代码，测试会被目标变化门禁拦住而不是检查聚焦顺序。
        sender = Phase47RecordingLowLevelSender()  # 新增代码+ForegroundTargetRouting: 创建记录型低层 sender；如果没有这行代码，测试无法看到第一条事件是不是 set_foreground。
        executor = self._executor(verifier, sender)  # 新增代码+ForegroundTargetRouting: 创建 executor-dispatcher 链路；如果没有这行代码，高层 drag_path 无法进入低层展开逻辑。
        window = {"window_id": "hwnd:925582", "app_id": "mspaintapp:pid:51180", "title_preview": "Untitled - Paint"}  # 新增代码+ForegroundTargetRouting: 模拟真实验收中 Paint 返回的窗口身份；如果没有这行代码，动作没有 hwnd 可用于前台聚焦。
        result = executor.execute("drag_path", {"window": window, "points": [{"x": 320, "y": 320}, {"x": 360, "y": 360}, {"x": 400, "y": 320}]})  # 新增代码+ForegroundTargetRouting: 执行一条真实画线会用到的窗口绑定拖拽路径；如果没有这行代码，测试不会覆盖 Paint 空白画布的最后一跳链路。
        event_types = [str(event.get("type", "")) for event in sender.low_level_events]  # 新增代码+ForegroundTargetRouting: 收集低层事件顺序；如果没有这行代码，无法断言 set_foreground 是否排在鼠标事件之前。
        self.assertTrue(result.ok)  # 新增代码+ForegroundTargetRouting: 断言动作链路本身成功；如果没有这行代码，后续事件顺序可能只是失败路径残留。
        self.assertGreater(len(sender.low_level_events), 1)  # 新增代码+ForegroundTargetRouting: 断言确实生成了多个低层事件；如果没有这行代码，空事件也可能让顺序检查失真。
        self.assertEqual(event_types[0], "set_foreground")  # 新增代码+ForegroundTargetRouting: 断言第一条事件必须先聚焦目标窗口；如果没有这行代码，鼠标事件可能发给终端或旧窗口。
        self.assertEqual(sender.low_level_events[0].get("hwnd"), 925582)  # 新增代码+ForegroundTargetRouting: 断言 set_foreground 使用真实 hwnd 数字；如果没有这行代码，聚焦事件可能缺少 Windows API 需要的句柄。
        self.assertIn("mouse_down", event_types)  # 新增代码+ForegroundTargetRouting: 断言拖拽仍展开出鼠标按下；如果没有这行代码，测试只证明聚焦但不证明画线事件仍存在。
        self.assertTrue(any(event.get("target", {}).get("window_id") == "hwnd:925582" for event in sender.low_level_events if event.get("type") == "mouse_down"))  # 新增代码+ForegroundTargetRouting: 断言鼠标事件仍携带目标身份；如果没有这行代码，最后一跳 sender 无法审计事件属于哪个窗口。
    # 新增代码+ForegroundTargetRouting: 函数段结束，test_window_bound_drag_path_focuses_target_before_mouse_events 到此结束；如果没有这个边界说明，读者不容易看出窗口聚焦回归范围。
