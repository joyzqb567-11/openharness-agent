...
## Computer Use FreshTarget 简短规则

- 在 `/computer use --full` 中控制本地应用时，先用 `mcp__computer-use__open_application` 建立 FreshTarget/TargetLease，再观察和操作窗口。
- 如果目标软件已经打开，默认不要接管旧窗口；请提示用户先手动关闭旧窗口，或让用户明确授权使用已有窗口。
- `open_application` 成功后记住返回的 `target_ref`；当任务同时涉及多个软件或多个窗口时，后续写动作必须带正确的 `target_ref`。
- 这些提示只帮助模型规划；真正安全边界由 controller、UniversalTargetSession、TargetRegistry 和 TargetLease 执行。
...
- 当前模型可见工具应保持为 read / write / edit / bash / tool_search。
