...
DESKTOP_RESOURCE_PREPARATION_OBSERVE_REQUIRED_MARKER = "OPENHARNESS_DESKTOP_RESOURCE_PREPARATION_OBSERVE_REQUIRED"  # 新增代码+ResourcePreparationConvergence：桌面旧资源执行新建空白准备后必须 observe 的稳定标记；如果没有这行代码，收敛层会把“先观察确认”当成普通失败。
DESKTOP_TASK_INCOMPLETE_MARKER = "OPENHARNESS_DESKTOP_TASK_INCOMPLETE"  # 新增代码+DesktopTaskIncompleteGate：桌面阶段任务未完成的稳定标记；如果没有这行代码，模型会从 desktop_task 退回 key/click/drag 零散动作。
...
ACTIONABILITY_MARKERS = {  # 修改代码+FreshTargetUserAction：集中列出所有 marker 并加入用户动作阻断；如果没有这段代码，解析函数要到处重复硬编码。
    ...
    DESKTOP_RESOURCE_PREPARATION_OBSERVE_REQUIRED_MARKER,  # 新增代码+ResourcePreparationConvergence：加入资源准备后观察确认 marker；如果没有这行代码，Ctrl+N 后不会强制 observe。
    DESKTOP_TASK_INCOMPLETE_MARKER,  # 新增代码+DesktopTaskIncompleteGate：加入桌面任务未完成 marker；如果没有这行代码，record_actionability_from_tool_result 解析不到 Stage Runtime 未完成事实。
...
ACTIONABILITY_ALLOWED_FIELDS = {  # 新增代码+ObservePlanActVerify：字段白名单段开始，只保存动作必需的低敏字段；如果没有这段代码，网页标签或用户输入可能被放进长期状态。
    ...
    "stage_count",  # 新增代码+DesktopTaskIncompleteGate：允许保存总阶段数；如果没有这行代码，未完成任务的 pending 缺少阶段审计。
    "completed_stage_count",  # 新增代码+DesktopTaskIncompleteGate：允许保存已完成阶段数；如果没有这行代码，模型不知道任务只完成了一部分。
    "desktop_task_incomplete",  # 新增代码+DesktopTaskIncompleteGate：允许保存桌面任务未完成布尔证据；如果没有这行代码，后续门禁只能依赖自然语言。
    "desktop_task_completed",  # 新增代码+DesktopTaskIncompleteGate：允许保存桌面任务完成布尔证据；如果没有这行代码，状态协议无法表达任务是否已闭环。
    "needs_user",  # 新增代码+DesktopTaskIncompleteGate：允许保存是否需要用户介入；如果没有这行代码，单实例或旧窗口场景会被当成可自动恢复。
...
def _store_pending_actionability(runtime_state: dict[str, Any] | None, marker: str, fields: dict[str, str], source_tool_name: str) -> None:  # 新增代码+ObservePlanActVerify：函数段开始，保存解析后的下一步动作；如果没有这段函数，状态写入规则会散落。
    ...
    if marker == DESKTOP_RESOURCE_PREPARATION_OBSERVE_REQUIRED_MARKER:  # 新增代码+ResourcePreparationConvergence：资源准备阻断进入 observe pending；如果没有这行代码，模型可能继续输入而不观察空白资源。
        pending.setdefault("actionability_kind", "desktop_resource_preparation_observe_required")  # 新增代码+ResourcePreparationConvergence：补齐 pending 分类；如果没有这行代码，日志看不出这是 Ctrl+N 后的观察确认。
        pending.setdefault("next_required_tool", "mcp__computer-use__observe")  # 新增代码+ResourcePreparationConvergence：默认下一步必须调用 observe；如果没有这行代码，marker 缺字段时收敛层不知道要求什么。
    if marker == DESKTOP_TASK_INCOMPLETE_MARKER:  # 新增代码+DesktopTaskIncompleteGate：桌面阶段任务未完成时进入高层任务收敛；如果没有这行代码，模型会把失败拆成原子鼠标键盘动作。
        pending.setdefault("actionability_kind", "desktop_task_incomplete")  # 新增代码+DesktopTaskIncompleteGate：补齐 pending 分类；如果没有这行代码，阻断函数无法识别这是 Stage 任务未完成。
        pending.setdefault("next_required_tool", "mcp__computer-use__desktop_task")  # 新增代码+DesktopTaskIncompleteGate：默认下一步回到高层 desktop_task；如果没有这行代码，模型不会继续阶段任务循环。
        pending.setdefault("next_allowed_tools", "mcp__computer-use__desktop_task,mcp__computer-use__observe,mcp__computer-use__request_access")  # 新增代码+DesktopTaskIncompleteGate：允许高层任务、阶段观察和必要授权；如果没有这行代码，正确恢复路径可能被过度阻断。
        pending.setdefault("next_required_reason", "desktop_task_incomplete")  # 新增代码+DesktopTaskIncompleteGate：补齐低敏原因码；如果没有这行代码，提示只会说有 pending 但不说明为什么。
...
def record_actionability_from_tool_result(tool_name: str, output_text: str, runtime_state: dict[str, Any] | None) -> None:  # 新增代码+ObservePlanActVerify：函数段开始，从工具结果沉淀下一步动作状态；如果没有这段函数，工具 marker 无法控制后续模型行为。
    ...
    existing_pending = get_pending_actionability(runtime_state)  # 新增代码+DesktopTaskIncompleteGate：读取 marker 到来前的 pending；如果没有这行代码，observe 结果可能覆盖高层桌面任务未完成状态。
    normalized_source_tool = normalize_actionability_tool_name(tool_name)  # 新增代码+DesktopTaskIncompleteGate：归一化当前工具名；如果没有这行代码，mcp__computer-use__observe 无法识别为 observe。
    if marker == DESKTOP_ACTION_REQUIRED_MARKER and existing_pending.get("marker") == DESKTOP_TASK_INCOMPLETE_MARKER and normalized_source_tool in {"observe", "screenshot"}:  # 新增代码+DesktopTaskIncompleteGate：Stage 未完成 pending 下的观察结果不能降级覆盖；如果没有这行代码，observe 后又会放行 key/click/drag。
        state = _runtime_dict(runtime_state)  # 新增代码+DesktopTaskIncompleteGate：取得可写运行态；如果没有这行代码，无法留下保护审计信息。
        state[ACTIONABILITY_LAST_CLEARED_KEY] = "desktop_task_incomplete_preserved_after_observe"  # 新增代码+DesktopTaskIncompleteGate：记录本次没有覆盖 pending 的原因；如果没有这行代码，排查时看不出为什么保留旧 pending。
        return  # 新增代码+DesktopTaskIncompleteGate：保持原 desktop_task_incomplete pending 不变；如果没有这行代码，下面会把 observe marker 写成新 pending。
...
def pending_actionability_argument_mismatch(tool_name: str, tool_arguments: Any, pending: dict[str, Any]) -> str:  # 新增代码+PendingArgumentGate：函数段开始，检查正确工具是否用了错误的 pending 参数；如果没有这段函数，模型会把旧 element_id 填到错误按钮。
    ...
    if pending.get("marker") == DESKTOP_RESOURCE_PREPARATION_OBSERVE_REQUIRED_MARKER and normalized_tool in {"observe", "screenshot"}:  # 新增代码+ResourcePreparationConvergence：资源准备确认允许 observe 走 adapter active target 自动绑定；如果没有这行代码，模型缺 target_ref 时会被过度拦截。
        return ""  # 新增代码+ResourcePreparationConvergence：返回无参数不匹配；如果没有这行代码，observe 无法完成 pending。
...
def should_block_tool_for_pending_actionability(tool_name: str, tool_call_text: str, pending: dict[str, Any], user_debug_intent: bool) -> bool:  # 新增代码+ObservePlanActVerify：函数段开始，判断 pending 下是否阻断绕行工具；如果没有这段函数，读日志跑偏 bug 会复发。
    ...
    if pending.get("marker") == DESKTOP_TASK_INCOMPLETE_MARKER or str(pending.get("actionability_kind", "") or "") == "desktop_task_incomplete":  # 新增代码+DesktopTaskIncompleteGate：Stage 任务未完成时只允许高层恢复工具；如果没有这行代码，模型会继续调用 key/click/drag 绕开阶段验证。
        return True  # 新增代码+DesktopTaskIncompleteGate：阻断所有非允许工具；如果没有这行代码，复杂 GUI 任务会再次用零散原子动作失控。
...
__all__ = [  # 新增代码+ObservePlanActVerify：公开接口清单段开始；如果没有这段代码，其他模块导入边界不清楚。
    ...
    "DESKTOP_RESOURCE_PREPARATION_OBSERVE_REQUIRED_MARKER",  # 新增代码+ResourcePreparationConvergence：导出资源准备观察 marker；如果没有这行代码，adapter 和测试无法复用稳定协议名。
    "DESKTOP_RESOURCE_USER_ACTION_REQUIRED_MARKER",  # 新增代码+ResourceFreshness：导出旧资源用户动作 marker；如果没有这行代码，测试和 adapter 无法复用稳定协议名。
    "DESKTOP_TASK_INCOMPLETE_MARKER",  # 新增代码+DesktopTaskIncompleteGate：导出桌面任务未完成 marker；如果没有这行代码，desktop_task 工具和测试无法复用稳定协议名。
