...
## Universal Computer Use Stage Runtime (2026-06-19)

Computer Use full 模式新增通用 Stage Runtime。它不是 Notepad、Paint 或浏览器的专用自动化脚本，而是把用户的桌面任务拆成通用阶段：准备目标窗口、探测窗口能力、批量执行内容工作、提交或保存资源、观察验证结果。

这个运行时用 `DesktopTaskPlan` 表达完整任务，用 `StagePlan` 表达阶段，用 `ActionBatch` 把同一阶段中的多次键盘、鼠标、拖动或保存动作合并执行。这样复杂任务不需要每一个原子动作都重新 observe 和重新问模型，速度会比旧的原子循环更适合真实本地软件压力测试。

目标窗口通过 `target_ref` 绑定。多软件任务会有多个目标绑定，但每个写动作批次只能写入一个目标窗口。如果发现用户已有窗口，默认要求用户关闭或授权，避免把用户旧文档、旧聊天窗口或旧浏览器标签误当成 agent 新目标。

最终回答会读取阶段证据。只有 `desktop_task_completed=true`、阶段数完成、批执行和阶段观察证据成立时，agent 才能把 Computer Use 桌面任务报告为完成；否则必须继续调用桌面工具，或者明确返回需要用户处理的原因。
...
- Tool Catalog / Tool Pool / Tool Policy 分层：完整工具目录仍由 Python 内部保存，但模型首轮 Tool Pool 暴露 `read / write / edit / bash / tool_search` 五个基础工具；其他能力通过 `learning_agent/skills/tool_list.md` -> `SKILL.md` -> `rules/*.md` 三级动态规则树或 `tool_search` 按需发现后再执行。
...
第三步，`run()` 调用 `self.model.chat(messages, 当前 Tool Pool)`。其中 `messages` 是当前对话历史，当前首轮 Tool Pool 默认包含 `read / write / edit / bash / tool_search` 五个基础工具的说明书。
...
为了让工具数量继续增长后仍然可控，当前 agent 已经进入 Tool Architecture v2 的极简首轮工具面。完整 `Tool Catalog` 仍由 Python 内部保存所有内置工具、MCP 工具和历史兼容入口，但当前 `Tool Pool` 默认把 `read / write / edit / bash / tool_search` 五个基础工具 schema 暴露给模型。

这一次的重点是在保留可读文件发现路线的同时恢复 ClaudeCode 风格工具发现入口：模型需要更多能力时，可以先用 `read` 读取 `learning_agent/skills/tool_list.md`，也可以用 `tool_search` 搜索完整 Tool Catalog。`SKILL.md` 只做能力判断和子规则索引，真正细节下沉到 `rules/*.md`，只有任务确实需要时才继续读取；真正的文件读取、写入、编辑和命令执行仍通过 `read / write / edit / bash` 完成。旧的 `skill_list` / `skill_load` 仍保留为内部兼容能力，但 `tool_search` 已恢复为默认首轮模型可见入口。
...
这个配置会启动名为 `browser_search`、`workspace_tools` 和 `browser_automation` 的三个 MCP server。用户同意启动后，完整 Tool Catalog 会包含这些 MCP 工具；它们不会再默认全部挤进当前 Tool Pool。默认首轮模型看到 `read / write / edit / bash / tool_search`，需要 MCP 细节时可以先读取 `learning_agent/skills/tool_list.md`，再读取 `mcp`、`browser_automation`、`real_chrome` 或 `file_operations` 等对应 skill，也可以用 `tool_search` 搜索和选择 deferred 工具：
...
返回 JSON 会包含 agent 最终回答和当前可见工具。当前首轮可见工具仍应是 `read / write / edit / bash / tool_search`。
