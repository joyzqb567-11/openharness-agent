...
from learning_agent.app.gui_provider_settings import GuiProviderSettingsError, build_provider_settings_payload, disconnect_provider, save_custom_provider, set_model_visibility, set_provider_auth, test_provider_connection  # 新增代码+ProviderSettings：导入 Provider 设置 helper、mutation 和连接探针；如果没有这行，bridge 无法返回或修改 provider/model 设置页数据。
...
class GuiRunManager:  # 新增代码+DesktopGUILifecycle：类段开始，管理 GUI turn 生命周期；如果没有这个类，handler 会堆满业务状态逻辑。
    def __init__(self, workspace: str | Path, answer_runner: GuiAnswerRunner | None = None, agent_adapter: GuiAgentAdapter | None = None) -> None:  # 修改代码+GuiAgentAdapter：初始化 run manager 并允许注入 V2 adapter；如果没有这个参数，bridge 无法从 fake adapter 平滑升级到真实 harness adapter。
        ...
        self.agent_adapter = agent_adapter or self._default_agent_adapter()  # 修改代码+真实模型GUI桥接：按环境变量选择 fake 或 real adapter；如果没有这一行，real 模式打开后仍会 fake streaming。
    ...
    def _default_agent_adapter(self) -> GuiAgentAdapter:  # 新增代码+真实模型GUI桥接：函数段开始，按模式创建默认 GUI adapter；如果没有这段，__init__ 会堆入真实/fake 分支细节。
        if self.agent_adapter_mode == "real" or (self.agent_adapter_mode == "auto" and self._openai_real_provider_connected("openai")):  # 修改代码+ComposerRealModelRouting：显式 real 或 auto 且 OpenAI 已连接时启用真实模型；如果没有这一行，登录成功后主聊天仍会 fake。
            self.agent_adapter_mode = "real"  # 新增代码+真实模型GUI桥接：把 auto 命中的真实模式固化到本 manager；如果没有这一行，_run_adapter_turn 会把请求误标成 fake。
            return RealModelGuiAgentAdapter(connected_provider_reader=self._openai_real_provider_connected)  # 新增代码+真实模型GUI桥接：创建带 provider 门禁的真实 adapter；如果没有这一行，OpenHarness Desktop 无法接到 CodexCliChatModel。
        ...
        return FakeStreamingGuiAgentAdapter()  # 修改代码+真实模型GUI桥接：默认仍返回 deterministic fake adapter；如果没有这一行，普通 GUI smoke 会变得依赖真实模型。
    ...
    def _adapter_for_turn(self, turn: GuiTurn) -> GuiAgentAdapter:  # 新增代码+GuiAgentAdapter：函数段开始，选择本轮使用 fake adapter 还是真实 harness shell；如果没有这段，真实模式触发语义会散落在 worker 中。
        ...
        if self.agent_adapter_mode == "auto" and self._openai_real_provider_connected(turn.provider_id or "openai"):  # 新增代码+ComposerRealModelRouting：auto 模式每轮重新检查 provider 是否刚完成 OAuth；如果没有这一行，窗口启动后再连接仍会继续 fake。
            return RealModelGuiAgentAdapter(connected_provider_reader=self._openai_real_provider_connected)  # 新增代码+ComposerRealModelRouting：为本轮创建真实模型 adapter；如果没有这一行，OAuth 连接后的下一条消息无法进入真实模型。
    ...
    def _run_adapter_turn(self, turn: GuiTurn) -> GuiAgentRunResult:  # 新增代码+GuiAgentAdapter：函数段开始，把 GUI turn 转交给 adapter；如果没有这段，worker 只能调用旧 answer_runner。
        ...
        request_mode = "real" if isinstance(adapter, (DefaultHarnessGuiAgentAdapter, RealModelGuiAgentAdapter)) or self.agent_adapter_mode == "real" else "fake"  # 修改代码+ComposerRealModelRouting：根据本轮 adapter 类型标记请求模式；如果没有这一行，auto 动态切到真实 adapter 时事件会被误标成 fake。
