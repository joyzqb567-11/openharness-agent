...
def _codex_chatgpt_oauth_model_rows(settings: dict[str, Any]) -> list[dict[str, Any]]:  # 新增代码+CodexOAuthModels：函数段开始，生成 Codex ChatGPT OAuth 真实可选模型；如果没有这段，GUI 可能继续显示 ChatGPT 账号不支持的模型。
    ...
    return [_model("openai", model_id, display_name, _visibility(settings, "openai", model_id, True), True, True) for model_id, display_name in model_pairs]  # 新增代码+CodexOAuthModels：把模型元数据转换成 provider catalog 行；如果没有这行，Composer 底部菜单拿不到可选模型。
...
def _openai_model_rows(settings: dict[str, Any], auth_mode: str) -> list[dict[str, Any]]:  # 新增代码+CodexOAuthModels：函数段开始，按 OpenAI 认证模式选择模型列表；如果没有这段，API key 和 ChatGPT OAuth 会混用不兼容模型。
    ...
    return [_model("openai", "gpt-4.1", "GPT-4.1", _visibility(settings, "openai", "gpt-4.1", True), True, True)]  # 新增代码+CodexOAuthModels：API key/mock 路径继续展示普通 OpenAI API 模型；如果没有这行，未启用 OAuth 时模型列表会不稳定。
