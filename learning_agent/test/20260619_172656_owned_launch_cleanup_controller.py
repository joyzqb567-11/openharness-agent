...
import re  # 新增代码+ControlledResourceLaunch：解析用户任务中的桌面 1.txt 这类受控文档资源名；如果没有这一行，controller 只能裸启动文档应用。
import signal  # 新增代码+OwnedLaunchCleanup：用于终止 agent 自己启动的残留进程；如果没有这一行，cleanup 回调无法跨平台请求进程退出。
import subprocess  # 新增代码+OwnedLaunchCleanup：用于 Windows taskkill 兜底关闭 agent-owned 进程树；如果没有这一行，WM_CLOSE/TERM 失败后 Paint 仍可能残留。
import sys  # 新增代码+Phase8ProductionEdges: 判断当前平台是否为 Windows；如果没有这行代码，非 Windows 环境可能误报支持真实桌面控制。
import time  # 新增代码+OwnedLaunchCleanup：用于短轮询等待 agent-owned 进程退出；如果没有这一行，cleanup 可能过早报告完成。
...
DESKTOP_USER_ACTION_REQUIRED_MARKER_TEXT = "OPENHARNESS_DESKTOP_USER_ACTION_REQUIRED"  # 新增代码+FreshTargetUserAction：本地保存旧窗口用户动作 marker 文本；如果没有这行代码，controller 的拒绝输出无法触发收敛器终止态。
CONTROLLED_RESOURCE_ARGV_TEXT_APPS = {"notepad", "notepad.exe"}  # 新增代码+ControlledResourceLaunch：声明哪些文档类应用已验证支持“应用 + 文件路径”argv 启动；如果没有这一行，未知应用可能被错误塞入文件参数。
CONTROLLED_RESOURCE_TEXT_FILENAME_RE = re.compile(r"(?<![A-Za-z0-9_.\\/-])([A-Za-z0-9][A-Za-z0-9 _.-]{0,80}\.txt)(?![A-Za-z0-9_.\\/-])", re.IGNORECASE)  # 新增代码+ControlledResourceLaunch：匹配 1.txt 这类安全文本文件名；如果没有这一行，controller 无法从自然语言任务里抽出目标资源。


def _owned_launch_int(value: Any, default: int = 0) -> int:  # 新增代码+OwnedLaunchCleanup：函数段开始，把 pid/hwnd 这类外部字段安全转成整数；如果没有这段函数，坏窗口字段会让 cleanup 崩溃。
    try:  # 新增代码+OwnedLaunchCleanup：尝试按整数解析字段；如果没有这一行，字符串 pid 无法进入进程清理。
        return int(str(value or "").removeprefix("hwnd:"))  # 新增代码+OwnedLaunchCleanup：兼容 hwnd:123 和普通数字；如果没有这一行，窗口句柄文本无法用于 WM_CLOSE。
    except (TypeError, ValueError):  # 新增代码+OwnedLaunchCleanup：捕获空值和非数字文本；如果没有这一行，异常会中断 launch_app 成功路径。
        return default  # 新增代码+OwnedLaunchCleanup：坏值回退为默认值；如果没有这一行，调用方拿不到安全兜底。
# 新增代码+OwnedLaunchCleanup：函数段结束，_owned_launch_int 到此结束；如果没有这个边界说明，用户不容易看出整数清洗范围。


def _owned_launch_process_alive(process_id: Any) -> bool:  # 新增代码+OwnedLaunchCleanup：函数段开始，检查 agent-owned 进程是否仍存活；如果没有这段函数，residual_owned_process 只能靠猜。
    safe_pid = _owned_launch_int(process_id)  # 新增代码+OwnedLaunchCleanup：先把 pid 清洗成整数；如果没有这一行，字符串 pid 可能导致系统调用失败。
    if safe_pid <= 0:  # 新增代码+OwnedLaunchCleanup：无效 pid 直接视为无残留；如果没有这一行，0 pid 可能误查当前进程或报错。
        return False  # 新增代码+OwnedLaunchCleanup：返回没有存活进程；如果没有这一行，后续会继续执行无意义系统调用。
    if sys.platform == "win32":  # 新增代码+OwnedLaunchCleanup：Windows 使用 OpenProcess/GetExitCodeProcess 检查真实进程；如果没有这一行，os.kill(pid, 0) 在 Windows 上不够稳定。
        try:  # 新增代码+OwnedLaunchCleanup：保护 ctypes 调用避免低层异常打断 cleanup；如果没有这一行，权限或竞态会让残留检查崩溃。
            import ctypes  # 新增代码+OwnedLaunchCleanup：延迟导入 ctypes 访问 Windows 进程 API；如果没有这一行，无法可靠判断 PID 是否仍活着。
            kernel32 = ctypes.windll.kernel32  # 新增代码+OwnedLaunchCleanup：取得 kernel32 入口；如果没有这一行，OpenProcess 无法调用。
            process_query_limited_information = 0x1000  # 新增代码+OwnedLaunchCleanup：声明最小查询权限；如果没有这一行，OpenProcess 权限会不清晰。
            still_active = 259  # 新增代码+OwnedLaunchCleanup：Windows 用 259 表示进程仍在运行；如果没有这一行，退出码判断没有基准。
            handle = kernel32.OpenProcess(process_query_limited_information, False, safe_pid)  # 新增代码+OwnedLaunchCleanup：打开目标 PID 的查询句柄；如果没有这一行，无法读取退出码。
            if not handle:  # 新增代码+OwnedLaunchCleanup：打开失败通常表示进程不存在或无权访问；如果没有这一行，空句柄会进入 GetExitCodeProcess。
                return False  # 新增代码+OwnedLaunchCleanup：保守视为无可清理残留；如果没有这一行，残留检查可能因空句柄误报。
            try:  # 新增代码+OwnedLaunchCleanup：确保查询后关闭句柄；如果没有这一行，长期任务会泄漏 Windows handle。
                exit_code = ctypes.c_ulong()  # 新增代码+OwnedLaunchCleanup：准备接收进程退出码；如果没有这一行，GetExitCodeProcess 没有输出位置。
                if not kernel32.GetExitCodeProcess(handle, ctypes.byref(exit_code)):  # 新增代码+OwnedLaunchCleanup：读取退出码并检查调用是否成功；如果没有这一行，失败调用可能被当成存活。
                    return False  # 新增代码+OwnedLaunchCleanup：读取失败时不把未知进程当残留；如果没有这一行，残留检查会过度失败。
                return int(exit_code.value) == still_active  # 新增代码+OwnedLaunchCleanup：退出码仍为 STILL_ACTIVE 才说明进程存活；如果没有这一行，已退出进程会被误判残留。
            finally:  # 新增代码+OwnedLaunchCleanup：无论读取成功失败都关闭句柄；如果没有这一行，系统资源会泄漏。
                kernel32.CloseHandle(handle)  # 新增代码+OwnedLaunchCleanup：释放进程查询句柄；如果没有这一行，长任务多次检查会堆积句柄。
        except Exception:  # 新增代码+OwnedLaunchCleanup：兜底处理 ctypes 不可用或系统竞态；如果没有这一行，残留检查可能抛出到模型层。
            return False  # 新增代码+OwnedLaunchCleanup：未知错误保守不报残留，避免误杀非自有进程；如果没有这一行，异常路径没有返回值。
    try:  # 新增代码+OwnedLaunchCleanup：非 Windows 使用 signal 0 检查进程；如果没有这一行，跨平台单元测试无法判断进程状态。
        os.kill(safe_pid, 0)  # 新增代码+OwnedLaunchCleanup：发送空信号只做存在性检查；如果没有这一行，POSIX 下无法确认进程是否存在。
        return True  # 新增代码+OwnedLaunchCleanup：没有异常说明进程可见且仍存在；如果没有这一行，存活进程会被误判已退出。
    except ProcessLookupError:  # 新增代码+OwnedLaunchCleanup：进程不存在时会抛出该异常；如果没有这一行，正常退出会变成错误。
        return False  # 新增代码+OwnedLaunchCleanup：返回无残留；如果没有这一行，已退出进程会被误报。
    except PermissionError:  # 新增代码+OwnedLaunchCleanup：无权限但进程存在时会抛出该异常；如果没有这一行，受限进程会被误判不存在。
        return True  # 新增代码+OwnedLaunchCleanup：权限不足仍说明 PID 存在；如果没有这一行，残留检查可能漏报。
    except OSError:  # 新增代码+OwnedLaunchCleanup：处理其它平台级异常；如果没有这一行，残留检查可能泄漏底层错误。
        return False  # 新增代码+OwnedLaunchCleanup：保守返回无残留；如果没有这一行，函数没有兜底返回。
# 新增代码+OwnedLaunchCleanup：函数段结束，_owned_launch_process_alive 到此结束；如果没有这个边界说明，用户不容易看出残留检查如何确认。


def _owned_launch_post_close_window(hwnd: Any) -> bool:  # 新增代码+OwnedLaunchCleanup：函数段开始，优先向 agent-owned 窗口发送温和关闭请求；如果没有这段函数，cleanup 只能强杀进程。
    safe_hwnd = _owned_launch_int(hwnd)  # 新增代码+OwnedLaunchCleanup：把 hwnd 清洗成整数；如果没有这一行，hwnd:123 无法传给 Win32 API。
    if sys.platform != "win32" or safe_hwnd <= 0:  # 新增代码+OwnedLaunchCleanup：只在 Windows 且 hwnd 有效时发送 WM_CLOSE；如果没有这一行，非 Windows 测试会调用不存在的 API。
        return False  # 新增代码+OwnedLaunchCleanup：返回未发送关闭消息；如果没有这一行，调用方无法知道是否走了温和路径。
    try:  # 新增代码+OwnedLaunchCleanup：保护 user32 调用；如果没有这一行，窗口已消失时的竞态可能中断 cleanup。
        import ctypes  # 新增代码+OwnedLaunchCleanup：延迟导入 ctypes 访问 user32；如果没有这一行，无法发送窗口关闭消息。
        wm_close = 0x0010  # 新增代码+OwnedLaunchCleanup：定义 Windows 关闭窗口消息；如果没有这一行，魔法数字含义不清楚。
        return bool(ctypes.windll.user32.PostMessageW(safe_hwnd, wm_close, 0, 0))  # 新增代码+OwnedLaunchCleanup：异步发送关闭消息并返回是否投递成功；如果没有这一行，窗口无法优雅收尾。
    except Exception:  # 新增代码+OwnedLaunchCleanup：发送失败时返回 False 而不是抛异常；如果没有这一行，cleanup 会被窗口竞态打断。
        return False  # 新增代码+OwnedLaunchCleanup：返回温和关闭未成功；如果没有这一行，函数没有兜底返回。
# 新增代码+OwnedLaunchCleanup：函数段结束，_owned_launch_post_close_window 到此结束；如果没有这个边界说明，用户不容易看出 WM_CLOSE 范围。


def _owned_launch_wait_for_process_exit(process_id: Any, timeout_seconds: float = 2.0) -> bool:  # 新增代码+OwnedLaunchCleanup：函数段开始，等待 agent-owned 进程退出；如果没有这段函数，cleanup 会在系统还没释放进程时误判。
    deadline = time.monotonic() + max(0.1, float(timeout_seconds))  # 新增代码+OwnedLaunchCleanup：计算等待截止时间且避免 0 秒忙等；如果没有这一行，等待上限不稳定。
    while time.monotonic() < deadline:  # 新增代码+OwnedLaunchCleanup：在截止前重复检查进程状态；如果没有这一行，只检查一次会放大竞态。
        if not _owned_launch_process_alive(process_id):  # 新增代码+OwnedLaunchCleanup：进程已经退出则立即成功；如果没有这一行，函数会无意义等满。
            return True  # 新增代码+OwnedLaunchCleanup：返回已退出；如果没有这一行，调用方不知道等待成功。
        time.sleep(0.05)  # 新增代码+OwnedLaunchCleanup：短睡避免 CPU 空转；如果没有这一行，轮询会浪费计算资源。
    return not _owned_launch_process_alive(process_id)  # 新增代码+OwnedLaunchCleanup：超时后再确认一次最终状态；如果没有这一行，函数没有稳定返回。
# 新增代码+OwnedLaunchCleanup：函数段结束，_owned_launch_wait_for_process_exit 到此结束；如果没有这个边界说明，用户不容易看出等待边界。


def _owned_launch_terminate_process(process_id: Any, hwnd: Any = 0) -> dict[str, Any]:  # 新增代码+OwnedLaunchCleanup：函数段开始，关闭 agent-owned 进程并返回证据；如果没有这段函数，cleanup 无法从“已登记”走到“已关闭”。
    safe_pid = _owned_launch_int(process_id)  # 新增代码+OwnedLaunchCleanup：清洗目标 pid；如果没有这一行，终止调用可能收到坏值。
    if safe_pid <= 0:  # 新增代码+OwnedLaunchCleanup：无效 pid 直接失败；如果没有这一行，cleanup 可能误操作 pid 0。
        return {"cleanup_completed": False, "reason": "missing_process_id", "process_id": safe_pid}  # 新增代码+OwnedLaunchCleanup：返回缺 pid 证据；如果没有这一行，调用方不知道为什么没清理。
    close_message_sent = _owned_launch_post_close_window(hwnd)  # 新增代码+OwnedLaunchCleanup：先尝试温和关闭窗口；如果没有这一行，所有 GUI 应用都会直接强杀。
    if close_message_sent and _owned_launch_wait_for_process_exit(safe_pid, timeout_seconds=2.0):  # 新增代码+OwnedLaunchCleanup：温和关闭后等待进程退出；如果没有这一行，WM_CLOSE 成功也会继续强杀。
        return {"cleanup_completed": True, "cleanup_mode": "wm_close", "process_id": safe_pid, "window_close_message_sent": True}  # 新增代码+OwnedLaunchCleanup：返回温和关闭成功证据；如果没有这一行，日志无法区分关闭方式。
    terminate_error = ""  # 新增代码+OwnedLaunchCleanup：准备记录终止异常；如果没有这一行，失败原因无法回传。
    if _owned_launch_process_alive(safe_pid):  # 新增代码+OwnedLaunchCleanup：只有进程仍存活时才发送终止信号；如果没有这一行，已退出进程会被重复操作。
        try:  # 新增代码+OwnedLaunchCleanup：保护 os.kill 调用；如果没有这一行，权限或竞态会打断 cleanup。
            os.kill(safe_pid, signal.SIGTERM)  # 新增代码+OwnedLaunchCleanup：请求终止 agent-owned 进程；如果没有这一行，WM_CLOSE 失败后无法继续收尾。
        except Exception as error:  # 新增代码+OwnedLaunchCleanup：捕获终止失败原因；如果没有这一行，失败不会结构化返回。
            terminate_error = f"{type(error).__name__}:{error}"  # 新增代码+OwnedLaunchCleanup：保存错误摘要；如果没有这一行，用户不知道为何没终止。
    if _owned_launch_wait_for_process_exit(safe_pid, timeout_seconds=2.0):  # 新增代码+OwnedLaunchCleanup：等待 SIGTERM/TerminateProcess 生效；如果没有这一行，报告可能早于进程退出。
        return {"cleanup_completed": True, "cleanup_mode": "terminate", "process_id": safe_pid, "window_close_message_sent": close_message_sent, "terminate_error": terminate_error}  # 新增代码+OwnedLaunchCleanup：返回终止成功证据；如果没有这一行，cleanup 方式不可审计。
    taskkill_error = ""  # 新增代码+OwnedLaunchCleanup：准备记录 taskkill 兜底异常；如果没有这一行，强制兜底失败原因会丢失。
    if sys.platform == "win32" and _owned_launch_process_alive(safe_pid):  # 新增代码+OwnedLaunchCleanup：Windows 上仍残留时使用 taskkill /T /F 兜底；如果没有这一行，Paint 这类 GUI 子进程树可能继续存在。
        try:  # 新增代码+OwnedLaunchCleanup：保护 taskkill 调用；如果没有这一行，缺命令或超时会抛到模型层。
            subprocess.run(["taskkill", "/PID", str(safe_pid), "/T", "/F"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=5, check=False)  # 新增代码+OwnedLaunchCleanup：强制关闭 agent-owned 进程树且不通过 shell；如果没有这一行，顽固 GUI 残留无法收尾。
        except Exception as error:  # 新增代码+OwnedLaunchCleanup：捕获 taskkill 异常；如果没有这一行，兜底失败没有结构化证据。
            taskkill_error = f"{type(error).__name__}:{error}"  # 新增代码+OwnedLaunchCleanup：保存 taskkill 错误摘要；如果没有这一行，排查只能看外层失败。
    cleanup_completed = not _owned_launch_process_alive(safe_pid)  # 新增代码+OwnedLaunchCleanup：最终以真实进程状态决定是否完成；如果没有这一行，cleanup 可能只因调用过命令就报成功。
    return {"cleanup_completed": cleanup_completed, "cleanup_mode": "taskkill" if cleanup_completed else "cleanup_failed", "process_id": safe_pid, "window_close_message_sent": close_message_sent, "terminate_error": terminate_error, "taskkill_error": taskkill_error}  # 新增代码+OwnedLaunchCleanup：返回最终关闭证据；如果没有这一行，registry 无法把失败写入记录。
# 新增代码+OwnedLaunchCleanup：函数段结束，_owned_launch_terminate_process 到此结束；如果没有这个边界说明，用户不容易看出进程关闭路径。


def _controlled_resource_normalized_app_name(target_app: Any) -> str:  # 新增代码+ControlledResourceLaunch：函数段开始，统一清洗受控资源启动使用的应用名；如果没有这段函数，notepad 和 notepad.exe 会被当成不同能力。
    return str(target_app or "").strip().lower()  # 新增代码+ControlledResourceLaunch：返回小写短应用名；如果没有这一行，大小写或空格会导致能力判断不稳定。
# 新增代码+ControlledResourceLaunch：函数段结束，_controlled_resource_normalized_app_name 到此结束；如果没有这个边界说明，读者不容易看出应用名清洗范围。


def _controlled_resource_app_accepts_text_argv(target_app: Any) -> bool:  # 新增代码+ControlledResourceLaunch：函数段开始，判断目标应用是否支持通过 argv 打开文本资源；如果没有这段函数，controller 会对所有应用盲目加文件参数。
    return _controlled_resource_normalized_app_name(target_app) in CONTROLLED_RESOURCE_ARGV_TEXT_APPS  # 新增代码+ControlledResourceLaunch：只放行已验证的文本 argv 应用；如果没有这一行，微信等单实例应用可能收到错误文件参数。
# 新增代码+ControlledResourceLaunch：函数段结束，_controlled_resource_app_accepts_text_argv 到此结束；如果没有这个边界说明，读者不容易看出能力边界。


def _controlled_resource_context_text(arguments: dict[str, Any]) -> str:  # 新增代码+ControlledResourceLaunch：函数段开始，汇总 open_application 和授权阶段留下的任务上下文；如果没有这段函数，模型在 open_application.reason 漏写 1.txt 时无法恢复用户真实目标。
    context_fields = ("controlled_resource_path", "controlled_resource_name", "reason", "session_task_context", "task_context", "request_reason", "user_prompt", "prompt")  # 新增代码+ControlledResourceLaunch：列出可承载用户资源目标的字段；如果没有这一行，信息来源会散落多处且容易漏掉授权原因。
    return " ".join(str(arguments.get(field_name, "") or "") for field_name in context_fields)  # 新增代码+ControlledResourceLaunch：把所有上下文字段合并成可搜索文本；如果没有这一行，资源提取只能看单个字段。
# 新增代码+ControlledResourceLaunch：函数段结束，_controlled_resource_context_text 到此结束；如果没有这个边界说明，读者不容易看出上下文来源。


def _controlled_resource_desktop_dir() -> str:  # 新增代码+ControlledResourceLaunch：函数段开始，取得当前 Windows 用户桌面目录；如果没有这段函数，测试和生产路径会各自拼接桌面。
    user_profile = str(os.environ.get("USERPROFILE") or os.path.expanduser("~")).strip()  # 新增代码+ControlledResourceLaunch：优先使用 Windows USERPROFILE；如果没有这一行，桌面路径可能跑到错误用户目录。
    return os.path.normpath(os.path.join(user_profile, "Desktop"))  # 新增代码+ControlledResourceLaunch：返回标准化 Desktop 路径；如果没有这一行，后端 argv 会出现不稳定的斜杠或空路径。
# 新增代码+ControlledResourceLaunch：函数段结束，_controlled_resource_desktop_dir 到此结束；如果没有这个边界说明，读者不容易看出桌面路径规则。


def _controlled_resource_explicit_path(arguments: dict[str, Any]) -> str:  # 新增代码+ControlledResourceLaunch：函数段开始，读取调用方已经明确给出的受控资源路径；如果没有这段函数，显式路径会被自然语言解析覆盖。
    explicit_path = str(arguments.get("controlled_resource_path", "") or "").strip().strip("\"'`")  # 新增代码+ControlledResourceLaunch：清理显式路径的空白和包裹引号；如果没有这一行，带引号路径会传给 Notepad 失败。
    if not explicit_path:  # 新增代码+ControlledResourceLaunch：没有显式路径时交给自然语言解析；如果没有这一行，空字符串会被当成路径。
        return ""  # 新增代码+ControlledResourceLaunch：返回空表示无显式资源；如果没有这一行，调用方无法区分未提供和无效。
    normalized_path = os.path.normpath(explicit_path)  # 新增代码+ControlledResourceLaunch：标准化显式路径；如果没有这一行，路径比较和审计输出会漂移。
    if not normalized_path.lower().endswith(".txt"):  # 新增代码+ControlledResourceLaunch：只允许文本资源路径；如果没有这一行，任意文件类型都可能被文档启动链误打开。
        return ""  # 新增代码+ControlledResourceLaunch：非 txt 显式路径保守拒绝；如果没有这一行，资源绑定可能扩大到未验证格式。
    if normalized_path.startswith("\\\\"):  # 新增代码+ControlledResourceLaunch：拒绝 UNC 网络路径；如果没有这一行，Computer Use 可能无意访问网络共享。
        return ""  # 新增代码+ControlledResourceLaunch：网络路径不进入本地桌面控制；如果没有这一行，验收范围会从本机桌面扩大。
    return normalized_path  # 新增代码+ControlledResourceLaunch：返回安全显式路径；如果没有这一行，调用方拿不到受控资源。
# 新增代码+ControlledResourceLaunch：函数段结束，_controlled_resource_explicit_path 到此结束；如果没有这个边界说明，读者不容易看出显式路径规则。


def _controlled_resource_filename_from_context(arguments: dict[str, Any]) -> str:  # 新增代码+ControlledResourceLaunch：函数段开始，从自然语言任务里提取目标 txt 文件名；如果没有这段函数，保存到桌面 1.txt 的意图无法进入启动层。
    context_text = _controlled_resource_context_text(arguments)  # 新增代码+ControlledResourceLaunch：读取合并后的任务上下文；如果没有这一行，解析器没有输入文本。
    if "桌面" not in context_text and "desktop" not in context_text.lower():  # 新增代码+ControlledResourceLaunch：只在用户明确提到桌面时自动解析；如果没有这一行，普通提到 1.txt 的任务也可能被强行绑定到桌面。
        return ""  # 新增代码+ControlledResourceLaunch：没有桌面语义时不自动生成路径；如果没有这一行，资源绑定可能超出用户意图。
    matches = [match.group(1).strip() for match in CONTROLLED_RESOURCE_TEXT_FILENAME_RE.finditer(context_text)]  # 新增代码+ControlledResourceLaunch：提取上下文里的所有安全 txt 文件名；如果没有这一行，controller 无法找到 1.txt。
    if not matches:  # 新增代码+ControlledResourceLaunch：没有文件名时放弃资源绑定；如果没有这一行，后续会用空文件名拼出 Desktop。
        return ""  # 新增代码+ControlledResourceLaunch：返回空表示没有可绑定资源；如果没有这一行，调用方无法安全降级。
    return os.path.basename(matches[-1])  # 新增代码+ControlledResourceLaunch：使用最后一个文件名作为最终保存目标；如果没有这一行，提示词先提旧文件再提 1.txt 时可能选错。
# 新增代码+ControlledResourceLaunch：函数段结束，_controlled_resource_filename_from_context 到此结束；如果没有这个边界说明，读者不容易看出自然语言解析范围。


def _controlled_resource_path_for_launch(target_app: Any, arguments: dict[str, Any]) -> str:  # 新增代码+ControlledResourceLaunch：函数段开始，为 launch_app 计算可传入 argv 的受控资源路径；如果没有这段函数，Notepad 启动仍无法绕开会话恢复旧文档。
    if not _controlled_resource_app_accepts_text_argv(target_app):  # 新增代码+ControlledResourceLaunch：先检查目标应用是否支持文本 argv；如果没有这一行，未知应用可能被错误加路径。
        return ""  # 新增代码+ControlledResourceLaunch：不支持的应用保持原启动方式；如果没有这一行，通用 Computer Use 会因误加参数变脆。
    explicit_path = _controlled_resource_explicit_path(arguments)  # 新增代码+ControlledResourceLaunch：优先读取显式受控路径；如果没有这一行，内部已解析路径可能被自然语言误覆盖。
    if explicit_path:  # 新增代码+ControlledResourceLaunch：检查是否已有安全显式路径；如果没有这一行，显式路径会继续走自然语言解析。
        return explicit_path  # 新增代码+ControlledResourceLaunch：返回调用方明确提供的路径；如果没有这一行，显式资源绑定不会生效。
    filename = _controlled_resource_filename_from_context(arguments)  # 新增代码+ControlledResourceLaunch：从任务上下文提取桌面文件名；如果没有这一行，open_application.reason 漏文件名时无法恢复。
    if not filename:  # 新增代码+ControlledResourceLaunch：没有可解析文件名时保持裸启动；如果没有这一行，空文件名会生成错误路径。
        return ""  # 新增代码+ControlledResourceLaunch：返回空表示不绑定资源；如果没有这一行，controller 无法安全降级。
    return os.path.normpath(os.path.join(_controlled_resource_desktop_dir(), filename))  # 新增代码+ControlledResourceLaunch：拼出本机桌面目标文件路径；如果没有这一行，Notepad 后端拿不到要打开的 1.txt。
# 新增代码+ControlledResourceLaunch：函数段结束，_controlled_resource_path_for_launch 到此结束；如果没有这个边界说明，读者不容易看出受控资源启动范围。
...
class ComputerUseController:  # 新增代码+OSComputerUse: 在 agent 和后端之间提供安全控制层；若没有这段代码，真实桌面操作缺少统一确认和参数校验。
    ...
    def __init__(self, backend: ComputerUseBackend | None = None, lock_manager: ComputerUseLockManager | None = None, owner_session_id: str = "learning-agent-default-session", auto_acquire_lock: bool | None = None, audit_store: ComputerUseAuditStore | None = None, approval_model: Any | None = None, target_session_runtime: Any | None = None, session_runtime: Any | None = None) -> None:  # 修改代码+OwnedLaunchCleanup：初始化控制器并允许注入同一 Computer Use session runtime；如果没有这个参数，launch_app 和 /computer stop 会继续使用两套资源状态。
        ...
        self.session_runtime = session_runtime  # 新增代码+OwnedLaunchCleanup：保存和 /computer stop 共用的 session runtime；如果没有这一行，controller 无法把 agent-owned launch 登记给 cleanup。
        self.owned_resource_registry = getattr(session_runtime, "owned_resource_registry", None) if session_runtime is not None else None  # 新增代码+OwnedLaunchCleanup：读取 runtime 内的自有资源注册表；如果没有这一行，stop 无法知道 controller 本轮启动了哪个进程。
    ...
    def _cleanup_agent_owned_process_record(self, record: dict[str, Any]) -> bool:  # 新增代码+OwnedLaunchCleanup：函数段开始，关闭 registry 中登记的 agent-owned 进程；如果没有这段函数，/computer stop 只能改状态不能关闭本轮启动的软件。
        process_id = _owned_launch_int(record.get("process_id", 0))  # 新增代码+OwnedLaunchCleanup：从 registry 记录读取 pid；如果没有这一行，cleanup 不知道要关闭哪个进程。
        metadata = dict(record.get("metadata", {}) or {}) if isinstance(record.get("metadata", {}), dict) else {}  # 新增代码+OwnedLaunchCleanup：读取 launch 阶段保存的窗口元数据；如果没有这一行，无法找到 hwnd 做温和关闭。
        window_id = metadata.get("window_id", record.get("window_id", ""))  # 新增代码+OwnedLaunchCleanup：优先使用元数据里的窗口 id；如果没有这一行，进程记录没有 window_id 时无法 WM_CLOSE。
        hwnd = metadata.get("hwnd", window_id)  # 新增代码+OwnedLaunchCleanup：读取 hwnd 或 hwnd:xxx 文本；如果没有这一行，关闭窗口只能走强杀。
        cleanup_result = _owned_launch_terminate_process(process_id, hwnd=hwnd)  # 新增代码+OwnedLaunchCleanup：执行通用 agent-owned 进程关闭流程；如果没有这一行，记录不会触发真实清理。
        record["cleanup_result"] = dict(cleanup_result)  # 新增代码+OwnedLaunchCleanup：把关闭证据写回 registry 记录；如果没有这一行，验收日志看不到关闭方式和失败原因。
        return bool(cleanup_result.get("cleanup_completed", False))  # 新增代码+OwnedLaunchCleanup：只有真实进程不再存活才告诉 registry 成功；如果没有这一行，调用过关闭命令也会被误报完成。
    # 新增代码+OwnedLaunchCleanup：函数段结束，_cleanup_agent_owned_process_record 到此结束；如果没有这个边界说明，用户不容易看出进程清理回调范围。

    def _agent_owned_process_residual_present(self, record: dict[str, Any]) -> bool:  # 新增代码+OwnedLaunchCleanup：函数段开始，检查 registry 中登记的 agent-owned 进程是否仍残留；如果没有这段函数，cleanup 后 residual_owned_process 会再次变成空猜测。
        process_id = _owned_launch_int(record.get("process_id", 0))  # 新增代码+OwnedLaunchCleanup：从记录读取 pid；如果没有这一行，残留检查不知道目标。
        return _owned_launch_process_alive(process_id)  # 新增代码+OwnedLaunchCleanup：以真实系统进程状态返回残留结果；如果没有这一行，报告可能和桌面实际状态矛盾。
    # 新增代码+OwnedLaunchCleanup：函数段结束，_agent_owned_process_residual_present 到此结束；如果没有这个边界说明，用户不容易看出残留检查回调范围。

    def _register_agent_owned_launch_for_cleanup(self, target_app: str, target_window: dict[str, Any], session_report: dict[str, Any], target_ref: str) -> dict[str, Any]:  # 新增代码+OwnedLaunchCleanup：函数段开始，把 launch_app 新建的 agent-owned 进程登记到 stop/cleanup；如果没有这段函数，真实终端会继续留下 Paint。
        registry = self.owned_resource_registry  # 新增代码+OwnedLaunchCleanup：读取和 session runtime 共用的 registry；如果没有这一行，登记会写不到 /computer stop 使用的事实源。
        if registry is None or not hasattr(registry, "register_owned_process"):  # 新增代码+OwnedLaunchCleanup：缺 registry 时返回可审计跳过；如果没有这一行，老测试或独立 controller 会崩溃。
            return {"registered": False, "reason": "owned_resource_registry_missing"}  # 新增代码+OwnedLaunchCleanup：说明没有登记是因为缺少注册表；如果没有这一行，结果字段会不稳定。
        fresh_target_class = str(session_report.get("fresh_target_class", "") or "")  # 新增代码+OwnedLaunchCleanup：读取 FreshTarget 分类；如果没有这一行，无法区分新窗口和用户授权旧窗口。
        if fresh_target_class == "user_granted_existing_window" or bool(session_report.get("target_window_existed_before_launch", False)):  # 新增代码+OwnedLaunchCleanup：用户授权或启动前已存在的窗口不能自动清理；如果没有这一行，微信等旧窗口可能被 stop 关闭。
            return {"registered": False, "reason": "not_agent_owned_fresh_window", "fresh_target_class": fresh_target_class}  # 新增代码+OwnedLaunchCleanup：返回跳过原因；如果没有这一行，授权旧窗口路径难以审计。
        if session_report.get("owned_process_registered") is False:  # 新增代码+OwnedLaunchCleanup：尊重启动 runtime 明确声明非自有进程；如果没有这一行，单实例应用可能被误登记。
            return {"registered": False, "reason": "runtime_reported_not_owned_process"}  # 新增代码+OwnedLaunchCleanup：返回非自有跳过原因；如果没有这一行，结果里看不出为何未登记。
        process_id = _owned_launch_int(target_window.get("window_process_id") or target_window.get("pid") or session_report.get("process_id"))  # 新增代码+OwnedLaunchCleanup：优先使用真实窗口 PID，其次使用启动报告 PID；如果没有这一行，launcher pid 和真实窗口 pid 不一致时会清错对象。
        if process_id <= 0:  # 新增代码+OwnedLaunchCleanup：没有可信 pid 时不能登记清理；如果没有这一行，cleanup 可能误操作无效进程。
            return {"registered": False, "reason": "missing_process_id"}  # 新增代码+OwnedLaunchCleanup：返回缺 pid 原因；如果没有这一行，排查只能看空记录。
        window_id = str(target_window.get("window_id") or (f"hwnd:{target_window.get('hwnd')}" if target_window.get("hwnd") else ""))  # 新增代码+OwnedLaunchCleanup：保存窗口 id 方便审计和 WM_CLOSE；如果没有这一行，记录里只有 pid 难以对应桌面窗口。
        executable = str(target_window.get("process_name") or session_report.get("process_executable") or target_app)  # 新增代码+OwnedLaunchCleanup：保存进程名或应用名；如果没有这一行，cleanup 报告看不出关闭了哪个软件。
        metadata = {"target_app": target_app, "target_ref": target_ref, "window_id": window_id, "hwnd": target_window.get("hwnd", ""), "title_preview": target_window.get("title_preview") or target_window.get("title") or "", "origin": "agent_owned_launch"}  # 新增代码+OwnedLaunchCleanup：保存清理审计上下文；如果没有这一行，stop 日志无法解释这条进程记录来自哪个 target。
        record = registry.register_owned_process(process_id, executable=executable, session_id=self.owner_session_id, cleanup_callback=self._cleanup_agent_owned_process_record, residual_check_callback=self._agent_owned_process_residual_present, metadata=metadata)  # 新增代码+OwnedLaunchCleanup：登记进程并绑定关闭/残留回调；如果没有这一行，/computer stop 仍不会处理真实 agent-owned 进程。
        return {"registered": True, "record": dict(record), "process_id": process_id, "target_ref": target_ref, "window_id": window_id}  # 新增代码+OwnedLaunchCleanup：返回登记证据给 launch_app 结果；如果没有这一行，真实终端验收无法断言 cleanup 责任链已建立。
    # 新增代码+OwnedLaunchCleanup：函数段结束，_register_agent_owned_launch_for_cleanup 到此结束；如果没有这个边界说明，用户不容易看出 agent-owned 登记边界。
    ...
    def _execute_launch_app_action(self, action: str, arguments: dict[str, Any]) -> ComputerUseActionResult:  # 新增代码+ModelLoopLaunchAppTool: 函数段开始，执行模型主循环的启动应用动作；如果没有这段函数，launch_app 会被错误交给鼠标键盘后端。
        ...
        if not bool(fresh_preflight.get("allowed", False)):  # 新增代码+FreshTargetPolicy：检查预检是否拒绝本次启动；如果没有这一行，拒绝报告会被忽略继续启动。
            existing_target_window_count = int(fresh_preflight.get("existing_target_window_count", 0) or 0)  # 新增代码+FreshTargetUserAction：提取旧窗口数量；如果没有这一行，marker 和 data 会重复写易错表达式。
            requires_user_to_close = bool(fresh_preflight.get("requires_user_to_close_existing_app", False))  # 新增代码+FreshTargetUserAction：提取是否需要用户关闭旧窗口；如果没有这一行，恢复提示可能和策略字段不一致。
            allows_existing_window_authorization = bool(fresh_preflight.get("allows_explicit_existing_window_authorization", False))  # 新增代码+FreshTargetUserAction：提取是否允许用户显式授权；如果没有这一行，单实例应用恢复路径可能丢失。
            fresh_target_decision = str(fresh_preflight.get("decision", ""))  # 新增代码+FreshTargetUserAction：提取 FreshTarget 决策码；如果没有这一行，marker 和 data 可能出现不同原因码。
            user_action_required_lines = [  # 新增代码+FreshTargetUserAction：构造上层可解析的用户动作阻断协议；如果没有这段代码，模型会把旧窗口拒绝当成可重试失败。
                DESKTOP_USER_ACTION_REQUIRED_MARKER_TEXT,  # 新增代码+FreshTargetUserAction：输出稳定 marker；如果没有这一行，actionability_state 无法进入用户动作阻断态。
                "actionability_kind=fresh_target_existing_window_user_action_required",  # 新增代码+FreshTargetUserAction：声明阻断类型；如果没有这一行，收敛器无法区分旧窗口拒绝和普通失败。
                f"target_app={target_app}",  # 新增代码+FreshTargetUserAction：输出目标应用名；如果没有这一行，重复启动门禁不能确认是不是同一软件。
                f"fresh_target_decision={fresh_target_decision}",  # 新增代码+FreshTargetUserAction：输出 FreshTarget 决策码；如果没有这一行，最终提示缺少稳定原因。
                "retry_launch_allowed=false",  # 新增代码+FreshTargetUserAction：声明默认不允许继续 launch；如果没有这一行，模型会重复调用 open_application。
                "next_required_response=ask_user_to_close_or_authorize",  # 新增代码+FreshTargetUserAction：声明下一步必须回答用户；如果没有这一行，模型不知道应该停止工具。
                f"requires_user_to_close_existing_app={str(requires_user_to_close).lower()}",  # 新增代码+FreshTargetUserAction：输出关闭旧窗口恢复条件；如果没有这一行，最终提示不够可操作。
                f"allows_explicit_existing_window_authorization={str(allows_existing_window_authorization).lower()}",  # 新增代码+FreshTargetUserAction：输出显式授权恢复条件；如果没有这一行，微信等单实例应用会缺少安全路径。
                f"existing_target_window_count={existing_target_window_count}",  # 新增代码+FreshTargetUserAction：输出旧窗口数量；如果没有这一行，验收日志无法证明旧窗口命中。
                "low_level_event_count=0",  # 新增代码+FreshTargetUserAction：输出零事件证据；如果没有这一行，安全验收无法确认没有触发鼠标键盘。
            ]  # 新增代码+FreshTargetUserAction：协议行列表结束；如果没有这一行，Python 列表语法不完整。
            data = {"action": action, "action_class": "launch_app", "target_app": target_app, "fresh_target_preflight": dict(fresh_preflight), "fresh_target_decision": fresh_target_decision, "fresh_target_class": str(fresh_preflight.get("fresh_target_class", "")), "existing_target_window_count": existing_target_window_count, "requires_user_to_close_existing_app": requires_user_to_close, "allows_explicit_existing_window_authorization": allows_existing_window_authorization, "fresh_target_user_action_required": True, "retry_launch_allowed": False, "next_required_response": "ask_user_to_close_or_authorize", "actionability_marker": DESKTOP_USER_ACTION_REQUIRED_MARKER_TEXT, "target_ref": "", "target_lease": {}, "target_resolution_source": "fresh_target_preflight_rejected", "target_resolution_error": fresh_target_decision, "recovery_next_allowed_actions": ["ask_user_to_close_or_authorize"], "real_desktop_touched": False, "low_level_event_count": 0}  # 修改代码+FreshTargetUserAction：构造旧窗口预检零事件且不可重试的拒绝数据；如果没有这一行，模型不知道必须让用户关闭或授权。
            message = str(fresh_preflight.get("message") or "检测到目标软件已有窗口，已拒绝默认接管旧窗口。")  # 新增代码+FreshTargetUserAction：准备用户可读拒绝说明；如果没有这一行，marker 输出会缺少自然语言解释。
            result = ComputerUseActionResult(False, f"{message}\n" + "\n".join(user_action_required_lines), data)  # 修改代码+FreshTargetUserAction：返回带 marker 的中文拒绝说明；如果没有这一行，上层收敛器无法从工具文本识别终止态。
        ...
        controlled_resource_path = _controlled_resource_path_for_launch(target_app, arguments)  # 新增代码+ControlledResourceLaunch：从任务上下文提取桌面 1.txt 这类受控文档路径；如果没有这一行，Notepad 会继续裸启动并恢复旧标签页。
        try:  # 新增代码+ControlledResourceLaunch：优先调用支持受控资源的新 runtime 协议；如果没有这一行，旧测试 fake runtime 会因为新参数不兼容而失败。
            session_report = dict(open_method(target_app, user_authorized_window=user_authorized_window, controlled_resource_path=controlled_resource_path))  # 修改代码+ControlledResourceLaunch：把受控资源路径传给通用 session；如果没有这一行，路径不会进入 Phase110 argv 启动计划。
        except TypeError:  # 新增代码+ControlledResourceLaunch：兼容只接受旧参数的测试替身或旧 runtime；如果没有这一行，未迁移测试会被接口扩展打断。
            session_report = dict(open_method(target_app, user_authorized_window=user_authorized_window))  # 新增代码+ControlledResourceLaunch：旧 runtime 降级为原启动方式；如果没有这一行，兼容路径没有返回值。
        if controlled_resource_path:  # 新增代码+ControlledResourceLaunch：只有本轮确实解析到受控资源时才补充报告；如果没有这一行，普通应用启动会多出误导字段。
            session_report.setdefault("controlled_resource_path", controlled_resource_path)  # 新增代码+ControlledResourceLaunch：把受控资源路径写入 session 报告；如果没有这一行，controller 审计看不到源头绑定目标。
            session_report.setdefault("resource_bound_launch", True)  # 新增代码+ControlledResourceLaunch：声明本次启动是资源绑定启动；如果没有这一行，验收无法区分裸启动和文件路径启动。
        ...
        owned_cleanup_registration = {"registered": False, "reason": "launch_not_ready"}  # 新增代码+OwnedLaunchCleanup：先准备默认未登记证据；如果没有这一行，失败启动路径会缺少稳定 cleanup 字段。
        if launch_ok and target_window:  # 修改代码+TargetRegistryRootRemediation: 只在 session 成功且有窗口时注册 active target；如果没有这一行，空窗口也会污染 registry。
            ...
            owned_cleanup_registration = self._register_agent_owned_launch_for_cleanup(target_app, target_window, session_report, target_ref)  # 新增代码+OwnedLaunchCleanup：把新建 agent-owned 进程登记给 /computer stop；如果没有这一行，真实应用会继续残留在桌面。
            session_report["owned_process_cleanup_registration"] = dict(owned_cleanup_registration)  # 新增代码+OwnedLaunchCleanup：把登记证据写回 session 报告；如果没有这一行，终端日志无法解释 cleanup 责任链。
