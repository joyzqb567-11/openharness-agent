...
class LearningAgent:  # 作用: 教学版 agent 主类
    ...
    def _desktop_task_runtime_for_current_run(self) -> Any:  # 新增代码+DesktopTaskRuntimeRoute：函数段开始，取得当前 run 应使用的桌面任务 runtime；如果没有这段函数，测试无法注入 full-mode runtime。
        ...
        try:  # 修改代码+UniversalDesktopAdapter：优先按包路径导入桌面任务 runtime 和通用 adapter；如果没有这一行，项目根运行时无法构造已接通用闭环的默认 runtime。
            ...
            if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.desktop_task_runtime", "learning_agent.computer_use.universal_desktop_execution_loop"}:  # 修改代码+UniversalDesktopAdapter：脚本模式兜底名单加入通用 adapter；如果没有这一行，bat 入口可能因新 adapter 包路径缺失而失败。
        ...
        runtime = ComputerUseDesktopTaskRuntime(base_dir=runtime_root, real_execution_loop=UniversalDesktopExecutionLoopAdapter())  # 修改代码+UniversalLoopSlimming：默认生产路径只接通用 observe-plan-act-verify adapter，不再自动开启 Paint/Pikachu/动物专用桥；如果没有这一行，/computer use --full 会继续把少数硬编码绘图对象误包装成成熟的通用 computer use 能力。
