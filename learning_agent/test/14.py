...
class ComputerUseController:  # 新增代码+OSComputerUse: 在 agent 和后端之间提供安全控制层；若没有这段代码，真实桌面操作缺少统一确认和参数校验。
    ...
    def _reject_invalid_target_lease(self, action: str, arguments: dict[str, Any], target_resolution: dict[str, Any]) -> ComputerUseActionResult | None:  # 新增代码+ControllerTargetLease：函数段开始，在真实后端前执行通用目标租约门禁；如果没有这段函数，target_ref 只代表窗口，不代表有权控制。
        ...
        data = {"action": action, "resolved_target_ref": target_ref or lease.target_ref, "target_lease": lease.to_dict(), "target_lease_verification": verification_report, "target_drift_blocks_action": bool(verification_report.get("target_drift_blocks_action")), "low_level_event_count": 0, "recovery_next_allowed_actions": self._target_resolution_recovery_actions()}  # 新增代码+ControllerTargetLease：构造租约失败零事件数据；如果没有这一行，错窗口拒绝无法展示 expected/current。
    ...
    def _resolve_action_target(self, action: str, arguments: dict[str, Any]) -> dict[str, Any]:  # 新增代码+TargetRegistryRootRemediation: 函数段开始，在真实动作前统一解析 target_ref/window/active target；如果没有这段函数，S2 仍会依赖模型手动复制窗口。
        ...
        active_target = self.target_registry.get_active_target()  # 新增代码+TargetRegistryRootRemediation: 在缺失 window 时读取 session active target；如果没有这一行，S2 漏传 window 无法恢复。
        if isinstance(active_target, dict):  # 新增代码+TargetRegistryRootRemediation: 只有存在 active target 才自动注入；如果没有这一行，空 registry 会触发属性错误。
            active_window = dict(active_target.get("window", {})) if isinstance(active_target.get("window", {}), dict) else dict(active_target.get("raw_window", {}))  # 新增代码+TargetRegistryRootRemediation: 取出 active target 的真实 window；如果没有这一行，后端无法收到目标窗口。
            if active_window:  # 新增代码+TargetRegistryRootRemediation: 确认 active window 非空再注入；如果没有这一行，空窗口会绕过未知窗口门禁。
                resolved_arguments["window"] = active_window  # 新增代码+TargetRegistryRootRemediation: 自动注入 active target window；如果没有这一行，模型漏传 window 仍会失败或裸执行。
                return {"ok": True, "arguments": resolved_arguments, "source": "active_session", "target_ref": str(active_target.get("target_ref", "")), "target_window_present": True, "error": "", "recovery_next_allowed_actions": []}  # 新增代码+TargetRegistryRootRemediation: 返回 active session 注入报告；如果没有这一行，S2 无法证明目标由 session 托管。
    ...
    def _execute_launch_app_action(self, action: str, arguments: dict[str, Any]) -> ComputerUseActionResult:  # 新增代码+ModelLoopLaunchAppTool: 函数段开始，执行模型主循环的启动应用动作；如果没有这段函数，launch_app 会被错误交给鼠标键盘后端。
        ...
        data = {"action": action, "action_class": "launch_app", "target_app": target_app, "session": session_report, "target_ref": target_ref, "target_lease": target_lease, "resolved_target_ref": target_ref, "resolved_target_window_present": bool(target_window), "target_resolution_source": "launch_app" if launch_ok and target_window else "launch_app_failed", "target_resolution_error": "" if launch_ok and target_window else "target_window_missing", "recovery_next_allowed_actions": [] if launch_ok and target_window else self._target_resolution_recovery_actions(), "target_window": target_window, "proxy_window_bound": bool(session_report.get("proxy_window_bound")), "proxy_window_binding": session_report.get("proxy_window_binding", {}), "window_binding_reason": str(session_report.get("window_binding_reason", "")), "window_binding_confidence": str(session_report.get("window_binding_confidence", "")), "external_app_window_bound": bool(launch_ok and target_window), "real_desktop_touched": bool(session_report.get("real_desktop_touched")), "low_level_event_count": int(session_report.get("low_level_event_count", 0) or 0)}  # 修改代码+ControllerTargetLease：生成带 target_ref、target_lease 和代理窗口绑定证据的启动结果；如果没有这一行，模型不知道下一步可用短 ref，也看不到控制权限来源。
