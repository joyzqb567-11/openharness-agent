...
def stage_plan_to_dict(stage: StagePlan) -> dict[str, Any]:  # 新增代码+UniversalStageModels：函数段开始，序列化阶段计划；如果没有这段函数，阶段证据无法进入 JSON。
    return {"stage_id": stage.stage_id, "stage_kind": stage.stage_kind, "target_ref": stage.target_ref, "observation_policy": stage.observation_policy, "batch": action_batch_to_dict(stage.batch) if stage.batch is not None else None, "verifier": dict(stage.verifier), "repair_policy": dict(stage.repair_policy)}  # 新增代码+UniversalStageModels：返回阶段字典；如果没有这行代码，运行报告会丢失阶段结构。
...
def stage_plan_from_dict(payload: Mapping[str, Any]) -> StagePlan:  # 新增代码+UniversalStageModels：函数段开始，反序列化阶段计划；如果没有这段函数，落盘计划无法恢复。
    return StagePlan(stage_id=payload.get("stage_id", ""), stage_kind=payload.get("stage_kind", ""), target_ref=payload.get("target_ref", ""), observation_policy=payload.get("observation_policy", "before_and_after_stage"), batch=action_batch_from_dict(payload.get("batch")), verifier=dict(payload.get("verifier", {}) or {}), repair_policy=dict(payload.get("repair_policy", {}) or {}))  # 新增代码+UniversalStageModels：构造阶段计划；如果没有这行代码，阶段恢复会停在 dict。
