...
try:  # 新增代码+AppSplit: 优先按包路径导入轮次格式化和验收事件 helper；若没有这行代码，包运行模式下交互层无法复用通用能力。
    ...
    if error.name not in {"learning_agent", "learning_agent.app", "learning_agent.app.chrome_status_renderer", "learning_agent.app.status_renderer", "learning_agent.browser_extension_host", "learning_agent.browser_extension_host.bridge_server", "learning_agent.browser_extension_host.manifest_installer", "learning_agent.core", "learning_agent.core.config", "learning_agent.observability", "learning_agent.observability.acceptance_events", "learning_agent.observability.run_records", "learning_agent.runtime", "learning_agent.runtime.status_snapshot", "learning_agent.sdk", "learning_agent.sdk.status"}:  # 修改代码+Phase15ChromePairingTrigger: 允许 bridge server 在脚本模式 fallback；若没有这行代码，bat 入口下 pairing-start-confirm 可能导入失败。
...
def run_chrome_terminal_command(workspace: Path, user_input: str, registry_adapter: Any | None = None) -> str:  # 修改代码+Phase10ChromeInstallConfirm: 执行 `/chrome` 子命令并允许测试注入 fake registry；如果没有这段函数，终端动作无法被单元测试和真实终端复用。
    ...
    return f"Chrome Action\n- 不支持的 /chrome 子命令。\n- 可用命令：/chrome、/chrome install-preview [extension_id]、/chrome install-confirm <extension_id> {CHROME_INSTALL_CONFIRM_TOKEN}、/chrome repair、/chrome pairing-diagnose、/chrome pairing-preview、/chrome pairing-start-confirm {CHROME_PAIRING_CONFIRM_TOKEN}、/chrome uninstall-preview、/chrome uninstall-confirm {CHROME_UNINSTALL_CONFIRM_TOKEN}\n"  # 修改代码+Phase15ChromePairingTrigger: 给未知命令返回包含配对触发的清楚用法；如果没有这行代码，输错命令会被误发给模型或无响应。
