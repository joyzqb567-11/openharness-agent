...
def _phase75_import_contracts() -> dict[str, Any]:  # 新增代码+Phase75HumanlikeMatrix: 函数段开始，延迟导入 Phase65-74 合同入口；如果没有这段函数，模块导入时会急切加载所有阶段并增加循环依赖风险。
    ...
    from learning_agent.computer_use.app_window_control import phase69_cli_line, run_phase69_app_window_control_contract  # 新增代码+Phase75HumanlikeMatrix: 导入 Phase69 app/window 合同；如果没有这行代码，最终矩阵无法覆盖应用启动和窗口聚焦。
