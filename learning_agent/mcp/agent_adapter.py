...
try:  # 修改代码+ComputerUseMcpV2：优先按包路径导入 v2 agent-side wrapper；如果没有这一段，mcp__computer-use__ 工具会继续走旧 session adapter。
    from learning_agent.computer_use_mcp_v2.claudecode_bridge.wrapper import execute_agent_side_tool as computer_use_mcp_v2_execute_agent_side_tool  # 修改代码+ComputerUseMcpV2：导入 v2 agent 侧执行入口；如果没有这一行，agent 主循环无法把 MCP 工具接到 v2 runtime。
    ...
    from computer_use_mcp_v2.claudecode_bridge.wrapper import execute_agent_side_tool as computer_use_mcp_v2_execute_agent_side_tool  # type: ignore  # 修改代码+ComputerUseMcpV2：脚本模式导入 v2 agent 侧执行入口；如果没有这一行，bat 启动时 Computer Use MCP v2 不能执行。
...
def execute_computer_use_mcp_tool(agent: Any, tool_call: ToolCall, safe_arguments: dict[str, Any]) -> str:  # 修改代码+ComputerUseMcpV2：函数段开始，执行 v2 agent-side Computer Use MCP 工具；如果没有这段函数，mcp__computer-use__ 会继续落回旧 executor。
    try:  # 修改代码+ComputerUseMcpV2：捕获 v2 runtime 绑定和工具执行异常；如果没有这一行，单个桌面 MCP 工具错误会中断整个 agent。
        ...
        result_text = computer_use_mcp_v2_execute_agent_side_tool(agent, tool_call.name, safe_arguments)  # 修改代码+ComputerUseMcpV2：调用 v2 wrapper 绑定 agent 回调并执行工具；如果没有这一行，v2 session context、trace、permission 都不会接入主循环。
        record_mcp_call_progress(agent, tool_call, "completed", safe_arguments, f"agent-side v2 chars={len(result_text)}")  # 修改代码+ComputerUseMcpV2：记录 v2 工具完成和输出长度；如果没有这一行，长任务审计无法确认 v2 执行结果。
