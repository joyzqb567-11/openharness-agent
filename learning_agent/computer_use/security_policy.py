...
DESKTOP_ACTIONS: tuple[str, ...] = ("move", "move_mouse", "click", "double_click", "scroll", "press_key", "type_text")  # 新增代码+Phase48WindowsSecurityPolicy: 定义普通桌面写动作集合；如果没有这行代码，点击和输入无法走 desktopAction grant。
...
def _is_system_key_combo(action: str, arguments: dict[str, Any]) -> bool:  # 新增代码+Phase48WindowsSecurityPolicy: 函数段开始，判断动作是否属于系统组合键；如果没有这段函数，危险快捷键无法被额外拦截。
    key_text = _safe_text(dict(arguments or {}).get("key", ""), 160).lower()  # 新增代码+Phase48WindowsSecurityPolicy: 读取并规范化 key 参数；如果没有这行代码，Ctrl+Alt+Delete 的大小写变体会漏检。
    return action == "press_key" and any(token in key_text for token in SYSTEM_KEY_COMBO_TOKENS)  # 新增代码+Phase48WindowsSecurityPolicy: 命中系统键 token 时返回 true；如果没有这行代码，系统快捷键会被当作普通按键。
...
def run_phase48_security_policy_contract() -> dict[str, Any]:  # 新增代码+Phase48WindowsSecurityPolicy: 函数段开始，运行无副作用合同自检；如果没有这段函数，真实终端无法快速验收 Phase48。
    ...
    from learning_agent.computer_use.approval import WindowsComputerUseApprovalModel  # 新增代码+Phase48WindowsSecurityPolicy: 延迟导入审批模型避免模块循环；如果没有这行代码，自检无法证明 controller 审批链能接入策略。
