...
class Phase97WindowsNotepadLiveEditDriver:  # 新增代码+Phase97ControlledNotepadLiveEdit：类段开始，实现 Task3 可注入 Windows Notepad live edit driver；如果没有这个类，Phase97 真实编辑路径仍停留在占位失败。
    def __init__(self, launcher: Any | None = None, window_probe: Any | None = None, focuser: Any | None = None, sender: Any | None = None, verifier: Any | None = None, recheck_attempts: int = 20, recheck_interval_seconds: float = 0.25) -> None:  # 修改代码+Phase97ControlledNotepadLiveEdit：函数段开始，允许测试注入桌面依赖并配置窗口复核轮询；如果没有这段函数，单测会被迫启动真实进程或真实 Notepad 启动稍慢就失败。
