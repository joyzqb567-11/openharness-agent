...
PHASE49_OPERATION_VALUES: tuple[str, ...] = ("status", "discover", "observe", "action", "mode", "run_prompt")  # 修改代码+ComputerDiscoverTool：把只读应用发现加入兼容入口枚举；如果没有这行代码，operation=discover 会被兼容层拒绝。
...
def normalize_computer_use_compat_arguments(arguments: dict[str, Any]) -> dict[str, Any]:  # 新增代码+Phase49ComputerUseToolSurface: 函数段开始，把兼容工具参数转成旧三工具调用；如果没有这段函数，兼容工具只能另写旁路逻辑。
    ...
    if operation == "discover":  # 新增代码+ComputerDiscoverTool：识别只读应用发现；如果没有这行代码，兼容入口无法转发到 computer_discover。
        discover_arguments = {"query": _safe_text(raw_arguments.get("query", raw_arguments.get("target_app", raw_arguments.get("app_name", ""))), limit=200)}  # 新增代码+ComputerDiscoverTool：读取查询词并兼容 target_app/app_name；如果没有这行代码，模型通过兼容入口查应用时拿不到用户意图。
        if "max_results" in raw_arguments:  # 新增代码+ComputerDiscoverTool：只有模型提供数量时才透传；如果没有这行代码，空数量字段会污染默认值。
            discover_arguments["max_results"] = raw_arguments.get("max_results")  # 新增代码+ComputerDiscoverTool：透传最大候选数；如果没有这行代码，兼容入口无法控制 discover 输出长度。
        if "include_common" in raw_arguments:  # 新增代码+ComputerDiscoverTool：只有模型提供公共兜底开关时才透传；如果没有这行代码，默认兜底语义可能被空值覆盖。
            discover_arguments["include_common"] = raw_arguments.get("include_common")  # 新增代码+ComputerDiscoverTool：透传公共兜底开关；如果没有这行代码，兼容入口无法决定是否包含 Paint 等兜底。
        return {"ok": True, "target_tool": "computer_discover", "arguments": discover_arguments, "operation": operation, "tool_surface": PHASE49_TOOL_SURFACE_CONTRACT}  # 新增代码+ComputerDiscoverTool：返回只读 discover 工具调用描述；如果没有这行代码，operation=discover 没有正式转发目标。
