...
def _capture_result(**kwargs: Any) -> Any:  # 新增代码+Phase36WindowsWGCProvider: 函数段开始，创建统一 NativeWindowCaptureResult；如果没有这段函数，WGC provider 会和 GDI 截图合同分裂。
    from learning_agent.computer_use.native_helper import NativeWindowCaptureResult  # 新增代码+Phase36WindowsWGCProvider: 延迟导入截图结果避免 native_diagnostics 循环导入；如果没有这行代码，WGC 与 native_helper 可能互相卡住。
...
def _coerce_capture_result(raw_result: Any) -> Any:  # 新增代码+Phase36WindowsWGCProvider: 函数段开始，把注入实现结果转为统一截图合同；如果没有这段函数，测试 seam 和未来真实实现返回值会不兼容。
    from learning_agent.computer_use.native_helper import NativeWindowCaptureResult  # 新增代码+Phase36WindowsWGCProvider: 延迟导入结果类型用于 isinstance；如果没有这行代码，无法识别已有统一结果。
