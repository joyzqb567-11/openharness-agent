...
try:  # 新增代码+UniversalDesktopAdapter：优先按包路径导入现有通用闭环；如果没有这段代码，项目根目录运行时无法复用已完成的 Phase119 能力。
    ...
    from learning_agent.computer_use.natural_language_semantic_planner import phase123_semantic_intent_from_prompt  # 新增代码+GenericSemanticPlanner：导入通用自然语言语义抽取入口；如果没有这一行，adapter 无法把 Notepad/浏览器等非绘图任务转成统一意图。
    from learning_agent.computer_use.universal_observe_plan_act_verify import Phase120VisualTaskPlanner, UniversalObservePlanActVerifyLoop  # 修改代码+VisualPlannerLoop：导入通用 loop 和视觉 planner；如果没有这一行，adapter 仍会只用旧静态动作复制器。
    ...
    from learning_agent.computer_use.visual_semantic_planner import phase122_visual_intent_from_prompt  # 新增代码+VisualSemanticPlanner：导入脱敏视觉意图抽取函数；如果没有这一行，adapter 无法把中文房子 prompt 传给语义 planner。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.universal_action_dsl", "learning_agent.computer_use.natural_language_semantic_planner", "learning_agent.computer_use.universal_observe_plan_act_verify", "learning_agent.computer_use.universal_real_observation", "learning_agent.computer_use.universal_target_session", "learning_agent.computer_use.visual_semantic_planner", "learning_agent.computer_use.paint_pikachu_real_loop"}:  # 修改代码+GenericSemanticPlanner：脚本模式兜底名单加入通用语义 planner；如果没有这一行，bat 入口可能因新模块包前缀失败。
    ...
    from computer_use.natural_language_semantic_planner import phase123_semantic_intent_from_prompt  # type: ignore  # 新增代码+GenericSemanticPlanner：脚本模式导入通用语义抽取入口；如果没有这一行，真实 bat 入口无法识别非绘图桌面任务。
    from computer_use.universal_observe_plan_act_verify import Phase120VisualTaskPlanner, UniversalObservePlanActVerifyLoop  # type: ignore  # 修改代码+VisualPlannerLoop：脚本模式下导入视觉 planner 和通用闭环；如果没有这一行，真实 bat 入口不会走新规划器。
    ...
    from computer_use.visual_semantic_planner import phase122_visual_intent_from_prompt  # type: ignore  # 新增代码+VisualSemanticPlanner：脚本模式导入脱敏视觉意图抽取函数；如果没有这一行，真实终端入口不能识别房子语义。
