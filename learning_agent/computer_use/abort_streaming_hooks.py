...
try:  # 新增代码+Phase61AbortStreamingHooks: 优先按包路径导入 Computer Use 运行时和文件工具；如果没有这段代码，包运行模式无法复用锁、通知和 JSONL。
    from learning_agent.computer_use.audit import ComputerUseAuditStore  # 新增代码+Phase61AbortStreamingHooks: 导入审计仓库用于 flush 状态；如果没有这行代码，cleanup hook 无法说明审计位置。
    from learning_agent.computer_use.lock import ComputerUseLockManager, phase30_lock_timestamp  # 新增代码+Phase61AbortStreamingHooks: 导入 durable lock 和时间戳；如果没有这行代码，abort-aware sender 没有急停事实源。
    from learning_agent.computer_use.session_runtime import PHASE40_DEFAULT_SESSION_ID, WindowsComputerUseSessionRuntime  # 新增代码+Phase61AbortStreamingHooks: 导入现有 runtime cleanup/recovery 能力；如果没有这行代码，异常路径要重复实现清理。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.audit", "learning_agent.computer_use.lock", "learning_agent.computer_use.session_runtime", "learning_agent.runtime", "learning_agent.runtime.files"}:  # 新增代码+Phase61AbortStreamingHooks: 只允许包路径缺失时 fallback；如果没有这行代码，内部真实错误会被误吞。
        raise  # 新增代码+Phase61AbortStreamingHooks: 重新抛出非路径类导入错误；如果没有这行代码，排查底层模块问题会困难。
    from computer_use.audit import ComputerUseAuditStore  # 新增代码+Phase61AbortStreamingHooks: 脚本模式导入审计仓库；如果没有这行代码，bat 入口无法显示审计 flush 状态。
    from computer_use.lock import ComputerUseLockManager, phase30_lock_timestamp  # 新增代码+Phase61AbortStreamingHooks: 脚本模式导入锁管理器；如果没有这行代码，bat 入口无法读取 abort flag。
    from computer_use.session_runtime import PHASE40_DEFAULT_SESSION_ID, WindowsComputerUseSessionRuntime  # 新增代码+Phase61AbortStreamingHooks: 脚本模式导入 session runtime；如果没有这行代码，bat 入口无法执行 cleanup/recover。
...
class WindowsComputerUseAbortStreamingHooks:  # 新增代码+Phase61AbortStreamingHooks: 类段开始，组合 abort-aware sender、cleanup hook、stale recovery 和 terminal status；如果没有这个类，Phase61 能力会散落在各模块。
    ...
    def evaluate_hotkey_plan(self) -> dict[str, Any]:  # 新增代码+Phase61AbortStreamingHooks: 函数段开始，评估 Windows 全局热键方案；如果没有这段函数，状态可能假装热键已经注册。
        return {"global_hotkey_registered": False, "terminal_abort_fallback": True, "controller_abort_fallback": True, "hotkey_mode": "terminal_and_controller_fallback", "reason": "Phase61 默认不注册全局热键或低层键盘 hook，避免留下不可撤销的系统级钩子；请使用 /computer abort 或 controller abort。"}  # 新增代码+Phase61AbortStreamingHooks: 返回诚实降级状态；如果没有这行代码，用户会误以为全局热键可用。
...
def run_phase61_abort_streaming_hooks_contract(base_dir: str | Path | None = None) -> dict[str, Any]:  # 新增代码+Phase61AbortStreamingHooks: 函数段开始，运行 Phase61 abort/streaming hooks 合同自检；如果没有这段函数，CLI 和真实终端没有统一验收入口。
    ...
    from learning_agent.computer_use.real_sendinput_guard import Phase58RecordingLowLevelSender, Phase58StaticSafeWindowObserver, WindowsRealSendInputGuardRuntime  # 新增代码+Phase61AbortStreamingHooks: 延迟导入 Phase58 安全 runtime 避免顶层循环；如果没有这行代码，合同无法证明低层事件被拦截。
    from learning_agent.computer_use.windows_backend import StaticWindowsWindowInventory  # 新增代码+Phase61AbortStreamingHooks: 延迟导入静态 inventory；如果没有这行代码，合同会依赖真实桌面。
