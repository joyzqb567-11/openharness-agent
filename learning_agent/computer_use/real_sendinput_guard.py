...
class WindowsSendInputLowLevelSender:  # 新增代码+Phase58RealSendInputGuard: 类段开始，定义真实 Windows 低层发送器；如果没有这个类，Phase58 只能停留在 fake sender 合同。
    ...
    def _send_virtual_key(self, key: str, down: bool) -> bool:  # 修改代码+Phase97ControlledNotepadLiveEdit: 函数段开始，用 SendInput 发送普通虚拟键；如果没有这段函数，真实后端无法表达 Ctrl+S 保存快捷键。
        key_map = {"ctrl": 0xA2, "control": 0xA2, "s": 0x53, "v": 0x56}  # 修改代码+Phase97ControlledNotepadLiveEdit: 只开放 Phase97 当前需要的左 Ctrl、S 和粘贴用 V；如果没有这一行，未知按键可能绕过极简白名单进入真实键盘。
        virtual_key = int(key_map.get(str(key).strip().lower(), 0))  # 修改代码+Phase97ControlledNotepadLiveEdit: 把按键名转换成 Windows 虚拟键码；如果没有这一行，SendInput 不知道要按哪个键。
