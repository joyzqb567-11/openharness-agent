...
DANGEROUS_TARGET_TERMS = (  # 新增代码+Phase98UniversalComputerUseMode：函数段落开始，列出危险窗口关键词；如果没有这段代码，模式可能误控高风险目标。
    "powershell",  # 新增代码+Phase98UniversalComputerUseMode：拦截 PowerShell；如果没有这行代码，命令行高风险入口可能被误操作。
    "cmd",  # 新增代码+Phase98UniversalComputerUseMode：拦截 cmd；如果没有这行代码，命令行高风险入口可能被误操作。
    "terminal",  # 新增代码+Phase98UniversalComputerUseMode：拦截 terminal；如果没有这行代码，终端窗口可能被误输入。
    "administrator",  # 新增代码+Phase98UniversalComputerUseMode：拦截管理员上下文；如果没有这行代码，高权限窗口风险不可控。
    "uac",  # 新增代码+Phase98UniversalComputerUseMode：拦截 UAC；如果没有这行代码，系统授权弹窗可能被误点。
    "windows security",  # 新增代码+Phase98UniversalComputerUseMode：拦截 Windows Security；如果没有这行代码，安全设置可能被误操作。
    "defender",  # 新增代码+Phase98UniversalComputerUseMode：拦截 Defender；如果没有这行代码，防护设置可能被误改。
    "firewall",  # 新增代码+Phase98UniversalComputerUseMode：拦截 firewall；如果没有这行代码，网络安全策略可能被误改。
    "registry",  # 新增代码+Phase98UniversalComputerUseMode：拦截 registry；如果没有这行代码，注册表可能被误操作。
    "services",  # 新增代码+Phase98UniversalComputerUseMode：拦截 services；如果没有这行代码，系统服务可能被误停。
    "password",  # 新增代码+Phase98UniversalComputerUseMode：拦截 password；如果没有这行代码，密码内容可能被误处理。
    "credential",  # 新增代码+Phase98UniversalComputerUseMode：拦截 credential；如果没有这行代码，凭据窗口可能被误操作。
    "captcha",  # 新增代码+Phase98UniversalComputerUseMode：拦截 captcha；如果没有这行代码，验证码场景边界不清楚。
    "payment",  # 新增代码+Phase98UniversalComputerUseMode：拦截 payment；如果没有这行代码，支付场景可能被误操作。
    "token",  # 新增代码+Phase98UniversalComputerUseMode：拦截 token；如果没有这行代码，令牌隐私可能被误处理。
    "api key",  # 新增代码+Phase98UniversalComputerUseMode：拦截 API key；如果没有这行代码，密钥场景可能被误操作。
    "private key",  # 新增代码+Phase98UniversalComputerUseMode：拦截 private key；如果没有这行代码，私钥场景可能被误操作。
    "验证码",  # 新增代码+Phase98UniversalComputerUseMode：拦截中文验证码；如果没有这行代码，中文认证场景会漏检。
    "密码",  # 新增代码+Phase98UniversalComputerUseMode：拦截中文密码；如果没有这行代码，中文密码窗口会漏检。
    "支付",  # 新增代码+Phase98UniversalComputerUseMode：拦截中文支付；如果没有这行代码，中文支付场景会漏检。
    "付款",  # 新增代码+Phase98UniversalComputerUseMode：拦截中文付款；如果没有这行代码，中文付款场景会漏检。
    "令牌",  # 新增代码+Phase98UniversalComputerUseMode：拦截中文令牌；如果没有这行代码，中文 token 场景会漏检。
    "私钥",  # 新增代码+Phase98UniversalComputerUseMode：拦截中文私钥；如果没有这行代码，中文私钥场景会漏检。
    "管理员",  # 新增代码+Phase98UniversalComputerUseMode：拦截中文管理员；如果没有这行代码，中文高权限窗口会漏检。
    "安全",  # 新增代码+Phase98UniversalComputerUseMode：拦截中文安全；如果没有这行代码，中文安全设置窗口会漏检。
)  # 新增代码+Phase98UniversalComputerUseMode：函数段落结束，危险关键词列表到此结束；如果没有这行代码，Python 无法关闭元组。
...
def _phase98_window_text(window: dict[str, Any]) -> str:  # 新增代码+Phase98UniversalComputerUseMode：函数段落开始，把窗口信息合并为风险扫描文本；如果没有这段函数，危险目标检查会漏字段。
    ...
    parts = [str(value or "") for value in window.values()]  # 新增代码+Phase98UniversalComputerUseMode：收集窗口所有字段值；如果没有这行代码，只看进程名会漏标题和描述。
    return " ".join(parts).lower()  # 新增代码+Phase98UniversalComputerUseMode：合并并小写化用于关键词匹配；如果没有这行代码，大小写差异会导致漏拦截。
...
class ComputerUseModeSessionStore:  # 新增代码+Phase98UniversalComputerUseMode：类段落开始，管理 Computer Use 模式 session；如果没有这个类，外部没有统一 API 打开/停止/查询模式。
    ...
    def _build_state(self, mode: str, reason: str, ttl_seconds: int | None) -> dict[str, Any]:  # 新增代码+Phase98UniversalComputerUseMode：函数段落开始，构建可落盘状态；如果没有这段函数，多个入口会写出不一致字段。
        normalized_mode = mode if mode in {"normal", "observe", "full"} else "normal"  # 新增代码+Phase98UniversalComputerUseMode：限制模式只在已知集合中；如果没有这行代码，拼写错误会污染状态。
    ...
    def open_mode(self, mode: str = "normal", reason: str = "", ttl_seconds: int | None = None) -> dict[str, Any]:  # 新增代码+Phase98UniversalComputerUseMode：函数段落开始，打开 normal/observe/full 模式；如果没有这段函数，用户无法启用 computer use 权限模式。
    ...
    def confirm_full_mode(self, confirmation_token: str, reason: str = "") -> dict[str, Any]:  # 新增代码+Phase98UniversalComputerUseMode：函数段落开始，用 token 确认 full mode；如果没有这段函数，二次确认流程无法完成。
        ...
        token_matches = str(pending.get("confirmation_token", "")) == str(confirmation_token or "")  # 新增代码+Phase98UniversalComputerUseMode：比较 token；如果没有这行代码，错误 token 也可能打开 full。
        ...
        if not token_matches or not token_live:  # 新增代码+Phase98UniversalComputerUseMode：判断是否拒绝确认；如果没有这行代码，full mode 安全边界会失效。
            return {"opened": False, "full_mode": False, "decision": "invalid_or_expired_confirmation_token", "marker": PHASE98_COMPUTER_USE_MODE_READY, "ok_token": PHASE98_COMPUTER_USE_MODE_OK}  # 新增代码+Phase98UniversalComputerUseMode：返回拒绝结果；如果没有这行代码，调用方无法知道确认失败原因。
        opened = self.open_mode(mode="full", reason=reason, ttl_seconds=DEFAULT_FULL_TTL_SECONDS)  # 新增代码+Phase98UniversalComputerUseMode：确认通过后打开 5 分钟 full；如果没有这行代码，full mode 不会真正激活。
    ...
    def status(self) -> dict[str, Any]:  # 新增代码+Phase98UniversalComputerUseMode：函数段落开始，读取当前模式状态；如果没有这段函数，终端和测试无法确认权限模式。
        ...
        result["full_mode"] = bool(mode == "full" and not expired and not bool(state.get("stopped")))  # 新增代码+Phase98UniversalComputerUseMode：只有未停用未过期的 full 才算 full_mode；如果没有这行代码，请求阶段可能被误认为 full。
        result["allowed_action_classes"] = self._allowed_actions_for_mode(mode) if not expired else []  # 新增代码+Phase98UniversalComputerUseMode：按模式返回当前允许动作；如果没有这行代码，status 可能显示过期模式仍可操作。
