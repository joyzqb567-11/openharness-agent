...
try:  # 新增代码+PaintPikachuRealLoop：优先按包路径导入项目已有 Computer Use 组件；如果没有这一段，单元测试和真实 bat 入口无法共享实现。
    from learning_agent.computer_use.drawing_primitives import build_cat_drag_plan, build_elephant_drag_plan, build_pikachu_drag_plan, expand_drag_path_to_low_level_events  # 修改代码+PaintCatSubject：同时复用猫、大象和皮卡丘拖拽 primitive；如果没有这一行，猫 prompt 无法进入真实 Paint 计划。
    from learning_agent.computer_use.windows_launch_resolver import resolve_generic_app_launch_target  # 新增代码+PaintPikachuRealLoop：复用通用应用发现和高风险拒绝；如果没有这一行，Paint 启动会变成新白名单补丁。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use", "learning_agent.computer_use.drawing_primitives", "learning_agent.computer_use.windows_launch_resolver", "learning_agent.computer_use.generic_launch_backend", "learning_agent.computer_use.real_sendinput_guard", "learning_agent.computer_use.screenshot_runtime", "learning_agent.computer_use.windows_backend"}:  # 修改代码+PaintVisualGuard：允许脚本模式兜底截图 runtime 路径；如果没有这一行，bat 入口会把正常包路径差异误当内部错误。
        raise  # 新增代码+PaintPikachuRealLoop：重新抛出非路径类导入错误；如果没有这一行，排查真实控制链问题会很困难。
    from computer_use.drawing_primitives import build_cat_drag_plan, build_elephant_drag_plan, build_pikachu_drag_plan, expand_drag_path_to_low_level_events  # type: ignore  # 修改代码+PaintCatSubject：脚本模式同时复用猫、大象和皮卡丘 primitive；如果没有这一行，bat 入口下猫仍会被拒绝或误画。
    from computer_use.windows_launch_resolver import resolve_generic_app_launch_target  # type: ignore  # 新增代码+PaintPikachuRealLoop：脚本模式复用通用应用发现；如果没有这一行，bat 入口会失去高风险拒绝和安全计划。
