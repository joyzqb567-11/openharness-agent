...
COMPUTER_USE_OPERATION_RAW_TOOL_NAMES = {"request_access", "screenshot", "observe", "cursor_position", "mouse_move", "left_click", "double_click", "right_click", "type", "key", "scroll", "wait", "read_clipboard", "write_clipboard", "open_application", "list_granted_applications", "computer_batch"}  # 修改代码+ComputerUseMcpV2：只列出蓝图确认的 17 个 v2 Computer Use 工具；如果没有这一行，zoom/hold_key/left_click_drag 等旧实验接口会继续同轮暴露给模型。
...
COMPUTER_USE_RESERVED_RAW_TOOL_NAMES = {"middle_click", "triple_click", "left_mouse_down", "left_mouse_up"}  # 新增代码+ClaudeCodeToolSurface：列出预留但默认不暴露的低优先级鼠标工具；如果没有这行代码，未成熟工具可能被模型当成首选动作。
