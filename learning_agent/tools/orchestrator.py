...
try:  # 新增代码+Stage15F: 包运行模式下导入工具调用类型；若没有这行代码，编排器无法读取 tool_call.name 和 call_id。
    ...
    if error.name not in {"learning_agent", "learning_agent.core", "learning_agent.core.messages", "learning_agent.tools", "learning_agent.tools.streaming_executor"}:  # 修改代码+Phase4StreamingExecutor: 允许 streaming executor 包路径缺失时走脚本 fallback；若没有这行代码，bat 入口可能导入失败。
...
def _streaming_event_sink(agent: Any) -> Callable[[dict[str, Any]], None] | None:  # 新增代码+Phase4StreamingExecutor: 为全局执行器构造 observation sink；若没有这段函数，流式事件无法进入 agent 观测层。
    recorder = getattr(agent, "_record_observation", None)  # 新增代码+Phase4StreamingExecutor: 动态读取 agent observation 记录器；若没有这行代码，orchestrator 会强依赖 LearningAgent。
    if not callable(recorder):  # 新增代码+Phase4StreamingExecutor: 没有记录器时返回空 sink；若没有这行代码，fake agent 或旧对象会调用失败。
        return None  # 新增代码+Phase4StreamingExecutor: 无记录器时保持执行器可用；若没有这行代码，测试和简化对象无法使用 orchestrator。
    return lambda payload: recorder("streaming_tool_executor", payload)  # 新增代码+Phase4StreamingExecutor: 把执行器事件写成统一 observation；若没有这行代码，状态 UI 看不到 started/chunk/completed/failed。
