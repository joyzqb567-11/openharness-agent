...
class ComputerUseModeCommandPhase98Tests(unittest.TestCase):  # 新增代码+Phase98UniversalComputerUseMode：类段开始，集中验证 Phase98 模式命令真实终端契约；如果没有这个类，Task3 的 `/computer` 接入没有红绿灯。
    ...
    def test_computer_full_request_requires_confirmation_and_confirm_activates_full(self) -> None:  # 新增代码+Phase98UniversalComputerUseMode：函数段开始，验证 full 请求只生成强确认且 token 确认后才激活；如果没有这段测试，高风险 full 可能被一步打开。
        ...
        self.assert_contains_all(request_output, ["Computer Use Full Request", "strong_confirmation_required=true", "full_mode=false", "opened=false", "low_level_event_count=0"])  # 新增代码+Phase98UniversalComputerUseMode：断言 full 请求没有提权也没有低层事件；如果没有这行代码，风险边界可能被绕过。
        self.assert_contains_all(confirm_output, ["Computer Use Mode", "mode=full", "full_mode=true", "opened=true", "ttl_seconds="])  # 新增代码+Phase98UniversalComputerUseMode：断言 confirm 后进入 full 且显示 TTL；如果没有这行代码，用户无法确认 full 有短时限。
    # 新增代码+Phase98UniversalComputerUseMode：函数段结束，test_computer_full_request_requires_confirmation_and_confirm_activates_full 到此结束；如果没有这个边界说明，读者不容易看出 full 请求/确认测试范围。
