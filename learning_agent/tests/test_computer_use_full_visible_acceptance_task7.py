...
TREE_SCENARIO_PATH = PROJECT_ROOT / "learning_agent" / "acceptance_controller" / "scenarios" / "computer_use_full_model_loop_paint_tree_real.json"  # 新增代码+FullModeAutoClearAbort：定位真实 Paint 画树模型主循环场景；如果没有这一行，普通用户路径可能继续偷偷依赖 clear-abort。
...
class ComputerUseFullVisibleAcceptanceTask7Tests(unittest.TestCase):  # 新增代码+DesktopTaskVisibleAcceptance：类段开始，集中验证 Task7 的场景和 controller 合同；如果没有这个类，unittest 不会组织这些门禁。
    ...
    def test_tree_model_loop_scenario_does_not_require_clear_abort(self) -> None:  # 新增代码+FullModeAutoClearAbort：函数段开始，验证真实画树验收只使用普通用户会输入的 full 命令和自然语言；如果没有这个测试，场景可能继续依赖开发者维护命令。
        scenario = json.loads(TREE_SCENARIO_PATH.read_text(encoding="utf-8"))  # 新增代码+FullModeAutoClearAbort：读取真实画树场景 JSON；如果没有这一行，场景里的 prompt_lines 不会被测试锁住。
        self.assertTrue(scenario["multi_prompt_lines"])  # 新增代码+FullModeAutoClearAbort：断言场景仍是多轮真实终端输入；如果没有这一行，controller 可能把两行压成一行导致用户习惯失真。
        self.assertEqual(scenario["prompt_lines"][0], "/computer use --full")  # 新增代码+FullModeAutoClearAbort：断言第一行就是普通用户打开 full 模式；如果没有这一行，验收可能继续把内部恢复命令当前置条件。
        self.assertEqual(scenario["prompt_lines"][1], "请使用本地电脑的画图软件画一棵树。")  # 新增代码+FullModeAutoClearAbort：断言第二行才是自然语言任务；如果没有这一行，场景可能没有覆盖真实用户 prompt。
        self.assertNotIn("/computer clear-abort", scenario["prompt_lines"])  # 新增代码+FullModeAutoClearAbort：断言普通用户场景不再输入 clear-abort；如果没有这一行，用户仍需要知道内部急停恢复命令。
        self.assertNotIn("clear-abort", scenario["event_payload_contains"])  # 新增代码+FullModeAutoClearAbort：断言验收不再把 clear-abort 当通过证据；如果没有这一行，旧维护命令可能悄悄回到成功条件。
    # 新增代码+FullModeAutoClearAbort：函数段结束，test_tree_model_loop_scenario_does_not_require_clear_abort 到此结束；如果没有这个边界说明，代码小白不容易看出画树用户路径测试范围。
