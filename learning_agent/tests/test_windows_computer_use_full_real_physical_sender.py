...
from learning_agent.computer_use.desktop_task_runtime import build_default_desktop_task_runtime  # 新增代码+RuntimeFactorySlimming：导入迁移后的默认桌面任务 runtime 工厂；如果没有这一行，测试仍会误依赖 LearningAgent 主类里的旧工厂入口。
...
class WindowsComputerUseFullRealPhysicalSenderTests(unittest.TestCase):  # 新增代码+RealPhysicalFullMode：类段开始，集中验证 full 模式默认真实物理 sender 接线；如果没有这个类，本轮“打开真实物理鼠标键盘”的升级没有自动化护栏。
    ...
    def test_default_agent_runtime_wires_controlled_physical_sender_to_windows_sendinput_backend(self) -> None:  # 新增代码+RealPhysicalFullMode：函数段开始，验证默认 agent runtime 接入 Phase95 受控物理 sender 和 Windows SendInput 后端；如果没有这个测试，生产路径可能继续停在只观察不动作。
        with tempfile.TemporaryDirectory() as raw_dir:  # 新增代码+RealPhysicalFullMode：创建一次性 workspace；如果没有这一行，测试会污染用户真实 memory。
            ...
            self.assertFalse(hasattr(agent, "_desktop_task_runtime_for_current_run"))  # 新增代码+RuntimeFactorySlimming：断言 LearningAgent 主类不再暴露旧 runtime 工厂；如果没有这一行，后续可能再次把兼容 runtime 误接回模型主循环。
            runtime = build_default_desktop_task_runtime(Path(raw_dir))  # 修改代码+RuntimeFactorySlimming：通过 computer_use 模块级工厂构造默认 runtime；如果没有这一行，默认 sender 接线测试仍会依赖主类历史包袱。
