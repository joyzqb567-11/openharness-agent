...
import subprocess  # 新增代码+OwnedLaunchCleanupTest：启动一个无窗口的测试子进程模拟 agent-owned 应用进程；如果没有这一行，测试无法证明 cleanup 会真正关闭自有进程。
import sys  # 新增代码+OwnedLaunchCleanupTest：读取当前 Python 解释器路径来启动测试子进程；如果没有这一行，测试在不同机器上可能找不到 python。
import time  # 新增代码+OwnedLaunchCleanupTest：短暂轮询测试子进程是否退出；如果没有这一行，断言可能早于操作系统释放进程。
...
from learning_agent.computer_use_mcp_v2.windows_runtime.owned_resource_registry import OwnedResourceRegistry  # 新增代码+OwnedLaunchCleanupTest：导入自有资源注册表用于验证 launch_app 是否接入 stop/cleanup；如果没有这一行，测试只能看 controller 结果不能看 cleanup 事实源。
...
class SequenceTargetRuntime:  # 新增代码+FreshTargetControllerTest：类段开始，按顺序返回目标窗口；如果没有这个类，多目标测试无法稳定产生两个 target_ref。
    def __init__(self, windows: list[dict[str, Any]]) -> None:  # 新增代码+FreshTargetControllerTest：函数段开始，保存待返回窗口；如果没有这段函数，runtime 没有目标样本。
        ...
        self.open_calls: list[dict[str, Any]] = []  # 新增代码+ControlledResourceLaunchTest：记录每次启动调用参数；如果没有这一行，测试无法证明 controller 是否把桌面 1.txt 传给通用启动层。
    # 新增代码+FreshTargetControllerTest：函数段结束，SequenceTargetRuntime.__init__ 到此结束；如果没有这个边界说明，用户不容易看出初始化范围。

    def open_target_session(self, target_app: str, user_authorized_window: bool = False, controlled_resource_path: str = "") -> dict[str, Any]:  # 修改代码+ControlledResourceLaunchTest：函数段开始，模拟通用目标 session 并接收受控资源路径；如果没有这个参数，测试无法发现 Notepad 仍然裸启动。
        self.open_calls.append({"target_app": target_app, "user_authorized_window": user_authorized_window, "controlled_resource_path": controlled_resource_path})  # 新增代码+ControlledResourceLaunchTest：保存启动参数；如果没有这一行，断言只能看结果，不能确认资源路径是否进入源头启动链路。
...
class _CleanupRuntime:  # 新增代码+OwnedLaunchCleanupTest：类段开始，提供只带 owned_resource_registry 的轻量 cleanup runtime；如果没有这个类，controller 测试无法复用 stop 使用的同一注册表。
    def __init__(self, registry: OwnedResourceRegistry) -> None:  # 新增代码+OwnedLaunchCleanupTest：函数段开始，保存外部注入的注册表；如果没有这段函数，runtime 没有 cleanup 事实源。
        self.owned_resource_registry = registry  # 新增代码+OwnedLaunchCleanupTest：暴露和生产 WindowsComputerUseSessionRuntime 同名的 registry 字段；如果没有这一行，controller 无法把 agent-owned launch 登记进去。
    # 新增代码+OwnedLaunchCleanupTest：函数段结束，_CleanupRuntime.__init__ 到此结束；如果没有这个边界说明，用户不容易看出 runtime 只模拟哪一部分。
# 新增代码+OwnedLaunchCleanupTest：类段结束，_CleanupRuntime 到此结束；如果没有这个边界说明，用户不容易看出测试替身范围。


def _process_still_running(process: subprocess.Popen[str]) -> bool:  # 新增代码+OwnedLaunchCleanupTest：函数段开始，判断测试子进程是否仍存活；如果没有这个函数，断言会重复写 poll 细节。
    return process.poll() is None  # 新增代码+OwnedLaunchCleanupTest：poll 返回 None 表示子进程还活着；如果没有这一行，测试无法判断 cleanup 是否真的杀掉进程。
# 新增代码+OwnedLaunchCleanupTest：函数段结束，_process_still_running 到此结束；如果没有这个边界说明，用户不容易看出进程判断范围。


def _wait_until_process_exits(process: subprocess.Popen[str], timeout_seconds: float = 5.0) -> bool:  # 新增代码+OwnedLaunchCleanupTest：函数段开始，等待测试子进程退出；如果没有这个函数，cleanup 后立刻断言可能遇到系统释放延迟。
    deadline = time.monotonic() + timeout_seconds  # 新增代码+OwnedLaunchCleanupTest：计算等待截止时间；如果没有这一行，循环无法按固定上限收敛。
    while time.monotonic() < deadline:  # 新增代码+OwnedLaunchCleanupTest：在截止前循环检查；如果没有这一行，等待只会检查一次。
        if not _process_still_running(process):  # 新增代码+OwnedLaunchCleanupTest：发现进程已退出就结束等待；如果没有这一行，测试会无意义等满超时。
            return True  # 新增代码+OwnedLaunchCleanupTest：返回已退出；如果没有这一行，调用方无法知道 cleanup 是否生效。
        time.sleep(0.05)  # 新增代码+OwnedLaunchCleanupTest：短睡避免忙等占用 CPU；如果没有这一行，测试会在等待期间空转。
    return not _process_still_running(process)  # 新增代码+OwnedLaunchCleanupTest：超时后再查一次并返回最终状态；如果没有这一行，函数没有稳定返回值。
# 新增代码+OwnedLaunchCleanupTest：函数段结束，_wait_until_process_exits 到此结束；如果没有这个边界说明，用户不容易看出等待逻辑范围。
...
def test_launch_app_passes_desktop_txt_resource_to_target_runtime(monkeypatch) -> None:  # 新增代码+ControlledResourceLaunchTest：函数段开始，验证文档任务会把桌面 1.txt 绑定到源头启动参数；如果没有这段测试，Notepad 会继续裸启动并被会话恢复旧文档劫持。
    monkeypatch.setenv("USERPROFILE", r"C:\Users\joyzq")  # 新增代码+ControlledResourceLaunchTest：固定测试里的用户桌面目录；如果没有这一行，不同机器的 home 路径会让断言不稳定。
    backend = FreshTargetBackend([])  # 新增代码+ControlledResourceLaunchTest：让启动前预检看到没有已打开 Notepad；如果没有这一行，测试会被旧窗口预检提前阻断。
    runtime = SequenceTargetRuntime([_window(pid=1201, hwnd=1301, title="1.txt - Notepad")])  # 新增代码+ControlledResourceLaunchTest：准备成功绑定的新目标窗口；如果没有这一行，controller 无法完成 launch_app 成功路径。
    controller = ComputerUseController(backend=backend, owner_session_id="controlled-resource-test", target_session_runtime=runtime)  # 新增代码+ControlledResourceLaunchTest：装配被测 controller 和记录型 runtime；如果没有这一行，测试没有生产级调用链入口。
    result = controller.execute({"action": "launch_app", "app_name": "notepad", "reason": "打开一个新的真实记事本窗口。", "session_task_context": "请打开本地真实记事本，输入 hello everyone，最后保存文件名为 1.txt 到本地电脑桌面。", "confirm_desktop_control": True})  # 新增代码+ControlledResourceLaunchTest：模拟模型 open_application reason 漏掉文件名但授权上下文包含 1.txt；如果没有这一行，真实失败路径不会被覆盖。
    expected_path = r"C:\Users\joyzq\Desktop\1.txt"  # 新增代码+ControlledResourceLaunchTest：定义期望的受控目标文件路径；如果没有这一行，下面断言会重复硬编码难以维护。
    assert result.ok is True  # 新增代码+ControlledResourceLaunchTest：确认资源绑定不应破坏正常 launch_app 成功路径；如果没有这一行，路径传递可能以失败为代价。
    assert runtime.open_calls[0]["controlled_resource_path"] == expected_path  # 新增代码+ControlledResourceLaunchTest：确认 controller 把桌面 1.txt 传给源头启动 runtime；如果没有这一行，裸启动回归不会被发现。
    assert result.data["controlled_resource_path"] == expected_path  # 新增代码+ControlledResourceLaunchTest：确认工具结果把受控资源路径回显给审计和验收；如果没有这一行，终端日志看不到本轮目标文件。
# 新增代码+ControlledResourceLaunchTest：函数段结束，test_launch_app_passes_desktop_txt_resource_to_target_runtime 到此结束；如果没有这个边界说明，用户不容易看出受控资源启动测试范围。


def test_launch_app_registers_agent_owned_process_for_turn_cleanup() -> None:  # 新增代码+OwnedLaunchCleanupTest：函数段开始，验证 launch_app 会把 agent-owned 进程登记给 stop/cleanup；如果没有这段测试，终端可能报告已清理但真实应用仍残留。
    process = subprocess.Popen([sys.executable, "-c", "import time; time.sleep(60)"])  # 新增代码+OwnedLaunchCleanupTest：启动一个可安全关闭的测试子进程；如果没有这一行，cleanup 是否真的关闭进程无法被验证。
    try:  # 新增代码+OwnedLaunchCleanupTest：确保测试失败时也会回收子进程；如果没有这一行，失败测试可能留下 python 进程。
        registry = OwnedResourceRegistry(session_id="owned-cleanup-test", clock=lambda: "2026-06-19T00:00:00Z")  # 新增代码+OwnedLaunchCleanupTest：创建和 stop 共用语义一致的自有资源注册表；如果没有这一行，controller 没有 cleanup 事实源。
        cleanup_runtime = _CleanupRuntime(registry)  # 新增代码+OwnedLaunchCleanupTest：把注册表包装成轻量 runtime；如果没有这一行，controller 无法按生产字段读取 registry。
        backend = FreshTargetBackend([])  # 新增代码+OwnedLaunchCleanupTest：让启动前预检看到桌面没有旧 Paint；如果没有这一行，launch_app 会被 FreshTarget 提前拒绝。
        window = _window(app_id="mspaint.exe", pid=process.pid, hwnd=2401, title="Untitled - Paint")  # 新增代码+OwnedLaunchCleanupTest：把测试子进程伪装成新启动的 Paint 窗口；如果没有这一行，launch_app 没有可登记的 pid/window。
        runtime = SequenceTargetRuntime([window])  # 新增代码+OwnedLaunchCleanupTest：让 controller 的 launch_app 成功返回这个 agent-owned 目标；如果没有这一行，注册路径不会执行。
        controller = ComputerUseController(backend=backend, owner_session_id="owned-cleanup-test", target_session_runtime=runtime, session_runtime=cleanup_runtime)  # 新增代码+OwnedLaunchCleanupTest：把 controller 接到同一 cleanup runtime；如果没有这一行，launch_app 和 stop 会继续使用两套状态。
        result = controller.execute({"action": "launch_app", "app_name": "mspaint", "confirm_desktop_control": True})  # 新增代码+OwnedLaunchCleanupTest：执行真实 controller 的 launch_app 成功路径；如果没有这一行，资源登记不会触发。
        cleanup = registry.cleanup_owned_resources(session_id="owned-cleanup-test", reason="unit test cleanup")  # 新增代码+OwnedLaunchCleanupTest：模拟 /computer stop 调用自有资源清理；如果没有这一行，进程不会被 cleanup 回调处理。
        residual = registry.check_residuals(session_id="owned-cleanup-test")  # 新增代码+OwnedLaunchCleanupTest：清理后再跑残留检查；如果没有这一行，测试只能证明调用过 cleanup，不能证明没有残留。
        assert result.ok is True  # 新增代码+OwnedLaunchCleanupTest：确认 launch_app 成功后才谈 cleanup；如果没有这一行，失败启动也可能误导后续断言。
        assert result.data["owned_process_cleanup_registered"] is True  # 新增代码+OwnedLaunchCleanupTest：确认结果显式报告已登记自有进程；如果没有这一行，终端日志看不到 cleanup 责任链。
        assert cleanup["cleaned_resource_count"] >= 1  # 新增代码+OwnedLaunchCleanupTest：确认 cleanup 至少处理了一个自有进程记录；如果没有这一行，空 registry 也会假装成功。
        assert _wait_until_process_exits(process) is True  # 新增代码+OwnedLaunchCleanupTest：确认测试子进程已被 cleanup 关闭；如果没有这一行，报告成功但进程残留的问题不会被抓住。
        assert residual["residual_check_passed"] is True  # 新增代码+OwnedLaunchCleanupTest：确认残留检查通过；如果没有这一行，清理后残留风险不会进入回归测试。
        assert residual["residual_owned_process"] is False  # 新增代码+OwnedLaunchCleanupTest：确认没有 agent-owned 进程残留；如果没有这一行，原始 Paint 残留问题可能复发。
    finally:  # 新增代码+OwnedLaunchCleanupTest：无论断言结果如何都兜底清理测试子进程；如果没有这一行，失败路径会污染开发机。
        if _process_still_running(process):  # 新增代码+OwnedLaunchCleanupTest：只有进程还活着才需要兜底 kill；如果没有这一行，已退出进程会被重复操作。
            process.kill()  # 新增代码+OwnedLaunchCleanupTest：强制结束测试子进程；如果没有这一行，红灯阶段会留下后台进程。
            process.wait(timeout=5)  # 新增代码+OwnedLaunchCleanupTest：等待强制结束完成；如果没有这一行，操作系统可能还没释放进程。
    # 新增代码+OwnedLaunchCleanupTest：函数段结束，test_launch_app_registers_agent_owned_process_for_turn_cleanup 到此结束；如果没有这个边界说明，用户不容易看出本测试范围。
