...
EXPECTED_INFERRED_ANT_MCP_FILES = {  # 新增代码+ComputerUseMcpV2RedTests：列出缺失外部 @ant/computer-use-mcp 的推断替代文件；如果没有这组断言，推断包可能和桥接包混在一起。
    ...
    "claudecode_protocol.py",  # 新增代码+ClaudeCodeProtocolParity：ClaudeCode 协议常量文件必须存在；如果没有这一项，协议字段会继续散落在 schema 和 runtime 中。
    ...
    "protocol_normalizer.py",  # 新增代码+ClaudeCodeProtocolParity：协议参数归一化文件必须存在；如果没有这一项，ClaudeCode 字段无法统一转给 Windows runtime。
...
class ComputerUseMcpV2ContractTests(unittest.TestCase):  # 新增代码+ComputerUseMcpV2RedTests：定义 v2 合同测试类；如果没有这段代码，unittest 不会发现这些合同检查。
    ...
    def test_claudecode_parity_tool_schemas_lock_required_fields(self) -> None:  # 新增代码+ClaudeCodeParity：声明 parity schema 合同测试；如果没有这一行，unittest 不会执行新增工具的 schema 形状检查。
        ...
        self.assertEqual({"region"}, set(zoom_schema["required"]))  # 修改代码+ClaudeCodeProtocolParity：断言 zoom 以 ClaudeCode 的 region 作为主必填字段；如果没有这一行，zoom schema 可能退回旧 x/y/width/height 主字段。
        ...
        self.assertEqual({"text"}, set(hold_key_schema["required"]))  # 修改代码+ClaudeCodeProtocolParity：断言 hold_key 以 ClaudeCode 的 text 作为主必填字段；如果没有这一行，公共合同会继续主推 keys 数组。
        self.assertEqual("array", hold_key_schema["properties"]["keys"]["type"])  # 修改代码+ClaudeCodeProtocolParity：断言 hold_key 仍兼容旧 keys 数组；如果没有这一行，Windows runtime 历史调用可能被破坏。
        self.assertIn("duration", hold_key_schema["properties"])  # 修改代码+ClaudeCodeProtocolParity：断言 hold_key 暴露 ClaudeCode duration；如果没有这一行，长按时长字段无法对齐 ClaudeCode。
        self.assertIn("duration_seconds", hold_key_schema["properties"])  # 修改代码+ClaudeCodeProtocolParity：断言 hold_key 保留旧 duration_seconds 兼容；如果没有这一行，现有 Windows 调用会丢时长。
        self.assertNotIn("duration", hold_key_schema["required"])  # 修改代码+ClaudeCodeProtocolParity：断言 duration 不是必填；如果没有这一行，模型会被迫提供非必要时长。
        self.assertEqual({"start_coordinate", "coordinate"}, set(drag_schema["required"]))  # 修改代码+ClaudeCodeProtocolParity：断言拖拽主必填字段是 ClaudeCode 起点和终点坐标；如果没有这一行，拖拽 schema 可能退回旧 start/end 四字段。
        self.assertEqual({"coordinate"}, set(middle_schema["required"]))  # 修改代码+ClaudeCodeProtocolParity：断言中键点击主必填 coordinate；如果没有这一行，中键工具可能继续主推 x/y。
        self.assertEqual({"coordinate"}, set(triple_schema["required"]))  # 修改代码+ClaudeCodeProtocolParity：断言三击主必填 coordinate；如果没有这一行，三击工具可能继续主推 x/y。
        self.assertEqual({"coordinate"}, set(mouse_down_schema["required"]))  # 修改代码+ClaudeCodeProtocolParity：断言左键按下主必填 coordinate；如果没有这一行，按下动作可能继续主推 x/y。
        self.assertEqual({"coordinate"}, set(mouse_up_schema["required"]))  # 修改代码+ClaudeCodeProtocolParity：断言左键释放主必填 coordinate；如果没有这一行，释放动作可能继续主推 x/y。
