...
from learning_agent.computer_use_mcp_v2.windows_runtime.sendinput_executor import WindowsSendInputExecutor  # 新增代码+ClaudeCodeParity: 导入 v2 executor 验证新动作不再被拒绝；如果没有这行代码，Task4 可能只修到旧包。
...
class ComputerUseMcpV2SendInputParityTask4Tests(unittest.TestCase):  # 新增代码+ClaudeCodeParity: 类段开始，集中覆盖 Task4 的 v2 SendInput parity；如果没有这个类，新增测试不会被 unittest 组织。
    ...
    def test_v2_executor_supports_and_emits_parity_actions(self) -> None:  # 新增代码+ClaudeCodeParity: 函数段开始，验证 executor 支持并生成新动作规范事件；如果没有这个测试，新 MCP tool 到低层会继续被拒绝。
        ...
        self.assertFalse(executor.execute("hold_key", {"keys": ["a"], "duration_seconds": 31}).ok)  # 新增代码+ClaudeCodeParity: 断言超长按住时间被拒绝；如果没有这行代码，坏参数可能让真实输入长时间卡住。
