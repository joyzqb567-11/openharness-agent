...
class WindowsComputerUseInteractiveGenericLaunchMaturityTests(unittest.TestCase):  # 新增代码+InteractiveGenericLaunchMaturity：类段开始，集中验收 Task5 交互通用启动成熟接线；如果没有这个类，蓝图第 5 项没有红绿灯保护。
    ...
    def _confirm_full_mode(self, workspace: Path) -> None:  # 新增代码+InteractiveGenericLaunchMaturity：函数段开始，用真实用户命令确认 full 模式；如果没有这段函数，测试会绕过强确认安全门。
        request_output = run_computer_terminal_command(workspace, "/computer use --full")  # 新增代码+InteractiveGenericLaunchMaturity：先申请 full 模式确认 token；如果没有这一行，后续确认命令没有合法 token。
        token = self._confirmation_token(request_output)  # 新增代码+InteractiveGenericLaunchMaturity：从申请输出读取动态 token；如果没有这一行，测试无法跟随真实确认流程。
        confirm_output = run_computer_terminal_command(workspace, f"/computer use --full-confirm {token}")  # 新增代码+InteractiveGenericLaunchMaturity：执行真实 full-confirm 命令；如果没有这一行，launch 命令应该仍被权限门拦截。
        self.assertIn("full_mode=true", confirm_output)  # 新增代码+InteractiveGenericLaunchMaturity：断言 full 模式确实打开；如果没有这一行，后续启动测试的前置条件可能是假的。
    # 新增代码+InteractiveGenericLaunchMaturity：函数段结束，_confirm_full_mode 到此结束；如果没有这个边界说明，初学者不容易看出 full 确认范围。
