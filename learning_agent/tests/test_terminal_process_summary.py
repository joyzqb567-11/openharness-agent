...
class TerminalProcessSummaryTests(unittest.TestCase):  # 新增代码+ProcessSummaryUX: 定义终端过程摘要测试类；若没有这行代码，unittest 无法组织这些断言。
    ...
    def test_renderer_prints_visual_observation_decision_note_even_when_turn_progress_was_throttled(self) -> None:  # 新增代码+VisibleObservationSummary: 函数段开始，复现模型已经在 decision_note 描述真实画面但终端没有显示的问题；若没有这段测试，用户会继续误以为模型没有观察截图。
        stream = io.StringIO()  # 新增代码+VisibleObservationSummary: 创建内存终端输出；若没有这行代码，测试无法检查用户实际能看到的过程摘要。
        renderer = TerminalProcessSummaryRenderer(stream=stream)  # 新增代码+VisibleObservationSummary: 创建过程摘要渲染器；若没有这行代码，模型事件不会被转换成人类可读文案。
        renderer.handle_event(create_agent_event("model_request_started", run_id="run_summary", sequence=1, session_id="session_summary", timestamp="2026-06-06T12:00:00Z", payload={"turn": 0}))  # 新增代码+VisibleObservationSummary: 先模拟同一轮已经打印过“正在规划”；若没有这行代码，无法复现真实运行中 completed 事件被同轮节流挡住的形状。
        renderer.handle_event(create_agent_event("model_message_completed", run_id="run_summary", sequence=2, session_id="session_summary", timestamp="2026-06-06T12:00:01Z", payload={"turn": 0, "message": {"decision_note": "已观察到画图窗口和空白画布，下一步使用画笔在画布中央拖拽绘制树的轮廓。", "tool_calls": [{"tool_name": "computer_action", "arguments": {"action": "drag_path"}}]}}))  # 新增代码+VisibleObservationSummary: 模拟生产主循环把视觉观察摘要放在 message.decision_note；若没有这行代码，测试无法证明终端应显示模型看到的画面。
        output = stream.getvalue()  # 新增代码+VisibleObservationSummary: 读取渲染后的终端文本；若没有这行代码，后续断言没有可检查对象。
        self.assertIn("已观察到画图窗口和空白画布", output)  # 新增代码+VisibleObservationSummary: 断言用户能看到模型观察到的画面状态；若没有这行代码，修复可能只显示工具名而继续隐藏观察内容。
        self.assertIn("computer_action", output)  # 新增代码+VisibleObservationSummary: 断言显示观察摘要时仍保留下一步工具选择；若没有这行代码，用户看不到观察和行动之间的联系。
        self.assertNotIn("base64", output.lower())  # 新增代码+VisibleObservationSummary: 断言终端不泄露图片编码或原始负载；若没有这行代码，安全摘要可能退化成原始日志输出。
        # 新增代码+VisibleObservationSummary: 函数段结束，锁定“显示模型看到什么，但不显示隐藏推理和原始图片数据”的用户体验；若没有这段注释，后续容易把观察摘要和内部推理混为一谈。
