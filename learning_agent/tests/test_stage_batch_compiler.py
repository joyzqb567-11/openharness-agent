...
def test_prepare_stage_creates_generic_new_resource_when_existing_file_title_seen() -> None:  # 新增代码+FreshResourceTest：函数段开始，验证旧文件标题会触发通用新资源动作；如果没有这个测试，新进程恢复旧文件会再次绕过阶段层。
    stage = StagePlan(stage_id="stage_prepare", stage_kind="prepare_target", target_ref="target-a", observation_policy="before_stage", verifier={"fresh_resource_required": True})  # 新增代码+FreshResourceTest：构造要求新资源的准备阶段；如果没有这行代码，测试无法表达写入任务边界。
    frame = {"target_window": {"title_preview": "draft.md - Generic Editor"}}  # 新增代码+FreshResourceTest：构造含通用文件扩展名的窗口标题；如果没有这行代码，编译器看不到已有资源证据。
    batch = compile_stage_to_batch(stage, frame, AppCapabilityProfile())  # 新增代码+FreshResourceTest：执行准备阶段编译；如果没有这行代码，无法判断是否插入新建动作。
    assert [action["type"] for action in batch.actions] == ["focus_window", "hotkey", "wait", "observe"]  # 新增代码+FreshResourceTest：确认准备批先聚焦再新建并观察；如果没有这行代码，旧资源可能直接被写入。
    assert batch.actions[1]["keys"] == ["CTRL", "N"]  # 新增代码+FreshResourceTest：确认使用通用应用内新建快捷键；如果没有这行代码，测试无法防止专项控制器回归。
    assert batch.guardrails["existing_resource_title_detected"] is True  # 新增代码+FreshResourceTest：确认 guardrail 记录检测到已有资源；如果没有这行代码，验收报告无法解释为什么新建。
# 新增代码+FreshResourceTest：函数段结束，test_prepare_stage_creates_generic_new_resource_when_existing_file_title_seen 到此结束；如果没有这个边界说明，初学者不容易看出测试范围。


def test_prepare_stage_keeps_fresh_resource_without_extra_new_shortcut() -> None:  # 新增代码+FreshResourceTest：函数段开始，验证新资源标题不会重复新建；如果没有这个测试，空白新窗口可能被无意义重置。
    stage = StagePlan(stage_id="stage_prepare", stage_kind="prepare_target", target_ref="target-a", observation_policy="before_stage", verifier={"fresh_resource_required": True})  # 新增代码+FreshResourceTest：构造要求新资源的准备阶段；如果没有这行代码，测试无法覆盖 fresh_resource_required。
    frame = {"target_window": {"title_preview": "Untitled - Generic Editor"}}  # 新增代码+FreshResourceTest：构造通用未命名标题；如果没有这行代码，测试无法证明新资源不会被误判。
    batch = compile_stage_to_batch(stage, frame, AppCapabilityProfile())  # 新增代码+FreshResourceTest：执行准备阶段编译；如果没有这行代码，无法读取动作列表。
    assert [action["type"] for action in batch.actions] == ["focus_window"]  # 新增代码+FreshResourceTest：确认新资源只需要聚焦；如果没有这行代码，执行层可能重复创建资源。
    assert batch.guardrails["existing_resource_title_detected"] is False  # 新增代码+FreshResourceTest：确认没有误报旧资源；如果没有这行代码，验收证据可能显示错误原因。
# 新增代码+FreshResourceTest：函数段结束，test_prepare_stage_keeps_fresh_resource_without_extra_new_shortcut 到此结束；如果没有这个边界说明，初学者不容易看出测试范围。
