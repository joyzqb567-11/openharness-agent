...
def test_text_payload_extraction_handles_input_text_label() -> None:  # 新增代码+TextPayloadLabelTest：函数段开始，验证“输入文本”这种用户常见说法不会把“文本”二字写进应用；如果没有这个测试，真实验收会输入多余说明词。
    prompt = "请使用本机文本编辑软件，输入文本 `hello everyone`，然后通过软件界面保存。"  # 新增代码+TextPayloadLabelTest：构造真实终端失败里出现的中文提示形态；如果没有这行代码，无法复现“文本 `hello everyone`”被当正文的问题。
    plan = UniversalDesktopStagePlanner().plan(prompt)  # 新增代码+TextPayloadLabelTest：生成通用阶段计划；如果没有这行代码，测试没有 planner 事实来源。
    perform_stage = next(stage for stage in plan.stages if stage.stage_kind == "perform_content_work")  # 新增代码+TextPayloadLabelTest：提取内容执行阶段；如果没有这行代码，无法读取 requested_text。
    assert perform_stage.verifier["requested_text"] == "hello everyone"  # 新增代码+TextPayloadLabelTest：确认只保留用户真正要求输入的正文；如果没有这行代码，批执行会污染应用文本。
# 新增代码+TextPayloadLabelTest：函数段结束，test_text_payload_extraction_handles_input_text_label 到此结束；如果没有这个边界说明，初学者不容易看出测试范围。


def test_text_payload_extraction_handles_exact_text_label() -> None:  # 新增代码+TextPayloadExactLabelTest：函数段开始，验证修复提示里的“准确文本”能被识别；如果没有这个测试，桌面任务修复轮会丢失正文。
    prompt = "继续完成当前编辑任务，准确文本：hello everyone，必须通过图形界面输入。"  # 新增代码+TextPayloadExactLabelTest：构造 acceptance 修复轮里出现的自然语言；如果没有这行代码，无法复现 requested_text 为空的问题。
    plan = UniversalDesktopStagePlanner().plan(prompt)  # 新增代码+TextPayloadExactLabelTest：生成通用阶段计划；如果没有这行代码，测试没有 planner 输出。
    perform_stage = next(stage for stage in plan.stages if stage.stage_kind == "perform_content_work")  # 新增代码+TextPayloadExactLabelTest：提取内容阶段；如果没有这行代码，无法读取抽取结果。
    assert perform_stage.verifier["requested_text"] == "hello everyone"  # 新增代码+TextPayloadExactLabelTest：确认“准确文本”后的内容被保留下来；如果没有这行代码，修复轮会空跑。
# 新增代码+TextPayloadExactLabelTest：函数段结束，test_text_payload_extraction_handles_exact_text_label 到此结束；如果没有这个边界说明，初学者不容易看出测试范围。
