...
from learning_agent.computer_use.interactive_launch_target import PHASE107_INTERACTIVE_LAUNCH_TARGET_MARKER, PHASE107_INTERACTIVE_LAUNCH_TARGET_OK_TOKEN, resolve_interactive_launch_target  # 新增代码+Phase107InteractiveLaunchTarget：导入即将实现的 Phase107 目标解析器；如果没有这行代码，测试无法锁定新合同。
...
class WindowsComputerUseInteractiveLaunchTargetPhase107Tests(unittest.TestCase):  # 新增代码+Phase107InteractiveLaunchTarget：类段开始，验证 Phase107 交互启动目标解析；如果没有这个类，别名识别和高风险拒绝没有回归保护。
    ...
    def _confirm_full_mode(self, workspace: Path) -> None:  # 新增代码+Phase107InteractiveLaunchTarget：函数段开始，按真实命令确认 full mode；如果没有这段函数，测试会绕过用户强确认设计。
        request_output = run_computer_terminal_command(workspace, "/computer use --full")  # 新增代码+Phase107InteractiveLaunchTarget：申请 full mode token；如果没有这行代码，确认命令没有合法 token。
        token = self._confirmation_token(request_output)  # 新增代码+Phase107InteractiveLaunchTarget：读取动态 token；如果没有这行代码，full-confirm 无法执行。
        confirm_output = run_computer_terminal_command(workspace, f"/computer use --full-confirm {token}")  # 新增代码+Phase107InteractiveLaunchTarget：用真实命令确认 full；如果没有这行代码，后续 launch 应被拒绝。
        self.assertIn("full_mode=true", confirm_output)  # 新增代码+Phase107InteractiveLaunchTarget：断言 full 已打开；如果没有这行代码，后续测试前置条件可能是假的。
    # 新增代码+Phase107InteractiveLaunchTarget：函数段结束，_confirm_full_mode 到此结束；如果没有这个边界说明，初学者不容易看出 full 确认范围。
    ...
    def test_launch_powershell_is_refused_before_any_real_action(self) -> None:  # 新增代码+Phase107InteractiveLaunchTarget：函数段开始，验证 powershell 高风险目标被零副作用拒绝；如果没有这段测试，命令可能打开终端类高风险应用。
        ...
        self.assertIn("decision=phase107_high_risk_target_refused", output)  # 新增代码+Phase107InteractiveLaunchTarget：断言拒绝原因明确；如果没有这行代码，高风险边界不可验收。
