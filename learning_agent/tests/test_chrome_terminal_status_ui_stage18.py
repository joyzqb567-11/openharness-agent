...
class ChromeTerminalStatusUiStage18Tests(unittest.TestCase):  # 新增代码+ChromeTerminalUI: 定义 Phase 6 的终端 Chrome 状态 UI 测试集合；若没有这段代码，新增 /chrome 命令没有自动化红线。
    ...
    def test_render_chrome_status_labels_current_state_risk_and_confirmation(self) -> None:  # 新增代码+Phase17ChromeOperableGuide: 验证 /chrome 可操作向导包含当前状态、风险和确认要求；如果没有这段测试，动作菜单仍然容易误操作。
        text = render_chrome_status({"browser": {"provider_status": {"chrome_extension": {"paired": False}, "native_host": {"installer_state": "registry_registered"}}}})  # 新增代码+Phase17ChromeOperableGuide: 构造已注册但未配对状态；如果没有这行代码，无法验证当前状态标签。
        self.assertIn("Chrome Current", text)  # 新增代码+Phase17ChromeOperableGuide: 输出必须有当前状态区；如果没有这行断言，用户仍要从多行字段自己推断。
        self.assertIn("state=registered_unpaired", text)  # 新增代码+Phase17ChromeOperableGuide: 已注册未配对应有明确状态码；如果没有这行断言，后续 guide 无法扫描。
        self.assertIn("risk=low confirm=no command=/chrome install-preview", text)  # 新增代码+Phase17ChromeOperableGuide: 只读预览命令必须标低风险且无需确认；如果没有这行断言，用户不知道哪些命令安全。
        self.assertIn("risk=high confirm=yes command=/chrome install-confirm", text)  # 新增代码+Phase17ChromeOperableGuide: 写 registry 命令必须标高风险且需要确认；如果没有这行断言，生产写入边界不清楚。
        self.assertIn("risk=medium confirm=yes command=/chrome pairing-start-confirm", text)  # 新增代码+Phase17ChromeOperableGuide: 写 bridge 请求命令必须标中风险且需要确认；如果没有这行断言，配对触发安全边界不清楚。
        self.assertLessEqual(max(len(line) for line in text.splitlines()), 260)  # 新增代码+Phase17ChromeOperableGuide: 可见终端单行不应过长；如果没有这行断言，动作菜单可能横向溢出。
