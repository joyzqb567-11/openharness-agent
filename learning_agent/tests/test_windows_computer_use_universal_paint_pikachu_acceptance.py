...
class UniversalPaintPikachuAcceptanceTests(unittest.TestCase):  # 新增代码+URG5PaintPikachu：测试类段开始，集中验证 Paint/Pikachu 代表验收是否接在通用真实 GUI 链路上；如果没有这个类，URG-5 没有自动护栏。
    def test_contract_reports_representative_paint_as_not_real_maturity(self) -> None:  # 修改代码+源码复核门禁：函数段开始，验证代表性 Paint 验收不能冒充真实成熟；如果没有这段测试，字段可能只写文案不写机器事实。
        ...
        self.assertFalse(report["passed"])  # 修改代码+源码复核门禁：断言代表性合同不能作为真实成熟通过；如果没有这一行，局部字段成功会掩盖整体失败。
        ...
        self.assertFalse(report["real_paint_window_verified"])  # 修改代码+源码复核门禁：断言代表窗口不能算真实 Paint 窗口；如果没有这一行，动作目标可能没有窗口边界。
        self.assertFalse(report["real_canvas_region_detected"])  # 修改代码+源码复核门禁：断言代表画布不能算真实画布区域；如果没有这一行，拖拽可能没有落点。
        self.assertFalse(report["real_drag_path_dispatched"])  # 修改代码+源码复核门禁：断言代表性 sender 不能算真实拖拽派发；如果没有这一行，画图可能只是计划文本。
        self.assertFalse(report["canvas_changed_after_real_actions"])  # 修改代码+源码复核门禁：断言模拟画布变化不能算真实动作后变化；如果没有这一行，空画布也可能误过。
        ...
        self.assertFalse(report["real_desktop_execution_mature"])  # 修改代码+源码复核门禁：断言未接入物理桌面前不能报告真实桌面成熟；如果没有这一行，最终矩阵会继续误判。
        self.assertTrue(report["representative_drag_path_dispatched"])  # 新增代码+源码复核门禁：断言代表性拖拽仍被记录为开发证据；如果没有这一行，排查时会看不到模拟链路是否存在。
        self.assertFalse(report["physical_desktop_dispatch_performed"])  # 新增代码+源码复核门禁：断言物理桌面派发没有发生；如果没有这一行，用户无法区分 representative 和 real。
        ...
        self.assertNotIn(PHASE120_UNIVERSAL_PAINT_PIKACHU_ACCEPTANCE_OK_TOKEN, cli_line)  # 修改代码+源码复核门禁：断言未真实成熟时 CLI 不能输出 OK token；如果没有这一行，失败输出无法被稳定识别。
    ...
    def test_main_prints_visible_terminal_tokens(self) -> None:  # 新增代码+URG5PaintPikachu：函数段开始，验证命令行入口会打印 URG-5 固定 token；如果没有这段测试，可见终端验收可能匹配不到最终答案。
        ...
        with redirect_stdout(buffer):  # 新增代码+URG5PaintPikachu：捕获 main 的标准输出；如果没有这一行，断言只能看退出码。
            exit_code = main([])  # 修改代码+源码复核门禁：运行 URG-5 命令行入口并期待当前真实成熟失败；如果没有这一行，真实终端入口没有自动化覆盖。
        output = buffer.getvalue()  # 新增代码+URG5PaintPikachu：读取捕获输出；如果没有这一行，后续 token 断言没有对象。
        self.assertEqual(1, exit_code)  # 修改代码+源码复核门禁：断言未真实成熟时入口必须失败退出；如果没有这一行，失败也可能因为打印了字段被误判。
        self.assertIn("paint_is_acceptance_only=true", output)  # 新增代码+URG5PaintPikachu：断言 Paint 样本定位 token；如果没有这一行，用户看不到架构边界。
        self.assertIn("real_drag_path_dispatched=false", output)  # 修改代码+源码复核门禁：断言真实拖拽派发为 false；如果没有这一行，用户看不到动作事实。
