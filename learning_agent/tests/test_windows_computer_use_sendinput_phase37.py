...
from learning_agent.computer_use.controller import ComputerUseController, WindowsComputerUseBackend, build_default_computer_use_backend  # 修改代码+RealComputerActionExecutor: 导入默认后端工厂用于验证真实用户路径；如果没有这行代码，测试只能证明手动注入路径，无法发现 start_oauth_agent.bat 里真实动作执行器缺实现。
...
class WindowsComputerUseSendInputPhase37Tests(unittest.TestCase):  # 新增代码+Phase37WindowsSendInputExecutor: 定义 Phase37 SendInput 执行器测试集合；如果没有这个类，unittest 不会组织这些安全契约检查。
    ...
    def test_default_real_backend_injects_real_sendinput_dispatcher(self) -> None:  # 新增代码+RealComputerActionExecutor: 函数段开始，验证默认真实后端会带上可执行的低层输入实现；如果没有这个测试，真实终端会再次出现 implementation_available=false。
        backend = build_default_computer_use_backend({"LEARNING_AGENT_ENABLE_WINDOWS_COMPUTER_USE": "1"})  # 新增代码+RealComputerActionExecutor: 用显式环境创建默认真实后端；如果没有这行代码，测试无法复现普通用户开启 /computer use --full 后的默认后端形态。
        status = backend.status()  # 新增代码+RealComputerActionExecutor: 读取后端状态用于检查 action executor；如果没有这行代码，断言没有事实来源。
        executor_status = status["action_executor"]  # 新增代码+RealComputerActionExecutor: 提取动作执行器状态；如果没有这行代码，测试无法精确定位到 SendInput 实现是否接上。
        self.assertTrue(executor_status["implementation_available"])  # 新增代码+RealComputerActionExecutor: 断言默认真实后端已注入 sendinput_impl；如果没有这行断言，implementation_available=false 的真实终端失败会漏过。
        self.assertTrue(executor_status["real_input_enabled"])  # 新增代码+RealComputerActionExecutor: 断言 executor 在 Windows+启用+实现存在时进入真实输入可用状态；如果没有这行断言，工具仍可能拒绝鼠标键盘动作。
        dispatch_status = executor_status["dispatch"]["status"]  # 新增代码+RealComputerActionExecutor: 提取 Phase47 dispatcher 状态；如果没有这行代码，只能看到外层 executor，不能证明最后一跳低层 sender 可用。
        self.assertEqual(dispatch_status["model"], "phase47_windows_sendinput_dispatcher")  # 新增代码+RealComputerActionExecutor: 断言实现来自通用 dispatcher 而不是新造旁路；如果没有这行断言，代码可能绕过既有 Phase47 目标校验模型。
        self.assertTrue(dispatch_status["sender_available"])  # 新增代码+RealComputerActionExecutor: 断言 dispatcher 背后有低层 sender；如果没有这行断言，事件展开后仍可能没有真实发送对象。
    # 新增代码+RealComputerActionExecutor: 函数段结束，test_default_real_backend_injects_real_sendinput_dispatcher 到此结束；如果没有这个边界说明，初学者不容易看出默认真实后端接线测试范围。
    def test_controller_routes_generic_drag_path_to_sendinput(self) -> None:  # 新增代码+GenericDragPathToolSurface: 函数段开始，验证模型主循环可用的通用拖拽路径能进入 SendInput；如果没有这个测试，画图类任务会继续只能点选不能连续画线。
        recorder = Phase37RecordingInputImplementation()  # 新增代码+GenericDragPathToolSurface: 创建记录型输入实现；如果没有这行代码，测试无法在不触碰真实鼠标的情况下检查发送事件。
        executor = WindowsSendInputExecutor(platform="win32", enabled=True, sendinput_impl=recorder)  # 新增代码+GenericDragPathToolSurface: 创建启用但注入 fake 的执行器；如果没有这行代码，测试不能覆盖真实执行器入口。
        window = self._window()  # 新增代码+GenericDragPathToolSurface: 取得带 rect 的可信窗口样本；如果没有这行代码，窗口相对点无法转换为屏幕坐标。
        inventory = StaticWindowsWindowInventory([window], captured_at="2026-06-06T00:00:00Z", source="phase37-drag-static")  # 新增代码+GenericDragPathToolSurface: 用静态窗口目录通过目标校验；如果没有这行代码，controller 会读取真实桌面导致测试不稳定。
        backend = WindowsComputerUseBackend(inventory=inventory, real_actions_enabled=True, action_executor=executor)  # 新增代码+GenericDragPathToolSurface: 把 SendInput 执行器注入 Windows 后端；如果没有这行代码，动作不会走被测的真实执行链。
        path_points = [{"x": 10, "y": 20}, {"x": 30, "y": 40}, {"x": 50, "y": 60}]  # 新增代码+GenericDragPathToolSurface: 准备窗口相对拖拽点；如果没有这行代码，测试没有连续绘图路径输入。
        with tempfile.TemporaryDirectory() as temp_dir:  # 新增代码+GenericDragPathToolSurface: 创建临时锁目录；如果没有这行代码，测试可能污染真实 desktop lock。
            lock_manager = ComputerUseLockManager(base_dir=Path(temp_dir))  # 新增代码+GenericDragPathToolSurface: 创建隔离锁管理器；如果没有这行代码，controller 会因缺少当前锁拒绝动作。
            lock_manager.acquire("phase37-session", owner_label="unit-test")  # 新增代码+GenericDragPathToolSurface: 给测试会话持有桌面控制锁；如果没有这行代码，安全门会在后端前拦截。
            controller = ComputerUseController(backend=backend, lock_manager=lock_manager, owner_session_id="phase37-session")  # 新增代码+GenericDragPathToolSurface: 创建完整 controller；如果没有这行代码，测试不能覆盖模型工具到后端的真实路由。
            result = controller.execute({"action": "drag_path", "confirm_desktop_control": True, "window": window, "points": path_points})  # 新增代码+GenericDragPathToolSurface: 执行通用拖拽动作；如果没有这行代码，红灯测试不会暴露 drag_path 未接入的问题。
        self.assertTrue(result.ok)  # 新增代码+GenericDragPathToolSurface: 断言 controller 接受并执行 drag_path；如果没有这行断言，动作被拒绝也可能被忽略。
        self.assertEqual(recorder.calls[0]["type"], "drag_path")  # 新增代码+GenericDragPathToolSurface: 断言执行器生成通用拖拽事件；如果没有这行断言，路径可能被错误拆成不可审计的点选。
        self.assertEqual(recorder.calls[0]["points"][0], {"x": 110, "y": 220})  # 新增代码+GenericDragPathToolSurface: 断言第一个点从窗口相对坐标转换到屏幕坐标；如果没有这行断言，真实画线会落错位置。
        self.assertEqual(recorder.calls[0]["points"][-1], {"x": 150, "y": 260})  # 新增代码+GenericDragPathToolSurface: 断言最后一个点也完成转换；如果没有这行断言，路径可能只转换起点。
        self.assertEqual(result.data["action_evidence"]["coordinate_used"]["point_count"], 3)  # 新增代码+GenericDragPathToolSurface: 断言证据记录路径点数量；如果没有这行断言，用户无法从日志知道画线动作规模。
    # 新增代码+GenericDragPathToolSurface: 函数段结束，test_controller_routes_generic_drag_path_to_sendinput 到此结束；如果没有这个边界说明，初学者不容易看出通用拖拽测试范围。
