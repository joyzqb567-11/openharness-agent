...
class UniversalRealGuiComputerUseFinalMatrixTests(unittest.TestCase):  # 新增代码+URG6FinalMatrix：测试类段开始，集中验证跨应用最终成熟度矩阵；如果没有这个类，URG-6 没有自动护栏。
    def _required_tokens(self) -> list[str]:  # 新增代码+URG6FinalMatrix：函数段开始，集中列出蓝图要求的最终 token；如果没有这段函数，多处断言容易漂移。
        return ["UNIVERSAL_REAL_GUI_COMPUTER_USE_READY", "single_universal_real_gui_loop=true", "per_app_controller_required=false", "hardcoded_app_whitelist_required=false", "ordinary_apps_controlled_by_generic_runtime=true", "representative_apps_are_acceptance_only=true", "real_window_observation=false", "real_uia_or_vision_targeting=false", "real_sendinput_dispatch=false", "target_identity_rechecked_before_each_action=true", "observe_plan_act_verify_loop=true", "paint_pikachu_real_acceptance=false", "notepad_real_acceptance=false", "calculator_real_acceptance=false", "browser_real_acceptance=false", "script_artifact_route_blocked=true", "real_desktop_execution_mature=false", "uncontrolled_high_risk_actions_allowed=false"]  # 修改代码+源码复核门禁：把代表性 sender 场景的期望改成不成熟；如果没有这一行，测试会继续把模拟证据误判为真实成熟。
    ...
    def test_final_matrix_reports_all_blueprint_tokens(self) -> None:  # 新增代码+URG6FinalMatrix：函数段开始，验证结构化矩阵和 CLI 行；如果没有这段测试，最终输出可能只覆盖部分应用。
        ...
        self.assertFalse(report["passed"])  # 修改代码+源码复核门禁：断言代表性证据不能让最终矩阵通过；如果没有这一行，假成熟会继续从自动测试溜走。
        self.assertFalse(report["paint_pikachu_real_acceptance"])  # 修改代码+源码复核门禁：断言 Paint/Pikachu 未接入真实桌面前不能算真实验收；如果没有这一行，核心场景会被代表记录冒充。
        self.assertFalse(report["notepad_real_acceptance"])  # 修改代码+源码复核门禁：断言 Notepad 代表动作未真实触桌面前不能算真实验收；如果没有这一行，文本输入成熟度会被误报。
        self.assertFalse(report["calculator_real_acceptance"])  # 修改代码+源码复核门禁：断言 Calculator 代表动作未真实触桌面前不能算真实验收；如果没有这一行，计算器成熟度会被误报。
        self.assertFalse(report["browser_real_acceptance"])  # 修改代码+源码复核门禁：断言 Browser 代表动作未真实触桌面前不能算真实验收；如果没有这一行，浏览器成熟度会被误报。
    ...
    def test_main_prints_final_marker(self) -> None:  # 新增代码+URG6FinalMatrix：函数段开始，验证命令行入口输出最终 marker；如果没有这段测试，可见终端场景可能没有锚点。
        ...
        with redirect_stdout(buffer):  # 新增代码+URG6FinalMatrix：捕获标准输出；如果没有这一行，断言只能看退出码。
            exit_code = main([])  # 修改代码+源码复核门禁：运行 URG-6 命令行入口并期待当前真实成熟失败；如果没有这一行，真实终端入口没有自动化覆盖。
        output = buffer.getvalue()  # 新增代码+URG6FinalMatrix：读取输出文本；如果没有这一行，后续 token 断言没有对象。
        self.assertEqual(1, exit_code)  # 修改代码+源码复核门禁：断言尚未真实成熟时 CLI 必须失败退出；如果没有这一行，自动化会继续把假成熟当成功。
        self.assertIn(UNIVERSAL_REAL_GUI_COMPUTER_USE_MARKER, output)  # 新增代码+URG6FinalMatrix：断言最终 marker 出现；如果没有这一行，用户看不到终局标识。
        self.assertIn("real_desktop_execution_mature=false", output)  # 修改代码+源码复核门禁：断言输出明确显示不成熟；如果没有这一行，用户看不到真实边界。
