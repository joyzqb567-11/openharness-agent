...
from learning_agent.computer_use_mcp_v2.windows_runtime.fresh_target_policy import (  # 新增代码+FreshTargetPolicyTest：导入被测 FreshTarget 策略；如果没有这一行，测试无法覆盖新硬门禁。
    ...
    decide_recovery_after_drift,  # 新增代码+DriftRecoveryTest：导入漂移恢复策略；如果没有这一行，测试无法约束 target_lease_not_verified 不能反复启动应用。
...
def test_target_lease_not_verified_recovery_stops_relaunch_loop() -> None:  # 新增代码+DriftRecoveryTest：函数段开始，验证租约未验证时不能继续循环启动应用；如果没有这段测试，agent 会像截图里一样反复 open_application。
    recovery = decide_recovery_after_drift("cu-target-test-0001", "target_lease_not_verified")  # 新增代码+DriftRecoveryTest：生成租约未验证后的恢复建议；如果没有这一行，测试没有被测输出。
    assert recovery["decision"] == "target_lease_invalidated"  # 新增代码+DriftRecoveryTest：确认失效决策 token 稳定；如果没有这一行，上层无法识别 target_ref 已不可用。
    assert recovery["fresh_target_relaunch_required"] is False  # 新增代码+DriftRecoveryTest：确认不要求立即重启应用；如果没有这一行，模型会把底层绑定问题误当成需要再打开软件。
    assert "observe" in recovery["recovery_next_allowed_actions"]  # 新增代码+DriftRecoveryTest：确认允许观察以收集证据；如果没有这一行，agent 失去无副作用排查能力。
    assert "launch_app" not in recovery["recovery_next_allowed_actions"]  # 新增代码+DriftRecoveryTest：确认不允许直接重启应用；如果没有这一行，反复启动窗口的问题会回归。
    assert recovery["low_level_event_count"] == 0  # 新增代码+DriftRecoveryTest：确认恢复建议本身零桌面动作；如果没有这一行，拒绝路径可能产生副作用。
# 新增代码+DriftRecoveryTest：函数段结束，test_target_lease_not_verified_recovery_stops_relaunch_loop 到此结束；如果没有这个边界说明，用户不容易看出循环启动保护范围。
