...
class WindowsPaintPikachuRealExecutionLoopTests(unittest.TestCase):  # 新增代码+PaintPikachuRealLoop：类段开始，集中验证 Paint 皮卡丘真实闭环；如果没有这个类，测试用例无法组织运行。
    ...
    def test_elephant_prompt_uses_elephant_plan_not_pikachu_plan(self) -> None:  # 新增代码+GenericPaintSubject：函数段开始，验证大象 prompt 不能再复用皮卡丘计划；如果没有这个测试，用户输入大象时仍会画出皮卡丘。
        with tempfile.TemporaryDirectory() as raw_dir:  # 新增代码+GenericPaintSubject：创建隔离临时目录保存 fake 截图；如果没有这一行，测试图片会污染项目真实证据目录。
            screenshot_path = Path(raw_dir) / "paint_elephant_after.png"  # 新增代码+GenericPaintSubject：定义大象测试的 fake 截图路径；如果没有这一行，截图 runtime 没有可返回的证据文件。
            write_fake_paint_screenshot(screenshot_path, changed=True)  # 新增代码+GenericPaintSubject：生成带笔迹的 fake Paint 截图；如果没有这一行，测试会失败在视觉门禁而不是绘画对象选择。
            sender = FakeLowLevelSender()  # 新增代码+GenericPaintSubject：创建 fake sender 记录低层事件；如果没有这一行，测试无法证明大象计划也能展开成真实拖拽事件。
            loop = WindowsPaintPikachuRealExecutionLoop(inventory=FakePaintInventory(), low_level_sender=sender, launch_backend=FakePaintLaunchBackend(), screenshot_runtime=FakePaintScreenshotRuntime(screenshot_path), platform="win32", poll_timeout_seconds=0.01)  # 新增代码+GenericPaintSubject：创建注入 fake 组件的 Paint loop；如果没有这一行，测试可能真实打开 Paint。
            report = loop.run_desktop_task(target_app="mspaint", prompt="请使用本机电脑画图软件画一个大象")  # 新增代码+GenericPaintSubject：运行用户实际反馈的大象 prompt；如果没有这一行，bug 没有被真实复现。
            elements = set(report["drawing_plan"]["elements"])  # 新增代码+GenericPaintSubject：读取返回的绘图元素集合；如果没有这一行，测试无法判断画的是大象还是皮卡丘。
            self.assertEqual(report.get("drawing_subject"), "elephant")  # 修改代码+GenericPaintSubject：用 get 读取报告对象以产生清晰断言失败；如果没有这一行，旧代码会用 KeyError 掩盖“仍未识别大象”的真实问题。
            self.assertIn("trunk", elements)  # 新增代码+GenericPaintSubject：断言大象鼻子进入绘图计划；如果没有这一行，大象最关键特征可能缺失。
            self.assertIn("large_ears", elements)  # 新增代码+GenericPaintSubject：断言大象大耳朵进入绘图计划；如果没有这一行，图形可能不像大象。
            self.assertNotIn("lightning_tail", elements)  # 新增代码+GenericPaintSubject：断言不再出现皮卡丘闪电尾巴；如果没有这一行，旧硬编码计划会继续通过。
            self.assertNotIn("red_cheeks", elements)  # 新增代码+GenericPaintSubject：断言不再出现皮卡丘红脸颊；如果没有这一行，大象 prompt 仍可能混入皮卡丘元素。
            self.assertEqual(report["decision"], "paint_elephant_real_execution_finished")  # 新增代码+GenericPaintSubject：断言成功决策码表达大象；如果没有这一行，终端仍会显示 paint_pikachu_real_execution_finished 误导用户。
            self.assertGreater(len(sender.events), 0)  # 新增代码+GenericPaintSubject：断言大象计划生成低层事件；如果没有这一行，测试只验证字段不验证可执行性。
    # 新增代码+GenericPaintSubject：函数段结束，test_elephant_prompt_uses_elephant_plan_not_pikachu_plan 到此结束；如果没有这个边界说明，初学者不容易看出大象回归测试范围。

    def test_cat_prompt_uses_cat_plan_not_pikachu_or_elephant_plan(self) -> None:  # 新增代码+PaintCatSubject：函数段开始，验证猫 prompt 必须进入猫计划；如果没有这个测试，用户输入猫时仍可能被旧皮卡丘/大象特例误导。
        with tempfile.TemporaryDirectory() as raw_dir:  # 新增代码+PaintCatSubject：创建隔离临时目录保存 fake 截图；如果没有这一行，测试图片会污染项目真实证据目录。
            screenshot_path = Path(raw_dir) / "paint_cat_after.png"  # 新增代码+PaintCatSubject：定义猫测试的 fake 截图路径；如果没有这一行，截图 runtime 没有可返回的证据文件。
            write_fake_paint_screenshot(screenshot_path, changed=True)  # 新增代码+PaintCatSubject：生成带笔迹的 fake Paint 截图；如果没有这一行，测试会失败在视觉门禁而不是猫对象选择。
            sender = FakeLowLevelSender()  # 新增代码+PaintCatSubject：创建 fake sender 记录低层事件；如果没有这一行，测试无法证明猫计划也能展开成真实拖拽事件。
            loop = WindowsPaintPikachuRealExecutionLoop(inventory=FakePaintInventory(), low_level_sender=sender, launch_backend=FakePaintLaunchBackend(), screenshot_runtime=FakePaintScreenshotRuntime(screenshot_path), platform="win32", poll_timeout_seconds=0.01)  # 新增代码+PaintCatSubject：创建注入 fake 组件的 Paint loop；如果没有这一行，测试可能真实打开 Paint。
            report = loop.run_desktop_task(target_app="mspaint", prompt="请使用本机电脑画图软件画一只猫")  # 新增代码+PaintCatSubject：运行真实用户风格猫 prompt；如果没有这一行，猫 bug 没有被真实复现。
            elements = set(report["drawing_plan"]["elements"])  # 新增代码+PaintCatSubject：读取返回的绘图元素集合；如果没有这一行，测试无法判断画的是猫还是旧对象。
            self.assertEqual(report.get("drawing_subject"), "cat")  # 新增代码+PaintCatSubject：断言报告对象为猫；如果没有这一行，未知对象拒绝或旧对象 fallback 会漏过。
            self.assertIn("cat_ears", elements)  # 新增代码+PaintCatSubject：断言猫耳朵进入绘图计划；如果没有这一行，猫的头部特征可能缺失。
            self.assertIn("whiskers", elements)  # 新增代码+PaintCatSubject：断言胡须进入绘图计划；如果没有这一行，猫计划可能只是普通圆脸。
            self.assertIn("curled_tail", elements)  # 新增代码+PaintCatSubject：断言猫尾巴进入绘图计划；如果没有这一行，猫的身体特征不足。
            self.assertNotIn("lightning_tail", elements)  # 新增代码+PaintCatSubject：断言不再出现皮卡丘闪电尾巴；如果没有这一行，旧硬编码计划会继续通过。
            self.assertNotIn("trunk", elements)  # 新增代码+PaintCatSubject：断言不再出现大象鼻子；如果没有这一行，猫计划可能混入大象元素。
            self.assertEqual(report["decision"], "paint_cat_real_execution_finished")  # 新增代码+PaintCatSubject：断言成功决策码表达猫；如果没有这一行，终端仍可能显示其他对象误导用户。
            self.assertGreater(len(sender.events), 0)  # 新增代码+PaintCatSubject：断言猫计划生成低层事件；如果没有这一行，测试只验证字段不验证可执行性。
    # 新增代码+PaintCatSubject：函数段结束，test_cat_prompt_uses_cat_plan_not_pikachu_or_elephant_plan 到此结束；如果没有这个边界说明，初学者不容易看出猫回归测试范围。
