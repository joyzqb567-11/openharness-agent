...
DEFAULT_MODEL_TOOL_LOOP_MAX_TURNS: int | None = None  # 修改代码+上下文收束闭环: 默认不再用固定轮次截断工具循环；如果没有这行代码，项目会继续把 40 轮当成治本方案而违背 ClaudeCode 式主循环语义。
...
class AgentRuntimeConfig:  # 新增代码+CoreConfig: 保存 learning_agent 运行时策略；如果没有这个类，主循环配置缺少统一容器。
    max_turns: int | None = DEFAULT_MODEL_TOOL_LOOP_MAX_TURNS  # 修改代码+上下文收束闭环: 默认使用 None 表示不按固定轮次主动停止；如果没有这行代码，运行配置会继续保护错误的 40 轮治标方案。
...
def load_agent_runtime_config(workspace: str | Path) -> AgentRuntimeConfig:  # 新增代码+CoreConfig: 从工作区配置文件和环境变量加载运行策略；如果没有这行代码，max_turns 只能靠代码传入。
    ...
    max_turns: int | None = DEFAULT_MODEL_TOOL_LOOP_MAX_TURNS  # 修改代码+上下文收束闭环: 缺配置时默认不按固定轮次主动停止；如果没有这行代码，start_oauth_agent.bat 会继续继承 40 轮治标策略。
    ...
    if config_path.exists():  # 新增代码+CoreConfig: 只有配置文件存在时才读取；如果没有这行代码，缺省启动会因为找不到文件报错。
        ...
        if "max_turns" in payload:  # 新增代码+CoreConfig: 只在配置显式提供时解析；如果没有这行代码，缺字段会被误当错误。
            raw_file_max_turns = payload.get("max_turns")  # 修改代码+工具循环护栏: 先取出文件里的原始 max_turns；如果没有这行代码，无法区分文件 null 和普通数字。
            if raw_file_max_turns is None:  # 修改代码+上下文收束闭环: 把 runtime_config.json 里的 null 当作不按固定轮次主动停止；如果没有这行代码，用户写 null 的语义会继续被错误改成 40。
                max_turns = None  # 修改代码+上下文收束闭环: 明确让文件 null 进入无固定轮次策略；如果没有这行代码，配置层会继续把治标护栏伪装成默认生产策略。
            else:  # 修改代码+工具循环护栏: 文件里不是 null 时走原有数字或字符串解析；如果没有这行代码，合法配置值会被跳过。
                max_turns = parse_max_turns_value(raw_file_max_turns, f"{config_path} 的 max_turns")  # 修改代码+工具循环护栏: 解析文件中的 max_turns；如果没有这行代码，配置文件数字或显式 unlimited 字符串不会生效。
