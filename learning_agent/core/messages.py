...
class ModelMessage:  # 新增代码+CoreMessages: 表示模型返回给 agent 的一条消息；如果没有这个类，文本回答和工具调用会缺少统一容器。
    ...
    native_output_items: list[dict[str, Any]] = field(default_factory=list)  # 新增代码+OAuthNativeOutputParser: 保存 Responses 原生 output item 审计证据；如果没有这一行，tool_search_call/tool_search_output 会在转成 ToolCall 后丢失。
