...
try:  # 修改代码+CoreSplit: 优先从新 core 包导入配置与消息结构；若没有这行代码，learning_agent.py 会继续承载过多基础定义。
    ...
    if error.name not in {"learning_agent", "learning_agent.core", "learning_agent.core.config", "learning_agent.core.messages"}:  # 修改代码+CoreSplit: 只允许目标包路径缺失时 fallback；若没有这行代码，core 内部真实 bug 会被误吞。
...
try:  # 修改代码+ModelsSplit: 优先从新 models 包导入模型接口和适配器；若没有这行代码，learning_agent.py 会继续承载模型实现细节。
    from learning_agent.models.base import ChatModel  # 修改代码+ModelsSplit: 包运行模式下导入模型接口；若没有这行代码，LearningAgent 类型边界会留在主文件。
    ...
    from models.base import ChatModel  # 修改代码+ModelsSplit: 脚本运行模式下导入模型接口；若没有这行代码，直接执行文件时 LearningAgent 类型边界会找不到。
