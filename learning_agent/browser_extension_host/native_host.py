...
import os  # 新增代码+Phase24RealExtensionE2E: 读取 launcher 注入的工作区环境变量；如果没有这行代码，Chrome 启动 host 时只能依赖不可靠的当前目录。
...
NATIVE_HOST_WORKSPACE_ENV = "OPENHARNESS_LEARNING_AGENT_WORKSPACE"  # 新增代码+Phase24RealExtensionE2E: 固定 native host 工作区环境变量名；如果没有这行代码，launcher 和 host 之间没有稳定约定。


def resolve_native_host_workspace(cwd: Path | None = None) -> Path:  # 新增代码+Phase24RealExtensionE2E: 解析 Chrome native host 应写入的 learning_agent 工作区；如果没有这段函数，真实 Chrome 从任意 cwd 启动时会写错 memory。
    env_workspace = os.environ.get(NATIVE_HOST_WORKSPACE_ENV, "").strip()  # 新增代码+Phase24RealExtensionE2E: 优先读取 launcher 注入的工作区；如果没有这行代码，Chrome 当前目录会继续决定状态文件位置。
    if env_workspace:  # 新增代码+Phase24RealExtensionE2E: 环境变量存在时直接使用；如果没有这行代码，显式配置不会生效。
        return Path(env_workspace)  # 新增代码+Phase24RealExtensionE2E: 返回环境变量路径；如果没有这行代码，host 仍然不能定位真实项目 memory。
    base = Path(cwd) if cwd is not None else Path.cwd()  # 新增代码+Phase24RealExtensionE2E: 没有环境变量时保留旧的 cwd 兼容逻辑；如果没有这行代码，旧测试和手动运行会断开。
    if base.name.lower() == "learning_agent":  # 新增代码+Phase24RealExtensionE2E: 识别已经位于 learning_agent 包目录的场景；如果没有这行代码，路径会重复拼出 learning_agent/learning_agent。
        return base  # 新增代码+Phase24RealExtensionE2E: 直接返回包目录；如果没有这行代码，真实 bat 工作区会被错误扩展。
    return base / "learning_agent"  # 新增代码+Phase24RealExtensionE2E: 兼容从仓库根目录启动 host 的旧路径；如果没有这行代码，本地开发直接运行会找不到 memory。


def bridge_state_path_for_workspace(workspace: Path) -> Path:  # 新增代码+Phase24RealExtensionE2E: 计算 bridge 状态文件路径；如果没有这段函数，main 和测试会重复拼路径，容易再次分裂。
    return Path(workspace) / "memory" / "chrome_extension_bridge.json"  # 新增代码+Phase24RealExtensionE2E: bridge 固定写入 learning_agent/memory；如果没有这行代码，扩展连接状态不会被 /chrome 读到。


def runtime_queue_dir_for_workspace(workspace: Path) -> Path:  # 新增代码+Phase24RealExtensionE2E: 计算浏览器 prompt 的 runtime queue 目录；如果没有这段函数，Chrome 侧 prompt 可能进入错误队列。
    return Path(workspace) / "memory" / "runtime"  # 新增代码+Phase24RealExtensionE2E: runtime queue 固定写入同一工作区；如果没有这行代码，session sync 证据会和 agent 主循环分离。
...
def main() -> int:  # 修改代码+Phase24RealExtensionE2E: native host 主入口现在使用稳定工作区；若没有这行代码，脚本只能被导入不能运行，也不能避免 Chrome cwd 风险。
    workspace = resolve_native_host_workspace()  # 新增代码+Phase24RealExtensionE2E: 解析真实 learning_agent 工作区；如果没有这行代码，下面两个状态目录会继续依赖不可靠 cwd。
    state_path = bridge_state_path_for_workspace(workspace)  # 修改代码+Phase24RealExtensionE2E: bridge 状态写入稳定工作区；如果没有这行代码，/chrome 可能读不到真实扩展连接。
    bridge = ChromeExtensionBridgeState(state_path)  # 新增代码+ChromeExtensionStage5: 创建 bridge 状态对象；若没有这行代码，消息无法持久化。
    command_queue = RuntimeCommandQueue(runtime_queue_dir_for_workspace(workspace))  # 修改代码+Phase24RealExtensionE2E: 浏览器 prompt 写入同一工作区队列；如果没有这行代码，Chrome 侧任务可能不会进入当前 agent 主循环。
