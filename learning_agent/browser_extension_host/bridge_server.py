...
class ChromeExtensionBridgeState:  # 新增代码+ChromeExtensionStage5: 保存 Chrome 扩展只读连接状态；若没有这个类，provider 无法读取插件结果。
    ...
    def enqueue_browser_prompt(self, command_queue: Any, message: dict[str, Any]) -> dict[str, Any]:  # 新增代码+Phase2Pairing: 把浏览器侧 prompt 写入 durable queue；如果没有这段函数，扩展发起任务会绕过主循环。
        ...
        browser_prompt_payload = {"prompt": prompt, "url": url, "title": title, "tab_id": tab_id, "selected_text": selected_text, "source": "chrome_extension"}  # 新增代码+Phase16SessionSyncClosure: 保存机器可读浏览器来源字段；如果没有这行代码，verifier 只能从文本里猜 URL/title/tab。
        command = command_queue.enqueue(mode="prompt", payload={"text": composed_prompt, "browser_prompt": browser_prompt_payload}, priority="next")  # 修改代码+Phase16SessionSyncClosure: 仍用 prompt 模式但保留结构化 browser_prompt；如果没有这行代码，主循环能消费但审计无法证明来源。
