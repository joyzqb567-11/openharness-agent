...
def _workspace_for_host_script(host_script: Path) -> Path:  # 新增代码+Phase24RealExtensionE2E: 从 native_host.py 路径推导 learning_agent 工作区；如果没有这段函数，launcher 无法告诉 Chrome host 应该把 bridge 写到哪里。
    safe_host_script = Path(host_script)  # 新增代码+Phase24RealExtensionE2E: 规范化 host 脚本路径；如果没有这行代码，字符串路径没有 parents 可用。
    if safe_host_script.parent.name == "browser_extension_host":  # 新增代码+Phase24RealExtensionE2E: 识别项目内 browser_extension_host/native_host.py；如果没有这行代码，工作区可能被推成错误层级。
        return safe_host_script.parent.parent  # 新增代码+Phase24RealExtensionE2E: 返回 learning_agent 包目录；如果没有这行代码，/chrome 会读不到真实扩展状态。
    return safe_host_script.parent  # 新增代码+Phase24RealExtensionE2E: 非标准路径时退回脚本所在目录；如果没有这行代码，测试或外部脚本没有兜底工作区。


def build_native_host_launcher(target_dir: Path, python_executable: str, host_script: Path, workspace: Path | None = None) -> Path:  # 修改代码+Phase24RealExtensionE2E: 生成可被 manifest 直接指向且带稳定工作区的 Windows launcher；如果没有这段函数，Chrome 可能尝试直接执行不能携带解释器参数的裸 Python 脚本。
    ...
    safe_workspace = Path(workspace) if workspace is not None else _workspace_for_host_script(Path(host_script))  # 新增代码+Phase24RealExtensionE2E: 计算 host 应使用的 learning_agent 工作区；如果没有这行代码，环境变量没有正确值。
    safe_pythonpath_root = safe_workspace.parent  # 新增代码+Phase25RealExtensionConnect: 计算 OpenHarness 仓库根目录给 Python 导入包；如果没有这行代码，Chrome 启动 host 时会找不到 learning_agent 包。
    launcher_text = f'@echo off\r\nset "OPENHARNESS_LEARNING_AGENT_WORKSPACE={safe_workspace}"\r\nset "PYTHONPATH={safe_pythonpath_root};%PYTHONPATH%"\r\n"{python_executable or "python"}" "{Path(host_script)}"\r\n'  # 修改代码+Phase25RealExtensionConnect: 用 cmd 同时设置工作区和 PYTHONPATH 再启动 Python host；如果没有这行代码，真实 Chrome 会启动 host 但 host 会在导入阶段崩溃。
...
def build_native_host_manifest(target_dir: Path, extension_id: str, python_executable: str, host_script: Path) -> Path:  # 新增代码+ChromeExtensionStage5: 生成 native host manifest 文件；若没有这行代码，插件无法知道本地 host 入口。
    ...
    launcher_path = build_native_host_launcher(safe_target_dir, python_executable, host_script, _workspace_for_host_script(Path(host_script)))  # 修改代码+Phase24RealExtensionE2E: 先生成带稳定工作区的 launcher 再写 manifest；如果没有这行代码，manifest 仍会指向裸脚本且可能写错 memory。
