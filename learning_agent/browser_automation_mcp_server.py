...
try:  # 新增代码+BrowserHarnessStage11: 优先从包路径导入 browser 到 harness 的投影器；若没有这行代码，浏览器 run 仍会停留在旁路 runtime。
    from learning_agent.browser.harness_integration import BrowserHarnessMirror  # 新增代码+BrowserHarnessStage11: 导入 harness 投影器；若没有这行代码，server 无法创建同 id harness run。
except ModuleNotFoundError as import_error:  # 新增代码+BrowserHarnessStage11: 兼容本文件作为独立 MCP 脚本运行；若没有这行代码，stdio 模式可能找不到 learning_agent 包。
    if import_error.name not in {"learning_agent", "learning_agent.browser", "learning_agent.browser.harness_integration"}:  # 新增代码+BrowserHarnessStage11: 只允许包路径缺失时 fallback；若没有这行代码，投影器内部真实 bug 会被误吞。
        raise  # 新增代码+BrowserHarnessStage11: 重新抛出真实内部导入错误；若没有这行代码，排查方向会被 fallback 误导。
    from browser.harness_integration import BrowserHarnessMirror  # 新增代码+BrowserHarnessStage11: 脚本模式导入 harness 投影器；若没有这行代码，直接运行 MCP server 时无法同步 harness。
...
BROWSER_RETRYABLE_TOOLS = {"browser_open", "browser_snapshot", "browser_click", "browser_type", "browser_type_secret", "browser_form_input", "browser_press_key", "browser_shortcuts_execute", "browser_wait", "browser_screenshot", "browser_tabs", "browser_tabs_context", "browser_console", "browser_network", "browser_upload_file", "browser_downloads", "browser_recover_page", "browser_visual_locate"}  # 修改代码+Phase3HighLevelTools: 把批量填表和快捷键执行纳入可短重试工具；若没有这行代码，高层浏览器动作遇到临时失败不能走统一恢复。
...
BROWSER_REPLAY_BLOCKED_TOOLS = {"browser_type", "browser_type_secret", "browser_form_input", "browser_shortcuts_execute", "browser_upload_file", "browser_evaluate", "browser_connect_real_chrome", "browser_disconnect_real_chrome", "browser_site_grant", "browser_launch_visible"}  # 修改代码+Phase3HighLevelTools: 禁止回放批量填表和快捷键执行；若没有这行代码，包含密码或提交动作的高层流程可能被重复执行。
BROWSER_TABS_CONTEXT_REQUIRED_WRITE_TOOLS = {"browser_click", "browser_type", "browser_type_secret", "browser_form_input", "browser_press_key", "browser_shortcuts_execute", "browser_upload_file"}  # 修改代码+Phase3HighLevelTools: 高层填表和快捷键执行也必须先确认真实 Chrome 标签页上下文；若没有这行代码，模型可能在未知标签页批量输入或提交。
...
TOOLS: list[dict[str, Any]] = [  # 新增代码+BrowserAutomation工具清单: 定义成熟版浏览器自动化工具列表；若省略: tools/list 无法暴露能力
    ...
    {"name": "browser_form_input", "description": "高层批量填表工具：按 fields 顺序输入多个字段，可选 submit=true 用回车提交。敏感字段必须使用 secret_env_var。", "inputSchema": {"type": "object", "properties": {"page_id": {"type": "string", "description": "可选的目标页面 id，会传给提交按键。"}, "fields": {"type": "array", "description": "字段列表，每项用 selector、label 或 element_id 定位，并且只能提供 text 或 secret_env_var 之一。", "items": {"type": "object", "properties": {"page_id": {"type": "string"}, "element_id": {"type": "string"}, "selector": {"type": "string"}, "label": {"type": "string"}, "text": {"type": "string"}, "secret_env_var": {"type": "string"}, "clear": {"type": "boolean"}, "timeout_ms": {"type": "integer"}}}}, "submit": {"description": "true 表示按 Enter 提交，也可以传 submit、enter 等快捷键别名。"}}, "required": ["fields"]}},  # 新增代码+Phase3HighLevelTools: 暴露批量填表入口；若没有这行代码，模型只能手动串多个输入工具且更容易漏掉敏感输入规则。
    ...
    {"name": "browser_shortcuts_list", "description": "列出浏览器高层快捷键别名，例如 submit、select_all、back、reload。", "inputSchema": {"type": "object", "properties": {}}},  # 新增代码+Phase3HighLevelTools: 暴露快捷键发现入口；若没有这行代码，用户和模型不知道可用快捷键别名。
    {"name": "browser_shortcuts_execute", "description": "按快捷键别名执行浏览器键盘动作，例如 submit 或 select_all。", "inputSchema": {"type": "object", "properties": {"page_id": {"type": "string", "description": "要操作的页面 id，缺省使用当前页面。"}, "shortcut": {"type": "string", "description": "快捷键别名，例如 submit、select_all、back、reload。"}}, "required": ["shortcut"]}},  # 新增代码+Phase3HighLevelTools: 暴露快捷键执行入口；若没有这行代码，用户自然说“提交表单”不能稳定映射到底层按键。
    ...
    {"name": "browser_extension_install", "description": "Chrome 扩展 native host 安装工具：默认 dry_run=true 只生成 manifest 和审计预览；dry_run=false 才写 HKCU registry。", "inputSchema": {"type": "object", "properties": {"extension_id": {"type": "string", "description": "Chrome 扩展 id，用于 allowed_origins。"}, "python_executable": {"type": "string", "description": "可选 Python 可执行文件路径，默认当前 Python。"}, "host_script": {"type": "string", "description": "可选 native_host.py 路径，默认项目内 browser_extension_host/native_host.py。"}, "dry_run": {"type": "boolean", "description": "默认 true；true 不写注册表，false 才注册 native host。"}}}},  # 新增代码+Phase1Installer: 暴露安全安装工具；如果没有这行代码，模型无法生成或注册 native host manifest。
    {"name": "browser_extension_uninstall", "description": "Chrome 扩展 native host 卸载工具：默认 dry_run=true 只预览；dry_run=false 才删除 HKCU registry 项。", "inputSchema": {"type": "object", "properties": {"dry_run": {"type": "boolean", "description": "默认 true；true 不删除注册表，false 才删除 native host 注册项。"}}}},  # 新增代码+Phase1Installer: 暴露安全卸载工具；如果没有这行代码，用户无法从 agent 回滚注册项。
    {"name": "browser_extension_repair_hint", "description": "Chrome 扩展 native host 修复建议工具：根据 manifest 和 registry 状态返回下一步操作。", "inputSchema": {"type": "object", "properties": {}}},  # 新增代码+Phase1Installer: 暴露修复建议工具；如果没有这行代码，小白用户只能看到状态码。
...
class BrowserAutomationServer:  # 新增代码+BrowserAutomation服务: 定义浏览器自动化 server 状态类；若省略: tools/call 无法复用浏览器状态
    def __init__(self, workspace: Path, native_host_registry_adapter: Any | None = None) -> None:  # 修改代码+Phase25RegistryTestIsolation: 初始化 server 工作区、运行状态和可选 registry 适配器；若没有这行代码，单元测试会读取用户真实 HKCU registry 导致结果随电脑状态变化。
        ...
        self.browser_harness_mirror = BrowserHarnessMirror(self.workspace)  # 新增代码+BrowserHarnessStage11: 创建 browser runtime 到 harness 的投影器；若没有这行代码，浏览器任务不会生成同 id harness run。
        ...
        from learning_agent.browser_extension_host.manifest_installer import ChromeNativeHostInstaller  # 新增代码+Phase1Installer: 延迟导入 native host 安装器；如果没有这行代码，server 无法查询 manifest/registry 安装状态。
        self.chrome_native_host_installer = ChromeNativeHostInstaller(self.workspace / "learning_agent" / "memory" / "chrome_native_host", registry_adapter=native_host_registry_adapter)  # 修改代码+Phase25RegistryTestIsolation: 创建 native host 安装器并允许测试注入内存 registry；如果没有这行代码，真实 registry 会污染临时 workspace 测试。
    ...
    def call(self, name: str, arguments: dict[str, Any]) -> str:  # 修改代码+BrowserAutomation分发: 工具方法统一返回文本结果；若省略: 协议层无法正确包装 MCP content 文本
        dispatch = {  # 新增代码+BrowserAutomation分发: 建立工具名到方法的映射；若省略: 需要大量重复 if 分支
            ...
            "browser_form_input": self.browser_form_input,  # 新增代码+Phase3HighLevelTools: 分发高层批量填表工具；若没有这行代码，schema 可见但调用会变成未知工具。
            "browser_press_key": self.browser_press_key,  # 新增代码+BrowserAutomation分发: 分发按键工具；若省略: browser_press_key 会被当成未知工具
            "browser_shortcuts_list": self.browser_shortcuts_list,  # 新增代码+Phase3HighLevelTools: 分发快捷键清单工具；若没有这行代码，用户无法发现可用快捷键。
            "browser_shortcuts_execute": self.browser_shortcuts_execute,  # 新增代码+Phase3HighLevelTools: 分发快捷键执行工具；若没有这行代码，快捷键别名无法执行。
            ...
            "browser_extension_install": self.browser_extension_install,  # 新增代码+Phase1Installer: 分发 native host 安装工具；如果没有这行代码，schema 可见但调用会未知工具。
            "browser_extension_uninstall": self.browser_extension_uninstall,  # 新增代码+Phase1Installer: 分发 native host 卸载工具；如果没有这行代码，用户无法通过 agent 回滚 registry。
            "browser_extension_repair_hint": self.browser_extension_repair_hint,  # 新增代码+Phase1Installer: 分发 native host 修复建议工具；如果没有这行代码，排障提示无法调用。
        ...
        try:  # 新增代码+BrowserRuntimeStore: 捕获成功和失败以便写 completed/failed 事件；若没有这行代码，durable event log 会缺少收尾。
            ...
            if browser_run is None:  # 修改代码+BrowserActionExecuteLayer: 如果 run 文件被外部清理则降级为旧执行路径；若没有这行代码，少见状态缺失会让工具直接崩溃。
                ...
                def on_action_finished(action: BrowserAction) -> None:  # 修改代码+BrowserActionExecuteLayer: 执行器结束 action 后清理活动 action；若没有这行代码，后续 observation 可能误挂旧动作。
                    self.browser_harness_mirror.append_action_evidence(browser_run.run_id, action)  # 新增代码+Phase22BrowserHarnessFusion: 把完成后的浏览器动作同步到 harness 事件；如果没有这行代码，真实工具 action 证据不会进入长任务验收链。
    ...
    def _record_browser_provider_decision(self, decision: BrowserProviderDecision) -> None:  # 新增代码+BrowserProviderAdapters: 把 provider 决策写入当前 browser runtime run；若没有这行代码，真实执行轨道不可审计。
        ...
        self.browser_harness_mirror.append_provider_decision(self.browser_runtime_active_run_id, decision)  # 新增代码+BrowserHarnessStage11: 把同一 provider 决策镜像到 harness event log；若没有这行代码，状态 CLI/API 仍看不到浏览器轨道选择。
    ...
    def _start_browser_runtime_run(self, tool_name: str) -> None:  # 新增代码+BrowserRuntimeStore: 为一次顶层浏览器工具调用创建 durable run；若没有这行代码，真实浏览器任务没有可恢复根状态。
        ...
        self.browser_harness_mirror.start_run(browser_run, tool_name)  # 新增代码+BrowserHarnessStage11: 同步创建同 id harness run；若没有这行代码，真实浏览器任务会绕开长任务 harness。
    ...
    def _finish_browser_runtime_run(self, success: bool, tool_name: str, message: str) -> None:  # 新增代码+BrowserRuntimeStore: 顶层工具调用结束时收尾 browser run；若没有这行代码，run 会永久停留 running。
        ...
        self.browser_harness_mirror.finish_run(browser_run, tool_name, success, clipped_message)  # 新增代码+BrowserHarnessStage11: 把 browser run 结果同步成 harness stage/verifier；若没有这行代码，浏览器完成状态没有长任务验收证据。
    ...
    def browser_flow_run(self, arguments: dict[str, Any]) -> str:  # 新增代码+BrowserRuntime复杂流程: 按阶段执行多步浏览器流程；若没有这行代码，复杂网站任务无法一次性验收
        ...
        self.browser_harness_mirror.sync_flow_report(self.browser_runtime_active_run_id, flow_plan, report)  # 新增代码+BrowserHarnessStage11: 把流程阶段和 checkpoint 结果同步进 harness；若没有这行代码，已完成 browser stage 仍无法被长任务状态看到。
    ...
    def browser_extension_status(self, arguments: dict[str, Any]) -> str:  # 新增代码+ChromeExtensionStage5: 报告 Chrome 扩展 provider 连接状态；若没有这行代码，用户无法判断插件轨道是否可用。
        ...
        installer_status = self.chrome_native_host_installer.status()  # 新增代码+Phase1Installer: 读取 native host 安装状态；如果没有这行代码，扩展状态看不到 manifest/registry 进度。
        return self.chrome_extension_bridge.status_text() + "\n" + "\n".join([f"installer_state={installer_status.get('state', '')}", f"native_host_manifest={installer_status.get('manifest_path', '')}", f"native_host_registry_key={installer_status.get('registry_key_path', '')}"])  # 修改代码+Phase1Installer: 把 bridge 状态和 installer 状态合并输出；如果没有这行代码，用户要分两处排查。

    def browser_extension_install(self, arguments: dict[str, Any]) -> str:  # 新增代码+Phase1Installer: 执行 Chrome native host 安装或 dry-run；如果没有这段函数，agent 无法从终端完成插件 host 准备。
        extension_id = str(arguments.get("extension_id", "") or "").strip()  # 新增代码+Phase1Installer: 读取扩展 id；如果没有这行代码，manifest allowed_origins 无法绑定扩展。
        if not extension_id:  # 新增代码+Phase1Installer: 安装必须知道扩展 id；如果没有这行代码，Chrome 会拒绝来源为空的 host。
            raise RuntimeError("browser_extension_install 需要 extension_id。")  # 新增代码+Phase1Installer: 给出中文错误；如果没有这行代码，用户不知道缺哪个参数。
        python_executable = str(arguments.get("python_executable", "") or sys.executable or "python")  # 新增代码+Phase1Installer: 读取 Python 入口；如果没有这行代码，manifest metadata 缺少运行环境线索。
        host_script_text = str(arguments.get("host_script", "") or "")  # 新增代码+Phase1Installer: 读取可选 host 脚本路径；如果没有这行代码，用户无法覆盖默认路径。
        host_script = Path(host_script_text) if host_script_text else self.workspace / "learning_agent" / "browser_extension_host" / "native_host.py"  # 新增代码+Phase1Installer: 计算 native_host.py 路径；如果没有这行代码，manifest path 会为空。
        dry_run = arguments.get("dry_run", True) is not False  # 新增代码+Phase1Installer: 默认 dry-run，只有明确 false 才写 registry；如果没有这行代码，工具可能静默改系统。
        result = self.chrome_native_host_installer.install(extension_id=extension_id, python_executable=python_executable, host_script=host_script, dry_run=dry_run)  # 新增代码+Phase1Installer: 调用安装器；如果没有这行代码，工具只会空转。
        return "\n".join(["browser_extension_install 成功", f"dry_run={str(bool(result.get('dry_run', True))).lower()}", f"host_name={result.get('host_name', '')}", f"manifest_path={result.get('manifest_path', '')}", f"registry_key_path={result.get('registry_key_path', '')}", f"registry_value={result.get('registry_value', '')}"])  # 新增代码+Phase1Installer: 返回稳定审计文本；如果没有这行代码，验收器无法确认是否写 registry。

    def browser_extension_uninstall(self, arguments: dict[str, Any]) -> str:  # 新增代码+Phase1Installer: 执行 Chrome native host 卸载或 dry-run；如果没有这段函数，用户无法回滚 registry 注册。
        dry_run = arguments.get("dry_run", True) is not False  # 新增代码+Phase1Installer: 默认 dry-run，只有明确 false 才删 registry；如果没有这行代码，状态查询可能误删配置。
        result = self.chrome_native_host_installer.uninstall(dry_run=dry_run)  # 新增代码+Phase1Installer: 调用安装器卸载逻辑；如果没有这行代码，工具不会产生任何效果。
        return "\n".join(["browser_extension_uninstall 成功", f"dry_run={str(bool(result.get('dry_run', True))).lower()}", f"host_name={result.get('host_name', '')}", f"registry_key_path={result.get('registry_key_path', '')}", f"registry_value_before={result.get('registry_value_before', '')}"])  # 新增代码+Phase1Installer: 返回稳定卸载审计文本；如果没有这行代码，用户不知道删除目标。

    def browser_extension_repair_hint(self, arguments: dict[str, Any]) -> str:  # 新增代码+Phase1Installer: 返回 native host 修复建议；如果没有这段函数，用户遇到连接失败只能看日志。
        del arguments  # 新增代码+Phase1Installer: 修复建议不使用参数；如果没有这行代码，读者会误以为会读取页面或系统敏感数据。
        return "browser_extension_repair_hint\n" + self.chrome_native_host_installer.repair_hint()  # 新增代码+Phase1Installer: 返回稳定标题和中文建议；如果没有这行代码，验收器和用户都难以识别输出。
    ...
    def browser_form_input(self, arguments: dict[str, Any]) -> str:  # 新增代码+Phase3HighLevelTools: 提供批量填表高层入口；若没有这段函数，模型只能手动串多个底层输入动作。
        try:  # 新增代码+Phase3HighLevelTools: 优先按包路径导入填表规划器；若没有这行代码，模块模式和测试模式无法复用同一 helper。
            from learning_agent.browser.high_level_tools import build_form_input_plan  # 新增代码+Phase3HighLevelTools: 导入批量填表计划函数；若没有这行代码，高层工具无法拆成底层动作。
        except ModuleNotFoundError as import_error:  # 新增代码+Phase3HighLevelTools: 兼容 MCP server 直接脚本运行时的导入路径；若没有这行代码，stdio 启动可能找不到 learning_agent 包。
            if import_error.name not in {"learning_agent", "learning_agent.browser", "learning_agent.browser.high_level_tools"}:  # 新增代码+Phase3HighLevelTools: 只把包路径缺失当作可 fallback；若没有这行代码，helper 内部真实 bug 会被误吞。
                raise  # 新增代码+Phase3HighLevelTools: 重新抛出真实内部导入错误；若没有这行代码，排查方向会被 fallback 误导。
            from browser.high_level_tools import build_form_input_plan  # 新增代码+Phase3HighLevelTools: 脚本模式从同目录 browser 包导入 helper；若没有这行代码，直接运行 MCP server 时高层工具不可用。
        plan = build_form_input_plan(arguments)  # 新增代码+Phase3HighLevelTools: 把高层请求转换为有序底层动作；若没有这行代码，后续无法知道该调用哪些低层工具。
        result_lines = ["browser_form_input 成功"]  # 新增代码+Phase3HighLevelTools: 准备返回成功标题；若没有这行代码，调用方难以识别高层工具结果。
        for index, step in enumerate(plan, start=1):  # 新增代码+Phase3HighLevelTools: 按计划顺序执行每个底层动作；若没有这行代码，批量填表不会真正执行。
            tool_name = str(step.get("tool_name", "") or "")  # 新增代码+Phase3HighLevelTools: 读取底层工具名；若没有这行代码，无法选择执行函数。
            step_arguments = dict(step.get("arguments", {}) or {})  # 新增代码+Phase3HighLevelTools: 复制底层参数避免 helper 返回值被执行过程污染；若没有这行代码，调试和复用都不稳定。
            if tool_name == "browser_type":  # 新增代码+Phase3HighLevelTools: 普通字段走普通输入工具；若没有这行代码，普通文本无法输入。
                step_result = self.browser_type(step_arguments)  # 新增代码+Phase3HighLevelTools: 复用已审计的普通输入实现；若没有这行代码，高层工具会绕开底层安全逻辑。
            elif tool_name == "browser_type_secret":  # 新增代码+Phase3HighLevelTools: 敏感字段走 secret 输入工具；若没有这行代码，密码字段无法安全输入。
                step_result = self.browser_type_secret(step_arguments)  # 新增代码+Phase3HighLevelTools: 复用不回显明文的敏感输入实现；若没有这行代码，高层工具可能泄露秘密。
            elif tool_name == "browser_press_key":  # 新增代码+Phase3HighLevelTools: 提交步骤走按键工具；若没有这行代码，submit=true 不会触发表单提交。
                step_result = self.browser_press_key(step_arguments)  # 新增代码+Phase3HighLevelTools: 复用既有按键实现和审计；若没有这行代码，高层提交会绕开统一按键入口。
            else:  # 新增代码+Phase3HighLevelTools: 防御 helper 返回未知底层工具；若没有这行代码，错误计划可能静默跳过。
                raise RuntimeError(f"browser_form_input 生成了未知底层工具：{tool_name}")  # 新增代码+Phase3HighLevelTools: 明确指出内部计划错误；若没有这行代码，维护者难以定位 helper 问题。
            result_lines.append(f"step_{index}={tool_name}")  # 新增代码+Phase3HighLevelTools: 只记录步骤名不记录输入文本；若没有这行代码，调用方看不到执行进度。
            result_lines.append(str(step_result))  # 新增代码+Phase3HighLevelTools: 附加底层安全回执；若没有这行代码，用户不知道每一步是否成功。
        self._audit_browser_tool("browser_form_input", {"steps": len(plan), "has_submit": any(step.get("tool_name") == "browser_press_key" for step in plan)})  # 新增代码+Phase3HighLevelTools: 真实 Chrome 模式只审计步骤数和是否提交；若没有这行代码，高层填表缺少安全摘要留痕。
        return "\n".join(result_lines)  # 新增代码+Phase3HighLevelTools: 返回多行执行摘要；若没有这行代码，MCP client 收不到高层结果。

    def browser_shortcuts_list(self, arguments: dict[str, Any]) -> str:  # 新增代码+Phase3HighLevelTools: 提供快捷键发现入口；若没有这段函数，用户不知道支持哪些快捷键别名。
        del arguments  # 新增代码+Phase3HighLevelTools: 本工具不读取输入参数；若没有这行代码，读者可能误以为参数会影响输出。
        try:  # 新增代码+Phase3HighLevelTools: 优先按包路径导入格式化函数；若没有这行代码，模块模式无法复用 helper。
            from learning_agent.browser.high_level_tools import format_shortcuts_list  # 新增代码+Phase3HighLevelTools: 导入快捷键清单格式化函数；若没有这行代码，server 需要重复映射输出。
        except ModuleNotFoundError as import_error:  # 新增代码+Phase3HighLevelTools: 兼容直接脚本运行路径；若没有这行代码，stdio 模式可能导入失败。
            if import_error.name not in {"learning_agent", "learning_agent.browser", "learning_agent.browser.high_level_tools"}:  # 新增代码+Phase3HighLevelTools: 只允许包路径缺失 fallback；若没有这行代码，真实 helper bug 会被隐藏。
                raise  # 新增代码+Phase3HighLevelTools: 重新抛出内部导入错误；若没有这行代码，排查会被误导。
            from browser.high_level_tools import format_shortcuts_list  # 新增代码+Phase3HighLevelTools: 脚本模式导入快捷键清单函数；若没有这行代码，直接运行 MCP server 时清单不可用。
        return format_shortcuts_list()  # 新增代码+Phase3HighLevelTools: 返回稳定快捷键清单；若没有这行代码，工具没有输出。

    def browser_shortcuts_execute(self, arguments: dict[str, Any]) -> str:  # 新增代码+Phase3HighLevelTools: 按快捷键别名执行键盘动作；若没有这段函数，用户必须记住底层 Playwright 按键名。
        try:  # 新增代码+Phase3HighLevelTools: 优先按包路径导入快捷键映射函数；若没有这行代码，模块模式无法复用 helper。
            from learning_agent.browser.high_level_tools import browser_shortcut_key  # 新增代码+Phase3HighLevelTools: 导入快捷键别名转换函数；若没有这行代码，server 会重复维护映射。
        except ModuleNotFoundError as import_error:  # 新增代码+Phase3HighLevelTools: 兼容直接脚本运行路径；若没有这行代码，stdio 模式可能导入失败。
            if import_error.name not in {"learning_agent", "learning_agent.browser", "learning_agent.browser.high_level_tools"}:  # 新增代码+Phase3HighLevelTools: 只允许包路径缺失 fallback；若没有这行代码，helper 内部错误会被误吞。
                raise  # 新增代码+Phase3HighLevelTools: 重新抛出真实内部导入错误；若没有这行代码，维护者难以定位问题。
            from browser.high_level_tools import browser_shortcut_key  # 新增代码+Phase3HighLevelTools: 脚本模式导入快捷键转换函数；若没有这行代码，直接运行 MCP server 时快捷键不可用。
        shortcut_name = str(arguments.get("shortcut", "") or "").strip()  # 新增代码+Phase3HighLevelTools: 读取用户传入的快捷键别名；若没有这行代码，工具不知道要执行哪个快捷键。
        if not shortcut_name:  # 新增代码+Phase3HighLevelTools: 检查快捷键别名不能为空；若没有这行代码，空快捷键会进入底层按键工具。
            raise RuntimeError("browser_shortcuts_execute 缺少 shortcut 参数。")  # 新增代码+Phase3HighLevelTools: 用中文说明缺参；若没有这行代码，新手不知道如何修正。
        key = browser_shortcut_key(shortcut_name)  # 新增代码+Phase3HighLevelTools: 把别名转换成真实按键名；若没有这行代码，底层 browser_press_key 无法执行。
        press_arguments = {"key": key}  # 新增代码+Phase3HighLevelTools: 准备底层按键参数；若没有这行代码，按键工具缺少 key。
        if "page_id" in arguments:  # 新增代码+Phase3HighLevelTools: 如果用户指定 page_id 就透传；若没有这行代码，跨标签页快捷键可能打到默认页。
            press_arguments["page_id"] = arguments["page_id"]  # 新增代码+Phase3HighLevelTools: 保存目标页面 id；若没有这行代码，指定页面参数会丢失。
        result = self.browser_press_key(press_arguments)  # 新增代码+Phase3HighLevelTools: 复用底层按键实现；若没有这行代码，高层快捷键会绕开统一执行和审计。
        self._audit_browser_tool("browser_shortcuts_execute", {"shortcut": shortcut_name, "key_present": True})  # 新增代码+Phase3HighLevelTools: 真实 Chrome 模式审计快捷键别名和存在按键；若没有这行代码，高层按键缺少安全留痕。
        return f"browser_shortcuts_execute 成功\nshortcut={shortcut_name}\nkey={key}\n底层结果：{result}"  # 新增代码+Phase3HighLevelTools: 返回快捷键执行摘要；若没有这行代码，用户不知道别名映射成了什么。
