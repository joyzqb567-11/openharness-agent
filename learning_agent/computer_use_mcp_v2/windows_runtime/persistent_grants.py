...
def _phase2_request_access_app_list(values: Any) -> list[str]:  # 新增代码+Phase2SessionGrants：函数段开始，规范化 request_access grant 的应用列表；如果没有这段函数，allowed_apps/denied_apps 会出现大小写和空值噪音。
    raw_values = values if isinstance(values, list) else [values]  # 新增代码+Phase2SessionGrants：兼容单个字符串和列表输入；如果没有这一行，调用方传单个 app 时会被错误拆解。
    result: list[str] = []  # 新增代码+Phase2SessionGrants：准备保存去重后的应用名；如果没有这一行，函数没有返回容器。
    for value in list(raw_values or []):  # 新增代码+Phase2SessionGrants：遍历输入应用名；如果没有这一行，应用列表不会被处理。
        app = _phase60_normalized_app(value)  # 新增代码+Phase2SessionGrants：复用 Phase60 app 清洗逻辑；如果没有这一行，同一 app 的不同写法无法匹配。
        if app and app not in result:  # 新增代码+Phase2SessionGrants：跳过空值和重复值；如果没有这一行，授权对象会越来越乱。
            result.append(app)  # 新增代码+Phase2SessionGrants：保存唯一应用名；如果没有这一行，有效应用会丢失。
    return result  # 新增代码+Phase2SessionGrants：返回规范化应用列表；如果没有这一行，调用方拿不到 allowed_apps。
# 新增代码+Phase2SessionGrants：函数段结束，_phase2_request_access_app_list 到此结束；如果没有这个边界说明，用户不容易看出应用列表清理范围。


def create_request_access_grant(session_id: Any, allowed_apps: Any, grant_flags: dict[str, bool] | None = None, ttl_seconds: int | float = 900, denied_apps: Any = None, revoked: bool = False, now_epoch: float | None = None) -> dict[str, Any]:  # 新增代码+Phase2SessionGrants：函数段开始，创建 request_access 专属授权对象；如果没有这段函数，Phase 2 没有可审计的会话授权结构。
    safe_session_id = _phase60_safe_session_id(session_id)  # 新增代码+Phase2SessionGrants：清理授权会话 id；如果没有这一行，request_access_session_id 可能带危险字符或空值。
    safe_allowed_apps = _phase2_request_access_app_list(allowed_apps)  # 新增代码+Phase2SessionGrants：清理允许应用列表；如果没有这一行，grant_scope_matches_target 无法稳定匹配。
    safe_denied_apps = _phase2_request_access_app_list(denied_apps)  # 新增代码+Phase2SessionGrants：清理拒绝应用列表；如果没有这一行，显式拒绝目标无法被门禁识别。
    flags = _phase60_normalized_flags(grant_flags or {})  # 新增代码+Phase2SessionGrants：清理授权权限位；如果没有这一行，desktopAction 等 flag 可能被字符串误导。
    created_epoch = time.time() if now_epoch is None else float(now_epoch)  # 新增代码+Phase2SessionGrants：确定授权创建时间；如果没有这一行，TTL 和审计时间无法对齐。
    expires_epoch = created_epoch + float(ttl_seconds)  # 新增代码+Phase2SessionGrants：计算授权过期时间；如果没有这一行，grant_expired 无法自动判断。
    grant_expired = bool(expires_epoch < created_epoch)  # 新增代码+Phase2SessionGrants：记录创建时是否已经过期；如果没有这一行，负 TTL 测试无法稳定表达过期。
    grant_revoked = bool(revoked)  # 新增代码+Phase2SessionGrants：记录创建时是否已撤销；如果没有这一行，撤销测试只能依赖临时字典。
    requires_approval = bool(not safe_allowed_apps or grant_expired or grant_revoked)  # 新增代码+Phase2SessionGrants：没有可用授权范围或已失效时仍需审批；如果没有这一行，失效 grant 可能被误读为可放行。
    seed = f"request-access:{safe_session_id}:{','.join(safe_allowed_apps)}:{','.join(safe_denied_apps)}:{created_epoch}"  # 新增代码+Phase2SessionGrants：组装稳定 grant_id 种子；如果没有这一行，授权记录难以审计定位。
    return {  # 新增代码+Phase2SessionGrants：返回 request_access 授权对象；如果没有这一行，调用方拿不到结构化 grant。
        "grant_id": "request-access-grant-" + hashlib.sha256(seed.encode("utf-8")).hexdigest()[:16],  # 新增代码+Phase2SessionGrants：生成短 grant_id；如果没有这一行，撤销和审计无法引用具体授权。
        "request_access_session_id": safe_session_id,  # 新增代码+Phase2SessionGrants：保存 request_access 会话 id；如果没有这一行，顶层矩阵和审计无法确认 Phase 2 会话授权存在。
        "session_id": safe_session_id,  # 新增代码+Phase2SessionGrants：保存兼容 session_id；如果没有这一行，旧 Phase60 评估和新授权无法共享会话归属。
        "allowed_apps": safe_allowed_apps,  # 新增代码+Phase2SessionGrants：保存允许应用列表；如果没有这一行，授权没有实际范围。
        "denied_apps": safe_denied_apps,  # 新增代码+Phase2SessionGrants：保存拒绝应用列表；如果没有这一行，拒绝范围无法覆盖通配授权。
        "grant_flags": flags,  # 新增代码+Phase2SessionGrants：保存权限位；如果没有这一行，桌面动作与只读观察无法区分。
        "created_at": _phase60_timestamp(created_epoch),  # 新增代码+Phase2SessionGrants：保存可读创建时间；如果没有这一行，人类看不懂授权新旧。
        "created_at_epoch": created_epoch,  # 新增代码+Phase2SessionGrants：保存机器可比较创建时间；如果没有这一行，测试和排序无法稳定。
        "expires_at": _phase60_timestamp(expires_epoch),  # 新增代码+Phase2SessionGrants：保存可读过期时间；如果没有这一行，人类看不懂 TTL。
        "expires_at_epoch": expires_epoch,  # 新增代码+Phase2SessionGrants：保存机器可比较过期时间；如果没有这一行，deny_action_without_request_access 无法按时间拒绝。
        "grant_expired": grant_expired,  # 新增代码+Phase2SessionGrants：保存过期标记；如果没有这一行，过期状态不能直接进入矩阵证据。
        "grant_revoked": grant_revoked,  # 新增代码+Phase2SessionGrants：保存撤销标记；如果没有这一行，撤销状态不能直接进入矩阵证据。
        "requires_approval": requires_approval,  # 新增代码+Phase2SessionGrants：保存是否仍需审批；如果没有这一行，无授权/失效授权的下一步不清楚。
        "model": "phase2_request_access_session_grant",  # 新增代码+Phase2SessionGrants：标明授权模型；如果没有这一行，审计无法区分旧 Phase60 grant 和 request_access grant。
        "actions_expanded": PHASE60_ACTIONS_EXPANDED,  # 新增代码+Phase2SessionGrants：保留动作面边界；如果没有这一行，用户可能误以为生成 grant 就执行了桌面动作。
    }  # 新增代码+Phase2SessionGrants：request_access 授权对象结束；如果没有这一行，Python 字典结构不完整。
# 新增代码+Phase2SessionGrants：函数段结束，create_request_access_grant 到此结束；如果没有这个边界说明，用户不容易看出它只是创建授权对象不碰桌面。
