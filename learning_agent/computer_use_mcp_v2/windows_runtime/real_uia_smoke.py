...
try:  # 新增代码+Phase35WindowsRealUIASmoke: 包住包模式导入；如果没有这行代码，start_oauth_agent.bat 的脚本路径可能导入失败。
    from learning_agent.computer_use_mcp_v2.windows_runtime.native_helper import NativeWindowTextResult, WindowsUiautomationTextProvider, parse_hwnd_from_window  # 新增代码+Phase35WindowsRealUIASmoke: 复用 Phase32/34 文本 provider 合同；如果没有这行代码，Phase35 会重复造一套 UIA 读取逻辑。
    from learning_agent.computer_use_mcp_v2.windows_runtime.windows_backend import WindowsWindowInventoryProbe  # 新增代码+Phase35WindowsRealUIASmoke: 复用 Phase28 真实窗口 inventory；如果没有这行代码，safe window 查找会和生产 observe 规则不一致。
except ModuleNotFoundError as error:  # 新增代码+Phase35WindowsRealUIASmoke: 兼容直接脚本运行时包名不同的情况；如果没有这行代码，bat 入口更容易因为路径失败。
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.native_helper", "learning_agent.computer_use_mcp_v2.windows_runtime.windows_backend"}:  # 新增代码+Phase35WindowsRealUIASmoke: 只吞掉预期包路径错误；如果没有这行代码，真实内部 bug 可能被误吞。
        raise  # 新增代码+Phase35WindowsRealUIASmoke: 重新抛出非路径类导入错误；如果没有这行代码，排查真实依赖问题会很困难。
    from computer_use_mcp_v2.windows_runtime.native_helper import NativeWindowTextResult, WindowsUiautomationTextProvider, parse_hwnd_from_window  # 新增代码+Phase35WindowsRealUIASmoke: 脚本模式下导入文本 provider；如果没有这行代码，learning_agent 目录内运行模块会失败。
    from computer_use_mcp_v2.windows_runtime.windows_backend import WindowsWindowInventoryProbe  # 新增代码+Phase35WindowsRealUIASmoke: 脚本模式下导入真实窗口 inventory；如果没有这行代码，bat 场景无法枚举安全窗口。
