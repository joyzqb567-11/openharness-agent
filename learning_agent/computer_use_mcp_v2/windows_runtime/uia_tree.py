...
try:  # 新增代码+Phase46WindowsUiaTree: 优先按包模式导入 evidence 过滤和 hwnd 解析；如果没有这行代码，unittest 和生产入口无法复用现有安全逻辑。
    from learning_agent.computer_use_mcp_v2.windows_runtime.evidence import SENSITIVE_ACCESSIBILITY_KEYWORDS, filter_accessibility_text  # 新增代码+Phase46WindowsUiaTree: 复用 Phase29 敏感文本过滤；如果没有这行代码，UIA 控件名称可能泄露密码或 token。
    from learning_agent.computer_use_mcp_v2.windows_runtime.native_helper import parse_hwnd_from_window  # 新增代码+Phase46WindowsUiaTree: 复用 Phase32 hwnd 解析；如果没有这行代码，runtime 会和窗口合同割裂。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.evidence", "learning_agent.computer_use_mcp_v2.windows_runtime.native_helper"}:  # 新增代码+Phase46WindowsUiaTree: 只允许目标包路径缺失时 fallback；如果没有这行代码，真实内部 bug 会被误吞。
        raise  # 新增代码+Phase46WindowsUiaTree: 重新抛出非路径类导入错误；如果没有这行代码，排查 evidence/native_helper 内部错误会困难。
    from computer_use_mcp_v2.windows_runtime.evidence import SENSITIVE_ACCESSIBILITY_KEYWORDS, filter_accessibility_text  # 新增代码+Phase46WindowsUiaTree: 脚本模式复用敏感文本过滤；如果没有这行代码，bat 入口无法安全脱敏。
    from computer_use_mcp_v2.windows_runtime.native_helper import parse_hwnd_from_window  # 新增代码+Phase46WindowsUiaTree: 脚本模式复用 hwnd 解析；如果没有这行代码，bat 入口无法识别目标窗口。
...
def run_phase46_uia_tree_contract() -> dict[str, Any]:  # 新增代码+Phase46WindowsUiaTree: 函数段开始，运行 Phase46 安全自检合同；如果没有这段函数，真实终端场景没有稳定命令入口。
    from learning_agent.computer_use_mcp_v2.windows_runtime.native_host import InProcessWindowsNativeHostClient, WindowsComputerUseNativeHost  # 新增代码+Phase46WindowsUiaTree: 延迟导入 native host 避免循环依赖；如果没有这行代码，自检无法证明 host observe 集成。
