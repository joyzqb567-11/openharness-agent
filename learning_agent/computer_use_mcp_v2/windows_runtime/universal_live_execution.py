...
try:  # 新增代码+Phase93UniversalLiveExecutionGate：优先按 learning_agent 包路径导入已有组件；如果没有这段代码，单元测试和生产入口不能共享同一套模块。
    from learning_agent.computer_use_mcp_v2.windows_runtime.closed_loop_executor import WindowsClosedLoopComputerExecutor  # 新增代码+Phase93UniversalLiveExecutionGate：复用 Phase68 闭环执行器；如果没有这行代码，Phase93 会变成静态状态汇总。
    from learning_agent.computer_use_mcp_v2.windows_runtime.generic_control_actions import Phase70RecordingHighLevelTool, WindowsGenericControlActionRuntime  # 新增代码+Phase93UniversalLiveExecutionGate：复用 Phase70 通用点击/输入动作层；如果没有这行代码，系统可能退回每个软件硬编码动作。
    from learning_agent.computer_use_mcp_v2.windows_runtime.generic_input_actions import Phase71RecordingInputSender, WindowsGenericInputActionRuntime  # 新增代码+Phase93UniversalLiveExecutionGate：复用 Phase71 通用热键/菜单/滚动/拖拽事件层；如果没有这行代码，输入能力会分散。
    from learning_agent.computer_use_mcp_v2.windows_runtime.mode_session import ComputerUseModeSessionStore  # 新增代码+Phase99UniversalComputerUseModeGate：导入 Phase98 mode store；如果没有这行代码，Phase93 无法在真实动作前询问 normal/observe/stopped/expired 模式。
    from learning_agent.computer_use_mcp_v2.windows_runtime.persistent_grants import WindowsComputerUsePersistentGrantStore  # 新增代码+Phase93UniversalLiveExecutionGate：复用 Phase60 持久授权事实源；如果没有这行代码，真实执行门禁没有可审计授权来源。
    from learning_agent.computer_use_mcp_v2.windows_runtime.production_live_control import WindowsProductionComputerUseHostAdapter  # 新增代码+Phase93UniversalLiveExecutionGate：复用 Phase76-89 生产 host adapter；如果没有这行代码，Phase93 无法连接生产级桥接结构。
    from learning_agent.computer_use_mcp_v2.windows_runtime.real_app_safety_boundary import WindowsRealAppSafetyBoundary  # 新增代码+Phase93UniversalLiveExecutionGate：复用 Phase72 真实应用安全边界；如果没有这行代码，危险窗口和未授权窗口无法统一拒绝。
    from learning_agent.computer_use_mcp_v2.windows_runtime.universal_mode import UniversalWindowsComputerUseRuntime  # 新增代码+Phase93UniversalLiveExecutionGate：复用 Phase92 单一通用 prompt mode；如果没有这行代码，Phase93 可能重复造一个偏离主线的 runtime。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.mode_session", "learning_agent.runtime", "learning_agent.runtime.files"}:  # 修改代码+Phase99UniversalComputerUseModeGate：允许脚本模式下 mode_session 路径缺失进入 fallback；如果没有这行代码，start_oauth_agent.bat 可能无法导入 Phase99 gate。
        raise  # 新增代码+Phase93UniversalLiveExecutionGate：重新抛出非路径类导入错误；如果没有这行代码，依赖内部错误会被隐藏。
    from computer_use_mcp_v2.windows_runtime.closed_loop_executor import WindowsClosedLoopComputerExecutor  # type: ignore  # 新增代码+Phase93UniversalLiveExecutionGate：脚本模式复用 Phase68 闭环执行器；如果没有这行代码，bat 入口无法运行 Phase93。
    from computer_use_mcp_v2.windows_runtime.generic_control_actions import Phase70RecordingHighLevelTool, WindowsGenericControlActionRuntime  # type: ignore  # 新增代码+Phase93UniversalLiveExecutionGate：脚本模式复用 Phase70 通用动作层；如果没有这行代码，bat 入口缺少通用控制能力。
    from computer_use_mcp_v2.windows_runtime.generic_input_actions import Phase71RecordingInputSender, WindowsGenericInputActionRuntime  # type: ignore  # 新增代码+Phase93UniversalLiveExecutionGate：脚本模式复用 Phase71 输入事件层；如果没有这行代码，bat 入口缺少通用输入能力。
    from computer_use_mcp_v2.windows_runtime.mode_session import ComputerUseModeSessionStore  # type: ignore  # 新增代码+Phase99UniversalComputerUseModeGate：脚本模式导入 Phase98 mode store；如果没有这行代码，可见终端入口不能执行 Phase99 mode gate。
    from computer_use_mcp_v2.windows_runtime.persistent_grants import WindowsComputerUsePersistentGrantStore  # type: ignore  # 新增代码+Phase93UniversalLiveExecutionGate：脚本模式复用 Phase60 授权 store；如果没有这行代码，bat 入口无法验证授权门禁。
    from computer_use_mcp_v2.windows_runtime.production_live_control import WindowsProductionComputerUseHostAdapter  # type: ignore  # 新增代码+Phase93UniversalLiveExecutionGate：脚本模式复用生产 host adapter；如果没有这行代码，bat 入口缺少生产桥接状态。
    from computer_use_mcp_v2.windows_runtime.real_app_safety_boundary import WindowsRealAppSafetyBoundary  # type: ignore  # 新增代码+Phase93UniversalLiveExecutionGate：脚本模式复用安全边界；如果没有这行代码，bat 入口无法阻断危险窗口。
    from computer_use_mcp_v2.windows_runtime.universal_mode import UniversalWindowsComputerUseRuntime  # type: ignore  # 新增代码+Phase93UniversalLiveExecutionGate：脚本模式复用 Phase92 通用 runtime；如果没有这行代码，bat 入口会偏离通用模式主线。
...
def _phase113_resolve_generic_report(raw_target: Any, generic_report: dict[str, Any] | None = None) -> dict[str, Any]:  # 新增代码+InteractiveGenericLaunchMaturity：函数段开始，读取或生成 Phase108 通用发现报告；如果没有这段函数，Phase113 会绕开已有通用发现模型。
    ...
    try:  # 新增代码+InteractiveGenericLaunchMaturity：优先按包路径导入 Phase108 发现器；如果没有这一行，项目根运行时无法加载标准模块。
        from learning_agent.computer_use_mcp_v2.windows_runtime.windows_launch_resolver import resolve_generic_app_launch_target  # 新增代码+InteractiveGenericLaunchMaturity：导入通用目标解析器；如果没有这一行，raw_target 无法转成安全启动计划。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.windows_launch_resolver"}:  # 新增代码+InteractiveGenericLaunchMaturity：只兜底包路径缺失；如果没有这一行，Phase108 内部 bug 会被误吞。
            raise  # 新增代码+InteractiveGenericLaunchMaturity：重新抛出真实内部错误；如果没有这一行，排查会被错误 fallback 干扰。
        from computer_use_mcp_v2.windows_runtime.windows_launch_resolver import resolve_generic_app_launch_target  # type: ignore  # 新增代码+InteractiveGenericLaunchMaturity：脚本模式导入同一解析器；如果没有这一行，双击 bat 后 Phase113 无法发现普通应用。
...
def _phase113_prepare_candidate_report(raw_target: Any, generic_report: dict[str, Any], phase109_report: dict[str, Any] | None = None) -> dict[str, Any]:  # 新增代码+InteractiveGenericLaunchMaturity：函数段开始，读取或生成 Phase109 候选报告；如果没有这段函数，Phase113 输出会丢失身份和清理候选证据。
    ...
    try:  # 新增代码+InteractiveGenericLaunchMaturity：优先按包路径导入 Phase109 候选器；如果没有这一行，项目根运行时无法加载标准模块。
        from learning_agent.computer_use_mcp_v2.windows_runtime.generic_launch_backend import prepare_phase109_generic_real_launch_candidate  # 新增代码+InteractiveGenericLaunchMaturity：导入通用真实启动候选准备函数；如果没有这一行，Phase113 只能看到后端而缺少候选成熟证据。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.generic_launch_backend"}:  # 新增代码+InteractiveGenericLaunchMaturity：只兜底包路径缺失；如果没有这一行，Phase109 内部 bug 会被误吞。
            raise  # 新增代码+InteractiveGenericLaunchMaturity：重新抛出真实内部错误；如果没有这一行，排查 Phase109 会被 fallback 干扰。
        from computer_use_mcp_v2.windows_runtime.generic_launch_backend import prepare_phase109_generic_real_launch_candidate  # type: ignore  # 新增代码+InteractiveGenericLaunchMaturity：脚本模式导入同一候选器；如果没有这一行，bat 入口无法显示候选证据。
...
def _phase113_generic_backend_tools() -> tuple[Any, Any]:  # 新增代码+InteractiveGenericLaunchMaturity：函数段开始，延迟导入 Phase110 通用后端工具；如果没有这段函数，脚本模式和包模式会重复写导入分支。
    try:  # 新增代码+InteractiveGenericLaunchMaturity：优先按 learning_agent 包路径导入；如果没有这一行，项目根运行时无法加载标准模块。
        from learning_agent.computer_use_mcp_v2.windows_runtime.generic_launch_backend import Phase110ProductionGenericLaunchBackend, run_generic_launch_backend  # 新增代码+InteractiveGenericLaunchMaturity：导入 production 后端和编排函数；如果没有这一行，Phase113 无法到达通用启动后端。
    except ModuleNotFoundError as error:  # 新增代码+InteractiveGenericLaunchMaturity：兼容 start_oauth_agent.bat 从 learning_agent 目录运行；如果没有这一行，真实可见终端可能导入失败。
        if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.generic_launch_backend"}:  # 新增代码+InteractiveGenericLaunchMaturity：只兜底包路径缺失；如果没有这一行，Phase110 内部 bug 会被误吞。
            raise  # 新增代码+InteractiveGenericLaunchMaturity：重新抛出真实内部错误；如果没有这一行，排查后端接线会被 fallback 干扰。
        from computer_use_mcp_v2.windows_runtime.generic_launch_backend import Phase110ProductionGenericLaunchBackend, run_generic_launch_backend  # type: ignore  # 新增代码+InteractiveGenericLaunchMaturity：脚本模式导入同一后端工具；如果没有这一行，bat 入口无法接通通用后端。
...
class Phase105ControlledNotepadSmokeLaunchCandidate:  # 新增代码+Phase105FullModeControlledRealLaunch：类段开始，把 full-mode launch_app 桥接到 Phase104 Notepad smoke；如果没有这个类，真实启动可能绕过窗口验证和清理。
    ...
    def launch(self, app_name: str, enable_real_launch: bool | None = None, test_file: str | None = None) -> dict[str, Any]:  # 新增代码+Phase105FullModeControlledRealLaunch：函数段开始，按 Phase103 候选接口接收 full gate 请求；如果没有这段函数，Universal gate 无法调用 Phase104 smoke。
        ...
        try:  # 新增代码+Phase105FullModeControlledRealLaunch：优先按包路径导入 Phase104 smoke runtime；如果没有这段代码，测试和真实终端不能共享窗口验证清理实现。
            from learning_agent.computer_use_mcp_v2.windows_runtime.controlled_notepad_launch_smoke import WindowsControlledNotepadLaunchSmoke  # 新增代码+Phase105FullModeControlledRealLaunch：导入 Phase104 可见 Notepad smoke；如果没有这行代码，Phase105 会缺少真实窗口验证和清理。
            ...
            if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.controlled_notepad_launch_smoke"}:  # 新增代码+Phase105FullModeControlledRealLaunch：只兜底包路径缺失；如果没有这行代码，Phase104 内部 bug 会被误吞。
                raise  # 新增代码+Phase105FullModeControlledRealLaunch：重新抛出真实内部导入错误；如果没有这行代码，排查会被错误 fallback 干扰。
            from computer_use_mcp_v2.windows_runtime.controlled_notepad_launch_smoke import WindowsControlledNotepadLaunchSmoke  # type: ignore  # 新增代码+Phase105FullModeControlledRealLaunch：脚本模式导入 Phase104 smoke；如果没有这行代码，bat 入口无法运行 Phase105 真实分支。
...
def run_phase105_full_mode_controlled_real_launch_contract(base_dir: str | Path | None = None, real_launch: bool | None = None, allow_real_gate: bool | None = None, real_launch_candidate: Any | None = None, launch_backend: Any | None = None, window_probe: Any | None = None, platform: str | None = None) -> dict[str, Any]:  # 新增代码+Phase105FullModeControlledRealLaunch：函数段开始，运行 Phase105 full-mode 受控真实启动合同；如果没有这段函数，真实终端没有统一验收入口。
    try:  # 新增代码+Phase105FullModeControlledRealLaunch：优先按包路径导入 Phase103 安全候选；如果没有这段代码，默认安全分支无法复用受控启动计划。
        from learning_agent.computer_use_mcp_v2.windows_runtime.generic_launch_backend import Phase103RecordingLaunchBackend, WindowsControlledAppLaunchCandidate  # 新增代码+Phase105FullModeControlledRealLaunch：导入记录后端和受控候选；如果没有这行代码，合同无法证明默认关闭和启用桥接。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.generic_launch_backend"}:  # 新增代码+Phase105FullModeControlledRealLaunch：只兜底包路径缺失；如果没有这行代码，Phase103 内部 bug 会被误吞。
            raise  # 新增代码+Phase105FullModeControlledRealLaunch：重新抛出非路径类错误；如果没有这行代码，真实问题会被隐藏。
        from computer_use_mcp_v2.windows_runtime.generic_launch_backend import Phase103RecordingLaunchBackend, WindowsControlledAppLaunchCandidate  # type: ignore  # 新增代码+Phase105FullModeControlledRealLaunch：脚本模式导入 Phase103 候选；如果没有这行代码，bat 入口无法运行 Phase105 默认合同。
