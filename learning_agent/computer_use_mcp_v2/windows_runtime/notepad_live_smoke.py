...
from learning_agent.computer_use_mcp_v2.windows_runtime.live_app_dispatcher import WindowsLiveAppDispatcher  # 新增代码+Phase91NotepadLiveSmoke：复用 Phase90 统一 app/window/授权/输入调度器；如果没有这行代码，Notepad 路径可能绕开既有安全链路。
from learning_agent.computer_use_mcp_v2.windows_runtime.real_sendinput_guard import WindowsSendInputLowLevelSender  # 新增代码+Phase91NotepadLiveSmoke：复用 Phase58 低层 SendInput sender 作为显式真实 smoke 的发送层；如果没有这行代码，真实可选路径会重复造轮子。
from learning_agent.computer_use_mcp_v2.windows_runtime.windows_backend import WindowsWindowInventoryProbe  # 新增代码+Phase91NotepadLiveSmoke：复用 Phase28 只读窗口枚举来寻找专属 Notepad 窗口；如果没有这行代码，真实 smoke 无法复验目标窗口。
