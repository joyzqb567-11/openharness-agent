...
try:  # 新增代码+Phase66ObservationFusion: 优先按 learning_agent 包路径复用既有脱敏函数；如果没有这段代码，unittest 和生产入口不能共享 evidence 过滤逻辑。
    from learning_agent.computer_use_mcp_v2.windows_runtime.evidence import filter_accessibility_text  # 新增代码+Phase66ObservationFusion: 导入 UIA 文本过滤函数；如果没有这行代码，融合层可能把 password/token 类控件文本泄露出去。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.evidence"}:  # 新增代码+Phase66ObservationFusion: 只允许包路径缺失时 fallback；如果没有这行代码，真实内部 bug 会被误吞。
        raise  # 新增代码+Phase66ObservationFusion: 重新抛出非路径类导入错误；如果没有这行代码，排查 evidence 内部问题会困难。
    from computer_use_mcp_v2.windows_runtime.evidence import filter_accessibility_text  # 新增代码+Phase66ObservationFusion: 脚本模式导入 UIA 文本过滤函数；如果没有这行代码，bat 入口无法安全脱敏。
