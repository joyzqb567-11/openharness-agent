...
def _phase52_import_contracts() -> dict[str, Any]:  # 新增代码+Phase52WindowsProductionMatrix: 定义合同导入 helper；如果没有这行代码，run 函数会堆满重复导入代码。
    try:  # 新增代码+Phase52WindowsProductionMatrix: 优先按 learning_agent 包模式导入；如果没有这行代码，正常 unittest 和项目根目录运行无法加载阶段合同。
        ...
        from learning_agent.computer_use_mcp_v2.windows_runtime.native_diagnostics import phase43_cli_line, run_phase43_native_capability_matrix_contract  # 新增代码+Phase52WindowsProductionMatrix: 导入 Phase43 原生能力矩阵合同；如果没有这行代码，总矩阵无法覆盖诊断层。
        from learning_agent.computer_use_mcp_v2.windows_runtime.native_host import phase44_cli_line, run_phase44_native_host_contract  # 新增代码+Phase52WindowsProductionMatrix: 导入 Phase44 native host 合同；如果没有这行代码，总矩阵无法覆盖 host 协议。
        from learning_agent.computer_use_mcp_v2.windows_runtime.screenshot_runtime import phase45_cli_line, run_phase45_screenshot_runtime_contract  # 新增代码+Phase52WindowsProductionMatrix: 导入 Phase45 截图 runtime 合同；如果没有这行代码，总矩阵无法覆盖截图和 artifact。
        from learning_agent.computer_use_mcp_v2.windows_runtime.security_policy import phase48_cli_line, run_phase48_security_policy_contract  # 新增代码+Phase52WindowsProductionMatrix: 导入 Phase48 安全策略合同；如果没有这行代码，总矩阵无法覆盖授权和高风险默认拒绝。
        from learning_agent.computer_use_mcp_v2.windows_runtime.sendinput_dispatcher import phase47_cli_line, run_phase47_sendinput_dispatcher_contract  # 新增代码+Phase52WindowsProductionMatrix: 导入 Phase47 SendInput dispatcher 合同；如果没有这行代码，总矩阵无法覆盖动作分发器。
        from learning_agent.computer_use_mcp_v2.windows_runtime.session_runtime import phase50_cli_line, run_phase50_recovery_contract  # 新增代码+Phase52WindowsProductionMatrix: 导入 Phase50 恢复 runtime 合同；如果没有这行代码，总矩阵无法覆盖 stale lock 和 journal。
        from learning_agent.computer_use_mcp_v2.windows_runtime.tool_surface import phase49_cli_line, run_phase49_tool_surface_contract  # 新增代码+Phase52WindowsProductionMatrix: 导入 Phase49 工具面合同；如果没有这行代码，总矩阵无法覆盖兼容工具入口。
        from learning_agent.computer_use_mcp_v2.windows_runtime.uia_tree import phase46_cli_line, run_phase46_uia_tree_contract  # 新增代码+Phase52WindowsProductionMatrix: 导入 Phase46 UIA tree 合同；如果没有这行代码，总矩阵无法覆盖控件树观察。
        ...
        if error.name not in {"learning_agent", "learning_agent.app", "learning_agent.app.computer_status_renderer", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.native_diagnostics", "learning_agent.computer_use_mcp_v2.windows_runtime.native_host", "learning_agent.computer_use_mcp_v2.windows_runtime.screenshot_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.security_policy", "learning_agent.computer_use_mcp_v2.windows_runtime.sendinput_dispatcher", "learning_agent.computer_use_mcp_v2.windows_runtime.session_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.tool_surface", "learning_agent.computer_use_mcp_v2.windows_runtime.uia_tree"}:  # 新增代码+Phase52WindowsProductionMatrix: 只允许目标包路径缺失时 fallback；如果没有这行代码，真实内部 bug 会被误吞。
        ...
        from computer_use_mcp_v2.windows_runtime.native_diagnostics import phase43_cli_line, run_phase43_native_capability_matrix_contract  # 新增代码+Phase52WindowsProductionMatrix: 脚本模式导入 Phase43 合同；如果没有这行代码，bat 入口无法运行原生能力总验收。
        from computer_use_mcp_v2.windows_runtime.native_host import phase44_cli_line, run_phase44_native_host_contract  # 新增代码+Phase52WindowsProductionMatrix: 脚本模式导入 Phase44 合同；如果没有这行代码，bat 入口无法运行 native host 总验收。
        from computer_use_mcp_v2.windows_runtime.screenshot_runtime import phase45_cli_line, run_phase45_screenshot_runtime_contract  # 新增代码+Phase52WindowsProductionMatrix: 脚本模式导入 Phase45 合同；如果没有这行代码，bat 入口无法运行截图 runtime 总验收。
        from computer_use_mcp_v2.windows_runtime.security_policy import phase48_cli_line, run_phase48_security_policy_contract  # 新增代码+Phase52WindowsProductionMatrix: 脚本模式导入 Phase48 合同；如果没有这行代码，bat 入口无法运行安全策略总验收。
        from computer_use_mcp_v2.windows_runtime.sendinput_dispatcher import phase47_cli_line, run_phase47_sendinput_dispatcher_contract  # 新增代码+Phase52WindowsProductionMatrix: 脚本模式导入 Phase47 合同；如果没有这行代码，bat 入口无法运行 dispatcher 总验收。
        from computer_use_mcp_v2.windows_runtime.session_runtime import phase50_cli_line, run_phase50_recovery_contract  # 新增代码+Phase52WindowsProductionMatrix: 脚本模式导入 Phase50 合同；如果没有这行代码，bat 入口无法运行恢复 runtime 总验收。
        from computer_use_mcp_v2.windows_runtime.tool_surface import phase49_cli_line, run_phase49_tool_surface_contract  # 新增代码+Phase52WindowsProductionMatrix: 脚本模式导入 Phase49 合同；如果没有这行代码，bat 入口无法运行工具面总验收。
        from computer_use_mcp_v2.windows_runtime.uia_tree import phase46_cli_line, run_phase46_uia_tree_contract  # 新增代码+Phase52WindowsProductionMatrix: 脚本模式导入 Phase46 合同；如果没有这行代码，bat 入口无法运行 UIA tree 总验收。
...
def run_phase52_production_matrix_contract() -> dict[str, Any]:  # 新增代码+Phase52WindowsProductionMatrix: 定义 Phase52 总合同入口；如果没有这行代码，测试和终端场景没有统一自检函数。
    ...
    tool_surface = bool(phase49_report.get("legacy_tools_hidden") and phase49_report.get("catalog_clean") and phase49_report.get("mcp_tools_present") and phase49_report.get("duplicate_raw_names_absent") and phase49_report.get("compat_dispatch_blocked"))  # 修改代码+ComputerUseMcpV2ResidualCleanup：Phase49 现在验证旧入口隐藏和 v2 MCP 工具齐全；如果没有这行代码，生产矩阵会继续把旧兼容入口存在当成成功。
