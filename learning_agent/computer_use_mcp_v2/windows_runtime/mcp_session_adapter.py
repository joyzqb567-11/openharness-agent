...
class ComputerUseMcpSessionState:  # 新增代码+McpSessionAdapter: 函数段开始，保存一次 agent 会话内共享状态；如果没有这个类，request_access 和 clipboard 无法跨工具调用闭环。
...
def _controller_arguments_for_tool(tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:  # 新增代码+McpSessionAdapter: 函数段开始，把 MCP 原子名映射为旧 controller action；如果没有这段函数，click/type/key 无法复用旧 computer_action。
...
def _action_can_reuse_observed_window(legacy_arguments: dict[str, Any]) -> bool:  # 新增代码+McpObservedWindowFix: 函数段开始，判断旧动作是否适合复用 observe 窗口；如果没有这段函数，launch_app 这类动作可能被错误塞入旧窗口。
    ...
    reusable_actions = {"click", "double_click", "move_mouse", "type_text", "press_key", "scroll", "drag_path"}  # 新增代码+McpObservedWindowFix: 限定需要桌面目标的动作集合；如果没有这一行，窗口复用范围会不清楚。
...
class ComputerUseMcpSessionAdapter:  # 新增代码+McpSessionAdapter: 函数段开始，执行 MCP 原子工具并复用 agent 旧能力；如果没有这个类，stdio MCP server 无法拿到 agent 回调。
    ...
    def call_atomic_tool(self, tool_name: str, arguments: dict[str, Any] | None = None) -> dict[str, Any]:  # 新增代码+McpSessionAdapter: 函数段开始，公开执行单个 MCP 原子工具；如果没有这段函数，agent_adapter 无法调用这层绑定。
        ...
        if normalized_tool_name in {"triple_click", "left_mouse_down", "left_mouse_up"}:  # 新增代码+McpSessionAdapter: 暂未接真实低层事件的预留工具；如果没有这一行，预留名会进入 controller 返回不清楚错误。
            return _json_result(normalized_tool_name, False, {"reason": f"{normalized_tool_name} 已预留，但当前后端尚未接入真实低层事件实现。"}, error_class="unsupported_computer_use_tool")  # 新增代码+McpSessionAdapter: 返回明确 unsupported；如果没有这一行，模型无法区分参数错和功能未接。
