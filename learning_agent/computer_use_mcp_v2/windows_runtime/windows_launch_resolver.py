...
try:  # 新增代码+WindowsLaunchResolver：优先按包路径导入统一应用清单；如果没有这一行，项目根目录运行时无法复用 Phase122 inventory。
    from learning_agent.computer_use_mcp_v2.windows_runtime.windows_app_inventory import build_windows_app_inventory  # 新增代码+WindowsLaunchResolver：读取统一 Windows 应用清单构造器；如果没有这一行，resolver 会重新发明一套候选清洗逻辑。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.windows_app_inventory"}:  # 新增代码+WindowsLaunchResolver：只兜底包路径缺失；如果没有这一行，inventory 内部真实 bug 会被误吞。
        raise  # 新增代码+WindowsLaunchResolver：重新抛出真实内部错误；如果没有这一行，排查 resolver 失败会被隐藏。
    from computer_use_mcp_v2.windows_runtime.windows_app_inventory import build_windows_app_inventory  # type: ignore  # 新增代码+WindowsLaunchResolver：脚本模式导入统一 inventory；如果没有这一行，bat 启动方式下 resolver 不可用。
