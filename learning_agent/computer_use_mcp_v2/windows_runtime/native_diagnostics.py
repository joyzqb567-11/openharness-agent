...
try:  # 新增代码+Phase36WindowsWGCProvider: 优先按包模式导入 Phase36 WGC provider；如果没有这行代码，diagnostics 无法复用真实 provider 合同。
    from learning_agent.computer_use_mcp_v2.windows_runtime.wgc_capture import WindowsGraphicsCaptureProvider  # 新增代码+Phase36WindowsWGCProvider: 导入 WGC provider 状态构建器；如果没有这行代码，WGC 诊断会停留在 Phase33 占位文案。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.wgc_capture"}:  # 新增代码+Phase36WindowsWGCProvider: 只处理预期包路径缺失；如果没有这行代码，真实内部 bug 会被误吞。
        raise  # 新增代码+Phase36WindowsWGCProvider: 重新抛出非路径类导入错误；如果没有这行代码，排查 provider 内部错误会困难。
    from computer_use_mcp_v2.windows_runtime.wgc_capture import WindowsGraphicsCaptureProvider  # 新增代码+Phase36WindowsWGCProvider: 脚本模式导入 WGC provider；如果没有这行代码，直接从 learning_agent 目录运行会缺少 Phase36 诊断。
