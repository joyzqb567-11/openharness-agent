...
try:  # 新增代码+Phase39WindowsCoordinates: 优先用包路径导入 Phase39 坐标模型；如果没有这行代码，动作策略无法接入 DPI 和多显示器换算。
    from learning_agent.computer_use_mcp_v2.windows_runtime.coordinates import PHASE39_COORDINATE_MODEL, build_coordinate_context  # 新增代码+Phase39WindowsCoordinates: 导入坐标模型版本和核心换算函数；如果没有这行代码，后端仍会收到旧逻辑屏幕坐标。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.coordinates"}:  # 新增代码+Phase39WindowsCoordinates: 只允许目标包路径缺失时 fallback；如果没有这行代码，coordinates 内部真实 bug 会被误吞。
...
def prepare_action_arguments(action: str, arguments: dict[str, Any]) -> dict[str, Any]:  # 新增代码+Phase30ComputerUseActionGate: 定义动作参数准备函数；如果没有这行代码，controller 需要直接操作坐标细节。
    ...
    if action == "drag_path":  # 新增代码+GenericDragPathToolSurface: 单独处理连续路径坐标；如果没有这行代码，画图类任务只能点选，不能把一串点转换成真实拖拽。
        raw_points = arguments.get("points", [])  # 新增代码+GenericDragPathToolSurface: 读取模型给出的路径点列表；如果没有这行代码，后端拿不到要拖拽的轨迹。
        point_items = list(raw_points) if isinstance(raw_points, list) else []  # 新增代码+GenericDragPathToolSurface: 只接受列表形态的 points；如果没有这行代码，坏参数可能在遍历时崩溃。
        converted_points: list[dict[str, int]] = []  # 新增代码+GenericDragPathToolSurface: 准备保存后端可用的屏幕坐标点；如果没有这行代码，转换结果没有容器。
        for point in point_items:  # 新增代码+GenericDragPathToolSurface: 逐点转换路径；如果没有这行代码，只能处理一个点，无法形成连续线条。
            if not isinstance(point, dict):  # 新增代码+GenericDragPathToolSurface: 跳过非对象点；如果没有这行代码，坏点会导致 get 调用崩溃。
                continue  # 新增代码+GenericDragPathToolSurface: 忽略坏点继续处理剩余路径；如果没有这行代码，一个坏点会破坏整条线。
            relative_x = safe_int(point.get("x"))  # 新增代码+GenericDragPathToolSurface: 读取当前点的窗口相对 x；如果没有这行代码，横坐标无法进入转换模型。
            relative_y = safe_int(point.get("y"))  # 新增代码+GenericDragPathToolSurface: 读取当前点的窗口相对 y；如果没有这行代码，纵坐标无法进入转换模型。
            coordinate_context = build_coordinate_context(raw_window, relative_x, relative_y) if rect else {}  # 新增代码+GenericDragPathToolSurface: 有窗口 rect 时复用统一坐标模型；如果没有这行代码，高 DPI 或副屏路径可能画错位置。
            physical_screen = dict(coordinate_context.get("physical_screen", {})) if coordinate_context else {}  # 新增代码+GenericDragPathToolSurface: 读取物理屏幕坐标；如果没有这行代码，后端无法和 SendInput 坐标空间对齐。
            screen_x = safe_int(physical_screen.get("x"), relative_x) if rect else relative_x  # 新增代码+GenericDragPathToolSurface: 计算最终屏幕 x；如果没有这行代码，路径点会停留在窗口相对坐标。
            screen_y = safe_int(physical_screen.get("y"), relative_y) if rect else relative_y  # 新增代码+GenericDragPathToolSurface: 计算最终屏幕 y；如果没有这行代码，路径点会停留在窗口相对坐标。
            converted_points.append({"x": screen_x, "y": screen_y})  # 新增代码+GenericDragPathToolSurface: 追加转换后的点；如果没有这行代码，最终路径为空。
        backend_arguments["points"] = converted_points  # 新增代码+GenericDragPathToolSurface: 把后端参数替换成屏幕坐标路径；如果没有这行代码，真实 dispatcher 会按错误坐标执行。
        coordinate_used = {"space": "screen", "source": "window_relative_path" if rect else "screen_input_path", "point_count": len(converted_points), "points_preview": converted_points[:3], "window_origin": {"x": safe_int(rect.get("left")), "y": safe_int(rect.get("top"))} if rect else {}, "model": PHASE39_COORDINATE_MODEL}  # 新增代码+GenericDragPathToolSurface: 记录路径坐标证据但只预览前三点；如果没有这行代码，用户无法审计路径转换，日志也可能过长。
        return {"backend_arguments": backend_arguments, "coordinate_used": coordinate_used, "target_window": target_window_for_evidence(raw_window)}  # 新增代码+GenericDragPathToolSurface: 返回拖拽路径专用准备结果；如果没有这行代码，后续单点坐标分支会误处理 points。
