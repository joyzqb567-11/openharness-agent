...
try:  # 新增代码+Phase57RealUiaLocator: 优先按包模式导入既有 Computer Use 组件；如果没有这段代码，unittest 和生产入口不能复用项目模块。
    from learning_agent.computer_use_mcp_v2.windows_runtime.evidence import filter_accessibility_text  # 新增代码+Phase57RealUiaLocator: 复用既有 UIA 文本脱敏函数；如果没有这行代码，控件名称可能泄露 password/token。
    from learning_agent.computer_use_mcp_v2.windows_runtime.native_helper import parse_hwnd_from_window  # 新增代码+Phase57RealUiaLocator: 复用统一 hwnd 解析逻辑；如果没有这行代码，不同模块会用不同窗口身份规则。
    from learning_agent.computer_use_mcp_v2.windows_runtime.windows_backend import WindowsWindowInventoryProbe  # 修改代码+Phase57RealUiaLocator: 复用真实窗口枚举能力定位自有安全窗体；如果没有这行代码，real smoke 找不到安全窗口。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.evidence", "learning_agent.computer_use_mcp_v2.windows_runtime.native_helper", "learning_agent.computer_use_mcp_v2.windows_runtime.windows_backend"}:  # 新增代码+Phase57RealUiaLocator: 只允许包路径缺失时 fallback；如果没有这行代码，真实内部 bug 会被误吞。
        raise  # 新增代码+Phase57RealUiaLocator: 重新抛出非路径类导入错误；如果没有这行代码，排查 evidence/native_helper 内部错误会困难。
    from computer_use_mcp_v2.windows_runtime.evidence import filter_accessibility_text  # 新增代码+Phase57RealUiaLocator: 脚本模式复用 UIA 文本脱敏函数；如果没有这行代码，bat 入口无法安全脱敏。
    from computer_use_mcp_v2.windows_runtime.native_helper import parse_hwnd_from_window  # 新增代码+Phase57RealUiaLocator: 脚本模式复用 hwnd 解析；如果没有这行代码，bat 入口无法识别目标窗口。
    from computer_use_mcp_v2.windows_runtime.windows_backend import WindowsWindowInventoryProbe  # 新增代码+Phase57RealUiaLocator: 脚本模式复用窗口枚举；如果没有这行代码，bat 入口无法定位安全窗口。
...
def run_phase57_real_uia_locator_contract(real_smoke: bool = True) -> dict[str, Any]:  # 新增代码+Phase57RealUiaLocator: 函数段开始，运行 Phase57 合同自检和可选真实 smoke；如果没有这段函数，CLI 和真实终端没有统一入口。
    ...
    from learning_agent.computer_use_mcp_v2.windows_runtime.native_helper_v2 import WindowsNativeHelperV2Worker  # 新增代码+Phase57RealUiaLocator: 延迟导入 helper v2 避免模块循环；如果没有这行代码，合同无法证明 helper read_uia_tree 接入。
