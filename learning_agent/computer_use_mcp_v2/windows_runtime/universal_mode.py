...
try:  # 新增代码+Phase92UniversalComputerUse：优先按 learning_agent 包路径导入已有组件；如果没有这段代码，单元测试和生产入口不能共享同一套模块。
    from learning_agent.computer_use_mcp_v2.windows_runtime.generic_control_actions import Phase70RecordingHighLevelTool, WindowsGenericControlActionRuntime  # 新增代码+Phase92UniversalComputerUse：复用通用控件动作层；如果没有这行代码，Phase92 会退回按应用写控制器。
    from learning_agent.computer_use_mcp_v2.windows_runtime.generic_input_actions import Phase71RecordingInputSender, WindowsGenericInputActionRuntime  # 新增代码+Phase92UniversalComputerUse：复用通用键鼠事件构建层；如果没有这行代码，菜单、热键、滚轮和拖拽无法纳入统一模式。
    from learning_agent.computer_use_mcp_v2.windows_runtime.observation_fusion import WindowsObservationFusionRuntime  # 新增代码+Phase92UniversalComputerUse：复用观察融合层；如果没有这行代码，运行时无法统一看截图、UIA、OCR 和窗口状态。
    from learning_agent.computer_use_mcp_v2.windows_runtime.persistent_grants import WindowsComputerUsePersistentGrantStore  # 新增代码+Phase92UniversalComputerUse：复用持久授权 store；如果没有这行代码，真实应用安全边界没有授权事实源。
    from learning_agent.computer_use_mcp_v2.windows_runtime.production_live_control import WindowsProductionComputerUseHostAdapter  # 新增代码+Phase92UniversalComputerUse：复用生产 host adapter；如果没有这行代码，运行时无法声明和真实 Windows 控制桥的连接点。
    from learning_agent.computer_use_mcp_v2.windows_runtime.prompt_task_planner import WindowsPromptTaskPlanner, classify_risk  # 新增代码+Phase92UniversalComputerUse：复用 prompt 任务规划器和风险分类；如果没有这行代码，自然语言任务无法转成统一步骤。
    from learning_agent.computer_use_mcp_v2.windows_runtime.real_app_safety_boundary import WindowsRealAppSafetyBoundary  # 新增代码+Phase92UniversalComputerUse：复用真实应用安全边界；如果没有这行代码，未授权窗口和高风险窗口无法统一拒绝。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.runtime", "learning_agent.runtime.files"}:  # 新增代码+Phase92UniversalComputerUse：只对包路径缺失做 fallback；如果没有这行代码，真实内部 bug 会被误吞。
        raise  # 新增代码+Phase92UniversalComputerUse：重新抛出非路径类导入错误；如果没有这行代码，依赖内部错误会被隐藏。
    from computer_use_mcp_v2.windows_runtime.generic_control_actions import Phase70RecordingHighLevelTool, WindowsGenericControlActionRuntime  # type: ignore  # 新增代码+Phase92UniversalComputerUse：脚本模式复用通用控件动作层；如果没有这行代码，bat 入口无法加载 Phase92 控件动作依赖。
    from computer_use_mcp_v2.windows_runtime.generic_input_actions import Phase71RecordingInputSender, WindowsGenericInputActionRuntime  # type: ignore  # 新增代码+Phase92UniversalComputerUse：脚本模式复用通用输入动作层；如果没有这行代码，bat 入口无法构建热键菜单滚轮拖拽能力。
    from computer_use_mcp_v2.windows_runtime.observation_fusion import WindowsObservationFusionRuntime  # type: ignore  # 新增代码+Phase92UniversalComputerUse：脚本模式复用观察融合层；如果没有这行代码，bat 入口无法构建统一观察对象。
    from computer_use_mcp_v2.windows_runtime.persistent_grants import WindowsComputerUsePersistentGrantStore  # type: ignore  # 新增代码+Phase92UniversalComputerUse：脚本模式复用持久授权 store；如果没有这行代码，bat 入口无法验证未授权拒绝。
    from computer_use_mcp_v2.windows_runtime.production_live_control import WindowsProductionComputerUseHostAdapter  # type: ignore  # 新增代码+Phase92UniversalComputerUse：脚本模式复用生产 host adapter；如果没有这行代码，bat 入口无法连接生产控制桥声明。
    from computer_use_mcp_v2.windows_runtime.prompt_task_planner import WindowsPromptTaskPlanner, classify_risk  # type: ignore  # 新增代码+Phase92UniversalComputerUse：脚本模式复用任务规划器；如果没有这行代码，bat 入口无法从 prompt 得到步骤。
    from computer_use_mcp_v2.windows_runtime.real_app_safety_boundary import WindowsRealAppSafetyBoundary  # type: ignore  # 新增代码+Phase92UniversalComputerUse：脚本模式复用安全边界；如果没有这行代码，bat 入口无法执行高风险拒绝。
