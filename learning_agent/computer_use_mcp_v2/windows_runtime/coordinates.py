...
CLAUDECODE_DISPLAY_STATE_MODEL = "claudecode_display_state_v1"  # 新增代码+ClaudeCodeDisplayParity: 定义 ClaudeCode display state 抽取合同版本；如果没有这行代码，审计无法判断 selectedDisplayId 等字段来自哪套规则。
...
# 新增代码+ClaudeCodeDisplayParity: 函数段开始，把任意值规范成正整数；如果没有这段函数，display state 读取截图尺寸时会把负数或坏字符串带给模型。
def _positive_int(value: Any) -> int:  # 新增代码+ClaudeCodeDisplayParity: 定义正整数读取 helper；如果没有这行代码，宽高字段会在多个分支重复写转换逻辑。
    number = safe_int(value)  # 新增代码+ClaudeCodeDisplayParity: 复用现有安全取整规则；如果没有这行代码，字符串宽高和浮点宽高无法兼容。
    return number if number > 0 else 0  # 新增代码+ClaudeCodeDisplayParity: 只允许正数尺寸进入状态；如果没有这行代码，0 或负数截图尺寸会误导后续 zoom。
# 新增代码+ClaudeCodeDisplayParity: 函数段结束，_positive_int 到此结束；如果没有这个边界说明，读者不容易看出尺寸兜底范围。


# 新增代码+ClaudeCodeDisplayParity: 函数段开始，从 payload 的多种命名里提取截图宽高；如果没有这段函数，lastScreenshotDims 会依赖单一后端字段而容易丢失。
def _dims_from_mapping(value: Any) -> dict[str, int]:  # 新增代码+ClaudeCodeDisplayParity: 定义宽高提取 helper；如果没有这行代码，观察层需要理解所有截图 payload 形状。
    source = value if isinstance(value, dict) else {}  # 新增代码+ClaudeCodeDisplayParity: 只信任字典来源；如果没有这行代码，字符串或 None 会触发 get 错误。
    width = _positive_int(source.get("width", source.get("screenshot_width", source.get("image_width", 0))))  # 新增代码+ClaudeCodeDisplayParity: 兼容 width/screenshot_width/image_width；如果没有这行代码，常见截图宽度字段会被漏掉。
    height = _positive_int(source.get("height", source.get("screenshot_height", source.get("image_height", 0))))  # 新增代码+ClaudeCodeDisplayParity: 兼容 height/screenshot_height/image_height；如果没有这行代码，常见截图高度字段会被漏掉。
    if width > 0 and height > 0:  # 新增代码+ClaudeCodeDisplayParity: 先使用直接字段；如果没有这行代码，已经明确的尺寸会被后续嵌套字段覆盖。
        return {"width": width, "height": height}  # 新增代码+ClaudeCodeDisplayParity: 返回 ClaudeCode lastScreenshotDims 形状；如果没有这行代码，调用方拿不到直接尺寸。
    for nested_key in ("screenshot_pixel_size", "screenshot_pixel_rect", "coordinate_mapping"):  # 新增代码+ClaudeCodeDisplayParity: 继续检查截图坐标映射中的尺寸字段；如果没有这行代码，坐标模块产出的尺寸不会回流 display state。
        nested_value = source.get(nested_key)  # 修改代码+ClaudeCodeDisplayParity：先取出嵌套值再判断类型；如果没有这行代码，None 会被递归成空字典并造成无限递归。
        if not isinstance(nested_value, dict) or not nested_value:  # 修改代码+ClaudeCodeDisplayParity：只递归非空字典；如果没有这行代码，缺失字段会反复递归直到 RecursionError。
            continue  # 修改代码+ClaudeCodeDisplayParity：跳过空嵌套值；如果没有这行代码，后续仍会递归坏输入。
        nested_dims = _dims_from_mapping(nested_value)  # 修改代码+ClaudeCodeDisplayParity：递归读取有效嵌套尺寸；如果没有这行代码，坐标映射里的尺寸无法回流 display state。
        if nested_dims:  # 新增代码+ClaudeCodeDisplayParity: 命中嵌套尺寸时立即返回；如果没有这行代码，后续空分支可能覆盖有效结果。
            return nested_dims  # 新增代码+ClaudeCodeDisplayParity: 返回嵌套读取到的尺寸；如果没有这行代码，lastScreenshotDims 会继续为空。
    for list_key in ("image_results", "zoom_image_results"):  # 新增代码+ClaudeCodeDisplayParity: 检查普通截图和 zoom 截图列表；如果没有这行代码，图片条目里的宽高不会进入上下文。
        entries = source.get(list_key) if isinstance(source.get(list_key), list) else []  # 新增代码+ClaudeCodeDisplayParity: 只读取列表形图片条目；如果没有这行代码，坏字段类型会被误遍历。
        for entry in entries:  # 新增代码+ClaudeCodeDisplayParity: 逐个尝试图片条目；如果没有这行代码，多张图片时无法找到第一张有效尺寸。
            entry_dims = _dims_from_mapping(entry)  # 新增代码+ClaudeCodeDisplayParity: 复用同一宽高读取规则；如果没有这行代码，image_results 的字段兼容会分叉。
            if entry_dims:  # 新增代码+ClaudeCodeDisplayParity: 找到有效图片尺寸时停止；如果没有这行代码，有效尺寸可能继续被空结果覆盖。
                return entry_dims  # 新增代码+ClaudeCodeDisplayParity: 返回图片条目尺寸；如果没有这行代码，context 无法记录截图实际像素。
    legacy_result = source.get("legacy_result") if isinstance(source.get("legacy_result"), dict) else {}  # 新增代码+ClaudeCodeDisplayParity: 读取旧 adapter 嵌套结果；如果没有这行代码，legacy payload 里的截图尺寸会被漏掉。
    legacy_payload = legacy_result.get("payload") if isinstance(legacy_result.get("payload"), dict) else {}  # 新增代码+ClaudeCodeDisplayParity: 读取 legacy payload；如果没有这行代码，深层尺寸无法递归查找。
    return _dims_from_mapping(legacy_payload) if legacy_payload and legacy_payload is not source else {}  # 新增代码+ClaudeCodeDisplayParity: 安全递归 legacy payload；如果没有这行代码，旧包装结果不会更新 lastScreenshotDims。
# 新增代码+ClaudeCodeDisplayParity: 函数段结束，_dims_from_mapping 到此结束；如果没有这个边界说明，读者不容易看出宽高抽取范围。


# 新增代码+ClaudeCodeDisplayParity: 函数段开始，从 payload 的多种位置提取 display id；如果没有这段函数，selectedDisplayId 只能依赖模型显式传参。
def _selected_display_id_from_mapping(value: Any) -> str:  # 新增代码+ClaudeCodeDisplayParity: 定义 display id 提取 helper；如果没有这行代码，观察层会散落多套字段名判断。
    source = value if isinstance(value, dict) else {}  # 新增代码+ClaudeCodeDisplayParity: 只信任字典来源；如果没有这行代码，坏类型会触发 get 错误。
    direct_value = source.get("selectedDisplayId", source.get("selected_display_id", source.get("display_id", source.get("displayId", ""))))  # 新增代码+ClaudeCodeDisplayParity: 优先读取 ClaudeCode 和 Windows 常见直出字段；如果没有这行代码，最直接的选择状态会丢失。
    if direct_value:  # 新增代码+ClaudeCodeDisplayParity: 命中直接字段时返回；如果没有这行代码，后续嵌套字段可能错误覆盖模型选择。
        return str(direct_value)  # 新增代码+ClaudeCodeDisplayParity: 统一转成字符串；如果没有这行代码，数字 id 或对象 id 可能污染 JSON。
    for nested_key in ("display", "coordinate_model", "coordinate_context", "coordinate_used", "window"):  # 新增代码+ClaudeCodeDisplayParity: 检查 Windows 坐标和窗口元数据里的 display；如果没有这行代码，自动解析显示器无法回填 selectedDisplayId。
        nested_source = source.get(nested_key) if isinstance(source.get(nested_key), dict) else {}  # 新增代码+ClaudeCodeDisplayParity: 只递归字典嵌套；如果没有这行代码，列表或字符串会触发错误。
        nested_value = nested_source.get("display_id", nested_source.get("displayId", nested_source.get("id", "")))  # 新增代码+ClaudeCodeDisplayParity: 读取嵌套 display_id/displayId/id；如果没有这行代码，坐标模型里的屏幕 id 无法使用。
        if nested_value:  # 新增代码+ClaudeCodeDisplayParity: 命中嵌套 id 时返回；如果没有这行代码，display id 可能继续为空。
            return str(nested_value)  # 新增代码+ClaudeCodeDisplayParity: 返回规范字符串 id；如果没有这行代码，调用方需要处理混合类型。
    legacy_result = source.get("legacy_result") if isinstance(source.get("legacy_result"), dict) else {}  # 新增代码+ClaudeCodeDisplayParity: 读取 legacy 嵌套结果；如果没有这行代码，旧 adapter 的显示器字段会被漏掉。
    legacy_payload = legacy_result.get("payload") if isinstance(legacy_result.get("payload"), dict) else {}  # 新增代码+ClaudeCodeDisplayParity: 读取 legacy payload；如果没有这行代码，递归没有输入。
    return _selected_display_id_from_mapping(legacy_payload) if legacy_payload and legacy_payload is not source else ""  # 新增代码+ClaudeCodeDisplayParity: 从 legacy payload 继续查找；如果没有这行代码，旧包装链路无法填 selectedDisplayId。
# 新增代码+ClaudeCodeDisplayParity: 函数段结束，_selected_display_id_from_mapping 到此结束；如果没有这个边界说明，读者不容易看出 display id 抽取范围。


# 新增代码+ClaudeCodeDisplayParity: 函数段开始，从窗口元数据生成 app-display 解析记录；如果没有这段函数，displayResolvedForApps 不会反映 Windows 观察到的真实窗口。
def _resolved_app_from_mapping(value: Any, selected_display_id: str) -> dict[str, Any]:  # 新增代码+ClaudeCodeDisplayParity: 定义 app 到 display 记录 helper；如果没有这行代码，观察层需要手写 app/window 字段兼容。
    source = value if isinstance(value, dict) else {}  # 新增代码+ClaudeCodeDisplayParity: 只信任字典 payload；如果没有这行代码，坏类型会触发 get 错误。
    window = source.get("target_window") if isinstance(source.get("target_window"), dict) else source.get("window") if isinstance(source.get("window"), dict) else source.get("active_window") if isinstance(source.get("active_window"), dict) else {}  # 新增代码+ClaudeCodeDisplayParity: 兼容 target_window/window/active_window；如果没有这行代码，不同 host 的窗口字段无法统一解析。
    app_id = str(window.get("app_id", window.get("bundle_id", window.get("bundleId", ""))) or "")  # 新增代码+ClaudeCodeDisplayParity: 读取应用标识；如果没有这行代码，displayResolvedForApps 无法说明是哪一个 app。
    display_id = selected_display_id or _selected_display_id_from_mapping(window)  # 新增代码+ClaudeCodeDisplayParity: 优先使用已选显示器，必要时从窗口里补；如果没有这行代码，解析记录可能缺少 displayId。
    if not app_id or not display_id:  # 新增代码+ClaudeCodeDisplayParity: app 或 display 任一缺失就不生成记录；如果没有这行代码，模型会看到半截无效解析。
        return {}  # 新增代码+ClaudeCodeDisplayParity: 返回空记录表示无法解析；如果没有这行代码，调用方要处理无效字段。
    return {"appId": app_id, "displayId": display_id, "titlePreview": str(window.get("title_preview", window.get("title", "")) or ""), "windowId": str(window.get("window_id", window.get("id", "")) or "")}  # 新增代码+ClaudeCodeDisplayParity: 返回 ClaudeCode camelCase 解析记录；如果没有这行代码，模型无法把 app 和 display 关联起来。
...
def claudecode_display_state_from_payload(payload: Any) -> dict[str, Any]:  # 新增代码+ClaudeCodeDisplayParity: 定义公开 display state 抽取 API；如果没有这行代码，observation.py 无法复用坐标模块事实。
    ...
    selected_display_id = _selected_display_id_from_mapping(source)  # 新增代码+ClaudeCodeDisplayParity: 提取当前显示器 id；如果没有这行代码，selectedDisplayId 无法由截图结果回填。
    last_screenshot_dims = _dims_from_mapping(source)  # 新增代码+ClaudeCodeDisplayParity: 提取最近截图宽高；如果没有这行代码，lastScreenshotDims 无法写入上下文。
    resolved_for_apps: list[dict[str, Any]] = []  # 新增代码+ClaudeCodeDisplayParity: 准备 app-display 解析列表；如果没有这行代码，多应用解析无法累积。
    app_resolution = _resolved_app_from_mapping(source, selected_display_id)  # 新增代码+ClaudeCodeDisplayParity: 读取当前 payload 的 app-display 记录；如果没有这行代码，displayResolvedForApps 总是空。
    if app_resolution:  # 新增代码+ClaudeCodeDisplayParity: 有效记录才追加；如果没有这行代码，空字典会污染返回。
        resolved_for_apps.append(app_resolution)  # 新增代码+ClaudeCodeDisplayParity: 保存当前解析记录；如果没有这行代码，调用方看不到应用落屏信息。
    legacy_result = source.get("legacy_result") if isinstance(source.get("legacy_result"), dict) else {}  # 新增代码+ClaudeCodeDisplayParity: 读取 legacy 结果以兼容旧 adapter 包装；如果没有这行代码，深层观察事实会被漏掉。
    legacy_payload = legacy_result.get("payload") if isinstance(legacy_result.get("payload"), dict) else {}  # 新增代码+ClaudeCodeDisplayParity: 读取 legacy payload；如果没有这行代码，递归没有输入。
    if legacy_payload and legacy_payload is not source:  # 新增代码+ClaudeCodeDisplayParity: 防止空 payload 和自引用递归；如果没有这行代码，坏数据可能导致无限递归。
        nested_state = claudecode_display_state_from_payload(legacy_payload)  # 新增代码+ClaudeCodeDisplayParity: 递归抽取旧包装状态；如果没有这行代码，legacy adapter 的 display 信息无法上浮。
        selected_display_id = selected_display_id or str(nested_state.get("selectedDisplayId", "") or "")  # 新增代码+ClaudeCodeDisplayParity: 外层缺 display 时用内层补；如果没有这行代码，selectedDisplayId 可能保持为空。
        last_screenshot_dims = last_screenshot_dims or dict(nested_state.get("lastScreenshotDims", {}) or {})  # 新增代码+ClaudeCodeDisplayParity: 外层缺尺寸时用内层补；如果没有这行代码，legacy 截图宽高可能丢失。
        resolved_for_apps.extend([dict(item) for item in nested_state.get("displayResolvedForApps", []) if isinstance(item, dict)])  # 新增代码+ClaudeCodeDisplayParity: 合并内层 app-display 记录；如果没有这行代码，嵌套窗口解析无法返回。
    deduped: list[dict[str, Any]] = []  # 新增代码+ClaudeCodeDisplayParity: 准备去重后的解析记录；如果没有这行代码，同一窗口可能重复出现在结果中。
    seen_keys: set[tuple[str, str, str]] = set()  # 新增代码+ClaudeCodeDisplayParity: 记录 appId/displayId/windowId 组合；如果没有这行代码，去重无法判断重复。
    for item in resolved_for_apps:  # 新增代码+ClaudeCodeDisplayParity: 遍历候选解析记录；如果没有这行代码，无法生成稳定列表。
        key = (str(item.get("appId", "")), str(item.get("displayId", "")), str(item.get("windowId", "")))  # 新增代码+ClaudeCodeDisplayParity: 构造去重 key；如果没有这行代码，同一记录无法识别。
        if key in seen_keys or not key[0] or not key[1]:  # 新增代码+ClaudeCodeDisplayParity: 跳过重复或缺关键字段记录；如果没有这行代码，返回会包含噪声。
            continue  # 新增代码+ClaudeCodeDisplayParity: 跳过当前无效项；如果没有这行代码，后续会加入坏记录。
        seen_keys.add(key)  # 新增代码+ClaudeCodeDisplayParity: 标记该组合已使用；如果没有这行代码，后续重复不会被拦截。
        deduped.append(item)  # 新增代码+ClaudeCodeDisplayParity: 保存有效解析记录；如果没有这行代码，displayResolvedForApps 会空。
    return {"model": CLAUDECODE_DISPLAY_STATE_MODEL, "selectedDisplayId": selected_display_id, "displayResolvedForApps": deduped, "lastScreenshotDims": last_screenshot_dims}  # 新增代码+ClaudeCodeDisplayParity: 返回 ClaudeCode-facing display state 摘要；如果没有这行代码，观察层拿不到统一结果。
# 新增代码+ClaudeCodeDisplayParity: 函数段结束，claudecode_display_state_from_payload 到此结束；如果没有这个边界说明，读者不容易看出 display state 抽取范围。
...
def run_phase39_coordinates_contract() -> dict[str, Any]:  # 新增代码+Phase39WindowsCoordinates: 运行 DPI、多显示器、动作策略和窗口状态四类合同自检；如果没有这行代码，CLI 输出没有结构化依据。
    ...
    try:  # 新增代码+Phase39WindowsCoordinates: 延迟导入动作策略和控制器避免模块顶层循环；如果没有这行代码，包导入顺序可能出问题。
        from learning_agent.computer_use_mcp_v2.windows_runtime.action_policy import prepare_action_arguments  # 新增代码+Phase39WindowsCoordinates: 包模式导入动作策略；如果没有这行代码，自检无法证明执行链接入。
        from learning_agent.computer_use_mcp_v2.windows_runtime.controller import ComputerUseController, WindowsComputerUseBackend  # 新增代码+Phase39WindowsCoordinates: 包模式导入控制器和 Windows 后端；如果没有这行代码，自检无法证明 observe 状态接入。
        from learning_agent.computer_use_mcp_v2.windows_runtime.windows_backend import StaticWindowsWindowInventory  # 新增代码+Phase39WindowsCoordinates: 包模式导入静态 inventory；如果没有这行代码，自检会依赖真实桌面。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.action_policy", "learning_agent.computer_use_mcp_v2.windows_runtime.controller", "learning_agent.computer_use_mcp_v2.windows_runtime.windows_backend"}:  # 新增代码+Phase39WindowsCoordinates: 只允许目标包路径缺失时 fallback；如果没有这行代码，内部真实 bug 会被误吞。
            raise  # 新增代码+Phase39WindowsCoordinates: 重新抛出非路径类导入错误；如果没有这行代码，排查会被错误 fallback 掩盖。
        from computer_use_mcp_v2.windows_runtime.action_policy import prepare_action_arguments  # 新增代码+Phase39WindowsCoordinates: 脚本模式导入动作策略；如果没有这行代码，bat 入口自检无法运行。
        from computer_use_mcp_v2.windows_runtime.controller import ComputerUseController, WindowsComputerUseBackend  # 新增代码+Phase39WindowsCoordinates: 脚本模式导入控制器和 Windows 后端；如果没有这行代码，bat 入口无法验证状态接入。
        from computer_use_mcp_v2.windows_runtime.windows_backend import StaticWindowsWindowInventory  # 新增代码+Phase39WindowsCoordinates: 脚本模式导入静态 inventory；如果没有这行代码，bat 入口无法构造安全窗口。
