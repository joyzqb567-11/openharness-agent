...
def normalize_executable_name(value: Any) -> str:  # 新增代码+TargetIdentityMaturity：定义可执行名归一化 helper；如果没有这一行，进程名比较会到处重复。
    ...
    for suffix in (".exe", ".lnk", ".appref-ms", ".url"):  # 修改代码+ExternalAppWindowBinding：同时处理可执行文件和 Windows 启动器后缀；如果没有这一行，网易云音乐.lnk 这类启动器会和真实窗口身份误判不同。
        if leaf.endswith(suffix):  # 新增代码+ExternalAppWindowBinding：检查文件名是否以当前后缀结尾；如果没有这一行，后缀剥离会盲目截断正常名称。
            return leaf[: -len(suffix)]  # 新增代码+ExternalAppWindowBinding：返回去掉后缀后的应用名；如果没有这一行，启动器和真实进程无法按同一名字比较。
    return leaf  # 修改代码+ExternalAppWindowBinding：没有已知后缀时返回原文件名；如果没有这一行，普通进程名会隐式返回 None。
