...
try:  # 新增代码+Phase137CalculatorLiveSum: 优先按包路径导入项目内组件；如果没有这段代码，单测和真实终端不能共享同一套实现。
    ...
    from learning_agent.computer_use_mcp_v2.windows_runtime.real_uia_locator import PowerShellUiaTreeProvider, _default_runner, _parse_json_output, _powershell_command  # 修改代码+ProductionAcceptance：复用 PowerShell/.NET UIA provider 与安全脚本运行 helper；如果没有这些导入，Calculator 无法用 UIA Invoke 稳定点击 Plus 按钮。
    ...
    from computer_use_mcp_v2.windows_runtime.real_uia_locator import PowerShellUiaTreeProvider, _default_runner, _parse_json_output, _powershell_command  # type: ignore  # 修改代码+ProductionAcceptance：脚本模式下导入 UIA provider 与脚本运行 helper；如果没有这些导入，bat 入口无法用 UIA Invoke 稳定点击 Calculator 按钮。
...
class Phase137PowerShellCalculatorButtonInvoker:  # 新增代码+ProductionAcceptance：类段开始，使用 PowerShell UIA InvokePattern 点击 Calculator 固定按钮；如果没有这个类，Calculator 会继续依赖不稳定的键盘加号。
    def __init__(self, runner: Any | None = None, timeout_seconds: float = 8.0) -> None:  # 新增代码+ProductionAcceptance：函数段开始，初始化脚本执行器和超时；如果没有这段函数，测试无法注入 fake runner，生产路径也没有超时边界。
        self.runner = runner or _default_runner  # 新增代码+ProductionAcceptance：保存真实或 fake PowerShell runner；如果没有这一行，UIA Invoke 脚本无法执行或测试会触碰真实系统。
        self.timeout_seconds = max(1.0, float(timeout_seconds))  # 新增代码+ProductionAcceptance：限制最小超时为 1 秒；如果没有这一行，坏参数可能让 UIA 调用立即失败或无界等待。
    # 新增代码+ProductionAcceptance：函数段结束，Phase137PowerShellCalculatorButtonInvoker.__init__ 到此结束；如果没有这个边界说明，初学者不容易看出初始化范围。

    def invoke_sequence(self, hwnd: int, button_names: list[str]) -> dict[str, Any]:  # 新增代码+ProductionAcceptance：函数段开始，按固定按钮名批量执行 UIA Invoke；如果没有这段函数，Calculator 无法稳定输入 1+1。
        safe_hwnd = max(0, int(hwnd or 0))  # 新增代码+ProductionAcceptance：清理 hwnd 并防止负数；如果没有这一行，PowerShell 可能收到无效窗口句柄。
        clean_buttons = [str(name or "").strip() for name in list(button_names or []) if str(name or "").strip()]  # 新增代码+ProductionAcceptance：清理按钮名列表；如果没有这一行，空按钮名会让脚本查找无意义控件。
        if sys.platform != "win32":  # 新增代码+ProductionAcceptance：非 Windows 平台直接拒绝；如果没有这一行，Linux/macOS 会尝试运行 Windows UIA。
            return {"ok": False, "backend": "powershell_uia_invoke", "reason": "platform_not_windows", "invoked_count": 0, "raw_text_included": False}  # 新增代码+ProductionAcceptance：返回平台不支持摘要；如果没有这一行，调用方无法温和退回键盘路径。
        if safe_hwnd <= 0 or not clean_buttons:  # 新增代码+ProductionAcceptance：检查窗口句柄和按钮序列是否有效；如果没有这一行，坏输入会进入 PowerShell 并产生难懂错误。
            return {"ok": False, "backend": "powershell_uia_invoke", "reason": "invalid_hwnd_or_buttons", "invoked_count": 0, "raw_text_included": False}  # 新增代码+ProductionAcceptance：返回输入无效摘要；如果没有这一行，调用方不知道为什么没有语义动作。
        script = self._invoke_script(safe_hwnd, clean_buttons)  # 新增代码+ProductionAcceptance：构造受控 PowerShell UIA Invoke 脚本；如果没有这一行，runner 没有可执行内容。
        try:  # 新增代码+ProductionAcceptance：保护 PowerShell 调用；如果没有这一行，脚本异常会直接打断 Calculator 清理流程。
            result = dict(self.runner(_powershell_command(script), self.timeout_seconds))  # 新增代码+ProductionAcceptance：执行编码后的 PowerShell 脚本；如果没有这一行，UIA Invoke 不会真正发生。
        except Exception as error:  # 新增代码+ProductionAcceptance：捕获 runner 异常；如果没有这一行，PowerShell 启动失败会以堆栈形式泄漏。
            return {"ok": False, "backend": "powershell_uia_invoke", "reason": f"uia_invoke_runner_failed:{type(error).__name__}", "invoked_count": 0, "raw_text_included": False}  # 新增代码+ProductionAcceptance：返回 runner 异常摘要；如果没有这一行，调用方无法退回旧键盘路径。
        payload = _parse_json_output(result.get("stdout", ""))  # 新增代码+ProductionAcceptance：解析 PowerShell JSON 输出；如果没有这一行，脚本成功与否无法机器判断。
        ok = bool(result.get("returncode") == 0 and payload.get("ok"))  # 新增代码+ProductionAcceptance：同时检查退出码和 payload ok；如果没有这一行，脚本失败也可能被误报成功。
        return {"ok": ok, "backend": "powershell_uia_invoke", "reason": "" if ok else str(payload.get("reason") or result.get("stderr") or "uia_invoke_failed"), "invoked_count": _phase137_safe_int(payload.get("invoked_count")), "invoked_buttons": list(payload.get("invoked_buttons", []) if isinstance(payload.get("invoked_buttons", []), list) else []), "raw_text_included": False}  # 新增代码+ProductionAcceptance：返回脱敏 UIA Invoke 摘要；如果没有这一行，driver 无法记录语义动作证据。
    # 新增代码+ProductionAcceptance：函数段结束，Phase137PowerShellCalculatorButtonInvoker.invoke_sequence 到此结束；如果没有这个边界说明，初学者不容易看出执行范围。

    def _invoke_script(self, hwnd: int, button_names: list[str]) -> str:  # 新增代码+ProductionAcceptance：函数段开始，生成只点击固定按钮名的 PowerShell 脚本；如果没有这段函数，UIA Invoke 命令会散落在 Python 字符串里。
        buttons_json = json.dumps(list(button_names), ensure_ascii=False)  # 新增代码+ProductionAcceptance：把按钮名编码成 JSON；如果没有这一行，按钮名中的引号会破坏 PowerShell 脚本。
...
try {{
  ...
  if ($null -eq $root) {{ throw "calculator_window_not_found" }}
  $buttonNames = ConvertFrom-Json @'
{buttons_json}
'@
  $invoked = @()
  foreach ($name in @($buttonNames)) {{
    $condition = [System.Windows.Automation.PropertyCondition]::new([System.Windows.Automation.AutomationElement]::NameProperty, [string]$name)
    $element = $root.FindFirst([System.Windows.Automation.TreeScope]::Descendants, $condition)
    if ($null -eq $element) {{ throw ("button_not_found:" + [string]$name) }}
    $patternObject = $element.GetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern)
    if ($null -eq $patternObject) {{ throw ("invoke_pattern_missing:" + [string]$name) }}
    ([System.Windows.Automation.InvokePattern]$patternObject).Invoke()
    $invoked += [string]$name
    Start-Sleep -Milliseconds 120
  }}
  $payload = @{{ ok = $true; backend = "powershell_uia_invoke"; invoked_buttons = $invoked; invoked_count = $invoked.Count }}
}} catch {{
  $payload = @{{ ok = $false; backend = "powershell_uia_invoke"; reason = $_.Exception.Message; invoked_buttons = @(); invoked_count = 0 }}
}}
$payload | ConvertTo-Json -Compress -Depth 8
"""  # 新增代码+ProductionAcceptance：返回完整 PowerShell UIA Invoke 脚本；如果没有这一行，Calculator 语义按钮路径没有可执行脚本。
    # 新增代码+ProductionAcceptance：函数段结束，Phase137PowerShellCalculatorButtonInvoker._invoke_script 到此结束；如果没有这个边界说明，初学者不容易看出脚本生成范围。
# 新增代码+ProductionAcceptance：类段结束，Phase137PowerShellCalculatorButtonInvoker 到此结束；如果没有这个边界说明，用户不容易看出 UIA 按钮 invoker 的全部范围。
...
class Phase137WindowsCalculatorLiveSumDriver:  # 新增代码+Phase137CalculatorLiveSum: 类段开始，执行受控真实 Calculator 1+1 验收；如果没有这个类，CLI 双门打开后没有真实 driver。
    def __init__(self, inventory: Any | None = None, sender: Any | None = None, observation_runtime: Any | None = None, uia_provider: Any | None = None, button_invoker: Any | None = None, launch_command: list[str] | None = None, timeout_seconds: float = 10.0) -> None:  # 修改代码+ProductionAcceptance：函数段开始，注入或创建真实依赖并支持 UIA 按钮 invoker；如果没有 button_invoker，Calculator 会继续依赖不稳定键盘加号。
        ...
        self.button_invoker = button_invoker if button_invoker is not None else Phase137PowerShellCalculatorButtonInvoker()  # 新增代码+ProductionAcceptance：保存 UIA 按钮 invoker；如果没有这一行，driver 无法稳定点击 Plus/Equals 按钮。
    ...
    def _dispatch_expression(self, window: dict[str, Any]) -> dict[str, Any]:  # 新增代码+Phase137CalculatorLiveSum: 函数段开始，发送受控 1+1 Enter 键序列；如果没有这段函数，Calculator 不会真实计算。
        ...
        semantic_result = dict(self.button_invoker.invoke_sequence(hwnd, ["One", "Plus", "One", "Equals"])) if hasattr(self.button_invoker, "invoke_sequence") else {"ok": False, "reason": "button_invoker_missing"}  # 新增代码+ProductionAcceptance：优先用 UIA Invoke 点击 Calculator 按钮；如果没有这一行，当前 Windows Calculator 会把键盘加号吞成等号。
        if semantic_result.get("ok"):  # 新增代码+ProductionAcceptance：检查 UIA 语义按钮路径是否成功；如果没有这一行，成功后还会继续走不稳定键盘 fallback。
            return {"ok": True, "dispatch_path": "powershell_uia_invoke", "semantic_action_dispatch": True, "semantic_action_count": _phase137_safe_int(semantic_result.get("invoked_count")), "low_level_event_count": 0, "sender": "powershell_uia_invoke", "raw_text_included": False, "uia_invoke_sequence_used": True, "semantic_result": semantic_result}  # 新增代码+ProductionAcceptance：返回语义动作成功摘要；如果没有这一行，上层报告无法证明 Calculator 走的是 UIA 按钮路径。
        ...
        fallback_result = self.sender.send_low_level(events) if hasattr(self.sender, "send_low_level") else {"ok": False, "low_level_event_count": 0, "sender": type(self.sender).__name__, "raw_text_included": False, "reason": "sender_missing_send_low_level"}  # 修改代码+ProductionAcceptance：UIA 不可用时才退回旧键盘路径；如果没有这一行，无法保留低层 fallback 兼容性。
        fallback_payload = dict(fallback_result)  # 新增代码+ProductionAcceptance：复制 fallback 结果避免污染 sender 原始返回；如果没有这一行，后续补字段可能改坏外部对象。
        fallback_payload["dispatch_path"] = "windows_sendinput_low_level"  # 新增代码+ProductionAcceptance：标记 fallback 路径；如果没有这一行，报告无法解释为何没有走 UIA。
        fallback_payload["semantic_action_dispatch"] = False  # 新增代码+ProductionAcceptance：标记没有语义动作；如果没有这一行，矩阵可能误把键盘 fallback 当 UIA 成功。
        fallback_payload["semantic_action_count"] = 0  # 新增代码+ProductionAcceptance：语义动作数量归零；如果没有这一行，报告字段不稳定。
        fallback_payload["uia_invoke_sequence_used"] = False  # 新增代码+ProductionAcceptance：明确 UIA Invoke 未使用；如果没有这一行，失败排查时看不到路径选择。
        fallback_payload["semantic_fallback_reason"] = str(semantic_result.get("reason", ""))  # 新增代码+ProductionAcceptance：记录 UIA fallback 原因；如果没有这一行，键盘路径失败时无法知道前置 UIA 为什么不可用。
        return fallback_payload  # 新增代码+ProductionAcceptance：返回带路径证据的 fallback 结果；如果没有这一行，函数没有返回值。
    ...
    def run(self, *, run_root: Path, expression: str, expected_result: str) -> dict[str, Any]:  # 新增代码+Phase137CalculatorLiveSum: 函数段开始，执行完整真实 Calculator 闭环；如果没有这段函数，合同无法触达真实代表应用。
        ...
        try:  # 新增代码+Phase137CalculatorLiveSum: 保护启动和动作全过程；如果没有这行代码，异常会绕过结构化失败报告。
            ...
            semantic_action_count = _phase137_safe_int(dispatch_result.get("semantic_action_count"))  # 新增代码+ProductionAcceptance：读取 UIA 语义按钮动作数量；如果没有这一行，UIA Invoke 成功会被误当没有触桌面。
            ...
            semantic_dispatch = bool(dispatch_result.get("semantic_action_dispatch") and semantic_action_count > 0 and dispatch_result.get("ok"))  # 新增代码+ProductionAcceptance：判断 UIA 语义动作是否真实执行；如果没有这一行，Calculator UIA 按钮路径无法算作生产动作。
            action_dispatch = bool(physical_dispatch or semantic_dispatch)  # 新增代码+ProductionAcceptance：统一物理 SendInput 与 UIA Invoke 两种真实动作背书；如果没有这一行，UIA 成功仍会让 driver 失败。
            return {"ok": bool(calculator_verified and input_window and result_window and expression == "1+1" and result_seen and action_dispatch), "driver": "phase137_windows_calculator_live_sum_driver", "reason": "" if result_seen else "calculator_result_not_observed", "calculator_process_verified": bool(calculator_verified), "target_rechecked_before_input": bool(input_window), "target_rechecked_before_result": bool(result_window), "expression_verified": bool(expression == "1+1"), "observed_result_matches_expected": bool(result_seen), "result_observation": result_summary, "target_window": _phase137_window_identity(result_window or input_window or window), "before_observation": before_observation, "after_observation": after_observation, "cleanup_evidence": cleanup_evidence, "real_desktop_touched": bool(low_level_event_count or semantic_action_count), "physical_desktop_dispatch_performed": physical_dispatch, "real_sendinput_dispatch": physical_dispatch, "semantic_action_dispatch": semantic_dispatch, "semantic_action_count": semantic_action_count, "uia_invoke_sequence_used": bool(dispatch_result.get("uia_invoke_sequence_used")), "dispatch_path": str(dispatch_result.get("dispatch_path", "")), "sender_kind": str(dispatch_result.get("sender") or sender_kind), "low_level_event_count": low_level_event_count, "raw_text_included": False}  # 修改代码+ProductionAcceptance：返回脱敏闭环报告并纳入 UIA Invoke 证据；如果没有这些字段，合同层无法证明 Calculator 真实语义按钮路径。
...
def run_phase137_controlled_calculator_live_sum_contract(base_dir: str | Path | None = None, real_run: bool | None = None, allow_real_gate: bool | None = None, calculator_driver: Any | None = None, require_injected_driver: bool = False, raw_prompt_text: str | None = None) -> dict[str, Any]:  # 新增代码+Phase137CalculatorLiveSum: 函数段开始，运行 Phase137 总合同入口；如果没有这段函数，测试和 CLI 没有统一事实源。
    ...
    report_without_raw_check.update({"target_window": _phase137_safe_dict(driver_report.get("target_window")), "before_observation": _phase137_safe_dict(driver_report.get("before_observation")), "after_observation": _phase137_safe_dict(driver_report.get("after_observation")), "cleanup_evidence": _phase137_safe_dict(driver_report.get("cleanup_evidence")), "physical_desktop_dispatch_performed": bool(driver_report.get("physical_desktop_dispatch_performed")), "real_sendinput_dispatch": bool(driver_report.get("real_sendinput_dispatch")), "semantic_action_dispatch": bool(driver_report.get("semantic_action_dispatch")), "semantic_action_count": _phase137_safe_int(driver_report.get("semantic_action_count")), "uia_invoke_sequence_used": bool(driver_report.get("uia_invoke_sequence_used")), "dispatch_path": str(driver_report.get("dispatch_path") or ""), "sender_kind": str(driver_report.get("sender_kind") or ""), "low_level_event_count": _phase137_safe_int(driver_report.get("low_level_event_count"))})  # 修改代码+ProductionAcceptance：合并 Phase8 与 UIA Invoke 关键证据字段；如果没有这些字段，builder 无法恢复真实语义动作闭环。
...
__all__ = ["DEFAULT_PHASE137_CONTROLLED_CALCULATOR_LIVE_SUM_ROOT", "PHASE137_CONTROLLED_CALCULATOR_LIVE_SUM_MARKER", "PHASE137_CONTROLLED_CALCULATOR_LIVE_SUM_MODEL", "PHASE137_CONTROLLED_CALCULATOR_LIVE_SUM_OK_TOKEN", "PHASE137_REAL_CALCULATOR_SUM_ENV", "PHASE137_REAL_CALCULATOR_SUM_REQUEST_ENV", "PHASE137_UNCONTROLLED_ACTIONS_EXPANDED", "Phase137PowerShellCalculatorButtonInvoker", "Phase137WindowsCalculatorLiveSumDriver", "build_phase137_calculator_real_desktop_closure_evidence", "main", "phase137_cli_line", "run_phase137_controlled_calculator_live_sum_contract"]  # 修改代码+ProductionAcceptance：公开 Calculator UIA 按钮 invoker 便于测试和审计；如果没有这一项，外部无法复用或验证语义按钮路径。
