...
try:  # 新增代码+Phase58RealSendInputGuard: 优先按包模式导入 Phase57/28 能力；如果没有这段代码，单测和生产入口无法复用已有安全窗口与 inventory。
    from learning_agent.computer_use_mcp_v2.windows_runtime.real_uia_locator import Phase57DedicatedSafeWindowLauncher, WindowsRealUiaLocatorRuntime, _phase57_find_safe_edit_control, _poll_phase57_safe_window  # 新增代码+Phase58RealSendInputGuard: 复用 Phase57 自建安全窗口、真实 UIA 和安全窗口轮询；如果没有这行代码，Phase58 会重复造窗口和定位逻辑。
    from learning_agent.computer_use_mcp_v2.windows_runtime.windows_backend import WindowsWindowInventoryProbe  # 新增代码+Phase58RealSendInputGuard: 复用真实 Win32 只读窗口枚举；如果没有这行代码，真实 smoke 找不到自建窗口。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.real_uia_locator", "learning_agent.computer_use_mcp_v2.windows_runtime.windows_backend"}:  # 新增代码+Phase58RealSendInputGuard: 只允许包路径缺失时 fallback；如果没有这行代码，真实内部 bug 会被误吞。
        raise  # 新增代码+Phase58RealSendInputGuard: 重新抛出非路径类导入错误；如果没有这行代码，排查 Phase57 或 inventory 内部错误会困难。
    from computer_use_mcp_v2.windows_runtime.real_uia_locator import Phase57DedicatedSafeWindowLauncher, WindowsRealUiaLocatorRuntime, _phase57_find_safe_edit_control, _poll_phase57_safe_window  # 新增代码+Phase58RealSendInputGuard: 脚本模式复用安全窗口和 UIA runtime；如果没有这行代码，bat 入口无法运行真实 smoke。
    from computer_use_mcp_v2.windows_runtime.windows_backend import WindowsWindowInventoryProbe  # 新增代码+Phase58RealSendInputGuard: 脚本模式复用窗口枚举；如果没有这行代码，bat 入口无法定位自建窗口。
...
def _phase58_virtual_key_code(key: Any) -> int:  # 新增代码+Phase137CalculatorLiveSum: 函数段开始，把受控按键名翻译成 Windows 虚拟键码；如果没有这段函数，Calculator 的真实键盘闭环会卡在底层 sender。
    normalized = str(key or "").strip().lower()  # 新增代码+Phase137CalculatorLiveSum: 统一清洗按键名并转小写；如果没有这一行，ADD、add、空格等输入会造成白名单判断不稳定。
    if len(normalized) == 1 and normalized.isdigit():  # 新增代码+Phase137CalculatorLiveSum: 只允许单个数字走数字键路径；如果没有这一行，Calculator 无法输入 1+1 里的数字。
        return 0x30 + int(normalized)  # 新增代码+Phase137CalculatorLiveSum: 返回 Windows VK_0 到 VK_9；如果没有这一行，数字键会被当成未知按键拒绝发送。
    key_map = {"ctrl": 0xA2, "control": 0xA2, "shift": 0xA0, "a": 0x41, "c": 0x43, "s": 0x53, "v": 0x56, "n": 0x4E, "enter": 0x0D, "return": 0x0D, "add": 0x6B, "plus": 0x6B, "+": 0x6B}  # 修改代码+Phase150MultiAppTransfer: 在既有 Ctrl/S/V/N 等白名单上只补跨应用复制必需的 A 与 C；如果没有这一行，Ctrl+A/C 无法从受控 Notepad 复制文本，但若开放任意键又会扩大真实桌面风险。
    return int(key_map.get(normalized, 0))  # 新增代码+Phase137CalculatorLiveSum: 未命中白名单时返回 0 表示拒绝；如果没有这一行，未知按键可能被误发到真实桌面。
# 新增代码+Phase137CalculatorLiveSum: 函数段结束，_phase58_virtual_key_code 到此结束；如果没有这个边界说明，初学者不容易看出按键白名单范围。
...
class WindowsSendInputLowLevelSender:  # 新增代码+Phase58RealSendInputGuard: 类段开始，定义真实 Windows 低层发送器；如果没有这个类，Phase58 只能停留在 fake sender 合同。
    ...
    def _set_cursor(self, x: int, y: int, use_set_cursor: bool = True) -> bool:  # 修改代码+PaintDragMoveGuard：函数段开始，把鼠标移动作为真实输入事件送入系统队列，并允许拖拽中禁用预先瞬移；如果没有这段函数，Paint 拖拽可能只移动光标但不画线。
        import ctypes  # 修改代码+PaintPikachuRealLoop：延迟导入 ctypes；如果没有这行代码，无法调用 user32.dll 的鼠标输入 API。
        user32 = ctypes.windll.user32  # 新增代码+PaintPikachuRealLoop：缓存 user32.dll 入口；如果没有这一行，后续每次调用都要重复访问动态库。
        screen_width = max(1, int(user32.GetSystemMetrics(0)))  # 新增代码+PaintPikachuRealLoop：读取主屏幕宽度并兜底至少 1；如果没有这一行，绝对鼠标坐标换算可能除零。
        screen_height = max(1, int(user32.GetSystemMetrics(1)))  # 新增代码+PaintPikachuRealLoop：读取主屏幕高度并兜底至少 1；如果没有这一行，绝对鼠标坐标换算可能除零。
        absolute_x = max(0, min(65535, int(int(x) * 65535 / max(1, screen_width - 1))))  # 新增代码+PaintPikachuRealLoop：把屏幕 x 坐标换算为 mouse_event 绝对坐标；如果没有这一行，Paint 收不到真实移动位置。
        absolute_y = max(0, min(65535, int(int(y) * 65535 / max(1, screen_height - 1))))  # 新增代码+PaintPikachuRealLoop：把屏幕 y 坐标换算为 mouse_event 绝对坐标；如果没有这一行，Paint 收不到真实移动位置。
        moved = bool(user32.SetCursorPos(int(x), int(y))) if use_set_cursor else True  # 修改代码+PaintDragMoveGuard：只有非拖拽定位才先放光标；如果没有这一行，按下期间的真实移动会被 SetCursorPos 提前吃掉。
        extra_type = ctypes.c_ulonglong if ctypes.sizeof(ctypes.c_void_p) == 8 else ctypes.c_ulong  # 新增代码+PaintDragMoveGuard：选择 ULONG_PTR 宽度；如果没有这一行，64 位 Windows 的 INPUT 结构大小可能错误。
        class MOUSEINPUT(ctypes.Structure):  # 新增代码+PaintDragMoveGuard：定义 SendInput 鼠标移动结构；如果没有这个类，无法把绝对坐标交给 SendInput。
            _fields_ = [("dx", ctypes.c_long), ("dy", ctypes.c_long), ("mouseData", ctypes.c_ulong), ("dwFlags", ctypes.c_ulong), ("time", ctypes.c_ulong), ("dwExtraInfo", extra_type)]  # 新增代码+PaintDragMoveGuard：定义鼠标字段；如果没有这一行，结构和 Win32 API 不匹配。
        class INPUTUNION(ctypes.Union):  # 新增代码+PaintDragMoveGuard：定义 INPUT union；如果没有这个类，SendInput 结构缺少 mi 分支。
            _fields_ = [("mi", MOUSEINPUT)]  # 新增代码+PaintDragMoveGuard：只声明鼠标分支；如果没有这一行，移动事件无法放入 INPUT。
        class INPUT(ctypes.Structure):  # 新增代码+PaintDragMoveGuard：定义 INPUT 结构；如果没有这个类，SendInput 参数无法构造。
            _fields_ = [("type", ctypes.c_ulong), ("union", INPUTUNION)]  # 新增代码+PaintDragMoveGuard：type=0 表示鼠标输入；如果没有这一行，系统无法识别事件类型。
        input_item = INPUT(0, INPUTUNION(mi=MOUSEINPUT(absolute_x, absolute_y, 0, 0x0001 | 0x8000, 0, extra_type(0))))  # 新增代码+PaintDragMoveGuard：构造绝对鼠标移动 INPUT；如果没有这一行，鼠标移动仍会依赖旧 mouse_event。
        sent = user32.SendInput(1, ctypes.byref(input_item), ctypes.sizeof(INPUT))  # 新增代码+PaintDragMoveGuard：用 SendInput 发送鼠标移动；如果没有这一行，按钮和移动 API 混用会让 Paint 拖拽不稳定。
        return bool(moved and sent == 1)  # 修改代码+PaintDragMoveGuard：只有定位允许且移动事件发送成功才返回成功；如果没有这一行，调用方无法发现 SendInput move 失败。
    # 修改代码+PaintDragMoveGuard：函数段结束，WindowsSendInputLowLevelSender._set_cursor 到此结束；如果没有这个边界说明，初学者不容易看出真实移动事件范围。
    ...
    def _send_virtual_key(self, key: str, down: bool) -> bool:  # 修改代码+Phase97ControlledNotepadLiveEdit: 函数段开始，用 SendInput 发送普通虚拟键；如果没有这段函数，真实后端无法表达 Ctrl+S 保存快捷键。
        virtual_key = _phase58_virtual_key_code(key)  # 修改代码+Phase137CalculatorLiveSum: 复用统一白名单把按键名转换成 Windows 虚拟键码；如果没有这一行，Calculator 新增按键和 Notepad 旧按键会分裂成两套规则。
        if virtual_key <= 0:  # 修改代码+Phase97ControlledNotepadLiveEdit: 拒绝未列入白名单的按键；如果没有这一行，拼写错误或未知键可能产生不可审计动作。
            return False  # 修改代码+Phase97ControlledNotepadLiveEdit: 未知按键返回失败且不发送；如果没有这一行，调用方无法知道事件被安全拒绝。
        import ctypes  # 修改代码+Phase97ControlledNotepadLiveEdit: 延迟导入 ctypes 构造 KEYBDINPUT；如果没有这一行，无法调用 Win32 SendInput。
        extra_type = ctypes.c_ulonglong if ctypes.sizeof(ctypes.c_void_p) == 8 else ctypes.c_ulong  # 修改代码+Phase97ControlledNotepadLiveEdit: 选择 ULONG_PTR 宽度；如果没有这一行，64 位 Windows 的 INPUT 结构可能不匹配。
        class MOUSEINPUT(ctypes.Structure):  # 修改代码+Phase97ControlledNotepadLiveEdit: 声明鼠标分支撑满 union 尺寸；如果没有这个类，键盘 INPUT 在部分系统上可能因结构过小失败。
            _fields_ = [("dx", ctypes.c_long), ("dy", ctypes.c_long), ("mouseData", ctypes.c_ulong), ("dwFlags", ctypes.c_ulong), ("time", ctypes.c_ulong), ("dwExtraInfo", extra_type)]  # 修改代码+Phase97ControlledNotepadLiveEdit: 定义鼠标字段作为 union 最大分支；如果没有这一行，SendInput 的 cbSize 可能错误。
        class KEYBDINPUT(ctypes.Structure):  # 修改代码+Phase97ControlledNotepadLiveEdit: 定义键盘分支；如果没有这个类，普通按键事件无法装入 INPUT。
            _fields_ = [("wVk", ctypes.c_ushort), ("wScan", ctypes.c_ushort), ("dwFlags", ctypes.c_ulong), ("time", ctypes.c_ulong), ("dwExtraInfo", extra_type)]  # 修改代码+Phase97ControlledNotepadLiveEdit: 定义虚拟键字段；如果没有这一行，Win32 不知道按键码和按下/抬起状态。
        class INPUTUNION(ctypes.Union):  # 修改代码+Phase97ControlledNotepadLiveEdit: 定义 SendInput union；如果没有这个类，键盘结构无法传给系统。
            _fields_ = [("mi", MOUSEINPUT), ("ki", KEYBDINPUT)]  # 修改代码+Phase97ControlledNotepadLiveEdit: 同时放入鼠标和键盘分支保持结构大小正确；如果没有这一行，真实发送可能返回 0。
        class INPUT(ctypes.Structure):  # 修改代码+Phase97ControlledNotepadLiveEdit: 定义 SendInput 顶层结构；如果没有这个类，系统 API 参数无法构造。
            _fields_ = [("type", ctypes.c_ulong), ("union", INPUTUNION)]  # 修改代码+Phase97ControlledNotepadLiveEdit: type=1 表示键盘输入；如果没有这一行，系统无法识别这是键盘事件。
        flags = 0 if down else 0x0002  # 修改代码+Phase97ControlledNotepadLiveEdit: 抬起事件设置 KEYEVENTF_KEYUP；如果没有这一行，按键可能一直处于按下状态。
        input_item = INPUT(1, INPUTUNION(ki=KEYBDINPUT(virtual_key, 0, flags, 0, extra_type(0))))  # 修改代码+Phase97ControlledNotepadLiveEdit: 构造单个普通按键 INPUT；如果没有这一行，SendInput 没有事件可发。
        sent = ctypes.windll.user32.SendInput(1, ctypes.byref(input_item), ctypes.sizeof(INPUT))  # 修改代码+Phase97ControlledNotepadLiveEdit: 调用系统发送按键；如果没有这一行，Ctrl+S 不会真实触发保存。
        time.sleep(0.03)  # 修改代码+Phase97ControlledNotepadLiveEdit: 给 Notepad 一点时间处理修饰键状态；如果没有这一行，Ctrl+S 在真实 Notepad 中可能退化成普通 s。
        return sent == 1  # 修改代码+Phase97ControlledNotepadLiveEdit: 返回是否成功发送一个按键事件；如果没有这一行，调用方无法审计保存快捷键是否发出。
    # 修改代码+Phase97ControlledNotepadLiveEdit: 函数段结束，WindowsSendInputLowLevelSender._send_virtual_key 到此结束；如果没有这个边界说明，初学者不容易看出普通虚拟键范围。

    def _clipboard_text(self) -> str | None:  # 修改代码+Phase97ControlledNotepadLiveEdit: 函数段开始，读取当前 Unicode 剪贴板文本；如果没有这段函数，粘贴后无法尽量恢复用户剪贴板。
        import ctypes  # 修改代码+Phase97ControlledNotepadLiveEdit: 延迟导入 ctypes 访问剪贴板 API；如果没有这一行，无法调用 Win32 剪贴板。
        user32 = ctypes.windll.user32  # 修改代码+Phase97ControlledNotepadLiveEdit: 获取 user32；如果没有这一行，剪贴板函数没有入口。
        kernel32 = ctypes.windll.kernel32  # 修改代码+Phase97ControlledNotepadLiveEdit: 获取 kernel32；如果没有这一行，无法锁定剪贴板内存。
        user32.GetClipboardData.restype = ctypes.c_void_p  # 修改代码+Phase97ControlledNotepadLiveEdit: 把剪贴板句柄声明为指针宽度；如果没有这一行，64 位句柄会被截断导致读取崩溃。
        kernel32.GlobalLock.argtypes = [ctypes.c_void_p]  # 修改代码+Phase97ControlledNotepadLiveEdit: 声明 GlobalLock 入参为指针；如果没有这一行，锁定高位句柄可能失败。
        kernel32.GlobalLock.restype = ctypes.c_void_p  # 修改代码+Phase97ControlledNotepadLiveEdit: 声明 GlobalLock 返回指针；如果没有这一行，wstring_at 会读到截断地址。
        kernel32.GlobalUnlock.argtypes = [ctypes.c_void_p]  # 修改代码+Phase97ControlledNotepadLiveEdit: 声明 GlobalUnlock 入参为指针；如果没有这一行，释放锁可能用错地址。
        if not user32.OpenClipboard(None):  # 修改代码+Phase97ControlledNotepadLiveEdit: 打开剪贴板；如果没有这一行，直接读可能失败或和其它进程冲突。
            return None  # 修改代码+Phase97ControlledNotepadLiveEdit: 打不开时返回 None；如果没有这一行，调用方无法区分不可恢复状态。
        try:  # 修改代码+Phase97ControlledNotepadLiveEdit: 确保读取后关闭剪贴板；如果没有这一行，剪贴板可能被本进程占住。
            handle = user32.GetClipboardData(13)  # 修改代码+Phase97ControlledNotepadLiveEdit: 读取 CF_UNICODETEXT 数据句柄；如果没有这一行，无法取得现有文本。
            if not handle:  # 修改代码+Phase97ControlledNotepadLiveEdit: 没有 Unicode 文本时返回空标记；如果没有这一行，空句柄会被错误锁定。
                return None  # 修改代码+Phase97ControlledNotepadLiveEdit: 无文本可恢复；如果没有这一行，恢复逻辑会误写空字符串。
            pointer = kernel32.GlobalLock(handle)  # 修改代码+Phase97ControlledNotepadLiveEdit: 锁定全局内存；如果没有这一行，无法安全读取文本指针。
            if not pointer:  # 修改代码+Phase97ControlledNotepadLiveEdit: 锁定失败时返回 None；如果没有这一行，wstring_at 可能读坏地址。
                return None  # 修改代码+Phase97ControlledNotepadLiveEdit: 返回不可恢复；如果没有这一行，调用方无法兜底。
            try:  # 修改代码+Phase97ControlledNotepadLiveEdit: 确保锁定后释放；如果没有这一行，剪贴板内存可能保持锁定。
                return ctypes.wstring_at(pointer)  # 修改代码+Phase97ControlledNotepadLiveEdit: 读取 Unicode 文本；如果没有这一行，无法备份用户剪贴板文本。
            finally:  # 修改代码+Phase97ControlledNotepadLiveEdit: 释放全局内存锁；如果没有这一行，剪贴板资源可能泄漏。
                kernel32.GlobalUnlock(handle)  # 修改代码+Phase97ControlledNotepadLiveEdit: 解锁剪贴板数据；如果没有这一行，其它进程可能受影响。
        finally:  # 修改代码+Phase97ControlledNotepadLiveEdit: 关闭剪贴板；如果没有这一行，剪贴板会被占用。
            user32.CloseClipboard()  # 修改代码+Phase97ControlledNotepadLiveEdit: 释放剪贴板访问权；如果没有这一行，用户或系统无法正常用剪贴板。
    # 修改代码+Phase97ControlledNotepadLiveEdit: 函数段结束，WindowsSendInputLowLevelSender._clipboard_text 到此结束；如果没有这个边界说明，初学者不容易看出剪贴板读取范围。

    def _set_clipboard_text(self, text: str) -> bool:  # 修改代码+Phase97ControlledNotepadLiveEdit: 函数段开始，设置 Unicode 剪贴板文本；如果没有这段函数，无法用粘贴方式可靠输入 Notepad。
        import ctypes  # 修改代码+Phase97ControlledNotepadLiveEdit: 延迟导入 ctypes 访问 Win32 API；如果没有这一行，无法分配剪贴板内存。
        user32 = ctypes.windll.user32  # 修改代码+Phase97ControlledNotepadLiveEdit: 获取 user32；如果没有这一行，剪贴板 API 没有入口。
        kernel32 = ctypes.windll.kernel32  # 修改代码+Phase97ControlledNotepadLiveEdit: 获取 kernel32；如果没有这一行，无法分配全局内存。
        kernel32.GlobalAlloc.argtypes = [ctypes.c_uint, ctypes.c_size_t]  # 修改代码+Phase97ControlledNotepadLiveEdit: 声明 GlobalAlloc 参数宽度；如果没有这一行，分配大小或 flag 可能传错。
        kernel32.GlobalAlloc.restype = ctypes.c_void_p  # 修改代码+Phase97ControlledNotepadLiveEdit: 声明 GlobalAlloc 返回指针；如果没有这一行，64 位内存句柄会被截断。
        kernel32.GlobalLock.argtypes = [ctypes.c_void_p]  # 修改代码+Phase97ControlledNotepadLiveEdit: 声明 GlobalLock 入参为指针；如果没有这一行，写入文本时可能锁错地址。
        kernel32.GlobalLock.restype = ctypes.c_void_p  # 修改代码+Phase97ControlledNotepadLiveEdit: 声明 GlobalLock 返回指针；如果没有这一行，memmove 可能写坏地址。
        kernel32.GlobalUnlock.argtypes = [ctypes.c_void_p]  # 修改代码+Phase97ControlledNotepadLiveEdit: 声明 GlobalUnlock 入参为指针；如果没有这一行，解锁可能失败。
        user32.SetClipboardData.argtypes = [ctypes.c_uint, ctypes.c_void_p]  # 修改代码+Phase97ControlledNotepadLiveEdit: 声明 SetClipboardData 入参；如果没有这一行，剪贴板接管内存时句柄可能被截断。
        user32.SetClipboardData.restype = ctypes.c_void_p  # 修改代码+Phase97ControlledNotepadLiveEdit: 声明 SetClipboardData 返回句柄；如果没有这一行，成功判断可能不稳定。
        payload = str(text) + "\0"  # 修改代码+Phase97ControlledNotepadLiveEdit: 给 Unicode 文本补结尾空字符；如果没有这一行，剪贴板文本可能没有正确终止。
        byte_count = len(payload.encode("utf-16-le"))  # 修改代码+Phase97ControlledNotepadLiveEdit: 计算 UTF-16LE 字节数；如果没有这一行，全局内存大小可能不够。
        if not user32.OpenClipboard(None):  # 修改代码+Phase97ControlledNotepadLiveEdit: 打开剪贴板；如果没有这一行，无法写入文本。
            return False  # 修改代码+Phase97ControlledNotepadLiveEdit: 打不开时返回失败；如果没有这一行，调用方会误以为粘贴已准备好。
        try:  # 修改代码+Phase97ControlledNotepadLiveEdit: 确保写入后关闭剪贴板；如果没有这一行，异常会占住剪贴板。
            user32.EmptyClipboard()  # 修改代码+Phase97ControlledNotepadLiveEdit: 清空剪贴板准备写入；如果没有这一行，旧数据可能和新文本冲突。
            handle = kernel32.GlobalAlloc(0x0042, byte_count)  # 修改代码+Phase97ControlledNotepadLiveEdit: 分配可移动且清零的全局内存；如果没有这一行，SetClipboardData 没有数据块。
            if not handle:  # 修改代码+Phase97ControlledNotepadLiveEdit: 检查内存分配；如果没有这一行，后续锁定空句柄会失败。
                return False  # 修改代码+Phase97ControlledNotepadLiveEdit: 分配失败返回失败；如果没有这一行，调用方无法知道未写入。
            pointer = kernel32.GlobalLock(handle)  # 修改代码+Phase97ControlledNotepadLiveEdit: 锁定内存以写入文本；如果没有这一行，无法得到可写指针。
            if not pointer:  # 修改代码+Phase97ControlledNotepadLiveEdit: 检查锁定结果；如果没有这一行，memmove 可能写坏地址。
                return False  # 修改代码+Phase97ControlledNotepadLiveEdit: 锁定失败返回失败；如果没有这一行，调用方无法知道粘贴不可用。
            ctypes.memmove(pointer, payload.encode("utf-16-le"), byte_count)  # 修改代码+Phase97ControlledNotepadLiveEdit: 写入 UTF-16LE 文本；如果没有这一行，剪贴板内容不会变成目标文本。
            kernel32.GlobalUnlock(handle)  # 修改代码+Phase97ControlledNotepadLiveEdit: 解锁全局内存；如果没有这一行，SetClipboardData 可能不能接管。
            return bool(user32.SetClipboardData(13, handle))  # 修改代码+Phase97ControlledNotepadLiveEdit: 设置 CF_UNICODETEXT 并交给系统接管；如果没有这一行，Ctrl+V 无文本可粘贴。
        finally:  # 修改代码+Phase97ControlledNotepadLiveEdit: 关闭剪贴板；如果没有这一行，剪贴板会被占住。
            user32.CloseClipboard()  # 修改代码+Phase97ControlledNotepadLiveEdit: 释放剪贴板访问权；如果没有这一行，用户剪贴板会卡住。
    # 修改代码+Phase97ControlledNotepadLiveEdit: 函数段结束，WindowsSendInputLowLevelSender._set_clipboard_text 到此结束；如果没有这个边界说明，初学者不容易看出剪贴板写入范围。

    def _paste_clipboard_text(self, text: str) -> int:  # 修改代码+Phase97ControlledNotepadLiveEdit: 函数段开始，临时剪贴板粘贴受控文本并恢复；如果没有这段函数，Phase97 无法可靠输入完整 Notepad 文本。
        previous_text = self._clipboard_text()  # 修改代码+Phase97ControlledNotepadLiveEdit: 备份当前剪贴板文本；如果没有这一行，粘贴验收会永久覆盖用户文本剪贴板。
        if not self._set_clipboard_text(text):  # 修改代码+Phase97ControlledNotepadLiveEdit: 写入受控文本到剪贴板；如果没有这一行，Ctrl+V 没有可粘贴内容。
            return 0  # 修改代码+Phase97ControlledNotepadLiveEdit: 剪贴板写入失败时零事件返回；如果没有这一行，后续 Ctrl+V 可能粘贴旧内容。
        try:  # 修改代码+Phase97ControlledNotepadLiveEdit: 粘贴后尽量恢复剪贴板；如果没有这一行，异常会跳过恢复。
            ok = self._send_virtual_key("ctrl", True) and self._send_virtual_key("v", True) and self._send_virtual_key("v", False) and self._send_virtual_key("ctrl", False)  # 修改代码+Phase97ControlledNotepadLiveEdit: 发送 Ctrl+V；如果没有这一行，剪贴板文本不会进入 Notepad。
            time.sleep(0.2)  # 修改代码+Phase97ControlledNotepadLiveEdit: 等待 Notepad 完成粘贴；如果没有这一行，马上保存可能早于文本进入编辑区。
            return 1 if ok else 0  # 修改代码+Phase97ControlledNotepadLiveEdit: 返回粘贴动作是否成功；如果没有这一行，调用方无法审计粘贴阶段。
        finally:  # 修改代码+Phase97ControlledNotepadLiveEdit: 尽力恢复剪贴板；如果没有这一行，用户剪贴板会被测试文本长期覆盖。
            if previous_text is not None:  # 修改代码+Phase97ControlledNotepadLiveEdit: 只有原来是文本剪贴板时恢复文本；如果没有这一行，非文本剪贴板可能被误写成空文本。
                self._set_clipboard_text(previous_text)  # 修改代码+Phase97ControlledNotepadLiveEdit: 恢复原剪贴板文本；如果没有这一行，Phase97 会留下系统剪贴板副作用。
    # 修改代码+Phase97ControlledNotepadLiveEdit: 函数段结束，WindowsSendInputLowLevelSender._paste_clipboard_text 到此结束；如果没有这个边界说明，初学者不容易看出粘贴范围。
    ...
    def _send_unicode_text(self, text: str) -> int:  # 新增代码+Phase58RealSendInputGuard: 函数段开始，用 SendInput Unicode 键盘事件输入文本；如果没有这段函数，type_text 没有真实写入能力。
        ...
        for char in text:  # 新增代码+Phase58RealSendInputGuard: 遍历每个 Unicode 字符；如果没有这行代码，文本不会被拆成键盘事件。
            ...
            time.sleep(0.005)  # 修改代码+Phase97ControlledNotepadLiveEdit: 每个字符后短暂停顿，避免 Notepad 丢字符；如果没有这一行，真实长文本可能输入不完整。
...
def run_phase58_real_sendinput_guard_contract(real_smoke: bool = True) -> dict[str, Any]:  # 新增代码+Phase58RealSendInputGuard: 函数段开始，运行合同自检和可选真实 smoke；如果没有这段函数，CLI 和测试没有统一入口。
    from learning_agent.computer_use_mcp_v2.windows_runtime.windows_backend import StaticWindowsWindowInventory  # 新增代码+Phase58RealSendInputGuard: 延迟导入静态 inventory 避免脚本模式循环；如果没有这行代码，合同无法构造可信快照。
