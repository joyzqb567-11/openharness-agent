...
try:  # 新增代码+Phase72RealAppSafetyBoundary: 优先按包路径导入 Phase60 授权 store；如果没有这段代码，正常包运行无法复用持久授权。
    from learning_agent.computer_use_mcp_v2.windows_runtime.persistent_grants import DEFAULT_PERSISTENT_GRANT_SESSION_ID, DEFAULT_PERSISTENT_GRANTS_ROOT, WindowsComputerUsePersistentGrantStore  # 新增代码+Phase72RealAppSafetyBoundary: 导入持久授权事实源；如果没有这行代码，安全边界只能凭窗口字段猜测是否允许。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.persistent_grants"}:  # 新增代码+Phase72RealAppSafetyBoundary: 只对包路径缺失做 fallback；如果没有这行代码，真实内部错误会被误吞。
        raise  # 新增代码+Phase72RealAppSafetyBoundary: 重新抛出非路径类导入错误；如果没有这行代码，底层 bug 会被隐藏。
    from computer_use_mcp_v2.windows_runtime.persistent_grants import DEFAULT_PERSISTENT_GRANT_SESSION_ID, DEFAULT_PERSISTENT_GRANTS_ROOT, WindowsComputerUsePersistentGrantStore  # 新增代码+Phase72RealAppSafetyBoundary: 脚本模式导入持久授权 store；如果没有这行代码，bat 入口无法运行 Phase72 自检。
