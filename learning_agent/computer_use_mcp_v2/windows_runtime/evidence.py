...
try:  # 新增代码+Phase29ComputerUse: 包运行模式下导入文本清理 helper；如果没有这行代码，证据摘要无法复用协议清理规则。 
    from learning_agent.computer_use_mcp_v2.windows_runtime.models import clean_protocol_text  # 新增代码+Phase29ComputerUse: 复用 Phase 27 文本清理；如果没有这行代码，UIA 摘要可能携带换行和超长空白。 
except ModuleNotFoundError as error:  # 新增代码+Phase29ComputerUse: 捕获脚本模式下包路径不可用；如果没有这行代码，start_oauth_agent.bat 入口可能导入失败。 
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.models"}:  # 新增代码+Phase29ComputerUse: 只允许目标包路径缺失时 fallback；如果没有这行代码，真实 bug 会被误吞。 
        raise  # 新增代码+Phase29ComputerUse: 重新抛出非目标导入错误；如果没有这行代码，排查 models 内部错误会很困难。 
    from computer_use_mcp_v2.windows_runtime.models import clean_protocol_text  # 新增代码+Phase29ComputerUse: 脚本模式下从本地包导入清理 helper；如果没有这行代码，直接运行入口无法加载 evidence。 

try:  # 新增代码+ClaudeCodeParityScreenshot: 优先按包路径导入截图坐标映射 helper；如果没有这行代码，evidence 层无法在正常测试入口复用坐标合同。
    from learning_agent.computer_use_mcp_v2.windows_runtime.coordinates import SCREENSHOT_COORDINATE_MODEL, build_screenshot_coordinate_mapping  # 新增代码+ClaudeCodeParityScreenshot: 导入截图映射版本和构造函数；如果没有这行代码，v2 result 和 metadata 无法生成 scale 元数据。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.coordinates"}:  # 新增代码+ClaudeCodeParityScreenshot: 只允许目标坐标模块路径缺失时 fallback；如果没有这行代码，coordinates 内部真实 bug 可能被误吞。
        raise  # 新增代码+ClaudeCodeParityScreenshot: 重新抛出非目标导入错误；如果没有这行代码，截图映射问题会被隐藏成脚本路径问题。
    from computer_use_mcp_v2.windows_runtime.coordinates import SCREENSHOT_COORDINATE_MODEL, build_screenshot_coordinate_mapping  # 新增代码+ClaudeCodeParityScreenshot: 脚本模式导入 v2 截图坐标 helper；如果没有这行代码，直接运行 evidence.py 时无法写入映射合同。
...
class ComputerUseEvidenceStore:  # 新增代码+Phase29ComputerUse: 定义 Computer Use 证据仓库；如果没有这个类，截图和 metadata 没有统一保存入口。 
    ...
    def save_window_state(self, *, window: dict[str, Any], payload: Any, fallback_width: int, fallback_height: int) -> dict[str, Any]:  # 新增代码+Phase29ComputerUse: 保存一次窗口状态证据；如果没有这段函数，get_window_state 无法落盘截图和 metadata。 
        ...
        screenshot_coordinate_mapping = build_screenshot_coordinate_mapping(window, screenshot_width, screenshot_height)  # 新增代码+ClaudeCodeParityScreenshot: 生成本次截图的坐标映射；如果没有这行代码，v2 result、image_result 和 metadata 无法共享 scale。
        ...
        result["screenshot_coordinate_model"] = SCREENSHOT_COORDINATE_MODEL  # 新增代码+ClaudeCodeParityScreenshot: 把坐标映射版本写入 v2 工具结果；如果没有这行代码，调用方无法判断 scale 字段协议版本。
        result["screenshot_coordinate_mapping"] = screenshot_coordinate_mapping  # 新增代码+ClaudeCodeParityScreenshot: 把坐标映射写入 v2 工具结果；如果没有这行代码，模型和测试无法从 result 直接换算截图坐标。
...
def run_phase41_image_results_contract() -> dict[str, Any]:  # 新增代码+Phase41WindowsImageResults: 定义 Phase41 安全合同入口；如果没有这行代码，验收脚本无法调用统一自检。
    ...
    try:  # 新增代码+Phase41WindowsImageResults: 优先用包模式导入控制器和测试 helper；如果没有这行代码，模块直接运行时无法兼容不同入口。
        from learning_agent.computer_use_mcp_v2.windows_runtime.controller import ComputerUseController, WindowsComputerUseBackend  # 新增代码+Phase41WindowsImageResults: 导入真实 Computer Use 控制链；如果没有这行代码，自检只能验证孤立 evidence。
        from learning_agent.computer_use_mcp_v2.windows_runtime.helper_client import StaticWindowObservationHelper, WindowObservationPayload  # 新增代码+Phase41WindowsImageResults: 导入静态观察 helper；如果没有这行代码，自检会需要真实桌面截图。
        from learning_agent.computer_use_mcp_v2.windows_runtime.windows_backend import StaticWindowsWindowInventory  # 新增代码+Phase41WindowsImageResults: 导入静态窗口 inventory；如果没有这行代码，自检会依赖用户当前窗口。
        from learning_agent.core.agent import LearningAgent  # 修改代码+AgentPyPhaseKFakeModel: 只从核心层导入真实 agent；如果没有这行代码，active_artifacts 集成无法自检。
        from learning_agent.core.messages import ModelMessage, ToolCall  # 修改代码+AgentPyPhaseKFakeModel: 从消息层导入模型消息和工具调用对象；如果没有这行代码，Phase41 自检无法构造假模型响应和 computer_observe 调用。
        from learning_agent.models.fake import ToolCallingFakeModel  # 修改代码+AgentPyPhaseKFakeModel: 从模型层导入测试假模型；如果没有这行代码，Phase41 自检会继续让 agent.py 承担假模型实现。
        ...
        if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.controller", "learning_agent.computer_use_mcp_v2.windows_runtime.helper_client", "learning_agent.computer_use_mcp_v2.windows_runtime.windows_backend", "learning_agent.core", "learning_agent.core.agent", "learning_agent.core.messages", "learning_agent.models", "learning_agent.models.fake", "learning_agent.tools", "learning_agent.tools.executor"}:  # 修改代码+GenericLocalAppLaunchGate: 允许内部 executor adapter 在脚本模式 fallback；如果没有这行代码，bat 自检会把包路径差异误报为导入失败。
            raise  # 新增代码+Phase41WindowsImageResults: 重新抛出非路径类导入错误；如果没有这行代码，排查真实 bug 会很困难。
        from computer_use_mcp_v2.windows_runtime.controller import ComputerUseController, WindowsComputerUseBackend  # 新增代码+Phase41WindowsImageResults: 脚本模式导入 Computer Use 控制链；如果没有这行代码，直接运行 evidence.py 无法自检。
        from computer_use_mcp_v2.windows_runtime.helper_client import StaticWindowObservationHelper, WindowObservationPayload  # 新增代码+Phase41WindowsImageResults: 脚本模式导入静态 helper；如果没有这行代码，自检无法模拟截图。
        from computer_use_mcp_v2.windows_runtime.windows_backend import StaticWindowsWindowInventory  # 新增代码+Phase41WindowsImageResults: 脚本模式导入静态 inventory；如果没有这行代码，自检无法模拟窗口。
        from core.agent import LearningAgent  # 修改代码+AgentPyPhaseKFakeModel: 脚本模式导入真实 agent；如果没有这行代码，agent artifact 集成无法自检。
        from core.messages import ModelMessage, ToolCall  # 修改代码+AgentPyPhaseKFakeModel: 脚本模式导入模型消息和工具调用对象；如果没有这行代码，bat 入口自检无法构造假模型响应和 computer_observe 调用。
        from models.fake import ToolCallingFakeModel  # 修改代码+AgentPyPhaseKFakeModel: 脚本模式从模型层导入测试假模型；如果没有这行代码，直接运行 evidence.py 时假模型兼容入口会断开。
    ...
    with tempfile.TemporaryDirectory() as raw_dir:  # 新增代码+Phase41WindowsImageResults: 创建临时根目录；如果没有这行代码，自检文件会留在真实项目目录。
        ...
        result_text = result.to_text("mcp__computer-use__observe")  # 修改代码+ComputerUseMcpV2ResidualCleanup：自检文本使用 v2 observe 名称；如果没有这行代码，Phase41 审计会误以为旧 computer_observe 仍是模型可见入口。
        ...
        agent_output = agent._execute_tool(ToolCall(name="mcp__computer-use__observe", arguments={"action": "get_window_state", "window": window, "reason": "phase41 v2 selftest"}))  # 修改代码+ComputerUseMcpV2ResidualCleanup：通过模型真实可见的 v2 MCP observe 读取窗口状态；如果没有这行代码，自检会继续依赖已删除的旧 executor 私有函数并误导后续排查。
