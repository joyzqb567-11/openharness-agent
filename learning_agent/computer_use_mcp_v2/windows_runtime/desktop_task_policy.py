...
_DIAGNOSTIC_COMMAND_PATTERNS = (  # 新增代码+DesktopTaskPolicy：元组段开始，列出桌面任务期间允许的只读诊断命令；如果没有这个元组，agent 无法安全检查 Paint 是否存在或运行。
    r"^python\s+acceptance_controller[\\/]+scripts[\\/]+daily_chrome_profile_connect_probe\.py$",  # 新增代码+DesktopBypassGate：允许 acceptance controller 的真实 Chrome profile 只读探针；如果没有这一项，修复 CDP 绕行时会误杀验收前置诊断。
...
_FORBIDDEN_BROWSER_AUTOMATION_BYPASS_PATTERNS = (  # 新增代码+DesktopBypassGate：元组段开始，列出桌面任务期间禁止用 shell/CDP/脚本直接操控浏览器的路线；如果没有这个元组，模型会继续绕过 browser_type/browser_click。
    (r"127\.0\.0\.1:9222|localhost:9222", "Chrome CDP local endpoint"),  # 新增代码+DesktopBypassGate：识别本机 Chrome DevTools 调试端口；如果没有这一项，脚本可直接连接真实 Chrome。
    (r"/json/(?:version|list|new)|chrome-devtools|devtools/browser", "Chrome CDP discovery endpoint"),  # 新增代码+DesktopBypassGate：识别 CDP 发现接口；如果没有这一项，脚本可枚举浏览器页面绕过工具链。
    (r"websocket|websocket-client|create_connection", "WebSocket CDP client"),  # 新增代码+DesktopBypassGate：识别 WebSocket 客户端路线；如果没有这一项，脚本可通过 ws.send 操作浏览器。
    (r"runtime\.evaluate|page\.navigate|input\.dispatch|dom\.queryselector", "CDP browser command"),  # 新增代码+DesktopBypassGate：识别典型 CDP 方法名；如果没有这一项，脚本可直接执行 JS、导航和输入。
    (r"document\.queryselector|queryselectorall|contenteditable", "DOM locator script"),  # 新增代码+DesktopBypassGate：识别直接 DOM 查找/输入脚本；如果没有这一项，模型会跳过 snapshot 的元素 id。
    (r"tmp_qianwen_style|qianwen_style_r3", "temporary browser automation script"),  # 新增代码+DesktopBypassGate：识别本次真实失败日志里的临时脚本命名；如果没有这一项，同类压力测试会继续伪完成。
    (r"set-content\b.*\.py\b.*(?:websocket|runtime\.evaluate|127\.0\.0\.1:9222|/json/version)", "generated Python browser automation script"),  # 新增代码+DesktopBypassGate：识别 shell 写 Python 浏览器自动化脚本；如果没有这一项，生成脚本后执行的绕行仍会通过。
    (r"subprocess\.popen\b.*(?:chrome|explorer|cloudmusic|netease|网易云)", "subprocess desktop app launch bypass"),  # 修改代码+ComputerUseMcpV2ResidualCleanup：识别 Python subprocess 直接启动桌面 App；如果没有这一项，网易云流程可能绕过 v2 Computer Use MCP 工具链。
)  # 新增代码+DesktopBypassGate：元组段结束，浏览器/桌面脚本绕行规则到此固定；如果没有这一行，Python 元组语法不完整。
...
# 新增代码+DesktopBypassGate：函数段开始，识别桌面任务里用 shell/CDP/Python 绕过真实浏览器和 Computer Use 工具链的命令；如果没有这段函数，千问/网易云压力测试会继续用脚本伪完成，作者意图是把网页输入和桌面 App 控制都拉回 observe-plan-act-verify，本函数与 evaluate_desktop_bash_command 配合使用，函数段到 return 结束。
def _desktop_task_policy_browser_bypass_matches(normalized_command: str) -> list[str]:  # 新增代码+DesktopBypassGate：定义浏览器/桌面脚本绕行匹配 helper；如果没有这一行，主函数会堆满 CDP/WebSocket 正则。
    matches: list[str] = []  # 新增代码+DesktopBypassGate：准备保存命中的绕行规则名称；如果没有这一行，后续无法逐条追加原因。
    for pattern, name in _FORBIDDEN_BROWSER_AUTOMATION_BYPASS_PATTERNS:  # 新增代码+DesktopBypassGate：逐条检查 CDP/WebSocket/DOM/subprocess 绕行规则；如果没有这一行，规则表只是静态数据不会生效。
        if re.search(pattern, normalized_command):  # 新增代码+DesktopBypassGate：用正则检查当前命令是否命中绕行特征；如果没有这一行，危险命令不会被识别。
            matches.append(name)  # 新增代码+DesktopBypassGate：记录命中的规则名称；如果没有这一行，拒绝原因无法告诉用户是哪类绕行。
    return matches  # 新增代码+DesktopBypassGate：返回全部浏览器/桌面绕行命中项；如果没有这一行，主策略拿不到拒绝依据。
# 新增代码+DesktopBypassGate：函数段结束，_desktop_task_policy_browser_bypass_matches 到此结束；如果没有这个边界说明，代码小白不容易看出新门禁只处理 CDP/WebSocket/脚本绕行。


# 新增代码+DesktopTaskPolicy：函数段开始，评估桌面任务期间 bash/PowerShell 命令是否允许执行；如果没有这段函数，Task 3 无法在命令执行前阻止脚本生成最终图片，作者意图是让 `_bash_atom` 在请求权限前先做 GUI 路线门禁，本函数与 Learning旧 bash 包装 配合使用，函数段到返回字典结束为止。
def evaluate_desktop_bash_command(command: str, desktop_task_active: bool) -> dict[str, object]:  # 新增代码+DesktopTaskPolicy：定义 Task 3 公开策略函数；如果没有这一行，测试和 agent 都没有稳定入口可调用。
    ...
    browser_bypass_matches = _desktop_task_policy_browser_bypass_matches(normalized_command)  # 新增代码+DesktopBypassGate：先检查 CDP/WebSocket/Python 浏览器绕行；如果没有这一行，tmp_qianwen_style_r3.py 仍会被普通 active 放行。
    if browser_bypass_matches:  # 新增代码+DesktopBypassGate：命中任一绕行特征就拒绝；如果没有这一行，脚本可以继续代替 browser_type/browser_click。
        return {  # 新增代码+DesktopBypassGate：浏览器/桌面绕行拒绝结果字典段开始；如果没有这一行，调用方拿不到结构化拒绝信息。
            "allowed": False,  # 新增代码+DesktopBypassGate：声明该命令不允许执行；如果没有这一项，_bash_atom 仍会继续真实运行危险绕行脚本。
            "decision": "desktop_task_requires_observe_plan_act_verify_route",  # 新增代码+DesktopBypassGate：返回专门的治本拒绝码；如果没有这一项，验收无法区分脚本制品问题和动作链绕行问题。
            "desktop_task_active": True,  # 新增代码+DesktopBypassGate：记录当前桌面任务已激活；如果没有这一项，日志里看不出为什么策略生效。
            "forbidden_script_artifact_route": True,  # 新增代码+DesktopBypassGate：复用风险字段说明这是脚本绕行路线；如果没有这一项，上层报告无法统一识别禁止路线。
            "reason": f"当前是桌面/真实浏览器任务，命令命中 CDP/WebSocket/Python 绕行路线：{', '.join(browser_bypass_matches)}；请改用 browser_profile_status -> browser_connect_real_chrome -> browser_open/browser_snapshot/browser_type/browser_click，桌面 App 请改用 mcp__computer-use__observe 和 mcp__computer-use__computer_batch 或具体原子动作工具。",  # 修改代码+ComputerUseMcpV2ResidualCleanup：给出 v2 MCP 替代路径；如果没有这一项，模型只知道被拒绝却可能回到旧裸名工具链。
            "matched_forbidden_patterns": browser_bypass_matches,  # 新增代码+DesktopBypassGate：返回命中的绕行规则名称；如果没有这一项，用户无法学习是哪类脚本被拦住。
        }  # 新增代码+DesktopBypassGate：浏览器/桌面绕行拒绝结果字典段结束；如果没有这一行，Python 字典语法不完整。
