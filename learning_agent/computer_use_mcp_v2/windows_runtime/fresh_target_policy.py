...
def decide_recovery_after_drift(target_ref: Any, reason: Any = "target_lease_drift_rejected") -> dict[str, Any]:  # 新增代码+FreshTargetPolicy：函数段开始，生成漂移后的恢复动作；如果没有这段函数，漂移后可能继续用旧 target_ref 重试。
    ...
    if safe_reason == "target_lease_not_verified":  # 新增代码+ProxyPidLease：区分租约构建/绑定失败和真实窗口漂移；如果没有这一行，agent 会把底层绑定 bug 误当成需要反复打开应用。
        return {"decision": "target_lease_invalidated", "invalidated_target_ref": safe_ref, "reason": safe_reason, "fresh_target_relaunch_required": False, "recovery_next_allowed_actions": ["observe"], "requires_binding_error_report": True, "low_level_event_count": 0}  # 新增代码+ProxyPidLease：只允许观察和报告绑定错误；如果没有这一行，失败后会继续 launch_app 造成截图里的循环。
