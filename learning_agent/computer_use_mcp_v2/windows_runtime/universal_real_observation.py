...
try:  # 新增代码+URG1RealObservationFrame：优先按 learning_agent 包路径导入真实基座；如果没有这一段，单测和生产入口无法共享同一实现。
    ...
    from computer_use_mcp_v2.windows_runtime.windows_backend import WindowsWindowInventoryProbe  # type: ignore  # 新增代码+URG1RealObservationFrame：脚本模式导入窗口 inventory；如果没有这一行，bat 入口无法观察窗口。
...
class UniversalRealObservationFrameRuntime:  # 新增代码+URG1RealObservationFrame：类段开始，组合窗口、截图、UIA 和融合层生成统一 ObservationFrame；如果没有这个类，URG-1 会停留在分散工具。
    ...
    def observe(self, target_hint: str = "", real_desktop_touched: bool = False, target_window: dict[str, Any] | None = None) -> dict[str, Any]:  # 修改代码+BoundObservationTarget：函数段开始，执行一次只读 ObservationFrame 生成并优先使用绑定窗口；如果没有 target_window，真实 full loop 会在多个同名窗口中看错目标。
        ...
        bound_target_window = _phase116_safe_dict(target_window)  # 新增代码+BoundObservationTarget：清洗 loop 传入的 agent-owned 目标窗口；如果没有这一行，绑定窗口参数可能以坏类型打断观察。
        target_window = bound_target_window if bound_target_window else _phase116_select_target_window(inventory, target_hint=target_hint)  # 修改代码+BoundObservationTarget：优先使用已绑定目标，否则才按 hint/活动窗口选择；如果没有这一行，旧 Paint/同名窗口会污染观察。
