...
try:  # 新增代码+Phase28ComputerUse: 包运行模式下导入协议模型 helper；如果没有这行代码，窗口清理和身份键无法复用 Phase 27 合同。
    from learning_agent.computer_use_mcp_v2.windows_runtime.models import build_window_ref, clean_protocol_text, window_ref_identity  # 新增代码+Phase28ComputerUse: 复用窗口引用构造和文本清理；如果没有这行代码，inventory 会和 controller 使用不同身份规则。
except ModuleNotFoundError as error:  # 新增代码+Phase28ComputerUse: 捕获脚本模式下包路径不可用的情况；如果没有这行代码，start_oauth_agent.bat 入口可能导入失败。
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.models"}:  # 新增代码+Phase28ComputerUse: 只允许目标包路径缺失时 fallback；如果没有这行代码，models 内部真实 bug 会被误吞。
        raise  # 新增代码+Phase28ComputerUse: 重新抛出真实导入错误；如果没有这行代码，排查窗口协议模型问题会很困难。
    from computer_use_mcp_v2.windows_runtime.models import build_window_ref, clean_protocol_text, window_ref_identity  # 新增代码+Phase28ComputerUse: 脚本模式下从本地包导入 helper；如果没有这行代码，直接执行入口无法加载 inventory。

try:  # 修改代码+TargetIdentityMaturity：优先按包路径导入 Task 3 标题摘要和哈希 helper；如果没有这一段，窗口 inventory 无法给目标身份绑定提供统一脱敏字段。
    from learning_agent.computer_use_mcp_v2.windows_runtime.target_identity import summarize_window_title, window_title_sha256_16  # 修改代码+TargetIdentityMaturity：复用目标身份模块的标题摘要和哈希规则；如果没有这一行，windows_backend 会和动作前校验使用不同标题规则。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.target_identity"}:  # 修改代码+TargetIdentityMaturity：只允许包路径缺失时 fallback；如果没有这一行，target_identity 内部真实 bug 可能被误吞。
        raise  # 修改代码+TargetIdentityMaturity：重新抛出真实导入错误；如果没有这一行，目标身份模块的内部问题会很难排查。
    from computer_use_mcp_v2.windows_runtime.target_identity import summarize_window_title, window_title_sha256_16  # type: ignore  # 修改代码+TargetIdentityMaturity：脚本模式下从本地包导入同一 helper；如果没有这一行，双击 bat 后窗口身份字段可能不可用。
...
class WindowsWindowInventoryProbe:  # 新增代码+Phase28ComputerUse: 定义真实 Win32 只读窗口枚举 probe；如果没有这个类，Windows 后端无法实际发现窗口。
    ...
    def _collect_raw_windows(self) -> tuple[list[dict[str, Any]], int | None]:  # 新增代码+Phase28ComputerUse: 使用 ctypes 枚举 Win32 顶层窗口；如果没有这段代码，真实 inventory 无法读取本机窗口。
        ...
        kernel32 = ctypes.windll.kernel32  # 新增代码+ExternalAppWindowBinding：获取 kernel32 API 用于按 pid 查询进程路径；如果没有这一行，真实窗口 process_name 仍会为空。
        process_query_limited_information = 0x1000  # 新增代码+ExternalAppWindowBinding：使用最小查询权限读取进程路径；如果没有这一行，OpenProcess 权限会不清楚且可能过宽。
        ...
        def query_process_summary(process_id: int) -> tuple[str, str]:  # 新增代码+ExternalAppWindowBinding：函数段开始，按 pid 查询进程名和脱敏路径；如果没有这段函数，多进程窗口绑定缺少真实 process_name。
            process_handle = kernel32.OpenProcess(process_query_limited_information, False, int(process_id))  # 新增代码+ExternalAppWindowBinding：用最小权限打开进程句柄；如果没有这一行，无法读取窗口所属程序名。
            if not process_handle:  # 新增代码+ExternalAppWindowBinding：部分系统进程可能无法打开；如果没有这一行，后续 QueryFullProcessImageNameW 会收到空句柄。
                return "", ""  # 新增代码+ExternalAppWindowBinding：无法读取时优雅返回空摘要；如果没有这一行，受保护进程会让整个窗口枚举失败。
            try:  # 新增代码+ExternalAppWindowBinding：确保无论读取成功失败都关闭句柄；如果没有这一行，窗口轮询会泄露句柄。
                buffer_size = wintypes.DWORD(32768)  # 新增代码+ExternalAppWindowBinding：准备 Windows 路径最大缓冲长度；如果没有这一行，长路径可能读不完整。
                path_buffer = ctypes.create_unicode_buffer(buffer_size.value)  # 新增代码+ExternalAppWindowBinding：创建路径接收缓冲区；如果没有这一行，API 没有输出位置。
                ok = kernel32.QueryFullProcessImageNameW(process_handle, 0, path_buffer, ctypes.byref(buffer_size))  # 新增代码+ExternalAppWindowBinding：读取进程完整路径；如果没有这一行，真实 process_name 仍只能为空。
                if not ok:  # 新增代码+ExternalAppWindowBinding：处理 API 读取失败；如果没有这一行，失败时会把空缓冲误当路径。
                    return "", ""  # 新增代码+ExternalAppWindowBinding：读取失败返回空摘要；如果没有这一行，后续 basename 可能得到无意义值。
                process_path = str(path_buffer.value or "")  # 新增代码+ExternalAppWindowBinding：取出进程路径字符串；如果没有这一行，路径无法参与哈希和文件名提取。
                process_name = process_path.replace("/", "\\").rsplit("\\", 1)[-1] if process_path else ""  # 新增代码+ExternalAppWindowBinding：从路径取文件名作为 process_name；如果没有这一行，网易云音乐窗口匹配缺少进程名证据。
                return process_name, process_path_hash(process_path)  # 新增代码+ExternalAppWindowBinding：返回进程名和脱敏路径哈希；如果没有这一行，调用方拿不到安全身份摘要。
            finally:  # 新增代码+ExternalAppWindowBinding：进入句柄清理分支；如果没有这一行，异常时句柄可能泄露。
                kernel32.CloseHandle(process_handle)  # 新增代码+ExternalAppWindowBinding：关闭进程句柄；如果没有这一行，长任务轮询窗口会逐渐耗尽系统资源。
        # 新增代码+ExternalAppWindowBinding：函数段结束，query_process_summary 到此结束；如果没有这个边界说明，初学者不容易看出进程摘要读取范围。
        ...
        def collect_window(hwnd: Any, lparam: Any) -> bool:  # 新增代码+Phase28ComputerUse: 定义单个窗口回调；如果没有这段代码，EnumWindows 无法把窗口逐个交给 Python。
            ...
            process_name, process_hash = query_process_summary(safe_int(pid.value))  # 新增代码+ExternalAppWindowBinding：读取窗口进程名和脱敏路径哈希；如果没有这一行，外部应用窗口绑定缺少真实进程证据。
            raw_windows.append({"hwnd": safe_int(hwnd), "pid": safe_int(pid.value), "process_name": process_name, "process_path_hash": process_hash, "class_name": class_buffer.value, "title": title_buffer.value, "rect": {"left": safe_int(rect.left), "top": safe_int(rect.top), "right": safe_int(rect.right), "bottom": safe_int(rect.bottom)}})  # 修改代码+ExternalAppWindowBinding：保存原始窗口记录并带进程名；如果没有这行代码，快照无法包含该窗口或无法绑定多进程真实应用。
