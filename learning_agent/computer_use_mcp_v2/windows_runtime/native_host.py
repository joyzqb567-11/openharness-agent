...
try:  # 新增代码+Phase44WindowsNativeHost: 优先按包模式导入 helper 合同；如果没有这行代码，正常 unittest 和 agent 入口无法复用观察 helper。
    from learning_agent.computer_use_mcp_v2.windows_runtime.helper_client import NullWindowObservationHelper, StaticWindowObservationHelper, WindowObservationPayload  # 新增代码+Phase44WindowsNativeHost: 导入默认 helper、静态 helper 和 payload；如果没有这行代码，host 无法安全观察或自检。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.helper_client"}:  # 新增代码+Phase44WindowsNativeHost: 只允许目标包路径缺失时 fallback；如果没有这行代码，真实内部 bug 会被误吞。
        raise  # 新增代码+Phase44WindowsNativeHost: 重新抛出非路径类导入错误；如果没有这行代码，排查 helper 内部问题会困难。
    from computer_use_mcp_v2.windows_runtime.helper_client import NullWindowObservationHelper, StaticWindowObservationHelper, WindowObservationPayload  # 新增代码+Phase44WindowsNativeHost: 脚本模式导入 helper 合同；如果没有这行代码，bat 入口无法运行 Phase44 自检。
...
def _runtime_action_summary(action_result: Any) -> dict[str, Any]:  # 新增代码+Phase47WindowsSendInputDispatcher: 函数段开始，把 action executor 结果转成 host 可返回的安全摘要；如果没有这段函数，native host action 可能把原始文本塞回 IPC。
    data = getattr(action_result, "data", {})  # 新增代码+Phase47WindowsSendInputDispatcher: 读取执行器 data 字段；如果没有这行代码，结果对象无法被转换。
    summary = dict(data) if isinstance(data, dict) else {}  # 新增代码+Phase47WindowsSendInputDispatcher: 只复制 dict data；如果没有这行代码，异常 data 类型可能污染响应。
    summary["ok"] = bool(getattr(action_result, "ok", False))  # 新增代码+Phase47WindowsSendInputDispatcher: 写入执行成功状态；如果没有这行代码，host 调用方无法知道动作是否成功。
    summary["message"] = str(getattr(action_result, "message", ""))  # 新增代码+Phase47WindowsSendInputDispatcher: 写入可读消息；如果没有这行代码，失败原因会丢失。
    summary.pop("text", None)  # 新增代码+Phase47WindowsSendInputDispatcher: 删除可能存在的原始 text 字段；如果没有这行代码，type_text 原文可能外泄。
    summary.pop("raw_text", None)  # 新增代码+Phase47WindowsSendInputDispatcher: 删除兼容 raw_text 字段；如果没有这行代码，未来底层返回换名仍可能泄露。
    summary["raw_text_included"] = False  # 新增代码+Phase47WindowsSendInputDispatcher: 明确 host action 不返回原始文本；如果没有这行代码，安全审计无法确认脱敏结果。
    return summary  # 新增代码+Phase47WindowsSendInputDispatcher: 返回安全 action 摘要；如果没有这行代码，host action 分支拿不到可序列化结果。
# 新增代码+Phase47WindowsSendInputDispatcher: 函数段结束，_runtime_action_summary 到此结束；如果没有这个边界说明，读者不容易看出 action 脱敏范围。
...
class WindowsComputerUseNativeHost:  # 新增代码+Phase44WindowsNativeHost: 类段开始，定义进程内 Windows native host；如果没有这个类，主 agent 没有独立 host 边界可调用。
    def __init__(self, helper: Any | None = None, platform: str | None = None, host_id: str = "windows-computer-use-native-host", real_actions_enabled: bool = False, screenshot_runtime: Any | None = None, uia_tree_runtime: Any | None = None, action_executor: Any | None = None) -> None:  # 修改代码+Phase47WindowsSendInputDispatcher: 函数段开始，初始化 helper、平台、观察运行时和可选动作执行器；如果没有这段函数，host 无法接入 Phase47 动作调度链路。
        ...
        self.action_executor = action_executor  # 新增代码+Phase47WindowsSendInputDispatcher: 保存可选动作执行器；如果没有这行代码，action 消息无法在显式启用时走 dispatcher。
    ...
    # 修改代码+Phase47WindowsSendInputDispatcher: 函数段结束，WindowsComputerUseNativeHost.__init__ 到此结束；如果没有这个边界说明，读者不容易看出初始化范围。
    ...
    def status(self) -> dict[str, Any]:  # 新增代码+Phase44WindowsNativeHost: 函数段开始，返回 host 状态；如果没有这段函数，client 和 /computer 无法查看协议能力。
        ...
        action_executor_status = self.action_executor.status() if hasattr(self.action_executor, "status") else {}  # 新增代码+Phase47WindowsSendInputDispatcher: 读取动作执行器状态；如果没有这行代码，host status 看不到 Phase47 dispatcher 链路。
        return {"marker": PHASE44_WINDOWS_NATIVE_HOST_MARKER, "host_id": self.host_id, "protocol": PHASE44_NATIVE_HOST_PROTOCOL, "platform": self.platform, "started": bool(self.started), "healthy": bool(self.started), "supported_messages": list(PHASE44_SUPPORTED_MESSAGES), "helper": dict(helper_status) if isinstance(helper_status, dict) else {}, "screenshot_runtime_available": bool(self.screenshot_runtime is not None), "screenshot_runtime": dict(screenshot_runtime_status) if isinstance(screenshot_runtime_status, dict) else {}, "uia_tree_runtime_available": bool(self.uia_tree_runtime is not None), "uia_tree_runtime": dict(uia_tree_runtime_status) if isinstance(uia_tree_runtime_status, dict) else {}, "action_executor_available": bool(self.action_executor is not None), "action_executor": dict(action_executor_status) if isinstance(action_executor_status, dict) else {}, "safe_observe_only": not bool(self.real_actions_enabled and self.action_executor is not None), "real_actions_enabled": bool(self.real_actions_enabled), "actions_expanded": bool(self.real_actions_enabled and self.action_executor is not None)}  # 修改代码+Phase47WindowsSendInputDispatcher: 返回包含动作执行器的完整状态；如果没有这行代码，健康检查和动作能力展示没有统一事实源。
    # 修改代码+Phase47WindowsSendInputDispatcher: 函数段结束，WindowsComputerUseNativeHost.status 到此结束；如果没有这个边界说明，读者不容易看出状态范围。
    ...
    def handle_message(self, message: dict[str, Any]) -> dict[str, Any]:  # 新增代码+Phase44WindowsNativeHost: 函数段开始，处理一条 native host 消息；如果没有这段函数，host 只有状态没有协议入口。
        ...
        if op == "action":  # 新增代码+Phase44WindowsNativeHost: 识别动作消息但默认拒绝；如果没有这行代码，未来动作协议没有安全占位。
            if self.real_actions_enabled and self.action_executor is not None:  # 新增代码+Phase47WindowsSendInputDispatcher: 只有显式启用且注入执行器时才处理动作；如果没有这行代码，host 可能绕过安全默认拒绝。
                action_name = str(payload.get("action", "") or "").strip()  # 新增代码+Phase47WindowsSendInputDispatcher: 读取动作名；如果没有这行代码，执行器不知道要调度什么动作。
                action_result = self.action_executor.execute(action_name, dict(payload))  # 新增代码+Phase47WindowsSendInputDispatcher: 调用注入执行器；如果没有这行代码，native host action 无法进入 Phase47 dispatcher。
                action_summary = _runtime_action_summary(action_result)  # 新增代码+Phase47WindowsSendInputDispatcher: 脱敏执行结果；如果没有这行代码，raw text 可能进入 host 响应。
                if bool(getattr(action_result, "ok", False)):  # 新增代码+Phase47WindowsSendInputDispatcher: 判断动作是否成功；如果没有这行代码，失败也会被包装成成功。
                    return self._ok(op, action_summary)  # 新增代码+Phase47WindowsSendInputDispatcher: 返回成功 action 响应；如果没有这行代码，host 调用方拿不到执行摘要。
                return self._error(op, str(action_summary.get("decision", "action_executor_failed")), str(action_summary.get("message", "action executor failed")))  # 新增代码+Phase47WindowsSendInputDispatcher: 返回执行器失败响应；如果没有这行代码，失败路径不可解释。
