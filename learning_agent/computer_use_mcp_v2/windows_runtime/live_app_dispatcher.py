...
from learning_agent.computer_use_mcp_v2.windows_runtime.windows_launch_resolver import Phase69RecordingFocuser, Phase69RecordingLauncher, WindowsAppWindowControlRuntime, build_launch_plan  # 新增代码+Phase90LiveAppDispatcher: 复用 Phase69 启动和聚焦合同；如果没有这行代码，Phase90 会重复实现脆弱的 app/window 控制。
from learning_agent.computer_use_mcp_v2.windows_runtime.generic_input_actions import Phase71RecordingInputSender, WindowsGenericInputActionRuntime, build_drag_path  # 新增代码+Phase90LiveAppDispatcher: 复用 Phase71 通用输入事件和记录型 sender；如果没有这行代码，拖拽和记录派发格式会漂移。
from learning_agent.computer_use_mcp_v2.windows_runtime.persistent_grants import WindowsComputerUsePersistentGrantStore  # 新增代码+Phase90LiveAppDispatcher: 复用 Phase60 持久授权事实源；如果没有这行代码，真实应用动作缺少可审计授权。
from learning_agent.computer_use_mcp_v2.windows_runtime.real_app_safety_boundary import Phase72RecordingAbortGate, WindowsRealAppSafetyBoundary  # 新增代码+Phase90LiveAppDispatcher: 复用 Phase72 安全边界和急停 gate；如果没有这行代码，危险窗口和 abort 无法统一拒绝。
from learning_agent.computer_use_mcp_v2.windows_runtime.representative_e2e_matrix import WindowsRepresentativeE2EMatrix  # 新增代码+Phase90LiveAppDispatcher: 复用 Phase74 代表性 E2E 和 Paint 皮卡丘计划；如果没有这行代码，用户点名场景会重复造轮子。
...
class WindowsLiveAppDispatcher:
    ...
    def build_app_window(self, app_name: Any) -> dict[str, Any]:
        ...
        window_id = f"phase132-window:{executable or 'blocked'}"  # 修改代码+CompatSlimming：使用统一窗口记录器的 phase132 窗口 id 规则；如果没有这行代码，Phase60 授权会和真实 dispatch 目标窗口匹配不上。
        return {"app_id": executable, "process_name": executable, "window_id": window_id, "display_id": "DISPLAY1", "title_preview": f"{executable} - recording", "safe_to_target": bool(plan.get("safe_to_launch"))}  # 修改代码+CompatSlimming：返回与 resolver 记录窗口一致的摘要；如果没有这行代码，Phase60/72 无法评估目标。
...
def _phase90_optional_safe_window_smoke(real_smoke: bool) -> dict[str, Any]:
    ...
    try:  # 新增代码+Phase90LiveAppDispatcher: 真实 smoke 可能受桌面环境影响；如果没有这行代码，异常会打断整个验收。
        from learning_agent.computer_use_mcp_v2.windows_runtime.real_sendinput_guard import run_phase58_real_sendinput_guard_contract  # 新增代码+Phase90LiveAppDispatcher: 复用 Phase58 安全窗口真实 SendInput；如果没有这行代码，Phase90 会重复造低层真实输入。
