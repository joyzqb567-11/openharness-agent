...
try:  # 新增代码+Phase70GenericControlActions: 优先按包路径导入 Phase57 语义控件定位器；如果没有这段代码，unittest 和生产入口无法复用真实 locator。
    from learning_agent.computer_use_mcp_v2.windows_runtime.real_uia_locator import SemanticControlLocator  # 新增代码+Phase70GenericControlActions: 复用 Phase57 控件定位器；如果没有这行代码，Phase70 会重复造定位逻辑。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.real_uia_locator"}:  # 新增代码+Phase70GenericControlActions: 只允许包路径缺失时 fallback；如果没有这行代码，内部真实 bug 会被误吞。
        raise  # 新增代码+Phase70GenericControlActions: 重新抛出非路径类导入错误；如果没有这行代码，底层 locator 问题会被隐藏。
    from computer_use_mcp_v2.windows_runtime.real_uia_locator import SemanticControlLocator  # type: ignore  # 新增代码+Phase70GenericControlActions: 脚本模式导入 Phase57 locator；如果没有这行代码，bat 入口无法运行 Phase70 合同。
try:  # 新增代码+VerifiedWindowActionsMaturity: 优先按包路径导入 Phase114 已验证窗口动作门禁；如果没有这段代码，Phase70 点击/输入无法复用统一身份门禁。
    from learning_agent.computer_use_mcp_v2.windows_runtime.closed_loop_executor import phase114_verified_action_gate  # 新增代码+VerifiedWindowActionsMaturity: 导入统一动作身份门禁函数；如果没有这行代码，点击和输入会各自手写身份判断。
    ...
    from computer_use_mcp_v2.windows_runtime.closed_loop_executor import phase114_verified_action_gate  # type: ignore  # 新增代码+VerifiedWindowActionsMaturity: 脚本模式导入统一身份门禁；如果没有这行代码，bat 入口无法执行 Task6 动作门禁。
