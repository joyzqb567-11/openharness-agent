...
try:  # 新增代码+Phase96ControlledPhysicalLiveSmoke：优先按包路径导入项目组件；如果没有这段代码，单元测试和生产入口不能共享同一实现。
    from learning_agent.computer_use_mcp_v2.windows_runtime.controlled_physical_sendinput import Phase95RecordingSendInputBackend, WindowsControlledPhysicalSendInputSender  # 新增代码+Phase96ControlledPhysicalLiveSmoke：复用 Phase95 受控物理 sender；如果没有这行代码，Phase96 可能绕开安全链路。
    from learning_agent.computer_use_mcp_v2.windows_runtime.persistent_grants import DEFAULT_PERSISTENT_GRANTS_ROOT  # 新增代码+Phase96ControlledPhysicalLiveSmoke：复用项目 Computer Use 运行根目录；如果没有这行代码，验收报告落点会分散。
    from learning_agent.computer_use_mcp_v2.windows_runtime.real_sendinput_guard import WindowsSendInputLowLevelSender, run_phase58_real_sendinput_guard_smoke  # 新增代码+Phase96ControlledPhysicalLiveSmoke：复用 Phase58 真实 SendInput 和键盘 guard；如果没有这行代码，Phase96 会重复造轮子。
    ...
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.runtime", "learning_agent.runtime.files"}:  # 新增代码+Phase96ControlledPhysicalLiveSmoke：只对包前缀缺失做 fallback；如果没有这行代码，内部真实 bug 可能被误吞。
        raise  # 新增代码+Phase96ControlledPhysicalLiveSmoke：重抛非路径类导入错误；如果没有这行代码，排查底层模块问题会很困难。
    from computer_use_mcp_v2.windows_runtime.controlled_physical_sendinput import Phase95RecordingSendInputBackend, WindowsControlledPhysicalSendInputSender  # type: ignore  # 新增代码+Phase96ControlledPhysicalLiveSmoke：脚本模式复用 Phase95 sender；如果没有这行代码，bat 入口无法运行合同。
    from computer_use_mcp_v2.windows_runtime.persistent_grants import DEFAULT_PERSISTENT_GRANTS_ROOT  # type: ignore  # 新增代码+Phase96ControlledPhysicalLiveSmoke：脚本模式复用默认目录；如果没有这行代码，报告目录无法稳定定位。
    from computer_use_mcp_v2.windows_runtime.real_sendinput_guard import WindowsSendInputLowLevelSender, run_phase58_real_sendinput_guard_smoke  # type: ignore  # 新增代码+Phase96ControlledPhysicalLiveSmoke：脚本模式复用 Phase58；如果没有这行代码，bat 入口无法验证真实输入 guard。
...
def _phase96_run_real_mouse_bridge(platform: str | None = None) -> dict[str, Any]:  # 新增代码+Phase96ControlledPhysicalLiveSmoke：函数段开始，在专用安全窗口中执行真实鼠标 bridge；如果没有这段函数，Phase96 只能停留在 fake 后端。
    ...
    from learning_agent.computer_use_mcp_v2.windows_runtime.real_sendinput_guard import Phase58UiaSafeWindowObserver  # 新增代码+Phase96ControlledPhysicalLiveSmoke：延迟导入 UIA observer；如果没有这行代码，真实窗口坐标无法定位。
    from learning_agent.computer_use_mcp_v2.windows_runtime.real_uia_locator import Phase57DedicatedSafeWindowLauncher, _phase57_find_safe_edit_control, _poll_phase57_safe_window  # 新增代码+Phase96ControlledPhysicalLiveSmoke：延迟导入专用安全窗口工具；如果没有这行代码，真实动作没有隔离目标。
    from learning_agent.computer_use_mcp_v2.windows_runtime.windows_backend import WindowsWindowInventoryProbe  # 新增代码+Phase96ControlledPhysicalLiveSmoke：延迟导入真实窗口枚举；如果没有这行代码，无法找到自建安全窗口。
