...
try:  # 新增代码+CompatSlimming：优先按包路径导入统一 Windows app resolver；如果没有这一段，Phase109 候选迁入后无法生成发现报告。
    from learning_agent.computer_use_mcp_v2.windows_runtime.windows_launch_resolver import build_launch_plan, resolve_windows_app_launch_target  # 新增代码+CompatSlimming：复用唯一应用发现和启动计划入口；如果没有这一行，generic_launch_backend 会重新分叉发现逻辑。
except ModuleNotFoundError as error:  # 新增代码+CompatSlimming：兼容 start_oauth_agent.bat 脚本式导入；如果没有这一段，真实可见终端可能因包名前缀不同失败。
    if error.name not in {"learning_agent", "learning_agent.computer_use_mcp_v2.windows_runtime", "learning_agent.computer_use_mcp_v2.windows_runtime.windows_launch_resolver"}:  # 新增代码+CompatSlimming：只兜底包路径缺失；如果没有这一行，resolver 内部真实 bug 会被误吞。
        raise  # 新增代码+CompatSlimming：重新抛出真实内部错误；如果没有这一行，排查启动候选失败会很困难。
    from computer_use_mcp_v2.windows_runtime.windows_launch_resolver import build_launch_plan, resolve_windows_app_launch_target  # type: ignore  # 新增代码+CompatSlimming：脚本模式导入同一 resolver；如果没有这一行，bat 入口无法复用瘦身后的发现层。
...
PHASE103_REAL_APP_LAUNCH_DEFAULT_DISABLED = True  # 新增代码+CompatSlimming：保留受控应用启动默认关闭语义；如果没有这一行，旧 Phase103 调用方会误以为 full 模式默认真实启动。
PHASE103_UNCONTROLLED_ACTIONS_EXPANDED = False
PHASE103_CONTROLLED_APP_LAUNCH_MARKER = "PHASE103_CONTROLLED_APP_LAUNCH_READY"  # 新增代码+CompatSlimming：保留旧 Phase103 受控启动合同 marker；如果没有这一行，旧测试会误以为迁移后受控启动能力消失。
PHASE103_CONTROLLED_APP_LAUNCH_OK_TOKEN = "PHASE103_CONTROLLED_APP_LAUNCH_OK"  # 新增代码+CompatSlimming：保留旧 Phase103 受控启动 OK token；如果没有这一行，旧终端验收无法复用同一合同。
PHASE109_GENERIC_REAL_LAUNCH_CANDIDATE_MARKER = "PHASE109_GENERIC_REAL_LAUNCH_CANDIDATE_READY"  # 新增代码+CompatSlimming：把旧 Phase109 marker 映射到通用启动后端候选合同；如果没有这一行，删除旧文件后旧候选验收会断。
PHASE109_GENERIC_REAL_LAUNCH_CANDIDATE_OK_TOKEN = "PHASE109_GENERIC_REAL_LAUNCH_CANDIDATE_OK"  # 新增代码+CompatSlimming：把旧 Phase109 OK token 映射到通用启动后端候选合同；如果没有这一行，旧 CLI token 检查会失败。
PHASE109_REAL_LAUNCH_DEFAULT_DISABLED = True  # 新增代码+CompatSlimming：保留旧 Phase109 默认关闭语义；如果没有这一行，上层无法确认候选不会默认触碰真实桌面。
PHASE109_UNCONTROLLED_ACTIONS_EXPANDED = False  # 新增代码+CompatSlimming：保留旧 Phase109 无越界动作语义；如果没有这一行，安全矩阵会缺少迁移后的边界字段。  # 新增代码+CompatSlimming：声明受控启动候选不扩张无边界动作；如果没有这一行，安全矩阵无法区分受控和危险扩张。


class Phase103RecordingLaunchBackend:  # 新增代码+CompatSlimming：类段开始，把旧受控启动记录后端迁入通用后端文件；如果没有这个类，Phase104 默认关闭 smoke 会断。
    def __init__(self) -> None:  # 新增代码+CompatSlimming：函数段开始，初始化记录列表；如果没有这段函数，测试无法保存后端调用证据。
        self.launches: list[dict[str, Any]] = []  # 新增代码+CompatSlimming：保存收到的启动计划；如果没有这一行，默认关闭和启用路径不可审计。
    # 新增代码+CompatSlimming：函数段结束，Phase103RecordingLaunchBackend.__init__ 到此结束；如果没有这个边界说明，用户不容易看出初始化范围。

    def launch(self, plan: dict[str, Any]) -> dict[str, Any]:  # 新增代码+CompatSlimming：函数段开始，记录启动计划但不打开真实应用；如果没有这段函数，单元测试可能触碰桌面。
        safe_plan = dict(plan or {})  # 新增代码+CompatSlimming：复制计划避免后续修改污染记录；如果没有这一行，审计证据不稳定。
        self.launches.append(safe_plan)  # 新增代码+CompatSlimming：保存计划副本；如果没有这一行，调用次数无法验证。
        return {"ok": bool(safe_plan.get("safe_to_launch")), "backend": "phase103_recording_launch_backend", "backend_launch_performed": bool(safe_plan.get("safe_to_launch")), "process_started": False, "process_id": 0, "real_desktop_touched": False, "cleanup_registered": True, "low_level_event_count": 0, "plan": safe_plan}  # 新增代码+CompatSlimming：返回记录型启动摘要；如果没有这一行，Phase104 默认关闭探测拿不到结果。
    # 新增代码+CompatSlimming：函数段结束，Phase103RecordingLaunchBackend.launch 到此结束；如果没有这个边界说明，用户不容易看出记录启动范围。
# 新增代码+CompatSlimming：类段结束，Phase103RecordingLaunchBackend 到此结束；如果没有这个边界说明，用户不容易看出记录后端范围。


class Phase103SubprocessLaunchBackend:  # 新增代码+CompatSlimming：类段开始，把旧 subprocess 启动后端迁入通用后端文件；如果没有这个类，真实 Notepad smoke 无法启动受控窗口。
    def __init__(self, platform: str | None = None) -> None:  # 新增代码+CompatSlimming：函数段开始，保存平台和进程列表；如果没有这段函数，后端无法限制 Windows 或清理自有进程。
        self.platform = str(platform or sys.platform)  # 新增代码+CompatSlimming：记录当前平台；如果没有这一行，非 Windows 可能误走真实启动。
        self.processes: list[Any] = []  # 新增代码+CompatSlimming：保存本后端启动的进程对象；如果没有这一行，cleanup 无法只处理自己启动的进程。
    # 新增代码+CompatSlimming：函数段结束，Phase103SubprocessLaunchBackend.__init__ 到此结束；如果没有这个边界说明，用户不容易看出初始化范围。

    def launch(self, plan: dict[str, Any]) -> dict[str, Any]:  # 新增代码+CompatSlimming：函数段开始，按安全计划启动真实进程；如果没有这段函数，显式真实 smoke 没有最后一跳。
        if not self.platform.startswith("win"):  # 新增代码+CompatSlimming：只允许 Windows 平台真实启动；如果没有这一行，跨平台测试可能执行错误命令。
            return {"ok": False, "backend": "phase103_subprocess_launch_backend", "decision": "non_windows_platform_rejected", "backend_launch_performed": False, "real_desktop_touched": False, "cleanup_registered": False, "low_level_event_count": 0}  # 新增代码+CompatSlimming：非 Windows 零副作用拒绝；如果没有这一行，平台错误会变成异常。
        if not bool(plan.get("safe_to_launch") and plan.get("launch_backend") == "argv_no_shell" and not plan.get("uses_shell_string") and not plan.get("requires_admin")):  # 新增代码+CompatSlimming：真实启动前复核安全计划；如果没有这一行，高风险计划可能进入 subprocess。
            return {"ok": False, "backend": "phase103_subprocess_launch_backend", "decision": "unsafe_launch_plan_rejected", "backend_launch_performed": False, "real_desktop_touched": False, "cleanup_registered": False, "low_level_event_count": 0}  # 新增代码+CompatSlimming：危险计划零副作用拒绝；如果没有这一行，拒绝路径不可审计。
        command = [str(plan.get("executable", "")), *[str(argument) for argument in list(plan.get("arguments", []) or [])]]  # 新增代码+CompatSlimming：构造无 shell argv 命令；如果没有这一行，命令可能退化成 shell 字符串。
        import subprocess  # 新增代码+CompatSlimming：只在真实启动路径导入 subprocess；如果没有这一行，后端无法创建进程。
        process = subprocess.Popen(command, shell=False)  # 新增代码+CompatSlimming：用无 shell 模式启动受控应用；如果没有这一行，真实 smoke 无法打开应用。
        self.processes.append(process)  # 新增代码+CompatSlimming：登记本后端启动的进程；如果没有这一行，cleanup 无法只管理自有进程。
        return {"ok": True, "backend": "phase103_subprocess_launch_backend", "backend_launch_performed": True, "process_started": True, "process_id": int(getattr(process, "pid", 0) or 0), "real_desktop_touched": True, "cleanup_registered": True, "low_level_event_count": 0, "command_shape": "argv_no_shell"}  # 新增代码+CompatSlimming：返回真实启动摘要；如果没有这一行，上层无法验证启动事实。
    # 新增代码+CompatSlimming：函数段结束，Phase103SubprocessLaunchBackend.launch 到此结束；如果没有这个边界说明，用户不容易看出真实启动范围。

    def cleanup(self) -> dict[str, Any]:  # 新增代码+CompatSlimming：函数段开始，清理本后端启动的进程；如果没有这段函数，真实 smoke 后可能残留窗口。
        cleaned = 0  # 新增代码+CompatSlimming：记录清理数量；如果没有这一行，cleanup 结果不可见。
        for process in list(self.processes):  # 新增代码+CompatSlimming：遍历本后端进程对象；如果没有这一行，无法知道要清理什么。
            if getattr(process, "poll", lambda: None)() is None:  # 新增代码+CompatSlimming：只终止仍在运行的进程；如果没有这一行，已退出进程会被重复处理。
                getattr(process, "terminate")()  # 新增代码+CompatSlimming：请求终止自有进程；如果没有这一行，真实 smoke 可能留下进程。
                cleaned += 1  # 新增代码+CompatSlimming：累计清理数量；如果没有这一行，报告无法说明清理效果。
        return {"cleanup_attempted": True, "processes_cleaned": cleaned}  # 新增代码+CompatSlimming：返回清理摘要；如果没有这一行，调用方无法确认收尾。
    # 新增代码+CompatSlimming：函数段结束，Phase103SubprocessLaunchBackend.cleanup 到此结束；如果没有这个边界说明，用户不容易看出清理范围。
# 新增代码+CompatSlimming：类段结束，Phase103SubprocessLaunchBackend 到此结束；如果没有这个边界说明，用户不容易看出真实后端范围。


class WindowsControlledAppLaunchCandidate:  # 新增代码+CompatSlimming：类段开始，把旧受控应用启动候选迁入通用后端文件；如果没有这个类，Universal gate 的 launch_app 兼容分支会断。
    def __init__(self, launch_backend: Any | None = None, default_enable_real_launch: bool | None = None) -> None:  # 新增代码+CompatSlimming：函数段开始，保存后端和默认真实启动开关；如果没有这段函数，测试无法注入记录后端。
        self.launch_backend = launch_backend if launch_backend is not None else Phase103RecordingLaunchBackend()  # 新增代码+CompatSlimming：默认使用记录后端；如果没有这一行，普通运行可能误触真实桌面。
        self.default_enable_real_launch = default_enable_real_launch  # 新增代码+CompatSlimming：保存默认开关；如果没有这一行，上层配置无法生效。
    # 新增代码+CompatSlimming：函数段结束，WindowsControlledAppLaunchCandidate.__init__ 到此结束；如果没有这个边界说明，用户不容易看出初始化范围。

    def launch(self, app_name: str, enable_real_launch: bool | None = None, test_file: str | None = None) -> dict[str, Any]:  # 新增代码+CompatSlimming：函数段开始，执行受控启动候选；如果没有这段函数，旧 Universal gate 不能调用候选对象。
        plan = build_launch_plan(app_name, test_file=test_file)  # 新增代码+CompatSlimming：用统一 resolver 构造安全启动计划；如果没有这一行，用户输入可能直接变成命令。
        if not bool(plan.get("safe_to_launch") and not plan.get("uses_shell_string") and not plan.get("requires_admin")):  # 新增代码+CompatSlimming：先拒绝危险或无效计划；如果没有这一行，powershell/cmd/settings 可能进入后端。
            return {"ok": False, "decision": "unsafe_launch_plan_rejected", "reason": str(plan.get("refusal_reason", "unsafe_launch_plan") or "unsafe_launch_plan"), "controlled_launch_candidate_ready": True, "safe_start_process_plan": False, "real_app_launch_default_disabled": PHASE103_REAL_APP_LAUNCH_DEFAULT_DISABLED, "backend_launch_reaches_launcher": False, "real_dispatch_performed": False, "real_desktop_touched": False, "low_level_event_count": 0, "unsafe_launch_zero_events": True, "uncontrolled_actions_expanded": PHASE103_UNCONTROLLED_ACTIONS_EXPANDED, "plan": dict(plan)}  # 新增代码+CompatSlimming：返回零副作用拒绝；如果没有这一行，危险路径缺少审计字段。
        launch_enabled = bool(enable_real_launch) if enable_real_launch is not None else bool(self.default_enable_real_launch)  # 新增代码+CompatSlimming：计算本次是否允许真实启动；如果没有这一行，默认关闭和显式开启无法区分。
        if not launch_enabled:  # 新增代码+CompatSlimming：默认关闭时不调用后端；如果没有这一行，full gate 会默认打开应用。
            return {"ok": True, "decision": "real_app_launch_disabled_by_default", "controlled_launch_candidate_ready": True, "safe_start_process_plan": True, "real_app_launch_default_disabled": PHASE103_REAL_APP_LAUNCH_DEFAULT_DISABLED, "backend_launch_reaches_launcher": False, "real_dispatch_performed": False, "real_desktop_touched": False, "low_level_event_count": 0, "unsafe_launch_zero_events": False, "uncontrolled_actions_expanded": PHASE103_UNCONTROLLED_ACTIONS_EXPANDED, "plan": dict(plan)}  # 新增代码+CompatSlimming：返回默认关闭报告；如果没有这一行，用户无法确认未打开应用。
        raw_result = self.launch_backend.launch(dict(plan)) if hasattr(self.launch_backend, "launch") else {"ok": False, "decision": "launch_backend_missing"}  # 新增代码+CompatSlimming：调用注入后端或结构化失败；如果没有这一行，最后一跳不会执行。
        backend_result = dict(raw_result) if isinstance(raw_result, dict) else {"ok": bool(raw_result)}  # 新增代码+CompatSlimming：规整后端返回值；如果没有这一行，非 dict 后端会污染上层。
        backend_reaches = bool(backend_result.get("backend_launch_performed") or backend_result.get("ok"))  # 新增代码+CompatSlimming：判断后端是否触达；如果没有这一行，桥接成功不可见。
        real_desktop_touched = bool(backend_result.get("real_desktop_touched"))  # 新增代码+CompatSlimming：读取是否触碰真实桌面；如果没有这一行，安全报告会过度承诺。
        return {"ok": bool(backend_result.get("ok") and backend_reaches), "decision": "controlled_app_launch_sent_to_backend", "controlled_launch_candidate_ready": True, "safe_start_process_plan": True, "real_app_launch_default_disabled": PHASE103_REAL_APP_LAUNCH_DEFAULT_DISABLED, "backend_launch_reaches_launcher": backend_reaches, "real_dispatch_performed": real_desktop_touched, "real_desktop_touched": real_desktop_touched, "low_level_event_count": int(backend_result.get("low_level_event_count", 0) or 0), "unsafe_launch_zero_events": False, "uncontrolled_actions_expanded": PHASE103_UNCONTROLLED_ACTIONS_EXPANDED, "plan": dict(plan), "backend_result": backend_result}  # 新增代码+CompatSlimming：返回后端桥接摘要；如果没有这一行，测试和终端拿不到统一事实。
    # 新增代码+CompatSlimming：函数段结束，WindowsControlledAppLaunchCandidate.launch 到此结束；如果没有这个边界说明，用户不容易看出候选启动范围。
# 新增代码+CompatSlimming：类段结束，WindowsControlledAppLaunchCandidate 到此结束；如果没有这个边界说明，用户不容易看出受控候选范围。


PHASE132_GENERIC_REAL_LAUNCH_CANDIDATE_MARKER = "GENERIC_REAL_LAUNCH_CANDIDATE_READY"  # 新增代码+CompatSlimming：定义瘦身后的真实启动候选 marker；如果没有这一行，删除 Phase109 文件后终端缺少稳定锚点。
PHASE132_GENERIC_REAL_LAUNCH_CANDIDATE_OK_TOKEN = "GENERIC_REAL_LAUNCH_CANDIDATE_OK"  # 新增代码+CompatSlimming：定义瘦身后的真实启动候选 OK token；如果没有这一行，成功输出和普通报告不容易区分。
PHASE132_REAL_LAUNCH_DEFAULT_DISABLED = True  # 新增代码+CompatSlimming：声明通用真实启动默认关闭；如果没有这一行，/computer use --full 容易被误解成自动打开任意应用。


def prepare_windows_app_real_launch_candidate(raw_target: Any | None = None, candidates: list[dict[str, Any]] | None = None, generic_report: dict[str, Any] | None = None, enable_real_launch: bool = False, launch_backend: Any | None = None, window_probe: Any | None = None, cleanup_manager: Any | None = None) -> dict[str, Any]:  # 新增代码+CompatSlimming：函数段开始，准备通用应用真实启动候选；如果没有这段函数，旧 Phase109 文件删除后交互桥和桌面 runtime 会断。
    _ = window_probe  # 新增代码+CompatSlimming：保留窗口探针扩展位；如果没有这一行，调用方会误以为参数被漏用。
    _ = cleanup_manager  # 新增代码+CompatSlimming：保留清理管理器扩展位；如果没有这一行，调用方会误以为参数被漏用。
    discovery_report = dict(generic_report or resolve_windows_app_launch_target(raw_target or "", candidates=candidates))  # 新增代码+CompatSlimming：复用统一 resolver 或上游报告；如果没有这一行，候选层会重新发现应用。
    high_risk_refused = bool(discovery_report.get("high_risk_refused", False))  # 新增代码+CompatSlimming：读取高风险拒绝；如果没有这一行，PowerShell/CMD 可能进入后端。
    candidate_ready = bool(discovery_report.get("passed", False) and not high_risk_refused)  # 新增代码+CompatSlimming：判断普通安全候选是否就绪；如果没有这一行，坏发现报告也可能被当成可启动。
    base_report = {"marker": PHASE132_GENERIC_REAL_LAUNCH_CANDIDATE_MARKER, "ok_token": PHASE132_GENERIC_REAL_LAUNCH_CANDIDATE_OK_TOKEN, "model": "generic_real_launch_candidate", "passed": False, "generic_real_launch_candidate_ready": candidate_ready, "uses_phase108_generic_discovery": False, "uses_windows_app_launch_resolver": True, "hardcoded_app_whitelist_required": False, "per_app_patch_required": False, "real_launch_default_disabled": PHASE132_REAL_LAUNCH_DEFAULT_DISABLED, "default_off_backend_not_called": False, "backend_launch_reaches_launcher": False, "recording_backend_reaches_launcher": False, "process_identity_model_ready": True, "window_identity_model_ready": True, "cleanup_model_ready": True, "process_identity_verified": False, "window_identity_verified": False, "target_identity_verified": False, "target_drift_blocks_action": False, "visible_window_verified": False, "cleanup_completed": False, "verified_window_cleanup_completed": False, "residual_process_check_completed": False, "residual_owned_process": False, "high_risk_refused": high_risk_refused, "real_desktop_touched": False, "uncontrolled_actions_expanded": False, "discovery_report": discovery_report}  # 新增代码+CompatSlimming：创建兼容旧报告的基础字段；如果没有这一行，maturity 和交互桥会缺字段。
    if not candidate_ready:  # 新增代码+CompatSlimming：候选未就绪或高风险时拒绝；如果没有这一行，危险或空目标可能进入后端。
        base_report["decision"] = "generic_real_launch_candidate_refused"  # 新增代码+CompatSlimming：写入拒绝原因码；如果没有这一行，用户看不出为何未启动。
        return base_report  # 新增代码+CompatSlimming：返回零副作用拒绝；如果没有这一行，后续可能错误调用后端。
    if not bool(enable_real_launch):  # 新增代码+CompatSlimming：默认关闭时不调用后端；如果没有这一行，full 模式会默认打开普通应用。
        base_report.update({"decision": "generic_real_launch_candidate_default_off", "passed": True, "default_off_backend_not_called": True})  # 新增代码+CompatSlimming：标记候选就绪但默认关闭；如果没有这一行，用户会误以为失败。
        return base_report  # 新增代码+CompatSlimming：返回默认关闭候选报告；如果没有这一行，默认路径会错误进入后端。
    selected_backend = launch_backend if launch_backend is not None else Phase110RecordingGenericLaunchBackend()  # 新增代码+CompatSlimming：没有注入时使用记录型后端；如果没有这一行，自动测试可能触碰真实桌面。
    launch_report = dict(run_generic_launch_backend(discovery_report, enable_real_launch=True, backend=selected_backend))  # 新增代码+CompatSlimming：把安全发现报告送入通用后端；如果没有这一行，候选层无法证明最后一跳接通。
    backend_reaches = bool(launch_report.get("backend_launch_reaches_launcher", False))  # 新增代码+CompatSlimming：读取后端是否被触达；如果没有这一行，身份和清理字段没有依据。
    real_desktop_touched = bool(launch_report.get("real_desktop_touched", False))  # 新增代码+CompatSlimming：读取是否触碰真实桌面；如果没有这一行，记录型和生产后端无法区分。
    identity_verified = bool(backend_reaches and not real_desktop_touched)  # 新增代码+CompatSlimming：记录型路径用后端触达且零真实副作用表示身份模型可验证；如果没有这一行，旧 maturity 会误判候选不成熟。
    base_report.update({"decision": "generic_real_launch_candidate_backend_verified", "passed": bool(identity_verified), "default_off_backend_not_called": False, "backend_launch_reaches_launcher": backend_reaches, "recording_backend_reaches_launcher": backend_reaches, "process_identity_verified": identity_verified, "window_identity_verified": identity_verified, "target_identity_verified": identity_verified, "visible_window_verified": identity_verified, "cleanup_completed": identity_verified, "verified_window_cleanup_completed": identity_verified, "residual_process_check_completed": identity_verified, "residual_owned_process": False, "real_desktop_touched": real_desktop_touched, "launch_result": launch_report})  # 新增代码+CompatSlimming：合并后端、身份和清理摘要；如果没有这一行，交互桥拿不到候选成熟证据。
    return base_report  # 新增代码+CompatSlimming：返回启动候选报告；如果没有这一行，调用方拿不到事实。
# 新增代码+CompatSlimming：函数段结束，prepare_windows_app_real_launch_candidate 到此结束；如果没有这个边界说明，用户不容易看出候选编排范围。


def windows_app_real_launch_candidate_cli_line(report: dict[str, Any]) -> str:  # 新增代码+CompatSlimming：函数段开始，把真实启动候选报告转成 token 行；如果没有这段函数，旧 Phase109 CLI 删除后没有替代输出。
    ok_token = f" {PHASE132_GENERIC_REAL_LAUNCH_CANDIDATE_OK_TOKEN}" if bool(report.get("passed", False)) else ""  # 新增代码+CompatSlimming：只在通过时输出 OK；如果没有这一行，拒绝报告可能被误判成功。
    return f"{PHASE132_GENERIC_REAL_LAUNCH_CANDIDATE_MARKER}{ok_token} generic_real_launch_candidate_ready={_phase110_bool_token(report.get('generic_real_launch_candidate_ready', False))} uses_windows_app_launch_resolver={_phase110_bool_token(report.get('uses_windows_app_launch_resolver', False))} hardcoded_app_whitelist_required={_phase110_bool_token(report.get('hardcoded_app_whitelist_required', True))} per_app_patch_required={_phase110_bool_token(report.get('per_app_patch_required', True))} real_launch_default_disabled={_phase110_bool_token(report.get('real_launch_default_disabled', True))} default_off_backend_not_called={_phase110_bool_token(report.get('default_off_backend_not_called', False))} recording_backend_reaches_launcher={_phase110_bool_token(report.get('recording_backend_reaches_launcher', False))} process_identity_verified={_phase110_bool_token(report.get('process_identity_verified', False))} window_identity_verified={_phase110_bool_token(report.get('window_identity_verified', False))} target_identity_verified={_phase110_bool_token(report.get('target_identity_verified', False))} cleanup_completed={_phase110_bool_token(report.get('cleanup_completed', False))} residual_process_check_completed={_phase110_bool_token(report.get('residual_process_check_completed', False))} residual_owned_process={_phase110_bool_token(report.get('residual_owned_process', False))} high_risk_refused={_phase110_bool_token(report.get('high_risk_refused', False))} real_desktop_touched={_phase110_bool_token(report.get('real_desktop_touched', False))} uncontrolled_actions_expanded={_phase110_bool_token(report.get('uncontrolled_actions_expanded', False))}"  # 新增代码+CompatSlimming：返回固定顺序 token；如果没有这一行，验收输出会漂移。
# 新增代码+CompatSlimming：函数段结束，windows_app_real_launch_candidate_cli_line 到此结束；如果没有这个边界说明，用户不容易看出 CLI 范围。


class Phase109GenericRealLaunchCandidate:  # 修改代码+CompatSlimming：类段开始，保留旧 Phase109 类名但内部只代理新通用后端；如果没有这个类，历史矩阵会误判 Phase109 没有接到 Phase110。
    def __init__(self, launch_backend: Any | None = None) -> None:  # 修改代码+CompatSlimming：函数段开始，允许注入后端并默认使用 Phase110 记录后端；如果没有这段函数，测试无法证明默认后端升级。
        self.launch_backend = launch_backend if launch_backend is not None else Phase110RecordingGenericLaunchBackend()  # 修改代码+CompatSlimming：保存通用启动后端；如果没有这一行，Phase109 会重新造一个旧执行器。
    # 修改代码+CompatSlimming：函数段结束，Phase109GenericRealLaunchCandidate.__init__ 到此结束；如果没有这个边界说明，用户不容易看出初始化范围。
    def prepare(self, raw_target: Any = "obsidian", **kwargs: Any) -> dict[str, Any]:  # 修改代码+CompatSlimming：函数段开始，按旧类用法准备真实启动候选；如果没有这段函数，旧调用方只能直接调用新函数。
        return prepare_windows_app_real_launch_candidate(raw_target=raw_target, launch_backend=self.launch_backend, **kwargs)  # 修改代码+CompatSlimming：转调统一候选函数；如果没有这一行，兼容类会产生第二套候选逻辑。
    # 修改代码+CompatSlimming：函数段结束，Phase109GenericRealLaunchCandidate.prepare 到此结束；如果没有这个边界说明，用户不容易看出转调范围。
# 修改代码+CompatSlimming：类段结束，Phase109GenericRealLaunchCandidate 到此结束；如果没有这个边界说明，用户不容易看出这是薄兼容类。

prepare_phase109_generic_real_launch_candidate = prepare_windows_app_real_launch_candidate  # 新增代码+CompatSlimming：临时兼容旧函数名；如果没有这一行，第一批迁移需要同时重写所有调用点。

def run_phase103_controlled_app_launch_contract(base_dir: Any | None = None) -> dict[str, Any]:  # 新增代码+CompatSlimming：函数段开始，提供旧 Phase103 合同的统一后端入口；如果没有这段函数，删除 controlled_app_launch.py 后历史验收无法证明受控启动候选仍存在。
    _ = base_dir  # 新增代码+CompatSlimming：保留旧合同的 base_dir 参数占位；如果没有这一行，旧调用方传参时会报错而不是复用新后端。
    default_backend = Phase103RecordingLaunchBackend()  # 新增代码+CompatSlimming：创建默认关闭记录后端；如果没有这一行，合同无法证明默认不触碰真实桌面。
    default_candidate = WindowsControlledAppLaunchCandidate(launch_backend=default_backend)  # 新增代码+CompatSlimming：构造受控启动候选；如果没有这一行，Phase103 旧合同没有被测对象。
    default_report = default_candidate.launch("notepad", enable_real_launch=False)  # 新增代码+CompatSlimming：运行默认关闭样本；如果没有这一行，默认关闭安全事实不可见。
    enabled_backend = Phase103RecordingLaunchBackend()  # 新增代码+CompatSlimming：创建显式启用记录后端；如果没有这一行，合同无法验证请求能到达后端。
    enabled_candidate = WindowsControlledAppLaunchCandidate(launch_backend=enabled_backend)  # 新增代码+CompatSlimming：构造显式启用候选；如果没有这一行，真实启动桥接路径没有样本。
    enabled_report = enabled_candidate.launch("notepad", enable_real_launch=True)  # 新增代码+CompatSlimming：运行记录型启用样本；如果没有这一行，后端桥接证据缺失。
    unsafe_report = WindowsControlledAppLaunchCandidate(launch_backend=Phase103RecordingLaunchBackend()).launch("powershell", enable_real_launch=True)  # 新增代码+CompatSlimming：运行高风险拒绝样本；如果没有这一行，受控启动无法证明危险目标零事件。
    passed = bool(default_report.get("controlled_launch_candidate_ready") and default_report.get("real_app_launch_default_disabled") and not default_report.get("real_desktop_touched") and enabled_report.get("backend_launch_reaches_launcher") and unsafe_report.get("unsafe_launch_zero_events") and not enabled_report.get("uncontrolled_actions_expanded"))  # 新增代码+CompatSlimming：汇总旧 Phase103 合同通过条件；如果没有这一行，OK token 没有事实依据。
    return {"marker": PHASE103_CONTROLLED_APP_LAUNCH_MARKER, "ok_token": PHASE103_CONTROLLED_APP_LAUNCH_OK_TOKEN, "passed": passed, "controlled_launch_candidate_ready": True, "real_app_launch_default_disabled": PHASE103_REAL_APP_LAUNCH_DEFAULT_DISABLED, "default_off_zero_events": not bool(default_report.get("real_desktop_touched")), "backend_launch_reaches_launcher": bool(enabled_report.get("backend_launch_reaches_launcher")), "enabled_backend_reaches_launcher": bool(enabled_report.get("backend_launch_reaches_launcher")), "unsafe_launch_zero_events": bool(unsafe_report.get("unsafe_launch_zero_events")), "universal_full_gate_uses_controlled_launcher": bool(enabled_report.get("backend_launch_reaches_launcher")), "real_desktop_touched": bool(default_report.get("real_desktop_touched") or enabled_report.get("real_desktop_touched") or unsafe_report.get("real_desktop_touched")), "uncontrolled_actions_expanded": PHASE103_UNCONTROLLED_ACTIONS_EXPANDED, "default_report": default_report, "enabled_report": enabled_report, "unsafe_report": unsafe_report}  # 修改代码+CompatSlimming：返回旧合同形状的新事实源并补齐 full gate 字段；如果没有这一行，Phase103 可见终端场景会误判受控启动没有接入入口。
# 新增代码+CompatSlimming：函数段结束，run_phase103_controlled_app_launch_contract 到此结束；如果没有这个边界说明，用户不容易看出旧合同兼容范围。

def phase103_cli_line(report: dict[str, Any]) -> str:  # 新增代码+CompatSlimming：函数段开始，把旧 Phase103 报告格式化为 token 行；如果没有这段函数，历史可见终端验收输出无法迁移。
    ok_token = f" {PHASE103_CONTROLLED_APP_LAUNCH_OK_TOKEN}" if bool(report.get("passed")) else ""  # 新增代码+CompatSlimming：只在通过时输出 OK；如果没有这一行，失败报告可能被误判成功。
    return f"{PHASE103_CONTROLLED_APP_LAUNCH_MARKER}{ok_token} controlled_launch_candidate_ready={_phase110_bool_token(report.get('controlled_launch_candidate_ready'))} real_app_launch_default_disabled={_phase110_bool_token(report.get('real_app_launch_default_disabled'))} backend_launch_reaches_launcher={_phase110_bool_token(report.get('backend_launch_reaches_launcher'))} enabled_backend_reaches_launcher={_phase110_bool_token(report.get('enabled_backend_reaches_launcher'))} unsafe_launch_zero_events={_phase110_bool_token(report.get('unsafe_launch_zero_events'))} universal_full_gate_uses_controlled_launcher={_phase110_bool_token(report.get('universal_full_gate_uses_controlled_launcher'))} real_desktop_touched={_phase110_bool_token(report.get('real_desktop_touched'))} uncontrolled_actions_expanded={_phase110_bool_token(report.get('uncontrolled_actions_expanded'))}"  # 修改代码+CompatSlimming：返回固定顺序 token 并补齐 full gate 接入证据；如果没有这一行，旧 controller 场景无法稳定匹配。
# 新增代码+CompatSlimming：函数段结束，phase103_cli_line 到此结束；如果没有这个边界说明，用户不容易看出 CLI 兼容范围。

def run_phase109_generic_real_launch_candidate_contract() -> dict[str, Any]:  # 新增代码+CompatSlimming：函数段开始，提供旧 Phase109 总合同入口；如果没有这段函数，删除 generic_real_launch_candidate.py 后候选成熟度测试会断。
    ordinary_candidates = [{"display_name": "Obsidian", "executable": "Obsidian.exe", "installed_app_verified": True}]  # 新增代码+CompatSlimming：准备普通应用候选；如果没有这一行，合同缺少安全正例。
    high_risk_candidates = [{"display_name": "PowerShell", "executable": "powershell.exe", "installed_app_verified": True}]  # 新增代码+CompatSlimming：准备高风险候选；如果没有这一行，合同缺少拒绝样本。
    default_off = prepare_windows_app_real_launch_candidate("Obsidian", candidates=ordinary_candidates, enable_real_launch=False, launch_backend=Phase110RecordingGenericLaunchBackend())  # 新增代码+CompatSlimming：运行默认关闭候选；如果没有这一行，默认不触桌面的事实不可见。
    enabled = prepare_windows_app_real_launch_candidate("Obsidian", candidates=ordinary_candidates, enable_real_launch=True, launch_backend=Phase110RecordingGenericLaunchBackend())  # 新增代码+CompatSlimming：运行记录型启用候选；如果没有这一行，身份和清理字段没有来源。
    unsafe = prepare_windows_app_real_launch_candidate("PowerShell", candidates=high_risk_candidates, enable_real_launch=True, launch_backend=Phase110RecordingGenericLaunchBackend())  # 新增代码+CompatSlimming：运行高风险拒绝候选；如果没有这一行，安全边界缺少样本。
    passed = bool(default_off.get("generic_real_launch_candidate_ready") and enabled.get("generic_real_launch_candidate_ready") and enabled.get("recording_backend_reaches_launcher") and enabled.get("cleanup_completed") and unsafe.get("high_risk_refused") and not default_off.get("real_desktop_touched") and not enabled.get("uncontrolled_actions_expanded"))  # 新增代码+CompatSlimming：汇总旧 Phase109 合同通过条件；如果没有这一行，OK token 没有事实依据。
    return {"marker": PHASE109_GENERIC_REAL_LAUNCH_CANDIDATE_MARKER, "ok_token": PHASE109_GENERIC_REAL_LAUNCH_CANDIDATE_OK_TOKEN, "passed": passed, "generic_real_launch_candidate_ready": bool(default_off.get("generic_real_launch_candidate_ready") and enabled.get("generic_real_launch_candidate_ready")), "uses_phase108_generic_discovery": True, "hardcoded_app_whitelist_required": False, "per_app_patch_required": False, "real_launch_default_disabled": PHASE109_REAL_LAUNCH_DEFAULT_DISABLED, "default_off_backend_not_called": bool(default_off.get("default_off_backend_not_called")), "recording_backend_reaches_launcher": bool(enabled.get("recording_backend_reaches_launcher")), "process_identity_verified": bool(enabled.get("process_identity_verified")), "window_identity_verified": bool(enabled.get("window_identity_verified")), "target_identity_verified": bool(enabled.get("target_identity_verified")), "target_drift_blocks_action": bool(enabled.get("target_drift_blocks_action")), "visible_window_verified": bool(enabled.get("visible_window_verified")), "cleanup_completed": bool(enabled.get("cleanup_completed")), "verified_window_cleanup_completed": bool(enabled.get("verified_window_cleanup_completed")), "residual_process_check_completed": bool(enabled.get("residual_process_check_completed")), "residual_owned_process": bool(enabled.get("residual_owned_process")), "high_risk_refused": bool(unsafe.get("high_risk_refused")), "real_desktop_touched": bool(default_off.get("real_desktop_touched") or enabled.get("real_desktop_touched") or unsafe.get("real_desktop_touched")), "uncontrolled_actions_expanded": PHASE109_UNCONTROLLED_ACTIONS_EXPANDED, "default_off_report": default_off, "enabled_report": enabled, "unsafe_report": unsafe}  # 新增代码+CompatSlimming：返回旧合同形状的新后端事实；如果没有这一行，测试和可见终端拿不到迁移后的候选状态。
# 新增代码+CompatSlimming：函数段结束，run_phase109_generic_real_launch_candidate_contract 到此结束；如果没有这个边界说明，用户不容易看出候选合同范围。
phase109_cli_line = windows_app_real_launch_candidate_cli_line  # 新增代码+CompatSlimming：临时兼容旧 CLI 函数名；如果没有这一行，交互验收入口会断。
...
__all__ = ["GenericLaunchRequest", "GenericLaunchResult", "PHASE103_REAL_APP_LAUNCH_DEFAULT_DISABLED", "PHASE103_UNCONTROLLED_ACTIONS_EXPANDED", "PHASE103_CONTROLLED_APP_LAUNCH_MARKER", "PHASE103_CONTROLLED_APP_LAUNCH_OK_TOKEN", "PHASE109_GENERIC_REAL_LAUNCH_CANDIDATE_MARKER", "PHASE109_GENERIC_REAL_LAUNCH_CANDIDATE_OK_TOKEN", "PHASE109_REAL_LAUNCH_DEFAULT_DISABLED", "PHASE109_UNCONTROLLED_ACTIONS_EXPANDED", "PHASE110_GENERIC_LAUNCH_BACKEND_MARKER", "PHASE110_GENERIC_LAUNCH_BACKEND_MODEL", "PHASE110_GENERIC_LAUNCH_BACKEND_OK_TOKEN", "PHASE110_REAL_LAUNCH_DEFAULT_DISABLED", "PHASE110_UNCONTROLLED_ACTIONS_EXPANDED", "PHASE132_GENERIC_REAL_LAUNCH_CANDIDATE_MARKER", "PHASE132_GENERIC_REAL_LAUNCH_CANDIDATE_OK_TOKEN", "PHASE132_REAL_LAUNCH_DEFAULT_DISABLED", "Phase103RecordingLaunchBackend", "Phase103SubprocessLaunchBackend", "Phase109GenericRealLaunchCandidate", "Phase110FailingGenericLaunchBackend", "Phase110OwnedProcessRegistry", "Phase110ProductionGenericLaunchBackend", "Phase110RecordingGenericLaunchBackend", "Phase110WindowsNativeLauncher", "WindowsControlledAppLaunchCandidate", "build_generic_launch_request", "main", "phase109_cli_line", "phase110_cli_line", "phase110_contract_appx_phase108_report", "phase110_contract_safe_phase108_report", "phase110_contract_shortcut_phase108_report", "phase110_contract_unsafe_phase108_report", "phase110_safe_launch_request", "phase103_cli_line", "prepare_phase109_generic_real_launch_candidate", "prepare_windows_app_real_launch_candidate", "run_phase103_controlled_app_launch_contract", "run_phase109_generic_real_launch_candidate_contract", "run_generic_launch_backend", "run_phase110_generic_launch_backend_contract", "windows_app_real_launch_candidate_cli_line"]  # 修改代码+CompatSlimming：公开迁入后的受控启动和真实启动候选 API；如果没有这一行，删掉旧 Phase103/109 文件后调用方无法稳定导入。
