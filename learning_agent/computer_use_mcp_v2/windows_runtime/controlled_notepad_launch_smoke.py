...
try:  # 新增代码+Phase104ControlledNotepadLaunchSmoke：优先按包路径导入项目组件；如果没有这段代码，单元测试和真实终端入口不能共享实现。
    from learning_agent.computer_use_mcp_v2.windows_runtime.generic_launch_backend import Phase103RecordingLaunchBackend, Phase103SubprocessLaunchBackend, WindowsControlledAppLaunchCandidate  # 修改代码+CompatSlimming：改为从统一通用启动后端复用 Phase103 启动候选；如果没有这一行，删除旧 controlled_app_launch.py 后 Phase104 真实 Notepad smoke 会导入失败。
    from learning_agent.computer_use_mcp_v2.windows_runtime.persistent_grants import DEFAULT_PERSISTENT_GRANTS_ROOT  # 新增代码+Phase104ControlledNotepadLaunchSmoke：复用 Computer Use memory 根；如果没有这行代码，报告证据会散落。
    from learning_agent.computer_use_mcp_v2.windows_runtime.windows_backend import WindowsWindowInventoryProbe  # 新增代码+Phase104ControlledNotepadLaunchSmoke：复用只读窗口枚举探测真实 Notepad 窗口；如果没有这行代码，真实窗口可见性无法验证。
    ...
    from computer_use_mcp_v2.windows_runtime.generic_launch_backend import Phase103RecordingLaunchBackend, Phase103SubprocessLaunchBackend, WindowsControlledAppLaunchCandidate  # type: ignore  # 修改代码+CompatSlimming：脚本模式从统一启动后端导入 Phase103 候选；如果没有这一行，bat 入口删除旧文件后无法运行 Phase104。
    from computer_use_mcp_v2.windows_runtime.persistent_grants import DEFAULT_PERSISTENT_GRANTS_ROOT  # type: ignore  # 新增代码+Phase104ControlledNotepadLaunchSmoke：脚本模式复用默认 memory 根；如果没有这行代码，报告目录无法稳定定位。
    from computer_use_mcp_v2.windows_runtime.windows_backend import WindowsWindowInventoryProbe  # type: ignore  # 新增代码+Phase104ControlledNotepadLaunchSmoke：脚本模式复用窗口枚举；如果没有这行代码，真实窗口验证无法运行。
