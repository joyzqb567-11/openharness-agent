...
COMPUTER_USE_MOUSE_KEYBOARD_ACTIONS = {"move_mouse", "click", "double_click", "drag_path", "type_text", "press_key", "scroll"}  # 新增代码+AgentPySplitPhase4: 集中保存会真实改变桌面的鼠标键盘动作；如果没有这行代码，多个门禁会各自维护一份动作名单并容易漂移。
COMPUTER_USE_COMPLETION_CHANGE_ACTIONS = {"drag_path", "click", "double_click", "type_text", "press_key", "scroll"}  # 新增代码+AgentPySplitPhase4: 集中保存完成门需要拦截的继续改变桌面动作；如果没有这行代码，完成门会和观察门禁的意图区分不清。
