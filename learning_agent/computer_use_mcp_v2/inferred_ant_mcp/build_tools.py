...
COMPUTER_USE_MCP_TOOL_NAMES = (  # 新增代码+ComputerUseMcpV2：函数段开始，按用户确认顺序列出唯一可见工具；如果没有这段常量，工具面容易多出旧接口。
    ...
    "left_mouse_down",  # 新增代码+ClaudeCodeParity：公开 ClaudeCode parity 左键按下工具；如果没有这一项，拆分式鼠标按下动作无法表达。
    "left_mouse_up",  # 新增代码+ClaudeCodeParity：公开 ClaudeCode parity 左键释放工具；如果没有这一项，拆分式鼠标释放动作无法表达。
...
def computer_use_mcp_tools(app_inventory_hint: str = "") -> list[dict[str, Any]]:  # 修改代码+ClaudeCodeDynamicToolsList：函数段开始，返回 v2 工具清单副本并允许 tools/list 注入安全应用提示；如果没有这段函数，server 和测试没有统一工具面入口。
    ...
    tools = [  # 新增代码+ComputerUseMcpV2：开始构造工具列表；如果没有这行代码，函数没有可返回内容。
        ...
        _tool("left_mouse_down", "在指定坐标按下左键但暂不释放。推荐使用 coordinate: [x, y]；兼容旧 x/y。", {**coordinate, **point, "reason": {"type": "string"}}, [CLAUDECODE_COORDINATE_FIELD]),  # 修改代码+ClaudeCodeProtocolParity：定义 ClaudeCode 风格左键按下 schema；如果没有这行代码，拆分鼠标动作参数不一致。
        _tool("left_mouse_up", "释放当前按下的左键。推荐使用 coordinate: [x, y]；兼容旧 x/y。", {**coordinate, **point, "reason": {"type": "string"}}, [CLAUDECODE_COORDINATE_FIELD]),  # 修改代码+ClaudeCodeProtocolParity：定义 ClaudeCode 风格左键释放 schema；如果没有这行代码，释放点字段会继续旧格式。
        ...
        _tool("left_click_drag", "从起点按住左键拖拽到终点。推荐使用 start_coordinate 和 coordinate；兼容旧 start_x/start_y/end_x/end_y。", {**start_coordinate, **coordinate, "start_x": {"type": "integer"}, "start_y": {"type": "integer"}, "end_x": {"type": "integer"}, "end_y": {"type": "integer"}, CLAUDECODE_DURATION_FIELD: {"type": "number"}, "duration_seconds": {"type": "number"}, "reason": {"type": "string"}}, [CLAUDECODE_START_COORDINATE_FIELD, CLAUDECODE_COORDINATE_FIELD]),  # 修改代码+ClaudeCodeProtocolParity：定义 ClaudeCode 风格拖拽 schema；如果没有这行代码，拖拽仍和 ClaudeCode 字段不一致。
