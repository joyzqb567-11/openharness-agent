...
def computer_use_mcp_tools() -> list[dict[str, Any]]:  # 新增代码+ComputerUseMcpV2：函数段开始，返回 v2 工具清单副本；如果没有这段函数，server 和测试没有统一工具面入口。
    ...
    tools = [  # 新增代码+ComputerUseMcpV2：开始构造工具列表；如果没有这行代码，函数没有可返回内容。
        ...
        _tool("hold_key", "按住指定按键一段时间。", {"key": {"type": "string"}, "duration_seconds": {"type": "number"}, "reason": {"type": "string"}}, ["key"]),  # 新增代码+ClaudeCodeParity：定义按住按键 schema；如果没有这行代码，长按动作没有参数契约。
        ...
        _tool("left_click_drag", "从起点按住左键拖拽到终点。", {"start_x": {"type": "integer"}, "start_y": {"type": "integer"}, "end_x": {"type": "integer"}, "end_y": {"type": "integer"}, "duration_seconds": {"type": "number"}, "reason": {"type": "string"}}, ["start_x", "start_y", "end_x", "end_y"]),  # 新增代码+ClaudeCodeParity：定义左键拖拽 schema；如果没有这行代码，拖拽动作没有起点终点契约。
        _tool("zoom", "读取并放大指定屏幕区域。", {"x": {"type": "integer"}, "y": {"type": "integer"}, "width": {"type": "integer"}, "height": {"type": "integer"}, "reason": {"type": "string"}}, ["x", "y", "width", "height"], read_only=True),  # 新增代码+ClaudeCodeParity：定义只读局部放大 schema；如果没有这行代码，zoom 无法作为安全观察工具进入 MCP 表面。
