...
def request_access(context: ComputerUseMcpV2Context, arguments: dict[str, Any]) -> dict[str, Any]:  # 新增代码+ComputerUseMcpV2：函数段开始，执行授权申请；如果没有这段函数，模型无法通过 v2 请求权限。
    ...
    prompt = "Computer Use v2 请求控制应用：\n" + json.dumps({"apps": apps, "applications": applications, "grantFlags": grant_flags, "sentinelWarnings": sentinel_warnings, "reason": reason}, ensure_ascii=False, indent=2)  # 修改代码+ClaudeCodePermissionParity：生成包含 allowlist、flags 和风险提示的授权提示；如果没有这行代码，用户看不到完整授权范围。
