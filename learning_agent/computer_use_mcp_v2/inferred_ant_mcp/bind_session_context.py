...
def _display_state_from_agent(agent: Any) -> dict[str, Any]:  # 新增代码+ClaudeCodeDisplayParity：函数段开始，从 agent 读取可选 display 初始状态；如果没有这段函数，context 每次新建都会丢失主循环已知的屏幕状态。
    ...
    return {"selected_display_id": selected_display_id, "display_pinned_by_model": display_pinned_by_model, "display_resolved_for_apps": safe_resolved_for_apps, "last_screenshot_dims": safe_screenshot_dims}  # 新增代码+ClaudeCodeDisplayParity：返回可展开到 dataclass 的 display 字段；如果没有这行代码，绑定层无法保持初始化代码简洁。
...
def bind_session_context(agent: Any) -> ComputerUseMcpV2Context:  # 修改代码+ClaudeCodeLockParity：函数段开始，从 agent 构建或复用 v2 context；如果没有这段函数，agent-side wrapper 会重复拼装回调并丢失状态。
    ...
    context = ComputerUseMcpV2Context(host=host, ask_permission=ask_permission, record_observation=record_observation, record_runtime_trace=record_runtime_trace, emit_acceptance_event=emit_acceptance, **lock_callbacks, **display_state)  # 修改代码+ClaudeCodeDisplayParity：创建绑定主循环能力、锁生命周期和 display 状态的 v2 context；如果没有这一行，v2 工具只能孤立运行且不会按 ClaudeCode display state 语义执行。
