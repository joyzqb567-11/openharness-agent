...
from typing import Any, Callable  # 修改代码+PermissionUIDecision：导入通用类型和回调类型；如果没有这行代码，结构化权限回调和 agent-side 绑定字段含义不清楚。
...
class ComputerUseMcpV2Context:  # 新增代码+ComputerUseMcpV2：类段开始，保存一次 v2 MCP 调用链共享的宿主、权限、trace 和状态；如果没有这段类，stdio server 和 agent-side wrapper 无法共享能力。
    ...
    ask_permission: Callable[[str], Any] | None = None  # 修改代码+PermissionUIDecision：保存 agent 主循环授权回调并允许返回 bool 或结构化 dict；如果没有这一行，request_access 不能复用用户确认能力或未来权限 UI。
    ...
    mark_expected_escape: Callable[[str, int], dict[str, Any]] | None = None  # 新增代码+ClaudeCodeLifecycleParity：保存模型计划内 Escape 的声明回调；如果没有这一行，模型自己发送 Esc 可能被误判成用户急停。
    is_computer_use_disabled: Callable[[], bool] | None = None  # 新增代码+ClaudeCodeLifecycleParity：保存 Computer Use 模式级禁用判断；如果没有这一行，standalone tools/list 不能对齐 ClaudeCode disabled 空工具列表。
    is_computer_use_context_disabled: Callable[[], bool] | None = None  # 新增代码+ClaudeCodeLifecycleParity：保存当前上下文禁用判断；如果没有这一行，单轮禁用时仍可能向模型暴露工具。
    ...
    permission_decisions: list[dict[str, Any]] = field(default_factory=list)  # 新增代码+PermissionUIDecision：保存本会话权限决策审计列表；如果没有这一行，list_granted_applications 无法解释最近允许或拒绝来源。
    ...
    display_resolved_for_apps_key: str = ""  # 新增代码+ClaudeCodeLifecycleParity：保存 ClaudeCode 外部协议使用的 app key 字符串；如果没有这一行，context 重建后无法恢复 displayResolvedForApps 字符串形态。
    ...
    element_cache: Any | None = None  # 新增代码+CuaDriverBorrowing：保存观察帧生成的元素索引缓存；如果没有这一行，动作层无法按 element_index 找回 UIA 元素。
    semantic_action_dispatcher: Callable[[str, Any, dict[str, Any]], dict[str, Any]] | None = None  # 新增代码+CuaDriverBorrowing：保存可替换的语义动作分发器；如果没有这一行，测试和运行时无法注入更接近 Cua Driver 的 UIA Pattern 路径。
    integrity_diagnostic_provider: Callable[[dict[str, Any]], dict[str, Any]] | None = None  # 新增代码+CuaDriverBorrowing：保存可选完整性诊断回调；如果没有这一行，动作层无法把后台派发失败解释为 UIPI 或目标不可用。
    last_observation_target: dict[str, Any] = field(default_factory=dict)  # 新增代码+CuaDriverBorrowing：保存最近观察目标的 pid/window_id 摘要；如果没有这一行，后续动作无法比对是否仍在同一窗口。
    clipboard_text: str = ""  # 修改代码+ClipboardSystemBridge：保留旧受控内存剪贴板文本作为兼容字段；如果没有这一行，旧调试和历史状态读取会直接断开。
    clipboard_backend: Any | None = None  # 新增代码+ClipboardSystemBridge：保存真实或测试剪贴板后端；如果没有这一行，read_clipboard/write_clipboard 无法对齐 ClaudeCode 的系统剪贴板行为。
