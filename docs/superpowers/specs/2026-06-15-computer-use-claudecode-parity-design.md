...
`learning_agent/computer_use_mcp_v2/inferred_ant_mcp/` 是本期对齐 ClaudeCode computer use 外部包的反推契约目录。ClaudeCode 源码中真实工具 schema 和 tool handler 主要来自外部 `@ant/computer-use-mcp` 包，本仓库拿不到该包源码，所以 OpenHarness 在该目录内保留按 ClaudeCode 行为、工具名、MCP 数据结构和调用语义反推出来的兼容层。这个目录不应被当作普通 Windows runtime 实现目录，也不应混入平台细节；它的核心职责是模拟外部包暴露给 ClaudeCode 主程序的 MCP 契约。
...
主目录：`learning_agent/computer_use_mcp_v2/inferred_ant_mcp/`

主文件：`learning_agent/computer_use_mcp_v2/inferred_ant_mcp/build_tools.py`

该目录定位为 ClaudeCode 外部包 `@ant/computer-use-mcp` 的 OpenHarness 反推版本。目录内文件应该优先表达外部包可能提供的 MCP 契约、工具表面、参数形态、返回结构和 dispatch 语义；真实 Windows 截图、SendInput、窗口枚举、剪贴板、锁、审计等平台实现必须留在 `windows_runtime/`，避免把反推契约层写成 Windows 执行层。
...
- `inferred_ant_mcp/` 只保存对 ClaudeCode 外部 computer use MCP 包的反推契约和轻量分发。
- `inferred_ant_mcp/` 不能直接承担 Windows API、SendInput、WGC/GDI 截图、真实剪贴板、窗口枚举等平台实现。
- 如果需要调用真实能力，只能通过 host/context/adapter 边界调用 `windows_runtime/`。
- 后续对齐 ClaudeCode 工具表面时，优先改这个目录中的工具契约；后续对齐 Windows 实际动作时，优先改 `windows_runtime/`。
