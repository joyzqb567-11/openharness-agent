# Browser Runtime ClaudeCode Alignment Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use `superpowers:subagent-driven-development` (recommended) or `superpowers:executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 把 `learning_agent` 的真实浏览器能力升级为可观察、可恢复、可回放、可验收、接入主循环的 Browser Runtime 子系统。

**Architecture:** 参考 ClaudeCode 的 `claude-in-chrome`、MCP 工具协议、流式工具执行器、权限模式和状态 UI，但不依赖 ClaudeCode 外部闭源包。learning_agent 侧采用 Browser Protocol、Runtime Store、Session Manager、Observation Engine、Locator Engine、Action Executor、Recovery/Replay/Verifier、Status Ecosystem 八层架构。

**Tech Stack:** Python、Playwright/CDP、learning_agent MCP tools、harness event log、JSON/JSONL runtime store、unittest、PowerShell acceptance controller、`start_oauth_agent.bat` 真实终端验收。
