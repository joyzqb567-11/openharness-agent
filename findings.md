...
## 2026-06-04 Phase93 Universal Live Execution Gate Findings

- Phase92's universal runtime can be composed into a single live execution gate without adding per-app controllers.
- Phase93 should stay recording-only by default; the authorized positive path proves the action layer and safety boundary are wired, not that real desktop input was sent.
- The sandboxed shell in this Codex session rejects rename/delete operations, so atomic-write CLI checks can fail in sandbox while succeeding when escalated outside the sandbox.
- The Phase93 visible-terminal scenario reached the real `start_oauth_agent.bat` prompt, but the agent model call failed before tool use with `OAuth state 不匹配`.
- Therefore Phase93 is code-ready and automated-contract-ready, but not Rule-17-complete until the OAuth/model-call path is fixed and the visible-terminal final answer contains all Phase93 tokens.

Retest finding:
- After the OAuth/model-call path was repaired, the same Phase93 visible-terminal scenario passed in `learning_agent/acceptance_controller/runs/agent_capability_phase93_universal_live_execution_gate-20260604_203855/result.json`.
- The accepted result has `completed=true`, `assertion_passed=true`, and `permission_sent_count=0`.
- Phase93 is now Rule-17-complete for the recording-only universal live execution gate.

## 2026-06-04 Phase94 Authorized Real Dispatch Candidate Findings

- Phase93's remaining execution gap was specifically the `recording_dispatch_only` positive path after authorization.
- Phase94 fills that gap by adding an authorized low-level dispatch candidate layer that reaches an injected sender after Phase60 grant approval, Phase72 safety review, and target recheck.
- The default Phase94 sender is intentionally `Phase94RecordingLowLevelSender`, so the contract can prove the bridge without sending physical mouse or keyboard input.
- The target recheck should use `app_id + window_id + title_sha256_16` as the hard identity. `display_id` is useful audit context, but current static inventory normalization can omit it, so treating it as mandatory causes false target-drift failures.
- Raw text must not enter reports; Phase94 stores only text length and a short digest.
- The accepted visible terminal scenario proves the safe contract and token line through `start_oauth_agent.bat`, not unrestricted physical control of all local apps.
---

## 2026-06-04 ClaudeCode Computer Use Source Comparison Findings

ClaudeCode source facts read so far:
- `D:\ClaudeCode-main\ClaudeCode-main\utils\computerUse\setup.ts` builds dynamic MCP config for server name `computer-use` and allowed `mcp__computer-use__*` tool names via `@ant/computer-use-mcp`.
- `D:\ClaudeCode-main\ClaudeCode-main\utils\computerUse\mcpServer.ts` creates an in-process MCP server with tools from `buildComputerUseTools`, timed installed-app enumeration, and `getComputerUseHostAdapter`.
- `D:\ClaudeCode-main\ClaudeCode-main\utils\computerUse\wrapper.tsx` binds `@ant/computer-use-mcp` session context to ClaudeCode `ToolUseContext`, including allowed apps, grant flags, selected display, screenshot dims, permission dialog, app-hidden state, display pinning, lock checks, lock acquisition, OS notifications, and global Escape abort.
- `D:\ClaudeCode-main\ClaudeCode-main\utils\computerUse\executor.ts` is a macOS-only native executor. It wraps `@ant/computer-use-swift` for screenshots, display geometry, app/window functions, TCC checks, and `@ant/computer-use-input` for mouse/keyboard/frontmost-app control.
- `D:\ClaudeCode-main\ClaudeCode-main\utils\computerUse\toolRendering.tsx` exposes rendered tool names for `screenshot`, `zoom`, `request_access`, click variants, type, key, hold_key, scroll, drag, clipboard, open_application, wait, and `computer_batch`.
- `D:\ClaudeCode-main\ClaudeCode-main\utils\computerUse\computerUseLock.ts`, `cleanup.ts`, and `escHotkey.ts` implement file lock ownership, stale lock recovery, turn-end unhide/release, OS notification exit, and global Escape abort.
- Boundary: the actual package internals of `@ant/computer-use-mcp`, `@ant/computer-use-swift`, and `@ant/computer-use-input` are imported dependencies, not fully present as source under the provided `D:\ClaudeCode-main\ClaudeCode-main` tree.

## 2026-06-17 Latest CodeGraph Computer Use Comparison Findings

- Both CodeGraph indexes are current: OpenHarness `Files 1,260 / Nodes 48,491 / Edges 159,608`; ClaudeCode `Files 1,902 / Nodes 43,685 / Edges 140,720`.
- ClaudeCode client wiring runs the Computer Use MCP server in-process for server name `computer-use`, then overrides each listed MCP tool with `getComputerUseMCPToolOverrides(tool.name)`.
- OpenHarness client wiring mirrors that shape by intercepting `mcp__computer-use__*` in `learning_agent/mcp/agent_adapter.py` and dispatching to `computer_use_mcp_v2.claudecode_bridge.wrapper.execute_agent_side_tool`.
- OpenHarness is aligned on the main observable surface: 24 raw tool names, `mcp__computer-use__` prefix, ClaudeCode primary fields such as `coordinate`, `start_coordinate`, `region`, `bundle_id`, `apps`, `grantFlags`, `actions`, MCP text/image content blocks, dynamic `tools/list`, disabled tools-list empty result, lock acquire/cleanup lifecycle, display state keying, and old shell/file/legacy tool blocking.
- Current OpenHarness alignment matrix reports `CLAUDECODE_ALIGNMENT_PARTIAL aligned=11/14 partial=3 missing=0`, not full parity. Partial dimensions are CA07, CA13, and CA14.
- The most actionable remaining source-level gaps are: hidden `@ant/computer-use-mcp` internals cannot be verified from the ClaudeCode repo; OpenHarness clipboard v2 is context-memory rather than obviously system clipboard; permission UI differs from ClaudeCode `ComputerUseApproval`; and OpenHarness still uses a legacy Windows adapter layer below the inferred MCP facade while ClaudeCode goes package-to-hostAdapter-to-executor.
---

## 2026-06-05 Universal Real GUI Computer Use Implementation Findings

- The formal blueprint is now backed by an implementation plan at `docs/superpowers/plans/2026-06-05-universal-real-gui-computer-use.md`.
- The correct next engineering task is URG-1, not Paint-specific drawing logic.
- Existing reusable building blocks are already present:
  - `learning_agent/computer_use/real_screenshot_pipeline.py` provides real screenshot artifacts and pixel guard.
  - `learning_agent/computer_use/real_uia_locator.py` provides real UIA tree reading and semantic locating.
  - `learning_agent/computer_use/observation_fusion.py` provides a safe fused observation contract.
- URG-1 must compose these building blocks into one universal read-only `ObservationFrame`.
- URG-1 must keep `actions_expanded=false`, `real_desktop_touched=false`, and `low_level_event_count=0`.
- Multi-agent delegation is not used for this step because the available subagent spawn tool requires an explicit user request for delegation; the user requested continuous execution, not subagent delegation.
- The current branch has existing dirty Computer Use history and evidence, so this work continues in-place with backups rather than creating a detached worktree.
- URG-1 implementation finding: the new `UniversalRealObservationFrameRuntime` composes existing Phase28 inventory, Phase56 screenshot, Phase57 UIA, and Phase66 fusion contracts rather than replacing them.
- URG-1 visible acceptance finding: the real visible terminal run passed in `learning_agent/acceptance_controller/runs/agent_capability_universal_real_gui_observation_frame-20260605_213157/`.
- URG-1 boundary finding: the acceptance proves a read-only universal ObservationFrame, not real mouse or keyboard control.
- URG-2 implementation finding: the new `UniversalTargetSessionRuntime` composes Phase108 generic target discovery and Phase111 identity verification rather than adding per-app controllers.
- URG-2 representative sample finding: Notepad, Paint, and Calculator samples all build sessions through the same runtime and the same resolver interface.
- URG-2 drift finding: target drift produces `target_drift_zero_events=true` and `low_level_event_count=0`.
- URG-2 visible acceptance finding: the real visible terminal run passed in `learning_agent/acceptance_controller/runs/agent_capability_universal_target_session_identity_guard-20260605_213836/`.
- URG-2 boundary finding: the acceptance proves target session and identity guard readiness, not real SendInput dispatch.
- URG-3 implementation finding: `UniversalActionDslRuntime` composes URG-2 target identity checks and the existing Phase47 `WindowsSendInputDispatcher` rather than adding app-specific controllers.
- URG-3 dispatcher finding: `hotkey` is now a generic Phase47 event type that expands into key-down events followed by reverse key-up events.
- URG-3 text finding: `type_text` enters the dispatcher only as length plus SHA256-16 hash; the original text does not appear in reports or low-level events.
- URG-3 safety finding: target drift, abort, and unknown sessions all return zero low-level events.
- URG-3 visible acceptance finding: the real visible terminal run passed in `learning_agent/acceptance_controller/runs/agent_capability_universal_action_dsl_sendinput_bridge-20260605_214901/`.
- URG-3 boundary finding: the accepted contract proves a safe DSL-to-SendInput bridge reaching a recording low-level sender with `low_level_event_count=22`, while keeping `real_dispatch_performed=false` and `real_desktop_touched=false`.

## 2026-06-05 URG-4 Universal Observe-Plan-Act-Verify Findings

- URG-4 implementation finding: `UniversalObservePlanActVerifyLoop` composes URG-1 style ObservationFrame evidence, URG-2 target session identity, and URG-3 DSL dispatch instead of adding app-specific controllers.
- URG-4 contract finding: Notepad, Calculator, and Paint representative samples all run through the same loop and produce `low_level_event_count=14` in the safe recording-sender contract.
- URG-4 verification finding: every action records before and after observations, the generic verifier decides complete/retry/stop, and forced failure exits after a bounded retry with cleanup evidence.
- URG-4 safety finding: target drift and abort zero-event facts are inherited from the URG-3 safety contract; `real_dispatch_performed=false` and `real_desktop_touched=false` remain explicit for this stage.
- URG-4 packaging finding: `learning_agent/computer_use/__init__.py` had been compressed into two comment-heavy lines, so package-level imports were not reliable; it was rewritten into executable multiline exports while preserving Phase98, Phase93, URG-1, URG-2, and URG-3 names.
- URG-4 visible acceptance finding: the real visible terminal run passed in `learning_agent/acceptance_controller/runs/agent_capability_universal_observe_plan_act_verify_loop-20260605_222142/` with `completed=true`, `final_printed=true`, `assertion_passed=true`, and `permission_sent_count=0`.

## 2026-06-18 Fresh CodeGraph ClaudeCode/OpenHarness Computer Use Audit Findings

- Fresh CodeGraph status is current for both projects: ClaudeCode `Files 1,902 / Nodes 43,685 / Edges 140,720`; OpenHarness `Files 1,331 / Nodes 50,132 / Edges 50,608`.
- ClaudeCode total chain is: `setup.ts` builds server `computer-use` and `mcp__computer-use__*` allowed tools; `services/mcp/client.ts` intercepts that server as an in-process MCP server; `wrapper.tsx` overrides actual tool calls with `bindSessionContext`; `hostAdapter.ts` supplies permissions, lock, display state, and macOS executor; `executor.ts` performs macOS Swift/input actions; `cleanup.ts`, `computerUseLock.ts`, and `escHotkey.ts` close the turn lifecycle.
- OpenHarness total chain is: model-visible `mcp__computer-use__*` names from `inferred_ant_mcp/build_tools.py`; `learning_agent/mcp/agent_adapter.py` intercepts those names before normal external MCP registry calls; `claudecode_bridge/wrapper.py` executes the v2 runtime and maps text/image content blocks; `bind_session_context.py` binds ask_permission, trace, acceptance events, system clipboard backend, display state, lock callbacks, and a legacy Windows host adapter; `windows_runtime/mcp_session_adapter.py` maps v2 atomic tools onto the mature Windows controller path.
- Model-visible tool surface is strongly aligned: OpenHarness exposes the 24 ClaudeCode-style raw names and blocks legacy/raw/shell names such as `click`, `computer_action`, `computer_use`, `bash`, `powershell`, and `start_background_command`.
- Prior clipboard risk is reclassified: OpenHarness v2 agent-side context now binds `WindowsClipboardBackend` by default through `bind_session_context.py`; the old memory clipboard remains in `windows_runtime/mcp_session_adapter.py` as legacy adapter behavior, not the primary inferred v2 clipboard path.
- Important nuance: `claudecode_bridge/hostAdapter.py`, `computerUseLock.py`, and `cleanup.py` are thin same-name bridge files; the real OpenHarness lifecycle and host execution live under `inferred_ant_mcp/` and `windows_runtime/`. Future audits should not judge parity from the thin bridge files alone.
- Remaining non-OS, non-external-package gaps: ClaudeCode package internals from `@ant/computer-use-mcp` are not inspectable in the source tree; OpenHarness is an inferred Python implementation, not a vendored copy. Permission UX is terminal/prompt based rather than ClaudeCode React/Ink `ComputerUseApproval`. The Windows backend still routes through a mature legacy adapter/controller layer below the v2 facade, while ClaudeCode routes package -> hostAdapter -> executor directly.
- Current conclusion: OpenHarness is aligned on the observable Computer Use contract and main runtime lifecycle, but not fully identical at implementation/UX depth. Treat it as high parity on protocol/tool surface and partial parity on hidden package internals, UI, and backend layering.
